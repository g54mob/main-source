...
namespace RimWorld
{
	public static class ChildcareUtility
	{
		public enum BreastfeedFailReason
		{
			...
			MomInMentalState = 1,
			BabyTooFull = 2,
			BabyForbiddenToMom = 3,
			BabyForbiddenToHauler = 4,
			MomForbiddenToHauler = 5,
			MomIsMobile = 6,
			HaulerCannotReachBaby = 7,
			HaulerCannotReachMom = 8,
			NoMomsAssignedToBaby = 9,
			MomNotAssignedToChildcare = 10,
			MomInsideContainer = 11,
			MomBeingCarried = 12,
			NoBiotechMod = 13,
			MomNull = 14,
			BabyNull = 15,
			...
			MomNotOnMap = 27,
			HaulerNotOnMap = 28,
			HaulerNotOnBabyMap = 29,
			HaulerNotOnMomMap = 30,
			MomNotAssignedToBaby = 31,
			BabyInIncompatibleFactionToMom = 32,
			BabyInIncompatibleFactionToHauler = 33,
			MomInIncompatibleFactionToHauler = 34,
			HaulerCannotReserveBaby = 35,
			HaulerCannotReserveMom = 36,
			HaulerIncapableOfManipulation = 37,
			BabyShambler = 38
		}
