...
namespace NAudio.MediaFoundation
{
	public static class MediaFoundationApi
	{
		...
		{
			MediaFoundationInterop.MFTEnumEx(category, _MFT_ENUM_FLAG.MFT_ENUM_FLAG_ALL, null, null, out var interfacesPointer, out var interfaceCount);
			...
			for (int i = 0; i < interfaceCount; i++)
			{
				...
				interfaces[i] = (IMFActivate)Marshal.GetObjectForIUnknown(pUnk);
			}
			try
			{
