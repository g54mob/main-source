...
public class TechTreeNode : MonoBehaviour, ITooltipTitleSource, ITooltipCustomSource
{
	...
	private static Color _lightGray = new Color(0.75f, 0.75f, 0.75f);

	[SerializeField]
	protected SpriteRenderer _button;
	...
	public void UpdateStatus()
	{
		if (Node != null)
		{
			...
			_icon.color = (Node.IsPurchased ? _lightGray : Color.white);
			_button.color = (Node.IsPurchased ? _lightGray : Color.white);
	...
	public void AddTooltipCustomContent(UITooltip tooltip)
	{
		...
		{
			tooltip.AddTextLine(Translation.Translate("@TechNodeRequiresTier", Node.Tier));
		}
		else
		{
			tooltip.AddTextLine(Translation.Translate("@TechNodeRequiresTech", Node.Previous.Name));
		}
	}

	private void _addActiveTooltipContent(UITooltip tooltip)
	{
		string text = ((!(Node.Identifier == "t1f_glitched_frame")) ? (Node.NodeType switch
		{
			TechNodeType.Frame => "@TechNodeUnlocksFrame", 
			TechNodeType.Upgrade => "@TechNodeUnlocksUpgrade", 
			TechNodeType.Ability => "@TechNodeUnlocksAbility", 
			_ => "", 
		}) : "");
		...
		if (Node.IsPurchased)
		{
			tooltip.AddTextLine("<color=green>" + Translation.Translate("@TechNodeUnlocked") + "</color>");
		}
