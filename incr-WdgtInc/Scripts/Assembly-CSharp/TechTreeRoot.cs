...
public class TechTreeRoot : TraversableView
{
	...
	private void OnEnable()
	{
		...
		{
			if (node.Hidden || (node.RequiresGlitchedFrame && !GamePlayer.Current.GlitchFrameInteracted))
			{
				continue;
			}
			TechTreeNode techTreeNode = Object.Instantiate(_nodePrefab, _nodesParent);
			techTreeNode.Node = node;
			if (node.Identifier == "t1f_glitched_frame")
			{
				techTreeNode.gameObject.AddComponent<GlitchedIcon>();
			}
