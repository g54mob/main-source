...
namespace Assets.Behaviour.Frame.Parts
{
	public class T8QuantumWidgetGem : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (Selected)
			{
				...
				if (PlayerControls.InteractRelease)
				{
					...
					T8QuantumWidgetSecretGem t8QuantumWidgetSecretGem = UIHelper.GetMouseOverGameObject()?.GetComponent<T8QuantumWidgetSecretGem>();
					if ((bool)t8QuantumWidgetSecretGem)
					{
						_parent.FinalizeSecret(t8QuantumWidgetSecretGem);
					}
				}
		...
		public void ConnectTo(T8QuantumWidgetSecretGem other)
		{
			Locked = true;
			_line.SetPosition(1, new Vector3(other.transform.position.x - _line.transform.position.x, other.transform.position.y - _line.transform.position.y, 0f));
			_line = null;
		}
	}
}
