...
namespace Assets.Behaviour.Frame.Parts
{
	public class T12OmegaWidgetPuzzle : MonoBehaviour
	{
		...
		private bool _checkCasing()
		{
			if (!_hasCasing)
			{
				_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "@T12OmegaWidgetCasingWarning");
				return false;
			}
			return true;
		}
		...
		private bool _checkParts()
		{
			...
			{
				_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "@T12OmegaWidgetWarning");
				return false;
			}
			return true;
		}
		...
		public void FinalizeButtonClicked()
		{
			if (_checkCasing() && _checkParts())
			{
				if (!_hasShielding)
				{
					_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "@T12OmegaWidgetShieldingWarning");
					return;
				}
				_finalButton.SetActive(active: false);
				_parent.ButtonClicked(new WorldAnchor(WorldAnchorType.HandCraft, 0));
				this.StartImportantCoroutine(_setupPuzzle());
			}
		}
