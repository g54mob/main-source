...
namespace Assets.Behaviour.Frame.Parts
{
	public class T9UnshackledWidgetBall : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (!_isGhost)
			{
				RaycastHit2D raycastHit2D = Physics2D.Raycast(base.transform.position, (_move.y > 0f) ? Vector2.up : Vector2.down, Mathf.Max(0.16f, 1.1f * _velocity * Time.deltaTime));
				if ((bool)raycastHit2D.collider)
				{
					_move.y *= -1f;
					transform = raycastHit2D.transform;
				}
				RaycastHit2D raycastHit2D2 = Physics2D.Raycast(base.transform.position, (_move.x > 0f) ? Vector2.right : Vector2.left, Mathf.Max(0.16f, 1.1f * _velocity * Time.deltaTime));
