...
namespace Assets.Behaviour.UI
{
	public class UIAlertWindow : MonoBehaviour
	{
		...
		{
			Show(title, message, new string[1] { "@DialogOkay" }, delegate
			{
				onClose?.Invoke();
			});
		}
		...
		{
			Show(title, message, new string[2] { "@DialogYes", "@DialogNo" }, delegate(string label)
			{
				if (label == "@DialogYes")
				{
					onYes?.Invoke();
