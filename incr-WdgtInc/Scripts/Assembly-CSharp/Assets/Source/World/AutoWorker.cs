...
namespace Assets.Source.World
{
	public abstract class AutoWorker : IJsonSource
	{
		private class AutoWorkerConstructionProgress : ConstructionProgress
		{
			...
			public override string Name => Translation.Translate("@ConstructionAutoWorker", _worker.Parent.DisplayName);
			...
			public AutoWorkerConstructionProgress(AutoWorker parent, float time, IEnumerable<KeyValuePair<ItemType, BigInteger>> materials)
				: base(time, materials)
			{
				_worker = parent;
			}
			...
			protected override void OnConstructionCompleted()
			{
				...
				WorldFrame.UpdateFullyUpgradedAchievement();
			}
