...
namespace Assets.Source.World
{
	public class FrameUpgrade
	{
		...
		private Dictionary<ItemType, BigInteger> _calcCost;
		...
		public double UpgradeMultiplier => RequiredTech.UpgradeMultiplier;

		public int UpgradeFlag => RequiredTech.UpgradeFlag;
		...
		public IEnumerable<KeyValuePair<ItemType, BigInteger>> GetCost()
		{
			if (_calcCost == null)
			{
				_calcCost = GameMath.CreateItemCost("Upgrade" + RequiredTech.Identifier, WorldFrame.GetPreview(Frame).Tier, 20.0, RequiredTech.CostItems);
			}
			return _calcCost;
		}
