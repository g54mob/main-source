...
public class WorldOverview : MonoBehaviour
{
	...
	[SerializeField]
	private WorldOverviewCell _glitchedPrefab;

	[SerializeField]
	private WorldOverviewGlitchHint _glitchHintPrefab;

	private WorldFrame _currentHighlight;

	private float _glitchHintTimer = 60f;
	...
	private void Update()
	{
		...
		if (!GamePlayer.Current.GlitchFrameInteracted)
		{
			_glitchHintTimer -= Time.deltaTime;
			if (_glitchHintTimer < 0f)
			{
				SpawnGlitchHint();
				_glitchHintTimer = SeededRandom.Global.RandomRange(60f, 180f);
			}
		}
	}

	private void SpawnGlitchHint()
	{
		Vector2 vector = new Vector2((float)WorldMap.Current.GlitchLocation.x * 1.5f, (float)WorldMap.Current.GlitchLocation.y * 1.5f);
		Vector2 vector2 = (Vector2)Camera.main.transform.position + new Vector2(SeededRandom.Global.RandomRange(-20f, 20f), SeededRandom.Global.RandomRange(-20f, 20f));
		Vector2 normalized = (vector2 - vector).normalized;
		Object.Instantiate(_glitchHintPrefab, vector2 + normalized * 10f, Quaternion.identity, base.transform);
	}
	...
	{
		WorldOverviewCell worldOverviewCell = Object.Instantiate((frame is GlitchedFrame || frame is T1GlitchedFrame) ? _glitchedPrefab : _cellPrefab, _cellsParent);
