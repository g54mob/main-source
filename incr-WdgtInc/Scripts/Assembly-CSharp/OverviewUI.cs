...
public class OverviewUI : FullScreenUI
{
	...
	[SerializeField]
	private OverviewAbilityGhost _abilityGhostPrefab;
	...
	private OverviewAbilityGhost _currentAbilityGhost;
	...
	public bool AbilityActive => _currentAbilityGhost;

	public ActivatedAbility CurrentAbility => _currentAbilityGhost.Ability;
	...
	public void ShowPurchaseGhost(FramePrefabSet prefab, WorldFrame frame, bool isCopy = false)
	{
		...
		{
			_placementGuideText.TL(frame.PlacementTech.Description);
		}
		else if (frame.PlacementGuideHint != null)
		{
			_placementGuideText.TL(frame.PlacementGuideHint);
		}
		else
		{
			_placementGuideText.TL("@HintFreePlacement");
		}
	}
	...
	public void ShowAbilityGhost(ActivatedAbility ability)
	{
		ToggleBuildMenu(show: false);
		StopCopy();
		StopDeconstruct();
		StopBuildGhost();
		StopAbilityTargeting();
		_currentAbilityGhost = UnityEngine.Object.Instantiate(_abilityGhostPrefab, Traversable.transform);
		_currentAbilityGhost.SetAbility(ability);
		GameUI.Instance.SetSelectedAbility(ability);
	}

	public void StopAbilityTargeting()
	{
		if ((bool)_currentAbilityGhost)
		{
			UnityEngine.Object.Destroy(_currentAbilityGhost.gameObject);
		}
		GameUI.Instance.SetSelectedAbility(null);
	}
	...
	public void TogglePicker()
	{
		...
		else if (DeconstructActive)
		{
			StopDeconstruct();
		}
		else if ((bool)WorldOverviewCell.Highlighted)
		{
			WorldOverviewCell.Highlighted.TogglePicker();
		}
	}
