...
public class OverviewBuildGhost : MonoBehaviour
{
	...
	public void SetFrame(FramePrefabSet prefab, WorldFrame frame)
	{
		...
		if (frame is T1GlitchedFrame)
		{
			base.gameObject.AddComponent<GlitchedIcon>();
		}
	}
	...
	public void SetRelocateFrame(WorldOverviewCell cell)
	{
		...
		if (cell.Frame is T1GlitchedFrame)
		{
			base.gameObject.AddComponent<GlitchedIcon>();
		}
	}
	...
	private void Update()
	{
		...
		UnityEngine.Vector2 mouseWorld = PlayerControls.MouseWorld;
		...
		if (PlayerControls.InteractRelease && !isMouseOverUi)
		{
			...
			{
				OverviewUI.Instance.ShowWarning(base.transform, "@WarningCantBuild");
	...
	private void DoRelocateToPosition(Vector2Int pos)
	{
		...
		{
			worldFrame.StartConstruction(new Dictionary<ItemType, BigInteger>());
		}
	...
	{
		IEnumerable<KeyValuePair<ItemType, BigInteger>> purchaseCost = _frame.GetPurchaseCost();
	...
	{
		OverviewBuildGhost[] componentsInChildren;
		if (GamePlayer.Current.HasTech(OverviewUI.MoveAreaTech) && PlayerControls.ModifierControl)
		{
			componentsInChildren = GetComponentsInChildren<OverviewBuildGhost>(includeInactive: true);
			foreach (OverviewBuildGhost overviewBuildGhost in componentsInChildren)
			{
		...
		if (num >= 100)
		{
			SteamAchievement.Trigger("BigCopy");
		}
	}
	...
	internal void SetCopySlave(Vector2Int offset)
	{
		...
		base.enabled = false;
		base.transform.localPosition = new UnityEngine.Vector3((float)offset.x * 1.5f, (float)offset.y * 1.5f, 0f);
	}
}
