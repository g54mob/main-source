...
namespace Assets.Behaviour.Util
{
	public class SteamStatsManager : MonoBehaviour
	{
		...
		private string _getRichPresenceText()
		{
			...
			default:
				if (GamePlayer.Current.RocketParts > 0)
				{
		...
		public static void ItemProduced(ItemType type, int count)
		{
			if ((bool)_instance && _instance._statsByItem.TryGetValue(type, out var value))
			{
				value.Add(count);
			}
		}
