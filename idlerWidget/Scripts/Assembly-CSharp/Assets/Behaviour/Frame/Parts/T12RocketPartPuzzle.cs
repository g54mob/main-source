...
namespace Assets.Behaviour.Frame.Parts
{
	public class T12RocketPartPuzzle : MonoBehaviour
	{
		...
		private void Update()
		{
			...
			if (!t7FuelRodItem)
			{
				_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "Omega Casing required.");
				_resetting = true;
				return;
			}
			if (!t7FuelRodItem2)
			{
				_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "Omega Shielding required.");
				_resetting = true;
				return;
			}
			UISounds.CraftStep();
