...
namespace Assets.Behaviour.Frame.Parts
{
	public class T11PicoprocessorPuzzle : MonoBehaviour
	{
		...
		{
			UISounds.CraftStep();
			StartCoroutine(_doInteraction(pass: true));
		}

		public void DoFail()
		{
			UISounds.CraftStep();
			StartCoroutine(_doInteraction(pass: false));
		}
		...
		private IEnumerator _doInteraction(bool pass)
		{
			...
			{
				_parent.ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "Quality control failure!");
		...
		private IEnumerator _swapSamples()
		{
			...
			_sample = Object.Instantiate(old, pos + new Vector3(0f, -3f, 0f), Quaternion.identity, old.transform.parent);
			SetupPuzzle();
			float time = 0f;
			while (time < 1f)
			{
				time += Time.deltaTime;
				_sample.transform.position = pos + new Vector3(0f, Mathf.SmoothStep(-3f, 0f, time));
				old.transform.position = pos + new Vector3(0f, Mathf.SmoothStep(0f, 3f, time));
				yield return null;
			}
