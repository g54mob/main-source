using System;
using System.Collections;
using System.Collections.Generic;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using MEC;
using UnityEngine;

public class HarvesterHoverObj : MonoBehaviour
{
	[CompilerGenerated]
	private sealed class _003C_Run_003Ed__9 : IEnumerator<float>, IEnumerator, IDisposable
	{
		private int _003C_003E1__state;

		private float _003C_003E2__current;

		public HarvesterHoverObj _003C_003E4__this;

		private float _003CstartTime_003E5__2;

		float IEnumerator<float>.Current
		{
			[DebuggerHidden]
			get
			{
				return 0f;
			}
		}

		object IEnumerator.Current
		{
			[DebuggerHidden]
			get
			{
				return null;
			}
		}

		[DebuggerHidden]
		public _003C_Run_003Ed__9(int _003C_003E1__state)
		{
		}

		[DebuggerHidden]
		void IDisposable.Dispose()
		{
		}

		private bool MoveNext()
		{
			return false;
		}

		bool IEnumerator.MoveNext()
		{
			//ILSpy generated this explicit interface implementation from .override directive in MoveNext
			return this.MoveNext();
		}

		[DebuggerHidden]
		void IEnumerator.Reset()
		{
		}
	}

	public SpriteRenderer Rend;

	public BuildingObj TgtBuilding;

	private Color _tgtColor;

	private CoroutineHandle _curAnim;

	private List<BuildingObj> _bldsInRange;

	public void Init(BuildingObj b)
	{
	}

	private void HighlightBuilding(BuildingObj b)
	{
	}

	public void RefreshHighlightedBuildings(float x, float y)
	{
	}

	public void Deactivate()
	{
	}

	[IteratorStateMachine(typeof(_003C_Run_003Ed__9))]
	private IEnumerator<float> _Run()
	{
		return null;
	}
}
