...
namespace Rewired.Utils
{
	public static class UnityTools
	{
		internal struct PLjezcBkFGJQfWOkGFiEWPRPdDHUA
		{
			...
			{
				eHHQNMlYSngIGWtqSoJhFkHbbgQaA = default(Platform);
				YcIFSYCNiADtdLnKqTzbFXaOBVMmA = default(Platform);
				GEghAnAkLKBSIPlsUnModuFNbQpLA = default(EditorPlatform);
				JpTRfxFlDlxaeRgqNpcdNaxuqZQO = false;
				bRXuMJVZiPPbIwEwImjSmkSmlsGH = default(WebplayerPlatform);
				LBIwilefmVLwhnhfYhJVVShgnMFm = default(ScriptingBackend);
				UTaoVazfTUEdZDoLnXvLlMsWNhwd = default(ScriptingAPILevel);
				LVLBRkiEknWSnDEjvMiIPYsDZzBn = null;
			}
		}
		...
		private static UnityVersion wGbFcXhbNmDAbMGJXtZFugAhRagd;
		...
		private static bool MXIOINOowHSalHhEeObWTmckZzRD => false;
