...
namespace Dissonance.Config
{
	public class ChatRoomSettings : ScriptableObject
	{
		...
		public static readonly string SettingsFilePath = Path.Combine("Assets/ThirdParty/Dissonance/Resources", "ChatRoomSettings.asset");
		...
		public string FindRoomById(ushort id)
		{
			if (_nameLookup == null)
			{
				...
				{
					dictionary[new RoomName(Names[i]).ToRoomId()] = Names[i];
				}
