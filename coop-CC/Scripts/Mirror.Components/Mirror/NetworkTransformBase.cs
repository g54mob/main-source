...
namespace Mirror
{
	public abstract class NetworkTransformBase : NetworkBehaviour
	{
		...
		[ClientRpc]
		public void RpcLocalTeleport(Vector3 destination, Quaternion rotation)
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			writer.WriteVector3(destination);
			writer.WriteQuaternion(rotation);
			SendRPCInternal("System.Void Mirror.NetworkTransformBase::RpcLocalTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)", 1812272906, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		[Server]
		public void ServerLocalTeleport(Vector3 destination, Quaternion rotation)
		{
			if (!NetworkServer.active)
			{
				Debug.LogWarning("[Server] function 'System.Void Mirror.NetworkTransformBase::ServerLocalTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)' called when server was not active");
				return;
			}
			OnLocalTeleport(destination, rotation);
			RpcLocalTeleport(destination, rotation);
		}
		...
		protected virtual void OnLocalTeleport(Vector3 destination, Quaternion rotation)
		{
		...
		protected void UserCode_RpcLocalTeleport__Vector3__Quaternion(Vector3 destination, Quaternion rotation)
		{
			OnLocalTeleport(destination, rotation);
		}

		protected static void InvokeUserCode_RpcLocalTeleport__Vector3__Quaternion(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcLocalTeleport called on server.");
			}
			else
			{
				((NetworkTransformBase)obj).UserCode_RpcLocalTeleport__Vector3__Quaternion(reader.ReadVector3(), reader.ReadQuaternion());
			}
		}
		...
		static NetworkTransformBase()
		{
			...
			RemoteProcedureCalls.RegisterRpc(typeof(NetworkTransformBase), "System.Void Mirror.NetworkTransformBase::RpcLocalTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)", InvokeUserCode_RpcLocalTeleport__Vector3__Quaternion);
