	...
	public class PredictedRigidbody : NetworkBehaviour
	{
		...
		private void RecordState()
		{
			...
			if (predictedTime != lastRecorded.timestamp)
			{
				...
				tf.GetPositionAndRotation(out var position, out var rotation);
				Vector3 velocity = predictedRigidbody.velocity;
				...
				if (count > 0)
				{
					...
					velocityDelta = velocity - rigidbodyState.velocity;
				...
				RigidbodyState value = new RigidbodyState(predictedTime, positionDelta, position, rotationDelta, rotation, velocityDelta, velocity, angularVelocityDelta, angularVelocity);
		...
		{
			Log("ApplyState PRE");
			if (predictedRigidbody.velocity.magnitude <= snapThreshold && predictedRigidbody.angularVelocity.magnitude <= snapThreshold)
			{
		...
		private void OnReceivedState(double timestamp, RigidbodyState state)
		{
			...
			if (state.timestamp < rigidbodyState.timestamp)
			{
				...
				Log("OnReceivedState Edge Case 1");
			...
			else if (rigidbodyState2.timestamp < state.timestamp)
			{
				...
				{
					Log("OnReceivedState Edge Case 2");
					ApplyState(state.timestamp, state.position, state.rotation, state.velocity, state.angularVelocity);
				}
			}
			else if (!Prediction.Sample(stateHistory, timestamp, out before, out after, out afterIndex, out t))
			{
				...
				Log("OnReceivedState Edge Case 3");
				ApplyState(state.timestamp, state.position, state.rotation, state.velocity, state.angularVelocity);
			}
			else
			{
				...
				{
					Log("OnReceivedState Pre Correct");
					...
					Log("OnReceivedState Post Correct");
					OnCorrected();
				}
			}
		}
		...
		public void ClientClearState()
		{
			...
			{
				Debug.LogWarning("[Client] function 'System.Void Mirror.PredictedRigidbody::ClientClearState()' called when client was not active");
				return;
			}
			lastRecorded = new RigidbodyState(NetworkTime.predictedTime, Vector3.zero, predictedRigidbody.position, Quaternion.identity, predictedRigidbody.rotation, Vector3.zero, predictedRigidbody.velocity, Vector3.zero, predictedRigidbody.angularVelocity);
			lastRecordTime = lastRecorded.timestamp;
			stateHistory.Clear();
			stateHistory.Add(lastRecorded.timestamp, lastRecorded);
		}
		...
		public void ServerTeleport(Vector3 position, Quaternion rotation)
		{
			...
			{
				Debug.LogWarning("[Server] function 'System.Void Mirror.PredictedRigidbody::ServerTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)' called when server was not active");
				return;
			}
			predictedRigidbody.position = position;
			predictedRigidbody.rotation = rotation;
		...
		private void RpcTeleport(Vector3 position, Quaternion rotation)
		{
			...
			SendRPCInternal("System.Void Mirror.PredictedRigidbody::RpcTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)", 1426078395, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}

		[Server]
		public void ServerTeleport(Vector3 position, Vector3 velocity, Quaternion rotation)
		{
			if (!NetworkServer.active)
			{
				Debug.LogWarning("[Server] function 'System.Void Mirror.PredictedRigidbody::ServerTeleport(UnityEngine.Vector3,UnityEngine.Vector3,UnityEngine.Quaternion)' called when server was not active");
				return;
			}
			predictedRigidbody.position = position;
			predictedRigidbody.rotation = rotation;
			...
			RpcTeleport(position, velocity, rotation);
		}

		[ClientRpc]
		private void RpcTeleport(Vector3 position, Vector3 velocity, Quaternion rotation)
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			writer.WriteVector3(position);
			writer.WriteVector3(velocity);
			writer.WriteQuaternion(rotation);
			SendRPCInternal("System.Void Mirror.PredictedRigidbody::RpcTeleport(UnityEngine.Vector3,UnityEngine.Vector3,UnityEngine.Quaternion)", -2001486158, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}

		[Server]
		public void ServerSetEnabled2(bool set)
		{
			if (!NetworkServer.active)
			{
				Debug.LogWarning("[Server] function 'System.Void Mirror.PredictedRigidbody::ServerSetEnabled2(System.Boolean)' called when server was not active");
				return;
			}
			predictedRigidbody.isKinematic = !set;
			base.enabled = set;
			RpcSetEnabled(set);
		}

		[ClientRpc]
		private void RpcSetEnabled(bool set)
		{
			NetworkWriterPooled writer = NetworkWriterPool.Get();
			writer.WriteBool(set);
			SendRPCInternal("System.Void Mirror.PredictedRigidbody::RpcSetEnabled(System.Boolean)", -666874560, writer, 0, includeOwner: true);
			NetworkWriterPool.Return(writer);
		}
		...
		protected void UserCode_RpcTeleport__Vector3__Quaternion(Vector3 position, Quaternion rotation)
		{
			if (!base.isServer)
			{
				predictedRigidbody.position = position;
		...
		protected static void InvokeUserCode_RpcTeleport__Vector3__Quaternion(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			...
			{
				((PredictedRigidbody)obj).UserCode_RpcTeleport__Vector3__Quaternion(reader.ReadVector3(), reader.ReadQuaternion());
			}
		}

		protected void UserCode_RpcTeleport__Vector3__Vector3__Quaternion(Vector3 position, Vector3 velocity, Quaternion rotation)
		{
			if (!base.isServer)
			{
				predictedRigidbody.position = position;
		...
		protected static void InvokeUserCode_RpcTeleport__Vector3__Vector3__Quaternion(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			if (!NetworkClient.active)
			{
				Debug.LogError("RPC RpcTeleport called on server.");
			...
			{
				((PredictedRigidbody)obj).UserCode_RpcTeleport__Vector3__Vector3__Quaternion(reader.ReadVector3(), reader.ReadVector3(), reader.ReadQuaternion());
			}
		}

		protected void UserCode_RpcSetEnabled__Boolean(bool set)
		{
			if (!base.isServer)
			{
				if (set)
		...
		protected static void InvokeUserCode_RpcSetEnabled__Boolean(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
		{
			...
			{
				((PredictedRigidbody)obj).UserCode_RpcSetEnabled__Boolean(reader.ReadBool());
			}
		}

		static PredictedRigidbody()
		{
			RemoteProcedureCalls.RegisterRpc(typeof(PredictedRigidbody), "System.Void Mirror.PredictedRigidbody::RpcTeleport(UnityEngine.Vector3,UnityEngine.Quaternion)", InvokeUserCode_RpcTeleport__Vector3__Quaternion);
			RemoteProcedureCalls.RegisterRpc(typeof(PredictedRigidbody), "System.Void Mirror.PredictedRigidbody::RpcTeleport(UnityEngine.Vector3,UnityEngine.Vector3,UnityEngine.Quaternion)", InvokeUserCode_RpcTeleport__Vector3__Vector3__Quaternion);
			RemoteProcedureCalls.RegisterRpc(typeof(PredictedRigidbody), "System.Void Mirror.PredictedRigidbody::RpcSetEnabled(System.Boolean)", InvokeUserCode_RpcSetEnabled__Boolean);
		}
	}
}
