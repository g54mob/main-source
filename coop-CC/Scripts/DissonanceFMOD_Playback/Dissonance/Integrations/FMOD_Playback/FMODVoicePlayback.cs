...
namespace Dissonance.Integrations.FMOD_Playback
{
	public class FMODVoicePlayback : BaseVoicePlayback
	{
		...
		private GCHandle _customRollOffHandle;
		...
		public void SetCustomRollOff(VECTOR[] rollOff)
		{
			if (_customRollOffHandle.IsAllocated)
			{
				_customRollOffHandle.Free();
			}
			_customRollOff = new VECTOR[rollOff.Length];
			_customRollOffHandle = GCHandle.Alloc(_customRollOff, GCHandleType.Pinned);
			Array.Copy(rollOff, _customRollOff, rollOff.Length);
			RolloffMode = (RolloffMode)67108864u;
		}
		...
		private void UpdatePositionalPlayback(PlaybackOptions options)
		{
			...
			if (((IVoicePlaybackInternal)this).AllowPositionalPlayback && !DisablePositionalAudio && isPositional)
			{
				if ((mode & MODE._2D) == MODE._2D || ((uint)mode & (uint)RolloffMode) != (uint)RolloffMode)
				{
					...
					if (RolloffMode == (RolloffMode)67108864u)
					{
						_channel.set3DCustomRolloff(ref _customRollOff[0], 2);
					}
				}
		...
		{
			if (((DSP_STATE_FUNCTIONS)Marshal.PtrToStructure(dsp_state.functions, typeof(DSP_STATE_FUNCTIONS))).getuserdata(ref dsp_state, out var userdata) != RESULT.OK || userdata == IntPtr.Zero)
			{
