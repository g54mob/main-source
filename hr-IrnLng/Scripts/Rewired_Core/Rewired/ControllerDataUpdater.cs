...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] iEreyZchHxXkxADwquUvsqdpvzT;
		...
		public ControllerDataUpdater(InputSource source, int axisCount, int buttonCount, UnknownControllerHat[] unknownControllerHats)
		{
			...
			iEreyZchHxXkxADwquUvsqdpvzT = unknownControllerHats;
		...
		{
			if (iEreyZchHxXkxADwquUvsqdpvzT == null)
			{
				return false;
			}
			for (int i = 0; i < iEreyZchHxXkxADwquUvsqdpvzT.Length; i++)
			{
				if (iEreyZchHxXkxADwquUvsqdpvzT[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < iEreyZchHxXkxADwquUvsqdpvzT.Length; i++)
			{
				if (iEreyZchHxXkxADwquUvsqdpvzT[i].ContainsButtonIndex(buttonIndex))
				{
					return iEreyZchHxXkxADwquUvsqdpvzT[i].GetButtons();
				}
			}
			return null;
		}
