...
namespace Rewired.Components
{
	...
	public class PlayerController : ComponentWrapper<Rewired.PlayerController>, IPlayerController
	{
		...
		protected override void OnAwakeFinished()
		{
			...
			if (base.initialized)
			{
				GflfmMGmvPjwrerNZcYpjHlbnrmM(true);
			}
		}
		...
		protected override void Subscribe()
		{
			...
			if (base.source != null)
			{
				base.source.ButtonStateChangedEvent += EJRbZPCsRGUXIqeOPeXqcxZuMHBP;
				base.source.AxisValueChangedEvent += dlfTjjOSglEtOSsfBFnLjBJfdgnH;
				base.source.EnabledStateChangedEvent += GflfmMGmvPjwrerNZcYpjHlbnrmM;
			}
		}
		...
		internal virtual List<ElementInfo> OaIaHDdXOJqNhMKplJcWJEXwmiN()
		{
		...
		private void EJRbZPCsRGUXIqeOPeXqcxZuMHBP(int P_0, bool P_1)
		{
		...
		private void dlfTjjOSglEtOSsfBFnLjBJfdgnH(int P_0, float P_1)
		{
			if (!base.isActiveAndEnabled)
			{
				return;
		...
		private void GflfmMGmvPjwrerNZcYpjHlbnrmM(bool P_0)
		{
			try
			{
		...
		private void wAVRwkaeKIxPStlGaanYEiqozMm()
		{
			if (_elements == null || _elements.Count <= 0)
			{
				_elements = OaIaHDdXOJqNhMKplJcWJEXwmiN();
			}
		}
	}
}
