...
namespace Rewired.Utils
{
	...
	{
		private static uint WYyBNSbLGoEjKjuadJExLZnxKxEm = 0u;

		private static uint DEGRDfHmOkhXretHxBqRFpYhoZk = 1u;

		private static uint hpDGkmhLnDFyqXbRqzVLPnfIysp = uint.MaxValue;

		private static int RkWkfWNIABQrGpdoinBHsEbpgUQi = 0;

		private static int JNAErpdPgWuHtRbnHSHEsIZNhEH = int.MaxValue;

		private static int pQWNVhPmpjXeLGuapJGdhlVuBBgH = -1;
		...
		{
			counter = ((counter != hpDGkmhLnDFyqXbRqzVLPnfIysp) ? (counter + 1) : DEGRDfHmOkhXretHxBqRFpYhoZk);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != JNAErpdPgWuHtRbnHSHEsIZNhEH) ? (counter + 1) : RkWkfWNIABQrGpdoinBHsEbpgUQi);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != DEGRDfHmOkhXretHxBqRFpYhoZk) ? (counter - 1) : hpDGkmhLnDFyqXbRqzVLPnfIysp);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= pQWNVhPmpjXeLGuapJGdhlVuBBgH)
			{
				return JNAErpdPgWuHtRbnHSHEsIZNhEH;
			}
			counter = ((counter != RkWkfWNIABQrGpdoinBHsEbpgUQi) ? (counter - 1) : JNAErpdPgWuHtRbnHSHEsIZNhEH);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != WYyBNSbLGoEjKjuadJExLZnxKxEm;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > pQWNVhPmpjXeLGuapJGdhlVuBBgH;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == WYyBNSbLGoEjKjuadJExLZnxKxEm)
			{
				return false;
			}
			if (tick2 == WYyBNSbLGoEjKjuadJExLZnxKxEm)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= pQWNVhPmpjXeLGuapJGdhlVuBBgH)
			{
				return false;
			}
			if (tick2 <= pQWNVhPmpjXeLGuapJGdhlVuBBgH)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = hpDGkmhLnDFyqXbRqzVLPnfIysp - num + num2;
			uint dEGRDfHmOkhXretHxBqRFpYhoZk = DEGRDfHmOkhXretHxBqRFpYhoZk;
			uint num4 = num3 - dEGRDfHmOkhXretHxBqRFpYhoZk;
			if (tick1 >= tick2)
			{
				return 0L - (long)num4;
			}
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = JNAErpdPgWuHtRbnHSHEsIZNhEH - num + num2;
			int rkWkfWNIABQrGpdoinBHsEbpgUQi = RkWkfWNIABQrGpdoinBHsEbpgUQi;
			int num4 = num3 - rkWkfWNIABQrGpdoinBHsEbpgUQi;
			if (tick1 >= tick2)
			{
				return -num4;
			}
