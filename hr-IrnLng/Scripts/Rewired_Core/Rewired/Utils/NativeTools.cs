...
namespace Rewired.Utils
{
	...
	{
		private static byte[] jxoWixQQWzeDznDIwXBGYTlDuaq;
		...
		public static bool FillMemory(IntPtr buffer, int startIndex, int length, byte value, bool throwOnError = true)
		{
			...
			if (value != 0)
			{
				if (jxoWixQQWzeDznDIwXBGYTlDuaq == null)
				{
					jxoWixQQWzeDznDIwXBGYTlDuaq = new byte[8];
				}
				bool flag = false;
				if (num >= 8)
				{
					long val;
					lock (jxoWixQQWzeDznDIwXBGYTlDuaq)
					{
						for (int i = 0; i < 8; i++)
						{
							jxoWixQQWzeDznDIwXBGYTlDuaq[i] = value;
						}
						flag = true;
						val = BitConverter.ToInt64(jxoWixQQWzeDznDIwXBGYTlDuaq, 0);
					}
					int num2 = length / 8 * 8;
				...
				if (num >= 4)
				{
					...
					lock (jxoWixQQWzeDznDIwXBGYTlDuaq)
					{
						if (!flag)
						{
							for (int k = 0; k < 4; k++)
							{
								jxoWixQQWzeDznDIwXBGYTlDuaq[k] = value;
							}
							flag = true;
						}
						val2 = BitConverter.ToInt32(jxoWixQQWzeDznDIwXBGYTlDuaq, 0);
					}
					int num3 = length / 4 * 4;
				...
				if (num >= 2)
				{
					...
					lock (jxoWixQQWzeDznDIwXBGYTlDuaq)
					{
						if (!flag)
						{
							for (int m = 0; m < 2; m++)
							{
								jxoWixQQWzeDznDIwXBGYTlDuaq[m] = value;
							}
							flag = true;
						}
						val3 = BitConverter.ToInt16(jxoWixQQWzeDznDIwXBGYTlDuaq, 0);
					}
					int num4 = length / 2 * 2;
