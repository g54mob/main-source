...
namespace Rewired.Utils.Libraries.TinyJson
{
	public static class JsonParser
	{
		...
		private static StringBuilder ZKzFcPVJcvXzRwISDkRDlHqRGGX = new StringBuilder();
		...
		internal static bool TryFromJson<T>(string json, out T value, Type preferredAnonymousObjectType)
		{
			try
			{
				...
				ZKzFcPVJcvXzRwISDkRDlHqRGGX.Length = 0;
				for (int i = 0; i < json.Length; i++)
				{
					char c = json[i];
					if (c == '"')
					{
						i = wNyfiVxirndntXCOwedQTsAODwI(true, i, json);
					}
					else if (!char.IsWhiteSpace(c))
					{
						ZKzFcPVJcvXzRwISDkRDlHqRGGX.Append(c);
					}
				}
				value = (T)COxOlGchuHGHxBtutSccJKeQmfb(typeof(T), ZKzFcPVJcvXzRwISDkRDlHqRGGX.ToString(), preferredAnonymousObjectType, out var _);
				return true;
			}
			catch
			{
		...
		internal static T FromJson<T>(string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return (T)COxOlGchuHGHxBtutSccJKeQmfb(typeof(T), ZKzFcPVJcvXzRwISDkRDlHqRGGX.ToString(), preferredAnonymousObjectType, out flag);
		}
		...
		internal static object FromJson(Type type, string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return COxOlGchuHGHxBtutSccJKeQmfb(type, ZKzFcPVJcvXzRwISDkRDlHqRGGX.ToString(), preferredAnonymousObjectType, out flag);
		}

		private static object COxOlGchuHGHxBtutSccJKeQmfb(Type P_0, string P_1, Type P_2, out bool P_3)
		{
			...
			if (object.ReferenceEquals(P_0, typeof(Guid)))
			{
				...
				{
					bool flag;
					string g = (string)COxOlGchuHGHxBtutSccJKeQmfb(typeof(string), P_1, P_2, out flag);
					if (!flag)
					{
						P_3 = false;
						return Guid.Empty;
			...
			if (ReflectionTools.IsEnum(P_0))
			{
				...
				object obj2 = COxOlGchuHGHxBtutSccJKeQmfb(underlyingEnumType, P_1, P_2, out var flag2);
				...
				try
				{
					obj2 = COxOlGchuHGHxBtutSccJKeQmfb(typeof(string), P_1, P_2, out flag2);
			...
			{
				return DACxQZOEYDoSpoyKGjihJlCXHcl(P_1, P_2, out P_3);
			}
			if (ReflectionTools.IsArray(P_0))
			{
				...
				List<string> list = DycaqtmagsPTmNOqWfDHhpbcIyX(P_1);
				Array array = Array.CreateInstance(elementType, list.Count);
				for (int i = 0; i < list.Count; i++)
				{
					array.SetValue(COxOlGchuHGHxBtutSccJKeQmfb(elementType, list[i], P_2, out var _), i);
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(List<>))
			{
				...
				List<string> list3 = DycaqtmagsPTmNOqWfDHhpbcIyX(P_1);
				for (int j = 0; j < list3.Count; j++)
				{
					list2.Add(COxOlGchuHGHxBtutSccJKeQmfb(type, list3[j], P_2, out var _));
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(Dictionary<, >))
			{
				...
				List<string> list4 = DycaqtmagsPTmNOqWfDHhpbcIyX(P_1);
				try
				{
					if (list4.Count % 2 != 0)
					{
					...
					for (int k = 0; k < list4.Count; k += 2)
					{
						if (list4[k].Length > 2)
						{
							...
							bool flag6;
							object value = COxOlGchuHGHxBtutSccJKeQmfb(type3, list4[k + 1], P_2, out flag6);
			...
			{
				P_3 = true;
				return EEQbfgBtpgkZsPfmQqAHmhxxHL(P_0, P_1, P_2);
			}
			P_3 = false;
			return null;
		}

		private static object DACxQZOEYDoSpoyKGjihJlCXHcl(string P_0, Type P_1, out bool P_2)
		{
			if (P_0.Length == 0)
			{
				P_2 = false;
			...
			{
				List<string> list = DycaqtmagsPTmNOqWfDHhpbcIyX(P_0);
				try
				{
					if (list.Count % 2 != 0)
					{
					...
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddKeyValue<string, object>)))
					{
						...
						{
							addKeyValue.Add(list[i].Substring(1, list[i].Length - 2), DACxQZOEYDoSpoyKGjihJlCXHcl(list[i + 1], P_1, out var _));
						}
						P_2 = true;
						return addKeyValue;
					}
					...
					{
						dictionary.Add(list[j].Substring(1, list[j].Length - 2), DACxQZOEYDoSpoyKGjihJlCXHcl(list[j + 1], P_1, out var _));
					}
					P_2 = true;
					return dictionary;
				}
			...
			if (P_0[0] == '[' && P_0[P_0.Length - 1] == ']')
			{
				List<string> list2 = DycaqtmagsPTmNOqWfDHhpbcIyX(P_0);
				try
				{
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddValue<object>)))
					{
						...
						{
							addValue.Add(DACxQZOEYDoSpoyKGjihJlCXHcl(list2[k], P_1, out var _));
						}
						P_2 = true;
						return addValue;
					}
					...
					{
						list3.Add(DACxQZOEYDoSpoyKGjihJlCXHcl(list2[l], P_1, out var _));
					}
					P_2 = true;
					return list3;
				}
		...
		private static object EEQbfgBtpgkZsPfmQqAHmhxxHL(Type P_0, string P_1, Type P_2)
		{
			...
			try
			{
				...
				for (int num = 0; num < list.Count; num += 2)
				{
					if (list[num].Length > 2)
					{
						...
						{
							value3.SetValue(obj, COxOlGchuHGHxBtutSccJKeQmfb(value3.FieldType, text, P_2, out var _));
						}
						else if (value2.TryGetValue(key, out value4) && value4.CanWrite)
						{
							value4.SetValue(obj, COxOlGchuHGHxBtutSccJKeQmfb(value4.PropertyType, text, P_2, out var _), null);
						}
					}
				}
		...
		private static int wNyfiVxirndntXCOwedQTsAODwI(bool P_0, int P_1, string P_2)
		{
			ZKzFcPVJcvXzRwISDkRDlHqRGGX.Append(P_2[P_1]);
			for (int i = P_1 + 1; i < P_2.Length; i++)
			{
				if (P_2[i] == '\\')
				{
					if (P_0)
					{
						ZKzFcPVJcvXzRwISDkRDlHqRGGX.Append(P_2[i]);
					}
					ZKzFcPVJcvXzRwISDkRDlHqRGGX.Append(P_2[i + 1]);
					i++;
				}
				else
				{
		...
		private static List<string> DycaqtmagsPTmNOqWfDHhpbcIyX(string P_0)
		{
			...
			for (int i = 1; i < P_0.Length - 1; i++)
			{
				...
				case '}':
					...
					break;
				case '"':
					i = wNyfiVxirndntXCOwedQTsAODwI(true, i, P_0);
					continue;
				case ',':
				case ':':
					if (num == 0)
					{
						list.Add(ZKzFcPVJcvXzRwISDkRDlHqRGGX.ToString());
						ZKzFcPVJcvXzRwISDkRDlHqRGGX.Length = 0;
						continue;
					}
					break;
				}
				ZKzFcPVJcvXzRwISDkRDlHqRGGX.Append(P_0[i]);
			}
			if (ZKzFcPVJcvXzRwISDkRDlHqRGGX.Length == 0)
			{
				return list;
			}
			list.Add(ZKzFcPVJcvXzRwISDkRDlHqRGGX.ToString());
		...
		[CompilerGenerated]
		private static bool xhSIsMfgmmciyGhMfSjAULkcVLDv(FieldInfo P_0)
		{
		...
		[CompilerGenerated]
		private static string ZPvDuYYgVbDUCLkIHZWVxBNygpw(FieldInfo P_0)
		{
			string name;
		...
		[CompilerGenerated]
		private static bool lJCQdwwhMgdwXpQzUqNJhOTHLip(PropertyInfo P_0)
		{
		...
		[CompilerGenerated]
		private static string tBHcCOFCrApfSxakUFYZFHsVpLHa(PropertyInfo P_0)
		{
			string name;
			if (P_0.IsDefined(typeof(SerializeAttribute), inherit: true) && !string.IsNullOrEmpty(name = (CollectionTools.GetValue(P_0.GetCustomAttributes(typeof(SerializeAttribute), inherit: true), 0) as SerializeAttribute).Name))
			{
