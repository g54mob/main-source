...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		private static Action<StringBuilder, object> anpbkODqKZaDQfeBXTZkecRDWUOc;

		private static Action<StringBuilder, object> appendValueDelegate => XEihKwDzDEGLiakoEzSelPkaUoLG;

		public static string ToJson(object item)
		{
			StringBuilder stringBuilder = new StringBuilder();
			XEihKwDzDEGLiakoEzSelPkaUoLG(stringBuilder, item);
			return stringBuilder.ToString();
		}

		private static void XEihKwDzDEGLiakoEzSelPkaUoLG(StringBuilder P_0, object P_1)
		{
			if (P_1 == null)
			{
				P_0.Append("null");
			...
			{
				XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, P_1.ToString());
				return;
			}
			if (ReflectionTools.IsEnum(type))
			{
				Type underlyingType = Enum.GetUnderlyingType(type);
				XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, Convert.ChangeType(P_1, underlyingType));
				return;
			}
			if (ReflectionTools.DoesTypeImplement(type, typeof(IList)))
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, list[i]);
				}
				P_0.Append(']');
				return;
			}
			if (ReflectionTools.IsGenericType(type) && ReflectionTools.DoesTypeImplement(type, typeof(IDictionary)))
			{
				...
				foreach (object key in dictionary.Keys)
				{
					...
					P_0.Append("\":");
					XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, dictionary[key]);
				}
				P_0.Append('}');
				return;
			}
			...
			foreach (FieldInfo item in fields)
			{
				...
				if (value != null)
				{
					...
					P_0.Append(name);
					P_0.Append("\":");
					XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, value);
				}
			}
			...
			foreach (PropertyInfo item2 in properties)
			{
				...
				if (value2 != null)
				{
					...
					P_0.Append(name2);
					P_0.Append("\":");
					XEihKwDzDEGLiakoEzSelPkaUoLG(P_0, value2);
				}
			}
			P_0.Append('}');
		}
