...
namespace Rewired
{
	...
	{
		internal class nVKdNlGaejzDgsTfDjPFiRPkzxZ
		{
			public bool tuOoaWjJcsSsTfeBIBlbcFvEohHN;

			public bool QEkNIaSiNtHCGrfDkwaIGgNnXKQ;

			public yHeSKBPrBThpnUTMvsTADEFobHAk[] bLOwMGqKBTPxyCzCjARskoyUTma;

			public nVKdNlGaejzDgsTfDjPFiRPkzxZ(bool enabled, bool loadFromUserDataStore, yHeSKBPrBThpnUTMvsTADEFobHAk[] startingRuleSets)
			{
				tuOoaWjJcsSsTfeBIBlbcFvEohHN = enabled;
				QEkNIaSiNtHCGrfDkwaIGgNnXKQ = loadFromUserDataStore;
		...
		private bool fwCOJJzhymqJZycZQMRxuQVuMXi = true;

		private Player UoLXbrXQwBeLviIASurDpYTunPn;

		private nVKdNlGaejzDgsTfDjPFiRPkzxZ EOKlZufNtTPjOMfHekgiKjBAtbg;

		private readonly int VumWnlylMgxSbyJcluXptXvaaZa;

		private List<RuleSet> BtJXMUlGJDBupmSNdYxjyXVRDVA;

		private Action DooDnwmMIPtrRzRIyJaGyRiHkiA;
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return fwCOJJzhymqJZycZQMRxuQVuMXi;
			}
			set
			{
				fwCOJJzhymqJZycZQMRxuQVuMXi = value;
			}
		}
		...
		internal event Action ApplyCalledEvent
		{
			add
			{
				DooDnwmMIPtrRzRIyJaGyRiHkiA = (Action)Delegate.Combine(DooDnwmMIPtrRzRIyJaGyRiHkiA, value);
			}
			remove
			{
				DooDnwmMIPtrRzRIyJaGyRiHkiA = (Action)Delegate.Remove(DooDnwmMIPtrRzRIyJaGyRiHkiA, value);
			}
		}

		internal ControllerMapLayoutManager(Player player, nVKdNlGaejzDgsTfDjPFiRPkzxZ startingSettings)
		{
			if (player == null)
			{
		...
		public void Apply()
		{
			...
			DooDnwmMIPtrRzRIyJaGyRiHkiA?.Invoke();
			...
			list2.AddRange(UoLXbrXQwBeLviIASurDpYTunPn.controllers.Controllers);
			...
			for (int i = 0; i < count; i++)
			{
				...
				for (int j = 0; j < count2; j++)
				{
					...
					for (int num = count3 - 1; num >= 0; num--)
					{
						...
						{
							list.RemoveAt(num);
							UoLXbrXQwBeLviIASurDpYTunPn.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in UoLXbrXQwBeLviIASurDpYTunPn.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = UoLXbrXQwBeLviIASurDpYTunPn.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (fwCOJJzhymqJZycZQMRxuQVuMXi && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(UoLXbrXQwBeLviIASurDpYTunPn.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									UoLXbrXQwBeLviIASurDpYTunPn.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							UoLXbrXQwBeLviIASurDpYTunPn.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(EOKlZufNtTPjOMfHekgiKjBAtbg.bLOwMGqKBTPxyCzCjARskoyUTma[i].id);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = EOKlZufNtTPjOMfHekgiKjBAtbg.bLOwMGqKBTPxyCzCjARskoyUTma[i].startEnabled;
			...
			if (EOKlZufNtTPjOMfHekgiKjBAtbg != null)
			{
				...
				fwCOJJzhymqJZycZQMRxuQVuMXi = EOKlZufNtTPjOMfHekgiKjBAtbg.QEkNIaSiNtHCGrfDkwaIGgNnXKQ;
			}
		...
		private void jcgUSwYyXKIwVuYwxHnWUgkgsoK(SerializedObject P_0)
		{
			...
			P_0.Add("loadFromUserDataStore", fwCOJJzhymqJZycZQMRxuQVuMXi);
		...
		private bool tlMbXbDwaaKJTudkJIuTPdZmwuo(SerializedObject P_0)
		{
			...
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref fwCOJJzhymqJZycZQMRxuQVuMXi);
