...
namespace Rewired
{
	...
	public static class Consts
	{
		...
		internal const string schemaVersionCalibrationMap = "1.4";
		...
		internal const int calibrationMapDataVersion = 5;
		...
		private static readonly Guid[] oXuKEqNYYtAykoWXLILyNKIdSOmc;

		internal static readonly ReadOnlyCollection<Guid> reservedHardwareTypeGuids;

		private static ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS[] swLZCBLKXjbIefnOmznpXhwPjJGx;

		private static ReadOnlyCollection<ControllerElementIdentifier> DSZgaAGOIZaFwCmlGTlvOBpHoqkPC;

		private static ReadOnlyCollection<ControllerElementIdentifier> iqsgedcYmigdTFYIkqLQWvmkIUhH;

		private static ReadOnlyCollection<HardwareJoystickMap.CompoundElement> bnwrpljLpSjtJKbOojperZmuctPf;

		internal static readonly IList<string> mouseAxisUnityNames;

		private static readonly string[] qUvmiFtkAABCpXqShhwlWWCmAJot;

		internal static readonly IList<string> mouseButtonUnityNames;

		private static readonly string[] pFyOCwplZVEQuBdaiDqULQXJlPfeA;

		internal static readonly IList<string> keyboardKeyNames;

		private static readonly string[] xKOXLYAieeGeUeGYvyLHMyqPMTfbA;

		internal static readonly IList<int> keyboardKeyValues;

		internal static readonly int[] _keyboardKeyValues;

		private static readonly IList<string> YoUBRHAPTFAgncAzHKYPzDmwtXMdb;

		private static readonly string[] AsrdKMaTPHGATfYAEjNmiSIbKUUrB;

		private static readonly IList<string> jGYveQvdjcEsfbOWOCXIuXkKljctA;

		private static readonly string[] ytBMPTCwbUiFjhubUFGdsqObRHsi;
		...
		private static readonly PidVid[] qknrQRDoIqjftTMigGJthBLkoiUHA;

		internal static readonly IList<string> productNames_sony_dualShock4;

		private static readonly string[] lBtPoLcNkkFtpXYrcRprEOChzdRx;

		internal static readonly IList<PidVid> pidVids_sony_dualSense;

		private static readonly PidVid[] BWJeUOuExyQrWciFjXvQfiMRdLOd;

		internal static readonly IList<string> productNames_sony_dualSense;

		private static readonly string[] nYdyFtuyvYcpQxUiCaVokFVYdjYCA;

		private static readonly ControllerElementIdentifier[] ydJfcjCnQVDSIVutqtcNjGrWoBuU;
		...
		internal static ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS[] commonMouseElementIdentifierInitOptions
		{
			get
			{
				if (swLZCBLKXjbIefnOmznpXhwPjJGx != null)
				{
					return swLZCBLKXjbIefnOmznpXhwPjJGx;
				}
				swLZCBLKXjbIefnOmznpXhwPjJGx = new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS[12]
				{
					new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS
					{
						id = 0,
					...
					new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS
					{
						...
						positiveKey = "",
						negativeKey = "",
						elementType = ControllerElementType.CompoundElement,
						compoundElementType = CompoundControllerElementType.Axis2D
					}
				};
				return swLZCBLKXjbIefnOmznpXhwPjJGx;
		...
		internal static IList<ControllerElementIdentifier> unityUnifiedMouseElementIdentifiers => DSZgaAGOIZaFwCmlGTlvOBpHoqkPC ?? (DSZgaAGOIZaFwCmlGTlvOBpHoqkPC = new ReadOnlyCollection<ControllerElementIdentifier>(new ControllerElementIdentifier[12]
		{
			...
			new ControllerElementIdentifier(commonMouseElementIdentifierInitOptions[11])
		}));

		internal static IList<ControllerElementIdentifier> rawInputUnifiedMouseElementIdentifiers => iqsgedcYmigdTFYIkqLQWvmkIUhH ?? (iqsgedcYmigdTFYIkqLQWvmkIUhH = new ReadOnlyCollection<ControllerElementIdentifier>(new ControllerElementIdentifier[10]
		{
			new ControllerElementIdentifier(commonMouseElementIdentifierInitOptions[0]),
			new ControllerElementIdentifier(commonMouseElementIdentifierInitOptions[1]),
			new ControllerElementIdentifier(commonMouseElementIdentifierInitOptions[2]),
		...
		internal static IList<HardwareJoystickMap.CompoundElement> unifiedMouseCompoundElements_readOnly => bnwrpljLpSjtJKbOojperZmuctPf ?? (bnwrpljLpSjtJKbOojperZmuctPf = new ReadOnlyCollection<HardwareJoystickMap.CompoundElement>(new HardwareJoystickMap.CompoundElement[1]
		{
			new HardwareJoystickMap.CompoundElement
			{
				elementIdentifier = 11,
				type = CompoundControllerElementType.Axis2D,
				componentElementIdentifiers = new int[2] { 0, 1 }
			}
		}));

		public static IList<string> keyboardKeyKeys => YoUBRHAPTFAgncAzHKYPzDmwtXMdb;

		public static IList<string> keyboardModifierKeyKeys => jGYveQvdjcEsfbOWOCXIuXkKljctA;

		internal static ControllerElementIdentifier[] unknownJoystickElementIdentifiers_orig => ydJfcjCnQVDSIVutqtcNjGrWoBuU;
		...
		static Consts()
		{
			...
			oXuKEqNYYtAykoWXLILyNKIdSOmc = new Guid[3] { joystickGuid_unknownController, hardwareTypeGuid_universalKeyboard, hardwareTypeGuid_universalMouse };
			qUvmiFtkAABCpXqShhwlWWCmAJot = new string[3] { "MouseAxis1", "MouseAxis2", "MouseAxis3" };
			pFyOCwplZVEQuBdaiDqULQXJlPfeA = new string[7] { "MouseButton0", "MouseButton1", "MouseButton2", "MouseButton3", "MouseButton4", "MouseButton5", "MouseButton6" };
			xKOXLYAieeGeUeGYvyLHMyqPMTfbA = new string[132]
			{
			...
			AsrdKMaTPHGATfYAEjNmiSIbKUUrB = new string[132]
			{
				"", "a", "b", "c", "d", "e", "f", "g", "h", "i",
				"j", "k", "l", "m", "n", "o", "p", "q", "r", "s",
			...
			ytBMPTCwbUiFjhubUFGdsqObRHsi = new string[5] { "", "control", "alt", "shift", "command" };
			...
			qknrQRDoIqjftTMigGJthBLkoiUHA = new PidVid[3]
			{
				new PidVid(1476, 1356),
				new PidVid(2976, 1356),
				new PidVid(2508, 1356)
			};
			lBtPoLcNkkFtpXYrcRprEOChzdRx = new string[5] { "Sony Computer Entertainment Wireless Controller", "Sony Interactive Entertainment DUALSHOCK®4 USB Wireless Adaptor", "Wireless Controller", "Sony Interactive Entertainment Wireless Controller", "Wireless Controller Touchpad" };
			BWJeUOuExyQrWciFjXvQfiMRdLOd = new PidVid[2]
			{
				new PidVid(3302, 1356),
				new PidVid(3570, 1356)
			};
			nYdyFtuyvYcpQxUiCaVokFVYdjYCA = new string[1] { "DualSense Wireless Controller" };
			mouseAxisUnityNames = new ReadOnlyCollection<string>(qUvmiFtkAABCpXqShhwlWWCmAJot);
			mouseButtonUnityNames = new ReadOnlyCollection<string>(pFyOCwplZVEQuBdaiDqULQXJlPfeA);
			keyboardKeyNames = new ReadOnlyCollection<string>(xKOXLYAieeGeUeGYvyLHMyqPMTfbA);
			keyboardKeyValues = new ReadOnlyCollection<int>(_keyboardKeyValues);
			YoUBRHAPTFAgncAzHKYPzDmwtXMdb = new ReadOnlyCollection<string>(AsrdKMaTPHGATfYAEjNmiSIbKUUrB);
			jGYveQvdjcEsfbOWOCXIuXkKljctA = new ReadOnlyCollection<string>(ytBMPTCwbUiFjhubUFGdsqObRHsi);
			pidVids_sony_dualShock4 = new ReadOnlyCollection<PidVid>(qknrQRDoIqjftTMigGJthBLkoiUHA);
			productNames_sony_dualShock4 = new ReadOnlyCollection<string>(lBtPoLcNkkFtpXYrcRprEOChzdRx);
			pidVids_sony_dualSense = new ReadOnlyCollection<PidVid>(BWJeUOuExyQrWciFjXvQfiMRdLOd);
			productNames_sony_dualSense = new ReadOnlyCollection<string>(nYdyFtuyvYcpQxUiCaVokFVYdjYCA);
			reservedHardwareTypeGuids = new ReadOnlyCollection<Guid>(oXuKEqNYYtAykoWXLILyNKIdSOmc);
			...
			ydJfcjCnQVDSIVutqtcNjGrWoBuU = ZfcgpIgZLcPSfQQheGOxTSNcIYki();
			if (132 != xKOXLYAieeGeUeGYvyLHMyqPMTfbA.Length)
			{
			...
			if (132 != AsrdKMaTPHGATfYAEjNmiSIbKUUrB.Length)
			{
				Logger.LogError("Consts.keyboardKeyCount does not match _keyboardKeyStringKeys.Length!");
			}
		}

		private static ControllerElementIdentifier[] ZfcgpIgZLcPSfQQheGOxTSNcIYki()
		{
			int num = 0;
			List<ControllerElementIdentifier> list = new List<ControllerElementIdentifier>(288);
			ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS mlDJkVjamumzDMNFFhJnSSbvSsLS = new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS();
			for (int i = 0; i < 32; i++)
			{
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Axis " + i;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.positiveName = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.negativeName = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.ConcatenateKeyStrings("axis", i.ToString());
				mlDJkVjamumzDMNFFhJnSSbvSsLS.positiveKey = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.negativeKey = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.elementType = ControllerElementType.Axis;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.compoundElementType = CompoundControllerElementType.Axis2D;
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				num++;
			}
			for (int j = 0; j < 128; j++)
			{
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Button " + j;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.positiveName = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.negativeName = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.ConcatenateKeyStrings("button", j.ToString());
				mlDJkVjamumzDMNFFhJnSSbvSsLS.positiveKey = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.negativeKey = string.Empty;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.elementType = ControllerElementType.Button;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.compoundElementType = CompoundControllerElementType.Axis2D;
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				num++;
			}
			int num2 = num;
			int num3 = num + 64;
			mlDJkVjamumzDMNFFhJnSSbvSsLS = new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS();
			mlDJkVjamumzDMNFFhJnSSbvSsLS.elementType = ControllerElementType.Button;
			mlDJkVjamumzDMNFFhJnSSbvSsLS.compoundElementType = CompoundControllerElementType.Axis2D;
			for (int k = 0; k < 16; k++)
			{
				string a = LocalizationManager.ConcatenateKeyStrings("hat", k.ToString());
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num2++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Up";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "up");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num3++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Up Right";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "up_right");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num2++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Right";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "right");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num3++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Down Right";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "down_right");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num2++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Down";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "down");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num3++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Down Left";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "down_left");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num2++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Left";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "left");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
				mlDJkVjamumzDMNFFhJnSSbvSsLS.id = num3++;
				mlDJkVjamumzDMNFFhJnSSbvSsLS.name = "Hat " + k + " Up Left";
				mlDJkVjamumzDMNFFhJnSSbvSsLS.key = LocalizationManager.AppendToKeyAsPath(a, "up_left");
				list.Add(new ControllerElementIdentifier(mlDJkVjamumzDMNFFhJnSSbvSsLS));
			}
			return list.ToArray();
		}
