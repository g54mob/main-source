...
namespace Rewired
{
	...
	{
		private enum QYHVaNaXGIjOBhZmpUoffaDDGnWu
		{
			Error = 0,
			FoundIndex = 1,
			IsWholeElement = 2
		...
		public readonly Axis2DCalibrationData[] hwAxis2DCalibrationData;
		...
		private readonly DeviceLocalizationInfo ZxSKoxrHjLFTuCJhYnxIJADFXIkRA;

		public DeviceLocalizationInfo deviceLocalizationInfo => ZxSKoxrHjLFTuCJhYnxIJADFXIkRA;
		...
		public HardwareControllerMap_Game(string P_0, DeviceLocalizationInfo P_1, int P_2, ControllerElementIdentifier[] P_3, int[] P_4, int[] P_5, AxisCalibrationData[] P_6, AxisRange[] P_7, HardwareAxisInfo[] P_8, HardwareButtonInfo[] P_9, HardwareJoystickMap.CompoundElement[] P_10)
			: this(P_0, P_1, P_3, P_4, P_5, P_6, null, P_7, P_8, P_9, P_10)
		{
			customControllerSourceId = P_2;
		}

		public HardwareControllerMap_Game(string P_0, DeviceLocalizationInfo P_1, HardwareControllerMapIdentifier P_2, JoystickType[] P_3, ControllerElementIdentifier[] P_4, int[] P_5, int[] P_6, AxisCalibrationData[] P_7, Axis2DCalibrationData[] P_8, AxisRange[] P_9, HardwareAxisInfo[] P_10, HardwareButtonInfo[] P_11, HardwareJoystickMap.CompoundElement[] P_12)
			: this(P_0, P_1, P_4, P_5, P_6, P_7, P_8, P_9, P_10, P_11, P_12)
		{
		...
		public HardwareControllerMap_Game(string P_0, HardwareControllerMapIdentifier P_1, ControllerElementIdentifier[] P_2, int[] P_3, int[] P_4, AxisCalibrationData[] P_5, AxisRange[] P_6, HardwareAxisInfo[] P_7, HardwareButtonInfo[] P_8, HardwareJoystickMap.CompoundElement[] P_9)
			: this(P_0, string.Equals(P_0, "Keyboard", StringComparison.OrdinalIgnoreCase) ? new DeviceLocalizationInfo(ControllerType.Keyboard, false, Consts.hardwareTypeGuid_universalKeyboard, new List<string> { "keyboard" }, null) : (string.Equals(P_0, "Mouse", StringComparison.OrdinalIgnoreCase) ? new DeviceLocalizationInfo(ControllerType.Mouse, false, Consts.hardwareTypeGuid_universalMouse, new List<string> { "mouse" }, null) : new DeviceLocalizationInfo()), P_1, null, P_2, P_3, P_4, P_5, null, P_6, P_7, P_8, P_9)
		{
		}

		private HardwareControllerMap_Game(string P_0, DeviceLocalizationInfo P_1, ControllerElementIdentifier[] P_2, int[] P_3, int[] P_4, AxisCalibrationData[] P_5, Axis2DCalibrationData[] P_6, AxisRange[] P_7, HardwareAxisInfo[] P_8, HardwareButtonInfo[] P_9, HardwareJoystickMap.CompoundElement[] P_10)
		{
			controllerName = P_0;
			ZxSKoxrHjLFTuCJhYnxIJADFXIkRA = P_1;
			if (ZxSKoxrHjLFTuCJhYnxIJADFXIkRA == null)
			{
				ZxSKoxrHjLFTuCJhYnxIJADFXIkRA = new DeviceLocalizationInfo();
			}
			ZxSKoxrHjLFTuCJhYnxIJADFXIkRA.FinishRuntimeSetup();
			bool flag = ZxSKoxrHjLFTuCJhYnxIJADFXIkRA.controllerType != ControllerType.Keyboard && ZxSKoxrHjLFTuCJhYnxIJADFXIkRA.controllerType != ControllerType.Mouse;
			for (int i = 0; i < P_2.Length; i++)
			{
				...
				if (flag)
				{
					if (ControllerElementIdentifier.bynVTTSgSxOQdaApptfwfVMhuAVq.TXsaGGEzAKsMMOmllodYZbvsmJDL(ZxSKoxrHjLFTuCJhYnxIJADFXIkRA, P_2[i], out var controllerElementIdentifier))
					{
						P_2[i] = controllerElementIdentifier;
						continue;
					}
					ControllerElementIdentifier.bynVTTSgSxOQdaApptfwfVMhuAVq.RdYPLYahvLQFjXWsakzmTmhbvMfu(ZxSKoxrHjLFTuCJhYnxIJADFXIkRA, P_2[i]);
				}
				P_2[i].FinishRuntimeSetup(ZxSKoxrHjLFTuCJhYnxIJADFXIkRA, ZxSKoxrHjLFTuCJhYnxIJADFXIkRA.controllerType);
			}
			elementIdentifierCount = ((P_2 != null) ? P_2.Length : 0);
			int num = ((P_3 != null) ? P_3.Length : 0);
			int num2 = ((P_4 != null) ? P_4.Length : 0);
			P_10 = ArrayTools.DeepClone(P_10);
			compoundElements = P_10;
			compoundElementCount = ((P_10 != null) ? P_10.Length : 0);
			int num3 = 0;
			int num4 = 0;
			int num5 = 0;
			...
			for (int j = 0; j < compoundElementCount; j++)
			{
				if (P_10[j] != null)
				{
					...
					case CompoundControllerElementType.Axis2D:
						num3++;
						list.Add(P_10[j].elementIdentifier);
						break;
					case CompoundControllerElementType.Hat:
						num4++;
						list2.Add(P_10[j].elementIdentifier);
						HardwareJoystickMap.CompoundElement.SortHatElementsClockwise(P_10[j]);
						break;
					case CompoundControllerElementType.DPad:
						num5++;
						list3.Add(P_10[j].elementIdentifier);
						break;
					}
				}
			}
			...
			for (int l = 0; l < num; l++)
			{
				int num6 = HuYNqxgEyaDAymCKuhXYgxoVXdhwA(P_2, P_3[l]);
				if (num6 < 0)
				{
			...
			{
				int num7 = HuYNqxgEyaDAymCKuhXYgxoVXdhwA(P_2, P_4[m]);
				if (num7 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num8 = HuYNqxgEyaDAymCKuhXYgxoVXdhwA(P_2, array[n]);
				if (num8 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num10 = HuYNqxgEyaDAymCKuhXYgxoVXdhwA(P_2, array2[num9]);
				if (num10 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num12 = HuYNqxgEyaDAymCKuhXYgxoVXdhwA(P_2, array3[num11]);
				if (num12 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
		...
		public bool TryGetCompoundElementMemberCombinedLocalizedName(IList<ActionElementMap> aems, out string result)
		{
			...
			for (int i = 0; i < compoundElements.Length; i++)
			{
				...
				int num2;
				switch (jObSjwsxAkbnHpOBwIxMVEDKAsgm(compoundElement, elementIdentifierById, aems, out num2))
				{
				case QYHVaNaXGIjOBhZmpUoffaDDGnWu.IsWholeElement:
					result = elementIdentifierById.Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Ename;
					break;
				case QYHVaNaXGIjOBhZmpUoffaDDGnWu.FoundIndex:
					if (num2 >= 0)
					{
		...
		private int HuYNqxgEyaDAymCKuhXYgxoVXdhwA(ControllerElementIdentifier[] P_0, int P_1)
		{
			if (P_0 == null)
			{
				return -1;
		...
		private static QYHVaNaXGIjOBhZmpUoffaDDGnWu jObSjwsxAkbnHpOBwIxMVEDKAsgm(HardwareJoystickMap.CompoundElement P_0, ControllerElementIdentifier P_1, IList<ActionElementMap> P_2, out int P_3)
		{
			P_3 = -1;
			if (P_0 == null || P_1 == null || P_2 == null)
			{
				return QYHVaNaXGIjOBhZmpUoffaDDGnWu.Error;
			}
			...
			MJyJuisFiOmfspJhIRvXPkFSAPFT.eSrXtKMIUZjpxRfWzNHzGoDkUsXw eSrXtKMIUZjpxRfWzNHzGoDkUsXw;
			...
			case CompoundControllerElementType.Axis2D:
			{
				...
				if (num3 == 1)
				{
					if (num4 == 1)
					{
						...
						return QYHVaNaXGIjOBhZmpUoffaDDGnWu.IsWholeElement;
					}
					if (num7 == 1 && num8 == 1)
					{
						if (count > 3)
				...
				else if (num5 == 1 && num6 == 1)
				{
					...
					if (MJyJuisFiOmfspJhIRvXPkFSAPFT.yzssAvNAWgRZfJKNdgmvMuNHsBFU(resultCompoundElementType, out eSrXtKMIUZjpxRfWzNHzGoDkUsXw))
					{
						P_3 = eSrXtKMIUZjpxRfWzNHzGoDkUsXw.lFBHGOjGPnfslATwiJdyghyJDnFxA(AxisDirection.Horizontal);
						if (P_3 >= 0)
						{
							return QYHVaNaXGIjOBhZmpUoffaDDGnWu.FoundIndex;
						}
						return QYHVaNaXGIjOBhZmpUoffaDDGnWu.Error;
					}
				}
				else
				{
					...
					if (MJyJuisFiOmfspJhIRvXPkFSAPFT.yzssAvNAWgRZfJKNdgmvMuNHsBFU(resultCompoundElementType, out eSrXtKMIUZjpxRfWzNHzGoDkUsXw))
					{
						P_3 = eSrXtKMIUZjpxRfWzNHzGoDkUsXw.lFBHGOjGPnfslATwiJdyghyJDnFxA(AxisDirection.Vertical);
						if (P_3 >= 0)
						{
							return QYHVaNaXGIjOBhZmpUoffaDDGnWu.FoundIndex;
						}
