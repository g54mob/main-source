...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			pmihIRNLodcRXDmyqfHbKHDqJjAeb(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return NPmHLldnCQTCmOhozWieNmGbehBp().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				GJRapspNDjAQkfTiApqoSutnSVSv(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return NPmHLldnCQTCmOhozWieNmGbehBp().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				GJRapspNDjAQkfTiApqoSutnSVSv(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			pmihIRNLodcRXDmyqfHbKHDqJjAeb(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.XEVFsuEMxHkciZSLzbKRBSDQzeCr(_id);
			if (inputBehavior != null)
			{
				pmihIRNLodcRXDmyqfHbKHDqJjAeb(inputBehavior, this, true);
			}
		}

		internal SerializedObject NPmHLldnCQTCmOhozWieNmGbehBp()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.GlzRlrSmJMPGyhIzDQJHlmQHORtg
			{
				pdYiVMKqONWNQjSqPcOhYrKSabZR = "id",
				colvBdeALTpVyhJTAuogspkzwFfR = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.GlzRlrSmJMPGyhIzDQJHlmQHORtg
			{
		...
		internal void GJRapspNDjAQkfTiApqoSutnSVSv(SerializedObject P_0)
		{
			Reset();
		...
		private static void pmihIRNLodcRXDmyqfHbKHDqJjAeb(InputBehavior P_0, InputBehavior P_1, bool P_2)
		{
			if (P_2)
			{
				P_1._id = P_0._id;
