...
namespace Rewired
{
	...
	public sealed class ActionElementMap
	{
		...
		[NonSerialized]
		internal ControllerMap SJKUZyLtwpanDYOUUcKgIbLBAesr;

		[NonSerialized]
		internal bool hrXjVMVBGWHRhCIrzlnSmtoGojQeb = true;

		[NonSerialized]
		internal int fpLTJzOTpoUWkyThKhrqRzXDquMW;

		[NonSerialized]
		internal readonly int gjHUlVyQSQsjZEOHtHfmeehEQpiIA;

		[NonSerialized]
		private uint HPEAEdunYkaVtxWMDJTEzmEMdMAI;

		[NonSerialized]
		private string uEsWzOEQHcFJNJegZAXGWknFagQs;

		[NonSerialized]
		private string zcACNMBTRvXOAVXoLfuIHiCUUHIkA;

		[NonSerialized]
		private ModifierKeyFlags pBwnLDEgxUlyylxGTBtmDPkOjnhaA;

		[NonSerialized]
		private HardwareControllerMap_Game biKWDvLXXzgXYvNdGjCLoXodFzqC;

		[NonSerialized]
		private double umNQvPJkSzKRFKteZPGBqzrZyzMD;

		[NonSerialized]
		private ActionElementMap UbyyzSRixoeEDbIobbKqYCATBALV;

		[NonSerialized]
		private ActionElementMap phiHNWdAPICbVAPvgALmFIDiVXiIc;

		[NonSerialized]
		private int IUYJUSJfoWaJqVrBeCdDKuyGkWerA;
		...
		public int elementIdentifierId
		{
			...
			set
			{
				...
				if (ReInput.isReady && SJKUZyLtwpanDYOUUcKgIbLBAesr != null)
				{
					Controller controller = ReInput.AtHYwRgWVYrmVOsWolCxiSLKHuEp.DFkLoOraYKZxPsJxXLcjqacSAAGg(SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType, SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerId, true);
					if (controller != null)
					{
						Controller.Element elementById = controller.GetElementById(value);
						if (elementById != null && elementById.type != _elementType)
						{
							SJKUZyLtwpanDYOUUcKgIbLBAesr.UGuYgGVZKXVfNbvQbryOLFsKAFlj(gjHUlVyQSQsjZEOHtHfmeehEQpiIA, elementById.type);
						}
					}
				}
				if (ReInput.isReady)
				{
					pandLAHLrwAbBavtribqUEfJqRJFA(false);
				}
				bficrGUDNywRTpHoFGmJjjCndKvX();
			}
		}
		...
		public KeyboardKeyCode keyboardKeyCode
		{
			...
			set
			{
				...
				if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType != ControllerType.Keyboard)
				{
				...
				if (ReInput.isReady)
				{
					pandLAHLrwAbBavtribqUEfJqRJFA(true);
				}
				bficrGUDNywRTpHoFGmJjjCndKvX();
			}
		}
		...
		public KeyCode keyCode
		{
			get
			{
				return Keyboard.EwBIUZKiwNBGxivIziALtbknsGpgA(_keyboardKeyCode);
			}
			set
			{
				keyboardKeyCode = Keyboard.GqKWxMODjRKwgzCxHKHcWfJaWFzv(value);
			}
		}
		...
		[CustomObfuscation(rename = false)]
		internal double modifiedTime => umNQvPJkSzKRFKteZPGBqzrZyzMD;
		...
		internal bool isModified
		{
			get
			{
				return umNQvPJkSzKRFKteZPGBqzrZyzMD > 0.0;
			}
			set
			{
				if (value)
				{
					umNQvPJkSzKRFKteZPGBqzrZyzMD = ReInput.realTime;
				}
				else
				{
					umNQvPJkSzKRFKteZPGBqzrZyzMD = 0.0;
				}
			}
		}

		public ControllerMap controllerMap => SJKUZyLtwpanDYOUUcKgIbLBAesr;

		public bool enabled
		{
			get
			{
				return hrXjVMVBGWHRhCIrzlnSmtoGojQeb;
			}
			set
			{
				hrXjVMVBGWHRhCIrzlnSmtoGojQeb = value;
				if (UbyyzSRixoeEDbIobbKqYCATBALV != null)
				{
					UbyyzSRixoeEDbIobbKqYCATBALV.hrXjVMVBGWHRhCIrzlnSmtoGojQeb = value;
				}
				if (phiHNWdAPICbVAPvgALmFIDiVXiIc != null)
				{
					phiHNWdAPICbVAPvgALmFIDiVXiIc.hrXjVMVBGWHRhCIrzlnSmtoGojQeb = value;
				}
			}
		}
		...
		public string elementIdentifierName
		{
			get
			{
				if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType == ControllerType.Keyboard)
				{
					return DmDFssSsGhOlQrjYEXnVNGGcOCZp();
				}
				HardwareControllerMap_Game hardwareControllerMap_Game = ((SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controller != null) ? SJKUZyLtwpanDYOUUcKgIbLBAesr.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : biKWDvLXXzgXYvNdGjCLoXodFzqC);
		...
		public string actionDescriptiveName
		{
			get
			{
				InputAction inputAction = ReInput.puvsCcoEkpSrGAnbdVqxjXrCengH.sAOuuCAMJXQrJLEfgeCrKQxwHbAU(_actionId);
				if (inputAction == null)
				{
					return string.Empty;
				}
		...
		public int elementIndex => fpLTJzOTpoUWkyThKhrqRzXDquMW;

		public int id => gjHUlVyQSQsjZEOHtHfmeehEQpiIA;
		...
		private bool fPkXbfZgpTWpCykySbRGKUgIFsBh
		{
			get
			{
				if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null)
				{
					return SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType == ControllerType.Keyboard;
				}
				return false;
			}
		}

		private static int qwNeJZArHpWwPBSmCEMzBjxlAQvCb
		{
			get
			{
		...
		internal static bool HemHiKuzUFnJbWaULfzZroJDbkNZ(ActionElementMap P_0, out ActionElementMap P_1, out ActionElementMap P_2)
		{
			...
			bool flag = false;
			P_2 = P_0.phiHNWdAPICbVAPvgALmFIDiVXiIc;
			if (P_2 == null)
			{
				P_2 = new ActionElementMap(P_0);
				P_0.phiHNWdAPICbVAPvgALmFIDiVXiIc = P_2;
				flag = true;
			}
			if (P_2.IUYJUSJfoWaJqVrBeCdDKuyGkWerA != P_0.IUYJUSJfoWaJqVrBeCdDKuyGkWerA)
			{
				viNjfTNevQcmMdfLgbNtOsfJfQSRA(P_0, P_2);
				flag = true;
			}
			if (flag)
			{
				P_2._axisRange = AxisRange.Positive;
				P_2._axisContribution = (P_0._invert ? Pole.Negative : Pole.Positive);
				P_2._invert = false;
				if (ReInput.isReady)
				{
					P_2.bficrGUDNywRTpHoFGmJjjCndKvX();
				}
				P_2.IUYJUSJfoWaJqVrBeCdDKuyGkWerA = P_0.IUYJUSJfoWaJqVrBeCdDKuyGkWerA;
			}
			flag = false;
			P_1 = P_0.UbyyzSRixoeEDbIobbKqYCATBALV;
			if (P_1 == null)
			{
				P_1 = new ActionElementMap(P_0);
				P_0.UbyyzSRixoeEDbIobbKqYCATBALV = P_1;
				flag = true;
			}
			if (P_1.IUYJUSJfoWaJqVrBeCdDKuyGkWerA != P_0.IUYJUSJfoWaJqVrBeCdDKuyGkWerA)
			{
				viNjfTNevQcmMdfLgbNtOsfJfQSRA(P_0, P_1);
				flag = true;
			}
			if (flag)
			{
				P_1._axisRange = AxisRange.Negative;
				P_1._axisContribution = ((!P_0._invert) ? Pole.Negative : Pole.Positive);
				P_1._invert = false;
				if (ReInput.isReady)
				{
					P_1.bficrGUDNywRTpHoFGmJjjCndKvX();
				}
				P_1.IUYJUSJfoWaJqVrBeCdDKuyGkWerA = P_0.IUYJUSJfoWaJqVrBeCdDKuyGkWerA;
			}
			return true;
		}

		internal static bool aODDhkZGfEYPsGiSODPHcMQUMixpA(ActionElementMap P_0)
		{
			if (P_0 == null)
			{
				return false;
			}
			if (P_0._actionId != -1 && !ReInput.puvsCcoEkpSrGAnbdVqxjXrCengH.SbvIUabXsNXWvItKnYCZlGRehkIm(P_0._actionId))
			{
				return false;
			}
			return true;
		...
		internal static void viNjfTNevQcmMdfLgbNtOsfJfQSRA(ActionElementMap P_0, ActionElementMap P_1)
		{
			if (P_0 == null)
			{
			...
			P_1.SJKUZyLtwpanDYOUUcKgIbLBAesr = P_0.SJKUZyLtwpanDYOUUcKgIbLBAesr;
			P_1.fpLTJzOTpoUWkyThKhrqRzXDquMW = P_0.fpLTJzOTpoUWkyThKhrqRzXDquMW;
			P_1.hrXjVMVBGWHRhCIrzlnSmtoGojQeb = P_0.hrXjVMVBGWHRhCIrzlnSmtoGojQeb;
			P_1.umNQvPJkSzKRFKteZPGBqzrZyzMD = P_0.umNQvPJkSzKRFKteZPGBqzrZyzMD;
			P_1.HPEAEdunYkaVtxWMDJTEzmEMdMAI = P_0.HPEAEdunYkaVtxWMDJTEzmEMdMAI;
			P_1.uEsWzOEQHcFJNJegZAXGWknFagQs = P_0.uEsWzOEQHcFJNJegZAXGWknFagQs;
			P_1.zcACNMBTRvXOAVXoLfuIHiCUUHIkA = P_0.zcACNMBTRvXOAVXoLfuIHiCUUHIkA;
			P_1.pBwnLDEgxUlyylxGTBtmDPkOjnhaA = P_0.pBwnLDEgxUlyylxGTBtmDPkOjnhaA;
			P_1.biKWDvLXXzgXYvNdGjCLoXodFzqC = P_0.biKWDvLXXzgXYvNdGjCLoXodFzqC;
			P_1.IUYJUSJfoWaJqVrBeCdDKuyGkWerA++;
		}
		...
		public static bool TryGetCombinedElementIdentifierName(IList<ActionElementMap> actionElementMaps, out string result)
		{
			...
			for (int i = 0; i < count; i++)
			{
				...
				HardwareControllerMap_Game hardwareControllerMap_Game2 = ((actionElementMap.SJKUZyLtwpanDYOUUcKgIbLBAesr != null && actionElementMap.SJKUZyLtwpanDYOUUcKgIbLBAesr.controller != null) ? actionElementMap.SJKUZyLtwpanDYOUUcKgIbLBAesr.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : actionElementMap.biKWDvLXXzgXYvNdGjCLoXodFzqC);
				if (hardwareControllerMap_Game != null)
				{
			...
			if (hardwareControllerMap_Game.TryGetCompoundElementMemberCombinedLocalizedName(actionElementMaps, out result))
			{
				return true;
			}
			result = null;
		...
		{
			string text;
			return hSLYfclZsMUQEYTPUSoRIQsmXcTh(actionElementMaps, true, false, out result, out text);
		}
		...
		{
			object obj;
			return hSLYfclZsMUQEYTPUSoRIQsmXcTh(actionElementMaps, false, true, out obj, out result);
		}

		private static bool hSLYfclZsMUQEYTPUSoRIQsmXcTh(IList<ActionElementMap> P_0, bool P_1, bool P_2, out object P_3, out string P_4)
		{
			int count;
			...
			if (hardwareControllerMap_Game.TryGetCompoundElementMemberCombinedGlyph(P_0, P_1, P_2, out P_3, out P_4))
			{
				return true;
			}
			P_3 = null;
		...
		{
			gjHUlVyQSQsjZEOHtHfmeehEQpiIA = qwNeJZArHpWwPBSmCEMzBjxlAQvCb;
			_actionId = -1;
			_elementIdentifierId = -1;
			hrXjVMVBGWHRhCIrzlnSmtoGojQeb = true;
		}

		public ActionElementMap(ActionElementMap P_0)
			: this()
		{
			viNjfTNevQcmMdfLgbNtOsfJfQSRA(P_0, this);
		}
			...
			: this()
		{
			...
			_modifierKey2 = P_5;
			_modifierKey3 = P_6;
			BgSEVLGhkSPAXKeJPJNvFxQFgdzBA();
		}

		public bool CheckForAssignmentConflict(ElementAssignment elementAssignment)
		{
			if (!yJUForcLyINDJeuWAKxpJQnOATTsB(elementAssignment.type))
			{
				return false;
			}
			if (fPkXbfZgpTWpCykySbRGKUgIFsBh || _keyboardKeyCode != KeyboardKeyCode.None)
			{
				KeyCode keyCode = elementAssignment.keyboardKey;
				if (keyCode == KeyCode.None)
				{
					keyCode = ReInput.AtHYwRgWVYrmVOsWolCxiSLKHuEp.IeYgCxBcbnFZhKaxGJMqKHnEVRHi.GetKeyCodeById(elementAssignment.elementIdentifierId);
				}
				return UrvRaRneEosIPxjmIMuMkOzCNqFX(Keyboard.GqKWxMODjRKwgzCxHKHcWfJaWFzv(keyCode), elementAssignment.modifierKeyFlags);
			}
			return uxUpMeGhrwqLgcvPymqvAVkUcDAt(elementAssignment.elementIdentifierId, elementAssignment.axisRange);
		}
		...
		public bool CheckForAssignmentConflict(ActionElementMap elementMap)
		{
			...
			if (fPkXbfZgpTWpCykySbRGKUgIFsBh || _keyboardKeyCode != KeyboardKeyCode.None)
			{
				return UrvRaRneEosIPxjmIMuMkOzCNqFX(elementMap._keyboardKeyCode, elementMap.modifierKeyFlags);
			}
			return uxUpMeGhrwqLgcvPymqvAVkUcDAt(elementMap._elementIdentifierId, elementMap._axisRange);
		}

		public bool ShowInField(AxisRange fieldActionRange)
		{
			if (!ReInput.puvsCcoEkpSrGAnbdVqxjXrCengH.SbvIUabXsNXWvItKnYCZlGRehkIm(_actionId))
			{
				return false;
			}
			if (fieldActionRange == AxisRange.Full)
			{
				...
				else if (_elementType == ControllerElementType.Button && ReInput.puvsCcoEkpSrGAnbdVqxjXrCengH.sAOuuCAMJXQrJLEfgeCrKQxwHbAU(_actionId).type == InputActionType.Axis)
				{
					return false;
				}
			}
			else
			{
				...
				if (ReInput.puvsCcoEkpSrGAnbdVqxjXrCengH.sAOuuCAMJXQrJLEfgeCrKQxwHbAU(_actionId).type == InputActionType.Axis)
				{
		...
		public bool IsTarget(ControllerElementTarget elementTarget)
		{
			...
			bool result = IsTarget(xuJpBJvxrqVOEMAPQQDPCLYEUJbk);
		...
		public bool IsTarget(IControllerElementTarget elementTarget)
		{
			...
			if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null)
			{
				...
				if (controller.id != SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerId || controller.type != SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType)
				{
					return false;
				}
			}
		...
		public int GetElementIdentifierGlyphs(ICollection<object> results)
		{
			...
			if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType == ControllerType.Keyboard)
			{
				return QXxXieXYxzyNcaaLgysplXciRxCf(results);
			}
			HardwareControllerMap_Game hardwareControllerMap_Game = ((SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controller != null) ? SJKUZyLtwpanDYOUUcKgIbLBAesr.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : biKWDvLXXzgXYvNdGjCLoXodFzqC);
			if (hardwareControllerMap_Game == null)
			{
		...
		public int GetElementIdentifierFinalGlyphKeys(ICollection<string> results)
		{
			...
			if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType == ControllerType.Keyboard)
			{
				return xZsHzludVLuacPFdcojUQZLWdNJR(results);
			}
			HardwareControllerMap_Game hardwareControllerMap_Game = ((SJKUZyLtwpanDYOUUcKgIbLBAesr != null && SJKUZyLtwpanDYOUUcKgIbLBAesr.controller != null) ? SJKUZyLtwpanDYOUUcKgIbLBAesr.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : biKWDvLXXzgXYvNdGjCLoXodFzqC);
			if (hardwareControllerMap_Game == null)
			{
		...
		internal void FNqTNkOozAgwnWePEBwoFWAPyUfy(ControllerMap P_0)
		{
			SJKUZyLtwpanDYOUUcKgIbLBAesr = P_0;
			ControllerType controllerType = P_0.controllerType;
			HardwareControllerMap_Game hardwareControllerMap_Game = ((P_0.controller != null) ? P_0.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : null);
			DwwbinQpnZfiyHpYDQKzdXpWlSBRA(controllerType, hardwareControllerMap_Game, controllerType == ControllerType.Keyboard && _elementIdentifierId <= 0);
		}

		internal void taGTQuetFzGXqFgVBSxSviRmeDYP(ControllerMap P_0, HardwareControllerMap_Game P_1)
		{
			SJKUZyLtwpanDYOUUcKgIbLBAesr = P_0;
			biKWDvLXXzgXYvNdGjCLoXodFzqC = P_1;
			DwwbinQpnZfiyHpYDQKzdXpWlSBRA(P_0.controllerType, P_1, P_0.controllerType == ControllerType.Keyboard && _elementIdentifierId <= 0);
		}

		private void pandLAHLrwAbBavtribqUEfJqRJFA(bool P_0)
		{
			if (SJKUZyLtwpanDYOUUcKgIbLBAesr != null)
			{
				DwwbinQpnZfiyHpYDQKzdXpWlSBRA(SJKUZyLtwpanDYOUUcKgIbLBAesr.controllerType, (SJKUZyLtwpanDYOUUcKgIbLBAesr.controller != null) ? SJKUZyLtwpanDYOUUcKgIbLBAesr.controller.JEexZOPzSUUjNTHjvxywblgJdFqE : null, P_0);
			}
		}

		private void DwwbinQpnZfiyHpYDQKzdXpWlSBRA(ControllerType P_0, HardwareControllerMap_Game P_1, bool P_2)
		{
			if (SJKUZyLtwpanDYOUUcKgIbLBAesr == null)
			{
				return;
			...
			if (P_0 == ControllerType.Keyboard)
			{
				...
				if (P_2)
				{
					fpLTJzOTpoUWkyThKhrqRzXDquMW = keyboard.GetButtonIndex(_keyboardKeyCode);
					BgSEVLGhkSPAXKeJPJNvFxQFgdzBA();
				}
				else
				{
					fpLTJzOTpoUWkyThKhrqRzXDquMW = keyboard.GetButtonIndexById(_elementIdentifierId);
					vOQZjGcpyXDWyYIsBJDjaYARMxPK();
				}
			}
			else
			{
				...
				{
				case ControllerElementType.Axis:
					fpLTJzOTpoUWkyThKhrqRzXDquMW = P_1.GetAxisIndex(_elementIdentifierId);
					break;
				case ControllerElementType.Button:
					fpLTJzOTpoUWkyThKhrqRzXDquMW = P_1.GetButtonIndex(_elementIdentifierId);
					break;
				default:
					throw new NotImplementedException();
				}
			}
			IUYJUSJfoWaJqVrBeCdDKuyGkWerA++;
		}

		private string DmDFssSsGhOlQrjYEXnVNGGcOCZp()
		{
			string text = Keyboard.GetKeyName((KeyCode)_keyboardKeyCode);
			if (string.Equals(text, uEsWzOEQHcFJNJegZAXGWknFagQs, StringComparison.Ordinal) && pBwnLDEgxUlyylxGTBtmDPkOjnhaA == modifierKeyFlags && (!LocalizationManager.isEnabled || HPEAEdunYkaVtxWMDJTEzmEMdMAI == LocalizationManager.version))
			{
				return zcACNMBTRvXOAVXoLfuIHiCUUHIkA;
			}
			uEsWzOEQHcFJNJegZAXGWknFagQs = text;
			pBwnLDEgxUlyylxGTBtmDPkOjnhaA = modifierKeyFlags;
			if (LocalizationManager.isEnabled)
			{
				HPEAEdunYkaVtxWMDJTEzmEMdMAI = LocalizationManager.version;
			}
			...
			zcACNMBTRvXOAVXoLfuIHiCUUHIkA = text;
			return zcACNMBTRvXOAVXoLfuIHiCUUHIkA;
		}

		private int QXxXieXYxzyNcaaLgysplXciRxCf(ICollection<object> P_0)
		{
		...
		private int xZsHzludVLuacPFdcojUQZLWdNJR(ICollection<string> P_0)
		{
			...
			using (TempListPool.TList<string> tList = TempListPool.GetTList<string>())
			{
				...
				if (_modifierKey1 != ModifierKey.None)
				{
					string text = Keyboard.CcxPLVAHiJMJMMsVPTTXURhmxvzL(_modifierKey1);
					if (text == null)
					{
						return 0;
					}
				...
				if (_modifierKey2 != ModifierKey.None)
				{
					string text2 = Keyboard.CcxPLVAHiJMJMMsVPTTXURhmxvzL(_modifierKey2);
					if (text2 == null)
					{
						return 0;
					}
				...
				if (_modifierKey3 != ModifierKey.None)
				{
					string text3 = Keyboard.CcxPLVAHiJMJMMsVPTTXURhmxvzL(_modifierKey3);
					if (text3 == null)
					{
						return 0;
					}
		...
		internal void crLlVTiUiqPOhNdOCmHobhupgSse()
		{
			_actionCategoryId = -1;
			_actionId = -1;
			...
			SJKUZyLtwpanDYOUUcKgIbLBAesr = null;
			hrXjVMVBGWHRhCIrzlnSmtoGojQeb = true;
			uEsWzOEQHcFJNJegZAXGWknFagQs = null;
			zcACNMBTRvXOAVXoLfuIHiCUUHIkA = null;
			HPEAEdunYkaVtxWMDJTEzmEMdMAI = 0u;
			pBwnLDEgxUlyylxGTBtmDPkOjnhaA = ModifierKeyFlags.None;
			fpLTJzOTpoUWkyThKhrqRzXDquMW = -1;
			IUYJUSJfoWaJqVrBeCdDKuyGkWerA++;
		}

		private bool UrvRaRneEosIPxjmIMuMkOzCNqFX(KeyboardKeyCode P_0, ModifierKeyFlags P_1)
		{
			if (_elementType != ControllerElementType.Button)
			{
				return false;
			...
			if (Keyboard.DTbCltXVNympCJCVOWWXkHHIaIpW(modifierKeyFlags) != Keyboard.DTbCltXVNympCJCVOWWXkHHIaIpW(P_1))
			{
				return false;
			}
			return true;
		...
		private bool uxUpMeGhrwqLgcvPymqvAVkUcDAt(int P_0, AxisRange P_1)
		{
		...
		private bool yJUForcLyINDJeuWAKxpJQnOATTsB(ElementAssignmentType P_0)
		{
			if (_elementType == ControllerElementType.Button)
			{
		...
		private void BgSEVLGhkSPAXKeJPJNvFxQFgdzBA()
		{
			_elementIdentifierId = Keyboard.DEhTpUaGcEJooYCldgqNbQDDLkKHA(_keyboardKeyCode);
		}

		private void vOQZjGcpyXDWyYIsBJDjaYARMxPK()
		{
			...
			{
				_keyboardKeyCode = Keyboard.GqKWxMODjRKwgzCxHKHcWfJaWFzv(ReInput.AtHYwRgWVYrmVOsWolCxiSLKHuEp.IeYgCxBcbnFZhKaxGJMqKHnEVRHi.GetKeyCodeById(_elementIdentifierId));
			}
		}

		private void dUeiekJIxWyoUgknKgLnyMVZDiFc()
		{
			if (_modifierKey1 != ModifierKey.None)
			{
		...
		internal void bficrGUDNywRTpHoFGmJjjCndKvX()
		{
			if (!ControllerMap.bXuCYzgaSWwQwnpVFQfBnyFBhfFj)
			{
				umNQvPJkSzKRFKteZPGBqzrZyzMD = ReInput.realTime;
				IUYJUSJfoWaJqVrBeCdDKuyGkWerA++;
			}
		...
		internal SerializedObject mPXookHxaeOSKUADmcsOiVKFOQqi()
		{
			return new SerializedObject(GetType(), SerializedObject.ObjectType.Object)
			{
				...
				{ "enabled", hrXjVMVBGWHRhCIrzlnSmtoGojQeb }
			};
		}

		internal void qbSInOBzRpLbKGdknAatzCqAWxtMA(SerializedObject P_0)
		{
			_actionCategoryId = -1;
			_actionId = -1;
			_elementIdentifierId = -1;
			...
			P_0.TryGetDeserializedValueByRef("enabled", ref hrXjVMVBGWHRhCIrzlnSmtoGojQeb);
			bficrGUDNywRTpHoFGmJjjCndKvX();
		}
		...
		public override string ToString()
		{
			...
			StringTools.WriteVar(s_toStringSB, "Id", gjHUlVyQSQsjZEOHtHfmeehEQpiIA);
			StringTools.WriteVar(s_toStringSB, "Enabled", hrXjVMVBGWHRhCIrzlnSmtoGojQeb);
			StringTools.WriteVar(s_toStringSB, "Controller Map Id", (SJKUZyLtwpanDYOUUcKgIbLBAesr != null) ? SJKUZyLtwpanDYOUUcKgIbLBAesr.id : (-1));
			...
			StringTools.WriteVar(s_toStringSB, "Element Index", fpLTJzOTpoUWkyThKhrqRzXDquMW);
