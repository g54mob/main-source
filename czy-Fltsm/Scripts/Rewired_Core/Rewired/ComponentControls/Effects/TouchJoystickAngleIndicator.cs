...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchJoystickAngleIndicator : MonoBehaviour, IVisibilityChangedHandler, TouchJoystick.IStickPositionChangedHandler
	{
		...
		private Image vrIguaamdQSFDOtdwRkpsHxYYJZrA;

		private RectTransform wYolWdEwMRVxglEXYCsFGDWZvxCTA;

		private Vector2 wFFLoNBqinCOtEDmHatogrOIxzam;

		private bool DCnbexFzJBAIqXRMyvtvTeFHJxVbb;

		private IRegistrar<TouchJoystickAngleIndicator> mKueajkRvmgzNhYyjCMHXBMCMvuGb;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					OcuoNoxfPcdBhxXreJuxVMSsQhUU(value, false);
					CvznDnAStbAXrIwqfnaswYCIbjIi();
				}
			}
		}
		...
		internal Image TmJnNDkcrgpVYKjCcFdzofZFZYQg => vrIguaamdQSFDOtdwRkpsHxYYJZrA ?? (vrIguaamdQSFDOtdwRkpsHxYYJZrA = GetComponent<Image>());

		internal Sprite BqtfodMOzufadgDtOokwsrJYTgBPA
		{
			get
			{
				if (TmJnNDkcrgpVYKjCcFdzofZFZYQg == null)
				{
					return null;
				}
				if (vrIguaamdQSFDOtdwRkpsHxYYJZrA.overrideSprite != null)
				{
					return vrIguaamdQSFDOtdwRkpsHxYYJZrA.overrideSprite;
				}
				return vrIguaamdQSFDOtdwRkpsHxYYJZrA.sprite;
			}
		}

		internal RectTransform puDBkICGthUbpacgqyTIWbLVlIav => wYolWdEwMRVxglEXYCsFGDWZvxCTA ?? (wYolWdEwMRVxglEXYCsFGDWZvxCTA = GetComponent<RectTransform>());
		...
		internal bool SaKBteOkXcRboFtoZKgFNlgyPzCl(out Vector2 P_0)
		{
			P_0 = Vector2.zero;
			if (TmJnNDkcrgpVYKjCcFdzofZFZYQg == null)
			{
			...
			Sprite sprite = vrIguaamdQSFDOtdwRkpsHxYYJZrA.overrideSprite ?? vrIguaamdQSFDOtdwRkpsHxYYJZrA.sprite;
			if (sprite == null)
			{
				return false;
			}
		...
		private void OnEnable()
		{
			...
			{
				lksEmGruBwXaLKKytFBHwoSpMiwi();
				PEdtocOxtLJHZXQGMzxxZfXvEUKL();
			}
			TWPrtynMkrXubmJwhZCBpHYDadCK(wFFLoNBqinCOtEDmHatogrOIxzam);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			sZwEyeRALPEHtCWnkKCjTeldQAfA();
		}

		[CustomObfuscation(rename = false)]
		private void OnValidate()
		{
			awGkyTTpDzuHuvEwVCgsrukUpMIN();
			TWPrtynMkrXubmJwhZCBpHYDadCK(wFFLoNBqinCOtEDmHatogrOIxzam);
		}
		...
		private void OcuoNoxfPcdBhxXreJuxVMSsQhUU(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
				if (!P_0)
				{
					...
					targetColor.a = 0f;
					TmJnNDkcrgpVYKjCcFdzofZFZYQg.CrossFadeColor(targetColor, 0f, ignoreTimeScale: true, useAlpha: true);
				}
				else
				{
					TWPrtynMkrXubmJwhZCBpHYDadCK(wFFLoNBqinCOtEDmHatogrOIxzam);
		...
		private void TWPrtynMkrXubmJwhZCBpHYDadCK(Vector2 P_0)
		{
			if (!_visible)
			{
				Color targetColor = _normalColor;
			...
			else if (!MathTools.ApproximatelyZero(P_0.sqrMagnitude))
			{
				...
				TmJnNDkcrgpVYKjCcFdzofZFZYQg.CrossFadeColor(targetColor2, 0f, ignoreTimeScale: true, useAlpha: true);
			}
			else
			{
				TmJnNDkcrgpVYKjCcFdzofZFZYQg.CrossFadeColor(_normalColor, 0f, ignoreTimeScale: true, useAlpha: true);
			}
		}

		private void lksEmGruBwXaLKKytFBHwoSpMiwi()
		{
			DCnbexFzJBAIqXRMyvtvTeFHJxVbb = _visible;
		}

		private void awGkyTTpDzuHuvEwVCgsrukUpMIN()
		{
			if (DCnbexFzJBAIqXRMyvtvTeFHJxVbb != _visible)
			{
				DCnbexFzJBAIqXRMyvtvTeFHJxVbb = _visible;
				OcuoNoxfPcdBhxXreJuxVMSsQhUU(_visible, true);
			}
		}

		private void CvznDnAStbAXrIwqfnaswYCIbjIi()
		{
		}

		private void PEdtocOxtLJHZXQGMzxxZfXvEUKL()
		{
			sZwEyeRALPEHtCWnkKCjTeldQAfA();
			...
			if (!componentInSelfOrParents.IsNullOrDestroyed())
			{
				...
				mKueajkRvmgzNhYyjCMHXBMCMvuGb = componentInSelfOrParents;
			}
		}

		private void sZwEyeRALPEHtCWnkKCjTeldQAfA()
		{
			if (mKueajkRvmgzNhYyjCMHXBMCMvuGb.IsNullOrDestroyed())
			{
				if (mKueajkRvmgzNhYyjCMHXBMCMvuGb != null)
				{
					mKueajkRvmgzNhYyjCMHXBMCMvuGb = null;
				}
			}
			else
			{
				mKueajkRvmgzNhYyjCMHXBMCMvuGb.Deregister(this);
				mKueajkRvmgzNhYyjCMHXBMCMvuGb = null;
			}
		}
		...
		{
			OcuoNoxfPcdBhxXreJuxVMSsQhUU(state, false);
		}
		...
		public void OnTouchJoystickStickPositionChanged(Vector2 value)
		{
			if (!(this == null))
			{
				wFFLoNBqinCOtEDmHatogrOIxzam = value;
				if (UnityTools.IsActiveAndEnabled(this) && _visible)
				{
					TWPrtynMkrXubmJwhZCBpHYDadCK(value);
				}
			}
		}
