...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchInteractableTransitioner : MonoBehaviour, IVisibilityChangedHandler, TouchInteractable.IInteractionStateTransitionHandler
	{
		...
		private TouchInteractable.InteractionState IcFvovbrsZonYYhYNkLZZniIhpIcA;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					hsqCvgUcKMQSZSjfeEzTAkGQEYJ(value, false);
					dxqJuzEVmOHWgrjKSDQyCoaqnhMA();
				}
			}
		}
		...
		private void Awake()
		{
			if (Application.isPlaying)
			{
				...
				hsqCvgUcKMQSZSjfeEzTAkGQEYJ(_visible, true);
			}
		}
		...
		private void OnEnable()
		{
			...
			EsQsUVLNRtmtICnIgMMbusxUZAXM(true);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			wbKvPBVqNGOoRnpmnwmwEwnwhQrf();
		}
		...
		private void OnValidate()
		{
			...
			{
				IoKbxtAfHnSSdCSqGgfeIutCJFFOB(null);
				zuWWJGAPJPKdPYgWBUCMauVVpSwR(Color.white, true);
				zyMcbgiEFensKMylyaCtztdSLIAwA(_transitionAnimationTriggers.normalTrigger);
				EsQsUVLNRtmtICnIgMMbusxUZAXM(true);
			}
			TnTZgxLbJRPRmBGSyfzCnSkSoBWl();
		}
		...
		private void dxqJuzEVmOHWgrjKSDQyCoaqnhMA()
		{
			TnTZgxLbJRPRmBGSyfzCnSkSoBWl();
		}

		private void TnTZgxLbJRPRmBGSyfzCnSkSoBWl()
		{
			if (!Application.isPlaying)
			{
				EsQsUVLNRtmtICnIgMMbusxUZAXM(true);
			...
			{
				EsQsUVLNRtmtICnIgMMbusxUZAXM(false);
			}
		}

		private void EsQsUVLNRtmtICnIgMMbusxUZAXM(bool P_0)
		{
			JXeaJzXCJATrvUFSLboTtxOYSbBT(IcFvovbrsZonYYhYNkLZZniIhpIcA, P_0);
		}

		private void hsqCvgUcKMQSZSjfeEzTAkGQEYJ(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
		...
		private bool fJuSznrvVbqAtKPAwEDZMDAbhktl()
		{
			return UnityTools.IsActiveAndEnabled(this);
		}

		private void wbKvPBVqNGOoRnpmnwmwEwnwhQrf()
		{
			...
			{
				zyMcbgiEFensKMylyaCtztdSLIAwA(normalTrigger);
			}
		}

		private void JXeaJzXCJATrvUFSLboTtxOYSbBT(TouchInteractable.InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					zuWWJGAPJPKdPYgWBUCMauVVpSwR(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					zuWWJGAPJPKdPYgWBUCMauVVpSwR(color, P_1);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.SpriteSwap) != TouchInteractable.TransitionTypeFlags.None)
				{
					IoKbxtAfHnSSdCSqGgfeIutCJFFOB(sprite);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.Animation) != TouchInteractable.TransitionTypeFlags.None)
				{
					zyMcbgiEFensKMylyaCtztdSLIAwA(text);
				}
			}
		}

		private void zuWWJGAPJPKdPYgWBUCMauVVpSwR(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void IoKbxtAfHnSSdCSqGgfeIutCJFFOB(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void zyMcbgiEFensKMylyaCtztdSLIAwA(string P_0)
		{
		...
		{
			IcFvovbrsZonYYhYNkLZZniIhpIcA = args.state;
