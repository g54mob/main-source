	...
	public sealed class TiltControl : CustomControllerControl
	{
		...
		public TiltDirection axesToUse
		{
			...
			set
			{
				...
				{
					dryXkIbMBVhgSqOARFaaIHyyDBNrA(value);
		...
		internal StandaloneAxis2D nGVUnSVpaWEharcXknrLRwnclnOn => _axis2D;

		private Vector3 nuhJEECzetbVTegGsESnJKRXtVHLA
		{
			get
			{
		...
		{
			Vector3 vector = nuhJEECzetbVTegGsESnJKRXtVHLA;
		...
		{
			base.Awake();
			if (Application.isPlaying && _axis2D != null)
			{
		...
		internal override void OnValidate()
		{
			...
			{
				pndjNJgebabCdDATJOOpDlxPynpd();
			}
		}

		internal bool XmCdCRoGFuacfjfYUbYwcgTHAUCXA()
		{
		...
		internal void tkjfmIAYFqlnJhIXUhKPBrYHfpbkA()
		{
			base.ZCxYpOKPlUdrVINhgqDHNCUEVWof();
			if (base.mGSVjCDCJJtDWhXvCjLMSqYiVZpn)
			{
				KogfxEiXbgHNtujjnKYMXilgJcJS();
			}
		}

		internal void ZBXyEfksbeDAyAxIyamOFkgqfpAj()
		{
			if (base.mGSVjCDCJJtDWhXvCjLMSqYiVZpn && lgrxeUlsSPQSCicUhAbuoUnLaBDCA)
			{
				if (_useFAxis)
				{
					RhRZaqQiFdWPJAfrENbBHnpVmOZu(_forwardTiltCustomControllerElement, _axis2D.yAxis.value, _axis2D.yAxis.buttonActivationThreshold);
				}
				if (_useHAxis)
				{
					RhRZaqQiFdWPJAfrENbBHnpVmOZu(_horizontalTiltCustomControllerElement, _axis2D.xAxis.value, _axis2D.xAxis.buttonActivationThreshold);
				}
			}
		}
		...
		public override void ClearValue()
		{
			...
			{
				base.BrLhsTCJDDhEOtjjAgHezhkdDyDBA.ClearElementValue(_horizontalTiltCustomControllerElement);
				base.BrLhsTCJDDhEOtjjAgHezhkdDyDBA.ClearElementValue(_forwardTiltCustomControllerElement);
			}
		}

		private void KogfxEiXbgHNtujjnKYMXilgJcJS()
		{
			if (_useHAxis)
			{
				float rawValue;
				if (nuhJEECzetbVTegGsESnJKRXtVHLA == Vector3.zero)
				{
					rawValue = 0f;
				}
				else
				{
					float value = Mathf.Atan2(nuhJEECzetbVTegGsESnJKRXtVHLA.x, 0f - nuhJEECzetbVTegGsESnJKRXtVHLA.y) * 57.29578f + _horizontalRestAngle;
			...
			if (_useFAxis)
			{
				...
				{
					float value2 = Mathf.Atan2(nuhJEECzetbVTegGsESnJKRXtVHLA.z, 0f - nuhJEECzetbVTegGsESnJKRXtVHLA.y) * 57.29578f + _forwardRestAngle;
		...
		private void pndjNJgebabCdDATJOOpDlxPynpd()
		{
			dryXkIbMBVhgSqOARFaaIHyyDBNrA(_allowedTiltDirections);
			if (lgrxeUlsSPQSCicUhAbuoUnLaBDCA)
			{
				if (_useHAxis)
				{
					base.BrLhsTCJDDhEOtjjAgHezhkdDyDBA.ValidateElements(_horizontalTiltCustomControllerElement);
				}
				if (_useFAxis)
				{
					base.BrLhsTCJDDhEOtjjAgHezhkdDyDBA.ValidateElements(_forwardTiltCustomControllerElement);
				}
			}
		}

		private void dryXkIbMBVhgSqOARFaaIHyyDBNrA(TiltDirection P_0)
		{
