...
namespace Rewired.Utils
{
	...
	{
		internal struct RNjjjtClhSaKBnMSFTjnCxDgvEBU
		{
			public Platform sLNuTTBMqzIYcbXWBGXIAgXUxAEH;

			public Platform UVUFKPjROWIlXKcqdVuWUEerfUCz;

			public EditorPlatform IEaHCoQqzEQKqYiWTdDRfkNigTnO;

			public bool VMZvfwxmxbigCoXUUCvAIcrFqAOS;

			public WebplayerPlatform rvPqOIdVlZNkmPUcHfCrfnOBwGIL;

			public ScriptingBackend ZYKozmIRURJzRIbDLckuWjbXkMFK;

			public ScriptingAPILevel KXuqitBlRAINbiFIcQnqqTObEFkL;

			public IExternalTools NXLGPzmCGryKDdtRebdlXTsoWwFO;

			public RNjjjtClhSaKBnMSFTjnCxDgvEBU(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				sLNuTTBMqzIYcbXWBGXIAgXUxAEH = P_0;
				UVUFKPjROWIlXKcqdVuWUEerfUCz = P_1;
				IEaHCoQqzEQKqYiWTdDRfkNigTnO = P_2;
				VMZvfwxmxbigCoXUUCvAIcrFqAOS = P_3;
				rvPqOIdVlZNkmPUcHfCrfnOBwGIL = P_4;
				ZYKozmIRURJzRIbDLckuWjbXkMFK = P_5;
				KXuqitBlRAINbiFIcQnqqTObEFkL = P_6;
				NXLGPzmCGryKDdtRebdlXTsoWwFO = P_7;
			}
		}
		...
		{
			public enum xMLFNZGoiEOxKTDeWwNwjClUAhYCA
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly xMLFNZGoiEOxKTDeWwNwjClUAhYCA type;

			public readonly int build;

			public UnityVersionClass(string P_0)
			{
				type = xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Normal;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Beta;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Patch;
					}
			...
			public override string ToString()
			{
				...
				array[5] = fsEdsvdTndtFFNPbTwGOvnmsAmBO(type);
				num = build;
				array[6] = num.ToString();
				return string.Concat(array);
			}

			private string fsEdsvdTndtFFNPbTwGOvnmsAmBO(xMLFNZGoiEOxKTDeWwNwjClUAhYCA P_0)
			{
				return P_0 switch
				{
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Normal => "f", 
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Beta => "b", 
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Patch => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (krUZVOVYAhyXnwbAYGcWqWGibKMW(a.type) > krUZVOVYAhyXnwbAYGcWqWGibKMW(b.type))
				{
					return 1;
				}
				if (krUZVOVYAhyXnwbAYGcWqWGibKMW(a.type) < krUZVOVYAhyXnwbAYGcWqWGibKMW(b.type))
			...
			private static int krUZVOVYAhyXnwbAYGcWqWGibKMW(xMLFNZGoiEOxKTDeWwNwjClUAhYCA P_0)
			{
				return P_0 switch
				{
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Beta => 0, 
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Normal => 10, 
					xMLFNZGoiEOxKTDeWwNwjClUAhYCA.Patch => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion eVJRQzxHdNvzGUEupgNruCBvlwKD = UnityVersion.UNITY_5_0;

		private static UnityVersionClass ZMiwSpHbWZtagOyjiOgJiXbtGete;

		private static UnityVersion olnTLUXXHqNTHtweAipgdeoOEXeh = UnityVersion.Unknown;

		private static string aWrKmzAvWCTjVmuSOelFEgpcIHQK;

		private static Platform MeZkGconEOWOtuSlJBdxirfDIiwG;

		private static EditorPlatform YNWyxWyKsNFiNmgVFzNnYDPXJNwN;

		private static bool aWduCTYgMShjwYgLjeePGUPTyquD;

		private static bool HWKBToAijHPHcdbvilCqesLFQFFKB;

		private static bool rhzRygsTPblDxpbpImLYtScAFBcu;

		private static WebplayerPlatform hEbFqnRXfYEBtkkDDTtQXRNZgabd;

		private static bool MJUBUHAiNFryqbdmcxneCuSEcGlCb;

		private static bool HBgMYdqxdjDKTYrLyHUpyltUlPUH;

		private static bool vNAZRGUEcJqKwQXDIyFYGSNSymhE;

		private static bool KkumtxElYOBvKapyZQwEReYCcUamA;

		private static bool nvjIRLJbTGYczNmwdgMTCrwrZwsI;

		private static bool gPahlaBOJFpsDtAaOKDDiqGaytqlb;

		private static string xxaUaUnEkRkNEnQxNDhkZAaXXjvE;

		private static ScriptingBackend FJCvbycdXdayzEJJsYHNIxrPKuwJ;

		private static ScriptingAPILevel EuvLhLPiumHrJjbNGTOCvtsemQhQ;

		private static bool yKbEEpjOQKhoMrpyYLYPKrHMmnCkA;

		private static IExternalTools zqvcmWymJrjkrqKvDbDYYwnzYZGF;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper KZsSdpmbDYCNUEefZFVWSBYXCvegA;

		private static bool BUBpHIlphFOvqaLSrsmtvtLNEJeX;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				if (!MdSMqTwzGJSBIiaZlZhhAzmTSjRf)
				{
					return null;
				}
				return ZMiwSpHbWZtagOyjiOgJiXbtGete;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return aWrKmzAvWCTjVmuSOelFEgpcIHQK;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return MeZkGconEOWOtuSlJBdxirfDIiwG;
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return YNWyxWyKsNFiNmgVFzNnYDPXJNwN;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return aWduCTYgMShjwYgLjeePGUPTyquD;
			}
		}

		public static bool isPlaying => HWKBToAijHPHcdbvilCqesLFQFFKB;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return rhzRygsTPblDxpbpImLYtScAFBcu;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return hEbFqnRXfYEBtkkDDTtQXRNZgabd;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (aWduCTYgMShjwYgLjeePGUPTyquD || Application.isEditor)
				{
					return true;
				}
				...
				switch (MeZkGconEOWOtuSlJBdxirfDIiwG)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!rhzRygsTPblDxpbpImLYtScAFBcu)
					{
						return FJCvbycdXdayzEJJsYHNIxrPKuwJ == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return YNWyxWyKsNFiNmgVFzNnYDPXJNwN switch
				{
					EditorPlatform.Windows => MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.Windows, 
					EditorPlatform.OSX => MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.OSX, 
					EditorPlatform.Linux => MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return MJUBUHAiNFryqbdmcxneCuSEcGlCb;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return HBgMYdqxdjDKTYrLyHUpyltUlPUH;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool supportsPhysicalKeys
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_2021_2;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.Android && MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.Ouya && MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.AmazonFireTV)
				{
					return MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.iOS)
				{
					return MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.Windows && MeZkGconEOWOtuSlJBdxirfDIiwG != Platform.Linux)
				{
					return MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return vNAZRGUEcJqKwQXDIyFYGSNSymhE;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_0)
				{
					return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return yKbEEpjOQKhoMrpyYLYPKrHMmnCkA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => FJCvbycdXdayzEJJsYHNIxrPKuwJ;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => EuvLhLPiumHrJjbNGTOCvtsemQhQ;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return zqvcmWymJrjkrqKvDbDYYwnzYZGF;
			}
		}

		internal static IAndroidFallbackPlatformHelper qSLOmZsIlGbabZAzlCUuQvguDNzM
		{
			[CompilerGenerated]
			get
			{
				return KZsSdpmbDYCNUEefZFVWSBYXCvegA;
			}
			[CompilerGenerated]
			set
			{
				KZsSdpmbDYCNUEefZFVWSBYXCvegA = kZsSdpmbDYCNUEefZFVWSBYXCvegA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => BUBpHIlphFOvqaLSrsmtvtLNEJeX;

		private static bool MdSMqTwzGJSBIiaZlZhhAzmTSjRf => UXwSiFWuQEODBUmFaaerISTCBeICb();

		private static bool UXwSiFWuQEODBUmFaaerISTCBeICb()
		{
			if (BUBpHIlphFOvqaLSrsmtvtLNEJeX)
			{
				return true;
			}
			try
			{
				aWrKmzAvWCTjVmuSOelFEgpcIHQK = Application.unityVersion;
				ZMiwSpHbWZtagOyjiOgJiXbtGete = new UnityVersionClass(aWrKmzAvWCTjVmuSOelFEgpcIHQK);
				AtrcBphcsrpFebbagDkEaxcmlbpaA();
				BUBpHIlphFOvqaLSrsmtvtLNEJeX = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return BUBpHIlphFOvqaLSrsmtvtLNEJeX;
		}

		internal static void oTramjqKaPSYcnltKtIiVwtyniLT(RNjjjtClhSaKBnMSFTjnCxDgvEBU P_0)
		{
			if (MdSMqTwzGJSBIiaZlZhhAzmTSjRf)
			{
				if (P_0.UVUFKPjROWIlXKcqdVuWUEerfUCz == Platform.Windows81Store)
				{
					P_0.UVUFKPjROWIlXKcqdVuWUEerfUCz = Platform.WindowsUWP;
				}
				MeZkGconEOWOtuSlJBdxirfDIiwG = P_0.UVUFKPjROWIlXKcqdVuWUEerfUCz;
				YNWyxWyKsNFiNmgVFzNnYDPXJNwN = P_0.IEaHCoQqzEQKqYiWTdDRfkNigTnO;
				aWduCTYgMShjwYgLjeePGUPTyquD = P_0.VMZvfwxmxbigCoXUUCvAIcrFqAOS;
				hEbFqnRXfYEBtkkDDTtQXRNZgabd = P_0.rvPqOIdVlZNkmPUcHfCrfnOBwGIL;
				FJCvbycdXdayzEJJsYHNIxrPKuwJ = P_0.ZYKozmIRURJzRIbDLckuWjbXkMFK;
				EuvLhLPiumHrJjbNGTOCvtsemQhQ = P_0.KXuqitBlRAINbiFIcQnqqTObEFkL;
				if (zqvcmWymJrjkrqKvDbDYYwnzYZGF != null)
				{
					zqvcmWymJrjkrqKvDbDYYwnzYZGF.Destroy();
				}
				zqvcmWymJrjkrqKvDbDYYwnzYZGF = P_0.NXLGPzmCGryKDdtRebdlXTsoWwFO;
				rhzRygsTPblDxpbpImLYtScAFBcu = Debug.isDebugBuild;
				HWKBToAijHPHcdbvilCqesLFQFFKB = true;
				HiEwZoGePYEMZeYiDSCKdQwAkvQP();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			RsdbiqnwYzNofAkkdhYxpFlvnSBj(minVersionStr, out var num);
			RsdbiqnwYzNofAkkdhYxpFlvnSBj(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (num3 && ZMiwSpHbWZtagOyjiOgJiXbtGete < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag && ZMiwSpHbWZtagOyjiOgJiXbtGete >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag && ZMiwSpHbWZtagOyjiOgJiXbtGete > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool RsdbiqnwYzNofAkkdhYxpFlvnSBj(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void AtrcBphcsrpFebbagDkEaxcmlbpaA()
		{
			olnTLUXXHqNTHtweAipgdeoOEXeh = kkoUQXDLBnjIxJWyWosplMxWjJNLA(Application.unityVersion);
			if (olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_3_5 && olnTLUXXHqNTHtweAipgdeoOEXeh < UnityVersion.UNITY_4_0)
			{
				MJUBUHAiNFryqbdmcxneCuSEcGlCb = true;
			}
			else if (olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_0)
			{
				HBgMYdqxdjDKTYrLyHUpyltUlPUH = true;
			}
		}

		private static UnityVersion kkoUQXDLBnjIxJWyWosplMxWjJNLA(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion FdefChEcREhOYFmhPkKADwKBLIjZB(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion cazkmDDsMMWHliQZNEIGJOeITcgM(int P_0)
		{
			return P_0 switch
			{
		...
		private static void HiEwZoGePYEMZeYiDSCKdQwAkvQP()
		{
			switch (MeZkGconEOWOtuSlJBdxirfDIiwG)
			{
			...
			case Platform.RazerForgeTV:
				KkumtxElYOBvKapyZQwEReYCcUamA = true;
				nvjIRLJbTGYczNmwdgMTCrwrZwsI = true;
				break;
			case Platform.PS4:
				KkumtxElYOBvKapyZQwEReYCcUamA = true;
				xxaUaUnEkRkNEnQxNDhkZAaXXjvE = "Empty";
				gPahlaBOJFpsDtAaOKDDiqGaytqlb = true;
				break;
			case Platform.Windows:
				if ((olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_6_3p1 && olnTLUXXHqNTHtweAipgdeoOEXeh < UnityVersion.UNITY_5_0) || olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_0_0p1)
				{
					vNAZRGUEcJqKwQXDIyFYGSNSymhE = true;
					KkumtxElYOBvKapyZQwEReYCcUamA = true;
				}
				break;
			case Platform.Linux:
			...
			if (aWduCTYgMShjwYgLjeePGUPTyquD && YNWyxWyKsNFiNmgVFzNnYDPXJNwN == EditorPlatform.Windows && ((olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_6_3p1 && olnTLUXXHqNTHtweAipgdeoOEXeh < UnityVersion.UNITY_5_0) || olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_5_0_0p1))
			{
				vNAZRGUEcJqKwQXDIyFYGSNSymhE = true;
				KkumtxElYOBvKapyZQwEReYCcUamA = true;
			}
			if ((!aWduCTYgMShjwYgLjeePGUPTyquD && MeZkGconEOWOtuSlJBdxirfDIiwG == Platform.Windows) || (aWduCTYgMShjwYgLjeePGUPTyquD && YNWyxWyKsNFiNmgVFzNnYDPXJNwN == EditorPlatform.Windows))
			{
				yKbEEpjOQKhoMrpyYLYPKrHMmnCkA = olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type xqTyQbLWGJtPirpIOcZqEvZzeZFLA(KRitJYRVucDrJLuSLfADcaiKYEht P_0)
		{
			if (!MdSMqTwzGJSBIiaZlZhhAzmTSjRf)
			{
				return null;
			}
			if (olnTLUXXHqNTHtweAipgdeoOEXeh >= UnityVersion.UNITY_4_3)
			{
				return wFHXgLagpEfqSAzUFGsAwTHdTUyh(P_0);
			}
			return null;
		}

		private static Type wFHXgLagpEfqSAzUFGsAwTHdTUyh(KRitJYRVucDrJLuSLfADcaiKYEht P_0)
		{
			return P_0 switch
			{
				KRitJYRVucDrJLuSLfADcaiKYEht.RigidbodyInterpolation2D => typeof(RigidbodyInterpolation2D), 
				KRitJYRVucDrJLuSLfADcaiKYEht.RigidbodySleepMode2D => typeof(RigidbodySleepMode2D), 
				KRitJYRVucDrJLuSLfADcaiKYEht.CollisionDetectionMode2D => typeof(CollisionDetectionMode2D), 
				KRitJYRVucDrJLuSLfADcaiKYEht.PhysicsMaterial2D => typeof(PhysicsMaterial2D), 
				KRitJYRVucDrJLuSLfADcaiKYEht.Collider2D => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return jIIvMaDpvaZVRkWwcwysYgrwcSMf(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return jIIvMaDpvaZVRkWwcwysYgrwcSMf((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return jIIvMaDpvaZVRkWwcwysYgrwcSMf((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return jIIvMaDpvaZVRkWwcwysYgrwcSMf(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && vNAZRGUEcJqKwQXDIyFYGSNSymhE)
				{
					return false;
				}
				if (KkumtxElYOBvKapyZQwEReYCcUamA)
				{
					return false;
				}
			}
			...
			{
				if (gPahlaBOJFpsDtAaOKDDiqGaytqlb && name.Equals(xxaUaUnEkRkNEnQxNDhkZAaXXjvE, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (nvjIRLJbTGYczNmwdgMTCrwrZwsI && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static _0001 jIIvMaDpvaZVRkWwcwysYgrwcSMf<_0001>(_0001 P_0) where _0001 : class
		{
			if (P_0 == null)
			{
				return null;
		...
		internal static ButtonStateFlags stLIzdnjxcOSKZAgjrcfOELCUdJD(KeyCode P_0)
		{
		...
		internal static ButtonStateFlags GJFUFTlnrZkKnaSBmEBRsknETwfK(string P_0)
		{
