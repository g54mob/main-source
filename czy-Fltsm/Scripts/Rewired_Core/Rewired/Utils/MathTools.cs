...
namespace Rewired.Utils
{
	...
	{
		private const float UWNFysGKrSGFeEJJggQmEnvhEPbhC = 1E-10f;

		private const double NBPjkYxWdcvRTIXQPyhSjYAvkZrE = 1E-10;

		private const float RKgleEyiYrIfzGsadIlnhbdwcwJaA = 0.0001f;
		...
		public static float ValueInNewRange(float oldValue, float oldMin, float oldMax, float newMin, float newMax, bool clamp)
		{
			if (clamp)
			{
				if (oldValue < oldMin)
		...
		{
			return ValueInNewRange(oldValue, oldMin, oldMax, newMin, newMax, clamp: true);
		}
		...
		internal static bool YxebIRhuPpYsoBotXPtKJBjVwYOc(int P_0, int P_1)
		{
			if (P_0 == 0 && P_1 == 0)
			{
				return false;
		...
		public static Vector2 RotateWorldPoint(Vector2 point, Vector2 center, float angle)
		{
			...
			float value = MathF.PI / 180f * ClampAngle360(angle);
		...
		public static bool LineIntersectsRect(Vector2 point1, Vector2 point2, Rect rect, out float sqrMagnitude)
		{
			...
			bool flag4 = LineSegementsIntersect(point1, point2, new Vector2(rect.xMin, rect.yMin), new Vector2(rect.xMax, rect.yMin), out intersection4, collinearIntersects: true);
		...
		private static bool QioTMbXTGutgHhejiWkiUVSNUomG(Vector2 P_0, Vector2 P_1, Vector2 P_2, Vector2 P_3, out Vector2 P_4)
		{
