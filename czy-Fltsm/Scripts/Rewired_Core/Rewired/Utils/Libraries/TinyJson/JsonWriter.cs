...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		private static Action<StringBuilder, object> pTVzyqywmfQKTcGVsUPJuWcDHlIT;

		private static Action<StringBuilder, object> awNQiiHUpeKCymnBaugaaLNCnfDD => OEAHflQKVHjDwrLcKeZvLgecgQsU;

		public static string ToJson(object item)
		{
			StringBuilder stringBuilder = new StringBuilder();
			OEAHflQKVHjDwrLcKeZvLgecgQsU(stringBuilder, item);
			return stringBuilder.ToString();
		}

		private static void OEAHflQKVHjDwrLcKeZvLgecgQsU(StringBuilder P_0, object P_1)
		{
			if (P_1 == null)
			{
				P_0.Append("null");
			...
			{
				((IExportToJson)P_1).WriteJson(P_0, awNQiiHUpeKCymnBaugaaLNCnfDD);
				return;
			}
			if ((object)type == typeof(string))
			{
				P_0.Append('"');
				OvNACXBpVDceXcDVeUSLRNbhWBdp(P_0, (string)P_1);
				P_0.Append('"');
				return;
			}
			...
			{
				OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, P_1.ToString());
				return;
			}
			if (ReflectionTools.IsEnum(type))
			{
				Type underlyingType = Enum.GetUnderlyingType(type);
				OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, Convert.ChangeType(P_1, underlyingType));
				return;
			}
			if ((object)type == typeof(AnimationCurve))
			{
				P_0.Append('{');
				P_0.Append("\"keys\":");
				OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, ((AnimationCurve)P_1).keys);
				P_0.Append('}');
				return;
			}
			if ((object)type == typeof(Keyframe))
			{
				...
				P_0.Append('{');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "time", keyframe.time);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "value", keyframe.value);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "inTangent", keyframe.inTangent);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "outTangent", keyframe.outTangent);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "inWeight", keyframe.inWeight);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "outWeight", keyframe.outWeight);
				P_0.Append(',');
				hhDcDVkndEEzIHLxnPMqniLOzPsNA(P_0, "weightedMode", keyframe.weightedMode);
				P_0.Append('}');
				return;
			}
			if (ReflectionTools.DoesTypeImplement(type, typeof(IList)))
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, list[i]);
				}
				P_0.Append(']');
				return;
			}
			if (ReflectionTools.IsGenericType(type) && ReflectionTools.DoesTypeImplement(type, typeof(IDictionary)))
			{
				...
				foreach (object key in dictionary.Keys)
				{
					...
					P_0.Append("\":");
					OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, dictionary[key]);
				}
				P_0.Append('}');
				return;
			}
			...
			foreach (FieldInfo field in ReflectionTools.GetFields(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value != null)
				{
					...
					P_0.Append(name);
					P_0.Append("\":");
					OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, value);
				}
			}
			foreach (PropertyInfo property in ReflectionTools.GetProperties(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value2 != null)
				{
					...
					P_0.Append(name2);
					P_0.Append("\":");
					OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, value2);
				}
			}
			P_0.Append('}');
		}

		private static void hhDcDVkndEEzIHLxnPMqniLOzPsNA(StringBuilder P_0, string P_1, object P_2)
		{
			...
			OEAHflQKVHjDwrLcKeZvLgecgQsU(P_0, P_2);
		}

		private static void OvNACXBpVDceXcDVeUSLRNbhWBdp(StringBuilder P_0, string P_1)
		{
			if (string.IsNullOrEmpty(P_1))
			{
				return;
