...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> bvFDrdjsvaslVoalVgkFmXlYXCYN;

		private static Action<object, Action> invokeDelegate => RlcemmEuMuojYhQVAdKPASuJMvmmB;
		...
		private static void RlcemmEuMuojYhQVAdKPASuJMvmmB(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	{
		private T wNsdbIVvrMltZKjidvXVpCIlVwzp;

		private static Action<object, Action<T>> WgYaKrorOWQtwUcyaGatuxGRNrlJ;

		private static Action<object, Action<T>> invokeDelegate => KVnQnNPCliDiLssYvoxJiPeDSbnu;
		...
		{
			wNsdbIVvrMltZKjidvXVpCIlVwzp = arg0;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			wNsdbIVvrMltZKjidvXVpCIlVwzp = default(T);
		}
		...
		private static void KVnQnNPCliDiLssYvoxJiPeDSbnu(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.wNsdbIVvrMltZKjidvXVpCIlVwzp);
			}
		}
	...
	{
		private T aIvlTJVPxJDPhfGzlDwgflXAXcuO;

		private T2 RLvbVeaatPJDgZvPlumnPipwEtzAb;

		private static Action<object, Action<T, T2>> UpxEdMtVSSSGzhRixTunhMSdEmaX;

		private static Action<object, Action<T, T2>> invokeDelegate => vJZyyrETyrIKFsPTmlQmsmLhtnhk;
		...
		{
			aIvlTJVPxJDPhfGzlDwgflXAXcuO = arg0;
			RLvbVeaatPJDgZvPlumnPipwEtzAb = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			aIvlTJVPxJDPhfGzlDwgflXAXcuO = default(T);
			RLvbVeaatPJDgZvPlumnPipwEtzAb = default(T2);
		}
		...
		private static void vJZyyrETyrIKFsPTmlQmsmLhtnhk(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.aIvlTJVPxJDPhfGzlDwgflXAXcuO, safeAction.RLvbVeaatPJDgZvPlumnPipwEtzAb);
			}
		}
