...
namespace Rewired.Utils
{
	...
	public static class ReflectionTools
	{
		...
		{
			return AOGKisRqMqwBSEHriYmuDQMygIIAA(classPath, true, ignoreCase);
		}

		internal static Type GetTypeInUnityBuildAssembly(string classPath, bool ignoreCase = false)
		{
			return AOGKisRqMqwBSEHriYmuDQMygIIAA(classPath, false, ignoreCase);
		}

		private static Type AOGKisRqMqwBSEHriYmuDQMygIIAA(string P_0, bool P_1, bool P_2 = false)
		{
			Assembly[] assemblies = AppDomain.CurrentDomain.GetAssemblies();
			for (int i = 0; i < assemblies.Length; i++)
			{
