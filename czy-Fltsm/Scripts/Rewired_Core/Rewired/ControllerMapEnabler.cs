...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.RDtTFCChfbuGeHQlJGPuhqsProjJA)
				{
		...
		internal class LoufCxnWRPkzSbBIKyhZshokUpWL
		{
			public bool CakGUHcwDJpyWnIFqEUJPngfbanmA;

			public UGcaqZCDBFRgWVBlZOOhcpgBRMFkc[] yOYqynEYOTApZUuuAQnuwvvrwRwc;

			public LoufCxnWRPkzSbBIKyhZshokUpWL(bool P_0, UGcaqZCDBFRgWVBlZOOhcpgBRMFkc[] P_1)
			{
				CakGUHcwDJpyWnIFqEUJPngfbanmA = P_0;
				yOYqynEYOTApZUuuAQnuwvvrwRwc = P_1;
			}
		}

		private bool BBsywVyTRzLbPFncAlumrJwRlRVg;

		private Player jrPDcdpNjruUvmqRFARlVJzSSUPt;

		private LoufCxnWRPkzSbBIKyhZshokUpWL uTVmlomiLDeEnLixUQmtfYrjUKLS;

		private readonly int vJiFqkweKIBKQXWZBdXCJuePGZRz;

		private List<RuleSet> oLgcKfyNXIemSclyvVcSJWopvufRA;

		public bool enabled
		{
			get
			{
				return BBsywVyTRzLbPFncAlumrJwRlRVg;
			}
			set
			{
				BBsywVyTRzLbPFncAlumrJwRlRVg = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return oLgcKfyNXIemSclyvVcSJWopvufRA;
			}
			set
			{
				if (value == null)
				...
				oLgcKfyNXIemSclyvVcSJWopvufRA = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, LoufCxnWRPkzSbBIKyhZshokUpWL P_1)
		{
			if (P_0 == null)
			{
			...
			vJiFqkweKIBKQXWZBdXCJuePGZRz = ReInput.id;
			jrPDcdpNjruUvmqRFARlVJzSSUPt = P_0;
			uTVmlomiLDeEnLixUQmtfYrjUKLS = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != vJiFqkweKIBKQXWZBdXCJuePGZRz)
			{
				ReInput.CheckInitialized(vJiFqkweKIBKQXWZBdXCJuePGZRz);
			}
			else
			{
				if (!BBsywVyTRzLbPFncAlumrJwRlRVg || oLgcKfyNXIemSclyvVcSJWopvufRA == null)
				{
					return;
				}
				int count = oLgcKfyNXIemSclyvVcSJWopvufRA.Count;
				if (count == 0)
				{
					return;
				}
				...
				jrPDcdpNjruUvmqRFARlVJzSSUPt.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = oLgcKfyNXIemSclyvVcSJWopvufRA[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((uTVmlomiLDeEnLixUQmtfYrjUKLS != null && uTVmlomiLDeEnLixUQmtfYrjUKLS.yOYqynEYOTApZUuuAQnuwvvrwRwc != null) ? uTVmlomiLDeEnLixUQmtfYrjUKLS.yOYqynEYOTApZUuuAQnuwvvrwRwc.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(uTVmlomiLDeEnLixUQmtfYrjUKLS.yOYqynEYOTApZUuuAQnuwvvrwRwc[i].GZeupQJbOKjotgFdvbvFbTbbKbKXB);
				...
				controllerMapEnablerRuleSetInstance.enabled = uTVmlomiLDeEnLixUQmtfYrjUKLS.yOYqynEYOTApZUuuAQnuwvvrwRwc[i].uDroyIswjZdPbGCoFzsovbswnixj;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (uTVmlomiLDeEnLixUQmtfYrjUKLS != null)
			{
				BBsywVyTRzLbPFncAlumrJwRlRVg = uTVmlomiLDeEnLixUQmtfYrjUKLS.CakGUHcwDJpyWnIFqEUJPngfbanmA;
			}
			oLgcKfyNXIemSclyvVcSJWopvufRA = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return msGKYTXzDppVkybGIWcXNgmpDwSw().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return msGKYTXzDppVkybGIWcXNgmpDwSw().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				zhFPJwHIokiRmekJUuvoiEMJzvOrA(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				zhFPJwHIokiRmekJUuvoiEMJzvOrA(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject msGKYTXzDppVkybGIWcXNgmpDwSw()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			lhSnRNuFkHmYgGOBSUMZmnzoHvBd(serializedObject);
			return serializedObject;
		}

		private void lhSnRNuFkHmYgGOBSUMZmnzoHvBd(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", BBsywVyTRzLbPFncAlumrJwRlRVg);
			P_0.Add("ruleSets", oLgcKfyNXIemSclyvVcSJWopvufRA);
		}

		private bool zhFPJwHIokiRmekJUuvoiEMJzvOrA(SerializedObject P_0)
		{
			BBsywVyTRzLbPFncAlumrJwRlRVg = false;
			oLgcKfyNXIemSclyvVcSJWopvufRA = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref BBsywVyTRzLbPFncAlumrJwRlRVg);
