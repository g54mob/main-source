...
namespace Rewired.Data
{
	...
	public sealed class CustomController_Editor
	{
		...
		private sealed class wWrcNCnqqrGeeENtqeysGQtNaCls : IEnumerable<ControllerElementIdentifier>, IEnumerable, IEnumerator<ControllerElementIdentifier>, IEnumerator, IDisposable
		{
			private int pEMWWUcrKofVquswVKDQbmmANEDr;

			private ControllerElementIdentifier eLIPdgADnmFyCwMFCiqXghwVInPCb;

			private int uCFXjSGvgQRlMEeuIBJieHPxPCdj;

			public CustomController_Editor ajSCErGrCOcgmcFerciwRgxsFsrCb;

			private int LPHmEXvTvLmbkcyrPhotKpxkMPqb;
			...
			ControllerElementIdentifier IEnumerator<ControllerElementIdentifier>.Current
			{
				...
				get
				{
					return eLIPdgADnmFyCwMFCiqXghwVInPCb;
				}
			}
			...
			[DebuggerHidden]
			public wWrcNCnqqrGeeENtqeysGQtNaCls(int P_0)
			{
				pEMWWUcrKofVquswVKDQbmmANEDr = P_0;
				uCFXjSGvgQRlMEeuIBJieHPxPCdj = Environment.CurrentManagedThreadId;
			}

			[DebuggerHidden]
			void IDisposable.Dispose()
			{
				pEMWWUcrKofVquswVKDQbmmANEDr = -2;
			}

			private bool MoveNext()
			{
				int num = pEMWWUcrKofVquswVKDQbmmANEDr;
				CustomController_Editor customController_Editor = ajSCErGrCOcgmcFerciwRgxsFsrCb;
				switch (num)
				{
				default:
					return false;
				case 0:
					pEMWWUcrKofVquswVKDQbmmANEDr = -1;
					if (customController_Editor._elementIdentifiers == null)
					{
						return false;
					}
					LPHmEXvTvLmbkcyrPhotKpxkMPqb = 0;
					break;
				case 1:
					pEMWWUcrKofVquswVKDQbmmANEDr = -1;
					LPHmEXvTvLmbkcyrPhotKpxkMPqb++;
					break;
				}
				if (LPHmEXvTvLmbkcyrPhotKpxkMPqb < customController_Editor._elementIdentifiers.Count)
				{
					eLIPdgADnmFyCwMFCiqXghwVInPCb = customController_Editor._elementIdentifiers[LPHmEXvTvLmbkcyrPhotKpxkMPqb];
					pEMWWUcrKofVquswVKDQbmmANEDr = 1;
					return true;
				}
				return false;
			...
			{
				wWrcNCnqqrGeeENtqeysGQtNaCls wWrcNCnqqrGeeENtqeysGQtNaCls2;
				if (pEMWWUcrKofVquswVKDQbmmANEDr == -2 && uCFXjSGvgQRlMEeuIBJieHPxPCdj == Environment.CurrentManagedThreadId)
				{
					pEMWWUcrKofVquswVKDQbmmANEDr = 0;
					wWrcNCnqqrGeeENtqeysGQtNaCls2 = this;
				}
				else
				{
					wWrcNCnqqrGeeENtqeysGQtNaCls2 = new wWrcNCnqqrGeeENtqeysGQtNaCls(0);
					wWrcNCnqqrGeeENtqeysGQtNaCls2.ajSCErGrCOcgmcFerciwRgxsFsrCb = ajSCErGrCOcgmcFerciwRgxsFsrCb;
				}
				return wWrcNCnqqrGeeENtqeysGQtNaCls2;
			}
		...
		{
			[IteratorStateMachine(typeof(wWrcNCnqqrGeeENtqeysGQtNaCls))]
			get
			{
				return new wWrcNCnqqrGeeENtqeysGQtNaCls(-2)
				{
					ajSCErGrCOcgmcFerciwRgxsFsrCb = this
				};
			}
		}
		...
		public void AddAxis()
		{
			axes.Add((Axis)zPHyPtJKlZwynWkvqmbFLIkNGdrd(ControllerElementType.Axis));
		}

		public void AddButton()
		{
			buttons.Add((Button)zPHyPtJKlZwynWkvqmbFLIkNGdrd(ControllerElementType.Button));
		}
		...
		public void InsertAxis(int index)
		{
			...
			axes.Insert(index, (Axis)zPHyPtJKlZwynWkvqmbFLIkNGdrd(ControllerElementType.Axis));
		}
		...
		public void InsertButton(int index)
		{
			...
			buttons.Insert(index, (Button)zPHyPtJKlZwynWkvqmbFLIkNGdrd(ControllerElementType.Button));
		}
		...
		public void DuplicateElement(ControllerElementType type, int index)
		{
			...
			{
			case ControllerElementType.Axis:
				ikamsmYqQmxdYicfflAHqOuTVUDF(index, axes);
				break;
			case ControllerElementType.Button:
				ikamsmYqQmxdYicfflAHqOuTVUDF(index, buttons);
				break;
			default:
				throw new NotImplementedException();
			}
		...
		private void ikamsmYqQmxdYicfflAHqOuTVUDF<_0001>(int P_0, List<_0001> P_1) where _0001 : Element
		{
			...
			ControllerElementIdentifier controllerElementIdentifier = kOgHCKnDdvLnWHBzgAUAkkQbmWYH(val.elementIdentifierId, text);
		...
		private ControllerElementIdentifier kOgHCKnDdvLnWHBzgAUAkkQbmWYH(int P_0, string P_1)
		{
		...
		private Element zPHyPtJKlZwynWkvqmbFLIkNGdrd(ControllerElementType P_0)
		{
			switch (P_0)
			{
			case ControllerElementType.Axis:
			{
				...
				ControllerElementIdentifier controllerElementIdentifier2 = jNkrysupqTveKHoipCzxQGiTAyZe(P_0, text2, string.Empty, string.Empty, key, string.Empty, string.Empty);
			...
			case ControllerElementType.Button:
			{
				...
				ControllerElementIdentifier controllerElementIdentifier = jNkrysupqTveKHoipCzxQGiTAyZe(P_0, text, string.Empty, string.Empty, key, string.Empty, string.Empty);
		...
		private ControllerElementIdentifier jNkrysupqTveKHoipCzxQGiTAyZe(ControllerElementType P_0, string P_1, string P_2, string P_3, string P_4, string P_5, string P_6)
		{
			int elementIdentifierIdCounter = _elementIdentifierIdCounter;
			_elementIdentifierIdCounter++;
			ControllerElementIdentifier controllerElementIdentifier = new ControllerElementIdentifier(new ControllerElementIdentifier.mlDJkVjamumzDMNFFhJnSSbvSsLS
