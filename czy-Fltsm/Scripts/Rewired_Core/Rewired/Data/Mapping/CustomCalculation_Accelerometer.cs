...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool tzbeLbeUTZaKJAkAiRhgiZbySWYaA()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = IegMjSPVdylcNgdxHocRtlPzhFbJ();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = mbrxeKgfzHszttJZmdEaFfLUoGjq();
					flag = true;
					break;
				}
			}
		...
		private float IegMjSPVdylcNgdxHocRtlPzhFbJ()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => RVSLYmcugreubzEAyDkNeTJYFkgGb(num2), 
				_ => 0f, 
			};
		}

		private float mbrxeKgfzHszttJZmdEaFfLUoGjq()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float RVSLYmcugreubzEAyDkNeTJYFkgGb(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
