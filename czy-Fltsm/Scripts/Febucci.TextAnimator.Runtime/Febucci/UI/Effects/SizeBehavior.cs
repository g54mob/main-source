...
namespace Febucci.UI.Effects
{
	...
	public sealed class SizeBehavior : BehaviorScriptableBase
	{
		...
		public override void SetModifier(ModifierInfo modifier)
		{
			...
			{
			case "a":
				amplitude = baseAmplitude * modifier.value * -1f + 1f;
				break;
			case "f":
		...
		{
			character.current.positions.LerpUnclamped(character.current.positions.GetMiddlePos(), (Mathf.Cos(animator.time.timeSinceStart * frequency + (float)character.index * waveSize) + 1f) / 2f * amplitude);
		}
	}
}
