...
namespace Febucci.UI.Core
{
	...
	public abstract class TAnimCore : MonoBehaviour
	{
		...
		private void ConvertText(string textToParse, ShowTextMode showTextMode)
		{
			...
			textWithoutTextAnimTags = (StyleSheet ? TextParser.ParseText(_text, new StylesParser('<', '/', '>', StyleSheet)) : _text);
			textWithoutTextAnimTags = TextParser.ParseText(textWithoutTextAnimTags, list.ToArray());
			...
			void PopulateCharacters(bool resetVisibility)
			{
				...
				{
					characters[j].ResetInfo(j, resetVisibility);
				...
				float CalculateRegionMaxDuration(AnimationRegion[] tags)
				{
					...
					foreach (AnimationRegion animationRegion in tags)
					{
						...
						for (int l = 0; l < ranges.Length; l++)
						{
							...
							if (j >= tagRange.indexes.x && j < tagRange.indexes.y)
							{
		...
		private void ProcessAnimationRegions(AnimationRegion[] regions)
		{
			foreach (AnimationRegion animationRegion in regions)
			{
				...
				for (int j = 0; j < ranges.Length; j++)
				{
					...
					animationRegion.SetupContextFor(this, tagRange.modifiers);
					for (int k = tagRange.indexes.x; k < tagRange.indexes.y && k < charactersCount; k++)
					{
						if (!(characters[k].passedTime <= 0f) && animationRegion.IsVisibilityPolicySatisfied(IsCharacterAppearing(k)) && animationRegion.animation.CanApplyEffectTo(characters[k], this))
						{
							animationRegion.animation.ApplyEffectTo(ref characters[k], this);
						}
					}
				}
			}
