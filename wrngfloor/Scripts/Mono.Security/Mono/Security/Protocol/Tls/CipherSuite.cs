...
namespace Mono.Security.Protocol.Tls
{
	internal abstract class CipherSuite
	{
		...
		private void createEncryptionCipher()
		{
			...
			if (context is ClientContext)
			{
				clientHMAC = new Mono.Security.Cryptography.HMAC(CreateHashAlgorithm(), context.Negotiating.ClientWriteMAC);
			}
			else
			{
				serverHMAC = new Mono.Security.Cryptography.HMAC(CreateHashAlgorithm(), context.Negotiating.ServerWriteMAC);
			}
		}
