...
namespace GameCreator.Runtime.VisualScripting
{
	...
	[Keywords(new string[] { "Audio", "Ambience", "Background", "Fade", "Mute" })]
	...
	public class InstructionCommonAudioAmbientStop : Instruction
	{
		...
		public override string Title => string.Format("Stop Ambient: {0} {1}", m_AudioClip, (transitionOut < float.Epsilon) ? string.Empty : string.Format("in {0} second{1}", transitionOut, Mathf.Approximately(transitionOut, 1f) ? string.Empty : "s"));
		...
		protected override async Task Run(Args args)
		{
			...
			if (!(audioClip == null))
			{
				if (m_WaitToComplete)
				{
					await Singleton<AudioManager>.Instance.Ambient.Stop(audioClip, transitionOut);
				}
				else
				{
					Singleton<AudioManager>.Instance.Ambient.Stop(audioClip, transitionOut);
				}
			}
		}
	}
