...
namespace GameCreator.Runtime.VisualScripting
{
	...
	[Keywords(new string[] { "Check", "Collide", "Touch", "Suit", "Square", "Cube", "3D" })]
	...
	public class ConditionPhysicsCheckBox3D : Condition
	{
		...
		[SerializeField]
		private PropertyGetRotation m_Rotation = GetRotationCharacter.Create;

		[SerializeField]
		private Vector3 m_HalfExtents = Vector3.one * 0.5f;

		[SerializeField]
		private LayerMask m_LayerMask = -5;

		protected override string Summary => $"check Box 3D at {m_Position}";

		protected override bool Run(Args args)
		{
			return Physics.CheckBox(m_Position.Get(args), m_HalfExtents, m_Rotation.Get(args), m_LayerMask);
		}
	}
}
