...
namespace GameCreator.Runtime.VisualScripting
{
	...
	public class InstructionCommonAudioAmbientStopAll : Instruction
	{
		...
		public override string Title => string.Format("Stop Ambient: {0}", (transitionOut < float.Epsilon) ? string.Empty : string.Format("in {0} second{1}", transitionOut, Mathf.Approximately(transitionOut, 1f) ? string.Empty : "s"));

		protected override async Task Run(Args args)
		{
			if (m_WaitToComplete)
			{
				await Singleton<AudioManager>.Instance.Ambient.StopAll(transitionOut);
			}
			else
			{
				Singleton<AudioManager>.Instance.Ambient.StopAll(transitionOut);
			}
		}
	}
}
