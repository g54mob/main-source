...
namespace Mirror.FizzySteam
{
	public class NextServer : NextCommon, IServer
	{
		...
		public static NextServer CreateServer(FizzySteamworks transport, int maxConnections)
		{
			...
			server.OnReceivedData += delegate(int id, ArraySegment<byte> segment, int channelId)
			{
				transport.OnServerDataReceived(id, segment, channelId);
			};
		...
		public void ReceiveData()
		{
			foreach (HSteamNetConnection item in connToMirrorID.FirstTypes.ToList())
			{
				...
				int num;
				if ((num = SteamNetworkingSockets.ReceiveMessagesOnConnection(item, messagePtrs, 256)) > 0)
				{
					for (int j = 0; j < num; j++)
					{
						var (arg, arg2) = ProcessMessage(messagePtrs[j]);
		...
		public void Send(int connectionId, ArraySegment<byte> segment, int channelId)
		{
			if (connToMirrorID.TryGetValue(connectionId, out var value))
			{
				EResult eResult = SendSocket(value, segment, channelId);
				switch (eResult)
				{
