...
namespace Mirror.FizzySteam
{
	public class NextClient : NextCommon, IClient
	{
		...
		private readonly IntPtr[] messagePtrs = new IntPtr[256];
		...
		public static NextClient CreateClient(FizzySteamworks transport, string host)
		{
			...
			nextClient.OnReceivedData += delegate(ArraySegment<byte> segment, int channelId)
			{
				transport.OnClientDataReceived(segment, channelId);
			};
			try
			{
		...
		public void ReceiveData()
		{
			...
			int num;
			if ((num = SteamNetworkingSockets.ReceiveMessagesOnConnection(HostConnection, messagePtrs, 256)) <= 0)
			{
				return;
			}
			for (int j = 0; j < num; j++)
			{
				var (arg, num2) = ProcessMessage(messagePtrs[j]);
				if (Connected)
				{
					this.OnReceivedData(arg, num2);
					continue;
				}
				byte[] segmentCopy = new byte[arg.Count];
				Array.Copy(arg.Array, arg.Offset, segmentCopy, 0, arg.Count);
				int channelIdCopy = num2;
				BufferedData.Add(delegate
				{
					this.OnReceivedData(segmentCopy, channelIdCopy);
				});
			}
		}
		...
		public void Send(ArraySegment<byte> segment, int channelId)
		{
			try
			{
				EResult eResult = SendSocket(HostConnection, segment, channelId);
				switch (eResult)
				{
