...
namespace Digger.Modules.Core.Sources.Operations
{
	public class BasicOperation : IOperation<VoxelModificationJob>
	{
		...
		public ModificationArea GetAreaToModify(DiggerSystem digger)
		{
			...
			if (Params.Brush == BrushType.Custom && (bool)Params.CustomBrush)
			{
		...
		{
			NativeArray<Voxel> inputVoxels = new NativeArray<Voxel>(0, Allocator.Persistent);
			int3 inputSizeVox = new int3(0, 0, 0);
			int3 inputOriginVox = new int3(0, 0, 0);
			if (Params.Brush == BrushType.Custom && (bool)Params.CustomBrush && Params.CustomBrush.InputVoxels != null)
			{
				inputVoxels.Dispose();
			...
			VoxelModificationJob result = new VoxelModificationJob
			{
		...
		public ModificationResult Complete(VoxelModificationJob job, VoxelChunk chunk)
		{
			...
			TerrainCutter cutter = chunk.Cutter;
			...
			{
				RemovedMatterQuantity = (double)job.RemovedMatterCounter[0] / 1000000.0,
				AddedMatterQuantity = (double)job.AddedMatterCounter[0] / 1000000.0,
