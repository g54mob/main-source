...
namespace Digger.Modules.Core.Sources.Generators
{
	...
	{
		public JobHandle GenerateVoxels(float[] heightArray, int3 chunkPosition, int sizeVox, float3 heightmapScale, NativeArray<float> heights, NativeArray<Voxel> voxels, bool refreshOnly)
		{
			return IJobParallelForExtensions.Schedule(new SimpleVoxelGenerationJob
			{
