...
namespace Digger.Modules.Core.Sources.VoxelPhysics
{
	public static class RemoveFloatingVoxels
	{
		...
		{
			return new RemoveFloatingVoxelsJob
			{
		...
		public static void Complete(RemoveFloatingVoxelsJob job, VoxelChunk chunk)
		{
			...
			foreach (int key in chunk.LabelMap.Keys)
			{
				if (!chunk.LabelsConnectedToTheGroundThroughNeighbors.Contains(key))
				{
					LabelsToRemove.Value.Add(key);
				}
			}
			foreach (int item in LabelsToRemove.Value)
			{
				chunk.LabelMap.Remove(item);
			}
