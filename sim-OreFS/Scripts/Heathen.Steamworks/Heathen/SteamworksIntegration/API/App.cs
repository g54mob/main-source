	...
	public static class App
	{
		public static class Client
		{
			...
			public static UnityEventServersDisconnected EventServersDisconnected
			{
				get
				{
					if (m_SteamServersDisconnected_t == null)
					{
			...
			public static void Initialize(AppData appId, InventorySettings inventory, LeaderboardObject[] leaderboards, InputAction[] actions)
			{
				...
				else
				{
					...
					else
					{
						...
						{
							m_UserStatsReceived_t = Callback<UserStatsReceived_t>.Create(HandleUserStatsReceived_t);
						}
						StatsAndAchievements.Client.RequestCurrentStats();
						if (isDebugging)
						{
							Debug.Log("Steam API has been initialized with App ID: " + SteamUtils.GetAppID().ToString() + " user stats have been requested.");
						}
			...
			private static void HandleUserStatsReceived_t(UserStatsReceived_t param)
			{
				if (isDebugging)
				{
					Debug.Log($"Received User Stats per request response {param.m_eResult}.");
				}
			}
			...
			public static void GetAppName(AppData appId, Action<string, bool> callback)
			{
				...
				if (!appsListLoaded)
				{
					LoadAppNames(delegate
					{
						...
						{
							callback?.Invoke("Unknown", arg2: true);
						}
					});
				}
		...
		public static UnityEvent<string> evtSteamInitializationError = new UnityEvent<string>();
