	...
	public class WideOutline : ScriptableRendererFeature
	{
		private class WideOutlinePass : ScriptableRenderPass
		{
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				if (!silhouetteHandle.IsValid() || !silhouetteDepthHandle.IsValid() || !pingHandle.IsValid() || !pongHandle.IsValid())
				{
					return;
				}
				PassData passData;
				using (IRasterRenderGraphBuilder rasterRenderGraphBuilder = renderGraph.AddRasterRenderPass<PassData>("Mask (Wide Outline)", out passData, ".\\Packages\\dev.ameye.linework\\Runtime\\WideOutline\\WideOutline.cs", 160))
				{
				...
				PassData passData2;
				using (IRasterRenderGraphBuilder rasterRenderGraphBuilder2 = renderGraph.AddRasterRenderPass<PassData>("Silhouette (Wide Outline)", out passData2, ".\\Packages\\dev.ameye.linework\\Runtime\\WideOutline\\WideOutline.cs", 184))
				{
				...
				PassData passData3;
				using (IUnsafeRenderGraphBuilder unsafeRenderGraphBuilder = renderGraph.AddUnsafePass<PassData>("Flood (Wide Outline)", out passData3, ".\\Packages\\dev.ameye.linework\\Runtime\\WideOutline\\WideOutline.cs", 222))
				{
					...
					unsafeRenderGraphBuilder.SetRenderFunc(delegate(PassData _, UnsafeGraphContext context)
					{
						...
						for (int num = Mathf.CeilToInt(Mathf.Log(settings.width * cameraData.renderScale + 1f, 2f)) - 1; num >= 0; num--)
						{
				...
				using IRasterRenderGraphBuilder rasterRenderGraphBuilder3 = renderGraph.AddRasterRenderPass<PassData>("Outline (Wide Outline)", out passData3, ".\\Packages\\dev.ameye.linework\\Runtime\\WideOutline\\WideOutline.cs", 253);
				rasterRenderGraphBuilder3.UseTexture(in pingHandle);
				rasterRenderGraphBuilder3.SetRenderAttachment(universalResourceData.activeColorTexture, 0);
				rasterRenderGraphBuilder3.SetRenderAttachmentDepth(settings.customDepthBuffer ? silhouetteDepthHandle : universalResourceData.activeDepthTexture);
				rasterRenderGraphBuilder3.AllowPassCulling(value: true);
			...
			private static void CreateRenderGraphTextures(RenderGraph renderGraph, UniversalCameraData cameraData, out TextureHandle silhouetteHandle, out TextureHandle silhouetteDepthHandle, out TextureHandle pingHandle, out TextureHandle pongHandle)
			{
				RenderTextureDescriptor cameraTargetDescriptor = cameraData.cameraTargetDescriptor;
				cameraTargetDescriptor.graphicsFormat = GraphicsFormat.R8G8B8A8_UNorm;
				cameraTargetDescriptor.depthBufferBits = 0;
				cameraTargetDescriptor.sRGB = false;
				cameraTargetDescriptor.useMipMap = false;
				cameraTargetDescriptor.autoGenerateMips = false;
				silhouetteHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, cameraTargetDescriptor, "_SilhouetteBuffer", clear: false);
				...
				silhouetteDepthHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, cameraTargetDescriptor2, "_SilhouetteDepthBuffer", clear: false);
				RenderTextureDescriptor cameraTargetDescriptor3 = cameraData.cameraTargetDescriptor;
				cameraTargetDescriptor3.graphicsFormat = (SystemInfo.IsFormatSupported(GraphicsFormat.R16G16_SNorm, GraphicsFormatUsage.Render) ? GraphicsFormat.R16G16_SNorm : GraphicsFormat.R32G32_SFloat);
				cameraTargetDescriptor3.depthBufferBits = 0;
				pingHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, cameraTargetDescriptor3, "_PingBuffer", clear: false);
				pongHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, cameraTargetDescriptor3, "_PongBuffer", clear: false);
			}
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				using (new ProfilingScope(commandBuffer2, silhouetteSampler))
				{
					...
					foreach (Outline outline2 in settings.Outlines)
					{
						...
						BlendState defaultValue4 = BlendState.defaultValue;
						defaultValue4.blendState0 = new RenderTargetBlendState((ColorWriteMask)0);
						stateBlock2.blendState = defaultValue4;
				...
				using (new ProfilingScope(commandBuffer3, floodSampler))
				{
					...
					for (int num3 = Mathf.CeilToInt(Mathf.Log(settings.width * renderingData.cameraData.renderScale + 1f, 2f)) - 1; num3 >= 0; num3--)
					{
