...
namespace MonoMod
{
	internal static class MMDbgLog
	{
		...
		static MMDbgLog()
		{
			Tag = typeof(MonoMod.MMDbgLog).Assembly.GetName().Name;
			if (!(Environment.GetEnvironmentVariable("MONOMOD_DBGLOG") == "1"))
			{
				...
				else
				{
					...
					obj = ((text != null) ? new bool?(global::MultiTargetShims.Contains(text, Tag.ToLower(CultureInfo.InvariantCulture), StringComparison.Ordinal)) : ((bool?)null));
				}
				bool? flag = obj;
				if (flag != true)
				{
