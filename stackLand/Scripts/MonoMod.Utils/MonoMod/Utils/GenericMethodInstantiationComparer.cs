...
namespace MonoMod.Utils
{
	public class GenericMethodInstantiationComparer : IEqualityComparer<MethodBase>
	{
		...
		public int GetHashCode(MethodBase method)
		{
			...
			{
				num ^= method.DeclaringType.Assembly.GetHashCode();
				num ^= genericTypeComparer.GetHashCode(method.DeclaringType);
			}
			num ^= MultiTargetShims.GetHashCode(method.Name, StringComparison.Ordinal);
			...
			MethodBase methodBase = ((!(method is MethodInfo method2)) ? method.GetUnfilledMethodOnGenericType() : method2.GetActualGenericMethodDefinition());
