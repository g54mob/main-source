	...
	public class HarmonyManipulator
	{
		...
		private bool WritePrefixes(ILEmitter.Label returnLabel)
		{
			...
			if (!variables.TryGetValue(ResultVar, out var value))
			{
				...
				variableDefinition = (variables[ResultVar] = ((returnedType == typeof(void)) ? null : il.DeclareVariable(returnedType)));
		...
		private (ILEmitter.Label, bool) MakeReturnLabel()
		{
			...
			label.emitted = false;
			bool item = false;
			foreach (Instruction item2 in il.IL.Body.Instructions.Where((Instruction ins) => ins.MatchRet()))
			{
				item = true;
		...
		private bool WriteFinalizers(ILEmitter.Label returnLabel)
		{
			...
			{
				Type returnedType = AccessTools.GetReturnedType(original);
				VariableDefinition variableDefinition = (variables[ResultVar] = ((returnedType == typeof(void)) ? null : il.DeclareVariable(returnedType)));
