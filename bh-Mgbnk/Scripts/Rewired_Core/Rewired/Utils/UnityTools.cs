...
namespace Rewired.Utils
{
	...
	{
		internal struct EeMzBLeLNTmTALqdvIAKIfdYhceUA
		{
			public Platform zOoGnnrXSkgHbPnhlNzdfCtkaHpaA;

			public Platform LDrVujBCoZsyUclZBhVnlLITuynN;

			public EditorPlatform XUTscGeqTFEVjmcbxpuuYWhShvYM;

			public bool MecFFGLkDexdHGChiOCjbVVrEybp;

			public WebplayerPlatform wyyEAiRFbUMbvjbJdYSOMCuxULvJ;

			public ScriptingBackend MvpAKNayaIUbWksSzHkFhNknYLqE;

			public ScriptingAPILevel VVDYjBdlsVXpgKzOClFNBiiNiyDI;

			public IExternalTools SDkBrLHCgmvPSQRuUiQUagMSrScyA;

			public EeMzBLeLNTmTALqdvIAKIfdYhceUA(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				zOoGnnrXSkgHbPnhlNzdfCtkaHpaA = default(Platform);
				LDrVujBCoZsyUclZBhVnlLITuynN = default(Platform);
				XUTscGeqTFEVjmcbxpuuYWhShvYM = default(EditorPlatform);
				MecFFGLkDexdHGChiOCjbVVrEybp = false;
				wyyEAiRFbUMbvjbJdYSOMCuxULvJ = default(WebplayerPlatform);
				MvpAKNayaIUbWksSzHkFhNknYLqE = default(ScriptingBackend);
				VVDYjBdlsVXpgKzOClFNBiiNiyDI = default(ScriptingAPILevel);
				SDkBrLHCgmvPSQRuUiQUagMSrScyA = null;
			}
		}
		...
		{
			public enum kcwVvvePOFAkZpTRcgwBHKHioJpcA
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly kcwVvvePOFAkZpTRcgwBHKHioJpcA type;
			...
			private string kdbQNHXBPkSjGzZQjtMbSzUWMmqF(kcwVvvePOFAkZpTRcgwBHKHioJpcA P_0)
			{
				return null;
			}
			...
			private static int tAnOlqxlyqtuqWgpwePnNweACmxx(kcwVvvePOFAkZpTRcgwBHKHioJpcA P_0)
			{
				return 0;
			}
		}

		private const UnityVersion pQwfELRcjEgwRwilNVDYBPiNwTpR = UnityVersion.UNITY_5_0;

		private static UnityVersionClass EhZIfezsNCoJvuKFUbDPHNnPcSEI;

		private static UnityVersion lJWnywzpcrUMGVXncLNDYSMckHRd;

		private static string hOUiLNwsEVAIEGndyVkcrLjEyfdh;

		private static Platform NQwWUQONHVBzuKOrtimYXaVnceNs;

		private static EditorPlatform JbBSgFCuJCQMEAozhYGzljxbdLJb;

		private static bool ryQWrberCTgqxeQPXZmoviqjOrPk;

		private static bool UDfopEAFLItWlHoCQijNDQlgIpyM;

		private static bool guOfGCkAbmwEiBDOivazEMUulzTrA;

		private static WebplayerPlatform kgAfAMpvxBDzuSHLnaOlgBjpSAEl;

		private static bool JifOihCdpMvjhZOJOITFTikmJkYs;

		private static bool OyJvgXYkBaNKQmhuSQgGNLVcrJjN;

		private static bool ywxvUnqOuMbUXkTsaZQhvWswScMi;

		private static bool RqJcJHFawNygFBLFlrRjuFgiruDAA;

		private static bool uRYeTzfxbVBpcdTJNLYinvQRvDJk;

		private static bool ldPQTUenbQilQyDXibokDXiUcZXKA;

		private static string oWHwOqPusOhHTJfhphaTakmzhvWp;

		private static ScriptingBackend CFrEDSYMzoxjyggySJuyxlPlAcXS;

		private static ScriptingAPILevel DbUxfdphUfamSFvkstNhOeAOdsWP;

		private static bool xGYLwRoGwNMlJDFZuDtscYheZPnIA;

		private static IExternalTools uTEMKwGjYgieuOMjrSkhjVSDuwpE;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper LmHbaTQjjZJNJqwIppalhawheTLHA;

		private static bool IyefxeBODCdkhKOzPDXGmKfxCdLvA;
		...
		internal static IAndroidFallbackPlatformHelper xQsaKpOBEHsSabSmNncFrECApPQJ
		{
			[CompilerGenerated]
			get
			{
		...
		private static bool HjdkDxSbyAJtPSSuRuhWxdGpkQqg => false;
		...
		internal static void psKYLBSdWUVHpBtIgrlByAXACKyv(EeMzBLeLNTmTALqdvIAKIfdYhceUA P_0)
		{
		}
		...
		private static bool MBEDGORwnqIsBaPtZYEyAPHPXqiB(string P_0, out int P_1)
		{
			P_1 = default(int);
			return false;
		}
		...
		private static UnityVersion jgBfqrhPviuTqehVeRRCnaDgNleO(string P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion YOZreNSLvXZSVUBIvBjdZQefawGfA(int P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion dBUaShkvcXqSkZMyvJdxzkYsLYNwA(int P_0)
		{
			return default(UnityVersion);
		}
		...
		internal static Type ybmKmVfxiArOtNjxsquBMEfThzcbA(JYVfxadmCrgsMlznxhveVeOoxcWs P_0)
		{
			return null;
		}

		private static Type tfylgdYjPNiiPebjfGvldJlZnpXYA(JYVfxadmCrgsMlznxhveVeOoxcWs P_0)
		{
			return null;
		}
		...
		private static _0001 inBIKxZvtYWAEPqUToPvRxACpxD<_0001>(_0001 P_0) where _0001 : class
		{
			return null;
		}

		internal static ButtonStateFlags nckyPBRZVfXeVpznRIzCdrcoaQmL(KeyCode P_0)
		{
			return default(ButtonStateFlags);
		}

		internal static ButtonStateFlags BPghflXjRGUVgYZoMBasViRqnnGr(string P_0)
		{
			return default(ButtonStateFlags);
		}
	}
