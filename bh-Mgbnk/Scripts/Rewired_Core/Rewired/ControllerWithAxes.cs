...
namespace Rewired
{
	...
	{
		private sealed class gylTxPKNEpTIBybdrINKFVmXzzhbA : IEnumerable<ControllerPollingInfo>, IEnumerable, IEnumerator<ControllerPollingInfo>, IEnumerator, IDisposable
		{
			private int TWRxUxtKUtMTacTGOoubsESxdaA;

			private ControllerPollingInfo UoumvUSntODhfPqCPUJhwQRbGqIX;

			private int xkPNIBJOJXiDnAHdBYUVuVNbIFhM;

			public ControllerWithAxes CkCAfqfjZTAXtRrQleeecztoOXlH;

			private int TWDUUJoddRDuBFgNqKCytCMsfQAuA;
			...
			[DebuggerHidden]
			public gylTxPKNEpTIBybdrINKFVmXzzhbA(int P_0)
			{
			}
		...
		private sealed class YHfhRCzgItJfAVTZhHnJtzferlJA : IEnumerable<ControllerPollingInfo>, IEnumerable, IEnumerator<ControllerPollingInfo>, IEnumerator, IDisposable
		{
			private int WfOQEKSbZJZzisOWOdjaoDfHHhOs;

			private ControllerPollingInfo WPsVjYehpQKxihEDUjfrVEgeZFnW;

			private int mWtZbTKofmGwwdbgmdQVskWGKhVaA;

			public ControllerWithAxes aHTUkqiWydwAFOCFEAFoFboExmdF;

			private IEnumerator<ControllerPollingInfo> RNMJpfPhjwaMBPEDkjNWejUanZNUA;
			...
			[DebuggerHidden]
			public YHfhRCzgItJfAVTZhHnJtzferlJA(int P_0)
			{
			}
			...
			private void PYGzPHCkRTRoVuLsVjyLXLnDnijt()
			{
			}

			private void ccMGrTifhUcauGbHQCfkjTbzTbUAb()
			{
			}
		...
		private sealed class WRjcgPAtkNFlYEZFDjSBjAcfMnhnb : IEnumerable<ControllerPollingInfo>, IEnumerable, IEnumerator<ControllerPollingInfo>, IEnumerator, IDisposable
		{
			private int KNOQoyzxxnHiLpwzscoRtSTTBGnd;

			private ControllerPollingInfo fKDdfmIXrEyVrvepqnlUVHwOXBdL;

			private int pDcWBVEmJNidniVxnYeZmmoDgaVLA;

			public ControllerWithAxes WaINHhleCTDGJSTjmuieeTXDgInp;

			private IEnumerator<ControllerPollingInfo> ZMyJtdwLNwTkVzdiUzFkZIVrhAnB;
			...
			[DebuggerHidden]
			public WRjcgPAtkNFlYEZFDjSBjAcfMnhnb(int P_0)
			{
			}
			...
			private void lRrpHkXxZuxgFvfJBRpJUaQvREsA()
			{
			}

			private void ymoQJyEKFEbiujtaaFVBYusltZnSA()
			{
			}
		...
		private float[] YVaFlCtCgKKVdIEByDBjmIQhCeVC;

		private uint FBzvJfAnCDYFXWRyZhBgJDHaetEl;

		private TimerAbs ZcqrjRxJMVfjSPFLFZNzTTCuocxu;

		private float[] bkkTdHVzxktjSxnPucMgvtOhjqLl;

		private Func<int, int> FoPeMLbkOHGcORuiphsVCkEJYLHWb;
		...
		[IteratorStateMachine(typeof(YHfhRCzgItJfAVTZhHnJtzferlJA))]
		public override IEnumerable<ControllerPollingInfo> PollForAllElements()
		{
			return null;
		}

		[IteratorStateMachine(typeof(WRjcgPAtkNFlYEZFDjSBjAcfMnhnb))]
		public override IEnumerable<ControllerPollingInfo> PollForAllElementsDown()
		{
			return null;
		}

		[IteratorStateMachine(typeof(gylTxPKNEpTIBybdrINKFVmXzzhbA))]
		...
		internal bool hxoYAYjjcTqqBrYrzijEIwbqJdkT(ActionElementMap P_0, int P_1, bool P_2, bool P_3, out float P_4)
		{
			P_4 = default(float);
			return false;
		}

		internal override void NgSVRXtzGmXahkQxjWThqqmlVJXH(ControllerMap P_0)
		{
		}

		internal override void BEMHamOlnupxqKxDqmuyQAAUjGvI(ControllerMap P_0, ActionElementMap P_1)
		{
		}
		...
		[CompilerGenerated]
		private int mPjdoDVveSptqWtMVjQGnfKQLnEE(int P_0)
		{
			return 0;
		}
