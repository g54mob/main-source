...
public class Explosion : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [this @ rcx (Explosion)+50]");
		startAlpha = 0f;
	...
	private void Start()
	{
		...
		startSize = (Vector3)localScale.x;
		_ = localScale.z;
		nint num = (nint)typeof(Vector3);
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v51 @ rax_v5 (Il2CppClass<UnityEngine.Vector3>)+B8]");
		nint num2 = 0;
		float explosionRadius = PowerupConstants.GetExplosionRadius();
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v52 @ rcx_v4 (Il2CppStaticFields<UnityEngine.Vector3>)+14]");
		float num3 = 0f * explosionRadius;
		float num4 = num3 * sizeMultiplier;
