...
public class EnvironmentGenerator : MonoBehaviour
{
	...
	private bool SpawnCrystalAltars()
	{
		...
		{
			return CrystalAltars.Count == crystalAltarSpawnerData.ObjectsAmount;
		}
		return false;
	}
	...
	private bool SpawnCrystalFinder()
	{
		...
		{
			return CrystalFinders.Count == ((num != -1 && num < crystalFinderSpawnerData.ObjectsAmount) ? num : crystalFinderSpawnerData.ObjectsAmount);
		}
		return false;
	}
	...
	private bool SpawnSpecialBuildings()
	{
		...
		bool flag = false;
		bool num = LTFunctionLibrary.GetPlayerUpgradesManager().HasUnlockedUpgrade("PlayerUpgrade_specialBuildings_trader_unlock");
		...
		if (num)
		{
			...
			if (list.Count != traderSpawnerData.ObjectsAmount)
			{
				return false;
			}
		...
		if (flag)
		{
			...
			foreach (BaseSpawnerData baseSpawnerData in array)
			{
				...
				num2 += baseSpawnerData.ObjectsAmount;
			}
			if (list.Count != num2)
			{
				return false;
		...
		foreach (BaseSpawnerData baseSpawnerData2 in array)
		{
			...
			if (list.Count != baseSpawnerData2.ObjectsAmount)
			{
				return false;
			}
			SpecialBuildings.AddRange(list);
	...
	private bool SpawnObelisks()
	{
		...
		if (Obelisks != null)
		{
			return Obelisks.Count == obelisksSpawnerData.ObjectsAmount;
		}
		return false;
	}
	...
	private void SpawnTrees()
	{
		...
		for (int i = 0; i < grid.GetGridSize().x; i++)
		{
			for (int j = 0; j < grid.GetGridSize().y; j++)
			{
				...
				if ((float5.x <= treeClusterMaxSizeBias && UnityEngine.Random.value < Mathf.Lerp(treeClustesOuterDensity, treeClustesInnerDensity, 1f - float5.x / treeClusterMaxSizeBias)) || (float5.x > treeClusterMaxSizeBias && UnityEngine.Random.value < sparseTreeDensity))
				{
		...
		if (!allowAdjacentTrees)
		{
			for (int num2 = gameObject.transform.childCount - 1; num2 >= 0; num2--)
			{
				...
				if (num3 > 0 && UnityEngine.Random.value < removeAdjacentTreeProbability)
				{
					UnityEngine.Object.DestroyImmediate(child.gameObject);
				}
				else
				{
					child.gameObject.GetComponent<SaveComponent>().Id = "Tree_" + num2;
				}
			}
		}
		stopwatch.Stop();
