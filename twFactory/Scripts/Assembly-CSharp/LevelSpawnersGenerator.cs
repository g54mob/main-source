...
public class LevelSpawnersGenerator : MonoBehaviour, ISavable
{
	...
	public struct UnlockableCycleGroup
	{
		...
		{
			if (unlockedBy == null || LTFunctionLibrary.GetLevelsProgressionManager().IsLevelUnlocked(unlockedBy.Id))
			{
				if (!(lockedBy == null))
				{
					return !LTFunctionLibrary.GetLevelsProgressionManager().IsLevelUnlocked(lockedBy.Id);
				}
				return true;
			}
			return false;
	...
	private List<FEnemyInfo> GetUnlockedEnemies()
	{
		...
		{
			if (LTFunctionLibrary.GetLevelsProgressionManager().IsLevelUnlocked(availableEnemy.levelToUnlock.Id))
			{
	...
	private List<FBossGroup> GetUnlockedBosses()
	{
		...
		{
			if (LTFunctionLibrary.GetLevelsProgressionManager().IsLevelUnlocked(availableBossGroup.unlockedBy.Id))
			{
