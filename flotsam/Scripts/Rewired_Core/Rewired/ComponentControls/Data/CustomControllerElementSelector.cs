...
namespace Rewired.ComponentControls.Data
{
	...
	public sealed class CustomControllerElementSelector
	{
		...
		[HideInInspector]
		private int jMTuBbNIFTWXSxUCkvPlHNodROaH = -1;

		[HideInInspector]
		private int ROfFdgCVxeIeeTMpaIsJAnZlNBkyA = -1;
		...
		public int GetElementIndex(Rewired.CustomController customController)
		{
			...
			if (jMTuBbNIFTWXSxUCkvPlHNodROaH >= 0 && jMTuBbNIFTWXSxUCkvPlHNodROaH != customController.id)
			{
				ClearCache();
			}
			if (ROfFdgCVxeIeeTMpaIsJAnZlNBkyA >= 0)
			{
				return ROfFdgCVxeIeeTMpaIsJAnZlNBkyA;
			}
			jMTuBbNIFTWXSxUCkvPlHNodROaH = customController.id;
			switch (_selectorType)
			{
			case SelectorType.Id:
			{
				IList<ControllerElementIdentifier> list = RYDOqKhPoAcDoBxafWtpmMpioTWjA(customController, _elementType);
				for (int j = 0; j < list.Count; j++)
				{
					if (list[j].Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Eid == _elementId)
					{
						ROfFdgCVxeIeeTMpaIsJAnZlNBkyA = j;
						break;
					}
				}
				break;
			...
			case SelectorType.Index:
			{
				...
				ROfFdgCVxeIeeTMpaIsJAnZlNBkyA = _elementIndex;
				break;
			}
			case SelectorType.Name:
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					{
						ROfFdgCVxeIeeTMpaIsJAnZlNBkyA = i;
						break;
					}
				}
				break;
		...
		private IList<ControllerElementIdentifier> RYDOqKhPoAcDoBxafWtpmMpioTWjA(Rewired.CustomController P_0, ElementType P_1)
		{
			return P_1 switch
			{
		...
		{
			jMTuBbNIFTWXSxUCkvPlHNodROaH = -1;
			ROfFdgCVxeIeeTMpaIsJAnZlNBkyA = -1;
		}
	}
}
