...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal HiJTMJvhgQklUObpnWHEIWyMFAFg sfziLequFEeAtcaIUCRtEdehIdnab()
			{
				return new HiJTMJvhgQklUObpnWHEIWyMFAFg(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.VRUPdzKgeveqVvQabkIaYFoBcpSf VHwazEanfUAOnVtvxksxbTlFvmoLb()
			{
				return new ControllerMapLayoutManager.VRUPdzKgeveqVvQabkIaYFoBcpSf(_enabled, _loadFromUserDataStore, lSOfmSfFDazyqhcHwmkrsNHeVbaK());
			}

			private UGcaqZCDBFRgWVBlZOOhcpgBRMFkc[] lSOfmSfFDazyqhcHwmkrsNHeVbaK()
			{
				List<UGcaqZCDBFRgWVBlZOOhcpgBRMFkc> list = new List<UGcaqZCDBFRgWVBlZOOhcpgBRMFkc>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].yrLvDmjtggosCJeADufaUNfTwxiq());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.LoufCxnWRPkzSbBIKyhZshokUpWL BXBulpYKnSzHfiOUULhyuBUUjpKo()
			{
				return new ControllerMapEnabler.LoufCxnWRPkzSbBIKyhZshokUpWL(_enabled, kBbmKHcnsNwlDQFjedssHpGvcsTAA());
			}

			private UGcaqZCDBFRgWVBlZOOhcpgBRMFkc[] kBbmKHcnsNwlDQFjedssHpGvcsTAA()
			{
				List<UGcaqZCDBFRgWVBlZOOhcpgBRMFkc> list = new List<UGcaqZCDBFRgWVBlZOOhcpgBRMFkc>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal UGcaqZCDBFRgWVBlZOOhcpgBRMFkc yrLvDmjtggosCJeADufaUNfTwxiq()
			{
				return new UGcaqZCDBFRgWVBlZOOhcpgBRMFkc(_id, _enabled);
			}
		...
		internal ReblhCinFkWhDVFLEbjmzIdfVvaS xXNrcfAIdFvJZBPEZBBSuJbFyZXc()
		{
			HiJTMJvhgQklUObpnWHEIWyMFAFg[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new HiJTMJvhgQklUObpnWHEIWyMFAFg[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].sfziLequFEeAtcaIUCRtEdehIdnab();
				}
			}
			HiJTMJvhgQklUObpnWHEIWyMFAFg[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new HiJTMJvhgQklUObpnWHEIWyMFAFg[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].sfziLequFEeAtcaIUCRtEdehIdnab();
				}
			}
			HiJTMJvhgQklUObpnWHEIWyMFAFg[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new HiJTMJvhgQklUObpnWHEIWyMFAFg[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].sfziLequFEeAtcaIUCRtEdehIdnab();
				}
			}
			HiJTMJvhgQklUObpnWHEIWyMFAFg[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new HiJTMJvhgQklUObpnWHEIWyMFAFg[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].sfziLequFEeAtcaIUCRtEdehIdnab();
				}
			}
			return new ReblhCinFkWhDVFLEbjmzIdfVvaS(array, array2, array3, array4);
		}
	}
}
