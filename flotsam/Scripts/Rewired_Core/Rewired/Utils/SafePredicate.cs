...
namespace Rewired.Utils
{
	...
	{
		private T HmXdGcEjWnppkPBCeaklCkuHvkyab;

		private bool DgTqLXhIAtNHTmXQdwrMDjdPogPo;

		private static Action<object, Predicate<T>> bICIGRESjuPtaJDWXzoXcINvziZI;

		private static Action<object, Predicate<T>> invokeDelegate => OmbmlyDzxSIJFVgmseLVInScIVCCb;
		...
		{
			HmXdGcEjWnppkPBCeaklCkuHvkyab = arg0;
			try
			{
				Invoke(invokeDelegate);
				return DgTqLXhIAtNHTmXQdwrMDjdPogPo;
			}
			catch
			{
			...
			{
				HmXdGcEjWnppkPBCeaklCkuHvkyab = default(T);
				DgTqLXhIAtNHTmXQdwrMDjdPogPo = false;
			}
		}
		...
		private static void OmbmlyDzxSIJFVgmseLVInScIVCCb(object P_0, Predicate<T> P_1)
		{
			if (P_1 != null && P_0 is SafePredicate<T> safePredicate)
			{
				safePredicate.DgTqLXhIAtNHTmXQdwrMDjdPogPo = P_1(safePredicate.HmXdGcEjWnppkPBCeaklCkuHvkyab);
			}
		}
