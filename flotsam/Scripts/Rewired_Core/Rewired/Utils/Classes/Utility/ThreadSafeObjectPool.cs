...
namespace Rewired.Utils.Classes.Utility
{
	...
	{
		private const int yxmaAWBVCxasZijQdjHuflUbrxybb = 1;

		private const int nAVTBuGtuonsVHsIFrqLirUbVLmF = 0;
		...
		private ulong wOqIvjJLTEAHAioZCunUXxScBavf;

		private int CcyFWhKoUcvPgCKiFrcYpNJxSCmJ;

		protected ulong InstanceCount => wOqIvjJLTEAHAioZCunUXxScBavf;
		...
		{
			while (Interlocked.Exchange(ref CcyFWhKoUcvPgCKiFrcYpNJxSCmJ, 1) != 0)
			{
				Thread.SpinWait(1);
			}
			_pool.Clear();
			...
			Interlocked.Exchange(ref CcyFWhKoUcvPgCKiFrcYpNJxSCmJ, 0);
		}
		...
		private object lrvcubMDfevNkSgCWxmRgOKoLBKI()
		{
			return Get();
		}
		...
		{
			//ILSpy generated this explicit interface implementation from .override directive in lrvcubMDfevNkSgCWxmRgOKoLBKI
			return this.lrvcubMDfevNkSgCWxmRgOKoLBKI();
		}

		private bool IeFCEqeqUNAgeHCyerVvoymJPvSSA(object P_0)
		{
		...
		{
			//ILSpy generated this explicit interface implementation from .override directive in IeFCEqeqUNAgeHCyerVvoymJPvSSA
			return this.IeFCEqeqUNAgeHCyerVvoymJPvSSA(P_0);
		}
		...
		{
			wOqIvjJLTEAHAioZCunUXxScBavf = ((wOqIvjJLTEAHAioZCunUXxScBavf < ulong.MaxValue) ? (wOqIvjJLTEAHAioZCunUXxScBavf + 1) : 0);
			return wOqIvjJLTEAHAioZCunUXxScBavf;
		}
	}
}
