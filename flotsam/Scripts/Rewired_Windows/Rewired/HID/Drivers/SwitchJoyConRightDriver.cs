...
namespace Rewired.HID.Drivers
{
	...
	{
		private const int VDtEsGNgagEqxSEHicJlgzGRlLMU = 3;

		private const int TyEAUETWUkXEPKTUkDyUQtdxJCdO = 9;

		int NintendoSwitchJoyConDriver.byteIndexStartSticks => 9;

		public SwitchJoyConRightDriver(InitArgs P_0)
			: base(P_0, NMOoxbNrRRsluLpmhhjPhxWOwZVpA.JoyConRight)
		{
		}
		...
		protected override void UpdateButtons(NativeBuffer inputReport, double timestamp)
		{
			...
			buttons[1].AtQsHqTAryodwUVQnJukddZkgqvd((b & 2) != 0, timestamp);
			buttons[2].AtQsHqTAryodwUVQnJukddZkgqvd((b & 4) != 0, timestamp);
			buttons[3].AtQsHqTAryodwUVQnJukddZkgqvd((b & 1) != 0, timestamp);
			...
			b = array[1];
			buttons[8].AtQsHqTAryodwUVQnJukddZkgqvd((b & 2) != 0, timestamp);
			buttons[9].AtQsHqTAryodwUVQnJukddZkgqvd((b & 0x10) != 0, timestamp);
			buttons[10].AtQsHqTAryodwUVQnJukddZkgqvd((b & 4) != 0, timestamp);
		}
