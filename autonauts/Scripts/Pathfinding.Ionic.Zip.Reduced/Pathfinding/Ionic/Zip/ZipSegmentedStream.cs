...
namespace Pathfinding.Ionic.Zip
{
	internal class ZipSegmentedStream : Stream
	{
		...
		public override bool CanRead
		{
			get
			{
				return rwMode == RwMode.ReadOnly && _innerStream != null && _innerStream.CanRead;
			}
		}
		...
		public override bool CanSeek
		{
			get
			{
				return _innerStream != null && _innerStream.CanSeek;
			}
		}
		...
		public override bool CanWrite
		{
			get
			{
				return rwMode == RwMode.Write && _innerStream != null && _innerStream.CanWrite;
			}
		}
		...
		public static ZipSegmentedStream ForWriting(string name, int maxSegmentSize)
		{
			...
			ZipSegmentedStream zipSegmentedStream2 = zipSegmentedStream;
			if (zipSegmentedStream2._baseDir == string.Empty)
			{
		...
		public static Stream ForUpdate(string name, uint diskNumber)
		{
			...
			string path = string.Format("{0}.z{1:D2}", Path.Combine(Path.GetDirectoryName(name), Path.GetFileNameWithoutExtension(name)), diskNumber + 1);
		...
		private string _NameForSegment(uint diskNumber)
		{
			...
			return string.Format("{0}.z{1:D2}", Path.Combine(Path.GetDirectoryName(_baseName), Path.GetFileNameWithoutExtension(_baseName)), diskNumber + 1);
		}
		...
		protected override void Dispose(bool disposing)
		{
			try
			{
				...
				{
					_innerStream.Dispose();
					if (rwMode == RwMode.Write && !_exceptionPending)
					{
					}
				}
			}
