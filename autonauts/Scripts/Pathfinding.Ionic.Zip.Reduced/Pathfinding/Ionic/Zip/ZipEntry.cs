...
namespace Pathfinding.Ionic.Zip
{
	...
	public class ZipEntry
	{
		...
		internal bool AttributesIndicateDirectory
		{
			get
			{
				return _InternalFileAttrs == 0 && (_ExternalFileAttrs & 0x10) == 16;
			}
		}
		...
		public DateTime LastModified
		{
			...
			set
			{
				_LastModified = ((value.Kind != DateTimeKind.Unspecified) ? value.ToLocalTime() : DateTime.SpecifyKind(value, DateTimeKind.Local));
		...
		public bool UseUnicodeAsNecessary
		{
			get
			{
				return AlternateEncoding == Encoding.GetEncoding("UTF-8") && AlternateEncodingUsage == ZipOption.AsNecessary;
			}
			set
			{
				if (value)
		...
		internal static ZipEntry ReadDirEntry(ZipFile zf, Dictionary<string, object> previouslySeen)
		{
			...
			zipEntry.AlternateEncoding = (((zipEntry._BitField & 0x800) != 2048) ? encoding : Encoding.UTF8);
		...
		internal CrcCalculatorStream InternalOpenReader(string password)
		{
			...
			long length = ((_CompressionMethod_FromZipFile != 0) ? UncompressedSize : _CompressedFileDataSize);
		...
		private int ExtractOne(Stream output)
		{
			...
			try
			{
				...
				long num2 = ((_CompressionMethod_FromZipFile == 0) ? _CompressedFileDataSize : UncompressedSize);
				_inputDecryptorStream = GetExtractDecryptor(archiveStream);
				Stream extractDecompressor = GetExtractDecompressor(_inputDecryptorStream);
				long num3 = 0L;
				using (CrcCalculatorStream crcCalculatorStream = new CrcCalculatorStream(extractDecompressor))
				{
					while (num2 > 0)
					{
						int count = (int)((num2 <= array.Length) ? num2 : array.Length);
		...
		internal void WriteCentralDirectoryEntry(Stream s)
		{
			...
			short num2 = (short)((VersionNeeded == 0) ? 20 : VersionNeeded);
			bool? outputUsesZip = _OutputUsesZip64;
			if (!outputUsesZip.HasValue)
			{
				_OutputUsesZip64 = _container.Zip64 == Zip64Option.Always;
			}
			short num3 = (short)((!_OutputUsesZip64.Value) ? num2 : 45);
		...
		internal void WriteHeader(Stream s, int cycle)
		{
			...
			_future_ROLH = ((countingStream == null) ? s.Position : countingStream.ComputedPosition);
			int num = 0;
			int num2 = 0;
			byte[] array = new byte[30];
			array[num2++] = 80;
			...
			short num3 = (short)((!_presumeZip64) ? 20 : 45);
		...
		private void PrepSourceStream()
		{
			...
			long? sourceStreamOriginalPosition = _sourceStreamOriginalPosition;
		...
		private long SetInputAndFigureFileLength(ref Stream input)
		{
			...
			{
				string password = ((_Encryption_FromZipFile != EncryptionAlgorithm.None) ? (_Password ?? _container.Password) : null);
		...
		private void CopyThroughWithRecompute(Stream outstream)
		{
			...
			if (!FileName.EndsWith("/"))
			{
				...
				while (num2 > 0)
				{
					lengthOfCryptoHeaderBytes = (int)((num2 <= array.Length) ? num2 : array.Length);
		...
		private void CopyThroughWithNoChange(Stream outstream)
		{
			...
			_RelativeOffsetOfLocalHeader = ((countingStream2 == null) ? outstream.Position : countingStream2.ComputedPosition);
