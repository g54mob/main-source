	...
	public class HierarchicalGraph
	{
		...
		internal void AddDirtyNode(GraphNode node)
		{
			...
			catch
			{
				numDirtyNodes--;
				int num = 0;
				for (int num2 = numDirtyNodes - 1; num2 >= 0; num2--)
				{
					if (dirtyNodes[num2].Destroyed)
					{
						numDirtyNodes--;
						dirty[dirtyNodes[num2].HierarchicalNodeIndex] = 1;
						dirtyNodes[num2] = dirtyNodes[numDirtyNodes];
						dirtyNodes[numDirtyNodes] = null;
					}
					else
					{
						num = Math.Max(num, dirtyNodes[num2].NodeIndex);
					}
				}
				if (numDirtyNodes >= dirtyNodes.Length)
				{
					throw new Exception("Failed to compactify dirty nodes array. This should not happen. " + num + " " + numDirtyNodes + " " + dirtyNodes.Length);
				}
				AddDirtyNode(node);
			}
		}
		...
		private void RemoveHierarchicalNode(int hierarchicalNode, bool removeAdjacentSmallNodes)
		{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (dirty[num] == 0)
				{
					...
					{
						dirty[num] = 2;
						RemoveHierarchicalNode(num, false);
					}
					else
					{
