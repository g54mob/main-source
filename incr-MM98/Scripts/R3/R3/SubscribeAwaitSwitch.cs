...
namespace R3
{
	internal sealed class SubscribeAwaitSwitch<T> : AwaitOperationSwitchObserver<T>
	{
		public SubscribeAwaitSwitch(Func<T, CancellationToken, ValueTask> onNextAsync, Action<Exception> onErrorResume, Action<Result> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
	...
	internal sealed class SubscribeAwaitSwitch<T, TState> : AwaitOperationSwitchObserver<T>
	{
		public SubscribeAwaitSwitch(TState state, Func<T, TState, CancellationToken, ValueTask> onNextAsync, Action<Exception, TState> onErrorResume, Action<Result, TState> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
