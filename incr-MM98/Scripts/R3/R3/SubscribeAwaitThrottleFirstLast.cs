...
namespace R3
{
	internal sealed class SubscribeAwaitThrottleFirstLast<T> : AwaitOperationThrottleFirstLastObserver<T>
	{
		public SubscribeAwaitThrottleFirstLast(Func<T, CancellationToken, ValueTask> onNextAsync, Action<Exception> onErrorResume, Action<Result> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
	...
	internal sealed class SubscribeAwaitThrottleFirstLast<T, TState> : AwaitOperationThrottleFirstLastObserver<T>
	{
		public SubscribeAwaitThrottleFirstLast(TState state, Func<T, TState, CancellationToken, ValueTask> onNextAsync, Action<Exception, TState> onErrorResume, Action<Result, TState> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
