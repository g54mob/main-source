...
namespace R3
{
	internal sealed class ObservePropertyChanged<T, TProperty> : Observable<TProperty> where T : INotifyPropertyChanged
	{
		private sealed class _ObservePropertyChanged : IDisposable
		{
			...
			public _ObservePropertyChanged(Observer<TProperty> observer, T value, Func<T, TProperty> propertySelector, string propertyName, CancellationToken cancellationToken)
			{
				...
				if (cancellationToken.CanBeCanceled)
				{
					...
					{
						((_ObservePropertyChanged)state).CompleteDispose();
					}, this);
				}
			}

			private void PublishOnNext(object? sender, PropertyChangedEventArgs e)
			{
			...
			public void Dispose()
			{
				if (Interlocked.Exchange(ref eventHandler, null) != null)
				{
					...
					T val = value;
					val.PropertyChanged -= eventHandler;
				}
			}
		}

		public ObservePropertyChanged(T value, Func<T, TProperty> propertySelector, string propertyName, bool pushCurrentValueOnSubscribe, CancellationToken cancellationToken)
		{
		...
		protected override IDisposable SubscribeCore(Observer<TProperty> observer)
		{
			...
			return new _ObservePropertyChanged(observer, _003Cvalue_003EP, _003CpropertySelector_003EP, _003CpropertyName_003EP, _003CcancellationToken_003EP);
		}
	}
}
