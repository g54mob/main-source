...
namespace R3
{
	internal sealed class SubscribeAwaitParallel<T> : AwaitOperationParallelObserver<T>
	{
		public SubscribeAwaitParallel(Func<T, CancellationToken, ValueTask> onNextAsync, Action<Exception> onErrorResume, Action<Result> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
	...
	internal sealed class SubscribeAwaitParallel<T, TState> : AwaitOperationParallelObserver<T>
	{
		public SubscribeAwaitParallel(TState state, Func<T, TState, CancellationToken, ValueTask> onNextAsync, Action<Exception, TState> onErrorResume, Action<Result, TState> onCompleted, bool configureAwait, bool cancelOnCompleted)
		{
