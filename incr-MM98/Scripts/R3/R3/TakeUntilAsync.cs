...
namespace R3
{
	internal sealed class TakeUntilAsync<T> : Observable<T>
	{
		private sealed class _TakeUntil : Observer<T>, IDisposable
		{
			...
			public _TakeUntil(Observer<T> observer, Func<T, CancellationToken, ValueTask> asyncFunc, bool configureAwait)
			{
		...
		public TakeUntilAsync(Observable<T> source, Func<T, CancellationToken, ValueTask> asyncFunc, bool configureAwait)
		{
		...
		{
			return _003Csource_003EP.Subscribe(new _TakeUntil(observer, _003CasyncFunc_003EP, _003CconfigureAwait_003EP));
		}
	}
}
