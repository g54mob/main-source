...
public static class PersistentMessagePipe
{
	private static IServiceProvider provider;

	private static EventFactory eventFactory;

	private static MessagePipeDiagnosticsInfo diagnosticsInfo;
	...
	public static void SetProvider(IServiceProvider newProvider)
	{
		provider = newProvider;
		eventFactory = newProvider.GetRequiredService<EventFactory>();
		diagnosticsInfo = newProvider.GetRequiredService<MessagePipeDiagnosticsInfo>();
	}
	...
	private static void ThrowIfNotInitialized()
	{
		...
		{
			throw new InvalidOperationException("Require to call `SetProvider` before use PersistentMessagePipe.");
		}
	}
}
