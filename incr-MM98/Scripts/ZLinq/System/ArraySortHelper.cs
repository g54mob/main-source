...
namespace System
{
	internal sealed class ArraySortHelper<T>
	{
		...
		public static void Sort(Span<T> keys, IComparer<T>? comparer)
		{
			...
			{
				throw new InvalidOperationException("compare failed", innerException);
			}
		}
		...
		private static void IntroSort(Span<T> keys, int depthLimit, Comparison<T> comparer)
		{
			...
			while (num > 1)
			{
				...
				int num3 = num2 + 1;
				IntroSort(keys.Slice(num3, num - num3), depthLimit, comparer);
				num = num2;
			}
		}
	...
	internal static class ArraySortHelper<TKey, TValue>
	{
		...
		private static void IntroSort(Span<TKey> keys, Span<TValue> values, int depthLimit, IComparer<TKey> comparer)
		{
			...
			while (num > 1)
			{
				...
				int num3 = num2 + 1;
				Span<TKey> keys2 = keys.Slice(num3, num - num3);
				num3 = num2 + 1;
				IntroSort(keys2, values.Slice(num3, num - num3), depthLimit, comparer);
				num = num2;
			}
		}
