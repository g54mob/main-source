...
namespace ZLinq.Linq
{
	...
	public struct SelectMany4<TEnumerator, TEnumerator2, TSource, TCollection, TResult> : IValueEnumerator<TResult>, IDisposable where TEnumerator : struct, IValueEnumerator<TSource> where TEnumerator2 : struct, IValueEnumerator<TCollection>
	{
		...
		public bool TryGetNext(out TResult current)
		{
			while (true)
			{
				...
				innerEnumerator = _003CcollectionSelector_003EP(current3, index++).Enumerator;
				hasInner = true;
			}
	...
	[StructLayout(LayoutKind.Auto)]
	[EditorBrowsable(EditorBrowsableState.Never)]
	public struct SelectMany4<TEnumerator, TSource, TCollection, TResult> : IValueEnumerator<TResult>, IDisposable where TEnumerator : struct, IValueEnumerator<TSource>
	{
		private TEnumerator source;
		...
		public bool TryGetNext(out TResult current)
		{
			while (true)
			{
				...
				currentSource = current3;
				innerEnumerator = _003CcollectionSelector_003EP(current3, index++).AsValueEnumerable().Enumerator;
				hasInner = true;
			}
			Unsafe.SkipInit<TResult>(out current);
			return false;
