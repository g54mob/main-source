...
namespace ZLinq.Linq
{
	...
	public struct FromInt16Sequence : IValueEnumerator<short>, IDisposable
	{
		private bool calledGetNext;

		public FromInt16Sequence(short currentValue, short endInclusive, short step, bool isIncrement)
		{
		...
		public bool TryGetNext(out short current)
		{
			...
			{
				short num = (short)(_003CcurrentValue_003EP + _003Cstep_003EP);
			...
			short num2 = (short)(_003CcurrentValue_003EP + _003Cstep_003EP);
		...
		public bool TryCopyTo([ScopedRef] Span<short> destination, Index offset)
		{
			...
			{
				FillIncremental(destination.Slice(0, count2), (short)(_003CcurrentValue_003EP + (short)start));
				return true;
			}
			return false;
		}
		...
		{
			if (_003Cstep_003EP == 1 && _003CendInclusive_003EP - _003CcurrentValue_003EP + 1 <= 32767)
			{
				return true;
			}
			return false;
		...
		private static void FillIncremental(Span<short> span, short start)
		{
			ref short reference = ref MemoryMarshal.GetReference(span);
			ref short right = ref Unsafe.Add(ref reference, span.Length);
