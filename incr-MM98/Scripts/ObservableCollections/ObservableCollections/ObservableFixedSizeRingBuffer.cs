...
{
	public class ObservableFixedSizeRingBuffer<T> : IList<T>, ICollection<T>, IEnumerable<T>, IEnumerable, IReadOnlyList<T>, IReadOnlyCollection<T>, IObservableCollection<T>
	{
		...
		public void AddFirst(T item)
		{
			lock (SyncRoot)
			{
				...
				if (capacity == buffer.Count)
				{
					...
					NotifyCollectionChangedEventHandler<T>? notifyCollectionChangedEventHandler = this.CollectionChanged;
					if (notifyCollectionChangedEventHandler != null)
					{
						e = NotifyCollectionChangedEventArgs<T>.Remove(oldItem, capacity - 1);
						notifyCollectionChangedEventHandler(in e);
					}
				}
				buffer.AddFirst(item);
				NotifyCollectionChangedEventHandler<T>? notifyCollectionChangedEventHandler2 = this.CollectionChanged;
				if (notifyCollectionChangedEventHandler2 != null)
				{
					e = NotifyCollectionChangedEventArgs<T>.Add(item, 0);
					notifyCollectionChangedEventHandler2(in e);
				}
			}
		}
