...
namespace UImGui
{
	...
	internal struct FontConfig
	{
		...
		[Tooltip("Rasterize at higher quality for sub-pixel positioning. Note the difference between 2 and 3 is minimal so you can reduce this to 2 to save memory. Read https://github.com/nothings/stb/blob/master/tests/oversample/README.md for details. (default=3)")]
		public int OversampleH;

		[Tooltip("Rasterize at higher quality for sub-pixel positioning. This is not really useful as we don't use sub-pixel positions on the Y axis. (default=1)")]
		public int OversampleV;

		[Tooltip("Align every glyph to pixel boundary. Useful e.g. if you are merging a non-pixel aligned font with the default font.  (default=false)If enabled, you can set OversampleH/V to 1.")]
		...
		[Tooltip("Merge into previous ImFont, so you can combine multiple inputs font into one ImFont (e.g. ASCII font + icons + Japanese glyphs). You may want to use GlyphOffset.y when merge font of different heights. (default=false)")]
		...
		[Tooltip("Brighten (>1.0f) or darken (<1.0f) font output. Brightening small fonts may be a good workaround to make them more readable. (default=1.0f)")]
		public float RasterizerMultiply;

		[Tooltip("Explicitly specify unicode codepoint of ellipsis character. When fonts are being merged first specified ellipsis will be used. (default=-1)")]
		...
		public unsafe void SetDefaults()
		{
			...
			SetFrom(ptr);
			ImGuiNative.ImFontConfig_destroy(ptr);
		}
		...
		{
			ImFontAtlas* self = null;
			List<ushort> ranges = new List<ushort>();
			if ((GlyphRanges & ScriptGlyphRanges.Default) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesDefault(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Cyrillic) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesCyrillic(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Japanese) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesJapanese(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Korean) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesKorean(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Thai) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesThai(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Vietnamese) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesVietnamese(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.ChineseSimplified) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesChineseSimplifiedCommon(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.ChineseFull) != 0)
			{
				AddRangePtr(ImGuiNative.ImFontAtlas_GetGlyphRangesChineseFull(self));
			}
			if ((GlyphRanges & ScriptGlyphRanges.Custom) != 0)
			{
				Range[] customGlyphRanges = CustomGlyphRanges;
				for (int i = 0; i < customGlyphRanges.Length; i++)
				{
					Range range = customGlyphRanges[i];
					ranges.AddRange(new ushort[2] { range.Start, range.End });
				}
			}
			return ranges;
			unsafe void AddRangePtr(ushort* r)
			{
				while (*r != 0)
				{
					ranges.Add(*(r++));
