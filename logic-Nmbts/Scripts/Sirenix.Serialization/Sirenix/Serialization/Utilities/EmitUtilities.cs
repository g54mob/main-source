...
namespace Sirenix.Serialization.Utilities
{
	internal static class EmitUtilities
	{
		...
		public static Action<object> CreateWeakInstanceMethodCaller(MethodInfo methodInfo)
		{
			...
			if ((object)methodInfo.ReturnType != null && (object)methodInfo.ReturnType != typeof(void))
			{
		...
		public static InstanceRefMethodCaller<InstanceType> CreateInstanceRefMethodCaller<InstanceType>(MethodInfo methodInfo)
		{
			...
			Type declaringType = methodInfo.DeclaringType;
			string name = methodInfo.ReflectedType.FullName + ".call_" + methodInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(void), new Type[1] { typeof(InstanceType).MakeByRefType() }, true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			if (declaringType.IsValueType)
			{
				iLGenerator.Emit(OpCodes.Ldarg_0);
