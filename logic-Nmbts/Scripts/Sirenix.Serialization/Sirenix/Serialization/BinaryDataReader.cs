	...
	public class BinaryDataReader : BaseDataReader
	{
		...
		public override bool EnterNode(out Type type)
		{
			...
			if (peekedBinaryEntryType == BinaryEntryType.NamedStartOfReferenceNode || peekedBinaryEntryType == BinaryEntryType.UnnamedStartOfReferenceNode)
			{
				...
				type = ReadTypeEntry();
				int value;
				if (!UNSAFE_Read_4_Int32(out value))
				{
					type = null;
					return false;
				}
		...
		public unsafe override bool ReadPrimitiveArray<T>(out T[] array)
		{
			...
			if (peekedEntryType == EntryType.PrimitiveArray)
			{
				...
				int value;
				int value2;
				if (!UNSAFE_Read_4_Int32(out value) || !UNSAFE_Read_4_Int32(out value2))
				{
					array = null;
					return false;
				}
		...
		public override bool ReadInt64(out long value)
		{
			...
			if (peekedEntryType == EntryType.Integer)
			{
				try
				{
					...
					{
						byte value6;
						if (!UNSAFE_Read_1_Byte(out value6))
						{
							value = 0L;
							return false;
						}
					...
					{
						short value7;
						if (!UNSAFE_Read_2_Int16(out value7))
						{
							value = 0L;
							return false;
						}
					...
					{
						ushort value4;
						if (!UNSAFE_Read_2_UInt16(out value4))
						{
							value = 0L;
							return false;
						}
					...
					{
						int value5;
						if (!UNSAFE_Read_4_Int32(out value5))
						{
							value = 0L;
							return false;
						}
					...
					{
						uint value3;
						if (!UNSAFE_Read_4_UInt32(out value3))
						{
							value = 0L;
							return false;
						}
					...
					{
						ulong value2;
						if (!UNSAFE_Read_8_UInt64(out value2))
						{
							value = 0L;
							return false;
						}
		...
		public override bool ReadUInt64(out ulong value)
		{
			...
			if (peekedEntryType == EntryType.Integer)
			{
				try
				{
					...
					{
						byte value7;
						if (!UNSAFE_Read_1_Byte(out value7))
						{
							value = 0uL;
							return false;
						}
					...
					{
						short value4;
						if (!UNSAFE_Read_2_Int16(out value4))
						{
							value = 0uL;
							return false;
						}
					...
					{
						ushort value6;
						if (!UNSAFE_Read_2_UInt16(out value6))
						{
							value = 0uL;
							return false;
						}
					...
					{
						uint value2;
						if (!UNSAFE_Read_4_UInt32(out value2))
						{
							value = 0uL;
							return false;
						}
					...
					{
						long value3;
						if (!UNSAFE_Read_8_Int64(out value3))
						{
							value = 0uL;
							return false;
						}
		...
		public override bool ReadSingle(out float value)
		{
			...
			if (peekedBinaryEntryType == BinaryEntryType.NamedDouble || peekedBinaryEntryType == BinaryEntryType.UnnamedDouble)
			{
				MarkEntryContentConsumed();
				double value2;
				if (!UNSAFE_Read_8_Float64(out value2))
				{
					value = 0f;
					return false;
				}
			...
			if (peekedBinaryEntryType == BinaryEntryType.NamedDecimal || peekedBinaryEntryType == BinaryEntryType.UnnamedDecimal)
			{
				MarkEntryContentConsumed();
				decimal value3;
				if (!UNSAFE_Read_16_Decimal(out value3))
				{
					value = 0f;
					return false;
				}
		...
		public override bool ReadDouble(out double value)
		{
			...
			if (peekedBinaryEntryType == BinaryEntryType.NamedFloat || peekedBinaryEntryType == BinaryEntryType.UnnamedFloat)
			{
				MarkEntryContentConsumed();
				float value2;
				if (!UNSAFE_Read_4_Float32(out value2))
				{
					value = 0.0;
					return false;
				}
		...
		public override bool ReadDecimal(out decimal value)
		{
			...
			if (peekedBinaryEntryType == BinaryEntryType.NamedFloat || peekedBinaryEntryType == BinaryEntryType.UnnamedFloat)
			{
				MarkEntryContentConsumed();
				float value3;
				if (!UNSAFE_Read_4_Float32(out value3))
				{
					value = default(decimal);
					return false;
				}
		...
		public unsafe override string GetDataDump()
		{
			...
			else
			{
				...
				fixed (IntPtr* ptr = buffer)
				{
					...
					{
						UnsafeUtilities.MemoryCopy(ptr, to, array.Length);
					}
				}
			}
		...
		{
			byte value;
			if (!UNSAFE_Read_1_Byte(out value))
			{
				return null;
			}
			int value2;
		...
		private void SkipStringValue()
		{
			byte value;
			int value2;
			if (UNSAFE_Read_1_Byte(out value) && UNSAFE_Read_4_Int32(out value2))
			{
				if (value != 0)
				{
					value2 *= 2;
