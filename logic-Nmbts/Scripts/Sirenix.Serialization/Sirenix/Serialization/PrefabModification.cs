...
namespace Sirenix.Serialization
{
	public sealed class PrefabModification
	{
		...
		private void ApplyValue(UnityEngine.Object unityObject)
		{
			...
			if ((object)type != null && ReferencePaths != null && ReferencePaths.Count > 0)
			{
				for (int i = 0; i < ReferencePaths.Count; i++)
				{
					...
					try
					{
						...
						if (instanceFromPath != null && (object)instanceFromPath.GetType() == type)
						{
		...
		private void ApplyListLength(UnityEngine.Object unityObject)
		{
			...
			else if (typeof(IList).IsAssignableFrom(type))
			{
				...
				bool flag = (object)type2 != null && type2.IsValueType;
				int num = 0;
		...
		private static void ReplaceAllReferencesInGraph(object graph, object oldReference, object newReference, HashSet<object> processedReferences = null)
		{
			...
			for (int j = 0; j < serializableMembers.Length; j++)
			{
				...
				if (fieldInfo.FieldType.IsPrimitive || (object)fieldInfo.FieldType == typeof(SerializationData) || (object)fieldInfo.FieldType == typeof(string))
				{
					continue;
				}
				...
				if (!type.IsPrimitive && (object)type != typeof(SerializationData) && (object)type != typeof(string))
				{
