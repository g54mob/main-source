...
namespace Sirenix.Serialization
{
	public static class FormatterLocator
	{
		...
		static FormatterLocator()
		{
			...
			foreach (Assembly assembly in assemblies)
			{
				try
				{
					...
					if (name.StartsWith("System.") || name.StartsWith("UnityEngine") || name.StartsWith("UnityEditor") || name == "mscorlib" || ((assembly.GetName().Name == "Sirenix.Serialization.AOTGenerated" || assembly.IsDefined(typeof(EmittedAssemblyAttribute), true)) && EmitUtilities.CanEmit))
					{
						continue;
					}
					object[] array = assembly.SafeGetCustomAttributes(typeof(RegisterFormatterAttribute), true);
					...
					object[] array2 = assembly.SafeGetCustomAttributes(typeof(RegisterFormatterLocatorAttribute), true);
		...
		public static IFormatter GetFormatter(Type type, ISerializationPolicy policy)
		{
			...
			lock (LOCK)
			{
				if (!TypeFormatterMap.TryGetInnerValue(type, policy, out value))
				{
					try
					{
		...
		internal static List<IFormatter> GetAllCompatiblePredefinedFormatters(Type type, ISerializationPolicy policy)
		{
			...
			for (int i = 0; i < FormatterLocators.Count; i++)
			{
				try
				{
					...
					if (FormatterLocators[i].LocatorInstance.TryGetFormatter(type, FormatterLocationStep.BeforeRegisteredFormatters, policy, out formatter))
					{
						list.Add(formatter);
					}
				}
			...
			for (int j = 0; j < FormatterInfos.Count; j++)
			{
				...
				{
					Type[] inferredParams;
					if (formatterInfo.FormatterType.TryInferGenericParameters(out inferredParams, type))
					{
			...
			for (int k = 0; k < FormatterLocators.Count; k++)
			{
				try
				{
					...
					if (FormatterLocators[k].LocatorInstance.TryGetFormatter(type, FormatterLocationStep.AfterRegisteredFormatters, policy, out formatter2))
					{
						list.Add(formatter2);
					}
				}
		...
		{
			IFormatter value;
			if (!FormatterInstances.TryGetValue(type, out value))
			{
				try
				{
