			...
			get
			{
				if (everythingPolicy == null)
				{
					lock (LOCK)
					{
						if (everythingPolicy == null)
						{
							everythingPolicy = new CustomSerializationPolicy("OdinSerializerPolicies.Everything", true, delegate(MemberInfo member)
							{
								if (!(member is FieldInfo))
								{
									return false;
								}
								return member.IsDefined<OdinSerializeAttribute>(true) || !member.IsDefined<NonSerializedAttribute>(true);
							});
						}
					}
				}
				...
				if (unityPolicy == null)
				{
					lock (LOCK)
					{
						if (unityPolicy == null)
						{
							...
							unityPolicy = new CustomSerializationPolicy("OdinSerializerPolicies.Unity", true, delegate(MemberInfo member)
							{
								...
								if (member is FieldInfo && ((member as FieldInfo).IsPublic || (member.DeclaringType.IsNestedPrivate && member.DeclaringType.IsDefined<CompilerGeneratedAttribute>()) || ((object)tupleInterface != null && tupleInterface.IsAssignableFrom(member.DeclaringType))))
								{
									return true;
								}
			...
			get
			{
				if (strictPolicy == null)
				{
					lock (LOCK)
					{
						if (strictPolicy == null)
						{
							strictPolicy = new CustomSerializationPolicy("OdinSerializerPolicies.Strict", true, delegate(MemberInfo member)
							{
								if (member is PropertyInfo && !((PropertyInfo)member).IsAutoProperty())
								{
									return false;
