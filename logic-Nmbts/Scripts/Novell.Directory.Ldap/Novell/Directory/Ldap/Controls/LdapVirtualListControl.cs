...
namespace Novell.Directory.Ldap.Controls
{
	public class LdapVirtualListControl : LdapControl
	{
		...
		private void BuildTypedVLVRequest()
		{
			...
			m_vlvRequest.add(new Asn1Tagged(new Asn1Identifier(2, false, GREATERTHANOREQUAL), new Asn1OctetString(m_jumpTo), false));
			if (m_context != null)
			{
				m_vlvRequest.add(new Asn1OctetString(m_context));
			}
		...
		private void BuildIndexedVLVRequest()
		{
			...
			m_vlvRequest.add(new Asn1Tagged(new Asn1Identifier(2, true, BYOFFSET), asn1Sequence, false));
			if (m_context != null)
			{
				m_vlvRequest.add(new Asn1OctetString(m_context));
			}
