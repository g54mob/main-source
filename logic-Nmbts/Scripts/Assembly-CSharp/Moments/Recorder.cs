...
namespace Moments
{
	...
	public sealed class Recorder : MonoBehaviour
	{
		...
		public Action<Texture[]> OnPreProcessingDone;
		...
		public float EstimatedMemoryUse
		{
			get
			{
				return (float)m_FramePerSecond * m_BufferSize * (float)(m_Width * m_Height * 4) / 1048576f;
			}
		}
		...
		public void ComputeWidth()
		{
			if (m_AutoAspect)
			{
				m_Width = Mathf.RoundToInt((float)m_Height * GetComponent<Camera>().aspect);
			}
		}
		...
		private string GenerateFileName()
		{
			...
			string text = dateTime.ToString("yyyyMMddHHmmssffff");
			return "Nimbatus_GIF_" + text;
		}
		...
		private IEnumerator PreProcess(string filename)
		{
			...
			List<Texture> unityFrames = new List<Texture>(m_Frames.Count);
			...
			while (m_Frames.Count > 0)
			{
				...
				unityFrames.Add(renderTexture);
				GifFrame item = ToGifFrame(renderTexture, temp);
				frames.Add(item);
				yield return null;
			}
			Flush(temp);
			...
			{
				OnPreProcessingDone(unityFrames.ToArray());
			}
