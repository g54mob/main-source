...
namespace OxOD
{
	public class FileDialog : MonoBehaviour
	{
		...
		private string GetFileSizeText(long size)
		{
			...
			{
				return ((float)size / 1024f).ToString(text, CultureInfo.InvariantCulture) + " Kb";
			}
			if ((float)size / 1024f / 1024f < 1024f)
			{
				return ((float)size / 1024f / 1024f).ToString(text, CultureInfo.InvariantCulture) + " Mb";
			}
			return ((float)size / 1024f / 1024f / 1024f).ToString(text, CultureInfo.InvariantCulture) + " Gb";
		}
					...
					for (int num = 0; num < array.Length; num++)
					{
						if (maxSize > 0)
						{
							if (array[num].Length < maxSize)
							{
								...
								obj2.transform.SetParent(gameObject.transform, true);
						...
						else
						{
							...
							obj3.transform.SetParent(gameObject.transform, true);
							obj3.transform.localPosition = Vector3.zero;
							obj3.transform.localScale = new Vector3(1f, 1f, 1f);
							obj3.transform.localRotation = Quaternion.Euler(Vector3.zero);
				...
				for (int num2 = 0; num2 < files.Length; num2++)
				{
					if (maxSize > 0)
					{
						if (files[num2].Length < maxSize)
						{
							...
							obj4.transform.SetParent(gameObject.transform, true);
							obj4.transform.localPosition = Vector3.zero;
							obj4.transform.localScale = new Vector3(1f, 1f, 1f);
							obj4.transform.localRotation = Quaternion.Euler(Vector3.zero);
					...
					else
					{
						...
						obj5.transform.SetParent(gameObject.transform, true);
						obj5.transform.localPosition = Vector3.zero;
						obj5.transform.localScale = new Vector3(1f, 1f, 1f);
						obj5.transform.localRotation = Quaternion.Euler(Vector3.zero);
