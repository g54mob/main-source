...
namespace SharpJson
{
	internal class Lexer
	{
		...
		{
			float result;
			if (!float.TryParse(GetNumberString(), NumberStyles.Float, CultureInfo.InvariantCulture, out result))
			{
				return 0f;
			}
			return result;
		...
		{
			double result;
			if (!double.TryParse(GetNumberString(), NumberStyles.Any, CultureInfo.InvariantCulture, out result))
			{
				return 0.0;
			}
			return result;
