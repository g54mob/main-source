...
namespace tk2dRuntime.TileMap
{
	public static class ColliderBuilder2D
	{
		...
		public static void BuildPolygonCollidersForChunk(tk2dTileMap tileMap, SpriteChunk chunk, int baseX, int baseY)
		{
			...
			for (int j = 0; j < tileMap.partitionSizeY; j++)
			{
				float num2 = (float)((baseY + j) & 1) * x;
				for (int k = 0; k < tileMap.partitionSizeX; k++)
				{
					int rawTile = spriteIds[j * tileMap.partitionSizeX + k];
					int tileFromRawTile = BuilderUtil.GetTileFromRawTile(rawTile);
					Vector2 vector2 = new Vector2(vector.x * ((float)k + num2), vector.y * (float)j);
					...
					if (tk2dSpriteDefinition2.colliderType == tk2dSpriteDefinition.ColliderType.Box)
					{
						...
						{
							list3.Add(BuilderUtil.ApplySpriteVertexTileFlags(tileMap, tk2dSpriteDefinition2, array[l], flag, flag2, rot) + vector2);
						}
						list.Add(list3);
					}
					else
					{
						...
						foreach (tk2dCollider2DData tk2dCollider2DData2 in polygonCollider2D)
						{
							...
							{
								list2.Add(BuilderUtil.ApplySpriteVertexTileFlags(tileMap, tk2dSpriteDefinition2, pos, flag, flag2, rot) + vector2);
							}
			...
			while (num9 > 0)
			{
				int num10 = Mathf.Min(a, num9);
				PolygonCollider2D polygonCollider2D2 = chunk.gameObject.AddComponent<PolygonCollider2D>();
				polygonCollider2D2.pathCount = num10;
				for (int num11 = 0; num11 < num10; num11++)
				{
					polygonCollider2D2.SetPath(num11, list[num8 + num11].ToArray());
				}
				num8 += num10;
				num9 -= num10;
			}
			...
			for (int i = 0; i < tileMap.partitionSizeY; i++)
			{
				...
				for (int j = 0; j < tileMap.partitionSizeX; j++)
				{
					...
					else
					{
						...
						edgeCollider2D = tk2dSpriteDefinition2.polygonCollider2D;
						foreach (tk2dCollider2DData tk2dCollider2DData3 in edgeCollider2D)
						{
							count = list.Count;
							Vector2[] points = tk2dCollider2DData3.points;
							...
							int num6 = tk2dCollider2DData3.points.Length;
							if (flag3)
							{
