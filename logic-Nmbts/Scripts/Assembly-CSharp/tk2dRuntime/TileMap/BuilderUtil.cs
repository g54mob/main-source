	...
	public static class BuilderUtil
	{
		...
		public static void HideTileMapPrefabs(tk2dTileMap tileMap)
		{
			...
			for (int m = 0; m < tilePrefabsListCount; m++)
			{
				...
				if (!array[m])
				{
					int num = ((x2 >= 0 && x2 < tileMap.width && y2 >= 0 && y2 < tileMap.height) ? tileMap.GetTile(x2, y2, layer3) : (-1));
					if (num >= 0 && num < array2.Length && array2[num] != null)
					{
						array[m] = true;
					}
				}
		...
		public static void CreateRenderData(tk2dTileMap tileMap, bool editMode, Dictionary<Layer, bool> layersActive)
		{
			...
			foreach (Layer layer in layers)
			{
				...
				if (layer.gameObject != null)
				{
					...
					layer.gameObject.transform.localPosition = new Vector3(0f, 0f, tileMap.data.layersFixedZ ? (0f - z) : num);
