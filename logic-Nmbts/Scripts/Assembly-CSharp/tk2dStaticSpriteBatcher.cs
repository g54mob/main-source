...
public class tk2dStaticSpriteBatcher : MonoBehaviour, ISpriteCollectionForceBuild
{
		...
		if (flag)
		{
			...
			list.Sort((tk2dBatchedSprite a, tk2dBatchedSprite b) => a.parentId.CompareTo(b.parentId));
			{
				foreach (tk2dBatchedSprite item in list)
				{
					...
					item.relativeMatrix = ((item.parentId == -1) ? Matrix4x4.identity : batchedSprites[item.parentId].relativeMatrix) * matrix4x;
				}
				return;
			}
		}
	...
	private void BuildRenderMesh()
	{
		...
		foreach (tk2dBatchedSprite tk2dBatchedSprite4 in array)
		{
			...
			Vector3[] array8 = new Vector3[num4];
			Vector2[] array9 = new Vector2[num4];
			Vector2[] array10 = (flag3 ? new Vector2[num4] : null);
			Color32[] array11 = new Color32[num4];
			Vector3[] array12 = (flag ? new Vector3[num4] : null);
			Vector4[] array13 = (flag2 ? new Vector4[num4] : null);
			...
			case tk2dBatchedSprite.Type.TextMesh:
			{
				...
				if (!geomData.fontInst.isPacked)
				{
					...
					Color32 color2 = (tk2dTextMeshData4.useGradient ? tk2dTextMeshData4.color2 : tk2dTextMeshData4.color);
					for (int j = 0; j < array11.Length; j++)
					{
						Color32 color3 = ((j % 4 < 2) ? color : color2);
						byte b = (byte)(array11[j].r * color3.r / 255);
						byte b2 = (byte)(array11[j].g * color3.g / 255);
						byte b3 = (byte)(array11[j].b * color3.b / 255);
						byte b4 = (byte)(array11[j].a * color3.a / 255);
			...
			{
				Vector3 point = Vector3.Scale(array8[k], tk2dBatchedSprite4.baseScale);
				...
				if (flag2)
				{
					Vector3 vector = new Vector3(array13[k].x, array13[k].y, array13[k].z);
					vector = tk2dBatchedSprite4.rotation * vector;
					array3[num2 + k] = new Vector4(vector.x, vector.y, vector.z, array13[k].w);
				}
			}
		...
		foreach (tk2dBatchedSprite tk2dBatchedSprite2 in array3)
		{
			...
			if (flag)
			{
				...
				foreach (tk2dCollider2DData tk2dCollider2DData3 in edgeCollider2D)
				{
					...
					{
						array5[num3] = matrix4x.MultiplyPoint(tk2dCollider2DData3.points[num3]);
					}
