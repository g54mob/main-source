...
public class UICamera : MonoBehaviour
{
	...
	public const float tooltipDelay = 0.25f;

	public bool longPressTooltip;

	public float mouseDragThreshold = 10f;
		...
		get
		{
			...
			if (mLastOverCheck != frameCount)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					if (IsPartOfUI((mouseOrTouch.pressed != null) ? mouseOrTouch.pressed : mouseOrTouch.current))
					{
						mLastOverResult = true;
						return mLastOverResult;
					}
			...
			if (uICamera.eventType == EventType.UI_3D)
			{
				...
				if (num > 1)
				{
					...
					for (int num2 = 0; num2 < mHits.size; num2++)
					{
						...
						{
							lastHit = mHits[num2].hit;
							mRayHitObject = mHits[num2].go;
							lastWorldRay = ray;
							lastWorldPosition = mHits[num2].point;
							mHits.Clear();
							return true;
						}
					}
	...
	public void ProcessMouse()
	{
		...
		if (!flag && flag3 && (!stickyTooltip || flag4))
		{
			if (mTooltipTime != 0f)
			{
				mTooltipTime = Time.unscaledTime + 0.25f;
			}
		...
		if (currentTouch.pressed != null)
		{
			...
			if (currentTouch.dragged == currentTouch.current || (currentScheme != ControlScheme.Controller && currentTouch.clickNotification != ClickNotification.None && currentTouch.totalDelta.sqrMagnitude < drag))
			{
				if (currentTouch.clickNotification != ClickNotification.None && currentTouch.pressed == currentTouch.current)
				{
					...
					if (currentTouch.clickTime + 0.35f > time && (double)(_LastClickPos - currentTouch.pos).magnitude < 0.1)
					{
						Debug.Log((lastEventPosition - currentTouch.pos).magnitude);
					...
					_LastClickPos = currentTouch.pos;
