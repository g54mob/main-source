	...
	public class AnimationState
	{
		...
		private void QueueEvents(TrackEntry entry, float animationTime)
		{
			...
			if (!entry.loop)
			{
				...
				num3 = entry.animationLast < animationEnd;
			}
			else
			{
				num3 = num2 > entry.trackTime % num;
			}
			if (num3)
			{
		...
		public TrackEntry AddAnimation(int trackIndex, Animation animation, bool loop, float delay)
		{
			...
			if (trackEntry == null)
			{
				SetCurrent(trackIndex, trackEntry2, true);
				queue.Drain();
			}
			else
			{
				...
				if (delay <= 0f)
				{
					...
					if (num != 0f)
					{
						delay = ((!trackEntry.loop) ? (delay + num) : (delay + num * (float)(1 + (int)(trackEntry.trackTime / num))));
		...
		{
			TrackEntry trackEntry = SetAnimation(trackIndex, EmptyAnimation, false);
		...
		public TrackEntry AddEmptyAnimation(int trackIndex, float mixDuration, float delay)
		{
			...
			TrackEntry trackEntry = AddAnimation(trackIndex, EmptyAnimation, false, delay);
			trackEntry.mixDuration = mixDuration;
			trackEntry.trackEnd = mixDuration;
			return trackEntry;
		}
