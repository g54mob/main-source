...
namespace Spine
{
	public class SkeletonBinary
	{
		...
		public SkeletonData ReadSkeletonData(Stream input)
		{
			...
			int i = 0;
			for (int num = ReadVarint(input, true); i < num; i++)
			{
				...
				boneData.transformMode = TransformModeValues[ReadVarint(input, true)];
				if (flag)
				{
					ReadInt(input);
				}
			...
			int j = 0;
			for (int num2 = ReadVarint(input, true); j < num2; j++)
			{
				string name2 = ReadString(input);
				BoneData boneData2 = skeletonData.bones.Items[ReadVarint(input, true)];
			...
			int k = 0;
			for (int num5 = ReadVarint(input, true); k < num5; k++)
			{
				IkConstraintData ikConstraintData = new IkConstraintData(ReadString(input));
				ikConstraintData.order = ReadVarint(input, true);
				int l = 0;
				for (int num6 = ReadVarint(input, true); l < num6; l++)
				{
			...
			int m = 0;
			for (int num7 = ReadVarint(input, true); m < num7; m++)
			{
				TransformConstraintData transformConstraintData = new TransformConstraintData(ReadString(input));
				transformConstraintData.order = ReadVarint(input, true);
				int n = 0;
				for (int num8 = ReadVarint(input, true); n < num8; n++)
				{
					transformConstraintData.bones.Add(skeletonData.bones.Items[ReadVarint(input, true)]);
				}
			...
			int num9 = 0;
			for (int num10 = ReadVarint(input, true); num9 < num10; num9++)
			{
				PathConstraintData pathConstraintData = new PathConstraintData(ReadString(input));
				pathConstraintData.order = ReadVarint(input, true);
				int num11 = 0;
				for (int num12 = ReadVarint(input, true); num11 < num12; num11++)
				{
					pathConstraintData.bones.Add(skeletonData.bones.Items[ReadVarint(input, true)]);
				}
				pathConstraintData.target = skeletonData.slots.Items[ReadVarint(input, true)];
				pathConstraintData.positionMode = (PositionMode)Enum.GetValues(typeof(PositionMode)).GetValue(ReadVarint(input, true));
				pathConstraintData.spacingMode = (SpacingMode)Enum.GetValues(typeof(SpacingMode)).GetValue(ReadVarint(input, true));
				pathConstraintData.rotateMode = (RotateMode)Enum.GetValues(typeof(RotateMode)).GetValue(ReadVarint(input, true));
			...
			int num13 = 0;
			for (int num14 = ReadVarint(input, true); num13 < num14; num13++)
			{
			...
			for (int count = linkedMeshes.Count; num15 < count; num15++)
			{
				...
				Skin obj = ((linkedMesh.skin == null) ? skeletonData.DefaultSkin : skeletonData.FindSkin(linkedMesh.skin));
				if (obj == null)
				{
					throw new Exception("Skin not found: " + linkedMesh.skin);
				}
				Attachment attachment = obj.GetAttachment(linkedMesh.slotIndex, linkedMesh.parent);
				if (attachment == null)
				{
			...
			linkedMeshes.Clear();
			int num16 = 0;
			for (int num17 = ReadVarint(input, true); num16 < num17; num16++)
			{
			...
			int num18 = 0;
			for (int num19 = ReadVarint(input, true); num18 < num19; num18++)
			{
		...
		private Attachment ReadAttachment(Stream input, SkeletonData skeletonData, Skin skin, int slotIndex, string attachmentName, bool nonessential)
		{
			...
			{
				int num20 = ReadVarint(input, true);
			...
			case AttachmentType.Mesh:
			{
				...
				int num10 = ReadVarint(input, true);
				int[] edges = null;
				float num11 = 0f;
				float num12 = 0f;
				if (nonessential)
			...
			case AttachmentType.Path:
			{
				...
				int num16 = ReadVarint(input, true);
			...
			{
				int num = ReadVarint(input, true);
				int num2 = ReadVarint(input, true);
		...
		private Vertices ReadVertices(Stream input, int vertexCount)
		{
			...
			for (int i = 0; i < vertexCount; i++)
			{
				...
				{
					exposedList2.Add(ReadVarint(input, true));
		...
		private void ReadAnimation(string name, Stream input, SkeletonData skeletonData)
		{
			...
			float num = 0f;
			int i = 0;
			for (int num2 = ReadVarint(input, true); i < num2; i++)
			{
				int slotIndex = ReadVarint(input, true);
				int j = 0;
				for (int num3 = ReadVarint(input, true); j < num3; j++)
				{
			...
			int n = 0;
			for (int num9 = ReadVarint(input, true); n < num9; n++)
			{
				int boneIndex = ReadVarint(input, true);
				int num10 = 0;
				for (int num11 = ReadVarint(input, true); num10 < num11; num10++)
				{
			...
			int num17 = 0;
			for (int num18 = ReadVarint(input, true); num17 < num18; num17++)
			{
				int ikConstraintIndex = ReadVarint(input, true);
			...
			int num21 = 0;
			for (int num22 = ReadVarint(input, true); num21 < num22; num21++)
			{
				int transformConstraintIndex = ReadVarint(input, true);
				int num23 = ReadVarint(input, true);
			...
			int num35 = 0;
			for (int num36 = ReadVarint(input, true); num35 < num36; num35++)
			{
				Skin skin = skeletonData.skins.Items[ReadVarint(input, true)];
				int num37 = 0;
				for (int num38 = ReadVarint(input, true); num37 < num38; num37++)
				{
					int slotIndex2 = ReadVarint(input, true);
					int num39 = 0;
					for (int num40 = ReadVarint(input, true); num39 < num40; num39++)
					{
						...
						for (int num43 = 0; num43 < num42; num43++)
						{
							...
							int num44 = ReadVarint(input, true);
							float[] array;
							if (num44 == 0)
							{
			...
			if (num50 > 0)
			{
				...
				for (int num51 = 0; num51 < num50; num51++)
				{
					...
					int num52 = ReadVarint(input, true);
					...
					{
						int num57 = ReadVarint(input, true);
						while (num54 != num57)
						{
							array3[num55++] = num54++;
						}
						array2[num54 + ReadVarint(input, true)] = num54++;
					}
					while (num54 < count)
					{
						array3[num55++] = num54++;
			...
			int num59 = ReadVarint(input, true);
			if (num59 > 0)
			{
				...
				for (int num60 = 0; num60 < num59; num60++)
				{
					...
					obj.Int = ReadVarint(input, false);
					obj.Float = ReadFloat(input);
					obj.String = (ReadBoolean(input) ? ReadString(input) : eventData.String);
