	...
	public class SkeletonJson
	{
		...
		public SkeletonData ReadSkeletonData(TextReader reader)
		{
			...
			foreach (Dictionary<string, object> item in (List<object>)dictionary["bones"])
			{
				...
				boneData2.transformMode = (TransformMode)Enum.Parse(typeof(TransformMode), value, true);
			...
			for (int count = linkedMeshes.Count; i < count; i++)
			{
				...
				Attachment attachment2 = obj.GetAttachment(linkedMesh.slotIndex, linkedMesh.parent);
				if (attachment2 == null)
				{
		...
		private Attachment ReadAttachment(Dictionary<string, object> map, Skin skin, int slotIndex, string name, SkeletonData skeletonData)
		{
			...
			case AttachmentType.Linkedmesh:
			{
				...
				string text3 = GetString(map, "parent", null);
				if (text3 != null)
				{
					meshAttachment.InheritDeform = GetBoolean(map, "deform", true);
					linkedMeshes.Add(new LinkedMesh(meshAttachment, GetString(map, "skin", null), slotIndex, text3));
			...
			case AttachmentType.Clipping:
			{
				...
				string text2 = GetString(map, "end", null);
				if (text2 != null)
				{
					SlotData slotData = skeletonData.FindSlot(text2);
					if (slotData == null)
					{
						throw new Exception("Clipping end slot not found: " + text2);
					}
		...
		private void ReadAnimation(Dictionary<string, object> map, string name, SkeletonData skeletonData)
		{
				...
				foreach (KeyValuePair<string, object> item8 in (Dictionary<string, object>)map["bones"])
				{
					...
					foreach (KeyValuePair<string, object> item9 in (Dictionary<string, object>)item8.Value)
					{
						...
						case "rotate":
						{
							...
							{
								rotateTimeline.SetFrame(num10, (float)item10["time"], (float)item10["angle"]);
						...
						case "shear":
						{
							...
							foreach (Dictionary<string, object> item11 in list2)
							{
								...
								float num8 = GetFloat(item11, "x", 0f);
								float num9 = GetFloat(item11, "y", 0f);
			...
			if (map.ContainsKey("ik"))
			{
				foreach (KeyValuePair<string, object> item12 in (Dictionary<string, object>)map["ik"])
				{
					...
					foreach (Dictionary<string, object> item13 in obj4)
					{
						...
						float mix = GetFloat(item13, "mix", 1f);
						bool boolean = GetBoolean(item13, "bendPositive", true);
						ikConstraintTimeline.SetFrame(num11, time5, mix, boolean ? 1 : (-1));
			...
			if (map.ContainsKey("transform"))
			{
				foreach (KeyValuePair<string, object> item14 in (Dictionary<string, object>)map["transform"])
				{
					...
					TransformConstraintTimeline transformConstraintTimeline = new TransformConstraintTimeline(obj6.Count)
					{
					...
					foreach (Dictionary<string, object> item15 in obj6)
					{
						...
						float rotateMix = GetFloat(item15, "rotateMix", 1f);
						float translateMix = GetFloat(item15, "translateMix", 1f);
						float scaleMix = GetFloat(item15, "scaleMix", 1f);
						float shearMix = GetFloat(item15, "shearMix", 1f);
						transformConstraintTimeline.SetFrame(num12, time6, rotateMix, translateMix, scaleMix, shearMix);
				...
				foreach (KeyValuePair<string, object> item16 in (Dictionary<string, object>)map["paths"])
				{
					...
					foreach (KeyValuePair<string, object> item17 in (Dictionary<string, object>)item16.Value)
					{
						...
						case "spacing":
						{
							...
							{
								pathConstraintPositionTimeline.SetFrame(num16, (float)item18["time"], GetFloat(item18, key5, 0f) * num15);
						...
						case "mix":
						{
							...
							{
								pathConstraintMixTimeline.SetFrame(num14, (float)item19["time"], GetFloat(item19, "rotateMix", 1f), GetFloat(item19, "translateMix", 1f));
				...
				foreach (KeyValuePair<string, object> item20 in (Dictionary<string, object>)map["deform"])
				{
					...
					foreach (KeyValuePair<string, object> item21 in (Dictionary<string, object>)item20.Value)
					{
						...
						foreach (KeyValuePair<string, object> item22 in (Dictionary<string, object>)item21.Value)
						{
							...
							DeformTimeline deformTimeline = new DeformTimeline(obj8.Count)
							{
								slotIndex = num17,
							...
							foreach (Dictionary<string, object> item23 in obj8)
							{
								...
								deformTimeline.SetFrame(num19, (float)item23["time"], array);
			...
			if (map.ContainsKey("drawOrder") || map.ContainsKey("draworder"))
			{
				...
				foreach (Dictionary<string, object> item24 in obj9)
				{
					...
					drawOrderTimeline.SetFrame(num22++, (float)item24["time"], array2);
				}
			...
			if (map.ContainsKey("events"))
			{
				...
				foreach (Dictionary<string, object> item26 in obj10)
				{
					...
					Event obj11 = new Event((float)item26["time"], eventData);
					obj11.Int = GetInt(item26, "int", eventData.Int);
					obj11.Float = GetFloat(item26, "float", eventData.Float);
					obj11.String = GetString(item26, "string", eventData.String);
					eventTimeline.SetFrame(num29++, obj11);
				}
