...
{
	public class IntSettingControl : MonoBehaviour
	{
		public UILabel Name;
		...
		public void Init(string title, int min, int max, int steps, FieldInfo fieldInfo, List<DronePart> parentObjects, UndoManager.EStoreReason storeReason)
		{
			Name.text = title + ":";
			...
			for (int i = 0; i < parentObjects.Count; i++)
			{
				...
				int num2 = (int)fieldInfo.GetValue(obj);
				if (i == 0)
				{
					num = num2;
				}
