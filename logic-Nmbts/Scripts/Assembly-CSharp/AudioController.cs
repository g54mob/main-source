...
public class AudioController : SingletonMonoBehaviour<AudioController>, ISerializationCallbackReceiver
{
	...
	{
		return SingletonMonoBehaviour<AudioController>.Instance._PlayEx(audioID, AudioChannelType.Default, 1f, worldPosition, parentObj, 0f, 0f, false);
	}
	...
	{
		return SingletonMonoBehaviour<AudioController>.Instance._PlayEx(audioID, AudioChannelType.Default, volume, worldPosition, parentObj, 0f, startTime, false, dspTime);
	}
	...
	protected AudioObject _PlayMusic(string audioID, Vector3 position, Transform parentObj, float volume, float delay, float startTime)
	{
		...
		_currentMusic = _PlayEx(audioID, AudioChannelType.Music, volume, position, parentObj, delay, startTime, false, 0.0, null, (!flag) ? 1 : 0);
	...
	protected AudioObject _PlayAmbienceSound(string audioID, Vector3 position, Transform parentObj, float volume, float delay, float startTime)
	{
		...
		_currentAmbienceSound = _PlayEx(audioID, AudioChannelType.Ambience, volume, position, parentObj, delay, startTime, false, 0.0, null, (!flag) ? 1 : 0);
