...
namespace emotiontheory
{
	public class BezierSpline : MonoBehaviour
	{
		...
		public void RemoveCurve(int index)
		{
			...
			int num = (index + 1) / 3;
			List<BezierControlPointMode> list = new List<BezierControlPointMode>();
			for (int i = 0; i < modes.Length; i++)
			{
				BezierControlPointMode item = modes[i];
				if (i != num && (!loop || (num != 0 && num != modes.Length - 1)))
				{
					list.Add(item);
				}
			}
