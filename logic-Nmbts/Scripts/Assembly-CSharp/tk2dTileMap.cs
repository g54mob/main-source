...
public class tk2dTileMap : MonoBehaviour, ISpriteCollectionForceBuild
{
	...
	public Vector3 GetTilePosition(int x, int y)
	{
		...
		Vector3 point2 = new Vector3(((float)x + (((y & 1) == 0) ? 0f : 0.5f)) * data.tileSize.x + data.tileOrigin.x, (float)y * data.tileSize.y + data.tileOrigin.y, 0f);
	...
	public bool UsesSpriteCollection(tk2dSpriteCollectionData spriteCollection)
	{
		...
		{
			if (!(spriteCollection == this.spriteCollection))
			{
				return spriteCollection == this.spriteCollection.inst;
			}
			return true;
		}
		return false;
