...
namespace UniJSON
{
	public static class MsgPackParser
	{
		...
		private static ArraySegment<byte> GetItemCount(ArraySegment<byte> bytes, MsgPackType formatType, out uint count)
		{
			switch (formatType)
			{
		...
		{
			MsgPackType format = GetFormat(bytes);
			if (format.IsArray())
			{
				ListTreeNode<MsgPackValue> listTreeNode = tree.AddValue(bytes, ValueNodeType.Array);
				bytes = GetItemCount(bytes, format, out var count);
				for (int i = 0; i < count; i++)
				{
					ListTreeNode<MsgPackValue> listTreeNode2 = _Parse(listTreeNode, bytes);
					bytes = bytes.Advance(listTreeNode2.Value.Bytes.Count);
				}
				listTreeNode.SetValueBytesCount(bytes.Offset - listTreeNode.Value.Bytes.Offset);
				return listTreeNode;
			}
			if (format.IsMap())
			{
				ListTreeNode<MsgPackValue> listTreeNode3 = tree.AddValue(bytes, ValueNodeType.Object);
				bytes = GetItemCount(bytes, format, out var count2);
				for (int j = 0; j < count2; j++)
				{
					ListTreeNode<MsgPackValue> listTreeNode4 = _Parse(listTreeNode3, bytes);
					bytes = bytes.Advance(listTreeNode4.Value.Bytes.Count);
					ListTreeNode<MsgPackValue> listTreeNode5 = _Parse(listTreeNode3, bytes);
					bytes = bytes.Advance(listTreeNode5.Value.Bytes.Count);
				}
				listTreeNode3.SetValueBytesCount(bytes.Offset - listTreeNode3.Value.Bytes.Offset);
				return listTreeNode3;
			}
			ArraySegment<byte> body = GetBody(bytes, format);
			int n = body.Offset - bytes.Offset + body.Count;
			return tree.AddValue(bytes.Take(n), ValueNodeType.Null);
		}
