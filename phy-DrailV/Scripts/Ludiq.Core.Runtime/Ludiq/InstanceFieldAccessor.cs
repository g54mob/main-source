	...
	public class InstanceFieldAccessor<TTarget, TField> : IOptimizedAccessor
	{
		...
		public void Compile()
		{
			if (OptimizedReflection.useJit)
			{
				...
				getter = Expression.Lambda<Func<TTarget, TField>>(body, new ParameterExpression[1] { parameterExpression }).Compile();
				if (fieldInfo.CanWrite())
				{
					DynamicMethod dynamicMethod = new DynamicMethod("setter", typeof(void), new Type[2]
					{
						typeof(TTarget),
						typeof(TField)
					}, typeof(TTarget), skipVisibility: true);
					ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
					iLGenerator.Emit(OpCodes.Ldarg_0);
					iLGenerator.Emit(OpCodes.Ldarg_1);
					iLGenerator.Emit(OpCodes.Stfld, fieldInfo);
					iLGenerator.Emit(OpCodes.Ret);
					setter = (Action<TTarget, TField>)dynamicMethod.CreateDelegate(typeof(Action<TTarget, TField>));
				}
				return;
			}
