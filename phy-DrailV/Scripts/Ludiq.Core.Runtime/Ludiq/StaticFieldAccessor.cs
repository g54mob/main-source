...
namespace Ludiq
{
	public class StaticFieldAccessor<TField> : IOptimizedAccessor
	{
		...
		public void Compile()
		{
			...
			{
				MemberExpression body = Expression.Field(null, fieldInfo);
				getter = Expression.Lambda<Func<TField>>(body, Array.Empty<ParameterExpression>()).Compile();
				if (fieldInfo.CanWrite())
				{
					DynamicMethod dynamicMethod = new DynamicMethod("setter", typeof(void), new Type[1] { typeof(TField) }, targetType, skipVisibility: true);
					ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
					iLGenerator.Emit(OpCodes.Ldarg_0);
					iLGenerator.Emit(OpCodes.Stsfld, fieldInfo);
					iLGenerator.Emit(OpCodes.Ret);
					setter = (Action<TField>)dynamicMethod.CreateDelegate(typeof(Action<TField>));
				}
				return;
			}
