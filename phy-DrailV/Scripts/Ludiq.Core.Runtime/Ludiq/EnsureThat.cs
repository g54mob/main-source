...
namespace Ludiq
{
	public class EnsureThat
	{
		...
		private void HasConstructorAccepting(Type param, Type[] parameterTypes, bool nonPublic)
		{
			...
			string format = (nonPublic ? ExceptionMessages.Reflection_HasConstructor_Failed : ExceptionMessages.Reflection_HasPublicConstructor_Failed);
			throw new ArgumentException(format.Inject(param.ToString(), parameterTypes.ToCommaSeparatedString()), paramName);
		}
