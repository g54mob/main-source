...
namespace Ludiq
{
	...
	public sealed class Member : ISerializationCallbackReceiver
	{
		...
		public void Reflect()
		{
			...
			methodInvoker = null;
			MemberInfo[] extendedMember = targetType.GetExtendedMember(name, MemberTypes.Constructor | MemberTypes.Field | MemberTypes.Method | MemberTypes.Property, BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public | BindingFlags.NonPublic | BindingFlags.FlattenHierarchy);
			if (extendedMember.Length == 0)
			{
				Dictionary<string, string> dictionary = RuntimeCodebase.RenamedMembers(targetType);
