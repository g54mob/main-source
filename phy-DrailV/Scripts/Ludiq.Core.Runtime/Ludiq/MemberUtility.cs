...
namespace Ludiq
{
	public static class MemberUtility
	{
		...
		{
			extensionMethodsCache = (from method in RuntimeCodebase.types.Where((Type type) => type.IsStatic() && !type.IsGenericType && !type.IsNested).SelectMany((Type type) => type.GetMethods())
		...
		public static MethodInfo MakeGenericMethodVia(this MethodInfo openConstructedMethod, params Type[] closedConstructedParameterTypes)
		{
			...
			Type[] genericArguments = openConstructedMethod.GetGenericArguments();
			Type[] typeArguments = genericArguments.Select((Type openConstructedGenericArgument) => resolvedGenericParameters.ContainsKey(openConstructedGenericArgument) ? resolvedGenericParameters[openConstructedGenericArgument] : openConstructedGenericArgument).ToArray();
		...
		{
			MethodInfo[] array = extensionMethodsCache;
			foreach (MethodInfo methodInfo in array)
			{
				...
				if (methodInfo.ContainsGenericParameters)
				{
					...
					genericExtensionMethods.Add(methodInfo2);
		...
		public static IEnumerable<MethodInfo> GetExtensionMethods(this Type thisArgumentType, bool inherited = true)
		{
			if (inherited)
			{
				...
				{
					if (!inheritedExtensionMethodsCache.TryGetValue(thisArgumentType, out var value))
					{
						value = GetInheritedExtensionMethods(thisArgumentType).ToArray();
						inheritedExtensionMethodsCache.Add(thisArgumentType, value);
					}
					return value;
				}
			}
			return extensionMethodsCache.Where((MethodInfo method) => method.GetParameters()[0].ParameterType == thisArgumentType);
		}
