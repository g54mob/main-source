...
namespace Rewired.HID.Drivers
{
	...
	internal abstract class HIDDeviceDriver : IDisposable, IControllerDriver
	{
		...
		internal interface IHIDDevice
		{
			...
			bool WriteSync(xDlFkKEEsqHDzeOiaTIGueyqTccYA outputReport, int timeoutMs);

			void WriteAsync(xDlFkKEEsqHDzeOiaTIGueyqTccYA outputReport, int timeoutMs);
		...
		internal class InitArgs
		{
			...
			public readonly PWHRTOVLUXMumxboQQmQIFMHEBfDA connectionType;
			...
			public InitArgs(UpdateLoopSetting P_0, PWHRTOVLUXMumxboQQmQIFMHEBfDA P_1, int P_2, int P_3, int P_4, int P_5, IHIDDevice P_6)
			{
		...
		public vapXGbCthTfrBlIUGtkgzOtCLETf[] axes;

		public UGvkBdUzfogfxagdjdQqdinGSMwv[] buttons;

		public DgGSAFeoadnaMFTBvLhTaezSCUDD[] hats;

		public olIxPUWFAfTtYSNqDeGoXGwRumpd[] accelerometers;

		public qVVbimaITgoplhjrKwIaqtLqwxTAc[] gyroscopes;

		public IRcdnSIjiuKLhXFkJwhyNQabopZH[] touchpads;

		public pmTlTYxlhgTeYOMZqBSNaIrfQJzO[] vibrationMotors;

		public wcZVsiHdENbhsBlZJfyeZHJzcruiA[] lights;
		...
		protected bool disposed => JWXwfaUAOJsMCNExsMKmFgNcBZSc;
		...
		public static HIDDeviceDriver GetDriver(DriverType driverId, InitArgs hidDriverInitArgs)
		{
			...
			case DriverType.SwitchProController:
				if (hidDriverInitArgs.connectionType != PWHRTOVLUXMumxboQQmQIFMHEBfDA.Bluetooth)
				{
					return null;
				}
