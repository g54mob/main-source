		...
		private sealed class EnumeratorPromise : IUniTaskSource, IPlayerLoopItem, ITaskPoolNode<EnumeratorPromise>
		{
			...
			private static IEnumerator ConsumeEnumerator(IEnumerator enumerator)
			{
				while (enumerator.MoveNext())
				{
					...
					CustomYieldInstruction cyi = (customYieldInstruction = current as CustomYieldInstruction);
					...
					if (current is YieldInstruction)
					{
						...
						if (obj != null)
						{
							if (!(obj is AsyncOperation asyncOperation))
							{
								...
								{
									WaitForSeconds waitForSeconds2 = waitForSeconds;
									innerCoroutine = UnwrapWaitForSeconds(waitForSeconds2);
								}
							}
							else
							{
								AsyncOperation asyncOperation2 = asyncOperation;
								innerCoroutine = UnwrapWaitAsyncOperation(asyncOperation2);
							}
						}
