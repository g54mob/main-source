...
namespace DelaunatorSharp
{
	public class Delaunator
	{
		...
		public IEnumerable<IVoronoiCell> GetVoronoiCells(Func<int, IPoint> triangleVerticeSelector = null)
		{
			...
			List<IPoint> vertices = new List<IPoint>(10);
			for (int triangleId = 0; triangleId < Triangles.Length; triangleId++)
			{
				int num = Triangles[NextHalfedge(triangleId)];
				if (!seen.Add(num))
				{
					continue;
				}
				foreach (int item in EdgesAroundPoint(triangleId))
				{
		...
		{
			return points.Zip(points.Skip(1).Append(points.FirstOrDefault()), (IPoint a, IPoint b) => new Edge(0, a, b)).OfType<IEdge>();
		}
