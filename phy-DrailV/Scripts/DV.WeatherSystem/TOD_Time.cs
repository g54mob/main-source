...
public class TOD_Time : MonoBehaviour
{
	...
	[TOD_Min(0f)]
	public float initialDayLengthInMinutes = 30f;
	...
	private float referenceDayLength = 30f;

	public OverridableValue<float> DayLengthInMinutes { get; private set; } = new OverridableValue<float>(30f);

	public float TimeRate
	{
		get
		{
			return referenceDayLength / (float)DayLengthInMinutes;
		}
		set
		{
			DayLengthInMinutes.CurrentValue = referenceDayLength / value;
		}
	}
	...
	public float ApplyTimeCurve(float deltaTime)
	{
		...
		deltaTime = timeCurve.Evaluate(num) - (float)sky.Cycle.Hour;
		if (num >= 24f)
		{
	...
	public void AddHours(float hours, bool adjust = true, bool useRealDateTime = false)
	{
		...
		if (useRealDateTime)
		{
			sky.Cycle.RealDateTime = dateTime2;
		}
		else
		{
	...
	{
		DayLengthInMinutes = new OverridableValue<float>(initialDayLengthInMinutes);
		referenceDayLength = DayLengthInMinutes;
		...
		if (UseDeviceTime)
		{
			sky.Cycle.Hour.CurrentValue = (float)DateTime.Now.TimeOfDay.TotalHours;
		}
		RefreshTimeCurve();
	}
	...
	{
		if (ProgressTime && !sky.Cycle.Hour.IsOverridden && (float)DayLengthInMinutes > 0f)
		{
			float num = 1440f / (float)DayLengthInMinutes;
			AddSeconds(Time.fixedDeltaTime * num);
		}
	}
}
