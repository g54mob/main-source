...
namespace PlaceholderSoftware.WetStuff.Rendering
{
	internal struct InstanceProperties
	{
		...
		private static readonly int AffectCeilingId = Shader.PropertyToID("_AffectCeiling");
		...
		public void LoadInto([NotNull] MaterialPropertyBlock properties, LayerMode mode)
		{
			...
			properties.SetFloat(AffectCeilingId, AffectCeiling ? 1 : 0);
		...
		public void LoadInto(int index, [NotNull] float[] saturation, [NotNull] float[] fadeout, [NotNull] float[] edgeSharpness, [NotNull] float[] affectCeiling, LayerArrays? x, LayerArrays? y, LayerArrays? z)
		{
			saturation[index] = Saturation;
			fadeout[index] = Fadeout;
			edgeSharpness[index] = EdgeSharpness;
			affectCeiling[index] = (AffectCeiling ? 1 : 0);
			if (x.HasValue)
			{
		...
		public static void LoadInto([NotNull] MaterialPropertyBlock properties, [NotNull] float[] saturation, [NotNull] float[] fadeout, [NotNull] float[] edgeSharpness, [NotNull] float[] affectCeiling, LayerArrays? x, LayerArrays? y, LayerArrays? z)
		{
			...
			properties.SetFloatArray(AffectCeilingId, affectCeiling);
			if (x.HasValue)
			{
