		...
		public class DecalBatch : IDisposable, IComparable<DecalBatch>
		{
			...
			private void DrawSingle([NotNull] CommandBuffer cmd, [NotNull] WetDecalSystem.DecalRenderInstance decal, int shaderPass)
			{
				...
				if (decal.Decal.Settings.Shape == DecalShape.Mesh)
				{
					...
					{
						Mesh mesh = decal.Decal.Mesh;
						for (int i = 0; i < mesh.subMeshCount; i++)
						{
							cmd.DrawMesh(mesh, decal.Decal.WorldTransform, Batch.GetMaterial(instanced: false), i, shaderPass, decal.PropertyBlock);
						}
					}
				}
				else
			...
			private void DrawInstanced([NotNull] CommandBuffer cmd, ArraySegment<int> visible, int shaderPass)
			{
				...
				EnsureArrayCapacity(ref _affectCeiling, visible.Count);
				...
				for (int i = 0; i < visible.Count; i++)
				{
					...
					if (decalRenderInstance.Decal.Settings.Shape == DecalShape.Mesh)
					{
						DrawSingle(cmd, decalRenderInstance, shaderPass);
						_saturation[i] = 0f;
					}
					else
					{
						decalRenderInstance.Properties.LoadInto(i, _saturation, _edgeFadeout, _edgeSharpness, _affectCeiling, x, y, z);
					}
					_matrices[i] = decalRenderInstance.Decal.WorldTransform;
				}
				_instancingPropertyBlock.Clear();
				InstanceProperties.LoadInto(_instancingPropertyBlock, _saturation, _edgeFadeout, _edgeSharpness, _affectCeiling, x, y, z);
