...
public class TerrainNeighbours : MonoBehaviour
{
	...
	public void CreateNeighbours()
	{
		...
		{
			_terrains.Add(item2);
			Vector3 vector = item2.transform.AbsolutePosition();
			int[] key = new int[2]
			{
				Mathf.RoundToInt((vector.x - firstPosition.x) / (float)sizeX),
				Mathf.RoundToInt((vector.z - firstPosition.y) / (float)sizeZ)
			};
