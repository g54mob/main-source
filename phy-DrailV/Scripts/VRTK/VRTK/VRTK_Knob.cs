...
namespace VRTK
{
	...
	public class VRTK_Knob : VRTK_Control
	{
		...
		protected virtual void InitKnob()
		{
			...
			if ((bool)connectedTo && connectedTo.GetComponent<Rigidbody>() == null)
			{
				Rigidbody rigidbody2 = connectedTo.AddComponent<Rigidbody>();
		...
		protected virtual KnobDirection DetectDirection()
		{
			...
			Physics.Raycast(bounds.center, Vector3.forward, out var hitInfo, bounds.extents.z * 3f, -5, QueryTriggerInteraction.UseGlobal);
			Physics.Raycast(bounds.center, Vector3.back, out var hitInfo2, bounds.extents.z * 3f, -5, QueryTriggerInteraction.UseGlobal);
			Physics.Raycast(bounds.center, Vector3.left, out var hitInfo3, bounds.extents.x * 3f, -5, QueryTriggerInteraction.UseGlobal);
			Physics.Raycast(bounds.center, Vector3.right, out var hitInfo4, bounds.extents.x * 3f, -5, QueryTriggerInteraction.UseGlobal);
			Physics.Raycast(bounds.center, Vector3.up, out var hitInfo5, bounds.extents.y * 3f, -5, QueryTriggerInteraction.UseGlobal);
			Physics.Raycast(bounds.center, Vector3.down, out var hitInfo6, bounds.extents.y * 3f, -5, QueryTriggerInteraction.UseGlobal);
		...
		protected virtual float CalculateValue()
		{
			if (!subDirectionFound)
			{
				...
				{
					subDirection = ((!(num < num2)) ? ((num < num3) ? KnobDirection.z : KnobDirection.x) : ((!(num2 < num3)) ? KnobDirection.y : KnobDirection.z));
