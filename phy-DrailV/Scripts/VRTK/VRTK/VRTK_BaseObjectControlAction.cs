...
namespace VRTK
{
	public abstract class VRTK_BaseObjectControlAction : MonoBehaviour
	{
		...
		protected virtual void OnEnable()
		{
			...
			internalBodyPhysics = ((internalBodyPhysics == null) ? VRTK_SharedMethods.FindEvenInactiveComponent<VRTK_BodyPhysics>(searchAllScenes: true) : internalBodyPhysics);
		}
		...
		protected virtual Vector3 GetObjectCenter(Transform checkObject)
		{
			if (centerCollider == null || checkObject != controlledTransform)
			{
				...
				if (checkObject == playArea)
				{
					...
					{
						CapsuleCollider capsuleCollider = (CapsuleCollider)(centerCollider = internalBodyPhysics.GetBodyColliderContainer().GetComponent<CapsuleCollider>());
		...
		protected virtual void CheckForPlayerBeforeRotation(GameObject controlledGameObject)
		{
			...
			validPlayerObject = component != null && component.objectType == VRTK_PlayerObject.ObjectTypes.CameraRig && headsetTransform != null;
