...
namespace VRTK.Examples.Tests
{
	public class VRTK_ControllerReference_Setup : VRTK_BaseTest
	{
		...
		{
			base.SetUp(message);
			actualController = ((overrideActualController == null) ? VRTK_SDK_Bridge.GetControllerByHand(actualControllerHand, actual: true) : overrideActualController);
			aliasController = ((overrideAliasController == null) ? VRTK_SDK_Bridge.GetControllerByHand(actualControllerHand, actual: false) : overrideAliasController);
		...
		protected virtual void TestStaticIndex()
		{
			...
			Assert("testReference.actual == actualController", testReference.actual == actualController, string.Concat("The reference actual [", testReference.actual, "] does not match the actual [", actualController, "]"));
			Assert("testReference.scriptAlias == aliasController", testReference.scriptAlias == aliasController, string.Concat("The reference script alias [", testReference.scriptAlias, "] does not match the actual [", aliasController, "]"));
			Assert("testReference.model == modelController", testReference.model == modelController, string.Concat("The reference model [", testReference.model, "] does not match the model [", modelController, "]"));
			Assert("testReference.hand == actualControllerHand", testReference.hand == actualControllerHand, string.Concat("The reference hand [", testReference.hand, "] does not match the hand [", actualControllerHand, "]"));
			foreach (SDK_BaseController.ControllerHand otherHand in GetOtherHands(actualControllerHand))
			{
				Assert("testReference.hand == " + otherHand, testReference.hand != otherHand, string.Concat("The reference hand [", testReference.hand, "] should not match the hand [", otherHand, "]"));
			}
			TearDown();
		}
		...
		protected virtual void TestStaticIsValid()
		{
			...
			Assert("!VRTK_ControllerReference.IsValid(VRTK_ControllerReference.GetControllerReference(999))", !VRTK_ControllerReference.IsValid(VRTK_ControllerReference.GetControllerReference(999u)), "invalid reference should not be valid");
		...
		protected virtual void TestStaticGetRealIndex()
		{
			...
			Assert("VRTK_ControllerReference.GetRealIndex(VRTK_ControllerReference.GetControllerReference(999)) == uint.MaxValue", VRTK_ControllerReference.GetRealIndex(VRTK_ControllerReference.GetControllerReference(999u)) == uint.MaxValue, "invalid reference should have index of uint.MaxValue");
