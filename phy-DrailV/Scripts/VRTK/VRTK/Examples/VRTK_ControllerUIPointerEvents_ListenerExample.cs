...
namespace VRTK.Examples
{
	public class VRTK_ControllerUIPointerEvents_ListenerExample : MonoBehaviour
	{
		...
		{
			VRTK_Logger.Info("UI Pointer entered " + e.currentTarget.name + " on Controller index [" + VRTK_ControllerReference.GetRealIndex(e.controllerReference) + "] and the state was " + e.isActive.ToString() + " ### World Position: " + e.raycastResult.worldPosition);
			if (togglePointerOnHit && (bool)GetComponent<VRTK_Pointer>())
			{
				GetComponent<VRTK_Pointer>().Toggle(state: true);
			}
		}

		private void VRTK_ControllerUIPointerEvents_ListenerExample_UIPointerElementExit(object sender, UIPointerEventArgs e)
		{
			VRTK_Logger.Info("UI Pointer exited " + e.previousTarget.name + " on Controller index [" + VRTK_ControllerReference.GetRealIndex(e.controllerReference) + "] and the state was " + e.isActive.ToString());
			if (togglePointerOnHit && (bool)GetComponent<VRTK_Pointer>())
			{
				GetComponent<VRTK_Pointer>().Toggle(state: false);
			}
		}

		private void VRTK_ControllerUIPointerEvents_ListenerExample_UIPointerElementClick(object sender, UIPointerEventArgs e)
		{
			VRTK_Logger.Info("UI Pointer clicked " + e.currentTarget.name + " on Controller index [" + VRTK_ControllerReference.GetRealIndex(e.controllerReference) + "] and the state was " + e.isActive.ToString() + " ### World Position: " + e.raycastResult.worldPosition);
		}

		private void VRTK_ControllerUIPointerEvents_ListenerExample_UIPointerElementDragStart(object sender, UIPointerEventArgs e)
		{
			VRTK_Logger.Info("UI Pointer started dragging " + e.currentTarget.name + " on Controller index [" + VRTK_ControllerReference.GetRealIndex(e.controllerReference) + "] and the state was " + e.isActive.ToString() + " ### World Position: " + e.raycastResult.worldPosition);
		}

		private void VRTK_ControllerUIPointerEvents_ListenerExample_UIPointerElementDragEnd(object sender, UIPointerEventArgs e)
		{
			VRTK_Logger.Info("UI Pointer stopped dragging " + e.currentTarget.name + " on Controller index [" + VRTK_ControllerReference.GetRealIndex(e.controllerReference) + "] and the state was " + e.isActive.ToString() + " ### World Position: " + e.raycastResult.worldPosition);
		}
	}
}
