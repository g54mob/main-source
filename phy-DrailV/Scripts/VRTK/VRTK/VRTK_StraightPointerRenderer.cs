...
namespace VRTK
{
	...
	public class VRTK_StraightPointerRenderer : VRTK_BasePointerRenderer
	{
		...
		{
			return new GameObject[3] { actualContainer, actualCursor, actualTracer };
		}
		...
		protected virtual float CastRayForward()
		{
			...
			RaycastHit hitData;
			bool flag = VRTK_CustomRaycast.Raycast(ray: new Ray(origin.position, origin.forward), customCast: customRaycast, hitData: out hitData, ignoreLayers: defaultIgnoreLayer, length: maximumLength);
		...
		{
			if (!(actualContainer != null) || !(controllingPointer != null))
			{
				return;
			}
			...
			Transform origin = GetOrigin();
			float num2 = 1.05f;
			ScaleObjectInteractor(actualCursor.transform.lossyScale * num2);
			if (destinationHit.transform != null)
			{
				...
				{
					float num3 = Vector3.Distance(destinationHit.point, origin.position);
					actualCursor.transform.localScale = Vector3.Min(cursorOriginalScale * num3, maximumCursorScale);
				}
			}
			else
			{
			...
			ToggleRenderer(controllingPointer.IsPointerActive(), actualState: false);
