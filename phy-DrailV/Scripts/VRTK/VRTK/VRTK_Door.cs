...
namespace VRTK
{
	...
	public class VRTK_Door : VRTK_Control
	{
		...
		protected override void OnDrawGizmos()
		{
			...
			{
				Vector3 obj = (handles ? bounds.center : bounds2.center);
				Vector3 vector3 = obj + vector2 * num * subDirection - vector * (num / 2f) * subDirection;
				Gizmos.DrawLine(obj, vector3);
				Gizmos.DrawSphere(vector3, num / 8f);
			}
			if ((!flag && openOutward) || (flag && openInward))
			{
				Vector3 obj2 = (handles ? bounds.center : bounds2.center);
				Vector3 vector4 = obj2 + vector2 * num * subDirection + vector * (num / 2f) * subDirection;
				Gizmos.DrawLine(obj2, vector4);
		...
		protected override bool DetectSetup()
		{
			...
			if (doorHinge == null || doorHingeCreated)
			{
				...
				else
				{
					...
					{
					case Direction.x:
						secondaryDirection = ((bounds.extents.y > bounds.extents.z) ? Vector3.up : Vector3.forward);
						break;
					case Direction.y:
						secondaryDirection = ((bounds.extents.x > bounds.extents.z) ? Vector3.right : Vector3.forward);
						break;
					case Direction.z:
						secondaryDirection = ((bounds.extents.y > bounds.extents.x) ? Vector3.up : Vector3.right);
						break;
					}
					subDirection = 1f;
				}
			...
			if ((bool)doorHinge)
			{
				...
				limits.min = (openInward ? (0f - maxAngle) : 0f);
				limits.max = (openOutward ? maxAngle : 0f);
		...
		protected virtual void InitHandle()
		{
			if (!(handles == null))
			{
				...
				if (handles.GetComponent<FixedJoint>() == null)
				{
					handles.AddComponent<FixedJoint>().connectedBody = doorRigidbody;
				}
				if (handleInteractableOnly)
				{
