...
namespace VRTK
{
	public class VRTK_HeadsetCollider : MonoBehaviour
	{
		...
		{
			if ((!parent.ignoreTriggerColliders || !(collider != null) || !collider.isTrigger) && base.enabled && !VRTK_PlayerObject.IsPlayerObject(collider.gameObject) && ValidTarget(collider.transform))
			{
