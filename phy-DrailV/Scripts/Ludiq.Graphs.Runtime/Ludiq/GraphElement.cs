...
namespace Ludiq
{
	public abstract class GraphElement<TGraph> : IGraphElement, IGraphItem, INotifiedCollectionItem, IDisposable, IPrewarmable, IAotStubbable, IIdentifiable where TGraph : class, IGraph
	{
		[Serialize]
		...
		{
			IGraphNesterElement parent = (IGraphNesterElement)this;
			HashSet<GraphReference> hashSet = GraphInstances.ChildrenOfPooled(parent);
			foreach (GraphReference item in hashSet)
			{
