...
namespace Crosstales.NLayer.Decoder
{
	internal sealed class LayerIIIDecoder : LayerDecoderBase
	{
		private class HybridMDCT
		{
			...
			private static readonly float[] icos72_table;

			private readonly List<float[]> _prevBlock;

			private readonly List<float[]> _nextBlock;

			private readonly float[] _imdctTemp = new float[18];

			private readonly float[] _imdctResult = new float[36];
			...
			private static void ShortIMDCT(float[] invec, int inIdx, float[] outvec)
			{
				...
				float num2 = array[4] / 2f + invec[inIdx];
				float num3 = array[2] * 0.8660254f;
				array3[0] = num2 + num3;
				array3[1] = invec[inIdx] - array[4];
				array3[2] = num2 - num3;
				float num4 = array[3] + array[5];
				num2 = num4 / 2f + array[1];
				num3 = (array[1] + array[3]) * 0.8660254f;
				array4[0] = num2 + num3;
				array4[1] = array[1] - num4;
				array4[2] = num2 - num3;
				array4[0] *= 0.5176381f;
				array4[1] *= 0.70710677f;
				array4[2] *= 1.9318516f;
				array2[0] = (array3[0] + array4[0]) * 0.5043145f;
				array2[1] = (array3[1] + array4[1]) * 0.5411961f;
				array2[2] = (array3[2] + array4[2]) * 0.6302362f;
				array2[3] = (array3[2] - array4[2]) * 0.82133985f;
				array2[4] = (array3[1] - array4[1]) * 1.306563f;
				array2[5] = (array3[0] - array4[0]) * 3.830649f;
				outvec[0] = array2[3] * _swin[2][0];
				outvec[1] = array2[4] * _swin[2][1];
				outvec[2] = array2[5] * _swin[2][2];
				outvec[3] = (0f - array2[5]) * _swin[2][3];
				outvec[4] = (0f - array2[4]) * _swin[2][4];
				outvec[5] = (0f - array2[3]) * _swin[2][5];
				outvec[6] = (0f - array2[2]) * _swin[2][6];
				outvec[7] = (0f - array2[1]) * _swin[2][7];
				outvec[8] = (0f - array2[0]) * _swin[2][8];
				outvec[9] = (0f - array2[0]) * _swin[2][9];
				outvec[10] = (0f - array2[1]) * _swin[2][10];
				outvec[11] = (0f - array2[2]) * _swin[2][11];
			}
		}
		...
		private readonly BitReservoir _bitRes = new BitReservoir();

		private int _channels;

		private int _mainDataBegin;

		private readonly int[][] _scfsi = new int[2][]
		{
			new int[4],
			new int[4]
		};

		private readonly int[][] _part23Length = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly int[][] _bigValues = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly float[][] _globalGain = new float[2][]
		{
			new float[2],
			new float[2]
		};

		private readonly int[][] _scalefacCompress = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly bool[][] _blockSplitFlag = new bool[2][]
		{
			new bool[2],
			new bool[2]
		};

		private readonly bool[][] _mixedBlockFlag = new bool[2][]
		{
			new bool[2],
			new bool[2]
		};

		private readonly int[][] _blockType = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly int[][][] _tableSelect;

		private readonly float[][][] _subblockGain;

		private readonly int[][] _regionAddress1 = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly int[][] _regionAddress2 = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly int[][] _preflag = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private readonly float[][] _scalefacScale = new float[2][]
		{
			new float[2],
			new float[2]
		};

		private readonly int[][] _count1TableSelect = new int[2][]
		{
			new int[2],
			new int[2]
		};

		private static readonly float[] GAIN_TAB = new float[256]
		{
			1.5700924E-16f,
			1.8671652E-16f,
			2.220446E-16f,
		...
		private readonly byte[] _cbLookupL = new byte[576];

		private readonly byte[] _cbLookupS = new byte[576];

		private readonly byte[] _cbwLookupS = new byte[576];
		...
		private readonly int[][][] _scalefac = new int[2][][]
		{
			new int[4][]
			{
				new int[13],
		...
		private readonly float[] _reorderBuf = new float[576];
		...
		private readonly float[] _polyPhase = new float[32];
			...
			for (int i = 0; i < num3; i++)
			{
				...
				for (int k = num; k <= num2; k++)
				{
					...
					if (flag && num5 == 2)
					{
						if (flag2)
						{
							Reorder(array2, mixedBlock: true);
							AntiAlias(array2, mixedBlock: true);
						}
						else
						{
							Reorder(array2, mixedBlock: false);
						}
					}
					else
					{
						AntiAlias(array2, mixedBlock: false);
					}
					_hybrid.Apply(array2, k, num5, flag && flag2);
					FrequencyInversion(array2);
					InversePolyphase(array2, k, num4, array[k]);
				}
				num4 += 576;
			}
			return num4;
		...
		private void ReadSideInfo(IMpegFrame frame)
		{
			...
			for (int l = 0; l < 2; l++)
			{
				...
				{
					_part23Length[l][m] = frame.ReadBits(12);
					_bigValues[l][m] = frame.ReadBits(9);
					_globalGain[l][m] = GAIN_TAB[frame.ReadBits(8)];
					_scalefacCompress[l][m] = frame.ReadBits(9);
					_blockSplitFlag[l][m] = frame.ReadBits(1) == 1;
					if (_blockSplitFlag[l][m])
					{
						_blockType[l][m] = frame.ReadBits(2);
						_mixedBlockFlag[l][m] = frame.ReadBits(1) == 1;
						_tableSelect[l][m][0] = frame.ReadBits(5);
						_tableSelect[l][m][1] = frame.ReadBits(5);
						_tableSelect[l][m][2] = 0;
						if (_blockType[l][m] == 2 && !_mixedBlockFlag[l][m])
						{
							_regionAddress1[l][m] = 8;
						}
						else
						{
							_regionAddress1[l][m] = 7;
						}
						_regionAddress2[l][m] = 20 - _regionAddress1[l][m];
						_subblockGain[l][m][0] = (float)frame.ReadBits(3) * -2f;
						_subblockGain[l][m][1] = (float)frame.ReadBits(3) * -2f;
						_subblockGain[l][m][2] = (float)frame.ReadBits(3) * -2f;
					}
					else
					{
						_tableSelect[l][m][0] = frame.ReadBits(5);
						_tableSelect[l][m][1] = frame.ReadBits(5);
						_tableSelect[l][m][2] = frame.ReadBits(5);
						_regionAddress1[l][m] = frame.ReadBits(4);
						_regionAddress2[l][m] = frame.ReadBits(3);
						_blockType[l][m] = 0;
						_subblockGain[l][m][0] = 0f;
						_subblockGain[l][m][1] = 0f;
						_subblockGain[l][m][2] = 0f;
					}
					_scalefacScale[l][m] = 0.5f * (1f + (float)frame.ReadBits(1));
					_count1TableSelect[l][m] = frame.ReadBits(1);
				}
			}
		}
		...
		private int ReadLsfScalefactors(int gr, int ch, int chanModeExt)
		{
			...
			if ((chanModeExt & 1) == 1 && ch == 1)
			{
				...
				{
					array[0] = (num3 - 180) % 64 >> 4;
					array[1] = (num3 - 180) % 16 >> 2;
					array[2] = (num3 - 180) % 4;
					array[3] = 0;
					_preflag[gr][ch] = 0;
					num4 = 4;
			...
			int num5 = 0;
			int[] array3 = _sfbBlockCntTab[num4][num2];
			for (int i = 0; i < 4; i++)
			{
				if (array[i] != 0)
				{
					...
					while (num6 < array3[i])
					{
						array2[num5] = _bitRes.GetBits(array[i]);
						num6++;
						num5++;
					}
				}
			...
			return array[0] * array3[0] + array[1] * array3[1] + array[2] * array3[2] + array[3] * array3[3];
		}
		...
		private void ReadSamples(int sfBits, int gr, int ch)
		{
			...
			while (num3 > _bitRes.BitsRead && num4 < 576)
			{
