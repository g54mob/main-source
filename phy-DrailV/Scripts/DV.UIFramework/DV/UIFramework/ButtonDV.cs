...
{
	public class ButtonDV : Button, IClickable, IHoverable
	{
		private InteractableOverrider interactableOverrider;
		...
		public virtual void ToggleInteractable(bool newInteractable)
		{
			if (newInteractable)
			{
				InteractableOverrider obj = interactableOverrider;
				if ((object)obj != null && !obj.CanInteract())
				{
					return;
				}
			}
		...
		{
			base.Awake();
			TryGetComponent<InteractableOverrider>(out interactableOverrider);
