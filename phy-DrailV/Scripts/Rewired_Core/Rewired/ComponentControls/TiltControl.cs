...
namespace Rewired.ComponentControls
{
	...
	public sealed class TiltControl : CustomControllerControl
	{
		...
		private Vector3 GSKqCMXbpoFJKsDTkqEqdSiojHVN
		{
			get
			{
		...
		{
			Vector3 vector = GSKqCMXbpoFJKsDTkqEqdSiojHVN;
		...
		internal override void NSaIxTLXSfKHgYqfDPqUzdSfjLOK()
		{
			if (base.DlyzgeEtPbGSRivIvEmZhBSIEqiU && UTvbNmLtOtvCXnKmzpVoOCmLyTeb)
			{
				if (_useFAxis)
				{
					wJuChGHELKYHUkqGfCzcAspJNjWPB(_forwardTiltCustomControllerElement, _axis2D.yAxis.value, _axis2D.yAxis.buttonActivationThreshold);
				}
				if (_useHAxis)
				{
					wJuChGHELKYHUkqGfCzcAspJNjWPB(_horizontalTiltCustomControllerElement, _axis2D.xAxis.value, _axis2D.xAxis.buttonActivationThreshold);
				}
			}
		}
		...
		public override void ClearValue()
		{
			...
			{
				base.yBVYaZymnHfILCjQopwadWNgxbeH.ClearElementValue(_horizontalTiltCustomControllerElement);
				base.yBVYaZymnHfILCjQopwadWNgxbeH.ClearElementValue(_forwardTiltCustomControllerElement);
			}
		}

		private void rQOFZMVPcZhYwtsTDdhHdFinbtKL()
		{
			if (_useHAxis)
			{
				float rawValue;
				if (GSKqCMXbpoFJKsDTkqEqdSiojHVN == Vector3.zero)
				{
					rawValue = 0f;
				}
				else
				{
					float value = Mathf.Atan2(GSKqCMXbpoFJKsDTkqEqdSiojHVN.x, 0f - GSKqCMXbpoFJKsDTkqEqdSiojHVN.y) * 57.29578f + _horizontalRestAngle;
			...
			if (_useFAxis)
			{
				...
				{
					float value2 = Mathf.Atan2(GSKqCMXbpoFJKsDTkqEqdSiojHVN.z, 0f - GSKqCMXbpoFJKsDTkqEqdSiojHVN.y) * 57.29578f + _forwardRestAngle;
		...
		private void FgUIMbAtgnIDIdtFHCuWgYTiuTXDA()
		{
			yrqYbeKIijUVZAnVVTnpKMepYUwF(_allowedTiltDirections);
			if (UTvbNmLtOtvCXnKmzpVoOCmLyTeb)
			{
				if (_useHAxis)
				{
					base.yBVYaZymnHfILCjQopwadWNgxbeH.ValidateElements(_horizontalTiltCustomControllerElement);
				}
				if (_useFAxis)
				{
					base.yBVYaZymnHfILCjQopwadWNgxbeH.ValidateElements(_forwardTiltCustomControllerElement);
				}
			}
		}

		private void yrqYbeKIijUVZAnVVTnpKMepYUwF(TiltDirection P_0)
		{
