...
namespace Rewired.Data.Mapping
{
	public sealed class HardwareJoystickTemplateMap : HardwareControllerTemplateMap, IHardwareControllerMap, IHardwareControllerTemplateMap, IHardwareControllerMap_Internal, IHardwareControllerTemplateMap_Internal
	{
		...
		private sealed class qLBKyQdFmdfCYwHkbxwYbNnQaZLS : IDisposable, IEnumerable, IEnumerator, IEnumerable<IControllerElementIdentifierCommon_Internal>, IEnumerator<IControllerElementIdentifierCommon_Internal>
		{
			...
			public HardwareJoystickTemplateMap zITtixdgVFWlEnpDnrTdnZsdTFkt;
			...
			[DebuggerHidden]
			public qLBKyQdFmdfCYwHkbxwYbNnQaZLS(int P_0)
			{
			...
			private bool MoveNext()
			{
				...
				HardwareJoystickTemplateMap hardwareJoystickTemplateMap = zITtixdgVFWlEnpDnrTdnZsdTFkt;
				switch (num)
				{
				default:
					return false;
				...
				if (XFqmAWzGaybkkIOLbVBNhzaWDOgGA < hardwareJoystickTemplateMap.elementIdentifiers.Length)
				{
					vjnbYLtrPMftzpjohNfommerCnGo = hardwareJoystickTemplateMap.elementIdentifiers[XFqmAWzGaybkkIOLbVBNhzaWDOgGA];
					hMnbMujJvihgLcBmOvURwCGCKZDT = 1;
					return true;
				}
				return false;
			...
			{
				qLBKyQdFmdfCYwHkbxwYbNnQaZLS qLBKyQdFmdfCYwHkbxwYbNnQaZLS2;
				if (hMnbMujJvihgLcBmOvURwCGCKZDT == -2 && AyagikQIJAatoHzFlyaifyWyaTktA == Thread.CurrentThread.ManagedThreadId)
				{
					...
					qLBKyQdFmdfCYwHkbxwYbNnQaZLS2 = this;
				}
				else
				{
					qLBKyQdFmdfCYwHkbxwYbNnQaZLS2 = new qLBKyQdFmdfCYwHkbxwYbNnQaZLS(0);
					qLBKyQdFmdfCYwHkbxwYbNnQaZLS2.zITtixdgVFWlEnpDnrTdnZsdTFkt = zITtixdgVFWlEnpDnrTdnZsdTFkt;
				}
				return qLBKyQdFmdfCYwHkbxwYbNnQaZLS2;
			}
		...
		private sealed class CJJPOAAXOGPYBaSfsJjRjlspPHMd : IDisposable, IEnumerable, IEnumerator, IEnumerable<ControllerTemplateElementIdentifier>, IEnumerator<ControllerTemplateElementIdentifier>
		{
			private int hMnbMujJvihgLcBmOvURwCGCKZDT;

			private ControllerTemplateElementIdentifier vjnbYLtrPMftzpjohNfommerCnGo;
			...
			[DebuggerHidden]
			public CJJPOAAXOGPYBaSfsJjRjlspPHMd(int P_0)
			{
				hMnbMujJvihgLcBmOvURwCGCKZDT = P_0;
				AyagikQIJAatoHzFlyaifyWyaTktA = Thread.CurrentThread.ManagedThreadId;
			}
			...
			{
				CJJPOAAXOGPYBaSfsJjRjlspPHMd cJJPOAAXOGPYBaSfsJjRjlspPHMd;
				if (hMnbMujJvihgLcBmOvURwCGCKZDT == -2 && AyagikQIJAatoHzFlyaifyWyaTktA == Thread.CurrentThread.ManagedThreadId)
				{
					hMnbMujJvihgLcBmOvURwCGCKZDT = 0;
					cJJPOAAXOGPYBaSfsJjRjlspPHMd = this;
				}
				else
				{
					cJJPOAAXOGPYBaSfsJjRjlspPHMd = new CJJPOAAXOGPYBaSfsJjRjlspPHMd(0);
					cJJPOAAXOGPYBaSfsJjRjlspPHMd.zITtixdgVFWlEnpDnrTdnZsdTFkt = zITtixdgVFWlEnpDnrTdnZsdTFkt;
				}
				return cJJPOAAXOGPYBaSfsJjRjlspPHMd;
			}
		...
		[NonSerialized]
		private Func<Guid, Entry> KGMKRQdSrgsbboSNURGQaIeolHsA;
		...
		public IEnumerable<ControllerTemplateElementIdentifier> ElementIdentifiers => new CJJPOAAXOGPYBaSfsJjRjlspPHMd(-2)
		{
		...
		IEnumerable<IControllerElementIdentifierCommon_Internal> IHardwareControllerMap_Internal.ElementIdentifiers => new qLBKyQdFmdfCYwHkbxwYbNnQaZLS(-2)
		{
			zITtixdgVFWlEnpDnrTdnZsdTFkt = this
		};
		...
		private Func<Guid, Entry> eCFtfNMnVMcKSDoGRlrovRmmxMEvA => jcQIPleqWWsZNlvEYGkHBahJWVvN;
		...
		public override ControllerTemplateElementIdentifier GetElementIdentifier(int id)
		{
			...
			return QFJfdAHwNyUBCHQpkstuLyHSqLCHA(array, id);
		}
		...
		{
			ControllerTemplateElementIdentifier[] array = elementIdentifiers;
			return XfWKThfjxlBGujZnuTEvKdeGvqmD(array, id);
		}
		...
		internal bool bzBWCkBhyIdyYsaeDfXdNriXIRps(ControllerMap_Editor P_0, HardwareJoystickMap P_1, Guid P_2, out string P_3)
		{
			if (P_0 == null)
			{
			...
			Entry entry = jcQIPleqWWsZNlvEYGkHBahJWVvN(P_2);
			if (entry == null)
			{
				Guid guid = P_2;
				...
				using (TempListPool.TList<ElementIdentifierMap> tList2 = TempListPool.GetTList<ElementIdentifierMap>())
				{
					...
					for (int i = 0; i < actionElementMaps.Count; i++)
					{
						...
						for (int j = 0; j < list2.Count; j++)
						{
							...
							bool flag = false;
							ControllerTemplateElementIdentifier[] array = elementIdentifiers;
							int num = OqoOmyaNbpCRLjwHhjHHSFdrTJydA(array, elementIdentifierId);
							if (num >= 0 && num < elementIdentifiers.Length)
							{
								...
								if (elementIdentifier != null && controllerTemplateElementIdentifier_Editor != null)
								{
									...
									if (!uAOMfTHsnTLbvEUpHTchXYOhMgjh.TUibHCXgdJpNwgxVPYRazOMZLYAI(effectiveElementType, elementIdentifier.elementType))
									{
		...
		internal OkTaTaYFMOwbkgTtCFcCRyxWNNrJ wkfwLVeIbRSCbqMVBrXniCZhzBMA()
		{
			int num = ((joysticks != null) ? joysticks.Count : 0);
			...
			return new OkTaTaYFMOwbkgTtCFcCRyxWNNrJ(this, list, array);
		}

		private Entry jcQIPleqWWsZNlvEYGkHBahJWVvN(Guid P_0)
		{
			if (joysticks == null)
			{
				return null;
		...
		{
			return YKaEqGcaZJHnGQNjalomhNUXInAc(this, controller, elementIdentifierId, eCFtfNMnVMcKSDoGRlrovRmmxMEvA);
		}
		...
		{
			return RakZrwoFBOPwyzjkaeEXghlIxchT(this, controller, elementIdentifierId, eCFtfNMnVMcKSDoGRlrovRmmxMEvA);
		}

		internal static fMlgSaItucfCTlOMuaOrAzViaQaCA YKaEqGcaZJHnGQNjalomhNUXInAc(IHardwareControllerTemplateMap_Internal P_0, Controller P_1, int P_2, Func<Guid, Entry> P_3)
		{
			if (P_0 == null)
			{
				return null;
			...
			Entry entry = P_3(P_1.FZUSYXsTFrKCEfDGTdZDqHMyUGhC);
			if (entry == null)
			{
				return null;
			}
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (elementIdentifierMap != null && elementIdentifierMap.templateId == P_2)
				{
					if (elementIdentifierMap.splitAxis)
					{
						return new fMlgSaItucfCTlOMuaOrAzViaQaCA(ControllerTemplateElementType.Axis, true, new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, -1, AxisRange.Full), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, elementIdentifierMap.joystickId, AxisRange.Positive), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, elementIdentifierMap.joystickId2, AxisRange.Positive));
					}
					return new fMlgSaItucfCTlOMuaOrAzViaQaCA(ControllerTemplateElementType.Axis, false, new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, elementIdentifierMap.joystickId, AxisRange.Full), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, -1, AxisRange.Positive), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, -1, AxisRange.Positive));
				}
			}
			return null;
		}

		internal static fMlgSaItucfCTlOMuaOrAzViaQaCA RakZrwoFBOPwyzjkaeEXghlIxchT(IHardwareControllerTemplateMap_Internal P_0, Controller P_1, int P_2, Func<Guid, Entry> P_3)
		{
			if (P_0 == null)
			{
				return null;
			...
			for (int i = 0; i < count; i++)
			{
				...
				if (elementIdentifierMap != null && elementIdentifierMap.templateId == P_2)
				{
					return new fMlgSaItucfCTlOMuaOrAzViaQaCA(ControllerTemplateElementType.Button, false, new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, elementIdentifierMap.joystickId, AxisRange.Full), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, -1, AxisRange.Positive), new WortGyCOkKTpqRUAkJvQBKSaUPen(P_1, -1, AxisRange.Positive));
				}
			}
			return null;
		}

		internal static ControllerTemplateElementIdentifier QFJfdAHwNyUBCHQpkstuLyHSqLCHA(ControllerTemplateElementIdentifier[] P_0, int P_1)
		{
			int num = OqoOmyaNbpCRLjwHhjHHSFdrTJydA(P_0, P_1);
		...
		internal static int OqoOmyaNbpCRLjwHhjHHSFdrTJydA(ControllerTemplateElementIdentifier[] P_0, int P_1)
		{
			if (P_0 == null)
			{
				return -1;
		...
		internal static bool XfWKThfjxlBGujZnuTEvKdeGvqmD(ControllerTemplateElementIdentifier[] P_0, int P_1)
		{
			return OqoOmyaNbpCRLjwHhjHHSFdrTJydA(P_0, P_1) >= 0;
		}
	}
}
