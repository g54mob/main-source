...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal QBdQosWNnkVEuHmqVyCFNjseiAQp JVngcmKUsmCfXifxiicevJUYkKLw()
			{
				return new QBdQosWNnkVEuHmqVyCFNjseiAQp(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.QMsaCCjejTLLpcQRJGrdNeitKrRP MWrElALuoTPPAYbLsGATPtERRDZH()
			{
				return new ControllerMapLayoutManager.QMsaCCjejTLLpcQRJGrdNeitKrRP(_enabled, _loadFromUserDataStore, HKibeNPWhkerzgnLtBcYFAGFHnxk());
			}

			private RVSinwoHOjvLcEIstDsoKicftFIlA[] HKibeNPWhkerzgnLtBcYFAGFHnxk()
			{
				List<RVSinwoHOjvLcEIstDsoKicftFIlA> list = new List<RVSinwoHOjvLcEIstDsoKicftFIlA>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].kprwecoLoyOWGNYtNemvAqZRVPak());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.KiSiAESwVlRDyuCSgIKOzduAyJHX MWrElALuoTPPAYbLsGATPtERRDZH()
			{
				return new ControllerMapEnabler.KiSiAESwVlRDyuCSgIKOzduAyJHX(_enabled, HKibeNPWhkerzgnLtBcYFAGFHnxk());
			}
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal RVSinwoHOjvLcEIstDsoKicftFIlA kprwecoLoyOWGNYtNemvAqZRVPak()
			{
				return new RVSinwoHOjvLcEIstDsoKicftFIlA(_id, _enabled);
			}
		...
		internal WWVuXrRYVOzShWUocNjzkxVTwGrG JWdnIGZSohotIgnIhATKypsQkaCP()
		{
			QBdQosWNnkVEuHmqVyCFNjseiAQp[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new QBdQosWNnkVEuHmqVyCFNjseiAQp[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].JVngcmKUsmCfXifxiicevJUYkKLw();
				}
			}
			QBdQosWNnkVEuHmqVyCFNjseiAQp[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new QBdQosWNnkVEuHmqVyCFNjseiAQp[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].JVngcmKUsmCfXifxiicevJUYkKLw();
				}
			}
			QBdQosWNnkVEuHmqVyCFNjseiAQp[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new QBdQosWNnkVEuHmqVyCFNjseiAQp[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].JVngcmKUsmCfXifxiicevJUYkKLw();
				}
			}
			QBdQosWNnkVEuHmqVyCFNjseiAQp[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new QBdQosWNnkVEuHmqVyCFNjseiAQp[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].JVngcmKUsmCfXifxiicevJUYkKLw();
				}
			}
			return new WWVuXrRYVOzShWUocNjzkxVTwGrG(array, array2, array3, array4);
		}
	}
}
