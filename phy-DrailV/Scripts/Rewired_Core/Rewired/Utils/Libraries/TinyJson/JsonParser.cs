...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		[Serializable]
		private sealed class gdxhzCkNyobNwgKXZhYSKgbjWrWA
		{
			public static readonly gdxhzCkNyobNwgKXZhYSKgbjWrWA _003C_003E9 = new gdxhzCkNyobNwgKXZhYSKgbjWrWA();
			...
			internal bool SVLrStkcdGeKRxEhjCdgSFqGdFQP(FieldInfo P_0)
			{
			...
			internal string seUNUbBpgWsmAUAdicpQWodNIGUs(FieldInfo P_0)
			{
				string name;
			...
			internal bool BxcdRtgsZFOFavmeImedCBvQniEX(PropertyInfo P_0)
			{
			...
			internal string ckkexdKyGXCYxYdeiLtFQeNoePvc(PropertyInfo P_0)
			{
				string name;
				if (P_0.IsDefined(typeof(SerializeAttribute), inherit: true) && !string.IsNullOrEmpty(name = (CollectionTools.GetValue(P_0.GetCustomAttributes(typeof(SerializeAttribute), inherit: true), 0) as SerializeAttribute).Name))
				{
		...
		private static StringBuilder iSZUHoaffDPVKgXyRDwfAUZSNIHW = new StringBuilder();
		...
		internal static bool TryFromJson<T>(string json, out T value, Type preferredAnonymousObjectType)
		{
			try
			{
				...
				iSZUHoaffDPVKgXyRDwfAUZSNIHW.Length = 0;
				for (int i = 0; i < json.Length; i++)
				{
					char c = json[i];
					if (c == '"')
					{
						i = ZjCXPuIouHjNgNoowfWqgSnRFqCiA(true, i, json);
					}
					else if (!char.IsWhiteSpace(c))
					{
						iSZUHoaffDPVKgXyRDwfAUZSNIHW.Append(c);
					}
				}
				value = (T)vaNatbXSurgueByDblhYsFTZCsft(typeof(T), iSZUHoaffDPVKgXyRDwfAUZSNIHW.ToString(), preferredAnonymousObjectType, out var _);
				return true;
			}
			catch
			{
		...
		internal static T FromJson<T>(string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return (T)vaNatbXSurgueByDblhYsFTZCsft(typeof(T), iSZUHoaffDPVKgXyRDwfAUZSNIHW.ToString(), preferredAnonymousObjectType, out flag);
		}
		...
		internal static object FromJson(Type type, string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return vaNatbXSurgueByDblhYsFTZCsft(type, iSZUHoaffDPVKgXyRDwfAUZSNIHW.ToString(), preferredAnonymousObjectType, out flag);
		}

		private static object vaNatbXSurgueByDblhYsFTZCsft(Type P_0, string P_1, Type P_2, out bool P_3)
		{
			...
			if ((object)P_0 == typeof(Guid))
			{
				...
				{
					bool flag;
					string g = (string)vaNatbXSurgueByDblhYsFTZCsft(typeof(string), P_1, P_2, out flag);
					if (!flag)
					{
						P_3 = false;
						return Guid.Empty;
			...
			{
				object obj2 = vaNatbXSurgueByDblhYsFTZCsft(ReflectionTools.GetUnderlyingEnumType(P_0), P_1, P_2, out var flag2);
				...
				try
				{
					obj2 = vaNatbXSurgueByDblhYsFTZCsft(typeof(string), P_1, P_2, out flag2);
			...
			{
				return kRsIBzpDFkKCZmsGIeUOusEGnZcu(P_1, P_2, out P_3);
			}
			if (ReflectionTools.IsArray(P_0))
			{
				...
				List<string> list = yAENRAJvIWtdlZGWUuxlIqWvcFRi(P_1);
				Array array = Array.CreateInstance(elementType, list.Count);
				for (int i = 0; i < list.Count; i++)
				{
					array.SetValue(vaNatbXSurgueByDblhYsFTZCsft(elementType, list[i], P_2, out var _), i);
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(List<>))
			{
				...
				List<string> list3 = yAENRAJvIWtdlZGWUuxlIqWvcFRi(P_1);
				for (int j = 0; j < list3.Count; j++)
				{
					list2.Add(vaNatbXSurgueByDblhYsFTZCsft(type, list3[j], P_2, out var _));
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(Dictionary<, >))
			{
				...
				List<string> list4 = yAENRAJvIWtdlZGWUuxlIqWvcFRi(P_1);
				try
				{
					if (list4.Count % 2 != 0)
					{
					...
					for (int k = 0; k < list4.Count; k += 2)
					{
						if (list4[k].Length > 2)
						{
							...
							bool flag6;
							object value = vaNatbXSurgueByDblhYsFTZCsft(type3, list4[k + 1], P_2, out flag6);
			...
			{
				P_3 = true;
				return xVuxlGPvuBEROitPaTvomeAcJdJl(P_0, P_1, P_2);
			}
			P_3 = false;
			return null;
		}

		private static object kRsIBzpDFkKCZmsGIeUOusEGnZcu(string P_0, Type P_1, out bool P_2)
		{
			if (P_0.Length == 0)
			{
				P_2 = false;
			...
			{
				List<string> list = yAENRAJvIWtdlZGWUuxlIqWvcFRi(P_0);
				try
				{
					if (list.Count % 2 != 0)
					{
					...
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddKeyValue<string, object>)))
					{
						...
						{
							addKeyValue.Add(list[i].Substring(1, list[i].Length - 2), kRsIBzpDFkKCZmsGIeUOusEGnZcu(list[i + 1], P_1, out var _));
						}
						P_2 = true;
						return addKeyValue;
					}
					...
					{
						dictionary.Add(list[j].Substring(1, list[j].Length - 2), kRsIBzpDFkKCZmsGIeUOusEGnZcu(list[j + 1], P_1, out var _));
					}
					P_2 = true;
					return dictionary;
				}
			...
			if (P_0[0] == '[' && P_0[P_0.Length - 1] == ']')
			{
				List<string> list2 = yAENRAJvIWtdlZGWUuxlIqWvcFRi(P_0);
				try
				{
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddValue<object>)))
					{
						...
						{
							addValue.Add(kRsIBzpDFkKCZmsGIeUOusEGnZcu(list2[k], P_1, out var _));
						}
						P_2 = true;
						return addValue;
					}
					...
					{
						list3.Add(kRsIBzpDFkKCZmsGIeUOusEGnZcu(list2[l], P_1, out var _));
					}
					P_2 = true;
					return list3;
				}
		...
		private static object xVuxlGPvuBEROitPaTvomeAcJdJl(Type P_0, string P_1, Type P_2)
		{
			...
			try
			{
				...
				{
					value = ReflectionTools.GetFields(P_0, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic).Where(gdxhzCkNyobNwgKXZhYSKgbjWrWA._003C_003E9.SVLrStkcdGeKRxEhjCdgSFqGdFQP).ToDictionary(gdxhzCkNyobNwgKXZhYSKgbjWrWA._003C_003E9.seUNUbBpgWsmAUAdicpQWodNIGUs);
					dbimPDJIbNDiyJpDNwWaXGjOqRJA.Add(P_0, value);
				}
				if (!DWofKVDYuXJsQrJvXBQmPzZkZSKQ.TryGetValue(P_0, out var value2))
				{
					value2 = ReflectionTools.GetProperties(P_0, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic).Where(gdxhzCkNyobNwgKXZhYSKgbjWrWA._003C_003E9.BxcdRtgsZFOFavmeImedCBvQniEX).ToDictionary(gdxhzCkNyobNwgKXZhYSKgbjWrWA._003C_003E9.ckkexdKyGXCYxYdeiLtFQeNoePvc);
				...
				for (int i = 0; i < list.Count; i += 2)
				{
					if (list[i].Length > 2)
					{
						...
						{
							value3.SetValue(obj, vaNatbXSurgueByDblhYsFTZCsft(value3.FieldType, text, P_2, out var _));
						}
						else if (value2.TryGetValue(key, out value4) && value4.CanWrite)
						{
							value4.SetValue(obj, vaNatbXSurgueByDblhYsFTZCsft(value4.PropertyType, text, P_2, out var _), null);
						}
					}
				}
		...
		private static int ZjCXPuIouHjNgNoowfWqgSnRFqCiA(bool P_0, int P_1, string P_2)
		{
			iSZUHoaffDPVKgXyRDwfAUZSNIHW.Append(P_2[P_1]);
			for (int i = P_1 + 1; i < P_2.Length; i++)
			{
				if (P_2[i] == '\\')
				{
					if (P_0)
					{
						iSZUHoaffDPVKgXyRDwfAUZSNIHW.Append(P_2[i]);
					}
					iSZUHoaffDPVKgXyRDwfAUZSNIHW.Append(P_2[i + 1]);
					i++;
				}
				else
				{
		...
		private static List<string> yAENRAJvIWtdlZGWUuxlIqWvcFRi(string P_0)
		{
			...
			for (int i = 1; i < P_0.Length - 1; i++)
			{
				...
				case '}':
					...
					break;
				case '"':
					i = ZjCXPuIouHjNgNoowfWqgSnRFqCiA(true, i, P_0);
					continue;
				case ',':
				case ':':
					if (num == 0)
					{
						list.Add(iSZUHoaffDPVKgXyRDwfAUZSNIHW.ToString());
						iSZUHoaffDPVKgXyRDwfAUZSNIHW.Length = 0;
						continue;
					}
					break;
				}
				iSZUHoaffDPVKgXyRDwfAUZSNIHW.Append(P_0[i]);
			}
			if (iSZUHoaffDPVKgXyRDwfAUZSNIHW.Length == 0)
			{
				return list;
			}
			list.Add(iSZUHoaffDPVKgXyRDwfAUZSNIHW.ToString());
