...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		private static Action<StringBuilder, object> NsHoLhBRPdijHmkzTBqEFHsLOGCaA;

		private static Action<StringBuilder, object> JVeBHDnquRLBGCiHlYzxxwkJFYBn => kJCPlJUOCkGffyLCrJzUoCJBaVZk;

		public static string ToJson(object item)
		{
			StringBuilder stringBuilder = new StringBuilder();
			kJCPlJUOCkGffyLCrJzUoCJBaVZk(stringBuilder, item);
			return stringBuilder.ToString();
		}

		private static void kJCPlJUOCkGffyLCrJzUoCJBaVZk(StringBuilder P_0, object P_1)
		{
			if (P_1 == null)
			{
				P_0.Append("null");
			...
			{
				((IExportToJson)P_1).WriteJson(P_0, JVeBHDnquRLBGCiHlYzxxwkJFYBn);
				return;
			}
			if ((object)type == typeof(string))
			{
				P_0.Append('"');
				leBigsJxkNkLZroaanjeKnAXFZxsA(P_0, (string)P_1);
				P_0.Append('"');
				return;
			}
			...
			{
				kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, P_1.ToString());
				return;
			}
			if (ReflectionTools.IsEnum(type))
			{
				Type underlyingType = Enum.GetUnderlyingType(type);
				kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, Convert.ChangeType(P_1, underlyingType));
				return;
			}
			if (ReflectionTools.DoesTypeImplement(type, typeof(IList)))
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, list[i]);
				}
				P_0.Append(']');
				return;
			}
			if (ReflectionTools.IsGenericType(type) && ReflectionTools.DoesTypeImplement(type, typeof(IDictionary)))
			{
				...
				foreach (object key in dictionary.Keys)
				{
					...
					P_0.Append("\":");
					kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, dictionary[key]);
				}
				P_0.Append('}');
				return;
			}
			...
			foreach (FieldInfo field in ReflectionTools.GetFields(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value != null)
				{
					...
					P_0.Append(name);
					P_0.Append("\":");
					kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, value);
				}
			}
			foreach (PropertyInfo property in ReflectionTools.GetProperties(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value2 != null)
				{
					...
					P_0.Append(name2);
					P_0.Append("\":");
					kJCPlJUOCkGffyLCrJzUoCJBaVZk(P_0, value2);
				}
			}
			P_0.Append('}');
		}

		private static void leBigsJxkNkLZroaanjeKnAXFZxsA(StringBuilder P_0, string P_1)
		{
