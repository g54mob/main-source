...
namespace Rewired.Utils
{
	...
	{
		private static string KIuvdBCNXYilYBEJvsSZlDzFmuSP;
		...
		static StringTools()
		{
			KIuvdBCNXYilYBEJvsSZlDzFmuSP = Regex.Escape(new string(Path.GetInvalidFileNameChars()));
		}

		public static string CleanUpFileName(string name)
		{
			name = name.Trim();
			string pattern = "[ ~`,:;'\\.\\$\\^\\{\\}\\[\\]\\(\\|\\)\\*\\+\\?\\\\" + KIuvdBCNXYilYBEJvsSZlDzFmuSP + "]";
