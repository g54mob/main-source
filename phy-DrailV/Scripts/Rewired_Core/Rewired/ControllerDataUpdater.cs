...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] RSZRayPEQJnyEGCDqJIZBSLkdTHd;
		...
		public ControllerDataUpdater(InputSource P_0, int P_1, int P_2, UnknownControllerHat[] P_3)
		{
			...
			axisCount = P_1;
			buttonCount = P_2;
			RSZRayPEQJnyEGCDqJIZBSLkdTHd = P_3;
		...
		{
			if (RSZRayPEQJnyEGCDqJIZBSLkdTHd == null)
			{
				return false;
			}
			for (int i = 0; i < RSZRayPEQJnyEGCDqJIZBSLkdTHd.Length; i++)
			{
				if (RSZRayPEQJnyEGCDqJIZBSLkdTHd[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < RSZRayPEQJnyEGCDqJIZBSLkdTHd.Length; i++)
			{
				if (RSZRayPEQJnyEGCDqJIZBSLkdTHd[i].ContainsButtonIndex(buttonIndex))
				{
					return RSZRayPEQJnyEGCDqJIZBSLkdTHd[i].GetButtons();
				}
			}
			return null;
		}
