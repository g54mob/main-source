...
namespace Rewired
{
	...
	{
		private enum JEbUbqLkBgYUboOdNmVaaNVfsfBH
		{
			Error = 0,
			FoundIndex = 1,
			IsWholeElement = 2
		...
		public DeviceLocalizationInfo deviceLocalizationInfo => epyWrMiKarPbsrBGIHCyAJPFVlJb;
		...
		{
			controllerName = P_0;
			epyWrMiKarPbsrBGIHCyAJPFVlJb = P_1;
			if (epyWrMiKarPbsrBGIHCyAJPFVlJb == null)
			{
				epyWrMiKarPbsrBGIHCyAJPFVlJb = new DeviceLocalizationInfo();
			}
			...
			for (int i = 0; i < P_2.Length; i++)
			{
				...
				if (flag)
				{
					if (ControllerElementIdentifier.kFJMIubFNRLrPnUoPEZxqkAJZTEm.XoWrPhuuoYdElFYmsPRgFLepADbg(epyWrMiKarPbsrBGIHCyAJPFVlJb, P_2[i], out var controllerElementIdentifier))
					{
						P_2[i] = controllerElementIdentifier;
						continue;
					}
					ControllerElementIdentifier.kFJMIubFNRLrPnUoPEZxqkAJZTEm.fyeqCafQbFyflbNbajUvornPxfgy(epyWrMiKarPbsrBGIHCyAJPFVlJb, P_2[i]);
				}
				P_2[i].FinishRuntimeSetup(epyWrMiKarPbsrBGIHCyAJPFVlJb, epyWrMiKarPbsrBGIHCyAJPFVlJb.controllerType);
			}
			...
			for (int k = 0; k < elementIdentifierCount; k++)
			{
				...
				elementIdentifiers.Add(P_2[k].id, P_2[k]);
			}
			for (int l = 0; l < num; l++)
			{
				int num6 = YPhAmxneoBGkuANeQwTybiKgdmbH(P_2, P_3[l]);
				if (num6 < 0)
				{
			...
			{
				int num7 = YPhAmxneoBGkuANeQwTybiKgdmbH(P_2, P_4[m]);
				if (num7 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num8 = YPhAmxneoBGkuANeQwTybiKgdmbH(P_2, array[n]);
				if (num8 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num10 = YPhAmxneoBGkuANeQwTybiKgdmbH(P_2, array2[num9]);
				if (num10 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num12 = YPhAmxneoBGkuANeQwTybiKgdmbH(P_2, array3[num11]);
				if (num12 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
		...
		public bool TryGetCompoundElementMemberCombinedLocalizedName(IList<ActionElementMap> aems, out string result)
		{
			...
			for (int i = 0; i < compoundElements.Length; i++)
			{
				...
				int num2;
				switch (ulOZThfslkBrbXnrdFRskZoSAvtfA(compoundElement, elementIdentifierById, aems, out num2))
				{
				case JEbUbqLkBgYUboOdNmVaaNVfsfBH.IsWholeElement:
					result = elementIdentifierById.name;
					break;
				case JEbUbqLkBgYUboOdNmVaaNVfsfBH.FoundIndex:
					if (num2 >= 0)
					{
		...
		private int YPhAmxneoBGkuANeQwTybiKgdmbH(ControllerElementIdentifier[] P_0, int P_1)
		{
			if (P_0 == null)
			{
				return -1;
		...
		private static JEbUbqLkBgYUboOdNmVaaNVfsfBH ulOZThfslkBrbXnrdFRskZoSAvtfA(HardwareJoystickMap.CompoundElement P_0, ControllerElementIdentifier P_1, IList<ActionElementMap> P_2, out int P_3)
		{
			P_3 = -1;
			if (P_0 == null || P_1 == null || P_2 == null)
			{
				return JEbUbqLkBgYUboOdNmVaaNVfsfBH.Error;
			}
			...
			JSWalVgNfayAAqqgkCDSfWJdaAMTB.exlqDuEbMnCkpHMuXAQdzpkhfUUr exlqDuEbMnCkpHMuXAQdzpkhfUUr;
			...
			case CompoundControllerElementType.Axis2D:
			{
				...
				if (num3 == 1)
				{
					if (num4 == 1)
					{
						return JEbUbqLkBgYUboOdNmVaaNVfsfBH.IsWholeElement;
					}
					if (num7 == 1 && num8 == 1)
					{
						return JEbUbqLkBgYUboOdNmVaaNVfsfBH.IsWholeElement;
				...
				else if (num5 == 1 && num6 == 1)
				{
					...
					if (JSWalVgNfayAAqqgkCDSfWJdaAMTB.HiplrqmAxSeQEXpdvOXYKTZEASQi(resultCompoundElementType, out exlqDuEbMnCkpHMuXAQdzpkhfUUr))
					{
						P_3 = exlqDuEbMnCkpHMuXAQdzpkhfUUr.PujFpIgnaejxCcbCzrcoRIpZaecab(AxisDirection.Horizontal);
						if (P_3 >= 0)
						{
							return JEbUbqLkBgYUboOdNmVaaNVfsfBH.FoundIndex;
						}
						return JEbUbqLkBgYUboOdNmVaaNVfsfBH.Error;
					}
				}
				else
				{
					...
					if (JSWalVgNfayAAqqgkCDSfWJdaAMTB.HiplrqmAxSeQEXpdvOXYKTZEASQi(resultCompoundElementType, out exlqDuEbMnCkpHMuXAQdzpkhfUUr))
					{
						P_3 = exlqDuEbMnCkpHMuXAQdzpkhfUUr.PujFpIgnaejxCcbCzrcoRIpZaecab(AxisDirection.Vertical);
						if (P_3 >= 0)
						{
							return JEbUbqLkBgYUboOdNmVaaNVfsfBH.FoundIndex;
						}
