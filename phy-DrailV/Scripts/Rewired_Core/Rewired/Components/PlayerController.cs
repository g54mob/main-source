...
namespace Rewired.Components
{
	...
	public class PlayerController : ComponentWrapper<Rewired.PlayerController>, IPlayerController
	{
		...
		protected override void OnAwakeFinished()
		{
			...
			if (base.initialized)
			{
				nTZMPdDVmlBUunxpVsxHomAuIfkw(true);
			}
		}
		...
		protected override void Subscribe()
		{
			...
			if (base.source != null)
			{
				base.source.ButtonStateChangedEvent += dyboosTFKcanBBgiBQiGOUmrhJFFb;
				base.source.AxisValueChangedEvent += KvDhOKlllDnJVQRNDmKnUouiMevO;
				base.source.EnabledStateChangedEvent += nTZMPdDVmlBUunxpVsxHomAuIfkw;
			}
		}
		...
		internal virtual List<ElementInfo> driRyyGhYfItsSTwrekwkDqrsQXh()
		{
		...
		private void dyboosTFKcanBBgiBQiGOUmrhJFFb(int P_0, bool P_1)
		{
		...
		private void KvDhOKlllDnJVQRNDmKnUouiMevO(int P_0, float P_1)
		{
			if (!base.isActiveAndEnabled)
			{
				return;
		...
		private void nTZMPdDVmlBUunxpVsxHomAuIfkw(bool P_0)
		{
			try
			{
		...
		private void HGpaRBJHToazVZtusUWgtnBlLGiib()
		{
			if (_elements == null || _elements.Count <= 0)
			{
				_elements = driRyyGhYfItsSTwrekwkDqrsQXh();
			}
		}
	}
}
