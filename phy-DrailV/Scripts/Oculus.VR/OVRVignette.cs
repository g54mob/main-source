...
public class OVRVignette : MonoBehaviour
{
	...
	private void BuildMeshes()
	{
		...
		{
			float f = (float)(2 * i) * (float)Math.PI / (float)triangleCount;
			float x = Mathf.Cos(f);
			float y = Mathf.Sin(f);
	...
	private void BuildMaterials()
	{
		...
		if (_OpaqueMaterial == null)
		{
			...
			_OpaqueMaterial.SetFloat("_BlendSrc", 1f);
			_OpaqueMaterial.SetFloat("_BlendDst", 0f);
			_OpaqueMaterial.SetFloat("_ZWrite", 1f);
		}
		...
		if (_TransparentMaterial == null)
		{
			...
			_TransparentMaterial.SetFloat("_BlendSrc", 5f);
			_TransparentMaterial.SetFloat("_BlendDst", 10f);
			_TransparentMaterial.SetFloat("_ZWrite", 0f);
		}
	...
	{
		tanFovY = Mathf.Tan((float)Math.PI / 180f * _Camera.fieldOfView * 0.5f);
	...
	private void Update()
	{
		...
		float num = Mathf.Tan(VignetteFieldOfView * ((float)Math.PI / 180f) * 0.5f);
		float num2 = num * VignetteAspectRatio;
		float num3 = Mathf.Tan((VignetteFieldOfView + VignetteFalloffDegrees) * ((float)Math.PI / 180f) * 0.5f);
