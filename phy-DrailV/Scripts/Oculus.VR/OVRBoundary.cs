...
public class OVRBoundary
{
	...
	public Vector3[] GetGeometry(BoundaryType boundaryType)
	{
		if (OVRManager.loadedXRDevice != OVRManager.XRDevice.Oculus)
		{
			if (Boundary.TryGetGeometry(cachedGeometryList, (boundaryType != BoundaryType.PlayArea) ? Boundary.Type.TrackedArea : Boundary.Type.PlayArea))
			{
	...
	public Vector3 GetDimensions(BoundaryType boundaryType)
	{
		...
		if (Boundary.TryGetDimensions(out var dimensionsOut, (boundaryType != BoundaryType.PlayArea) ? Boundary.Type.TrackedArea : Boundary.Type.PlayArea))
		{
