...
public static class OVRInput
{
	...
	private abstract class OVRControllerBase
	{
		...
		{
			OVRPlugin.ControllerState4 controllerState = ((OVRManager.loadedXRDevice != OVRManager.XRDevice.OpenVR || (controllerType & Controller.Touch) == 0) ? OVRPlugin.GetControllerState4((uint)controllerType) : GetOpenVRControllerState(controllerType));
	...
	public static Vector3 GetLocalControllerPosition(Controller controllerType)
	{
		...
		case Controller.LHand:
		{
			...
			if (OVRNodeStateProperties.GetNodeStatePropertyVector3(XRNode.LeftHand, NodeStatePropertyType.Position, OVRPlugin.Node.HandLeft, stepType, out var retVec2))
			{
				return retVec2;
			}
			return Vector3.zero;
	...
	public static Quaternion GetLocalControllerRotation(Controller controllerType)
	{
		...
		case Controller.LHand:
		{
			...
			if (OVRNodeStateProperties.GetNodeStatePropertyQuaternion(XRNode.LeftHand, NodeStatePropertyType.Orientation, OVRPlugin.Node.HandLeft, stepType, out var retQuat2))
			{
				return retQuat2;
			}
