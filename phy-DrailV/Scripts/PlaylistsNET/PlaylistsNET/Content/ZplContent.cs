...
namespace PlaylistsNET.Content
{
	public class ZplContent : IPlaylistParser<ZplPlaylist>, IPlaylistWriter<ZplPlaylist>
	{
		public string ToText(ZplPlaylist playlist)
		{
			...
			stringBuilder.AppendLine("<?zpl version=\"2.0\"?>");
		...
		{
			ZplPlaylist zplPlaylist = new ZplPlaylist();
			XElement xElement = XDocument.Parse(playlistString).Element("smil");
			XElement xElement2 = xElement.Element("head");
			zplPlaylist.Author = ((string)xElement2.Element("author")) ?? "";
			zplPlaylist.Guid = ((string)xElement2.Element("guid")) ?? "";
			zplPlaylist.Title = ((string)xElement2.Element("title")) ?? "";
			foreach (XElement item in xElement2.Elements("meta"))
			{
				...
				{
				case "Generator":
					zplPlaylist.Generator = text2;
					break;
				case "ItemCount":
				{
					int result2 = 0;
					int.TryParse(text2, out result2);
					zplPlaylist.ItemCount = result2;
					break;
				}
				case "totalDuration":
				{
					...
					zplPlaylist.TotalDuration = TimeSpan.FromMilliseconds(result);
					break;
				}
				}
			}
		...
		private XElement CreateSeqWithMedia(ZplPlaylist playlist)
		{
			...
			foreach (ZplPlaylistEntry playlistEntry in playlist.PlaylistEntries)
			{
