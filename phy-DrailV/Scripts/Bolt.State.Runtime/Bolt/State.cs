...
namespace Bolt
{
	public abstract class State : GraphElement<StateGraph>, IState, IGraphElementWithDebugData, IGraphElement, IGraphItem, INotifiedCollectionItem, IDisposable, IPrewarmable, IAotStubbable, IIdentifiable, IGraphElementWithData
	{
		...
		public virtual void OnEnter(Flow flow, StateEnterReason reason)
		{
			...
			if (flow.enableDebug)
			{
				...
				elementDebugData.lastEnterFrame = EditorTimeBinding.frame;
			}
		...
		public virtual void OnExit(Flow flow, StateExitReason reason)
		{
			...
			{
				DebugData elementDebugData = flow.stack.GetElementDebugData<DebugData>(this);
				elementDebugData.lastExitTime = EditorTimeBinding.time;
			}
			foreach (IStateTransition item in outgoingTransitionsNoAlloc)
			{
				try
