...
namespace Bolt
{
	...
	public sealed class FlowState : NesterState<FlowGraph, FlowMacro>, IGraphEventListener
	{
		...
		public FlowState(FlowMacro macro)
			: base(macro)
		{
		}
		...
		protected override void OnEnterImplementation(Flow flow)
		{
			if (flow.stack.TryEnterParentElement(this))
			{
				...
				flow.stack.TriggerEventHandler((EventHook hook) => hook == "OnEnterState", default(EmptyEventArgs), (IGraphParentElement parent) => parent is SuperUnit, force: false);
		...
		protected override void OnExitImplementation(Flow flow)
		{
			if (flow.stack.TryEnterParentElement(this))
			{
				flow.stack.TriggerEventHandler((EventHook hook) => hook == "OnExitState", default(EmptyEventArgs), (IGraphParentElement parent) => parent is SuperUnit, force: false);
