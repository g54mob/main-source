...
namespace Rewired.UI.ControlMapper
{
	...
	public class CalibrationWindow : Window
	{
		...
		private ControllerWithAxes controller;
		...
		private bool axisSelected
		{
			get
			{
				...
				if (selectedAxis < 0 || selectedAxis >= controller.calibrationMap.axisCount)
				{
					return false;
				}
				return true;
		...
		public AxisCalibration axisCalibration
		{
			get
			{
				...
				return controller.calibrationMap.GetAxis(selectedAxis);
			}
		}
		...
		public void SetJoystick(int playerId, ControllerWithAxes controller)
		{
			if (!base.initialized)
			{
				return;
			...
			for (int i = 0; i < controller.axisCount; i++)
			{
				...
				if (componentInSelfOrChildren != null)
				{
					...
					componentInSelfOrChildren.controllerElementIdentifier = controller.AxisElementIdentifiers[i];
				}
				else
				{
					...
					{
						componentInSelfOrChildren2.text = ControlMapper.GetLanguage().GetElementIdentifierName(controller, controller.AxisElementIdentifiers[i].id, AxisRange.Full);
					}
				}
				if (num == 0f)
				{
			...
			axisScrollAreaContent.sizeDelta = new Vector2(axisScrollAreaContent.sizeDelta.x, Mathf.Max((float)controller.axisCount * (num + spacing) - spacing, axisScrollAreaContent.sizeDelta.y));
			origCalibrationData = controller.calibrationMap.ToXmlString();
		...
		public override void Cancel()
		{
			...
			if (controller != null)
			{
				controller.ImportCalibrationMapFromXmlString(origCalibrationData);
			}
		...
		{
			if (base.initialized && controller != null)
			{
				controller.calibrationMap.Reset();
				RefreshControls();
				Redraw();
			}
		}
		...
		private void RedrawValueMarkers()
		{
			...
			float axis = controller.GetAxis(selectedAxis);
			float num = Mathf.Clamp(controller.GetAxisRaw(selectedAxis), -1f, 1f);
		...
		private bool IsMenuAxis(int actionId, int axisIndex)
		{
			...
			{
				IList<JoystickMap> maps = allPlayers[i].controllers.maps.GetMaps<JoystickMap>(controller.id);
				if (maps == null)
				{
					continue;
				}
