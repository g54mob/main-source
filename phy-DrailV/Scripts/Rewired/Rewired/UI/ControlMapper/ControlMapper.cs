...
namespace Rewired.UI.ControlMapper
{
	...
	public class ControlMapper : MonoBehaviour
	{
		...
		protected class GUIButton : GUIElement
		{
		...
		public class GUIInputField : GUIElement
		{
			...
			public GUIToggle toggle { get; set; }
		...
		public class MappingSet
		{
			...
			public ActionListMode _actionListMode;
			...
			public MappingSet(int mapCategoryId, ActionListMode actionListMode, int[] actionCategoryIds, int[] actionIds)
			{
		...
		public class References
		{
			...
			[SerializeField]
			public Scrollbar _inputGridVScrollbar;

			[SerializeField]
			public ScrollRect _inputGridScrollRect;

			[SerializeField]
			public Transform _inputGridInnerGroup;

			[SerializeField]
			public TMP_Text _controllerNameLabel;
		...
		public class AxisCalibrator
		{
			...
			public readonly ControllerWithAxes controller;
			...
			public AxisCalibrator(ControllerWithAxes controller, int axisIndex)
			{
				data = default(AxisCalibrationData);
				this.controller = controller;
				this.axisIndex = axisIndex;
				if (controller != null && axisIndex >= 0 && controller.axisCount > axisIndex)
				{
					axis = controller.Axes[axisIndex];
					data = controller.calibrationMap.GetAxis(axisIndex).GetData();
				}
				firstRun = true;
			}
			...
			public void Commit()
			{
				if (axis != null)
				{
					AxisCalibration axisCalibration = controller.calibrationMap.GetAxis(axisIndex);
					if (axisCalibration != null && !((double)Mathf.Abs(data.max - data.min) < 0.1))
					{
						data.invert = Mathf.Abs(data.max - data.zero) < 0.1f;
		...
		{
			public InputGridEntryList list;
		...
		public class InputGridEntryList
		{
			...
			{
				public List<ActionEntry> _actionList;

				public IndexedDictionary<int, ActionCategoryEntry> _actionCategoryList;
			...
			{
				public IndexedDictionary<int, FieldSet> fieldSets;
			...
			public IndexedDictionary<int, MapCategoryEntry> entries;
			...
			public ActionEntry AddActionEntry(int mapCategoryId, InputAction action, AxisRange axisRange)
			{
				if (action == null)
				{
					return null;
			...
			public ActionCategoryEntry AddActionCategoryEntry(int mapCategoryId, int actionCategoryId)
			{
				if (!entries.TryGet(mapCategoryId, out var value))
				{
					return null;
			...
			public ActionEntry GetActionEntry(int mapCategoryId, int actionId, AxisRange axisRange)
			{
				if (actionId < 0)
				{
					return null;
			...
			public ActionEntry GetActionEntry(int mapCategoryId, InputAction action, AxisRange axisRange)
			{
				if (action == null)
				{
					return null;
		...
		protected const float blockInputOnFocusTimeout = 0.1f;

		protected const string buttonIdentifier_playerSelection = "PlayerSelection";

		protected const string buttonIdentifier_removeController = "RemoveController";

		protected const string buttonIdentifier_assignController = "AssignController";

		protected const string buttonIdentifier_calibrateController = "CalibrateController";

		protected const string buttonIdentifier_editInputBehaviors = "EditInputBehaviors";

		protected const string buttonIdentifier_mapCategorySelection = "MapCategorySelection";

		protected const string buttonIdentifier_assignedControllerSelection = "AssignedControllerSelection";

		protected const string buttonIdentifier_done = "Done";

		protected const string buttonIdentifier_restoreDefaults = "RestoreDefaults";

		[SerializeField]
		[Tooltip("Must be assigned a Rewired Input Manager scene object or prefab.")]
		protected InputManager _rewiredInputManager;
		...
		protected int _keyboardMapDefaultLayout;

		[SerializeField]
		[Tooltip("The Layout of the Mouse Maps to be displayed.")]
		protected int _mouseMapDefaultLayout;

		[SerializeField]
		[Tooltip("The Layout of the Mouse Maps to be displayed.")]
		protected int _joystickMapDefaultLayout;

		[SerializeField]
		protected MappingSet[] _mappingSets = new MappingSet[1] { MappingSet.Default };

		[SerializeField]
		[Tooltip("Display a selectable list of Players. If your game only supports 1 player, you can disable this.")]
		protected bool _showPlayers = true;

		[SerializeField]
		[Tooltip("Display the Controller column for input mapping.")]
		protected bool _showControllers = true;

		[SerializeField]
		[Tooltip("Display the Keyboard column for input mapping.")]
		protected bool _showKeyboard = true;

		[SerializeField]
		[Tooltip("Display the Mouse column for input mapping.")]
		protected bool _showMouse = true;
		...
		protected int _maxControllersPerPlayer = 1;
		...
		protected bool _showActionCategoryLabels;
		...
		protected int _keyboardInputFieldCount = 2;
		...
		protected int _mouseInputFieldCount = 1;
		...
		protected int _controllerInputFieldCount = 1;
		...
		protected bool _showFullAxisInputFields = true;
		...
		protected bool _showSplitAxisInputFields = true;
		...
		protected bool _showGlyphs = true;
		...
		protected bool _allowElementAssignmentConflicts;
		...
		protected float _controllerAssignmentTimeout = 5f;
		...
		protected float _preInputAssignmentTimeout = 5f;
		...
		protected bool _ignoreMouseXAxisAssignment = true;
		...
		protected bool _ignoreMouseYAxisAssignment = true;
		...
		protected bool _showAssignedControllers = true;

		protected Action _ScreenClosedEvent;

		protected Action _ScreenOpenedEvent;

		protected Action _PopupWindowOpenedEvent;

		protected Action _PopupWindowClosedEvent;

		protected Action _InputPollingStartedEvent;

		protected Action _InputPollingEndedEvent;

		protected Action _ThemeAppliedEvent;

		[SerializeField]
		[Tooltip("Event sent when the UI is closed.")]
		protected UnityEvent _onScreenClosed;

		[SerializeField]
		[Tooltip("Event sent when the UI is opened.")]
		protected UnityEvent _onScreenOpened;

		[SerializeField]
		[Tooltip("Event sent when a popup window is closed.")]
		protected UnityEvent _onPopupWindowClosed;

		[SerializeField]
		[Tooltip("Event sent when a popup window is opened.")]
		protected UnityEvent _onPopupWindowOpened;

		[SerializeField]
		[Tooltip("Event sent when polling for input has started.")]
		protected UnityEvent _onInputPollingStarted;

		[SerializeField]
		[Tooltip("Event sent when polling for input has ended.")]
		protected UnityEvent _onInputPollingEnded;

		protected static ControlMapper Instance;
		...
		[NonSerialized]
		protected int currentMapCategoryId;

		[NonSerialized]
		protected List<GUIButton> playerButtons;

		[NonSerialized]
		protected List<GUIButton> mapCategoryButtons;

		[NonSerialized]
		protected List<GUIButton> assignedControllerButtons;

		protected GUIButton assignedControllerButtonsPlaceholder;

		[NonSerialized]
		protected List<GameObject> miscInstantiatedObjects;

		[NonSerialized]
		protected GameObject canvas;

		[NonSerialized]
		protected GameObject lastUISelection;

		[NonSerialized]
		protected int customControllerId = -1;

		[NonSerialized]
		protected ControllerType currentControllerType = ControllerType.Joystick;

		[NonSerialized]
		protected float blockInputOnFocusEndTime;

		[NonSerialized]
		protected bool isPollingForInput;

		[NonSerialized]
		protected List<ThemedElement> themedElements = new List<ThemedElement>();

		[NonSerialized]
		protected InputMapping pendingInputMapping;

		[NonSerialized]
		protected AxisCalibrator pendingAxisCalibration;

		protected Action<InputFieldInfo> inputFieldActivatedDelegate;

		protected Action<ToggleInfo, bool> inputFieldInvertToggleStateChangedDelegate;

		protected Action _restoreDefaultsDelegate;
		...
		protected int inputGridColumnCount
		{
			get
			{
				int num = 1;
		...
		protected int inputGridWidth => _actionLabelWidth + (_showKeyboard ? _keyboardColMaxWidth : 0) + (_showMouse ? _mouseColMaxWidth : 0) + (_showControllers ? _controllerColMaxWidth : 0) + (inputGridColumnCount - 1) * _inputColumnSpacing;

		protected Player currentPlayer => ReInput.players.GetPlayer(currentPlayerId);

		protected InputCategory currentMapCategory => ReInput.mapping.GetMapCategory(currentMapCategoryId);

		protected MappingSet currentMappingSet
		{
			get
			{
		...
		protected ControllerWithAxes currentController => ReInput.controllers.GetController(currentControllerType, customControllerId) as ControllerWithAxes;

		protected bool isJoystickSelected => customControllerId >= 0;

		protected GameObject currentUISelection
		{
			get
			{
		...
		protected bool showMapCategories
		{
			get
			{
		...
		public static event Action ControlBindingChanged;

		public event Action<GUILabel, InputAction, AxisRange> LabelCreatedDV;
		...
		public event Action<GUIInputField, ControllerType> InputFieldCreated;

		private bool ShouldDoSplitAxis(InputAction action)
		{
			if (_showSplitAxisInputFields)
			{
				return !action.name.Contains("Absolute");
			}
			return false;
		}

		private bool ShouldShowFullAxis(InputAction action)
		{
		...
		private ControllerType GetFieldInfoType(InputFieldInfo fieldInfo)
		{
			if (fieldInfo.controllerType != ControllerType.Joystick)
			{
				return fieldInfo.controllerType;
			}
		...
		protected void OnJoystickConnected(ControllerStatusChangedEventArgs args)
		{
		...
		protected void OnJoystickDisconnected(ControllerStatusChangedEventArgs args)
		{
			if (initialized && _showControllers)
			{
				ClearVarsOnJoystickChange();
		...
		protected void OnJoystickPreDisconnect(ControllerStatusChangedEventArgs args)
		{
			if (initialized)
			{
				_ = _showControllers;
		...
		{
			OnButtonActivated(buttonInfo, force: false);
		}

		public void OnButtonActivated(ButtonInfo buttonInfo, bool force)
		{
			if (initialized && (inputAllowed || force))
			{
				...
				{
					int intData = buttonInfo.intData;
					int joystickId = intData & 0xFFFF;
					ControllerType type = (ControllerType)(intData >> 16);
					OnControllerSelected(joystickId, type);
					break;
				}
		...
		public void OnInputFieldActivated(InputFieldInfo fieldInfo)
		{
			...
			ControllerMap controllerMap = GetControllerMap(GetFieldInfoType(fieldInfo));
		...
		public void OnInputFieldInvertToggleStateChanged(ToggleInfo toggleInfo, bool newState)
		{
			...
			{
				ControllerType controllerType = toggleInfo.controllerType;
				if (controllerType == ControllerType.Joystick)
				{
					controllerType = currentControllerType;
				}
				SetActionAxisInverted(newState, controllerType, toggleInfo.actionElementMapId);
			}
		}

		protected void OnPlayerSelected(int playerId, bool redraw)
		{
			if (initialized)
			{
		...
		protected void OnControllerSelected(int joystickId, ControllerType type)
		{
			if (initialized)
			{
				customControllerId = joystickId;
				currentControllerType = type;
				Redraw(listsChanged: true, playTransitions: true);
			}
		}

		protected void OnRemoveCurrentController()
		{
			if (currentPlayer != null && customControllerId >= 0 && currentControllerType != ControllerType.Custom)
			{
				RemoveController(currentPlayer, customControllerId, currentControllerType);
				ClearVarsOnJoystickChange();
				Redraw(listsChanged: false, playTransitions: false);
			}
		}

		protected void OnMapCategorySelected(int id, bool redraw)
		{
			if (initialized)
			{
		...
		protected void OnRestoreDefaults()
		{
			if (initialized)
			{
		...
		protected void OnScreenToggleActionPressed(InputActionEventData data)
		{
			if (!isOpen)
			{
				Open();
		...
		protected void OnScreenOpenActionPressed(InputActionEventData data)
		{
			Open();
		}

		protected void OnScreenCloseActionPressed(InputActionEventData data)
		{
		...
		protected void OnUniversalCancelActionPressed(InputActionEventData data)
		{
		...
		protected void OnWindowCancel(int windowId)
		{
		...
		protected void OnRemoveElementAssignment(int windowId, ControllerMap map, ActionElementMap aem)
		{
			if (map != null && aem != null)
			{
				map.DeleteElementMap(aem.id);
				CloseWindow(windowId);
				ControlMapper.ControlBindingChanged?.Invoke();
			}
		}

		protected void OnBeginElementAssignment(InputFieldInfo fieldInfo, ControllerMap map, ActionElementMap aem, string actionName)
		{
			if (!(fieldInfo == null) && map != null)
			{
				pendingInputMapping = new InputMapping(actionName, fieldInfo, map, aem, GetFieldInfoType(fieldInfo), fieldInfo.controllerId);
				switch (GetFieldInfoType(fieldInfo))
				{
		...
		protected virtual void OnControllerAssignmentConfirmed(int windowId, Player player, int controllerId, ControllerType controllerType)
		{
			if (windowId >= 0 && player != null && controllerId >= 0)
			{
				AssignController(player, controllerId, controllerType);
				CloseWindow(windowId);
			}
		}

		protected void OnMouseAssignmentConfirmed(int windowId, Player player)
		{
		...
		protected void OnElementAssignmentConflictReplaceConfirmed(int windowId, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers, bool allowSwap)
		{
		...
		protected void OnElementAssignmentAddConfirmed(int windowId, InputMapping mapping, ElementAssignment assignment)
		{
			if (currentPlayer != null && mapping != null)
			{
				mapping.map.ReplaceOrCreateElementMap(assignment);
		...
		protected void OnRestoreDefaultsConfirmed(int windowId)
		{
		...
		protected void OnAssignControllerWindowUpdate(int windowId)
		{
			...
			ControllerPollingInfo info2 = ReInput.controllers.polling.PollAllControllersOfTypeForFirstElementDown(ControllerType.Custom);
			if (!TryInfo(info2))
			{
				TryInfo(info);
			}
			window.SetContentText(Mathf.CeilToInt(window.timer.remaining).ToString(), 1);
			bool TryInfo(ControllerPollingInfo controllerPollingInfo)
			{
				...
				InputPollingStopped();
				if (ReInput.controllers.IsControllerAssigned(controllerPollingInfo.controllerType, controllerPollingInfo.controllerId) && !currentPlayer.controllers.ContainsController(controllerPollingInfo.controllerType, controllerPollingInfo.controllerId))
				{
					ShowControllerAssignmentConflictWindow(controllerPollingInfo.controllerId, controllerPollingInfo.controllerType);
					return false;
				}
				if (controllerPollingInfo.controllerName.Equals("RailDriver"))
				{
					return false;
				}
				OnControllerAssignmentConfirmed(windowId, currentPlayer, controllerPollingInfo.controllerId, controllerPollingInfo.controllerType);
				return true;
			}
		}

		protected void OnElementAssignmentPrePollingWindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
			...
			if (!window.timer.finished)
			{
				...
				case ControllerType.Joystick:
					...
					controllerPollingInfo = ReInput.controllers.polling.PollControllerForFirstButtonDown(pendingInputMapping.controllerType, currentController.id);
					break;
				case ControllerType.Custom:
					if (currentPlayer.controllers.customControllerCount == 0)
					{
						return;
					}
					controllerPollingInfo = ReInput.controllers.polling.PollControllerForFirstButtonDown(pendingInputMapping.controllerType, currentController.id);
		...
		protected void OnJoystickElementAssignmentPollingWindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
			...
			window.SetContentText(Mathf.CeilToInt(window.timer.remaining).ToString(), 1);
			if (currentPlayer.controllers.JoystickAndCustomControllersCount() == 0)
			{
				return;
			}
			ControllerPollingInfo pollingInfo = ReInput.controllers.polling.PollControllerForFirstElementDown(currentControllerType, currentController.id);
		...
		protected void OnKeyboardElementAssignmentPollingWindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
		...
		protected void OnMouseElementAssignmentPollingWindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
		...
		protected void OnCalibrateAxisStep1WindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
			...
			{
				window.SetContentText(Mathf.CeilToInt(window.timer.remaining).ToString(), 1);
				if (currentPlayer.controllers.JoystickAndCustomControllersCount() == 0 || !pendingAxisCalibration.controller.PollForFirstButtonDown().success)
				{
					return;
				}
			}
		...
		protected void OnCalibrateAxisStep2WindowUpdate(int windowId)
		{
			if (currentPlayer == null)
			{
				return;
			...
			if (!window.timer.finished)
			{
				...
				if (currentPlayer.controllers.joystickCount == 0 || !pendingAxisCalibration.controller.PollForFirstButtonDown().success)
				{
					return;
				}
			}
		...
		protected void ShowAssignControllerWindow()
		{
		...
		protected void ShowControllerAssignmentConflictWindow(int controllerId, ControllerType controllerType)
		{
			if (currentPlayer == null || (controllerType == ControllerType.Joystick && ReInput.controllers.joystickCount == 0) || (controllerType == ControllerType.Custom && ReInput.controllers.customControllerCount == 0))
			{
				return;
			}
			Window window = OpenWindow(closeOthers: true);
			...
			for (int i = 0; i < players.Count; i++)
			{
				if (players[i] != currentPlayer && players[i].controllers.ContainsController(controllerType, controllerId))
				{
			...
			window.AddContentText(prefabs.windowContentText, UIPivot.TopCenter, UIAnchor.TopHStretch, new Vector2(0f, -100f), _language.GetControllerAssignmentConflictWindowMessage(_language.GetControllerName(controller), otherPlayerName, _language.GetPlayerName(currentPlayer.id)));
			...
			{
				OnControllerAssignmentConfirmed(window.id, currentPlayer, controllerId, controllerType);
		...
		protected void ShowBeginElementAssignmentReplacementWindow(InputFieldInfo fieldInfo, InputAction action, ControllerMap map, ActionElementMap aem, string actionName)
		{
		...
		protected void ShowCreateNewElementAssignmentWindow(InputFieldInfo fieldInfo, InputAction action, ControllerMap map, string actionName)
		{
		...
		protected void ShowElementAssignmentPrePollingWindow()
		{
		...
		protected void ShowElementAssignmentPollingWindow()
		{
			if (pendingInputMapping == null)
			{
				return;
		...
		protected void ShowJoystickElementAssignmentPollingWindow()
		{
			if (pendingInputMapping != null)
			{
				Window window = OpenWindow(closeOthers: true);
		...
		protected void ShowKeyboardElementAssignmentPollingWindow()
		{
			if (pendingInputMapping != null)
			{
				Window window = OpenWindow(closeOthers: true);
		...
		protected void ShowMouseElementAssignmentPollingWindow()
		{
			if (pendingInputMapping != null)
			{
				Window window = OpenWindow(closeOthers: true);
		...
		protected void ShowElementAssignmentConflictWindow(ElementAssignment assignment, bool skipOtherPlayers)
		{
			if (pendingInputMapping == null)
			{
				return;
		...
		protected void ShowMouseAssignmentConflictWindow()
		{
			if (currentPlayer == null)
			{
				return;
		...
		protected void ShowCalibrateControllerWindow()
		{
			if (currentPlayer != null && currentPlayer.controllers.JoystickAndCustomControllersCount() != 0)
			{
		...
		protected void ShowCalibrateAxisStep1Window()
		{
			if (currentPlayer == null)
			{
				return;
			...
			ControllerWithAxes controller = pendingAxisCalibration.controller;
			if (controller.axisCount == 0)
			{
				return;
			}
			int axisIndex = pendingAxisCalibration.axisIndex;
			if (axisIndex >= 0 && axisIndex < controller.axisCount)
			{
				window.CreateTitleText(prefabs.windowTitleText, Vector2.zero, _language.calibrateAxisStep1WindowTitle);
				window.AddContentText(prefabs.windowContentText, UIPivot.TopCenter, UIAnchor.TopHStretch, new Vector2(0f, -100f), _language.GetCalibrateAxisStep1WindowMessage(_language.GetElementIdentifierName(controller, controller.AxisElementIdentifiers[axisIndex].id, AxisRange.Full)));
				if (prefabs.centerStickGraphic != null)
				{
					window.AddContentImage(prefabs.centerStickGraphic, UIPivot.BottomCenter, UIAnchor.BottomCenter, new Vector2(0f, 40f));
				}
		...
		protected void ShowCalibrateAxisStep2Window()
		{
			if (currentPlayer == null)
			{
				return;
			...
			if (axisIndex >= 0 && axisIndex < controller.axisCount)
			{
				...
				window.AddContentText(prefabs.windowContentText, UIPivot.TopCenter, UIAnchor.TopHStretch, new Vector2(0f, -100f), _language.GetCalibrateAxisStep2WindowMessage(_language.GetElementIdentifierName(controller, controller.AxisElementIdentifiers[axisIndex].id, AxisRange.Full)));
		...
		protected void ShowEditInputBehaviorsWindow()
		{
		...
		protected void ShowRestoreDefaultsWindow()
		{
			if (currentPlayer != null)
			{
		...
		protected void CreateInputGrid()
		{
			InitializeInputGrid();
			CreateHeaderLabels();
		...
		protected void InitializeInputGrid()
		{
			if (inputGrid == null)
			{
		...
		protected void RefreshInputGridStructure()
		{
		...
		protected void CreateHeaderLabels()
		{
			...
			CreateLabel(prefabs.inputGridHeaderLabel, _language.actionColumnLabel, references.inputGridHeader1, Vector2.zero).SetTextAlignment(TextAlignmentOptions.Center);
			if (_showKeyboard)
			{
		...
		protected void CreateActionLabelColumn()
		{
		...
		protected void CreateKeyboardInputFieldColumn()
		{
			if (_showKeyboard)
			{
		...
		protected void CreateMouseInputFieldColumn()
		{
			if (_showMouse)
			{
		...
		protected void CreateControllerInputFieldColumn()
		{
			if (_showControllers)
			{
		...
		protected void CreateInputFieldColumn(string name, ControllerType controllerType, int maxWidth, int cols, bool disableFullAxis)
		{
		...
		protected void CreateInputActionLabels()
		{
						...
						foreach (InputAction item in ReInput.mapping.UserAssignableActionsInCategory(actionCategory.id, sort: true))
						{
							if (item.type == InputActionType.Axis)
							{
								if (ShouldShowFullAxis(item))
								{
									...
									num -= _inputRowHeight;
									this.LabelCreatedDV?.Invoke(gUILabel2, item, AxisRange.Full);
								}
								if (ShouldDoSplitAxis(item))
								{
									...
									num -= _inputRowHeight;
									this.LabelCreatedDV?.Invoke(gUILabel2, item, AxisRange.Positive);
									...
									num -= _inputRowHeight;
									this.LabelCreatedDV?.Invoke(gUILabel2, item, AxisRange.Negative);
								}
							}
							else if (item.type == InputActionType.Button)
							{
		...
		protected void CreateInputFields(Transform columnXform, ControllerType controllerType, int maxWidth, int cols, bool disableFullAxis)
		{
			for (int i = 0; i < _mappingSets.Length; i++)
			{
				MappingSet mappingSet = _mappingSets[i];
		...
		protected void CreateInputFieldSet(Transform parent, int mapCategoryId, InputAction action, AxisRange axisRange, ControllerType controllerType, int cols, int fieldWidth, ref int yPos, bool disableFullAxis)
		{
			...
			for (int i = 0; i < cols; i++)
			{
				...
				if (axisRange == AxisRange.Full && disableFullAxis)
				{
		...
		protected void PopulateInputFields()
		{
			...
			foreach (InputActionSet actionSet in inputGrid.GetActionSets(currentMapCategoryId))
			{
				...
				if (isJoystickSelected && currentPlayer.controllers.JoystickAndCustomControllersCount() > 0)
				{
					ControllerType controllerType = currentControllerType;
					int controllerId = currentController.id;
		...
		protected void PopulateInputFieldGroup(InputActionSet actionSet, ControllerMap controllerMap, ControllerType controllerType, int controllerId, int maxFields)
		{
			if (controllerMap == null)
			{
				return;
		...
		protected void DisableInputFieldGroup(InputActionSet actionSet, ControllerType controllerType, int fieldCount)
		{
		...
		protected void ResetInputGridScrollBar()
		{
		...
		protected void DrawPlayersGroup()
		{
			if (!showPlayers)
			{
				return;
		...
		protected void DrawControllersGroup()
		{
			if (_showControllers)
			{
		...
		protected void DrawSettingsGroup()
		{
			if (showSettings)
			{
		...
		protected void DrawMapCategoriesGroup()
		{
		...
		protected void DrawWindowButtonsGroup()
		{
		...
		protected void Redraw(bool listsChanged, bool playTransitions)
		{
		...
		protected void RedrawPlayerGroup(bool playTransitions)
		{
			if (showPlayers)
			{
		...
		protected virtual void RedrawControllerGroup()
		{
			int num = -1;
			...
			if (ShowAssignedControllers())
			{
				if (player.controllers.JoystickAndCustomControllersCount() > 0)
				{
					assignedControllerButtonsPlaceholder.SetActive(state: false);
				}
				foreach (Controller item in player.controllers.JoysticksAndCustomControllers())
				{
					GUIButton gUIButton2 = CreateButton(_language.GetControllerName(item), references.assignedControllersGroup.content, Vector2.zero);
					int id = item.id;
					id |= (int)item.type << 16;
					...
					if (item.id == customControllerId && item.type == currentControllerType)
					{
						gUIButton2.SetInteractible(state: false, playTransition: true);
					}
				}
				if (player.controllers.JoystickAndCustomControllersCount() > 0 && !isJoystickSelected)
				{
					Controller controller2 = player.controllers.JoysticksAndCustomControllers().First();
			...
			else if (player.controllers.JoystickAndCustomControllersCount() > 0 && !isJoystickSelected)
			{
				Controller controller3 = player.controllers.JoysticksAndCustomControllers().First();
				customControllerId = controller3.id;
				currentControllerType = controller3.type;
			}
			if (isJoystickSelected && player.controllers.JoystickAndCustomControllersCount() > 0)
			{
				references.removeControllerButton.interactable = true;
				references.controllerNameLabel.text = _language.GetControllerName(currentController);
				if (currentController.axisCount > 0)
				{
		...
		protected virtual void RedrawMapCategoriesGroup(bool playTransitions)
		{
		...
		protected void RedrawInputGrid(bool listsChanged)
		{
			if (listsChanged)
			{
		...
		protected void CreateInputCategoryRow(ref int rowCount, InputCategory category)
		{
		...
		protected GUILabel CreateLabel(string labelText, Transform parent, Vector2 offset)
		{
			return CreateLabel(prefabs.inputGridLabel, labelText, parent, offset);
		}

		protected GUILabel CreateLabel(GameObject prefab, string labelText, Transform parent, Vector2 offset)
		{
		...
		protected GUIButton CreateButton(string labelText, Transform parent, Vector2 offset)
		{
		...
		protected GUIButton CreateFitButton(string labelText, Transform parent, Vector2 offset)
		{
		...
		protected GUIInputField CreateInputField(Transform parent, Vector2 offset, string label, int actionId, AxisRange axisRange, ControllerType controllerType, int fieldIndex)
		{
			...
			this.InputFieldCreated?.Invoke(gUIInputField, controllerType);
			return gUIInputField;
		}

		protected GUIInputField CreateInputField(Transform parent, Vector2 offset)
		{
			return new GUIInputField(InstantiateGUIObject(prefabs.inputGridFieldButton, parent, offset));
		}

		protected GUIToggle CreateToggle(GameObject prefab, Transform parent, Vector2 offset, string label, int actionId, AxisRange axisRange, ControllerType controllerType, int fieldIndex)
		{
		...
		protected GUIToggle CreateToggle(GameObject prefab, Transform parent, Vector2 offset)
		{
			return new GUIToggle(InstantiateGUIObject(prefab, parent, offset));
		}

		protected GameObject InstantiateGUIObject(GameObject prefab, Transform parent, Vector2 offset)
		{
			if (prefab == null)
			{
		...
		protected GameObject CreateNewGUIObject(string name, Transform parent, Vector2 offset)
		{
		...
		protected GameObject InitializeNewGUIGameObject(GameObject gameObject, Transform parent, Vector2 offset)
		{
			if (gameObject == null)
			{
				Debug.LogError("Rewired Control Mapper: GameObject is null!");
		...
		protected GameObject CreateNewColumnGroup(string name, Transform parent, int maxWidth)
		{
		...
		protected Window OpenWindow(bool closeOthers)
		{
			return OpenWindow(string.Empty, closeOthers);
		}

		protected Window OpenWindow(string name, bool closeOthers)
		{
			if (closeOthers)
			{
		...
		protected Window OpenWindow(GameObject windowPrefab, bool closeOthers)
		{
			return OpenWindow(windowPrefab, string.Empty, closeOthers);
		}

		protected Window OpenWindow(GameObject windowPrefab, string name, bool closeOthers)
		{
			if (closeOthers)
			{
				windowManager.CancelAll();
		...
		protected void OpenModal(string title, string message, string confirmText, Action<int> confirmAction, string cancelText, Action<int> cancelAction, bool closeOthers)
		{
		...
		protected void CloseWindow(int windowId)
		{
			if (windowManager.isWindowOpen)
			{
		...
		protected void ChildWindowOpened()
		{
			if (windowManager.isWindowOpen)
			{
		...
		protected bool HasElementAssignmentConflicts(Player player, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
		{
		...
		protected bool IsBlockingAssignmentConflict(InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
		{
			if (!CreateConflictCheck(mapping, assignment, out var conflictCheck))
			{
				return false;
		...
		protected IEnumerable<ElementAssignmentConflictInfo> ElementAssignmentConflicts(Player player, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
		{
		...
		protected bool CreateConflictCheck(InputMapping mapping, ElementAssignment assignment, out ElementAssignmentConflictCheck conflictCheck)
		{
		...
		protected void PollKeyboardForAssignment(out ControllerPollingInfo pollingInfo, out bool modifierKeyPressed, out ModifierKeyFlags modifierFlags, out string label)
		{
			pollingInfo = default(ControllerPollingInfo);
			label = string.Empty;
		...
		protected bool GetFirstElementAssignmentConflict(ElementAssignmentConflictCheck conflictCheck, out ElementAssignmentConflictInfo conflict, bool skipOtherPlayers)
		{
		...
		protected bool GetFirstElementAssignmentConflict(Player player, ElementAssignmentConflictCheck conflictCheck, out ElementAssignmentConflictInfo conflict)
		{
		...
		protected void StartAxisCalibration(int axisIndex)
		{
			if (currentPlayer != null && currentPlayer.controllers.JoystickAndCustomControllersCount() != 0)
			{
				ControllerWithAxes controllerWithAxes = currentController;
				if (axisIndex >= 0 && axisIndex < controllerWithAxes.axisCount)
				{
					pendingAxisCalibration = new AxisCalibrator(controllerWithAxes, axisIndex);
					ShowCalibrateAxisStep1Window();
				}
			}
		}

		protected void EndAxisCalibration()
		{
			if (pendingAxisCalibration != null)
			{
				pendingAxisCalibration.Commit();
				pendingAxisCalibration = null;
				CalibrationWindow obj = (CalibrationWindow)windowManager.windows.FirstOrDefault((Window w) => w is CalibrationWindow);
				obj.SetSensitivity(obj.axisCalibration, 1f);
			}
		}

		protected void SetUISelection(GameObject selection)
		{
			if (!(EventSystem.current == null))
			{
		...
		protected void RestoreLastUISelection()
		{
		...
		protected void SetDefaultUISelection()
		{
			if (isOpen)
			{
		...
		protected void SelectDefaultMapCategory(bool redraw)
		{
		...
		protected void CheckUISelection()
		{
		...
		protected void OnUIElementSelected(GameObject selectedObject)
		{
			lastUISelection = selectedObject;
		}

		protected void SetIsFocused(bool state)
		{
		...
		protected void Open(bool force)
		{
			if (!initialized)
			{
				Initialize();
		...
		protected void ClearCompletely()
		{
			Clear();
			ClearSpawnedObjects();
			ClearAllVars();
		...
		protected void ClearSpawnedObjects()
		{
		...
		protected void ClearVarsOnPlayerChange()
		{
			customControllerId = -1;
		}

		protected void ClearVarsOnJoystickChange()
		{
			customControllerId = -1;
		}
		...
		protected void SetActionAxisInverted(bool state, ControllerType controllerType, int actionElementMapId)
		{
		...
		protected ControllerMap GetControllerMap(ControllerType type)
		{
			if (currentPlayer == null)
			{
				return null;
			...
			{
			case ControllerType.Joystick:
				if (currentPlayer.controllers.joystickCount > 0 && currentControllerType == ControllerType.Joystick)
				{
				...
				return null;
			case ControllerType.Custom:
				if (currentPlayer.controllers.customControllerCount > 0 && currentControllerType == ControllerType.Custom)
				{
					controllerId = currentController.id;
					break;
				}
		...
		protected ControllerMap GetControllerMapOrCreateNew(ControllerType controllerType, int controllerId, int layoutId)
		{
		...
		protected int CountIEnumerable<T>(IEnumerable<T> enumerable)
		{
			if (enumerable == null)
			{
				return 0;
		...
		protected int GetDefaultMapCategoryId()
		{
		...
		protected void SubscribeFixedUISelectionEvents()
		{
		...
		protected void SubscribeMenuControlInputEvents()
		{
		...
		protected void UnsubscribeMenuControlInputEvents()
		{
		...
		protected void SubscribeRewiredInputEventAllPlayers(int actionId, Action<InputActionEventData> callback)
		{
		...
		protected void UnsubscribeRewiredInputEventAllPlayers(int actionId, Action<InputActionEventData> callback)
		{
		...
		protected int GetMaxControllersPerPlayer()
		{
		...
		protected bool ShowAssignedControllers()
		{
			if (!_showControllers)
			{
				return false;
		...
		protected void InspectorPropertyChanged(bool reset = false)
		{
			if (reset)
			{
				Reset();
		...
		protected void AssignController(Player player, int controllerId, ControllerType controllerType)
		{
			...
			foreach (Player player2 in ReInput.players.Players)
			{
				if (player2 != player)
				{
					RemoveController(player2, controllerId, controllerType);
				}
			}
			player.controllers.AddController(controllerType, controllerId, removeFromOtherPlayers: false);
			if (ReInput.userDataStore != null)
			{
				ReInput.userDataStore.LoadControllerData(player.id, controllerType, controllerId);
			}
		}

		protected void RemoveAllControllers(Player player)
		{
			if (player == null)
			{
				return;
			...
			foreach (Controller item in player.controllers.JoysticksAndCustomControllers())
			{
				RemoveController(player, item.id, item.type);
			}
		}

		protected void RemoveController(Player player, int controllerId, ControllerType controllerType)
		{
			if (player != null && player.controllers.ContainsController(controllerType, controllerId))
			{
				if (ReInput.userDataStore != null)
				{
					ReInput.userDataStore.SaveControllerData(player.id, controllerType, controllerId);
				}
				player.controllers.RemoveController(controllerType, controllerId);
			}
		}

		protected bool IsAllowedAssignment(InputMapping pendingInputMapping, ControllerPollingInfo pollingInfo)
		{
			if (pendingInputMapping == null)
			{
				return false;
		...
		protected void InputPollingStarted()
		{
		...
		protected void InputPollingStopped()
		{
			bool num = isPollingForInput;
			isPollingForInput = false;
			if (num)
		...
		protected int GetControllerInputFieldCount(ControllerType controllerType)
		{
			switch (controllerType)
			{
			case ControllerType.Keyboard:
		...
		protected bool ShowSwapButton(int windowId, InputMapping mapping, ElementAssignment assignment, bool skipOtherPlayers)
		{
			if (currentPlayer == null)
			{
				return false;
		...
		protected bool SwapIsSameInputRange(ControllerElementType origElementType, AxisRange origAxisRange, Pole origAxisContribution, ControllerElementType conflictElementType, AxisRange conflictAxisRange, Pole conflictAxisContribution)
		{
