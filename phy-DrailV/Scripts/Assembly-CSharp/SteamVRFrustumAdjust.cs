...
[AddComponentMenu("DV/SteamVRFrustumAdjust")]
public class SteamVRFrustumAdjust : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		Vector2 vector = new Vector2(Mathf.Max(0f - pfLeft, pfRight), Mathf.Max(0f - pfTop, pfBottom));
		float num = Mathf.Acos(eyeToHeadTransform.m0);
		float num2 = Mathf.Atan(vector.x);
		float num3 = Mathf.Tan(num + num2);
		projectionMatrix.m00 = 1f / num3;
		float num4 = Mathf.Atan(0f - pfLeft);
		float num5 = vector.y * Mathf.Cos(num4) / Mathf.Cos(num4 + num);
