...
public static class Oni
{
	...
	public struct Rigidbody
	{
		...
		public void Set(UnityEngine.Rigidbody source, bool kinematicForParticles)
		{
			...
			Vector3 vector = new Vector3(((source.constraints & RigidbodyConstraints.FreezeRotationX) != RigidbodyConstraints.None) ? 0f : (1f / source.inertiaTensor.x), ((source.constraints & RigidbodyConstraints.FreezeRotationY) != RigidbodyConstraints.None) ? 0f : (1f / source.inertiaTensor.y), ((source.constraints & RigidbodyConstraints.FreezeRotationZ) != RigidbodyConstraints.None) ? 0f : (1f / source.inertiaTensor.z));
		...
		public void Set(Rigidbody2D source, bool kinematicForParticles)
		{
			...
			centerOfMass = source.transform.position + source.transform.rotation * source.centerOfMass;
			inertiaTensor = ((source.isKinematic || kinematicForParticles) ? Vector3.zero : new Vector3(0f, 0f, ((source.constraints & RigidbodyConstraints2D.FreezeRotation) != RigidbodyConstraints2D.None) ? 0f : (1f / source.inertia)));
			inverseMass = ((source.isKinematic || kinematicForParticles) ? 0f : (1f / source.mass));
		}
	}
