...
namespace VLB
{
	...
	public class EffectAbstractBase : MonoBehaviour
	{
		...
		{
			if (componentsToChange.HasIntFlag(ComponentsToChange.VolumetricLightBeam) && (bool)m_Beam)
			{
			...
			if (componentsToChange.HasIntFlag(ComponentsToChange.UnityLight) && (bool)m_Light)
			{
				m_Light.intensity = Mathf.Max(0f, m_BaseIntensityLight + additive);
			}
			if (componentsToChange.HasIntFlag(ComponentsToChange.VolumetricDustParticles) && (bool)m_Particles)
			{
