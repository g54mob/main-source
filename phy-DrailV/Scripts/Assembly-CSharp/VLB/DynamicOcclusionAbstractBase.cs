	...
	public abstract class DynamicOcclusionAbstractBase : MonoBehaviour
	{
		...
		protected void ProcessOcclusion(ProcessOcclusionSource source)
		{
			if (Config.Instance.featureEnabledDynamicOcclusion)
			{
				...
				if (updateRate.HasIntFlag(DynamicOcclusionUpdateRate.OnBeamMove))
				{
		...
		protected virtual void OnEnable()
		{
			...
			if (!updateRate.HasIntFlag(DynamicOcclusionUpdateRate.Never))
			{
		...
		private void OnWillCameraRender(Camera cam)
		{
			...
			{
				bool flag = false;
				if (!flag && updateRate.HasIntFlag(DynamicOcclusionUpdateRate.OnBeamMove) && !m_TransformPacked.IsSame(base.transform))
				{
					flag = true;
				}
				if (!flag && updateRate.HasIntFlag(DynamicOcclusionUpdateRate.EveryXFrames) && Time.frameCount >= m_LastFrameRendered + waitXFrames)
				{
					flag = true;
				}
				if (flag)
