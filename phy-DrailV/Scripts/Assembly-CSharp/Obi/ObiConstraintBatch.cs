...
namespace Obi
{
	...
	public abstract class ObiConstraintBatch
	{
		...
		{
			using (PooledArray<int> pooledArray = ArrayPool<int>.New(activeConstraints.Count))
			{
				activeConstraints.CopyTo(pooledArray);
				Oni.SetActiveConstraints(batch, pooledArray, activeConstraints.Count);
			}
		}
