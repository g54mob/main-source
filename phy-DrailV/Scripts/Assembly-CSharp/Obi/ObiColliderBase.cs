...
namespace Obi
{
	public abstract class ObiColliderBase : MonoBehaviour
	{
		...
		public bool findSolverInParentHierarchy;
		...
		private void FindSolvers(bool addToSolvers)
		{
			...
			solvers.Clear();
			bool flag = false;
			if (findSolverInParentHierarchy)
			{
				ObiSolver componentInParent = GetComponentInParent<ObiSolver>();
				if (componentInParent != null)
				{
					RegisterInSolver(componentInParent, addToSolvers);
					flag = true;
				}
				else
				{
					Debug.LogWarning("Could not find ObiSolver in parent of " + base.gameObject.name + ", will use scene solvers instead.");
				}
			}
			if (flag)
			{
			...
			foreach (ObiSolver sceneSolver in ObiSolver.sceneSolvers)
			{
				RegisterInSolver(sceneSolver, addToSolvers);
			}
		}
