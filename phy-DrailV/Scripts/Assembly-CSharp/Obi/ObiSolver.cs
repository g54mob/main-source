...
namespace Obi
{
	...
	public sealed class ObiSolver : MonoBehaviour
	{
		...
		internal static readonly List<ObiSolver> sceneSolvers = new List<ObiSolver>();
		...
		private void Start()
		{
			if (Application.isPlaying)
			{
				...
				{
					array[i].RegisterInSolver(this, addToSolver: true);
				}
			}
		}
		...
		private void OnDestroy()
		{
			if (Application.isPlaying)
			{
				...
				for (int i = 0; i < array.Length; i++)
				{
					array[i].RemoveFromSolver(this);
				}
			}
			sceneSolvers.Remove(this);
		}
		...
		public void EndFrame(float frameDelta)
		{
			...
			if (renderablePositionsClients > 0)
			{
				...
				if (simulateInLocalSpace)
				{
					...
					{
						renderablePositions[i] = localToWorldMatrix.MultiplyPoint3x4(renderablePositions[i]);
					}
				}
			}
		...
		private void CheckVisibility()
		{
			...
			Bounds bounds = (simulateInLocalSpace ? this.bounds.Transform(base.transform.localToWorldMatrix) : this.bounds);
