...
namespace Obi
{
	...
	public class ObiPinConstraintBatch : ObiConstraintBatch
	{
		...
		{
			using (PooledArray<float> pooledArray = ArrayPool<float>.New(base.ConstraintCount))
			{
				Oni.GetBatchConstraintForces(batch, pooledArray, base.ConstraintCount, 0);
				for (int i = 0; i < pooledArray.Length; i++)
				{
					if ((0f - pooledArray[i]) * 1000f > pinBreakResistance[i])
					{
