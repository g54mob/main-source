...
namespace Obi
{
	...
	public abstract class ObiActor : MonoBehaviour, IObiSolverClient
	{
		...
		public virtual void PullDataFromSolver(ParticleData data = ParticleData.NONE)
		{
			...
			for (int i = 0; i < particleIndices.Length; i++)
			{
				...
				if ((data & ParticleData.POSITIONS) != ParticleData.NONE)
				{
					...
					positions[i] = base.transform.InverseTransformPoint(array[0]);
				}
				if ((data & ParticleData.VELOCITIES) != ParticleData.NONE)
				{
					...
					velocities[i] = base.transform.InverseTransformVector(array2[0]);
				}
			}
		}
		...
		public virtual void OnSolverStepBegin()
		{
			...
			if (!base.transform.hasChanged && !transform.hasChanged)
			{
				return;
			}
			...
			Matrix4x4 matrix4x = ((!Solver.simulateInLocalSpace) ? ActorLocalToWorldMatrix : (transform.worldToLocalMatrix * ActorLocalToWorldMatrix));
			Matrix4x4 matrix4x2 = transform.worldToLocalMatrix * Solver.LastTransform;
			for (int i = 0; i < num; i++)
			{
				if (!flag || invMasses[i] == 0f)
				{
					simulationPosition[0] = matrix4x.MultiplyPoint3x4(positions[i]);
