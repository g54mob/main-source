...
namespace MiscUtil.Compression.Vcdiff
{
	public sealed class VcdiffDecoder
	{
		...
		private void ReadCodeTable()
		{
			...
			Decode(memoryStream, memoryStream2, memoryStream3);
		...
		private bool DecodeWindow()
		{
			...
			if (flag)
			{
				byte[] array3 = IOHelper.CheckedReadBytes(delta, 4);
				num6 = (array3[0] << 24) | (array3[1] << 16) | (array3[2] << 8) | array3[3];
			}
			byte[] array4 = IOHelper.CheckedReadBytes(delta, size);
			...
			while (true)
			{
				int num8 = memoryStream2.ReadByte();
				if (num8 == -1)
				{
					break;
				}
				for (int i = 0; i < 2; i++)
				{
					Instruction instruction = codeTable[num8, i];
					int num9 = instruction.Size;
					if (num9 == 0 && instruction.Type != InstructionType.NoOp)
					{
						num9 = IOHelper.ReadBigEndian7BitEncodedInt(memoryStream2);
					}
					switch (instruction.Type)
					{
					case InstructionType.Add:
						memoryStream.Write(array4, num7, num9);
						num7 += num9;
						break;
					case InstructionType.Copy:
					{
						int num10 = cache.DecodeAddress((int)memoryStream.Position + num3, instruction.Mode);
						if (array != null && num10 < array.Length)
						{
							memoryStream.Write(array, num10, num9);
							break;
						}
						num10 -= num3;
						if (num10 + num9 < memoryStream.Position)
						{
							memoryStream.Write(array2, num10, num9);
							break;
						}
						for (int k = 0; k < num9; k++)
						{
							memoryStream.WriteByte(array2[num10++]);
						}
						break;
					}
					case InstructionType.Run:
					{
						byte value = array4[num7++];
			...
			if (flag)
			{
				int num11 = Adler32.ComputeChecksum(1, array2);
				if (num11 != num6)
				{
