	...
	internal class WhenAllObservable<T> : OperatorObservableBase<T[]>
	{
		private class WhenAll : OperatorObserverBase<T[], T[]>
		{
			...
			public IDisposable Run()
			{
				...
				{
					IObservable<T> observable = sources[i];
					WhenAllCollectionObserver whenAllCollectionObserver = new WhenAllCollectionObserver(this, i);
					array[i] = observable.Subscribe(whenAllCollectionObserver);
				}
		...
		protected override IDisposable SubscribeCore(IObserver<T[]> observer, IDisposable cancel)
		{
			...
			IList<IObservable<T>> list = sourcesEnumerable as IList<IObservable<T>>;
			if (list == null)
			{
				list = new List<IObservable<T>>(sourcesEnumerable);
			}
	...
	internal class WhenAllObservable : OperatorObservableBase<Unit>
	{
		private class WhenAll : OperatorObserverBase<Unit, Unit>
		{
			...
			public IDisposable Run()
			{
				...
				{
					IObservable<Unit> observable = sources[i];
					WhenAllCollectionObserver whenAllCollectionObserver = new WhenAllCollectionObserver(this);
					array[i] = observable.Subscribe(whenAllCollectionObserver);
				}
				return StableCompositeDisposable.CreateUnsafe(array);
			}
		...
		protected override IDisposable SubscribeCore(IObserver<Unit> observer, IDisposable cancel)
		{
			...
			IList<IObservable<Unit>> list = sourcesEnumerable as IList<IObservable<Unit>>;
			if (list == null)
			{
				list = new List<IObservable<Unit>>(sourcesEnumerable);
			}
			return new WhenAll_(list, observer, cancel).Run();
		}
	}
