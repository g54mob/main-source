...
namespace UniRx.Operators
{
	internal class DelayFrameSubscriptionObservable<T> : OperatorObservableBase<T>
	{
		...
		protected override IDisposable SubscribeCore(IObserver<T> observer, IDisposable cancel)
		{
			...
			multipleAssignmentDisposable.Disposable = Observable.TimerFrame(frameCount, frameCountType).SubscribeWithState3(observer, multipleAssignmentDisposable, source, delegate(long _, IObserver<T> o, MultipleAssignmentDisposable disp, IObservable<T> s)
			{
