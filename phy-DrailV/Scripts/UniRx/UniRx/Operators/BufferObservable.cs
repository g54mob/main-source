	...
	internal class BufferObservable<T> : OperatorObservableBase<IList<T>>
	{
		...
		private class BufferT : OperatorObserverBase<T, IList<T>>
		{
			private class Buffer : IObserver<long>
			{
				...
				public void OnNext(long value)
				{
					...
					IObserver<IList<T>> observer = parent.observer;
					IList<T> value2;
					if (!flag)
					{
						IList<T> list2 = list;
					...
					{
						IList<T> list2 = BufferT.EmptyArray;
						value2 = list2;
					}
		...
		private class BufferTS : OperatorObserverBase<T, IList<T>>
		{
			...
			public IDisposable Run()
			{
				...
				CreateTimer();
				return StableCompositeDisposable.Create(parent.source.Subscribe(this), timerD);
			}
			...
			private void CreateTimer()
			{
				...
				TimeSpan timeSpan = (isSpan ? nextSpan : nextShift);
				TimeSpan dueTime = timeSpan - totalTime;
				totalTime = timeSpan;
				if (isSpan)
				{
		...
		private class BufferTC : OperatorObserverBase<T, IList<T>>
		{
			...
			private void CreateTimer()
			{
				...
				SingleAssignmentDisposable singleAssignmentDisposable = new SingleAssignmentDisposable();
				timerD.Disposable = singleAssignmentDisposable;
				if (parent.scheduler is ISchedulerPeriodic schedulerPeriodic)
				{
			...
			private void OnNextTick(long currentTimerId)
			{
				...
				IObserver<IList<T>> obj = observer;
				IList<T> value;
				if (!flag)
				{
					IList<T> list2 = list;
