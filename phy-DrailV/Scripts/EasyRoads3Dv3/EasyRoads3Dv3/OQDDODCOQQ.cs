...
{
	public class OQDDODCOQQ : MonoBehaviour
	{
		public static void OQDDCOCCOD(ERBend scr, ref List<Vector3> leftOuterSegments, ref List<Vector3> leftInnerSegments, ref List<Vector3> rightOuterSegments, ref List<Vector3> rightInnerSegments)
		{
			...
			{
				rightOuterSegments.Add(ERRoundabouts.OCQDOQQQOD(vector, pivot, Quaternion.Euler(0f, num2 + (float)i * num, 0f)));
				rightInnerSegments.Add(ERRoundabouts.OCQDOQQQOD(vector2, pivot, Quaternion.Euler(0f, num2 + (float)i * num, 0f)));
			}
			...
			for (int i = 0; i <= scr.roundingSegments; i++)
			{
				leftOuterSegments.Add(ERRoundabouts.OCQDOQQQOD(vector, pivot, Quaternion.Euler(0f, num2 + (float)i * num, 0f)));
				leftInnerSegments.Add(ERRoundabouts.OCQDOQQQOD(vector2, pivot, Quaternion.Euler(0f, num2 + (float)i * num, 0f)));
			}
		...
		public static List<Vector3> OQOODOQQDC(Vector3 prefabCenterpos, Vector3 cpCenterposV3, Vector3 v1, Vector3 cp, ref float totalDistance, ref List<Vector3> controlPoints)
		{
			...
			vA = OCQCDQCQOQ.OQQQDCODQD(vA, vB, v1);
			...
			for (int i = 1; i < controlPoints.Count - 2; i++)
			{
				...
				{
					vector2 = ERModularRoad.OQODDDCOQD(controlPoints[i - 1], controlPoints[i], controlPoints[i + 1], controlPoints[i + 2], num3, 0.5f);
		...
		public static List<Vector3> OCDCQDQOOQ(Vector3 prefabCenterpos, Vector3 cpCenterposV3, Vector3 v1, Vector3 cp, ref float totalDistance, ref List<Vector3> controlPoints, float angle, float multiplyFactor)
		{
			angle *= 0.55f;
			Vector3 normalized = (prefabCenterpos - cpCenterposV3).normalized;
			Vector3 zero = Vector3.zero;
			zero = ((!OCQCDQCQOQ.OOOOCDQQOC(cpCenterposV3, prefabCenterpos, v1)) ? OCQCDQCQOQ.OCQDOQQQOD(cpCenterposV3, prefabCenterpos, Quaternion.Euler(0f, 0f - angle, 0f)) : OCQCDQCQOQ.OCQDOQQQOD(cpCenterposV3, prefabCenterpos, Quaternion.Euler(0f, angle, 0f)));
			...
			vA = OCQCDQCQOQ.OQQQDCODQD(vA, vB, zero);
			...
			for (int i = 1; i < controlPoints.Count - 2; i++)
			{
				...
				{
					vector = ERModularRoad.OQODDDCOQD(controlPoints[i - 1], controlPoints[i], controlPoints[i + 1], controlPoints[i + 2], num4, 0.5f);
		...
		public static Vector3[] OCCDOOCDOC(ERCrossingPrefabs scr, int connection, List<Vector3> controlPoints, float segmentDistance, float defaultDistance, Vector3[] meshVecs, ref Vector3[] tCrossingTmpFullMeshVecs, float multiplyFactor, float angle, float curveStrength)
		{
			...
			if (!OCQCDQCQOQ.OOOOCDQQOC(pTarget, pSource, controlPoints[2]))
			{
				angle *= -1f;
			}
			float num = angle;
			...
			for (int i = 0; i < array.Length; i++)
			{
				if ((array[i].z > 0f && connection == 1) || ((array[i].z < 0f && connection == 0) ? true : false))
				{
					...
					zero = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num4, curveStrength);
					Vector3 vector2 = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num4 - 0.001f, curveStrength);
					Vector3 vector3 = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num4 + 0.001f, curveStrength);
				...
				reference = tCrossingTmpFullMeshVecs[i];
			}
			return array;
		}

		public static Vector3[] OODCQDOCQQ(ERCrossingPrefabs scr, int connection, List<Vector3> controlPoints, float segmentDistance, float defaultDistance, Vector3[] meshVecs, ref Vector3[] tmpSurfaceVecsTCrossings, float multiplyFactor, float angle, float curveStrength)
		{
			Transform transform = scr.transform;
			Vector3[] array = new Vector3[meshVecs.Length];
			Array.Copy(meshVecs, array, meshVecs.Length);
			...
			for (int i = 0; i < array.Length; i++)
			{
				if ((array[i].z > 0f && connection == 1) || ((array[i].z < 0f && connection == 0) ? true : false))
				{
					...
					Vector3 position = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num, curveStrength);
					Vector3 vector = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num - 0.001f, curveStrength);
					Vector3 vector2 = ERModularRoad.OQODDDCOQD(controlPoints[0], controlPoints[1], controlPoints[2], controlPoints[3], num + 0.001f, curveStrength);
					...
					ref Vector3 reference = ref array[i];
					reference = (tmpSurfaceVecsTCrossings[i] = position);
				}
				else
				{
					ref Vector3 reference2 = ref array[i];
					reference2 = tmpSurfaceVecsTCrossings[i];
				}
			}
			return array;
		}

		public static Vector3[] OCQQOQQCDO(ERCrossingPrefabs scr, int connection, List<Vector3> controlPoints, float segmentDistance, Vector3[] meshVecs, float multiplyFactor, float angle, Vector3 cpCenterPoint)
		{
			...
			if (!OCQCDQCQOQ.OOOOCDQQOC(new Vector3(0f, 0f, 1f), Vector3.zero, pCheck))
			{
				angle *= -1f;
			}
			Vector3[] array = new Vector3[meshVecs.Length];
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				ref Vector3 reference = ref array[i];
				reference = OCQCDQCQOQ.OCQDOQQQOD(array[i], zero, Quaternion.Euler(0f, num7, 0f));
		...
		{
			Vector3 vector = OCQCDQCQOQ.OCQDOQQQOD(vec, Vector3.zero, Quaternion.Euler(0f, angle, 0f));
			return Mathf.Abs(vec.z - vector.z);
		}

		public static void OOQCQOQOQO(ERCrossings scr, int connection, int x, int y, List<List<Vector3>> vecArray)
		{
			float num = 1f;
		...
		public static void OOCQCQDQDO(ERCrossings scr, int connection, List<List<Vector3>> vecArray, int leftright)
		{
			float num = 1f;
			int tCrossingLeftRight = scr.prefabScript.tCrossingLeftRight;
			float num2 = scr.frontRoadWidth * 0.5f;
