...
{
	public class OOCDOQCOCD : MonoBehaviour
	{
		public class OQCDDCDQQQ
		{
			public int start;

			public int end;

			public OQCDDCDQQQ(int startV3, int endV3)
			{
				start = startV3;
				end = endV3;
			}
		...
		public static void OQCOCOQQCO(List<ERRoundaboutElement> connections, List<Vector3> mainLeftPoints, ref List<Vector3> meshVecs, ref List<Vector2> meshUVs, ref List<int> fullTris)
		{
			int num = 0;
			int num2 = 10000000;
			int num3 = 10000000;
		...
		public static void OCODDQQQQC(List<ERRoundaboutElement> connections, List<Vector3> meshVecs, int vecCount, ref List<Vector3> connectionVecs, ref List<Vector2> connectionUVs, ref List<int> connectionTris, ref List<List<int>> triList, ref List<Material> materialList)
		{
			Vector3 vA;
			Vector3 vB;
			...
			for (int i = 0; i < connections.Count; i++)
			{
				...
				if (connections[i].blendFlag)
				{
					...
					Vector3 b = OQQQDCODQD(vA, vB, vector2);
				...
				for (int k = 0; k < list5.Count; k++)
				{
					...
					if (connections[i].blendFlag)
					{
						...
						Vector3 b = OQQQDCODQD(vA, vB, vector3);
				...
				List<TriangleER> list11 = delaunayER.Triangulate(list3);
				List<int> list12 = new List<int>();
				for (int k = 0; k < list11.Count; k++)
				{
					list12.Add(vecCount + delaunayER.FindVertice(new Vector3(list11[k].Vertex1.x, list11[k].Vertex1.z, list11[k].Vertex1.y), list));
					list12.Add(vecCount + delaunayER.FindVertice(new Vector3(list11[k].Vertex3.x, list11[k].Vertex3.z, list11[k].Vertex3.y), list));
					list12.Add(vecCount + delaunayER.FindVertice(new Vector3(list11[k].Vertex2.x, list11[k].Vertex2.z, list11[k].Vertex2.y), list));
				}
				for (int k = 0; k < list12.Count; k += 3)
				{
					Vector3 vector4 = (list[list12[k] - vecCount] + list[list12[k + 1] - vecCount] + list[list12[k + 2] - vecCount]) / 3f;
					if (OCCOQDODDD(list9.Count, list9, vector4.x, vector4.z))
					{
						list4.Add(list12[k]);
						list4.Add(list12[k + 1]);
						list4.Add(list12[k + 2]);
					}
				}
				...
				OQDQDQOQDO(meshVecs, list10, ref roadShapeVecs, ref roadShapeUVY);
		...
		public static void OQDQDQOQDO(List<Vector3> meshVecs, List<int> connectionVecInts, ref List<Vector2> roadShapeVecs, ref List<float> roadShapeUVY)
		{
			...
			for (int i = 0; i < connectionVecInts.Count; i++)
			{
				b = meshVecs[connectionVecInts[i]];
				b.y = 0f;
				...
				if (i < connectionVecInts.Count - 1)
				{
					num2 += Vector3.Distance(meshVecs[connectionVecInts[i]], meshVecs[connectionVecInts[i + 1]]);
				}
			}
			roadShapeUVY.Add(0f);
			float num3 = 0f;
			for (int i = 1; i < connectionVecInts.Count; i++)
			{
				num3 += Vector3.Distance(meshVecs[connectionVecInts[i - 1]], meshVecs[connectionVecInts[i]]);
		...
		public static bool OCCOQDODDD(int nvert, List<Vector2> vert, float testx, float testy)
		{
			int num = 0;
			bool flag = false;
			int num2 = 0;
		...
		public static bool ODOCQDODCO(int nvert, List<float> vertx, List<float> verty, float testx, float testy)
		{
			int num = 0;
			bool flag = false;
			int num2 = 0;
		...
		public static bool ODOOQODCOO(List<Vector3> vecs, List<OQCDDCDQQQ> edges, int p1, int p2, int p3)
		{
			bool result = true;
			for (int i = 0; i < edges.Count; i++)
			{
				...
				{
					if (!ERCrossingPrefabs.OOOOCDQQOC(vecs[p2], vecs[p1], vecs[p3]))
					{
						result = false;
					}
				}
				else if (edges[i].start == p2 && edges[i].end == p1 && !ERCrossingPrefabs.OOOOCDQQOC(vecs[p1], vecs[p2], vecs[p3]))
				{
					result = false;
				}
				if (edges[i].start == p1 && edges[i].end == p3)
				{
					if (!ERCrossingPrefabs.OOOOCDQQOC(vecs[p3], vecs[p1], vecs[p2]))
					{
						result = false;
					}
				}
				else if (edges[i].start == p3 && edges[i].end == p1 && !ERCrossingPrefabs.OOOOCDQQOC(vecs[p1], vecs[p3], vecs[p2]))
				{
					result = false;
				}
				if (edges[i].start == p2 && edges[i].end == p3)
				{
					if (!ERCrossingPrefabs.OOOOCDQQOC(vecs[p3], vecs[p2], vecs[p1]))
					{
						result = false;
					}
				}
				else if (edges[i].start == p3 && edges[i].end == p2 && !ERCrossingPrefabs.OOOOCDQQOC(vecs[p2], vecs[p3], vecs[p1]))
				{
					result = false;
				}
			}
