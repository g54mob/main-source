...
namespace AwesomeTechnologies.BillboardSystem
{
	public class BillboardCell : IHasRect
	{
		...
		public Rect Rectangle
		{
			get
			{
				return RectExtension.CreateRectFromBounds(BilllboardCellBounds);
			}
			set
			{
				BilllboardCellBounds = RectExtension.CreateBoundsFromRect(value);
			}
		}
		...
		{
			if (vegetationPackageIndex < VegetationPackageBillboardInstancesList.Count && vegetationItemIndex < VegetationPackageBillboardInstancesList[vegetationPackageIndex].BillboardInstanceList.Count)
			{
				VegetationPackageBillboardInstancesList[vegetationPackageIndex].BillboardInstanceList[vegetationItemIndex].ClearCache();
				Loaded = false;
			}
		}
		...
		public void PrepareBillboardCell(List<VegetationPackagePro> vegetationPackageProList)
		{
			...
			{
				VegetationPackageBillboardInstances item = new VegetationPackageBillboardInstances(vegetationPackageProList[i].VegetationInfoList.Count);
				VegetationPackageBillboardInstancesList.Add(item);
			}
			Prepared = true;
		}
	}
