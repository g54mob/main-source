...
namespace AwesomeTechnologies.VegetationSystem
{
	public class VegetationPackageProModelInfo
	{
		...
		public VegetationPackageProModelInfo(VegetationPackagePro vegetationPackagePro, EnvironmentSettings environmentSettings, List<GameObject> windSamplerList, int cameraCount, VegetationRenderSettings vegetationRenderSettings, VegetationSettings vegetationSettings)
		{
			EnvironmentSettings = environmentSettings;
			for (int i = 0; i <= vegetationPackagePro.VegetationInfoList.Count - 1; i++)
			{
				VegetationItemModelInfo item = new VegetationItemModelInfo(vegetationPackagePro.VegetationInfoList[i], EnvironmentSettings, windSamplerList, cameraCount, vegetationRenderSettings, vegetationSettings);
				VegetationItemModelList.Add(item);
			}
		}
		...
		public float GetAdditionalBoundingSphereRadius()
		{
			...
			{
				num = Mathf.Max(VegetationItemModelList[i].BoundingSphereRadius, num);
			}
			return num;
		}
