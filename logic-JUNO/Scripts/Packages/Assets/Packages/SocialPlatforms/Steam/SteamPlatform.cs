...
namespace Assets.Packages.SocialPlatforms.Steam
{
	public class SteamPlatform : ISteamPlatform, ISocialPlatformExt, ISocialPlatform
	{
		...
		{
			if (!SteamApps.GetCurrentBetaName(out var pchName, 256))
			{
				return "public";
			}
			return pchName;
		...
		{
			SteamManager.MonoBehaviour.StartCoroutine(IncrementAchievementCoroutine(achievementID, incrementAmount, showProgress, asInteger: true, callback));
		}

		public void IncrementAchievement(string achievementID, float incrementAmount, bool showProgress, Action<bool> callback)
		{
			SteamManager.MonoBehaviour.StartCoroutine(IncrementAchievementCoroutine(achievementID, incrementAmount, showProgress, asInteger: false, callback));
		}

		public void IncrementStat(string statID, int incrementAmount, ShouldShowProgress showProgress, Action<bool> callback)
		{
			SteamManager.MonoBehaviour.StartCoroutine(IncrementStatCoroutine(statID, incrementAmount, showProgress, asInteger: true, callback));
		}

		public void IncrementStat(string statID, float incrementAmount, ShouldShowProgress showProgress, Action<bool> callback)
		{
			SteamManager.MonoBehaviour.StartCoroutine(IncrementStatCoroutine(statID, incrementAmount, showProgress, asInteger: false, callback));
		}
		...
		public void LoadAchievementDescriptions(Action<IAchievementDescription[]> callback)
		{
			if (callback != null)
			{
				SteamManager.MonoBehaviour.StartCoroutine(WaitForUserStatsCoroutine(LoadAchievementDescriptionsCallback, callback));
			}
		}
		...
		public void LoadAchievements(Action<IAchievement[]> callback)
		{
			if (callback != null)
			{
				SteamManager.MonoBehaviour.StartCoroutine(WaitForUserStatsCoroutine(LoadAchievementsCallback, callback));
			}
		}
		...
		{
			SteamManager.MonoBehaviour.StartCoroutine(ReportProgressCoroutine(achievementID, progress, callback));
		}
		...
		{
			return SteamUtils.ShowFloatingGamepadTextInput((EFloatingGamepadTextInputMode)mode, (int)inputFieldPosition.x, (int)inputFieldPosition.y, (int)inputFieldPosition.width, (int)inputFieldPosition.height);
		}
		...
		private void StopStartCoroutine(ref Coroutine coroutineReference, Func<Action<bool>, IEnumerator> coroutineMethod, Action<bool> callback)
		{
			...
			{
				SteamManager.MonoBehaviour.StopCoroutine(coroutineReference);
			}
			coroutineReference = SteamManager.MonoBehaviour.StartCoroutine(coroutineMethod(callback));
		}
		...
		private void UpdateAchievements()
		{
			foreach (AchievementInfo achievement2 in _achievementDatabase.Achievements)
			{
				...
				{
					UnityEngine.Debug.LogErrorFormat("Unable to obtain achievement info for achievement '{0}'", steamId);
				}
			}
			if (DebuggingEnabled)
			{
