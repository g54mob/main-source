...
namespace Assets.Packages.DevConsole.Commands
{
	internal static class CommandEvaluator
	{
		...
		private static List<RegisteredCommandInfo> _commandRegistrations;

		public static IReadOnlyList<RegisteredCommandInfo> RegisteredCommands => _commandRegistrations;
		...
		static CommandEvaluator()
		{
			...
			_commandRegistrations = new List<RegisteredCommandInfo>();
		...
		public static void RegisterCommand(string command, Delegate commandAction, params string[] parameterNames)
		{
			RegisteredCommandInfo registeredCommandInfo = new RegisteredCommandInfo(command, commandAction, parameterNames);
			int num = -1;
			for (int i = 0; i < _commandRegistrations.Count; i++)
			{
				if (_commandRegistrations[i].CommandText == registeredCommandInfo.CommandText && _commandRegistrations[i].Parameters.Length == registeredCommandInfo.Parameters.Length)
				{
					num = i;
					break;
				}
			...
			if (num != -1)
			{
				_commandRegistrations[num] = registeredCommandInfo;
			}
			else
			{
				_commandRegistrations.Add(registeredCommandInfo);
			}
		}

		public static void UnregisterCommand(string command)
		{
			_commandRegistrations.RemoveAll((RegisteredCommandInfo x) => x.CommandText == command);
		}
		...
		private static void EvaluateFindAllChildComponents(ComponentListCommandSegment command, ConsoleCommandSegment previousCommand)
		{
			...
			if (gameObject == null)
			{
				...
				{
					command.Components = (from x in UnityEngine.Object.FindObjectsOfType<Component>()
		...
		private static void EvaluateFindAllChildGameObjects(GameObjectListCommandSegment command, ConsoleCommandSegment previousCommand)
		{
			...
			if (gameObject == null)
			{
				if (previousCommand == null)
				{
					command.GameObjects = (from x in UnityEngine.Object.FindObjectsOfType<GameObject>()
		...
		private static void EvaluateFindChildGameObjects(GameObjectListCommandSegment command, ConsoleCommandSegment previousCommand)
		{
			...
			if (gameObject == null)
			{
				if (previousCommand == null)
				{
					command.GameObjects = (from t in UnityEngine.Object.FindObjectsOfType<Transform>()
		...
		private static List<LogEntry> ExecuteCommand(ConsoleCommand command, ConsoleCommandSegment commandSegment, List<string> arguments)
		{
			...
			try
			{
				RegisteredCommandInfo? registeredCommandInfo = null;
				RegisteredCommandInfo? registeredCommandInfo2 = null;
				for (int i = 0; i < _commandRegistrations.Count; i++)
				{
					RegisteredCommandInfo value = _commandRegistrations[i];
					if (value.CommandText == commandSegment.CommandText)
					{
						if (value.Parameters.Length == arguments.Count)
						{
							registeredCommandInfo2 = value;
							break;
						}
						if (!registeredCommandInfo.HasValue)
						{
							registeredCommandInfo = value;
						}
					}
				}
				if (registeredCommandInfo2.HasValue)
				{
					registeredCommandInfo = registeredCommandInfo2;
				}
				if (!registeredCommandInfo.HasValue)
				{
				...
				Delegate commandDelegate = registeredCommandInfo.Value.CommandDelegate;
				ParameterInfo[] parameters = commandDelegate.Method.GetParameters();
				...
				object obj = commandDelegate.DynamicInvoke(array);
				...
				if (commandDelegate.Method.ReturnType != typeof(void))
				{
