...
public class WheelFrictionCurveSource
{
	...
	public AnimationCurve CreateCurve()
	{
		AnimationCurve animationCurve = new AnimationCurve();
		for (int i = 0; i < _extremePoints.Length; i++)
		{
			animationCurve.AddKey(new Keyframe(_extremePoints[i].SlipForcePoint.x, _extremePoints[i].SlipForcePoint.y));
		}
		for (int j = 0; j < _asymptotePoints.Length; j++)
		{
			animationCurve.AddKey(new Keyframe(_asymptotePoints[j].SlipForcePoint.x, _asymptotePoints[j].SlipForcePoint.y));
		}
