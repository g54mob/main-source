...
namespace Assets.Scripts.Ui.CurveEditor
{
	public class CurveEditorScript : MonoBehaviour
	{
		...
		[SerializeField]
		private SpinnerScript _wrappingSpinner;
		...
		{
			RectTransformUtility.ScreenPointToLocalPointInRectangle(_visulisationTransform, eventData.Midpoint, null, out var localPoint);
			float scale = (eventData.Distance - eventData.DistanceDelta) / eventData.Distance;
			ScaleWindow(localPoint, scale);
			return true;
		}
		...
		private void Start()
		{
			...
			inputResponder.OnPinch = OnPinch;
			...
			_wrappingSpinner.Values.Clear();
			_wrappingSpinner.Values.Add(WrapMode.Once.ToString());
			_wrappingSpinner.Values.Add(WrapMode.Loop.ToString());
			_wrappingSpinner.Values.Add(WrapMode.PingPong.ToString());
			SpinnerScript wrappingSpinner = _wrappingSpinner;
			wrappingSpinner.OnValueChanged = (Action<string>)Delegate.Combine(wrappingSpinner.OnValueChanged, new Action<string>(SetWrapMode));
		...
		private void UpdateKeyEditors()
		{
			if (GetSelected(out var k, out var index))
			{
				...
				_tangentLinkedToggle.SetIsOnWithoutNotify(k.tangentMode == 0);
				_weightedLeftToggle.SetIsOnWithoutNotify(k.weightedMode == WeightedMode.In || k.weightedMode == WeightedMode.Both);
				_weightedRightToggle.SetIsOnWithoutNotify(k.weightedMode == WeightedMode.Out || k.weightedMode == WeightedMode.Both);
				_tangentLinkedToggle.GetComponent<CustomToggleScript>()?.UpdateOffImage();
				_weightedLeftToggle.GetComponent<CustomToggleScript>()?.UpdateOffImage();
				...
				if (index == 0)
				{
					_wrappingSpinner.Value = _curve.preWrapMode.ToString().Replace("ClampForever", "Clamp");
					_wrappingSettings.SetActive(value: true);
				}
				else if (index == _curve.length - 1)
				{
					_wrappingSpinner.Value = _curve.postWrapMode.ToString().Replace("ClampForever", "Clamp");
					_wrappingSettings.SetActive(value: true);
				}
				else
				{
					_wrappingSettings.SetActive(value: false);
				}
			}
			else
			{
