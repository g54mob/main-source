...
namespace Assets.Scripts.Settings
{
	public class UserDataStore_Xml : UserDataStore
	{
		...
		private const string editorLoadedMessage = "\nIf unexpected input issues occur, the loaded XML data may be outdated or invalid. Delete <Application.persistentDataPath>/<SaveFilePath> using the inspector option on the UserDataStore_XML component.";
		...
		public override void LoadPlayerData(int playerId)
		{
			...
			bool num = _cachedXmlDocument != null;
			if (!num)
			{
		...
		private void SaveControllerMap(Player player, ControllerMapSaveData saveData)
		{
			...
			foreach (XElement item in xElement.Elements())
			{
				if (!(item.Name != xElement2.Name) && !(item.Element(xName).Value != xElement2.Element(xName).Value))
				{
