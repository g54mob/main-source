...
namespace Vectrosity
{
	...
	public class VectorLine
	{
		...
		private struct Draw3DJob : IJob
		{
			public bool useTransformMatrix;

			public float4x3 thisMatrix;

			public float4x3 InverseTransformMatrix;

			public float4x4 ScreenToWorldMatrix;

			public float4x4 LocalWorldToScreen;

			public float4x4 matrixVP;

			[NativeDisableUnsafePtrRestriction]
			public unsafe float3* m_points3;

			[NativeDisableUnsafePtrRestriction]
			public unsafe float3* m_lineVertices;

			[NativeDisableUnsafePtrRestriction]
			public unsafe float3* m_screenPoints;

			[NativeDisableUnsafePtrRestriction]
			public unsafe float* m_lineWidths;
			...
			public float3 CameraPlaneNormal;

			public float CameraPlaneDistance;

			public int idxStart;

			public int widthIdxStart;

			public int widthIdxAdd;
			...
			public float3 WorldToScreenPoint(float3 p)
			{
				float4 float5 = math.mul(matrixVP, new float4(p.x, p.y, p.z, 1f));
				if (float5.w != 0f)
				{
					return new float3((float5.x / float5.w + 1f) * ScreenWidthHalf, (float5.y / float5.w + 1f) * ScreenHeightHalf, float5.z);
				}
				return 0;
			}
			...
			{
				return math.mul(new float4(p, 1f), InverseTransformMatrix);
			}

			[MethodImpl(MethodImplOptions.NoInlining)]
			public bool IntersectAndDoSkip(ref float3 pos1, ref float3 pos2, ref float3 p1, ref float3 p2)
			{
				if (pos1.z < 0f)
				{
					if (pos2.z < 0f)
					...
					pos1 = WorldToScreenPoint(PlaneIntersectionPoint(ref p2, ref p1));
					float3 float5 = InverseTransformPoint(p1);
					if ((float5.y < -1f && pos1.y > ScreenHeight) || (float5.y > 1f && pos1.y < 0f))
					{
						return true;
					}
				}
				if (pos2.z < 0f)
				{
					pos2 = WorldToScreenPoint(PlaneIntersectionPoint(ref p1, ref p2));
					float3 float6 = InverseTransformPoint(p2);
					if ((float6.y < -1f && pos2.y > ScreenHeight) || (float6.y > 1f && pos2.y < 0f))
					{
						return true;
					}
				}
			...
			[MethodImpl(MethodImplOptions.NoInlining)]
			private float3 PlaneIntersectionPoint(ref float3 p1, ref float3 p2)
			{
				float3 float5 = p1;
				float3 float6 = p2 - p1;
				float num = CameraPlaneRaycast(float5, float6);
				return float5 + float6 * num;
			}

			[MethodImpl(MethodImplOptions.NoInlining)]
			public float CameraPlaneRaycast(float3 rayOrigin, float3 rayDirection)
			{
				float num = math.dot(rayDirection, CameraPlaneNormal);
				float num2 = math.abs(num);
				if (num2 < math.max(1E-06f * num2, 9.536743E-07f))
				{
					return 0f;
				}
				return (0f - math.dot(rayOrigin, CameraPlaneNormal) - CameraPlaneDistance) / num;
			}

			[MethodImpl(MethodImplOptions.NoInlining)]
			public unsafe void SkipQuad3D(ref int idx, ref int widthIdx, ref int widthIdxAdd)
			{
				m_lineVertices[idx] = 0;
				m_lineVertices[idx + 1] = 0;
				m_lineVertices[idx + 2] = 0;
				m_lineVertices[idx + 3] = 0;
				m_screenPoints[idx] = 0;
				m_screenPoints[idx + 1] = 0;
				m_screenPoints[idx + 2] = 0;
				m_screenPoints[idx + 3] = 0;
				idx += 4;
				widthIdx += widthIdxAdd;
			}

			[MethodImpl(MethodImplOptions.NoInlining)]
			private float3 ScreenToWorldPoint(float3 position)
			{
				float4 float5 = math.mul(LocalWorldToScreen, new float4(position, 1f));
				float num = float5.z / float5.w;
				float3 xyz = new float3(position.x / ScreenWidth, position.y / ScreenHeight, (num + 1f) / 2f) * 2f - new float3(1);
				float5 = math.mul(ScreenToWorldMatrix, new float4(xyz, 1f));
				return float5.xyz / float5.w;
			}

			[MethodImpl(MethodImplOptions.NoInlining)]
			public float3 ScreenToWorldPointSuppressViewFrustumError(float3 position)
			{
				float num = math.lengthsq(position);
				if (num < 1E+16f)
				{
					return ScreenToWorldPoint(position);
				}
				float3 position2 = math.normalize(position) * math.clamp(math.sqrt(num), 0f, 100000000f);
				return ScreenToWorldPoint(position2);
			}

			public unsafe void Execute()
			{
				int idx = idxStart;
				int widthIdx = widthIdxStart;
				for (int i = start; i < end; i += add)
				{
					...
					{
						p = math.mul(new float4(m_points3[i], 1f), thisMatrix);
						p2 = math.mul(new float4(m_points3[i + 1], 1f), thisMatrix);
					}
					else
					{
						p = m_points3[i];
					...
					float3 pos = WorldToScreenPoint(p);
					float3 pos2 = WorldToScreenPoint(p2);
					if ((pos.x == pos2.x && pos.y == pos2.y) || IntersectAndDoSkip(ref pos, ref pos2, ref p, ref p2) || (double)math.lengthsq(pos) > 1000000000000000.0 || (double)math.lengthsq(pos2) > 1000000000000000.0)
					{
						SkipQuad3D(ref idx, ref widthIdx, ref widthIdxAdd);
						continue;
					}
					float5.x = pos2.y - pos.y;
					float5.y = pos.x - pos2.x;
					float3 float6 = float5 / math.sqrt(float5.x * float5.x + float5.y * float5.y);
					float5.x = float6.x * m_lineWidths[widthIdx];
					float5.y = float6.y * m_lineWidths[widthIdx];
					m_screenPoints[idx].x = pos.x - float5.x;
					m_screenPoints[idx].y = pos.y - float5.y;
					m_screenPoints[idx].z = pos.z - float5.z;
					m_screenPoints[idx + 3].x = pos.x + float5.x;
					m_screenPoints[idx + 3].y = pos.y + float5.y;
					m_screenPoints[idx + 3].z = pos.z + float5.z;
					m_lineVertices[idx] = ScreenToWorldPointSuppressViewFrustumError(m_screenPoints[idx]);
					m_lineVertices[idx + 3] = ScreenToWorldPointSuppressViewFrustumError(m_screenPoints[idx + 3]);
					if (doSmooth && i < end - add)
					{
						float5.x = float6.x * m_lineWidths[widthIdx + 1];
						float5.y = float6.y * m_lineWidths[widthIdx + 1];
					}
					m_screenPoints[idx + 2].x = pos2.x + float5.x;
					m_screenPoints[idx + 2].y = pos2.y + float5.y;
					m_screenPoints[idx + 2].z = pos2.z + float5.z;
					m_screenPoints[idx + 1].x = pos2.x - float5.x;
					m_screenPoints[idx + 1].y = pos2.y - float5.y;
					m_screenPoints[idx + 1].z = pos2.z - float5.z;
					m_lineVertices[idx + 2] = ScreenToWorldPointSuppressViewFrustumError(m_screenPoints[idx + 2]);
					m_lineVertices[idx + 1] = ScreenToWorldPointSuppressViewFrustumError(m_screenPoints[idx + 1]);
					idx += 4;
					widthIdx += widthIdxAdd;
				}
			}
		...
		private Func<object, Array> ExtractArrayFromList = (Func<object, Array>)typeof(Vector3).Assembly.GetType("UnityEngine.NoAllocHelpers").GetMethod("ExtractArrayFromList").CreateDelegate(typeof(Func<object, Array>), null);
		...
		private void SetEndCapColors()
		{
			...
			if (m_capType >= EndCap.Both)
			{
				...
				int num3 = num2 * ((m_lineType == LineType.Continuous) ? 4 : 2) - 8;
				if (num3 < -4)
				{
					num3 = -4;
				}
				for (int j = 4; j < 8; j++)
				{
					m_lineColors[j + m_vertexCount] = (m_useCapColors ? m_backColor : m_lineColors[j + num3]);
				}
			}
			if (m_vectorObject != null)
			{
		...
		public void SetUvs(List<Vector2> uvs)
		{
			...
			if (m_lineType != LineType.Points)
			{
				if (WrongArrayLength(uvs.Count, FunctionName.SetColors))
				{
					return;
				}
			}
			else if (uvs.Count != pointsCount)
			{
			...
			if (smoothColor)
			{
				m_lineUVs[num] = uvs[start];
				m_lineUVs[num + 3] = uvs[start];
				m_lineUVs[num + 2] = uvs[start];
				m_lineUVs[num + 1] = uvs[start];
				num += 4;
				for (int i = start + 1; i < end; i++)
				{
					m_lineUVs[num] = uvs[i - 1];
					m_lineUVs[num + 3] = uvs[i - 1];
					m_lineUVs[num + 2] = uvs[i];
					m_lineUVs[num + 1] = uvs[i];
					num += 4;
				}
			}
			else
			{
				...
				{
					m_lineUVs[num] = uvs[j];
					m_lineUVs[num + 1] = uvs[j];
					m_lineUVs[num + 2] = uvs[j];
					m_lineUVs[num + 3] = uvs[j];
					num += 4;
				}
			}
			if (m_vectorObject != null)
		...
		public unsafe void Draw3D()
		{
			...
			Ray ray = new Ray(v3zero, v3zero);
			float screenHeight = Screen.height;
			if (m_lineManager.BurstOptimizedDraw3D)
			{
				Matrix4x4 worldToLocalMatrix = camTransform.worldToLocalMatrix;
				Matrix4x4 matrix4x = cam3D.worldToCameraMatrix.inverse * cam3D.projectionMatrix.inverse;
				Matrix4x4 projectionMatrix = cam3D.projectionMatrix;
				projectionMatrix.m23 = 0f;
				projectionMatrix *= cam3D.worldToCameraMatrix;
				ulong gcHandle;
				float3* ptr = (float3*)UnsafeUtility.PinGCArrayAndGetDataAddress(ExtractArrayFromList(m_points3), out gcHandle);
				ulong gcHandle2;
				float3* ptr2 = (float3*)UnsafeUtility.PinGCArrayAndGetDataAddress(m_lineVertices, out gcHandle2);
				ulong gcHandle3;
				float3* screenPoints = (float3*)UnsafeUtility.PinGCArrayAndGetDataAddress(m_screenPoints, out gcHandle3);
				ulong gcHandle4;
				float* lineWidths = (float*)UnsafeUtility.PinGCArrayAndGetDataAddress(m_lineWidths, out gcHandle4);
				try
				{
					new Draw3DJob
					{
						useTransformMatrix = flag,
						thisMatrix = new float4x3(thisMatrix.GetRow(0), thisMatrix.GetRow(1), thisMatrix.GetRow(2)),
						InverseTransformMatrix = new float4x3(worldToLocalMatrix.GetRow(0), worldToLocalMatrix.GetRow(1), worldToLocalMatrix.GetRow(2)),
						ScreenToWorldMatrix = matrix4x,
						LocalWorldToScreen = cam3D.projectionMatrix * cam3D.worldToCameraMatrix * camTransform.localToWorldMatrix,
						matrixVP = projectionMatrix,
						m_points3 = ptr,
						ScreenWidthHalf = (float)cam3D.pixelWidth * 0.5f,
						ScreenHeightHalf = (float)cam3D.pixelHeight * 0.5f,
						ScreenWidth = cam3D.pixelWidth,
						ScreenHeight = cam3D.pixelHeight,
						CameraPlaneNormal = cameraPlane.normal,
						CameraPlaneDistance = cameraPlane.distance,
						m_lineVertices = ptr2,
						m_screenPoints = screenPoints,
						m_lineWidths = lineWidths,
						idxStart = num2,
						widthIdxStart = widthIdx,
						widthIdxAdd = widthIdxAdd,
						start = start,
						end = end,
						add = num,
						doSmooth = flag2,
				...
				finally
				{
					...
					UnsafeUtility.ReleaseGCObject(gcHandle4);
				}
			}
			else
			{
				Matrix4x4 projectionMatrix2 = cam3D.projectionMatrix;
				projectionMatrix2.m23 = 0f;
				projectionMatrix2 *= cam3D.worldToCameraMatrix;
				float num3 = (float)cam3D.pixelWidth * 0.5f;
				float num4 = (float)cam3D.pixelHeight * 0.5f;
				for (int i = start; i < end; i += num)
				{
					if (flag)
					{
						vector3 = thisMatrix.MultiplyPoint3x4(m_points3[i]);
						vector4 = thisMatrix.MultiplyPoint3x4(m_points3[i + 1]);
					}
					else
					{
						vector3 = m_points3[i];
						vector4 = m_points3[i + 1];
					}
					float num5 = projectionMatrix2.m30 * vector3.x + projectionMatrix2.m31 * vector3.y + projectionMatrix2.m32 * vector3.z + projectionMatrix2.m33;
					if (num5 == 0f)
					{
						pos.x = 0f;
						pos.y = 0f;
					...
					{
						pos.x = (projectionMatrix2.m00 * vector3.x + projectionMatrix2.m01 * vector3.y + projectionMatrix2.m02 * vector3.z + projectionMatrix2.m03) * num3 / num5 + num3;
						pos.y = (projectionMatrix2.m10 * vector3.x + projectionMatrix2.m11 * vector3.y + projectionMatrix2.m12 * vector3.z + projectionMatrix2.m13) * num4 / num5 + num4;
						pos.z = projectionMatrix2.m20 * vector3.x + projectionMatrix2.m21 * vector3.y + projectionMatrix2.m22 * vector3.z + projectionMatrix2.m23;
					}
					num5 = projectionMatrix2.m30 * vector4.x + projectionMatrix2.m31 * vector4.y + projectionMatrix2.m32 * vector4.z + projectionMatrix2.m33;
					if (num5 == 0f)
					{
						pos2.x = 0f;
						pos2.y = 0f;
					...
					{
						pos2.x = (projectionMatrix2.m00 * vector4.x + projectionMatrix2.m01 * vector4.y + projectionMatrix2.m02 * vector4.z + projectionMatrix2.m03) * num3 / num5 + num3;
						pos2.y = (projectionMatrix2.m10 * vector4.x + projectionMatrix2.m11 * vector4.y + projectionMatrix2.m12 * vector4.z + projectionMatrix2.m13) * num4 / num5 + num4;
						pos2.z = projectionMatrix2.m20 * vector4.x + projectionMatrix2.m21 * vector4.y + projectionMatrix2.m22 * vector4.z + projectionMatrix2.m23;
					}
					if ((pos.x == pos2.x && pos.y == pos2.y) || IntersectAndDoSkip(ref pos, ref pos2, ref vector3, ref vector4, ref screenHeight, ref ray, ref cameraPlane))
					{
						SkipQuad3D(ref num2, ref widthIdx, ref widthIdxAdd);
						continue;
					}
					vector2.x = pos2.y - pos.y;
					vector2.y = pos.x - pos2.x;
					...
					m_screenPoints[num2].x = pos.x - vector2.x;
					m_screenPoints[num2].y = pos.y - vector2.y;
					m_screenPoints[num2].z = pos.z - vector2.z;
					m_screenPoints[num2 + 3].x = pos.x + vector2.x;
					m_screenPoints[num2 + 3].y = pos.y + vector2.y;
					m_screenPoints[num2 + 3].z = pos.z + vector2.z;
					m_lineVertices[num2] = ScreenToWorldPointSuppressViewFrustumError(cam3D, m_screenPoints[num2]);
					m_lineVertices[num2 + 3] = ScreenToWorldPointSuppressViewFrustumError(cam3D, m_screenPoints[num2 + 3]);
					if (flag2 && i < end - num)
					{
						vector2.x = vector.x * m_lineWidths[widthIdx + 1];
						vector2.y = vector.y * m_lineWidths[widthIdx + 1];
					}
					m_screenPoints[num2 + 2].x = pos2.x + vector2.x;
					m_screenPoints[num2 + 2].y = pos2.y + vector2.y;
					m_screenPoints[num2 + 2].z = pos2.z + vector2.z;
					m_screenPoints[num2 + 1].x = pos2.x - vector2.x;
					m_screenPoints[num2 + 1].y = pos2.y - vector2.y;
					m_screenPoints[num2 + 1].z = pos2.z - vector2.z;
					m_lineVertices[num2 + 2] = ScreenToWorldPointSuppressViewFrustumError(cam3D, m_screenPoints[num2 + 2]);
					m_lineVertices[num2 + 1] = ScreenToWorldPointSuppressViewFrustumError(cam3D, m_screenPoints[num2 + 1]);
					num2 += 4;
					widthIdx += widthIdxAdd;
				}
			}
		...
		private Vector3 ScreenToWorldPointSuppressViewFrustumError(Camera cam, Vector3 position)
		{
			if (position.sqrMagnitude < 1E+16f)
			{
				return cam3D.ScreenToWorldPoint(position);
			}
			Vector3 position2 = position.normalized * Mathf.Clamp(position.magnitude, 0f, 100000000f);
			if (float.IsNaN(position2.x) || float.IsNaN(position2.y) || float.IsNaN(position2.z))
			{
				Debug.Log($"About to get frustum nan errors for line: {m_rectTransform.gameObject}");
			}
			return cam3D.ScreenToWorldPoint(position2);
		}
