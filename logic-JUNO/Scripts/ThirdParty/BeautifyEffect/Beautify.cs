...
namespace BeautifyEffect
{
	...
	public class Beautify : MonoBehaviour
	{
		...
		public float SunFlareOcclusion = 1f;
		...
		private const string BEAUTIFY_BUILD_HINT = "BeautifyBuildHint103b1";
		...
		{
			bloomDepthAtten = Mathf.Max(0f, bloomDepthAtten);
			antialiasDepthAtten = Mathf.Max(0f, antialiasDepthAtten);
		...
		private void DoOnPreRenderTasks()
		{
			...
			if (base.enabled && base.gameObject.activeSelf && !(currentCamera == null) && !(bMat == null) && (_depthOfField || _bloom || _anamorphicFlares))
			{
				...
				if ((_bloom && (int)_bloomCullingMask > 0) || (_anamorphicFlares && (int)_anamorphicFlaresCullingMask > 0))
				{
		...
		private void CheckBloomCullingLayer()
		{
			...
			if (_bloom && (int)_bloomCullingMask > 0)
			{
		...
		protected virtual void OnRenderImage(RenderTexture source, RenderTexture destination)
		{
			...
			if (flag)
			{
				...
				if (_depthOfField)
				{
					...
					bMat.SetTexture(ShaderParams.DoFTexture, renderTexture6);
				}
			...
			if (flag && (_lensDirt || _bloom || _anamorphicFlares || flag3))
			{
				...
				if (flag3)
				{
					...
					else
					{
						...
						float magnitude = currentCamera.farClipPlane;
						direction = -_sun.transform.forward;
					}
					Vector3 vector2 = currentCamera.WorldToViewportPoint(vector, (!isVRActive) ? Camera.MonoOrStereoscopicEye.Mono : Camera.MonoOrStereoscopicEye.Left);
					float num9 = 0f;
					if (vector2.z > 0f && vector2.x >= -0.1f && vector2.x < 1.1f && vector2.y >= -0.1f && vector2.y < 1.1f && !Physics.Raycast(new Ray(currentCamera.transform.position, direction), float.MaxValue, _sunFlaresLayerMask, QueryTriggerInteraction.Ignore))
					{
						...
						num9 *= SunFlareOcclusion;
					}
					if (num9 > sunFlareCurrentIntensity)
					{
						sunFlareCurrentIntensity = Mathf.Lerp(sunFlareCurrentIntensity, num9, Application.isPlaying ? (30f * Time.unscaledDeltaTime * _sunFlaresRevealSpeed) : 1f);
					}
					else
					{
						sunFlareCurrentIntensity = Mathf.Lerp(sunFlareCurrentIntensity, num9, Application.isPlaying ? (30f * Time.unscaledDeltaTime * _sunFlaresHideSpeed) : 1f);
					}
				...
				{
					bMat.SetTexture(ShaderParams.BloomTexture, renderTexture8);
				}
				else
				{
					if (bMat.IsKeywordEnabled("BEAUTIFY_BLOOM"))
			...
			if (flag && (_eyeAdaptation || _purkinje))
			{
				...
				bMat.SetTexture(ShaderParams.EALumSrc, rtEA[num12]);
				if (rtEAacum == null)
				{
			...
			if (renderTexture2 != null)
			{
				...
				bMat.SetTexture(ShaderParams.CompareTexture, renderTexture2);
			...
			if (renderTexture3 != null)
			{
				...
				if (renderTexture2 != null)
				{
					bMat.SetTexture(ShaderParams.CompareTexture, renderTexture3);
		...
		private void SeparateOutlinePass(RenderTexture source)
		{
			...
			{
				BlurThisOutline(temporary, _outlineSpread, i);
			}
		...
		public void UpdateMaterialPropertiesNow()
		{
			...
			{
				renderPass = 0;
				float f = (_compareSameSide ? (MathF.PI / 2f) : _compareLineAngle);
				bMat.SetVector(ShaderParams.CompareData, new Vector4(Mathf.Cos(f), Mathf.Sin(f), _compareSameSide ? _comparePanning : (-10f), _compareLineWidth));
			}
			if (_quality != BEAUTIFY_QUALITY.Basic)
			{
				if (_lut && (_lutTexture != null || _lutTexture3D != null))
				{
					...
					else
					{
						...
						bMat.SetVector(ShaderParams.LUT3DParams, new Vector4(1f / (float)_lutTexture3D.width, (float)_lutTexture3D.width - 1f, 0f, 0f));
					}
		...
		private void UpdateMaterialBloomIntensityAndThreshold()
		{
			...
			bMat.SetVector(ShaderParams.Bloom, new Vector4(_bloomIntensity + (_anamorphicFlares ? 0.0001f : 0f), 0f, 0f, num));
		}
		...
		private void UpdateMaterialAnamorphicIntensityAndThreshold()
		{
			...
			bMat.SetVector(ShaderParams.Bloom, new Vector4(x, 0f, 0f, num));
		}
