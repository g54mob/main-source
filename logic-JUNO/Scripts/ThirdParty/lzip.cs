...
public class lzip
{
	...
	public class CustomWebRequest : DownloadHandlerScript
	{
		...
		protected override bool ReceiveData(byte[] bytesFromServer, int dataLength)
		{
			...
			GCHandle gCHandle = gcA(bytesFromServer);
			addToBuffer(nativeBuffer, nativeOffset, gCHandle.AddrOfPinnedObject(), dataLength);
	...
	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern bool zipBuf2File(int levelOfCompression, [MarshalAs(UnmanagedType.LPWStr)] string zipArchive, [MarshalAs(UnmanagedType.LPWStr)] string arcFilename, IntPtr buffer, int bufferSize, [MarshalAs(UnmanagedType.LPWStr)] string comment, [MarshalAs(UnmanagedType.LPStr)] string password, bool useBz2);

	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern int zipDeleteFile([MarshalAs(UnmanagedType.LPWStr)] string zipArchive, [MarshalAs(UnmanagedType.LPWStr)] string arcFilename, [MarshalAs(UnmanagedType.LPWStr)] string tempArchive);
	...
	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern int zipEntry([MarshalAs(UnmanagedType.LPWStr)] string zipArchive, [MarshalAs(UnmanagedType.LPWStr)] string arcFilename, [MarshalAs(UnmanagedType.LPWStr)] string outpath, IntPtr FileBuffer, int fileBufferLength, IntPtr proc, [MarshalAs(UnmanagedType.LPStr)] string password);

	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern int zipEntryList([MarshalAs(UnmanagedType.LPWStr)] string zipArchive, IntPtr outpath, IntPtr filename, int arrayLength, IntPtr FileBuffer, int fileBufferLength, IntPtr proc, [MarshalAs(UnmanagedType.LPStr)] string password);

	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern int zipEntries2Buffers([MarshalAs(UnmanagedType.LPWStr)] string zipArchive, IntPtr filenames, IntPtr buffers, int arrayLength, IntPtr FileBuffer, int fileBufferLength, IntPtr proc, [MarshalAs(UnmanagedType.LPStr)] string password);

	[DllImport("libzipw", CharSet = CharSet.Auto)]
	internal static extern uint getEntryDateTime([MarshalAs(UnmanagedType.LPWStr)] string zipArchive, [MarshalAs(UnmanagedType.LPWStr)] string arcFilename, IntPtr FileBuffer, int fileBufferLength);
	...
	[DllImport("libzipw")]
	internal static extern long unGzipBuffer2File(IntPtr inBuffer, int inLength, [MarshalAs(UnmanagedType.LPWStr)] string outFile, IntPtr progress);
	...
	private static bool checkObject(object o, string zipArchive, ref int len, ref IntPtr ptr)
	{
		...
		{
			inMemory inMemory2 = (inMemory)o;
			len = inMemory2.info[0];
			ptr = inMemory2.pointer;
		}
		return true;
	}
	...
	public static ulong getFileInfo(string zipArchive, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				num = zipGetInfoA(null, gCHandle.AddrOfPinnedObject(), gCHandle2.AddrOfPinnedObject(), array2.Length);
				gCHandle2.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				int len = 0;
				if (!checkObject(fileBuffer, zipArchive, ref len, ref ptr))
				{
					gCHandle.Free();
					return 0uL;
				}
				num = zipGetInfoA(null, gCHandle.AddrOfPinnedObject(), ptr, len);
			}
		}
		else
		{
		...
		GCHandle gCHandle5 = gcA(array5);
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array6 = (byte[])fileBuffer;
				GCHandle gCHandle6 = gcA(array6);
				intPtr = zipGetInfo(null, num, gCHandle3.AddrOfPinnedObject(), gCHandle4.AddrOfPinnedObject(), gCHandle5.AddrOfPinnedObject(), gCHandle6.AddrOfPinnedObject(), array6.Length);
				gCHandle6.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				int len2 = 0;
				if (!checkObject(fileBuffer, zipArchive, ref len2, ref ptr2))
				{
					gCHandle3.Free();
					gCHandle4.Free();
					gCHandle5.Free();
				...
				intPtr = zipGetInfo(null, num, gCHandle3.AddrOfPinnedObject(), gCHandle4.AddrOfPinnedObject(), gCHandle5.AddrOfPinnedObject(), ptr2, len2);
			}
		}
		else
		{
			intPtr = zipGetInfo(zipArchive, num, gCHandle3.AddrOfPinnedObject(), gCHandle4.AddrOfPinnedObject(), gCHandle5.AddrOfPinnedObject(), IntPtr.Zero, 0);
		}
		...
		for (int i = 0; i < array[0]; i++)
		{
			...
			{
				cinfo.Add(array4[i]);
				totalCompressedSize += array4[i];
			}
			if (array5 != null)
			{
				localOffset.Add(array5[i]);
			}
		}
		stringReader.Close();
	...
	public static int getTotalFiles(string zipArchive, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				GCHandle gCHandle = gcA(array);
				result = zipGetTotalFiles(null, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipGetTotalFiles(null, ptr, len);
			}
			return result;
		}
	...
	public static int getTotalEntries(string zipArchive, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				result = zipGetTotalEntries(null, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipGetTotalEntries(null, ptr, len);
			}
			return result;
		}
	...
	public static ulong getEntrySize(string zipArchive, string entry, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				result = zipGetEntrySize(null, entry, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipGetEntrySize(null, entry, ptr, len);
			}
			return result;
		}
	...
	public static bool entryExists(string zipArchive, string entry, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				result = zipEntryExists(null, entry, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipEntryExists(null, entry, ptr, len);
			}
			return result;
		}
	...
	public static bool buffer2File(int levelOfCompression, string zipArchive, string arcFilename, byte[] buffer, bool append = false, string comment = null, string password = null, bool useBz2 = false)
	{
		...
		bool result = zipBuf2File(levelOfCompression, zipArchive, arcFilename, gCHandle.AddrOfPinnedObject(), buffer.Length, comment, password, useBz2);
		gCHandle.Free();
		return result;
	}
	...
	public static int delete_entry(string zipArchive, string arcFilename)
	{
		...
		int num = zipDeleteFile(zipArchive, arcFilename, text);
		if (num > 0)
		{
			File.Delete(zipArchive);
	...
	{
		if (delete_entry(zipArchive, arcFilename) < 0)
		{
			return -3;
		}
		if (password == "")
	...
	public static int replace_entry(string zipArchive, string arcFilename, byte[] newFileBuffer, int level = 9, string password = null, bool useBz2 = false)
	{
		...
		if (buffer2File(level, zipArchive, arcFilename, newFileBuffer, append: true, null, password, useBz2))
		{
			return 1;
		}
		return -6;
	...
	public static int extract_entry(string zipArchive, string arcFilename, string outpath, object fileBuffer = null, ulong[] proc = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				result = zipEntry(null, arcFilename, outpath, gCHandle2.AddrOfPinnedObject(), array.Length, gCHandle.AddrOfPinnedObject(), password);
				gCHandle2.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipEntry(null, arcFilename, outpath, ptr, len, gCHandle.AddrOfPinnedObject(), password);
			}
			gCHandle.Free();
			return result;
		}
		result = zipEntry(zipArchive, arcFilename, outpath, IntPtr.Zero, 0, gCHandle.AddrOfPinnedObject(), password);
		gCHandle.Free();
		return result;
	}
	...
	public static int extract_entries(string zipArchive, string[] fileList, string outpath, object fileBuffer = null, ulong[] proc = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				GCHandle gCHandle4 = gcA(array4);
				result = zipEntryList(null, gCHandle3.AddrOfPinnedObject(), gCHandle2.AddrOfPinnedObject(), num, gCHandle4.AddrOfPinnedObject(), array4.Length, gCHandle.AddrOfPinnedObject(), password);
				gCHandle4.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipEntryList(null, gCHandle3.AddrOfPinnedObject(), gCHandle2.AddrOfPinnedObject(), num, ptr, len, gCHandle.AddrOfPinnedObject(), password);
			}
			gCHandle.Free();
			for (int k = 0; k < num; k++)
			{
		...
		result = zipEntryList(zipArchive, gCHandle3.AddrOfPinnedObject(), gCHandle2.AddrOfPinnedObject(), num, IntPtr.Zero, 0, gCHandle.AddrOfPinnedObject(), password);
		gCHandle.Free();
		for (int l = 0; l < num; l++)
		{
	...
	public static int decompress_File(string zipArchive, string outPath = null, int[] progress = null, object fileBuffer = null, ulong[] proc = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			...
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle3 = gcA(array);
				int result = zipEX(null, outPath, gCHandle.AddrOfPinnedObject(), gCHandle3.AddrOfPinnedObject(), array.Length, gCHandle2.AddrOfPinnedObject(), password);
				gCHandle3.Free();
				gCHandle.Free();
				gCHandle2.Free();
				return result;
			...
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				int result2 = zipEX(null, outPath, gCHandle.AddrOfPinnedObject(), ptr, len, gCHandle2.AddrOfPinnedObject(), password);
				gCHandle.Free();
				gCHandle2.Free();
				return result2;
			}
		}
		int result3 = zipEX(zipArchive, outPath, gCHandle.AddrOfPinnedObject(), IntPtr.Zero, 0, gCHandle2.AddrOfPinnedObject(), password);
		gCHandle.Free();
		gCHandle2.Free();
		return result3;
	}
	...
	public static DateTime entryDateTime(string zipArchive, string entry, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				num = getEntryDateTime(null, entry, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				num = getEntryDateTime(null, entry, ptr, len);
			}
		}
		else
		{
	...
	public static int decompress_Mem2File(inMemory t, string outPath, int[] progress = null, ulong[] proc = null, string password = null)
	{
		...
		if (t != null)
		{
			int result = zipEX(null, outPath, gCHandle.AddrOfPinnedObject(), t.pointer, t.info[0], gCHandle2.AddrOfPinnedObject(), password);
			gCHandle.Free();
			gCHandle2.Free();
			return result;
		}
	...
	public static int entry2Buffer(string zipArchive, string entry, ref byte[] buffer, object fileBuffer = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				num = (int)zipGetEntrySize(null, entry, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				num = (int)zipGetEntrySize(null, entry, ptr, len);
			}
		}
		else
		{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				result = zipEntry2Buffer(null, entry, gCHandle2.AddrOfPinnedObject(), num, gCHandle3.AddrOfPinnedObject(), array2.Length, password);
				gCHandle3.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipEntry2Buffer(null, entry, gCHandle2.AddrOfPinnedObject(), num, ptr2, len2, password);
			}
		}
		else
		{
	...
	public static int entry2FixedBuffer(string zipArchive, string entry, ref byte[] fixedBuffer, object fileBuffer = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array2 = (byte[])fileBuffer;
				GCHandle gCHandle3 = gcA(array2);
				num2 = zipEntry2Buffer(null, entry, gCHandle2.AddrOfPinnedObject(), num, gCHandle3.AddrOfPinnedObject(), array2.Length, password);
				gCHandle3.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				num2 = zipEntry2Buffer(null, entry, gCHandle2.AddrOfPinnedObject(), num, ptr2, len2, password);
			}
		}
		else
		{
	...
	public static byte[] entry2Buffer(string zipArchive, string entry, object fileBuffer = null, string password = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				...
				GCHandle gCHandle3 = gcA(array3);
				num2 = zipEntry2Buffer(null, entry, gCHandle2.AddrOfPinnedObject(), num, gCHandle3.AddrOfPinnedObject(), array3.Length, password);
				gCHandle3.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
	...
	public static byte[][] entries2Buffers(string zipArchive, string[] entries, ref int progress, object fileBuffer = null, string password = null)
	{
		if (password == "")
		{
			password = null;
		...
		if (fileBuffer != null)
		{
			...
			{
				byte[] array3 = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array3);
				for (int i = 0; i < num; i++)
				{
					array[i] = (int)zipGetEntrySize(null, entries[i], gCHandle.AddrOfPinnedObject(), array3.Length);
				}
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				for (int k = 0; k < num; k++)
				{
					array[k] = (int)zipGetEntrySize(null, entries[k], ptr, len);
				}
			}
		}
		else
		{
			for (int l = 0; l < num; l++)
			{
				array[l] = (int)zipGetEntrySize(zipArchive, entries[l], IntPtr.Zero, 0);
			}
		}
		GCHandle[] array4 = new GCHandle[num];
		for (int m = 0; m < num; m++)
		{
			array2[m] = new byte[array[m]];
			array4[m] = gcA(array2[m]);
		}
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array5 = (byte[])fileBuffer;
				GCHandle gCHandle2 = gcA(array5);
				for (int n = 0; n < num; n++)
				{
					if (zipEntry2Buffer(null, entries[n], array4[n].AddrOfPinnedObject(), array[n], gCHandle2.AddrOfPinnedObject(), array5.Length, password) != 1)
					{
						Debug.Log("Error extracting entry at index: " + n);
					}
				}
				gCHandle2.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				{
					if (zipEntry2Buffer(null, entries[num3], array4[num3].AddrOfPinnedObject(), array[num3], ptr2, len2, password) != 1)
					{
						Debug.Log("Error extracting entry at index: " + num3);
					}
				}
			}
		}
		else
		{
			...
			{
				if (zipEntry2Buffer(zipArchive, entries[num4], array4[num4].AddrOfPinnedObject(), array[num4], IntPtr.Zero, 0, password) != 1)
				{
					Debug.Log("Error extracting entry at index: " + num4);
				}
			}
		}
	...
	public static bool validateFile(string zipArchive, object fileBuffer = null)
	{
		...
		if (fileBuffer != null)
		{
			if (fileBuffer is byte[])
			{
				byte[] array = (byte[])fileBuffer;
				GCHandle gCHandle = gcA(array);
				result = zipValidateFile(null, gCHandle.AddrOfPinnedObject(), array.Length);
				gCHandle.Free();
			}
			if (fileBuffer is IntPtr || fileBuffer is inMemory)
			{
				...
				result = zipValidateFile(null, ptr, len);
			}
			return result;
		}
	...
	public static int decompressZipMerged(string file, string outPath, int[] progress = null, ulong[] proc = null, string password = null)
	{
		...
		if (mergedZip != null)
		{
			inMemory obj = new inMemory();
			GCHandle gCHandle = gcA(mergedZip);
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64());
			obj.info[0] = siz;
			result = decompress_Mem2File(obj, outPath, progress, proc, password);
			gCHandle.Free();
			obj.info = null;
	...
	public static int decompressZipMerged(byte[] buffer, string outPath, int[] progress = null, ulong[] proc = null, string password = null)
	{
		...
		if (getZipInfoMerged(buffer, ref pos, ref size))
		{
			inMemory obj = new inMemory();
			GCHandle gCHandle = gcA(buffer);
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64() + pos);
			obj.info[0] = size;
			result = decompress_Mem2File(obj, outPath, progress, proc, password);
			gCHandle.Free();
			obj.info = null;
	...
	public static int entry2FileMerged(string file, string entry, string outPath, string overrideEntryName = null, string password = null)
	{
		...
		if (mergedZip != null)
		{
			...
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64());
			obj.info[0] = siz;
			byte[] tb = entry2BufferMem(obj, entry, password);
			gCHandle.Free();
	...
	public static byte[] entry2BufferMerged(string file, string entry, string password = null)
	{
		...
		if (mergedZip != null)
		{
			...
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64());
			obj.info[0] = siz;
			byte[] result = entry2BufferMem(obj, entry, password);
			gCHandle.Free();
			obj.info = null;
			obj.pointer = IntPtr.Zero;
			mergedZip = null;
	...
	public static int entry2BufferMerged(string file, string entry, ref byte[] refBuffer, string password = null)
	{
		...
		if (mergedZip != null)
		{
			...
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64());
			obj.info[0] = siz;
			int result = entry2BufferMem(obj, entry, ref refBuffer, password);
			gCHandle.Free();
			obj.info = null;
			obj.pointer = IntPtr.Zero;
			mergedZip = null;
	...
	public static int entry2FixedBufferMerged(string file, string entry, ref byte[] fixedBuffer, string password = null)
	{
		...
		if (mergedZip != null)
		{
			...
			obj.pointer = new IntPtr(gCHandle.AddrOfPinnedObject().ToInt64());
			obj.info[0] = siz;
			int result = entry2FixedBufferMem(obj, entry, ref fixedBuffer, password);
			gCHandle.Free();
			obj.info = null;
			obj.pointer = IntPtr.Zero;
			mergedZip = null;
	...
	public static int unGzip2(object source, byte[] outBuffer, int intPtrLength = 0)
	{
		...
		if (source is byte[])
		{
			...
			GCHandle gCHandle = gcA(array);
			GCHandle gCHandle2 = gcA(outBuffer);
			result = zipUnGzip2(gCHandle.AddrOfPinnedObject(), array.Length, gCHandle2.AddrOfPinnedObject(), outBuffer.Length);
			gCHandle.Free();
			gCHandle2.Free();
		}
		if (source is IntPtr && intPtrLength > 0)
		{
			...
			GCHandle gCHandle3 = gcA(outBuffer);
			result = zipUnGzip2(source2, intPtrLength, gCHandle3.AddrOfPinnedObject(), outBuffer.Length);
			gCHandle3.Free();
		}
		else if (intPtrLength == 0 && source is IntPtr)
		{
			return -11;
		}
		return result;
	...
	public static long ungzipBuffer2File(byte[] fileBuffer, string outFile, ulong[] progress = null)
	{
		long result = 0L;
		if (fileBuffer == null)
		{
		...
		result = unGzipBuffer2File(gCHandle2.AddrOfPinnedObject(), fileBuffer.Length, outFile.Replace("//", "/"), gCHandle.AddrOfPinnedObject());
		gCHandle2.Free();
		gCHandle.Free();
		return result;
	}
	...
	public static IEnumerator downloadZipFileNative(string url, Action<bool> downloadDone, Action<inMemory> inmem, Action<IntPtr> pointer = null, Action<int> fileSize = null)
	{
		...
		else if (!nativeBufferIsBeingUsed)
		{
			...
			nativeBuffer = createBuffer(zipSize);
			nativeBufferIsBeingUsed = true;
			byte[] buffer = new byte[2048];
			nativeOffset = 0;
			using UnityWebRequest wwwSK = UnityWebRequest.Get(url);
			wwwSK.downloadHandler = new CustomWebRequest(buffer);
			...
			downloadDone(obj: true);
			if (inmem != null)
			{
				inMemory inMemory2 = new inMemory();
				inMemory2.pointer = nativeBuffer;
				inMemory2.info[0] = zipSize;
				inmem(inMemory2);
			}
			if (pointer != null)
			{
