...
namespace Rewired
{
	...
	public sealed class ControllerSetSelector : ISerializationCallbackReceiver, IDeepCloneable
	{
		...
		[NonSerialized]
		private Guid sEFMtFqfrlZkhQrPkconvPeurLqf;

		internal bool FHvZVuWkzIDKCbDbOJtHxepOWYcj => _type != Type.All;
		...
		public Guid hardwareTypeGuid
		{
			get
			{
				...
				return sEFMtFqfrlZkhQrPkconvPeurLqf;
			}
			set
			{
				...
				sEFMtFqfrlZkhQrPkconvPeurLqf = value;
		...
		public ControllerSetSelector(ControllerSetSelector P_0)
		{
			...
			sEFMtFqfrlZkhQrPkconvPeurLqf = P_0.sEFMtFqfrlZkhQrPkconvPeurLqf;
		}
		...
		internal ControllerSetSelector(Type P_0, ControllerType P_1, string P_2, string P_3, int P_4)
		{
			...
			_controllerType = P_1;
			_guid = P_2;
			sEFMtFqfrlZkhQrPkconvPeurLqf = StringTools.ToGuid(P_2);
		...
		public bool Matches(Controller controller)
		{
			...
			case Type.HardwareType:
				if (sEFMtFqfrlZkhQrPkconvPeurLqf != Guid.Empty)
				{
					return sEFMtFqfrlZkhQrPkconvPeurLqf == controller.hardwareTypeGuid;
				}
			...
			case Type.ControllerTemplateType:
				return controller.ImplementsTemplate(sEFMtFqfrlZkhQrPkconvPeurLqf);
			case Type.PersistentControllerInstance:
				return controller.deviceInstanceGuid == sEFMtFqfrlZkhQrPkconvPeurLqf;
		...
		private void XudbHxySrBCAhWeMUCfHgNVvqHCN()
		{
			_guid = string.Empty;
			sEFMtFqfrlZkhQrPkconvPeurLqf = Guid.Empty;
		}

		void ISerializationCallbackReceiver.OnAfterDeserialize()
		{
			sEFMtFqfrlZkhQrPkconvPeurLqf = StringTools.ToGuid(_guid);
		}
