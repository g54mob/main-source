...
namespace Rewired
{
	...
	{
		private const string iNaoqMSgUwejGFFoXqBhVlCkklQFA = "[^a-fA-F0-9]";
		...
		{
			if (string.IsNullOrEmpty(snogUAesMCVEyiqowUtWbeJRimIS(P_0)))
			{
				productId = 0;
				vendorId = 0;
				return;
		...
		{
			return MUlxWzrOAUtdFXPRtKHfsFYbllfs(snogUAesMCVEyiqowUtWbeJRimIS(pidVid));
		}
		...
		private bool MUlxWzrOAUtdFXPRtKHfsFYbllfs(string P_0)
		{
		...
		private static string snogUAesMCVEyiqowUtWbeJRimIS(string P_0)
		{
