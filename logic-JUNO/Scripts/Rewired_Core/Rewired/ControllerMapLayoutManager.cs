...
namespace Rewired
{
	...
	{
		internal class BDYFZVSWwKErjELSidgTrNxCIGTnA
		{
			public bool CKwChHVitTPognlNvZtUmPSeaTOJA;

			public bool sxmEigjJpHpZWQQJOuYEIlENsJEab;

			public OsbedCVLkTpeBrpSrCSZofCNmUcA[] GwTGythaHnUNnWqdIDuhzNDVJsxDA;

			public BDYFZVSWwKErjELSidgTrNxCIGTnA(bool P_0, bool P_1, OsbedCVLkTpeBrpSrCSZofCNmUcA[] P_2)
			{
				CKwChHVitTPognlNvZtUmPSeaTOJA = P_0;
				sxmEigjJpHpZWQQJOuYEIlENsJEab = P_1;
				GwTGythaHnUNnWqdIDuhzNDVJsxDA = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.FHvZVuWkzIDKCbDbOJtHxepOWYcj)
					{
		...
		private bool naEqjZSHXWBGZfwQTUiugMWvYWeb;

		private bool xrYBWJdQNhIDCRgxagfmXgoJecIrA = true;

		private Player yEvQiphUlUGMRKcQMiTYLpLlItyh;

		private BDYFZVSWwKErjELSidgTrNxCIGTnA cEzQnBnvwZjHbSdNAcmiVcPruoEJ;

		private readonly int heqXAVFIfSCcmXdpyUBHGAIOFEAd;

		private List<RuleSet> rXQfjmJPMxcpsKtFVZMUgecysGOj;

		private Action yZGzQaYALHAPfGiMgPZPLqsbCTKZ;

		public bool enabled
		{
			get
			{
				return naEqjZSHXWBGZfwQTUiugMWvYWeb;
			}
			set
			{
				naEqjZSHXWBGZfwQTUiugMWvYWeb = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return xrYBWJdQNhIDCRgxagfmXgoJecIrA;
			}
			set
			{
				xrYBWJdQNhIDCRgxagfmXgoJecIrA = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return rXQfjmJPMxcpsKtFVZMUgecysGOj;
			}
			set
			{
				if (value == null)
				...
				rXQfjmJPMxcpsKtFVZMUgecysGOj = value;
			}
		}

		internal event Action zNjuBCCyvWYQzcJMfVlZrRpLTHZL
		{
			add
			{
				yZGzQaYALHAPfGiMgPZPLqsbCTKZ = (Action)Delegate.Combine(yZGzQaYALHAPfGiMgPZPLqsbCTKZ, b);
			}
			remove
			{
				yZGzQaYALHAPfGiMgPZPLqsbCTKZ = (Action)Delegate.Remove(yZGzQaYALHAPfGiMgPZPLqsbCTKZ, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, BDYFZVSWwKErjELSidgTrNxCIGTnA P_1)
		{
			if (P_0 == null)
			{
			...
			heqXAVFIfSCcmXdpyUBHGAIOFEAd = ReInput.id;
			yEvQiphUlUGMRKcQMiTYLpLlItyh = P_0;
			cEzQnBnvwZjHbSdNAcmiVcPruoEJ = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != heqXAVFIfSCcmXdpyUBHGAIOFEAd)
			{
				ReInput.CheckInitialized(heqXAVFIfSCcmXdpyUBHGAIOFEAd);
				return;
			}
			yZGzQaYALHAPfGiMgPZPLqsbCTKZ?.Invoke();
			if (!naEqjZSHXWBGZfwQTUiugMWvYWeb || rXQfjmJPMxcpsKtFVZMUgecysGOj == null)
			{
				return;
			}
			int count = rXQfjmJPMxcpsKtFVZMUgecysGOj.Count;
			if (count == 0)
			{
				return;
			}
			...
			yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.maps.GetAllMaps(list);
			list2.AddRange(yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = rXQfjmJPMxcpsKtFVZMUgecysGOj[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (xrYBWJdQNhIDCRgxagfmXgoJecIrA && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(yEvQiphUlUGMRKcQMiTYLpLlItyh.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							yEvQiphUlUGMRKcQMiTYLpLlItyh.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((cEzQnBnvwZjHbSdNAcmiVcPruoEJ != null && cEzQnBnvwZjHbSdNAcmiVcPruoEJ.GwTGythaHnUNnWqdIDuhzNDVJsxDA != null) ? cEzQnBnvwZjHbSdNAcmiVcPruoEJ.GwTGythaHnUNnWqdIDuhzNDVJsxDA.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(cEzQnBnvwZjHbSdNAcmiVcPruoEJ.GwTGythaHnUNnWqdIDuhzNDVJsxDA[i].YcirogJSGffoXdrlqzSiCamDTVLD);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = cEzQnBnvwZjHbSdNAcmiVcPruoEJ.GwTGythaHnUNnWqdIDuhzNDVJsxDA[i].garTmoeVreesVgkcGInPfqjzFRyKA;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (cEzQnBnvwZjHbSdNAcmiVcPruoEJ != null)
			{
				naEqjZSHXWBGZfwQTUiugMWvYWeb = cEzQnBnvwZjHbSdNAcmiVcPruoEJ.CKwChHVitTPognlNvZtUmPSeaTOJA;
				xrYBWJdQNhIDCRgxagfmXgoJecIrA = cEzQnBnvwZjHbSdNAcmiVcPruoEJ.sxmEigjJpHpZWQQJOuYEIlENsJEab;
			}
			rXQfjmJPMxcpsKtFVZMUgecysGOj = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return KOpxGGkYNWqCgMXtUuLQEMvnZRdi().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return KOpxGGkYNWqCgMXtUuLQEMvnZRdi().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				YrUStvawFUXgpVPkaPkjMaUcbJPy(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				YrUStvawFUXgpVPkaPkjMaUcbJPy(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject KOpxGGkYNWqCgMXtUuLQEMvnZRdi()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			grzUCLGfBJbTtgiaBqbiIzTfKuSt(serializedObject);
			return serializedObject;
		}

		private void grzUCLGfBJbTtgiaBqbiIzTfKuSt(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", naEqjZSHXWBGZfwQTUiugMWvYWeb);
			P_0.Add("loadFromUserDataStore", xrYBWJdQNhIDCRgxagfmXgoJecIrA);
			P_0.Add("ruleSets", rXQfjmJPMxcpsKtFVZMUgecysGOj);
		}

		private bool YrUStvawFUXgpVPkaPkjMaUcbJPy(SerializedObject P_0)
		{
			naEqjZSHXWBGZfwQTUiugMWvYWeb = false;
			rXQfjmJPMxcpsKtFVZMUgecysGOj = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref naEqjZSHXWBGZfwQTUiugMWvYWeb);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref xrYBWJdQNhIDCRgxagfmXgoJecIrA);
