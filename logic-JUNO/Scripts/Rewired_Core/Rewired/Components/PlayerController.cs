...
namespace Rewired.Components
{
	...
	public class PlayerController : ComponentWrapper<Rewired.PlayerController>, IPlayerController
	{
		...
		protected override void OnAwakeFinished()
		{
			...
			if (base.initialized)
			{
				xAkvZwcJDlQRrNiUnXHVknctoNFb(true);
			}
		}
		...
		protected override void Subscribe()
		{
			...
			if (base.source != null)
			{
				base.source.Rewired_002EIPlayerController_002EButtonStateChangedEvent += fTPSNyfFVWxtrBZQENdehZpNKijl;
				base.source.Rewired_002EIPlayerController_002EAxisValueChangedEvent += QamEgrFdwgwbyuRKwPORGNkKuiWZb;
				base.source.Rewired_002EIPlayerController_002EEnabledStateChangedEvent += xAkvZwcJDlQRrNiUnXHVknctoNFb;
			}
		}
		...
		internal virtual List<ElementInfo> rFmLfaTexIEhWsBcEfIsAmWRWsAI()
		{
		...
		private void fTPSNyfFVWxtrBZQENdehZpNKijl(int P_0, bool P_1)
		{
		...
		private void QamEgrFdwgwbyuRKwPORGNkKuiWZb(int P_0, float P_1)
		{
			if (!base.isActiveAndEnabled)
			{
				return;
		...
		private void xAkvZwcJDlQRrNiUnXHVknctoNFb(bool P_0)
		{
			try
			{
		...
		private void bTiHOkBAHanvdUFBOeDZlIQxCbom()
		{
			if (_elements == null || _elements.Count <= 0)
			{
				_elements = rFmLfaTexIEhWsBcEfIsAmWRWsAI();
			}
		}
	}
}
