...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			lscGNxLgcOKIppHglLXMWVUxLHTT(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return VUcHkJzDQlBtIyfaaDcFIeFuLVKQ().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				ErVevUhLJOCHUCRkZcgZWRqmojZVA(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return VUcHkJzDQlBtIyfaaDcFIeFuLVKQ().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				ErVevUhLJOCHUCRkZcgZWRqmojZVA(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			lscGNxLgcOKIppHglLXMWVUxLHTT(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.TSZoBUIlaafTMpBTwSqsYUMLGdBF(_id);
			if (inputBehavior != null)
			{
				lscGNxLgcOKIppHglLXMWVUxLHTT(inputBehavior, this, true);
			}
		}

		internal SerializedObject VUcHkJzDQlBtIyfaaDcFIeFuLVKQ()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.StxLVFERPlwSUZNlMaKuFuVAjcqCb
			{
				...
				wqpBUPsVbkYZOHRjZkDHzExwrqmJ = 5.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.StxLVFERPlwSUZNlMaKuFuVAjcqCb
			{
				rVQdJsUVGueUoRlsQQCEHMDLFJOq = "id",
				wqpBUPsVbkYZOHRjZkDHzExwrqmJ = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.StxLVFERPlwSUZNlMaKuFuVAjcqCb
			{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.StxLVFERPlwSUZNlMaKuFuVAjcqCb
			{
				...
				JTcffmzfUBZAVjPblkObnRNPpqZG = null,
				wqpBUPsVbkYZOHRjZkDHzExwrqmJ = string.Format("{0} {1}{2}{3}{4}{5}", "http://guavaman.com/rewired", "http://guavaman.com/schemas/rewired/", "1.4", "/", GetType().Name, ".xsd")
			});
		...
		internal void ErVevUhLJOCHUCRkZcgZWRqmojZVA(SerializedObject P_0)
		{
			Reset();
		...
		private static void lscGNxLgcOKIppHglLXMWVUxLHTT(InputBehavior P_0, InputBehavior P_1, bool P_2)
		{
			if (P_2)
			{
				P_1._id = P_0._id;
