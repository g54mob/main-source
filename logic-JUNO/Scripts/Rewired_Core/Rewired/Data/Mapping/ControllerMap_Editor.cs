...
namespace Rewired.Data.Mapping
{
	...
	{
		private sealed class BvZZIfUcrFGPKmjrRbIIaSojVyxEA : IEnumerable<ActionElementMap>, IEnumerable, IEnumerator<ActionElementMap>, IEnumerator, IDisposable
		{
			private int hJZyoZcBUzhLIPFuIVXROzqVqTKT;

			private ActionElementMap gUMATaxCjIkaMXaiEejHTcJTOJP;

			private int gzZfIWDWazWiFktnBHeiebUkfLsXb;

			public ControllerMap_Editor cKPoSPrxZrjCeKEVGXKkLxbAhzhH;

			private int WIqsdvqtkbpliMeAzHuxAXlcLauvA;
			...
			ActionElementMap IEnumerator<ActionElementMap>.Current
			{
				...
				get
				{
					return gUMATaxCjIkaMXaiEejHTcJTOJP;
				}
			}
			...
			[DebuggerHidden]
			public BvZZIfUcrFGPKmjrRbIIaSojVyxEA(int P_0)
			{
				hJZyoZcBUzhLIPFuIVXROzqVqTKT = P_0;
				gzZfIWDWazWiFktnBHeiebUkfLsXb = Environment.CurrentManagedThreadId;
			}
			...
			{
				int num = hJZyoZcBUzhLIPFuIVXROzqVqTKT;
				ControllerMap_Editor controllerMap_Editor = cKPoSPrxZrjCeKEVGXKkLxbAhzhH;
				switch (num)
				{
				default:
					return false;
				case 0:
					hJZyoZcBUzhLIPFuIVXROzqVqTKT = -1;
					if (controllerMap_Editor.actionElementMaps == null)
					{
						return false;
					}
					WIqsdvqtkbpliMeAzHuxAXlcLauvA = 0;
					break;
				case 1:
					hJZyoZcBUzhLIPFuIVXROzqVqTKT = -1;
					WIqsdvqtkbpliMeAzHuxAXlcLauvA++;
					break;
				}
				if (WIqsdvqtkbpliMeAzHuxAXlcLauvA < controllerMap_Editor.actionElementMaps.Count)
				{
					gUMATaxCjIkaMXaiEejHTcJTOJP = controllerMap_Editor.actionElementMaps[WIqsdvqtkbpliMeAzHuxAXlcLauvA];
					hJZyoZcBUzhLIPFuIVXROzqVqTKT = 1;
					return true;
				}
				return false;
			...
			{
				BvZZIfUcrFGPKmjrRbIIaSojVyxEA bvZZIfUcrFGPKmjrRbIIaSojVyxEA;
				if (hJZyoZcBUzhLIPFuIVXROzqVqTKT == -2 && gzZfIWDWazWiFktnBHeiebUkfLsXb == Environment.CurrentManagedThreadId)
				{
					hJZyoZcBUzhLIPFuIVXROzqVqTKT = 0;
					bvZZIfUcrFGPKmjrRbIIaSojVyxEA = this;
				}
				else
				{
					bvZZIfUcrFGPKmjrRbIIaSojVyxEA = new BvZZIfUcrFGPKmjrRbIIaSojVyxEA(0);
					bvZZIfUcrFGPKmjrRbIIaSojVyxEA.cKPoSPrxZrjCeKEVGXKkLxbAhzhH = cKPoSPrxZrjCeKEVGXKkLxbAhzhH;
				}
				return bvZZIfUcrFGPKmjrRbIIaSojVyxEA;
			}
		...
		{
			[IteratorStateMachine(typeof(BvZZIfUcrFGPKmjrRbIIaSojVyxEA))]
			get
			{
				return new BvZZIfUcrFGPKmjrRbIIaSojVyxEA(-2)
				{
					cKPoSPrxZrjCeKEVGXKkLxbAhzhH = this
				};
			}
		}
		...
		internal JoystickMap GzWVrEYsmRkfzhyPqGfWHqUUaGBW(Func<int, bool> P_0, HardwareControllerMapIdentifier P_1, HardwareJoystickMap P_2, bool P_3)
		{
			JoystickMap joystickMap = new JoystickMap();
			mFIsuIRMqXpKOsyUoHeJMAZLryhv(P_0, joystickMap, P_1, P_2, P_3);
			return joystickMap;
		}

		internal KeyboardMap DtJplVINApQfcgyaPlrBoiCGqZZA(Func<int, bool> P_0)
		{
			KeyboardMap keyboardMap = new KeyboardMap();
			mFIsuIRMqXpKOsyUoHeJMAZLryhv(P_0, keyboardMap, default(HardwareControllerMapIdentifier), null, false);
			return keyboardMap;
		}

		internal MouseMap XfWiykqIpULysKOtRAuxVNnIQEgV(Func<int, bool> P_0)
		{
			MouseMap mouseMap = new MouseMap();
			mFIsuIRMqXpKOsyUoHeJMAZLryhv(P_0, mouseMap, default(HardwareControllerMapIdentifier), null, false);
			return mouseMap;
		}

		internal CustomControllerMap LLDeDgrsVgGTzsaDrDdiAirNEhKsA(Func<int, bool> P_0, CustomController_Editor P_1)
		{
			CustomControllerMap customControllerMap = new CustomControllerMap();
			FbTYLdvHzoZQfhUoJzuZZVvdbBQB(P_0, InputSource.Custom, customControllerMap, P_1);
			return customControllerMap;
		}

		internal ControllerTemplateMap YgfcEwhlFhKEWeigjOIlJgMOoJAlA()
		{
			if (!(ReInput.BTeBpTCtussbwcDUqvfkWderjnElA(hardwareGuid) is IHardwareControllerTemplateMap_Internal))
			{
				return null;
			}
			...
			for (int i = 0; i < num; i++)
			{
				...
				{
					ControllerTemplateActionElementMap controllerTemplateActionElementMap = ControllerTemplateActionElementMap.oOuyqdjomrtOyUekJNAQJuVucJtL(actionElementMap);
					if (controllerTemplateActionElementMap != null)
					{
						controllerTemplateMap.tzpINcgyDkSIVzVceGHiYToegKYM(controllerTemplateActionElementMap);
					}
				}
			}
		...
		private void mFIsuIRMqXpKOsyUoHeJMAZLryhv(Func<int, bool> P_0, ControllerMap P_1, HardwareControllerMapIdentifier P_2, HardwareJoystickMap P_3, bool P_4)
		{
			P_1.sourceMapId = id;
			...
			for (int i = 0; i < actionElementMaps.Count; i++)
			{
				...
				P_1.LtPlOjVYNTfYZlErEqgWUUqCmfNC(actionElementMap);
			}
		}

		private void FbTYLdvHzoZQfhUoJzuZZVvdbBQB(Func<int, bool> P_0, InputSource P_1, CustomControllerMap P_2, CustomController_Editor P_3)
		{
			P_2.sourceMapId = id;
			...
			for (int i = 0; i < actionElementMaps.Count; i++)
			{
				...
				P_2.LtPlOjVYNTfYZlErEqgWUUqCmfNC(actionElementMap);
			}
		}
		...
		public void CreateElementsFromHardwareMap(IHardwareControllerMap hardwareJoystickMap)
		{
			...
			foreach (IControllerElementIdentifierCommon_Internal elementIdentifier in (hardwareJoystickMap as IHardwareControllerMap_Internal).ElementIdentifiers)
			{
				...
				{
					ActionElementMap item = new ActionElementMap(-1, tqmHLUqTfYnnflPJaWxRPIPYjlrx.kIdanrEQgoqhtYbKMrksbOXKKFid(elementIdentifier.elementType), elementIdentifier.id);
		...
		{
			actionElementMaps.Add(hZGKQsCBpNhMqgzvHBRveRItGaMAb());
		}
		...
		public void InsertActionElementMap(int index)
		{
			...
			actionElementMaps.Insert(index, hZGKQsCBpNhMqgzvHBRveRItGaMAb());
		}
		...
		private ActionElementMap hZGKQsCBpNhMqgzvHBRveRItGaMAb()
		{
