...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_LogitechGRacingWheelPedals : CustomCalculation
	{
		...
		[NonSerialized]
		private Mode BXQEVsueKYyAeoHYNokAsLLAFeex;

		TypeWrapper.DataType SerializedMethod.ResultType => TypeWrapper.DataType.Single;

		internal bool MpqGKjsjuiFKXKJSFboTEmxYnTnFA()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				_result = ljxZVSVvmjfDTptmxGAHhTjZddwM();
				flag = true;
			}
			ClearData();
			_resultIsValid = flag;
		...
		private float ljxZVSVvmjfDTptmxGAHhTjZddwM()
		{
			if (base.DataCount < 2)
			{
				return 0f;
			...
			float num = _data[0];
			float num2 = _data[1];
			fkvTobSvRbTkRTORabtSrVlCgbsEA(num, num2);
			if (BXQEVsueKYyAeoHYNokAsLLAFeex == Mode.SharedAxis)
			{
				float oldValue = num2;
			...
			else if (BXQEVsueKYyAeoHYNokAsLLAFeex == Mode.SeparateAxes)
			{
				result = num;
			}
			return result;
		...
		private void fkvTobSvRbTkRTORabtSrVlCgbsEA(float P_0, float P_1)
		{
			switch (BXQEVsueKYyAeoHYNokAsLLAFeex)
			{
			case Mode.SharedAxis:
				if (MathTools.Abs(P_0) >= 0.01f && MathTools.Abs(P_1) <= 0.01f)
				{
					BXQEVsueKYyAeoHYNokAsLLAFeex = Mode.SeparateAxes;
				}
				break;
			case Mode.SeparateAxes:
				if (MathTools.Abs(P_1) >= 0.01f && MathTools.Abs(P_0) <= 0.01f)
				{
					BXQEVsueKYyAeoHYNokAsLLAFeex = Mode.SharedAxis;
				}
				break;
			}
		}
