...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool xnrZaTqHemDgxOdEdfmTILcvujBk()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = YdkhOwDXFNImfsStUPisHoScXDeIA();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = gRzwsoutVayDLJFFfQfLYbAJBuup();
					flag = true;
					break;
				}
			}
		...
		private float YdkhOwDXFNImfsStUPisHoScXDeIA()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => FNCQpCyKsCpOFZbMfHgoGASFcGziA(num2), 
				_ => 0f, 
			};
		}

		private float gRzwsoutVayDLJFFfQfLYbAJBuup()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float FNCQpCyKsCpOFZbMfHgoGASFcGziA(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
