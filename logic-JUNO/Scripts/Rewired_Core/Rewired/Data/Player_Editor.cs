...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal TsZwNfhMqvrruitjqbQpRTvZnhSS iYrdTKqsRxwTZSIIVBNIyydqOByt()
			{
				return new TsZwNfhMqvrruitjqbQpRTvZnhSS(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.BDYFZVSWwKErjELSidgTrNxCIGTnA NsidkifnfsXPBZrmHyEyMyukWrvA()
			{
				return new ControllerMapLayoutManager.BDYFZVSWwKErjELSidgTrNxCIGTnA(_enabled, _loadFromUserDataStore, hCEYbqjKHLGuKFTDrqkGwjIhfnlwA());
			}

			private OsbedCVLkTpeBrpSrCSZofCNmUcA[] hCEYbqjKHLGuKFTDrqkGwjIhfnlwA()
			{
				List<OsbedCVLkTpeBrpSrCSZofCNmUcA> list = new List<OsbedCVLkTpeBrpSrCSZofCNmUcA>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].quPZtKlkIXjGyjMMAbFXBBqCHSbi());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.PmwXpNvOYcWhwXXBRFyufZxzkmZh HnLlaVUFhvOWTOLOTpkNrZZPVVHf()
			{
				return new ControllerMapEnabler.PmwXpNvOYcWhwXXBRFyufZxzkmZh(_enabled, mklMydqFcsezlbkdbpuVsMFguAIz());
			}

			private OsbedCVLkTpeBrpSrCSZofCNmUcA[] mklMydqFcsezlbkdbpuVsMFguAIz()
			{
				List<OsbedCVLkTpeBrpSrCSZofCNmUcA> list = new List<OsbedCVLkTpeBrpSrCSZofCNmUcA>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal OsbedCVLkTpeBrpSrCSZofCNmUcA quPZtKlkIXjGyjMMAbFXBBqCHSbi()
			{
				return new OsbedCVLkTpeBrpSrCSZofCNmUcA(_id, _enabled);
			}
		...
		internal NavnfkqhNDpXrjtRFqpTiDksPJbU vxJEvCHACqGinMnRADBivjGCuCWgA()
		{
			TsZwNfhMqvrruitjqbQpRTvZnhSS[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new TsZwNfhMqvrruitjqbQpRTvZnhSS[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].iYrdTKqsRxwTZSIIVBNIyydqOByt();
				}
			}
			TsZwNfhMqvrruitjqbQpRTvZnhSS[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new TsZwNfhMqvrruitjqbQpRTvZnhSS[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].iYrdTKqsRxwTZSIIVBNIyydqOByt();
				}
			}
			TsZwNfhMqvrruitjqbQpRTvZnhSS[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new TsZwNfhMqvrruitjqbQpRTvZnhSS[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].iYrdTKqsRxwTZSIIVBNIyydqOByt();
				}
			}
			TsZwNfhMqvrruitjqbQpRTvZnhSS[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new TsZwNfhMqvrruitjqbQpRTvZnhSS[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].iYrdTKqsRxwTZSIIVBNIyydqOByt();
				}
			}
			return new NavnfkqhNDpXrjtRFqpTiDksPJbU(array, array2, array3, array4);
		}
	}
}
