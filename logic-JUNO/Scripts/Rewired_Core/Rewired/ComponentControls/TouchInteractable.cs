...
namespace Rewired.ComponentControls
{
	...
	public abstract class TouchInteractable : TouchControl, IPointerDownHandler, IEventSystemHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		{
			private TouchInteractable FUACHRojyPgOMhAJQzhHnVAOSNes;

			private InteractionState YaVfpCUSwJGIEWzimuzyISPXAqFq;

			private float VTiEneEtyBxXvEaZHDmdYEElcHLGb;

			public TouchInteractable sender => FUACHRojyPgOMhAJQzhHnVAOSNes;

			public InteractionState state => YaVfpCUSwJGIEWzimuzyISPXAqFq;

			public float duration => VTiEneEtyBxXvEaZHDmdYEElcHLGb;

			internal InteractionStateTransitionArgs()
			{
			}

			internal void sLvwQwgipCWnhfIfvGLiHPTgXutSA(TouchInteractable P_0, InteractionState P_1, float P_2)
			{
				FUACHRojyPgOMhAJQzhHnVAOSNes = P_0;
				YaVfpCUSwJGIEWzimuzyISPXAqFq = P_1;
				VTiEneEtyBxXvEaZHDmdYEElcHLGb = P_2;
			}
		}
		...
		[Serializable]
		private sealed class RpRgkwJnxlVGrpRcCeeAgjiiQzQqA
		{
			public static readonly RpRgkwJnxlVGrpRcCeeAgjiiQzQqA _003C_003E9 = new RpRgkwJnxlVGrpRcCeeAgjiiQzQqA();

			public static QpJRFmwRLCvgydNYJyjjhpGiILCeA.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> _003C_003E9__152_0;

			internal void lsalOcGbYwxPiGaEsOZrMeYVzAYd(IInteractionStateTransitionHandler P_0, InteractionStateTransitionArgs P_1)
			{
		...
		[NonSerialized]
		private bool pqOcehBHYZjvYUybOEUDZaSPCHpi;

		[NonSerialized]
		private bool piSVouLHcxsHucpvasjuvFawedujA;
		...
		private QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IVisibilityChangedHandler, bool> __hierarchyVisibilityChangedHandlers;

		private QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __hierarchyInteractionStateTransitionHandlers;

		private static QpJRFmwRLCvgydNYJyjjhpGiILCeA.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __interactionStateTransitionHandlerDelegate;

		private QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IVisibilityChangedHandler, bool> QxkLlMTkzZvopJjAuANqCIFPPBNYA
		{
			get
			{
				if (__hierarchyVisibilityChangedHandlers == null)
				{
					__hierarchyVisibilityChangedHandlers = new QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IVisibilityChangedHandler, bool>(eqUemAOpGVJoEHkHsfAVaStNXwAE.JKrUkhxyKlUxVUnUpncKRSSxinWy);
		...
		private QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> UHcHvNNNraBWxrTQCDYRTVbjGktE
		{
			get
			{
				if (__hierarchyInteractionStateTransitionHandlers == null)
				{
					__hierarchyInteractionStateTransitionHandlers = new QpJRFmwRLCvgydNYJyjjhpGiILCeA.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs>(eYhcmcdQWauriktWHjTvojnvrSCTA);
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					eNhGgqKNpCPtfbwgUYbCNXDFwCBx(value, false);
					GsBcXKiqrkqzLgoLvZRKojGmaLbaA();
				}
			}
		}
		...
		internal static QpJRFmwRLCvgydNYJyjjhpGiILCeA.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> eYhcmcdQWauriktWHjTvojnvrSCTA
		{
			get
			{
				if (__interactionStateTransitionHandlerDelegate == null)
				{
					__interactionStateTransitionHandlerDelegate = RpRgkwJnxlVGrpRcCeeAgjiiQzQqA._003C_003E9.lsalOcGbYwxPiGaEsOZrMeYVzAYd;
				}
		...
		internal override void OnEnable()
		{
			...
			xPoSEdSARnzRVPqkMtZdGENoKnWB(InteractionState.Normal);
			EspNEXXBpxrGVQzgorhYQnuylKtV(true);
		}

		[CustomObfuscation(rename = false)]
		internal override void OnDisable()
		{
			CNsUpzvpRmRVlQajNAVweEsNAupHA();
			base.OnDisable();
		}
		...
		internal override void OnValidate()
		{
			...
			if (PBRHZQINZfANWEOTugUlepRFdGfJ())
			{
				...
				TUAJlwacUWqqQgfwLZSOxBAdPKrH(null);
				QKVUpXSmVHkeowMhQhafAXBlkEOJ(Color.white, true);
				zDOGqzkpdiVTVEMwNmvMOfBmxpMY(_transitionAnimationTriggers.normalTrigger);
				EspNEXXBpxrGVQzgorhYQnuylKtV(true);
			}
			dStVUZtnTzDeBmXKEdbYfvQGIvVd();
			xvkoGyufkZHDtsLdDYxGvUZrcdfp();
		}
		...
		internal virtual void MXkwbfQjWFfLiAPyzgGSCyXlFhQW()
		{
			ssEsPiIOPSzIdOITKeljkvIlaxNt();
			xvkoGyufkZHDtsLdDYxGvUZrcdfp();
		}

		internal virtual void VtgqxjDyMIHFNHvBgBmunIksXrlp()
		{
			base.nXumCRMnDCjEenDugoLTYqsFcIGd();
			dStVUZtnTzDeBmXKEdbYfvQGIvVd();
		}

		private void CNsUpzvpRmRVlQajNAVweEsNAupHA()
		{
			string normalTrigger = _transitionAnimationTriggers.normalTrigger;
			pqOcehBHYZjvYUybOEUDZaSPCHpi = false;
			piSVouLHcxsHucpvasjuvFawedujA = false;
			...
			{
				zDOGqzkpdiVTVEMwNmvMOfBmxpMY(normalTrigger);
			}
		}

		private void DkTFgsKfuhxSPQghtcpbJOReaBZwb(InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					QKVUpXSmVHkeowMhQhafAXBlkEOJ(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					QKVUpXSmVHkeowMhQhafAXBlkEOJ(color, P_1);
				}
				if ((_transitionType & TransitionTypeFlags.SpriteSwap) != TransitionTypeFlags.None)
				{
					TUAJlwacUWqqQgfwLZSOxBAdPKrH(sprite);
				}
				if ((_transitionType & TransitionTypeFlags.Animation) != TransitionTypeFlags.None)
				{
					zDOGqzkpdiVTVEMwNmvMOfBmxpMY(text);
				}
			}
			if (_allowSendingEvents)
			{
				_transitionArgs.sLvwQwgipCWnhfIfvGLiHPTgXutSA(this, P_0, P_1 ? 0f : _transitionColorTint.fadeDuration);
				UHcHvNNNraBWxrTQCDYRTVbjGktE.ExecuteOnAll(_transitionArgs);
		...
		private void QKVUpXSmVHkeowMhQhafAXBlkEOJ(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void TUAJlwacUWqqQgfwLZSOxBAdPKrH(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void zDOGqzkpdiVTVEMwNmvMOfBmxpMY(string P_0)
		{
		...
		private void EspNEXXBpxrGVQzgorhYQnuylKtV(bool P_0)
		{
			...
			DkTFgsKfuhxSPQghtcpbJOReaBZwb(interactionState, P_0);
		}
		...
		internal virtual bool HhNSOpsjnoIyFxEaGnfIommsLJJc()
		{
			if (!PBRHZQINZfANWEOTugUlepRFdGfJ())
			{
				return false;
			}
			if (pqOcehBHYZjvYUybOEUDZaSPCHpi)
			{
				return piSVouLHcxsHucpvasjuvFawedujA;
			}
			return false;
		}

		internal void mqqgsskRuzXjTAiWvEdJawWAQYEob(BaseEventData P_0)
		{
			if (PBRHZQINZfANWEOTugUlepRFdGfJ() && IsInteractable())
			{
				InteractionState interactionState = CFqssQzKkbISnZtQGAyodtMYIuvT(P_0);
				if (interactionState != _interactionState)
				{
					xPoSEdSARnzRVPqkMtZdGENoKnWB(interactionState);
					EspNEXXBpxrGVQzgorhYQnuylKtV(false);
				}
			}
		}

		internal virtual bool plvFFPdzUNOHagtCgfoabMDAVjXWB(GameObject P_0)
		{
			return base.gameObject == P_0;
		}

		private bool oGbKcIlNjZHHphTaevaPxKPakMhc(BaseEventData P_0)
		{
			bool flag = P_0 is PointerEventData;
			return SxGrHVQpwFgaqPDDQSdAozMFoBKt(flag, flag ? (P_0 as PointerEventData).pointerPress : null);
		}

		private bool SxGrHVQpwFgaqPDDQSdAozMFoBKt(bool P_0, GameObject P_1)
		{
			if (!PBRHZQINZfANWEOTugUlepRFdGfJ())
			{
				return false;
			}
			if (HhNSOpsjnoIyFxEaGnfIommsLJJc())
			{
				return false;
			}
			bool flag = false;
			if (P_0)
			{
				return flag | ((piSVouLHcxsHucpvasjuvFawedujA && !pqOcehBHYZjvYUybOEUDZaSPCHpi && plvFFPdzUNOHagtCgfoabMDAVjXWB(P_1)) || (!piSVouLHcxsHucpvasjuvFawedujA && pqOcehBHYZjvYUybOEUDZaSPCHpi && plvFFPdzUNOHagtCgfoabMDAVjXWB(P_1)) || (!piSVouLHcxsHucpvasjuvFawedujA && pqOcehBHYZjvYUybOEUDZaSPCHpi && P_1 == null));
			}
			return flag | pqOcehBHYZjvYUybOEUDZaSPCHpi;
		}

		private InteractionState CFqssQzKkbISnZtQGAyodtMYIuvT(BaseEventData P_0)
		{
			if (HhNSOpsjnoIyFxEaGnfIommsLJJc())
			{
				return InteractionState.Pressed;
			}
			if (oGbKcIlNjZHHphTaevaPxKPakMhc(P_0))
			{
		...
		private bool xPoSEdSARnzRVPqkMtZdGENoKnWB(InteractionState P_0)
		{
			...
			PPqeVXazECzNBMbrwcVEoNibPLvp();
			return true;
		}

		private void PPqeVXazECzNBMbrwcVEoNibPLvp()
		{
			fprvUWldQRTiXiODnkZNNSYXIoSb();
		}

		private void fprvUWldQRTiXiODnkZNNSYXIoSb()
		{
			if (Application.isPlaying && _hideWhenIdle)
			{
				eNhGgqKNpCPtfbwgUYbCNXDFwCBx(_interactionState == InteractionState.Pressed, false);
			}
		}

		private void eNhGgqKNpCPtfbwgUYbCNXDFwCBx(bool P_0, bool P_1)
		{
			if (_visible == P_0 && !P_1)
			{
				return;
			...
			if (_allowSendingEvents)
			{
				QxkLlMTkzZvopJjAuANqCIFPPBNYA.ExecuteOnAll(P_0);
		...
		private void moGcyDhrgnoMcJRPTCxubcyicOYZb()
		{
			...
			{
				eNhGgqKNpCPtfbwgUYbCNXDFwCBx(_visible, true);
				fprvUWldQRTiXiODnkZNNSYXIoSb();
			}
			dStVUZtnTzDeBmXKEdbYfvQGIvVd();
			if (_allowSendingEvents)
			{
				QxkLlMTkzZvopJjAuANqCIFPPBNYA.ExecuteOnAll(_visible);
				if (_onVisibilityChanged != null)
				{
		...
		private void YVEWPWHBEZHlyicjAiYnkgTGtbPv()
		{
		...
		private void xvkoGyufkZHDtsLdDYxGvUZrcdfp()
		{
			YVEWPWHBEZHlyicjAiYnkgTGtbPv();
			fprvUWldQRTiXiODnkZNNSYXIoSb();
			if (!Application.isPlaying)
			{
				EspNEXXBpxrGVQzgorhYQnuylKtV(true);
		...
		private void dStVUZtnTzDeBmXKEdbYfvQGIvVd()
		{
			QxkLlMTkzZvopJjAuANqCIFPPBNYA.GetHandlers(base.transform);
			UHcHvNNNraBWxrTQCDYRTVbjGktE.GetHandlers(base.transform);
		}

		internal virtual void OnPointerDown(PointerEventData eventData)
		{
			if (JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerDown))
			{
				piSVouLHcxsHucpvasjuvFawedujA = true;
				mqqgsskRuzXjTAiWvEdJawWAQYEob(eventData);
			}
		}

		internal virtual void OnPointerUp(PointerEventData eventData)
		{
			if (JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerUp))
			{
				piSVouLHcxsHucpvasjuvFawedujA = false;
				mqqgsskRuzXjTAiWvEdJawWAQYEob(eventData);
			}
		...
		{
			if (JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerEnter))
			{
				pqOcehBHYZjvYUybOEUDZaSPCHpi = true;
				mqqgsskRuzXjTAiWvEdJawWAQYEob(eventData);
			}
		}
		...
		{
			if (JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerExit))
			{
				pqOcehBHYZjvYUybOEUDZaSPCHpi = false;
				mqqgsskRuzXjTAiWvEdJawWAQYEob(eventData);
			}
		...
		{
			JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.BeginDrag);
		}

		internal virtual void OnDrag(PointerEventData eventData)
		{
			JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.Drag);
		}

		internal virtual void OnEndDrag(PointerEventData eventData)
		{
			JkDrilijeoDrZkYsJxQuUPBCKtDZ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.EndDrag);
		}
		...
		internal static bool HSdHFEwTqLmljSAXMrqWISAaJUgd(int P_0)
		{
			if (P_0 == int.MinValue)
			{
				return false;
			}
			if (GIygVOeNjVaVjWDNqzboLLzUBflBA(P_0))
			{
				int num = QzpowUtAmadMfeHTOltJcdqWszLh(P_0);
				if (num < 0)
				{
					return false;
				}
			...
			if (vYPGbuTavXEIDJeeAuBdpuxoOPlc(P_0) && Input.mousePresent)
			{
				int num2 = ZnamfhBwbmTObgczRLMcdIWZKEIp(P_0);
				if (num2 >= 0)
				{
		...
		internal static Vector3 GRqXcYGMGEOcqtDjGIZnCAiUOlaGA(int P_0)
		{
			if (GIygVOeNjVaVjWDNqzboLLzUBflBA(P_0))
			{
				int num = QzpowUtAmadMfeHTOltJcdqWszLh(P_0);
			...
			else if (vYPGbuTavXEIDJeeAuBdpuxoOPlc(P_0) && Input.mousePresent)
			{
		...
		internal static bool GIygVOeNjVaVjWDNqzboLLzUBflBA(int P_0)
		{
			return P_0 >= 0;
		}

		internal static bool vYPGbuTavXEIDJeeAuBdpuxoOPlc(int P_0)
		{
			if (P_0 != -1 && P_0 != -3)
			{
				return P_0 == -2;
		...
		private static int QzpowUtAmadMfeHTOltJcdqWszLh(int P_0)
		{
			if (!GIygVOeNjVaVjWDNqzboLLzUBflBA(P_0))
			{
				return -1;
		...
		internal static bool XjBchoAWvwLziClWTOdRPdBDPZLwA(MouseButtonFlags P_0, int P_1)
		{
			if (vYPGbuTavXEIDJeeAuBdpuxoOPlc(P_1))
			{
				if (!Cursor.visible)
				{
					return false;
			...
			if (GIygVOeNjVaVjWDNqzboLLzUBflBA(P_1))
			{
				return true;
			}
			if (tQdAIqmJzfcvtArYBtiVRtTJYpvGA(P_0, P_1))
			{
				return true;
			}
			return false;
		...
		private static bool tQdAIqmJzfcvtArYBtiVRtTJYpvGA(MouseButtonFlags P_0, int P_1)
		{
			return P_1 switch
			{
		...
		private static int ZnamfhBwbmTObgczRLMcdIWZKEIp(int P_0)
		{
			if (!vYPGbuTavXEIDJeeAuBdpuxoOPlc(P_0))
			{
				return -1;
			}
			return P_0 switch
		...
		internal static bool ilvBBLoOJPAulAFbbfMMVlRblJpFb(MouseButtonFlags P_0, out int P_1)
		{
			for (int i = 0; i < 3; i++)
			{
		...
		internal static bool JkDrilijeoDrZkYsJxQuUPBCKtDZ(int P_0, MouseButtonFlags P_1, EventTriggerType P_2)
		{
			if (vYPGbuTavXEIDJeeAuBdpuxoOPlc(P_0) && (P_2 == EventTriggerType.PointerEnter || P_2 == EventTriggerType.PointerExit) && P_1 != MouseButtonFlags.None)
			{
				P_1 |= MouseButtonFlags.LeftButton;
			}
			return XjBchoAWvwLziClWTOdRPdBDPZLwA(P_1, P_0);
		}

		internal static bool tblDrhtvhdTFJgWkleujTILlbVxu(MouseButtonFlags P_0)
		{
			int num;
			return ilvBBLoOJPAulAFbbfMMVlRblJpFb(P_0, out num);
		}

		[CompilerGenerated]
		private void IzidznDRVslPCxmkxlAQDpZdwUhN(bool P_0)
		{
			_allowSendingEvents = P_0;
		}
	}
