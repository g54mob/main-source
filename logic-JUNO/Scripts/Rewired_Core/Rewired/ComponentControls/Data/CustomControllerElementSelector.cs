...
namespace Rewired.ComponentControls.Data
{
	...
	public sealed class CustomControllerElementSelector
	{
		...
		[HideInInspector]
		private int lvZncPFGPscBsfNUnNzCWUxsfpzRA = -1;

		[HideInInspector]
		private int PilnuKEfjBfpIhxzzeVuiYAwnJfL = -1;
		...
		public int GetElementIndex(Rewired.CustomController customController)
		{
			...
			if (lvZncPFGPscBsfNUnNzCWUxsfpzRA >= 0 && lvZncPFGPscBsfNUnNzCWUxsfpzRA != customController.id)
			{
				ClearCache();
			}
			if (PilnuKEfjBfpIhxzzeVuiYAwnJfL >= 0)
			{
				return PilnuKEfjBfpIhxzzeVuiYAwnJfL;
			}
			lvZncPFGPscBsfNUnNzCWUxsfpzRA = customController.id;
			switch (_selectorType)
			{
			case SelectorType.Id:
			{
				IList<ControllerElementIdentifier> list = XqFoZwbnihIlKDHgsklMZMctadVX(customController, _elementType);
				for (int j = 0; j < list.Count; j++)
				{
					if (list[j].Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Eid == _elementId)
					{
						PilnuKEfjBfpIhxzzeVuiYAwnJfL = j;
						break;
					}
				}
				break;
			...
			case SelectorType.Index:
			{
				...
				PilnuKEfjBfpIhxzzeVuiYAwnJfL = _elementIndex;
				break;
			}
			case SelectorType.Name:
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					{
						PilnuKEfjBfpIhxzzeVuiYAwnJfL = i;
						break;
					}
				}
				break;
		...
		private IList<ControllerElementIdentifier> XqFoZwbnihIlKDHgsklMZMctadVX(Rewired.CustomController P_0, ElementType P_1)
		{
			return P_1 switch
			{
		...
		{
			lvZncPFGPscBsfNUnNzCWUxsfpzRA = -1;
			PilnuKEfjBfpIhxzzeVuiYAwnJfL = -1;
		}
	}
}
