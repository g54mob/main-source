...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchInteractableTransitioner : MonoBehaviour, IVisibilityChangedHandler, TouchInteractable.IInteractionStateTransitionHandler
	{
		...
		private TouchInteractable.InteractionState SERgtFvetgeIcsWwWLRmGlEPFOBe;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					rZabwXcwGnUXySfIarwSuStJrcDnA(value, false);
					pknZVOQFVRbUgJSlZgBfCDhvRXex();
				}
			}
		}
		...
		private void Awake()
		{
			if (Application.isPlaying)
			{
				...
				rZabwXcwGnUXySfIarwSuStJrcDnA(_visible, true);
			}
		}
		...
		private void OnEnable()
		{
			...
			UhEOHdFbFKmsaoUCpENEfzkXsJQg(true);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			eKOlthTMTrURpNOmczwZDFapsTew();
		}
		...
		private void OnValidate()
		{
			...
			{
				KCEzFPhXNSDNTykNPlLFlgASxKpb(null);
				tKUBjuABBkanjyTGMMUjfkYGuDvg(Color.white, true);
				bnEhJWQHJTrfaMvkxIBWcyTZwHNd(_transitionAnimationTriggers.normalTrigger);
				UhEOHdFbFKmsaoUCpENEfzkXsJQg(true);
			}
			PxTSNFLpRkeYKxTUzJvrovbJxEPx();
		}
		...
		private void pknZVOQFVRbUgJSlZgBfCDhvRXex()
		{
			PxTSNFLpRkeYKxTUzJvrovbJxEPx();
		}

		private void PxTSNFLpRkeYKxTUzJvrovbJxEPx()
		{
			if (!Application.isPlaying)
			{
				UhEOHdFbFKmsaoUCpENEfzkXsJQg(true);
			...
			{
				UhEOHdFbFKmsaoUCpENEfzkXsJQg(false);
			}
		}

		private void UhEOHdFbFKmsaoUCpENEfzkXsJQg(bool P_0)
		{
			PvmxoDLPTbcSLeyYCquiuNDZsJSp(SERgtFvetgeIcsWwWLRmGlEPFOBe, P_0);
		}

		private void rZabwXcwGnUXySfIarwSuStJrcDnA(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
		...
		private bool jTquYJpFjCfbVcLLlpOwBVLwsHyR()
		{
			return UnityTools.IsActiveAndEnabled(this);
		}

		private void eKOlthTMTrURpNOmczwZDFapsTew()
		{
			...
			{
				bnEhJWQHJTrfaMvkxIBWcyTZwHNd(normalTrigger);
			}
		}

		private void PvmxoDLPTbcSLeyYCquiuNDZsJSp(TouchInteractable.InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					tKUBjuABBkanjyTGMMUjfkYGuDvg(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					tKUBjuABBkanjyTGMMUjfkYGuDvg(color, P_1);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.SpriteSwap) != TouchInteractable.TransitionTypeFlags.None)
				{
					KCEzFPhXNSDNTykNPlLFlgASxKpb(sprite);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.Animation) != TouchInteractable.TransitionTypeFlags.None)
				{
					bnEhJWQHJTrfaMvkxIBWcyTZwHNd(text);
				}
			}
		}

		private void tKUBjuABBkanjyTGMMUjfkYGuDvg(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void KCEzFPhXNSDNTykNPlLFlgASxKpb(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void bnEhJWQHJTrfaMvkxIBWcyTZwHNd(string P_0)
		{
		...
		{
			SERgtFvetgeIcsWwWLRmGlEPFOBe = args.state;
