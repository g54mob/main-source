...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchJoystickAngleIndicator : MonoBehaviour, IVisibilityChangedHandler, TouchJoystick.IStickPositionChangedHandler
	{
		...
		private Image tSIcUGoxtrCnDVtAfakGSitJffGD;

		private RectTransform idmUdRCUCgiSAXpVByauWuRUTLLF;

		private Vector2 gOVLFhLXqGoBByveCwnNfWPBRRpM;

		private bool FfbeNHbaNyDSIheUnxlCpbAQTwOK;

		private IRegistrar<TouchJoystickAngleIndicator> gicyNiwfFMqjBukokWcQPRNENdjA;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					SkmQaInJDRjIJNZzzLoEQAJjHNLfA(value, false);
					QQbNlFEHzEzLRaVwoJdRzrHFmJZs();
				}
			}
		}
		...
		internal Image BJDWtjcneXygkiJnhdHQBxKAsrBoA => tSIcUGoxtrCnDVtAfakGSitJffGD ?? (tSIcUGoxtrCnDVtAfakGSitJffGD = GetComponent<Image>());

		internal Sprite ZjxaTNWyrJjDJjTbLcmDgxULEoAH
		{
			get
			{
				if (BJDWtjcneXygkiJnhdHQBxKAsrBoA == null)
				{
					return null;
				}
				if (tSIcUGoxtrCnDVtAfakGSitJffGD.overrideSprite != null)
				{
					return tSIcUGoxtrCnDVtAfakGSitJffGD.overrideSprite;
				}
				return tSIcUGoxtrCnDVtAfakGSitJffGD.sprite;
			}
		}

		internal RectTransform hbDKswWFfSosPOoazjFxLlESoufs => idmUdRCUCgiSAXpVByauWuRUTLLF ?? (idmUdRCUCgiSAXpVByauWuRUTLLF = GetComponent<RectTransform>());
		...
		internal bool CvEklKMENPlsCpskMImaODhhVRZN(out Vector2 P_0)
		{
			P_0 = Vector2.zero;
			if (BJDWtjcneXygkiJnhdHQBxKAsrBoA == null)
			{
			...
			Sprite sprite = tSIcUGoxtrCnDVtAfakGSitJffGD.overrideSprite ?? tSIcUGoxtrCnDVtAfakGSitJffGD.sprite;
			if (sprite == null)
			{
				return false;
			}
		...
		private void OnEnable()
		{
			...
			{
				zHsWCqzVgDsSziUcguJopnPqVuho();
				TUzirOKrpcKYtlKHXdPYGWkywTLc();
			}
			TFBspOjFqKTIVAnHiDHmeqZQAMJbb(gOVLFhLXqGoBByveCwnNfWPBRRpM);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			uNPDAShXOmVXjnqWakSfzgbmggBtA();
		}

		[CustomObfuscation(rename = false)]
		private void OnValidate()
		{
			meIrqfHcPMpvIVlaKgkZForXjwRLA();
			TFBspOjFqKTIVAnHiDHmeqZQAMJbb(gOVLFhLXqGoBByveCwnNfWPBRRpM);
		}
		...
		private void SkmQaInJDRjIJNZzzLoEQAJjHNLfA(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
				if (!P_0)
				{
					...
					targetColor.a = 0f;
					BJDWtjcneXygkiJnhdHQBxKAsrBoA.CrossFadeColor(targetColor, 0f, ignoreTimeScale: true, useAlpha: true);
				}
				else
				{
					TFBspOjFqKTIVAnHiDHmeqZQAMJbb(gOVLFhLXqGoBByveCwnNfWPBRRpM);
		...
		private void TFBspOjFqKTIVAnHiDHmeqZQAMJbb(Vector2 P_0)
		{
			if (!_visible)
			{
				Color targetColor = _normalColor;
			...
			else if (!MathTools.ApproximatelyZero(P_0.sqrMagnitude))
			{
				...
				BJDWtjcneXygkiJnhdHQBxKAsrBoA.CrossFadeColor(targetColor2, 0f, ignoreTimeScale: true, useAlpha: true);
			}
			else
			{
				BJDWtjcneXygkiJnhdHQBxKAsrBoA.CrossFadeColor(_normalColor, 0f, ignoreTimeScale: true, useAlpha: true);
			}
		}

		private void zHsWCqzVgDsSziUcguJopnPqVuho()
		{
			FfbeNHbaNyDSIheUnxlCpbAQTwOK = _visible;
		}

		private void meIrqfHcPMpvIVlaKgkZForXjwRLA()
		{
			if (FfbeNHbaNyDSIheUnxlCpbAQTwOK != _visible)
			{
				FfbeNHbaNyDSIheUnxlCpbAQTwOK = _visible;
				SkmQaInJDRjIJNZzzLoEQAJjHNLfA(_visible, true);
			}
		}

		private void QQbNlFEHzEzLRaVwoJdRzrHFmJZs()
		{
		}

		private void TUzirOKrpcKYtlKHXdPYGWkywTLc()
		{
			uNPDAShXOmVXjnqWakSfzgbmggBtA();
			...
			if (!componentInSelfOrParents.IsNullOrDestroyed())
			{
				...
				gicyNiwfFMqjBukokWcQPRNENdjA = componentInSelfOrParents;
			}
		}

		private void uNPDAShXOmVXjnqWakSfzgbmggBtA()
		{
			if (gicyNiwfFMqjBukokWcQPRNENdjA.IsNullOrDestroyed())
			{
				if (gicyNiwfFMqjBukokWcQPRNENdjA != null)
				{
					gicyNiwfFMqjBukokWcQPRNENdjA = null;
				}
			}
			else
			{
				gicyNiwfFMqjBukokWcQPRNENdjA.Deregister(this);
				gicyNiwfFMqjBukokWcQPRNENdjA = null;
			}
		}
		...
		{
			SkmQaInJDRjIJNZzzLoEQAJjHNLfA(state, false);
		}
		...
		public void OnTouchJoystickStickPositionChanged(Vector2 value)
		{
			if (!(this == null))
			{
				gOVLFhLXqGoBByveCwnNfWPBRRpM = value;
				if (UnityTools.IsActiveAndEnabled(this) && _visible)
				{
					TFBspOjFqKTIVAnHiDHmeqZQAMJbb(value);
				}
			}
		}
