...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.FHvZVuWkzIDKCbDbOJtHxepOWYcj)
				{
		...
		internal class PmwXpNvOYcWhwXXBRFyufZxzkmZh
		{
			public bool YNuUalqJPgpPmwDAjMfwczIyEaoF;

			public OsbedCVLkTpeBrpSrCSZofCNmUcA[] mMSYgLUYAkPPtmoaPbYDvagqFEMhA;

			public PmwXpNvOYcWhwXXBRFyufZxzkmZh(bool P_0, OsbedCVLkTpeBrpSrCSZofCNmUcA[] P_1)
			{
				YNuUalqJPgpPmwDAjMfwczIyEaoF = P_0;
				mMSYgLUYAkPPtmoaPbYDvagqFEMhA = P_1;
			}
		}

		private bool FXaRevqLDQIChjZsNUjBaKnIOFWR;

		private Player dJXAgJbxzELDXISJQWDQQDcJFgCgA;

		private PmwXpNvOYcWhwXXBRFyufZxzkmZh ooBheYgHXkHoJdEvLcsIgpwurcUu;

		private readonly int tdgytEqHCrjVefQXAoNjArdKknON;

		private List<RuleSet> qsohYZiNNplvuXYqaaKfFRpoIvuF;

		public bool enabled
		{
			get
			{
				return FXaRevqLDQIChjZsNUjBaKnIOFWR;
			}
			set
			{
				FXaRevqLDQIChjZsNUjBaKnIOFWR = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return qsohYZiNNplvuXYqaaKfFRpoIvuF;
			}
			set
			{
				if (value == null)
				...
				qsohYZiNNplvuXYqaaKfFRpoIvuF = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, PmwXpNvOYcWhwXXBRFyufZxzkmZh P_1)
		{
			if (P_0 == null)
			{
			...
			tdgytEqHCrjVefQXAoNjArdKknON = ReInput.id;
			dJXAgJbxzELDXISJQWDQQDcJFgCgA = P_0;
			ooBheYgHXkHoJdEvLcsIgpwurcUu = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != tdgytEqHCrjVefQXAoNjArdKknON)
			{
				ReInput.CheckInitialized(tdgytEqHCrjVefQXAoNjArdKknON);
			}
			else
			{
				if (!FXaRevqLDQIChjZsNUjBaKnIOFWR || qsohYZiNNplvuXYqaaKfFRpoIvuF == null)
				{
					return;
				}
				int count = qsohYZiNNplvuXYqaaKfFRpoIvuF.Count;
				if (count == 0)
				{
					return;
				}
				...
				dJXAgJbxzELDXISJQWDQQDcJFgCgA.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = qsohYZiNNplvuXYqaaKfFRpoIvuF[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((ooBheYgHXkHoJdEvLcsIgpwurcUu != null && ooBheYgHXkHoJdEvLcsIgpwurcUu.mMSYgLUYAkPPtmoaPbYDvagqFEMhA != null) ? ooBheYgHXkHoJdEvLcsIgpwurcUu.mMSYgLUYAkPPtmoaPbYDvagqFEMhA.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(ooBheYgHXkHoJdEvLcsIgpwurcUu.mMSYgLUYAkPPtmoaPbYDvagqFEMhA[i].YcirogJSGffoXdrlqzSiCamDTVLD);
				...
				controllerMapEnablerRuleSetInstance.enabled = ooBheYgHXkHoJdEvLcsIgpwurcUu.mMSYgLUYAkPPtmoaPbYDvagqFEMhA[i].garTmoeVreesVgkcGInPfqjzFRyKA;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (ooBheYgHXkHoJdEvLcsIgpwurcUu != null)
			{
				FXaRevqLDQIChjZsNUjBaKnIOFWR = ooBheYgHXkHoJdEvLcsIgpwurcUu.YNuUalqJPgpPmwDAjMfwczIyEaoF;
			}
			qsohYZiNNplvuXYqaaKfFRpoIvuF = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return iyKMQzVXRYKWOIMUXmtmIkteYQNL().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return iyKMQzVXRYKWOIMUXmtmIkteYQNL().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				tLVXTMNhgVOPMYWXVloXJLoIVgFF(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				tLVXTMNhgVOPMYWXVloXJLoIVgFF(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject iyKMQzVXRYKWOIMUXmtmIkteYQNL()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			rESVlrwIDoJjKyuEVgImzGqrDxUy(serializedObject);
			return serializedObject;
		}

		private void rESVlrwIDoJjKyuEVgImzGqrDxUy(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", FXaRevqLDQIChjZsNUjBaKnIOFWR);
			P_0.Add("ruleSets", qsohYZiNNplvuXYqaaKfFRpoIvuF);
		}

		private bool tLVXTMNhgVOPMYWXVloXJLoIVgFF(SerializedObject P_0)
		{
			FXaRevqLDQIChjZsNUjBaKnIOFWR = false;
			qsohYZiNNplvuXYqaaKfFRpoIvuF = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref FXaRevqLDQIChjZsNUjBaKnIOFWR);
