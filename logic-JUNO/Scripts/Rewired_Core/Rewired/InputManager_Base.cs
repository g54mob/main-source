...
namespace Rewired
{
	...
	{
		private enum VyLcaPGXFiOoZsjxBarunYjtGBpx
		{
			Initialization = 0,
			Update = 1,
			Destroy = 2
		...
		[NonSerialized]
		private bool hKSnKDpreVtEdquhAzQkXdtYqfE;

		[NonSerialized]
		private bool RKACNdAQkPCkuIoxUmujDVDIHjRPA;
		...
		[NonSerialized]
		private bool LCjhVeknCofOigKSCKpjlHKPsIpVA;

		private bool arJyknMSyQbRoFciasEGBvXPTwUj;
		...
		{
			if ((Application.isPlaying || _userData.ConfigVars.runInEditMode) && (!Application.isPlaying || arJyknMSyQbRoFciasEGBvXPTwUj) && !hKSnKDpreVtEdquhAzQkXdtYqfE && !RKACNdAQkPCkuIoxUmujDVDIHjRPA)
			{
				rgpuibFIzocuWgqQQZCNfHosgwNDb();
			}
		}
		...
		{
			hKSnKDpreVtEdquhAzQkXdtYqfE = false;
			LCjhVeknCofOigKSCKpjlHKPsIpVA = false;
			RKACNdAQkPCkuIoxUmujDVDIHjRPA = false;
			try
			{
				if (ReInput.rewiredInputManager == this)
				{
					ReInput.iWImclvEMlWWWQxofRqsWhMbsaDV();
				}
			}
			catch (Exception ex)
			{
				vvJecSYVntxpSMLqVjRgHpoIcQTN(VyLcaPGXFiOoZsjxBarunYjtGBpx.Destroy, "destruction", ex);
			}
			OnDeinitialized();
		}
		...
		{
			if (!LCjhVeknCofOigKSCKpjlHKPsIpVA)
			{
				ReInput.nQGCFKCWNRLeGRKXhkSBzVUMTzDO(isFocused);
				_ = hKSnKDpreVtEdquhAzQkXdtYqfE;
			}
		}
		...
		private void Start()
		{
			if ((!UnityTools.isEditor || Application.isPlaying) && hKSnKDpreVtEdquhAzQkXdtYqfE && !RKACNdAQkPCkuIoxUmujDVDIHjRPA)
			{
				ReInput.KSlfZqOVAGbruWTomRPpgxLnoUDK();
			}
		}
		...
		private void Update()
		{
			if (hKSnKDpreVtEdquhAzQkXdtYqfE && !RKACNdAQkPCkuIoxUmujDVDIHjRPA && (!UnityTools.isEditor || Application.isPlaying))
			{
		...
		private void LateUpdate()
		{
			...
			try
			{
				ReInput.YftsaQZvtAaarSTOqPOXOCAGhCLR();
			}
			catch (Exception ex)
			{
				vvJecSYVntxpSMLqVjRgHpoIcQTN(VyLcaPGXFiOoZsjxBarunYjtGBpx.Update, "update (Late Update)", ex);
			}
		}
		...
		{
			if (!hKSnKDpreVtEdquhAzQkXdtYqfE || RKACNdAQkPCkuIoxUmujDVDIHjRPA)
			{
				return;
			}
			try
			{
				CheckRecompile();
				ReInput.iMRCRyQFgsiKlIkoaQrqpcvPruXfA(updateLoopType);
				if ((_userData.ConfigVars.updateLoop & updateLoopSettingBit) != UpdateLoopSetting.None)
				{
					ReInput.glqgsacLzcDFPARbIPppzVMSjvkG(updateLoopType);
				}
			}
			catch (Exception ex)
			{
				vvJecSYVntxpSMLqVjRgHpoIcQTN(VyLcaPGXFiOoZsjxBarunYjtGBpx.Update, "update (" + updateLoopType.ToString() + ")", ex);
			}
		}

		internal void TryStartRunInEditMode()
		{
			if (hKSnKDpreVtEdquhAzQkXdtYqfE || !Application.isEditor || Application.isPlaying)
			{
				return;
			}
			if (ReInput.isReady)
		...
		private bool rgpuibFIzocuWgqQQZCNfHosgwNDb()
		{
			if (hKSnKDpreVtEdquhAzQkXdtYqfE)
			{
				return true;
			}
			QLLSTwXHwotOIIZcLmQyyvcmWdtK();
			...
			return hKSnKDpreVtEdquhAzQkXdtYqfE;
		}

		private void QLLSTwXHwotOIIZcLmQyyvcmWdtK()
		{
			if (LCjhVeknCofOigKSCKpjlHKPsIpVA)
			{
				return;
			...
			try
			{
				if (!qAoAtmVqrKmkAhdCjPyiTimOyRgF())
				{
					return;
				}
				if (_dontDestroyOnLoad && Application.isPlaying)
				...
				string text = XVijVwkHntguXOqKObRQvLJDYqSh();
				UnityTools.RSjavNYDnzcBvAPUEIvOEFCjomGrA rSjavNYDnzcBvAPUEIvOEFCjomGrA = new UnityTools.RSjavNYDnzcBvAPUEIvOEFCjomGrA(platform, this.platform, editorPlatform, isEditor, webplayerPlatform, scriptingBackend, scriptingAPILevel, GetExternalTools());
				ReInput.OMYKFukhZskHtbQlRIOeCKIXVnlhA(this, nODLstJhwNrLCsDZAdZpwzpEspJS, _userData.ConfigVars, _controllerDataFiles, _userData, rSjavNYDnzcBvAPUEIvOEFCjomGrA, WaFjDulPdrQiersmLNTowygQPDcI);
				hKSnKDpreVtEdquhAzQkXdtYqfE = true;
				RKACNdAQkPCkuIoxUmujDVDIHjRPA = false;
				if (!string.IsNullOrEmpty(text))
				{
			...
			{
				vvJecSYVntxpSMLqVjRgHpoIcQTN(VyLcaPGXFiOoZsjxBarunYjtGBpx.Initialization, "initialization", ex);
			}
		}

		private void WaFjDulPdrQiersmLNTowygQPDcI(Platform P_0)
		{
			platform = P_0;
		}

		private object nODLstJhwNrLCsDZAdZpwzpEspJS(ConfigVars P_0)
		{
			List<Assembly> list = ((UnityTools.unityVersion >= UnityTools.UnityVersion.UNITY_5_0) ? null : tVbpCVToMQnqDMJgHeocIJOZbPSJ());
			return xkwDVLzkLVknJzRRMIYEQzJNeNRn.cKhUKnXksHdNdfwsnmKCDIDJiaAGA(cwtfsiaXjvaFhAxuXloyXnXFhOwU(), list, P_0);
		}

		private List<Assembly> tVbpCVToMQnqDMJgHeocIJOZbPSJ()
		{
			List<TextAsset> list = new List<TextAsset>();
			PGczgxhAvbmCxFbfCHKVqujHiaJf(list, UnityTools.GetCurrentPlatformResourecesDLLPaths());
		...
		private List<Assembly> gEmottQZVctnAbRRPcNVghiQglTkA()
		{
			bool flag = false;
			try
			{
				if (string.IsNullOrEmpty(cwtfsiaXjvaFhAxuXloyXnXFhOwU()))
				{
					return null;
				}
				...
				Assembly assembly = Array.Find(assemblies, (Assembly P_0) => string.Equals(P_0.GetName().Name, cwtfsiaXjvaFhAxuXloyXnXFhOwU(), StringComparison.OrdinalIgnoreCase));
				if (assembly == null)
				{
					flag = true;
					throw new Exception();
		...
		private byte[] seZxuwrydFsTRBEgTaHFDMGrRYWY()
		{
			try
			{
				string text = cwtfsiaXjvaFhAxuXloyXnXFhOwU();
				if (string.IsNullOrEmpty(text))
				{
					return null;
				}
		...
		private void PGczgxhAvbmCxFbfCHKVqujHiaJf(List<TextAsset> P_0, List<string> P_1)
		{
		...
		private string XVijVwkHntguXOqKObRQvLJDYqSh()
		{
			if (editorPlatform == EditorPlatform.None)
			{
				return null;
			}
			if (nyXiHYzPXGopWqUFCJmJQKSwSMOt())
			{
				return null;
			}
		...
		private bool nyXiHYzPXGopWqUFCJmJQKSwSMOt()
		{
			switch (editorPlatform)
			{
			case EditorPlatform.Windows:
		...
		private string cwtfsiaXjvaFhAxuXloyXnXFhOwU()
		{
		...
		private bool qAoAtmVqrKmkAhdCjPyiTimOyRgF()
		{
			LCjhVeknCofOigKSCKpjlHKPsIpVA = false;
			if (ReInput.isReady)
			{
				...
				LCjhVeknCofOigKSCKpjlHKPsIpVA = true;
		...
		{
			ReInput.AxrawJFNLmWSVOrLAaagfjWFwAkXb();
			ReInput.iWImclvEMlWWWQxofRqsWhMbsaDV();
		}
		...
		protected void OnSceneLoaded()
		{
			if (ReInput.isReady)
			{
				ReInput.IhnePsciDGJVhBjTvDMLscBloRRC();
			}
		}

		private void vvJecSYVntxpSMLqVjRgHpoIcQTN(VyLcaPGXFiOoZsjxBarunYjtGBpx P_0, string P_1, Exception P_2)
		{
			P_1 = "An exception occurred during " + P_1 + ".";
			bool flag = false;
			if (P_0 == VyLcaPGXFiOoZsjxBarunYjtGBpx.Initialization || P_0 == VyLcaPGXFiOoZsjxBarunYjtGBpx.Destroy)
			{
			...
			if (flag)
			{
				RKACNdAQkPCkuIoxUmujDVDIHjRPA = true;
			}
		}
		...
		{
			if (!hKSnKDpreVtEdquhAzQkXdtYqfE && !_detectedPlatformInEditor)
			{
				DetectPlatform();
			}
		...
		[CompilerGenerated]
		private bool RKgRYvXgApevJAGwMzvdBqTMytem(Assembly P_0)
		{
			return string.Equals(P_0.GetName().Name, cwtfsiaXjvaFhAxuXloyXnXFhOwU(), StringComparison.OrdinalIgnoreCase);
		}
	}
}
