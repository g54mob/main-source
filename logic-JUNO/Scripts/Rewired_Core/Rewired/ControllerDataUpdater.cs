...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] JHINgHkspuCyxuGTbstgfQgqbhJH;
		...
		public ControllerDataUpdater(InputSource P_0, int P_1, int P_2, UnknownControllerHat[] P_3)
		{
			...
			axisCount = P_1;
			buttonCount = P_2;
			JHINgHkspuCyxuGTbstgfQgqbhJH = P_3;
		...
		{
			if (JHINgHkspuCyxuGTbstgfQgqbhJH == null)
			{
				return false;
			}
			for (int i = 0; i < JHINgHkspuCyxuGTbstgfQgqbhJH.Length; i++)
			{
				if (JHINgHkspuCyxuGTbstgfQgqbhJH[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < JHINgHkspuCyxuGTbstgfQgqbhJH.Length; i++)
			{
				if (JHINgHkspuCyxuGTbstgfQgqbhJH[i].ContainsButtonIndex(buttonIndex))
				{
					return JHINgHkspuCyxuGTbstgfQgqbhJH[i].GetButtons();
				}
			}
			return null;
		}
