...
namespace Rewired.Utils
{
	...
	{
		internal struct RSjavNYDnzcBvAPUEIvOEFCjomGrA
		{
			public Platform qbJFZlTqLWJrETUdSIwrXWyTZITe;

			public Platform MOUASjtxCryOdwLayuXnHjfwaEPm;

			public EditorPlatform QJoUKUUxpvRsMchGCHEwmfYjGfmM;

			public bool HpXtRWpxlCtpaKAwZdQbZyRGeoDD;

			public WebplayerPlatform vrDQyodTzmWOChcqUJvEarTWQYTn;

			public ScriptingBackend BHErsMWoGsKYtkeFAfkNKNgKtIECA;

			public ScriptingAPILevel WTaRwRDCVpePFAOErsUZrSVkgylDA;

			public IExternalTools PqHCVPiTUMFVxTVZtErQMrxtcIUM;

			public RSjavNYDnzcBvAPUEIvOEFCjomGrA(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				qbJFZlTqLWJrETUdSIwrXWyTZITe = P_0;
				MOUASjtxCryOdwLayuXnHjfwaEPm = P_1;
				QJoUKUUxpvRsMchGCHEwmfYjGfmM = P_2;
				HpXtRWpxlCtpaKAwZdQbZyRGeoDD = P_3;
				vrDQyodTzmWOChcqUJvEarTWQYTn = P_4;
				BHErsMWoGsKYtkeFAfkNKNgKtIECA = P_5;
				WTaRwRDCVpePFAOErsUZrSVkgylDA = P_6;
				PqHCVPiTUMFVxTVZtErQMrxtcIUM = P_7;
			}
		}
		...
		{
			public enum vgLZrlGaynimodliZYTPggsRlBXr
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly vgLZrlGaynimodliZYTPggsRlBXr type;

			public readonly int build;

			public UnityVersionClass(string P_0)
			{
				type = vgLZrlGaynimodliZYTPggsRlBXr.Normal;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = vgLZrlGaynimodliZYTPggsRlBXr.Beta;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = vgLZrlGaynimodliZYTPggsRlBXr.Patch;
					}
			...
			public override string ToString()
			{
				...
				array[5] = dZCGkLdVfIdUtzlfCzAfadflpAOp(type);
				num = build;
				array[6] = num.ToString();
				return string.Concat(array);
			}

			private string dZCGkLdVfIdUtzlfCzAfadflpAOp(vgLZrlGaynimodliZYTPggsRlBXr P_0)
			{
				return P_0 switch
				{
					vgLZrlGaynimodliZYTPggsRlBXr.Normal => "f", 
					vgLZrlGaynimodliZYTPggsRlBXr.Beta => "b", 
					vgLZrlGaynimodliZYTPggsRlBXr.Patch => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (uFUZyMDADSfJHSKHDscdpgPlyNLh(a.type) > uFUZyMDADSfJHSKHDscdpgPlyNLh(b.type))
				{
					return 1;
				}
				if (uFUZyMDADSfJHSKHDscdpgPlyNLh(a.type) < uFUZyMDADSfJHSKHDscdpgPlyNLh(b.type))
			...
			private static int uFUZyMDADSfJHSKHDscdpgPlyNLh(vgLZrlGaynimodliZYTPggsRlBXr P_0)
			{
				return P_0 switch
				{
					vgLZrlGaynimodliZYTPggsRlBXr.Beta => 0, 
					vgLZrlGaynimodliZYTPggsRlBXr.Normal => 10, 
					vgLZrlGaynimodliZYTPggsRlBXr.Patch => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion sfTtNPfwVokywgVKocTOjzPaRmJi = UnityVersion.UNITY_5_0;

		private static UnityVersionClass VYgbyebFjuVmQNcyvQQZCvWoSiqY;

		private static UnityVersion iFnGXmXBNLYCfJzsZGuJaBzTLslR = UnityVersion.Unknown;

		private static string kdleKRApGzYQlMJGZhmieDkpocFRA;

		private static Platform UdJHsCFszhHPFLIYEhZIPhmUhkvnA;

		private static EditorPlatform SkCpeqaQKyLGjAGVEYtURYUQRbxP;

		private static bool mIngszCAinesITmyytcuLPXWwSdoA;

		private static bool ZlAFQFkznmAMpNvXzADxjEwDjMEc;

		private static bool fzlevEimHKGbBJznHJmzywnPnPph;

		private static WebplayerPlatform xPpaHMBRNxRLZuYiGvFbdESYRKmiA;

		private static bool EMQGjHmRBmxGrNiUxzVzhRhLqadB;

		private static bool NhwBEDqrlSIAvDoTbzZAAfiXhVHnA;

		private static bool fMKfNpFUAqPpsKwZPZyfhLLNHGgCb;

		private static bool QBqzKVGOOvcIeTyuIoPphBDJqovl;

		private static bool bfhXSdNqZdSBBvBogosoPjxscpjjA;

		private static bool wQgjQqSJggxjyqgaNJidhDctTbzd;

		private static string djoGUmcfYaKbmUXEAYDXOAVKRdaBA;

		private static ScriptingBackend PmMzVYwFZIdVZyBUjNczLuvGefob;

		private static ScriptingAPILevel AofHPvVEcBawxgBLLFUxcirjvagOb;

		private static bool gthATCUCzzhslFuFJKklsSFWZHhA;

		private static IExternalTools vmnriqufPUruTCUbAnnvZrwqloLS;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper SWsyAXiaNfaHsbyhGtFbIHFEPHtWA;

		private static bool HmRDwctrvwgcQIgAsoRCmiKSIrtu;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				if (!GXIDAvaSQyVueCjPqgVEDHbGQuEw)
				{
					return null;
				}
				return VYgbyebFjuVmQNcyvQQZCvWoSiqY;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return kdleKRApGzYQlMJGZhmieDkpocFRA;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return UdJHsCFszhHPFLIYEhZIPhmUhkvnA;
			}
		}
		...
		internal static Platform effectivePlatform
		{
			get
			{
				...
				if (!mIngszCAinesITmyytcuLPXWwSdoA)
				{
					return UdJHsCFszhHPFLIYEhZIPhmUhkvnA;
				}
				return SkCpeqaQKyLGjAGVEYtURYUQRbxP switch
				{
					EditorPlatform.Windows => Platform.Windows, 
					EditorPlatform.OSX => Platform.OSX, 
					EditorPlatform.Linux => Platform.Linux, 
					_ => UdJHsCFszhHPFLIYEhZIPhmUhkvnA, 
				};
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return SkCpeqaQKyLGjAGVEYtURYUQRbxP;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return mIngszCAinesITmyytcuLPXWwSdoA;
			}
		}

		public static bool isPlaying => ZlAFQFkznmAMpNvXzADxjEwDjMEc;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return fzlevEimHKGbBJznHJmzywnPnPph;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return xPpaHMBRNxRLZuYiGvFbdESYRKmiA;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (mIngszCAinesITmyytcuLPXWwSdoA || Application.isEditor)
				{
					return true;
				}
				...
				switch (UdJHsCFszhHPFLIYEhZIPhmUhkvnA)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!fzlevEimHKGbBJznHJmzywnPnPph)
					{
						return PmMzVYwFZIdVZyBUjNczLuvGefob == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return SkCpeqaQKyLGjAGVEYtURYUQRbxP switch
				{
					EditorPlatform.Windows => UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.Windows, 
					EditorPlatform.OSX => UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.OSX, 
					EditorPlatform.Linux => UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return EMQGjHmRBmxGrNiUxzVzhRhLqadB;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return NhwBEDqrlSIAvDoTbzZAAfiXhVHnA;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.Android && UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.Ouya && UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.AmazonFireTV)
				{
					return UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.iOS)
				{
					return UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.Windows && UdJHsCFszhHPFLIYEhZIPhmUhkvnA != Platform.Linux)
				{
					return UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return fMKfNpFUAqPpsKwZPZyfhLLNHGgCb;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_0)
				{
					return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool supportsStadia
		{
			get
			{
				...
				return iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_2019_3;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return gthATCUCzzhslFuFJKklsSFWZHhA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => PmMzVYwFZIdVZyBUjNczLuvGefob;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => AofHPvVEcBawxgBLLFUxcirjvagOb;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return vmnriqufPUruTCUbAnnvZrwqloLS;
			}
		}

		internal static IAndroidFallbackPlatformHelper cpLrCxcQtxKuFzfzsDGNdJvftxsmA
		{
			[CompilerGenerated]
			get
			{
				return SWsyAXiaNfaHsbyhGtFbIHFEPHtWA;
			}
			[CompilerGenerated]
			set
			{
				SWsyAXiaNfaHsbyhGtFbIHFEPHtWA = sWsyAXiaNfaHsbyhGtFbIHFEPHtWA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => HmRDwctrvwgcQIgAsoRCmiKSIrtu;

		private static bool GXIDAvaSQyVueCjPqgVEDHbGQuEw => YNykDjWaIvWOryYPpekQHuSXYADm();

		private static bool YNykDjWaIvWOryYPpekQHuSXYADm()
		{
			if (HmRDwctrvwgcQIgAsoRCmiKSIrtu)
			{
				return true;
			}
			try
			{
				kdleKRApGzYQlMJGZhmieDkpocFRA = Application.unityVersion;
				VYgbyebFjuVmQNcyvQQZCvWoSiqY = new UnityVersionClass(kdleKRApGzYQlMJGZhmieDkpocFRA);
				SZbhJNllaYHMCVTitFwnmwxvBTqZ();
				HmRDwctrvwgcQIgAsoRCmiKSIrtu = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return HmRDwctrvwgcQIgAsoRCmiKSIrtu;
		}

		internal static void aorjqHeBqaMJCLvbPCUZQgcbYWSL(RSjavNYDnzcBvAPUEIvOEFCjomGrA P_0)
		{
			if (GXIDAvaSQyVueCjPqgVEDHbGQuEw)
			{
				if (P_0.MOUASjtxCryOdwLayuXnHjfwaEPm == Platform.Windows81Store)
				{
					P_0.MOUASjtxCryOdwLayuXnHjfwaEPm = Platform.WindowsUWP;
				}
				UdJHsCFszhHPFLIYEhZIPhmUhkvnA = P_0.MOUASjtxCryOdwLayuXnHjfwaEPm;
				SkCpeqaQKyLGjAGVEYtURYUQRbxP = P_0.QJoUKUUxpvRsMchGCHEwmfYjGfmM;
				mIngszCAinesITmyytcuLPXWwSdoA = P_0.HpXtRWpxlCtpaKAwZdQbZyRGeoDD;
				xPpaHMBRNxRLZuYiGvFbdESYRKmiA = P_0.vrDQyodTzmWOChcqUJvEarTWQYTn;
				PmMzVYwFZIdVZyBUjNczLuvGefob = P_0.BHErsMWoGsKYtkeFAfkNKNgKtIECA;
				AofHPvVEcBawxgBLLFUxcirjvagOb = P_0.WTaRwRDCVpePFAOErsUZrSVkgylDA;
				if (vmnriqufPUruTCUbAnnvZrwqloLS != null)
				{
					vmnriqufPUruTCUbAnnvZrwqloLS.Destroy();
				}
				vmnriqufPUruTCUbAnnvZrwqloLS = P_0.PqHCVPiTUMFVxTVZtErQMrxtcIUM;
				fzlevEimHKGbBJznHJmzywnPnPph = Debug.isDebugBuild;
				ZlAFQFkznmAMpNvXzADxjEwDjMEc = true;
				DkQBPGEJnvXIbCaZWAdJapyRRFIc();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			DOxnvYjjGOiUJkzmmedUuncejZWP(minVersionStr, out var num);
			DOxnvYjjGOiUJkzmmedUuncejZWP(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (num3 && VYgbyebFjuVmQNcyvQQZCvWoSiqY < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag && VYgbyebFjuVmQNcyvQQZCvWoSiqY >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag && VYgbyebFjuVmQNcyvQQZCvWoSiqY > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool DOxnvYjjGOiUJkzmmedUuncejZWP(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void SZbhJNllaYHMCVTitFwnmwxvBTqZ()
		{
			iFnGXmXBNLYCfJzsZGuJaBzTLslR = uMgIvzZNBAPQRoenToDUHgETjfKg(Application.unityVersion);
			if (iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_3_5 && iFnGXmXBNLYCfJzsZGuJaBzTLslR < UnityVersion.UNITY_4_0)
			{
				EMQGjHmRBmxGrNiUxzVzhRhLqadB = true;
			}
			else if (iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_0)
			{
				NhwBEDqrlSIAvDoTbzZAAfiXhVHnA = true;
			}
		}

		private static UnityVersion uMgIvzZNBAPQRoenToDUHgETjfKg(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion BraEkZwXjrVPeWxUCGfvxPTEokwe(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion uShBqdBWAveWJUiTSoOzWazVuUhO(int P_0)
		{
			return P_0 switch
			{
		...
		private static void DkQBPGEJnvXIbCaZWAdJapyRRFIc()
		{
			switch (UdJHsCFszhHPFLIYEhZIPhmUhkvnA)
			{
			...
			case Platform.RazerForgeTV:
				QBqzKVGOOvcIeTyuIoPphBDJqovl = true;
				bfhXSdNqZdSBBvBogosoPjxscpjjA = true;
				break;
			case Platform.PS4:
				QBqzKVGOOvcIeTyuIoPphBDJqovl = true;
				djoGUmcfYaKbmUXEAYDXOAVKRdaBA = "Empty";
				wQgjQqSJggxjyqgaNJidhDctTbzd = true;
				break;
			case Platform.Windows:
				if ((iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_6_3p1 && iFnGXmXBNLYCfJzsZGuJaBzTLslR < UnityVersion.UNITY_5_0) || iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_0_0p1)
				{
					fMKfNpFUAqPpsKwZPZyfhLLNHGgCb = true;
					QBqzKVGOOvcIeTyuIoPphBDJqovl = true;
				}
				break;
			case Platform.Linux:
			...
			if (mIngszCAinesITmyytcuLPXWwSdoA && SkCpeqaQKyLGjAGVEYtURYUQRbxP == EditorPlatform.Windows && ((iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_6_3p1 && iFnGXmXBNLYCfJzsZGuJaBzTLslR < UnityVersion.UNITY_5_0) || iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_5_0_0p1))
			{
				fMKfNpFUAqPpsKwZPZyfhLLNHGgCb = true;
				QBqzKVGOOvcIeTyuIoPphBDJqovl = true;
			}
			if ((!mIngszCAinesITmyytcuLPXWwSdoA && UdJHsCFszhHPFLIYEhZIPhmUhkvnA == Platform.Windows) || (mIngszCAinesITmyytcuLPXWwSdoA && SkCpeqaQKyLGjAGVEYtURYUQRbxP == EditorPlatform.Windows))
			{
				gthATCUCzzhslFuFJKklsSFWZHhA = iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type jSTWcVHHWoEsYTcKZLUXqbQopPQI(OHcDgqPkmNgqhlCMACEqzNjNaeeH P_0)
		{
			if (!GXIDAvaSQyVueCjPqgVEDHbGQuEw)
			{
				return null;
			}
			if (iFnGXmXBNLYCfJzsZGuJaBzTLslR >= UnityVersion.UNITY_4_3)
			{
				return sPFtVfwYdxeeewbIIPQzdLQehfvo(P_0);
			}
			return null;
		}

		private static Type sPFtVfwYdxeeewbIIPQzdLQehfvo(OHcDgqPkmNgqhlCMACEqzNjNaeeH P_0)
		{
			return P_0 switch
			{
				OHcDgqPkmNgqhlCMACEqzNjNaeeH.RigidbodyInterpolation2D => typeof(RigidbodyInterpolation2D), 
				OHcDgqPkmNgqhlCMACEqzNjNaeeH.RigidbodySleepMode2D => typeof(RigidbodySleepMode2D), 
				OHcDgqPkmNgqhlCMACEqzNjNaeeH.CollisionDetectionMode2D => typeof(CollisionDetectionMode2D), 
				OHcDgqPkmNgqhlCMACEqzNjNaeeH.PhysicsMaterial2D => typeof(PhysicsMaterial2D), 
				OHcDgqPkmNgqhlCMACEqzNjNaeeH.Collider2D => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return vQMmxYNQjRCIxCkyneqJNkivcQDT(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return vQMmxYNQjRCIxCkyneqJNkivcQDT((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return vQMmxYNQjRCIxCkyneqJNkivcQDT((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return vQMmxYNQjRCIxCkyneqJNkivcQDT(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && fMKfNpFUAqPpsKwZPZyfhLLNHGgCb)
				{
					return false;
				}
				if (QBqzKVGOOvcIeTyuIoPphBDJqovl)
				{
					return false;
				}
			}
			...
			{
				if (wQgjQqSJggxjyqgaNJidhDctTbzd && name.Equals(djoGUmcfYaKbmUXEAYDXOAVKRdaBA, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (bfhXSdNqZdSBBvBogosoPjxscpjjA && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static _0001 vQMmxYNQjRCIxCkyneqJNkivcQDT<_0001>(_0001 P_0) where _0001 : class
		{
			if (P_0 == null)
			{
				return null;
		...
		internal static ButtonStateFlags ksLgIZAlvJXRyUpGqMdIiJFXdwYTA(KeyCode P_0)
		{
		...
		internal static ButtonStateFlags SZXZklrtjcPOFETmtRNcfyhRryyD(string P_0)
		{
