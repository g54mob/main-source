...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		private static Action<StringBuilder, object> hWXuUGeyzYNtfUTTdHmsvtTMTwVC;

		private static Action<StringBuilder, object> wHXfSAbVUTDJCzAtrTuLKxUNrTEfA => SKOtKJYCLcFKIJseLtTSILrpGipX;

		public static string ToJson(object item)
		{
			StringBuilder stringBuilder = new StringBuilder();
			SKOtKJYCLcFKIJseLtTSILrpGipX(stringBuilder, item);
			return stringBuilder.ToString();
		}

		private static void SKOtKJYCLcFKIJseLtTSILrpGipX(StringBuilder P_0, object P_1)
		{
			if (P_1 == null)
			{
				P_0.Append("null");
			...
			{
				((IExportToJson)P_1).WriteJson(P_0, wHXfSAbVUTDJCzAtrTuLKxUNrTEfA);
				return;
			}
			...
			{
				SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, P_1.ToString());
				return;
			}
			if (ReflectionTools.IsEnum(type))
			{
				Type underlyingType = Enum.GetUnderlyingType(type);
				SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, Convert.ChangeType(P_1, underlyingType));
				return;
			}
			if (ReflectionTools.DoesTypeImplement(type, typeof(IList)))
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, list[i]);
				}
				P_0.Append(']');
				return;
			}
			if (ReflectionTools.IsGenericType(type) && ReflectionTools.DoesTypeImplement(type, typeof(IDictionary)))
			{
				...
				foreach (object key in dictionary.Keys)
				{
					...
					P_0.Append("\":");
					SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, dictionary[key]);
				}
				P_0.Append('}');
				return;
			}
			...
			foreach (FieldInfo field in ReflectionTools.GetFields(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value != null)
				{
					...
					P_0.Append(name);
					P_0.Append("\":");
					SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, value);
				}
			}
			foreach (PropertyInfo property in ReflectionTools.GetProperties(type, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic))
			{
				...
				if (value2 != null)
				{
					...
					P_0.Append(name2);
					P_0.Append("\":");
					SKOtKJYCLcFKIJseLtTSILrpGipX(P_0, value2);
				}
			}
			P_0.Append('}');
		}
