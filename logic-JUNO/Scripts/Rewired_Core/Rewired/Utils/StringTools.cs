...
namespace Rewired.Utils
{
	...
	{
		private static string nJrcmaEuDoNvmCUKBfsvbwhfnIdQA;
		...
		static StringTools()
		{
			nJrcmaEuDoNvmCUKBfsvbwhfnIdQA = Regex.Escape(new string(Path.GetInvalidFileNameChars()));
		}

		public static string CleanUpFileName(string name)
		{
			name = name.Trim();
			string pattern = "[ ~`,:;'\\.\\$\\^\\{\\}\\[\\]\\(\\|\\)\\*\\+\\?\\\\" + nJrcmaEuDoNvmCUKBfsvbwhfnIdQA + "]";
