...
namespace Rewired.Utils.Classes
{
	...
	{
		private const int VJLCSchgGAnmoEfXiXbtlGKfdeYKA = 3;

		[NonSerialized]
		private bool oSwLENTFrMokEdYNAwectMqLiDjc;
		...
		{
			if (!oSwLENTFrMokEdYNAwectMqLiDjc)
			{
				GkvVTajQFGyHUGWXxBnniBwdUJUB();
			}
		...
		private void GkvVTajQFGyHUGWXxBnniBwdUJUB()
		{
			_data = new List<TypeWrapper>(3);
			oSwLENTFrMokEdYNAwectMqLiDjc = true;
		}
	}
}
