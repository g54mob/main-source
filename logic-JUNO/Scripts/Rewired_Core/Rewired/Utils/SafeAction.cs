...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> roVAnDhWjNgwzQwbSyOmdfkBowHg;

		private static Action<object, Action> invokeDelegate => LcfmYcaWFQyolcFPWAcfNjBuPdOA;
		...
		private static void LcfmYcaWFQyolcFPWAcfNjBuPdOA(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	{
		private T evcgjsTPbdukhoUqiTraqbLceEmq;

		private static Action<object, Action<T>> WpAMsZcoAlGDOiQermWSneDOmvcR;

		private static Action<object, Action<T>> invokeDelegate => EqnGExHKvXJbfQfGutjcfVzCtNyt;
		...
		{
			evcgjsTPbdukhoUqiTraqbLceEmq = arg0;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			evcgjsTPbdukhoUqiTraqbLceEmq = default(T);
		}
		...
		private static void EqnGExHKvXJbfQfGutjcfVzCtNyt(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.evcgjsTPbdukhoUqiTraqbLceEmq);
			}
		}
	...
	{
		private T qZtFFrXploYODNbbawPLgROZEwdh;

		private T2 LarDQGqnsgKCaTLzumYjtgdhTuTc;

		private static Action<object, Action<T, T2>> SIplWuhGGnCPZHhcsDyAmmPiHSvGA;

		private static Action<object, Action<T, T2>> invokeDelegate => dtXoiFCkgGnRfYRLrpnLbFGqRPkt;
		...
		{
			qZtFFrXploYODNbbawPLgROZEwdh = arg0;
			LarDQGqnsgKCaTLzumYjtgdhTuTc = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			qZtFFrXploYODNbbawPLgROZEwdh = default(T);
			LarDQGqnsgKCaTLzumYjtgdhTuTc = default(T2);
		}
		...
		private static void dtXoiFCkgGnRfYRLrpnLbFGqRPkt(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.qZtFFrXploYODNbbawPLgROZEwdh, safeAction.LarDQGqnsgKCaTLzumYjtgdhTuTc);
			}
		}
