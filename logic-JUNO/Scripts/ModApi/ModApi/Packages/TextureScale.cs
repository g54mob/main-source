...
namespace ModApi.Packages
{
	public class TextureScale
	{
		...
		public static void Point(Texture2D tex, int newWidth, int newHeight, int x, int y, int blockWidth, int blockHeight)
		{
			ThreadedScale(tex, tex.GetPixels(x, y, blockWidth, blockHeight), blockWidth, blockHeight, newWidth, newHeight, useBilinear: false);
		}
		...
		public static void Bilinear(Texture2D tex, int newWidth, int newHeight, int x, int y, int blockWidth, int blockHeight)
		{
			ThreadedScale(tex, tex.GetPixels(x, y, blockWidth, blockHeight), blockWidth, blockHeight, newWidth, newHeight, useBilinear: true);
		}

		private static void ThreadedScale(Texture2D tex, int newWidth, int newHeight, bool useBilinear)
		{
			ThreadedScale(tex, tex.GetPixels(), tex.width, tex.height, newWidth, newHeight, useBilinear);
		}

		private static void ThreadedScale(Texture2D tex, Color[] colors, int width, int height, int newWidth, int newHeight, bool useBilinear)
		{
			texColors = colors;
			newColors = new Color[newWidth * newHeight];
			if (useBilinear)
			{
				ratioX = 1f / ((float)newWidth / (float)(width - 1));
				ratioY = 1f / ((float)newHeight / (float)(height - 1));
			}
			else
			{
				ratioX = (float)width / (float)newWidth;
				ratioY = (float)height / (float)newHeight;
			}
			w = width;
			w2 = newWidth;
