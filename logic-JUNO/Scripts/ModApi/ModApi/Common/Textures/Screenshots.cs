...
namespace ModApi.Common.Textures
{
	public class Screenshots
	{
		...
		public static void TakeScreenShot(Rect screenSampleRect, Vector2i resolution, Action<Texture2D> onScreenshotComplete)
		{
			...
			int upsampleSize = 1;
			if ((float)resolution.x != screenSampleRect.width && !CurrentDevice.HasAnyFlag(DeviceFlags.LowRam))
			{
		...
		private static string CaptureScreenshot(string fileName, int superSize)
		{
			...
			{
				empty = Utilities.CombinePaths(ModApi.Game.PersistentDataPath, fileName);
				ScreenCapture.CaptureScreenshot(fileName, superSize);
			}
			else
			{
				empty = Utilities.CombinePaths(Application.temporaryCachePath, fileName);
