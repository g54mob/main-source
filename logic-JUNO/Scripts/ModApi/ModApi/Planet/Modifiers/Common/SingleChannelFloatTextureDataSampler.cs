...
{
	public class SingleChannelFloatTextureDataSampler : ISingleChannelTextureDataSampler
	{
		private float[] _data;
		...
		public SingleChannelFloatTextureDataSampler(Texture2D texture, int colorChannel)
			: this(texture.GetPixels(), colorChannel, texture.width, texture.height)
		{
		}

		public SingleChannelFloatTextureDataSampler(Color[] colors, int colorChannel, int width, int height)
		{
			_width = width;
			_height = height;
			...
			_data = new float[_paddedWidth * _paddedHeight];
			for (int i = 0; i < _paddedHeight; i++)
			{
				...
				for (int j = 0; j < _paddedWidth; j++)
				{
					...
					_data[num3] = colors[num4][colorChannel];
				}
			}
		}

		private SingleChannelFloatTextureDataSampler(float[] data, int width, int height)
		{
			_width = width;
			_height = height;
			_widthMinusOne = width - 1;
		...
		public static SingleChannelFloatTextureDataSampler Load(BinaryReader reader)
		{
			int num = reader.ReadInt32();
			int num2 = reader.ReadInt32();
			float[] array = new float[(num + 4) * (num2 + 4)];
			int num3 = 0;
			...
			return new SingleChannelFloatTextureDataSampler(array, num, num2);
		}
		...
		public float SampleBicubic(float u, float v, float[][] preallocatedArray)
		{
			...
			preallocatedArray[0][0] = _data[num5];
			preallocatedArray[1][0] = _data[num5 + 1];
			preallocatedArray[2][0] = _data[num5 + 2];
			preallocatedArray[3][0] = _data[num5 + 3];
			preallocatedArray[0][1] = _data[num6];
			preallocatedArray[1][1] = _data[num6 + 1];
			preallocatedArray[2][1] = _data[num6 + 2];
			preallocatedArray[3][1] = _data[num6 + 3];
			preallocatedArray[0][2] = _data[num7];
			preallocatedArray[1][2] = _data[num7 + 1];
			preallocatedArray[2][2] = _data[num7 + 2];
			preallocatedArray[3][2] = _data[num7 + 3];
			preallocatedArray[0][3] = _data[num8];
			preallocatedArray[1][3] = _data[num8 + 1];
			preallocatedArray[2][3] = _data[num8 + 2];
			preallocatedArray[3][3] = _data[num8 + 3];
		...
		public float SampleBilinear(float u, float v)
		{
			...
			int num12 = num11 + 1;
			return (_data[num9] * num7 + _data[num10] * num5) * num8 + (_data[num11] * num7 + _data[num12] * num5) * num6;
		}
