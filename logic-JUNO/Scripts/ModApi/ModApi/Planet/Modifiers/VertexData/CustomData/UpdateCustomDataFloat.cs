...
{
	[PlanetModifierInfo("Update Custom Data (Float)", "A planet modifier used to add the specified data input slot to the custom vertex data with the specified ID (to be used by a game mod).")]
	public class UpdateCustomDataFloat : UpdateCustomData<CustomPlanetVertexDataFloat>
	{
		[SerializeField]
		[Range(0f, 9f)]
		...
		public override List<FieldInfo> GetInspectorFields()
		{
			...
			inspectorFields.Add(typeof(UpdateCustomDataFloat).GetField("_dataIndex", BindingFlags.Instance | BindingFlags.NonPublic));
		...
		protected override void GetVertexData(double[] data, CustomPlanetVertexDataFloat customData)
		{
			customData.Value += (float)data[_dataIndex];
		}
