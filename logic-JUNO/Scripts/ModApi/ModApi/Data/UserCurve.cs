...
namespace ModApi.Data
{
	public class UserCurve
	{
		...
		public static bool TryAddKeyframes(AnimationCurve curve, string keyframes)
		{
			string[] array = (keyframes ?? string.Empty).Split(_separatorKeyframe, StringSplitOptions.RemoveEmptyEntries);
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				try
				{
					string[] array3 = array[i].Split(_separatorKeyframeValues, StringSplitOptions.RemoveEmptyEntries);
					if (array3.Length == 2)
					{
						keyframe = new Keyframe(DataIO.ParseFloat(array3[0]), DataIO.ParseFloat(array3[1]));
					}
					else if (array3.Length == 3)
					{
						float num = DataIO.ParseFloat(array3[2]);
						keyframe = new Keyframe(DataIO.ParseFloat(array3[0]), DataIO.ParseFloat(array3[1]), num, num);
					}
					else
					{
						if (array3.Length != 4)
						...
						keyframe = new Keyframe(DataIO.ParseFloat(array3[0]), DataIO.ParseFloat(array3[1]), DataIO.ParseFloat(array3[2]), DataIO.ParseFloat(array3[3]));
					}
				}
				catch
				{
			...
			Keyframe[] array4 = array2;
			foreach (Keyframe key in array4)
			{
				curve.AddKey(key);
			}
			return true;
