...
namespace ModApi.Settings.Core
{
	public abstract class SettingsCategory
	{
		...
		public static List<SettingsCategory> InitializeCategoryProperties<T>(T obj, XElement xml)
		{
		...
		{
			return AvailablePresets.FirstOrDefault();
		}
		...
		public virtual void RestoreFromXml(XElement settingsXml)
		{
			...
			Expanded = Utilities.GetBoolAttribute(xElement, "expanded", defaultValue: false);
		...
		public void SetPreset(SettingsCategoryPreset preset)
		{
			...
			ApplyPreset(Preset, CurrentDevice.Flags);
		}
		...
		{
			if ((State == SettingState.Enabled || State == SettingState.Hidden) && (setting.State == SettingState.Hidden || setting.State == SettingState.Hidden))
			{
		...
		protected ButtonSetting.ButtonSettingBuilder CreateButton(string labelText, string buttonText, string xmlName = null)
		{
			return ButtonSetting.Create(labelText, buttonText, this, xmlName);
		}
