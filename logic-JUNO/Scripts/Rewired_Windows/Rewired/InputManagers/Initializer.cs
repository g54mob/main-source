...
namespace Rewired.InputManagers
{
	...
	internal class Initializer : PlatformInitializer
	{
		...
		private static PlatformInitializer TRlEPcUHjfFKvwdrHxUInMOGqSu;

		public static PlatformInitializer GetPlatformInitializer()
		{
			if (TRlEPcUHjfFKvwdrHxUInMOGqSu == null)
			{
				TRlEPcUHjfFKvwdrHxUInMOGqSu = new Initializer();
			}
			return TRlEPcUHjfFKvwdrHxUInMOGqSu;
		}
		...
		public override object Initialize(IConfigVars_Internal configVars)
		{
			if (UnityTools.platform == Platform.Windows || UnityTools.platform == Platform.WindowsAppStore)
			{
				try
				{
					return new pgLfuJytUzHNywlhBuSxXGsrwvQS((ConfigVars)configVars, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId);
				}
				catch (Exception)
				{
		...
		public override IElementIdentifierTool CreateTool(string inputSourceString)
		{
			...
			{
				return new quSSJqRRJxlNxOnstFlgPukSuCvR();
			}
			if (inputSourceString == "RawInput")
			{
				return new WEieuGDTnjXvYozThkjZNIRXKAMy();
			}
			return null;
		}
	}
