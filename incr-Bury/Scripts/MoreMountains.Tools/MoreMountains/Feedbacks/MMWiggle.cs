...
namespace MoreMountains.Feedbacks
{
	...
	public class MMWiggle : MonoBehaviour
	{
		...
		{
			if (PositionWiggleProperties == null)
			{
				PositionWiggleProperties = new WiggleProperties();
			}
			if (RotationWiggleProperties == null)
			{
				RotationWiggleProperties = new WiggleProperties();
			}
			if (ScaleWiggleProperties == null)
			{
				ScaleWiggleProperties = new WiggleProperties();
			}
		...
		protected virtual void InitializeRandomValues(ref WiggleProperties properties, ref InternalWiggleProperties internalProperties)
		{
			...
			internalProperties.newValue = DetermineNewValue(properties, internalProperties.newValue, internalProperties.initialValue, ref internalProperties.startValue, ref internalProperties.randomAmplitude, ref internalProperties.randomFrequency, ref internalProperties.pauseDuration, firstPlay: true);
		}
		...
		protected virtual Vector3 AnimateCurveValue(ref InternalWiggleProperties internalProperties, WiggleProperties properties)
		{
			...
			if (internalProperties.timeSinceLastPause < internalProperties.pauseDuration)
			{
				...
				EvaluateCurve(properties.Curve, percent, internalProperties.remapZero, internalProperties.remapOne, ref internalProperties.newValue, properties);
			...
			if (internalProperties.randomFrequency > 0f)
			{
				...
				EvaluateCurve(properties.Curve, num, internalProperties.remapZero, internalProperties.remapOne, ref internalProperties.newValue, properties);
		...
		protected virtual Vector3 DetermineNewValue(WiggleProperties properties, Vector3 newValue, Vector3 initialValue, ref Vector3 startValue, ref Vector3 randomAmplitude, ref float randomFrequency, ref float pauseDuration, bool firstPlay = false)
		{
			...
			case WiggleTypes.PingPong:
				if (properties.RelativeAmplitude)
				{
					...
					else if (newValue == properties.AmplitudeMin + initialValue)
					{
						startValue = newValue;
						newValue = properties.AmplitudeMax * ApplyFalloff(properties) + initialValue;
					}
		...
		{
			base.transform.localPosition = _positionInternalProperties.initialValue;
			base.transform.localEulerAngles = _rotationInternalProperties.initialValue;
			base.transform.localScale = _scaleInternalProperties.initialValue;
		}
