	...
	public class MMF_TMPTextReveal : MMF_Feedback
	{
		...
		protected virtual IEnumerator RevealCharacters()
		{
			...
			while (visibleCharacters <= _totalCharacters && !Owner.SkippingToTheEnd)
			{
				...
				InvokeRevealEvents();
				int num = (int)Mathf.Round((time - lastCharAt) / IntervalBetweenReveals);
				for (int i = 0; i < num; i++)
				{
					visibleCharacters++;
				}
				...
				{
					int num2 = _totalCharacters - visibleCharacters;
					float num3 = time - startTime;
					if (num2 != 0)
					{
						_delay = (RevealDuration - num3) / (float)num2;
					}
					delay = _delay - FeedbackDeltaTime;
				}
