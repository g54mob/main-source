...
namespace MoreMountains.Feedbacks
{
	public class MMFloatingText : MonoBehaviour
	{
		...
		protected virtual void Initialization()
		{
			...
			FollowTarget = base.gameObject.MMGetOrAddComponent<MMFollowTarget>();
		}
		...
		protected virtual void HandleMovement()
		{
			if (_animateMovement)
			{
				...
				_newPosition.x = (_animateX ? MMMaths.Remap(_animateXCurve.Evaluate(_remappedTime), 0f, 1f, _remapXZero, _remapXOne) : 0f);
				_newPosition.y = (_animateY ? MMMaths.Remap(_animateYCurve.Evaluate(_remappedTime), 0f, 1f, _remapYZero, _remapYOne) : 0f);
				_newPosition.z = (_animateZ ? MMMaths.Remap(_animateZCurve.Evaluate(_remappedTime), 0f, 1f, _remapZZero, _remapZOne) : 0f);
		...
		protected virtual void HandleOpacity()
		{
			if (_animateOpacity)
			{
				float opacity = MMMaths.Remap(_animateOpacityCurve.Evaluate(_remappedTime), 0f, 1f, _remapOpacityZero, _remapOpacityOne);
				SetOpacity(opacity);
			}
		}
		...
		protected virtual void HandleAlignment()
		{
			...
			{
				MovingPart.transform.up = base.transform.up;
			}
