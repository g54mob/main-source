...
namespace MoreMountains.FeedbacksForThirdParty
{
	...
	public class MMF_CinemachineTransition : MMF_Feedback
	{
		...
		public override float FeedbackDuration
		{
			get
			{
				return ApplyTimeMultiplier(BlendDefintion.Time);
			}
			set
			{
				BlendDefintion.Time = value;
			}
		}
		...
		{
			TargetCinemachineCamera = FindAutomatedTarget<CinemachineCamera>();
		}
		...
		{
			PrioritySettings priority = TargetCinemachineCamera.Priority;
			priority.Value = newPriority;
			TargetCinemachineCamera.Priority = priority;
		}
		...
		protected override void CustomRestoreInitialValues()
		{
			if (Active && FeedbackTypeAuthorized)
			{
				MMCinemachinePriorityEvent.Trigger(ChannelData, ForceMaxPriority, 0, ForceTransition, _tempBlend, ResetValuesAfterTransition, ComputedTimescaleMode, restore: true);
			}
		}
	}
}
