...
namespace Lofelt.NiceVibrations
{
	public class HapticClipsDemoManager : DemoManager
	{
		...
		{
			HapticController.PlaybackStopped = (Action)Delegate.Remove(HapticController.PlaybackStopped, new Action(OnHapticsStopped));
		...
		{
			HapticController.PlaybackStopped = (Action)Delegate.Combine(HapticController.PlaybackStopped, new Action(OnHapticsStopped));
			StartCoroutine(BackToIdle());
		}
