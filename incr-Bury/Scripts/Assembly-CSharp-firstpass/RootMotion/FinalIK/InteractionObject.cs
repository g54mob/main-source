...
namespace RootMotion.FinalIK
{
	...
	public class InteractionObject : MonoBehaviour
	{
		...
		public void Apply(IKSolverFullBodyBiped solver, FullBodyBipedEffector effector, InteractionTarget target, float timer, float weight, bool isPaused)
		{
			for (int i = 0; i < weightCurves.Length; i++)
			{
				if (!isPaused || (weightCurves[i].type != WeightCurve.Type.PositionOffsetX && weightCurves[i].type != WeightCurve.Type.PositionOffsetY && weightCurves[i].type != WeightCurve.Type.PositionOffsetZ))
				{
			...
			{
				if (!isPaused || (multipliers[j].result != WeightCurve.Type.PositionOffsetX && multipliers[j].result != WeightCurve.Type.PositionOffsetY && multipliers[j].result != WeightCurve.Type.PositionOffsetZ))
				{
		...
		private void Apply(IKSolverFullBodyBiped solver, FullBodyBipedEffector effector, WeightCurve.Type type, float value, float weight)
		{
			...
			{
				Vector3 vector2 = ((positionOffsetSpace != null) ? positionOffsetSpace.rotation : solver.GetRoot().rotation) * Vector3.right * value;
				solver.GetEffector(effector).position += vector2 * weight;
				break;
			}
			case WeightCurve.Type.PositionOffsetY:
			{
				Vector3 vector3 = ((positionOffsetSpace != null) ? positionOffsetSpace.rotation : solver.GetRoot().rotation) * Vector3.up * value;
				solver.GetEffector(effector).position += vector3 * weight;
				break;
			}
			case WeightCurve.Type.PositionOffsetZ:
			{
				Vector3 vector = ((positionOffsetSpace != null) ? positionOffsetSpace.rotation : solver.GetRoot().rotation) * Vector3.forward * value;
				solver.GetEffector(effector).position += vector * weight;
				break;
			}
