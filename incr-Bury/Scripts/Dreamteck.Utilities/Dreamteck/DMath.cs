...
namespace Dreamteck
{
	public static class DMath
	{
		...
		public static void LerpVector3NonAlloc(Vector3 a, Vector3 b, double t, ref Vector3 target)
		{
			t = Clamp01(t);
			Vector3 vector = b - a;
			target.x = (float)((double)a.x + (double)vector.x * t);
			target.y = (float)((double)a.y + (double)vector.y * t);
			target.z = (float)((double)a.z + (double)vector.z * t);
		}

		public static Vector3 LerpVector3(Vector3 a, Vector3 b, double t)
		{
			Vector3 target = Vector3.zero;
			LerpVector3NonAlloc(a, b, t, ref target);
			return target;
		}
