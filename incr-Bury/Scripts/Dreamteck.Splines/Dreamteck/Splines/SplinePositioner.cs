...
namespace Dreamteck.Splines
{
	...
	public class SplinePositioner : SplineTracer
	{
		...
		public double position
		{
			...
			set
			{
				if (value != (double)_position)
				{
					_position = (float)value;
					if (mode == Mode.Distance)
					{
						SetDistance(_position, checkTriggers: true, handleJunctions: true);
					}
					else
					{
						SetPercent(value, checkTriggers: true, handleJunctions: true);
					}
				}
			}
		}
		...
		{
			float moved;
			double num = Travel(followTarget.result.percent, _followTargetDistance, _followTargetDirection, out moved);
			if (_followLoop && _followTargetDistance - moved > 1E-06f)
			{
				if (num <= 1E-06)
				{
					num = Travel(1.0, _followTargetDistance - moved, _followTargetDirection, out moved);
				}
				else if (num >= 0.999999)
				{
					num = Travel(0.0, _followTargetDistance - moved, _followTargetDirection, out moved);
				}
			}
			SetPercent(num, checkTriggers: true);
		}
		...
		protected override void OnDidApplyAnimationProperties()
		{
			if (_lastPosition != _position)
			{
				...
				if (mode == Mode.Distance)
				{
					SetDistance(_position, checkTriggers: true);
				}
				else
				{
		...
		public override void SetDistance(float distance, bool checkTriggers = false, bool handleJunctions = false)
		{
			...
			double num = Travel(0.0, distance);
			...
			_position = ((mode == Mode.Distance) ? distance : ((float)num));
			if (handleJunctions)
			{
				InvokeNodes();
			}
