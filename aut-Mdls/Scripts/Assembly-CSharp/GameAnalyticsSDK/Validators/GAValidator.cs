...
namespace GameAnalyticsSDK.Validators
{
	internal static class GAValidator
	{
		...
		public static bool ValidateProgressionEvent(GAProgressionStatus progressionStatus, string progression01, string progression02, string progression03)
		{
			...
			{
				Debug.Log("Validation fail - progression event: 03 found but 01+02 are invalid. Progression must be set as either 01, 01+02 or 01+02+03. " + progression01 + ":" + progression02 + ":" + progression03);
				return false;
			}
			if (!string.IsNullOrEmpty(progression02) && string.IsNullOrEmpty(progression01))
			{
				Debug.Log("Validation fail - progression event: 02 found but not 01. Progression must be set as either 01, 01+02 or 01+02+03" + progression01 + ":" + progression02 + ":" + progression03);
				return false;
			}
			if (string.IsNullOrEmpty(progression01))
			{
				Debug.Log("Validation fail - progression event: progression01 not valid. Progressions must be set as either 01, 01+02 or 01+02+03" + progression01 + ":" + progression02 + ":" + progression03);
				return false;
			}
		...
		public static bool ValidateArrayOfStrings(long maxCount, long maxStringLength, bool allowNoValues, string logTag, params string[] arrayOfStrings)
		{
			...
			{
				Debug.Log(text + " validation failed: array cannot exceed " + maxCount + " values. It has " + arrayOfStrings.Length + " values.");
				return false;
			}
			foreach (string text2 in arrayOfStrings)
			{
				...
				{
					Debug.Log(text + " validation failed: a string exceeded max allowed length (which is: " + maxStringLength + "). String was: " + text2);
					return false;
				}
			}
			return true;
