...
namespace Shapes
{
	...
	public class FpsController : ImmediateModeShapeDrawer
	{
		...
		public static void DrawRoundedArcOutline(Vector2 origin, float radius, float thickness, float outlineThickness, float angStart, float angEnd)
		{
			...
			Draw.Arc(vector, thickness / 2f, outlineThickness, angStart, angStart - MathF.PI);
			Draw.Arc(vector2, thickness / 2f, outlineThickness, angEnd, angEnd + MathF.PI);
		}
		...
		private void Update()
		{
			...
			if (InputFocus)
			{
				...
				if (Input.GetMouseButtonDown(0) && ammoBar.HasBulletsLeft)
				{
					ammoBar.Fire();
					crosshair.Fire();
					if (Physics.Raycast(new Ray(head.position, head.forward), out var hitInfo) && hitInfo.collider.gameObject.name == "Enemy")
					{
						crosshair.FireHit();
					}
				}
