...
namespace Linework.SurfaceFill
{
	...
	public class SurfaceFill : ScriptableRendererFeature
	{
		private class SurfaceFillPass : ScriptableRenderPass
		{
			...
			public bool Setup(ref SurfaceFillSettings surfaceFillSettings, ref Material maskMaterial, ref Material fillMaterial)
			{
				...
				foreach (Fill fill2 in settings.Fills)
				{
					...
					if (fill2.materialType == MaterialType.Custom && fill2.customMaterial != null)
					{
						fill2.customMaterial.SetInt(CommonShaderPropertyId.FullScreenColorBlendModeSource, value);
						fill2.customMaterial.SetInt(CommonShaderPropertyId.FullScreenColorBlendModeDestination, value2);
					}
					...
					fill2.material.SetColor(ShaderPropertyId.SecondaryColor, (fill2.pattern == Pattern.Glow) ? Color.clear : fill2.secondaryColor);
					...
					if (fill2.materialType == MaterialType.Custom && fill2.customMaterial != null)
					{
						fill2.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilComparison, 3f);
						fill2.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilReference, 1 << num);
						fill2.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilReadMask, 1 << num);
					}
					if (fill2.IsActive())
					{
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				PassData passData;
				using (IRasterRenderGraphBuilder rasterRenderGraphBuilder = renderGraph.AddRasterRenderPass<PassData>("Mask (Surface Fill)", out passData, "./Packages/dev.ameye.linework/Runtime/SurfaceFill/SurfaceFill.cs", 212))
				{
				...
				PassData passData2;
				using IRasterRenderGraphBuilder rasterRenderGraphBuilder2 = renderGraph.AddRasterRenderPass<PassData>("Fill (Surface Fill)", out passData2, "./Packages/dev.ameye.linework/Runtime/SurfaceFill/SurfaceFill.cs", 236);
					...
					foreach (Fill fill in settings.Fills)
					{
						...
						else
						{
							if (num == lastActiveFillIndex)
							{
								...
								{
									fill.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilPass, 1f);
									fill.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilFail, 1f);
								}
							}
			...
			private void InitMaskRendererLists(RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				foreach (Fill fill in settings.Fills)
				{
					...
					FilteringSettings filteringSettings = new FilteringSettings(fill.renderQueue switch
					{
					...
					}, fill.layerMask, fill.RenderingLayer);
				...
				using (new ProfilingScope(commandBuffer, fillSampler))
				{
					...
					foreach (Fill fill2 in settings.Fills)
					{
						...
						if (num2 == lastActiveFillIndex)
						{
							...
							if (fill2.materialType == MaterialType.Custom && fill2.customMaterial != null)
							{
								fill2.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilPass, 1f);
								fill2.customMaterial.SetFloat(CommonShaderPropertyId.FullScreenStencilFail, 1f);
							}
						}
						CoreUtils.SetRenderTarget(commandBuffer2, renderingData.cameraData.renderer.cameraColorTargetHandle, cameraDepthRTHandle);
						switch (fill2.materialType)
						{
						case MaterialType.Basic:
							Blitter.BlitTexture(commandBuffer2, Vector2.one, fill2.material, 0);
							break;
						case MaterialType.Custom:
							if (fill2.customMaterial != null)
							{
								Blitter.BlitTexture(commandBuffer2, Vector2.one, fill2.customMaterial, 0);
							}
							break;
						}
						num2++;
		...
		{
			if (!(settings == null) && surfaceFillPass != null && (renderingData.cameraData.cameraType != CameraType.SceneView || settings.ShowInSceneView))
			{
