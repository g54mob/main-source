	...
	public class FastOutline : ScriptableRendererFeature
	{
		private class FastOutlinePass : ScriptableRenderPass
		{
			...
			private void InitMaskRendererList(RenderGraph renderGraph, ContextContainer frameData, ref PassData passData)
			{
				...
				int layerMask = settings.Outlines.Where(ShouldRenderStencilMask).Aggregate(0, (int current, Outline outline) => current | outline.layerMask.value);
				FilteringSettings filteringSettings = new FilteringSettings(opaque, layerMask, renderingLayerMask);
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				using (new ProfilingScope(commandBuffer, maskSampler))
				{
					...
					int layerMask = settings.Outlines.Where(ShouldRenderStencilMask).Aggregate(0, (int num, Outline outline) => num | outline.layerMask.value);
					RenderQueueRange all = RenderQueueRange.all;
					FilteringSettings filteringSettings = new FilteringSettings(all, layerMask, seed);
				...
				using (new ProfilingScope(commandBuffer2, outlineSampler))
				{
					...
					foreach (Outline outline in settings.Outlines)
					{
						if (ShouldRenderOutline(outline))
						{
							...
							FilteringSettings filteringSettings2 = new FilteringSettings(opaque, outline.layerMask, outline.RenderingLayer);
				...
				using (new ProfilingScope(commandBuffer3, outlineSampler))
				{
					context.ExecuteCommandBuffer(commandBuffer3);
					commandBuffer3.Clear();
					CoreUtils.SetRenderTarget(commandBuffer3, renderingData.cameraData.renderer.cameraColorTargetHandle, cameraDepthRTHandle);
					commandBuffer3.DrawProcedural(Matrix4x4.identity, clear, 0, MeshTopology.Triangles, 3, 1);
				}
				context.ExecuteCommandBuffer(commandBuffer3);
		...
		{
			if (!(settings == null) && fastOutlinePass != null && (renderingData.cameraData.cameraType != CameraType.SceneView || settings.ShowInSceneView))
			{
