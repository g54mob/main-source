namespace Rewired
{
	public struct ControllerTemplateElementTarget
	{
		private IControllerTemplateElement XCbehkgqGvFzjTBKpUySTuuuAovKA;

		private AxisRange VNVNeREQMcPSMvkQHxDvdfyFFsjM;

		public AxisRange axisRange
		{
			get
			{
				return default(AxisRange);
			}
			set
			{
			}
		}

		public ControllerTemplateElementType elementType => default(ControllerTemplateElementType);

		public string descriptiveName => null;

		public IControllerTemplateElement element
		{
			get
			{
				return null;
			}
			set
			{
			}
		}

		public IControllerTemplate template => null;

		public bool hasTarget => false;

		internal ControllerTemplateElementTarget(IControllerTemplateElement P_0, AxisRange P_1)
		{
			XCbehkgqGvFzjTBKpUySTuuuAovKA = null;
			VNVNeREQMcPSMvkQHxDvdfyFFsjM = default(AxisRange);
		}

		public ControllerTemplateElementTarget(ControllerTemplateElementTarget P_0)
		{
			XCbehkgqGvFzjTBKpUySTuuuAovKA = null;
			VNVNeREQMcPSMvkQHxDvdfyFFsjM = default(AxisRange);
		}
	}
}
