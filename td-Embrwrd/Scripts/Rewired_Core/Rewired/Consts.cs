...
namespace Rewired
{
	...
	public static class Consts
	{
		...
		private static readonly Guid[] wFKnwygfUdscxubyQFZKcqwvumyeA;

		internal static readonly ReadOnlyCollection<Guid> reservedHardwareTypeGuids;

		private static ControllerElementIdentifier.kFpGcpABeeVqEMebUKyktFBdAbnL[] uTvyhxsmFzgTzfEmtkDgwcCRAfkW;

		private static ReadOnlyCollection<ControllerElementIdentifier> ZlnMPglLINhytaoFEcPqqfDmBStx;

		private static ReadOnlyCollection<ControllerElementIdentifier> gSYeQHBXioCCSPDytqxHmnSyMoVXA;

		private static ReadOnlyCollection<HardwareJoystickMap.CompoundElement> xVIgPLbApWTFEmOgbLolzCUatExVA;

		internal static readonly IList<string> mouseAxisUnityNames;

		private static readonly string[] ssPQdNIASWicvVakwjuYdksfbAbb;

		internal static readonly IList<string> mouseButtonUnityNames;

		private static readonly string[] faGqtEMRWVakvhCDjxLJbvmFzYXd;

		internal static readonly IList<string> keyboardKeyNames;

		private static readonly string[] zkchHaldwcFMPUlccMWMTxSNdrJP;

		internal static readonly IList<int> keyboardKeyValues;

		internal static readonly int[] _keyboardKeyValues;

		private static readonly IList<string> WGcpFpsHLZeGqAjNFOBYqEEqOziR;

		private static readonly string[] OQLeYicqRFiuEWFiDIDnbjwGsIsl;

		private static readonly IList<string> vBueKgGuzmFYmOJwFnSTgIWMOLUr;

		private static readonly string[] ixpWindKfYaYkpCRPIKkVUqxBjKW;
		...
		private static readonly PidVid[] kJmuvkIMySPeNPQpAOoyFjaAOafA;

		internal static readonly IList<string> productNames_sony_dualShock4;

		private static readonly string[] dDQmdZqJqRvgLTVjofudcbbBsrC;

		internal static readonly IList<PidVid> pidVids_sony_dualSense;

		private static readonly PidVid[] BUfkSiJcWkMwDyYWkdYZExEJTMwJ;

		internal static readonly IList<string> productNames_sony_dualSense;

		private static readonly string[] haPvvNVBjWcFJtvMXrMdXZbSSFkS;

		private static readonly ControllerElementIdentifier[] wZhUgDpEMNaVRTsRhrYIAFZUlEKH;

		internal const int updateLoopTypeCount = 3;

		internal static int nintendoSwitchPlugin_minPluginVersion => 0;

		internal static int nintendoSwitch2Plugin_minPluginVersion => 0;
		...
		internal static ControllerElementIdentifier.kFpGcpABeeVqEMebUKyktFBdAbnL[] commonMouseElementIdentifierInitOptions => null;
		...
		internal static IList<HardwareJoystickMap.CompoundElement> unifiedMouseCompoundElements_readOnly => null;
		...
		private static ControllerElementIdentifier[] XFOzAoBEZwefkGWBrTFeVsxqpmWPA()
		{
			return null;
		}
	}
