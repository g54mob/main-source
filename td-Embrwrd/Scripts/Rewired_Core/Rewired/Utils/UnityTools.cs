...
namespace Rewired.Utils
{
	...
	{
		internal struct PiLPbHlhwYsdUvbqAeFitwleenxJ
		{
			public Platform gEjdxtmkulSuzzgiYhTZIfnIxPgLA;

			public Platform KwckIfAkEEZiQIdWacnLrAIbhakZ;

			public EditorPlatform GbGgCOfWrAqMvYLuOUOEAgjcHhTN;

			public bool ZLxBPCOYpnWMFyUkHwqRhHXJfokr;

			public WebplayerPlatform rvvbYaYPzXCpzaVOANmmIGydVGqkB;

			public ScriptingBackend JEqdEGCnYXWbAhQtIofvdvHiDEjmb;

			public ScriptingAPILevel KXAAYRDiPGfgsamsAnFhXViMfeMEB;

			public IExternalTools NPnbNRVUZxeNWlruxcQoaUeoOZje;

			public PiLPbHlhwYsdUvbqAeFitwleenxJ(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				gEjdxtmkulSuzzgiYhTZIfnIxPgLA = default(Platform);
				KwckIfAkEEZiQIdWacnLrAIbhakZ = default(Platform);
				GbGgCOfWrAqMvYLuOUOEAgjcHhTN = default(EditorPlatform);
				ZLxBPCOYpnWMFyUkHwqRhHXJfokr = false;
				rvvbYaYPzXCpzaVOANmmIGydVGqkB = default(WebplayerPlatform);
				JEqdEGCnYXWbAhQtIofvdvHiDEjmb = default(ScriptingBackend);
				KXAAYRDiPGfgsamsAnFhXViMfeMEB = default(ScriptingAPILevel);
				NPnbNRVUZxeNWlruxcQoaUeoOZje = null;
			}
		}
		...
		{
			public enum rqjfHvpEgIFmRFWCNAPvQPNMResI
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly rqjfHvpEgIFmRFWCNAPvQPNMResI type;
			...
			private string dQgWEPUIhdVpENuFQOPZODOoLQfo(rqjfHvpEgIFmRFWCNAPvQPNMResI P_0)
			{
				return null;
			}
			...
			private static int spolHcupOxAmwoBwHjnRZHecmfqi(rqjfHvpEgIFmRFWCNAPvQPNMResI P_0)
			{
				return 0;
			}
		}

		private const UnityVersion wvvJbFIQBHfLFaKuqHnqaJsAlTaBc = UnityVersion.UNITY_5_0;

		private static UnityVersionClass RSUQYkiExRDVvCOSbKXxVQntgaPN;

		private static UnityVersion uJZYbueTJaqjUfeSFQfnUUQScqCt;

		private static string eXDzmDbKEUSlWcqkRsjEriNcIugBA;

		private static Platform QjrMSQVEfMfqcdiwWJIimFHNXeMOA;

		private static EditorPlatform AXyCIsDSKTetSEijQKyqchdbTjULb;

		private static bool ceJoEtbIwMdNnOTGepjQnGqRlGIt;

		private static bool ZyedrSLUdJjYpjvJlXTtJBlMdhvl;

		private static bool xDdUSTrXtjgqfeNLHSLMzGMExUs;

		private static WebplayerPlatform rmNRrEoOHGEukwxYSrSVeQjRhCJU;

		private static bool EBccFpZjFFISjbkYfQgfZkiEkoJU;

		private static bool NeUsDZRArnvVEQGpnQMcVhXABlui;

		private static bool nPqMjdbiSJEIHEWlDXlPgbgCZUTfA;

		private static bool OdMNeFhSvOPuTtQLYdsZDivWmCrc;

		private static bool hABiutwlZEbiibFSiGrEqbCzqfYIb;

		private static bool ijYLRUxjPJQcACCUVhAUFdowdXSy;

		private static string hdGNceAtEJJEPllyGwSpySkNHtRq;

		private static ScriptingBackend BKkbHIRsPhBSyONffgWSbmJFZkEP;

		private static ScriptingAPILevel YbHyzrwQeuwDOxbjZbVFhAQoXsFCA;

		private static bool gjVgCBhdMSEWXpdGXaLSIZrKCVew;

		private static IExternalTools tPLAOqTrXhXPiwBNAciRJlJzNsmDA;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper SDUEwRTzLQwcLObJMWQNnmwBsRYW;

		private static bool DlStoHWpXMXnBacuxfmmEdTbzCGb;
		...
		internal static IAndroidFallbackPlatformHelper qSrAkzBStYdNehHHytNpmvUmZpTY
		{
			[CompilerGenerated]
			get
			{
		...
		private static bool EdeZybXVKDFuFklnmcCgahAPlyhhA => false;

		private static bool EXUCvxpEGAcrEISbhIdoxaxWCKahA()
		{
			return false;
		}

		internal static void gRTTYDVoiZDulhaNXoRtgMVybMxeA(PiLPbHlhwYsdUvbqAeFitwleenxJ P_0)
		{
		}
		...
		private static bool JyFEDEdEAhjfyMSSiesiNEFrEBlsA(string P_0, out int P_1)
		{
			P_1 = default(int);
			return false;
		}

		private static void GPJwpXAogxpptfBIvwrBQxEgFrJm()
		{
		}

		private static UnityVersion uJMbgvcbRnUiyWpMXafqbNBUNrvm(string P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion DDEmXRPdTEgKTigPCJlZHWiRwUDF(int P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion csNQSfuOqSvGamgzCDMFmUsWKXID(int P_0)
		{
			return default(UnityVersion);
		}

		private static void VAafjYlZZIrgYcBQQlZDSNCOZrqk()
		{
		}

		internal static Type xifgWZqUYVpifdwmVMLzExqrpxlC(STQzvuuxiyFuMXTySTeCVASKokVg P_0)
		{
			return null;
		}

		private static Type sKfkfnNQbAKZRMHwCTONNxdhdZKi(STQzvuuxiyFuMXTySTeCVASKokVg P_0)
		{
			return null;
		}
		...
		private static _0001 tqkFRGcexqDrQpyGruhpBxJkjOcrA<_0001>(_0001 P_0) where _0001 : class
		{
			return null;
		}

		internal static ButtonStateFlags yTrLeNUivkuoNVnwiJsctAyWGsrgA(KeyCode P_0)
		{
			return default(ButtonStateFlags);
		}

		internal static ButtonStateFlags KGbDprSVvPuieyrdrHCQCVDQIvTZA(string P_0)
		{
			return default(ButtonStateFlags);
		}
	}
