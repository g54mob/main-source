...
namespace Rewired
{
	...
	{
		private enum GfxdbbIPEGqPYEzQaLNiBIffPZoKB
		{
			Error = 0,
			FoundIndex = 1,
			IsWholeElement = 2
		...
		private readonly DeviceLocalizationInfo NygUvJYdmXvOpZJKHcLVnffLuIWg;
		...
		private int BBuzzHZHccPstoEqdNUXCoCVRDJy(ControllerElementIdentifier[] P_0, int P_1)
		{
			return 0;
		}

		private static GfxdbbIPEGqPYEzQaLNiBIffPZoKB dsJBtSPAGoPSInGxrfNRaIvCWeUv(HardwareJoystickMap.CompoundElement P_0, ControllerElementIdentifier P_1, IList<ActionElementMap> P_2, out int P_3)
		{
			P_3 = default(int);
			return default(GfxdbbIPEGqPYEzQaLNiBIffPZoKB);
		}
	}
}
