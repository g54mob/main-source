	...
	public class PlayerController : IPlayerController
	{
		...
		public class Axis : ElementWithSource
		{
			public new class Definition : ElementWithSource.Definition
			{
				...
				internal override Element nhJyBmakXOGJlTVbMwuDiqHLBNHFA(PlayerController P_0)
				{
					return null;
				}
			}

			internal const float nPVKVzxBAfNarsiaQYAqsjupqDZN = 1f;

			internal const AbsoluteToRelativeScalingMode eungylMYDShSmKKOeWaEkDGXdYfiA = AbsoluteToRelativeScalingMode.None;

			[CustomObfuscation(rename = false)]
			internal const AxisCoordinateMode defaultAxisCoordinateMode = AxisCoordinateMode.Absolute;

			private float BQBjFnYAIXWRcLbKElVksVBaSNVK;

			private AxisCoordinateMode yXddLIwwnRByWzkRkrNuVoiaZwXH;

			private AbsoluteToRelativeScalingMode BMuTowRJCuABcFLspQTxnMnKqqkAb;
		...
		public class Axis2D : CompoundElement
		{
			...
			{
				private Axis.Definition XXvIjgbOfuUMHuWtuOXlERjgmLxs;

				private Axis.Definition gxgvHSHbmVnOGWGudHHUEyveRFMx;
			...
			internal const int oGMELUwESlbcMIGAVnbAOsleAIZTA = 0;

			internal const int nSSbsnjyAXsLiqcXqDlRZjjIWqBw = 1;

			internal const int aRlkuluzKnbmGAGokGaSNNHAVGuy = 2;
		...
		public abstract class CompoundElement : Element
		{
			...
			private readonly List<Element> LOnvqVgMXqRJqJIhmPpzCijiEmFi;

			internal int NTAiuTbXPrdScnWHJhjupuPxwFqu => 0;
			...
			internal _0001 BvrXvmkPPALmrSjaqiawgFXGMcvRA<_0001>(int P_0) where _0001 : Element
			{
				return null;
			}

			internal void CDyGAVpeIcJDZsykUpTcBFREfiDe(List<Element> P_0)
			{
			}

			internal void pZOeRrKdFeBWjBSrTADNbhEjOQxfA(Element P_0)
			{
			}
		}
		...
		public abstract class Element
		{
			...
			public abstract class Definition
			{
				...
				internal abstract Element nhJyBmakXOGJlTVbMwuDiqHLBNHFA(PlayerController P_0);
			}

			internal struct EjEWldkeKfAxHTexhZGGWhZlgpvm
			{
				public ControllerElementType YffDSNlVhopgKxdkfUZCXCDCNQjF;

				public int tgiLonNXHRtyhnQXrRSIydcwVwFp;

				public float xBSMpySKvRzQzuxwqliVDdkSCchV;

				public EjEWldkeKfAxHTexhZGGWhZlgpvm(ControllerElementType P_0, int P_1, float P_2)
				{
					YffDSNlVhopgKxdkfUZCXCDCNQjF = default(ControllerElementType);
					tgiLonNXHRtyhnQXrRSIydcwVwFp = 0;
					xBSMpySKvRzQzuxwqliVDdkSCchV = 0f;
				}
			}
			...
			private readonly PlayerController vBRiPvmyEtfpjyCwbpTpOrzXQboA;

			private bool IKdLDZsvaRQMRiosalSqWtXEgQQI;

			private bool LwFVXxJdTFqLXrhteqROhfNfGIKgA;

			private string WbCgBuXxniJdxJHChMPFPaxpuXxu;

			private static int[] ZdTigUdhjMKIGppCrmOKBQNFcLAIc;

			private static int[] NiELITIJTdeMdIrxmvhKhmZABYUy;

			protected Player player => null;

			protected bool selfAndParentEnabled => false;

			internal bool FTjFeBoBtbxQDqnajuImprkXJRtm
			{
				get
				{
					return false;
			...
			internal virtual void TzTZvYvnqpPbcwBGWROEUfiBCTpX()
			{
			}
		...
		public abstract class ElementWithSource : Element
		{
			public new abstract class Definition : Element.Definition
			{
				private int ePzOgkYByFIHczaDLJxlyfCmLczB;
			...
			private int ZASafJBFzgHViOFnomVErDPlvvAPA;
		...
		public sealed class MouseWheelAxis : Axis
		{
			...
			private const float mLIHCMUVJweQtghaQLxDnBOetfUS = 0.01f;

			private float tHhOesDYranxFkJYOaQROSKrhGqS;

			private double SvvKUmWrijhPUwkYBEAyhaxpXWQm;

			private float ZyHepkiETouXmsBmZCBhpZKCpoJR;
			...
			internal override void TzTZvYvnqpPbcwBGWROEUfiBCTpX()
			{
			}
			...
			private float VpibbmaBNrQZOFscktVYAPvMjFRWA()
			{
				return 0f;
			}

			private void cESWfhzNZdBoivWqpyozFYZWjbpx()
			{
			}
		}

		internal readonly int ZCAmNEnrzRPimZHxgczZcYOzBZkz;

		private bool iDSCNeaXNQFKhmLCYLmAiTYJHFEYB;

		private int MeuuqYCwCCztKqAHnbPbkuBfppqD;

		private readonly AList<Element> pGZEXVuikNfbPgaVmtPtNxuoHIEW;

		private readonly AList<Button> nUXzAwHViDCvrIzXJOZVASTAqwth;

		private readonly AList<Axis> YJjrLlcvcCvVcdqLnjarczySshhWA;

		private readonly ReadOnlyCollection<Element> jAUXxpfdisBhyyxRUALBAsoIHRfo;

		private readonly ReadOnlyCollection<Button> zvCAdNPIlzZlTkbdVFgCVowmuvzJ;

		private readonly ReadOnlyCollection<Axis> FyyUyAeacTwBTEmiFHXYyHzUPqks;

		private readonly List<Element.EjEWldkeKfAxHTexhZGGWhZlgpvm> qipGCQkoGpVjXJLxtKDecYBdNBrWb;

		private Action<int, bool> DriAcHjnEioyLnXIFqirfryzKDrEA;

		private Action<int, float> kTQKqljRsfPcsaVlWFoETDrhKQqA;

		private Action<bool> zZCauTWVAbgxZZHuPaVSoOioIcVb;

		private static Vector2 tGrhXAWAZucgprLJqWmwQzWzrRBc;
		...
		internal Player NdQQvJPfUKHWaEAWGCStSlpWTnLe => null;
		...
		private void lQEqUbNhfDgSuSCeXhGAqWEYsvTw(UpdateLoopType P_0)
		{
		}
		...
		internal void xrKiLyeMqDypdChAoSMjFbColQgL(Element P_0)
		{
		}

		private void FnubBmKIacZIPnPZglbxSsMZmaFH(Element P_0, List<Element> P_1, List<Element> P_2, List<Button> P_3, List<Axis> P_4)
		{
		}

		internal static int nNMoZOVhhapYVHOlqLBtKEGLbkRB<_0001>(IList<_0001> P_0, Predicate<_0001> P_1, int P_2) where _0001 : Element
		{
			return 0;
		}
	}
