	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				float num24 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType == TMP_TextElementType.Character) ? num2 : m_textInfo.characterInfo[m_characterCount].scale) + m_baselineOffset;
				...
				float num25 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType == TMP_TextElementType.Character) ? num2 : m_textInfo.characterInfo[m_characterCount].scale) + m_baselineOffset;
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.CaplineFlush:
				{
					...
					if ((!char.IsControl(character3) && lineNumber < m_lineNumber) || flag13 || tMP_LineInfo.maxAdvance > tMP_LineInfo.width)
					{
						...
						int num55 = (characterInfo[tMP_LineInfo.lastCharacterIndex].isVisible ? tMP_LineInfo.spaceCount : (tMP_LineInfo.spaceCount - 1));
						if (flag10)
						{
							num55--;
							num54++;
				...
				{
					num43 = ((j == m_characterCount - 1 && char.IsLetterOrDigit(character2)) ? j : (j - 1));
					flag11 = false;
				...
				if ((m_textInfo.characterInfo[j].style & FontStyles.Underline) == FontStyles.Underline)
				{
					...
					if (!char.IsWhiteSpace(character2) && character2 != '\u200b')
					{
						...
						num49 = Mathf.Min((pageNumber == num50) ? num49 : TMP_Text.k_LargePositiveFloat, m_textInfo.characterInfo[j].baseLine + base.font.fontInfo.Underline * num48);
						num50 = pageNumber;
					}
			...
			m_textInfo.lineCount = (short)num40;
			m_textInfo.wordCount = ((num39 == 0 || m_characterCount <= 0) ? 1 : ((short)num39));
