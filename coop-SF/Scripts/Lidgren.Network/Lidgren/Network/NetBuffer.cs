...
namespace Lidgren.Network
{
	public class NetBuffer
	{
		...
		public string ReadString()
		{
			...
			if ((m_readPosition & 7) == 0)
			{
				string result = Encoding.UTF8.GetString(m_data, m_readPosition >> 3, num);
				m_readPosition += 8 * num;
				return result;
			}
		...
		{
			uint result2;
			if (!ReadVariableUInt32(out result2))
			{
				result = string.Empty;
				return false;
			}
			...
			byte[] result3;
			if (!ReadBytes((int)result2, out result3))
			{
				result = string.Empty;
				return false;
			}
		...
		public void ReadAllFields(object target, BindingFlags flags)
		{
			...
			{
				MethodInfo value;
				if (s_readMethods.TryGetValue(fieldInfo.FieldType, out value))
				{
		...
		public void ReadAllProperties(object target, BindingFlags flags)
		{
			...
			{
				MethodInfo value;
				if (s_readMethods.TryGetValue(propertyInfo.PropertyType, out value))
				{
		...
		public void WriteAllFields(object ob, BindingFlags flags)
		{
			...
			FieldInfo[] fields = ob.GetType().GetFields(flags);
			NetUtility.SortMembersList(fields);
			FieldInfo[] array = fields;
			foreach (FieldInfo fieldInfo in array)
			{
				...
				MethodInfo value2;
				if (s_writeMethods.TryGetValue(fieldInfo.FieldType, out value2))
				{
		...
		public void WriteAllProperties(object ob, BindingFlags flags)
		{
			...
			PropertyInfo[] properties = ob.GetType().GetProperties(flags);
			NetUtility.SortMembersList(properties);
			PropertyInfo[] array = properties;
			foreach (PropertyInfo propertyInfo in array)
			{
				...
				if (getMethod != null)
				{
					...
					MethodInfo value;
					if (s_writeMethods.TryGetValue(propertyInfo.PropertyType, out value))
					{
