...
namespace Lidgren.Network
{
	...
	public class NetConnection
	{
		...
		public long RemoteUniqueIdentifier
		{
			get
			{
				return m_remoteUniqueIdentifier;
			}
		}
		...
		public float AverageRoundtripTime
		{
			get
			{
				return (float)m_averageRoundtripTime;
			}
		}
		...
		public float RemoteTimeOffset
		{
			get
			{
				return (float)m_remoteTimeOffset;
			}
		}
		...
		internal void Heartbeat(double now, uint frameCounter)
		{
			...
			if (frameCounter % 3 == 0)
			{
				while (m_queuedOutgoingAcks.Count > 0)
				{
					...
					for (int i = 0; i < num; i++)
					{
						NetTuple<NetMessageType, int> item;
						m_queuedOutgoingAcks.TryDequeue(out item);
				...
				{
					NetSenderChannelBase netSenderChannelBase = m_sendChannels[(uint)(item2.Item1 - 1)];
					if (netSenderChannelBase != null)
					{
						netSenderChannelBase.ReceiveAcknowledge(now, item2.Item2);
					}
				}
			}
			if (m_peer.m_executeFlushSendQueue)
			{
				...
				{
					NetSenderChannelBase netSenderChannelBase2 = m_sendChannels[num3];
					if (netSenderChannelBase2 != null)
					{
						netSenderChannelBase2.SendQueuedMessages(now);
						if (netSenderChannelBase2.NeedToSendMessages())
						{
		...
		private NetSenderChannelBase CreateSenderChannel(NetMessageType tp)
		{
			...
			lock (m_sendChannels)
			{
				...
				{
					netSenderChannelBase = m_sendChannels[num2];
				}
				else
				{
		...
		{
			return string.Concat("[NetConnection to ", m_remoteEndPoint, "]");
		}
		...
		internal void ExecuteDisconnect(string reason, bool sendByeMessage)
		{
			...
			{
				NetSenderChannelBase netSenderChannelBase = m_sendChannels[i];
				if (netSenderChannelBase != null)
				{
		...
		internal void SendPing()
		{
			...
			bool connectionReset;
			m_peer.SendPacket(numBytes, m_remoteEndPoint, 1, out connectionReset);
		...
		internal void ReceivedPong(double now, int pongNumber, float remoteSendTime)
		{
			...
			{
				NetReliableSenderChannel netReliableSenderChannel = sendChannels[i] as NetReliableSenderChannel;
