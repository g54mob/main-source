...
namespace Rewired.HID.Drivers
{
	...
	internal abstract class HIDDeviceDriver : IControllerDriver, IDisposable
	{
		...
		internal interface IHIDDevice
		{
			...
			bool WriteSync(dccInhMggZtLYGkWFjXacEyGQoUL outputReport, int timeoutMs);

			void WriteAsync(dccInhMggZtLYGkWFjXacEyGQoUL outputReport, int timeoutMs);
		...
		internal class InitArgs
		{
			...
			public readonly RXEzGxJeQkuaNxkYCJIkKyWznLNi connectionType;
			...
			public InitArgs(UpdateLoopSetting P_0, RXEzGxJeQkuaNxkYCJIkKyWznLNi P_1, int P_2, int P_3, int P_4, int P_5, IHIDDevice P_6)
			{
		...
		public nZeIQQWnQohhanyhWEOObGRunlRc[] axes;

		public YgmprUEDpDakYucBfpnWbXzouOGJ[] buttons;

		public XXzPjtyGkCdrTJCzxAmvdoaeCgbHb[] hats;

		public cMLqHjOwHUDOjQfvBFTMHfOrKnXJ[] accelerometers;

		public mtYfxDYuHHPxAtRRwphKvfBUCHvHA[] gyroscopes;

		public SRlmwzCpkDCiOPGALkZGROsZKGfx[] touchpads;

		public pMGtGvfvhFCynWDpoUnlyTrPulZp[] vibrationMotors;

		public iaSQTyJQfafVqZneFJUiRVRBDWdc[] lights;

		private bool OLMiVcaNIfbxxeSFNMSQYDTeQMHeb;
		...
		protected bool disposed => OLMiVcaNIfbxxeSFNMSQYDTeQMHeb;
		...
		public static HIDDeviceDriver GetDriver(DriverType driverId, InitArgs hidDriverInitArgs)
		{
			...
			case DriverType.SwitchProController:
				if (hidDriverInitArgs.connectionType != RXEzGxJeQkuaNxkYCJIkKyWznLNi.Bluetooth)
				{
					return null;
				}
		...
		{
			if (!OLMiVcaNIfbxxeSFNMSQYDTeQMHeb)
			{
				OLMiVcaNIfbxxeSFNMSQYDTeQMHeb = true;
			}
		}
	}
}
