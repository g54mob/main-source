...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_MMSoundManagerSound : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			float intensity = ComputeIntensity(feedbacksIntensity);
			if (Sfx != null)
			{
		...
		{
			if (!DoNotPlayIfClipAlreadyPlaying || ((!(_playedAudioSource != null) || !_playedAudioSource.isPlaying) && !(MMPersistentSingleton<MMSoundManager>.Instance.FindByClip(sfx) != null)))
			{
