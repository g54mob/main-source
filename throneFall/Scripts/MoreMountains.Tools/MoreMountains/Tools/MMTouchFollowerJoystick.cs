...
namespace MoreMountains.Tools
{
	...
	public class MMTouchFollowerJoystick : MMTouchJoystick
	{
		...
		public override void OnDrag(PointerEventData eventData)
		{
			...
			if (num >= MaxRange)
			{
				_backgroundPositionTarget = BackgroundCanvasGroup.transform.position + (_knobTransform.position - BackgroundCanvasGroup.transform.position).normalized * (num - MaxRange);
			}
			ConstrainBackground();
			ComputeJoystickValue();
		}
