...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Material : MMF_Feedback
	{
		...
		{
			if (ComputedTimescaleMode != TimescaleModes.Scaled)
			{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			else if (InterpolateTransition)
			{
				...
				{
					_coroutines[i] = Owner.StartCoroutine(TransitionMaterial(TargetRenderer.materials[RendererMaterialIndexes[i]], Materials[index], RendererMaterialIndexes[i]));
				}
			}
			else
			{
		...
		protected virtual int DetermineNextIndex()
		{
			...
			case Methods.Sequential:
				...
				{
					_currentIndex = ((!Loop) ? (_currentIndex - 1) : 0);
				}
				return _currentIndex;
			default:
				return 0;
