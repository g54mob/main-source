...
namespace Pathfinding
{
	public class RichFunnel : RichPathPart
	{
		...
		public float DistanceToEndOfPath
		{
			get
			{
				Vector3 vector = CurrentNode?.ClosestPointOnNode(currentPosition) ?? currentPosition;
		...
		public void BuildFunnelCorridor(List<GraphNode> nodes, int start, int end)
		{
			...
			if (funnelSimplification)
			{
				...
				int[] tagPenalties = ((path.seeker != null) ? path.seeker.tagPenalties : Path.ZeroTagPenalties);
				int traversableTags = ((path.seeker != null) ? path.seeker.traversableTags : (-1));
				...
				}, graph, nodes, list, tagPenalties, traversableTags);
			...
			{
				if (this.nodes[k].GetPortal(this.nodes[k + 1], out var item3, out var item4))
				{
					left.Add(item3);
					right.Add(item4);
					continue;
				}
				left.Add((Vector3)this.nodes[k].position);
				right.Add((Vector3)this.nodes[k].position);
				left.Add((Vector3)this.nodes[k + 1].position);
				right.Add((Vector3)this.nodes[k + 1].position);
			}
			left.Add(exactEnd);
			right.Add(exactEnd);
		}
		...
		private void UpdateFunnelCorridor(int splitIndex, List<TriangleMeshNode> prefix)
		{
			...
			{
				if (nodes[i].GetPortal(nodes[i + 1], out var item, out var item2))
				{
					left.Add(item);
					right.Add(item2);
				}
		...
		public Vector3 Update(Vector3 position, List<Vector3> buffer, int numCorners, out bool lastCorner, out bool requiresRepath)
		{
			...
			bool num = ClampToNavmeshInternal(ref position);
		...
		private bool ClampToNavmeshInternal(ref Vector3 position)
		{
			...
			while (queue.Count > 0)
			{
				...
				{
					if (triangleMeshNode3.connections[i].isOutgoing && triangleMeshNode3.connections[i].node is TriangleMeshNode { TemporaryFlag1: false } triangleMeshNode4)
					{
			...
			{
				return nodes[currentNode + 1].Destroyed;
			}
			return false;
		}
				...
				if (!(num < 0 && flag) && !(num > 0 && flag2))
				{
					...
					else
					{
						...
						else
						{
							...
							{
								if (!(triangleMeshNode2.connections[j].node is TriangleMeshNode triangleMeshNode4))
								{
									continue;
								}
								int num2 = -1;
