...
namespace Pathfinding.Serialization
{
	public class AstarSerializer
	{
		...
		{
			Type graphType = meta.GetGraphType(zipIndex, availableGraphTypes);
			...
			{
				TinyJsonDeserializer.Deserialize(GetString(GetEntry(name)), graphType, navGraph, contextRoot);
		...
		public NavGraph[] DeserializeGraphs(Type[] availableGraphTypes)
		{
			...
			for (int i = 0; i < meta.graphs; i++)
			{
				...
				NavGraph navGraph = DeserializeGraph(i, graphIndex, availableGraphTypes);
				if (navGraph != null)
				{
					list.Add(navGraph);
		...
		private void DeserializeAndRemoveOldNodeLinks(GraphSerializationContext ctx)
		{
			...
			for (int j = 0; j < graphs.Length; j++)
			{
				...
				if (graphs[j] != null && graphs[j] is PointGraph pointGraph)
				{
					...
					{
						((IGraphInternals)graphs[j]).DestroyAllNodes();
		...
		private void DeserializeExtraInfo()
		{
			...
			GraphNode[] array = DeserializeNodeReferenceMap();
			for (int j = 0; j < graphs.Length; j++)
			{
				DeserializeNodeReferences(graphs[j], array);
			}
			if (meta.version < V4_3_85)
			{
				...
				if (entry != null)
				{
					GraphSerializationContext ctx = new GraphSerializationContext(GetBinaryReader(entry), array, 0u, meta);
		...
		private void DeserializeEditorSettingsCompatibility()
		{
			for (int i = 0; i < graphs.Length; i++)
			{
				ZipEntry entry = GetEntry("graph" + graphIndexInZip[graphs[i]] + "_editor.json");
				if (entry != null)
				{
