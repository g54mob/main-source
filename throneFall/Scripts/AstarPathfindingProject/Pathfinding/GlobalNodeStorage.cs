...
namespace Pathfinding
{
	internal class GlobalNodeStorage
	{
		...
		{
			PathfindingThreadData pathfindingThreadData = this.pathfindingThreadData[threadID];
			int num = pathfindingThreadData.pathNodes.Length - (int)reservedPathNodeData;
			temporaryNodeCount = Math.Max(temporaryNodeCount, num * 8);
			int length = pathfindingThreadData.pathNodes.Length;
			pathfindingThreadData.pathNodes = pathfindingThreadData.pathNodes.Reallocate(Allocator.Persistent, (int)reservedPathNodeData + temporaryNodeCount);
