...
namespace Pathfinding
{
	...
	public abstract class NavmeshBase : NavGraph, INavmeshHolder, ITransformedGraph, IRaycastableGraph
	{
		...
		[Obsolete("Use GetTouchingTiles instead. This method will be removed in a future update.")]
			...
			for (int i = -1; i <= 1; i++)
			{
				...
				for (int j = -1; j <= 1; j++)
				{
					...
					if (num2 >= 0 && num2 < tileXCount && j == 0 != (i == 0))
					{
						...
						{
							ConnectTiles(navmeshTile, tile, TileWorldSizeX, TileWorldSizeZ, MaxTileConnectionEdgeDistance);
						}
					}
				}
			}
		...
		public override float NearestNodeDistanceSqrLowerBound(Vector3 position, NNConstraint constraint)
		{
			...
			return new BBTree.ProjectionParams(constraint, transform).SquaredRectPointDistanceOnPlane(new IntRect(0, 0, (int)((float)(1000 * tileXCount) * TileWorldSizeX), (int)((float)(1000 * tileZCount) * TileWorldSizeZ)), p);
		}
		...
		public override NNInfo GetNearest(Vector3 position, NNConstraint constraint, float maxDistanceSqr)
		{
			...
			float3 float5 = transform.InverseTransform(position);
			int value = (int)(float5.x / TileWorldSizeX);
			int value2 = (int)(float5.z / TileWorldSizeZ);
			value = Mathf.Clamp(value, 0, tileXCount - 1);
			value2 = Mathf.Clamp(value2, 0, tileZCount - 1);
			int num = Math.Max(tileXCount, tileZCount);
			NNInfo previous = NNInfo.Empty;
			float distanceSqr = maxDistanceSqr;
			BBTree.ProjectionParams projection = new BBTree.ProjectionParams(constraint, transform);
			float num2 = Math.Min(TileWorldSizeX, TileWorldSizeX);
			for (int i = 0; i < num; i++)
			{
				int num3 = Math.Min(i + value2 + 1, tileZCount);
				for (int j = Math.Max(-i + value2, 0); j < num3; j++)
				{
					int num4 = Math.Abs(i - Math.Abs(j - value2));
					int num5 = num4;
					do
					{
						int num6 = -num5 + value;
						if (num6 >= 0 && num6 < tileXCount)
						{
							NavmeshTile navmeshTile = tiles[num6 + j * tileXCount];
							if (navmeshTile != null && navmeshTile.bbTree.DistanceSqrLowerBound(float5, in projection) <= distanceSqr)
							{
								ref BBTree bbTree = ref navmeshTile.bbTree;
								float3 p = float5;
								GraphNode[] nodes = navmeshTile.nodes;
								bbTree.QueryClosest(p, constraint, in projection, ref distanceSqr, ref previous, nodes, navmeshTile.tris, navmeshTile.vertsInGraphSpace);
							}
						}
						num5 = -num5;
					}
				...
				int num7 = i + 1;
				float num8 = (float)math.max(0, num7 - 2) * num2;
				if (projection.alignedWithXZPlane && distanceSqr - 1E-05f <= num8 * num8)
				{
					break;
				}
			}
			...
			{
				previous = new NNInfo(previous.node, transform.Transform(previous.position), previous.distanceCostSqr);
			}
			return previous;
		}
		...
		public GraphNode PointOnNavmesh(Vector3 position, NNConstraint constraint)
		{
			...
			GraphNode node = GetNearest(position, constraint, 0f).node;
			...
			foreach (TriangleMeshNode triangleMeshNode2 in nodes)
			{
				...
				for (int l = 0; l < vertexCount2; l++)
				{
					...
					for (int m = 0; m < vertexCount3; m++)
					{
						...
						{
							list.Add(new Connection(triangleMeshNode3, (uint)(triangleMeshNode2.position - triangleMeshNode3.position).costMagnitude, Connection.PackShapeEdgeInfo((byte)l, (byte)m, areEdgesIdentical: true, isOutgoing: true, isIncoming: true)));
							break;
						}
					}
				}
						...
						for (int num12 = 0; num12 < vertexCount3; num12++)
						{
							...
							if (num13 != num14 && num11 > num13 && num10 < num14)
							{
								...
								if (flag || VectorMath.SqrDistanceSegmentSegment((Vector3)vertexInGraphSpace3, (Vector3)vertexInGraphSpace4, (Vector3)vertexInGraphSpace5, (Vector3)vertexInGraphSpace6) < maxTileConnectionEdgeDistance * maxTileConnectionEdgeDistance)
								{
									...
									triangleMeshNode2.AddPartialConnection(triangleMeshNode3, costMagnitude, Connection.PackShapeEdgeInfo((byte)m, (byte)num12, flag, isOutgoing: true, isIncoming: true));
									triangleMeshNode3.AddPartialConnection(triangleMeshNode2, costMagnitude, Connection.PackShapeEdgeInfo((byte)num12, (byte)m, flag, isOutgoing: true, isIncoming: true));
								}
							}
						}
					}
		...
		public void EndBatchTileUpdate()
		{
			...
			for (int j = 0; j < batchUpdatedTiles.Count; j++)
			{
				...
				rect = ((j != 0) ? rect.ExpandToContain(num, num2) : new IntRect(num, num2, num, num2));
				if (num > 0)
				{
			...
			DirtyBounds(GetTileBounds(rect));
		}
		...
		public void ClearTiles(IntRect tileRect)
		{
			...
			tileRect = IntRect.Intersection(b: new IntRect(0, 0, tileXCount - 1, tileZCount - 1), a: tileRect);
			for (int i = tileRect.ymin; i <= tileRect.ymax; i++)
			{
				for (int j = tileRect.xmin; j <= tileRect.xmax; j++)
				{
					ClearTile(j, i, NewEmptyTile(j, i));
				}
			}
			if (!flag)
			{
		...
		private void PrepareNodeRecycling(int x, int z, UnsafeSpan<Int3> verts, UnsafeSpan<int> tris, TriangleMeshNode[] recycledNodeBuffer)
		{
			...
			{
				dictionary[(verts[tris[num]].GetHashCode() * 31) ^ (verts[tris[num + 1]].GetHashCode() * 196613) ^ (verts[tris[num + 2]].GetHashCode() * 3145739)] = num2;
				num += 3;
				num2++;
			}
			List<Connection> list = ListPool<Connection>.Claim();
			for (int i = 0; i < nodes.Length; i++)
			{
				...
				int key = (v.GetHashCode() * 31) ^ (v2.GetHashCode() * 196613) ^ (v3.GetHashCode() * 3145739);
		...
		public void ReplaceTile(int x, int z, Int3[] verts, int[] tris, uint[] tags = null, bool tryPreserveExistingTagsAndPenalties = true)
		{
			...
			UnsafeSpan<int> unsafeSpan = new UnsafeSpan<int>(Allocator.Persistent, tris.Length);
			unsafeSpan.CopyFrom(tris);
			UnsafeSpan<Int3> unsafeSpan2 = new UnsafeSpan<Int3>(Allocator.Persistent, verts.Length);
			unsafeSpan2.CopyFrom(verts);
			Int3 int5 = (Int3)new Vector3((float)x * TileWorldSizeX, 0f, (float)z * TileWorldSizeZ);
			for (int i = 0; i < verts.Length; i++)
			{
				unsafeSpan2[i] += int5;
			}
			UnsafeSpan<Int3> unsafeSpan3 = unsafeSpan2.Clone(Allocator.Persistent);
			transform.Transform(unsafeSpan3);
			...
			PrepareNodeRecycling(x, z, unsafeSpan2, unsafeSpan, navmeshTile.nodes);
			ClearTile(x, z, null);
			...
			ulong gcHandle = 0uL;
			UnsafeSpan<uint> tags2 = ((tags != null) ? new UnsafeSpan<uint>(tags, out gcHandle) : default(UnsafeSpan<uint>));
			CreateNodes(navmeshTile, navmeshTile.tris, x + z * tileXCount, (uint)active.data.GetGraphIndex(this), tags2, initializeNodes: true, active, initialPenalty, tryPreserveExistingTagsAndPenalties);
			if (tags != null)
			{
				UnsafeUtility.ReleaseGCObject(gcHandle);
			}
			CreateNodeConnections(navmeshTile.nodes, keepExistingConnections: true);
			if (flag)
			{
				EndBatchTileUpdate();
			}
		...
		internal static void CreateNodes(NavmeshTile tile, UnsafeSpan<int> tris, int tileIndex, uint graphIndex, UnsafeSpan<uint> tags, bool initializeNodes, AstarPath astar, uint initialPenalty, bool tryPreserveExistingTagsAndPenalties)
		{
			...
			for (int i = 0; i < nodes.Length; i++)
			{
				...
				{
					flag = true;
					triangleMeshNode = ((!initializeNodes) ? (nodes[i] = new TriangleMeshNode()) : (nodes[i] = new TriangleMeshNode(astar)));
				}
				...
				triangleMeshNode.position = (tile.GetVertex(triangleMeshNode.v0) + tile.GetVertex(triangleMeshNode.v1) + tile.GetVertex(triangleMeshNode.v2)) * (1f / 3f);
			}
		}
		...
		public static bool Linecast(NavmeshBase graph, Vector3 origin, Vector3 end, GraphNode hint, out GraphHitInfo hit, List<GraphNode> trace, Func<GraphNode, bool> filter = null)
		{
			...
			if (triangleMeshNode == null)
			{
				NNInfo nearest = graph.GetNearest(origin, NNConstraintNoneXZ);
			...
			Int3 int5 = triangleMeshNode.ClosestPointOnNodeXZInGraphSpace(origin);
			hit.origin = graph.transform.Transform((Vector3)int5);
			...
			Int3 int6 = (Int3)graph.transform.InverseTransform(end);
			if (int5 == int6)
			{
				hit.point = hit.origin;
				hit.node = triangleMeshNode;
			...
			do
			{
				...
				int num2 = (int)VectorMath.SideXZ(int5, int6, v);
				num2 |= (int)((uint)VectorMath.SideXZ(int5, int6, v2) << 2);
				num2 |= (int)((uint)VectorMath.SideXZ(int5, int6, v3) << 4);
				...
				{
					if (connections[i].isEdgeShared && connections[i].isOutgoing && connections[i].shapeEdge == num3 && connections[i].node is TriangleMeshNode { Walkable: not false } triangleMeshNode3 && (filter == null || filter(triangleMeshNode3)))
					{
						int adjacentShapeEdge = connections[i].adjacentShapeEdge;
						Side side = VectorMath.SideXZ(int5, int6, triangleMeshNode3.GetVertexInGraphSpace(adjacentShapeEdge));
						Side side2 = VectorMath.SideXZ(int5, int6, triangleMeshNode3.GetVertexInGraphSpace((adjacentShapeEdge + 1) % 3));
			...
			Vector3 point = VectorMath.LineIntersectionPointXZ(vector, vector2, (Vector3)int5, (Vector3)int6);
		...
		public override void OnDrawGizmos(DrawingData gizmos, bool drawNodes, RedrawScope redrawScope)
		{
			...
			using (CommandBuilder commandBuilder = gizmos.GetBuilder(redrawScope))
			{
				Bounds bounds = default(Bounds);
				bounds.SetMinMax(Vector3.zero, forcedBoundsSize);
				using (commandBuilder.WithMatrix(CalculateTransform().matrix))
				{
					commandBuilder.WireBox(bounds, Color.white);
				}
			}
			if (tiles != null && (showMeshSurface || showMeshOutline || showNodeConnections))
			{
				NodeHasher nodeHasher = new NodeHasher(active);
				nodeHasher.Add(showMeshOutline ? 1 : 0);
				nodeHasher.Add(showMeshSurface ? 1 : 0);
				nodeHasher.Add(showNodeConnections ? 1 : 0);
				int num = 0;
				...
				for (int i = 0; i < tiles.Length; i++)
				{
					...
					if (!gizmos.Draw(nodeHasher2, redrawScope))
					{
						using GraphGizmoHelper graphGizmoHelper = GraphGizmoHelper.GetGizmoHelper(gizmos, active, nodeHasher2, redrawScope);
						if (showMeshSurface || showMeshOutline)
						{
							CreateNavmeshSurfaceVisualization(tiles, num, i + 1, graphGizmoHelper);
							CreateNavmeshOutlineVisualization(tiles, num, i + 1, graphGizmoHelper);
						}
						if (showNodeConnections)
						{
							...
							for (int k = num; k <= i; k++)
							{
								if (tiles[k] != null)
								{
									...
									{
										graphGizmoHelper.DrawConnections(nodes2[l]);
									}
								}
							}
							if (graphGizmoHelper.showSearchTree)
		...
		protected override void DeserializeExtraInfo(GraphSerializationContext ctx)
		{
			...
			for (int i = 0; i < tileZCount; i++)
			{
				for (int j = 0; j < tileXCount; j++)
				{
					...
					navmeshTile2.tris = new UnsafeSpan<int>(Allocator.Persistent, num5);
					for (int k = 0; k < navmeshTile2.tris.Length; k++)
					{
						navmeshTile2.tris[k] = reader.ReadInt32();
					}
					navmeshTile2.verts = new UnsafeSpan<Int3>(Allocator.Persistent, reader.ReadInt32());
					...
					{
						navmeshTile2.vertsInGraphSpace = new UnsafeSpan<Int3>(Allocator.Persistent, reader.ReadInt32());
					...
					{
						navmeshTile2.vertsInGraphSpace = new UnsafeSpan<Int3>(Allocator.Persistent, navmeshTile2.verts.Length);
						navmeshTile2.verts.CopyTo(navmeshTile2.vertsInGraphSpace);
		...
		protected override void PostDeserialization(GraphSerializationContext ctx)
		{
			if (ctx.meta.version < AstarSerializer.V4_3_68 && tiles != null)
			{
				...
				{
					CreateNodeConnections(array[num].nodes, keepExistingConnections: false);
				}
				array = tiles;
				...
				GetNodes(delegate(GraphNode node)
				{
					...
					foreach (Connection item in conns[triNode].Where((Connection conn) => !triNode.ContainsOutgoingConnection(conn.node)).ToList())
					{
						triNode.AddPartialConnection(item.node, item.cost, item.shapeEdgeInfo);
					}
				});
			}
			transform = CalculateTransform();
