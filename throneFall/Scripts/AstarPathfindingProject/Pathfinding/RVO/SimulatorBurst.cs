	...
	public class SimulatorBurst
	{
		...
		public JobHandle Update(JobHandle dependency, float dt, bool drawGizmos, Allocator allocator)
		{
			if (false)
			{
				...
				default(JobDestinationReached<XYMovementPlane>).Schedule();
				default(JobDestinationReached<XZMovementPlane>).Schedule();
				default(JobDestinationReached<ArbitraryMovementPlane>).Schedule();
			}
			if (movementPlane == MovementPlane.XY)
			{
		...
		private JobHandle UpdateInternal<T>(JobHandle dependency, float deltaTime, bool drawGizmos, Allocator allocator) where T : struct, IMovementPlaneWrapper
		{
			...
			JobHandle jobHandle = quadtree.BuildJob(simulationData.position, simulationData.version, outputData.speed, simulationData.radius, numAgents, movementPlane).Schedule(dependency);
			JobHandle job = new JobRVOPreprocess
			{
			...
			JobHandle jobHandle2 = new JobRVO<T>
			{
				agentData = simulationData,
				temporaryAgentData = temporaryAgentData,
			...
			}.ScheduleBatch(dependsOn: JobHandle.CombineDependencies(job3, job4, readLock.dependency), arrayLength: numAgents, minIndicesPerJobCount: minIndicesPerJobCount);
			if (num)
			{
			...
			JobHandle job5 = simulationData.collisionNormal.MemSet(float3.zero).Schedule(jobHandle3);
			JobHandle job6 = simulationData.manuallyControlled.MemSet(value: false).Schedule(jobHandle3);
			JobHandle job7 = simulationData.hierarchicalNodeIndex.MemSet(-1).Schedule(jobHandle3);
