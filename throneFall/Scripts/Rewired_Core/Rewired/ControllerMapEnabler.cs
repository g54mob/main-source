...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.mrHnssKPqvnBfcJsxqpLsnPxOquq)
				{
		...
		internal class ybCAJZdDMTpCNSaMwMfysgFQeUXm
		{
			public bool zzKbjvGaIBntFKdSUOiwzzRZUyweA;

			public bsCFPvFNILkrNbewcpeQWrFpdSUQB[] LAuRwLSLsXJSCfbqgikZiKWFsgMD;

			public ybCAJZdDMTpCNSaMwMfysgFQeUXm(bool P_0, bsCFPvFNILkrNbewcpeQWrFpdSUQB[] P_1)
			{
				zzKbjvGaIBntFKdSUOiwzzRZUyweA = P_0;
				LAuRwLSLsXJSCfbqgikZiKWFsgMD = P_1;
			}
		}

		private bool qoMLFdmvOnMhKsVnuJTRlkNbtzGi;

		private Player ScrLBZxbshAZmETUtmrCGRAwmWWKA;

		private ybCAJZdDMTpCNSaMwMfysgFQeUXm FyzCZQqGMTcTwePmsiMCJvSNjQILA;

		private readonly int UkWJRQwKNMETRenKzwjfTIZvLVMS;

		private List<RuleSet> FHCrhXmdCYedDSlbBZUlWpHRuwmn;

		public bool enabled
		{
			get
			{
				return qoMLFdmvOnMhKsVnuJTRlkNbtzGi;
			}
			set
			{
				qoMLFdmvOnMhKsVnuJTRlkNbtzGi = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return FHCrhXmdCYedDSlbBZUlWpHRuwmn;
			}
			set
			{
				if (value == null)
				...
				FHCrhXmdCYedDSlbBZUlWpHRuwmn = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, ybCAJZdDMTpCNSaMwMfysgFQeUXm P_1)
		{
			if (P_0 == null)
			{
			...
			UkWJRQwKNMETRenKzwjfTIZvLVMS = ReInput.id;
			ScrLBZxbshAZmETUtmrCGRAwmWWKA = P_0;
			FyzCZQqGMTcTwePmsiMCJvSNjQILA = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != UkWJRQwKNMETRenKzwjfTIZvLVMS)
			{
				ReInput.CheckInitialized(UkWJRQwKNMETRenKzwjfTIZvLVMS);
			}
			else
			{
				if (!qoMLFdmvOnMhKsVnuJTRlkNbtzGi || FHCrhXmdCYedDSlbBZUlWpHRuwmn == null)
				{
					return;
				}
				int count = FHCrhXmdCYedDSlbBZUlWpHRuwmn.Count;
				if (count == 0)
				{
					return;
				}
				...
				ScrLBZxbshAZmETUtmrCGRAwmWWKA.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = FHCrhXmdCYedDSlbBZUlWpHRuwmn[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((FyzCZQqGMTcTwePmsiMCJvSNjQILA != null && FyzCZQqGMTcTwePmsiMCJvSNjQILA.LAuRwLSLsXJSCfbqgikZiKWFsgMD != null) ? FyzCZQqGMTcTwePmsiMCJvSNjQILA.LAuRwLSLsXJSCfbqgikZiKWFsgMD.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(FyzCZQqGMTcTwePmsiMCJvSNjQILA.LAuRwLSLsXJSCfbqgikZiKWFsgMD[i].nXKxAkXnRYLlkmgwTSRoJOAaetXx);
				...
				controllerMapEnablerRuleSetInstance.enabled = FyzCZQqGMTcTwePmsiMCJvSNjQILA.LAuRwLSLsXJSCfbqgikZiKWFsgMD[i].BhLDjaqvsZwuenvhbTMJhNLObSwE;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (FyzCZQqGMTcTwePmsiMCJvSNjQILA != null)
			{
				qoMLFdmvOnMhKsVnuJTRlkNbtzGi = FyzCZQqGMTcTwePmsiMCJvSNjQILA.zzKbjvGaIBntFKdSUOiwzzRZUyweA;
			}
			FHCrhXmdCYedDSlbBZUlWpHRuwmn = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return ZGgfztHDSjEMvTIDcfQmDPVNEaZgb().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return ZGgfztHDSjEMvTIDcfQmDPVNEaZgb().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				CivryMVMtaAMlPPEmhNXWQfrVxXM(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				CivryMVMtaAMlPPEmhNXWQfrVxXM(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject ZGgfztHDSjEMvTIDcfQmDPVNEaZgb()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			YiwGrdmIOVnnvhqXuksaojGWXLIJ(serializedObject);
			return serializedObject;
		}

		private void YiwGrdmIOVnnvhqXuksaojGWXLIJ(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", qoMLFdmvOnMhKsVnuJTRlkNbtzGi);
			P_0.Add("ruleSets", FHCrhXmdCYedDSlbBZUlWpHRuwmn);
		}

		private bool CivryMVMtaAMlPPEmhNXWQfrVxXM(SerializedObject P_0)
		{
			qoMLFdmvOnMhKsVnuJTRlkNbtzGi = false;
			FHCrhXmdCYedDSlbBZUlWpHRuwmn = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref qoMLFdmvOnMhKsVnuJTRlkNbtzGi);
