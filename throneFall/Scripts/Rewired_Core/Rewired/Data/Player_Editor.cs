...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal cHjmmntdHIjqPzwYHqwnEXRwXGEG HIJGuQoeYYdTmFXTkgbKJhNTmdsjb()
			{
				return new cHjmmntdHIjqPzwYHqwnEXRwXGEG(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.ccDuPLOhlbrAqOTHJEHJSRpBmDEb wVKEIekxcMYBiRYsJEKMMHKDYgxqA()
			{
				return new ControllerMapLayoutManager.ccDuPLOhlbrAqOTHJEHJSRpBmDEb(_enabled, _loadFromUserDataStore, OrkAncvMWqownIQsWMrYsmTGRxWc());
			}

			private bsCFPvFNILkrNbewcpeQWrFpdSUQB[] OrkAncvMWqownIQsWMrYsmTGRxWc()
			{
				List<bsCFPvFNILkrNbewcpeQWrFpdSUQB> list = new List<bsCFPvFNILkrNbewcpeQWrFpdSUQB>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].BEnyeMtkvoszHmzLjVDHQBWlNvtu());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.ybCAJZdDMTpCNSaMwMfysgFQeUXm ezfZAXMwqAJQyPhVcbDVakfyCnBqA()
			{
				return new ControllerMapEnabler.ybCAJZdDMTpCNSaMwMfysgFQeUXm(_enabled, HqXRHpcLtJxRWtdiSMlLTjfRkPAF());
			}

			private bsCFPvFNILkrNbewcpeQWrFpdSUQB[] HqXRHpcLtJxRWtdiSMlLTjfRkPAF()
			{
				List<bsCFPvFNILkrNbewcpeQWrFpdSUQB> list = new List<bsCFPvFNILkrNbewcpeQWrFpdSUQB>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal bsCFPvFNILkrNbewcpeQWrFpdSUQB BEnyeMtkvoszHmzLjVDHQBWlNvtu()
			{
				return new bsCFPvFNILkrNbewcpeQWrFpdSUQB(_id, _enabled);
			}
		...
		internal wsTGaisyEgVqWobCcVTBhcYPpDji UJfCCdCTILqbCmYhhvIuqGgnoOQf()
		{
			cHjmmntdHIjqPzwYHqwnEXRwXGEG[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new cHjmmntdHIjqPzwYHqwnEXRwXGEG[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].HIJGuQoeYYdTmFXTkgbKJhNTmdsjb();
				}
			}
			cHjmmntdHIjqPzwYHqwnEXRwXGEG[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new cHjmmntdHIjqPzwYHqwnEXRwXGEG[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].HIJGuQoeYYdTmFXTkgbKJhNTmdsjb();
				}
			}
			cHjmmntdHIjqPzwYHqwnEXRwXGEG[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new cHjmmntdHIjqPzwYHqwnEXRwXGEG[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].HIJGuQoeYYdTmFXTkgbKJhNTmdsjb();
				}
			}
			cHjmmntdHIjqPzwYHqwnEXRwXGEG[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new cHjmmntdHIjqPzwYHqwnEXRwXGEG[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].HIJGuQoeYYdTmFXTkgbKJhNTmdsjb();
				}
			}
			return new wsTGaisyEgVqWobCcVTBhcYPpDji(array, array2, array3, array4);
		}
	}
}
