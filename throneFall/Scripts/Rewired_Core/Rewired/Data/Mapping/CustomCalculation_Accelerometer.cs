...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool IaXduNgyGPGRIDPZGrPRWFSQkUNDA()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = jDAxozDYokgQPFiBxQeDfcnVfuyb();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = DbBVTgsSaNQmsCDSQxcHRtcmKKqW();
					flag = true;
					break;
				}
			}
		...
		private float jDAxozDYokgQPFiBxQeDfcnVfuyb()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => gYkqjGmqbpGfybAZGfQmfPqhssdmb(num2), 
				_ => 0f, 
			};
		}

		private float DbBVTgsSaNQmsCDSQxcHRtcmKKqW()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float gYkqjGmqbpGfybAZGfQmfPqhssdmb(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
