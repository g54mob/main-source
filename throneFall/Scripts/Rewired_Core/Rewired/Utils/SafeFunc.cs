...
namespace Rewired.Utils
{
	...
	{
		private T YytfuJqQulrjluLXqcCCjQoBhjPs;

		private TResult GTUHZYndQbWbMCzjnrUFqxoCbWQl;

		private static Action<object, Func<T, TResult>> QmwpimHAdDYwbHxqotBIJKFNBqFAA;

		private static Action<object, Func<T, TResult>> invokeDelegate => KhZRxtTKIEpXGPoCxCIUHpkHVBgA;
		...
		{
			YytfuJqQulrjluLXqcCCjQoBhjPs = arg0;
			try
			{
				Invoke(invokeDelegate);
				return GTUHZYndQbWbMCzjnrUFqxoCbWQl;
			}
			catch
			{
			...
			{
				YytfuJqQulrjluLXqcCCjQoBhjPs = default(T);
				GTUHZYndQbWbMCzjnrUFqxoCbWQl = default(TResult);
			}
		}
		...
		private static void KhZRxtTKIEpXGPoCxCIUHpkHVBgA(object P_0, Func<T, TResult> P_1)
		{
			if (P_1 != null && P_0 is SafeFunc<T, TResult> safeFunc)
			{
				safeFunc.GTUHZYndQbWbMCzjnrUFqxoCbWQl = P_1(safeFunc.YytfuJqQulrjluLXqcCCjQoBhjPs);
			}
		}
