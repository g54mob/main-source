...
namespace Rewired
{
	...
	{
		private enum fmpafhimXMoNSAhlLmvCdNohhrVg
		{
			Error = 0,
			FoundIndex = 1,
			IsWholeElement = 2
		...
		private readonly DeviceLocalizationInfo cwsfPNroyXCMhaLoygJlcWmvrOhlA;

		public DeviceLocalizationInfo deviceLocalizationInfo => cwsfPNroyXCMhaLoygJlcWmvrOhlA;
		...
		{
			controllerName = P_0;
			cwsfPNroyXCMhaLoygJlcWmvrOhlA = P_1;
			if (cwsfPNroyXCMhaLoygJlcWmvrOhlA == null)
			{
				cwsfPNroyXCMhaLoygJlcWmvrOhlA = new DeviceLocalizationInfo();
			}
			cwsfPNroyXCMhaLoygJlcWmvrOhlA.FinishRuntimeSetup();
			bool flag = cwsfPNroyXCMhaLoygJlcWmvrOhlA.controllerType != ControllerType.Keyboard && cwsfPNroyXCMhaLoygJlcWmvrOhlA.controllerType != ControllerType.Mouse;
			for (int i = 0; i < P_2.Length; i++)
			{
				...
				if (flag)
				{
					if (ControllerElementIdentifier.YZHSetWXJjgByTksBDFHjlzVwSMEA.yGwJoWjmAgfFBjzLkWlyHWUouAEA(cwsfPNroyXCMhaLoygJlcWmvrOhlA, P_2[i], out var controllerElementIdentifier))
					{
						P_2[i] = controllerElementIdentifier;
						continue;
					}
					ControllerElementIdentifier.YZHSetWXJjgByTksBDFHjlzVwSMEA.qMeomicwiRdWaygxSCZXGRIHmEsnA(cwsfPNroyXCMhaLoygJlcWmvrOhlA, P_2[i]);
				}
				P_2[i].FinishRuntimeSetup(cwsfPNroyXCMhaLoygJlcWmvrOhlA, cwsfPNroyXCMhaLoygJlcWmvrOhlA.controllerType);
			}
			...
			for (int l = 0; l < num; l++)
			{
				int num6 = sXqCcDuUnmGVrBLFWHxpbKPxWlgP(P_2, P_3[l]);
				if (num6 < 0)
				{
			...
			{
				int num7 = sXqCcDuUnmGVrBLFWHxpbKPxWlgP(P_2, P_4[m]);
				if (num7 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num8 = sXqCcDuUnmGVrBLFWHxpbKPxWlgP(P_2, array[n]);
				if (num8 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num10 = sXqCcDuUnmGVrBLFWHxpbKPxWlgP(P_2, array2[num9]);
				if (num10 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num12 = sXqCcDuUnmGVrBLFWHxpbKPxWlgP(P_2, array3[num11]);
				if (num12 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
		...
		public bool TryGetCompoundElementMemberCombinedLocalizedName(IList<ActionElementMap> aems, out string result)
		{
			...
			for (int i = 0; i < compoundElements.Length; i++)
			{
				...
				int num2;
				switch (UcZaUIEyHuIqQeIYUvcnWReqPAhBA(compoundElement, elementIdentifierById, aems, out num2))
				{
				case fmpafhimXMoNSAhlLmvCdNohhrVg.IsWholeElement:
					result = elementIdentifierById.Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Ename;
					break;
				case fmpafhimXMoNSAhlLmvCdNohhrVg.FoundIndex:
					if (num2 >= 0)
					{
		...
		private int sXqCcDuUnmGVrBLFWHxpbKPxWlgP(ControllerElementIdentifier[] P_0, int P_1)
		{
			if (P_0 == null)
			{
				return -1;
		...
		private static fmpafhimXMoNSAhlLmvCdNohhrVg UcZaUIEyHuIqQeIYUvcnWReqPAhBA(HardwareJoystickMap.CompoundElement P_0, ControllerElementIdentifier P_1, IList<ActionElementMap> P_2, out int P_3)
		{
			P_3 = -1;
			if (P_0 == null || P_1 == null || P_2 == null)
			{
				return fmpafhimXMoNSAhlLmvCdNohhrVg.Error;
			}
			...
			jjEiJGkdrKfqxJAsceTeoFkgoNMlA.PEZtEoWUXFickyhTPzxGMRoQswOL pEZtEoWUXFickyhTPzxGMRoQswOL;
			...
			case CompoundControllerElementType.Axis2D:
			{
				...
				if (num3 == 1)
				{
					if (num4 == 1)
					{
						return fmpafhimXMoNSAhlLmvCdNohhrVg.IsWholeElement;
					}
					if (num7 == 1 && num8 == 1)
					{
						return fmpafhimXMoNSAhlLmvCdNohhrVg.IsWholeElement;
				...
				else if (num5 == 1 && num6 == 1)
				{
					...
					if (jjEiJGkdrKfqxJAsceTeoFkgoNMlA.BYUvlRLHXoZMeceCTjMMIYmbtBMmA(resultCompoundElementType, out pEZtEoWUXFickyhTPzxGMRoQswOL))
					{
						P_3 = pEZtEoWUXFickyhTPzxGMRoQswOL.GufixudKIxAbkoThGnHLzHVrbpWbA(AxisDirection.Horizontal);
						if (P_3 >= 0)
						{
							return fmpafhimXMoNSAhlLmvCdNohhrVg.FoundIndex;
						}
						return fmpafhimXMoNSAhlLmvCdNohhrVg.Error;
					}
				}
				else
				{
					...
					if (jjEiJGkdrKfqxJAsceTeoFkgoNMlA.BYUvlRLHXoZMeceCTjMMIYmbtBMmA(resultCompoundElementType, out pEZtEoWUXFickyhTPzxGMRoQswOL))
					{
						P_3 = pEZtEoWUXFickyhTPzxGMRoQswOL.GufixudKIxAbkoThGnHLzHVrbpWbA(AxisDirection.Vertical);
						if (P_3 >= 0)
						{
							return fmpafhimXMoNSAhlLmvCdNohhrVg.FoundIndex;
						}
