...
namespace Rewired
{
	...
	{
		private const int coiINBzbFnHvXFwmeibYqqqVKhqMA = 132;

		private static HardwareControllerMap_Game xfOMBGSoRmeslcLRUPSRIbMWdxEj;

		private bool CayaMHkOYqlZBoGDLIcDsLsDvIWbA;

		private bool ShCBXgcHoalABQODSoUwUghTlpcZ;

		bool IGetSetEnabled.enabled
		{
			get
			{
				return CayaMHkOYqlZBoGDLIcDsLsDvIWbA;
			}
			set
			{
				if (CayaMHkOYqlZBoGDLIcDsLsDvIWbA != value)
				{
					CayaMHkOYqlZBoGDLIcDsLsDvIWbA = value;
					Clear();
		...
		HardwareControllerMap_Game IUnifiedKeyboardSource.hardwareMap
		{
			get
			{
				if (xfOMBGSoRmeslcLRUPSRIbMWdxEj == null)
				{
					xfOMBGSoRmeslcLRUPSRIbMWdxEj = CreateHardwareMap();
				}
				return xfOMBGSoRmeslcLRUPSRIbMWdxEj;
			}
		}
		...
		{
			if (CayaMHkOYqlZBoGDLIcDsLsDvIWbA)
			{
		...
		{
			if (!ShCBXgcHoalABQODSoUwUghTlpcZ)
			{
				if (disposing && CayaMHkOYqlZBoGDLIcDsLsDvIWbA)
				{
					ThreadSafeUnityInput.keyboard.Monitor(state: false);
				}
				ShCBXgcHoalABQODSoUwUghTlpcZ = true;
			}
		}
		...
		public static ControllerElementType GetHardwareElementType(int elementIdentifierId)
		{
			...
			return xfOMBGSoRmeslcLRUPSRIbMWdxEj.GetElementType(elementIdentifierId);
		}
	}
}
