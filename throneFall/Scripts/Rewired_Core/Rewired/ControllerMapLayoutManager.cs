...
namespace Rewired
{
	...
	{
		internal class ccDuPLOhlbrAqOTHJEHJSRpBmDEb
		{
			public bool ldChEFVTsiSqJggKQGVIDjyHLzCbb;

			public bool PJAVqdVaEmBkjZSRlgOXywcejFUC;

			public bsCFPvFNILkrNbewcpeQWrFpdSUQB[] nzEXvfgKEAFQhdktlGhYQzdqWnEc;

			public ccDuPLOhlbrAqOTHJEHJSRpBmDEb(bool P_0, bool P_1, bsCFPvFNILkrNbewcpeQWrFpdSUQB[] P_2)
			{
				ldChEFVTsiSqJggKQGVIDjyHLzCbb = P_0;
				PJAVqdVaEmBkjZSRlgOXywcejFUC = P_1;
				nzEXvfgKEAFQhdktlGhYQzdqWnEc = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.mrHnssKPqvnBfcJsxqpLsnPxOquq)
					{
		...
		private bool MYwaNLdKSzlLtdevAgmiYlwOrgOLb;

		private bool IccdcBEWMGRfXludDNcynYseGUQD = true;

		private Player DNFsndzJivBGqFwRxdWGQBjKBmeIA;

		private ccDuPLOhlbrAqOTHJEHJSRpBmDEb RNFqjRtjveBbGDZIrfWuKCvAOYAm;

		private readonly int GPYAoUbZRzYSNcSmNBoTYHqrXfSvA;

		private List<RuleSet> YhqmAaDNVMTuRTZOerbAjPQZfOAy;

		private Action FmsjBmMcEyXlOncDHxpZCaIEfTAI;

		public bool enabled
		{
			get
			{
				return MYwaNLdKSzlLtdevAgmiYlwOrgOLb;
			}
			set
			{
				MYwaNLdKSzlLtdevAgmiYlwOrgOLb = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return IccdcBEWMGRfXludDNcynYseGUQD;
			}
			set
			{
				IccdcBEWMGRfXludDNcynYseGUQD = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return YhqmAaDNVMTuRTZOerbAjPQZfOAy;
			}
			set
			{
				if (value == null)
				...
				YhqmAaDNVMTuRTZOerbAjPQZfOAy = value;
			}
		}

		internal event Action YzTLDMMsotHSOvKHCMxXuKFsAtNy
		{
			add
			{
				FmsjBmMcEyXlOncDHxpZCaIEfTAI = (Action)Delegate.Combine(FmsjBmMcEyXlOncDHxpZCaIEfTAI, b);
			}
			remove
			{
				FmsjBmMcEyXlOncDHxpZCaIEfTAI = (Action)Delegate.Remove(FmsjBmMcEyXlOncDHxpZCaIEfTAI, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, ccDuPLOhlbrAqOTHJEHJSRpBmDEb P_1)
		{
			if (P_0 == null)
			{
			...
			GPYAoUbZRzYSNcSmNBoTYHqrXfSvA = ReInput.id;
			DNFsndzJivBGqFwRxdWGQBjKBmeIA = P_0;
			RNFqjRtjveBbGDZIrfWuKCvAOYAm = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != GPYAoUbZRzYSNcSmNBoTYHqrXfSvA)
			{
				ReInput.CheckInitialized(GPYAoUbZRzYSNcSmNBoTYHqrXfSvA);
				return;
			}
			FmsjBmMcEyXlOncDHxpZCaIEfTAI?.Invoke();
			if (!MYwaNLdKSzlLtdevAgmiYlwOrgOLb || YhqmAaDNVMTuRTZOerbAjPQZfOAy == null)
			{
				return;
			}
			int count = YhqmAaDNVMTuRTZOerbAjPQZfOAy.Count;
			if (count == 0)
			{
				return;
			}
			...
			DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.maps.GetAllMaps(list);
			list2.AddRange(DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = YhqmAaDNVMTuRTZOerbAjPQZfOAy[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (IccdcBEWMGRfXludDNcynYseGUQD && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(DNFsndzJivBGqFwRxdWGQBjKBmeIA.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							DNFsndzJivBGqFwRxdWGQBjKBmeIA.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((RNFqjRtjveBbGDZIrfWuKCvAOYAm != null && RNFqjRtjveBbGDZIrfWuKCvAOYAm.nzEXvfgKEAFQhdktlGhYQzdqWnEc != null) ? RNFqjRtjveBbGDZIrfWuKCvAOYAm.nzEXvfgKEAFQhdktlGhYQzdqWnEc.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(RNFqjRtjveBbGDZIrfWuKCvAOYAm.nzEXvfgKEAFQhdktlGhYQzdqWnEc[i].nXKxAkXnRYLlkmgwTSRoJOAaetXx);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = RNFqjRtjveBbGDZIrfWuKCvAOYAm.nzEXvfgKEAFQhdktlGhYQzdqWnEc[i].BhLDjaqvsZwuenvhbTMJhNLObSwE;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (RNFqjRtjveBbGDZIrfWuKCvAOYAm != null)
			{
				MYwaNLdKSzlLtdevAgmiYlwOrgOLb = RNFqjRtjveBbGDZIrfWuKCvAOYAm.ldChEFVTsiSqJggKQGVIDjyHLzCbb;
				IccdcBEWMGRfXludDNcynYseGUQD = RNFqjRtjveBbGDZIrfWuKCvAOYAm.PJAVqdVaEmBkjZSRlgOXywcejFUC;
			}
			YhqmAaDNVMTuRTZOerbAjPQZfOAy = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return zJNKMIeRKxkuXJzcxMXOVGDCDftR().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return zJNKMIeRKxkuXJzcxMXOVGDCDftR().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				vIyFhvqRMhPTKEidJffbFswXgbRt(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				vIyFhvqRMhPTKEidJffbFswXgbRt(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject zJNKMIeRKxkuXJzcxMXOVGDCDftR()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			VbBovDEyWaBVMbmhwWRcZdjClMUo(serializedObject);
			return serializedObject;
		}

		private void VbBovDEyWaBVMbmhwWRcZdjClMUo(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", MYwaNLdKSzlLtdevAgmiYlwOrgOLb);
			P_0.Add("loadFromUserDataStore", IccdcBEWMGRfXludDNcynYseGUQD);
			P_0.Add("ruleSets", YhqmAaDNVMTuRTZOerbAjPQZfOAy);
		}

		private bool vIyFhvqRMhPTKEidJffbFswXgbRt(SerializedObject P_0)
		{
			MYwaNLdKSzlLtdevAgmiYlwOrgOLb = false;
			YhqmAaDNVMTuRTZOerbAjPQZfOAy = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref MYwaNLdKSzlLtdevAgmiYlwOrgOLb);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref IccdcBEWMGRfXludDNcynYseGUQD);
