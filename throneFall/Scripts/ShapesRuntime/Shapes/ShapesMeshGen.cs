	...
	public static class ShapesMeshGen
	{
		...
		public static void GenPolylineMesh(Mesh mesh, IList<PolylinePoint> path, bool closed, PolylineJoins joins, bool flattenZ, bool useColors)
		{
			...
			for (int i = 0; i < num; i++)
			{
				...
				Color color = (useColors ? path[i].color.ColorSpaceAdjusted() : default(Color));
				int num21 = i * num2;
				int num22;
				if (flag)
				{
				...
				if (flag)
				{
					SetUv(array4, uvEndpointVal, thickness, num21, 0f, 0f);
					SetUv(array4, uvEndpointVal, thickness, num22, -1f, -1f);
					SetUv(array4, uvEndpointVal, thickness, num12, -1f, 1f);
					SetUv(array4, uvEndpointVal, thickness, num13, 1f, -1f);
					SetUv(array4, uvEndpointVal, thickness, num14, 1f, 1f);
					if (flag5)
					{
						SetUv(array4, uvEndpointVal, thickness, num15, 0f, 0f);
						if (flag2)
						{
							SetUv(array4, uvEndpointVal, thickness, num16, 1f, -1f);
							SetUv(array4, uvEndpointVal, thickness, num17, 1f, 1f);
						}
						else
						{
							SetUv(array4, uvEndpointVal, thickness, num16, 1f, -1f);
							SetUv(array4, uvEndpointVal, thickness, num17, -1f, -1f);
							SetUv(array4, uvEndpointVal, thickness, num18, -1f, 1f);
							SetUv(array4, uvEndpointVal, thickness, num19, 1f, 1f);
						}
					}
				}
				else
				{
					SetUv(array4, uvEndpointVal, thickness, num21, -1f, i);
					SetUv(array4, uvEndpointVal, thickness, num22, 1f, i);
				}
				if (!(!flag3 || closed))
				{
					continue;
			...
			static void SetUv(Vector4[] uvArr, float z, float pathThicc, int id, float x, float y)
			{
