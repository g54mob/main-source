...
namespace Assets.Behaviour.Frame.Parts
{
	public class T9AICorePuzzle : MonoBehaviour
	{
		...
		private void Update()
		{
			if ((bool)ActiveNode && PlayerControls.InputCancel)
			{
				ActiveNode.ClearActive();
				ActiveNode = null;
			}
		}
		...
		{
			PuzzleActive = false;
			GetComponentInParent<ActiveWorldFrame>().ShowWarning(new WorldAnchor(WorldAnchorType.HandCraft, 0), "Conflict detected!");
