...
namespace Assets.Source.Player
{
	public class TechNode
	{
		...
		public float LowerTierMultiplier = 1f;
		...
		public string Description => StaticDescription ?? _getDynamicDescription();
		...
		public Dictionary<ItemType, int> Cost { private get; set; }
		...
		public Dictionary<ItemType, int> GetCost()
		{
			if (Cost == null && CostItems != null)
			{
				Cost = GameMath.CreateItemCost(Identifier, Tier, CostMultiplier * GetDefaultCostMultiplier() * 40f, CostItems);
			}
			return Cost;
		}
		...
		private string _getDynamicDescription()
		{
			...
			{
				WorldFrame preview = WorldManager.Instance.GetFramePrefabSet(UpgradedFrame).GetPreview();
				string text = UpgradeType switch
				{
					FrameUpgradeType.Speed => "crafting speed", 
					FrameUpgradeType.Productivity => "productivity", 
					_ => throw new Exception(), 
				};
				_dynamicDescription = "Increases " + preview.ItemHint.DisplayName.ToLower() + " " + text + " by " + UIHelper.HighlightText(GameMath.FormatPercentage(UpgradeMultiplier - 1f)) + ".";
			}
