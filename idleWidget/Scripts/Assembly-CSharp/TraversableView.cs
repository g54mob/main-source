...
public class TraversableView : MonoBehaviour
{
	...
	{
		_boundsMin.x = Mathf.Min(_boundsMin.x, position.x);
		_boundsMin.y = Mathf.Min(_boundsMin.y, position.y);
		_boundsMax.x = Mathf.Max(_boundsMax.x, position.x);
		_boundsMax.y = Mathf.Max(_boundsMax.y, position.y);
	}
