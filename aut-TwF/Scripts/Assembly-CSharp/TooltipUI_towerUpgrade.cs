...
public class TooltipUI_towerUpgrade : TooltipUI
{
	...
	{
		Tower.FTowerStats totalTowerStats = Tower.GetTotalTowerStats(tower, Tower.FTowerStats.EApplyEffectMode.OnlyBase);
		Tower.FTowerStats totalTowerStats2 = Tower.GetTotalTowerStats(currentBaseTower ?? tower.GetComponent<GameplayObject>().ObjectData.BaseObject.Prefab.GetComponent<Tower>(), Tower.FTowerStats.EApplyEffectMode.OnlyBase);
		List<GameplayEffectData> baseTowerEffects = GetBaseTowerEffects();
		baseTowerEffects.AddRange(LTFunctionLibrary.GetGameplayEffectDatasToApplyToBuilding(tower.GetComponent<GameplayObject>().ObjectData));
		totalTowerStats.ApplyGE(baseTowerEffects, Tower.FTowerStats.EApplyEffectMode.OnlyNonBase);
		totalTowerStats2.ApplyGE(baseTowerEffects, Tower.FTowerStats.EApplyEffectMode.OnlyNonBase);
