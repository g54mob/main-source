...
public class PathBasedSpawnerData : BaseSpawnerData
{
	...
	private List<Vector2> GetRandomPathBasedPositions(Grid grid, PathTile[] pathTiles, List<(Vector3 position, float radius)> invalidAreas)
	{
		...
		while (list.Count < Mathf.Max(minGeneratedPositions, GetObjectsAmount(num)) && num3 < 3)
		{
			bool flag = true;
			num2 = 0;
			while (num2 < base.MaxIterations && flag)
			{
				num2++;
				flag = false;
			...
			{
				num3++;
				circleRadius = Mathf.RoundToInt(base.MinDistanceBetweenObjects * (1f - 0.25f * (float)num3));
				if (debugLog)
				{
		...
		if (debugLog && list.Count < Mathf.Max(minGeneratedPositions, GetObjectsAmount(num)))
		{
			Debug.LogWarning("Finalmente, no se consiguieron spawnear suficientes posiciones válidas (" + base.ObjectsToSpawn.Name + ", generadas " + list.Count + " de " + Mathf.Max(minGeneratedPositions, GetObjectsAmount(num)) + ")");
		}
		return list;
	}
}
