...
public class LevelsProgressionManager : MonoBehaviour, ISavable
{
	...
	public class FLevelProgressionInfo : ISavable
	{
		...
		[SerializeField]
		private EMatchMode matchMode = EMatchMode.Campaign;
		...
		[Savable("scoresByMapSize", true, false)]
		private Dictionary<EMapSize, int> scoresByMapSize = new Dictionary<EMapSize, int>();

		public LevelData LevelData => levelData;

		public LevelData UnlockedBy => unlockedBy;

		public EMatchMode MatchMode => matchMode;
		...
		public void SetScore(EMapSize mapSize, int score)
		{
			if (scoresByMapSize.ContainsKey(mapSize))
			{
				scoresByMapSize[mapSize] = score;
			}
			else
			{
				scoresByMapSize.Add(mapSize, score);
			}
		}

		public int GetScore(EMapSize mapSize)
		{
			if (scoresByMapSize.ContainsKey(mapSize))
			{
				return scoresByMapSize[mapSize];
			}
			return 0;
		}
	...
	public FLevelProgressionInfo GetLevelProgressionInfoByID(string levelID)
	{
