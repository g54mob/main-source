...
public class GroundGenerator : MonoBehaviour
{
	...
	public IEnumerator GenerateGround(KeyValuePair<PathTile, EOrientation> firstPathTile, KeyValuePair<PathTile, EOrientation> lastPathTile, Grid grid, PathTile[] pathTiles)
	{
		...
		GenerateBorder(grid);
		stopwatch.Stop();
		UnityEngine.Debug.Log("GROUND (" + grid.GetGridSize().x + "x" + grid.GetGridSize().y + "): " + (float)stopwatch.ElapsedMilliseconds / 1000f + "s");
	}
	...
	private void SpawnGroundChunks(KeyValuePair<PathTile, EOrientation> firstPathTile, KeyValuePair<PathTile, EOrientation> lastPathTile, Grid grid, PathTile[] pathTiles)
	{
		...
		{
			if (!groundChunksSpawnerData || groundChunksSpawnerData.GetObjectsAmount(num) <= 0)
			{
				continue;
			}
			...
			int num3 = 0;
			while (num2 < groundChunksSpawnerData.GetObjectsAmount(num))
			{
				if (list2.Count == 0)
				{
				...
				{
					if (TrySpawnGroundChunk(grid, groundChunksSpawnerData.ObjectsToSpawn.GetRandomElement(), centerPosition, groundChunksSpawnerData.BuildableRadiusAroundObject, groundChunksSpawnerData.ExcludedTileTypes))
					{
						num2++;
						break;
					}
	...
	private bool TrySpawnGroundChunk(Grid grid, GameObject groundChunkPrefab, Vector3 centerPosition, int buildableRadiusAroundObject, Tile.ETileType[] excludedTileTypes)
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			int num2 = 0;
			Source[] sources = component2.GetSources();
			foreach (Source source in sources)
			{
				source.gameObject.GetComponent<SaveComponent>().Id = "GroundChunkSource_" + component2.transform.position.x + "_" + component2.transform.position.y + "_" + num2;
				num2++;
				Vector3[] occupiedPositions = source.PlacementComponent.GetOccupiedPositions();
				foreach (Vector3 position in occupiedPositions)
				{
					grid.GetGridCell(position).BuiltObject = source.PlacementComponent;
				}
			}
			return true;
		}
