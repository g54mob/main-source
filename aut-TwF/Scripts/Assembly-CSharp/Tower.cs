...
public class Tower : Character, ISelectable, ISampleableData, ISavable
{
	public struct FTowerStats
	{
		...
		public void ApplyGE(IEnumerable<GameplayEffectData> GEDatas, EApplyEffectMode applyEffectMode)
		{
			float baseDamage = damage;
			float baseAttackSpeed = attackSpeed;
			float baseRange = range;
			List<GE_StatModifierData> list = new List<GE_StatModifierData>();
			List<GE_StatModifierData> list2 = new List<GE_StatModifierData>();
			foreach (GameplayEffectData GEData in GEDatas)
			{
				if (GEData is GE_StatModifierData)
				{
					if ((GEData as GE_StatModifierData).ModifyStatBase)
					{
						list.Add(GEData as GE_StatModifierData);
					}
					else
					{
						list2.Add(GEData as GE_StatModifierData);
					}
				}
			}
			if (applyEffectMode == EApplyEffectMode.All || applyEffectMode == EApplyEffectMode.OnlyBase)
			{
				foreach (GE_StatModifierData item in list)
				{
					ApplyStatModifier(item, baseDamage, baseAttackSpeed, baseRange);
				}
			}
			if (applyEffectMode != EApplyEffectMode.All && applyEffectMode != EApplyEffectMode.OnlyNonBase)
			{
				return;
			}
			baseDamage = damage;
			...
			baseRange = range;
			foreach (GE_StatModifierData item2 in list2)
			{
				ApplyStatModifier(item2, baseDamage, baseAttackSpeed, baseRange);
			}
		}

		private void ApplyStatModifier(GE_StatModifierData geStatModifierData, float baseDamage, float baseAttackSpeed, float baseRange)
		{
			switch (geStatModifierData.Stat)
			{
			case EStats.BaseDamage:
				if (geStatModifierData.ModifierOperation == ModifierOperation.Additive)
				{
					damage += geStatModifierData.StatValue;
				}
				else if (geStatModifierData.ModifierOperation == ModifierOperation.Multiplicative)
				{
					damage += baseDamage * geStatModifierData.StatValue;
				}
				break;
			case EStats.AttackSpeed:
				if (geStatModifierData.ModifierOperation == ModifierOperation.Additive)
				{
					attackSpeed += geStatModifierData.StatValue;
				}
				else if (geStatModifierData.ModifierOperation == ModifierOperation.Multiplicative)
				{
					attackSpeed += baseAttackSpeed * geStatModifierData.StatValue;
				}
				break;
			case EStats.Range:
				if (geStatModifierData.ModifierOperation == ModifierOperation.Additive)
				{
					range += geStatModifierData.StatValue;
				}
				else if (geStatModifierData.ModifierOperation == ModifierOperation.Multiplicative)
				{
					range += baseRange * geStatModifierData.StatValue;
				}
				break;
			}
		}
	...
	private float GetTowerRange()
	{
		if (PlacementComponent.IsPlaced)
		{
			return StatsComponent.GetStat(EStats.Range);
		}
		float num = StatsComponent.GetConfigStat(EStats.Range);
		...
		foreach (GemData gems in GemsComponent.GemsList)
		{
			gameplayEffectDatasToApplyToBuilding.AddRange(gems.GameplayEffectsToApply);
		}
	...
	public static FTowerStats GetTotalTowerStats(Tower tower, FTowerStats.EApplyEffectMode applyEffectMode)
	{
		...
		result.ApplyGE(LTFunctionLibrary.GetGameplayEffectDatasToApplyToBuilding(component2.ObjectData), applyEffectMode);
		return result;
	}
