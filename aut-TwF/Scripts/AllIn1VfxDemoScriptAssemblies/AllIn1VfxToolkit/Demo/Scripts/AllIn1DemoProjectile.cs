...
namespace AllIn1VfxToolkit.Demo.Scripts
{
	...
	{
		[SerializeField]
		private float inaccurateAmount = 0.05f;
		...
		{
			projectileDir.x += Random.Range(0f - inaccurateAmount, inaccurateAmount);
			projectileDir.y += Random.Range(0f - inaccurateAmount, inaccurateAmount);
			projectileDir.z += Random.Range(0f - inaccurateAmount, inaccurateAmount);
		}
		...
		private void OnTriggerEnter(Collider other)
		{
			if (doImpactSpawn)
			{
				Transform obj = Object.Instantiate(currentImpactPrefab, base.transform.position, Quaternion.identity).transform;
				obj.parent = currentHierarchyParent;
				obj.localScale *= impactScaleSize;
			}
			if (doShake)
			{
				AllIn1Shaker.i.DoCameraShake(shakeAmountOnImpact);
			}
