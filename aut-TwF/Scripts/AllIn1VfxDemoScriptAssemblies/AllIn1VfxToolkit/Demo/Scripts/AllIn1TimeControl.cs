...
namespace AllIn1VfxToolkit.Demo.Scripts
{
	...
	{
		[SerializeField]
		private KeyCode increaseTimeScale = KeyCode.UpArrow;

		[SerializeField]
		private KeyCode increaseTimeScaleAlt = KeyCode.W;

		[SerializeField]
		private KeyCode decreaseTimeScale = KeyCode.DownArrow;

		[SerializeField]
		private KeyCode decreaseTimeScaleAlt = KeyCode.S;
		...
		[SerializeField]
		[Range(0f, 1f)]
		private float timeScaleInterval = 0.05f;
		...
		private void Update()
		{
			...
			{
				ChangeTimeScale(0f - timeScaleInterval);
			}
			if (Input.GetKeyDown(pauseKey))
			{
				if (Time.timeScale < 0.01f)
				{
					ChangeTimeScale(1f - Time.timeScale);
				}
				else
				{
					ChangeTimeScale(0f - Time.timeScale);
				}
				pausedButtonTween.ScaleUpTween();
			}
			float num = 0.1f;
			if (Time.unscaledTime - lastChangeTime > num)
			{
		...
		{
			Time.timeScale = Mathf.Clamp(Time.timeScale + changeAmount, 0f, 1f);
