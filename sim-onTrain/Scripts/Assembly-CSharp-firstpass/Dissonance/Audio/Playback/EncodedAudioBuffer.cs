...
namespace Dissonance.Audio.Playback
{
	internal class EncodedAudioBuffer
	{
		...
		public void Push(VoicePacket frame)
		{
			...
			{
				Log.Warn(Log.PossibleBugMessage($"Encoded audio heap is getting very large ({_count} items)", "59EE0102-FF75-467A-A50D-00BF670E9B8C"));
			}
		}
		...
		public bool Read(out VoicePacket? frame, out bool lostPacket)
		{
			...
			while (_heap.Count > 0 && _heap.Minimum.SequenceNumber < sequenceNumber)
			{
				...
				if (num > 30)
				{
					Log.Warn(Log.PossibleBugMessage($"Received a very late packet ({num} packets late)", "30EF1B03-7BBC-49D3-A23E-6E84781FF29F"));
				}
