...
namespace Dissonance.Audio.Playback
{
	internal class DecoderPipeline : IDecoderPipeline, IVolumeProvider, IRemoteChannelProvider
	{
		...
		public DecoderPipeline([NotNull] IVoiceDecoder decoder, uint inputFrameSize, [NotNull] Action<DecoderPipeline> completionHandler, bool softClip = true)
		{
			...
			_firstFrameSeq = 0u;
			BufferedDecoder source = new BufferedDecoder(decoder, inputFrameSize, decoder.Format, RecycleFrame);
			FrameToSampleConverter frameToSampleConverter = new FrameToSampleConverter(new VolumeRampedFrameSource(source, this));
			ISampleSource source2 = frameToSampleConverter;
			if (softClip)
			{
				source2 = new SoftClipSampleSource(frameToSampleConverter);
			}
			Resampler output = new Resampler(source2);
			_source = source;
			_output = output;
		}
		...
		public float Push(VoicePacket packet, DateTime now)
		{
			...
			ArraySegment<byte> encodedAudioFrame = ArraySegmentExtensions.CopyTo(packet.EncodedAudioFrame, _bytePool.Get());
