...
namespace Dissonance.Networking.Client
{
	internal class SendQueue<TPeer> : ISendQueue<TPeer> where TPeer : struct
	{
		...
		public ConcurrentPool<byte[]> SendBufferPool => _sendBufferPool;
		...
		public void Stop()
		{
			_ = _serverReliableQueue.Count;
			_ = _serverUnreliableQueue.Count;
			_ = _reliableP2PQueue.Count;
			_ = _unreliableP2PQueue.Count;
			_serverReliableQueue.Clear();
			_serverUnreliableQueue.Clear();
			_reliableP2PQueue.Clear();
			_unreliableP2PQueue.Clear();
