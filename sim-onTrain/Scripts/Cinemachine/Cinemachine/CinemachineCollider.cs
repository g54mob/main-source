				...
				if (m_AvoidObstacles)
				{
					...
					if (deltaTime >= 0f && base.VirtualCamera.PreviousStateIsValid && m_DampingWhenOccluded + m_Damping > 0.0001f)
					{
						...
						if (num > 0f)
						{
							...
							Vector3 vector5 = (flag ? extra.previousDisplacement : (state.ReferenceLookAt + quaternion * extra.previousCameraOffset - correctedPosition));
							vector = vector5 + Damper.Damp(vector - vector5, num, deltaTime);
						}
					}
					...
					extra.previousCameraOffset = vector4 - state.ReferenceLookAt;
