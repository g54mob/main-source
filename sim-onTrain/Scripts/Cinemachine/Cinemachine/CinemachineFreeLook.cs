...
namespace Cinemachine
{
	...
	public class CinemachineFreeLook : CinemachineVirtualCameraBase
	{
		...
		{
			float num = InitialGuess(cameraOffset);
			for (int i = 0; i < 10; i++)
			{
				float num2 = AngleFunction(num);
				float num3 = SlopeOfAngleFunction(num);
				if (Mathf.Abs(num3) < 5E-05f || Mathf.Abs(num2) < 5E-05f)
				{
					break;
				}
			...
			{
				UpdateCachedSpline();
				Vector4 vector = m_CachedKnots[1];
				Vector4 vector2 = m_CachedKnots[2];
				Vector4 vector3 = m_CachedKnots[3];
				float t = cameraPosInRigSpace.ClosestPointOnSegment(vector, vector2);
				float num4 = Vector3.SqrMagnitude(Vector3.Lerp(vector, vector2, t) - cameraPosInRigSpace);
				float t2 = cameraPosInRigSpace.ClosestPointOnSegment(vector2, vector3);
				float num5 = Vector3.SqrMagnitude(Vector3.Lerp(vector2, vector3, t2) - cameraPosInRigSpace);
				if (!(num4 < num5))
				{
					return Mathf.Lerp(0.5f, 1f, t2);
				}
				return Mathf.Lerp(0f, 0.5f, t);
			}
			float SlopeOfAngleFunction(float input)
			{
				float num4 = AngleFunction(input - 5E-05f);
				return (AngleFunction(input + 5E-05f) - num4) / 0.0001f;
			}
		}
