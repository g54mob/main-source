...
namespace Mirror.SimpleWeb
{
	internal static class ReceiveLoop
	{
		...
		public static void Loop(Config config)
		{
			...
			try
			{
				try
				{
					...
					Log.Info($"[SimpleWebTransport] {connection} Not Connected");
				}
				catch (Exception)
				{
			...
			{
				Log.InfoException(e3);
			}
			catch (ReadHelperException e4)
			{
				Log.InfoException(e4);
			}
			catch (SocketException ex2)
			{
				Log.Warn("[SimpleWebTransport] ReceiveLoop SocketException\n" + ex2.Message, showColor: false);
				concurrentQueue.Enqueue(new Message(connection.connId, ex2));
			}
			catch (IOException ex3)
			{
				Log.Warn("[SimpleWebTransport] ReceiveLoop IOException\n" + ex3.Message, showColor: false);
				concurrentQueue.Enqueue(new Message(connection.connId, ex3));
			}
			catch (InvalidDataException ex4)
			{
				Log.Error($"[SimpleWebTransport] Invalid data from {connection}: {ex4.Message}");
				concurrentQueue.Enqueue(new Message(connection.connId, ex4));
			}
			catch (Exception ex5)
			{
				Log.Exception(ex5);
				concurrentQueue.Enqueue(new Message(connection.connId, ex5));
			}
			finally
			{
		...
		{
			Config config2 = config;
			config2.Deconstruct(out var conn, out var _, out var expectMask, out var queue, out var bufferPool);
			Connection connection = conn;
			bool expectMask2 = expectMask;
			ConcurrentQueue<Message> concurrentQueue = queue;
			ArrayBuffer arrayBuffer = CopyMessageToBuffer(bufferPool, expectMask2, buffer, msgOffset, payloadLength);
			Log.DumpBuffer("[SimpleWebTransport] Message", arrayBuffer);
			concurrentQueue.Enqueue(new Message(connection.connId, arrayBuffer));
		}
