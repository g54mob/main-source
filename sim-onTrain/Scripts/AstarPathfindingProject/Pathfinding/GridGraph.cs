...
namespace Pathfinding
{
	...
	public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph, IRaycastableGraph
	{
		...
		public void AlignToTilemap(GridLayout grid)
		{
			...
			{
				Quaternion quaternion2 = Quaternion.LookRotation(rhs.normalized, -Vector3.Cross(lhs, rhs).normalized);
				nodeSize = rhs.magnitude;
				isometricAngle = 0f;
				aspectRatio = lhs.magnitude / nodeSize;
				if (float.IsInfinity(aspectRatio) || float.IsNaN(aspectRatio))
				{
					aspectRatio = 1f;
				}
				rotation = quaternion2.eulerAngles;
				uniformEdgeCosts = false;
				if (neighbours == NumNeighbours.Six)
				{
					neighbours = NumNeighbours.Eight;
			...
			case GridLayout.CellLayout.Isometric:
			{
				...
				Quaternion quaternion = Quaternion.LookRotation(a.normalized, -Vector3.Cross(b, a).normalized) * Quaternion.Euler(0f, -45f, 0f);
			...
			case GridLayout.CellLayout.Hexagon:
			{
				...
				_ = grid.CellToWorld(new Vector3Int(0, 1, 0)) - vector;
				...
				rotation = Quaternion.LookRotation(rhs3.normalized, -Vector3.Cross(lhs2, rhs3).normalized).eulerAngles;
				uniformEdgeCosts = true;
				neighbours = NumNeighbours.Six;
			...
			Vector3 point = new Vector3((width % 2 == 0 != flag) ? 0f : 0.5f, 0f, (depth % 2 == 0 != flag) ? 0f : 0.5f);
			Vector3 vector2 = transform.TransformVector(point);
