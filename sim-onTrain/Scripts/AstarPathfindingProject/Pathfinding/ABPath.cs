	...
	public class ABPath : Path
	{
		...
		public static ABPath FakePath(List<Vector3> vectorPath, List<GraphNode> nodePath = null)
		{
			...
			if (nodePath != null)
			{
				...
				if (nodePath.Count > 0)
				{
					aBPath.startNode = nodePath[0];
					aBPath.endNode = nodePath[nodePath.Count - 1];
				}
			}
			return aBPath;
		}
		...
		protected virtual bool EndPointGridGraphSpecialCase(GraphNode closestWalkableEndNode)
		{
			if (closestWalkableEndNode is GridNode gridNode)
			{
				...
				GridNode gridNode2 = AstarPath.active.GetNearest(originalEndPoint, NNConstraintNone).node as GridNode;
