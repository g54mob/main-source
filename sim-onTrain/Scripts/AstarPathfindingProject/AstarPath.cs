...
public class AstarPath : VersionedMonoBehaviour
{
	...
	public static readonly Version Version = new Version(4, 2, 19);
	...
	public NNInfo GetNearest(Vector3 position, NNConstraint constraint, GraphNode hint)
	{
		...
		if (array != null)
		{
			for (int i = 0; i < array.Length; i++)
			{
				...
				float sqrMagnitude = (nNInfoInternal.clampedPosition - position).sqrMagnitude;
				if (prioritizeGraphs)
				{
					if (sqrMagnitude < prioritizeGraphsLimit * prioritizeGraphsLimit)
					{
						num = sqrMagnitude;
