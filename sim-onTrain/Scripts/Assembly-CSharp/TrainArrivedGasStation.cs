...
public class TrainArrivedGasStation : NetworkBehaviour
{
	[SerializeField]
	private string fallbackMessage = "Tren İstasyonuna vardınız";
	...
	private void OnTriggerEnter(Collider other)
	{
		if (base.isServer && !triggered)
		{
			...
			Debug.Log($"[GasStationChecker] TriggerEnter: {other.gameObject.name} | root: {other.transform.root.name} | TrainController(direct)={flag} | TrainController(parent)={flag2}");
			if (flag || flag2)
			{
				triggered = true;
	...
	private void RpcShowMessage()
	{
		...
		SendRPCInternal("System.Void TrainArrivedGasStation::RpcShowMessage()", 170440650, writer, 0, includeOwner: true);
	...
	protected static void InvokeUserCode_RpcShowMessage(NetworkBehaviour obj, NetworkReader reader, NetworkConnectionToClient senderConnection)
	{
		...
		{
			((TrainArrivedGasStation)obj).UserCode_RpcShowMessage();
		}
	}

	static TrainArrivedGasStation()
	{
		RemoteProcedureCalls.RegisterRpc(typeof(TrainArrivedGasStation), "System.Void TrainArrivedGasStation::RpcShowMessage()", InvokeUserCode_RpcShowMessage);
	}
}
