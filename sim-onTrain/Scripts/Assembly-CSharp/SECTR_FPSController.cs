...
public class SECTR_FPSController : SECTR_FPController
{
	...
	{
		base.Update();
		Vector3 vector = ((!Input.multiTouchEnabled || Application.isEditor) ? new Vector3(Input.GetAxis("Horizontal"), Input.GetAxis("Vertical"), 0f) : ((Vector3)GetScreenJoystick(left: false)));
		...
		vector = Quaternion.FromToRotation(-base.transform.forward, base.transform.up) * vector;
