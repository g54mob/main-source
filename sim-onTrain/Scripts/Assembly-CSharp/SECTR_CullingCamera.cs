...
public class SECTR_CullingCamera : MonoBehaviour
{
	...
	{
		Camera camera = ((cullingProxy != null) ? cullingProxy : myCamera);
		Vector3 position = camera.transform.position;
		float num = Mathf.Cos(Mathf.Max(camera.fieldOfView, camera.fieldOfView * camera.aspect) * 0.5f * (MathF.PI / 180f));
		float num2 = Mathf.Max(0f, camera.nearClipPlane / num * 1.001f);
		...
		if (!SimpleCulling)
		{
			...
			num3 = initialSectors.Count;
			for (int j = 0; j < num3; j++)
			{
				if (initialSectors[j].IsConnectedTerrain)
				{
					SimpleCulling = true;
					break;
				}
		...
		while (nodeStack.Count > 0)
		{
			...
			if (visibilityNode.frustumPlanes != null)
			{
				...
				for (int n = 0; n < count5; n++)
				{
					...
					float num4 = Vector3.Dot(plane.normal, plane2.normal);
					if (num4 < -0.9f && num4 > -0.99f)
					{
				...
				if (SECTR_Occluder.All.Count > 0)
				{
					...
					if (occludersInSector != null)
					{
						...
						for (int num7 = 0; num7 < count6; num7++)
						{
							SECTR_Occluder sECTR_Occluder = occludersInSector[num7];
							...
							int num8 = vertsCW.Length;
							occluderVerts.Clear();
							Bounds bounds = new Bounds(sECTR_Occluder.transform.position, Vector3.zero);
							for (int num9 = 0; num9 < num8; num9++)
							{
								Vector3 point = cullingMatrix.MultiplyPoint3x4(vertsCW[num9]);
								bounds.Encapsulate(point);
								occluderVerts.Add(new ClipVertex(new Vector4(point.x, point.y, point.z, 1f), 0f));
							}
							if (SECTR_Geometry.FrustumIntersectsBounds(sECTR_Occluder.BoundingBox, cullingPlanes, num5, out var _))
							{
								List<Plane> list;
				...
				if (count2 > 0)
				{
					...
					{
						cullingWorkQueue.Enqueue(new ThreadCullData(sector2, this, position, cullingPlanes, occluderFrustums, num5, shadowDistance, SimpleCulling));
				...
				{
					_FrustumCullSector(sector2, position, cullingPlanes, occluderFrustums, num5, shadowDistance, SimpleCulling, ref visibleRenderers, ref visibleLights, ref visibleTerrains, ref shadowLights);
				}
				int num10 = ((!SimpleCulling) ? visibilityNode.sector.Portals.Count : 0);
				for (int num11 = 0; num11 < num10; num11++)
				{
					SECTR_Portal sECTR_Portal = visibilityNode.sector.Portals[num11];
					...
					if (!flag3 && !flag)
					{
						...
						{
							if (SECTR_Geometry.FrustumContainsBounds(sECTR_Portal.BoundingBox, occluderFrustums[num12]))
							{
								flag3 = true;
								break;
							}
					...
					if (!flag)
					{
						...
						if (vertsCW2 != null)
						{
							int num13 = vertsCW2.Length;
							for (int num14 = 0; num14 < num13; num14++)
							{
								Vector3 vector3 = localToWorldMatrix.MultiplyPoint3x4(vertsCW2[num14]);
					...
					if (!flag && !sECTR_Portal.IsPointInHull(position, num2))
					{
						...
						{
							Plane plane3 = visibilityNode.frustumPlanes[num15];
							Vector4 a = new Vector4(plane3.normal.x, plane3.normal.y, plane3.normal.z, plane3.distance);
							bool flag4 = true;
							bool flag5 = true;
							for (int num16 = 0; num16 < portalVertices.Count; num16++)
							{
								Vector4 vertex = portalVertices[num16].vertex;
								float num17 = Vector4.Dot(a, vertex);
								portalVertices[num16] = new ClipVertex(vertex, num17);
								flag4 = flag4 && num17 > 0f;
								flag5 = flag5 && num17 <= -0.001f;
							}
							if (flag5)
							{
								portalVertices.Clear();
							...
							int num18 = portalVertices.Count;
							for (int num19 = 0; num19 < num18; num19++)
							{
								int index = (num19 + 1) % portalVertices.Count;
								float side = portalVertices[num19].side;
								float side2 = portalVertices[index].side;
								if ((side > 0f && side2 <= -0.001f) || (side2 > 0f && side <= -0.001f))
								{
									Vector4 vertex2 = portalVertices[num19].vertex;
									Vector4 vertex3 = portalVertices[index].vertex;
									float num20 = side / Vector4.Dot(a, vertex2 - vertex3);
									Vector4 vertex4 = vertex2 + num20 * (vertex3 - vertex2);
									vertex4.w = 1f;
									portalVertices.Insert(num19 + 1, new ClipVertex(vertex4, 0f));
									num18++;
								}
							}
							int num21 = 0;
							while (num21 < num18)
							{
								if (portalVertices[num21].side < -0.001f)
								{
									portalVertices.RemoveAt(num21);
									num18--;
								}
								else
								{
		...
		if (shadowLights.Count > 0 && CullShadows)
		{
			...
			while (enumerator.MoveNext())
			{
				...
				int count9 = sectors.Count;
				for (int num22 = 0; num22 < count9; num22++)
				{
					SECTR_Sector key2 = sectors[num22];
		...
		_ApplyCulling(visible: true);
		int count10 = occluderFrustums.Count;
		for (int num23 = 0; num23 < count10; num23++)
		{
			occluderFrustums[num23].Clear();
			frustumPool.Push(occluderFrustums[num23]);
		}
	...
	private void _FrustumCullSectorThread(ThreadCullData cullData)
	{
		...
		{
			dictionary = ((threadVisibleListPool.Count > 0) ? threadVisibleListPool.Pop() : new Dictionary<int, SECTR_Member.Child>(32));
			dictionary2 = ((threadVisibleListPool.Count > 0) ? threadVisibleListPool.Pop() : new Dictionary<int, SECTR_Member.Child>(32));
			dictionary3 = ((threadVisibleListPool.Count > 0) ? threadVisibleListPool.Pop() : new Dictionary<int, SECTR_Member.Child>(32));
		}
	...
	private void _HideAllMembers()
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			for (int j = 0; j < count2; j++)
			{
				...
				{
					if (child.renderer.GetType() == typeof(SkinnedMeshRenderer) && child.gameObject.GetComponent<Cloth>() != null && child.renderer.enabled)
					{
	...
	private void _ApplyCulling(bool visible)
	{
		...
		foreach (SECTR_Member.Child visibleClothRenderer in visibleClothRenderers)
		{
			if ((bool)visibleClothRenderer.renderer && !visibleRenderers.ContainsValue(visibleClothRenderer))
			{
				visibleClothRenderer.renderer.enabled = false;
			}
		}
		didCull = true;
	}
