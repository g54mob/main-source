...
namespace VLB
{
	...
	public class VolumetricShadowHD : MonoBehaviour
	{
		...
		[SerializeField]
		private int m_DepthMapDepth = 16;
		...
		public int depthMapDepth
		{
			...
			set
			{
				if (m_DepthCamera != null && Application.isPlaying)
				{
					Debug.LogErrorFormat(Consts.Shadow.GetErrorChangeRuntimeDepthMapDepth(this));
				}
		...
		private void ProcessOcclusionInternal()
		{
			...
			if (m_DepthCamera.targetTexture == null || !m_DepthCamera.targetTexture.IsCreated())
			{
				RenderTexture targetTexture = m_DepthCamera.targetTexture;
				int num = ((targetTexture != null) ? targetTexture.width : depthMapResolution);
				int depth = ((targetTexture != null) ? targetTexture.depth : depthMapDepth);
				RenderTexture renderTexture = new RenderTexture(num, num, depth, RenderTextureFormat.R8);
		...
		private void InstantiateOrActivateDepthCamera()
		{
			...
			if ((bool)m_DepthCamera && (bool)m_Master)
			{
				...
				RenderTexture renderTexture = new RenderTexture(depthMapResolution, depthMapResolution, depthMapDepth, RenderTextureFormat.R8);
				renderTexture.Create();
				m_DepthCamera.targetTexture = renderTexture;
				UpdateDepthCameraPropertiesAccordingToBeam();
			}
