...
public abstract class SECTR_FPController : MonoBehaviour
{
	...
	protected virtual void Update()
	{
		if (focused)
		{
			...
			{
				_UpdateTouches();
				screenJoystick = GetScreenJoystick(left: true);
			}
			else
			{
	...
	private void _UpdateTouches()
	{
		...
		for (int i = 0; i < touchCount; i++)
		{
			...
			{
				Debug.Log("Touch " + touch.fingerId + "Started at : " + touch.position.ToString());
			...
			{
				Debug.Log("Touch " + touch.fingerId + "Ended at : " + touch.position.ToString());
