...
namespace Michsky.UI.Heat
{
	...
	public class MenuManager : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			if (initPanel != null)
			{
				initPanel.gameObject.SetActive(value: true);
			}
			if (splashScreen != null)
			{
				splashScreen.gameObject.SetActive(value: false);
			}
		}
		...
		private void Initialize()
		{
			...
			mainContent.gameObject.SetActive(value: false);
			if (!bypassSplashScreen && UIManagerAsset.enableSplashScreen)
			{
				...
				AnimationClip[] animationClips = splashScreen.runtimeAnimatorController.animationClips;
				splashInTime = animationClips[0].length;
				splashOutTime = animationClips[1].length;
				splashScreen.enabled = true;
				splashScreen.gameObject.SetActive(value: true);
		...
		private IEnumerator FinalizeSplashScreen()
		{
			...
			mainContent.gameObject.SetActive(value: true);
			if (ControllerManager.instance != null && ControllerManager.instance.gamepadEnabled && ControllerManager.instance.firstSelected != null && ControllerManager.instance.firstSelected.activeInHierarchy)
			{
