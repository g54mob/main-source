...
namespace Michsky.UI.Heat
{
	public class SettingsDescriptionManager : MonoBehaviour
	{
		...
		private void CheckForLocalization()
		{
			...
			{
				localizedObject = base.gameObject.AddComponent<LocalizedObject>();
				...
				LocalizationSettings localizationSettings = LocalizationManager.instance.UIManagerAsset.localizationSettings;
				foreach (LocalizationSettings.Table table in localizationSettings.tables)
				{
					if (tableID == table.tableID)
					{
						localizedObject.tableIndex = localizationSettings.tables.IndexOf(table);
						break;
					}
				}
			}
