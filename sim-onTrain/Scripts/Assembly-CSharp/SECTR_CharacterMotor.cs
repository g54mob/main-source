...
public class SECTR_CharacterMotor : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (MoveWithPlatform())
		{
			...
			float y = (movingPlatform.activePlatform.rotation * movingPlatform.activeLocalRotation * Quaternion.Inverse(movingPlatform.activeGlobalRotation)).eulerAngles.y;
			if (y != 0f)
			{
		...
		{
			if (Vector3.SqrMagnitude(position - lastFootstepPosition) >= movement.footstepDistance * movement.footstepDistance)
			{
	...
	{
		return groundNormal.y <= Mathf.Cos(cachedController.slopeLimit * (MathF.PI / 180f));
	}
