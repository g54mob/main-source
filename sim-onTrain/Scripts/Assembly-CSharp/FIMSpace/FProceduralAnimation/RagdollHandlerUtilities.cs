	...
	public static class RagdollHandlerUtilities
	{
		...
		public static void AddPhysicsComponentsOnTheCharacterBones(RagdollHandler handler)
		{
			...
			foreach (RagdollBonesChain chain in handler.Chains)
			{
				foreach (RagdollChainBone boneSetup in chain.BoneSetups)
				{
					boneSetup.RefreshJoint(chain, isFallingOrSleep, onSource: true, playmodeRefresh: false, handler.InstantConnectedMassChange);
		...
		public static void User_UpdatePhysicsParametersForAllBones(this IRagdollAnimator2HandlerOwner iHandler)
		{
			...
			foreach (RagdollBonesChain chain in getRagdollHandler.Chains)
			{
				foreach (RagdollChainBone boneSetup in chain.BoneSetups)
				{
					boneSetup.RefreshJoint(chain, isFallingOrSleep, onSource: false, playmodeRefresh: true, getRagdollHandler.InstantConnectedMassChange);
				}
			}
		}
		...
		{
			RagdollHandler getRagdollHandler = iHandler.GetRagdollHandler;
			RagdollChainBone getAnchorBoneController = getRagdollHandler.GetAnchorBoneController;
			Vector3 vector = newPosition - getAnchorBoneController.GameRigidbody.position;
			bool isKinematic = getAnchorBoneController.GameRigidbody.isKinematic;
			getAnchorBoneController.GameRigidbody.isKinematic = true;
			getRagdollHandler.Dummy_Container.position += vector;
		...
		public static Quaternion User_GetMappedRotationHeadToHips(this IRagdollAnimator2HandlerOwner iHandler, Vector3? up = null, bool checkIfOnBack = true)
		{
			...
			if (checkIfOnBack)
			{
				if (iHandler.User_IsOnBack(canBeNone: false, vector))
				{
					return Quaternion.LookRotation(Vector3.ProjectOnPlane(-(getAnchorBoneController.PhysicalDummyBone.position - bone.PhysicalDummyBone.position), vector), vector);
				}
				return Quaternion.LookRotation(Vector3.ProjectOnPlane(getAnchorBoneController.PhysicalDummyBone.position - bone.PhysicalDummyBone.position, vector), vector);
			}
			return Quaternion.LookRotation(Vector3.ProjectOnPlane(getAnchorBoneController.PhysicalDummyBone.position - bone.PhysicalDummyBone.position, vector), vector);
