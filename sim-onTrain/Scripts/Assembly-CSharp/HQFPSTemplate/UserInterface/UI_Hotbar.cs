...
namespace HQFPSTemplate.UserInterface
{
	public class UI_Hotbar : UserInterfaceBehaviour
	{
		...
		[SerializeField]
		[EnableIf("m_FadeHotbarWhenNotUsed", true, 0f)]
		...
		[SerializeField]
		private TextAnchor m_NumberAnchor;
		...
		private void Start()
		{
			...
			if (m_EnableCustomNumbering)
			{
				...
				{
					Text text = GUIUtils.CreateTextUnder("Number", m_ContainerInterface.SlotInterfaces[i].GetComponent<RectTransform>(), m_NumberAnchor, m_NumberOffset);
					text.text = (i + 1).ToString();
					text.transform.SetAsLastSibling();
					text.color = m_NumberColor;
					text.font = ((m_CustomNumberingFont == null) ? base.UIManager.MainFont : m_CustomNumberingFont);
		...
		{
			if (m_FadeHotbarWhenNotUsed && m_NextTimeDisableHotbar < Time.time && !m_HotbarDisabled)
			{
		...
		private void SelectSlot(int slotIndex)
		{
			...
			RectTransform component = m_Frame.GetComponent<RectTransform>();
			component.SetParent(m_SelectedSlot.transform);
			component.localPosition = Vector2.zero;
			component.sizeDelta = m_SelectedSlot.GetComponent<RectTransform>().sizeDelta;
