...
namespace HQFPSTemplate.UserInterface
{
	public class UI_ItemWheel : UserInterfaceBehaviour
	{
		...
		[BHeader("Effects...")]
		[SerializeField]
		private bool m_EnableWheelArrow;

		[SerializeField]
		[ShowIf("m_EnableWheelArrow", true, 10f)]
		private RectTransform m_WheelArrow;

		[SerializeField]
		private bool m_SlowDownTime;

		[SerializeField]
		[ShowIf("m_SlowDownTime", true, 10f)]
		...
		public override void OnAttachment()
		{
			...
			base.UIManager.ItemWheel.SetStartTryer(TryStart_ItemWheelInspection);
			base.UIManager.ItemWheel.SetStopTryer(TryStop_ItemWheelInspection);
		}
		...
		private void Awake()
		{
			...
			{
				m_WheelArrow.gameObject.SetActive(value: false);
			}
		}

		private bool TryStart_ItemWheelInspection()
		{
			if (!base.Player.Aim.Active && Time.time > m_NextTimeCanToggleWheel && !base.Player.Healing.Active)
			{
				m_Panel.TryShow(show: true);
		...
		private void Update()
		{
			if (Input.GetKey(m_ItemWheelKey))
			{
				...
				{
					if (base.UIManager.ItemWheel.TryStart())
					{
			...
			else if (base.UIManager.ItemWheel.Active && base.UIManager.ItemWheel.TryStop())
			{
		...
		{
			Vector2 vector = new Vector2(Input.GetAxis("Mouse X"), Input.GetAxis("Mouse Y")).normalized * m_Range;
			if (vector != Vector2.zero)
			{
				m_DirectionOfSelection = Vector2.Lerp(m_DirectionOfSelection, vector, Time.deltaTime * m_Sensitivity);
			}
			m_CursorPos = m_DirectionOfSelection;
			float num = 0f - Vector2.SignedAngle(Vector2.up, m_CursorPos);
			if (num < 0f)
			{
			...
			{
				m_WheelArrow.rotation = Quaternion.Euler(0f, 0f, num);
			}
			for (int i = 0; i < m_WheelSlots.Length; i++)
			{
				if (num >= m_WheelSlots[i].AngleCoverage.x && num <= m_WheelSlots[i].AngleCoverage.y)
				{
					return i;
				}
			}
		...
		{
			if (!m_SlotDictionary.TryGetValue(slot, out var value))
			{
				return;
			}
			if (value != null && value.HasItem)
			{
				m_ItemNameText.text = value.Item.Name;
				if (value.Item.Info.Description.Length > 0)
				{
					m_DescriptionText.text = value.Item.Info.Description;
				}
				if (m_TileSeparator != null && !m_TileSeparator.activeSelf)
				{
					m_TileSeparator.SetActive(value: true);
				}
			}
			else
			{
				...
				{
					m_TileSeparator.SetActive(value: false);
				}
			}
		}
