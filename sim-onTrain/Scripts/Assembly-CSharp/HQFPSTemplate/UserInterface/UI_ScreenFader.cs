...
namespace HQFPSTemplate.UserInterface
{
	public class UI_ScreenFader : MonoBehaviour
	{
		...
		{
			StopAllCoroutines();
			StartCoroutine(C_FadeScreen(targetAlpha));
		}
		...
		private void Start()
		{
			...
			{
				Singleton<GameManager>.Instance.CurrentPlayer.Death.AddListener(On_Death);
				Singleton<GameManager>.Instance.CurrentPlayer.Respawn.AddListener(On_Respawn);
			});
