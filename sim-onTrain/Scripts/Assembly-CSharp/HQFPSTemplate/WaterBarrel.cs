...
namespace HQFPSTemplate
{
	public class WaterBarrel : MonoBehaviour, IDamageable
	{
		...
		public void TakeDamage(DamageInfo damage)
		{
			...
			{
				ParticleSystem particleSystem = Object.Instantiate(m_WaterFX, damage.HitPoint, Quaternion.LookRotation((damage.HitNormal == Vector3.zero) ? (-damage.HitDirection) : damage.HitNormal));
				particleSystem.transform.SetParent(base.transform);
				m_WaterLeaks.Add(particleSystem);
		...
		private void Update()
		{
			if (m_CurrentWaterQuantity <= 0f || m_WaterLeaks.Count == 0)
			{
				return;
			}
			m_CurrentWaterQuantity -= Time.deltaTime * (float)m_WaterLeaks.Count;
			for (int i = 0; i < m_WaterLeaks.Count; i++)
			{
				float num = Mathf.Clamp(m_DefaultStartSpeed * (m_CurrentWaterQuantity / m_WaterQuantity), 0f, m_DefaultStartSpeed);
				ParticleSystem.MainModule main = m_WaterLeaks[i].main;
				main.startSpeedMultiplier = num;
				if (num <= 0f)
				{
					m_WaterLeaks[i].Stop();
					m_AudioSource.Stop();
