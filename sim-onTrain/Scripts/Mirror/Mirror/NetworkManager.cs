...
namespace Mirror
{
	...
	public class NetworkManager : MonoBehaviour
	{
		...
		{
			if (NetworkClient.active && !(NetworkTime.time < lastConnectionQualityUpdate + (double)connectionQualityInterval))
			{
		...
		public void StopServer()
		{
			if (NetworkServer.active)
			{
				...
				if (base.gameObject != null && base.gameObject.scene.name == "DontDestroyOnLoad" && !string.IsNullOrWhiteSpace(offlineScene))
				{
					_ = SceneManager.GetActiveScene().path != offlineScene;
				}
				OnStopServer();
