	...
	public class NetworkTransformReliable : NetworkTransformBase
	{
		...
		protected virtual void OnClientToServerSync(Vector3? position, Quaternion? rotation, Vector3? scale)
		{
			if (syncDirection == SyncDirection.ClientToServer && serverSnapshots.Count < base.connectionToClient.snapshotBufferSizeLimit)
			{
				...
				{
					RewriteHistory(serverSnapshots, base.connectionToClient.remoteTimeStamp, NetworkTime.localTime, NetworkServer.sendInterval * (float)sendIntervalMultiplier, GetPosition(), GetRotation(), GetScale());
				}
		...
		protected virtual void OnServerToClientSync(Vector3? position, Quaternion? rotation, Vector3? scale)
		{
			if (!base.IsClientWithAuthority)
			{
				...
				{
					RewriteHistory(clientSnapshots, NetworkClient.connection.remoteTimeStamp, NetworkTime.localTime, NetworkClient.sendInterval * (float)sendIntervalMultiplier, GetPosition(), GetRotation(), GetScale());
				}
