...
namespace Mirror.SimpleWeb
{
	...
	public class SimpleWebTransport : Transport, PortTransport
	{
		...
		public override void ServerSend(int connectionId, ArraySegment<byte> segment, int channelId)
		{
			if (!ServerActive())
			{
				Log.Error("[SimpleWebTransport] Server Not Active", showColor: false);
				return;
			}
			if (segment.Count > maxMessageSize)
			{
				Log.Error("[SimpleWebTransport] Message greater than max size", showColor: false);
				return;
			}
			if (segment.Count == 0)
			{
				Log.Error("[SimpleWebTransport] Message count was zero", showColor: false);
				return;
			}
