...
namespace GPUInstancerPro
{
	public class GPUICameraData : IGPUIDisposable, IDisposable
	{
		private struct GPUICameraBufferData
		{
			...
			public Vector4 cameraPositionAndHalfAngle;
		}
		...
		protected void MakeVisibilityCalculations()
		{
			...
			bool flag2 = OcclusionCullingData != null && OcclusionCullingData.IsHiZDepthUpdated;
			...
			foreach (GPUIRenderSourceGroup value in GPUIRenderingSystem.Instance.RenderSourceGroupProvider.Values)
			{
				...
				GPUIShaderBuffer transformBuffer = value.TransformBufferData.GetTransformBuffer(this);
				if (transformBuffer.Buffer == null)
				{
					continue;
				}
				_additionalValues.z = Mathf.Max(maximumLODLevel, value.Profile.maximumLODLevel);
				GPUIShaderBuffer instanceDataBuffer = value.TransformBufferData.GetInstanceDataBuffer(this);
				...
				if (flag3)
				{
					...
					{
						if (value.Profile.isAnimateCrossFade && !value.TransformBufferData.IsCameraBasedBuffer)
						{
							...
							_additionalValues.x = ((value.TransformBufferData.resetCrossFadeDataFrame == frameCount) ? 0f : GPUIRenderingSystem.Instance.TimeSinceLastDrawCall);
						}
						else
						{
							_CS_CameraVisibility.DisableKeyword("GPUI_LOD");
				...
				for (int i = 0; i < value.RenderSources.Count; i++)
				{
					...
					int num = (value.TransformBufferData.IsCameraBasedBuffer ? value.BufferSize : gPUIRenderSource.instanceCount);
