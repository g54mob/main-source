	...
	public class GPUIDetailManager : GPUITerrainManager<GPUIDetailPrototypeData>
	{
		...
		private void UpdateDetailMatrices(GPUICameraData cameraData)
		{
			...
			for (int j = 0; j < num; j++)
			{
				...
				if (GPUIRenderingSystem.TryGetRenderSourceGroup(_runtimeRenderKeys[j], out var _) && cameraData.TryGetShaderBuffer(_runtimeRenderKeys[j], out var shaderBuffer))
				{
					...
					foreach (GPUITerrain item in activeTerrainValues)
					{
						...
						{
							if (!item.IsDetailDensityTexturesLoaded)
							{
		...
		protected override void BeginDeterminePrototypeIndexes()
		{
			...
			for (int num = terrainCount - 1; num >= 0; num--)
			{
				...
				if (!(terrain == null) && terrain.DetailPrototypeIndexes != null && terrain.DetailPrototypes != null)
				{
					...
					{
						int terrainDetailPrototypeIndex = terrain.GetTerrainDetailPrototypeIndex(i);
						if (terrainDetailPrototypeIndex >= 0 && terrain.DetailPrototypes.Length > terrainDetailPrototypeIndex)
						{
							DetailPrototype detailPrototype = terrain.DetailPrototypes[terrainDetailPrototypeIndex];
							if (_prototypeDataArray[i].IsMatchingPrefabAndTexture(detailPrototype, _prototypes[i], checkPropertyValues: false))
							{
								_prototypeDataArray[i].ReadFromDetailPrototypeData(terrain.DetailPrototypes[terrainDetailPrototypeIndex], terrain.DetailPrototypeIndexes[terrainDetailPrototypeIndex] / 1000, this, i);
							}
						}
					}
				}
