...
namespace StylizedWater2
{
	public static class Buoyancy
	{
		...
		private static float4 realSpeed;
		...
		{
			return waterMat.IsKeywordEnabled("_WAVES");
		}

		[Obsolete("Use the function with a 4th bool parameter to indicate if the water material is dynamic")]
		public static float SampleWaves(float3 position, WaterObject waterObject, float rollStrength, out Vector3 normal)
		{
			return SampleWaves(position, waterObject.material, waterObject.transform.position.y, rollStrength, dynamicMaterial: false, out normal);
		}

		[Obsolete("Use the function with a 4th bool parameter to indicate if the water material is dynamic")]
		public static float SampleWaves(float3 position, Material waterMat, float waterLevel, float rollStrength, out Vector3 normal)
		{
			return SampleWaves(position, waterMat, waterLevel, rollStrength, dynamicMaterial: false, out normal);
		}
		...
		private static void SampleWaves(Vector3 position, Material waterMat, float waterLevel, float rollStrength, bool dynamicMaterial, out Vector3 offset, out Vector3 normal)
		{
			...
			{
				normal = Vector3.up;
				offset = new float3(0f, waterLevel, 0f);
			}
			...
			float4 obj = new float4(1.3f, 1.35f, 1.25f, 1.25f) * (1f - waveParameters.distance) * 3f;
			...
			for (int i = 0; i <= waveParameters.count; i++)
			{
				...
				zero.x += Dot4(cosine, new float4(AB.x, AB.z, CD.x, CD.z));
				zero.y += Dot4(sine, amp);
				zero.z += Dot4(cosine, new float4(AB.y, AB.w, CD.y, CD.w));
			}
			rollStrength *= Mathf.Lerp(0.001f, 0.1f, waveParameters.steepness);
			normal = new float3((0f - zero.x) * rollStrength * waveParameters.height, 2f, (0f - zero.z) * rollStrength * waveParameters.height);
			normal = math.normalize(normal);
			zero.y /= waveParameters.count;
			zero.y = zero.y * waveParameters.height + waterLevel;
			offset = zero;
		}
