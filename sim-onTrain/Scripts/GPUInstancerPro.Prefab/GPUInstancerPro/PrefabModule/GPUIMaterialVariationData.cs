	...
	public class GPUIMaterialVariationData : IGPUIDisposable, IDisposable
	{
		...
		internal void Initialize()
		{
			...
			if (!(_definition.material.shader == shader))
			{
				...
				int key = _definition.material.GetInstanceID() + string.Concat(list).GetHashCode();
				if (!GPUIRenderingSystem.Instance.MaterialProvider.TryGetData(key, out var result) || !(result != null) || !result.name.EndsWith("_MV" + GPUIShaderBindings.GPUI_REPLACEMENT_MATERIAL_NAME_SUFFIX))
				{
					...
					GPUIRenderingSystem.Instance.MaterialProvider.AddOrSet(key, result);
					list.Add("LOD_FADE_CROSSFADE");
					list.Sort();
					key = _definition.material.GetInstanceID() + string.Concat(list).GetHashCode();
