...
namespace Pathfinding.Recast
{
	internal class RecastMeshGatherer
	{
		...
		public void CollectSceneMeshes(List<RasterizationMesh> meshes)
		{
			...
			List<MeshFilter> list = FilterMeshes(UnityEngine.Object.FindObjectsOfType<MeshFilter>(), tagMask, mask);
		...
		private void CollectTreeMeshes(Terrain terrain, List<RasterizationMesh> result)
		{
			...
			for (int i = 0; i < terrainData.treeInstances.Length; i++)
			{
				...
				Vector3 a = new Vector3(treeInstance.widthScale, treeInstance.heightScale, treeInstance.widthScale);
				a = Vector3.Scale(a, treePrototype.prefab.transform.localScale);
				if (component == null)
				{
					...
					Matrix4x4 matrix = Matrix4x4.TRS(pos, Quaternion.identity, a * 0.5f);
					RasterizationMesh item = new RasterizationMesh(BoxColliderVerts, BoxColliderTris, bounds, matrix);
					result.Add(item);
					continue;
				}
				RasterizationMesh rasterizationMesh = RasterizeCollider(component, Matrix4x4.TRS(pos, Quaternion.identity, a));
		...
		private RasterizationMesh RasterizeCapsuleCollider(float radius, float height, Bounds bounds, Matrix4x4 localToWorldMatrix)
		{
			...
			if (capsuleCache == null)
			{
				...
				for (int j = 0; j < num; j++)
				{
					...
					{
						array[k + j * num2] = new Vector3(Mathf.Cos((float)k * (float)Math.PI * 2f / (float)num2) * Mathf.Sin((float)j * (float)Math.PI / (float)(num - 1)), Mathf.Cos((float)j * (float)Math.PI / (float)(num - 1)) + ((j < num / 2) ? height : (0f - height)), Mathf.Sin((float)k * (float)Math.PI * 2f / (float)num2) * Mathf.Sin((float)j * (float)Math.PI / (float)(num - 1)));
					}
				}
				...
				for (int l = 1; l < num; l++)
				{
					...
					while (num4 < num2)
					{
						list.Add(l * num2 + num4);
						list.Add(l * num2 + num5);
						list.Add((l - 1) * num2 + num4);
