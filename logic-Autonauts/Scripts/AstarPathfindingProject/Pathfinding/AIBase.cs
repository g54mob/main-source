...
namespace Pathfinding
{
	...
	public abstract class AIBase : VersionedMonoBehaviour
	{
		...
		protected virtual bool shouldRecalculatePath
		{
			get
			{
				if (Time.time - lastRepath >= repathRate && !waitingForPathCalculation && canSearch)
				{
					return !float.IsPositiveInfinity(destination.x);
				}
				return false;
			}
		}
		...
		protected void ApplyGravity(float deltaTime)
		{
			...
			{
				float elevation;
				velocity2D += movementPlane.ToPlane(deltaTime * (float.IsNaN(gravity.x) ? Physics.gravity : gravity), out elevation);
