	...
	public class BZip2InputStream : Stream
	{
		...
		private void InitBlock()
		{
			...
			if (c != '1' || c2 != 'A' || c3 != 'Y' || c4 != '&' || c5 != 'S' || c6 != 'Y')
			{
				...
				string message = string.Format("bad block header at offset 0x{0:X}", input.Position);
				throw new IOException(message);
			}
		...
		private void EndBlock()
		{
			...
			{
				string message = string.Format("BZip2 CRC error (expected {0:X8}, computed {1:X8})", storedBlockCRC, computedBlockCRC);
				throw new IOException(message);
			}
		...
		private void complete()
		{
			...
			{
				string message = string.Format("BZip2 CRC error (expected {0:X8}, computed {1:X8})", storedCombinedCRC, computedCombinedCRC);
				throw new IOException(message);
			}
		}
		...
		private void SetupBlock()
		{
			...
			for (i = 0; i <= 256; i++)
			{
				...
				{
					string message = string.Format("BZ_DATA_ERROR: cftab[{0}]={1} last={2}", i, decompressionState.cftab[i], last);
