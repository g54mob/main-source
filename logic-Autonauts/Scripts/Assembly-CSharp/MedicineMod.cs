...
public class MedicineMod : Medicine
{
	public override void Restart()
	{
		...
		Vector3 Rot;
		if (ModManager.Instance.ModMedicineClass.GetModelRotation(m_TypeIdentifier, out Rot))
		{
			m_ModelRoot.transform.localRotation = Quaternion.Euler(Rot.x, Rot.y, Rot.z);
		}
		Vector3 Trans;
		if (ModManager.Instance.ModMedicineClass.GetModelTranslation(m_TypeIdentifier, out Trans))
		{
