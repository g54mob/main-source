	...
	public class SoundGroupVariationUpdater : MonoBehaviour
	{
		...
		public bool RayCastForOcclusion()
		{
			...
			if (_maThisFrame.occlusionUseLayerMask)
			{
				...
				{
					RaycastHit hitInfo;
					if (Physics.Raycast(vector, vector2, out hitInfo, magnitude, _maThisFrame.occlusionLayerMask.value))
					{
						flag3 = true;
			...
			else
			{
				...
				{
					RaycastHit hitInfo2;
					if (Physics.Raycast(vector, vector2, out hitInfo2, magnitude))
					{
						flag3 = true;
		...
		public void MaybeChain()
		{
			...
			if (playSoundParm.AttachToSource || playSoundParm.SourceTrans != null)
			{
				...
				{
					MasterAudio.PlaySound3DFollowTransform(playSoundParm.SoundType, playSoundParm.SourceTrans, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, _clipSchedEndTime, true);
				}
				else
				{
					MasterAudio.PlaySound3DAtTransform(playSoundParm.SoundType, playSoundParm.SourceTrans, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, _clipSchedEndTime, true);
				}
			}
			else
			{
				MasterAudio.PlaySound(playSoundParm.SoundType, playSoundParm.VolumePercentage, playSoundParm.Pitch, delaySoundTime, null, _clipSchedEndTime, true);
			}
		}
