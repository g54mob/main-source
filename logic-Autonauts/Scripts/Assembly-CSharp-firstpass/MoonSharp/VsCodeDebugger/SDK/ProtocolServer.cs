	...
	public abstract class ProtocolServer
	{
		...
		private void ProcessData()
		{
			while (true)
			{
				...
				string text = _rawData.GetString(Encoding);
				int num = text.IndexOf("\r\n\r\n");
				if (num != -1)
				{
					Match match = CONTENT_LENGTH_MATCHER.Match(text);
		...
		protected void SendMessage(ProtocolMessage message)
		{
			...
			if (TRACE && message.type == "event")
			{
				...
				Console.Error.WriteLine(string.Format("E {0}: {1}", obj.@event, JsonTableConverter.ObjectToJson(obj.body)));
			}
