...
namespace MoonSharp.VsCodeDebugger.DebuggerLogic
{
	internal class MoonSharpDebugSession : DebugSession, IAsyncDebuggerClient
	{
		...
		{
			string text = getString(args, "expression");
			int num = getInt(args, "frameId", 0);
			string text2 = getString(args, "context") ?? "hover";
			if (num != 0 && text2 != "repl")
			{
				SendText("Warning : Evaluation of variables/watches is always done with the top-level scope.");
			}
			if (text2 == "repl" && text.StartsWith("!"))
			{
				ExecuteRepl(text.Substring(1));
				SendResponse(response);
				return;
			}
			DynValue dynValue = m_Debug.Evaluate(text) ?? DynValue.Nil;
		...
		public override void StackTrace(Response response, Table args)
		{
			...
			int i = 0;
			for (int num2 = Math.Min(num - 3, watches.Count); i < num2; i++)
			{
		...
		{
			int num = getInt(arguments, "variablesReference", -1);
			...
			else if (num < 0 || num >= m_Variables.Count)
			{
				list.Add(new Variable("<error>", null));
			}
			else
			{
				VariableInspector.InspectVariable(m_Variables[num], list);
			}
