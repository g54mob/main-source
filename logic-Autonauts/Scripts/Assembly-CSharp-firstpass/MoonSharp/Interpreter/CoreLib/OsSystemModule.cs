...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class OsSystemModule
	{
		...
		{
			string text = args.AsType(0, "remove", DataType.String).String;
			try
			{
				if (Script.GlobalOptions.Platform.OS_FileExists(text))
				{
					Script.GlobalOptions.Platform.OS_FileDelete(text);
					return DynValue.True;
				}
				return DynValue.NewTuple(DynValue.Nil, DynValue.NewString("{0}: No such file or directory.", text), DynValue.NewNumber(-1.0));
			}
			catch (Exception ex)
			{
		...
		{
			string text = args.AsType(0, "rename", DataType.String).String;
			string dst = args.AsType(1, "rename", DataType.String).String;
			try
			{
				if (!Script.GlobalOptions.Platform.OS_FileExists(text))
				{
				...
				Script.GlobalOptions.Platform.OS_FileMove(text, dst);
				return DynValue.True;
			}
			catch (Exception ex)
			{
