...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class IoModule
	{
		...
		private static void SetStandardFile(Script S, StandardFileType file, Stream optionsStream)
		{
			...
			registry.Set(value: UserData.Create((file != StandardFileType.StdIn) ? StandardIOFileUserDataBase.CreateOutputStream(optionsStream) : StandardIOFileUserDataBase.CreateInputStream(optionsStream)), key: "853BEAAF298648839E2C99D005E1DF94_STD_" + file);
		}
		...
		public static DynValue open(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			try
			{
				...
				return UserData.Create(Open(executionContext, filename, encoding, text));
			}
			catch (Exception ex)
			{
				return DynValue.NewTuple(DynValue.Nil, DynValue.NewString(IoExceptionToLuaMessage(ex, filename)));
			}
		}
