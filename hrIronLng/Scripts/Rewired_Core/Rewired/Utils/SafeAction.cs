...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> buNgGXopEIgyYeMtrvoysyFJnUaM;

		private static Action<object, Action> invokeDelegate => PPJRmIdjvlGCIhzCKQhRGPLcljj;
		...
		private static void PPJRmIdjvlGCIhzCKQhRGPLcljj(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	internal sealed class SafeAction<T> : SafeDelegate<Action<T>>
	{
		...
		private static Action<object, Action<T>> buNgGXopEIgyYeMtrvoysyFJnUaM;

		private static Action<object, Action<T>> invokeDelegate => PPJRmIdjvlGCIhzCKQhRGPLcljj;
		...
		private static void PPJRmIdjvlGCIhzCKQhRGPLcljj(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.tQdDnYjKVGARcwqmQpfqoOulamx);
			}
		}
	...
	internal sealed class SafeAction<T, T2> : SafeDelegate<Action<T, T2>>
	{
		...
		private T2 OORmxRCgSueUoidHAWNzWeNLUOS;

		private static Action<object, Action<T, T2>> buNgGXopEIgyYeMtrvoysyFJnUaM;

		private static Action<object, Action<T, T2>> invokeDelegate => PPJRmIdjvlGCIhzCKQhRGPLcljj;
		...
		{
			tQdDnYjKVGARcwqmQpfqoOulamx = arg0;
			OORmxRCgSueUoidHAWNzWeNLUOS = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			tQdDnYjKVGARcwqmQpfqoOulamx = default(T);
			OORmxRCgSueUoidHAWNzWeNLUOS = default(T2);
		}
		...
		private static void PPJRmIdjvlGCIhzCKQhRGPLcljj(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.tQdDnYjKVGARcwqmQpfqoOulamx, safeAction.OORmxRCgSueUoidHAWNzWeNLUOS);
			}
		}
