...
namespace Rewired.Utils
{
	public static class UnityTools
	{
		...
		{
			public enum BsBnrDUceCautaMFaiIHnJMzJcDj
			{
				odeWcOuWsGKaiRcjzjJIChubjUVx = 0,
				nCwfLfGPqASVtmeIqdGSQdFkmjc = 1,
				BHeIhcMUJELNPwEvlNlPTwchCIx = 2
			}
			...
			public readonly BsBnrDUceCautaMFaiIHnJMzJcDj type;

			public readonly int build;

			public UnityVersionClass(string versionString)
			{
				type = BsBnrDUceCautaMFaiIHnJMzJcDj.odeWcOuWsGKaiRcjzjJIChubjUVx;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = BsBnrDUceCautaMFaiIHnJMzJcDj.nCwfLfGPqASVtmeIqdGSQdFkmjc;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = BsBnrDUceCautaMFaiIHnJMzJcDj.BHeIhcMUJELNPwEvlNlPTwchCIx;
					}
			...
			{
				return major + "." + minor + "." + maintenance + NvjCPxhGWmdWzcMrPmsVrbChsWb(type) + build;
			}

			private string NvjCPxhGWmdWzcMrPmsVrbChsWb(BsBnrDUceCautaMFaiIHnJMzJcDj P_0)
			{
				return P_0 switch
				{
					BsBnrDUceCautaMFaiIHnJMzJcDj.odeWcOuWsGKaiRcjzjJIChubjUVx => "f", 
					BsBnrDUceCautaMFaiIHnJMzJcDj.nCwfLfGPqASVtmeIqdGSQdFkmjc => "b", 
					BsBnrDUceCautaMFaiIHnJMzJcDj.BHeIhcMUJELNPwEvlNlPTwchCIx => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (ycwCZFjJqKhKBItyiACqGkdxCBW(a.type) > ycwCZFjJqKhKBItyiACqGkdxCBW(b.type))
				{
					return 1;
				}
				if (ycwCZFjJqKhKBItyiACqGkdxCBW(a.type) < ycwCZFjJqKhKBItyiACqGkdxCBW(b.type))
			...
			private static int ycwCZFjJqKhKBItyiACqGkdxCBW(BsBnrDUceCautaMFaiIHnJMzJcDj P_0)
			{
				return P_0 switch
				{
					BsBnrDUceCautaMFaiIHnJMzJcDj.nCwfLfGPqASVtmeIqdGSQdFkmjc => 0, 
					BsBnrDUceCautaMFaiIHnJMzJcDj.odeWcOuWsGKaiRcjzjJIChubjUVx => 10, 
					BsBnrDUceCautaMFaiIHnJMzJcDj.BHeIhcMUJELNPwEvlNlPTwchCIx => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion eBEWSgBsZjTAgSGzvgOzunVMjwl = UnityVersion.UNITY_5_0;

		private static UnityVersionClass NmOtStLrFmNqVqFRDKbAinHPhYL;

		private static UnityVersion tyxrnFCsUIWgzhpagLhcSkMzfXg = UnityVersion.Unknown;

		private static string RdXAjFMYiRsgwwaXcFOYHBlzLdf;

		private static Platform KyfhhHnFzYINjAaEMDzZGerAzRSl;

		private static EditorPlatform tKHPgJVsdrQQjIgOmOzDCJwSQLI;

		private static bool TRYyhztFLnjkRDpKoBNFZJHtUIf;

		private static bool nPLLKPaRzzMBweCuqwFziTYjFUw;

		private static bool edCMJYfZRHjxWjXwDCAuaIhbFSY;

		private static WebplayerPlatform BCOeRMjFOnlSbaKyMghOdWRgLhUr;

		private static bool kotBjBcNGGLDGhFKZTvedUYBhOqr;

		private static bool lyFjHCDYlvPGUUZVthczhZAEWtD;

		private static bool rJdlKPfgQKHtlDAtkIVQmPJndmx;

		private static bool jPRLOUkrzfIOOeaLcCekqCAqvFy;

		private static bool aanviCImfOzJVFDynbrOoxyGQEf;

		private static bool FFROfbTjbqXjdMnwwXUfZLbIcTU;

		private static string rhmzlZDoZIXNqddaRCOilXEIYVh;

		private static ScriptingBackend pzANuuZgTdmgyVhRNOWiXlbLemr;

		private static ScriptingAPILevel USVmZfvafJavIzFdQKwbyWmdRJQ;

		private static bool iAfIUWYUAqdDChvbEdbsELWGLvYg;

		private static IExternalTools RMaapQeVnyNDkTiNBhoJXDxGFKK;

		private static bool mprztguOflQxkSCjtHDpdlZFCU;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper RQKBSREktSMsErrEYmLVnDfjPFn;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				...
				return NmOtStLrFmNqVqFRDKbAinHPhYL;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return RdXAjFMYiRsgwwaXcFOYHBlzLdf;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return KyfhhHnFzYINjAaEMDzZGerAzRSl;
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return tKHPgJVsdrQQjIgOmOzDCJwSQLI;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return TRYyhztFLnjkRDpKoBNFZJHtUIf;
			}
		}

		public static bool isPlaying => nPLLKPaRzzMBweCuqwFziTYjFUw;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return edCMJYfZRHjxWjXwDCAuaIhbFSY;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return BCOeRMjFOnlSbaKyMghOdWRgLhUr;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (TRYyhztFLnjkRDpKoBNFZJHtUIf || Application.isEditor)
				{
					return true;
				}
				...
				switch (KyfhhHnFzYINjAaEMDzZGerAzRSl)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!edCMJYfZRHjxWjXwDCAuaIhbFSY)
					{
						return pzANuuZgTdmgyVhRNOWiXlbLemr == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return tKHPgJVsdrQQjIgOmOzDCJwSQLI switch
				{
					EditorPlatform.Windows => KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.Windows, 
					EditorPlatform.OSX => KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.OSX, 
					EditorPlatform.Linux => KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return kotBjBcNGGLDGhFKZTvedUYBhOqr;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return lyFjHCDYlvPGUUZVthczhZAEWtD;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.Android && KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.Ouya && KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.AmazonFireTV)
				{
					return KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.iOS)
				{
					return KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.Windows && KyfhhHnFzYINjAaEMDzZGerAzRSl != Platform.Linux)
				{
					return KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return rJdlKPfgQKHtlDAtkIVQmPJndmx;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_0)
				{
					return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool supportsStadia
		{
			get
			{
				...
				return tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_2019_3;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return iAfIUWYUAqdDChvbEdbsELWGLvYg;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => pzANuuZgTdmgyVhRNOWiXlbLemr;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => USVmZfvafJavIzFdQKwbyWmdRJQ;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return RMaapQeVnyNDkTiNBhoJXDxGFKK;
			}
		}
		...
		internal static IAndroidFallbackPlatformHelper androidFallbackPlatformHelper
		{
			...
			get
			{
				return RQKBSREktSMsErrEYmLVnDfjPFn;
			}
			[CompilerGenerated]
			set
			{
				RQKBSREktSMsErrEYmLVnDfjPFn = value;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => mprztguOflQxkSCjtHDpdlZFCU;

		private static bool initialized => mQcjpNTWZTUSZkPfZvNiwIrkwvr();

		private static bool mQcjpNTWZTUSZkPfZvNiwIrkwvr()
		{
			if (mprztguOflQxkSCjtHDpdlZFCU)
			{
				return true;
			}
			try
			{
				RdXAjFMYiRsgwwaXcFOYHBlzLdf = Application.unityVersion;
				NmOtStLrFmNqVqFRDKbAinHPhYL = new UnityVersionClass(RdXAjFMYiRsgwwaXcFOYHBlzLdf);
				NkYMuGhDvBbiGfUiCmZxEEpMwVQ();
				mprztguOflQxkSCjtHDpdlZFCU = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return mprztguOflQxkSCjtHDpdlZFCU;
		}

		internal static void iDBXctPcOcjjzWbKaCnxuPiVNUc(Platform P_0, EditorPlatform P_1, bool P_2, WebplayerPlatform P_3, ScriptingBackend P_4, ScriptingAPILevel P_5, IExternalTools P_6)
		{
			if (initialized)
			{
				...
				KyfhhHnFzYINjAaEMDzZGerAzRSl = P_0;
				tKHPgJVsdrQQjIgOmOzDCJwSQLI = P_1;
				TRYyhztFLnjkRDpKoBNFZJHtUIf = P_2;
				BCOeRMjFOnlSbaKyMghOdWRgLhUr = P_3;
				pzANuuZgTdmgyVhRNOWiXlbLemr = P_4;
				USVmZfvafJavIzFdQKwbyWmdRJQ = P_5;
				if (RMaapQeVnyNDkTiNBhoJXDxGFKK != null)
				{
					RMaapQeVnyNDkTiNBhoJXDxGFKK.Destroy();
				}
				RMaapQeVnyNDkTiNBhoJXDxGFKK = P_6;
				edCMJYfZRHjxWjXwDCAuaIhbFSY = Debug.isDebugBuild;
				nPLLKPaRzzMBweCuqwFziTYjFUw = true;
				EbtCckYSxDCxAeseSfxiXRehHfh();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			agvIUuvgPBjDdfzjTXdizlHNjym(minVersionStr, out var num);
			agvIUuvgPBjDdfzjTXdizlHNjym(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (flag && NmOtStLrFmNqVqFRDKbAinHPhYL < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag2 && NmOtStLrFmNqVqFRDKbAinHPhYL >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag2 && NmOtStLrFmNqVqFRDKbAinHPhYL > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool agvIUuvgPBjDdfzjTXdizlHNjym(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void NkYMuGhDvBbiGfUiCmZxEEpMwVQ()
		{
			tyxrnFCsUIWgzhpagLhcSkMzfXg = rcjvUkSZbnxCdkkZFYJQOJrQegf(Application.unityVersion);
			if (tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_3_5 && tyxrnFCsUIWgzhpagLhcSkMzfXg < UnityVersion.UNITY_4_0)
			{
				kotBjBcNGGLDGhFKZTvedUYBhOqr = true;
			}
			else if (tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_0)
			{
				lyFjHCDYlvPGUUZVthczhZAEWtD = true;
			}
		}

		private static UnityVersion rcjvUkSZbnxCdkkZFYJQOJrQegf(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion iamGYYEDeVVkflcgtKmtnbEvdYfL(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion xhrafjKKhAgMFOmrkKHmELcDzTtf(int P_0)
		{
			return P_0 switch
			{
		...
		private static void EbtCckYSxDCxAeseSfxiXRehHfh()
		{
			switch (KyfhhHnFzYINjAaEMDzZGerAzRSl)
			{
			...
			case Platform.RazerForgeTV:
				jPRLOUkrzfIOOeaLcCekqCAqvFy = true;
				aanviCImfOzJVFDynbrOoxyGQEf = true;
				break;
			case Platform.PS4:
				jPRLOUkrzfIOOeaLcCekqCAqvFy = true;
				rhmzlZDoZIXNqddaRCOilXEIYVh = "Empty";
				FFROfbTjbqXjdMnwwXUfZLbIcTU = true;
				break;
			case Platform.Windows:
				if ((tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_6_3p1 && tyxrnFCsUIWgzhpagLhcSkMzfXg < UnityVersion.UNITY_5_0) || tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_0_0p1)
				{
					rJdlKPfgQKHtlDAtkIVQmPJndmx = true;
					jPRLOUkrzfIOOeaLcCekqCAqvFy = true;
				}
				break;
			case Platform.Linux:
			...
			if (TRYyhztFLnjkRDpKoBNFZJHtUIf)
			{
				EditorPlatform editorPlatform = tKHPgJVsdrQQjIgOmOzDCJwSQLI;
				if (editorPlatform == EditorPlatform.Windows && ((tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_6_3p1 && tyxrnFCsUIWgzhpagLhcSkMzfXg < UnityVersion.UNITY_5_0) || tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_5_0_0p1))
				{
					rJdlKPfgQKHtlDAtkIVQmPJndmx = true;
					jPRLOUkrzfIOOeaLcCekqCAqvFy = true;
				}
			}
			if ((!TRYyhztFLnjkRDpKoBNFZJHtUIf && KyfhhHnFzYINjAaEMDzZGerAzRSl == Platform.Windows) || (TRYyhztFLnjkRDpKoBNFZJHtUIf && tKHPgJVsdrQQjIgOmOzDCJwSQLI == EditorPlatform.Windows))
			{
				iAfIUWYUAqdDChvbEdbsELWGLvYg = tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type ZVNkteZXhHrfwgGDUriTJDAnZLx(gQgAdCJxgadasEGbfdXkJAHxNFcj P_0)
		{
			if (!initialized)
			{
				return null;
			}
			if (tyxrnFCsUIWgzhpagLhcSkMzfXg >= UnityVersion.UNITY_4_3)
			{
				return KrREyZBxXzPPycjFywmhFQtCPTT(P_0);
			}
			return null;
		}

		private static Type KrREyZBxXzPPycjFywmhFQtCPTT(gQgAdCJxgadasEGbfdXkJAHxNFcj P_0)
		{
			return P_0 switch
			{
				gQgAdCJxgadasEGbfdXkJAHxNFcj.UGchKtJzRCIXfWjDccNDzMyMOLn => typeof(RigidbodyInterpolation2D), 
				gQgAdCJxgadasEGbfdXkJAHxNFcj.qRIFlJBLklocLccCqTTRVMPCRXx => typeof(RigidbodySleepMode2D), 
				gQgAdCJxgadasEGbfdXkJAHxNFcj.QdjjKDFMygjpOFizZyGPKVDMOmRK => typeof(CollisionDetectionMode2D), 
				gQgAdCJxgadasEGbfdXkJAHxNFcj.KaJrKsxJgmxZOXzKPQoTjiBnIrL => typeof(PhysicsMaterial2D), 
				gQgAdCJxgadasEGbfdXkJAHxNFcj.gPHdJCALXSDGUIXcJjpwGCOEBzbp => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return UehbBJobCGJxMARgGtgHfymglAz(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return UehbBJobCGJxMARgGtgHfymglAz((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return UehbBJobCGJxMARgGtgHfymglAz((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return UehbBJobCGJxMARgGtgHfymglAz(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && rJdlKPfgQKHtlDAtkIVQmPJndmx)
				{
					return false;
				}
				if (jPRLOUkrzfIOOeaLcCekqCAqvFy)
				{
					return false;
				}
			}
			...
			{
				if (FFROfbTjbqXjdMnwwXUfZLbIcTU && name.Equals(rhmzlZDoZIXNqddaRCOilXEIYVh, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (aanviCImfOzJVFDynbrOoxyGQEf && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static T UehbBJobCGJxMARgGtgHfymglAz<T>(T P_0) where T : class
		{
		...
		internal static ButtonStateFlags KXaPbukfFUJAJqHNnclnvNPyViv(KeyCode P_0)
		{
			ButtonStateFlags buttonStateFlags = (Input.GetKey(P_0) ? ButtonStateFlags.UplvurlztepPVkfDEPxOTCIsEIa : ButtonStateFlags.VgdbsUdbYtLLUhXvNBybiOERwsh);
		...
		internal static ButtonStateFlags JFLhhsViRZmASHFRAirmzVNMOhf(string P_0)
		{
			ButtonStateFlags buttonStateFlags = (Input.GetButton(P_0) ? ButtonStateFlags.UplvurlztepPVkfDEPxOTCIsEIa : ButtonStateFlags.VgdbsUdbYtLLUhXvNBybiOERwsh);
