...
namespace Rewired.Data
{
	...
	public sealed class CustomController_Editor
	{
		...
		private sealed class QkDZQjMQKPEGbjgDmwgnQgTZidL : IDisposable, IEnumerable<ControllerElementIdentifier>, IEnumerator<ControllerElementIdentifier>, IEnumerator, IEnumerable
		{
			...
			public CustomController_Editor GxphHAMqMhNBLjnlhXuBQmXaALiE;
			...
			{
				QkDZQjMQKPEGbjgDmwgnQgTZidL qkDZQjMQKPEGbjgDmwgnQgTZidL;
				if (Thread.CurrentThread.ManagedThreadId == dFCUHNznYmJZjnnffQJUVAprSDy && SRJUeDWyyYFsEaMQQCwxNbjBZLJ == -2)
				{
					...
					qkDZQjMQKPEGbjgDmwgnQgTZidL = this;
				}
				else
				{
					qkDZQjMQKPEGbjgDmwgnQgTZidL = new QkDZQjMQKPEGbjgDmwgnQgTZidL(0);
					qkDZQjMQKPEGbjgDmwgnQgTZidL.GxphHAMqMhNBLjnlhXuBQmXaALiE = GxphHAMqMhNBLjnlhXuBQmXaALiE;
				}
				return qkDZQjMQKPEGbjgDmwgnQgTZidL;
			}
			...
			private bool MoveNext()
			{
				...
				case 0:
					...
					if (GxphHAMqMhNBLjnlhXuBQmXaALiE._elementIdentifiers == null)
					{
						break;
					}
				...
				case 1:
					...
					IL_006e:
					if (ruJbLfoHPdAHKbMrcgEkYTYDkQVf < GxphHAMqMhNBLjnlhXuBQmXaALiE._elementIdentifiers.Count)
					{
						WCNlIsEdYuVTqbNYvICUPcTebLU = GxphHAMqMhNBLjnlhXuBQmXaALiE._elementIdentifiers[ruJbLfoHPdAHKbMrcgEkYTYDkQVf];
						SRJUeDWyyYFsEaMQQCwxNbjBZLJ = 1;
						return true;
					}
					break;
			...
			[DebuggerHidden]
			public QkDZQjMQKPEGbjgDmwgnQgTZidL(int _003C_003E1__state)
			{
		...
		public IEnumerable<ControllerElementIdentifier> ElementIdentifiers
		{
			get
			{
				QkDZQjMQKPEGbjgDmwgnQgTZidL qkDZQjMQKPEGbjgDmwgnQgTZidL = new QkDZQjMQKPEGbjgDmwgnQgTZidL(-2);
				qkDZQjMQKPEGbjgDmwgnQgTZidL.GxphHAMqMhNBLjnlhXuBQmXaALiE = this;
				return qkDZQjMQKPEGbjgDmwgnQgTZidL;
			}
		}
		...
		internal ControllerElementType LmHdLrqIfLArdUMOMFJZMjiFGhv(int P_0)
		{
		...
		internal bool TbDbJDemyidfwIcWoPhRPcXIkxb(int P_0, out AxisRange P_1)
		{
			ControllerElementIdentifier elementIdentifier = GetElementIdentifier(P_0);
			if (elementIdentifier == null)
			{
		...
		public void AddAxis()
		{
			axes.Add((Axis)fEVMobjKXMzBCBBCLhnxfZikjMr(ControllerElementType.Axis));
		}

		public void AddButton()
		{
			buttons.Add((Button)fEVMobjKXMzBCBBCLhnxfZikjMr(ControllerElementType.Button));
		}
		...
		public void InsertAxis(int index)
		{
			...
			axes.Insert(index, (Axis)fEVMobjKXMzBCBBCLhnxfZikjMr(ControllerElementType.Axis));
		}
		...
		public void InsertButton(int index)
		{
			...
			buttons.Insert(index, (Button)fEVMobjKXMzBCBBCLhnxfZikjMr(ControllerElementType.Button));
		}
		...
		public void DuplicateElement(ControllerElementType type, int index)
		{
			...
			{
			case ControllerElementType.Axis:
				emIScPSjjCDcQeOwmPcWRroJwtd(index, axes);
				break;
			case ControllerElementType.Button:
				emIScPSjjCDcQeOwmPcWRroJwtd(index, buttons);
				break;
			default:
				throw new NotImplementedException();
			}
		...
		private void emIScPSjjCDcQeOwmPcWRroJwtd<T>(int P_0, List<T> P_1) where T : Element
		{
			...
			ControllerElementIdentifier controllerElementIdentifier = zQidUOrFzrahikPmHtUjkUHKCSA(val.elementIdentifierId, text);
		...
		private ControllerElementIdentifier zQidUOrFzrahikPmHtUjkUHKCSA(int P_0, string P_1)
		{
		...
		private Element fEVMobjKXMzBCBBCLhnxfZikjMr(ControllerElementType P_0)
		{
			switch (P_0)
			{
			case ControllerElementType.Axis:
			{
				...
				ControllerElementIdentifier controllerElementIdentifier2 = qFZxqMFsMyRKDsMImCemkPrbmDzy(P_0, text2, string.Empty, string.Empty);
			...
			case ControllerElementType.Button:
			{
				...
				ControllerElementIdentifier controllerElementIdentifier = qFZxqMFsMyRKDsMImCemkPrbmDzy(P_0, text, string.Empty, string.Empty);
		...
		private ControllerElementIdentifier qFZxqMFsMyRKDsMImCemkPrbmDzy(ControllerElementType P_0, string P_1, string P_2, string P_3)
		{
			int elementIdentifierIdCounter = _elementIdentifierIdCounter;
			_elementIdentifierIdCounter++;
		...
		internal HardwareControllerMap_Game yuVTyXTeLkmAEriUUAaddbTLpoaJ()
		{
			int num = axisCount;
			int num2 = buttonCount;
