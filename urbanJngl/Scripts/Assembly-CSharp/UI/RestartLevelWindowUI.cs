using System;
using Infrastructure.Services;
using UnityEngine;
using UnityEngine.UI;

namespace UI
{
	public class RestartLevelWindowUI : MonoBehaviour
	{
		[SerializeField]
		private Button yesButton;

		[SerializeField]
		private Button noButton;

		private bool restart;

		private void Awake()
		{
			yesButton.onClick.AddListener(delegate
			{
				RestartLevel();
			});
			noButton.onClick.AddListener(delegate
			{
				Hide();
			});
		}

		private void Start()
		{
			MainMenuUI.Instance.OnRestartLevelButton += MainMenuUI_OnNewGameButton;
			InputManager.Instance.OnEscape += InputManager_OnEscape;
			Hide();
		}

		private void OnDestroy()
		{
			MainMenuUI.Instance.OnRestartLevelButton -= MainMenuUI_OnNewGameButton;
			InputManager.Instance.OnEscape -= InputManager_OnEscape;
		}

		private void RestartLevel()
		{
			if (!restart)
			{
				restart = true;
				AllServices.Container.Single<Loader>().StartLevelOver();
			}
		}

		private void InputManager_OnEscape(object sender, EventArgs e)
		{
			Hide();
		}

		private void MainMenuUI_OnNewGameButton(object sender, EventArgs e)
		{
			Show();
		}

		private void Hide()
		{
			if (base.isActiveAndEnabled)
			{
				MainMenuUI.Instance.ToggleMainMenu(value: true);
				MainMenuUI.Instance.InnerWindowOpen = false;
				base.gameObject.SetActive(value: false);
			}
		}

		private void Show()
		{
			MainMenuUI.Instance.InnerWindowOpen = true;
			base.gameObject.SetActive(value: true);
		}
	}
}
