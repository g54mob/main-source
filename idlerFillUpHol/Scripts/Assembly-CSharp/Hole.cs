...
public class Hole : MonoBehaviour
{
	...
	private void UpdateBackgroundY()
	{
		...
		BackgroundImage.transform.localPosition = new Vector3(BackgroundImage.transform.localPosition.x, num, BackgroundImage.transform.localPosition.z);
		BackgroundImage2.transform.localPosition = new Vector3(BackgroundImage2.transform.localPosition.x, num - 1f, BackgroundImage2.transform.localPosition.z);
	}
	...
	public void ProcessGarbage(GarbageInfo g, float x)
	{
		...
		else if (g.GarbageType == GarbageInfo.GarbageTypeEnum.Book)
		{
			...
			{
				GameController.Instance.ToastPanel.AddItem("Book found. More information in help.");
			}
	...
	public void SetAsStatic(HoleGarbage g)
	{
		if (KeepGarbage)
		{
			...
			if (_topElement < g.transform.position.y)
			{
				...
				if (_topElement > _topForGarbage - _deltaRangeForDestroy - 2f || _staticList.Count >= _maxElements)
				{
					RemoveBottomGarbage();
				}
			}
	...
	private void RemoveBottomGarbage()
	{
		...
		for (int num = _staticList.Count - 1; num >= 0; num--)
		{
			if (_staticList[num].transform.position.y < _bottomForGarbage + _deltaRangeForDestroy)
			{
				DestroyGarbage(_staticList[num]);
			}
			else
			{
				_staticList[num].transform.position += vector;
			}
		}
	}
}
