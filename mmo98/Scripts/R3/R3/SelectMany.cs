...
namespace R3
{
	internal sealed class SelectMany<TSource, TCollection, TResult> : Observable<TResult>
	{
		private sealed class _SelectMany : Observer<TSource>
		{
			private sealed class _SelectManyCollectionObserver : Observer<TCollection>
			{
				public _SelectManyCollectionObserver(TSource sourceValue, _SelectMany parent)
				{
				...
				{
					TResult value2 = _003Cparent_003EP.resultSelector(_003CsourceValue_003EP, value);
			...
			private readonly Func<TSource, Observable<TCollection>> collectionSelector;
			...
			public _SelectMany(Observer<TResult> observer, Func<TSource, Observable<TCollection>> collectionSelector, Func<TSource, TCollection, TResult> resultSelector)
			{
			...
			{
				Observable<TCollection> observable = collectionSelector(value);
				_SelectManyCollectionObserver item = new _SelectManyCollectionObserver(value, this);
		...
		public SelectMany(Observable<TSource> source, Func<TSource, Observable<TCollection>> collectionSelector, Func<TSource, TCollection, TResult> resultSelector)
		{
