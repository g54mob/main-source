...
namespace ObservableCollections
{
	...
	public readonly ref struct NotifyCollectionChangedEventArgs<T>
	{
		...
		public NotifyCollectionChangedEventArgs(NotifyCollectionChangedAction action, bool isSingleItem, T newItem = default(T), T oldItem = default(T), ReadOnlySpan<T> newItems = default(ReadOnlySpan<T>), ReadOnlySpan<T> oldItems = default(ReadOnlySpan<T>), int newStartingIndex = -1, int oldStartingIndex = -1, SortOperation<T> sortOperation = default(SortOperation<T>))
		{
			Action = action;
		...
		{
			SortOperation<T> sortOperation = SortOperation<T>.CreateReverse(index, count);
			return new NotifyCollectionChangedEventArgs<T>(NotifyCollectionChangedAction.Reset, isSingleItem: true, default(T), default(T), default(ReadOnlySpan<T>), default(ReadOnlySpan<T>), -1, -1, sortOperation);
		}

		public static NotifyCollectionChangedEventArgs<T> Sort(int index, int count, IComparer<T>? comparer)
		{
			SortOperation<T> sortOperation = new SortOperation<T>(index, count, comparer);
			return new NotifyCollectionChangedEventArgs<T>(NotifyCollectionChangedAction.Reset, isSingleItem: true, default(T), default(T), default(ReadOnlySpan<T>), default(ReadOnlySpan<T>), -1, -1, sortOperation);
		}
	}
}
