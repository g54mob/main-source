...
namespace Pathfinding.Drawing
{
	public class DrawingData
	{
		...
		internal struct ProcessedBuilderData
		{
			...
			public unsafe void SchedulePersistFilter(int version, int lastTickVersion, float time, int sceneModeVersion)
			{
				...
				else if (meta.version < lastTickVersion || submitted)
				{
					...
					buildJob.Complete();
					splitterJob = IJobExtensions.Schedule(new PersistentFilterJob
					{
		...
		internal struct BuilderData : IDisposable
		{
			...
			public unsafe delegate bool AnyBuffersWrittenTo_0000022E_0024PostfixBurstDelegate(UnsafeAppendBuffer* buffers, int numBuffers);

			internal static class AnyBuffersWrittenTo_0000022E_0024BurstDirectCall
			{
				...
				private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
				{
					if (Pointer == (IntPtr)0)
					{
						Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(AnyBuffersWrittenTo_0000022E_0024PostfixBurstDelegate).TypeHandle);
					}
					P_0 = Pointer;
				}
				...
				static AnyBuffersWrittenTo_0000022E_0024BurstDirectCall()
				{
					Constructor();
				}
				...
				public unsafe static bool Invoke(UnsafeAppendBuffer* buffers, int numBuffers)
				{
					...
					return AnyBuffersWrittenTo_0024BurstManaged(buffers, numBuffers);
				}
			}

			public unsafe delegate void ResetAllBuffers_0000022F_0024PostfixBurstDelegate(UnsafeAppendBuffer* buffers, int numBuffers);

			internal static class ResetAllBuffers_0000022F_0024BurstDirectCall
			{
				private static IntPtr Pointer;
				...
				private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
				{
					if (Pointer == (IntPtr)0)
					{
						Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(ResetAllBuffers_0000022F_0024PostfixBurstDelegate).TypeHandle);
					}
					P_0 = Pointer;
				}
				...
				static ResetAllBuffers_0000022F_0024BurstDirectCall()
				{
					Constructor();
				}
				...
				public unsafe static void Invoke(UnsafeAppendBuffer* buffers, int numBuffers)
				{
					...
					ResetAllBuffers_0024BurstManaged(buffers, numBuffers);
				}
			}
			...
			public void Init(Hasher hasher, RedrawScope frameRedrawScope, RedrawScope customRedrawScope, bool isGizmos, int drawOrderIndex, int sceneModeVersion)
			{
				...
				{
					commandBuffers = new NativeArray<UnsafeAppendBuffer>(128, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
			...
			{
				return AnyBuffersWrittenTo_0000022E_0024BurstDirectCall.Invoke(buffers, numBuffers);
			}
			...
			{
				ResetAllBuffers_0000022F_0024BurstDirectCall.Invoke(buffers, numBuffers);
			}
			...
			public unsafe void Submit(DrawingData gizmos)
			{
				...
				splitterJob = IJobExtensions.Schedule(new StreamSplitter
				{
		...
		{
			if (persistentRedrawScopes.TryGetValue(scope.id, out var value))
			{
				return value;
			}
			return null;
		...
		{
			foreach (KeyValuePair<int, GameObject> persistentRedrawScope in persistentRedrawScopes)
			{
				processedData.SetVersion(new RedrawScope(this, persistentRedrawScope.Key), version);
			}
		}
		...
		public static void Initialize_0024BuilderData_AnyBuffersWrittenTo_0000022E_0024BurstDirectCall()
		{
			BuilderData.AnyBuffersWrittenTo_0000022E_0024BurstDirectCall.Initialize();
		}

		public static void Initialize_0024BuilderData_ResetAllBuffers_0000022F_0024BurstDirectCall()
		{
			BuilderData.ResetAllBuffers_0000022F_0024BurstDirectCall.Initialize();
		}
	}
}
