...
namespace HeathenEngineering.SteamworksIntegration.UI
{
	public class QuickMatchStatusLabel : MonoBehaviour
	{
		...
		{
			return input.Replace("%player%", (controller.Slots - controller.MemberCount > 1) ? players : player).Replace("%timer%", ((int)controller.Timer).ToString()).Replace("%count%", controller.MemberCount.ToString())
