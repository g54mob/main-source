...
namespace ToonyColorsPro.Demo
{
	public class TCP2_Demo_Camera : MonoBehaviour
	{
		...
		private void OnEnable()
		{
			...
			Vector2 vector = Vector2.Scale(ignoreMouseRect.rect.size, ignoreMouseRect.lossyScale);
			Rect rect = new Rect(ignoreMouseRect.position.x, ignoreMouseRect.position.y, vector.x, vector.y);
			rect.x -= ignoreMouseRect.pivot.x * vector.x;
			rect.y -= (0f - ignoreMouseRect.pivot.y) * vector.y;
		...
		private void Update()
		{
			...
			bool flag = ignoreMouseRect != null && ignoreMouseActualRect.Contains(Input.mousePosition);
			...
			Vector3 vector = Pivot.position + pivotOffset;
			base.transform.RotateAround(vector, base.transform.right, orbitAcceleration.y * Time.deltaTime);
			base.transform.RotateAround(vector, Vector3.up, orbitAcceleration.x * Time.deltaTime);
			pivotOffset += base.transform.TransformDirection(panAcceleration) * Time.deltaTime;
			...
			zoomDistance = Vector3.Distance(base.transform.position, vector);
