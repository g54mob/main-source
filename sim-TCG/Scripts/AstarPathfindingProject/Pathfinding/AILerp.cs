	...
	public class AILerp : VersionedMonoBehaviour, IAstarAI
	{
		...
		public NativeMovementPlane movementPlane
		{
			get
			{
				...
				{
					NavGraph graph = path.path[0].Graph;
		...
		protected virtual bool shouldRecalculatePath
		{
			get
			{
				if (canSearchAgain)
				{
					return autoRepath.ShouldRecalculatePath(position, 0f, destination, Time.time);
				}
				return false;
			}
		}
		...
		{
			if (migrations.TryMigrateFromLegacyFormat(out var legacyVersion) && legacyVersion <= 3)
			{
		...
		{
			tr = base.transform;
			autoRepath.DrawGizmos(Draw.editor, position, 0f, new NativeMovementPlane((orientation == OrientationMode.YAxisForward) ? Quaternion.Euler(-90f, 0f, 0f) : Quaternion.identity));
		}
	}
}
