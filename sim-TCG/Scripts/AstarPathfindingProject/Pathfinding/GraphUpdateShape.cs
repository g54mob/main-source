	...
	public class GraphUpdateShape
	{
		public struct BurstShape
		{
			...
			public BurstShape(GraphUpdateShape scene, Allocator allocator)
			{
				...
				{
					points = new NativeArray<Vector3>(array, allocator);
				}
				origin = scene.origin;
				right = scene.right;
			...
			public bool Contains(float3 point)
			{
				...
				point -= origin;
				float3 float5 = new float3(math.dot(point, right), 0f, math.dot(point, forward));
				int index = points.Length - 1;
				bool flag = false;
				int num = 0;
				while (num < points.Length)
				{
					if (((points[num].z <= float5.z && float5.z < points[index].z) || (points[index].z <= float5.z && float5.z < points[num].z)) && float5.x < (points[index].x - points[num].x) * (float5.z - points[num].z) / (points[index].z - points[num].z) + points[num].x)
					{
						flag = !flag;
					}
					index = num++;
