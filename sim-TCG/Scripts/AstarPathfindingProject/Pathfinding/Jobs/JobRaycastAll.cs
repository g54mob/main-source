...
namespace Pathfinding.Jobs
{
	public struct JobRaycastAll
	{
		...
		private struct JobCreateCommands : IJobParallelFor
		{
			...
			public void Execute(int index)
			{
				...
				if (raycastHit.normal != default(Vector3))
				{
					...
					commands[index] = new RaycastCommand(vector, raycastCommand.direction, distance, raycastCommand.layerMask);
				}
				else
				{
					commands[index] = new RaycastCommand(Vector3.zero, Vector3.up, 1f, 0);
				}
			}
		}
		...
		public JobHandle Schedule(JobHandle dependency)
		{
			...
			return Unity.Jobs.IJobExtensions.Schedule(new JobCombineResults
			{
