...
namespace Pathfinding.Graphs.Grid
{
	public struct GridGraphNodeData
	{
		...
		public unsafe JobHandle Rotate2D(int dx, int dz, JobHandle dependency)
		{
			...
			{
				Unity.Jobs.IJobExtensions.Schedule(positions.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(normals.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(connections.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(penalties.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(walkable.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(walkableWithErosion.Rotate3D(size, dx, dz), dependency),
				Unity.Jobs.IJobExtensions.Schedule(tags.Rotate3D(size, dx, dz), dependency)
			}, 7);
		}
		...
		public void CopyFrom(GridGraphNodeData other, IntBounds bounds, bool copyPenaltyAndTags, JobDependencyTracker dependencyTracker)
		{
			...
			{
				Unity.Jobs.IJobExtensions.Run(jobCopyBuffers);
			}
		}
