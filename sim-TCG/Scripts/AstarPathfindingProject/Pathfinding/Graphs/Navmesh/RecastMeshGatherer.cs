...
namespace Pathfinding.Graphs.Navmesh
{
	...
	public class RecastMeshGatherer
	{
		...
		public delegate void CalculateBounds_00000AB4_0024PostfixBurstDelegate(ref UnsafeSpan<float3> vertices, ref float4x4 localToWorldMatrix, out Bounds bounds);

		internal static class CalculateBounds_00000AB4_0024BurstDirectCall
		{
			...
			private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(CalculateBounds_00000AB4_0024PostfixBurstDelegate).TypeHandle);
				}
				P_0 = Pointer;
			}
			...
			static CalculateBounds_00000AB4_0024BurstDirectCall()
			{
				Constructor();
			}
		...
		{
			CalculateBounds_00000AB4_0024BurstDirectCall.Invoke(ref vertices, ref localToWorldMatrix, out bounds);
		}
		...
		public void CollectTerrainMeshes(bool rasterizeTrees, float desiredChunkSize)
		{
			...
			for (int i = 0; i < activeTerrains.Length; i++)
			{
				...
				{
					GenerateTerrainChunks(activeTerrains[i], bounds, desiredChunkSize);
					if (rasterizeTrees)
					{
		...
		private void GenerateTerrainChunks(Terrain terrain, Bounds bounds, float desiredChunkSize)
		{
			...
			IntRect intRect = new IntRect(0, 0, heightmapResolution / a, heightmapResolution2 / a2);
			IntRect a3 = (float.IsFinite(bounds.size.x) ? new IntRect(Mathf.FloorToInt((bounds.min.x - position.x) / num), Mathf.FloorToInt((bounds.min.z - position.z) / num2), Mathf.FloorToInt((bounds.max.x - position.x) / num), Mathf.FloorToInt((bounds.max.z - position.z) / num2)) : intRect);
			a3 = IntRect.Intersection(a3, intRect);
			if (!a3.IsValid())
			{
				return;
			}
			IntRect intRect2 = new IntRect(a3.xmin * a, a3.ymin * a2, Mathf.Min(heightmapResolution, (a3.xmax + 1) * a) - 1, Mathf.Min(heightmapResolution2, (a3.ymax + 1) * a2) - 1);
			float[,] heights = terrainData.GetHeights(intRect2.xmin, intRect2.ymin, intRect2.Width, intRect2.Height);
			bool[,] holes = terrainData.GetHoles(intRect2.xmin, intRect2.ymin, intRect2.Width - 1, intRect2.Height - 1);
			Vector3 offset = position + new Vector3((float)(a3.xmin * a) * heightmapScale.x, 0f, (float)(a3.ymin * a2) * heightmapScale.z);
			for (int i = a3.ymin; i <= a3.ymax; i++)
			{
				for (int j = a3.xmin; j <= a3.xmax; j++)
				{
					GatheredMesh item = ConvertHeightmapChunkToGatheredMesh(heights, holes, heightmapScale, offset, (j - a3.xmin) * a, (i - a3.ymin) * a2, a, a2, terrainDownsamplingFactor);
		...
		public GatheredMesh ConvertHeightmapChunkToGatheredMesh(float[,] heights, bool[,] holes, Vector3 sampleSize, Vector3 offset, int x0, int z0, int width, int depth, int stride)
		{
			...
			int num = CeilDivision(Mathf.Min(width, length2 - x0), stride) + 1;
			int num2 = CeilDivision(Mathf.Min(depth, length - z0), stride) + 1;
			int length3 = num * num2;
			NativeArray<Vector3> nativeArray = new NativeArray<Vector3>(length3, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
			int length4 = (num - 1) * (num2 - 1) * 2 * 3;
			NativeArray<int> nativeArray2 = new NativeArray<int>(length4, Allocator.Persistent, NativeArrayOptions.UninitializedMemory);
			UnsafeSpan<Vector3> unsafeSpan = nativeArray.AsUnsafeSpan();
			for (int i = 0; i < num2; i++)
			{
				int num3 = Math.Min(z0 + i * stride, length - 1);
				for (int j = 0; j < num; j++)
				{
					int num4 = Math.Min(x0 + j * stride, length2 - 1);
					unsafeSpan[i * num + j] = new Vector3((float)num4 * sampleSize.x, heights[num3, num4] * sampleSize.y, (float)num3 * sampleSize.z) + offset;
				}
			}
			int num5 = 0;
			UnsafeSpan<int> unsafeSpan2 = nativeArray2.AsUnsafeSpan();
			for (int k = 0; k < num2 - 1; k++)
			{
				for (int l = 0; l < num - 1; l++)
				{
					int num6 = Math.Min(x0 + stride / 2 + l * stride, length2 - 2);
					int num7 = Math.Min(z0 + stride / 2 + k * stride, length - 2);
					if (holes[num7, num6])
					{
						unsafeSpan2[num5] = k * num + l;
						unsafeSpan2[num5 + 1] = (k + 1) * num + l + 1;
						unsafeSpan2[num5 + 2] = k * num + l + 1;
						num5 += 3;
						unsafeSpan2[num5] = k * num + l;
						unsafeSpan2[num5 + 1] = (k + 1) * num + l;
						unsafeSpan2[num5 + 2] = (k + 1) * num + l + 1;
						num5 += 3;
					}
				}
			}
			int meshDataIndex = AddMeshBuffers(nativeArray, nativeArray2);
			return new GatheredMesh
			{
		...
		private void CollectTreeMeshes(Terrain terrain)
		{
			...
			for (int i = 0; i < treeInstances.Length; i++)
			{
				...
				Vector3 s = Vector3.Scale(new Vector3(treeInstance.widthScale, treeInstance.heightScale, treeInstance.widthScale), treePrototype.prefab.transform.localScale);
				float num = (value.supportsRotation ? treeInstance.rotation : 0f);
				Matrix4x4 matrix4x = Matrix4x4.TRS(pos, Quaternion.AngleAxis(num * 57.29578f, Vector3.up), s);
				for (int k = 0; k < value.submeshes.Count; k++)
				{
					GatheredMesh item = value.submeshes[k];
