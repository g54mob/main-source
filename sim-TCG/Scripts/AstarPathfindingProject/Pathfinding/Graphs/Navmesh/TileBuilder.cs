...
namespace Pathfinding.Graphs.Navmesh
{
	public struct TileBuilder
	{
		...
		public RecastMeshGatherer.MeshCollection CollectMeshes(Bounds bounds)
		{
			...
			{
				float desiredChunkSize = tileLayout.cellSize * (float)math.max(tileLayout.tileSizeInVoxels.x, tileLayout.tileSizeInVoxels.y);
		...
		public Promise<TileBuilderOutput> Schedule(DisposeArena arena)
		{
			...
			for (int k = 0; k < array.Length; k++)
			{
				...
				{
					jobHandle2 = Unity.Jobs.IJobExtensions.Schedule(jobData, jobHandle2);
				}
