...
namespace Pathfinding.Graphs.Navmesh
{
	...
	public static class ColliderMeshBuilder2D
	{
		...
		public delegate int GenerateMeshesFromShapes_00000AA8_0024PostfixBurstDelegate(ref UnsafeSpan<PhysicsShape2D> shapes, ref UnsafeSpan<float2> vertices, ref UnsafeSpan<Matrix4x4> shapeMatrices, ref UnsafeSpan<int> groupIndices, ref UnsafeList<float3> outputVertices, ref UnsafeList<int3> outputIndices, ref UnsafeSpan<ShapeMesh> outputShapeMeshes, float maxError);

		internal static class GenerateMeshesFromShapes_00000AA8_0024BurstDirectCall
		{
			...
			private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(GenerateMeshesFromShapes_00000AA8_0024PostfixBurstDelegate).TypeHandle);
				}
				P_0 = Pointer;
			}
			...
			static GenerateMeshesFromShapes_00000AA8_0024BurstDirectCall()
			{
				Constructor();
			}
		...
		public unsafe static int GenerateMeshesFromColliders(Collider2D[] colliders, int numColliders, float maxError, out NativeArray<float3> outputVertices, out NativeArray<int> outputIndices, out NativeArray<ShapeMesh> outputShapeMeshes)
		{
			...
			for (int i = 0; i < numColliders; i++)
			{
				...
				{
					if (collider2D is TilemapCollider2D tilemapCollider2D)
					{
						tilemapCollider2D.ProcessTilemapChanges();
					}
					shapes = collider2D.GetShapes(physicsShapeGroup2D);
				}
				else
				{
					...
					shapes = attachedRigidbody.GetShapes(physicsShapeGroup2D);
				}
				list.Length += shapes;
		...
		{
			return GenerateMeshesFromShapes_00000AA8_0024BurstDirectCall.Invoke(ref shapes, ref vertices, ref shapeMatrices, ref groupIndices, ref outputVertices, ref outputIndices, ref outputShapeMeshes, maxError);
		}
