...
public abstract class AkObstructionOcclusion : MonoBehaviour
{
	...
	protected readonly List<AkAudioListener> currentListenerList = new List<AkAudioListener>();
	...
	{
		for (int i = 0; i < currentListenerList.Count; i++)
		{
		...
		{
			if (!currentListenerList.Contains(obstructionOcclusionValue.Key))
			{
	...
	private void CastRays()
	{
		if (refreshTime > refreshInterval)
		{
			...
			foreach (KeyValuePair<AkAudioListener, ObstructionOcclusionValue> obstructionOcclusionValue in ObstructionOcclusionValues)
			{
				...
				{
					value.targetValue = (Physics.Raycast(base.transform.position, vector / magnitude, magnitude, LayerMask.value) ? 1f : 0f);
				}
			}
		}
