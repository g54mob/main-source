...
public class AkGameObjEnvironmentData
{
	...
	private void AddHighestPriorityEnvironmentsFromPortals(Vector3 position)
	{
		...
		{
			AkEnvironmentPortal akEnvironmentPortal = activePortals[i];
			if (akEnvironmentPortal.EnvironmentsShareAuxBus)
			{
				AkEnvironment akEnvironment = akEnvironmentPortal.environments[0];
				...
				if (!auxSendValues.Contains(id))
				{
					auxSendValues.Add(id, 1f);
			...
			for (int j = 0; j < 2; j++)
			{
				AkEnvironment akEnvironment2 = akEnvironmentPortal.environments[j];
				if (akEnvironment2 == null)
				{
					continue;
				}
				int num2 = activeEnvironmentsFromPortals.BinarySearch(akEnvironment2, AkEnvironment.s_compareByPriority);
				if (num2 < 0 || num2 >= 4)
				{
					continue;
				}
				uint id2 = akEnvironment2.data.Id;
				if (!auxSendValues.Contains(id2))
				{
					auxSendValues.Add(id2, akEnvironmentPortal.GetAuxSendValueForPosition(position, j));
					if (auxSendValues.isFull)
					{
						return;
					}
	...
	private void AddHighestPriorityEnvironments(Vector3 position)
	{
		...
		for (int i = 0; i < activeEnvironments.Count; i++)
		{
			...
			uint id = akEnvironment.data.Id;
			if ((!akEnvironment.isDefault || i == 0) && !auxSendValues.Contains(id))
			{
				auxSendValues.Add(id, 1f);
	...
	public void RemoveAkEnvironment(Collider environmentCollider, Collider gameObjectCollider)
	{
		...
		if (component != null)
		{
			for (int i = 0; i < 2; i++)
			{
				...
				if (akEnvironment != null && !gameObjectCollider.bounds.Intersects(akEnvironment.Collider.bounds))
				{
