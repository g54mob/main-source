...
namespace Rewired.Libraries.SharpDX.DirectInput
{
	...
	internal struct DeviceObjectId
	{
		...
		public lXAXFkMOntMikfBMvxfdIESZBAu Flags => (lXAXFkMOntMikfBMvxfdIESZBAu)(_rawType & -16776961);

		public int InstanceNumber => (_rawType >> 8) & 0xFFFF;

		public DeviceObjectId(lXAXFkMOntMikfBMvxfdIESZBAu typeFlags, int instanceNumber)
		{
			this = default(DeviceObjectId);
			_rawType = (int)(typeFlags & ~lXAXFkMOntMikfBMvxfdIESZBAu.rnFZFXkBfvCClktbehtPZMcvdZsy) | ((!(instanceNumber < 0 || instanceNumber > 65534)) ? ((instanceNumber & 0xFFFF) << 8) : 0);
		}
