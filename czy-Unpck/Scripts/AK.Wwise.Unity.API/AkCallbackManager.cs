...
public static class AkCallbackManager
{
	...
	private static readonly AkEventCallbackInfo AkEventCallbackInfo = new AkEventCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkDynamicSequenceItemCallbackInfo AkDynamicSequenceItemCallbackInfo = new AkDynamicSequenceItemCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkMIDIEventCallbackInfo AkMIDIEventCallbackInfo = new AkMIDIEventCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkMarkerCallbackInfo AkMarkerCallbackInfo = new AkMarkerCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkDurationCallbackInfo AkDurationCallbackInfo = new AkDurationCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkMusicSyncCallbackInfo AkMusicSyncCallbackInfo = new AkMusicSyncCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkMusicPlaylistCallbackInfo AkMusicPlaylistCallbackInfo = new AkMusicPlaylistCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkAudioSourceChangeCallbackInfo AkAudioSourceChangeCallbackInfo = new AkAudioSourceChangeCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkMonitoringCallbackInfo AkMonitoringCallbackInfo = new AkMonitoringCallbackInfo(IntPtr.Zero, cMemoryOwn: false);

	private static readonly AkBankCallbackInfo AkBankCallbackInfo = new AkBankCallbackInfo(IntPtr.Zero, cMemoryOwn: false);
	...
	{
		ms_sourceChangeCallbackPkg = new BGMCallbackPackage
		{
			m_Callback = in_CB,
			m_Cookie = in_cookie
		};
			...
			while (intPtr != IntPtr.Zero)
			{
				...
				case AkCallbackType.AK_Bank:
				{
					...
					if (value2 != null && value2.m_Callback != null)
					{
						...
						value2.m_Callback(AkBankCallbackInfo.bankID, AkBankCallbackInfo.inMemoryBankPtr, AkBankCallbackInfo.loadResult, value2.m_Cookie);
					}
					break;
				}
				default:
				{
					...
					case AkCallbackType.AK_MusicPlaylistSelect:
						...
						break;
					default:
						Debug.LogError(string.Concat("WwiseUnity: Undefined callback type <", akCallbackType, "> received. Callback object possibly corrupted."));
						break;
					}
