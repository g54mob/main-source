...
public class AkTimelineEventPlayableBehavior : PlayableBehaviour
{
	...
	private bool IsScrubbing(Playable playable, FrameData info)
	{
		...
		if (info.deltaTime != 0f || (!(previousTime > 0.0) && !(time > 0.0)))
		{
			return num > (double)info.deltaTime;
		}
		return true;
	}
	...
	private void PrintInfo(string FunctionName, Playable playable, FrameData info)
	{
		if (PrintDebugInformation)
		{
			...
			double time = playable.GetTime();
			double num = Math.Abs(time - previousTime);
			Debug.Log($"{FunctionName}: prevTime={previousTime}; curTime={time}; computedDelta={num}; evalType={info.evaluationType}; deltaTime={info.deltaTime}; playState={info.effectivePlayState}; timeHeld={info.timeHeld}; speed={info.effectiveSpeed}; parentSpeed={info.effectiveParentSpeed}");
		}
	}
