...
{
	public abstract class CollectionChangeEventListener<T> : ChangeEventListener<List<T>>, ICollectionChangeEventListener<T>, IChangeEventListener<List<T>>, IGameEventListener<List<T>>, IGameEventListener
	{
		...
		public override void OnEventRaised(EventData<List<T>> data)
		{
			...
			CollectionChangeEventData<T> arg = new CollectionChangeEventData<T>(data.sender, data.value);
			m_collectionresponce.Invoke(arg);
		}

		public override void OnEventRaised(EventData data)
		{
			base.OnEventRaised(data);
			CollectionChangeEventData<T> arg = new CollectionChangeEventData<T>(data.sender, null);
			m_collectionresponce.Invoke(arg);
		}
		...
		{
			base.OnEventRaised(data);
			CollectionChangeEventData<T> arg = new CollectionChangeEventData<T>(data.sender, data.newValue);
			m_collectionresponce.Invoke(arg);
		}
		...
		public virtual void OnEventRaised(CollectionChangeEventData<T> data)
		{
			...
			m_changeresponce.Invoke(new ChangeEventData<List<T>>(data.sender, null, data.newState));
