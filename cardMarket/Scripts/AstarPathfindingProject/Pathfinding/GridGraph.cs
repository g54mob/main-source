...
namespace Pathfinding
{
	...
	public class GridGraph : NavGraph, IUpdatableGraph, ITransformedGraph, IRaycastableGraph
	{
		...
		private class GridGraphMovePromise : IGraphUpdatePromise
		{
			...
			public GridGraphMovePromise(GridGraph graph, int dx, int dz)
			{
				...
				for (int i = 0; i < rects.Length; i++)
				{
					...
					promises[i] = new GridGraphUpdatePromise(graph, transform, nodes, startingSize, rects[i], dependencyTracker, default(JobHandle), Allocator.Persistent, RecalculationMode.RecalculateMinimal, null, ownsJobDependencyTracker: true);
				}
			}
		...
		protected override IGraphUpdatePromise ScanInternal(bool async)
		{
			...
			{
				nodes = array
			}, new int3(width, 1, depth), new IntRect(0, 0, width - 1, depth - 1), dependencyTracker, dependency, Allocator.Persistent, RecalculationMode.RecalculateFromScratch, null, ownsJobDependencyTracker: true);
		}
