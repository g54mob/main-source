...
namespace Pathfinding.Graphs.Navmesh.Voxelization.Burst
{
	...
	public struct JobBuildMesh : IJob
	{
		...
		public void Execute()
		{
			...
			NativeParallelHashMap<Int3, int> vertexMap = new NativeParallelHashMap<Int3, int>(num2, Allocator.Temp);
			int num5 = 0;
			int num6 = 0;
		...
		private void RemoveVertex(ref VoxelMesh mesh, int vertexToRemove)
		{
			...
			NativeList<int> list = new NativeList<int>(nativeList.Length / 2 + 1, Allocator.Temp);
			list.Add(nativeList[nativeList.Length - 2]);
			list.Add(nativeList[nativeList.Length - 1]);
			nativeList.Length -= 2;
			while (nativeList.Length > 0)
			{
				for (int num2 = nativeList.Length - 2; num2 >= 0; num2 -= 2)
				{
					...
					if (list[list.Length - 1] == num3)
					{
						list.AddNoResize(num4);
						flag = true;
					}
					if (flag)
					{
			...
			{
				if (list[l] > vertexToRemove)
				{
					list[l]--;
				}
			}
			int num5 = (list.Length - 2) * 3;
			int length = mesh.tris.Length;
			mesh.tris.Length += num5;
			int num6 = Triangulate(list.Length, mesh.verts.AsArray().Reinterpret<int>(12), list.AsArray(), mesh.tris.AsArray().GetSubArray(length, num5));
			if (num6 < 0)
			{
				num6 = -num6;
			}
			mesh.tris.ResizeUninitialized(length + num6 * 3);
			mesh.areas.AddReplicate(value, num6);
		}
