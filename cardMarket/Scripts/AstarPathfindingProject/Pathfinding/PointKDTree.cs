...
namespace Pathfinding
{
	public class PointKDTree
	{
		...
		{
			GraphNode best = null;
			long bestSqrDist = ((distanceSqr < float.PositiveInfinity) ? ((long)(1000000f * distanceSqr)) : long.MaxValue);
			GetNearestInternal(1, point, constraint, ref best, ref bestSqrDist);
			distanceSqr = ((best != null) ? (1.0000001E-06f * (float)bestSqrDist) : float.PositiveInfinity);
			return best;
		}
		...
		public GraphNode GetNearestConnection(Int3 point, NNConstraint constraint, long maximumSqrConnectionLength)
		{
			...
			long distanceThresholdOffset = (maximumSqrConnectionLength + 3) / 4;
			GetNearestConnectionInternal(1, point, constraint, ref best, ref bestSqrDist, distanceThresholdOffset);
			return best;
		}
		...
		private void GetNearestConnectionInternal(int index, Int3 point, NNConstraint constraint, ref GraphNode best, ref long bestSqrDist, long distanceThresholdOffset)
		{
			...
			if (data != null)
			{
				...
				{
					long sqrMagnitudeLong = (data[num].position - point).sqrMagnitudeLong;
					if (sqrMagnitudeLong - distanceThresholdOffset < bestSqrDist && (constraint == null || constraint.Suitable(data[num])))
					{
						Connection[] connections = (data[num] as PointNode).connections;
						if (connections != null)
						{
							Vector3 vector = (Vector3)data[num].position;
							for (int i = 0; i < connections.Length; i++)
							{
								Vector3 b = ((Vector3)connections[i].node.position + vector) * 0.5f;
								long num2 = (long)(VectorMath.SqrDistancePointSegment(vector, b, p) * 1000f * 1000f);
								if (num2 < bestSqrDist)
								{
									bestSqrDist = num2;
									best = data[num];
			...
			{
				long num3 = point[tree[index].splitAxis] - tree[index].split;
				int num4 = 2 * index + ((num3 >= 0) ? 1 : 0);
				GetNearestConnectionInternal(num4, point, constraint, ref best, ref bestSqrDist, distanceThresholdOffset);
				if (num3 * num3 - distanceThresholdOffset < bestSqrDist)
				{
					GetNearestConnectionInternal(num4 ^ 1, point, constraint, ref best, ref bestSqrDist, distanceThresholdOffset);
				}
			}
		}
		...
		private void GetInRangeInternal(int index, Int3 point, long sqrRadius, List<GraphNode> buffer)
		{
			...
			if (data != null)
			{
				for (int num = tree[index].count - 1; num >= 0; num--)
				{
					if ((data[num].position - point).sqrMagnitudeLong < sqrRadius)
					{
						buffer.Add(data[num]);
					}
				}
