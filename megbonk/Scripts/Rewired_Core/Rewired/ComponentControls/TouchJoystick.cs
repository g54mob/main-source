...
namespace Rewired.ComponentControls
{
	...
	public sealed class TouchJoystick : TouchInteractable
	{
		...
		private enum fGEsAHSFJlSnyjOCTuBwMnTPhdPS
		{
			None = 0,
			TowardTouch = 1,
			TowardHome = 2
		...
		private enum gHeicMOeEPTpTPljsYlKQfBjIzwm
		{
			Local = 0,
			TouchRegion = 1
		}
		...
		[Serializable]
		private sealed class BGTzhqheupLWCGOxaRjSKLaiorVy
		{
			public static readonly BGTzhqheupLWCGOxaRjSKLaiorVy _003C_003E9;

			public static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IValueChangedHandler, Vector2> _003C_003E9__277_0;

			public static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IStickPositionChangedHandler, Vector2> _003C_003E9__280_0;

			internal void RToBtrkpXvMgWVsJOGJcXKqRkvhqA(IValueChangedHandler P_0, Vector2 P_1)
			{
			}

			internal void rTldoGCskzsSKvbYzKMcZWGctweV(IStickPositionChangedHandler P_0, Vector2 P_1)
			{
			}
		}

		private sealed class jAxstCEihqaxkabqESzMSHXsLVboA : IEnumerator<object>, IEnumerator, IDisposable
		{
			private int WtiTtkowKcBqHELwdhtdJCwSXBjEA;

			private object YYyQyGRbftwJjQTbHbkbyRWMjEfM;

			public float tWnThKcAFJbdKPhnTjBhCASlObCt;

			public TouchJoystick LzKUoCYPMHzqtfMwrFxGQQroSJMv;

			public PositionType WSCnAumBAaVeKwZObkdEvGQaLXgr;

			public Vector2 gpcAwjHQBzaVfAtEukhxMFZuwfGlA;

			public fGEsAHSFJlSnyjOCTuBwMnTPhdPS oHEESjHDIhzANEZURzZFtBYWNACqA;

			private RectTransform UyZkcJtJFbGlCkwLMoPfWNTfYTmMA;

			private Vector2 DBegqfusWtoyamAsosRqADZMQWZD;

			private float rmWkEvEQdaysLFpvmXRLidHUXkFC;

			private float znDfTTCiPtUfqtdCLFhvzBDQrRmU;
			...
			[DebuggerHidden]
			public jAxstCEihqaxkabqESzMSHXsLVboA(int P_0)
			{
			}
		...
		private VRubiuUbWcigTnrJaYOjDhZZJCsd.HierarchyEventHelper<IValueChangedHandler, Vector2> _hierarchyValueChangedHandlers;

		private VRubiuUbWcigTnrJaYOjDhZZJCsd.HierarchyEventHelper<IStickPositionChangedHandler, Vector2> _hierarchyStickPositionChangedHandlers;
		...
		private fGEsAHSFJlSnyjOCTuBwMnTPhdPS _moveDirection;
		...
		[NonSerialized]
		private bool HdiWgFbEXSOVdOZwETnuIfOmtZyu;

		[NonSerialized]
		private bool xbQEYQHHwlgFxIxlNqDtFmWxtRuh;

		private bool _pointerDownIsFake;

		private Vector2 _lastPressStartingValue;

		private gHeicMOeEPTpTPljsYlKQfBjIzwm _lastClaimSource;
		...
		private ArkyMExcqvAjSSEgvKopSLEhHuEEA _imageRaycastHelper;
		...
		private Action<fGEsAHSFJlSnyjOCTuBwMnTPhdPS> __moveStartedDelegate;

		private Action<fGEsAHSFJlSnyjOCTuBwMnTPhdPS> __moveEndedDelegate;
		...
		private static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IValueChangedHandler, Vector2> __valueChangedHandlerDelegate;

		private static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IStickPositionChangedHandler, Vector2> __stickPositionChangedHandlerDelegate;
		...
		private StickBounds gysENviCwaWUJerwrsVFaiEfZEFfA
		{
			get
			{
		...
		private bool vlOJISRkwPtOnPfLnGPuBMEdHbWw => false;

		internal StandaloneAxis2D WXBwcwdevEeGrkSpYtTmlNMJioMDb => null;

		private Action<fGEsAHSFJlSnyjOCTuBwMnTPhdPS> yAFHrlZbNRNrjpiJiakabjAaiNad => null;

		private Action<fGEsAHSFJlSnyjOCTuBwMnTPhdPS> xNeiemUwwvQbrpULwSzGMPuVveom => null;

		private int vnSbsZDTQALzKBTxIxUGXBMfsUki => 0;

		private RectTransform qbViowpEvrNDQLSBeXVuPfAxsfDl => null;

		private float UcUzqOPbiknOiaGddbUZFNFCZgQd => 0f;

		internal static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IValueChangedHandler, Vector2> jjNYIojDDHEsgjnqonKkKnVvfBKDA => null;

		internal static VRubiuUbWcigTnrJaYOjDhZZJCsd.EventFunction<IStickPositionChangedHandler, Vector2> wNqWTuOTgDkvxySbmddvTNfUNDdI => null;
		...
		private void IqHhTJfHTkmIxPKwQsXfiHDGUdvv(Vector2 P_0)
		{
		}
		...
		internal override bool muEjuLNOgxFpTfrtZBaeqossxvzd(GameObject P_0)
		{
			return false;
		}
		...
		private void fjTuazmhrYFVmbsGVfKmyfMHXQPj(TouchRegion P_0)
		{
		}

		private void IuHrWbEPZKDNsrhGfpDHuXfSqNmT(TouchRegion P_0)
		{
		}
		...
		private void prARgWLLQmrmIEBDQtuYwqpxHAUt(Vector2 P_0, bool P_1, float P_2, fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_3)
		{
		}

		private void IYZDWKFYIUsXzdnxLewiqXUiSKVm(Vector2 P_0, PositionType P_1, bool P_2, float P_3, fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_4)
		{
		}

		[IteratorStateMachine(typeof(jAxstCEihqaxkabqESzMSHXsLVboA))]
		private IEnumerator QnQfAeaqQASZjhNfNLBUmKUcEqnA(Vector2 P_0, PositionType P_1, float P_2, fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_3)
		{
			return null;
		}

		private void MDcGppoSlSjOWiAtnflGvIwlmUpV(fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_0, Vector2 P_1, PositionType P_2)
		{
		}

		private void ggQQnyvoxgjNgGrqTwuqvbJRpVHe(fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_0)
		{
		}

		private void czGSxWdDqeBEGTWMOgxdnrKLeeMU(fGEsAHSFJlSnyjOCTuBwMnTPhdPS P_0)
		{
		}
		...
		private void gpKaMFGGZUKhbFoFvwphihzAsIKKB(int P_0, Vector2 P_1, PositionType P_2)
		{
		}
		...
		private void uSvvrnQLlhdyjhbAmpsjpLAfcIhG(ref Vector2 P_0)
		{
		}
		...
		private bool hoMaJNBftmPbIiCRqgVIcWbSDrEDA(int P_0)
		{
			return false;
		}

		private PointerEventData JkJSubDHPViBBJbAcSwjGTefvlJK(int P_0, GameObject P_1)
		{
			return null;
		}

		private PointerEventData ElcDfYlLeAxgXJXswtADXYZPynIJ(int P_0, GameObject P_1)
		{
			return null;
		}

		private PointerEventData JOOevZHKsijLrFDhitfhTClVXPUMA(int P_0)
		{
			return null;
		}

		private void vrqFpBbfboeWWcPKapaEkWfzwBNAc(PointerEventData P_0)
		{
		}

		private void nxoXBAMzgBvvdXkHlidlhuKXxFDxA(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private PointerEventData RUlGpgAWHAbJnqLOBziBzkPTMpLj(int P_0)
		{
			return null;
		}
		...
		private void rNBCGsotbUnuXuUnuogLOWEDOUG(AxisDirection P_0)
		{
		}

		private void obNRRLKRXptKuPHojhhaDPIVnSiVA(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void XbqoumvQtRpUqZBHaLDojCilQYDF(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void yRhHcnBbQnbmwaOohwEaafXhmjuTA(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void LCwdRFeusmtuoADzVcTvmFeTDmFsA(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void YRGsCLLRZhODAPNxrIxbZuyUkqEq(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void aXHmmzGlPTaGmvUuFsFFTOkVygel(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void oopejoGBNqfazJWfJuaTAMflJUCFb(PointerEventData P_0, gHeicMOeEPTpTPljsYlKQfBjIzwm P_1)
		{
		}

		private void KRtrYehMOVJKzpoBBtysVMAULKlH(int P_0, Vector2 P_1, gHeicMOeEPTpTPljsYlKQfBjIzwm P_2)
		{
		}
		...
		private void xIoitajGPxZWWcEbNuNFdyqXyuzH(PointerEventData P_0)
		{
		}

		private void XHWNQalHQwsfDRbjDyzgoDVkiHPG(PointerEventData P_0)
		{
		}

		private void ThfMPjCnfuNOAjWqbdSCdbPYKCGG(PointerEventData P_0)
		{
		}

		private void YxEKUcMyRdfFOJJKLPIHNkNLilyW(PointerEventData P_0)
		{
		}

		private void LkVNuGIxIBvTxchQHahVUBMUGYSeA(PointerEventData P_0)
		{
		}

		private void UjTwuVarFvmYQXIFwqGYaBNZoSkj(PointerEventData P_0)
		{
		}

		private void EnzOoapzRsqjzRzPIhGfpnkvXsTQ(PointerEventData P_0)
		{
		}

		private void yMmOhlFbOcjetZCklxdRdaGGZgCM(Vector2 P_0)
		{
		}
	}
}
