	...
	public class MMPathMovement : MonoBehaviour
	{
		...
		protected virtual void Initialization()
		{
			...
			if (PathElements != null && PathElements.Count >= 1)
			{
				...
				base.transform.position = PointPosition(_currentPoint.Current);
			}
		}
		...
		protected virtual void Move()
		{
			...
			MoveAlongThePath();
			_distanceToNextPoint = (base.transform.position - PointPosition(_currentPoint.Current)).magnitude;
		...
		public virtual void MoveAlongThePath()
		{
			...
			case PossibleAccelerationType.ConstantSpeed:
				base.transform.position = Vector3.MoveTowards(base.transform.position, PointPosition(_currentPoint.Current), Time.deltaTime * MovementSpeed);
				break;
			case PossibleAccelerationType.EaseOut:
				base.transform.position = Vector3.Lerp(base.transform.position, PointPosition(_currentPoint.Current), Time.deltaTime * MovementSpeed);
				break;
			case PossibleAccelerationType.AnimationCurve:
			{
				...
				if (!(num <= 0f))
				{
					...
					base.transform.position = Vector3.MoveTowards(base.transform.position, PointPosition(_currentPoint.Current), Time.deltaTime * MovementSpeed * num2);
				}
				break;
			}
			}
		...
		public virtual Vector3 PointPosition(Vector3 relativePointPosition)
		{
			...
			{
				AlignmentModes.None => _originalTransformPosition + relativePointPosition, 
				AlignmentModes.ThisRotation => _originalTransformPosition + base.transform.rotation * relativePointPosition, 
				AlignmentModes.ParentRotation => _originalTransformPosition + base.transform.parent.rotation * relativePointPosition, 
				_ => Vector3.zero, 
			};
		}
