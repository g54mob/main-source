...
namespace MoreMountains.Tools
{
	public static class MMDebug
	{
		...
		public struct MMDebugLogEvent
		{
			...
			{
				MMDebugLogEvent.OnEvent?.Invoke(item);
			}
		}
		...
		public static void DebugLogCommand(string command)
		{
			...
			if (array.Length > 1)
			{
				...
				{
					LogCommand("A version of command " + commandFirst + " needs at least " + mMDebugLogCommandArgumentCountAttribute.ArgumentCount + " arguments.", "#ff2a00");
					return;
				}
		...
		public static void DebugLogTime(object message, string color = "", int timePrecision = 3, bool displayFrameCount = true)
		{
			...
			{
				string text = "";
				text = new StackTrace().GetFrame(1).GetMethod().ReflectedType.Name;
				...
				{
					text4 = text4 + "<color=#82d3f9>[f" + Time.frameCount + "]</color> ";
				}
				text4 = text4 + "<color=#f9a682>[" + MMTime.FloatToTimeString(Time.time, displayHours: false, displayMinutes: true, displaySeconds: true, displayMilliseconds: true) + "]</color> ";
				text4 = text4 + text + " : ";
				text4 = text4 + text2 + message?.ToString() + text3;
		...
		public static void InstantiateOnScreenConsole(int fontSize = 25)
		{
			if (DebugLogsEnabled)
			{
				if (_console == null)
				{
					_console = (MMDebugOnScreenConsole)UnityEngine.Object.FindAnyObjectByType(typeof(MMDebugOnScreenConsole));
				}
				if (_console == null)
				{
