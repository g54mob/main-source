...
namespace MoreMountains.Tools
{
	...
	public class MMGyroscope : MonoBehaviour
	{
		...
		{
			if ((float)Time.frameCount != _lastGetValuesAt)
			{
				AutoCalibration();
				GetGyroValues();
		...
		{
			float num = ((TimeScale == TimeScales.Scaled) ? Time.deltaTime : Time.unscaledDeltaTime);
			...
			m_LerpedCalibratedInputAcceleration = Vector3.Lerp(m_LerpedCalibratedInputAcceleration, m_CalibratedInputAcceleration, num * LerpSpeed);
			m_LerpedCalibratedGyroscopeGravity = Vector3.Lerp(m_LerpedCalibratedGyroscopeGravity, m_CalibratedGyroscopeGravity, num * LerpSpeed);
		}
		...
		{
			Quaternion q = Quaternion.FromToRotation(-Vector3.forward, initialAcceleration);
			return Matrix4x4.TRS(Vector3.zero, q, Vector3.one).inverse;
		}

		private static Vector3 CalibratedAcceleration(Vector3 accelerator, Matrix4x4 matrix)
		{
			return matrix.MultiplyVector(accelerator);
		}
	}
}
