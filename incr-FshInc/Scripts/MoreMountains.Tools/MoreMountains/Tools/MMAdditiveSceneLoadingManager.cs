...
namespace MoreMountains.Tools
{
	public class MMAdditiveSceneLoadingManager : MMMonoBehaviour
	{
		...
		{
			AfterEntryFade = 0,
			AfterUnloadOriginScene = 1,
			BeforeSceneActivation = 2,
			BeforeExitFade = 3
		}
		...
		{
			{
				HoldModes.AfterEntryFade,
				false
			},
			{
				HoldModes.AfterUnloadOriginScene,
				false
			},
			{
				HoldModes.BeforeSceneActivation,
				false
			},
			{
				HoldModes.BeforeExitFade,
				false
			}
		};
		...
		{
			LoadScene(sceneToLoadName, settings.LoadingSceneName, settings.ThreadPriority, settings.SecureLoad, settings.InterpolateProgress, settings.BeforeEntryFadeDelay, settings.EntryFadeDuration, settings.AfterEntryFadeDelay, settings.BeforeSceneActivationDelay, settings.AfterSceneActivationDelay, settings.ExitFadeDuration, settings.EntryFadeTween, settings.ExitFadeTween, settings.ProgressBarSpeed, settings.FadeMode, settings.UnloadMethod, settings.AntiSpillSceneName, settings.SpeedIntervals, settings.DebugMode);
		}
		...
		public static void LoadScene(string sceneToLoadName, string loadingSceneName = "MMAdditiveLoadingScreen", ThreadPriority threadPriority = ThreadPriority.High, bool secureLoad = true, bool interpolateProgress = true, float beforeEntryFadeDelay = 0f, float entryFadeDuration = 0.25f, float afterEntryFadeDelay = 0.1f, float beforeSceneActivationDelay = 0.25f, float afterSceneActivationDelay = 0f, float exitFadeDuration = 0.2f, MMTweenType entryFadeTween = null, MMTweenType exitFadeTween = null, float progressBarSpeed = 5f, FadeModes fadeMode = FadeModes.FadeInThenOut, MMAdditiveSceneLoadingManagerSettings.UnloadMethods unloadMethod = MMAdditiveSceneLoadingManagerSettings.UnloadMethods.AllScenes, string antiSpillSceneName = "", List<MMSceneLoadingSpeedInterval> speedIntervals = null, bool debugMode = false)
		{
			...
			if (secureLoad)
			{
				...
				{
					Debug.LogError("MMLoadingSceneManagerAdditive : impossible to load the '" + sceneToLoadName + "' scene, there is no such scene in the project's build settings.");
					return;
				}
				if (!_scenesInBuild.Contains(loadingSceneName))
				{
					Debug.LogError("MMLoadingSceneManagerAdditive : impossible to load the '" + loadingSceneName + "' scene, there is no such scene in the project's build settings.");
					return;
				}
			}
		...
		protected virtual void Initialization()
		{
			...
			if (DebugMode)
			{
				Scene[] initialScenes = _initialScenes;
		...
		public static float ComputeInterpolationSpeed(float t)
		{
			...
			{
				foreach (MMSceneLoadingSpeedInterval speedInterval in _speedIntervals)
				{
					if (speedInterval.Interval.Contains(t))
					{
						return speedInterval.Speed;
					}
				}
			}
		...
		protected virtual IEnumerator EntryFade()
		{
			if (_entryFadeDuration > 0f)
			{
				...
				{
					yield return null;
					MMFadeOutEvent.Trigger(_entryFadeDuration, _entryFadeTween, FaderID);
				}
				else
				{
					yield return null;
					MMFadeInEvent.Trigger(_entryFadeDuration, _entryFadeTween, FaderID);
				}
		...
		protected virtual IEnumerator UnloadOriginScenes()
		{
			...
			for (int i = 0; i < initialScenes.Length; i++)
			{
				...
				SetAudioListener(state: true);
				while (_unloadOriginAsyncOperation.progress < 0.9f)
				{
					yield return null;
				}
			}
		...
		protected virtual IEnumerator LoadDestinationScene()
		{
			...
			while (_loadDestinationAsyncOperation.progress < 0.9f)
			{
		...
		protected virtual IEnumerator DestinationSceneActivation()
		{
			...
			while (_loadDestinationAsyncOperation.progress < 1f)
			{
				yield return null;
			}
		...
		protected virtual IEnumerator ExitFade()
		{
			...
			if (_exitFadeDuration > 0f)
			{
				...
				if (_fadeMode == FadeModes.FadeOutThenIn)
				{
					MMFadeInEvent.Trigger(_exitFadeDuration, _exitFadeTween, FaderID);
				}
				else
				{
					MMFadeOutEvent.Trigger(_exitFadeDuration, _exitFadeTween, FaderID);
				}
		...
		protected virtual IEnumerator UnloadSceneLoader()
		{
			...
			while (_unloadLoadingAsyncOperation.progress < 0.9f)
			{
				yield return null;
			}
		}
		...
		protected virtual void MMLoadingSceneDebug(string message)
		{
			if (DebugMode)
			{
				MMDebug.DebugLogInfo(string.Concat(string.Concat("" + "<color=#82d3f9>[" + Time.frameCount + "]</color> ", "<color=#f9a682>[", MMTime.FloatToTimeString(Time.time, displayHours: false, displayMinutes: true, displaySeconds: true, displayMilliseconds: true), "]</color> "), message));
			}
		}
	}
}
