	...
	public class MMPropertyLinkBool : MMPropertyLink
	{
		...
		public override void CreateGettersAndSetters(MMProperty property)
		{
			...
			if (property.MemberType == MMProperty.MemberTypes.Property)
			{
				...
				{
					GetBoolDelegate = (Func<bool>)Delegate.CreateDelegate(typeof(Func<bool>), firstArgument, property.MemberPropertyInfo.GetGetMethod());
				}
				if (property.MemberPropertyInfo.GetSetMethod() != null)
				{
					SetBoolDelegate = (Action<bool>)Delegate.CreateDelegate(typeof(Action<bool>), firstArgument, property.MemberPropertyInfo.GetSetMethod());
				}
		...
		{
			return emitter.Level = (GetValueOptimized(property) ? emitter.BoolRemapTrue : emitter.BoolRemapFalse);
		}

		public override float GetLevel(MMPropertyReceiver receiver, MMProperty property)
		{
			if (!GetValueOptimized(property))
			{
				return 0f;
			}
			return 1f;
		...
		protected virtual bool GetValueOptimized(MMProperty property)
		{
			...
			{
				return (bool)GetPropertyValue(property);
			}
