...
namespace MoreMountains.Tools
{
	...
	public class MMTouchFollowerJoystick : MMTouchJoystick
	{
		...
		protected bool _canvasPlaneInitialized;
		...
		public override void OnPointerDown(PointerEventData data)
		{
			...
			{
				_canvasPlane = new Plane(-TargetCamera.transform.forward, base.transform.position);
				_canvasPlaneInitialized = true;
			}
			Vector3 newPosition = ConvertToWorld(data.position);
			if (_canvasPlaneInitialized)
			{
				Ray ray = TargetCamera.ScreenPointToRay(data.position);
				if (_canvasPlane.Raycast(ray, out var enter))
				{
		...
		{
			OnDragEvent.Invoke();
			Vector3 position = ConvertToWorld(eventData.position);
			if (ParentCanvasRenderMode == RenderMode.ScreenSpaceCamera && TargetCamera != null)
			{
				Plane plane = new Plane(-TargetCamera.transform.forward, BackgroundCanvasGroup.transform.position);
			...
			_knobTransform.position = position;
			float num = Vector3.Distance(_knobTransform.position, BackgroundCanvasGroup.transform.position);
			if (num >= base.ComputedMaxRange)
			{
				Vector3 normalized = (_knobTransform.position - BackgroundCanvasGroup.transform.position).normalized;
				_backgroundPositionTarget = BackgroundCanvasGroup.transform.position + normalized * (num - base.ComputedMaxRange);
			}
			ConstrainBackground();
			ComputeJoystickValue();
		}
		...
		{
			Vector3 worldVector = _knobTransform.position - BackgroundCanvasGroup.transform.position;
			Vector3 vector = TransformToLocalSpace(worldVector);
			float magnitude = worldVector.magnitude;
			if (magnitude <= base.ComputedMaxRange)
			{
			...
			{
				RawValue.x = Mathf.InverseLerp(0f, magnitude, Mathf.Abs(vector.x)) * Mathf.Sign(vector.x);
				RawValue.y = Mathf.InverseLerp(0f, magnitude, Mathf.Abs(vector.y)) * Mathf.Sign(vector.y);
			}
		}
