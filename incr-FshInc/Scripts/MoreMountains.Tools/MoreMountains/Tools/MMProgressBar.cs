...
namespace MoreMountains.Tools
{
	...
	public class MMProgressBar : MMMonoBehaviour
	{
		...
		public AnimationCurve BumpIntensityMultiplier = new AnimationCurve(new Keyframe(-1f, 1f), new Keyframe(1f, 1f));
		...
		protected virtual void ComputeUpdatedText()
		{
			...
			if (DisplayTotal)
			{
				_updatedText = _updatedText + TotalSeparator + TextValueMultiplier.ToString(TextFormat);
			}
		...
		protected virtual void UpdateBars()
		{
			...
			{
				float num = ComputeNewFill(LerpForegroundBar, LerpIncreasingDelayedBarSpeed, LerpIncreasingDelayedBarDuration, LerpIncreasingDelayedBarCurve, 0f, _delayedBarIncreasingProgress, out t2);
				SetBarInternal(num, DelayedBarIncreasing, _delayedIncreasingImage, _initialBarSize);
				DelayedBarIncreasingProgress = num;
		...
		{
			float time = _newPercent - _percentLastTimeBarWasUpdated;
			float num = BumpIntensityMultiplier.Evaluate(time);
			bool flag = false;
			if (!_initialized)
			{
				return;
			...
			if (flag)
			{
				if (base.gameObject.activeInHierarchy)
				{
					StartCoroutine(BumpCoroutine(num));
				}
				OnBump?.Invoke();
				OnBumpIntensity?.Invoke(ApplyBumpIntensityMultiplier ? num : 1f);
			}
		}
		...
		protected virtual IEnumerator BumpCoroutine(float intensityMultiplier)
		{
			...
			while (journey <= BumpDuration)
			{
				...
				{
					float num2 = Mathf.Abs(1f - num) * intensityMultiplier;
					num = 1f + num2;
				}
