...
namespace MoreMountains.Tools
{
	public static class MMColors
	{
		...
		{
			Gradient gradient = new Gradient();
			GradientColorKey[] colorKeys = a.colorKeys;
			int num = Mathf.Max(b: b.colorKeys.Length, a: colorKeys.Length);
			GradientColorKey[] array = new GradientColorKey[num];
			for (int i = 0; i < num; i++)
			{
				float time = (float)i / ((float)num - 1f);
				Color a2 = a.Evaluate(time);
				Color b2 = b.Evaluate(time);
				array[i] = new GradientColorKey(Color.Lerp(a2, b2, t), time);
			}
			GradientAlphaKey[] alphaKeys = a.alphaKeys;
			int num2 = Mathf.Max(b: b.alphaKeys.Length, a: alphaKeys.Length);
			GradientAlphaKey[] array2 = new GradientAlphaKey[num2];
			for (int j = 0; j < num2; j++)
			{
				float time2 = (float)j / ((float)num2 - 1f);
				float a3 = a.Evaluate(time2).a;
				float a4 = b.Evaluate(time2).a;
				array2[j] = new GradientAlphaKey(Mathf.Lerp(a3, a4, t), time2);
			}
