...
namespace MoreMountains.Tools
{
	...
	public class MMAutoRotate : MonoBehaviour
	{
		...
		{
			_rotationPlane = default(Plane);
			_startTime = Time.time;
			_initialDelay = Random.Range(RandomInitialDelay.x, RandomInitialDelay.y);
			if (RandomizeRotationSpeed)
			{
				RotationSpeed = new Vector3(Random.Range(RotationSpeedMin.x, RotationSpeedMax.x), Random.Range(RotationSpeedMin.y, RotationSpeedMax.y), Random.Range(RotationSpeedMin.z, RotationSpeedMax.z));
			}
		}

		protected virtual void Update()
		{
			if (!(Time.time - _startTime < _initialDelay) && UpdateMode == UpdateModes.Update)
			{
				Rotate();
			}
		}
