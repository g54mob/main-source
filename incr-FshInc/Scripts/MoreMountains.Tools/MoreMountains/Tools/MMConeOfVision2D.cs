...
namespace MoreMountains.Tools
{
	...
	public class MMConeOfVision2D : MonoBehaviour
	{
		...
		protected virtual void ScanForTargets()
		{
			...
			_targetsWithinDistance = Physics2D.OverlapCircleAll(base.transform.position, VisionRadius, TargetMask);
			Collider2D[] targetsWithinDistance = _targetsWithinDistance;
			foreach (Collider2D collider2D in targetsWithinDistance)
			{
				_target = collider2D.transform;
				_directionToTarget = (_target.position - base.transform.position).normalized;
				if (Vector3.Angle(Direction, _directionToTarget) < VisionAngle / 2f)
				{
					_distanceToTarget = Vector3.Distance(base.transform.position, _target.position);
					_scanForTargetsHit2D = Physics2D.Raycast(base.transform.position, _directionToTarget, _distanceToTarget, ObstacleMask);
		...
		private RaycastData RaycastAtAngle(float angle)
		{
			...
			_raycastAtAngleHit2D = Physics2D.Raycast(base.transform.position, _direction, VisionRadius, ObstacleMask);
			if ((bool)_raycastAtAngleHit2D)
			{
			...
			else
			{
				...
				_returnRaycastData.Point = base.transform.position + _direction * VisionRadius;
