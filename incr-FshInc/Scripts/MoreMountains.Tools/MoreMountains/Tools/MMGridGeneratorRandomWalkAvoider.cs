	...
	public class MMGridGeneratorRandomWalkAvoider : MMGridGenerator
	{
		public static int[,] Generate(int width, int height, int seed, int fillPercentage, Vector2Int startingPoint, int[,] obstacles, int obstacleDistance, int maxIterations)
		{
			...
			while (num2 < num && num5 < maxIterations)
			{
				...
				{
				case 0:
					if (num4 + 1 <= height && !ObstacleAt(obstacles, num3, num4 + obstacleDistance))
					{
						num4++;
					...
					break;
				case 1:
					if (num4 - 1 > 1 && !ObstacleAt(obstacles, num3, num4 - obstacleDistance))
					{
						num4--;
						array = Carve(array, num3, num4, ref num2);
					}
					break;
				case 2:
					if (num3 - 1 > 1 && !ObstacleAt(obstacles, num3 - obstacleDistance, num4))
					{
						num3--;
						array = Carve(array, num3, num4, ref num2);
					}
					break;
				case 3:
					if (num3 + 1 <= width && !ObstacleAt(obstacles, num3 + obstacleDistance, num4))
					{
						num3++;
						array = Carve(array, num3, num4, ref num2);
					}
		...
		{
			if (MMGridGenerator.GetValueAtGridCoordinate(grid, x, y, 0) == 1)
			{
				MMGridGenerator.SetGridCoordinate(grid, x, y, 0);
				fillCounter++;
			}
			return grid;
		}
