...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_MaterialSetProperty : MMF_Feedback
	{
		...
		public MMTweenType InterpolationCurve = new MMTweenType(new AnimationCurve(new Keyframe(0f, 0f), new Keyframe(0.3f, 1f), new Keyframe(1f, 0f)), "InterpolateValue", "");
		...
		{
			if (!Active || !FeedbackTypeAuthorized || TargetRenderer == null)
			{
				return;
			}
			if (InterpolateValue)
		...
		protected virtual void SetValueAtTime(float t, float intensityMultiplier)
		{
			...
			case PropertyTypes.Float:
			{
				float num2 = MMTween.Tween(t, 0f, 1f, _initialFloat, NewFloat, InterpolationCurve);
				SetFloat(num2);
				break;
			}
			case PropertyTypes.Integer:
			{
				int num = (int)MMTween.Tween(t, 0f, 1f, _initialInt, NewInt, InterpolationCurve);
				SetInt(num);
				break;
			}
			case PropertyTypes.Texture:
		...
		protected virtual void SetColor(Color newColor)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetColor(_propertyID, newColor);
			}
		}
		...
		protected virtual void SetFloat(float newFloat)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetFloat(_propertyID, newFloat);
			}
		}
		...
		protected virtual void SetInt(int newInt)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetInt(_propertyID, newInt);
			}
		}
		...
		protected virtual void SetTexture(Texture newTexture)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetTexture(_propertyID, newTexture);
			}
		}
		...
		protected virtual void SetTextureOffset(Vector2 newOffset)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetTextureOffset(_propertyID, newOffset);
			}
		}
		...
		protected virtual void SetTextureScale(Vector2 newScale)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetTextureScale(_propertyID, newScale);
			}
		}
		...
		protected virtual void SetVector(Vector4 newVector)
		{
			...
			foreach (ExtraRendererData extraTargetRenderer in ExtraTargetRenderers)
			{
				extraTargetRenderer.TargetRenderer.materials[extraTargetRenderer.MaterialID].SetVector(_propertyID, newVector);
			}
		}
