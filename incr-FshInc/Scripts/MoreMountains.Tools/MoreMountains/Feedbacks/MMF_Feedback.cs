...
namespace MoreMountains.Feedbacks
{
	...
	public abstract class MMF_Feedback
	{
		...
		protected virtual IEnumerator TriggerRepeatedPlay(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			{
				float num = (InScaledTimescaleMode ? Time.time : Time.unscaledTime);
				TriggerCustomPlay(position, feedbacksIntensity);
				float repeatStartTime = num;
				float delay = Timing.DelayBetweenRepeats + FeedbackDuration;
				if (_repeatOffset <= Timing.DelayBetweenRepeats)
				{
					delay = Timing.DelayBetweenRepeats + FeedbackDuration - _repeatOffset;
				}
				yield return WaitFor(delay);
				yield return null;
				num = (InScaledTimescaleMode ? Time.time : Time.unscaledTime);
				_repeatOffset = num - repeatStartTime - (Timing.DelayBetweenRepeats + FeedbackDuration);
			}
			else
			{
				_sequenceCoroutine = Owner.StartCoroutine(SequenceCoroutine(position, feedbacksIntensity));
				float delay2 = ApplyTimeMultiplier(Timing.DelayBetweenRepeats) + Timing.Sequence.Length;
				yield return WaitFor(delay2);
			}
		}
