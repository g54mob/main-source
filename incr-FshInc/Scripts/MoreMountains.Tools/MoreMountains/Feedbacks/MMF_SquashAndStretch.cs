...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_SquashAndStretch : MMF_Feedback
	{
		...
		{
			SquashAndStretchTarget = FindAutomatedTarget<Transform>();
		}
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			if (Mode == Modes.Absolute || Mode == Modes.Additive)
			{
				...
				_coroutine = Owner.StartCoroutine(AnimateScale(SquashAndStretchTarget, FeedbackDuration, AnimateCurve, Axis, RemapCurveZero, RemapCurveOne * num));
			}
		...
		protected virtual IEnumerator AnimateScale(Transform targetTransform, float duration, AnimationCurve curve, PossibleAxis axis, float remapCurveZero = 0f, float remapCurveOne = 1f)
		{
			if (!(targetTransform == null) && duration != 0f)
			{
				...
				float percent2 = (NormalPlayDirection ? 1f : 0f);
				ComputeAndApplyScale(percent2, curve, remapCurveZero, remapCurveOne, targetTransform);
				_coroutine = null;
				IsPlaying = false;
				yield return null;
			}
		...
		{
			float x = curve.Evaluate(percent) + Offset;
			x = MMFeedbacksHelpers.Remap(x, 0f, 1f, remapCurveZero, remapCurveOne);
		...
		protected virtual void ApplyScale(float newScale)
		{
			...
			case PossibleAxis.XtoYZ:
				...
				_newScale.y = _initialScale.y * num;
				_newScale.z = _initialScale.z * num;
				break;
			case PossibleAxis.XtoY:
				_newScale.x = _initialScale.x * newScale;
				_newScale.y = _initialScale.y * num;
			...
			case PossibleAxis.XtoZ:
				...
				break;
			case PossibleAxis.YtoXZ:
				_newScale.x = _initialScale.x * num;
