...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Image : MMF_Feedback
	{
		public enum Modes
		{
			OverTime = 0,
			Instant = 1,
			ToDestinationColor = 2,
			ToDestinationColorAndBack = 3
		}
		...
		[Tooltip("the color to move to in instant mode")]
		[MMFEnumCondition("Mode", new int[] { 1, 2, 3 })]
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			case Modes.ToDestinationColor:
				if (AllowAdditivePlays || _coroutine == null)
				{
					...
					_coroutine = Owner.StartCoroutine(ToDestinationSequence(andBack: false));
				}
				break;
			case Modes.ToDestinationColorAndBack:
				if (AllowAdditivePlays || _coroutine == null)
				{
					...
					_coroutine = Owner.StartCoroutine(ToDestinationSequence(andBack: true));
				}
				break;
			}
		}
		...
		protected virtual IEnumerator ToDestinationSequence(bool andBack)
		{
			...
			while (journey >= 0f && journey <= FeedbackDuration && FeedbackDuration > 0f)
			{
				...
				if (ModifyColor)
				{
					BoundImage.color = Color.LerpUnclamped(_initialColor, ToDestinationColor, t);
				}
