...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks, MMEventListener<MMF_PlayerEvent>, MMEventListenerBase
	{
		[Serializable]
		...
		public MMChannelModes MMF_ChannelMode;

		[Tooltip("the channel to listen to - has to match the one on the feedback/event you use to trigger it")]
		[MMEnumCondition("MMF_ChannelMode", new int[] { 0 })]
		public int MMF_Channel;
		...
		[MMEnumCondition("MMF_ChannelMode", new int[] { 1 })]
		public MMChannel MMF_ChannelDefinition;
		...
		{
			if (mmfPlayerEvent.ChannelData == null || !MMChannel.Match(mmfPlayerEvent.ChannelData, MMF_ChannelMode, MMF_Channel, MMF_ChannelDefinition))
			{
				return;
			}
			...
			case MMF_PlayerEvent.Modes.PlayFeedbacks:
				...
				{
					PlayFeedbacks(base.transform.position, mmfPlayerEvent.FeedbacksIntensity);
				}
				break;
