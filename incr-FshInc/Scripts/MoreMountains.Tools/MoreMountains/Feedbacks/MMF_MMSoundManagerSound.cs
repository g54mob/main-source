...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_MMSoundManagerSound : MMF_Feedback
	{
		...
		[MMFEnumCondition("Source", new int[] { 0 })]
		...
		[MMFEnumCondition("Source", new int[] { 1 })]
		public AudioResource AudioResourceToPlay;

		[MMFInspectorGroup("Random Sound", true, 39, true, false)]
		...
		[MMFInspectorGroup("Fade In", true, 30, false, false)]
		...
		[MMFInspectorGroup("Fade Out", true, 30, false, false)]
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			{
				PlaySound(Sfx, position, intensity, AudioResourceToPlay);
			}
		}
		...
		protected virtual IEnumerator FadeOutCo()
		{
			...
			{
				MMPersistentSingleton<MMSoundManager>.Instance.FadeSound(_playedAudioSource, FadeOutDuration, _playedAudioSource.volume, 0f, FadeOutTween);
		...
		{
			if ((!DoNotPlayIfClipAlreadyPlaying || !(MMPersistentSingleton<MMSoundManager>.Instance.FindByClip(sfx) != null) || !MMPersistentSingleton<MMSoundManager>.Instance.FindByClip(sfx).isPlaying) && (MaximumConcurrentInstances < 0 || MMPersistentSingleton<MMSoundManager>.Instance.CurrentlyPlayingCount(sfx) < MaximumConcurrentInstances))
			{
		...
		{
			if ((MMSoundManager)UnityEngine.Object.FindAnyObjectByType(typeof(MMSoundManager)) == null)
			{
		...
		public virtual async void TestPlaySound()
		{
			...
			if (audioClip == null && AudioResourceToPlay == null)
			{
			...
			PlayAudioSource(_editorAudioSource, audioClip, volume, num, _randomPlaybackTime, _randomPlaybackDuration, AudioResourceToPlay);
			_lastPlayTimestamp = FeedbackTime;
