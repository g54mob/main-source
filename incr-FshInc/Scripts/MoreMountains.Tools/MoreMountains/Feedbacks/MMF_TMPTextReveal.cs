...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_TMPTextReveal : MMF_Feedback
	{
		...
		public bool HideTextOnInitialization;
		...
		protected override void CustomInitialization(MMF_Player owner)
		{
			...
			if (Active && FeedbackTypeAuthorized && !(TargetTMPText == null) && HideTextOnInitialization)
			{
				switch (RevealMode)
				{
				case RevealModes.Character:
		...
		protected virtual IEnumerator RevealCharacters()
		{
			...
			{
				float num = FeedbackTime - startTime;
				int value = ((DurationMode != DurationModes.Interval) ? Mathf.FloorToInt((float)_totalCharacters * num / RevealDuration) : Mathf.FloorToInt(num / IntervalBetweenReveals));
				value = Mathf.Clamp(value, 0, _totalCharacters);
				if (value > visibleCharacters)
				{
		...
		protected int RichTextLength(string richText)
		{
			...
			foreach (char c in text)
			{
				...
				case '<':
					...
					break;
				case '>':
					if (stringBuilder.ToString().StartsWith("sprite"))
					{
						num++;
					}
					flag = false;
