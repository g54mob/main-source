...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_ParticlesInstantiation : MMF_Feedback
	{
		...
		protected virtual void CreatePools(MMF_Player owner)
		{
			...
			if ((ParticlesPrefab == null && RandomParticlePrefabs.Count == 0) || _poolCreatedOrFound)
			{
				return;
			}
			...
			foreach (ParticleSystem randomParticlePrefab in RandomParticlePrefabs)
			{
				...
				gameObject2.name = Owner.name + "_" + randomParticlePrefab.name + "_ObjectPooler";
				MMMiniObjectPooler mMMiniObjectPooler = gameObject2.AddComponent<MMMiniObjectPooler>();
				mMMiniObjectPooler.GameObjectToPool = randomParticlePrefab.gameObject;
				mMMiniObjectPooler.PoolSize = ObjectPoolSize;
				mMMiniObjectPooler.NestWaitingPool = NestParticles;
				if (ParentTransform != null)
				{
					mMMiniObjectPooler.transform.SetParent(ParentTransform);
				}
				else
				{
					mMMiniObjectPooler.transform.SetParent(Owner.transform);
				}
				mMMiniObjectPooler.MutualizeWaitingPools = MutualizePools;
				mMMiniObjectPooler.FillObjectPool();
				if (Owner != null && gameObject2.transform.parent == null)
				{
					SceneManager.MoveGameObjectToScene(gameObject2, Owner.gameObject.scene);
				}
				_objectPoolers.Add(mMMiniObjectPooler);
			}
		}
		...
		protected virtual void InitializeWeights()
		{
			...
			int initialCapacity = ((Mode == Modes.Pool) ? (RandomParticleWeights.Count + 1) : RandomParticleWeights.Count);
			_weightShuffleBag = new MMShufflebag<int>(initialCapacity);
			if (Mode == Modes.Pool)
			{
				_weightShuffleBag.Add(0, MainParticlesPrefabWeight);
			}
			for (int j = 0; j < RandomParticleWeights.Count; j++)
			{
				int item = ((Mode == Modes.Pool) ? (j + 1) : j);
				_weightShuffleBag.Add(item, RandomParticleWeights[j]);
			}
		}
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			if (Mode == Modes.Pool)
			{
				...
				{
					int index = _weightShuffleBag.Pick();
					_newGameObject = _objectPoolers[index].GetPooledGameObject();
					_instantiatedParticleSystem = _newGameObject.MMFGetComponentNoAlloc<ParticleSystem>();
					if (_instantiatedParticleSystem != null)
					{
						PositionParticleSystem(_instantiatedParticleSystem);
			...
			int index2 = _weightShuffleBag.Pick();
			PlayTargetParticleSystem(_instantiatedRandomParticleSystems[index2]);
		}
		...
		protected virtual void PlayTargetParticleSystem(ParticleSystem targetParticleSystem)
		{
			...
			{
				ParticleSystem.MainModule main2 = targetParticleSystem.main;
				main2.simulationSpeed = Random.Range(ForcedSimulationSpeed.x, ForcedSimulationSpeed.y);
			}
