...
namespace MoreMountains.Feedbacks
{
	...
	public class MMBlink : MMMonoBehaviour
	{
		...
		[MMFEnumCondition("Method", new int[] { 1, 2, 3 })]
		public AnimationCurve Curve = new AnimationCurve(new Keyframe(0f, 0f), new Keyframe(0.3f, 1.05f), new Keyframe(1f, 0f));
		...
		[MMInspectorButtonBar(new string[] { "ToggleBlinking", "StartBlinking", "StopBlinking" }, new string[] { "ToggleBlinking", "StartBlinking", "StopBlinking" }, new bool[] { true, true, true }, new string[] { "main-call-to-action", "", "" })]
		...
		public virtual float Duration
		{
			get
			{
				if (RepeatCount < 0 || Phases.Count == 0)
				{
					return 0f;
				}
				float num = 0f;
				...
				{
					num += phase.PhaseDuration;
				}
				return num + num * (float)RepeatCount;
			}
		}
		...
		protected virtual void DetermineState()
		{
			...
			else if (GetTime() - _lastBlinkAt > Phases[CurrentPhaseIndex].OffDuration)
			{
				Active = true;
				_lastBlinkAt = GetTime();
			}
		...
		{
			_ = _currentColor;
			float c = (Active ? OffValue : OnValue);
			float num = (Active ? OnValue : OffValue);
			float num2 = num;
			if (LerpValue && GetTime() - _lastBlinkAt < _currentLerpDuration)
			{
				float time = MMFeedbacksHelpers.Remap(GetTime() - _lastBlinkAt, 0f, _currentLerpDuration, 0f, 1f);
				num2 = Curve.Evaluate(time);
				num2 = MMFeedbacksHelpers.Remap(num2, 0f, 1f, c, num);
			}
			else
			{
				num2 = num;
			}
			ApplyBlink(Active, num2);
		}
		...
		protected virtual void ApplyBlink(bool active, float value)
		{
			...
			case Methods.SetGameObjectActive:
				...
				{
					foreach (GameObject extraGameObject in ExtraGameObjects)
					{
						extraGameObject.SetActive(active);
					}
					break;
				}
			case Methods.MaterialAlpha:
			{
				...
				for (int j = 0; j < ExtraRenderers.Count; j++)
				{
					BlinkTargetRenderer blinkTargetRenderer2 = ExtraRenderers[j];
					ApplyCurrentColor(blinkTargetRenderer2.TargetRenderer, blinkTargetRenderer2.TargetMaterialIndex);
				}
				break;
			}
			case Methods.MaterialEmissionIntensity:
			{
				...
				ApplyCurrentColor(TargetRenderer, MaterialIndex);
				for (int k = 0; k < ExtraRenderers.Count; k++)
				{
					BlinkTargetRenderer blinkTargetRenderer3 = ExtraRenderers[k];
					ApplyCurrentColor(blinkTargetRenderer3.TargetRenderer, blinkTargetRenderer3.TargetMaterialIndex);
				}
				break;
			}
			case Methods.ShaderFloatValue:
			{
				...
				{
					BlinkTargetRenderer blinkTargetRenderer = ExtraRenderers[i];
					ApplyFloatValue(blinkTargetRenderer.TargetRenderer, value, blinkTargetRenderer.TargetMaterialIndex);
				}
				break;
			}
			}
		...
		{
			if (Phases[CurrentPhaseIndex].PhaseDuration <= 0f)
			{
				return;
			}
			...
			if (ForceStateOnExit)
			{
				...
				{
					ApplyBlink(active: false, 0f);
				}
				else
				{
					ApplyBlink(active: true, 1f);
				}
			}
			Blinking = false;
		}
