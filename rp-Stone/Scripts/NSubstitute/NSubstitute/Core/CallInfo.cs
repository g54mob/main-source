...
namespace NSubstitute.Core
{
	public class CallInfo
	{
		...
		public object this[int index]
		{
			get
			{
				return _callArguments[index].Value;
			}
			set
			{
				Argument argument = _callArguments[index];
		...
		public object[] Args()
		{
			return _callArguments.Select((Argument x) => x.Value).ToArray();
		}

		public Type[] ArgTypes()
		{
			return _callArguments.Select((Argument x) => x.DeclaredType).ToArray();
		}
		...
		{
			value = default(T);
			IEnumerable<Argument> enumerable = _callArguments.Where(condition);
			if (!enumerable.Any())
			{
				return false;
			}
