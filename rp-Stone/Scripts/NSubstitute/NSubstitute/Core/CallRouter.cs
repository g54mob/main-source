...
namespace NSubstitute.Core
{
	public class CallRouter : ICallRouter
	{
		...
		public bool CallBaseByDefault
		{
			get
			{
				return _substituteState.CallBaseConfiguration.CallBaseByDefault;
			}
			set
			{
		...
		public object? Route(ICall call)
		{
			...
			bool isQuerying = _threadContext.IsQuerying;
			Func<ICall, object[]> pendingRaisingEventArgs = _threadContext.UsePendingRaisingEventArgumentsFactory();
			Func<ISubstituteState, IRoute> queuedNextRouteFactory = _threadContext.UseNextRoute(this);
		...
		private IRoute ResolveCurrentRoute(ICall call, bool isQuerying, Func<ICall, object?[]>? pendingRaisingEventArgs, Func<ISubstituteState, IRoute>? queuedNextRouteFactory)
		{
			...
			{
				return queuedNextRouteFactory(_substituteState);
			}
