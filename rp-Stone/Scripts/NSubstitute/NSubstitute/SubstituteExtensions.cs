...
namespace NSubstitute
{
	public static class SubstituteExtensions
	{
		...
		{
			IReturn value = ((returnThese != null && returnThese.Length != 0) ? ((IReturn)new ReturnMultipleValues<T>(new T[1] { returnThis }.Concat<T>(returnThese).ToArray())) : ((IReturn)new ReturnValue(returnThis)));
		...
		{
			IReturn value = ((returnThese != null && returnThese.Length != 0) ? ((IReturn)new ReturnMultipleFuncsValues<T>(new Func<CallInfo, T>[1] { returnThis }.Concat<Func<CallInfo, T>>(returnThese).ToArray())) : ((IReturn)new ReturnValueFromFunc<T>(returnThis)));
			return SubstitutionContext.Current.ThreadContext.LastCallShouldReturn(value, matchArgs);
		}
