	...
	public static bool EvaluateInstaKill(StatModifier instigator, Character target)
	{
		if (!target.tags.Contains("boss") || (EventController.singleton.IsObjectiveActive("unmake_boss") && !target.id.Contains("dysangelos")))
		{
			...
			if (num > 0f && Random.Range(0f, 100f) <= num)
			{
				...
				target.Die(Character.DeathReason.Unmake);
