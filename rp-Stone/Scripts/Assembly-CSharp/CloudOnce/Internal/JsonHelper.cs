...
namespace CloudOnce.Internal
{
	public static class JsonHelper
	{
		...
		public static JSONObject ToJsonObject<T>(Dictionary<string, T> serializableDictionary) where T : IJsonSerializable
		{
			Dictionary<string, IJsonSerializable> dictionary = ConvertToSerializable(serializableDictionary);
			Dictionary<string, JSONObject> dictionary2 = new Dictionary<string, JSONObject>();
			foreach (KeyValuePair<string, IJsonSerializable> item in dictionary)
			{
		...
		public static JSONObject ToJsonObject<T>(List<T> serializableList) where T : IJsonSerializable
		{
		...
		private static Dictionary<string, IJsonSerializable> ConvertToSerializable<T>(Dictionary<string, T> dictionary) where T : IJsonSerializable
		{
			Dictionary<string, IJsonSerializable> dictionary2 = new Dictionary<string, IJsonSerializable>();
