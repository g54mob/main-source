...
namespace CloudOnce.QuickStart
{
	...
	public class AchievementsButton : MonoBehaviour
	{
		...
		private void Awake()
		{
			...
			if (button == null)
			{
				Debug.LogError("Show Achievements Button script placed on GameObject that is not a button. Script is only compatible with UI buttons created from GameObject menu (GameObjects -> UI -> Button).");
			}
		}
