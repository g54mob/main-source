...
namespace Castle.Core.Resource
{
	public class AssemblyResource : AbstractStreamResource
	{
		...
		{
			return string.Format(CultureInfo.CurrentCulture, "AssemblyResource: [{0}] [{1}]", new object[2] { assemblyName, resourcePath });
		}
		...
		private Stream CreateResourceFromUri(CustomUri resourcex, string path)
		{
			...
			if (nameFound == null)
			{
				throw new ResourceException(string.Format(CultureInfo.InvariantCulture, "The assembly resource {0} could not be located", new object[1] { resourcePath }));
			}
		...
		private string ConvertToPath(string resource)
		{
			...
			if (text[0] != '/')
			{
				text = string.Format(CultureInfo.CurrentCulture, "/{0}", new object[1] { text });
			}
			return text;
		}
		...
		private static Assembly ObtainAssembly(string assemblyName)
		{
			...
			{
				throw new ResourceException(string.Format(CultureInfo.InvariantCulture, "The assembly {0} could not be loaded", new object[1] { assemblyName }), innerException);
			}
		}
	}
}
