...
namespace Castle.Core.Internal
{
	public static class AttributesUtil
	{
		...
		{
			object[] customAttributes = type.GetTypeInfo().GetCustomAttributes(typeof(T), inherit: false);
		...
		{
			AttributeUsageAttribute[] array = attributeType.GetTypeInfo().GetCustomAttributes<AttributeUsageAttribute>(inherit: true).ToArray();
			if (array.Length == 0)
			{
