		...
		public override int Read(byte[] array, int offset, int count)
		{
			lock (lockObject)
			{
				if (Enabled)
				{
					...
					int num = sourceStream.Read(sourceBuffer, 0, count);
					int num2 = num / (bytesPerSample * channels);
					for (int i = 0; i < num2; i++)
					{
						int num3 = i * bytesPerSample * channels;
						ReadSamples(sourceBuffer, num3, out var left, out var right);
						simpleCompressor.Process(ref left, ref right);
						WriteSamples(array, offset + num3, left, right);
					}
					return count;
				}
