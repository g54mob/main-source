...
namespace NUnit.Framework.Api
{
	public class FrameworkController : LongLivedMarshalByRefObject
	{
		...
		private void Initialize(string assemblyPath, IDictionary settings)
		{
			...
			{
				InternalTraceLevel level = (InternalTraceLevel)Enum.Parse(typeof(InternalTraceLevel), (string)Settings["InternalTraceLevel"], ignoreCase: true);
				...
				string path2 = $"InternalTrace.{Process.GetCurrentProcess().Id}.{Path.GetFileName(assemblyPath)}.log";
		...
		public string LoadTests()
		{
			...
			return Runner.LoadedTest.ToXml(recursive: false).OuterXml;
		}
		...
		public string ExploreTests(string filter)
		{
			...
			return Runner.LoadedTest.ToXml(recursive: true).OuterXml;
		}

		public string RunTests(string filter)
		{
			Guard.ArgumentNotNull(filter, "filter");
			TNode tNode = Runner.Run(new TestProgressReporter(null), TestFilter.FromXml(filter)).ToXml(recursive: true);
			if (Settings != null)
			{
		...
		public string RunTests(Action<string> callback, string filter)
		{
			...
			TNode tNode = Runner.Run(new TestProgressReporter(handler), TestFilter.FromXml(filter)).ToXml(recursive: true);
			if (Settings != null)
			{
				InsertSettingsElement(tNode, Settings);
			}
		...
		private void ExploreTests(ICallbackEventHandler handler, string filter)
		{
			...
			handler.RaiseCallbackEvent(Runner.LoadedTest.ToXml(recursive: true).OuterXml);
		}
