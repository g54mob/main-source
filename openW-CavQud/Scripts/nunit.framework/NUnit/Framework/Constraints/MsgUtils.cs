...
namespace NUnit.Framework.Constraints
{
	internal static class MsgUtils
	{
		...
		static MsgUtils()
		{
			...
			AddFormatter((ValueFormatter next) => (object val) => (!(val is ValueType)) ? next(val) : string.Format(Fmt_ValueType, val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is DateTime)) ? next(val) : FormatDateTime((DateTime)val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is DateTimeOffset)) ? next(val) : FormatDateTimeOffset((DateTimeOffset)val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is decimal)) ? next(val) : FormatDecimal((decimal)val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is float)) ? next(val) : FormatFloat((float)val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is double)) ? next(val) : FormatDouble((double)val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is char)) ? next(val) : string.Format(Fmt_Char, val));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is IEnumerable)) ? next(val) : FormatCollection((IEnumerable)val, 0L, 10));
			AddFormatter((ValueFormatter next) => (object val) => (!(val is string)) ? next(val) : FormatString((string)val));
			AddFormatter((ValueFormatter next) => (object val) => (!val.GetType().IsArray) ? next(val) : FormatArray((Array)val));
		}
