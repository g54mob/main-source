			...
			try
			{
				...
				for (int count = Paths.Count; i < count; i++)
				{
					...
					if (flag2 && cell != null && findPath2.Steps[0] != cell)
					{
						...
						if (i == count - 1)
						{
							CombatJuice.BlockUntilFinished((CombatJuiceEntry)entry, (IList<GameObject>)null, 1500, true);
						}
					}
					if (gameObject == null)
					{
