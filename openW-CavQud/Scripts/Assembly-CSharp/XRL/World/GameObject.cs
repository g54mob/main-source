...
namespace XRL.World
{
	public sealed class GameObject : IEventSource, IEventHandler
	{
		...
		private const int TRANSIENT_CACHE_WORSHIP_PROCESSED = 1;
		...
		public const int FLAG_NO_COMBAT = 64;

		public const int FLAG_ALWAYS_PAINT = 128;

		public const int FLAG_IMPORTANT_OVERLAY_OBJECT = 256;
		...
		public List<GameObject> GetInventoryEquipmentAndCybernetics(List<GameObject> Store, Predicate<GameObject> Filter)
		{
			...
			{
				Inventory.GetObjects(Store, Filter);
			}
			if (HasPart<Body>())
			{
				Body.GetEquippedObjects(Store, Filter);
		...
		public bool HasRegisteredEventFrom(int EventID, IEventHandler Handler)
		{
			if (RegisteredEvents != null && RegisteredEvents.TryGetValue(EventID, out var Handlers))
			{
		...
		public bool TryGetEffect<T>(Guid ID, out T Effect) where T : Effect
		{
			EffectRack effects = _Effects;
			if (ID != Guid.Empty && effects != null)
			{
				int i = 0;
				for (int count = effects.Count; i < count; i++)
				{
					if (effects[i].ID == ID && effects[i] is T val)
					{
						Effect = val;
						return true;
					}
		...
		public void ParticleText(string Text, bool IgnoreVisibility = false)
		{
			if (IgnoreVisibility || IsVisible())
			{
				GetCurrentCell()?.ParticleText(Text, true);
			}
		}
		...
		public bool CheckInfluence(string Type = null, GameObject By = null, bool Silent = false)
		{
			...
			if (!num && Validate(ref By) && By.IsPlayer())
			{
				...
				By.Fail(GameText.VariableReplace(FailureMessage, this, null, StripColors: true));
			}
			return num;
		}
		...
		{
			if (RegisteredEvents == null || !RegisteredEvents.TryGetValue(IComponent<GameObject>.TurnTickID, out var Handlers))
			{
				return;
			}
			EventRegistry.List.Entry[] array = Handlers.GetArray();
			int i = 0;
			for (int count = Handlers.Count; i < count; i++)
			{
				IEventHandler handler = array[i].Handler;
				handler.TurnTick(TimeTick, Amount);
				if (count != Handlers.Count)
				{
					count = Handlers.Count;
					if (i < count && array[i].Handler != handler)
					{
						i--;
					}
				}
		...
		{
			bool flag = RegisteredEvents?.Dispatch(E) ?? true;
			if (flag)
			{
		...
		public bool PerformThrow(GameObject Weapon, Cell TargetCell, GameObject ApparentTarget = null, MissilePath MPath = null, int Phase = 0, int? RangeVariance = null, int? DistanceVariance = null, int? EnergyCost = null)
		{
			...
			void ExecuteJuice()
			{
				if (Options.UseParticleVFX && showJuice && juiceEnd != Location2D.Invalid && !juiceTile.IsNullOrEmpty() && !juiceColor.IsNullOrEmpty() && !juiceDetail.IsNullOrEmpty() && InActiveZone())
				{
					...
					CombatJuice.BlockUntilFinished((CombatJuiceEntry)CombatJuice.Throw(this, Weapon, juiceStart, juiceEnd, Async: true), (IList<GameObject>)null, 1000, true);
				}
			}
		}
		...
		public void MakeLive()
		{
			if (!Live)
			{
				XRL.The.ActionManager.AddLiveObject(this);
				Live = true;
			}
		}
		...
		{
			if (RegisteredEvents != null && RegisteredEvents.TryGetValue(RenderEvent.ID, out var Handlers))
			{
				foreach (EventRegistry.List.Entry item in Handlers)
				{
					if (item.Handler is IComponent<GameObject> component && !component.Render(E))
					{
						return false;
					}
				}
		...
		{
			if (RegisteredEvents != null && RegisteredEvents.TryGetValue(RenderEvent.OverlayID, out var Handlers))
			{
				foreach (EventRegistry.List.Entry item in Handlers)
				{
					if (item.Handler is IComponent<GameObject> component && !component.OverlayRender(E))
					{
						return false;
					}
				}
		...
		public bool FinalRender(RenderEvent E)
		{
			if (RegisteredEvents != null && RegisteredEvents.TryGetValue(RenderEvent.FinalID, out var Handlers))
			{
				foreach (EventRegistry.List.Entry item in Handlers)
				{
					if (item.Handler is IComponent<GameObject> component && !component.FinalRender(E))
					{
						return false;
					}
				}
		...
		[Obsolete("Use FinalRender(RenderEvent)")]
