		...
		public static Cell ShowLooker(int Range, int StartX, int StartY)
		{
			...
			if (zone != null)
			{
				...
				while (!flag)
				{
					...
					string text = ((!bLocked) ? ("{{W|" + ControlManager.getCommandInputFormatted("Cancel") + "}} | {{hotkey|(" + ControlManager.getCommandInputFormatted("CmdLockUnlock") + ")}} {{W|l}}ock") : ("{{W|" + ControlManager.getCommandInputFormatted("Cancel") + "}} | {{hotkey|(" + ControlManager.getCommandInputFormatted("CmdLockUnlock") + ")}} {{W|u}}nlock"));
					if (num10 > 1)
					{
						text = text + " | {{hotkey|" + ControlManager.getCommandInputFormatted("V Positive") + "/" + ControlManager.getCommandInputFormatted("V Negative") + "}} change selection";
					}
					if (gameObject2 != null)
					{
						text = text + " | {{hotkey|" + ControlManager.getCommandInputFormatted("Accept") + "}} interact";
					}
					text = text + " | {{hotkey|" + ControlManager.getCommandInputFormatted("CmdWalk") + "}} walk";
