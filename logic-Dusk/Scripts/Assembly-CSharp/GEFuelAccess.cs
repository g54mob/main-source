...
public class GEFuelAccess : IGEObject
{
	...
	public GEObjectTypeEnum objectType
	{
		get
		{
			return GEObjectTypeEnum.FuelAccess;
		}
	}
	...
	public Color baseLightColor
	{
		get
		{
			return GameEditorScript.FuelColor;
		}
	}
	...
	private GEFuelAccess()
	{
		objectTiles = new TileData[width, height];
	}

	public GEFuelAccess(int width, int height)
	{
	...
	{
		Debug.Log("Attached Editor to FuelAccess");
		this.ge = ge;
		isInEditMode = true;
	}
	...
	public void SetLLCorner(int c, int r)
	{
		...
		for (int i = c; i < num; i++)
		{
			for (int j = r; j < num2; j++)
			{
				...
				objectTiles[num3, num4].currentTileGroupType = TileData.TileGroupEnum.FuelAccess;
	...
	public void Move(int cDelta, int rDelta)
	{
		...
		if (!(zero.x < boundsAsRect.x) && !(zero.x > boundsAsRect.width - 2f) && !(zero.y < boundsAsRect.y) && !(zero.y > boundsAsRect.height - 2f))
		{
			if (!isMoving)
			{
				isMoving = true;
	...
	private void HandleMouseUpOnTileEvent(TileData tile)
	{
		if (isMoving && linkedObjects.Count > 0)
		{
			...
			if (boundsAsRect.x >= boundsAsRect2.x && boundsAsRect.width <= boundsAsRect2.width && (boundsAsRect.y == boundsAsRect2.y || boundsAsRect.y == boundsAsRect2.height - 2f))
			{
				flag = true;
			}
			else if (boundsAsRect.y >= boundsAsRect2.y && boundsAsRect.height <= boundsAsRect2.height && (boundsAsRect.x == boundsAsRect2.x || boundsAsRect.x == boundsAsRect2.width - 2f))
			{
				flag = true;
			}
			if (!flag)
