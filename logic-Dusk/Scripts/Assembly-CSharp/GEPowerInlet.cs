...
public class GEPowerInlet : IGEObject
{
	...
	public GEObjectTypeEnum objectType
	{
		get
		{
			return GEObjectTypeEnum.PowerInlet;
		}
	}
	...
	public Color baseLightColor
	{
		get
		{
			return GameEditorScript.InletColor;
		}
	}
	...
	private GEPowerInlet()
	{
		objectTiles = new TileData[width, height];
	}

	public GEPowerInlet(int width, int height)
	{
	...
	{
		Debug.Log("Attached Editor to PowerInlet");
		this.ge = ge;
		isInEditMode = true;
	}
	...
	public void SetLLCorner(int c, int r)
	{
		...
		for (int i = c; i < num; i++)
		{
			for (int j = r; j < num2; j++)
			{
				...
				objectTiles[num3, num4].currentTileGroupType = TileData.TileGroupEnum.PowerInlet;
