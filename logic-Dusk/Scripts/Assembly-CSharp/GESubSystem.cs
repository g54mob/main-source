...
public class GESubSystem : IGEObject
{
	...
	public GEObjectTypeEnum objectType
	{
		get
		{
			return GEObjectTypeEnum.SubSystem;
		}
	}
	...
	public Color baseLightColor
	{
		get
		{
			return GameEditorScript.SubSystemColor;
		}
	}
	...
	public GESubSystem(int width, int height)
	{
	...
	{
		Debug.Log("Attached Editor to SubSystem");
		this.ge = ge;
		isInEditMode = true;
	}
	...
	public void SetLLCorner(int c, int r)
	{
		...
		for (int i = c; i < num; i++)
		{
			for (int j = r; j < num2; j++)
			{
				...
				objectTiles[num3, num4].currentTileGroupType = TileData.TileGroupEnum.SubSystem;
