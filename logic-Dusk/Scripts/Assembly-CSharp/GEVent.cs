...
public class GEVent : IGEObject
{
	...
	public GEVent(int width, int height)
	{
	...
	{
		Debug.Log("Attached Editor to Vent");
		this.ge = ge;
		isInEditMode = true;
	}
	...
	public void SetLLCorner(int c, int r)
	{
		...
		for (int i = c; i < num; i++)
		{
			for (int j = r; j < num2; j++)
			{
				...
				objectTiles[num3, num4].currentTileGroupType = TileData.TileGroupEnum.Vent;
