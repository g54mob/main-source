...
public static class DeviceStringLookup
{
	...
	{
		if (modelToDisplay.ContainsKey(model))
		{
			return modelToDisplay[model];
		}
		string value = Regex.Match(model, "^[^0-9]*").Value;
		if (formattedKeyToDisplay.ContainsKey(value))
		{
			return formattedKeyToDisplay[value];
		}
		return value;
	}
}
