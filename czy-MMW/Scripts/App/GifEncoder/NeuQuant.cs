...
namespace GifEncoder
{
	public class NeuQuant
	{
		...
		protected static readonly int initrad = PaletteSize >> 3;
		...
		public NeuQuant(byte[] thepic, int len, int sample)
		{
			...
			for (int i = 0; i < PaletteSize; i++)
			{
				network[i] = new int[4];
				int[] array = network[i];
				array[0] = (array[1] = (array[2] = (i << netbiasshift + 8) / PaletteSize));
				freq[i] = intbias / PaletteSize;
				bias[i] = 0;
			}
		}
		...
		{
			byte[] array = new byte[3 * PaletteSize];
			int[] array2 = new int[PaletteSize];
			for (int i = 0; i < PaletteSize; i++)
			{
