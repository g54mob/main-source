...
namespace Motorways
{
	public class BoatNetworkDefinition
	{
		...
		public static BoatNetworkDefinition CreateFromBoatPathTileCoordinates(Dictionary<Vector2Int, BoatPathType> boatPathTileCoordinates)
		{
			...
			foreach (Vector2Int key3 in boatPathTileCoordinates.Keys)
			{
			...
			while (true)
			{
				Vector2Int? vector2Int = FindNextTileToStartSearchFrom(boatPathTileCoordinates, dictionary);
				if (!vector2Int.HasValue)
				{
					break;
				}
				stack.Push(vector2Int.Value);
				BoatPathLineDefinition boatPathLineDefinition = boatNetworkDefinition.CreateBoatLine();
				while (stack.Count > 0)
				{
					...
					boatPathLineDefinition.AddBoatPath(new Vector2Int(vector2Int2.x, vector2Int2.y), boatPathTileCoordinates[vector2Int2]);
					int num = 0;
					int num2 = 0;
					...
					for (int i = 0; i < nonDiagonalDirections.Length; i++)
					{
						...
						if (boatPathTileCoordinates.ContainsKey(vector2Int3))
						{
							num2++;
					...
					foreach (TileDirection direction in nonDiagonalDirections)
					{
						...
						if (!boatPathTileCoordinates.ContainsKey(vector2Int4))
						{
							continue;
						}
						...
						bool num3 = boatPathTileCoordinates.ContainsKey(key);
						bool flag = boatPathTileCoordinates.ContainsKey(key2);
						if (!num3 && !flag)
						{
							num2++;
					...
					boatPathLineDefinition.isLoop = num == 2;
					boatPathLineDefinition.isValid = num2 <= 2;
				}
			}
