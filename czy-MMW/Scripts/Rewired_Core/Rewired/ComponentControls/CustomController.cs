...
namespace Rewired.ComponentControls
{
	...
	public class CustomController : ComponentController
	{
		...
		private struct DrfoGIzjzEsfJzjBsbWATimBDOMs
		{
			public CustomControllerElementSelector.ElementType JivwipRSibdmICbauCZgLBleUGHE;

			public int PkgQTeEkeLPTGQNeawYyZpqSkBZW;

			public float lQcBgeiQogNcQNokUBIgByEhxvCdc;

			public DrfoGIzjzEsfJzjBsbWATimBDOMs(CustomControllerElementSelector.ElementType P_0, int P_1, float P_2)
			{
				JivwipRSibdmICbauCZgLBleUGHE = P_0;
				PkgQTeEkeLPTGQNeawYyZpqSkBZW = P_1;
				lQcBgeiQogNcQNokUBIgByEhxvCdc = P_2;
			}

			public DrfoGIzjzEsfJzjBsbWATimBDOMs(CustomControllerElementSelector.ElementType P_0, int P_1, bool P_2)
			{
				JivwipRSibdmICbauCZgLBleUGHE = P_0;
				PkgQTeEkeLPTGQNeawYyZpqSkBZW = P_1;
				lQcBgeiQogNcQNokUBIgByEhxvCdc = (P_2 ? 1f : 0f);
			}

			public bool nTfOZvKIjqlqlPPyTQWfktCZpUUt(CustomControllerElementSelector.ElementType P_0, int P_1)
			{
				if (JivwipRSibdmICbauCZgLBleUGHE == P_0)
				{
					return PkgQTeEkeLPTGQNeawYyZpqSkBZW == P_1;
				}
				return false;
			}

			public void WbSHRTqVDDOHcfvkOFiNYfgJZmGv(float P_0)
			{
				lQcBgeiQogNcQNokUBIgByEhxvCdc = MathTools.MaxMagnitude(lQcBgeiQogNcQNokUBIgByEhxvCdc, P_0);
			}

			public void fjdFwqDnVlAzGDaZKVCqRqblfZhFb(bool P_0)
			{
				if (P_0)
				{
					lQcBgeiQogNcQNokUBIgByEhxvCdc = 1f;
				}
			}
		}
		...
		private List<DrfoGIzjzEsfJzjBsbWATimBDOMs> sBebUMqKccfaKKCFJJMnDzaheZKRb = new List<DrfoGIzjzEsfJzjBsbWATimBDOMs>(10);

		[NonSerialized]
		private int oabEHIQUdktPefwVMlVDxYQDxRiv = -1;

		private Action dWcfjgHYWilhRhWYrAkFjCklNIDM;
		...
		internal event Action InputSourceUpdateEvent
		{
			add
			{
				dWcfjgHYWilhRhWYrAkFjCklNIDM = (Action)Delegate.Combine(dWcfjgHYWilhRhWYrAkFjCklNIDM, value);
			}
			remove
			{
				dWcfjgHYWilhRhWYrAkFjCklNIDM = (Action)Delegate.Remove(dWcfjgHYWilhRhWYrAkFjCklNIDM, value);
			}
		}
		...
		{
			return AOadYdXJkwiEabsZjvxNruRBvwtqA(false);
		}
		...
		{
			base.OnEnable();
			_ = base.vVBMHOaiZSIwaPgabflmyrUvnkZp;
		}
		...
		{
			base.OnDisable();
			if (base.vVBMHOaiZSIwaPgabflmyrUvnkZp)
			{
				sBebUMqKccfaKKCFJJMnDzaheZKRb.Clear();
			}
		}
		...
		{
			base.OnDestroy();
			pmAkOVJkcmaIdMJNicrCntTlaMul();
		}

		internal virtual bool OnInitialize()
		{
			if (!base.sPkmSmDAUKcXrXAnJqkTKtIVIjdw())
			{
				return false;
			}
			if (GetUseCustomController())
			{
				if (!wMetPYgESRtUwMqiFLhrvwxCjZLc())
				{
					return false;
				}
				if (AOadYdXJkwiEabsZjvxNruRBvwtqA(true) == null)
				{
		...
		{
			base.HMvTWDIodFaaSNPNAbMfnNGGUOLd();
			BrYsCHknmsSsEufEcJfpwvYujXfg();
			if (ReInput.isReady)
			{
				ReInput.InputSourceUpdateEvent += nPLqQQgDyPDOydeJAtqlyMTNIobW;
			}
		}

		internal virtual void OnUnsubscribeEvents()
		{
			base.BrYsCHknmsSsEufEcJfpwvYujXfg();
			ReInput.InputSourceUpdateEvent -= nPLqQQgDyPDOydeJAtqlyMTNIobW;
		}
		...
		{
			if (!base.vVBMHOaiZSIwaPgabflmyrUvnkZp || element == null || !GetUseCustomController())
			{
				return;
			}
			Rewired.CustomController customController = AOadYdXJkwiEabsZjvxNruRBvwtqA(false);
			...
			_ = sBebUMqKccfaKKCFJJMnDzaheZKRb.Count;
			for (int i = 0; i < sBebUMqKccfaKKCFJJMnDzaheZKRb.Count; i++)
			{
				DrfoGIzjzEsfJzjBsbWATimBDOMs value2 = sBebUMqKccfaKKCFJJMnDzaheZKRb[i];
				if (value2.nTfOZvKIjqlqlPPyTQWfktCZpUUt(element.elementType, elementIndex))
				{
					value2.WbSHRTqVDDOHcfvkOFiNYfgJZmGv(value);
					sBebUMqKccfaKKCFJJMnDzaheZKRb[i] = value2;
					return;
				}
			}
			sBebUMqKccfaKKCFJJMnDzaheZKRb.Add(new DrfoGIzjzEsfJzjBsbWATimBDOMs(element.elementType, elementIndex, value));
		}
		...
		internal void SetButtonValue(CustomControllerElementSelector element, bool value)
		{
			...
			for (int i = 0; i < sBebUMqKccfaKKCFJJMnDzaheZKRb.Count; i++)
			{
				...
				if (value2.nTfOZvKIjqlqlPPyTQWfktCZpUUt(element.elementType, elementIndex))
				{
					value2.fjdFwqDnVlAzGDaZKVCqRqblfZhFb(value);
					sBebUMqKccfaKKCFJJMnDzaheZKRb[i] = value2;
					return;
				}
			}
		...
		internal void ClearElementValue(CustomControllerElementSelector element)
		{
			...
			_ = sBebUMqKccfaKKCFJJMnDzaheZKRb.Count;
			for (int num = sBebUMqKccfaKKCFJJMnDzaheZKRb.Count - 1; num >= 0; num--)
			{
				if (sBebUMqKccfaKKCFJJMnDzaheZKRb[num].nTfOZvKIjqlqlPPyTQWfktCZpUUt(element.elementType, elementIndex))
				{
					sBebUMqKccfaKKCFJJMnDzaheZKRb.RemoveAt(num);
				}
			}
		}
		...
		private void EsWhHLbSHPzoceBuBqySrKzXSHZG()
		{
			if (base.vVBMHOaiZSIwaPgabflmyrUvnkZp)
			{
				sBebUMqKccfaKKCFJJMnDzaheZKRb.Clear();
		...
		private bool wMetPYgESRtUwMqiFLhrvwxCjZLc()
		{
			if (ReInput.isReady)
			{
				return true;
		...
		private void pyCGKiBFDOlWlADpZvrOZuqodeyIb()
		{
			if (sBebUMqKccfaKKCFJJMnDzaheZKRb.Count == 0)
			{
				return;
			}
			Rewired.CustomController customController = AOadYdXJkwiEabsZjvxNruRBvwtqA(false);
			...
			for (int i = 0; i < sBebUMqKccfaKKCFJJMnDzaheZKRb.Count; i++)
			{
				DrfoGIzjzEsfJzjBsbWATimBDOMs drfoGIzjzEsfJzjBsbWATimBDOMs = sBebUMqKccfaKKCFJJMnDzaheZKRb[i];
				switch (drfoGIzjzEsfJzjBsbWATimBDOMs.JivwipRSibdmICbauCZgLBleUGHE)
				{
				case CustomControllerElementSelector.ElementType.Axis:
					customController.SetAxisValue(drfoGIzjzEsfJzjBsbWATimBDOMs.PkgQTeEkeLPTGQNeawYyZpqSkBZW, drfoGIzjzEsfJzjBsbWATimBDOMs.lQcBgeiQogNcQNokUBIgByEhxvCdc);
					break;
				case CustomControllerElementSelector.ElementType.Button:
					customController.SetButtonValue(drfoGIzjzEsfJzjBsbWATimBDOMs.PkgQTeEkeLPTGQNeawYyZpqSkBZW, drfoGIzjzEsfJzjBsbWATimBDOMs.lQcBgeiQogNcQNokUBIgByEhxvCdc != 0f);
					break;
				default:
					throw new NotImplementedException();
				}
		...
		private Rewired.CustomController AOadYdXJkwiEabsZjvxNruRBvwtqA(bool P_0)
		{
			if (!GetUseCustomController())
			{
				return null;
			...
			if (oabEHIQUdktPefwVMlVDxYQDxRiv >= 0)
			{
				customController = ReInput.controllers.GetCustomController(oabEHIQUdktPefwVMlVDxYQDxRiv);
				if (customController == null)
				{
					oabEHIQUdktPefwVMlVDxYQDxRiv = -1;
				}
			}
			else
			{
			...
			if (customController == null)
			{
				if (_createCustomControllerSettings.createCustomController)
				{
					...
					if (customController != null)
					{
						oabEHIQUdktPefwVMlVDxYQDxRiv = customController.id;
						pXwyeejgAoBuAWsAepqGsbyouOkp(customController);
					}
				}
				else
				{
		...
		private void pXwyeejgAoBuAWsAepqGsbyouOkp(Rewired.CustomController P_0)
		{
			if (P_0 == null)
			{
				return;
		...
		private void pmAkOVJkcmaIdMJNicrCntTlaMul()
		{
			if (oabEHIQUdktPefwVMlVDxYQDxRiv >= 0 && _createCustomControllerSettings.destroyCustomController)
			{
				Rewired.CustomController customController = AOadYdXJkwiEabsZjvxNruRBvwtqA(false);
		...
		private void nPLqQQgDyPDOydeJAtqlyMTNIobW()
		{
			if (dWcfjgHYWilhRhWYrAkFjCklNIDM != null)
			{
				dWcfjgHYWilhRhWYrAkFjCklNIDM();
			}
			pyCGKiBFDOlWlADpZvrOZuqodeyIb();
		}
	}
}
