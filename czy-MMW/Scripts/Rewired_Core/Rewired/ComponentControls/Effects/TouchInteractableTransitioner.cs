...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchInteractableTransitioner : MonoBehaviour, IVisibilityChangedHandler, TouchInteractable.IInteractionStateTransitionHandler
	{
		...
		private TouchInteractable.InteractionState ALMthBuajMkTtgIZcmczodTINfry;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					rdfOzNbFPNoTvjcZSZtPaANSAIlv(value, false);
					znmnUUPiSnPhvVdqxxqexwBmNbAu();
				}
			}
		}
		...
		private void Awake()
		{
			if (Application.isPlaying)
			{
				...
				rdfOzNbFPNoTvjcZSZtPaANSAIlv(_visible, true);
			}
		}
		...
		private void OnEnable()
		{
			...
			CoVXQxIwAeLyxcmTBbpFDgIICFsL(true);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			ahLBqdFKMJfsmaHjdYNCBvMmojKYB();
		}
		...
		private void OnValidate()
		{
			...
			{
				AZBacHaPWwgnOehrvEMQFiURFmcf(null);
				lrBDAgPwSWNXiuaDivdgJLqJWIVo(Color.white, true);
				rJVisCXrYpYEfQjeBlDHECOWhYxO(_transitionAnimationTriggers.normalTrigger);
				CoVXQxIwAeLyxcmTBbpFDgIICFsL(true);
			}
			HQAFELIRSEqlZdfLZvYiOUNUainR();
		}
		...
		private void znmnUUPiSnPhvVdqxxqexwBmNbAu()
		{
			HQAFELIRSEqlZdfLZvYiOUNUainR();
		}

		private void HQAFELIRSEqlZdfLZvYiOUNUainR()
		{
			if (!Application.isPlaying)
			{
				CoVXQxIwAeLyxcmTBbpFDgIICFsL(true);
			...
			{
				CoVXQxIwAeLyxcmTBbpFDgIICFsL(false);
			}
		}

		private void CoVXQxIwAeLyxcmTBbpFDgIICFsL(bool P_0)
		{
			XOrKjLUdUZNrGuALmpZfMxlSBlsx(ALMthBuajMkTtgIZcmczodTINfry, P_0);
		}

		private void rdfOzNbFPNoTvjcZSZtPaANSAIlv(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
		...
		private bool bnvDZHwHkkROIuLWJddzdfrlvzWsA()
		{
			return UnityTools.IsActiveAndEnabled(this);
		}

		private void ahLBqdFKMJfsmaHjdYNCBvMmojKYB()
		{
			...
			{
				rJVisCXrYpYEfQjeBlDHECOWhYxO(normalTrigger);
			}
		}

		private void XOrKjLUdUZNrGuALmpZfMxlSBlsx(TouchInteractable.InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					lrBDAgPwSWNXiuaDivdgJLqJWIVo(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					lrBDAgPwSWNXiuaDivdgJLqJWIVo(color, P_1);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.SpriteSwap) != TouchInteractable.TransitionTypeFlags.None)
				{
					AZBacHaPWwgnOehrvEMQFiURFmcf(sprite);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.Animation) != TouchInteractable.TransitionTypeFlags.None)
				{
					rJVisCXrYpYEfQjeBlDHECOWhYxO(text);
				}
			}
		}

		private void lrBDAgPwSWNXiuaDivdgJLqJWIVo(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void AZBacHaPWwgnOehrvEMQFiURFmcf(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void rJVisCXrYpYEfQjeBlDHECOWhYxO(string P_0)
		{
		...
		{
			ALMthBuajMkTtgIZcmczodTINfry = args.state;
