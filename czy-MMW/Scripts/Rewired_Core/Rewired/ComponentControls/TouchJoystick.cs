using System;
using System.Collections;
using System.Collections.Generic;
using System.Diagnostics;
using System.Runtime.CompilerServices;
using Rewired.ComponentControls.Data;
using Rewired.Internal;
using Rewired.Utils;
using Rewired.Utils.Attributes;
using Rewired.Utils.UI;
using UnityEngine;
using UnityEngine.EventSystems;
using UnityEngine.Events;

namespace Rewired.ComponentControls
{
	[Serializable]
	[DisallowMultipleComponent]
	[AddComponentMenu("Rewired/Touch Joystick")]
	public sealed class TouchJoystick : TouchInteractable
	{
		public enum AxisDirection
		{
			Both = 0,
			Horizontal = 1,
			Vertical = 2
		}

		public enum JoystickMode
		{
			Analog = 0,
			Digital = 1
		}

		public enum SnapDirections
		{
			None = 0,
			Four = 4,
			Eight = 8,
			Sixteen = 0x10,
			ThirtyTwo = 0x20,
			SixtyFour = 0x40
		}

		private enum iSozMBzlajItGvDiANwnYoMvLVNg
		{
			None = 0,
			TowardTouch = 1,
			TowardHome = 2
		}

		private enum vfQUXOvXrBtkhFTPfwODYhAVOLcu
		{
			Local = 0,
			TouchRegion = 1
		}

		public enum StickBounds
		{
			Circle = 0,
			Square = 1
		}

		[Serializable]
		public class ValueChangedEventHandler : UnityEvent<Vector2>
		{
		}

		[Serializable]
		public class StickPositionChangedEventHandler : UnityEvent<Vector2>
		{
		}

		[Serializable]
		public class TapEventHandler : UnityEvent
		{
		}

		[Serializable]
		public class TouchStartedEventHandler : UnityEvent
		{
		}

		[Serializable]
		public class TouchEndedEventHandler : UnityEvent
		{
		}

		public interface IValueChangedHandler
		{
			void OnValueChanged(Vector2 value);
		}

		public interface IStickPositionChangedHandler
		{
			void OnStickPositionChanged(Vector2 value);
		}

		[Serializable]
		private sealed class YTlAbsMTWdzGcUbPdpYNGOdMPpLL
		{
			public static readonly YTlAbsMTWdzGcUbPdpYNGOdMPpLL _003C_003E9 = new YTlAbsMTWdzGcUbPdpYNGOdMPpLL();

			public static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IValueChangedHandler, Vector2> _003C_003E9__277_0;

			public static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IStickPositionChangedHandler, Vector2> _003C_003E9__280_0;

			internal void UYBWtElodgNwmopPnZtOGztQNrM(IValueChangedHandler P_0, Vector2 P_1)
			{
				P_0.OnValueChanged(P_1);
			}

			internal void qSRjHOrTXdfrmdBmwAAxToTAAKcr(IStickPositionChangedHandler P_0, Vector2 P_1)
			{
				P_0.OnStickPositionChanged(P_1);
			}
		}

		private sealed class knBUZUtTMoCMUbaEDAxPLUGCIljx : IEnumerator<object>, IEnumerator, IDisposable
		{
			private int DfYSJaLfGwFgfGhSohSoSpfadLnK;

			private object JNAVFYsZQbyzBMCPGyTsiDZquQvH;

			public float qcXyyUBecFXIuZKLEkNsSKRPUNEX;

			public TouchJoystick UNcNCAbCvJTIZjRKydgHUVmClDKI;

			public PositionType ZfgjAiHovsLbakSkybiZzFJMHjoy;

			public Vector2 fQBRvKskjvqHbhsrJbwiRKSDRWqA;

			public iSozMBzlajItGvDiANwnYoMvLVNg begpbGefnpznTJqoEPKnXZDycUEc;

			private RectTransform VpvBJLYpadQuuwZnFNNoDQGBziol;

			private Vector2 SYOUVbThVdnZSyAkfeWhMiMmEoBQ;

			private float maqhJzhvzkNFdrFVhVTQociqnjXbA;

			private float qanAAXIvuljhAyviWjnsipKFgxkkb;

			object IEnumerator<object>.Current
			{
				[DebuggerHidden]
				get
				{
					return JNAVFYsZQbyzBMCPGyTsiDZquQvH;
				}
			}

			object IEnumerator.Current
			{
				[DebuggerHidden]
				get
				{
					return JNAVFYsZQbyzBMCPGyTsiDZquQvH;
				}
			}

			[DebuggerHidden]
			public knBUZUtTMoCMUbaEDAxPLUGCIljx(int P_0)
			{
				DfYSJaLfGwFgfGhSohSoSpfadLnK = P_0;
			}

			[DebuggerHidden]
			void IDisposable.Dispose()
			{
			}

			private bool MoveNext()
			{
				int dfYSJaLfGwFgfGhSohSoSpfadLnK = DfYSJaLfGwFgfGhSohSoSpfadLnK;
				TouchJoystick uNcNCAbCvJTIZjRKydgHUVmClDKI = UNcNCAbCvJTIZjRKydgHUVmClDKI;
				if (dfYSJaLfGwFgfGhSohSoSpfadLnK != 0)
				{
					if (dfYSJaLfGwFgfGhSohSoSpfadLnK != 1)
					{
						return false;
					}
					DfYSJaLfGwFgfGhSohSoSpfadLnK = -1;
					goto IL_010c;
				}
				DfYSJaLfGwFgfGhSohSoSpfadLnK = -1;
				if (!(qcXyyUBecFXIuZKLEkNsSKRPUNEX <= 0f))
				{
					VpvBJLYpadQuuwZnFNNoDQGBziol = uNcNCAbCvJTIZjRKydgHUVmClDKI.KXbFLgsvCYMSvuHjRTfknYKDbMAM;
					SYOUVbThVdnZSyAkfeWhMiMmEoBQ = DqaChzdTZYNFWhBQIpZWKgtwfiCfA.CyKeeehKagdHzEOJhkdvTCWQzUMsb(VpvBJLYpadQuuwZnFNNoDQGBziol, ZfgjAiHovsLbakSkybiZzFJMHjoy);
					float magnitude = (fQBRvKskjvqHbhsrJbwiRKSDRWqA - SYOUVbThVdnZSyAkfeWhMiMmEoBQ).magnitude;
					if (!(magnitude < 0.01f))
					{
						uNcNCAbCvJTIZjRKydgHUVmClDKI._isMoving = true;
						maqhJzhvzkNFdrFVhVTQociqnjXbA = magnitude / qcXyyUBecFXIuZKLEkNsSKRPUNEX;
						qanAAXIvuljhAyviWjnsipKFgxkkb = 0f;
						goto IL_010c;
					}
				}
				goto IL_0119;
				IL_0119:
				uNcNCAbCvJTIZjRKydgHUVmClDKI.TjANlvPJMKbQcwfLaQdRlvzJDifr(begpbGefnpznTJqoEPKnXZDycUEc, fQBRvKskjvqHbhsrJbwiRKSDRWqA, ZfgjAiHovsLbakSkybiZzFJMHjoy);
				return false;
				IL_010c:
				if (qanAAXIvuljhAyviWjnsipKFgxkkb <= 1f)
				{
					qanAAXIvuljhAyviWjnsipKFgxkkb += Time.unscaledDeltaTime / maqhJzhvzkNFdrFVhVTQociqnjXbA;
					DqaChzdTZYNFWhBQIpZWKgtwfiCfA.BsdJYCzGuBKzUbylUTSvUyoLgxek(VpvBJLYpadQuuwZnFNNoDQGBziol, Vector2.Lerp(SYOUVbThVdnZSyAkfeWhMiMmEoBQ, fQBRvKskjvqHbhsrJbwiRKSDRWqA, Mathf.SmoothStep(0f, 1f, qanAAXIvuljhAyviWjnsipKFgxkkb)), ZfgjAiHovsLbakSkybiZzFJMHjoy);
					JNAVFYsZQbyzBMCPGyTsiDZquQvH = null;
					DfYSJaLfGwFgfGhSohSoSpfadLnK = 1;
					return true;
				}
				goto IL_0119;
			}

			bool IEnumerator.MoveNext()
			{
				//ILSpy generated this explicit interface implementation from .override directive in MoveNext
				return this.MoveNext();
			}

			[DebuggerHidden]
			void IEnumerator.Reset()
			{
				throw new NotSupportedException();
			}
		}

		private const float MAX_MOVE_SPEED = 20f;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("The Custom Controller element(s) that will receive input values from the joystick's X axis.")]
		private CustomControllerElementTargetSetForFloat _horizontalAxisCustomControllerElement = new CustomControllerElementTargetSetForFloat();

		[SerializeField]
		[Tooltip("The Custom Controller element(s) that will receive input values from the joystick's Y axis.")]
		[CustomObfuscation(rename = false)]
		private CustomControllerElementTargetSetForFloat _verticalAxisCustomControllerElement = new CustomControllerElementTargetSetForFloat();

		[Tooltip("The Custom Controller element that will receive input values from taps.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private CustomControllerElementTargetSetForBoolean _tapCustomControllerElement = new CustomControllerElementTargetSetForBoolean();

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("The Rect Transform of the stick disc. This is moved around by the user when manipulating the joystick.")]
		private RectTransform _stickTransform;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("The joystick's mode of operation. Set this to Digital to simulate a D-Pad which has only On/Off states. If you want mimic a real D-Pad, you should also set Snap Directions to 8.")]
		private JoystickMode _joystickMode;

		[Range(0f, 1f)]
		[Tooltip("A dead zone which is applied when Stick Mode is set to Digital. This is used to filter out tiny stick movements near 0, 0.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private float _digitalModeDeadZone = 0.3f;

		[SerializeField]
		[Tooltip("The range of movement of the stick in Canvas pixels. The larger the number, the further the stick must be moved from center to register movement.")]
		[CustomObfuscation(rename = false)]
		[Range(0.01f, 1000f)]
		private float _stickRange = 60f;

		[Tooltip("If enabled, the stick range will scale with parent controls. Otherwise, the stick range will remain constant.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private bool _scaleStickRange = true;

		[CustomObfuscation(rename = false)]
		[Tooltip("The shape of the range of movement of the joystick.")]
		[SerializeField]
		private StickBounds _stickBounds;

		[CustomObfuscation(rename = false)]
		[Tooltip("The axis directions in which movement is allowed. You can restrict movement to one or both axes.")]
		[SerializeField]
		private AxisDirection _axesToUse;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("Snaps joystick movement to a fixed number of directions. This can be used to create a D-Pad, for example, setting it to 4 or 8 directions. If you want a true D-Pad, Stick Mode should be set to digital.")]
		private SnapDirections _snapDirections;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("If true, the stick disc will snap immediately to the touch position when initially touched. This results in the stick disc being centered to the touch position. This will cause the stick to generate input immediately when touched if not touched perfectly centered.If false, the stick disc will remain in its current position on touch, and when dragged will retain the same offset. The stick's center point will be set to the position of the touch. The initial touch will not cause the stick to pop in any direction.")]
		private bool _snapStickToTouch;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("If true, the stick will return to the center after it is released. Otherwise, the stick will remain in the last position and continue to return input.")]
		private bool _centerStickOnRelease = true;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("The underlying Axis 2D.")]
		private StandaloneAxis2D _axis2D = new StandaloneAxis2D();

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("If true, the joystick can be activated by a touch swipe that began in an area outside the joystick region. If false, the joystick can only be activated by a direct touch.")]
		private bool _activateOnSwipeIn;

		[CustomObfuscation(rename = false)]
		[Tooltip("If true, the joystick will stay engaged even if the touch that activated it moves outside the joystick region. If false, the joystick will be released once the touch that activated it moves outside the joystick region.")]
		[SerializeField]
		private bool _stayActiveOnSwipeOut = true;

		[SerializeField]
		[CustomObfuscation(rename = false)]
		[Tooltip("Should taps on the touch pad be processed?")]
		private bool _allowTap;

		[Tooltip("The maximum touch duration allowed for the touch to be considered a tap. A touch that lasts longer than this value will not trigger a tap when released.")]
		[FieldRange(0f, float.MaxValue)]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private float _tapTimeout = 0.25f;

		[FieldRange(-1, int.MaxValue)]
		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("The maximum movement distance allowed in pixels since the touch began for the touch to be considered a tap. [-1 = no limit]")]
		private int _tapDistanceLimit = 10;

		[SerializeField]
		[Tooltip("Optional external region to use for hover/click/touch detection. If set, this region will be used for touch detection instead of or in addition to the joystick's RectTransform. This can be useful if you want a larger area of the screen to act as a joystick.")]
		[CustomObfuscation(rename = false)]
		private TouchRegion _touchRegion;

		[SerializeField]
		[CustomObfuscation(rename = false)]
		[Tooltip("If True, hovers/clicks/touches on the local joystick will be ignored and only Touch Region touches will be used. Otherwise, both touches on the joystick and on the Touch Region will be used. This also applies to mouse hover. This setting has no effect if no Touch Region is set.")]
		private bool _useTouchRegionOnly = true;

		[SerializeField]
		[CustomObfuscation(rename = false)]
		[Tooltip("If True, the joystick will move to the location of the current touch in the Touch Region. This can be used to designate an area of the screen as a hot-spot for a joystick and have the joystick graphics follow the users touches. This only has an effect if a Touch Region is set.")]
		private bool _moveToTouchPosition;

		[SerializeField]
		[Tooltip("If Move To Touch Position is enabled, this will make the joystick return to its original position after the press is released. This only has an effect if a Touch Region is set.")]
		[CustomObfuscation(rename = false)]
		private bool _returnOnRelease = true;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("If True, the joystick will follow the touch around until released. This setting overrides Move To Touch Position.")]
		private bool _followTouchPosition;

		[CustomObfuscation(rename = false)]
		[SerializeField]
		[Tooltip("Should the joystick animate when moving to the touch point? This only has an effect if Move To Touch Position is True and a Touch Region is set. This setting is ignored if Follow Touch Position is True.")]
		private bool _animateOnMoveToTouch = true;

		[Tooltip("The speed at which the joystick will move toward the touch position measured in screens per second (based on the larger of width and height). [1.0 = Move 1 screen/sec]. This only has an effect if Move To Touch Position is True, Animate On Move To Touch is true, and a Touch Region is set. This setting is ignored if Follow Touch Position is True.")]
		[Range(0f, 20f)]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private float _moveToTouchSpeed = 2f;

		[Tooltip("Should the joystick animate when moving back to its original position? This only has an effect if Follow Touch Position is True, or if Move To Touch Position is True and a Touch Region is set, and Return on Release is True.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private bool _animateOnReturn = true;

		[SerializeField]
		[Tooltip("The speed at which the joystick will move back toward its original position measured in screens per second (based on the larger of width and height). [1.0 = Move 1 screen/sec]. This only has an effect if Follow Touch Position is True, or if Move To Touch Position is True and a Touch Region is set, and Return on Release and Animate on Return are both True.")]
		[Range(0f, 20f)]
		[CustomObfuscation(rename = false)]
		private float _returnSpeed = 2f;

		[Tooltip("If True, it will attempt to automatically manage Graphic component raycasting for best results based on your current settings.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private bool _manageRaycasting = true;

		private bool _useXAxis;

		private bool _useYAxis;

		private YVQfKeazIqgFlQlVhbEkyBsluncmA.HierarchyEventHelper<IValueChangedHandler, Vector2> _hierarchyValueChangedHandlers;

		private YVQfKeazIqgFlQlVhbEkyBsluncmA.HierarchyEventHelper<IStickPositionChangedHandler, Vector2> _hierarchyStickPositionChangedHandlers;

		private TouchRegion _workingTouchRegion;

		private Vector2 _origAnchoredPosition;

		private Vector2 _origStickAnchoredPosition;

		private Vector2 _lastPressAnchoredPosition;

		private bool _isMoving;

		private bool _isMovedFromDefaultPosition;

		private iSozMBzlajItGvDiANwnYoMvLVNg _moveDirection;

		private int _pointerId = int.MinValue;

		private int _realMousePointerId = int.MinValue;

		[NonSerialized]
		private bool YJYDYHWoCYyOHUKCZjPpKJHEtHeg;

		[NonSerialized]
		private bool wmyitCoQJvGHRkGBIlggqPNNGZoNA;

		private bool _pointerDownIsFake;

		private Vector2 _lastPressStartingValue;

		private vfQUXOvXrBtkhFTPfwODYhAVOLcu _lastClaimSource;

		private float _touchStartTime;

		private Vector2 _touchStartPosition;

		private IEnumerator _coroutineMove;

		private DVQzFAWdDpbloDWGesgiOGNXKEUXA _imageRaycastHelper = new DVQzFAWdDpbloDWGesgiOGNXKEUXA();

		private int _calculatedStickRange_lastUpdatedFrame = -1;

		private int _lastTapFrame = -1;

		private bool _isEligibleForTap;

		private float __calculatedStickRange_cachedValue;

		private Action<iSozMBzlajItGvDiANwnYoMvLVNg> __moveStartedDelegate;

		private Action<iSozMBzlajItGvDiANwnYoMvLVNg> __moveEndedDelegate;

		[SerializeField]
		[CustomObfuscation(rename = false)]
		[Tooltip("Event sent when the joystick value changes.")]
		private ValueChangedEventHandler _onValueChanged = new ValueChangedEventHandler();

		[Tooltip("Event sent when the joystick's stick position changes.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private ValueChangedEventHandler _onStickPositionChanged = new ValueChangedEventHandler();

		[Tooltip("Event sent when the joystick is touched.")]
		[SerializeField]
		[CustomObfuscation(rename = false)]
		private TouchStartedEventHandler _onTouchStarted = new TouchStartedEventHandler();

		[SerializeField]
		[CustomObfuscation(rename = false)]
		private TouchEndedEventHandler _onTouchEnded = new TouchEndedEventHandler();

		[Tooltip("Event sent when the touch pad is tapped. This event will only be sent if allowTap is True.")]
		[CustomObfuscation(rename = false)]
		[SerializeField]
		private TapEventHandler _onTap = new TapEventHandler();

		private Dictionary<int, PointerEventData> __fakePointerEventData;

		private static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IValueChangedHandler, Vector2> __valueChangedHandlerDelegate;

		private static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IStickPositionChangedHandler, Vector2> __stickPositionChangedHandlerDelegate;

		public CustomControllerElementTargetSetForFloat horizontalAxisCustomControllerElement => _horizontalAxisCustomControllerElement;

		public CustomControllerElementTargetSetForFloat verticalAxisCustomControllerElement => _verticalAxisCustomControllerElement;

		public CustomControllerElementTargetSetForBoolean tapCustomControllerElement => _tapCustomControllerElement;

		public RectTransform stickTransform
		{
			get
			{
				return _stickTransform;
			}
			set
			{
				if (!(_stickTransform == value))
				{
					_stickTransform = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public JoystickMode joystickMode
		{
			get
			{
				return _joystickMode;
			}
			set
			{
				if (_joystickMode != value)
				{
					_joystickMode = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public float digitalModeDeadZone
		{
			get
			{
				return _digitalModeDeadZone;
			}
			set
			{
				value = MathTools.Clamp01(value);
				if (_digitalModeDeadZone != value)
				{
					_digitalModeDeadZone = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public float stickRange
		{
			get
			{
				return _stickRange;
			}
			set
			{
				value = MathTools.Clamp(value, 1f, 1000f);
				if (_stickRange != value)
				{
					_stickRange = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool scaleStickRange
		{
			get
			{
				return _scaleStickRange;
			}
			set
			{
				if (_scaleStickRange != value)
				{
					_scaleStickRange = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		private StickBounds bOWgEzdHZgdhFxAeuXcKyTbZixPD
		{
			get
			{
				return _stickBounds;
			}
			set
			{
				if (_stickBounds != stickBounds)
				{
					_stickBounds = stickBounds;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public AxisDirection axesToUse
		{
			get
			{
				return _axesToUse;
			}
			set
			{
				if (_axesToUse != value)
				{
					qwzZgYRNQzLlANWmcEynJnDggpWt(value);
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public SnapDirections snapDirections
		{
			get
			{
				return _snapDirections;
			}
			set
			{
				if (_snapDirections != value)
				{
					_snapDirections = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool snapStickToTouch
		{
			get
			{
				return _snapStickToTouch;
			}
			set
			{
				if (_snapStickToTouch != value)
				{
					_snapStickToTouch = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool centerStickOnRelease
		{
			get
			{
				return _centerStickOnRelease;
			}
			set
			{
				if (_centerStickOnRelease != value)
				{
					_centerStickOnRelease = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool activateOnSwipeIn
		{
			get
			{
				return _activateOnSwipeIn;
			}
			set
			{
				if (_activateOnSwipeIn != value)
				{
					_activateOnSwipeIn = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool stayActiveOnSwipeOut
		{
			get
			{
				if (TvZIKxifGZnpXiSxsAUvjInjUZzh())
				{
					return true;
				}
				return _stayActiveOnSwipeOut;
			}
			set
			{
				if (_stayActiveOnSwipeOut != value)
				{
					_stayActiveOnSwipeOut = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool allowTap
		{
			get
			{
				return _allowTap;
			}
			set
			{
				if (_allowTap != value)
				{
					_allowTap = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public float tapTimeout
		{
			get
			{
				return _tapTimeout;
			}
			set
			{
				value = MathTools.Max(0f, value);
				if (_tapTimeout != value)
				{
					_tapTimeout = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public int tapDistanceLimit
		{
			get
			{
				return _tapDistanceLimit;
			}
			set
			{
				value = MathTools.Max(-1, value);
				if (_tapDistanceLimit != value)
				{
					_tapDistanceLimit = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public TouchRegion touchRegion
		{
			get
			{
				return _touchRegion;
			}
			set
			{
				if (!(_touchRegion == value))
				{
					_touchRegion = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool useTouchRegionOnly
		{
			get
			{
				return _useTouchRegionOnly;
			}
			set
			{
				if (_useTouchRegionOnly != value)
				{
					_useTouchRegionOnly = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool moveToTouchPosition
		{
			get
			{
				return _moveToTouchPosition;
			}
			set
			{
				if (_moveToTouchPosition != value)
				{
					_moveToTouchPosition = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool returnOnRelease
		{
			get
			{
				return _returnOnRelease;
			}
			set
			{
				if (_returnOnRelease != value)
				{
					_returnOnRelease = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool followTouchPosition
		{
			get
			{
				return _followTouchPosition;
			}
			set
			{
				if (_followTouchPosition != value)
				{
					_followTouchPosition = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool animateOnMoveToTouch
		{
			get
			{
				return _animateOnMoveToTouch;
			}
			set
			{
				if (_animateOnMoveToTouch != value)
				{
					_animateOnMoveToTouch = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public float moveToTouchSpeed
		{
			get
			{
				return _moveToTouchSpeed;
			}
			set
			{
				value = MathTools.Clamp(value, 0f, 20f);
				if (_moveToTouchSpeed != value)
				{
					_moveToTouchSpeed = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool animateOnReturn
		{
			get
			{
				return _animateOnReturn;
			}
			set
			{
				if (_animateOnReturn != value)
				{
					_animateOnReturn = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public float returnSpeed
		{
			get
			{
				return _returnSpeed;
			}
			set
			{
				value = MathTools.Clamp(value, 0f, 20f);
				if (_returnSpeed != value)
				{
					_returnSpeed = value;
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public bool manageRaycasting
		{
			get
			{
				return _manageRaycasting;
			}
			set
			{
				if (_manageRaycasting != value)
				{
					_manageRaycasting = value;
					if (value)
					{
						sTJMBeoePKCqVkrOPlVWZGnTIVjB();
					}
					else
					{
						_imageRaycastHelper.BYQOPWTaxsiwkGnKZTnrGCxUQYTA();
					}
					GVYKMShboUeUKeoSXOyDKQsdAjHGb();
				}
			}
		}

		public AxisCalibration horizontalAxisCalibration => _axis2D.xAxis.calibration;

		public AxisCalibration verticalAxisCalibration => _axis2D.yAxis.calibration;

		[Obsolete("Use axis2DCalibration instead.", false)]
		public Axis2DCalibration deadZoneType => _axis2D.calibration;

		public Axis2DCalibration axis2DCalibration => _axis2D.calibration;

		public int pointerId
		{
			get
			{
				return _pointerId;
			}
			set
			{
				_pointerId = value;
			}
		}

		public bool hasPointer => _pointerId != int.MinValue;

		private bool ibiDdAqpJHfONBSxwDXxeTLTULUNA => _lastTapFrame == Time.frameCount;

		internal StandaloneAxis2D ZctFieEAMGtKBOPtVDbdVLsfWxQD => _axis2D;

		private Action<iSozMBzlajItGvDiANwnYoMvLVNg> bXlUixqZQNEuPtPAbXmbtlyAdYur
		{
			get
			{
				if (__moveStartedDelegate == null)
				{
					return __moveStartedDelegate = tswTzkSkVgfCWMNHWNurbCedmLXx;
				}
				return __moveStartedDelegate;
			}
		}

		private Action<iSozMBzlajItGvDiANwnYoMvLVNg> ykYlHqntZxSpPfxdhhELSyvzcPot
		{
			get
			{
				if (__moveEndedDelegate == null)
				{
					return __moveEndedDelegate = xbmKdGUmDuWveHVyPFrkfpJvMSYM;
				}
				return __moveEndedDelegate;
			}
		}

		private int uamTWBewfIRksDnRXKnPHNNJaEuU
		{
			get
			{
				if (_pointerId == int.MinValue)
				{
					return int.MinValue;
				}
				if (_realMousePointerId != int.MinValue)
				{
					return _realMousePointerId;
				}
				return _pointerId;
			}
		}

		private RectTransform fzhxVoGNCdjqoBvpjALxDaFHbKPjA
		{
			get
			{
				if (_lastClaimSource != vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion)
				{
					return base.transform as RectTransform;
				}
				return base.transform.parent as RectTransform;
			}
		}

		private float BwcSQQclGkpGUmjewHpABHCagfUtA
		{
			get
			{
				if (Time.frameCount == _calculatedStickRange_lastUpdatedFrame)
				{
					return __calculatedStickRange_cachedValue;
				}
				RectTransform rectTransform = base.fgIqcgpLYsnGBSCwzxuZaMteMDDO;
				RectTransform rectTransform2 = fzhxVoGNCdjqoBvpjALxDaFHbKPjA;
				Vector3 position = new Vector3(0f, _stickRange, 0f);
				Vector3 vector = rectTransform.TransformPoint(position) - rectTransform.position;
				Vector3 a = rectTransform2.InverseTransformPoint(vector + rectTransform2.position);
				float magnitude;
				if (_scaleStickRange)
				{
					Vector3 lossyScale = rectTransform.lossyScale;
					Vector3 lossyScale2 = rectTransform2.lossyScale;
					if (lossyScale.x != 0f)
					{
						lossyScale2.x /= lossyScale.x;
					}
					if (lossyScale.y != 0f)
					{
						lossyScale2.y /= lossyScale.y;
					}
					if (lossyScale.z != 0f)
					{
						lossyScale2.z /= lossyScale.z;
					}
					if (_lastClaimSource == vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion)
					{
						lossyScale2.Scale(base.transform.localScale);
					}
					magnitude = Vector3.Scale(a, lossyScale2).magnitude;
				}
				else
				{
					magnitude = a.magnitude;
				}
				__calculatedStickRange_cachedValue = magnitude;
				_calculatedStickRange_lastUpdatedFrame = Time.frameCount;
				return magnitude;
			}
		}

		internal static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IValueChangedHandler, Vector2> yqrftsMNsZhVAdKQnxOhQBGZppKW
		{
			get
			{
				if (__valueChangedHandlerDelegate == null)
				{
					__valueChangedHandlerDelegate = YTlAbsMTWdzGcUbPdpYNGOdMPpLL._003C_003E9.UYBWtElodgNwmopPnZtOGztQNrM;
				}
				return __valueChangedHandlerDelegate;
			}
		}

		internal static YVQfKeazIqgFlQlVhbEkyBsluncmA.EventFunction<IStickPositionChangedHandler, Vector2> hYIrbknlFFAPFwlPjrwaDBaojlzu
		{
			get
			{
				if (__stickPositionChangedHandlerDelegate == null)
				{
					__stickPositionChangedHandlerDelegate = YTlAbsMTWdzGcUbPdpYNGOdMPpLL._003C_003E9.qSRjHOrTXdfrmdBmwAAxToTAAKcr;
				}
				return __stickPositionChangedHandlerDelegate;
			}
		}

		public event UnityAction<Vector2> ValueChangedEvent
		{
			add
			{
				_onValueChanged.AddListener(value);
			}
			remove
			{
				_onValueChanged.RemoveListener(value);
			}
		}

		public event UnityAction<Vector2> StickPositionChangedEvent
		{
			add
			{
				_onStickPositionChanged.AddListener(value);
			}
			remove
			{
				_onStickPositionChanged.RemoveListener(value);
			}
		}

		public event UnityAction TouchDownEvent
		{
			add
			{
				_onTouchStarted.AddListener(value);
			}
			remove
			{
				_onTouchStarted.RemoveListener(value);
			}
		}

		public event UnityAction TouchUpEvent
		{
			add
			{
				_onTouchEnded.AddListener(value);
			}
			remove
			{
				_onTouchEnded.RemoveListener(value);
			}
		}

		public event UnityAction TapEvent
		{
			add
			{
				_onTap.AddListener(value);
			}
			remove
			{
				_onTap.RemoveListener(value);
			}
		}

		[CustomObfuscation(rename = false)]
		private TouchJoystick()
		{
		}

		public Vector2 GetValue()
		{
			if (!base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				return _axis2D.rawZero;
			}
			return _axis2D.value;
		}

		public Vector2 GetRawValue()
		{
			if (!base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				return _axis2D.rawZero;
			}
			return _axis2D.rawValue;
		}

		public void SetRawValue(Vector2 value)
		{
			if (!base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				return;
			}
			if (_joystickMode == JoystickMode.Digital)
			{
				if (value.sqrMagnitude <= _digitalModeDeadZone * _digitalModeDeadZone)
				{
					value.x = 0f;
					value.y = 0f;
				}
				else
				{
					value.Normalize();
				}
			}
			if (_snapDirections != SnapDirections.None)
			{
				value = MathTools.SnapVectorToNearestAngle(value, 360f / (float)_snapDirections);
				if (value.x != 0f)
				{
					if (MathTools.IsNearZero(value.x, 0.0001f))
					{
						value.x = 0f;
					}
					else if (MathTools.IsNear(value.x, 1f, 0.0001f))
					{
						value.x = 1f;
					}
					else if (MathTools.IsNear(value.x, -1f, 0.0001f))
					{
						value.x = -1f;
					}
				}
				if (value.y != 0f)
				{
					if (MathTools.IsNearZero(value.y, 0.0001f))
					{
						value.y = 0f;
					}
					else if (MathTools.IsNear(value.y, 1f, 0.0001f))
					{
						value.y = 1f;
					}
					else if (MathTools.IsNear(value.y, -1f, 0.0001f))
					{
						value.y = -1f;
					}
				}
			}
			if (_useXAxis || _useYAxis)
			{
				_axis2D.SetRawValue(_useXAxis ? value.x : 0f, _useYAxis ? value.y : 0f);
			}
		}

		public void SetDefaultPosition()
		{
			TWzcPTCxqopKHLTUDVgykfWoVVrl(base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.anchoredPosition);
		}

		private void TWzcPTCxqopKHLTUDVgykfWoVVrl(Vector2 P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				_origAnchoredPosition = P_0;
			}
		}

		public void ReturnToDefaultPosition(bool instant)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				XonazMwdzQcpDvGJMfcxqJFCpwDR(_origAnchoredPosition, PositionType.Anchored, !instant && _animateOnReturn, _returnSpeed, iSozMBzlajItGvDiANwnYoMvLVNg.TowardHome);
			}
		}

		public void ReturnToDefaultPosition()
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				ReturnToDefaultPosition(instant: false);
			}
		}

		[CustomObfuscation(rename = false)]
		internal override void Awake()
		{
			base.Awake();
			if (Application.isPlaying)
			{
				_origAnchoredPosition = base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.anchoredPosition;
				if (_stickTransform != null)
				{
					_origStickAnchoredPosition = _stickTransform.anchoredPosition;
				}
				SetRawValue(ZctFieEAMGtKBOPtVDbdVLsfWxQD.rawZero);
			}
		}

		[CustomObfuscation(rename = false)]
		internal override void OnEnable()
		{
			base.OnEnable();
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				QwsgTmymTHCRICauPNARBbGhtlUxA();
			}
		}

		[CustomObfuscation(rename = false)]
		internal override void OnDisable()
		{
			base.OnDisable();
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				_axis2D.Deinitialize();
				pUDwhrfBtvoGUyjyVCwDYOFnTkUL();
			}
		}

		[CustomObfuscation(rename = false)]
		internal override void OnValidate()
		{
			base.OnValidate();
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				AqyddMigpYGaoVhAtYsWltHgHDBG();
				QwsgTmymTHCRICauPNARBbGhtlUxA();
			}
		}

		internal void CIOlaqlbtyBuyJCaqcqiSalPKcwP()
		{
			base.HDehBiVJQHtZseCWJjHvsFnOvLVX();
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				TiyiOknOcKothtVbtIdYPkXselEeA();
				OijgQHivQARArKrgUliJhBWhyRvrB();
				hQctSanBPzGEURcoYqDsimYpRynA();
			}
		}

		internal bool XoMoDTsYoJOafpcvAudjaztBJTYQ()
		{
			if (!eXuLZbsoevtAdlpQPYDImaDSfOHz())
			{
				return false;
			}
			AqyddMigpYGaoVhAtYsWltHgHDBG();
			_axis2D.Initialize();
			return true;
		}

		internal void xRjBlHGziSWnQHCXOlerdknvfodCA()
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && tCmDnaqSHMayjAbLSbCCIFMNrLegA)
			{
				Vector2 value = _axis2D.value;
				if (_useXAxis)
				{
					TcUpHSLRYsxYwaeotAfbcWODWZmj(_horizontalAxisCustomControllerElement, value.x, _axis2D.xAxis.buttonActivationThreshold);
				}
				if (_useYAxis)
				{
					TcUpHSLRYsxYwaeotAfbcWODWZmj(_verticalAxisCustomControllerElement, value.y, _axis2D.yAxis.buttonActivationThreshold);
				}
				if (_allowTap)
				{
					nEikJoTdlVAdXDkjoknMQxEqTHKfA(_tapCustomControllerElement, ibiDdAqpJHfONBSxwDXxeTLTULUNA);
				}
			}
		}

		internal void bazlZMFtZSUPbDQDMxJHwCYrQWvj()
		{
			ZRtPdJfFFlFAhGTFpzpDpLmfnXmn();
			_axis2D.ValueChangedEvent += nMQftlyndmBTJBWGihjUpxHmdQAeA;
		}

		internal void lizBUnFCwgPMbbLWnECbbgrzVJNp()
		{
			ePmTlETkzdjAnRGaNWOhsSwHQyI();
			_axis2D.ValueChangedEvent -= nMQftlyndmBTJBWGihjUpxHmdQAeA;
		}

		internal void XUMBSZJVPinzgUlGMpaExzuKcYWX()
		{
			UrxvCbTTyrcwzAlwHnkPcfjwZqaC();
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				AqyddMigpYGaoVhAtYsWltHgHDBG();
				QwsgTmymTHCRICauPNARBbGhtlUxA();
			}
		}

		internal void TbkQovIKwwrkbtNHUHLyTmMeAprm()
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				_pointerId = int.MinValue;
				_realMousePointerId = int.MinValue;
				YJYDYHWoCYyOHUKCZjPpKJHEtHeg = false;
				wmyitCoQJvGHRkGBIlggqPNNGZoNA = false;
				_pointerDownIsFake = false;
				_lastPressAnchoredPosition = Vector2.zero;
				_lastPressStartingValue = Vector2.zero;
				_calculatedStickRange_lastUpdatedFrame = -1;
				_lastTapFrame = -1;
				_isEligibleForTap = false;
				if (_returnOnRelease && _isMovedFromDefaultPosition && (_moveToTouchPosition || _followTouchPosition))
				{
					ReturnToDefaultPosition(instant: true);
				}
				_isMovedFromDefaultPosition = false;
				_isMoving = false;
				_moveDirection = iSozMBzlajItGvDiANwnYoMvLVNg.None;
				pteXXiJXsRvTwNKnIWZBUeRHGWYg();
				_axis2D.Clear();
				QwsgTmymTHCRICauPNARBbGhtlUxA();
			}
		}

		internal void jVfcJrYqXTOFmobjlEFDycJYrUzm()
		{
			VPtGcdIWFwGqODRIGeBhZDUxQRBJA();
			if (_hierarchyValueChangedHandlers == null)
			{
				_hierarchyValueChangedHandlers = new YVQfKeazIqgFlQlVhbEkyBsluncmA.HierarchyEventHelper<IValueChangedHandler, Vector2>(yqrftsMNsZhVAdKQnxOhQBGZppKW);
			}
			_hierarchyValueChangedHandlers.GetHandlers(base.transform);
			if (_hierarchyStickPositionChangedHandlers == null)
			{
				_hierarchyStickPositionChangedHandlers = new YVQfKeazIqgFlQlVhbEkyBsluncmA.HierarchyEventHelper<IStickPositionChangedHandler, Vector2>(hYIrbknlFFAPFwlPjrwaDBaojlzu);
			}
			_hierarchyStickPositionChangedHandlers.GetHandlers(base.transform);
		}

		public override void ClearValue()
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				_axis2D.Clear();
				_lastTapFrame = -1;
				if (tCmDnaqSHMayjAbLSbCCIFMNrLegA)
				{
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ClearElementValue(_horizontalAxisCustomControllerElement);
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ClearElementValue(_verticalAxisCustomControllerElement);
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ClearElementValue(_tapCustomControllerElement);
				}
			}
		}

		internal bool sXNofwIZimcDziYJXVcYZlsZTRSs()
		{
			if (!base.ufLiLiMyYKjqbRkyhyTivexwTJMJ)
			{
				return false;
			}
			if (!DfQIcSJUPXlHPQKgUHsgOrKCBhBG())
			{
				return false;
			}
			return YJYDYHWoCYyOHUKCZjPpKJHEtHeg;
		}

		internal bool TQxsgZvrqmTHAYEECZmBcElyhBxL(GameObject P_0)
		{
			if (P_0 == null)
			{
				return false;
			}
			if (base.xEwQUZcqFhGubfvZUNKxwzzGTqdF(P_0))
			{
				return true;
			}
			if (_workingTouchRegion != null)
			{
				return _workingTouchRegion.gameObject == P_0;
			}
			return false;
		}

		private void QwsgTmymTHCRICauPNARBbGhtlUxA()
		{
			_horizontalAxisCustomControllerElement.ClearElementCaches();
			_verticalAxisCustomControllerElement.ClearElementCaches();
			_tapCustomControllerElement.ClearElementCaches();
			hQctSanBPzGEURcoYqDsimYpRynA();
			sTJMBeoePKCqVkrOPlVWZGnTIVjB();
		}

		private void sTJMBeoePKCqVkrOPlVWZGnTIVjB()
		{
			if (_manageRaycasting)
			{
				_imageRaycastHelper.ewYNmyWCNHNCHmZClXtNQtKxUcEK(base.transform, OykslnDQPttNkmkntvmecDEbjutr());
			}
		}

		private bool OykslnDQPttNkmkntvmecDEbjutr()
		{
			if (_workingTouchRegion != null && _useTouchRegionOnly)
			{
				return false;
			}
			return true;
		}

		private void wDdxNjPJEGRkYzkcCAzpwZJjLhFO(TouchRegion P_0)
		{
			if (!(P_0 == null))
			{
				HTlxujnQmKgVQvgmwFwIaCiynmok(P_0);
				P_0.PointerDownEvent += sEUaPaEWenIiwqvPUfoEBddrYGdqA;
				P_0.PointerUpEvent += GEgamwAOgurNxBxJObqdGoKOoKDUA;
				P_0.PointerEnterEvent += GrPubzjcWgOgmvmygydDrEccXmIv;
				P_0.PointerExitEvent += RqwBhutDotykgJTgGbEQPTMzLZyP;
				P_0.BeginDragEvent += IBxNOArwdDUcBwyyQjmSSiTmgYGg;
				P_0.DragEvent += BpbtHPHLMhAPmPimfCoLBeYnFScgb;
				P_0.EndDragEvent += XZFXWeGiswOrJNifBEMgxfnZHEZy;
			}
		}

		private void HTlxujnQmKgVQvgmwFwIaCiynmok(TouchRegion P_0)
		{
			if (!(P_0 == null))
			{
				P_0.PointerDownEvent -= sEUaPaEWenIiwqvPUfoEBddrYGdqA;
				P_0.PointerUpEvent -= GEgamwAOgurNxBxJObqdGoKOoKDUA;
				P_0.PointerEnterEvent -= GrPubzjcWgOgmvmygydDrEccXmIv;
				P_0.PointerExitEvent -= RqwBhutDotykgJTgGbEQPTMzLZyP;
				P_0.BeginDragEvent -= IBxNOArwdDUcBwyyQjmSSiTmgYGg;
				P_0.DragEvent -= BpbtHPHLMhAPmPimfCoLBeYnFScgb;
				P_0.EndDragEvent -= XZFXWeGiswOrJNifBEMgxfnZHEZy;
			}
		}

		private void hQctSanBPzGEURcoYqDsimYpRynA()
		{
			if (!(_workingTouchRegion == _touchRegion))
			{
				HTlxujnQmKgVQvgmwFwIaCiynmok(_workingTouchRegion);
				_workingTouchRegion = _touchRegion;
				wDdxNjPJEGRkYzkcCAzpwZJjLhFO(_workingTouchRegion);
			}
		}

		private void sveLwIkQtyFMoGdjLoAFmCoLBmEy(Vector2 P_0, bool P_1, float P_2, iSozMBzlajItGvDiANwnYoMvLVNg P_3)
		{
			RectTransform rectTransform = base.transform.parent as RectTransform;
			Vector2 vector = DqaChzdTZYNFWhBQIpZWKgtwfiCfA.RPNHKzNFQfDoKApVrjsOYFxIcBjbb(base.krwSPbnrBGEzhAtvrnWbhxuaaPWOA, rectTransform, P_0);
			Vector2 pivot = base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.pivot;
			Vector2 sizeDelta = base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.sizeDelta;
			Vector3 localScale = base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.localScale;
			vector += new Vector2((pivot.x - 0.5f) * sizeDelta.x * localScale.x, (pivot.y - 0.5f) * sizeDelta.y * localScale.y);
			XonazMwdzQcpDvGJMfcxqJFCpwDR(vector, PositionType.Local, P_1, P_2, P_3);
		}

		private void XonazMwdzQcpDvGJMfcxqJFCpwDR(Vector2 P_0, PositionType P_1, bool P_2, float P_3, iSozMBzlajItGvDiANwnYoMvLVNg P_4)
		{
			if (_isMoving && P_2 && _moveDirection == P_4)
			{
				return;
			}
			if (_isMoving && _coroutineMove != null)
			{
				pteXXiJXsRvTwNKnIWZBUeRHGWYg();
				_isMoving = false;
				_moveDirection = iSozMBzlajItGvDiANwnYoMvLVNg.None;
			}
			if (base.krwSPbnrBGEzhAtvrnWbhxuaaPWOA == null)
			{
				Logger.LogWarning("Animation cannot be used without a Canvas.");
				P_2 = false;
			}
			else if (base.krwSPbnrBGEzhAtvrnWbhxuaaPWOA.renderMode == RenderMode.WorldSpace)
			{
				Logger.LogWarning("Animation can only be used with a screen space Canvas.");
				P_2 = false;
			}
			if (P_2)
			{
				Transform parent = base.transform;
				RectTransform rectTransform = base.fgIqcgpLYsnGBSCwzxuZaMteMDDO;
				Vector2 one = Vector2.one;
				while ((parent = parent.parent) != rectTransform && !(parent == null))
				{
					one.x *= parent.localScale.x;
					one.y *= parent.localScale.y;
				}
				Vector2 sizeDelta = rectTransform.sizeDelta;
				bool num = sizeDelta.x < sizeDelta.y;
				float num2 = MathTools.Max(sizeDelta.x, sizeDelta.y);
				float num3 = (num ? one.y : one.x);
				if (num3 == 0f)
				{
					num3 = 0.0001f;
				}
				P_3 = P_3 / num3 * num2;
				_coroutineMove = JPPBUIcVTMEpvrtvelTKWaRqsqsRA(P_0, P_1, P_3, P_4);
				StartCoroutine(_coroutineMove);
				_moveDirection = P_4;
				_isMovedFromDefaultPosition = true;
				bXlUixqZQNEuPtPAbXmbtlyAdYur(P_4);
			}
			else
			{
				bXlUixqZQNEuPtPAbXmbtlyAdYur(P_4);
				TjANlvPJMKbQcwfLaQdRlvzJDifr(P_4, P_0, P_1);
			}
		}

		[IteratorStateMachine(typeof(knBUZUtTMoCMUbaEDAxPLUGCIljx))]
		private IEnumerator JPPBUIcVTMEpvrtvelTKWaRqsqsRA(Vector2 P_0, PositionType P_1, float P_2, iSozMBzlajItGvDiANwnYoMvLVNg P_3)
		{
			return new knBUZUtTMoCMUbaEDAxPLUGCIljx(0)
			{
				UNcNCAbCvJTIZjRKydgHUVmClDKI = this,
				fQBRvKskjvqHbhsrJbwiRKSDRWqA = P_0,
				ZfgjAiHovsLbakSkybiZzFJMHjoy = P_1,
				qcXyyUBecFXIuZKLEkNsSKRPUNEX = P_2,
				begpbGefnpznTJqoEPKnXZDycUEc = P_3
			};
		}

		private void TjANlvPJMKbQcwfLaQdRlvzJDifr(iSozMBzlajItGvDiANwnYoMvLVNg P_0, Vector2 P_1, PositionType P_2)
		{
			DqaChzdTZYNFWhBQIpZWKgtwfiCfA.BsdJYCzGuBKzUbylUTSvUyoLgxek(base.KXbFLgsvCYMSvuHjRTfknYKDbMAM, P_1, P_2);
			_isMoving = false;
			_moveDirection = iSozMBzlajItGvDiANwnYoMvLVNg.None;
			switch (P_0)
			{
			case iSozMBzlajItGvDiANwnYoMvLVNg.TowardHome:
				_isMovedFromDefaultPosition = false;
				break;
			case iSozMBzlajItGvDiANwnYoMvLVNg.TowardTouch:
				_isMovedFromDefaultPosition = true;
				break;
			}
			pteXXiJXsRvTwNKnIWZBUeRHGWYg();
			ykYlHqntZxSpPfxdhhELSyvzcPot(P_0);
		}

		private void tswTzkSkVgfCWMNHWNurbCedmLXx(iSozMBzlajItGvDiANwnYoMvLVNg P_0)
		{
			if (_manageRaycasting)
			{
				bool flag = false;
				bool flag2 = false;
				if (((_followTouchPosition && stayActiveOnSwipeOut) || (!_followTouchPosition && _workingTouchRegion != null && !_useTouchRegionOnly && _moveToTouchPosition)) && _returnOnRelease && P_0 == iSozMBzlajItGvDiANwnYoMvLVNg.TowardTouch)
				{
					flag = true;
					flag2 = false;
				}
				if (flag)
				{
					_imageRaycastHelper.ewYNmyWCNHNCHmZClXtNQtKxUcEK(base.transform, flag2);
				}
			}
		}

		private void xbmKdGUmDuWveHVyPFrkfpJvMSYM(iSozMBzlajItGvDiANwnYoMvLVNg P_0)
		{
			if (_manageRaycasting)
			{
				bool flag = false;
				bool flag2 = false;
				if (((_followTouchPosition && stayActiveOnSwipeOut) || (!_followTouchPosition && _workingTouchRegion != null && !_useTouchRegionOnly && _moveToTouchPosition)) && _returnOnRelease && P_0 == iSozMBzlajItGvDiANwnYoMvLVNg.TowardHome)
				{
					flag = true;
					flag2 = OykslnDQPttNkmkntvmecDEbjutr();
				}
				if (flag)
				{
					_imageRaycastHelper.ewYNmyWCNHNCHmZClXtNQtKxUcEK(base.transform, flag2);
				}
			}
		}

		private void pteXXiJXsRvTwNKnIWZBUeRHGWYg()
		{
			if (_coroutineMove != null)
			{
				try
				{
					StopCoroutine(_coroutineMove);
				}
				catch
				{
				}
				_coroutineMove = null;
			}
		}

		private void fubbXfvkGvWNTcteYbaojkYjaUhb(int P_0, Vector2 P_1, PositionType P_2)
		{
			if (TouchInteractable.ZYwGYYdmSzHakANXcUYVgtqvzOUJ(P_0))
			{
				XonazMwdzQcpDvGJMfcxqJFCpwDR((Vector2)DqaChzdTZYNFWhBQIpZWKgtwfiCfA.CyKeeehKagdHzEOJhkdvTCWQzUMsb(base.KXbFLgsvCYMSvuHjRTfknYKDbMAM, P_2) + P_1, P_2, false, 0f, iSozMBzlajItGvDiANwnYoMvLVNg.TowardTouch);
				if (_lastClaimSource == vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion)
				{
					_lastPressAnchoredPosition += P_1;
				}
			}
		}

		private void OijgQHivQARArKrgUliJhBWhyRvrB()
		{
			if (!hasPointer)
			{
				return;
			}
			if (!TouchInteractable.ZYwGYYdmSzHakANXcUYVgtqvzOUJ(uamTWBewfIRksDnRXKnPHNNJaEuU))
			{
				PointerEventData pointerEventData = WeFfvhbvdCHWBkHlWxvIlTExcRPl(uamTWBewfIRksDnRXKnPHNNJaEuU);
				if (pointerEventData != null && pointerEventData.pointerPress != null)
				{
					ivQKKZEIcgdAaNynuyWFKuqCznDe(pointerEventData);
				}
				else
				{
					dABbqoBQcJkljiVsglDdENXiUJuGB();
				}
			}
			else if (_pointerDownIsFake)
			{
				PointerEventData pointerEventData2 = DOALcGKQFIlIdZFWnfpILTUjvEEV(uamTWBewfIRksDnRXKnPHNNJaEuU, (_workingTouchRegion != null && _useTouchRegionOnly) ? _workingTouchRegion.gameObject : ((_stickTransform != null) ? _stickTransform.gameObject : base.gameObject));
				if (pointerEventData2 != null)
				{
					qpGyREfiPFEhRPspuxjgmaNpkxJQ(pointerEventData2, _lastClaimSource);
				}
			}
		}

		private void TiyiOknOcKothtVbtIdYPkXselEeA()
		{
			if (hasPointer)
			{
				Vector2 vector = TouchInteractable.SlvklMDRFubBrxxwogKeezCPTKAI(uamTWBewfIRksDnRXKnPHNNJaEuU);
				rGDSAxzgynPABtCPxVfofBCTLKpN(ref vector);
			}
		}

		private void rGDSAxzgynPABtCPxVfofBCTLKpN(ref Vector2 P_0)
		{
			if (_allowTap && _isEligibleForTap && ((_tapTimeout > 0f && Time.realtimeSinceStartup - _touchStartTime > _tapTimeout) || (_tapDistanceLimit >= 0 && Vector2.Distance(_touchStartPosition, P_0) > (float)_tapDistanceLimit)))
			{
				_isEligibleForTap = false;
			}
		}

		private bool TvZIKxifGZnpXiSxsAUvjInjUZzh()
		{
			if (!_followTouchPosition)
			{
				return false;
			}
			if (_touchRegion != null && _useTouchRegionOnly)
			{
				return false;
			}
			return true;
		}

		private void nCjpEWudBTiNisUhnPDsewqksViB()
		{
			_pointerId = int.MinValue;
			_realMousePointerId = int.MinValue;
			_lastClaimSource = vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local;
		}

		private bool yYyDmNcJYeSWmcuvzSRTgJcqvHMM(int P_0)
		{
			if (P_0 == int.MinValue)
			{
				return false;
			}
			if (_pointerId == int.MinValue)
			{
				return false;
			}
			if (_pointerId == P_0)
			{
				return true;
			}
			if (TouchInteractable.zfMRdxEWpVdbDLsvqJxSDUMtmNpl(P_0) && _realMousePointerId != int.MinValue && P_0 == _realMousePointerId)
			{
				return true;
			}
			return false;
		}

		private PointerEventData YpvSlnqLoVFxlVimbiGiModHPVZP(int P_0, GameObject P_1)
		{
			PointerEventData pointerEventData = WeFfvhbvdCHWBkHlWxvIlTExcRPl(P_0);
			if (pointerEventData == null)
			{
				return null;
			}
			pointerEventData.position = TouchInteractable.SlvklMDRFubBrxxwogKeezCPTKAI(P_0);
			if (TouchInteractable.YOjWGKtwerAoiAXAEISfxlNHfJJw(P_0))
			{
				pointerEventData.eligibleForClick = true;
				pointerEventData.delta = Vector2.zero;
				pointerEventData.dragging = false;
				pointerEventData.useDragThreshold = true;
				pointerEventData.pressPosition = pointerEventData.position;
				pointerEventData.pointerPressRaycast = pointerEventData.pointerCurrentRaycast;
				if (pointerEventData.pointerEnter != P_1)
				{
					pointerEventData.pointerEnter = P_1;
				}
				float unscaledTime = Time.unscaledTime;
				if (P_1 == pointerEventData.lastPress)
				{
					if (unscaledTime - pointerEventData.clickTime < 0.3f)
					{
						int clickCount = pointerEventData.clickCount + 1;
						pointerEventData.clickCount = clickCount;
					}
					else
					{
						pointerEventData.clickCount = 1;
					}
					pointerEventData.clickTime = unscaledTime;
				}
				else
				{
					pointerEventData.clickCount = 1;
				}
				pointerEventData.pointerPress = P_1;
				pointerEventData.rawPointerPress = P_1;
				pointerEventData.clickTime = unscaledTime;
				pointerEventData.pointerDrag = P_1;
			}
			else
			{
				if (!TouchInteractable.zfMRdxEWpVdbDLsvqJxSDUMtmNpl(P_0))
				{
					Logger.LogWarning("Unsupported pointerId: " + P_0);
					return null;
				}
				pointerEventData.eligibleForClick = true;
				pointerEventData.delta = Vector2.zero;
				pointerEventData.dragging = false;
				pointerEventData.useDragThreshold = true;
				pointerEventData.pressPosition = pointerEventData.position;
				pointerEventData.pointerPressRaycast = pointerEventData.pointerCurrentRaycast;
				float unscaledTime2 = Time.unscaledTime;
				if (P_1 == pointerEventData.lastPress)
				{
					if (unscaledTime2 - pointerEventData.clickTime < 0.3f)
					{
						int clickCount = pointerEventData.clickCount + 1;
						pointerEventData.clickCount = clickCount;
					}
					else
					{
						pointerEventData.clickCount = 1;
					}
					pointerEventData.clickTime = unscaledTime2;
				}
				else
				{
					pointerEventData.clickCount = 1;
				}
				pointerEventData.pointerPress = P_1;
				pointerEventData.rawPointerPress = P_1;
				pointerEventData.clickTime = unscaledTime2;
				pointerEventData.pointerDrag = P_1;
			}
			return pointerEventData;
		}

		private PointerEventData DOALcGKQFIlIdZFWnfpILTUjvEEV(int P_0, GameObject P_1)
		{
			PointerEventData pointerEventData = WeFfvhbvdCHWBkHlWxvIlTExcRPl(P_0);
			if (pointerEventData == null)
			{
				return null;
			}
			Vector2 vector = TouchInteractable.SlvklMDRFubBrxxwogKeezCPTKAI(P_0);
			pointerEventData.delta = vector - pointerEventData.position;
			pointerEventData.position = vector;
			pointerEventData.dragging = true;
			pointerEventData.pointerDrag = P_1;
			pointerEventData.useDragThreshold = true;
			pointerEventData.pointerPress = null;
			pointerEventData.rawPointerPress = null;
			return pointerEventData;
		}

		private PointerEventData WapSPvxFuriRIPTbPzswOezHbAUA(int P_0)
		{
			PointerEventData pointerEventData = WeFfvhbvdCHWBkHlWxvIlTExcRPl(P_0);
			if (pointerEventData == null)
			{
				return null;
			}
			if (TouchInteractable.YOjWGKtwerAoiAXAEISfxlNHfJJw(P_0))
			{
				pointerEventData.eligibleForClick = false;
				pointerEventData.pointerPress = null;
				pointerEventData.rawPointerPress = null;
				pointerEventData.dragging = false;
				pointerEventData.pointerDrag = null;
				pointerEventData.pointerEnter = null;
			}
			else
			{
				if (!TouchInteractable.zfMRdxEWpVdbDLsvqJxSDUMtmNpl(P_0))
				{
					Logger.LogWarning("Unsupported pointerId: " + P_0);
					return null;
				}
				pointerEventData.eligibleForClick = false;
				pointerEventData.pointerPress = null;
				pointerEventData.rawPointerPress = null;
				pointerEventData.dragging = false;
				pointerEventData.pointerDrag = null;
			}
			return pointerEventData;
		}

		private void ivQKKZEIcgdAaNynuyWFKuqCznDe(PointerEventData P_0)
		{
			if (P_0 != null)
			{
				OnPointerUp(P_0);
				WapSPvxFuriRIPTbPzswOezHbAUA(uamTWBewfIRksDnRXKnPHNNJaEuU);
			}
		}

		private void qpGyREfiPFEhRPspuxjgmaNpkxJQ(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (P_0 != null)
			{
				switch (P_1)
				{
				case vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local:
					OnDrag(P_0);
					break;
				case vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion:
					BpbtHPHLMhAPmPimfCoLBeYnFScgb(P_0);
					break;
				default:
					throw new NotImplementedException();
				}
				WapSPvxFuriRIPTbPzswOezHbAUA(uamTWBewfIRksDnRXKnPHNNJaEuU);
			}
		}

		private PointerEventData WeFfvhbvdCHWBkHlWxvIlTExcRPl(int P_0)
		{
			if (P_0 == int.MinValue)
			{
				return null;
			}
			if (__fakePointerEventData == null)
			{
				__fakePointerEventData = new Dictionary<int, PointerEventData>();
			}
			if (!__fakePointerEventData.TryGetValue(P_0, out var value))
			{
				value = new PointerEventData(EventSystem.current);
				value.pointerId = P_0;
				__fakePointerEventData.Add(P_0, value);
				if (TouchInteractable.zfMRdxEWpVdbDLsvqJxSDUMtmNpl(P_0))
				{
					PointerEventData.InputButton button = P_0 switch
					{
						-1 => PointerEventData.InputButton.Left, 
						-2 => PointerEventData.InputButton.Right, 
						-3 => PointerEventData.InputButton.Middle, 
						_ => throw new NotImplementedException(), 
					};
					value.button = button;
				}
			}
			return value;
		}

		private void AqyddMigpYGaoVhAtYsWltHgHDBG()
		{
			qwzZgYRNQzLlANWmcEynJnDggpWt(_axesToUse);
			if (tCmDnaqSHMayjAbLSbCCIFMNrLegA && base.orQDXwdaVInPntlmeUOqFqpfOPBT.useCustomController)
			{
				if (_useXAxis)
				{
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ValidateElements(_horizontalAxisCustomControllerElement);
				}
				if (_useYAxis)
				{
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ValidateElements(_verticalAxisCustomControllerElement);
				}
				if (_allowTap)
				{
					base.TuEiudZyOALZvXpibFyWSoJvoXul.ValidateElements(_tapCustomControllerElement);
				}
			}
		}

		private void qwzZgYRNQzLlANWmcEynJnDggpWt(AxisDirection P_0)
		{
			bool flag = P_0 == AxisDirection.Both || P_0 == AxisDirection.Horizontal;
			if (_useXAxis != flag)
			{
				_useXAxis = flag;
				if (!flag && tCmDnaqSHMayjAbLSbCCIFMNrLegA)
				{
					int targetCount = _horizontalAxisCustomControllerElement.targetCount;
					for (int i = 0; i < targetCount; i++)
					{
						base.TuEiudZyOALZvXpibFyWSoJvoXul.ClearElementValue(_horizontalAxisCustomControllerElement[i]);
					}
				}
			}
			bool flag2 = P_0 == AxisDirection.Both || P_0 == AxisDirection.Vertical;
			if (_useYAxis != flag2)
			{
				_useYAxis = flag2;
				if (!flag2 && tCmDnaqSHMayjAbLSbCCIFMNrLegA)
				{
					int targetCount2 = _verticalAxisCustomControllerElement.targetCount;
					for (int j = 0; j < targetCount2; j++)
					{
						base.TuEiudZyOALZvXpibFyWSoJvoXul.ClearElementValue(_verticalAxisCustomControllerElement[j]);
					}
				}
			}
			_axesToUse = P_0;
		}

		private void zXtoIVtMyhdcIFsGezStHEZlaiyK(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (!hasPointer || yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				if (DfQIcSJUPXlHPQKgUHsgOrKCBhBG() && IsInteractable())
				{
					TUPsCeSUxBayDtflSTtfHkPoKprr(P_0.pointerId, P_0.pressPosition, P_1);
				}
				base.OnPointerDown(P_0);
			}
		}

		private void OyCtZyQXzBEKAHdnhDbbjQzXeDTK(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if ((!hasPointer || yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId)) && !TouchInteractable.ZYwGYYdmSzHakANXcUYVgtqvzOUJ(uamTWBewfIRksDnRXKnPHNNJaEuU))
			{
				dABbqoBQcJkljiVsglDdENXiUJuGB();
				base.OnPointerUp(P_0);
			}
		}

		private void phBBZhcZbjjLUSaOinGbhdCPhZyZ(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (hasPointer && !yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				return;
			}
			bool flag = TouchInteractable.zfMRdxEWpVdbDLsvqJxSDUMtmNpl(P_0.pointerId);
			bool flag2 = false;
			MouseButtonFlags mouseButtonFlags = P_1 switch
			{
				vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local => base.allowedMouseButtons, 
				vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion => _touchRegion.allowedMouseButtons, 
				_ => throw new NotImplementedException(), 
			};
			if (_activateOnSwipeIn && DfQIcSJUPXlHPQKgUHsgOrKCBhBG() && IsInteractable() && (!flag || TouchInteractable.bvaavvcdoRyDQyElLKVutjdctzVO(mouseButtonFlags)) && !YJYDYHWoCYyOHUKCZjPpKJHEtHeg)
			{
				if (flag)
				{
					if (TouchInteractable.wosOqXdFSlBXaFBgRvyXXgngpsXH(mouseButtonFlags, out var realMousePointerId))
					{
						_realMousePointerId = realMousePointerId;
					}
					else
					{
						_realMousePointerId = P_0.pointerId;
					}
				}
				flag2 = true;
			}
			base.OnPointerEnter(P_0);
			if (flag2)
			{
				GameObject gameObject = P_1 switch
				{
					vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local => base.gameObject, 
					vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion => _workingTouchRegion.gameObject, 
					_ => throw new NotImplementedException(), 
				};
				PointerEventData pointerEventData = YpvSlnqLoVFxlVimbiGiModHPVZP((_realMousePointerId != int.MinValue) ? _realMousePointerId : P_0.pointerId, gameObject);
				if (pointerEventData != null)
				{
					zXtoIVtMyhdcIFsGezStHEZlaiyK(pointerEventData, P_1);
					if (YJYDYHWoCYyOHUKCZjPpKJHEtHeg)
					{
						_pointerDownIsFake = true;
					}
				}
			}
			wmyitCoQJvGHRkGBIlggqPNNGZoNA = true;
		}

		private void QkYmDBZJyjNEaNJHSHaCFjSvQTTB(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (hasPointer && !yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				base.OnPointerExit(P_0);
				return;
			}
			if (!stayActiveOnSwipeOut && YJYDYHWoCYyOHUKCZjPpKJHEtHeg)
			{
				dABbqoBQcJkljiVsglDdENXiUJuGB();
			}
			base.OnPointerExit(P_0);
			wmyitCoQJvGHRkGBIlggqPNNGZoNA = false;
		}

		private void VVknYVsMwlkxgRpLqbscXAtsMkWI(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (hasPointer && yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				base.OnBeginDrag(P_0);
			}
		}

		private void zCxRXjrDuHnBSlxGEDgIXXzpOUuq(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (!hasPointer || !yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				return;
			}
			RectTransform rectTransform = fzhxVoGNCdjqoBvpjALxDaFHbKPjA;
			Vector2 vector = ((!_snapStickToTouch) ? _lastPressAnchoredPosition : DqaChzdTZYNFWhBQIpZWKgtwfiCfA.AvPocvELotzQePIgscOyeDgUIPTq(base.KXbFLgsvCYMSvuHjRTfknYKDbMAM, rectTransform, base.KXbFLgsvCYMSvuHjRTfknYKDbMAM.rect.center));
			if (!_centerStickOnRelease && !_snapStickToTouch)
			{
				vector -= _lastPressStartingValue * BwcSQQclGkpGUmjewHpABHCagfUtA;
			}
			Vector2 vector2 = DqaChzdTZYNFWhBQIpZWKgtwfiCfA.wSXCxUFXvQllwGFYubqcWiBsRlER(base.krwSPbnrBGEzhAtvrnWbhxuaaPWOA, rectTransform, P_0.position);
			Vector2 vector3 = new Vector2(_useXAxis ? (vector2.x - vector.x) : 0f, _useYAxis ? (vector2.y - vector.y) : 0f);
			Vector2 vector4;
			if (_stickBounds == StickBounds.Circle)
			{
				vector4 = Vector2.ClampMagnitude(vector3, BwcSQQclGkpGUmjewHpABHCagfUtA);
			}
			else
			{
				if (_stickBounds != StickBounds.Square)
				{
					throw new NotImplementedException();
				}
				vector4 = MathTools.Clamp(vector3, 0f - BwcSQQclGkpGUmjewHpABHCagfUtA, BwcSQQclGkpGUmjewHpABHCagfUtA);
			}
			Vector2 rawValue = vector4 / BwcSQQclGkpGUmjewHpABHCagfUtA;
			SetRawValue(rawValue);
			if (_followTouchPosition)
			{
				if (_stickBounds == StickBounds.Circle)
				{
					if (vector3.sqrMagnitude > BwcSQQclGkpGUmjewHpABHCagfUtA)
					{
						Vector2 vector5 = new Vector2(_useXAxis ? (vector3.x - vector4.x) : 0f, _useXAxis ? (vector3.y - vector4.y) : 0f);
						fubbXfvkGvWNTcteYbaojkYjaUhb(uamTWBewfIRksDnRXKnPHNNJaEuU, vector5, PositionType.Anchored);
					}
				}
				else
				{
					if (_stickBounds != StickBounds.Square)
					{
						throw new NotImplementedException();
					}
					bool flag = Mathf.Abs(vector3.x) > BwcSQQclGkpGUmjewHpABHCagfUtA;
					bool flag2 = Mathf.Abs(vector3.y) > BwcSQQclGkpGUmjewHpABHCagfUtA;
					if (flag || flag2)
					{
						Vector2 vector6 = new Vector2((_useXAxis && flag) ? (vector3.x - vector4.x) : 0f, (_useXAxis && flag2) ? (vector3.y - vector4.y) : 0f);
						fubbXfvkGvWNTcteYbaojkYjaUhb(uamTWBewfIRksDnRXKnPHNNJaEuU, vector6, PositionType.Anchored);
					}
				}
			}
			base.OnDrag(P_0);
		}

		private void lkFSpmdBeefLBGYLSwkEGcmTImOBA(PointerEventData P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_1)
		{
			if (hasPointer && yYyDmNcJYeSWmcuvzSRTgJcqvHMM(P_0.pointerId))
			{
				base.OnEndDrag(P_0);
			}
		}

		private void TUPsCeSUxBayDtflSTtfHkPoKprr(int P_0, Vector2 P_1, vfQUXOvXrBtkhFTPfwODYhAVOLcu P_2)
		{
			_pointerId = P_0;
			_lastClaimSource = P_2;
			_isEligibleForTap = true;
			_lastPressAnchoredPosition = DqaChzdTZYNFWhBQIpZWKgtwfiCfA.wSXCxUFXvQllwGFYubqcWiBsRlER(base.krwSPbnrBGEzhAtvrnWbhxuaaPWOA, fzhxVoGNCdjqoBvpjALxDaFHbKPjA, P_1);
			YJYDYHWoCYyOHUKCZjPpKJHEtHeg = true;
			_lastPressStartingValue.x = MathTools.Clamp(_axis2D.value.x, -1f, 1f);
			_lastPressStartingValue.y = MathTools.Clamp(_axis2D.value.y, -1f, 1f);
			_touchStartTime = Time.realtimeSinceStartup;
			_touchStartPosition = P_1;
			if (P_2 == vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion && (_moveToTouchPosition || _followTouchPosition))
			{
				if (_followTouchPosition)
				{
					sveLwIkQtyFMoGdjLoAFmCoLBmEy(P_1, false, 0f, iSozMBzlajItGvDiANwnYoMvLVNg.TowardTouch);
				}
				else
				{
					sveLwIkQtyFMoGdjLoAFmCoLBmEy(P_1, _animateOnMoveToTouch, _moveToTouchSpeed, iSozMBzlajItGvDiANwnYoMvLVNg.TowardTouch);
				}
			}
			if (_onTouchStarted != null)
			{
				_onTouchStarted.Invoke();
			}
			PointerEventData pointerEventData = DOALcGKQFIlIdZFWnfpILTUjvEEV(_pointerId, (P_2 == vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion) ? _workingTouchRegion.gameObject : ((_stickTransform != null) ? _stickTransform.gameObject : base.gameObject));
			if (pointerEventData != null)
			{
				qpGyREfiPFEhRPspuxjgmaNpkxJQ(pointerEventData, P_2);
			}
		}

		private void dABbqoBQcJkljiVsglDdENXiUJuGB()
		{
			nCjpEWudBTiNisUhnPDsewqksViB();
			bool num = _allowTap && _isEligibleForTap;
			YJYDYHWoCYyOHUKCZjPpKJHEtHeg = false;
			_pointerDownIsFake = false;
			_lastPressAnchoredPosition = Vector2.zero;
			_lastPressStartingValue = Vector2.zero;
			if ((_followTouchPosition || _moveToTouchPosition) && _returnOnRelease && _isMovedFromDefaultPosition)
			{
				ReturnToDefaultPosition();
			}
			if (_centerStickOnRelease)
			{
				SetRawValue(_axis2D.rawZero);
			}
			if (_onTouchEnded != null)
			{
				_onTouchEnded.Invoke();
			}
			_isEligibleForTap = false;
			if (num)
			{
				_lastTapFrame = Time.frameCount + 1;
				_onTap.Invoke();
			}
		}

		internal void uYjVMzaeScUZKIazymAPWgclIcXY(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.PointerUp) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				OyCtZyQXzBEKAHdnhDbbjQzXeDTK(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void TTBwKOWjprOLxKnaXnHbRWbYitvr(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.PointerDown) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				zXtoIVtMyhdcIFsGezStHEZlaiyK(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void vgdpDMjPptTPFojQCIDtphiCTYKi(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.PointerEnter) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				phBBZhcZbjjLUSaOinGbhdCPhZyZ(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void JKPemoFgfiZIIMqBgelniDkicwxr(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.PointerExit) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				QkYmDBZJyjNEaNJHSHaCFjSvQTTB(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void XoMOWdlAcQjMcOaaAZApvPYeRQqX(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.BeginDrag) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				VVknYVsMwlkxgRpLqbscXAtsMkWI(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void JRGLbaekjSNmdXqEfEVFAiZkAgPxA(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.Drag) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				zCxRXjrDuHnBSlxGEDgIXXzpOUuq(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		internal void XUUWjqOEYeZzDiNQepEMoRpFFDOe(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, base.allowedMouseButtons, EventTriggerType.EndDrag) && (!(_workingTouchRegion != null) || !_useTouchRegionOnly))
			{
				lkFSpmdBeefLBGYLSwkEGcmTImOBA(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.Local);
			}
		}

		private void sEUaPaEWenIiwqvPUfoEBddrYGdqA(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.PointerDown))
			{
				zXtoIVtMyhdcIFsGezStHEZlaiyK(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void GEgamwAOgurNxBxJObqdGoKOoKDUA(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.PointerUp))
			{
				OyCtZyQXzBEKAHdnhDbbjQzXeDTK(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void GrPubzjcWgOgmvmygydDrEccXmIv(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.PointerEnter))
			{
				phBBZhcZbjjLUSaOinGbhdCPhZyZ(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void RqwBhutDotykgJTgGbEQPTMzLZyP(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.PointerExit))
			{
				QkYmDBZJyjNEaNJHSHaCFjSvQTTB(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void IBxNOArwdDUcBwyyQjmSSiTmgYGg(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.BeginDrag))
			{
				VVknYVsMwlkxgRpLqbscXAtsMkWI(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void BpbtHPHLMhAPmPimfCoLBeYnFScgb(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.Drag))
			{
				zCxRXjrDuHnBSlxGEDgIXXzpOUuq(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void XZFXWeGiswOrJNifBEMgxfnZHEZy(PointerEventData P_0)
		{
			if (base.ufLiLiMyYKjqbRkyhyTivexwTJMJ && TouchInteractable.JGWFphEzfGdSUNCbtbtdWjvDOLzcb(P_0.pointerId, _touchRegion.allowedMouseButtons, EventTriggerType.EndDrag))
			{
				lkFSpmdBeefLBGYLSwkEGcmTImOBA(P_0, vfQUXOvXrBtkhFTPfwODYhAVOLcu.TouchRegion);
			}
		}

		private void nMQftlyndmBTJBWGihjUpxHmdQAeA(Vector2 P_0)
		{
			qTbQveApfHOMSehJBxKYMzaVLcgX(null);
			Vector2 value = P_0;
			if (_axis2D.xAxis.calibration.invert)
			{
				value.x *= -1f;
			}
			if (_axis2D.yAxis.calibration.invert)
			{
				value.y *= -1f;
			}
			value = MathTools.Clamp(value, -1f, 1f);
			if (_stickTransform != null)
			{
				RectTransform rectTransform = fzhxVoGNCdjqoBvpjALxDaFHbKPjA;
				Vector3 position = value * BwcSQQclGkpGUmjewHpABHCagfUtA;
				position += rectTransform.InverseTransformPoint(base.transform.position);
				Vector3 position2 = rectTransform.TransformPoint(position);
				Vector3 vector = _stickTransform.parent.InverseTransformPoint(position2);
				Vector2 anchoredPosition = DqaChzdTZYNFWhBQIpZWKgtwfiCfA.HVpeUrjECKlcHUllWaezzOTxQeZM(_stickTransform.parent as RectTransform, vector);
				anchoredPosition += _origStickAnchoredPosition;
				_stickTransform.anchoredPosition = anchoredPosition;
			}
			_hierarchyValueChangedHandlers.ExecuteOnAll(P_0);
			_hierarchyStickPositionChangedHandlers.ExecuteOnAll(value);
			_onValueChanged.Invoke(P_0);
			_onStickPositionChanged.Invoke(value);
		}
	}
}
