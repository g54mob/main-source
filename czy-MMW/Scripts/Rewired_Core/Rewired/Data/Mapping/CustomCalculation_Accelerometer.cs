...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool tJqYdLvCGWJciSPLPEWCecMoMIzu()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = KxnsJeQjOjyHwYHkkBJvKZorijYR();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = goobrkxjmEXPMHNOBBvAktmATQKT();
					flag = true;
					break;
				}
			}
		...
		private float KxnsJeQjOjyHwYHkkBJvKZorijYR()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => XGJZYlrjswSIrTDtDBpKcqOMsFvB(num2), 
				_ => 0f, 
			};
		}

		private float goobrkxjmEXPMHNOBBvAktmATQKT()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float XGJZYlrjswSIrTDtDBpKcqOMsFvB(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
