...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_LogitechGRacingWheelPedals : CustomCalculation
	{
		...
		[NonSerialized]
		private Mode FeTVUgfmRcndviTHlrJLKmbBDGSDA;

		TypeWrapper.DataType SerializedMethod.ResultType => TypeWrapper.DataType.Single;

		internal bool QSnNsnvryGzQUWRxfJcCSDEZrSDe()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				_result = lMknCWIkjVFuKvjhHptQVIDUgDAgA();
				flag = true;
			}
			ClearData();
			_resultIsValid = flag;
		...
		private float lMknCWIkjVFuKvjhHptQVIDUgDAgA()
		{
			if (base.DataCount < 2)
			{
				return 0f;
			...
			float num = _data[0];
			float num2 = _data[1];
			bDqebjLlKXEDEDDKIBURSDNRITCGb(num, num2);
			if (FeTVUgfmRcndviTHlrJLKmbBDGSDA == Mode.SharedAxis)
			{
				float oldValue = num2;
			...
			else if (FeTVUgfmRcndviTHlrJLKmbBDGSDA == Mode.SeparateAxes)
			{
				result = num;
			}
			return result;
		...
		private void bDqebjLlKXEDEDDKIBURSDNRITCGb(float P_0, float P_1)
		{
			switch (FeTVUgfmRcndviTHlrJLKmbBDGSDA)
			{
			case Mode.SharedAxis:
				if (MathTools.Abs(P_0) >= 0.01f && MathTools.Abs(P_1) <= 0.01f)
				{
					FeTVUgfmRcndviTHlrJLKmbBDGSDA = Mode.SeparateAxes;
				}
				break;
			case Mode.SeparateAxes:
				if (MathTools.Abs(P_1) >= 0.01f && MathTools.Abs(P_0) <= 0.01f)
				{
					FeTVUgfmRcndviTHlrJLKmbBDGSDA = Mode.SharedAxis;
				}
				break;
			}
		}
