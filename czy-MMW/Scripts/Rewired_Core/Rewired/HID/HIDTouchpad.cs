...
namespace Rewired.HID
{
	...
	internal class HIDTouchpad : HIDControllerElement
	{
		...
		private TouchpadInfo ieJkcRIEUKQNpPqIGNEttIgjIREc;

		private Queue<TouchData> nRVtXnRvJfpgVabOyoiQyiyncTSA;

		private TouchData[] QlawnFFfTWjjzhBllBVzYuZcYVor;

		private Action<NativeBuffer, TouchData[]> NVrJWcvGoJhdmBQLUXqaeUSElNsb;
		...
		public HIDTouchpad(byte P_0, TouchpadInfo P_1, HIDInfo P_2, Action<NativeBuffer, TouchData[]> P_3)
			: base(P_0, P_2)
		{
			ieJkcRIEUKQNpPqIGNEttIgjIREc = P_1;
			NVrJWcvGoJhdmBQLUXqaeUSElNsb = P_3;
			nRVtXnRvJfpgVabOyoiQyiyncTSA = new Queue<TouchData>(10);
			QlawnFFfTWjjzhBllBVzYuZcYVor = new TouchData[P_1.maxTouches];
		...
		{
			if (NVrJWcvGoJhdmBQLUXqaeUSElNsb == null)
			{
				return;
			}
			NVrJWcvGoJhdmBQLUXqaeUSElNsb(inputReport, QlawnFFfTWjjzhBllBVzYuZcYVor);
			lock (nRVtXnRvJfpgVabOyoiQyiyncTSA)
			{
				for (int i = 0; i < ieJkcRIEUKQNpPqIGNEttIgjIREc.maxTouches; i++)
				{
					nRVtXnRvJfpgVabOyoiQyiyncTSA.Enqueue(QlawnFFfTWjjzhBllBVzYuZcYVor[i]);
				}
			}
			ProcessQueue();
		}
		...
		public void ProcessQueue()
		{
			...
			lock (nRVtXnRvJfpgVabOyoiQyiyncTSA)
			{
				int num2 = nRVtXnRvJfpgVabOyoiQyiyncTSA.Count;
				while (num2 > 0)
				{
					TouchData data = nRVtXnRvJfpgVabOyoiQyiyncTSA.Dequeue();
					num2--;
					ieJkcRIEUKQNpPqIGNEttIgjIREc.CalculateTouch(ref data);
					values[num] = data;
					num++;
				}
			}
