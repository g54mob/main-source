...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			dLtXOtMlkczugrxNHgCPweuefUji(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return DObQSRgPULsoRkrXQZaCulQpzaoD().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				WxYTuIcQGgqaBJJrjAHUpTKnaJjBA(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return DObQSRgPULsoRkrXQZaCulQpzaoD().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				WxYTuIcQGgqaBJJrjAHUpTKnaJjBA(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			dLtXOtMlkczugrxNHgCPweuefUji(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.BZIlZOVMwCCENvAYKcvxgVgQRsvU(_id);
			if (inputBehavior != null)
			{
				dLtXOtMlkczugrxNHgCPweuefUji(inputBehavior, this, true);
			}
		}

		internal SerializedObject DObQSRgPULsoRkrXQZaCulQpzaoD()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EqkbSPJHEHHtJoXsspdzqAzVcAQUA
			{
				...
				sMgGiLjHAAIlXTFOzVTKBeTzOPUX = 5.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EqkbSPJHEHHtJoXsspdzqAzVcAQUA
			{
				rzFSJcZEFOpFlXqzyhdFdwpOrpaJ = "id",
				sMgGiLjHAAIlXTFOzVTKBeTzOPUX = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EqkbSPJHEHHtJoXsspdzqAzVcAQUA
			{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EqkbSPJHEHHtJoXsspdzqAzVcAQUA
			{
				...
				FqpwTkyfXldoEdOuFQPgNddSWNnN = null,
				sMgGiLjHAAIlXTFOzVTKBeTzOPUX = string.Format("{0} {1}{2}{3}{4}{5}", "http://guavaman.com/rewired", "http://guavaman.com/schemas/rewired/", "1.4", "/", GetType().Name, ".xsd")
			});
		...
		internal void WxYTuIcQGgqaBJJrjAHUpTKnaJjBA(SerializedObject P_0)
		{
			Reset();
		...
		private static void dLtXOtMlkczugrxNHgCPweuefUji(InputBehavior P_0, InputBehavior P_1, bool P_2)
		{
			if (P_2)
			{
				P_1._id = P_0._id;
