...
namespace Rewired
{
	...
	{
		internal class LGBrMNZdvgtEkBpNABJGpIHRtmbV
		{
			public bool QNrkJTUcshJLhxrGDUBNGAsdEtkQ;

			public bool sAlUlsYHahTaLIdAylrXHmsAxrix;

			public SBpvkrWOYEYLnbkXqvDBKiHTKeoL[] OPGqpfrcKTXyieXugXxwpBhWjYRn;

			public LGBrMNZdvgtEkBpNABJGpIHRtmbV(bool P_0, bool P_1, SBpvkrWOYEYLnbkXqvDBKiHTKeoL[] P_2)
			{
				QNrkJTUcshJLhxrGDUBNGAsdEtkQ = P_0;
				sAlUlsYHahTaLIdAylrXHmsAxrix = P_1;
				OPGqpfrcKTXyieXugXxwpBhWjYRn = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.XAmSiiJuYeuFDbmBewxUJBaRqyME)
					{
		...
		private bool fGVzsFZOOkiOTdIldvpfOWoXyzcj;

		private bool xUJlZNXcAHciZkigCiGlJKIYiYmuA = true;

		private Player qsPBbDsmbuxKSWHoIHFBfneesWDB;

		private LGBrMNZdvgtEkBpNABJGpIHRtmbV ooOXHbyfelQsCSGsyHvuhbOiCsic;

		private readonly int tArWcEYEVkzvnLjuAijEkEsJBvihA;

		private List<RuleSet> jrVoisELrNXjhMIZdiRZOGwrSaEB;

		private Action ydVlFcTcGpaqiduNYZqYjhMyMpaeA;

		public bool enabled
		{
			get
			{
				return fGVzsFZOOkiOTdIldvpfOWoXyzcj;
			}
			set
			{
				fGVzsFZOOkiOTdIldvpfOWoXyzcj = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return xUJlZNXcAHciZkigCiGlJKIYiYmuA;
			}
			set
			{
				xUJlZNXcAHciZkigCiGlJKIYiYmuA = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return jrVoisELrNXjhMIZdiRZOGwrSaEB;
			}
			set
			{
				if (value == null)
				...
				jrVoisELrNXjhMIZdiRZOGwrSaEB = value;
			}
		}

		internal event Action hHyjWwNggkfgmkXfLqDELNQAjexg
		{
			add
			{
				ydVlFcTcGpaqiduNYZqYjhMyMpaeA = (Action)Delegate.Combine(ydVlFcTcGpaqiduNYZqYjhMyMpaeA, b);
			}
			remove
			{
				ydVlFcTcGpaqiduNYZqYjhMyMpaeA = (Action)Delegate.Remove(ydVlFcTcGpaqiduNYZqYjhMyMpaeA, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, LGBrMNZdvgtEkBpNABJGpIHRtmbV P_1)
		{
			if (P_0 == null)
			{
			...
			tArWcEYEVkzvnLjuAijEkEsJBvihA = ReInput.id;
			qsPBbDsmbuxKSWHoIHFBfneesWDB = P_0;
			ooOXHbyfelQsCSGsyHvuhbOiCsic = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != tArWcEYEVkzvnLjuAijEkEsJBvihA)
			{
				ReInput.CheckInitialized(tArWcEYEVkzvnLjuAijEkEsJBvihA);
				return;
			}
			ydVlFcTcGpaqiduNYZqYjhMyMpaeA?.Invoke();
			if (!fGVzsFZOOkiOTdIldvpfOWoXyzcj || jrVoisELrNXjhMIZdiRZOGwrSaEB == null)
			{
				return;
			}
			int count = jrVoisELrNXjhMIZdiRZOGwrSaEB.Count;
			if (count == 0)
			{
				return;
			}
			...
			qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.maps.GetAllMaps(list);
			list2.AddRange(qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = jrVoisELrNXjhMIZdiRZOGwrSaEB[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (xUJlZNXcAHciZkigCiGlJKIYiYmuA && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(qsPBbDsmbuxKSWHoIHFBfneesWDB.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							qsPBbDsmbuxKSWHoIHFBfneesWDB.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((ooOXHbyfelQsCSGsyHvuhbOiCsic != null && ooOXHbyfelQsCSGsyHvuhbOiCsic.OPGqpfrcKTXyieXugXxwpBhWjYRn != null) ? ooOXHbyfelQsCSGsyHvuhbOiCsic.OPGqpfrcKTXyieXugXxwpBhWjYRn.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(ooOXHbyfelQsCSGsyHvuhbOiCsic.OPGqpfrcKTXyieXugXxwpBhWjYRn[i].QInoLyOVDXMHEjqmYOTrkTAOfpnj);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = ooOXHbyfelQsCSGsyHvuhbOiCsic.OPGqpfrcKTXyieXugXxwpBhWjYRn[i].sWgFzqluqGdVCcsbiKCCDYPeGnAQA;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (ooOXHbyfelQsCSGsyHvuhbOiCsic != null)
			{
				fGVzsFZOOkiOTdIldvpfOWoXyzcj = ooOXHbyfelQsCSGsyHvuhbOiCsic.QNrkJTUcshJLhxrGDUBNGAsdEtkQ;
				xUJlZNXcAHciZkigCiGlJKIYiYmuA = ooOXHbyfelQsCSGsyHvuhbOiCsic.sAlUlsYHahTaLIdAylrXHmsAxrix;
			}
			jrVoisELrNXjhMIZdiRZOGwrSaEB = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return OsgytOtEviVitOkHaPFVcEZijBXh().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return OsgytOtEviVitOkHaPFVcEZijBXh().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				MUXPdjnbIyikaXQlIsPeuKwbbCtl(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				MUXPdjnbIyikaXQlIsPeuKwbbCtl(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject OsgytOtEviVitOkHaPFVcEZijBXh()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			ykiJXRXICnkbkybKnAGjyJzsICwg(serializedObject);
			return serializedObject;
		}

		private void ykiJXRXICnkbkybKnAGjyJzsICwg(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", fGVzsFZOOkiOTdIldvpfOWoXyzcj);
			P_0.Add("loadFromUserDataStore", xUJlZNXcAHciZkigCiGlJKIYiYmuA);
			P_0.Add("ruleSets", jrVoisELrNXjhMIZdiRZOGwrSaEB);
		}

		private bool MUXPdjnbIyikaXQlIsPeuKwbbCtl(SerializedObject P_0)
		{
			fGVzsFZOOkiOTdIldvpfOWoXyzcj = false;
			jrVoisELrNXjhMIZdiRZOGwrSaEB = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref fGVzsFZOOkiOTdIldvpfOWoXyzcj);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref xUJlZNXcAHciZkigCiGlJKIYiYmuA);
