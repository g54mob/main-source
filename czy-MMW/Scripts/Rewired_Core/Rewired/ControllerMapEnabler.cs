...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.XAmSiiJuYeuFDbmBewxUJBaRqyME)
				{
		...
		internal class ZppkwHkpXIKClTnElCTrJXPsNYtW
		{
			public bool KKfTDprfYQOfncBMDEpxIBFnXoYr;

			public SBpvkrWOYEYLnbkXqvDBKiHTKeoL[] mqTjtBTvFCpoauArzgtINUApacyaA;

			public ZppkwHkpXIKClTnElCTrJXPsNYtW(bool P_0, SBpvkrWOYEYLnbkXqvDBKiHTKeoL[] P_1)
			{
				KKfTDprfYQOfncBMDEpxIBFnXoYr = P_0;
				mqTjtBTvFCpoauArzgtINUApacyaA = P_1;
			}
		}

		private bool JexHvlcjOkQxamztnwIIfWVTqzegA;

		private Player hnUjWVoyOiiAVQAXykLAaSATAuXA;

		private ZppkwHkpXIKClTnElCTrJXPsNYtW kKOwxGxZACbuQjZsffBDQiOdPAum;

		private readonly int lWnxSArVDVsUrdClskFekTPNFWcE;

		private List<RuleSet> yYtVTNxwQTMrzBGhYfDmbsDhGkIM;

		public bool enabled
		{
			get
			{
				return JexHvlcjOkQxamztnwIIfWVTqzegA;
			}
			set
			{
				JexHvlcjOkQxamztnwIIfWVTqzegA = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return yYtVTNxwQTMrzBGhYfDmbsDhGkIM;
			}
			set
			{
				if (value == null)
				...
				yYtVTNxwQTMrzBGhYfDmbsDhGkIM = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, ZppkwHkpXIKClTnElCTrJXPsNYtW P_1)
		{
			if (P_0 == null)
			{
			...
			lWnxSArVDVsUrdClskFekTPNFWcE = ReInput.id;
			hnUjWVoyOiiAVQAXykLAaSATAuXA = P_0;
			kKOwxGxZACbuQjZsffBDQiOdPAum = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != lWnxSArVDVsUrdClskFekTPNFWcE)
			{
				ReInput.CheckInitialized(lWnxSArVDVsUrdClskFekTPNFWcE);
			}
			else
			{
				if (!JexHvlcjOkQxamztnwIIfWVTqzegA || yYtVTNxwQTMrzBGhYfDmbsDhGkIM == null)
				{
					return;
				}
				int count = yYtVTNxwQTMrzBGhYfDmbsDhGkIM.Count;
				if (count == 0)
				{
					return;
				}
				...
				hnUjWVoyOiiAVQAXykLAaSATAuXA.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = yYtVTNxwQTMrzBGhYfDmbsDhGkIM[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((kKOwxGxZACbuQjZsffBDQiOdPAum != null && kKOwxGxZACbuQjZsffBDQiOdPAum.mqTjtBTvFCpoauArzgtINUApacyaA != null) ? kKOwxGxZACbuQjZsffBDQiOdPAum.mqTjtBTvFCpoauArzgtINUApacyaA.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(kKOwxGxZACbuQjZsffBDQiOdPAum.mqTjtBTvFCpoauArzgtINUApacyaA[i].QInoLyOVDXMHEjqmYOTrkTAOfpnj);
				...
				controllerMapEnablerRuleSetInstance.enabled = kKOwxGxZACbuQjZsffBDQiOdPAum.mqTjtBTvFCpoauArzgtINUApacyaA[i].sWgFzqluqGdVCcsbiKCCDYPeGnAQA;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (kKOwxGxZACbuQjZsffBDQiOdPAum != null)
			{
				JexHvlcjOkQxamztnwIIfWVTqzegA = kKOwxGxZACbuQjZsffBDQiOdPAum.KKfTDprfYQOfncBMDEpxIBFnXoYr;
			}
			yYtVTNxwQTMrzBGhYfDmbsDhGkIM = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return eRPDibIAUqhtTIuFnXebqAZzyMhl().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return eRPDibIAUqhtTIuFnXebqAZzyMhl().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				hMyKWdGvrNlLDSEhrGIMjlRVjjY(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				hMyKWdGvrNlLDSEhrGIMjlRVjjY(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject eRPDibIAUqhtTIuFnXebqAZzyMhl()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			zyREcxxIWSuOBkiPrPlhZqUsdBqv(serializedObject);
			return serializedObject;
		}

		private void zyREcxxIWSuOBkiPrPlhZqUsdBqv(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", JexHvlcjOkQxamztnwIIfWVTqzegA);
			P_0.Add("ruleSets", yYtVTNxwQTMrzBGhYfDmbsDhGkIM);
		}

		private bool hMyKWdGvrNlLDSEhrGIMjlRVjjY(SerializedObject P_0)
		{
			JexHvlcjOkQxamztnwIIfWVTqzegA = false;
			yYtVTNxwQTMrzBGhYfDmbsDhGkIM = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref JexHvlcjOkQxamztnwIIfWVTqzegA);
