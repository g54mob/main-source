...
namespace Rewired.Utils
{
	...
	{
		internal struct RwimbXPewXcmyPjTsMHTxYekEIoj
		{
			public Platform gyKIPpQGdyFiDXnLgdbarTkEWvbS;

			public Platform EiFNIxeCHHTSmcvvKPZchOPlMNnG;

			public EditorPlatform YdhNRCZalJyPRaNWmmAbAynwHkAD;

			public bool VvYcISchqqbMxYwLjcKopcSVUKhW;

			public WebplayerPlatform loWgxsaiyCwxTlAxuvYVKzvDBujR;

			public ScriptingBackend XDNFdYbBBEThwnkUeKDOLdOXigkjb;

			public ScriptingAPILevel MQzfbXhCOXMgAfMPBHnGELrljIFfc;

			public IExternalTools XJGEmHxcTayRwZQMJUaTePVgoreI;

			public RwimbXPewXcmyPjTsMHTxYekEIoj(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				gyKIPpQGdyFiDXnLgdbarTkEWvbS = P_0;
				EiFNIxeCHHTSmcvvKPZchOPlMNnG = P_1;
				YdhNRCZalJyPRaNWmmAbAynwHkAD = P_2;
				VvYcISchqqbMxYwLjcKopcSVUKhW = P_3;
				loWgxsaiyCwxTlAxuvYVKzvDBujR = P_4;
				XDNFdYbBBEThwnkUeKDOLdOXigkjb = P_5;
				MQzfbXhCOXMgAfMPBHnGELrljIFfc = P_6;
				XJGEmHxcTayRwZQMJUaTePVgoreI = P_7;
			}
		}
		...
		{
			public enum jJOCHfXafVPMnfexxGaWQpSQtxbU
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly jJOCHfXafVPMnfexxGaWQpSQtxbU type;

			public readonly int build;

			public UnityVersionClass(string P_0)
			{
				type = jJOCHfXafVPMnfexxGaWQpSQtxbU.Normal;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = jJOCHfXafVPMnfexxGaWQpSQtxbU.Beta;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = jJOCHfXafVPMnfexxGaWQpSQtxbU.Patch;
					}
			...
			public override string ToString()
			{
				...
				array[5] = dwNWdXiogiOvmpcgoVtoUUHmUwss(type);
				num = build;
				array[6] = num.ToString();
				return string.Concat(array);
			}

			private string dwNWdXiogiOvmpcgoVtoUUHmUwss(jJOCHfXafVPMnfexxGaWQpSQtxbU P_0)
			{
				return P_0 switch
				{
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Normal => "f", 
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Beta => "b", 
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Patch => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (imRMHsEUNoQpWCjTtRpiBnfwSfbL(a.type) > imRMHsEUNoQpWCjTtRpiBnfwSfbL(b.type))
				{
					return 1;
				}
				if (imRMHsEUNoQpWCjTtRpiBnfwSfbL(a.type) < imRMHsEUNoQpWCjTtRpiBnfwSfbL(b.type))
			...
			private static int imRMHsEUNoQpWCjTtRpiBnfwSfbL(jJOCHfXafVPMnfexxGaWQpSQtxbU P_0)
			{
				return P_0 switch
				{
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Beta => 0, 
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Normal => 10, 
					jJOCHfXafVPMnfexxGaWQpSQtxbU.Patch => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion sIAfYRyxYGKRbgsHYiVZhFxjmfpY = UnityVersion.UNITY_5_0;

		private static UnityVersionClass DStLbaCMyCATHcdnFazGELydFGWfb;

		private static UnityVersion eccWCkSAIhzbuRZxhMZEYjNGSWVn = UnityVersion.Unknown;

		private static string oJauLTBpFJnzeUbJtnFfnTSyNCbY;

		private static Platform GxORvGlLaJYkMYzHwvqFXmQFxQPW;

		private static EditorPlatform GQBdlaarDMhdseQYfqQPXxeFKRLCb;

		private static bool ecmQdjFqvBGNPoajAVNjrpdZFqDgA;

		private static bool ZHLArCppyCbFTXekNhTILSeKHOwi;

		private static bool bSovgSxYUovgQZyLfiBkGhHMBOFL;

		private static WebplayerPlatform bTsMOWShADkoSKHzealceeqDqtYl;

		private static bool SEHPtbnRGSUeBHpbBIsQHhfSKFAj;

		private static bool ZAzLqHfiusxqkwEIDfsPLyWQKvrS;

		private static bool fqPRGlBNZKBWtbgIlpRaAvlOroKZ;

		private static bool YjAeBaFBbHJzLXpapZyBTrISWNpB;

		private static bool fIeKTbKSSPfaYGzbEgZbbqLbANJbA;

		private static bool oxxmRKLQKCGLgafWviabBCdgtlRf;

		private static string vcdqFomLFKCYdRlFehmUsTnFlHKx;

		private static ScriptingBackend TSJaECtbSmKGQeSGFwWtlaMPGOHX;

		private static ScriptingAPILevel QKcVWbOmbfOLkPEMxvtuKbZuQEWW;

		private static bool kSaBGLHVNDULrRslrfWbOTqQhtvH;

		private static IExternalTools nSeFngadKaPLWOIigsCiQfKxRQlAA;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper WajmVHjgIRAyvieaoYkqappHvfTOA;

		private static bool HPIYHwwwIINhTSRtUPhNEudVRoBC;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				if (!YDJxBhfNLYKixSIUYAyVliLBqYcQ)
				{
					return null;
				}
				return DStLbaCMyCATHcdnFazGELydFGWfb;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return oJauLTBpFJnzeUbJtnFfnTSyNCbY;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return GxORvGlLaJYkMYzHwvqFXmQFxQPW;
			}
		}
		...
		internal static Platform effectivePlatform
		{
			get
			{
				...
				if (!ecmQdjFqvBGNPoajAVNjrpdZFqDgA)
				{
					return GxORvGlLaJYkMYzHwvqFXmQFxQPW;
				}
				return GQBdlaarDMhdseQYfqQPXxeFKRLCb switch
				{
					EditorPlatform.Windows => Platform.Windows, 
					EditorPlatform.OSX => Platform.OSX, 
					EditorPlatform.Linux => Platform.Linux, 
					_ => GxORvGlLaJYkMYzHwvqFXmQFxQPW, 
				};
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return GQBdlaarDMhdseQYfqQPXxeFKRLCb;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return ecmQdjFqvBGNPoajAVNjrpdZFqDgA;
			}
		}

		public static bool isPlaying => ZHLArCppyCbFTXekNhTILSeKHOwi;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return bSovgSxYUovgQZyLfiBkGhHMBOFL;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return bTsMOWShADkoSKHzealceeqDqtYl;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (ecmQdjFqvBGNPoajAVNjrpdZFqDgA || Application.isEditor)
				{
					return true;
				}
				...
				switch (GxORvGlLaJYkMYzHwvqFXmQFxQPW)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!bSovgSxYUovgQZyLfiBkGhHMBOFL)
					{
						return TSJaECtbSmKGQeSGFwWtlaMPGOHX == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return GQBdlaarDMhdseQYfqQPXxeFKRLCb switch
				{
					EditorPlatform.Windows => GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.Windows, 
					EditorPlatform.OSX => GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.OSX, 
					EditorPlatform.Linux => GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return SEHPtbnRGSUeBHpbBIsQHhfSKFAj;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return ZAzLqHfiusxqkwEIDfsPLyWQKvrS;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.Android && GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.Ouya && GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.AmazonFireTV)
				{
					return GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.iOS)
				{
					return GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.Windows && GxORvGlLaJYkMYzHwvqFXmQFxQPW != Platform.Linux)
				{
					return GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return fqPRGlBNZKBWtbgIlpRaAvlOroKZ;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_0)
				{
					return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool supportsStadia
		{
			get
			{
				...
				return eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_2019_3;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return kSaBGLHVNDULrRslrfWbOTqQhtvH;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => TSJaECtbSmKGQeSGFwWtlaMPGOHX;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => QKcVWbOmbfOLkPEMxvtuKbZuQEWW;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return nSeFngadKaPLWOIigsCiQfKxRQlAA;
			}
		}

		internal static IAndroidFallbackPlatformHelper gVOFJpatkJHJYZfiYMjQGfZaqTURA
		{
			[CompilerGenerated]
			get
			{
				return WajmVHjgIRAyvieaoYkqappHvfTOA;
			}
			[CompilerGenerated]
			set
			{
				WajmVHjgIRAyvieaoYkqappHvfTOA = wajmVHjgIRAyvieaoYkqappHvfTOA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => HPIYHwwwIINhTSRtUPhNEudVRoBC;

		private static bool YDJxBhfNLYKixSIUYAyVliLBqYcQ => KKbzxhHaLLkxmkDAXeXRlWuEZavN();

		private static bool KKbzxhHaLLkxmkDAXeXRlWuEZavN()
		{
			if (HPIYHwwwIINhTSRtUPhNEudVRoBC)
			{
				return true;
			}
			try
			{
				oJauLTBpFJnzeUbJtnFfnTSyNCbY = Application.unityVersion;
				DStLbaCMyCATHcdnFazGELydFGWfb = new UnityVersionClass(oJauLTBpFJnzeUbJtnFfnTSyNCbY);
				SwowQPcjhoUpHJcfBATsHQFiOnWqA();
				HPIYHwwwIINhTSRtUPhNEudVRoBC = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return HPIYHwwwIINhTSRtUPhNEudVRoBC;
		}

		internal static void suuYxLrSpAboTHHojwxOwzMmIykN(RwimbXPewXcmyPjTsMHTxYekEIoj P_0)
		{
			if (YDJxBhfNLYKixSIUYAyVliLBqYcQ)
			{
				if (P_0.EiFNIxeCHHTSmcvvKPZchOPlMNnG == Platform.Windows81Store)
				{
					P_0.EiFNIxeCHHTSmcvvKPZchOPlMNnG = Platform.WindowsUWP;
				}
				GxORvGlLaJYkMYzHwvqFXmQFxQPW = P_0.EiFNIxeCHHTSmcvvKPZchOPlMNnG;
				GQBdlaarDMhdseQYfqQPXxeFKRLCb = P_0.YdhNRCZalJyPRaNWmmAbAynwHkAD;
				ecmQdjFqvBGNPoajAVNjrpdZFqDgA = P_0.VvYcISchqqbMxYwLjcKopcSVUKhW;
				bTsMOWShADkoSKHzealceeqDqtYl = P_0.loWgxsaiyCwxTlAxuvYVKzvDBujR;
				TSJaECtbSmKGQeSGFwWtlaMPGOHX = P_0.XDNFdYbBBEThwnkUeKDOLdOXigkjb;
				QKcVWbOmbfOLkPEMxvtuKbZuQEWW = P_0.MQzfbXhCOXMgAfMPBHnGELrljIFfc;
				if (nSeFngadKaPLWOIigsCiQfKxRQlAA != null)
				{
					nSeFngadKaPLWOIigsCiQfKxRQlAA.Destroy();
				}
				nSeFngadKaPLWOIigsCiQfKxRQlAA = P_0.XJGEmHxcTayRwZQMJUaTePVgoreI;
				bSovgSxYUovgQZyLfiBkGhHMBOFL = Debug.isDebugBuild;
				ZHLArCppyCbFTXekNhTILSeKHOwi = true;
				PGFWnQBXYNsbcQIrohrqURTYpprJ();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			TlyeeWBiDwErCWszANOTkOEtVtcGA(minVersionStr, out var num);
			TlyeeWBiDwErCWszANOTkOEtVtcGA(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (num3 && DStLbaCMyCATHcdnFazGELydFGWfb < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag && DStLbaCMyCATHcdnFazGELydFGWfb >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag && DStLbaCMyCATHcdnFazGELydFGWfb > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool TlyeeWBiDwErCWszANOTkOEtVtcGA(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void SwowQPcjhoUpHJcfBATsHQFiOnWqA()
		{
			eccWCkSAIhzbuRZxhMZEYjNGSWVn = ytxXEjCXAgeKMysltTkFvqECJnmL(Application.unityVersion);
			if (eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_3_5 && eccWCkSAIhzbuRZxhMZEYjNGSWVn < UnityVersion.UNITY_4_0)
			{
				SEHPtbnRGSUeBHpbBIsQHhfSKFAj = true;
			}
			else if (eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_0)
			{
				ZAzLqHfiusxqkwEIDfsPLyWQKvrS = true;
			}
		}

		private static UnityVersion ytxXEjCXAgeKMysltTkFvqECJnmL(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion JXhRwXvuQJarzQQmqhBwTubLYLKl(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion mmovrnGdHPnFQStUwlOqcSROqPHG(int P_0)
		{
			return P_0 switch
			{
		...
		private static void PGFWnQBXYNsbcQIrohrqURTYpprJ()
		{
			switch (GxORvGlLaJYkMYzHwvqFXmQFxQPW)
			{
			...
			case Platform.RazerForgeTV:
				YjAeBaFBbHJzLXpapZyBTrISWNpB = true;
				fIeKTbKSSPfaYGzbEgZbbqLbANJbA = true;
				break;
			case Platform.PS4:
				YjAeBaFBbHJzLXpapZyBTrISWNpB = true;
				vcdqFomLFKCYdRlFehmUsTnFlHKx = "Empty";
				oxxmRKLQKCGLgafWviabBCdgtlRf = true;
				break;
			case Platform.Windows:
				if ((eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_6_3p1 && eccWCkSAIhzbuRZxhMZEYjNGSWVn < UnityVersion.UNITY_5_0) || eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_0_0p1)
				{
					fqPRGlBNZKBWtbgIlpRaAvlOroKZ = true;
					YjAeBaFBbHJzLXpapZyBTrISWNpB = true;
				}
				break;
			case Platform.Linux:
			...
			if (ecmQdjFqvBGNPoajAVNjrpdZFqDgA && GQBdlaarDMhdseQYfqQPXxeFKRLCb == EditorPlatform.Windows && ((eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_6_3p1 && eccWCkSAIhzbuRZxhMZEYjNGSWVn < UnityVersion.UNITY_5_0) || eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_5_0_0p1))
			{
				fqPRGlBNZKBWtbgIlpRaAvlOroKZ = true;
				YjAeBaFBbHJzLXpapZyBTrISWNpB = true;
			}
			if ((!ecmQdjFqvBGNPoajAVNjrpdZFqDgA && GxORvGlLaJYkMYzHwvqFXmQFxQPW == Platform.Windows) || (ecmQdjFqvBGNPoajAVNjrpdZFqDgA && GQBdlaarDMhdseQYfqQPXxeFKRLCb == EditorPlatform.Windows))
			{
				kSaBGLHVNDULrRslrfWbOTqQhtvH = eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type xYGJWHKCZAajZDLJzgcIFEmjKHiGb(WopAVcCfhdAXyfQZyefpXRHITOYW P_0)
		{
			if (!YDJxBhfNLYKixSIUYAyVliLBqYcQ)
			{
				return null;
			}
			if (eccWCkSAIhzbuRZxhMZEYjNGSWVn >= UnityVersion.UNITY_4_3)
			{
				return gtGfCfdLgDDFlkJVkCboKJsbkFZhA(P_0);
			}
			return null;
		}

		private static Type gtGfCfdLgDDFlkJVkCboKJsbkFZhA(WopAVcCfhdAXyfQZyefpXRHITOYW P_0)
		{
			return P_0 switch
			{
				WopAVcCfhdAXyfQZyefpXRHITOYW.RigidbodyInterpolation2D => typeof(RigidbodyInterpolation2D), 
				WopAVcCfhdAXyfQZyefpXRHITOYW.RigidbodySleepMode2D => typeof(RigidbodySleepMode2D), 
				WopAVcCfhdAXyfQZyefpXRHITOYW.CollisionDetectionMode2D => typeof(CollisionDetectionMode2D), 
				WopAVcCfhdAXyfQZyefpXRHITOYW.PhysicsMaterial2D => typeof(PhysicsMaterial2D), 
				WopAVcCfhdAXyfQZyefpXRHITOYW.Collider2D => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return zxVAoOdIuvklwrEdRZBSJrWapqhMA(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return zxVAoOdIuvklwrEdRZBSJrWapqhMA((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return zxVAoOdIuvklwrEdRZBSJrWapqhMA((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return zxVAoOdIuvklwrEdRZBSJrWapqhMA(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && fqPRGlBNZKBWtbgIlpRaAvlOroKZ)
				{
					return false;
				}
				if (YjAeBaFBbHJzLXpapZyBTrISWNpB)
				{
					return false;
				}
			}
			...
			{
				if (oxxmRKLQKCGLgafWviabBCdgtlRf && name.Equals(vcdqFomLFKCYdRlFehmUsTnFlHKx, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (fIeKTbKSSPfaYGzbEgZbbqLbANJbA && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static _0001 zxVAoOdIuvklwrEdRZBSJrWapqhMA<_0001>(_0001 P_0) where _0001 : class
		{
			if (P_0 == null)
			{
				return null;
		...
		internal static ButtonStateFlags cLGQDDuPabqufbGZErOFjZdUBEaT(KeyCode P_0)
		{
		...
		internal static ButtonStateFlags SwYWPpogcODeKUOEPCynJzGUtZQV(string P_0)
		{
