...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> nHEoXPgudpZmkKaQyHBbXBICOyzH;

		private static Action<object, Action> invokeDelegate => TstrDYnUNhRilqfWnHrhpIVSJvLW;
		...
		private static void TstrDYnUNhRilqfWnHrhpIVSJvLW(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	{
		private T mBdZimIqyPFNwuHzQuibSfbpdeCKA;

		private static Action<object, Action<T>> KVZadDBhXZAiNvydNHvFEHzRDTIyA;

		private static Action<object, Action<T>> invokeDelegate => EMcWNrWKabACcEYDYOKjZcNDlrWN;
		...
		{
			mBdZimIqyPFNwuHzQuibSfbpdeCKA = arg0;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			mBdZimIqyPFNwuHzQuibSfbpdeCKA = default(T);
		}
		...
		private static void EMcWNrWKabACcEYDYOKjZcNDlrWN(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.mBdZimIqyPFNwuHzQuibSfbpdeCKA);
			}
		}
	...
	{
		private T aduQtOEmbSrYARkbYHdCYwZAuaZD;

		private T2 XWwCcYzKoYjoVPCAYbRVXNSiQnSw;

		private static Action<object, Action<T, T2>> GmkqcwieFFmGEFtrKBnXSnzdidLf;

		private static Action<object, Action<T, T2>> invokeDelegate => rzGDhJTwbazgiSKAPgGURxelrtIN;
		...
		{
			aduQtOEmbSrYARkbYHdCYwZAuaZD = arg0;
			XWwCcYzKoYjoVPCAYbRVXNSiQnSw = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			aduQtOEmbSrYARkbYHdCYwZAuaZD = default(T);
			XWwCcYzKoYjoVPCAYbRVXNSiQnSw = default(T2);
		}
		...
		private static void rzGDhJTwbazgiSKAPgGURxelrtIN(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.aduQtOEmbSrYARkbYHdCYwZAuaZD, safeAction.XWwCcYzKoYjoVPCAYbRVXNSiQnSw);
			}
		}
