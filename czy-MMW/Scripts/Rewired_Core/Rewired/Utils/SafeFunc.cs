...
namespace Rewired.Utils
{
	...
	{
		private T ziAKVRbdqcIFZxeZnHnTCFctKpjp;

		private TResult bfvzPCyKJsSSmDvqeDhARDaexgaJ;

		private static Action<object, Func<T, TResult>> jyPEFyOhRMPjBOuFzWXBwJufebpD;

		private static Action<object, Func<T, TResult>> invokeDelegate => hdUrddyBYXsGpJmsZfHNhAdOWJrW;
		...
		{
			ziAKVRbdqcIFZxeZnHnTCFctKpjp = arg0;
			try
			{
				Invoke(invokeDelegate);
				return bfvzPCyKJsSSmDvqeDhARDaexgaJ;
			}
			catch
			{
			...
			{
				ziAKVRbdqcIFZxeZnHnTCFctKpjp = default(T);
				bfvzPCyKJsSSmDvqeDhARDaexgaJ = default(TResult);
			}
		}
		...
		private static void hdUrddyBYXsGpJmsZfHNhAdOWJrW(object P_0, Func<T, TResult> P_1)
		{
			if (P_1 != null && P_0 is SafeFunc<T, TResult> safeFunc)
			{
				safeFunc.bfvzPCyKJsSSmDvqeDhARDaexgaJ = P_1(safeFunc.ziAKVRbdqcIFZxeZnHnTCFctKpjp);
			}
		}
