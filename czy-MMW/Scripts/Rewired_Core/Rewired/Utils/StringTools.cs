...
namespace Rewired.Utils
{
	...
	{
		private static string xMePhefIAUBEfHMHljTeDQHcegXVb;
		...
		static StringTools()
		{
			xMePhefIAUBEfHMHljTeDQHcegXVb = Regex.Escape(new string(Path.GetInvalidFileNameChars()));
		}

		public static string CleanUpFileName(string name)
		{
			name = name.Trim();
			string pattern = "[ ~`,:;'\\.\\$\\^\\{\\}\\[\\]\\(\\|\\)\\*\\+\\?\\\\" + xMePhefIAUBEfHMHljTeDQHcegXVb + "]";
