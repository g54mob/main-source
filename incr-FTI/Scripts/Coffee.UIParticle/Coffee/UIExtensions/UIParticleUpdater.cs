	...
	internal static class UIParticleUpdater
	{
		...
		private static void Refresh()
		{
			...
			for (int i = 0; i < s_ActiveParticles.Count; i++)
			{
				...
				if ((bool)uIParticle && (bool)uIParticle.canvas && uIParticle.isPrimary && !s_UpdatedGroupIds.Contains(uIParticle.groupId))
				{
					s_UpdatedGroupIds.Add(uIParticle.groupId);
			...
			for (int j = 0; j < s_ActiveParticles.Count; j++)
			{
				...
				if ((bool)uIParticle2 && (bool)uIParticle2.canvas)
				{
					...
					else if (!s_UpdatedGroupIds.Contains(uIParticle2.groupId))
					{
						s_UpdatedGroupIds.Add(uIParticle2.groupId);
						uIParticle2.UpdateRenderers();
					}
				}
			}
			...
			for (int l = 0; l < s_ActiveParticles.Count; l++)
			{
				UIParticle uIParticle3 = s_ActiveParticles[l];
				if ((bool)uIParticle3 && (bool)uIParticle3.canvas)
				{
					uIParticle3.UpdateParticleCount();
				}
			}
		}
