...
namespace Coffee.UIExtensions
{
	...
	internal class UIParticleRenderer : MaskableGraphic
	{
		...
		public void UpdateMesh(Camera bakeCamera)
		{
			...
			{
				_renderer.BakeTrailsMesh(s_CombineInstances[0].mesh, bakeCamera, useTransform: true);
			}
			else if (_renderer.CanBakeMesh())
			{
				if (null == _renderer)
				{
					Debug.LogError("Null renderer in UpdateMesh");
				}
				else if (null == s_CombineInstances[0].mesh)
				{
					Debug.LogError("Null s_CombineInstances in UpdateMesh");
				}
				else
				{
					_renderer.BakeMesh(s_CombineInstances[0].mesh, bakeCamera, useTransform: true);
				}
			}
			else
			{
			...
			for (int j = 0; j < s_Renderers.Count; j++)
			{
				if (!(s_Renderers[j] == this))
				{
					s_Renderers[j].canvasRenderer.SetMesh(Graphic.workerMesh);
					s_Renderers[j]._lastBounds = _lastBounds;
				}
			}
			...
			if (!_parent.useMeshSharing)
			{
				...
				for (int k = 0; k < s_Renderers.Count; k++)
				{
					...
					{
						s_Renderers[k].canvasRenderer.materialCount = 1;
						s_Renderers[k].canvasRenderer.SetMaterial(_currentMaterialForRendering, 0);
					}
				}
			}
			s_Renderers.Clear();
		...
		{
			return _parent.scale3DForCalc.GetScaled(_parent.parentScale);
		}
		...
		private void ResolveResolutionChange(Vector3 psPos, Vector3 scale)
		{
			...
			if ((_prevScreenSize != vector2Int || _prevScale != scale) && flag)
			{
		...
		private void Simulate(Vector3 scale, bool paused)
		{
			...
			if (emission.enabled && 0f < emission.rateOverDistance.constant && 0f < emission.rateOverDistanceMultiplier && !paused)
			{
