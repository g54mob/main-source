...
namespace NUnit.Framework.Internal.Execution
{
	public static class CommandBuilder
	{
		public static TestCommand MakeOneTimeSetUpCommand(TestSuite suite, List<SetUpTearDownItem> setUpTearDown, List<TestActionItem> actions)
		{
			...
			{
				list = suite.TypeInfo.GetCustomAttributes<IApplyToContext>(inherit: true);
			}
			else if (suite.Method != null)
			{
				list = suite.Method.GetCustomAttributes<IApplyToContext>(inherit: true);
			}
			else if (suite is TestAssembly testAssembly)
			{
				list = (IApplyToContext[])testAssembly.Assembly.GetCustomAttributes(typeof(IApplyToContext), inherit: true);
			}
		...
		public static TestCommand MakeTestCommand(TestMethod test)
		{
			...
			IWrapTestMethod[] customAttributes = test.Method.GetCustomAttributes<IWrapTestMethod>(inherit: true);
			...
			IWrapSetUpTearDown[] customAttributes2 = test.Method.GetCustomAttributes<IWrapSetUpTearDown>(inherit: true);
			foreach (ICommandWrapper commandWrapper in customAttributes2)
			{
				testCommand = commandWrapper.Wrap(testCommand);
			}
			IApplyToContext[] customAttributes3 = test.Method.GetCustomAttributes<IApplyToContext>(inherit: true);
		...
		{
			MethodInfo[] methodsWithAttribute = Reflect.GetMethodsWithAttribute(fixtureType, setUpType, inherit: true);
			MethodInfo[] methodsWithAttribute2 = Reflect.GetMethodsWithAttribute(fixtureType, tearDownType, inherit: true);
			...
			while ((object)fixtureType != null && !fixtureType.Equals(typeof(object)))
			{
				...
				fixtureType = NUnit.Compatibility.TypeExtensions.GetTypeInfo(fixtureType).BaseType;
			}
			return list;
		}
