...
namespace NUnit.Framework.Internal
{
	public class ActionsHelper
	{
		...
		public static ITestAction[] GetActionsFromAttributeProvider(ICustomAttributeProvider attributeProvider)
		{
			...
			List<ITestAction> list = new List<ITestAction>((ITestAction[])attributeProvider.GetCustomAttributes(typeof(ITestAction), inherit: false));
		...
		public static ITestAction[] GetActionsFromTypesAttributes(Type type)
		{
			...
			list.AddRange(GetActionsFromTypesAttributes(NUnit.Compatibility.TypeExtensions.GetTypeInfo(type).BaseType));
			Type[] declaredInterfaces = GetDeclaredInterfaces(type);
			Type[] array = declaredInterfaces;
			foreach (Type type2 in array)
			{
				list.AddRange(GetActionsFromAttributeProvider(NUnit.Compatibility.TypeExtensions.GetTypeInfo(type2)));
			}
			list.AddRange(GetActionsFromAttributeProvider(NUnit.Compatibility.TypeExtensions.GetTypeInfo(type)));
			return list.ToArray();
		}
		...
		private static Type[] GetDeclaredInterfaces(Type type)
		{
			...
			if ((object)NUnit.Compatibility.TypeExtensions.GetTypeInfo(type).BaseType == typeof(object))
			{
				return list.ToArray();
			}
			List<Type> list2 = new List<Type>(NUnit.Compatibility.TypeExtensions.GetTypeInfo(type).BaseType.GetInterfaces());
