		...
		private static void PerformSpecialConversions(object[] arglist, IParameterInfo[] parameters)
		{
			for (int i = 0; i < arglist.Length; i++)
			{
				...
				else
				{
					...
					if (flag)
					{
						Type conversionType = ((NUnit.Compatibility.TypeExtensions.GetTypeInfo(parameterType).IsGenericType && (object)parameterType.GetGenericTypeDefinition() == typeof(Nullable<>)) ? parameterType.GetGenericArguments()[0] : parameterType);
