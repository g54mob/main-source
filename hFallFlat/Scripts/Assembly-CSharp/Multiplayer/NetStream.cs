...
namespace Multiplayer
{
	public class NetStream
	{
		...
		public int position
		{
			get
			{
				return currentByte * 8 + currentBit - offsetBits;
			}
		}
		...
		public static byte[] ReuseBuffer(byte[] data, bool allowAddNewEntry = true)
		{
			...
			{
				int value;
				if (!bufferRefCount.TryGetValue(data, out value))
				{
					if (!allowAddNewEntry)
					{
		...
		public static void ReleaseBuffer(int bufferTier, byte[] buffer, bool applyRefCount = true)
		{
			lock (poolLock)
			{
				...
				{
					int value;
					if (!bufferRefCount.TryGetValue(buffer, out value))
					{
						return;
					}
					if (--value > 0)
		...
		private void Alloc(int tier)
		{
			lock (poolLock)
			{
				...
				buffer = AllocateBuffer(tier, false);
		...
		public void Write(uint x, ushort bitfull)
		{
			...
			while (bitfull > 0 && num-- > 0)
			{
				...
				if (currentBit + bitfull < 8)
				{
					...
					b &= (byte)(255 << 8 - currentBit - bitfull);
		...
		{
			PadToByte();
			EnsureFitsBytes(array.Length + (lenBits + 7) / 8, true);
		...
		public void WriteStream(NetStream stream)
		{
			...
			{
				Write(num, 4, 14, 22);
				EnsureFitsBytes(stream.UseBuffedSize(), true);
				PadToByte();
