...
namespace Multiplayer
{
	public struct NetQuaternion
	{
		...
		public static NetQuaternion Quantize(Quaternion q, ushort bits)
		{
			...
			switch ((byte)((num > num2) ? ((num > num3) ? ((!(num > num4)) ? 3 : 0) : ((!(num3 > num4)) ? 3 : 2)) : ((num2 > num3) ? ((num2 > num4) ? 1 : 3) : ((!(num3 > num4)) ? 3 : 2))))
			{
			case 0:
		...
		public override string ToString()
		{
			...
			return string.Format("({0},{1},{2},{3}) ({4},{5},{6},{7}) {8}", sel, x, y, z, quaternion.x, quaternion.y, quaternion.z, quaternion.w, quaternion.eulerAngles);
		}
	}
}
