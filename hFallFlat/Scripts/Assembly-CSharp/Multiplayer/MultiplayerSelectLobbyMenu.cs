...
namespace Multiplayer
{
	public class MultiplayerSelectLobbyMenu : MenuTransition
	{
		...
		private void Start()
		{
			hostPrivateButton.gameObject.SetActive(false);
		...
		protected override void Update()
		{
			...
			{
				SetJoinFriendButtonState(false);
				mRefreshWait -= Time.deltaTime;
				if (mRefreshWait <= 0f && lobbySelectMenuOptions != null)
				{
					lobbySelectMenuOptions.ShowRefreshText(true);
					showFriendText = true;
				}
			}
			...
			if (sLobbyUpdateTimer < 0f)
			{
				...
				{
					NetGame.instance.transport.RequestLobbyDataRefresh(selectedMenuItem.boundData, false);
				}
		...
		public override void OnGotFocus()
		{
			...
			lobbySelectMenuOptions.SetFriendText(NetGame.friendly);
			showFriendText = true;
			lobbySelectMenuOptions.ShowFriendText(true);
			showJoinText = false;
			SetScreenText();
		...
		private void UpdateLobbies(bool focus = true)
		{
			...
			if (items.Count == 0)
			{
				...
				noLobbiesMessage.gameObject.SetActive(true);
				...
				hostButton.Select();
				LevelInformationBoxEnabled(false);
			}
			else
			{
				noLobbiesMessage.gameObject.SetActive(false);
				if (focus)
				{
					list.FocusItem(0);
				}
		...
		private void OnSubmit(ListViewItem item)
		{
			...
			selectedMenuItem = multiplayerSelectLobbyMenuItem;
			JoinClick();
		}
		...
		private void OnSelect(ListViewItem item)
		{
			...
			{
				LevelInformationBoxEnabled(true);
			}
		...
		{
			MultiplayerSelectLobbyMenuItem multiplayerSelectLobbyMenuItem = item as MultiplayerSelectLobbyMenuItem;
			if (multiplayerSelectLobbyMenuItem != null)
			{
				multiplayerSelectLobbyMenuItem.SetActive(false);
			}
			if (lobbySelectMenuOptions != null && selectedMenuItem != null)
			{
				showJoinText = selectedMenuItem.IsActive();
