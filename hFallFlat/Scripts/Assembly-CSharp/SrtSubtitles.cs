...
public class SrtSubtitles
{
	...
	public void Load(string name, string srt)
	{
		...
		while (j < array.Length)
		{
			...
			int result;
			if (j >= array.Length || !int.TryParse(array[j++], out result) || j >= array.Length)
			{
				continue;
			}
			...
			float time;
			float time2;
			if (array2.Length >= 3 && string.Equals(array2[1], "-->") && TryParseTime(array2[0], out time) && TryParseTime(array2[2], out time2) && j < array.Length)
			{
				string text = array[j++];
				...
				lines.Add(new SrtLine
				{
					...
					text = text,
					key = string.Format("{0}_{1}", name, result)
				});
			}
		}
	}
	...
	public bool TryParseTime(string str, out float time)
	{
		...
		int result3;
		int result4;
		if (int.TryParse(array[0], out result) && int.TryParse(array[1], out result2) && int.TryParse(array[2], out result3) && int.TryParse(array[3], out result4))
		{
