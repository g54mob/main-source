	...
	public class SoundLibrary : MonoBehaviour
	{
		...
		public class SerializedSampleLibrary
		{
			...
			public void AddSample(SerializedSample sample)
			{
				...
				if (!categoryMap.TryGetValue(sample.category, out value))
				{
			...
			public void RemoveMissing()
			{
				...
				for (int num = samples.Count - 1; num >= 0; num--)
				{
					...
					else
					{
						...
						SampleCategory value;
						if (!categoryMap.TryGetValue(serializedSample.category, out value))
						{
		...
		public class RandomContainer : IClipContainer, IClip
		{
			...
			{
				IClip value;
				if (children.TryGetValue(localId, out value))
				{
					if (value == container)
					{
						return;
			...
			{
				return string.Format("Random {0}", id);
			}
		}
		...
		public class LoopContainer : IClipContainer, IClip
		{
			...
			public override string ToString()
			{
				return string.Format("Loop {0}", id);
			}
		}
		...
		public class SwitchContainer : IClipContainer, IClip
		{
			...
			public override string ToString()
			{
				return string.Format("Switch {0}", id);
			}
		}
		...
		public class SerializedSample
		{
			...
			public float volume
			{
				get
				{
					return (vR != 0f) ? (vB * AudioUtils.DBToValue(UnityEngine.Random.Range(0f - vR, vR))) : vB;
				}
			}
			...
			public float pitch
			{
				get
				{
					return (pR != 0f) ? (pB * AudioUtils.CentsToRatio(UnityEngine.Random.Range(0f - pR, pR))) : pB;
				}
			}
			...
			{
				IClipContainer value;
				containers.TryGetValue(containerId, out value);
				return value;
			}
			...
			public SerializedClip AddClip(string path, string id, SerializedClip copyFrom = null, AudioClip loadedClip = null)
			{
				...
				if (clipContainer != null)
				{
					string containerId;
					string localId;
					ParseId(id, out containerId, out localId);
			...
			private IClipContainer EnsureContainerFor(string childId)
			{
				string containerId;
				string localId;
				ParseId(childId, out containerId, out localId);
				...
				if (clipContainer == null)
				{
					...
					else
					{
						string containerId2;
						string localId2;
						ParseId(containerId, out containerId2, out localId2);
			...
			public static SampleContainerChildType GetChildType(string localChildId)
			{
				...
				int result;
				if (int.TryParse(localChildId, out result))
				{
			...
			public SerializedClip GetClip(SerializedClip currentClip, char choice, SampleContainerChildType loopType)
			{
				...
				return (root as IClipContainer).GetClip((currentClip == null) ? null : currentClip.id, choice, loopType);
			}
		...
		public class SerializedClip : IClip
		{
			...
			public override string ToString()
			{
				return string.Format("Clip {0} {1}", id, filename);
			}
		}
		...
		public string jsonPath
		{
			get
			{
				return Path.Combine(wavPath, "library.txt");
			}
		}
		...
		public void Save()
		{
			string contents = JsonUtility.ToJson(library, true);
			...
			try
			{
				...
				for (int i = 0; i < directories.Length; i++)
				{
					...
					for (int j = 0; j < files.Length; j++)
					{
						...
						string text;
						string id;
						ParseSamplePath(files[j], out category, out text, out id);
						if (!text.StartsWith("._"))
						{
							SerializedSample serializedSample = library.GetSample(text);
							if (serializedSample == null)
							{
								SerializedSample serializedSample2 = new SerializedSample();
								serializedSample2.category = category;
								serializedSample2.name = text;
