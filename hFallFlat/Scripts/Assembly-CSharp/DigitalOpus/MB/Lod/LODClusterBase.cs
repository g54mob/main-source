		...
		public virtual HashSet<LODCombinedMesh> AdjustForMaxAllowedPerLevel()
		{
			...
			for (int n = 1; n < array.Length; n++)
			{
				foreach (MB2_LOD item in array[n])
				{
					...
					if (n >= item.levels.Length)
					{
						...
						{
							MB2_Log.Log(MB2_LogLevel.trace, string.Format("A Demoting obj in bucket={0} obj={1}", n, item), GetClusterManager().LOG_LEVEL);
						}
					...
					else
					{
						if (GetClusterManager().LOG_LEVEL >= MB2_LogLevel.trace)
						{
							MB2_Log.Log(MB2_LogLevel.trace, string.Format("B Demoting obj in bucket={0} obj={1}", n, item), GetClusterManager().LOG_LEVEL);
						}
						item.AdjustNextLevelIndex(n);
						hashSet.Add(item.GetCombiner());
					}
			...
			foreach (MB2_LOD item2 in hashSet2)
			{
				...
				if (num3 >= item2.levels.Length - 1)
				{
					if (GetClusterManager().LOG_LEVEL >= MB2_LogLevel.trace)
					{
						MB2_Log.Log(MB2_LogLevel.trace, string.Format("C Demoting obj in bucket={0} obj={1}", num3 + 1, item2), GetClusterManager().LOG_LEVEL);
					}
				...
				else
				{
					if (GetClusterManager().LOG_LEVEL >= MB2_LogLevel.trace)
					{
						MB2_Log.Log(MB2_LogLevel.trace, string.Format("D Demoting obj in bucket={0} obj={1}", num3 + 1, item2), GetClusterManager().LOG_LEVEL);
					}
					item2.AdjustNextLevelIndex(num3 + 1);
					hashSet.Add(item2.GetCombiner());
				}
