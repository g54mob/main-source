...
public class DialogOverlay : MonoBehaviour
{
	...
	{
		return (!IsOnIncludingDelay()) ? 0f : instance.background.color.a;
	}
	...
	public static void Show(float opacity, bool showProgress, string title, string description, string backLabel, Action backAction)
	{
		...
		{
			instance.descriptionText.gameObject.SetActive(false);
		}
		else
		{
			instance.descriptionText.gameObject.SetActive(true);
	...
	{
		if (hideDelay <= 0 && MenuSystem.CustomCanInvoke(false, false) && onBack != null)
		{
