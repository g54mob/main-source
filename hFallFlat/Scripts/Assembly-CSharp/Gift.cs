...
public class Gift : GiftBase, INetBehavior
{
	...
	{
		if (base.transform.position.y < netBody.despawnHeight + 10f)
		{
			GiftSpawn randomSpawn = group.GetRandomSpawn();
			Vector3 position = randomSpawn.GetPosition();
			netBody.Respawn(position - netBody.startPos + Vector3.up * Random.Range(20, 30));
