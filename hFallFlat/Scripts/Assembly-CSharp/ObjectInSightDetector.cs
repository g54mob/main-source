...
public class ObjectInSightDetector : Node
{
	...
	private bool IsObjectInSight()
	{
		foreach (Human item in humansInDetectionArea)
		{
			...
			{
				RaycastHit hitInfo;
				Physics.Raycast(gameCam.transform.position, Vector3.Normalize(objectToTrack.transform.position - gameCam.transform.position), out hitInfo, Vector3.Distance(objectToTrack.transform.position, gameCam.transform.position), layerMask.value);
