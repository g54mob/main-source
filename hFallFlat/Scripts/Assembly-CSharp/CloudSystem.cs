...
public class CloudSystem : MonoBehaviour
{
	...
	private void OnEnable()
	{
		...
		mesh = new Mesh();
		mesh.name = "Clouds";
		if (CloudSystem.OnSystemInit != null)
		{
			CloudSystem.OnSystemInit();
		}
	}
	...
	private void GenerateCloud(int index, Vector3 center)
	{
		...
		{
			Vector3 vector = new Vector3(particlesData[j].size / 2f, particlesData[j].size / 2f, particlesData[j].size / 2f);
			bounds.Encapsulate(particlesData[j].pos - vector);
			bounds.Encapsulate(particlesData[j].pos + vector);
		}
