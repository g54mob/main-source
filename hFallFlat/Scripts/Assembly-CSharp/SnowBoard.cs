...
public class SnowBoard : MonoBehaviour, IGrabbable, IPreReset, IRespawnable, IPreRespawn
{
	...
	private void Start()
	{
		...
		SnowBoardManager.Instance.AddSnowBoard(this);
	}
	...
	private void FixedUpdate()
	{
		...
		for (int num4 = 0; num4 < info.Length; num4++)
		{
			if (!(info[num4].transform == body.transform) && !(info[num4].transform == null) && info[num4].transform.gameObject.layer != 9)
			{
				if (vector2.magnitude < vector.magnitude)
				{
					float num5 = Vector3.Dot(info[num4].normal, -base.transform.forward);
					num5 += 1f;
					Debug.DrawLine(info[num4].point, info[num4].point - body.velocity * num5 * 1000f * Time.fixedDeltaTime, Color.green);
					body.AddForceAtPosition(-body.velocity * num5 * 200f, info[num4].point);
				}
				float num6 = Vector3.Dot(body.transform.right, body.velocity.normalized);
				float num7 = Mathf.Lerp(1f, 0f, Mathf.Abs(num6));
				Vector3 vector3 = new Vector3(body.velocity.x, 0f, body.velocity.z);
				vector3.Scale(body.velocity);
				num7 *= vector3.magnitude / 5f;
				float num8 = 0f;
				num8 = ((!(num6 < 0f)) ? Vector3.SignedAngle(body.transform.right, body.velocity, -body.transform.forward) : Vector3.SignedAngle(body.transform.right, body.velocity, body.transform.forward));
				body.SafeAddTorque(-body.transform.forward * num7 * num8);
			}
		}
	}
