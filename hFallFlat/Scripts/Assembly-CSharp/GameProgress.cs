...
public sealed class GameProgress : MonoBehaviour
{
	public sealed class ProgressData
	{
		...
		{
			uint value;
			if (store.TryGetValue(hash, out value) && checkpoint <= value)
			{
				return false;
			}
			store[hash] = checkpoint;
		...
		{
			uint value;
			if (store.TryGetValue(hash, out value))
			{
				return (int)value;
			}
			return -1;
	...
	public GameProgress()
	{
		...
		ulong workshopLevel;
		int checkpoint;
		GameSave.GetWorkshopCheckpoint(out workshopLevel, out checkpoint);
		if (workshopLevel != 0)
		{
