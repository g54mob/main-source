...
public class CollisionSensor : MonoBehaviour
{
	...
	public bool IsGrabbed(GameObject go)
	{
		...
		Transform parent = grabObject.transform;
		while (parent != null)
		{
	...
	{
		if (grabFilter != null || blockGrab > 0f || !(grabJoint == null) || !(blockGrabTrigger == null) || collisionTransform.CompareTag("NoGrab"))
		{
			return;
		}
		if ((targetPosition - contacts[0].point).magnitude > 0.5f)
		{
			float? valueFor = GrabDistanceOverrideManager.GetValueFor(collisionTransform);
			if (!valueFor.HasValue || (targetPosition - contacts[0].point).magnitude > valueFor.Value)
			{
				return;
			}
		}
		...
		if (CheatCodes.climbCheat || human.state != HumanState.Climb || !(otherSide.grabObject == grabObject) || !(base.transform.position.y > otherSide.transform.position.y + handToHandClimb))
		{
	...
	private void FixedUpdate()
	{
		...
		if (CheatCodes.climbCheat || human.state != HumanState.Climb || !(otherSide.grabObject == grabObject) || !(grabPosition.y > otherSide.transform.position.y + handToHandClimb))
		{
			grabJoint = base.gameObject.AddComponent<ConfigurableJoint>();
	...
	private void OnJointBreak(float breakForce)
	{
		...
		{
			Debug.LogError("Joint break force: " + breakForce);
			ReleaseGrab();
		}
	}
}
