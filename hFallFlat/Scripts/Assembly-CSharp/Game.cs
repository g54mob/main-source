...
public class Game : MonoBehaviour, IGame, IDependency
{
	...
	public const int kMaxBuiltInLevels = 32;
	...
	public void UnloadLevel()
	{
		...
		{
			menuCameraEffects.ForceDisableOcclusion(false);
		}
	}
	...
	public void RespawnAllPlayers(NetHost host)
	{
		...
		{
			float num = ((!checkpoint.tightSpawn) ? 2f : 0.5f);
			Respawn(host.players[i].human, Vector3.left * (i % 3) * num + Vector3.back * (i / 3) * num);
		}
	}
	...
	public void RespawnAllPlayers()
	{
		...
		{
			float num = ((!checkpoint.tightSpawn) ? 2f : 0.5f);
			Respawn(Human.all[i], Vector3.left * (i % 3) * num + Vector3.back * (i / 3) * num);
		}
	}
	...
	{
		if (workshopLevel == null && currentLevelType != WorkshopItemSource.EditorPick && (!lobby || multiplayerLobbyLevel <= 32))
		{
			return;
		}
	...
	private IEnumerator LoadLevel(string levelId, ulong levelNumber, int checkpointNumber, int checkpointSubObjectives, Action onComplete, WorkshopItemSource levelType)
	{
		...
		bool isBundle = !string.IsNullOrEmpty(levelId) || (levelNumber > 32 && levelNumber != ulong.MaxValue);
		if (isBundle)
		{
		...
		if (currentLevelNumber != (int)levelNumber)
		{
			...
			if (loader == null)
			{
				...
				while (!sceneLoader.isDone || !instance.HasSceneLoaded)
				{
					yield return null;
				}
				sceneLoader = null;
		...
		if (currentLevelNumber >= 0 && !isBundle && levelType != WorkshopItemSource.EditorPick)
		{
	...
	public void AfterLoad(int checkpointNumber, int subobjectives)
	{
		...
		if ((bool)menuCameraEffects && currentLevelNumber >= 0 && currentLevelNumber < levels.Length)
		{
			...
			{
				menuCameraEffects.ForceDisableOcclusion(true);
			}
			else
			{
				menuCameraEffects.ForceDisableOcclusion(false);
		...
		{
			currentLevel.prerespawn(currentCheckpointNumber, true);
		}
		RespawnAllPlayers();
	...
	public void RestartCheckpoint(int checkpointNumber, int subObjectives)
	{
		if (!currentLevel.respawnLocked)
		{
			...
			if (currentLevel.prerespawn != null)
			{
				currentLevel.prerespawn(checkpointNumber, false);
			}
			RespawnAllPlayers();
			currentLevel.Reset(currentCheckpointNumber, currentCheckpointSubObjectives);
			if (NetGame.isServer)
	...
	public void RestartLevel(bool reset = true)
	{
		...
		if (currentLevel.prerespawn != null)
		{
			currentLevel.prerespawn(-1, false);
		}
		RespawnAllPlayers();
		if (reset)
		{
