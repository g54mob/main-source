...
public class RecordingMovie : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (Input.GetKeyDown(KeyCode.Alpha5))
		{
			...
			if (disableHuman)
			{
				Human.instance.gameObject.SetActive(false);
			}
		}
		if (!playing)
		{
	...
	private void InterpolateCam(RecordingMovieCamera startCam, RecordingMovieCamera endCam)
	{
		if (!(startCam == null) && !(endCam == null))
		{
			...
			Vector3 vector = Vector3.Lerp(startCam.transform.position, endCam.transform.position, t);
			playerCam.transform.position = vector - playerCam.transform.forward * Mathf.Lerp(startCam.dist, endCam.dist, t);
