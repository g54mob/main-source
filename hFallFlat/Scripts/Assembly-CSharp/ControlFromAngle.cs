...
public class ControlFromAngle : Node, IReset
{
	...
	private void FixedUpdate()
	{
		...
		if (target != null)
		{
			IControllable component = target.GetComponent<IControllable>();
			if (component != null && (!(currentValue > 0f) || !(currentValue < 0.0009f)))
			{
				currentValue = (float)Math.Round(currentValue, 5);
				component.SetControlValue(currentValue);
			}
		}
	...
	private float FromEuler(Transform transform)
	{
		if (CalculateAngleCorrectly)
		{
			...
			{
			case Axis.X:
				return Mathf.Atan2(transform.forward.y, transform.forward.z) * 57.29578f;
			case Axis.Y:
			case Axis.Z:
