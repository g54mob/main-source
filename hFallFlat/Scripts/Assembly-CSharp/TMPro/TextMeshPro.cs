		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				else if (!m_isRightToLeft)
				{
					...
					m_xAdvance += ((m_cached_TextElement.xAdvance * num36 * num7 + m_characterSpacing + m_currentFontAsset.normalSpacingOffset) * num2 + m_cSpacing) * (1f - m_charWidthAdjDelta);
					if (char.IsWhiteSpace((char)num4) || num4 == 8203)
					{
						m_xAdvance += m_wordSpacing * num2;
					}
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				{
					num43 = ((j != m_characterCount - 1 || !char.IsLetterOrDigit(character2)) ? (j - 1) : j);
					flag11 = false;
