...
public class SteamRichPresence : MonoBehaviour
{
		...
		case AppSate.PlayLevel:
			...
			if (levelNumber != -1)
			{
				...
				WorkshopRepository.instance.levelRepo.GetLevel((ulong)levelNumber, levelType, delegate(WorkshopLevelMetadata metadata)
				{
					...
					{
						Debug.LogError(string.Format("[Steamworks:RichPresence] Level meta data not found for level number {0} of type {1}", levelNumber, levelType));
					}
				});
			}
			...
			{
				Debug.LogError(string.Format("[Steamworks:RichPresence] Invalid state {0} in {1}", levelType, state));
			}
			break;
	...
	{
		string text = string.Format("({0}/{1})", current, max);
