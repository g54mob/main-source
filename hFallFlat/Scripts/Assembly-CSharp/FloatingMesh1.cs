...
public class FloatingMesh1 : MonoBehaviour
{
	...
	private void ApplyTriangleForces(FloatingVertex a, FloatingVertex b, FloatingVertex c)
	{
		...
		else if (m.depth <= 0f)
		{
			...
			FloatingVertex t2 = new FloatingVertex
			{
				pos = l.pos + num2 * (t.pos - l.pos),
				velocity = l.velocity + num2 * (t.velocity - l.velocity),
				depth = 0f
			};
			FloatingVertex m2 = new FloatingVertex
			{
				pos = l.pos + num * (m.pos - l.pos),
				velocity = l.velocity + num * (m.velocity - l.velocity),
				depth = 0f
			};
		...
		else
		{
			...
			FloatingVertex t3 = new FloatingVertex
			{
				pos = t.pos + num4 * (l.pos - t.pos),
				velocity = t.velocity + num4 * (l.velocity - t.velocity),
				depth = 0f
			};
			FloatingVertex floatingVertex = new FloatingVertex
			{
				pos = t.pos + num3 * (m.pos - t.pos),
				velocity = t.velocity + num3 * (m.velocity - t.velocity),
				depth = 0f
			};
			ApplySumbergedTriangleForces(floatingVertex, m, l, areaNormal);
			ApplySumbergedTriangleForces(t3, floatingVertex, l, areaNormal);
		}
	}
	...
	private void ApplySumbergedTriangleForces(FloatingVertex t, FloatingVertex m, FloatingVertex l, Vector3 areaNormal)
	{
		...
		FloatingVertex floatingVertex = new FloatingVertex
		{
			pos = l.pos + num * (t.pos - l.pos),
			velocity = l.velocity + num * (t.velocity - l.velocity),
			depth = m.depth
		};
		TrianglePointingUp(t, m, floatingVertex, areaNormal);
		TrianglePointingDown(floatingVertex, m, l, areaNormal);
	}
	...
	private void Hydrostatic(FloatingVertex a, FloatingVertex b, FloatingVertex c, Vector3 areaNormal)
	{
		...
		if (!(magnitude2 < 0.001f) && !(magnitude < 0.001f) && !(num < 0.0001f))
		{
			...
			AddForceAtPosition(force, pos2, false);
		}
	}
	...
	private void Hydrodynamic(FloatingVertex a, FloatingVertex b, FloatingVertex c, Vector3 areaNormal)
	{
		if (!(body == null))
		{
			...
			AddForceAtPosition(num2 * vector, floatingVertex.pos, true);
			...
			AddForceAtPosition(num4 * vector, floatingVertex2.pos, true);
			...
			AddForceAtPosition(num6 * vector, floatingVertex3.pos, true);
		}
	}
	...
	private void Hydrodynamic(FloatingVertex v, float surface, Vector3 normal)
	{
		...
		AddForceAtPosition(num2 * normal, v.pos, true);
	}
