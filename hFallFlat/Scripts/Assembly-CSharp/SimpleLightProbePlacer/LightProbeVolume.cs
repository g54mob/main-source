...
namespace SimpleLightProbePlacer
{
	...
	public class LightProbeVolume : TransformVolume
	{
		...
		public Vector3 Density
		{
			get
			{
				return (m_type != LightProbeVolumeType.Fixed) ? m_densityFloat : m_densityFixed;
			}
			set
			{
		...
		{
			return (type != LightProbeVolumeType.Fixed) ? CreatePositionsFloat(base.transform, base.Origin, base.Size, Density) : CreatePositionsFixed(base.transform, base.Origin, base.Size, Density);
		}
		...
		public static List<Vector3> CreatePositionsFixed(Transform volumeTransform, Vector3 origin, Vector3 size, Vector3 density)
		{
			...
			for (int i = 0; (float)i <= density.x; i++)
			{
				for (int j = 0; (float)j <= density.y; j++)
				{
					...
					{
						Vector3 position = vector + new Vector3((float)i * num, (float)j * num2, (float)k * num3);
		...
		public static List<Vector3> CreatePositionsFloat(Transform volumeTransform, Vector3 origin, Vector3 size, Vector3 density)
		{
			...
			vector -= size * 0.5f;
			vector.x += (size.x - (float)num * density.x) * 0.5f;
			vector.y += (size.y - (float)num2 * density.y) * 0.5f;
			vector.z += (size.z - (float)num3 * density.z) * 0.5f;
			for (int i = 0; i <= num; i++)
			{
				for (int j = 0; j <= num2; j++)
				{
					...
					{
						Vector3 position = vector + new Vector3((float)i * density.x, (float)j * density.y, (float)k * density.z);
						position = volumeTransform.TransformPoint(position);
						list.Add(position);
					}
				}
