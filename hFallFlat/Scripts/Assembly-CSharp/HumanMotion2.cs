...
public class HumanMotion2 : MonoBehaviour
{
	...
	public static void AlignLook(HumanSegment segment, Quaternion targetRotation, float accelerationSpring, float damping)
	{
		float angle;
		Vector3 axis;
		(targetRotation * Quaternion.Inverse(segment.transform.rotation)).ToAngleAxis(out angle, out axis);
		if (angle > 180f)
		{
			angle -= 360f;
		}
	...
	public static void AlignToVector(Rigidbody body, Vector3 alignmentVector, Vector3 targetVector, float spring, float maxTorque)
	{
		...
		Vector3 vector2 = vector.normalized * Mathf.Asin(Mathf.Clamp01(vector.magnitude));
		Vector3 vector3 = spring * vector2;
		body.SafeAddTorque(Vector3.ClampMagnitude(vector3, maxTorque));
	}
