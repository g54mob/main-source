...
public static class SwitchAssetBundle
{
	...
	public class LoadingCurrentScene
	{
		...
		private void LoadNextRequired()
		{
			...
			AssetBundleCreateRequest assetBundleCreateRequest = SwitchAssetBundle.LoadBundle(mCurrentBundle.mRequires[mCurrentRequired], out returnedBundle, out mCurrentRequiredName, true);
			mCurrentRequired++;
		...
		{
			AssetBundle returnedBundle;
			AssetBundleCreateRequest assetBundleCreateRequest = SwitchAssetBundle.LoadBundle(mCurrentBundle.mSceneBundle, out returnedBundle, out mCurrentBundleName, true);
			if (assetBundleCreateRequest == null)
			{
				mCurrentPhase = LoadingScenePhase.kSceneLoad;
				return;
	...
	public static void LoadScene(string scene)
	{
		...
		for (int i = 0; i < num; i++)
		{
			LoadBundle(bundleForScene.mRequires[i], out returnedBundle, out variantBundleName, false);
		...
		LoadBundle(bundleForScene.mSceneBundle, out returnedBundle, out variantBundleName, false);
		if (returnedBundle == null)
		{
	...
	private static AssetBundleCreateRequest LoadBundle(string bundleName, out AssetBundle returnedBundle, out string variantBundleName, bool async)
	{
		...
		AssetBundle value;
		if (mLoadedBundles.TryGetValue(bundleName, out value))
		{
	...
	{
		AssetBundle value;
		if (mLoadedBundles.TryGetValue(scene, out value))
		{
			return value;
		}
		return null;
