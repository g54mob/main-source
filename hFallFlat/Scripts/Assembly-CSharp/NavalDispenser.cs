...
public class NavalDispenser : MonoBehaviour, IReset
{
	...
	private void SpawnDebris(int amount = 1)
	{
		...
		if (dispenserState != DispenserState.ReadyToDispense)
		{
			return;
		}
		...
		{
			if (count + num < debrisInstances.Count)
			{
				action();
			}
			else
	...
	private void Start()
	{
		...
		Transform[] componentsInChildren = debrisPool.gameObject.GetComponentsInChildren<Transform>(true);
