...
public class SnowBallGrowth : Node, IPostEndReset, IPreReset
{
	...
	private void FixedUpdate()
	{
		...
		float num = 0f;
		RaycastHit hitInfo;
		if ((InsideIceCaves.instance == null || !InsideIceCaves.instance.SnowballInsideCave(this)) && Physics.Raycast(base.transform.position, Vector3.down, out hitInfo, base.transform.lossyScale.x / 2f + 0.2f, 1))
		{
	...
	public void AdjustHandAnchors(ConfigurableJoint joint, float growth)
	{
		if (!(joint == null) && !(joint.connectedBody != rb))
		{
			...
			vector *= joint.connectedBody.transform.localScale.x * radius / vector.magnitude;
