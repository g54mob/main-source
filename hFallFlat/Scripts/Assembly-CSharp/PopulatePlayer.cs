...
public class PopulatePlayer : MonoBehaviour
{
	...
	private void Awake()
	{
		...
		m_arButtonList = base.gameObject.transform.GetComponentsInChildren<UpdatePlayerStatus>(true);
	...
	private void UpdateAllPlayerSlots()
	{
		...
		for (int i = 0; i < MaxPlayerCount; i++)
		{
			...
			{
				m_arButtonList[i].gameObject.SetActive(true);
				UpdatePlayer(i);
				flag = true;
			}
			else
			{
				m_arButtonList[i].gameObject.SetActive(false);
			}
		}
		UpdateNavigation();
	}
	...
	{
		ChatUserEventArgs e2 = (ChatUserEventArgs)e;
		if (e2 != null)
		{
			UpdatePlayer(e2.Index);
		}
		UpdateAllPlayerSlots();
	}
	...
	private void AddButton()
	{
		...
		{
			UnityEngine.Object.Instantiate(m_PlayerSlotButton, base.transform, false).SetActive(true);
		}
	}
}
