...
public class MB3_TextureBaker : MB3_MeshBakerRoot
{
	...
	private IEnumerator _CreateAtlasesCoroutineAtlases(MB3_TextureCombiner combiner, ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
	{
		...
		for (int j = 0; j < OnCombinedTexturesCoroutineAtlasesAndRects.Length; j++)
		{
			...
			if (_doMultiMaterial)
			{
				sourceMats = resultMaterials[j].sourceMaterials;
				resMatToPass = resultMaterials[j].combinedMaterial;
			...
			yield return combiner.CombineTexturesIntoAtlasesCoroutine(progressInfo, OnCombinedTexturesCoroutineAtlasesAndRects[j], resMatToPass, objsToMesh, sourceMats, editorMethods, coroutineResult2, maxTimePerFrame);
	...
	internal IEnumerator _CreateAtlasesCoroutineTextureArray(MB3_TextureCombiner combiner, ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
	{
		...
		{
			yield return MB_TextureArrays._CreateAtlasesCoroutineSingleResultMaterial(resMatIdx, bakedMatsAndSlices[resMatIdx], resultMaterialsTexArray[resMatIdx], objsToMesh, combiner, textureArrayOutputFormats, resultMaterialsTexArray, customShaderProperties, progressInfo, coroutineResult, saveAtlasesAsAssets, editorMethods, maxTimePerFrame);
			if (!coroutineResult.success)
			{
				yield break;
			}
	...
	private IEnumerator _CreateAtlasesCoroutine(ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
	{
		...
		{
			mb[k].textureBakeResults = textureBakeResults;
		}
		coroutineResult.isFinished = true;
	}
	...
	public MB3_TextureCombiner CreateAndConfigureTextureCombiner()
	{
		...
		mB3_TextureCombiner.maxAtlasHeightOverride = _maxAtlasHeightOverride;
		mB3_TextureCombiner.maxAtlasWidthOverride = _maxAtlasWidthOverride;
		mB3_TextureCombiner.useMaxAtlasHeightOverride = _useMaxAtlasHeightOverride;
		mB3_TextureCombiner.useMaxAtlasWidthOverride = _useMaxAtlasWidthOverride;
		...
		mB3_TextureCombiner.layerTexturePackerFastMesh = _layerTexturePackerFastMesh;
		mB3_TextureCombiner.resultType = _resultType;
