...
public class CursorController : ActiveComponent
{
	...
	{
		if (ActiveComponent.Model == null || (ActiveComponent.Model.construction != null && (ActiveComponent.Model.construction.attached != null || ActiveComponent.Model.currentChain != null)))
		{
			return;
		}
		if (!curImg.enabled)
	...
	private void Update()
	{
		...
		if (Time.unscaledTime - vibrateStart < vibrateTime)
		{
			ActiveComponent.Program.joyInput.Vibrate();
		}
		else
		{
			ActiveComponent.Program.joyInput.Vibrate(vibrate: false);
		}
		if (currentCanvas == null || !currentCanvas.gameObject.activeInHierarchy)
		{
		...
		if (Input.touchCount > 0)
		{
			if (curGo != null && curGo.activeInHierarchy && curPointerExitHandler != null)
			{
				IPointerExitHandler[] array = curPointerExitHandler;
				for (int i = 0; i < array.Length; i++)
				{
		...
		if (input.cursorMove)
		{
			...
			delta = vector - mainCam.WorldToScreenPoint(position2);
			...
			if (curGo != null && !input.dragStart && !input.drag && !input.dragEnd)
			{
				...
				if (vector3.magnitude < num)
				{
					...
					if ((double)((vector2.x * vector4.x + vector2.y * vector4.y) / vector2.magnitude / vector4.magnitude) > 0.2)
					{
						...
						pos = position2 + vector4 * 0.5f + zero * 0.5f;
						pos.z = 0f;
						pos.x = Mathf.Max(rect.xMin, Mathf.Min(pos.x, rect.xMax));
						pos.y = Mathf.Max(rect.yMin, Mathf.Min(pos.y, rect.yMax));
						base.transform.position = pos;
		...
		else
		{
			if (curImg.enabled && !input.dragStart && !input.drag && !input.dragEnd)
			{
				if (checkCounter % checkRate == 0 && OnDefaultCanvas())
				{
					...
					GameObject go2 = rayController.RayCastFromPoint(position3);
					UpdateCurGo(go2);
					checkCounter = 1;
				}
				checkCounter++;
		...
		if ((input.dragStart || input.drag || input.dragEnd) && OnDefaultCanvas())
		{
			...
			{
				Vector3 position4 = mainCam.WorldToScreenPoint(base.transform.position);
				GameObject go3 = rayController.RayCastFromPoint(position4, dragCheck: true);
				UpdateCurGo(go3);
			}
			if (curGo != null)
			{
				if (curBar == null && curSlider == null)
				{
					...
					if (input.drag && curDragHandler != null)
					{
						...
						IDragHandler[] array2 = curDragHandler;
						for (int i = 0; i < array2.Length; i++)
						{
							array2[i].OnDrag(pointerEventData2);
						}
					}
					if (input.dragEnd && curEndDragHandler != null)
					{
						...
						IEndDragHandler[] array3 = curEndDragHandler;
						for (int i = 0; i < array3.Length; i++)
						{
							array3[i].OnEndDrag(pointerEventData3);
						}
					}
				}
				else
		...
		else
		{
			...
			if (ActiveComponent.Program.joyInput.lmbDown)
			{
				if (curGo != null)
				{
					...
					if (curDropdown == null || OnDefaultCanvas())
					{
