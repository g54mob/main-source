...
public class from_field_to_text : MonoBehaviour
{
	public class tdtrainer_options
	{
		...
		public void Save(string name)
		{
			...
			PlayerPrefs.SetInt(name + ".batch_size", batch_size);
			PlayerPrefs.SetString(name + ".learning_rate", learning_rate.ToString());
			PlayerPrefs.SetString(name + ".momentum", momentum.ToString());
			PlayerPrefs.SetString(name + ".l2_decay", l2_decay.ToString());
		}
	...
	public class deepqlearn
	{
		public class Experience
		{
			...
			{
				PlayerPrefs.SetString(name + ".WasSaved", "t");
				PlayerPrefs.SetInt(name + ".action0", action0);
				PlayerPrefs.SetInt(name + ".state0.Length", state0.Length);
				for (int i = 0; i < state0.Length; i++)
				{
					PlayerPrefs.SetString(name + ".state0[" + i + "]", state0[i].ToString());
				}
				PlayerPrefs.SetInt(name + ".state1.Length", state1.Length);
				for (int j = 0; j < state1.Length; j++)
				{
					PlayerPrefs.SetString(name + ".state1[" + j + "]", state1[j].ToString());
				}
				PlayerPrefs.SetString(name + ".reward0", reward0.ToString());
			}
			...
			public bool Load(string name)
			{
				if (PlayerPrefs.HasKey(name + ".WasSaved") && PlayerPrefs.GetString(name + ".WasSaved") == "t")
				{
					action0 = PlayerPrefs.GetInt(name + ".action0");
					state0 = new double[PlayerPrefs.GetInt(name + ".state0.Length")];
					for (int i = 0; i < state0.Length; i++)
					{
						try
						{
							state0[i] = Convert.ToDouble(PlayerPrefs.GetString(name + "state0[" + i + "]"));
						}
						catch
						{
							state0[i] = 0.0;
					...
					state1 = new double[PlayerPrefs.GetInt(name + ".state1.Length")];
					for (int j = 0; j < state1.Length; j++)
					{
						try
						{
							state1[j] = Convert.ToDouble(PlayerPrefs.GetString(name + "state1[" + j + "]"));
						}
						catch
						{
							state1[j] = 0.0;
					...
					reward0 = Convert.ToDouble(PlayerPrefs.GetString(name + ".reward0"));
					return true;
				}
				return false;
			}
		...
		public class window
		{
			...
			{
				PlayerPrefs.SetString(name + ".WasSaved", "t");
				PlayerPrefs.SetInt(name + ".n", n);
				PlayerPrefs.SetInt(name + ".avav.Count", avav.Count);
				for (int i = 0; i < avav.Count; i++)
				{
					PlayerPrefs.SetString(name + ".avav[" + i + "]", avav[i].ToString());
				}
			}
			...
			public bool Load(string name)
			{
				if (PlayerPrefs.HasKey(name + ".WasSaved") && PlayerPrefs.GetString(name + ".WasSaved") == "t")
				{
					n = PlayerPrefs.GetInt(name + ".n");
					avav.Clear();
					for (int i = 0; i < n; i++)
					{
						avav.Add(Convert.ToDouble(PlayerPrefs.GetString(name + ".avav[" + i + "]")));
					}
					return true;
				}
				return false;
		...
		public class Brain
		{
			...
			public bool Load(string name)
			{
				if (PlayerPrefs.HasKey(name + ".WasSaved") && PlayerPrefs.GetString(name + ".WasSaved") == "t")
				{
					if (!value_net.Load(name + ".value_net"))
					{
						return false;
					}
			...
			public int forward(double[] input_array)
			{
				...
				if (forward_passes > temporal_window)
				{
					...
					num = ((!((double)UnityEngine.Random.Range(0f, 1f) < epsilon)) ? policy(netInput).action : random_action());
				}
				else
				{
			...
			public void backward(double reward)
			{
				...
				if (forward_passes > temporal_window + 1)
				{
					...
					{
						int num2 = UnityEngine.Random.Range(0, experience_size);
				...
				if (this.experience.Length > start_learn_threshold)
				{
					...
					{
						int num4 = UnityEngine.Random.Range(0, this.experience.Length);
	...
	public void study()
	{
		...
		Debug.Log(inputsss(0, 0));
		Debug.Log(inputsss(0, 1));
		Debug.Log(inputsss(1, 0));
		Debug.Log(inputsss(1, 1));
	}
	...
	public void reward()
	{
		double num = Convert.ToDouble(rew.GetComponent<Text>().text);
		brain.backward(num);
	}

	public void rewardsss(double reward)
	{
	...
	public void input()
	{
		...
		{
			Convert.ToDouble(in1.GetComponent<Text>().text),
			Convert.ToDouble(in2.GetComponent<Text>().text)
		};
		brain.forward(input_array);
	}

	public int inputsss(int x1, int x2)
	{
		double[] input_array = new double[2] { x1, x2 };
		return brain.forward(input_array);
	}
	...
	public void init(int inps, int acs, int temp)
	{
		...
		tdtrainer_options2.learning_rate = 0.02;
		tdtrainer_options2.momentum = 0.0;
		tdtrainer_options2.batch_size = 64;
		tdtrainer_options2.l2_decay = 0.01;
