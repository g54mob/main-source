...
public class Rosenblat : BaseBlock
{
	...
	[SceneBind("ErrorGlow")]
	public RectTransform ErrorGlow;
	...
	protected override void Active()
	{
		...
		if (BlockRandom.NextDouble() > (double)error)
		{
			...
			if (!IsTrained())
			{
				error = Mathf.Max(0.2f, Mathf.Min(1f, error + element.error));
			}
			else
			{
				error = 0.2f;
	...
	public override void Redraw()
	{
		...
		if (IsTrained())
		{
			ErrorGlow.gameObject.SetActive(value: false);
		}
	}
	...
	public override void Init()
	{
		...
		Redraw();
		ErrorGlow.gameObject.SetActive(QuestLine.IsLoadedInMemory("ROSENBLATT_LEARN") && !QuestLine.GetQuest("ROSENBLATT_LEARN").IsCompleted());
		if (IsTrained())
		{
			ErrorGlow.gameObject.SetActive(value: false);
		}
