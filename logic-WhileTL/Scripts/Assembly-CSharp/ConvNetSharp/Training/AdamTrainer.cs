		...
		protected override void TrainImplem()
		{
			...
			for (int i = 0; i < parametersAndGradients.Count; i++)
			{
				...
				for (int j = 0; j < num5; j++)
				{
					...
					array[j] = array[j] * Beta1 + (1.0 - Beta1) * num7;
					array2[j] = array2[j] * Beta2 + (1.0 - Beta2) * num7 * num7;
					double num8 = array[j] * (1.0 - Math.Pow(Beta1, K));
					double d = array2[j] * (1.0 - Math.Pow(Beta2, K));
					double num9 = (0.0 - LearningRate) * num8 / (Math.Sqrt(d) + Eps);
					parameters[j] += num9;
					gradients[j] = 0.0;
				}
			}
