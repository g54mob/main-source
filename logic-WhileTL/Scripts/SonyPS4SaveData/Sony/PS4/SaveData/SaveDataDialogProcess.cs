...
namespace Sony.PS4.SaveData
{
	public class SaveDataDialogProcess
	{
		...
		public static IEnumerator StartSaveDialogProcess(int userId, Dialogs.NewItem newItem, DirName newDirName, ulong newSaveDataBlocks, SaveDataParams saveDataParams, FileOps.FileOperationRequest fileRequest, FileOps.FileOperationResponse fileResponse, bool backup)
		{
			...
			while (true)
			{
				...
				case SaveState.ShowListStart:
					errorCode = ListDialog(userId, Dialogs.DialogType.Save, openDialogResponse, searchResponse, newItem);
