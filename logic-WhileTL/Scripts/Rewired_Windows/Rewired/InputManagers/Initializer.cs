...
namespace Rewired.InputManagers
{
	...
	internal class Initializer : PlatformInitializer
	{
		...
		private static PlatformInitializer NuBDdKUJCoFOxHtcBDZbzmwuhjdM;

		public static PlatformInitializer GetPlatformInitializer()
		{
			if (NuBDdKUJCoFOxHtcBDZbzmwuhjdM == null)
			{
				NuBDdKUJCoFOxHtcBDZbzmwuhjdM = new Initializer();
			}
			return NuBDdKUJCoFOxHtcBDZbzmwuhjdM;
		}
		...
		public override object Initialize(IConfigVars_Internal configVars)
		{
			if (UnityTools.platform == Platform.Windows || UnityTools.platform == Platform.WindowsAppStore)
			{
				try
				{
					return new ZvXGRXARsRAyWcsebKvThCOsAbjHb((ConfigVars)configVars, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId);
				}
				catch (Exception)
				{
		...
		public override IElementIdentifierTool CreateTool(string inputSourceString)
		{
			...
			{
				return new CwQQgszhjJTqRGLxFXKCQvGLnAWo();
			}
			if (inputSourceString == "RawInput")
			{
				return new mRshZYvRPZEOqsKKDOIjEclUqAru();
			}
			return null;
		}
	}
