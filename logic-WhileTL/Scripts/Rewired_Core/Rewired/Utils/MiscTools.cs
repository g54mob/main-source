...
namespace Rewired.Utils
{
	...
	{
		private static uint CtGzPoIctQiidDEiePJnvCrAxiGH = 0u;

		private static uint ZKmArVWTfILISLzRUrzFbHSzkDyN = 1u;

		private static uint jfjGaMsnKzDYHegJLMgTfmjWeqpv = uint.MaxValue;

		private static int ZJuFCaQnbrazvYnqRMEDAGnfMUWK = 0;

		private static int RaeKyBkwReGbEsorizVMSmLZZUVT = int.MaxValue;

		private static int lbyBzXQtQNIranlwCJBvNgDwVTeu = -1;
		...
		{
			counter = ((counter != jfjGaMsnKzDYHegJLMgTfmjWeqpv) ? (counter + 1) : ZKmArVWTfILISLzRUrzFbHSzkDyN);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != RaeKyBkwReGbEsorizVMSmLZZUVT) ? (counter + 1) : ZJuFCaQnbrazvYnqRMEDAGnfMUWK);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != ZKmArVWTfILISLzRUrzFbHSzkDyN) ? (counter - 1) : jfjGaMsnKzDYHegJLMgTfmjWeqpv);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= lbyBzXQtQNIranlwCJBvNgDwVTeu)
			{
				return RaeKyBkwReGbEsorizVMSmLZZUVT;
			}
			counter = ((counter != ZJuFCaQnbrazvYnqRMEDAGnfMUWK) ? (counter - 1) : RaeKyBkwReGbEsorizVMSmLZZUVT);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != CtGzPoIctQiidDEiePJnvCrAxiGH;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > lbyBzXQtQNIranlwCJBvNgDwVTeu;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == CtGzPoIctQiidDEiePJnvCrAxiGH)
			{
				return false;
			}
			if (tick2 == CtGzPoIctQiidDEiePJnvCrAxiGH)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= lbyBzXQtQNIranlwCJBvNgDwVTeu)
			{
				return false;
			}
			if (tick2 <= lbyBzXQtQNIranlwCJBvNgDwVTeu)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = jfjGaMsnKzDYHegJLMgTfmjWeqpv - num + num2;
			uint zKmArVWTfILISLzRUrzFbHSzkDyN = ZKmArVWTfILISLzRUrzFbHSzkDyN;
			uint num4 = num3 - zKmArVWTfILISLzRUrzFbHSzkDyN;
			if (tick1 >= tick2)
			{
				return 0L - (long)num4;
			}
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = RaeKyBkwReGbEsorizVMSmLZZUVT - num + num2;
			int zJuFCaQnbrazvYnqRMEDAGnfMUWK = ZJuFCaQnbrazvYnqRMEDAGnfMUWK;
			int num4 = num3 - zJuFCaQnbrazvYnqRMEDAGnfMUWK;
			if (tick1 >= tick2)
			{
				return -num4;
			}
