...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> paxalbbuvifbrduxOpNoQoZPYCof;

		private static Action<object, Action> invokeDelegate => NZnspyinGFmRpItAdHqFcoBaglpEA;
		...
		private static void NZnspyinGFmRpItAdHqFcoBaglpEA(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	internal sealed class SafeAction<T> : SafeDelegate<Action<T>>
	{
		...
		private static Action<object, Action<T>> paxalbbuvifbrduxOpNoQoZPYCof;

		private static Action<object, Action<T>> invokeDelegate => NZnspyinGFmRpItAdHqFcoBaglpEA;
		...
		private static void NZnspyinGFmRpItAdHqFcoBaglpEA(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.hbLihcaqzyiPPYHibtwglUIvpwkXA);
			}
		}
	...
	internal sealed class SafeAction<T, T2> : SafeDelegate<Action<T, T2>>
	{
		...
		private T2 KdfqGdTNbUoHJHjTzXEpwMFJAQAeA;

		private static Action<object, Action<T, T2>> paxalbbuvifbrduxOpNoQoZPYCof;

		private static Action<object, Action<T, T2>> invokeDelegate => NZnspyinGFmRpItAdHqFcoBaglpEA;
		...
		{
			hbLihcaqzyiPPYHibtwglUIvpwkXA = arg0;
			KdfqGdTNbUoHJHjTzXEpwMFJAQAeA = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			hbLihcaqzyiPPYHibtwglUIvpwkXA = default(T);
			KdfqGdTNbUoHJHjTzXEpwMFJAQAeA = default(T2);
		}
		...
		private static void NZnspyinGFmRpItAdHqFcoBaglpEA(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.hbLihcaqzyiPPYHibtwglUIvpwkXA, safeAction.KdfqGdTNbUoHJHjTzXEpwMFJAQAeA);
			}
		}
