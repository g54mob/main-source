...
namespace Rewired.Utils
{
	...
	{
		private static string vLmeDSynrICOqkPbYeydgHEOsLGF;
		...
		static StringTools()
		{
			vLmeDSynrICOqkPbYeydgHEOsLGF = Regex.Escape(new string(Path.GetInvalidFileNameChars()));
		}

		public static string CleanUpFileName(string name)
		{
			name = name.Trim();
			string pattern = "[ ~`,:;'\\.\\$\\^\\{\\}\\[\\]\\(\\|\\)\\*\\+\\?\\\\" + vLmeDSynrICOqkPbYeydgHEOsLGF + "]";
