...
namespace Rewired.Utils.Libraries.TinyJson
{
	...
	{
		[Serializable]
		private sealed class XLrimiQlhiKQhRzmyBmaLDVuGDlh
		{
			public static readonly XLrimiQlhiKQhRzmyBmaLDVuGDlh _003C_003E9 = new XLrimiQlhiKQhRzmyBmaLDVuGDlh();
			...
			internal bool tfDQsySlHERNvKuROmuCDEVLbXQGA(FieldInfo P_0)
			{
			...
			internal string JbEBGgCtMClPkRdJVrTuDXCAGGUvA(FieldInfo P_0)
			{
				string name;
			...
			internal bool sgmfSsFYlHVyEFWGnbKBjTCgJqEYB(PropertyInfo P_0)
			{
			...
			internal string TGeOdmyuUZrtXhEXLUlpFjPldyJFA(PropertyInfo P_0)
			{
				string name;
				if (P_0.IsDefined(typeof(SerializeAttribute), inherit: true) && !string.IsNullOrEmpty(name = (CollectionTools.GetValue(P_0.GetCustomAttributes(typeof(SerializeAttribute), inherit: true), 0) as SerializeAttribute).Name))
				{
		...
		private static StringBuilder RjJCZtImRRiYaPuOkInVFCeXWzBf = new StringBuilder();
		...
		internal static bool TryFromJson<T>(string json, out T value, Type preferredAnonymousObjectType)
		{
			try
			{
				...
				RjJCZtImRRiYaPuOkInVFCeXWzBf.Length = 0;
				for (int i = 0; i < json.Length; i++)
				{
					char c = json[i];
					if (c == '"')
					{
						i = caQQNfuaMDFaYcxGPbkErlMOvoCr(true, i, json);
					}
					else if (!char.IsWhiteSpace(c))
					{
						RjJCZtImRRiYaPuOkInVFCeXWzBf.Append(c);
					}
				}
				value = (T)MJDagqelWrNHGgozCuRcDjuWusjxA(typeof(T), RjJCZtImRRiYaPuOkInVFCeXWzBf.ToString(), preferredAnonymousObjectType, out var _);
				return true;
			}
			catch
			{
		...
		internal static T FromJson<T>(string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return (T)MJDagqelWrNHGgozCuRcDjuWusjxA(typeof(T), RjJCZtImRRiYaPuOkInVFCeXWzBf.ToString(), preferredAnonymousObjectType, out flag);
		}
		...
		internal static object FromJson(Type type, string json, Type preferredAnonymousObjectType)
		{
			...
			bool flag;
			return MJDagqelWrNHGgozCuRcDjuWusjxA(type, RjJCZtImRRiYaPuOkInVFCeXWzBf.ToString(), preferredAnonymousObjectType, out flag);
		}

		private static object MJDagqelWrNHGgozCuRcDjuWusjxA(Type P_0, string P_1, Type P_2, out bool P_3)
		{
			...
			if ((object)P_0 == typeof(Guid))
			{
				...
				{
					bool flag;
					string g = (string)MJDagqelWrNHGgozCuRcDjuWusjxA(typeof(string), P_1, P_2, out flag);
					if (!flag)
					{
						P_3 = false;
						return Guid.Empty;
			...
			{
				object obj2 = MJDagqelWrNHGgozCuRcDjuWusjxA(ReflectionTools.GetUnderlyingEnumType(P_0), P_1, P_2, out var flag2);
				...
				try
				{
					obj2 = MJDagqelWrNHGgozCuRcDjuWusjxA(typeof(string), P_1, P_2, out flag2);
			...
			{
				return LbcXNuTVveOfhJLofhgkejdHwBiNA(P_1, P_2, out P_3);
			}
			if (ReflectionTools.IsArray(P_0))
			{
				...
				List<string> list = PXMnSNpiLSNEPgUgzNMHRrlqOuHP(P_1);
				Array array = Array.CreateInstance(elementType, list.Count);
				for (int i = 0; i < list.Count; i++)
				{
					array.SetValue(MJDagqelWrNHGgozCuRcDjuWusjxA(elementType, list[i], P_2, out var _), i);
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(List<>))
			{
				...
				List<string> list3 = PXMnSNpiLSNEPgUgzNMHRrlqOuHP(P_1);
				for (int j = 0; j < list3.Count; j++)
				{
					list2.Add(MJDagqelWrNHGgozCuRcDjuWusjxA(type, list3[j], P_2, out var _));
				}
			...
			if (flag4 && (object)P_0.GetGenericTypeDefinition() == typeof(Dictionary<, >))
			{
				...
				List<string> list4 = PXMnSNpiLSNEPgUgzNMHRrlqOuHP(P_1);
				try
				{
					if (list4.Count % 2 != 0)
					{
					...
					for (int k = 0; k < list4.Count; k += 2)
					{
						if (list4[k].Length > 2)
						{
							...
							bool flag6;
							object value = MJDagqelWrNHGgozCuRcDjuWusjxA(type3, list4[k + 1], P_2, out flag6);
			...
			{
				P_3 = true;
				return ISeZyXvUMNdJcBBbPdAKhGrtELBo(P_0, P_1, P_2);
			}
			P_3 = false;
			return null;
		}

		private static object LbcXNuTVveOfhJLofhgkejdHwBiNA(string P_0, Type P_1, out bool P_2)
		{
			if (P_0.Length == 0)
			{
				P_2 = false;
			...
			{
				List<string> list = PXMnSNpiLSNEPgUgzNMHRrlqOuHP(P_0);
				try
				{
					if (list.Count % 2 != 0)
					{
					...
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddKeyValue<string, object>)))
					{
						...
						{
							addKeyValue.Add(list[i].Substring(1, list[i].Length - 2), LbcXNuTVveOfhJLofhgkejdHwBiNA(list[i + 1], P_1, out var _));
						}
						P_2 = true;
						return addKeyValue;
					}
					...
					{
						dictionary.Add(list[j].Substring(1, list[j].Length - 2), LbcXNuTVveOfhJLofhgkejdHwBiNA(list[j + 1], P_1, out var _));
					}
					P_2 = true;
					return dictionary;
				}
			...
			if (P_0[0] == '[' && P_0[P_0.Length - 1] == ']')
			{
				List<string> list2 = PXMnSNpiLSNEPgUgzNMHRrlqOuHP(P_0);
				try
				{
					if ((object)P_1 != null && ReflectionTools.DoesTypeImplement(P_1, typeof(IAddValue<object>)))
					{
						...
						{
							addValue.Add(LbcXNuTVveOfhJLofhgkejdHwBiNA(list2[k], P_1, out var _));
						}
						P_2 = true;
						return addValue;
					}
					...
					{
						list3.Add(LbcXNuTVveOfhJLofhgkejdHwBiNA(list2[l], P_1, out var _));
					}
					P_2 = true;
					return list3;
				}
		...
		private static object ISeZyXvUMNdJcBBbPdAKhGrtELBo(Type P_0, string P_1, Type P_2)
		{
			...
			try
			{
				...
				{
					value = ReflectionTools.GetFields(P_0, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic).Where(XLrimiQlhiKQhRzmyBmaLDVuGDlh._003C_003E9.tfDQsySlHERNvKuROmuCDEVLbXQGA).ToDictionary(XLrimiQlhiKQhRzmyBmaLDVuGDlh._003C_003E9.JbEBGgCtMClPkRdJVrTuDXCAGGUvA);
					SIpifCnSchXqUoCDmjIsFEzoasXbb.Add(P_0, value);
				}
				if (!mZcAZEwdQDfJsbcJuwyOHwutISGUA.TryGetValue(P_0, out var value2))
				{
					value2 = ReflectionTools.GetProperties(P_0, ReflectionTools.BindingFlags.Instance | ReflectionTools.BindingFlags.Public | ReflectionTools.BindingFlags.NonPublic).Where(XLrimiQlhiKQhRzmyBmaLDVuGDlh._003C_003E9.sgmfSsFYlHVyEFWGnbKBjTCgJqEYB).ToDictionary(XLrimiQlhiKQhRzmyBmaLDVuGDlh._003C_003E9.TGeOdmyuUZrtXhEXLUlpFjPldyJFA);
				...
				for (int i = 0; i < list.Count; i += 2)
				{
					if (list[i].Length > 2)
					{
						...
						{
							value3.SetValue(obj, MJDagqelWrNHGgozCuRcDjuWusjxA(value3.FieldType, text, P_2, out var _));
						}
						else if (value2.TryGetValue(key, out value4) && value4.CanWrite)
						{
							value4.SetValue(obj, MJDagqelWrNHGgozCuRcDjuWusjxA(value4.PropertyType, text, P_2, out var _), null);
						}
					}
				}
		...
		private static int caQQNfuaMDFaYcxGPbkErlMOvoCr(bool P_0, int P_1, string P_2)
		{
			RjJCZtImRRiYaPuOkInVFCeXWzBf.Append(P_2[P_1]);
			for (int i = P_1 + 1; i < P_2.Length; i++)
			{
				if (P_2[i] == '\\')
				{
					if (P_0)
					{
						RjJCZtImRRiYaPuOkInVFCeXWzBf.Append(P_2[i]);
					}
					RjJCZtImRRiYaPuOkInVFCeXWzBf.Append(P_2[i + 1]);
					i++;
				}
				else
				{
		...
		private static List<string> PXMnSNpiLSNEPgUgzNMHRrlqOuHP(string P_0)
		{
			...
			for (int i = 1; i < P_0.Length - 1; i++)
			{
				...
				case '}':
					...
					break;
				case '"':
					i = caQQNfuaMDFaYcxGPbkErlMOvoCr(true, i, P_0);
					continue;
				case ',':
				case ':':
					if (num == 0)
					{
						list.Add(RjJCZtImRRiYaPuOkInVFCeXWzBf.ToString());
						RjJCZtImRRiYaPuOkInVFCeXWzBf.Length = 0;
						continue;
					}
					break;
				}
				RjJCZtImRRiYaPuOkInVFCeXWzBf.Append(P_0[i]);
			}
			if (RjJCZtImRRiYaPuOkInVFCeXWzBf.Length == 0)
			{
				return list;
			}
			list.Add(RjJCZtImRRiYaPuOkInVFCeXWzBf.ToString());
