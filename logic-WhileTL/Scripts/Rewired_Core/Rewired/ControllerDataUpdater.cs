...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] gPTjOlfqqFhEAzqaBzblODdzGfRZ;
		...
		public ControllerDataUpdater(InputSource P_0, int P_1, int P_2, UnknownControllerHat[] P_3)
		{
			...
			axisCount = P_1;
			buttonCount = P_2;
			gPTjOlfqqFhEAzqaBzblODdzGfRZ = P_3;
		...
		{
			if (gPTjOlfqqFhEAzqaBzblODdzGfRZ == null)
			{
				return false;
			}
			for (int i = 0; i < gPTjOlfqqFhEAzqaBzblODdzGfRZ.Length; i++)
			{
				if (gPTjOlfqqFhEAzqaBzblODdzGfRZ[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < gPTjOlfqqFhEAzqaBzblODdzGfRZ.Length; i++)
			{
				if (gPTjOlfqqFhEAzqaBzblODdzGfRZ[i].ContainsButtonIndex(buttonIndex))
				{
					return gPTjOlfqqFhEAzqaBzblODdzGfRZ[i].GetButtons();
				}
			}
			return null;
		}
