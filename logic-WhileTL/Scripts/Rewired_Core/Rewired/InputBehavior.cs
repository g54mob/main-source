...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			fcqmDUxgAQsmARmFaWRZEhNbgirp(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return bttnuBpnLTDsfUymVGBRTFCOSIpp().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				MoPxkLbXJUzTnvkSIWUSBnnSbase(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return bttnuBpnLTDsfUymVGBRTFCOSIpp().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				MoPxkLbXJUzTnvkSIWUSBnnSbase(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			fcqmDUxgAQsmARmFaWRZEhNbgirp(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.nNMZEBuegSCRzyePfHlgaJhIrLQmA(_id);
			if (inputBehavior != null)
			{
				fcqmDUxgAQsmARmFaWRZEhNbgirp(inputBehavior, this, true);
			}
		}
		...
		internal SerializedObject bttnuBpnLTDsfUymVGBRTFCOSIpp()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.adZRTZDsgqtDqZBIYAKuebvqeDeUA
			{
				...
				pWbMhcBQKZEHHDwvEOhqpAUJhzfpA = 5.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.adZRTZDsgqtDqZBIYAKuebvqeDeUA
			{
				DBsVPUbyEmkoGqiATtBbUGsLwABr = "id",
				pWbMhcBQKZEHHDwvEOhqpAUJhzfpA = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.adZRTZDsgqtDqZBIYAKuebvqeDeUA
			{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.adZRTZDsgqtDqZBIYAKuebvqeDeUA
			{
				...
				OTermNiKyMWnSeUawIBObeynBxKj = null,
				pWbMhcBQKZEHHDwvEOhqpAUJhzfpA = string.Format("{0} {1}{2}{3}{4}{5}", "http://guavaman.com/rewired", "http://guavaman.com/schemas/rewired/", "1.4", "/", GetType().Name, ".xsd")
			});
