...
namespace Rewired.HID
{
	...
	internal class HIDTouchpad : HIDControllerElement
	{
		...
		public HIDTouchpad(byte P_0, TouchpadInfo P_1, HIDInfo P_2, Action<NativeBuffer, TouchData[]> P_3)
			: base(P_0, P_2)
		{
			sgVxbuDAuevAQEggkQAcSuZkVnGc = P_1;
			beJEcrXOTGYSxTusyERABNLRUOHi = P_3;
			mDqiKvMaAIvunncObOPHDSgwZkMf = new Queue<TouchData>(10);
			DNCCJTDFxIJHgCLqGnKRlJgQRYjTB = new TouchData[P_1.maxTouches];
		...
		public override void UpdateValue(NativeBuffer inputReport, double timestamp)
		{
			...
			beJEcrXOTGYSxTusyERABNLRUOHi(inputReport, DNCCJTDFxIJHgCLqGnKRlJgQRYjTB);
			lock (mDqiKvMaAIvunncObOPHDSgwZkMf)
			{
				for (int i = 0; i < sgVxbuDAuevAQEggkQAcSuZkVnGc.maxTouches; i++)
				{
					mDqiKvMaAIvunncObOPHDSgwZkMf.Enqueue(DNCCJTDFxIJHgCLqGnKRlJgQRYjTB[i]);
				}
			}
			ProcessQueue();
		}
		...
		public void ProcessQueue()
		{
			...
			lock (mDqiKvMaAIvunncObOPHDSgwZkMf)
			{
				int num2 = mDqiKvMaAIvunncObOPHDSgwZkMf.Count;
				while (num2 > 0)
				{
					TouchData data = mDqiKvMaAIvunncObOPHDSgwZkMf.Dequeue();
					num2--;
					sgVxbuDAuevAQEggkQAcSuZkVnGc.CalculateTouch(ref data);
					values[num] = data;
					num++;
				}
			}
