...
namespace Rewired
{
	...
	{
		internal class nwmaXXBRLHdsFSHrcaeHCCJdihJCc
		{
			public bool zbiIUygVHQtgeEPXnokrnmpIrTVg;

			public bool ArMQbUFpwVdPlYYZZLrAwYBhHAAy;

			public qmEcFrQmpjkfExHMGEuMliRmNVKH[] zcqaKwvyXhAGLtSnQTXsWmqKHjoD;

			public nwmaXXBRLHdsFSHrcaeHCCJdihJCc(bool P_0, bool P_1, qmEcFrQmpjkfExHMGEuMliRmNVKH[] P_2)
			{
				zbiIUygVHQtgeEPXnokrnmpIrTVg = P_0;
				ArMQbUFpwVdPlYYZZLrAwYBhHAAy = P_1;
				zcqaKwvyXhAGLtSnQTXsWmqKHjoD = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.HRCOHrbICjYrmPufacvtvotxDTAp)
					{
		...
		private bool lcutRjqFBODUwRTDtvYzMdLoGTsx = true;

		private Player UvBXHObDlZYGHHCzDkZZTJyJLvx;

		private nwmaXXBRLHdsFSHrcaeHCCJdihJCc MhmCTAhiehkWjtxhLprkaiHIUdqTA;

		private readonly int TcEXPUvjqSTMTFutCAtGRnMeNwub;

		private List<RuleSet> PblvdecObnfYBDkGUhZbCLLZIHRJA;

		private Action ZbQHhMApvlPgeYMRTNSOaEHDVeMnA;
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return lcutRjqFBODUwRTDtvYzMdLoGTsx;
			}
			set
			{
				lcutRjqFBODUwRTDtvYzMdLoGTsx = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return PblvdecObnfYBDkGUhZbCLLZIHRJA;
			}
			set
			{
				if (value == null)
				...
				PblvdecObnfYBDkGUhZbCLLZIHRJA = value;
			}
		}

		internal event Action JqrISrmXfbadXiBbijoAhvOtGFrGb
		{
			add
			{
				ZbQHhMApvlPgeYMRTNSOaEHDVeMnA = (Action)Delegate.Combine(ZbQHhMApvlPgeYMRTNSOaEHDVeMnA, b);
			}
			remove
			{
				ZbQHhMApvlPgeYMRTNSOaEHDVeMnA = (Action)Delegate.Remove(ZbQHhMApvlPgeYMRTNSOaEHDVeMnA, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, nwmaXXBRLHdsFSHrcaeHCCJdihJCc P_1)
		{
			if (P_0 == null)
			{
			...
			MhmCTAhiehkWjtxhLprkaiHIUdqTA = P_1;
		}
		...
		public void Apply()
		{
			...
			ZbQHhMApvlPgeYMRTNSOaEHDVeMnA?.Invoke();
			if (!llkLFSoLVtaASCstwdnHCsIDxnhYb || PblvdecObnfYBDkGUhZbCLLZIHRJA == null)
			{
				return;
			}
			int count = PblvdecObnfYBDkGUhZbCLLZIHRJA.Count;
			if (count == 0)
			{
				return;
			}
			...
			UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.maps.GetAllMaps(list);
			list2.AddRange(UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = PblvdecObnfYBDkGUhZbCLLZIHRJA[i];
				...
				for (int j = 0; j < count2; j++)
				{
					...
					for (int num = list.Count - 1; num >= 0; num--)
					{
						...
						{
							list.RemoveAt(num);
							UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (lcutRjqFBODUwRTDtvYzMdLoGTsx && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(UvBXHObDlZYGHHCzDkZZTJyJLvx.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							UvBXHObDlZYGHHCzDkZZTJyJLvx.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((MhmCTAhiehkWjtxhLprkaiHIUdqTA != null && MhmCTAhiehkWjtxhLprkaiHIUdqTA.zcqaKwvyXhAGLtSnQTXsWmqKHjoD != null) ? MhmCTAhiehkWjtxhLprkaiHIUdqTA.zcqaKwvyXhAGLtSnQTXsWmqKHjoD.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(MhmCTAhiehkWjtxhLprkaiHIUdqTA.zcqaKwvyXhAGLtSnQTXsWmqKHjoD[i].ZamYCQxLHAGKChjEHxjlKOSnIhez);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = MhmCTAhiehkWjtxhLprkaiHIUdqTA.zcqaKwvyXhAGLtSnQTXsWmqKHjoD[i].qrFAJzEnuAtGtkafVRuztFxWFLWaA;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (MhmCTAhiehkWjtxhLprkaiHIUdqTA != null)
			{
				llkLFSoLVtaASCstwdnHCsIDxnhYb = MhmCTAhiehkWjtxhLprkaiHIUdqTA.zbiIUygVHQtgeEPXnokrnmpIrTVg;
				lcutRjqFBODUwRTDtvYzMdLoGTsx = MhmCTAhiehkWjtxhLprkaiHIUdqTA.ArMQbUFpwVdPlYYZZLrAwYBhHAAy;
			}
			PblvdecObnfYBDkGUhZbCLLZIHRJA = list;
			Apply();
		}
		...
		private void tnEqLMFFwugjoHOyMvcImNymgKGl(SerializedObject P_0)
		{
			...
			P_0.Add("loadFromUserDataStore", lcutRjqFBODUwRTDtvYzMdLoGTsx);
			P_0.Add("ruleSets", PblvdecObnfYBDkGUhZbCLLZIHRJA);
		}

		private bool xIgDRHQmTOVJkRVsknhXpBHuPygR(SerializedObject P_0)
		{
			llkLFSoLVtaASCstwdnHCsIDxnhYb = false;
			PblvdecObnfYBDkGUhZbCLLZIHRJA = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref llkLFSoLVtaASCstwdnHCsIDxnhYb);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref lcutRjqFBODUwRTDtvYzMdLoGTsx);
