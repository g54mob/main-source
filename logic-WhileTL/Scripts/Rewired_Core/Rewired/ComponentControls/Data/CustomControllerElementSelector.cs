...
namespace Rewired.ComponentControls.Data
{
	...
	public sealed class CustomControllerElementSelector
	{
		...
		[HideInInspector]
		private int ixaJxirPKsqWlcunKuFepLLtBDjt = -1;

		[HideInInspector]
		private int uadoCSWdUFdTHaCNMYHnmmvfSpuhA = -1;
		...
		public int GetElementIndex(Rewired.CustomController customController)
		{
			...
			if (ixaJxirPKsqWlcunKuFepLLtBDjt >= 0 && ixaJxirPKsqWlcunKuFepLLtBDjt != customController.id)
			{
				ClearCache();
			}
			if (uadoCSWdUFdTHaCNMYHnmmvfSpuhA >= 0)
			{
				return uadoCSWdUFdTHaCNMYHnmmvfSpuhA;
			}
			ixaJxirPKsqWlcunKuFepLLtBDjt = customController.id;
			switch (_selectorType)
			{
			case SelectorType.Id:
			{
				IList<ControllerElementIdentifier> list = pocllWKIBagzoXZzgPmoCZWhGOUK(customController, _elementType);
				for (int j = 0; j < list.Count; j++)
				{
					if (list[j].id == _elementId)
					{
						uadoCSWdUFdTHaCNMYHnmmvfSpuhA = j;
						break;
					}
				}
				break;
			...
			case SelectorType.Index:
			{
				...
				uadoCSWdUFdTHaCNMYHnmmvfSpuhA = _elementIndex;
				break;
			}
			case SelectorType.Name:
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					{
						uadoCSWdUFdTHaCNMYHnmmvfSpuhA = i;
						break;
					}
				}
				break;
		...
		{
			ixaJxirPKsqWlcunKuFepLLtBDjt = -1;
			uadoCSWdUFdTHaCNMYHnmmvfSpuhA = -1;
		}
	}
}
