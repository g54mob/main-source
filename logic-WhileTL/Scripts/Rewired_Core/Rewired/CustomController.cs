using System;
using Rewired.Utils;

namespace Rewired
{
	public sealed class CustomController : ControllerWithAxes
	{
		private int daMfGKxhFqOdaowmOEUuPDAFZJxI;

		private Func<int, float> OOnViAdauVjqWOObywycmHzcGWbb;

		private Func<int, bool> aPoAmzKaBbxmNSLVwspWHjOkPpNx;

		private bool thTdkTaHvdpHtLxipWLVrmPjlMghA;

		private Guid xRlLReFRFhkpEOIYtEoQauXubQsGA;

		public int sourceControllerId => daMfGKxhFqOdaowmOEUuPDAFZJxI;

		public override Guid deviceInstanceGuid
		{
			get
			{
				if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
				{
					ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
					return Guid.Empty;
				}
				return xRlLReFRFhkpEOIYtEoQauXubQsGA;
			}
		}

		internal CustomController(YCtNtFhGhsVodFBjxRMexOEjLQUl P_0)
			: this(P_0.wKTIDzdbnMqFnJlBBeomtbaWsxjR, P_0.HfMOkyPcPsuKGzdAsQPYtfVrhuyu, P_0.PgQDAaNkfzUtivkZBGNaxIzBnTBN, P_0.yGdZHAmdUeDYveLTSINOCvUHtMoHA, P_0.kXiencEahrSUtKlFEOwKvjtarZHH, P_0.MKithPtgMWeNFboVvECkRPLhlfYjA, P_0.VvJgYeDfloTcrtoNAqSVgDQAMrjjb, P_0.jhazYdoXweuxJmcAJnlflvXbFGyT, P_0.yrHZhNoSpLMEzcgptuOphbaHHcuiA, P_0.dCYtOPOydCabMdmRsHTqLvuIwxCG, null, new ControllerDataUpdater(P_0.yGdZHAmdUeDYveLTSINOCvUHtMoHA, P_0.jhazYdoXweuxJmcAJnlflvXbFGyT, P_0.yrHZhNoSpLMEzcgptuOphbaHHcuiA, null))
		{
		}

		private CustomController(int P_0, int P_1, Guid P_2, InputSource P_3, string P_4, string P_5, string P_6, int P_7, int P_8, HardwareControllerMap_Game P_9, Extension P_10, ControllerDataUpdater P_11)
			: base(P_0, P_3, P_4, P_5, P_6, ControllerType.Custom, P_2, P_7, P_8, null, P_9, P_10, P_11)
		{
			daMfGKxhFqOdaowmOEUuPDAFZJxI = P_1;
			xRlLReFRFhkpEOIYtEoQauXubQsGA = MiscTools.CreateGuidHashSHA1("CustomController device instance GUID: sourceId = " + daMfGKxhFqOdaowmOEUuPDAFZJxI + ", controllerId = " + P_0);
			WCmnBnYePrGAMdoiUNBATVOhqgEEA();
		}

		internal void mQcCmiCxDWPcaRVyoGUaKDWMEZPOA()
		{
			if (!thTdkTaHvdpHtLxipWLVrmPjlMghA)
			{
				return;
			}
			if (OOnViAdauVjqWOObywycmHzcGWbb != null)
			{
				for (int i = 0; i < _axisCount; i++)
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.axisValues[i] = OOnViAdauVjqWOObywycmHzcGWbb(i);
				}
			}
			if (aPoAmzKaBbxmNSLVwspWHjOkPpNx != null)
			{
				for (int j = 0; j < _buttonCount; j++)
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.buttonValues[j] = aPoAmzKaBbxmNSLVwspWHjOkPpNx(j);
				}
			}
		}

		public void SetAxisValue(int index, float value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				if (index < 0 || index >= _axisCount)
				{
					Logger.LogWarning(index + " is not a valid Axis index.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.axisValues[index] = value;
				}
			}
		}

		public void SetAxisValue(string elementName, float value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int axisIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetAxisIndex(elementName);
				if (axisIndex < 0 || axisIndex >= _axisCount)
				{
					Logger.LogWarning("\"" + axisIndex + "\" is not a valid Axis name.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.axisValues[axisIndex] = value;
				}
			}
		}

		public void SetAxisValueById(int elementId, float value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int axisIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetAxisIndex(elementId);
				if (axisIndex < 0 || axisIndex >= _axisCount)
				{
					Logger.LogWarning(elementId + " is not a valid Axis id.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.axisValues[axisIndex] = value;
				}
			}
		}

		public void SetButtonValue(int index, bool value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				if (index < 0 || index >= _buttonCount)
				{
					Logger.LogWarning(index + " is not a valid Button index.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.buttonValues[index] = value;
				}
			}
		}

		public void SetButtonValue(string elementName, bool value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int buttonIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetButtonIndex(elementName);
				if (buttonIndex < 0 || buttonIndex >= _buttonCount)
				{
					Logger.LogWarning("\"" + buttonIndex + "\" is not a valid Button name.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.buttonValues[buttonIndex] = value;
				}
			}
		}

		public void SetButtonValueById(int elementId, bool value)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int buttonIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetButtonIndex(elementId);
				if (buttonIndex < 0 || buttonIndex >= _buttonCount)
				{
					Logger.LogWarning(elementId + " is not a valid Button id.");
				}
				else
				{
					WlduKdCdymfJzhLxPcswpRugJOzgb.buttonValues[buttonIndex] = value;
				}
			}
		}

		public void SetAxisUpdateCallback(Func<int, float> callback)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
				return;
			}
			OOnViAdauVjqWOObywycmHzcGWbb = callback;
			if (!thTdkTaHvdpHtLxipWLVrmPjlMghA)
			{
				thTdkTaHvdpHtLxipWLVrmPjlMghA = true;
			}
		}

		public void SetButtonUpdateCallback(Func<int, bool> callback)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
				return;
			}
			aPoAmzKaBbxmNSLVwspWHjOkPpNx = callback;
			if (!thTdkTaHvdpHtLxipWLVrmPjlMghA)
			{
				thTdkTaHvdpHtLxipWLVrmPjlMghA = true;
			}
		}

		public void ClearAxisValue(int index)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				if (index < 0 || index >= _axisCount)
				{
					Logger.LogWarning(index + " is not a valid Axis index.");
					return;
				}
				float num = ((_calibrationMap != null) ? _calibrationMap.GetAxis(index).calibratedZero : 0f);
				WlduKdCdymfJzhLxPcswpRugJOzgb.axisValues[index] = num;
			}
		}

		public void ClearAxisValue(string elementName)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int axisIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetAxisIndex(elementName);
				if (axisIndex < 0 || axisIndex >= _axisCount)
				{
					Logger.LogWarning("\"" + axisIndex + "\" is not a valid Axis name.");
				}
				else
				{
					ClearAxisValue(axisIndex);
				}
			}
		}

		public void ClearAxisValueById(int elementId)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int axisIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetAxisIndex(elementId);
				if (axisIndex < 0 || axisIndex >= _axisCount)
				{
					Logger.LogWarning(elementId + " is not a valid Axis id.");
				}
				else
				{
					ClearAxisValue(axisIndex);
				}
			}
		}

		public void ClearButtonValue(int index)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				if (index < 0 || index >= _buttonCount)
				{
					Logger.LogWarning(index + " is not a valid Button index.");
					return;
				}
				WlduKdCdymfJzhLxPcswpRugJOzgb.buttonValues[index] = false;
				WlduKdCdymfJzhLxPcswpRugJOzgb.buttonPressureValues[index] = 0f;
			}
		}

		public void ClearButtonValue(string elementName)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int buttonIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetButtonIndex(elementName);
				if (buttonIndex < 0 || buttonIndex >= _buttonCount)
				{
					Logger.LogWarning("\"" + buttonIndex + "\" is not a valid Button name.");
				}
				else
				{
					ClearButtonValue(buttonIndex);
				}
			}
		}

		public void ClearButtonValueById(int elementId)
		{
			if (ReInput._id != TcEXPUvjqSTMTFutCAtGRnMeNwub)
			{
				ReInput.CheckInitialized(TcEXPUvjqSTMTFutCAtGRnMeNwub);
			}
			else if (base.enabled)
			{
				int buttonIndex = jnGTQDFeNsixRwgRJcghDqCbQWSP.GetButtonIndex(elementId);
				if (buttonIndex < 0 || buttonIndex >= _buttonCount)
				{
					Logger.LogWarning(elementId + " is not a valid Button id.");
				}
				else
				{
					ClearButtonValue(buttonIndex);
				}
			}
		}
	}
}
