...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool SroJTizchUAifuuvWtjdRDDyBGwR()
		{
			...
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = jzPAQGDHfqDeXcOrkxVhGRsOqHcmc();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = RjPKGwdjOdJTrxEtzTNBTdfwEusdA();
					flag = true;
					break;
				}
			}
		...
		private float jzPAQGDHfqDeXcOrkxVhGRsOqHcmc()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => jmjJvfPhzCbYZWNmSqVHAENcbIfA(num2), 
				_ => 0f, 
			};
		}

		private float RjPKGwdjOdJTrxEtzTNBTdfwEusdA()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float jmjJvfPhzCbYZWNmSqVHAENcbIfA(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
