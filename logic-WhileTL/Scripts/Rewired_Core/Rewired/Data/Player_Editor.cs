...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			{
				return new fltsBxmjXaFeMfcWglOfHGHvtAQsA(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			{
				return new ControllerMapLayoutManager.nwmaXXBRLHdsFSHrcaeHCCJdihJCc(_enabled, _loadFromUserDataStore, aHyvaApLVqeTXLzJMqhuGzxGiBtm());
			}

			private qmEcFrQmpjkfExHMGEuMliRmNVKH[] aHyvaApLVqeTXLzJMqhuGzxGiBtm()
			{
				List<qmEcFrQmpjkfExHMGEuMliRmNVKH> list = new List<qmEcFrQmpjkfExHMGEuMliRmNVKH>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].RsljwjGGAgziyyfHkMsFHHwMaRkM());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			{
				return new ControllerMapEnabler.bfKxbNaTbdokMFkgReyogCBTNRVl(_enabled, aHyvaApLVqeTXLzJMqhuGzxGiBtm());
			}
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			{
				return new qmEcFrQmpjkfExHMGEuMliRmNVKH(_id, _enabled);
			}
		...
		{
			fltsBxmjXaFeMfcWglOfHGHvtAQsA[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new fltsBxmjXaFeMfcWglOfHGHvtAQsA[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].wEbSdzigWkbvxeLDTZQAhupBlEHeA();
				}
			}
			fltsBxmjXaFeMfcWglOfHGHvtAQsA[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new fltsBxmjXaFeMfcWglOfHGHvtAQsA[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].wEbSdzigWkbvxeLDTZQAhupBlEHeA();
				}
			}
			fltsBxmjXaFeMfcWglOfHGHvtAQsA[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new fltsBxmjXaFeMfcWglOfHGHvtAQsA[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].wEbSdzigWkbvxeLDTZQAhupBlEHeA();
				}
			}
			fltsBxmjXaFeMfcWglOfHGHvtAQsA[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new fltsBxmjXaFeMfcWglOfHGHvtAQsA[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].wEbSdzigWkbvxeLDTZQAhupBlEHeA();
				}
			}
			return new rTFRhglKgUYuRjbuHfpVdAGUmulr(array, array2, array3, array4);
		}
	}
}
