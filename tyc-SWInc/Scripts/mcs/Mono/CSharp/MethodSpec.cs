...
namespace Mono.CSharp
{
	public sealed class MethodSpec : MemberSpec, IParametersMember, IInterfaceMemberSpec
	{
		...
		public bool IsAsync
		{
			get
			{
				return (base.Modifiers & Modifiers.ASYNC) != 0;
			}
		}
		...
		public bool IsSealed
		{
			get
			{
				return (base.Modifiers & Modifiers.SEALED) != 0;
			}
		}
		...
		public bool IsVirtual
		{
			get
			{
				return (base.Modifiers & (Modifiers.ABSTRACT | Modifiers.VIRTUAL | Modifiers.OVERRIDE)) != 0;
			}
		}
		...
		{
			if ((object)inflatedMetaInfo == null)
			{
		...
		public override string GetSignatureForDocumentation()
		{
			...
			case MemberKind.Method:
				text = ((Arity <= 0) ? Name : (Name + "``" + Arity));
				break;
			default:
				text = Name;
				break;
			}
			text = base.DeclaringType.GetSignatureForDocumentation() + "." + text + parameters.GetSignatureForDocumentation();
			if (Kind == MemberKind.Operator)
			{
