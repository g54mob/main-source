		...
		public static Mesh CombineMeshes(Mesh[] meshes, Matrix4x4[] transforms, Material[][] materials, Transform[][] bones, out Material[] resultMaterials, out Transform[] resultBones)
		{
			...
			for (int i = 0; i < meshes.Length; i++)
			{
				...
				for (int j = 0; j < array.Length; j++)
				{
					if (array[j] == null)
					{
						throw new ArgumentException(string.Format("The material at index {0} for mesh at index {1} is null.", j, i), "materials");
					}
				}
				if (bones == null)
				{
			...
			Vector3[] normals2 = ((dest != null) ? dest.ToArray() : null);
			Vector4[] tangents2 = ((dest2 != null) ? dest2.ToArray() : null);
			Color[] colors2 = ((dest3 != null) ? dest3.ToArray() : null);
			BoneWeight[] boneWeights2 = ((dest4 != null) ? dest4.ToArray() : null);
			List<Vector4>[] uvs = array3.ToArray();
			Matrix4x4[] bindposes2 = ((list3 != null) ? list3.ToArray() : null);
			resultMaterials = list5.ToArray();
			resultBones = ((list4 != null) ? list4.ToArray() : null);
