	...
	public sealed class MeshSimplifier
	{
		...
		private void UpdateTriangles(int i0, int ia0, ref Vertex v, ResizableArray<bool> deleted, ref int deletedTriangles)
		{
			...
			for (int j = 0; j < tcount; j++)
			{
				...
				triangle.dirty = true;
				Vector3d result;
				triangle.err0 = CalculateError(ref data2[triangle.v0], ref data2[triangle.v1], out result);
		...
		private void RemoveVertexPass(int startTrisCount, int targetTrisCount, double threshold, ResizableArray<bool> deleted0, ResizableArray<bool> deleted1, ref int deletedTris)
		{
			...
			for (int i = 0; i < length; i++)
			{
				...
				for (int j = 0; j < 3; j++)
				{
					...
					Vector3d result;
					CalculateError(ref data2[num2], ref data2[num3], out result);
					...
					Vector3 result2;
					CalculateBarycentricCoords(ref result, ref data2[num2].p, ref data2[num3].p, ref data2[num5].p, out result2);
					data2[num2].p = result;
		...
		private void UpdateMesh(int iteration)
		{
			...
			for (int num22 = 0; num22 < num; num22++)
			{
				...
				Vector3d result2;
				data[num22].err0 = CalculateError(ref data2[triangle.v0], ref data2[triangle.v1], out result2);
		...
		public void AddBlendShapes(BlendShape[] blendShapes)
		{
			...
			for (int i = 0; i < blendShapes.Length; i++)
			{
				...
				if (frames == null || frames.Length == 0)
				{
					throw new ArgumentException(string.Format("The frames of blend shape at index {0} cannot be null or empty.", i), "blendShapes");
				}
