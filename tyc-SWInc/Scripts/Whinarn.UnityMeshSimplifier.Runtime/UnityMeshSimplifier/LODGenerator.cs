	...
	public static class LODGenerator
	{
		...
		public static LODGroup GenerateLODs(GameObject gameObject, LODLevel[] levels, bool autoCollectRenderers, SimplificationOptions simplificationOptions, string saveAssetsPath)
		{
			...
			for (int i = 0; i < levels.Length; i++)
			{
				...
				Transform transform3 = new GameObject(string.Format("Level{0:00}", i)).transform;
				...
				if (array3 != null && array3.Length != 0)
				{
					...
					if (array4 != null)
					{
						for (int num = 0; num < array4.Length; num++)
						{
							...
							Renderer item = CreateLevelRenderer(gameObject, i, ref level, transform3, num, ref renderer, ref simplificationOptions, saveAssetsPath);
							list2.Add(item);
						}
					}
					if (array5 != null)
					{
						for (int num2 = 0; num2 < array5.Length; num2++)
						{
							...
							Renderer item2 = CreateLevelRenderer(gameObject, i, ref level, transform3, num2, ref renderer2, ref simplificationOptions, saveAssetsPath);
							list2.Add(item2);
						}
					}
		...
		private static RendererInfo[] CombineStaticMeshes(Transform transform, int levelIndex, MeshRenderer[] renderers)
		{
			...
			string name = string.Format("{0}_combined_static", transform.name);
			list.Add(new RendererInfo
			{
				name = name,
				isStatic = true,
		...
		private static RendererInfo[] CombineSkinnedMeshes(Transform transform, int levelIndex, SkinnedMeshRenderer[] renderers)
		{
			...
			if (array.Length != 0)
			{
				...
				string name = string.Format("{0}_combined_skinned", transform.name);
				list.Add(new RendererInfo
				{
					name = name,
					isStatic = false,
		...
		private static Renderer CreateLevelRenderer(GameObject gameObject, int levelIndex, [In][IsReadOnly] ref LODLevel level, Transform levelTransform, int rendererIndex, [In][IsReadOnly] ref RendererInfo renderer, [In][IsReadOnly] ref SimplificationOptions simplificationOptions, string saveAssetsPath)
		{
			Mesh mesh = renderer.mesh;
			LODLevel lODLevel = level;
			if (lODLevel.Quality < 1f)
			{
				Mesh mesh2 = mesh;
				lODLevel = level;
				mesh = SimplifyMesh(mesh2, lODLevel.Quality, ref simplificationOptions);
			...
			{
				return CreateStaticLevelRenderer(string.Format("{0:000}_static_{1}", rendererIndex, renderer.name), levelTransform, renderer.transform, mesh, renderer.materials, ref level);
			}
			return CreateSkinnedLevelRenderer(string.Format("{0:000}_skinned_{1}", rendererIndex, renderer.name), levelTransform, renderer.transform, mesh, renderer.materials, renderer.rootBone, renderer.bones, ref level);
		}

		private static MeshRenderer CreateStaticLevelRenderer(string name, Transform parentTransform, Transform originalTransform, Mesh mesh, Material[] materials, [In][IsReadOnly] ref LODLevel level)
		{
		...
		private static SkinnedMeshRenderer CreateSkinnedLevelRenderer(string name, Transform parentTransform, Transform originalTransform, Mesh mesh, Material[] materials, Transform rootBone, Transform[] bones, [In][IsReadOnly] ref LODLevel level)
		{
		...
		private static void SetupLevelRenderer(Renderer renderer, [In][IsReadOnly] ref LODLevel level)
		{
			LODLevel lODLevel = level;
			renderer.shadowCastingMode = lODLevel.ShadowCastingMode;
			lODLevel = level;
			renderer.receiveShadows = lODLevel.ReceiveShadows;
			lODLevel = level;
			renderer.motionVectorGenerationMode = lODLevel.MotionVectorGenerationMode;
			lODLevel = level;
			renderer.lightProbeUsage = lODLevel.LightProbeUsage;
			lODLevel = level;
			renderer.reflectionProbeUsage = lODLevel.ReflectionProbeUsage;
			SkinnedMeshRenderer skinnedMeshRenderer = renderer as SkinnedMeshRenderer;
			if (skinnedMeshRenderer != null)
			{
				lODLevel = level;
				skinnedMeshRenderer.quality = lODLevel.SkinQuality;
				lODLevel = level;
				skinnedMeshRenderer.skinnedMotionVectors = lODLevel.SkinnedMotionVectors;
			}
		}
		...
		private static Mesh SimplifyMesh(Mesh mesh, float quality, [In][IsReadOnly] ref SimplificationOptions options)
		{
