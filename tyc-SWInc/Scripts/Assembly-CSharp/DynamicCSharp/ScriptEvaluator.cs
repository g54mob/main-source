...
namespace DynamicCSharp
{
	public sealed class ScriptEvaluator
	{
		...
		{
			foreach (string item in new List<string>(bindingDelegates.Keys))
			{
		...
		{
			return GetTemplateSource().Replace("[TAG_USINGSTATEMENTS]", GetUsingStatementsSource()).Replace("[TAG_DELEGATESTATEMENTS]", GetDelegateStatementsSource()).Replace("[TAG_FIELDSTATEMENTS]", GetFieldStatementsSource())
				.Replace("[TAG_CLASSNAME]", "_EvalClass" + Guid.NewGuid().ToString("N"))
				.Replace("[TAG_METHODNAME]", "_EvalEntry")
				.Replace("[TAG_METHODBODY]", source);
		}
		...
		private string GetDelegateStatementsSource()
		{
			...
			{
				MethodInfo method = bindingDelegate.Value.Method;
				...
				stringBuilder.Append(typeof(Func<>).FullName.Replace("`1", ""));
				stringBuilder.Append("<");
				if (array.Length != 0)
				{
		...
		private string GetFieldStatementsSource()
		{
			...
			foreach (Variable bindingVariable in bindingVariables)
			{
				...
				string fullName = ((value == null) ? typeof(object) : value.GetType()).FullName;
				stringBuilder.Append(fullName);
				stringBuilder.Append(" ");
