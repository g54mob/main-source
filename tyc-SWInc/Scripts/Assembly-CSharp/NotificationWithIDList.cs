...
public abstract class NotificationWithIDList<T> : NotificationMessage
{
	[NonSerialized]
	protected HashSet<T> _items = new HashSet<T>();

	protected HashSet<uint> _serializedItems = new HashSet<uint>();
	...
	public NotificationWithIDList()
	{
	}

	public NotificationWithIDList(string msg, string icon, SDateTime date, NotificationManager.NotificationType type, params T[] items)
	...
	public abstract uint GetID(T item);
