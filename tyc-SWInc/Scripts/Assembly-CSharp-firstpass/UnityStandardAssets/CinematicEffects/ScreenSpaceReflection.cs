...
namespace UnityStandardAssets.CinematicEffects
{
	...
	public class ScreenSpaceReflection : MonoBehaviour
	{
		...
		public void OnPreRender()
		{
			...
			material.SetInt("_RayStepSize", Mathf.RoundToInt((float)(settings.reflectionSettings.stepSize * camera_.pixelWidth) / (float)Screen.width));
			...
			material.SetVector("_envColor", RenderSettings.fogColor);
			...
			if (m_CommandBuffer == null)
			{
				...
				m_CommandBuffer.GetTemporaryRT(kHitPointTexture, width, height, 0, FilterMode.Bilinear, RenderTextureFormat.ARGBHalf, RenderTextureReadWrite.Linear);
				for (int i = 0; i < 5; i++)
				{
					m_CommandBuffer.GetTemporaryRT(kReflectionTextures[i], width >> i, height >> i, 0, FilterMode.Bilinear, format);
				}
				m_CommandBuffer.GetTemporaryRT(kFilteredReflections, width, height, 0, (!bilateralUpsample) ? FilterMode.Bilinear : FilterMode.Point, format);
				m_CommandBuffer.GetTemporaryRT(kFinalReflectionTexture, width, height, 0, FilterMode.Point, format);
				...
				for (int j = 1; j < 5; j++)
				{
					...
					int num9 = j;
					m_CommandBuffer.GetTemporaryRT(kBlurTexture, width >> num9, height >> num9, 0, FilterMode.Bilinear, format);
