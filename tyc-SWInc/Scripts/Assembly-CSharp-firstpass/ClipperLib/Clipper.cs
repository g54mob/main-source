	...
	public class Clipper : ClipperBase
	{
		...
		private void BuildIntersectList(long topY)
		{
			...
			while (flag && m_SortedEdges != null)
			{
				...
				while (tEdge.NextInSEL != null)
				{
					...
					{
						IntPoint ip;
						IntersectPoint(tEdge, nextInSEL, out ip);
						if (ip.Y < topY)
						{
		...
		private void FixupFirstLefts1(OutRec OldOutRec, OutRec NewOutRec)
		{
			for (int i = 0; i < m_PolyOuts.Count; i++)
			{
				...
				if (outRec.Pts != null && outRec2 == OldOutRec && Poly2ContainsPoly1(outRec.Pts, NewOutRec.Pts))
				{
		...
		internal static void AddPolyNodeToPaths(PolyNode polynode, NodeType nt, List<List<IntPoint>> paths)
		{
			...
			{
				AddPolyNodeToPaths(polynode.Childs[i], nt, paths);
			}
		}
