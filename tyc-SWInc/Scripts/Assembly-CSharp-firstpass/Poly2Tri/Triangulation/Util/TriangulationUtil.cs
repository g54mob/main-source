...
namespace Poly2Tri.Triangulation.Util
{
	public class TriangulationUtil
	{
		...
		private static bool PointOnLineSegment2D(double x1, double y1, double x2, double y2, double x, double y, double epsilon)
		{
		...
		private static double LI2DDotProduct(Point2D v0, Point2D v1)
		{
		...
		public static bool RaysIntersect2D(Point2D ptRayOrigin0, Point2D ptRayVector0, Point2D ptRayOrigin1, Point2D ptRayVector1, ref Point2D ptIntersection)
		{
			if (ptIntersection != null)
			{
				...
				double num = LI2DDotProduct(ptRayVector0, v2);
				if (Math.Abs(num) < 0.01)
				{
					return false;
				}
				double num2 = LI2DDotProduct(v, v2) / num;
				ptIntersection.X = ptRayOrigin0.X + ptRayVector0.X * num2;
				ptIntersection.Y = ptRayOrigin0.Y + ptRayVector0.Y * num2;
				return true;
			}
			if (Math.Abs(ptRayVector1.X - ptRayVector0.X) > 0.01 && Math.Abs(ptRayVector1.Y - ptRayVector0.Y) > 0.01)
			{
				return true;
			}
			return false;
