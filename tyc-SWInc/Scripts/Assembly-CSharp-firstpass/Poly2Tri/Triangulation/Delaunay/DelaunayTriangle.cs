...
namespace Poly2Tri.Triangulation.Delaunay
{
	public class DelaunayTriangle : IEquatable<DelaunayTriangle>
	{
		...
		private FixedArray3<bool> _edgeIsConstrained;
		...
		{
			TriangulationPoint item = Points.Item0;
			TriangulationPoint item2 = Points.Item1;
			TriangulationPoint item3 = Points.Item2;
			double num = (item.X + item3.X) / 2.0;
			double num2 = (item.Y + item3.Y) / 2.0;
			double num3 = (item2.X + item3.X) / 2.0;
			double num4 = (item2.Y + item3.Y) / 2.0;
			double num5;
			double num6;
			if (item.X == item3.X)
			{
			...
			{
				num5 = (item3.Y - item.Y) / (item.X - item3.X);
				num6 = 1.0;
			}
			double num7;
			double num8;
			...
			{
				num7 = (item2.Y - item3.Y) / (item2.X - item3.X);
				num8 = -1.0;
			}
		...
		private int IndexCCWFrom(TriangulationPoint p)
		{
		...
		{
			DelaunayTriangle delaunayTriangle = this;
			DelaunayTriangle delaunayTriangle2 = this;
			DelaunayTriangle delaunayTriangle3 = (Neighbors[2] = null);
			DelaunayTriangle value = (delaunayTriangle2.Neighbors[1] = delaunayTriangle3);
			delaunayTriangle.Neighbors[0] = value;
		}
		...
		public void Clear()
		{
			...
			ClearNeighbors();
			DelaunayTriangle delaunayTriangle2 = this;
			DelaunayTriangle delaunayTriangle3 = this;
			TriangulationPoint triangulationPoint = (Points[2] = null);
			TriangulationPoint value = (delaunayTriangle3.Points[1] = triangulationPoint);
			delaunayTriangle2.Points[0] = value;
		}
		...
		private void SetConstrainedEdge(int idx, bool ce)
		{
			_edgeIsConstrained[idx] = ce;
		}
		...
		private bool GetEdge(int idx, out DTSweepConstraint edge)
		{
			edge = null;
			if (idx < 0 || idx > 2)
			{
