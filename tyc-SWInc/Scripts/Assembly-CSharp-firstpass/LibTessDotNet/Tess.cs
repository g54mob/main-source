...
namespace LibTessDotNet
{
	public class Tess
	{
		...
		private class EdgeStackNode
		{
			public MeshUtils.Edge _edge;

			public EdgeStackNode _next;
		}

		private class EdgeStack
		{
			public EdgeStackNode _top;
		}
		...
		private ActiveRegion TopLeftRegion(ActiveRegion reg)
		{
			...
			{
				MeshUtils.Edge newEdge = _mesh.Connect(_pool, RegionBelow(reg)._eUp._Sym, reg._eUp._Lnext);
		...
		{
			ActiveRegion activeRegion = _pool.Get<ActiveRegion>();
		...
		private MeshUtils.Edge FinishLeftRegions(ActiveRegion regFirst, ActiveRegion regLast)
		{
			...
			while (activeRegion != regLast)
			{
				...
				if (edge._Org != eUp._Org)
				{
					...
					edge = _mesh.Connect(_pool, eUp._Lprev, edge._Sym);
					FixUpperEdge(activeRegion2, edge);
				}
				if (eUp._Onext != edge)
				{
					_mesh.Splice(_pool, edge._Oprev, edge);
					_mesh.Splice(_pool, eUp, edge);
				}
				FinishRegion(activeRegion);
		...
		private void AddRightEdges(ActiveRegion regUp, MeshUtils.Edge eFirst, MeshUtils.Edge eLast, MeshUtils.Edge eTopLeft, bool cleanUp)
		{
			...
			while (true)
			{
				...
				{
					_mesh.Splice(_pool, edge._Oprev, edge);
					_mesh.Splice(_pool, edge2._Oprev, edge);
				}
		...
		private void GetIntersectData(MeshUtils.Vertex isect, MeshUtils.Vertex orgUp, MeshUtils.Vertex dstUp, MeshUtils.Vertex orgLo, MeshUtils.Vertex dstLo)
		{
			...
			float w;
			float w2;
			VertexWeights(isect, orgUp, dstUp, out w, out w2);
			float w3;
			float w4;
			VertexWeights(isect, orgLo, dstLo, out w3, out w4);
		...
		private bool CheckForRightSplice(ActiveRegion regUp)
		{
			...
			if (Geom.VertLeq(eUp._Org, eUp2._Org))
			{
				...
				{
					_mesh.SplitEdge(_pool, eUp2._Sym);
					_mesh.Splice(_pool, eUp, eUp2._Oprev);
			...
			else
			{
				...
				_mesh.Splice(_pool, eUp2._Oprev, eUp);
			}
			return true;
		}
		...
		private bool CheckForLeftSplice(ActiveRegion regUp)
		{
			...
			if (Geom.VertLeq(eUp._Dst, eUp2._Dst))
			{
				...
				RegionAbove(regUp)._dirty = (regUp._dirty = true);
				MeshUtils.Edge edge = _mesh.SplitEdge(_pool, eUp);
				_mesh.Splice(_pool, eUp2._Sym, edge);
			...
			else
			{
				...
				regUp._dirty = (activeRegion._dirty = true);
				MeshUtils.Edge edge2 = _mesh.SplitEdge(_pool, eUp2);
				_mesh.Splice(_pool, eUp._Lnext, eUp2._Sym);
		...
		private bool CheckForIntersect(ActiveRegion regUp)
		{
			...
			MeshUtils.Vertex vertex = _pool.Get<MeshUtils.Vertex>();
			...
			if ((!Geom.VertEq(dst, _event) && Geom.EdgeSign(dst, _event, vertex) >= 0f) || (!Geom.VertEq(dst2, _event) && Geom.EdgeSign(dst2, _event, vertex) <= 0f))
			{
				...
				{
					_mesh.SplitEdge(_pool, eUp._Sym);
					_mesh.Splice(_pool, eUp2._Sym, eUp);
				...
				{
					_mesh.SplitEdge(_pool, eUp2._Sym);
					_mesh.Splice(_pool, eUp._Lnext, eUp2._Oprev);
					activeRegion = regUp;
		...
		private void ConnectRightVertex(ActiveRegion regUp, MeshUtils.Edge eBottomLeft)
		{
			...
			{
				_mesh.Splice(_pool, edge._Oprev, eUp);
				regUp = TopLeftRegion(regUp);
			...
			{
				_mesh.Splice(_pool, eBottomLeft, eUp2._Oprev);
				eBottomLeft = FinishLeftRegions(activeRegion, null);
				flag = true;
			}
			if (flag)
			{
				AddRightEdges(regUp, eBottomLeft._Onext, edge, edge, true);
				return;
			}
			MeshUtils.Edge eDst = ((!Geom.VertLeq(eUp2._Org, eUp._Org)) ? eUp : eUp2._Oprev);
			eDst = _mesh.Connect(_pool, eBottomLeft._Lprev, eDst);
			AddRightEdges(regUp, eDst, eDst._Onext, eDst._Onext, false);
		...
		private void ConnectLeftDegenerate(ActiveRegion regUp, MeshUtils.Vertex vEvent)
		{
			...
			if (!Geom.VertEq(eUp._Dst, vEvent))
			{
				...
				{
					_mesh.Delete(_pool, eUp._Onext);
					regUp._fixUpperEdge = false;
				}
				_mesh.Splice(_pool, vEvent._anEdge, eUp);
				SweepEvent(vEvent);
				return;
			}
			throw new InvalidOperationException("Vertices should have been merged before");
		...
		private void ConnectLeftVertex(MeshUtils.Vertex vEvent)
		{
			...
			{
				MeshUtils.Edge edge = ((activeRegion3 != key) ? _mesh.Connect(_pool, eUp2._Dnext, vEvent._anEdge)._Sym : _mesh.Connect(_pool, vEvent._anEdge._Sym, eUp._Lnext));
		...
		private void SweepEvent(MeshUtils.Vertex vEvent)
		{
			...
			{
				AddRightEdges(activeRegion, edge2._Onext, eUp, eUp, true);
			}
		}

		private void AddSentinel(float smin, float smax, float t)
		{
			MeshUtils.Edge edge = _mesh.MakeEdge(_pool);
			edge._Org._s = smax;
			edge._Org._t = t;
			edge._Dst._s = smin;
			edge._Dst._t = t;
		...
		private void ComputeNormal(ref Vec3 norm)
		{
			...
			Vec3 result;
			Vec3.Sub(ref obj._coords, ref vertex._coords, out result);
			Vec3 vec = default(Vec3);
			for (next = _mesh._vHead._next; next != _mesh._vHead; next = next._next)
			{
				Vec3 result2;
				Vec3.Sub(ref next._coords, ref vertex._coords, out result2);
		...
		private void TessellateMonoRegion(MeshUtils.Face face)
		{
			...
			while (edge._Lnext != edge2)
			{
				if (Geom.VertLeq(edge._Dst, edge2._Org))
				{
					...
					{
						edge2 = _mesh.Connect(_pool, edge2._Lnext, edge2)._Sym;
					}
					edge2 = edge2._Lprev;
				}
				else
				{
					...
					{
						edge = _mesh.Connect(_pool, edge, edge._Lprev)._Sym;
					}
					edge = edge._Lnext;
				}
			}
		...
		private float SignedArea(IList<ContourVertex> vertices)
		{
			...
			for (int i = 0; i < vertices.Count; i++)
			{
				...
				ContourVertex contourVertex2 = vertices[(i + 1) % vertices.Count];
		...
		public void AddContourFast(IList<Vector2> vertices, bool reverse)
		{
			if (_mesh == null)
			{
			...
			for (int i = 0; i < vertices.Count; i++)
			{
				if (edge == null)
				{
					edge = _mesh.MakeEdge(_pool);
					_mesh.Splice(_pool, edge, edge._Sym);
				}
				else
				{
					_mesh.SplitEdge(_pool, edge);
					edge = edge._Lnext;
				}
				int index = (reverse ? (vertices.Count - 1 - i) : i);
				Vector2 vector = vertices[index];
				edge._Org._coords = new Vec3(vector.x, 0f, vector.y);
				edge._Org._data = null;
				edge._winding = 1;
		...
		private void AddContourInternal(IList<ContourVertex> vertices, ContourOrientation forceOrientation)
		{
			...
			for (int i = 0; i < vertices.Count; i++)
			{
				...
				int index = (flag ? (vertices.Count - 1 - i) : i);
				edge._Org._coords = vertices[index].Position;
				edge._Org._data = vertices[index].Data;
				edge._winding = 1;
				edge._Sym._winding = -1;
			}
		}

		public void Tessellate(WindingRule windingRule = WindingRule.EvenOdd, ElementType elementType = ElementType.Polygons, int polySize = 3, CombineCallback combineCallback = null, Vec3 normal = default(Vec3))
		{
			_normal = normal;
			_vertices = null;
			_elements = null;
			...
			{
				TessellateInterior();
				if (elementType == ElementType.ConstrainedDelauneyTriangles)
				{
					tessMeshRefineDelaunay(_mesh);
		...
		private bool stackEmpty(EdgeStack stack)
		{
			return stack._top == null;
		}

		private void stackPush(EdgeStack stack, MeshUtils.Edge e)
		{
			EdgeStackNode edgeStackNode = new EdgeStackNode();
			edgeStackNode._edge = e;
			edgeStackNode._next = stack._top;
			stack._top = edgeStackNode;
		}

		private MeshUtils.Edge stackPop(EdgeStack stack)
		{
			MeshUtils.Edge result = null;
			EdgeStackNode top = stack._top;
			if (top != null)
			{
				stack._top = top._next;
				result = top._edge;
			}
			return result;
		}

		private int tessMeshRefineDelaunay(Mesh mesh)
		{
			EdgeStack stack = new EdgeStack();
			int num = 0;
			int num2 = 0;
			int num3 = 0;
			for (MeshUtils.Face next = mesh._fHead._next; next != mesh._fHead; next = next._next)
			{
				if (next._inside)
				{
					...
					do
					{
						edge._mark = Geom.EdgeIsInternal(edge);
						if (edge._mark && !edge._Sym._mark)
						{
							stackPush(stack, edge);
						}
						edge = edge._Lnext;
					}
			...
			num2 = num * num;
			MeshUtils.Edge[] array = new MeshUtils.Edge[4];
			while (!stackEmpty(stack) && num3 < num2)
			{
				MeshUtils.Edge edge = stackPop(stack);
				edge._mark = (edge._Sym._mark = false);
				if (!Geom.tesedgeIsLocallyDelaunay(edge))
				{
					Mesh.MeshFlipEdge(edge);
					array[0] = edge._Lnext;
					array[1] = edge._Lprev;
					array[2] = edge._Sym._Lnext;
					array[3] = edge._Sym._Lprev;
					for (int i = 0; i < 4; i++)
					{
						if (!array[i]._mark && Geom.EdgeIsInternal(array[i]))
						{
							array[i]._mark = (array[i]._Sym._mark = true);
