...
namespace MoreMountains.Feedbacks
{
	...
	public class MMFeedbackParticlesInstantiation : MMFeedback
	{
			...
			if (RandomParticlePrefabs.Count > 0)
			{
				if (Mode == Modes.Cached)
				{
					...
					foreach (ParticleSystem randomParticlePrefab in RandomParticlePrefabs)
					{
						...
						if (transform == null)
						{
							SceneManager.MoveGameObjectToScene(particleSystem.gameObject, base.gameObject.scene);
						}
				...
				else
				{
					...
					if (transform == null)
					{
						SceneManager.MoveGameObjectToScene(_instantiatedParticleSystem.gameObject, base.gameObject.scene);
					}
				}
			}
			else
		...
		{
			if (InstantiateParticlesPosition == null && base.Owner != null)
			{
				InstantiateParticlesPosition = base.Owner.transform;
			}
			if (system != null)
			{
				system.Stop();
				system.transform.position = GetPosition(base.transform.position);
				if (ApplyRotation)
				{
					system.transform.rotation = GetRotation(base.transform);
				}
				if (ApplyScale)
				{
					system.transform.localScale = GetScale(base.transform);
				}
				system.Clear();
			}
		}
		...
		protected virtual Vector3 GetPosition(Vector3 position)
		{
			return PositionMode switch
			{
				PositionModes.FeedbackPosition => base.transform.position + Offset, 
