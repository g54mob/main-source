...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks
	{
		...
		{
			if (!CanPlay || (base.IsPlaying && !CanPlayWhileAlreadyPlaying) || !EvaluateChance() || (CooldownDuration > 0f && GetTime() - _lastStartAt < CooldownDuration))
			{
				return;
			}
			SkippingToTheEnd = false;
		...
		{
			return duration * Mathf.Clamp(DurationMultiplier, 0.001f, float.MaxValue) * _randomDurationMultiplier;
		}
		...
		protected void OnDrawGizmosSelected()
		{
			if (FeedbacksList != null)
			{
				for (int num = FeedbacksList.Count - 1; num >= 0; num--)
				{
					FeedbacksList[num].OnDrawGizmosSelected();
				}
			}
		}
	}
