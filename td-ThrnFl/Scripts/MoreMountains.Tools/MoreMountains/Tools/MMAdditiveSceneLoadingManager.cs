...
namespace MoreMountains.Tools
{
	public class MMAdditiveSceneLoadingManager : MonoBehaviour
	{
		...
		{
			LoadScene(sceneToLoadName, settings.LoadingSceneName, settings.ThreadPriority, settings.SecureLoad, settings.InterpolateProgress, settings.BeforeEntryFadeDelay, settings.EntryFadeDuration, settings.AfterEntryFadeDelay, settings.BeforeExitFadeDelay, settings.ExitFadeDuration, settings.EntryFadeTween, settings.ExitFadeTween, settings.ProgressBarSpeed, settings.FadeMode, settings.UnloadMethod);
		}
		...
		protected virtual void MMLoadingSceneDebug(string message)
		{
			if (DebugMode)
			{
				Debug.Log(string.Concat(string.Concat("" + "<color=#82d3f9>[" + Time.frameCount + "]</color> ", "<color=#f9a682>[", MMTime.FloatToTimeString(Time.time, displayHours: false, displayMinutes: true, displaySeconds: true, displayMilliseconds: true), "]</color> "), message));
			}
		}
	}
}
