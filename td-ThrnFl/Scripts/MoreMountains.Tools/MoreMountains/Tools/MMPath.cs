...
namespace MoreMountains.Tools
{
	...
	public class MMPath : MonoBehaviour
	{
		...
		public virtual void Initialization()
		{
			...
			{
				Vector3 pathElementPosition = PathElements[0].PathElementPosition;
				base.transform.position += pathElementPosition;
				foreach (MMPathMovementElement pathElement in PathElements)
				{
					pathElement.PathElementPosition -= pathElementPosition;
				}
			}
		...
		public void SetPath(in Data configuration)
		{
			...
			PathElements.Clear();
			Vector3[] offsets = configuration.Offsets;
			foreach (Vector3 pathElementPosition in offsets)
			{
				PathElements.Add(new MMPathMovementElement
				{
					Delay = configuration.Delay,
					PathElementPosition = pathElementPosition
				});
			}
			if (PathElements != null && PathElements.Count >= 1)
			{
