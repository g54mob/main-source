	...
	public class MMSquashAndStretch : MonoBehaviour
	{
		...
		protected virtual void ComputeNewLocalScale()
		{
			if (_squashing)
			{
				...
				_newLocalScale.x = _initialScale.x + SquashCurve.Evaluate(num) * _squashIntensity;
				_newLocalScale.y = _initialScale.y - SquashCurve.Evaluate(num) * _squashIntensity;
				_newLocalScale.z = _initialScale.z + SquashCurve.Evaluate(num) * _squashIntensity;
				if (num >= 1f)
				{
					_squashing = false;
				}
