...
namespace MoreMountains.Tools
{
	...
	public class MMTilemapGenerator : MonoBehaviour
	{
		...
		public virtual void Generate()
		{
			...
			{
				GlobalSeed = Mathf.Abs(UnityEngine.Random.Range(int.MinValue, int.MaxValue));
			}
		...
		protected virtual void GenerateLayer(MMTilemapGeneratorLayer layer)
		{
			...
			if (layer.DoNotUseGlobalSeed)
			{
				...
				{
					layer.Seed = Mathf.Abs(UnityEngine.Random.Range(int.MinValue, int.MaxValue));
				}
			...
			num = (layer.DoNotUseGlobalSeed ? num2 : num3);
			...
			case GenerateMethods.Perlin:
				_grid = MMGridGeneratorPerlinNoise.Generate(width, height, num);
				layer.Grid = _grid;
				break;
			case GenerateMethods.PerlinGround:
				_grid = MMGridGeneratorPerlinNoiseGround.Generate(width, height, num);
				layer.Grid = _grid;
				break;
			...
			{
				int[,] obstacles = MMGridGenerator.TilemapToGrid(layer.RandomWalkAvoiderObstaclesTilemap, width, height);
				_grid = MMGridGeneratorRandomWalkAvoider.Generate(width, height, seed, layer.RandomWalkAvoiderPercent, layer.RandomWalkAvoiderStartingPoint, obstacles, layer.RandomWalkAvoiderObstaclesDistance, layer.RandomWalkAvoiderMaxIterations);
				layer.Grid = _grid;
				break;
			}
			case GenerateMethods.RandomWalkGround:
				_grid = MMGridGeneratorRandomWalkGround.Generate(width, height, seed, layer.RandomWalkGroundMinHeightDifference, layer.RandomWalkGroundMaxHeightDifference, layer.RandomWalkGroundMinFlatDistance, layer.RandomWalkGroundMaxFlatDistance, layer.RandomWalkGroundMaxHeight);
				layer.Grid = _grid;
				break;
			case GenerateMethods.Path:
				_grid = MMGridGeneratorPath.Generate(width, height, seed, layer.PathDirection, layer.PathStartPosition, layer.PathMinWidth, layer.PathMaxWidth, layer.PathDirectionChangeDistance, layer.PathWidthChangePercentage, layer.PathDirectionChangePercentage);
				layer.Grid = _grid;
				break;
		...
		private static async void DelayedCopy(MMTilemapGeneratorLayer layer)
		{
