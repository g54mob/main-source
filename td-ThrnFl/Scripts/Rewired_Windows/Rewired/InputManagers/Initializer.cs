...
namespace Rewired.InputManagers
{
	...
	internal class Initializer : PlatformInitializer
	{
		...
		private static PlatformInitializer nDuBluwRzVXmdTdQONUSRnFcWMBJ;

		public static PlatformInitializer GetPlatformInitializer()
		{
			if (nDuBluwRzVXmdTdQONUSRnFcWMBJ == null)
			{
				nDuBluwRzVXmdTdQONUSRnFcWMBJ = new Initializer();
			}
			return nDuBluwRzVXmdTdQONUSRnFcWMBJ;
		}
		...
		public override object Initialize(IConfigVars_Internal configVars)
		{
			if (UnityTools.platform == Platform.Windows || UnityTools.platform == Platform.WindowsAppStore)
			{
				...
				if (UnityTools.platform == Platform.Windows && configVars2.windowsStandalonePrimaryInputSource == WindowsStandalonePrimaryInputSource.SDL2)
				{
					try
					{
						if (new pVCnQGEndlyOQtYMurDRbysLyjGd(configVars2, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId, true, false, false) == null)
						{
							throw new Exception();
						}
					}
				...
				try
				{
					return new tQvvHtjvyoLHXIDvEFlotdGkgMVJ(configVars2, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId);
				}
				catch (Exception)
				{
		...
		public override IElementIdentifierTool CreateTool(string inputSourceString)
		{
			...
			{
				return new ULbhIZPknTdDYygRSVHyWazkBckdA();
			}
			if (inputSourceString == "RawInput")
			{
				return new gjJmrvJcRNPfzOIsYDNVKvGbIiPW();
			}
			return null;
		}
	}
