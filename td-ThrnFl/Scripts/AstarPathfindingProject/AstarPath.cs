...
public class AstarPath : VersionedMonoBehaviour
{
	...
	public static readonly Version Version = new Version(5, 1, 6);
	...
	public void Scan(NavGraph[] graphsToScan = null)
	{
		...
		if (asyncScanTask != null)
		{
			UnityEngine.Debug.LogError("An async scan was already running when a new scan was requested. Blocking until it is complete. You can check if a scan is currently in progress using the AstarPath.active.isScanning property.", this);
			BlockUntilAsyncScanComplete();
		}
	...
	public IEnumerable<Progress> ScanAsync(NavGraph[] graphsToScan = null)
	{
		if (asyncScanTask != null)
		{
			UnityEngine.Debug.LogError("An async scan was already running when a new async scan was requested. Blocking until the previous one is complete. You can check if a scan is currently in progress using the AstarPath.active.isScanning property.", this);
			BlockUntilAsyncScanComplete();
		}
		asyncScanTask = ScanInternal(graphsToScan, async: true).GetEnumerator();
		try
		{
