		...
		private void Serialize(object obj, bool serializePrivateFieldsByDefault = false)
		{
			...
			if (obj is IList)
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					Serialize(list[i], serializePrivateFieldsByDefault);
				}
				output.Append("]");
				return;
			}
			...
			do
			{
				...
				{
					if (!(fieldInfo.DeclaringType != type) && ((!flag && (fieldInfo.IsPublic || serializePrivateFieldsByDefault)) || fieldInfo.GetCustomAttributes(typeof(JsonMemberAttribute), inherit: true).Length != 0))
					{
						if (flag2)
						{
							output.Append(", ");
						...
						Serialize(fieldInfo.GetValue(obj), serializePrivateFieldsByDefault);
					}
				}
				type = type.BaseType;
			}
