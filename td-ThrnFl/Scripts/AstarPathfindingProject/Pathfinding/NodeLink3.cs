				...
				for (int j = 0; j < connectedNode2.GetVertexCount(); j++)
				{
					...
					if (Int3.DotLong((vertex4 - vertex3).Normal2D(), rhs) >= 0 && (double)Int3.Angle(vertex4 - vertex3, vertex2 - vertex) > 2.967059810956319)
					{
						...
						if (!(val2 < val))
						{
							...
							GraphNode.Connect(connectedNode1, startNode, (uint)Mathf.RoundToInt((float)((Int3)(clamped1 - StartTransform.position)).costMagnitude * costFactor));
							GraphNode.Connect(endNode, connectedNode2, (uint)Mathf.RoundToInt((float)((Int3)(clamped2 - EndTransform.position)).costMagnitude * costFactor));
							return;
						}
						Debug.LogError(string.Concat("Something went wrong! ", val.ToString(), " ", val2.ToString(), " ", vertex, " ", vertex2, " ", vertex3, " ", vertex4, "\nTODO, how can this happen?"));
					}
				}
			}
		}
		...
		public override void DrawGizmos()
		{
			...
			if (StartTransform != null && EndTransform != null)
			{
				...
				if (flag)
				{
					...
					NodeLink.DrawArch(StartTransform.position + normalized * 0.1f, EndTransform.position + normalized * 0.1f, Vector3.up, color);
					NodeLink.DrawArch(StartTransform.position - normalized * 0.1f, EndTransform.position - normalized * 0.1f, Vector3.up, color);
				}
			}
		}
