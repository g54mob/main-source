...
namespace Pathfinding
{
	...
	public abstract class Path : IPathInternals
	{
		...
		public delegate void OpenCandidateConnectionBurst_0000050F_0024PostfixBurstDelegate(ref OpenCandidateParams pars, ref BinaryHeap heap, ref HeuristicObjective heuristicObjective);

		internal static class OpenCandidateConnectionBurst_0000050F_0024BurstDirectCall
		{
			...
			private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(OpenCandidateConnectionBurst_0000050F_0024PostfixBurstDelegate).TypeHandle);
				}
				P_0 = Pointer;
			}
			...
			static OpenCandidateConnectionBurst_0000050F_0024BurstDirectCall()
			{
				Constructor();
			}
		...
		{
			ref PathNode reference = ref pathHandler.pathNodes[pathNodeIndex];
			reference.pathID = pathID;
			reference.heapIndex = ushort.MaxValue;
			reference.parentIndex = parentNodeIndex;
		...
		{
			OpenCandidateConnectionBurst_0000050F_0024BurstDirectCall.Invoke(ref pars, ref heap, ref heuristicObjective);
		}
		...
		private void InitializeNNConstraint()
		{
			...
			if (traversalProvider != null)
			{
				pathHandler.constraintWrapper.Set(this, nnConstraint, traversalProvider);
			}
			else
			{
				pathHandler.constraintWrapper.Reset();
			}
		}
		...
		protected virtual void CalculateStep(long targetTick)
		{
			...
			while (CompleteState == PathCompleteState.NotCalculated)
			{
				...
				uint g;
				uint f;
				uint num2 = pathHandler.heap.Remove(pathHandler.pathNodes, out g, out f);
				uint num3 = f - g;
