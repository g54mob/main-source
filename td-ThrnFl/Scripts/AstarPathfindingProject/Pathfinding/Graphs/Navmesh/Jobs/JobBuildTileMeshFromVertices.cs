...
namespace Pathfinding.Graphs.Navmesh.Jobs
{
	...
	public struct JobBuildTileMeshFromVertices : IJob
	{
		...
		{
			if (vertices.Length > 1048575)
			{
				throw new ArgumentException("Too many vertices in the navmesh graph. Provided " + vertices.Length + ", but the maximum number of vertices per tile is " + 1048575 + ". You can raise this limit by enabling ASTAR_RECAST_LARGER_TILES in the A* Inspector Optimizations tab");
			}
			NativeArray<TileMesh.TileMeshUnsafe> tiles = new NativeArray<TileMesh.TileMeshUnsafe>(1, Allocator.Persistent);
			return new Promise<BuildNavmeshOutput>(new JobBuildTileMeshFromVertices
			{
				vertices = vertices,
				indices = indices,
			...
			}.Schedule(), new BuildNavmeshOutput
			{
				tiles = tiles
			});
		}
