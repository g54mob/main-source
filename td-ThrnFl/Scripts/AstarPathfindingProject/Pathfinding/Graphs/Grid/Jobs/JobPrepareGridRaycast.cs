...
namespace Pathfinding.Graphs.Grid.Jobs
{
	...
	public struct JobPrepareGridRaycast : IJob
	{
		...
		public void Execute()
		{
			...
			QueryParameters queryParameters = new QueryParameters(raycastMask, hitMultipleFaces: false, QueryTriggerInteraction.Ignore);
			span.Fill(new RaycastCommand(physicsScene, Vector3.zero, normalized, queryParameters, magnitude));
			for (int i = 0; i < size.z; i++)
			{
				...
				for (int j = 0; j < size.x; j++)
				{
					...
					span[index].from = vector + raycastOffset;
				}
			}
		}
	}
