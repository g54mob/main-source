...
namespace Pathfinding.RVO
{
	...
	public static class RVOObstacleCache
	{
		...
		public unsafe delegate void TraceContours_00000F33_0024PostfixBurstDelegate(ref UnsafeSpan<ObstacleSegment> obstaclesSpan, ref NativeMovementPlane movementPlane, int obstacleId, UnmanagedObstacle* outputObstacles, ref SlabAllocator<float3> verticesAllocator, ref SlabAllocator<ObstacleVertexGroup> obstaclesAllocator, ref SpinLock spinLock, bool simplifyObstacles);

		internal static class TraceContours_00000F33_0024BurstDirectCall
		{
			...
			private unsafe static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = (nint)BurstCompiler.GetILPPMethodFunctionPointer2(DeferredCompilation, (RuntimeMethodHandle)/*OpCode not supported: LdMemberToken*/, typeof(TraceContours_00000F33_0024PostfixBurstDelegate).TypeHandle);
				}
				P_0 = Pointer;
			}
			...
			static TraceContours_00000F33_0024BurstDirectCall()
			{
				Constructor();
			}
		...
		{
			TraceContours_00000F33_0024BurstDirectCall.Invoke(ref obstaclesSpan, ref movementPlane, obstacleId, outputObstacles, ref verticesAllocator, ref obstaclesAllocator, ref spinLock, simplifyObstacles);
		}
		...
		public unsafe static void TraceContours_0024BurstManaged(ref UnsafeSpan<ObstacleSegment> obstaclesSpan, ref NativeMovementPlane movementPlane, int obstacleId, UnmanagedObstacle* outputObstacles, ref SlabAllocator<float3> verticesAllocator, ref SlabAllocator<ObstacleVertexGroup> obstaclesAllocator, ref SpinLock spinLock, bool simplifyObstacles)
		{
			...
			{
				if (nativeHashMap.TryGetValue(unsafeSpan[j].vertex2LinkId, out var item) && nativeArray[item] > 0)
				{
					nativeArray[item] = 1;
				}
			}
			...
			for (int k = 0; k <= 1; k++)
			{
				...
				for (int l = 0; l < unsafeSpan.Length; l++)
				{
					...
					while (nativeArray[index] != 0)
					{
						nativeArray[index] = 0;
						float3 value2;
						if (nativeHashMap.TryGetValue(unsafeSpan[index].vertex2LinkId, out var item2))
						{
