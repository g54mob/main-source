...
namespace Pathfinding.RVO
{
	...
	public class RVOSimulator : VersionedMonoBehaviour
	{
		...
		public SimulatorBurst GetSimulator()
		{
			...
			{
				simulatorBurst = new SimulatorBurst(movementPlane);
			}
		...
		private void Update()
		{
			if (Application.isPlaying)
			{
				...
				SimulatorBurst simulator = GetSimulator();
				simulator.DesiredDeltaTime = 1f / (float)desiredSimulationFPS;
				simulator.SymmetryBreakingBias = symmetryBreakingBias;
				simulator.HardCollisions = hardCollisions;
				simulator.drawQuadtree = drawQuadtree;
				simulator.UseNavmeshAsObstacle = useNavmeshAsObstacle;
				simulator.Update(default(JobHandle), Time.deltaTime, drawGizmos: true, Allocator.TempJob).Complete();
			}
		}
