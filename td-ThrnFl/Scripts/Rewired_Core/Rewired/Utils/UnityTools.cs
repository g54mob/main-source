...
namespace Rewired.Utils
{
	...
	{
		internal struct amJDSPAsoSsFGKpTfETGMJsMbYUx
		{
			public Platform HmfYsdTKnbYRlUQZnnozMmgaHnTHA;

			public Platform hXikvhrgLYyqQzWzHRKvCsZVMYHT;

			public EditorPlatform jvEibUQDyGcThFzRpxfstVwCaNgR;

			public bool kJtxEQzislVvFLCXcyThEqKhaQRU;

			public WebplayerPlatform UVzLeanWmLAGrqmxnJAMbnhpuOVi;

			public ScriptingBackend cMkLwMQgJRUDGhDQlAQREGOzNsIv;

			public ScriptingAPILevel xyONvXFCoMVhcNGRMoMPeHzXUmhf;

			public IExternalTools ixjDaTurTjuXEUWAMcHOPWNSeyOR;

			public amJDSPAsoSsFGKpTfETGMJsMbYUx(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				HmfYsdTKnbYRlUQZnnozMmgaHnTHA = P_0;
				hXikvhrgLYyqQzWzHRKvCsZVMYHT = P_1;
				jvEibUQDyGcThFzRpxfstVwCaNgR = P_2;
				kJtxEQzislVvFLCXcyThEqKhaQRU = P_3;
				UVzLeanWmLAGrqmxnJAMbnhpuOVi = P_4;
				cMkLwMQgJRUDGhDQlAQREGOzNsIv = P_5;
				xyONvXFCoMVhcNGRMoMPeHzXUmhf = P_6;
				ixjDaTurTjuXEUWAMcHOPWNSeyOR = P_7;
			}
		}
		...
		{
			public enum CbbModEdhQRkRqgjofnHKbMwRfXUA
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly CbbModEdhQRkRqgjofnHKbMwRfXUA type;

			public readonly int build;

			public UnityVersionClass(string P_0)
			{
				type = CbbModEdhQRkRqgjofnHKbMwRfXUA.Normal;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = CbbModEdhQRkRqgjofnHKbMwRfXUA.Beta;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = CbbModEdhQRkRqgjofnHKbMwRfXUA.Patch;
					}
			...
			public override string ToString()
			{
				...
				array[5] = EFeVLLrXgnOVIuqWfcAzlCFMgYSH(type);
				num = build;
				array[6] = num.ToString();
				return string.Concat(array);
			}

			private string EFeVLLrXgnOVIuqWfcAzlCFMgYSH(CbbModEdhQRkRqgjofnHKbMwRfXUA P_0)
			{
				return P_0 switch
				{
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Normal => "f", 
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Beta => "b", 
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Patch => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (VbsHgsGBPxDxyAPDbqStlonpQUVkB(a.type) > VbsHgsGBPxDxyAPDbqStlonpQUVkB(b.type))
				{
					return 1;
				}
				if (VbsHgsGBPxDxyAPDbqStlonpQUVkB(a.type) < VbsHgsGBPxDxyAPDbqStlonpQUVkB(b.type))
			...
			private static int VbsHgsGBPxDxyAPDbqStlonpQUVkB(CbbModEdhQRkRqgjofnHKbMwRfXUA P_0)
			{
				return P_0 switch
				{
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Beta => 0, 
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Normal => 10, 
					CbbModEdhQRkRqgjofnHKbMwRfXUA.Patch => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion LUlsgFfJONqBBhrLZSNQgTzBrbLi = UnityVersion.UNITY_5_0;

		private static UnityVersionClass oZALwaBqQJehXttNEuTJyiBHKgOb;

		private static UnityVersion BnNdqoDuEiRAMSwnaCYZrZFsWLtk = UnityVersion.Unknown;

		private static string TWTAjNCiDEbMCIRTuaGswWYINMJkA;

		private static Platform fDtLKnggoMFehBBYvxYtyOAvUpdB;

		private static EditorPlatform tBiZfucWFXSkSLOCvBYSGribTJfI;

		private static bool RLcZrbIdWmotGznhZScZShknebub;

		private static bool uAaIsYkogDMObSGsCDuJngkaxLMqA;

		private static bool KiTOLKgeOdWIqAYyuonbbiRyvFhO;

		private static WebplayerPlatform QDRkeWBULADWmFhLdnNvDaMnygmE;

		private static bool fuigfhKoYNhdhWGlSyJTmojmKMaZA;

		private static bool orUFzDuVwtrESlIICWrEsVIetxBm;

		private static bool SAsafNKJvFbRFxWWyAjEAvukqaSB;

		private static bool hLYRSDQRBAhoVIxfbTKzsMvmxWrS;

		private static bool ITJtrvLGcYBLiujgJEgqKeHJXehi;

		private static bool LoABIQDIYZkdAChpeufchgbMltfEA;

		private static string AUCxiCfXBudPTADjClVXHrfxTugA;

		private static ScriptingBackend yheyCSkRWprpatmCYqfsMlYvfCpy;

		private static ScriptingAPILevel vXJbudRqhoxaSCIQedovifZQBKohb;

		private static bool HHRgvFMVXCflRQtxeFcydnwwsrJcA;

		private static IExternalTools SbRRIycjSzuYkVLshTkxOJCDGQTl;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper pkWpgXwcCWRHFveobpKfAjpttRdh;

		private static bool mgjPeobasHeinaXHHBEMxbupVHlBA;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				if (!hqgCdtmiHNERFHOOVpzCYyVzMlQJ)
				{
					return null;
				}
				return oZALwaBqQJehXttNEuTJyiBHKgOb;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return TWTAjNCiDEbMCIRTuaGswWYINMJkA;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return fDtLKnggoMFehBBYvxYtyOAvUpdB;
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return tBiZfucWFXSkSLOCvBYSGribTJfI;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return RLcZrbIdWmotGznhZScZShknebub;
			}
		}

		public static bool isPlaying => uAaIsYkogDMObSGsCDuJngkaxLMqA;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return KiTOLKgeOdWIqAYyuonbbiRyvFhO;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return QDRkeWBULADWmFhLdnNvDaMnygmE;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (RLcZrbIdWmotGznhZScZShknebub || Application.isEditor)
				{
					return true;
				}
				...
				switch (fDtLKnggoMFehBBYvxYtyOAvUpdB)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!KiTOLKgeOdWIqAYyuonbbiRyvFhO)
					{
						return yheyCSkRWprpatmCYqfsMlYvfCpy == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return tBiZfucWFXSkSLOCvBYSGribTJfI switch
				{
					EditorPlatform.Windows => fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.Windows, 
					EditorPlatform.OSX => fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.OSX, 
					EditorPlatform.Linux => fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return fuigfhKoYNhdhWGlSyJTmojmKMaZA;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return orUFzDuVwtrESlIICWrEsVIetxBm;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool supportsPhysicalKeys
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_2021_2;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.Android && fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.Ouya && fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.AmazonFireTV)
				{
					return fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.iOS)
				{
					return fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.Windows && fDtLKnggoMFehBBYvxYtyOAvUpdB != Platform.Linux)
				{
					return fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return SAsafNKJvFbRFxWWyAjEAvukqaSB;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_0)
				{
					return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return HHRgvFMVXCflRQtxeFcydnwwsrJcA;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => yheyCSkRWprpatmCYqfsMlYvfCpy;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => vXJbudRqhoxaSCIQedovifZQBKohb;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return SbRRIycjSzuYkVLshTkxOJCDGQTl;
			}
		}

		internal static IAndroidFallbackPlatformHelper RddtjxiMwGqXwmDyXwJZYBQUTDqg
		{
			[CompilerGenerated]
			get
			{
				return pkWpgXwcCWRHFveobpKfAjpttRdh;
			}
			[CompilerGenerated]
			set
			{
				pkWpgXwcCWRHFveobpKfAjpttRdh = androidFallbackPlatformHelper;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => mgjPeobasHeinaXHHBEMxbupVHlBA;

		private static bool hqgCdtmiHNERFHOOVpzCYyVzMlQJ => fyOxHfOCDEGMUBdSMgYECSqIwkByb();

		private static bool fyOxHfOCDEGMUBdSMgYECSqIwkByb()
		{
			if (mgjPeobasHeinaXHHBEMxbupVHlBA)
			{
				return true;
			}
			try
			{
				TWTAjNCiDEbMCIRTuaGswWYINMJkA = Application.unityVersion;
				oZALwaBqQJehXttNEuTJyiBHKgOb = new UnityVersionClass(TWTAjNCiDEbMCIRTuaGswWYINMJkA);
				pfDuqBpEjjhQbCMnKSwnncRCedsU();
				mgjPeobasHeinaXHHBEMxbupVHlBA = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return mgjPeobasHeinaXHHBEMxbupVHlBA;
		}

		internal static void BEZAPDgZfPEHjYqsofsLFjGWikIAA(amJDSPAsoSsFGKpTfETGMJsMbYUx P_0)
		{
			if (hqgCdtmiHNERFHOOVpzCYyVzMlQJ)
			{
				if (P_0.hXikvhrgLYyqQzWzHRKvCsZVMYHT == Platform.Windows81Store)
				{
					P_0.hXikvhrgLYyqQzWzHRKvCsZVMYHT = Platform.WindowsUWP;
				}
				fDtLKnggoMFehBBYvxYtyOAvUpdB = P_0.hXikvhrgLYyqQzWzHRKvCsZVMYHT;
				tBiZfucWFXSkSLOCvBYSGribTJfI = P_0.jvEibUQDyGcThFzRpxfstVwCaNgR;
				RLcZrbIdWmotGznhZScZShknebub = P_0.kJtxEQzislVvFLCXcyThEqKhaQRU;
				QDRkeWBULADWmFhLdnNvDaMnygmE = P_0.UVzLeanWmLAGrqmxnJAMbnhpuOVi;
				yheyCSkRWprpatmCYqfsMlYvfCpy = P_0.cMkLwMQgJRUDGhDQlAQREGOzNsIv;
				vXJbudRqhoxaSCIQedovifZQBKohb = P_0.xyONvXFCoMVhcNGRMoMPeHzXUmhf;
				if (SbRRIycjSzuYkVLshTkxOJCDGQTl != null)
				{
					SbRRIycjSzuYkVLshTkxOJCDGQTl.Destroy();
				}
				SbRRIycjSzuYkVLshTkxOJCDGQTl = P_0.ixjDaTurTjuXEUWAMcHOPWNSeyOR;
				KiTOLKgeOdWIqAYyuonbbiRyvFhO = Debug.isDebugBuild;
				uAaIsYkogDMObSGsCDuJngkaxLMqA = true;
				oUmWyOIaMCaXAHwbvjybAnHgbtFuA();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			eSRKZStsJfGpulXjFXRSdFCJxCEI(minVersionStr, out var num);
			eSRKZStsJfGpulXjFXRSdFCJxCEI(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (num3 && oZALwaBqQJehXttNEuTJyiBHKgOb < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag && oZALwaBqQJehXttNEuTJyiBHKgOb >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag && oZALwaBqQJehXttNEuTJyiBHKgOb > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool eSRKZStsJfGpulXjFXRSdFCJxCEI(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void pfDuqBpEjjhQbCMnKSwnncRCedsU()
		{
			BnNdqoDuEiRAMSwnaCYZrZFsWLtk = TGUXrdZRUvDuyrxngUkOCzQkLTGI(Application.unityVersion);
			if (BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_3_5 && BnNdqoDuEiRAMSwnaCYZrZFsWLtk < UnityVersion.UNITY_4_0)
			{
				fuigfhKoYNhdhWGlSyJTmojmKMaZA = true;
			}
			else if (BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_0)
			{
				orUFzDuVwtrESlIICWrEsVIetxBm = true;
			}
		}

		private static UnityVersion TGUXrdZRUvDuyrxngUkOCzQkLTGI(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion gqOdbHGuMMrVRIJqxjofTmllDUsMA(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion PZZEPhLENGmYwPjOlwypZPZkkslm(int P_0)
		{
			return P_0 switch
			{
		...
		private static void oUmWyOIaMCaXAHwbvjybAnHgbtFuA()
		{
			switch (fDtLKnggoMFehBBYvxYtyOAvUpdB)
			{
			...
			case Platform.RazerForgeTV:
				hLYRSDQRBAhoVIxfbTKzsMvmxWrS = true;
				ITJtrvLGcYBLiujgJEgqKeHJXehi = true;
				break;
			case Platform.PS4:
				hLYRSDQRBAhoVIxfbTKzsMvmxWrS = true;
				AUCxiCfXBudPTADjClVXHrfxTugA = "Empty";
				LoABIQDIYZkdAChpeufchgbMltfEA = true;
				break;
			case Platform.Windows:
				if ((BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_6_3p1 && BnNdqoDuEiRAMSwnaCYZrZFsWLtk < UnityVersion.UNITY_5_0) || BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_0_0p1)
				{
					SAsafNKJvFbRFxWWyAjEAvukqaSB = true;
					hLYRSDQRBAhoVIxfbTKzsMvmxWrS = true;
				}
				break;
			case Platform.Linux:
			...
			if (RLcZrbIdWmotGznhZScZShknebub && tBiZfucWFXSkSLOCvBYSGribTJfI == EditorPlatform.Windows && ((BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_6_3p1 && BnNdqoDuEiRAMSwnaCYZrZFsWLtk < UnityVersion.UNITY_5_0) || BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_5_0_0p1))
			{
				SAsafNKJvFbRFxWWyAjEAvukqaSB = true;
				hLYRSDQRBAhoVIxfbTKzsMvmxWrS = true;
			}
			if ((!RLcZrbIdWmotGznhZScZShknebub && fDtLKnggoMFehBBYvxYtyOAvUpdB == Platform.Windows) || (RLcZrbIdWmotGznhZScZShknebub && tBiZfucWFXSkSLOCvBYSGribTJfI == EditorPlatform.Windows))
			{
				HHRgvFMVXCflRQtxeFcydnwwsrJcA = BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type YmpShZLiVRdIrWHVaUlBdMuNZTGS(peYZweFjfepeGswTzSkwwkZwcAgIA P_0)
		{
			if (!hqgCdtmiHNERFHOOVpzCYyVzMlQJ)
			{
				return null;
			}
			if (BnNdqoDuEiRAMSwnaCYZrZFsWLtk >= UnityVersion.UNITY_4_3)
			{
				return DenwuncfoYiMLtIXziSxmrqTNPdh(P_0);
			}
			return null;
		}

		private static Type DenwuncfoYiMLtIXziSxmrqTNPdh(peYZweFjfepeGswTzSkwwkZwcAgIA P_0)
		{
			return P_0 switch
			{
				peYZweFjfepeGswTzSkwwkZwcAgIA.RigidbodyInterpolation2D => typeof(RigidbodyInterpolation2D), 
				peYZweFjfepeGswTzSkwwkZwcAgIA.RigidbodySleepMode2D => typeof(RigidbodySleepMode2D), 
				peYZweFjfepeGswTzSkwwkZwcAgIA.CollisionDetectionMode2D => typeof(CollisionDetectionMode2D), 
				peYZweFjfepeGswTzSkwwkZwcAgIA.PhysicsMaterial2D => typeof(PhysicsMaterial2D), 
				peYZweFjfepeGswTzSkwwkZwcAgIA.Collider2D => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return OHcCvYTNkkGOOZjTGOfPEgGCyWNJ(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return OHcCvYTNkkGOOZjTGOfPEgGCyWNJ((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return OHcCvYTNkkGOOZjTGOfPEgGCyWNJ((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return OHcCvYTNkkGOOZjTGOfPEgGCyWNJ(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && SAsafNKJvFbRFxWWyAjEAvukqaSB)
				{
					return false;
				}
				if (hLYRSDQRBAhoVIxfbTKzsMvmxWrS)
				{
					return false;
				}
			}
			...
			{
				if (LoABIQDIYZkdAChpeufchgbMltfEA && name.Equals(AUCxiCfXBudPTADjClVXHrfxTugA, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (ITJtrvLGcYBLiujgJEgqKeHJXehi && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static _0001 OHcCvYTNkkGOOZjTGOfPEgGCyWNJ<_0001>(_0001 P_0) where _0001 : class
		{
			if (P_0 == null)
			{
				return null;
		...
		internal static ButtonStateFlags VzzfRlzisWRDJkLRERSsCluNQKCA(KeyCode P_0)
		{
		...
		internal static ButtonStateFlags nNtYedvlmJyVuLgOSmlsuWCiiTgX(string P_0)
		{
