...
namespace Rewired.Utils
{
	...
	{
		private static uint SppKwonDxSIAOuGKXfvnfdwYnmdnA = 0u;

		private static uint vKdfPziVdGeVjGOyCDJcLkmZeQcQB = 1u;

		private static uint fOGfbazJbKIMAIiXpiUdmAamDTDXA = uint.MaxValue;

		private static int tfaoeEOSxbFunIPEkatgyOvQuHfi = 0;

		private static int TySgZTdyTFUeNgCbmxxgoqmlrnOI = int.MaxValue;

		private static int HTYddlKIfMCxUHJckWnYztkgCNpe = -1;
		...
		{
			counter = ((counter != fOGfbazJbKIMAIiXpiUdmAamDTDXA) ? (counter + 1) : vKdfPziVdGeVjGOyCDJcLkmZeQcQB);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != TySgZTdyTFUeNgCbmxxgoqmlrnOI) ? (counter + 1) : tfaoeEOSxbFunIPEkatgyOvQuHfi);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != vKdfPziVdGeVjGOyCDJcLkmZeQcQB) ? (counter - 1) : fOGfbazJbKIMAIiXpiUdmAamDTDXA);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= HTYddlKIfMCxUHJckWnYztkgCNpe)
			{
				return TySgZTdyTFUeNgCbmxxgoqmlrnOI;
			}
			counter = ((counter != tfaoeEOSxbFunIPEkatgyOvQuHfi) ? (counter - 1) : TySgZTdyTFUeNgCbmxxgoqmlrnOI);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != SppKwonDxSIAOuGKXfvnfdwYnmdnA;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > HTYddlKIfMCxUHJckWnYztkgCNpe;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == SppKwonDxSIAOuGKXfvnfdwYnmdnA)
			{
				return false;
			}
			if (tick2 == SppKwonDxSIAOuGKXfvnfdwYnmdnA)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= HTYddlKIfMCxUHJckWnYztkgCNpe)
			{
				return false;
			}
			if (tick2 <= HTYddlKIfMCxUHJckWnYztkgCNpe)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = fOGfbazJbKIMAIiXpiUdmAamDTDXA - num + num2;
			uint num4 = vKdfPziVdGeVjGOyCDJcLkmZeQcQB;
			uint num5 = num3 - num4;
			if (tick1 >= tick2)
			{
				return 0L - (long)num5;
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = TySgZTdyTFUeNgCbmxxgoqmlrnOI - num + num2;
			int num4 = tfaoeEOSxbFunIPEkatgyOvQuHfi;
			int num5 = num3 - num4;
			if (tick1 >= tick2)
			{
				return -num5;
