...
namespace Rewired
{
	...
	{
		private enum cjfiHTQAGPYqwnncwGBiqBLKOttP
		{
			Initialization = 0,
			Update = 1,
			Destroy = 2
		...
		[NonSerialized]
		private bool IBgffWHWwHBZpsJhAZcYbnlOHsfiA;

		[NonSerialized]
		private bool oCcGgvKExeFiJjlapHObxOxthBTKA;
		...
		[NonSerialized]
		private bool mKHdosuhDBiQHhNPnoDhcKgKwizXB;

		private bool TjjmVxGGpdczLGtvPfSGgQlgyjGdA;
		...
		{
			if ((Application.isPlaying || _userData.ConfigVars.runInEditMode) && (!Application.isPlaying || TjjmVxGGpdczLGtvPfSGgQlgyjGdA) && !IBgffWHWwHBZpsJhAZcYbnlOHsfiA && !oCcGgvKExeFiJjlapHObxOxthBTKA)
			{
				UTPXBrVjuDkIdlaNbJiHCNKBIUTN();
			}
		}
		...
		private void OnDisable()
		{
			...
			{
				EjwxaGqkkMOhJdAhRhwUFffLuClrA(true);
			}
		}
		...
		{
			EjwxaGqkkMOhJdAhRhwUFffLuClrA(false);
		}

		private void EjwxaGqkkMOhJdAhRhwUFffLuClrA(bool P_0)
		{
			IBgffWHWwHBZpsJhAZcYbnlOHsfiA = false;
			mKHdosuhDBiQHhNPnoDhcKgKwizXB = false;
			oCcGgvKExeFiJjlapHObxOxthBTKA = false;
			try
			{
				if (ReInput.rewiredInputManager == this)
				{
					ReInput.TBgXUfbmBYODdNHdOSveXjyCBSLo();
				}
			}
			catch (Exception ex)
			{
				MzVZMAcgWDrlRnvychqSyKdHgPv(cjfiHTQAGPYqwnncwGBiqBLKOttP.Destroy, "destruction", ex);
			}
			OnDeinitialized();
		}
		...
		{
			if (!mKHdosuhDBiQHhNPnoDhcKgKwizXB)
			{
				ReInput.AxiDcWUBWsAafeGKYNyNRegxhJVGb(isFocused);
				_ = IBgffWHWwHBZpsJhAZcYbnlOHsfiA;
			}
		}
		...
		private void OnApplicationPause(bool isPaused)
		{
			if (!mKHdosuhDBiQHhNPnoDhcKgKwizXB)
			{
				ReInput.eUpNJxBTnrHDvfuvwTSuYlafNbkq(isPaused);
				_ = IBgffWHWwHBZpsJhAZcYbnlOHsfiA;
			}
		}
		...
		private void Start()
		{
			if ((!UnityTools.isEditor || Application.isPlaying) && IBgffWHWwHBZpsJhAZcYbnlOHsfiA && !oCcGgvKExeFiJjlapHObxOxthBTKA)
			{
				ReInput.tbFbwsEDVlNtPBYbXlzrrStSJABP();
			}
		}
		...
		private void Update()
		{
			if (IBgffWHWwHBZpsJhAZcYbnlOHsfiA && !oCcGgvKExeFiJjlapHObxOxthBTKA && (!UnityTools.isEditor || Application.isPlaying) && _userData.ConfigVars.updateMode != UpdateMode.Manual)
			{
		...
		{
			if (!IBgffWHWwHBZpsJhAZcYbnlOHsfiA || oCcGgvKExeFiJjlapHObxOxthBTKA || (UnityTools.isEditor && !Application.isPlaying))
			{
				return;
			}
			try
			{
				ReInput.rZBPDYNFwpVkSTbVTZpXDJejCsJW();
			}
			catch (Exception ex)
			{
				MzVZMAcgWDrlRnvychqSyKdHgPv(cjfiHTQAGPYqwnncwGBiqBLKOttP.Update, "update (Late Update)", ex);
			}
		}

		internal void OnGUIUpdate()
		{
			if (IBgffWHWwHBZpsJhAZcYbnlOHsfiA && !oCcGgvKExeFiJjlapHObxOxthBTKA && (!UnityTools.isEditor || Application.isPlaying) && _userData.ConfigVars.updateMode != UpdateMode.Manual && (_userData.ConfigVars.updateLoop & UpdateLoopSetting.OnGUI) != UpdateLoopSetting.None)
			{
		...
		{
			if (!IBgffWHWwHBZpsJhAZcYbnlOHsfiA || oCcGgvKExeFiJjlapHObxOxthBTKA)
			{
				return;
			}
			try
			{
				CheckRecompile();
				ReInput.TFngseMrhJgIIJntLiTkvjRkyEZaA(updateLoopType);
				if ((_userData.ConfigVars.updateLoop & updateLoopSettingBit) != UpdateLoopSetting.None)
				{
					ReInput.FhWOZsggaFCRaZsidilrDcytCTmMA(updateLoopType);
				}
			}
			catch (Exception ex)
			{
				MzVZMAcgWDrlRnvychqSyKdHgPv(cjfiHTQAGPYqwnncwGBiqBLKOttP.Update, "update (" + updateLoopType.ToString() + ")", ex);
			}
		}

		internal void TryStartRunInEditMode()
		{
			if (IBgffWHWwHBZpsJhAZcYbnlOHsfiA || !Application.isEditor || Application.isPlaying)
			{
				return;
			}
			if (ReInput.isReady)
		...
		private bool UTPXBrVjuDkIdlaNbJiHCNKBIUTN()
		{
			if (IBgffWHWwHBZpsJhAZcYbnlOHsfiA)
			{
				return true;
			}
			nohiDsBGlTWIxFntgCtgtUGRqTzF();
			...
			return IBgffWHWwHBZpsJhAZcYbnlOHsfiA;
		}

		private void nohiDsBGlTWIxFntgCtgtUGRqTzF()
		{
			if (mKHdosuhDBiQHhNPnoDhcKgKwizXB)
			{
				return;
			...
			try
			{
				if (!FtAMxcBAybNkheaXWRzaSmQdvIuW())
				{
					return;
				}
				if (_dontDestroyOnLoad && Application.isPlaying)
				...
				DetectPlatform();
				List<ICustomPlatformInitializer> componentsInSelfAndChildren = UnityTools.GetComponentsInSelfAndChildren<ICustomPlatformInitializer>(base.gameObject);
				if (componentsInSelfAndChildren != null)
				{
					for (int i = 0; i < componentsInSelfAndChildren.Count; i++)
					{
						...
						{
							NaCVyIMwuDhgdNZldvvjhuHYfOGS.SfQSLorWxgpuGqWHHrSXKyQlHXeE(customPlatformInitOptions);
							this.platform = Platform.Custom;
				...
				string text = wqGVGosfgEHaskFDlwFWHqjiuoYdA();
				UnityTools.amJDSPAsoSsFGKpTfETGMJsMbYUx amJDSPAsoSsFGKpTfETGMJsMbYUx = new UnityTools.amJDSPAsoSsFGKpTfETGMJsMbYUx(platform, this.platform, editorPlatform, isEditor, webplayerPlatform, scriptingBackend, scriptingAPILevel, GetExternalTools());
				ReInput.dssmoswSSXXfGqrywuweJDuoTcbl(this, AfzLcxJGnqcddjLMbjjrhvZxNZNl, _userData.ConfigVars, _controllerDataFiles, _userData, amJDSPAsoSsFGKpTfETGMJsMbYUx, rtxspwlzuWkJDuKpilcolfAdgjuU);
				IBgffWHWwHBZpsJhAZcYbnlOHsfiA = true;
				oCcGgvKExeFiJjlapHObxOxthBTKA = false;
				if (!string.IsNullOrEmpty(text))
				{
			...
			{
				MzVZMAcgWDrlRnvychqSyKdHgPv(cjfiHTQAGPYqwnncwGBiqBLKOttP.Initialization, "initialization", ex);
			}
		}

		private void rtxspwlzuWkJDuKpilcolfAdgjuU(Platform P_0)
		{
			platform = P_0;
		}

		private object AfzLcxJGnqcddjLMbjjrhvZxNZNl(ConfigVars P_0)
		{
			List<Assembly> list = ((UnityTools.unityVersion >= UnityTools.UnityVersion.UNITY_5_0) ? null : WrPqEVZnDzbnoHLbonGifBmboTGhb());
			return WPUsVLvIKoiluyoUtdcARrtcCvPs.BPNvZpVnJiVICyChSmySKUbsUOUf(PCNbNgoGgUOHMZSbkuEoQCtymyeN(), list, P_0);
		}

		private List<Assembly> WrPqEVZnDzbnoHLbonGifBmboTGhb()
		{
			List<TextAsset> list = new List<TextAsset>();
			mRMZCdhgRKvchEugnSlZngKawMZT(list, UnityTools.GetCurrentPlatformResourecesDLLPaths());
		...
		private List<Assembly> JICMgxAcKDEltkROshrTdoOroXHQ()
		{
			bool flag = false;
			try
			{
				if (string.IsNullOrEmpty(PCNbNgoGgUOHMZSbkuEoQCtymyeN()))
				{
					return null;
				}
				...
				Assembly assembly = Array.Find(assemblies, (Assembly P_0) => string.Equals(P_0.GetName().Name, PCNbNgoGgUOHMZSbkuEoQCtymyeN(), StringComparison.OrdinalIgnoreCase));
				if (assembly == null)
				{
					flag = true;
					throw new Exception();
		...
		private byte[] JuhLusbmYuXieUfMgfSHDgkKsDOf()
		{
			try
			{
				string text = PCNbNgoGgUOHMZSbkuEoQCtymyeN();
				if (string.IsNullOrEmpty(text))
				{
					return null;
				}
		...
		private void mRMZCdhgRKvchEugnSlZngKawMZT(List<TextAsset> P_0, List<string> P_1)
		{
		...
		private string wqGVGosfgEHaskFDlwFWHqjiuoYdA()
		{
			if (editorPlatform == EditorPlatform.None)
			{
				return null;
			}
			if (MGxuHSdZApfzdxFIzQMJJQuLeOWG())
			{
				return null;
			}
		...
		private bool MGxuHSdZApfzdxFIzQMJJQuLeOWG()
		{
			switch (editorPlatform)
			{
			case EditorPlatform.Windows:
		...
		private string PCNbNgoGgUOHMZSbkuEoQCtymyeN()
		{
		...
		private bool FtAMxcBAybNkheaXWRzaSmQdvIuW()
		{
			mKHdosuhDBiQHhNPnoDhcKgKwizXB = false;
			if (ReInput.isReady)
			{
				...
				mKHdosuhDBiQHhNPnoDhcKgKwizXB = true;
		...
		{
			ReInput.jBLTJZRGATWGsqKLdEGwsHkDqsqG();
			ReInput.TBgXUfbmBYODdNHdOSveXjyCBSLo();
		}
		...
		protected void OnSceneLoaded()
		{
			if (ReInput.isReady)
			{
				ReInput.ltHxLukkbhePSbKkCkrXTjSCMYDXA();
			}
		}

		private void MzVZMAcgWDrlRnvychqSyKdHgPv(cjfiHTQAGPYqwnncwGBiqBLKOttP P_0, string P_1, Exception P_2)
		{
			P_1 = "An exception occurred during " + P_1 + ".";
			bool flag = false;
			if (P_0 == cjfiHTQAGPYqwnncwGBiqBLKOttP.Initialization || P_0 == cjfiHTQAGPYqwnncwGBiqBLKOttP.Destroy)
			{
			...
			if (flag)
			{
				oCcGgvKExeFiJjlapHObxOxthBTKA = true;
			}
		}
		...
		{
			if (!IBgffWHWwHBZpsJhAZcYbnlOHsfiA && !_detectedPlatformInEditor)
			{
				DetectPlatform();
			}
		...
		[CompilerGenerated]
		private bool WAbtRnFTeSVIyEfURyGLpXofpbGg(Assembly P_0)
		{
			return string.Equals(P_0.GetName().Name, PCNbNgoGgUOHMZSbkuEoQCtymyeN(), StringComparison.OrdinalIgnoreCase);
		}
	}
}
