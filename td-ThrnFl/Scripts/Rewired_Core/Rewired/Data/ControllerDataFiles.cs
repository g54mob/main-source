...
namespace Rewired.Data
{
	public sealed class ControllerDataFiles : ScriptableObject
	{
		...
		[NonSerialized]
		private bool hOTSXRGSTUpfQwkxRFOxwghfFIwQ;

		[NonSerialized]
		private readonly ADictionary<Guid, aeTKcrzfQkODTQybGHqaOyCSCntK> FqSCIQrukqjEbekcvRpREGakWKQFA = new ADictionary<Guid, aeTKcrzfQkODTQybGHqaOyCSCntK>(EqualityComparerNoAlloc<Guid>.Default);
		...
		internal ControllerTemplateElementIdentifier HLFyBOdjBIFFFrTlJLCAXVyMGFbW(Guid P_0, int P_1, out HardwareJoystickMap P_2)
		{
			P_2 = null;
			if (P_1 < 0)
			{
			...
			{
				aeTKcrzfQkODTQybGHqaOyCSCntK aeTKcrzfQkODTQybGHqaOyCSCntK2 = RdHErwqZumdNWglnFCBLeTfvNNngA(templateGuid);
				if (aeTKcrzfQkODTQybGHqaOyCSCntK2 != null)
				{
					ControllerTemplateElementIdentifier controllerTemplateElementIdentifier = aeTKcrzfQkODTQybGHqaOyCSCntK2.IZaXoSJWdKSjxPetLPqSfEfbqLCk(P_0, P_1);
		...
		internal int RdLEUSHgbDOWAhtSAKwlEPbqtaaEc(Guid P_0, Guid P_1, int P_2, List<HardwareControllerTemplateMap.FEhsPJDpJWJwcwxoKFKKvWvFgmlN> P_3)
		{
			if (P_2 < 0)
			{
				return 0;
			...
			return RdHErwqZumdNWglnFCBLeTfvNNngA(P_0)?.semPIdnpgTrlyqbqibZiKvatbSUCA(P_1, P_2, P_3) ?? 0;
		}

		internal HardwareJoystickMap_InputManager JZrLJldcHgZaaqAkYOuvBECmOmao(Guid P_0, InputSource P_1)
		{
			GgWvIDqZkrbsJuYaMXTssIEAmeaB();
			...
			if (hardwareJoystickMap != null)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = SecRAIvCUJNwKjDJjKGFqxAkiABX(hardwareJoystickMap, bridgedController, true, out inputPlatform, out num, out platform);
		...
		internal HardwareJoystickMap_InputManager RsZhfHLkrSfOaCHswjERhSpfpzeG(BridgedControllerHWInfo P_0)
		{
			if (P_0 == null)
			{
				return null;
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = shgcLvNRCanhEfKCLwgevrcAHtjL(P_0);
				if (hardwareJoystickMap_InputManager != null)
				{
					return hardwareJoystickMap_InputManager;
				}
			...
			for (int i = 0; i < hardwareJoystickMaps.Length; i++)
			{
				...
				int num;
				HardwareJoystickMap.Platform platform;
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager2 = SecRAIvCUJNwKjDJjKGFqxAkiABX(hardwareJoystickMaps[i], P_0, true, out inputPlatform, out num, out platform);
			...
			for (int j = 0; j < hardwareJoystickMaps.Length; j++)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager3 = SecRAIvCUJNwKjDJjKGFqxAkiABX(hardwareJoystickMaps[j], P_0, false, out inputPlatform2, out num2, out platform2);
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager4 = DAuTImKhmXgnIdTiEziPoeldbvASA(P_0, "[UNITY PRECONFIGURED JOYSTICK]");
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager5 = shgcLvNRCanhEfKCLwgevrcAHtjL(P_0);
		...
		private HardwareJoystickMap_InputManager SecRAIvCUJNwKjDJjKGFqxAkiABX(HardwareJoystickMap P_0, BridgedControllerHWInfo P_1, bool P_2, out InputPlatform P_3, out int P_4, out HardwareJoystickMap.Platform P_5)
		{
		...
		private HardwareJoystickMap_InputManager DAuTImKhmXgnIdTiEziPoeldbvASA(BridgedControllerHWInfo P_0, string P_1)
		{
		...
		private HardwareJoystickMap_InputManager shgcLvNRCanhEfKCLwgevrcAHtjL(BridgedControllerHWInfo P_0)
		{
			HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = DAuTImKhmXgnIdTiEziPoeldbvASA(P_0, "[STANDARDIZED GAMEPAD]");
			if (hardwareJoystickMap_InputManager == null)
			{
				return null;
			}
		...
		internal aeTKcrzfQkODTQybGHqaOyCSCntK RdHErwqZumdNWglnFCBLeTfvNNngA(Guid P_0)
		{
			if (FqSCIQrukqjEbekcvRpREGakWKQFA.TryGetValue(P_0, out var value))
			{
				return value;
			}
			...
			value = joystickTemplate.akYUDiwoAbNPCQnIfBhIaXPrjeXK();
			FqSCIQrukqjEbekcvRpREGakWKQFA.Add(P_0, value);
			return value;
		}

		internal IHardwareControllerTemplateMap IUzmZIFMyTBnJcguSDJIbzJjDAQGA(Guid P_0)
		{
			return RdHErwqZumdNWglnFCBLeTfvNNngA(P_0);
		}

		private void GgWvIDqZkrbsJuYaMXTssIEAmeaB()
		{
			if (!hOTSXRGSTUpfQwkxRFOxwghfFIwQ)
			{
				...
				{
					hOTSXRGSTUpfQwkxRFOxwghfFIwQ = true;
				}
			}
		}
	}
