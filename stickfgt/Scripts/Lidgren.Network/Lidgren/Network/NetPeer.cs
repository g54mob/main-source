...
namespace Lidgren.Network
{
	public class NetPeer
	{
		...
		{
			NetConnection value;
			m_connectionLookup.TryGetValue(ep, out value);
			return value;
		}
		...
		{
			NetIncomingMessage item;
			if (m_releasedIncomingMessages.TryDequeue(out item) && item.MessageType == NetIncomingMessageType.StatusChanged)
			{
		...
		internal void SendLibrary(NetOutgoingMessage msg, IPEndPoint recipient)
		{
			...
			SendPacket(numBytes, recipient, 1, out connectionReset);
		...
		public virtual NetConnection Connect(IPEndPoint remoteEndPoint, NetOutgoingMessage hailMessage)
		{
			...
			lock (m_connections)
			{
				...
				NetConnection value;
				if (m_handshakes.TryGetValue(remoteEndPoint, out value))
				{
					...
					case NetConnectionStatus.RespondedConnect:
						value.SendConnectResponse(NetTime.Now, false);
						break;
					default:
		...
		public void RawSend(byte[] arr, int offset, int length, IPEndPoint destination)
		{
			...
			bool connectionReset;
			SendPacket(length, destination, 1, out connectionReset);
		}
		...
		private void HandleReleasedFragment(NetIncomingMessage im)
		{
			...
			Dictionary<int, ReceivedFragmentGroup> value;
			if (!m_receivedFragmentGroups.TryGetValue(im.SenderConnection, out value))
			{
				value = new Dictionary<int, ReceivedFragmentGroup>();
				m_receivedFragmentGroups[im.SenderConnection] = value;
			}
			ReceivedFragmentGroup value2;
			if (!value.TryGetValue(group, out value2))
			{
		...
		private void BindSocket(bool reBind)
		{
			...
			if (!(now - m_lastSocketBind < 1.0))
			{
				...
				EndPoint local_end = new IPEndPoint(m_configuration.LocalAddress, reBind ? m_listenPort : m_configuration.Port);
				m_socket.Bind(local_end);
				try
				{
					uint ioctl_code = 2550136844u;
					m_socket.IOControl((int)ioctl_code, new byte[1] { Convert.ToByte(false) }, null);
				}
				catch
				{
				}
		...
		private void Heartbeat()
		{
			...
			if (num > 1.0 / (double)num2 || num < 0.0)
			{
				...
				while (m_unsentUnconnectedMessages.TryDequeue(out item))
				{
					...
					bool connectionReset;
					SendPacket(numBytes, item.Item1, 1, out connectionReset);
				}
			}
			if (m_upnp != null)
			{
			...
			do
			{
				...
				{
					string text = Encoding.UTF8.GetString(m_receiveBuffer, 0, num4);
		...
		{
			NetConnection value;
			if (m_handshakes.TryGetValue(senderEndPoint, out value))
			{
			...
			case NetMessageType.ConnectResponse:
				...
				LogWarning(string.Concat("Received unhandled library message ", tp, " from ", senderEndPoint));
				break;
		...
		internal bool SendMTUPacket(int numBytes, IPEndPoint target)
		{
			...
			catch (SocketException ex)
			{
				...
				LogError(string.Concat("Failed to send packet: (", ex.SocketErrorCode, ") ", ex));
			}
			catch (Exception ex2)
			{
		...
		{
			NetOutgoingMessage item;
			if (m_outgoingMessagesPool == null || !m_outgoingMessagesPool.TryDequeue(out item))
			{
		...
		{
			NetIncomingMessage item;
			if (m_incomingMessagesPool == null || !m_incomingMessagesPool.TryDequeue(out item))
			{
