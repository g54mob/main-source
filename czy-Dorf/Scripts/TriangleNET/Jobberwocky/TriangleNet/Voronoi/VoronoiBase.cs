...
namespace Jobberwocky.TriangleNet.Voronoi
{
	public abstract class VoronoiBase : DcelMesh
	{
		...
		protected void Generate(Mesh mesh)
		{
			...
			Jobberwocky.TriangleNet.Topology.DCEL.Vertex[] array = new Jobberwocky.TriangleNet.Topology.DCEL.Vertex[mesh.triangles.Count + mesh.hullsize];
			...
			foreach (Jobberwocky.TriangleNet.Geometry.Vertex value in mesh.vertices.Values)
			{
				array2[value.id] = factory.CreateFace(value);
			}
			ComputeEdges(mesh, array, array2, map);
			ConnectEdges(map);
			vertices = new List<Jobberwocky.TriangleNet.Topology.DCEL.Vertex>(array);
			faces = new List<Face>(array2);
		}

		protected List<HalfEdge>[] ComputeVertices(Mesh mesh, Jobberwocky.TriangleNet.Topology.DCEL.Vertex[] vertices)
		{
			...
			foreach (Triangle triangle in mesh.triangles)
			{
				...
				Jobberwocky.TriangleNet.Topology.DCEL.Vertex vertex = factory.CreateVertex(point.x, point.y);
				vertex.id = id;
				vertices[id] = vertex;
		...
		protected void ComputeEdges(Mesh mesh, Jobberwocky.TriangleNet.Topology.DCEL.Vertex[] vertices, Face[] faces, List<HalfEdge>[] map)
		{
			...
			foreach (Triangle triangle in mesh.triangles)
			{
				...
				for (int i = 0; i < 3; i++)
				{
					...
					if (id < id2 || id2 < 0)
					{
						Jobberwocky.TriangleNet.Geometry.Vertex vertex = otri.Org();
						Jobberwocky.TriangleNet.Geometry.Vertex vertex2 = otri.Dest();
						Face face = faces[vertex.id];
						Face face2 = faces[vertex2.id];
						Jobberwocky.TriangleNet.Topology.DCEL.Vertex vertex3 = vertices[id];
						Jobberwocky.TriangleNet.Topology.DCEL.Vertex vertex4;
						HalfEdge halfEdge;
						HalfEdge halfEdge2;
						if (id2 < 0)
						{
