	...
	public class BoundedVoronoi : VoronoiBase
	{
		...
		private void PostProcess()
		{
			foreach (HalfEdge ray in rays)
			{
				...
				Jobberwocky.TriangleNet.Geometry.Vertex vertex = (Jobberwocky.TriangleNet.Geometry.Vertex)ray.face.generator;
				Jobberwocky.TriangleNet.Geometry.Vertex vertex2 = (Jobberwocky.TriangleNet.Geometry.Vertex)twin.face.generator;
				double num = predicates.CounterClockwise(vertex, vertex2, ray.origin);
				if (num <= 0.0)
				{
		...
		private void HandleCase1(HalfEdge edge, Jobberwocky.TriangleNet.Geometry.Vertex v1, Jobberwocky.TriangleNet.Geometry.Vertex v2)
		{
			Point origin = edge.twin.origin;
			origin.x = (v1.x + v2.x) / 2.0;
			origin.y = (v1.y + v2.y) / 2.0;
			Jobberwocky.TriangleNet.Topology.DCEL.Vertex vertex = factory.CreateVertex(v1.x, v1.y);
		...
		private void HandleCase2(HalfEdge edge, Jobberwocky.TriangleNet.Geometry.Vertex v1, Jobberwocky.TriangleNet.Geometry.Vertex v2)
		{
			Point c = edge.origin;
