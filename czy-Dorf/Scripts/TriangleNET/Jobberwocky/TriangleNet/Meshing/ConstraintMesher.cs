...
using Jobberwocky.TriangleNet.Meshing.Iterators;
	...
	internal class ConstraintMesher
	{
		...
		private void CarveHoles()
		{
			...
			if (!mesh.behavior.NoHoles)
			{
				foreach (Point hole in mesh.holes)
				{
					...
					if (predicates.CounterClockwise(a, b, hole) > 0.0)
					{
						LocateResult locateResult = mesh.locator.Locate(hole, ref searchtri);
						if (locateResult != LocateResult.Outside && !searchtri.IsInfected())
						{
							searchtri.Infect();
			...
			if (mesh.regions.Count > 0)
			{
				...
				foreach (RegionPointer region in mesh.regions)
				{
					...
					if (mesh.bounds.Contains(region.point))
					{
						...
						Vertex a = searchtri.Org();
						Vertex b = searchtri.Dest();
						if (predicates.CounterClockwise(a, b, region.point) > 0.0)
						{
							LocateResult locateResult = mesh.locator.Locate(region.point, ref searchtri);
							if (locateResult != LocateResult.Outside && !searchtri.IsInfected())
							{
			...
			if (behavior.Poly)
			{
				...
				foreach (ISegment segment in input.Segments)
				{
					...
					if (vertex.x == vertex2.x && vertex.y == vertex2.y)
					{
						if (Log.Verbose)
						{
							Debug.LogWarning("Endpoints of segment (IDs " + vertex.id + "/" + vertex2.id + ") are coincident. Mesh.FormSkeleton()");
						}
					}
					else
					{
		...
		private FindDirectionResult FindDirection(ref Otri searchtri, Vertex searchpoint)
		{
			...
			while (flag)
			{
				...
				{
					Debug.LogError("Unable to find a triangle on path. Mesh.FindDirection().1");
			...
			while (flag2)
			{
				...
				if (searchtri.tri.id == -1)
				{
					Debug.LogError("Unable to find a triangle on path. Mesh.FindDirection().2");
					throw new Exception("Unable to find a triangle on path.");
				}
				c = searchtri.Dest();
				num = num2;
		...
		private void SegmentIntersection(ref Otri splittri, ref Osub splitsubseg, Vertex endpoint2)
		{
			...
			if (num7 == 0.0)
			{
				Debug.LogError("Attempt to find intersection of parallel segments. Mesh.SegmentIntersection()");
			...
			{
				Debug.LogError("Failure to split a segment. Mesh.SegmentIntersection()");
			...
			{
				Debug.LogWarning("Topological inconsistency after splitting a segment. Mesh.SegmentIntersection()");
		...
		private void InsertSegment(Vertex endpoint1, Vertex endpoint2, int newmark)
		{
			...
			if (vertex != endpoint1)
			{
				...
				{
					Debug.LogError("Unable to locate PSLG vertex in triangulation. Mesh.InsertSegment().1");
			...
			if (vertex != endpoint2)
			{
				...
				{
					Debug.LogError("Unable to locate PSLG vertex in triangulation. Mesh.InsertSegment().2");
					throw new Exception("Unable to locate PSLG vertex in triangulation.");
				}
			}
