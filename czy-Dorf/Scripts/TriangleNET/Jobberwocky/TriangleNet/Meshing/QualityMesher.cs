...
using Jobberwocky.TriangleNet.Meshing.Data;
	...
	internal class QualityMesher
	{
		...
		private void SplitEncSegs(bool triflaws)
		{
			...
			while (badsubsegs.Count > 0 && mesh.steinerleft != 0)
			{
				...
				if (!Osub.IsDead(osub.seg) && vertex == badSubseg.org && vertex2 == badSubseg.dest)
				{
					...
					Vertex vertex4 = new Vertex(vertex.x + num3 * (vertex2.x - vertex.x), vertex.y + num3 * (vertex2.y - vertex.y), osub.seg.boundary, mesh.nextras);
					...
					{
						Debug.LogError("Ran out of precision: I attempted to split a segment to a smaller size than can be accommodated by the finite precision of floating point arithmetic. Quality.SplitEncSegs()");
					...
					{
						Debug.LogError("Failure to split a segment. Quality.SplitEncSegs()");
		...
		private void SplitTriangle(BadTriangle badtri)
		{
			...
			if ((point.x == vertex.x && point.y == vertex.y) || (point.x == vertex2.x && point.y == vertex2.y) || (point.x == vertex3.x && point.y == vertex3.y))
			{
				if (Log.Verbose)
				{
					Debug.LogWarning("New vertex falls on existing vertex. Quality.SplitTriangle()");
					flag = true;
				}
			}
			else
			{
				...
				case InsertVertexResult.Successful:
					...
					if (mesh.nextras > 0)
					{
						Interpolation.InterpolateAttributes(vertex4, vertex4.tri.tri, mesh.nextras);
					}
			...
			if (flag)
			{
				Debug.LogError("The new vertex is at the circumcenter of triangle: This probably means that I am trying to refine triangles to a smaller size than can be accommodated by the finite precision of floating point arithmetic. Quality.SplitTriangle()");
		...
		private void EnforceQuality()
		{
			...
			{
				Debug.LogWarning("I ran out of Steiner points, but the mesh has encroached subsegments, and therefore might not be truly Delaunay. If the Delaunay property is important to you, try increasing the number of Steiner points. Quality.EnforceQuality()");
			}
		}
	}
}
