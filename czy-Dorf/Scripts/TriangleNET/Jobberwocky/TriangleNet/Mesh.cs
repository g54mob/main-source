...
namespace Jobberwocky.TriangleNet
{
	public class Mesh : IMesh
	{
		...
		public void Renumber(NodeNumbering num)
		{
			...
			{
				CuthillMcKee cuthillMcKee = new CuthillMcKee();
				int[] array = cuthillMcKee.Renumber(this);
		...
		internal void TransferNodes(IList<Vertex> points)
		{
			...
			if (invertices < 3)
			{
				Debug.LogError("Input must have at least three input vertices. Mesh.TransferNodes()");
				throw new Exception("Input must have at least three input vertices.");
			}
			Vertex vertex = points[0];
			nextras = ((vertex.attributes != null) ? vertex.attributes.Length : 0);
