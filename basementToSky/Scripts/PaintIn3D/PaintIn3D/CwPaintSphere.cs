...
namespace PaintIn3D
{
	...
	public class CwPaintSphere : MonoBehaviour, IHitPoint, IHit, IHitLine, IHitTriangle, IHitQuad, IHitCoord
	{
		...
		{
			if (isPaintingNow && !wasPaintingLastFrame)
			{
				CwPaintSphere.OnStartPainting?.Invoke();
			}
			else if (!isPaintingNow && wasPaintingLastFrame)
			{
				CwPaintSphere.OnStopPainting?.Invoke();
			}
			wasPaintingLastFrame = isPaintingNow;
			isPaintingNow = false;
		}
