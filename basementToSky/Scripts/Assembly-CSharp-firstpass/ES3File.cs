...
public class ES3File
{
	...
	public T Load<T>(string key)
	{
		...
		eS3Settings.encryptionType = ES3.EncryptionType.None;
		eS3Settings.compressionType = ES3.CompressionType.None;
		if (typeof(T) != value.type.type && ES3Reflection.IsAssignableFrom(typeof(T), value.type.type))
		{
	...
	{
		cachedFiles.TryGetValue(settings.path, out var value);
		if (value != null)
		{
	...
	{
		ES3File eS3File = GetCachedFile(settings);
		if (eS3File == null)
		{
			eS3File = new ES3File(settings, syncWithFile: false);
			cachedFiles.Add(settings.path, eS3File);
	...
	internal static ES3File CacheFile(ES3Settings settings)
	{
		...
		obj.compressionType = ES3.CompressionType.None;
		obj.encryptionType = ES3.EncryptionType.None;
		ES3File eS3File = new ES3File(ES3.LoadRawBytes(obj), settings);
		eS3File.dirty = false;
	...
	{
		foreach (KeyValuePair<string, ES3File> cachedFile in cachedFiles)
		{
			if (cachedFile.Key != null && cachedFile.Value != null)
			{
				Store(cachedFile.Value.settings);
			}
		}
	}
