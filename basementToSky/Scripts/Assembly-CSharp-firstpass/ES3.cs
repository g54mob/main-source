...
public class ES3
{
	...
	public static T Load<T>(string key, ES3Settings settings)
	{
		if (settings.location == Location.Cache)
		{
			ES3File eS3File = ES3File.GetCachedFile(settings);
			if (settings.autoCacheFileOnLoad && eS3File == null)
			{
				eS3File = ES3File.CacheFile(settings);
			}
			if (eS3File == null)
			{
				throw new FileNotFoundException("File \"" + settings.FullPath + "\" could not be found.");
			}
			return eS3File.Load<T>(key);
		}
	...
	public static T Load<T>(string key, T defaultValue, ES3Settings settings)
	{
		if (settings.location == Location.Cache)
		{
			...
			return eS3File.Load(key, defaultValue);
		}
		using ES3Reader eS3Reader = ES3Reader.Create(settings);
		if (eS3Reader == null)
		{
	...
	public static void CopyDirectory(ES3Settings oldSettings, ES3Settings newSettings)
	{
		...
		{
			CopyFile(ES3IO.CombinePathAndFilename(oldSettings.path, fileOrDirectoryName), ES3IO.CombinePathAndFilename(newSettings.path, fileOrDirectoryName), oldSettings, newSettings);
		}
		files = GetDirectories(oldSettings);
		foreach (string fileOrDirectoryName2 in files)
		{
			CopyDirectory(ES3IO.CombinePathAndFilename(oldSettings.path, fileOrDirectoryName2), ES3IO.CombinePathAndFilename(newSettings.path, fileOrDirectoryName2), oldSettings, newSettings);
		}
	}
