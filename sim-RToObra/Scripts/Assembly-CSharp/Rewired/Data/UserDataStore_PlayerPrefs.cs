...
namespace Rewired.Data
{
	public class UserDataStore_PlayerPrefs : UserDataStore
	{
		private class ControllerAssignmentSaveInfo
		{
			public class PlayerInfo
			{
				...
				public int joystickCount
				{
					get
					{
						return (joysticks != null) ? joysticks.Length : 0;
					}
				}
		...
		private bool loadControllerAssignments
		{
			get
			{
				return loadKeyboardAssignments || loadMouseAssignments || loadJoystickAssignments;
			}
		}
					...
					foreach (Player allPlayer3 in ReInput.players.AllPlayers)
					{
						...
						for (int num = 0; num < playerInfo2.joystickCount; num++)
						{
							...
							else
							{
								...
								if (!TryFindJoysticksImprecise(joystickInfo2, out matches))
								{
									continue;
								}
		...
		{
			string controllerMapPlayerPrefsKey = GetControllerMapPlayerPrefsKey(player, controllerMap.controller, controllerMap.categoryId, controllerMap.layoutId, true);
			PlayerPrefs.SetString(controllerMapPlayerPrefsKey, controllerMap.ToXmlString());
			controllerMapPlayerPrefsKey = GetControllerMapKnownActionIdsPlayerPrefsKey(player, controllerMap.controller, controllerMap.categoryId, controllerMap.layoutId, true);
