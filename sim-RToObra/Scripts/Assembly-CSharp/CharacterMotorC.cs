...
public class CharacterMotorC : MonoBehaviour
{
	...
	private void UpdateFunction()
	{
		if (!Clock.play.running)
		{
			...
			movement.frameVelocity.x = 0f;
			movement.frameVelocity.z = 0f;
		}
		...
		Vector3 motion = velocity * Clock.play.deltaTime;
		float stepOffset = controller.stepOffset;
		float magnitude = new Vector3(motion.x, 0f, motion.z).magnitude;
		float num = Mathf.Max(stepOffset, magnitude);
		if (grounded)
		{
		...
		movement.velocity = (tr.position - position) / Clock.play.deltaTime;
	...
	private void FixedUpdate()
	{
		if (movingPlatform.enabled)
		{
			if (movingPlatform.activePlatform != null)
			{
				...
				{
					movingPlatform.platformVelocity = (movingPlatform.activePlatform.localToWorldMatrix.MultiplyPoint3x4(movingPlatform.activeLocalPoint) - movingPlatform.lastMatrix.MultiplyPoint3x4(movingPlatform.activeLocalPoint)) / Clock.play.deltaTime;
				}
	...
	private Vector3 ApplyInputVelocityChange(Vector3 velocity)
	{
		...
		float num = GetMaxAcceleration(grounded) * Clock.play.deltaTime;
	...
	private Vector3 ApplyGravityAndJumping(Vector3 velocity)
	{
		...
		{
			jumping.lastButtonDownTime = Clock.play.time;
		}
		if (grounded)
		{
			velocity.y = Mathf.Min(0f, velocity.y) - movement.gravity * Clock.play.deltaTime;
		}
		else
		{
			velocity.y = movement.velocity.y - movement.gravity * Clock.play.deltaTime;
			if (jumping.jumping && jumping.holdingJumpButton && Clock.play.time < jumping.lastStartTime + jumping.extraHeight / CalculateJumpVerticalSpeed(jumping.baseHeight))
			{
				velocity += jumping.jumpDir * movement.gravity * Clock.play.deltaTime;
			}
			velocity.y = Mathf.Max(velocity.y, 0f - movement.maxFallSpeed);
		}
		if (grounded)
		{
			if (jumping.enabled && canControl && (double)(Clock.play.time - jumping.lastButtonDownTime) < 0.2)
			{
				grounded = false;
				jumping.jumping = true;
				jumping.lastStartTime = Clock.play.time;
				jumping.lastButtonDownTime = -100f;
	...
	private Vector3 GetDesiredHorizontalVelocity()
	{
		...
		double num = MaxSpeedInDirection(vector);
		if (grounded)
		{
			float time = Mathf.Asin(movement.velocity.normalized.y) * 57.29578f;
			num *= (double)movement.slopeSpeedMultiplier.Evaluate(time);
		}
		return tr.TransformDirection(vector * (float)num);
	}

	private Vector3 AdjustGroundVelocityToNormal(Vector3 hVelocity, Vector3 groundNormal)
	{
		Vector3 lhs = Vector3.Cross(Vector3.up, hVelocity);
		Vector3 result = Vector3.Cross(lhs, groundNormal).normalized * hVelocity.magnitude;
		if (Vector3.Angle(hVelocity, groundNormal) < 60f)
		{
			result *= 0.5f;
		}
		return result;
	...
	private double MaxSpeedInDirection(Vector3 desiredMovementDirection)
	{
		if (desiredMovementDirection == Vector3.zero)
		{
			return 0.0;
		}
		double num = ((!(desiredMovementDirection.z > 0f)) ? movement.maxBackwardsSpeed : movement.maxForwardSpeed) / movement.maxSidewaysSpeed;
		float x = desiredMovementDirection.x;
		float z = (float)((double)desiredMovementDirection.z / num);
		Vector3 normalized = new Vector3(x, 0f, z).normalized;
		float x2 = normalized.x;
		float z2 = (float)((double)normalized.z * num);
		double num2 = new Vector3(x2, 0f, z2).magnitude * movement.maxSidewaysSpeed;
		return num2 * (double)Mathf.Lerp(0.25f, 1f, Player.cameraFovT);
	}
