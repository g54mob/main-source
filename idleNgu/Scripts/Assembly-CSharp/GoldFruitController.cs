...
public class GoldFruitController : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler
{
	...
	private readonly long baseSeedCost = 1L;

	private readonly long baseSeedReward = 1L;

	private readonly long activationCost = 100000L;

	private readonly string fruitName = "Fruit of Gold";
	...
	public void reset()
	{
		character.yggdrasil.goldFruit.reset(character.yggdrasil.resetFactor);
		updateButtonText();
		updateFruitSlider();
	}
	...
	{
		if (character.yggdrasil.goldFruit.activated)
		{
			character.yggdrasil.goldFruit.addTime();
		}
		updateFruitSlider();
		updateButtonText();
	}
	...
	{
		if (character.yggdrasil.goldFruit.growing())
		{
			if (character.yggdrasil.goldFruit.harvestTier() > 0)
			{
				action1 = consume;
				action2 = harvest;
	...
	public void activate()
	{
		...
		{
			tooltip.showTooltip("This fruit is still locked!");
		}
		else if (character.idleEnergy >= num)
		{
			character.idleEnergy -= num;
			character.curEnergy -= num;
			character.yggdrasil.goldFruit.activate();
			updateButtonText();
		}
		else
		{
	...
	public void consume()
	{
		int num = character.yggdrasil.goldFruit.harvestTier();
		double num2 = (double)((float)num * 1800f) * character.goldPerSecond();
		character.addGold(num2);
		character.yggdrasil.seeds += baseSeedReward * Mathf.CeilToInt(Mathf.Pow(num, 1.3f));
		character.yggdrasil.goldFruit.deactivate();
		updateButtonText();
		tooltip.showTooltip("You gained " + num2 + " Gold!");
		updateFruit();
	}
	...
	public void upgrade()
	{
		long num = baseSeedCost * Mathf.CeilToInt(Mathf.Pow(character.yggdrasil.goldFruit.maxTier + 1, 2f));
	...
	{
		float value = character.yggdrasil.goldFruit.seconds / 3600f % 1f;
	...
	{
		float a = character.yggdrasil.goldFruit.seconds / 3600f;
		a = Mathf.Min(a, character.yggdrasil.goldFruit.maxTier);
		float value = a % 1f;
		if (a >= 10f)
		{
	...
	public void showTooltip()
	{
		...
		message += "This fruit is so bright and shiny that it hurts your eyes to look at it. You can probably sell it online for some quick gold.";
		message = message + "\n\n<b>Current Fruit Tier:</b> " + character.yggdrasil.goldFruit.harvestTier();
		message = message + "\n\n<b>Time to next Tier:</b> " + timeToNextTier();
		message = message + "\n\n<b>Max Tier:</b> " + character.yggdrasil.goldFruit.maxTier;
		if (!character.yggdrasil.goldFruit.activated)
		{
	...
	{
		if (character.yggdrasil.goldFruit.harvestTier() == 10)
		{
			return "DONE";
		}
		int num = (int)(3600f - character.yggdrasil.goldFruit.seconds % 3600f);
		int num2 = num / 60;
		num %= 60;
		if (character.yggdrasil.goldFruit.seconds / 3600f > (float)character.yggdrasil.goldFruit.maxTier)
		{
			return "DONE";
		}
		if (num2 > 0)
