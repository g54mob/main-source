...
public class AdventureFruitController : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler
{
	...
	private long baseSeedCost = 25L;

	private long baseSeedReward = 1L;

	private long activationCost = 200000L;

	private string fruitName = "Fruit of Adventure";
	...
	public void reset()
	{
		character.yggdrasil.adventureFruit.reset(character.yggdrasil.resetFactor);
		updateButtonText();
		updateFruitSlider();
	}
	...
	{
		if (character.yggdrasil.adventureFruit.activated)
		{
			character.yggdrasil.adventureFruit.addTime();
		}
		updateFruitSlider();
	}
	...
	{
		if (character.yggdrasil.adventureFruit.growing())
		{
			if (character.yggdrasil.adventureFruit.harvestTier() > 0)
			{
				action1 = consume;
				action2 = harvest;
	...
	public void activate()
	{
		...
		if (character.yggdrasil.adventureFruit.maxTier == 0L)
		{
		...
		else if (character.idleEnergy >= num)
		{
			...
			character.yggdrasil.adventureFruit.activate();
			updateButtonText();
		}
		else
		{
	...
	public void consume()
	{
		int num = character.yggdrasil.adventureFruit.harvestTier();
		...
		float num5 = num3 * 3f;
		float num6 = num4 * 0.03f;
		character.yggdrasil.seeds += num2;
		character.adventure.attack += num3;
		character.adventure.defense += num4;
		character.adventure.maxHP += num5;
		character.adventure.regen += num6;
		string text = "You gained:\n\n" + num3 + " Attack!\n" + num4 + " Defense!\n" + num5 + " Max HP!\n" + num6 + " HP regen!\nAnd " + num2 + " Seeds!";
		tooltip.showTooltip(text, 3f);
		character.yggdrasil.adventureFruit.deactivate();
		updateButtonText();
		updateFruitSlider();
	}
	...
	public void upgrade()
	{
		long num = baseSeedCost * Mathf.CeilToInt(Mathf.Pow(character.yggdrasil.adventureFruit.maxTier + 1, 2f));
	...
	{
		float value = character.yggdrasil.adventureFruit.seconds / 3600f % 1f;
	...
	{
		float a = character.yggdrasil.adventureFruit.seconds / 3600f;
		a = Mathf.Min(a, character.yggdrasil.adventureFruit.maxTier);
		float value = a % 1f;
		if (a >= 10f)
		{
	...
	public void showTooltip()
	{
		...
		message += "This fruit looks like an orange, smells like an orange, even tastes like an orange! Okay, it's an orange. And it also improves your adventuring stats!";
		message = message + "\n\n<b>Current Fruit Tier:</b> " + character.yggdrasil.adventureFruit.harvestTier();
		message = message + "\n\n<b>Time to next Tier:</b> " + timeToNextTier();
		message = message + "\n\n<b>Max Tier:</b> " + character.yggdrasil.adventureFruit.maxTier;
		if (!character.yggdrasil.adventureFruit.activated)
		{
	...
	{
		if (character.yggdrasil.adventureFruit.harvestTier() == 10)
		{
			return "MAXED OUT";
		}
		int num = (int)(3600f - character.yggdrasil.adventureFruit.seconds % 3600f);
		int num2 = num / 60;
		num %= 60;
		if (character.yggdrasil.adventureFruit.seconds / 3600f > (float)character.yggdrasil.adventureFruit.maxTier)
		{
			return "DONE";
		}
		if (num2 > 0)
