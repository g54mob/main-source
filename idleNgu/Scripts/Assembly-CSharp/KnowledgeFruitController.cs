...
public class KnowledgeFruitController : MonoBehaviour, IPointerEnterHandler, IEventSystemHandler, IPointerExitHandler
{
	...
	private long baseSeedCost = 40L;

	private long baseSeedReward = 1L;

	private long activationCost = 1000000L;

	private string fruitName = "Fruit of Knowledge";
	...
	public void reset()
	{
		character.yggdrasil.knowledgeFruit.reset(character.yggdrasil.resetFactor);
		updateButtonText();
		updateFruitSlider();
	}
	...
	{
		if (character.yggdrasil.knowledgeFruit.activated)
		{
			character.yggdrasil.knowledgeFruit.addTime();
		}
		updateFruitSlider();
	}
	...
	{
		if (character.yggdrasil.knowledgeFruit.growing())
		{
			if (character.yggdrasil.knowledgeFruit.harvestTier() > 0)
			{
				action1 = consume;
				action2 = harvest;
	...
	public void activate()
	{
		...
		if (character.yggdrasil.knowledgeFruit.maxTier == 0L)
		{
		...
		else if (character.idleEnergy >= num)
		{
			...
			character.yggdrasil.knowledgeFruit.activate();
			updateButtonText();
		}
		else
		{
	...
	public void consume()
	{
		int num = character.yggdrasil.knowledgeFruit.harvestTier();
		long num2 = baseSeedReward * Mathf.CeilToInt(Mathf.Pow(num, 1.3f));
		int num3 = (int)((float)num / 10f * Mathf.Pow(character.stats.totalExp, 0.5f));
		character.addExp(num3);
		character.yggdrasil.seeds += num2;
		string text = "You gained:\n\n" + num3 + " EXP!";
		tooltip.showTooltip(text, 3f);
		character.yggdrasil.knowledgeFruit.deactivate();
		updateButtonText();
		updateFruitSlider();
	}
	...
	public void upgrade()
	{
		long num = baseSeedCost * Mathf.CeilToInt(Mathf.Pow(character.yggdrasil.knowledgeFruit.maxTier + 1, 2f));
	...
	{
		float value = character.yggdrasil.knowledgeFruit.seconds / 3600f % 1f;
	...
	{
		float a = character.yggdrasil.knowledgeFruit.seconds / 3600f;
		a = Mathf.Min(a, character.yggdrasil.knowledgeFruit.maxTier);
		float value = a % 1f;
		if (a >= 10f)
		{
	...
	public void showTooltip()
	{
		...
		message += "This fruit is a pale-ish pink, and looks like it's throbbing. you should totally eat this.";
		message = message + "\n\n<b>Current Fruit Tier:</b> " + character.yggdrasil.knowledgeFruit.harvestTier();
		message = message + "\n\n<b>Time to next Tier:</b> " + timeToNextTier();
		message = message + "\n\n<b>Max Tier:</b> " + character.yggdrasil.knowledgeFruit.maxTier;
		if (!character.yggdrasil.knowledgeFruit.activated)
		{
	...
	{
		if (character.yggdrasil.knowledgeFruit.harvestTier() == 10)
		{
			return "DONE";
		}
		int num = (int)(3600f - character.yggdrasil.knowledgeFruit.seconds % 3600f);
		int num2 = num / 60;
		num %= 60;
		if (character.yggdrasil.knowledgeFruit.seconds / 3600f > (float)character.yggdrasil.knowledgeFruit.maxTier)
		{
			return "DONE";
		}
		if (num2 > 0)
