...
namespace Jundroo.Common.Debugging
{
	public static class DebugGizmos
	{
		...
		{
			Transform ball = GetBall(name, 0, Color.white, emissive: false, create: false);
			if (ball != null)
			{
			...
			ball = GetCapsule(name);
			if (ball != null)
			{
				Object.Destroy(ball.gameObject);
			}
			VectorLine line = GetLine(name, create: false);
			if (line != null)
			{
				Object.Destroy(line.rectTransform.gameObject);
			}
			_balls.Remove(name);
			_lines.Remove(name);
			_texts.Remove(name);
		...
		public static void DestroyStale(float staleTimeSeconds = 1f)
		{
			DestroyStale(staleTimeSeconds, _balls);
			DestroyStale(staleTimeSeconds, _capsules);
			DestroyStale(staleTimeSeconds, _lines);
			DestroyStale(staleTimeSeconds, _texts);
		}
		...
		public static VectorLine GetLine(string name, bool create, int layer = 0)
		{
			if (create && !_lines.ContainsKey(name))
			{
				...
				LockPositionScript.Add(vectorLine.rectTransform.gameObject, Vector3.zero, useWorldPosition: false);
		...
		private static void DestroyStale(float staleTimeSeconds, Dictionary<string, Gizmo> gizmosToCheck)
		{
			...
			{
				if (item.Value.LastAccessTime + staleTimeSeconds < unscaledTime)
				{
					DestroyGizmo(item.Value);
					list.Add(item.Key);
				}
