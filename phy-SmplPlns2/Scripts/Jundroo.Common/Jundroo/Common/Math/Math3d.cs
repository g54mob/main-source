...
namespace Jundroo.Common.Math
{
	...
	{
		private static Transform _tempChild;
		...
		public static bool RayPlaneIntersection(out Vector3 intersection, Vector3 rayOrigin, Vector3 rayDirection, Vector3 planeNormal, Vector3 planePoint)
		{
			bool flag = LinePlaneIntersection(out intersection, rayOrigin, rayDirection, planeNormal, planePoint);
			if (flag && Vector3.Dot(rayDirection, intersection - rayOrigin) < 0f)
			{
				flag = false;
			}
			return flag;
		...
		public static void TransformWithParent(out Quaternion childRotation, out Vector3 childPosition, Quaternion parentRotation, Vector3 parentPosition, Quaternion startParentRotation, Vector3 startParentPosition, Quaternion startChildRotation, Vector3 startChildPosition)
		{
			...
			_tempParent.SetPositionAndRotation(startParentPosition, startParentRotation);
			_tempParent.localScale = Vector3.one;
			_tempChild.SetPositionAndRotation(startChildPosition, startChildRotation);
			_tempChild.localScale = Vector3.one;
			_tempParent.SetPositionAndRotation(parentPosition, parentRotation);
