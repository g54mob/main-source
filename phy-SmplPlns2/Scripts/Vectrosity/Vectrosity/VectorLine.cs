...
namespace Vectrosity
{
	...
	public class VectorLine
	{
		...
		private void AddColliderIfNeeded()
		{
			if (m_go.GetComponent<Collider2D>() == null)
			{
				m_go.AddComponent((m_lineType != LineType.Continuous) ? typeof(PolygonCollider2D) : typeof(EdgeCollider2D));
		...
		private void SetupTriangles(int startVert)
		{
			...
			if (pointsCount > 0)
			{
				...
				{
					num = ((m_joins != Joins.Fill) ? ((pointsCount - 1) * 6) : ((pointsCount - 1) * 12));
		...
		private void Resize()
		{
			...
			if (m_lineType != LineType.Points)
			{
				num2 = ((m_lineType != LineType.Continuous) ? (m_pointsCount / 2) : Mathf.Max(0, m_pointsCount - 1));
			}
			...
			if (m_lineWidths.Length > 1)
			{
				if (m_lineType != LineType.Points)
				{
					num3 = ((m_lineType != LineType.Continuous) ? (num3 / 2) : (num3 - 1));
				}
		...
		private void SetEndCapColors()
		{
			...
			{
				int num = ((m_lineType != LineType.Continuous) ? (m_drawStart * 2) : (m_drawStart * 4));
				for (int i = 0; i < 4; i++)
				{
			...
			if (m_capType >= EndCap.Both)
			{
				...
				int num3 = num2 * ((m_lineType != LineType.Continuous) ? 2 : 4) - 2;
				if (num3 < 0)
				{
					num3 = 0;
				}
				for (int j = 4; j < 8; j++)
				{
					...
					reference2 = ((!m_useCapColors) ? m_lineColors[num3] : m_backColor);
				}
			}
			if (m_vectorObject != null)
			{
		...
		private void DrawEndCap(bool draw3D)
		{
			...
			if (m_capType >= EndCap.Both)
			{
				...
				if (!draw3D)
				{
					...
					reference9 = m_lineVertices[num - 2] + vector5 + vector6;
					ref Vector3 reference10 = ref m_lineVertices[m_vertexCount + 5];
					reference10 = m_lineVertices[num - 3] + vector5 + vector6;
				...
				else
				{
					...
					reference11 = cam3D.ScreenToWorldPoint(m_screenPoints[num - 2] + vector7 + vector8);
					ref Vector3 reference12 = ref m_lineVertices[m_vertexCount + 5];
					reference12 = cam3D.ScreenToWorldPoint(m_screenPoints[num - 3] + vector7 + vector8);
					ref Vector3 reference13 = ref m_lineVertices[num - 3];
					reference13 = cam3D.ScreenToWorldPoint(m_screenPoints[num - 3] + vector8);
					ref Vector3 reference14 = ref m_lineVertices[num - 2];
					reference14 = cam3D.ScreenToWorldPoint(m_screenPoints[num - 2] + vector8);
				}
				ref Vector3 reference15 = ref m_lineVertices[m_vertexCount + 4];
				reference15 = m_lineVertices[num - 3];
				ref Vector3 reference16 = ref m_lineVertices[m_vertexCount + 7];
				reference16 = m_lineVertices[num - 2];
		...
		private void CheckDrawStartFill(int start)
		{
			if (m_joins == Joins.Fill)
			{
				...
				{
					ref Vector3 reference = ref m_lineVertices[num - 1];
					reference = m_lineVertices[num];
					ref Vector3 reference2 = ref m_lineVertices[num - 2];
					reference2 = m_lineVertices[num];
					ref Vector3 reference3 = ref m_lineVertices[num - 3];
					reference3 = m_lineVertices[num];
				}
			}
		}
		...
		public void SetDistances()
		{
			...
			{
				m_distances = new float[(m_lineType == LineType.Discrete) ? (pointsCount / 2 + 1) : pointsCount];
			}
			double num = 0.0;
		...
		public void MakeWireframe(Mesh mesh)
		{
			...
			for (int i = 0; i < mesh.subMeshCount; i++)
			{
				...
				int num = ((mesh.GetTopology(i) != MeshTopology.Triangles) ? 4 : 3);
