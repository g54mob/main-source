...
public class TBDragView : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (useAngularVelocity)
		{
			...
			if (Dragging)
			{
				b = sensitivity * dragGesture.DeltaMove.Centimeters();
				num = dragAcceleration;
			}
			angularVelocity = Vector2.Lerp(angularVelocity, b, Time.unscaledDeltaTime * num);
			Vector2 vector = Time.unscaledDeltaTime * angularVelocity;
			if (reverseControls)
			{
				vector = -vector;
			}
		...
		{
			cachedTransform.rotation = Quaternion.Slerp(cachedTransform.rotation, IdealRotation, Time.unscaledDeltaTime * idealRotationSmoothingSpeed);
		}
		else
		{
