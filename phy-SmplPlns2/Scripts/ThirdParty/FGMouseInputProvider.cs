...
public class FGMouseInputProvider : FGInputProvider
{
	...
	private void Update()
	{
		...
		if (Input.GetKey(pivotKey))
		{
			...
			if (!pivoting && Vector2.Distance(Input.mousePosition, pivot) > 50f)
			{
				pivoting = true;
			}
			if (pivoting)
	...
	private void UpdateTwistEmulation()
	{
		...
		if (twistKey != KeyCode.None && Input.GetKey(twistKey) && Mathf.Abs(num) > 0.0001f)
		{
			if (!twisting)
			{
				twisting = true;
