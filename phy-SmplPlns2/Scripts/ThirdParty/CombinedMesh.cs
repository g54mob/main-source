...
public class CombinedMesh : MonoBehaviour
{
	...
	public void Combine(CombineProgressDelegate progress)
	{
		...
		{
			progress?.Invoke("Preprocessing object " + meshFilter.name + "...", (float)num / (float)MeshObjects.Length);
			if (m_bCancelled)
			{
				return;
			}
		...
		if (m_dicMeshEntries.Count > 0)
		{
			...
			foreach (List<MaterialMeshInfo> value in m_dicMeshEntries.Values)
			{
				...
				{
					progress?.Invoke("Combining submesh for material " + array[num2].name + "...", (float)num3 / (float)value.Count);
					if (m_bCancelled)
					{
						return;
					}
					...
					else
					{
						...
						if (flag)
						{
							...
							if (flag6)
							{
								Debug.LogWarning($"Object {item3.transform.name} has mesh with no vertex normals, and some other objects have them. Dummy normals have been added");
							}
						...
						if (flag2)
						{
							...
							if (flag7)
							{
								Debug.LogWarning($"Object {item3.transform.name} has mesh with no vertex tangents, and some other objects have them. Dummy tangents have been added");
							}
						...
						if (flag3)
						{
							...
							if (flag10)
							{
								Debug.LogWarning($"Object {item3.transform.name} has mesh with no vertex colors, and some other objects have them. Dummy colors have been added");
							}
							Color32[] array5 = null;
							if (flag10)
							{
						...
						if (flag4)
						{
							...
							if (flag11)
							{
								Debug.LogWarning($"Object {item3.transform.name} has mesh with no vertex mapping (uv), and some other objects have them. Dummy mapping has been added");
							}
						...
						if (flag5)
						{
							...
							if (flag12)
							{
								Debug.LogWarning($"Object {item3.transform.name} has mesh with no vertex mapping (uv2), and some other objects have them. Dummy mapping has been added");
							}
			...
			if (!m_bCancelled)
			{
				progress?.Invoke("Building mesh...", 1f);
