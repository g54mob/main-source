	...
	public static class Fracturer
	{
		...
		private class VoronoiPointDistance
		{
			public class IncreasingDistanceComparer : IComparer<VoronoiPointDistance>
			{
				...
				{
					if (!(a.fDistanceSqr - b.fDistanceSqr < 0f))
					{
						return 1;
					}
					return -1;
		...
		private class SpaceTreeNode
		{
			...
			public static SpaceTreeNode BuildSpaceTree(MeshData meshDataIn, int nSubdivisionLevels, FracturedObject fracturedComponent, ProgressDelegate progress = null)
			{
				...
				while (queue.Count > 0)
				{
					...
					progress?.Invoke("Fracturing", $"Pre computing space volume (split {num2 + 1}/{num}, Depth {spaceTreeNode2.nLevel + 1})", Mathf.Clamp01((float)num2 / (float)num));
					...
					{
						if (SplitMeshUsingPlane(item, fracturedComponent, splitOptions, up, right, v3PlanePoint, out var listMeshDatasPosOut, out var listMeshDatasNegOut, progress))
						{
		...
		public static bool FractureToChunks(FracturedObject fracturedComponent, bool bPositionOnSourceAndHideOriginal, out List<GameObject> listGameObjectsOut, ProgressDelegate progress = null)
		{
			...
			if ((bool)sourceObject)
			{
				...
				if ((bool)meshFilter)
				{
					...
					mesh.name = $"mesh_{fracturedComponent.SourceObject.gameObject.name}{fracturedComponent.SourceObject.gameObject.GetInstanceID().ToString()}";
				}
			}
			...
			if (flag && !IsFracturingCancelled())
			{
				...
				fracturedComponent.SingleMeshObject.SetActive(value: true);
			}
			return flag;
		}
		...
		private static bool FractureToChunksBSP(FracturedObject fracturedComponent, bool bPositionOnSourceAndHideOriginal, out List<GameObject> listGameObjectsOut, ProgressDelegate progress = null)
		{
			...
			progress?.Invoke("Fracturing", "Initializing...", 0f);
			...
			MeshData meshData = new MeshData(component.transform, component.sharedMesh, aMaterials, fracturedComponent.SourceObject.transform.localToWorldMatrix, bTransformVerticesToWorld: true, num, bBuildVertexHashData: true);
			...
			if (fracturedComponent.GenerateIslands)
			{
				...
				if (component3 != null)
				{
					...
					for (int i = 0; i < component3.GetObjectCount(); i++)
					{
						...
						foreach (MeshData item in ComputeMeshDataIslands(new MeshData(component.transform, objectInfo.mesh, objectInfo.aMaterials, fracturedComponent.transform.localToWorldMatrix * objectInfo.mtxLocal, bTransformVerticesToWorld: true, -1, bBuildVertexHashData: true), bVerticesAreLocal: false, fracturedComponent, progress))
						{
							queue.Enqueue(item);
							queue2.Enqueue(0);
							list.Add(item);
					...
					if (fracturedComponent.GenerateChunkConnectionInfo)
					{
						for (int j = 0; j < list.Count; j++)
						{
							...
							for (int k = 0; k < list.Count; k++)
							{
								if (j != k)
								{
									ComputeIslandsMeshDataConnectivity(fracturedComponent, bVerticesAreLocal: false, list[j], list[k]);
								}
							}
						}
					}
				...
				{
					foreach (MeshData item2 in ComputeMeshDataIslands(meshData, bVerticesAreLocal: false, fracturedComponent, progress))
					{
						queue.Enqueue(item2);
						queue2.Enqueue(0);
					}
			...
			while (queue.Count < s_FracturingStats.nTotalChunks && !IsFracturingCancelled())
			{
				...
				progress?.Invoke("Fracturing", string.Format("Computing chunk {0}/{1} (Depth {2})", s_FracturingStats.nChunkCount + 1, s_FracturingStats.nTotalChunks, generateIslands ? ": size ordered traversal" : num2.ToString()), Mathf.Clamp01((float)s_FracturingStats.nChunkCount / (float)s_FracturingStats.nTotalChunks));
				...
				if (SplitMeshUsingPlane(meshDataIn, fracturedComponent, splitOptions, randomPlaneSplitMatrix.MultiplyVector(Vector3.up), randomPlaneSplitMatrix.MultiplyVector(Vector3.right), randomPlaneSplitMatrix.MultiplyPoint3x4(Vector3.zero), out var listMeshDatasPosOut, out var listMeshDatasNegOut, progress))
				{
			...
			if (!IsFracturingCancelled())
			{
				...
				{
					GameObject gameObject = CreateNewSplitGameObject(fracturedComponent.SourceObject, fracturedComponent, fracturedComponent.SourceObject.name + (l + 1), bTransformVerticesBackToLocal: true, array[l]);
			...
			if (bPositionOnSourceAndHideOriginal)
			{
				...
				fracturedComponent.SourceObject.SetActive(value: false);
			}
			else
			{
		...
		private static bool FractureToChunksVoronoi(FracturedObject fracturedComponent, bool bPositionOnSourceAndHideOriginal, out List<GameObject> listGameObjectsOut, ProgressDelegate progress = null)
		{
			...
			UnityEngine.Random.InitState(fracturedComponent.RandomSeed);
			FracturedChunk component2 = fracturedComponent.gameObject.GetComponent<FracturedChunk>();
			int num5 = ((component2 != null) ? component2.SplitSubMeshIndex : (-1));
			...
			Material[] aMaterials = (fracturedComponent.SourceObject.GetComponent<Renderer>() ? fracturedComponent.SourceObject.GetComponent<Renderer>().sharedMaterials : null);
			MeshData meshData = new MeshData(component.transform, component.sharedMesh, aMaterials, fracturedComponent.SourceObject.transform.localToWorldMatrix, bTransformVerticesToWorld: false, num5, bBuildVertexHashData: true);
			SpaceTreeNode spaceTree = (fracturedComponent.VoronoiVolumeOptimization ? SpaceTreeNode.BuildSpaceTree(meshData, 8, fracturedComponent, progress) : null);
			...
			int num9 = (fracturedComponent.VoronoiProximityOptimization ? 2 : 20);
			for (int i = 0; i < num; i++)
			{
			...
			{
				progress?.Invoke("Fracturing", $"Creating cell planes... (cell {l + 1}/{list.Count}) ", (float)l / (float)list.Count);
				if (IsFracturingCancelled())
				{
					break;
				}
			...
			Thread[] array = (fracturedComponent.VoronoiMultithreading ? new Thread[SystemInfo.processorCount] : new Thread[1]);
			...
			while (!IsFracturingCancelled())
			{
				...
				if (num23 != num22)
				{
					...
					{
						s_VoronoiThreadData.progress("Fracturing", $"Finding cell plane dependencies... (cell {num23 + 1}/{num4}) ", (float)num23 / (float)num4);
					}
					num22 = num23;
				}
				lock (typeof(VoronoiThreadData))
			...
			if (!IsFracturingCancelled())
			{
				if (fracturedComponent.GenerateIslands)
				{
					...
					if (component3 != null)
					{
						...
						for (int num24 = 0; num24 < component3.GetObjectCount(); num24++)
						{
							...
							foreach (MeshData item2 in ComputeMeshDataIslands(new MeshData(component.transform, objectInfo.mesh, objectInfo.aMaterials, fracturedComponent.transform.localToWorldMatrix * objectInfo.mtxLocal, bTransformVerticesToWorld: true, -1, bBuildVertexHashData: true), bVerticesAreLocal: true, fracturedComponent, progress))
							{
								list4.Add(item2);
							}
						}
						if (fracturedComponent.GenerateChunkConnectionInfo)
						{
							for (int num25 = 0; num25 < list4.Count; num25++)
							{
								...
								for (int num26 = 0; num26 < list4.Count; num26++)
								{
									if (num25 != num26)
									{
										ComputeIslandsMeshDataConnectivity(fracturedComponent, bVerticesAreLocal: true, list4[num25], list4[num26]);
									}
								}
							}
						}
					...
					{
						list4 = ComputeMeshDataIslands(meshData, bVerticesAreLocal: true, fracturedComponent, progress);
					}
				}
				else
				{
				...
				else
				{
					...
					while (!IsFracturingCancelled())
					{
						...
						if (s_VoronoiThreadData.progress != null)
						{
							s_VoronoiThreadData.progress("Fracturing", $"Computing cell {num28}/{num4}", Mathf.Clamp01((float)num28 / (float)num4));
						}
						num22 = num28;
						lock (typeof(VoronoiThreadData))
						{
			...
			if (!IsFracturingCancelled())
			{
				...
				{
					Debug.Log($"Computed {s_FracturingStats.nSplitCount} slices for {list3.Count} chunks (Average: {(float)s_FracturingStats.nSplitCount / (float)list3.Count}).");
				}
				if (list3.Count > 0)
				{
					for (int num29 = 0; num29 < list3.Count; num29++)
					{
						...
						GameObject gameObject = CreateNewSplitGameObject(fracturedComponent.SourceObject, fracturedComponent, fracturedComponent.SourceObject.name + (num29 + 1), bTransformVerticesBackToLocal: false, list3[num29]);
						gameObject.AddComponent<Rigidbody>();
						gameObject.GetComponent<Rigidbody>().isKinematic = true;
						listGameObjectsOut.Add(gameObject);
			...
			bool num32 = IsFracturingCancelled();
			if (bPositionOnSourceAndHideOriginal)
			{
				fracturedComponent.gameObject.transform.position = fracturedComponent.SourceObject.transform.position;
				fracturedComponent.gameObject.transform.rotation = fracturedComponent.SourceObject.transform.rotation;
		...
		public static void ThreadVoronoiComputePlaneDependencies()
		{
			...
			while (true)
			{
				...
				if (s_VoronoiThreadData.spaceTree != null && SpaceTreeNode.GetSmallestPossibleMeshData(s_VoronoiThreadData.spaceTree, s_VoronoiThreadData.listVoronoiCells[nCurrentCell].v3Min, s_VoronoiThreadData.listVoronoiCells[nCurrentCell].v3Max).Count == 0)
				{
				...
				for (int i = 0; i < s_VoronoiThreadData.listVoronoiCells[nCurrentCell].listCellFaces.Count; i++)
				{
					...
					{
						if (SplitMeshUsingPlane(item, s_VoronoiThreadData.fracturedComponent, splitOptions, face.mtxPlane.MultiplyVector(Vector3.up), face.mtxPlane.MultiplyVector(Vector3.right), face.mtxPlane.MultiplyPoint3x4(Vector3.zero), out var listMeshDatasPosOut, out var listMeshDatasNegOut, s_VoronoiThreadData.progress))
						{
		...
		public static void ThreadVoronoiComputeCells()
		{
			...
			while (true)
			{
				...
				for (int i = 0; i < s_VoronoiThreadData.listVoronoiCells[nCurrentCell].listCellFaces.Count; i++)
				{
					...
					foreach (MeshData item2 in list)
					{
						if (SplitMeshUsingPlane(item2, s_VoronoiThreadData.fracturedComponent, splitOptions, face.mtxPlane.MultiplyVector(Vector3.up), face.mtxPlane.MultiplyVector(Vector3.right), face.mtxPlane.MultiplyPoint3x4(Vector3.zero), out var listMeshDatasPosOut, out var _, s_VoronoiThreadData.progress))
						{
							lock (s_FracturingStats)
							{
								s_FracturingStats.nSplitCount++;
		...
		public static List<MeshData> ComputeMeshDataIslands(MeshData meshDataIn, bool bVerticesAreLocal, FracturedObject fracturedComponent, ProgressDelegate progress)
		{
			...
			List<MeshData> list2 = MeshData.PostProcessConnectivity(meshDataIn, meshFaceConnectivity, meshDataConnectivity, array, list, meshDataIn.nSplitCloseSubMesh, meshDataIn.nCurrentVertexHash, bTransformToLocal: false);
			if (fracturedComponent.GenerateChunkConnectionInfo)
			{
		...
		public static void ComputeChunkColliders(FracturedObject fracturedComponent, ProgressDelegate progress)
		{
			...
			foreach (FracturedChunk listFracturedChunk in fracturedComponent.ListFracturedChunks)
			{
				...
				progress?.Invoke("Computing colliders", $"Collider {num + 1}/{fracturedComponent.ListFracturedChunks.Count}", (float)num / (float)fracturedComponent.ListFracturedChunks.Count);
				if (listFracturedChunk == null)
				{
					continue;
				}
				...
				if (!flag)
				{
					listFracturedChunk.gameObject.AddComponent<BoxCollider>();
					num2 += 12;
				}
				if ((bool)listFracturedChunk.GetComponent<Collider>())
				{
		...
		private static void ComputeChunkConnections(FracturedObject fracturedObject, List<GameObject> listGameObjects, List<MeshData> listMeshDatas, ProgressDelegate progress = null)
		{
			...
			{
				progress?.Invoke("Fracturing", "Computing connections...", (float)i / (float)listGameObjects.Count);
				if (IsFracturingCancelled())
				{
					break;
				}
		...
		public static bool SplitMeshUsingPlane(GameObject gameObjectIn, FracturedObject fracturedComponent, SplitOptions splitOptions, Transform transformPlaneSplit, out List<GameObject> listGameObjectsPosOut, out List<GameObject> listGameObjectsNegOut, ProgressDelegate progress = null)
		{
			...
			if (!SplitMeshUsingPlane(new MeshData(component.transform, component.sharedMesh, aMaterials, component.transform.localToWorldMatrix, !splitOptions.bVerticesAreLocal, num, bBuildVertexHashData: true), fracturedComponent, splitOptions, transformPlaneSplit.up, transformPlaneSplit.right, transformPlaneSplit.position, out var listMeshDatasPosOut, out var listMeshDatasNegOut, progress))
			{
				return false;
			}
		...
		private static bool SplitMeshUsingPlane(MeshData meshDataIn, FracturedObject fracturedComponent, SplitOptions splitOptions, Vector3 v3PlaneNormal, Vector3 v3PlaneRight, Vector3 v3PlanePoint, out List<MeshData> listMeshDatasPosOut, out List<MeshData> listMeshDatasNegOut, ProgressDelegate progress = null)
		{
			...
			if (flag12)
			{
				List<MeshData> collection = MeshData.PostProcessConnectivity(meshDataIn, meshFaceConnectivity, meshDataConnectivity, array, list, num2, nCurrentVertexHash, bTransformToLocal: false);
				List<MeshData> collection2 = new List<MeshData>();
				if (!splitOptions.bIgnoreNegativeSide)
				{
					collection2 = MeshData.PostProcessConnectivity(meshDataIn, meshFaceConnectivity2, meshDataConnectivity2, array2, list2, num2, nCurrentVertexHash, bTransformToLocal: false);
				}
			...
			else
			{
				...
				{
					MeshData meshData = new MeshData(meshDataIn.aMaterials, array, list, num2, vector, meshDataIn.qRotation, meshDataIn.v3Scale, inverse, bUseTransform: false, bBuildVertexHashData: false);
				...
				{
					MeshData meshData2 = new MeshData(meshDataIn.aMaterials, array2, list2, num2, vector2, meshDataIn.qRotation, meshDataIn.v3Scale, inverse2, bUseTransform: false, bBuildVertexHashData: false);
		...
		private static void TriangulateConstrainedDelaunay(List<List<Vector3>> listlistPointsConstrainedDelaunay, List<List<int>> listlistHashValuesConstrainedDelaunay, bool bForceVertexSoup, FracturedObject fracturedComponent, bool bConnectivityPostprocess, MeshFaceConnectivity faceConnectivityPos, MeshFaceConnectivity faceConnectivityNeg, MeshDataConnectivity meshConnectivityPos, MeshDataConnectivity meshConnectivityNeg, int nForceMeshConnectivityHash, int nSplitCloseSubMesh, Matrix4x4 mtxPlane, Matrix4x4 mtxToLocalPos, Matrix4x4 mtxToLocalNeg, Vector3 v3CenterPos, Vector3 v3CenterNeg, List<int>[] aListIndicesPosInOut, List<VertexData> listVertexDataPosInOut, List<int>[] aListIndicesNegInOut, List<VertexData> listVertexDataNegInOut)
		{
			...
			if (!bForceVertexSoup)
			{
				for (int l = 0; l < list2.Count; l++)
				{
					for (int m = 0; m < list2.Count; m++)
					{
						...
						{
							if (PolygonUtil.PolygonContainsPolygon(list[l], list3[l].Bounds, list[m], list3[m].Bounds, runIntersectionTest: true))
							{
								num4 = l;
								break;
							}
							if (PolygonUtil.PolygonContainsPolygon(list[m], list3[m].Bounds, list[l], list3[l].Bounds, runIntersectionTest: true))
							{
								num4 = m;
								break;
							}
			...
			if (polygon != null && !bForceVertexSoup)
			{
				try
				{
					...
					if (polygon.Triangles != null)
					{
						...
						CreateIndexedMesh(polygon.Triangles, list6, list7, mtxPlane, bTransform: true);
				...
				catch (Exception ex)
				{
					if (fracturedComponent.Verbose)
					{
						Debug.LogWarning("Exception (" + ex.GetType()?.ToString() + ") using hole triangulation (holes = " + list2.Count + "). Trying to use constrained delaunay.");
					}
					flag2 = false;
				}
			}
			...
			if (bForceVertexSoup)
			{
				...
				try
				{
					if (list8.Count >= 3)
					{
						...
						if (pointSet.Triangles != null)
						{
							...
							CreateIndexedMesh(pointSet.Triangles, list9, list10, mtxPlane, bTransform: true);
				...
				catch (Exception ex2)
				{
					if (fracturedComponent.Verbose)
					{
						Debug.LogWarning("Exception (" + ex2.GetType()?.ToString() + ") using vertex soup triangulation.");
					}
					return;
				}
			}
			...
			foreach (List<PolygonPoint> item3 in list2)
			{
				...
				catch (Exception ex3)
				{
					if (fracturedComponent.Verbose)
					{
						Debug.LogWarning("Exception (" + ex3.GetType()?.ToString() + ") using polygon triangulation of cap polygon " + num5 + ". Trying to use non constrained");
					}
					list11 = null;
				}
				if (list11 == null)
				{
					...
					catch (Exception ex4)
					{
						if (fracturedComponent.Verbose)
						{
							Debug.LogWarning("Exception (" + ex4.GetType()?.ToString() + ") using non constrained triangulation of cap polygon " + num5 + ". Skipping");
						}
					}
				}
				if (list11 != null)
				{
					...
					CreateIndexedMesh(list11, list13, list14, mtxPlane, bTransform: true);
		...
		private static void Triangulate(List<Vector3> listVertices, List<int> listIndices, FracturedObject fracturedComponent, List<List<Vector3>> listlistPointsConstrainedDelaunay, List<List<int>> listlistHashValuesConstrainedDelaunay, bool bConnectivityPostprocess, MeshFaceConnectivity faceConnectivityPos, MeshFaceConnectivity faceConnectivityNeg, MeshDataConnectivity meshConnectivityPos, MeshDataConnectivity meshConnectivityNeg, int nForceMeshConnectivityHash, int nSplitCloseSubMesh, Matrix4x4 mtxPlane, Matrix4x4 mtxToLocalPos, Matrix4x4 mtxToLocalNeg, Vector3 v3CenterPos, Vector3 v3CenterNeg, List<int>[] aListIndicesPosInOut, List<VertexData> listVertexDataPosInOut, List<int>[] aListIndicesNegInOut, List<VertexData> listVertexDataNegInOut)
		{
			...
			for (int i = 0; i < listVertices.Count; i++)
			{
				...
				array[i] = new VertexData(nVertexHash, listVertices[i], v3Normal, right, color, vector2, vector2, bHasNormal: true, bHasTangent: true, listVertexDataPosInOut[0].bHasColor32, listVertexDataPosInOut[0].bHasMapping1, listVertexDataPosInOut[0].bHasMapping2);
				array2[i] = new VertexData(nVertexHash, listVertices[i], v3Normal2, right2, color, vector2, vector2, bHasNormal: true, bHasTangent: true, listVertexDataNegInOut[0].bHasColor32, listVertexDataNegInOut[0].bHasMapping1, listVertexDataNegInOut[0].bHasMapping2);
			}
		...
		private static bool ResolveCap(Dictionary<EdgeKeyByHash, CapEdge> dicCapEdges, List<List<Vector3>> listlistResolvedCapVertices, List<List<int>> listlistResolvedCapHashValues, FracturedObject fracturedComponent)
		{
			...
			{
				Debug.LogWarning($"Cap has {list2.Count}/{count} unresolved segments left");
			}
			return true;
		}
	}
