...
namespace BCnEncoder.Encoder
{
	internal class Bc1BlockEncoder : BaseBcBlockEncoder<Bc1Block, RawBlock4X4Rgba32>
	{
		private static class Bc1BlockEncoderFast
		{
			...
			{
				RgbBoundingBox.Create565(rawBlock.AsSpan, out var min, out var max);
		...
		private static class Bc1BlockEncoderBalanced
		{
			...
			internal static Bc1Block EncodeBlock(RawBlock4X4Rgba32 rawBlock)
			{
				...
				for (int i = 0; i < 48; i++)
				{
					...
					float error2;
					Bc1Block bc1Block = TryColors(rawBlock, colorRgb4, colorRgb5, out error2);
					if (error2 < error)
					{
						result = bc1Block;
						error = error2;
		...
		private static Bc1Block TryColors(RawBlock4X4Rgba32 rawBlock, ColorRgb565 color0, ColorRgb565 color1, out float error, float rWeight = 0.3f, float gWeight = 0.6f, float bWeight = 0.1f)
		{
			...
			Span<ColorRgb24> span = ((!result.HasAlphaOrBlack) ? stackalloc ColorRgb24[4]
			{
				colorRgb,
				colorRgb2,
