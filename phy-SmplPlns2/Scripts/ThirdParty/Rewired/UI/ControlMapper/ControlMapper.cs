...
namespace Rewired.UI.ControlMapper
{
	...
	public class ControlMapper : MonoBehaviour
	{
		...
		private class Prefabs
		{
			...
			[SerializeField]
			private GameObject _mouseAsJoystickWindow;

			[SerializeField]
			private GameObject _advancedSettingsWindow;
			...
			public GameObject mouseAsJoystickWindow => _mouseAsJoystickWindow;

			public GameObject advancedSettingsWindow => _advancedSettingsWindow;
			...
			public bool Check()
			{
				if (_button == null || _fitButton == null || _inputGridLabel == null || _inputGridHeaderLabel == null || _inputGridFieldButton == null || _inputGridFieldInvertToggle == null || _window == null || _windowTitleText == null || _windowContentText == null || _fader == null || _calibrationWindow == null || _inputBehaviorsWindow == null || _mouseAsJoystickWindow == null || _advancedSettingsWindow == null)
				{
					return false;
				}
				return true;
		...
		private const string buttonIdentifier_mouseAsJoystickSettings = "MouseAsJoystickSettings";

		private const string buttonIdentifier_advancedSettings = "AdvancedSettings";
		...
		public void OnButtonActivated(ButtonInfo buttonInfo)
		{
			if (initialized && inputAllowed)
			{
				...
				case "RestoreDefaults":
					OnRestoreDefaults();
					break;
				case "MouseAsJoystickSettings":
					ShowMouseAsJoystickSettingsWindow();
					break;
		...
		private void OnElementAssignmentPrePollingWindowUpdate(int windowId)
		{
			...
			if (currentPlayer.controllers.Keyboard.GetKeyDown(KeyCode.Escape))
			{
				CloseWindow(windowId);
				return;
			}
		...
		private void ShowMouseAsJoystickSettingsWindow()
		{
			Window window = OpenWindow(prefabs.mouseAsJoystickWindow, "MouseAsJoystickSettingsWindow", closeOthers: true);
			if (!(window == null))
			{
				window.CreateTitleText(prefabs.windowTitleText, Vector2.zero, _language.mouseAsJoystickSettingsWindowTitle);
				windowManager.Focus(window);
			}
		}
		...
		{
			Window window = OpenWindow(prefabs.advancedSettingsWindow, "AdvancedSettingsWindow", closeOthers: true);
			if (!(window == null))
			{
				window.CreateTitleText(prefabs.windowTitleText, Vector2.zero, _language.advancedSettingsWindowTitle);
				windowManager.Focus(window);
			}
		}
		...
		private void DrawSettingsGroup()
		{
			if (showSettings)
			{
				...
				gUIButton.SetOnClickCallback(OnButtonActivated);
				GUIButton gUIButton2 = CreateButton(_language.mouseAsJoystickSettingsButtonLabel, references.settingsGroup.content, Vector2.zero);
				gUIButton2.buttonInfo.OnSelectedEvent += OnUIElementSelected;
				gUIButton2.SetButtonInfoData("MouseAsJoystickSettings", 0);
				gUIButton2.SetOnClickCallback(OnButtonActivated);
				if (Application.platform != RuntimePlatform.IPhonePlayer)
				{
					GUIButton gUIButton3 = CreateButton(_language.advancedSettingsButtonLabel, references.settingsGroup.content, Vector2.zero);
					gUIButton3.buttonInfo.OnSelectedEvent += OnUIElementSelected;
					gUIButton3.SetButtonInfoData("AdvancedSettings", 0);
					gUIButton3.SetOnClickCallback(OnButtonActivated);
				}
			}
		}
