...
namespace Rewired.Utils
{
	...
	{
		private static uint sNeAaabDrrSaBrdTePsQLPIDBMyfb = 0u;

		private static uint BmmhXhbjrlTfaTszgTQZWtQnMadr = 1u;

		private static uint HAJhlehFvlqgPaxYIHTGDyACftASb = uint.MaxValue;

		private static int BkdcJUeftGnoqPOLZrwXSQPDUBqL = 0;

		private static int dOZqGZZaMooHYldoPyVXCPGyTWFH = int.MaxValue;

		private static int pnBlPrcAOrhkHGAMVPHpJyTfTokW = -1;
		...
		{
			counter = ((counter != HAJhlehFvlqgPaxYIHTGDyACftASb) ? (counter + 1) : BmmhXhbjrlTfaTszgTQZWtQnMadr);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != dOZqGZZaMooHYldoPyVXCPGyTWFH) ? (counter + 1) : BkdcJUeftGnoqPOLZrwXSQPDUBqL);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != BmmhXhbjrlTfaTszgTQZWtQnMadr) ? (counter - 1) : HAJhlehFvlqgPaxYIHTGDyACftASb);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= pnBlPrcAOrhkHGAMVPHpJyTfTokW)
			{
				return dOZqGZZaMooHYldoPyVXCPGyTWFH;
			}
			counter = ((counter != BkdcJUeftGnoqPOLZrwXSQPDUBqL) ? (counter - 1) : dOZqGZZaMooHYldoPyVXCPGyTWFH);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != sNeAaabDrrSaBrdTePsQLPIDBMyfb;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > pnBlPrcAOrhkHGAMVPHpJyTfTokW;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == sNeAaabDrrSaBrdTePsQLPIDBMyfb)
			{
				return false;
			}
			if (tick2 == sNeAaabDrrSaBrdTePsQLPIDBMyfb)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= pnBlPrcAOrhkHGAMVPHpJyTfTokW)
			{
				return false;
			}
			if (tick2 <= pnBlPrcAOrhkHGAMVPHpJyTfTokW)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = HAJhlehFvlqgPaxYIHTGDyACftASb - num + num2;
			uint bmmhXhbjrlTfaTszgTQZWtQnMadr = BmmhXhbjrlTfaTszgTQZWtQnMadr;
			uint num4 = num3 - bmmhXhbjrlTfaTszgTQZWtQnMadr;
			if (tick1 >= tick2)
			{
				return 0L - (long)num4;
			}
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = dOZqGZZaMooHYldoPyVXCPGyTWFH - num + num2;
			int bkdcJUeftGnoqPOLZrwXSQPDUBqL = BkdcJUeftGnoqPOLZrwXSQPDUBqL;
			int num4 = num3 - bkdcJUeftGnoqPOLZrwXSQPDUBqL;
			if (tick1 >= tick2)
			{
				return -num4;
			}
