	...
	public sealed class TouchPad : TouchInteractable, IPointerDownHandler, IEventSystemHandler, IPointerUpHandler
	{
		...
		private void kfBvoUvOjpcqXzqjoFwXtBqaQSCj(PointerEventData P_0)
		{
			...
			if (flag2)
			{
				...
				PointerEventData pointerEventData = dQPHBbAtNASxaeGZcyogtwRjlXHC((_realMousePointerId != int.MinValue) ? _realMousePointerId : P_0.pointerId, gameObject);
		...
		private bool kqyVZLAuUHmahTJZBQbIXptvFHnp(int P_0)
		{
			...
			if (TouchInteractable.mBmltzbovhkqiRskAgVuCNaKKMGn(P_0) && _realMousePointerId != int.MinValue && P_0 == _realMousePointerId)
			{
				return true;
			}
			return false;
