	...
	internal sealed class Connection
	{
				...
				try
				{
					if (in_Renamed == null || out_Renamed == null)
					{
						if (Ssl)
						{
							...
							NetworkStream innerStream = new NetworkStream(sock, ownsSocket: true);
							try
							{
							...
							SslStream sslStream = new SslStream(innerStream, leaveInnerStreamOpen: false);
		...
		internal void startTLS()
		{
			try
			{
				waitForReader(null);
				nonTLSBackup = socket;
				SslStream sslStream = new SslStream(socket.GetStream(), leaveInnerStreamOpen: false);
				sslStream.AuthenticateAsClient(host);
				in_Renamed = sslStream;
				out_Renamed = sslStream;
			}
