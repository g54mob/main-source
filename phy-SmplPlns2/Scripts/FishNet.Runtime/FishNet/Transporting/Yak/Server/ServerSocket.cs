...
namespace FishNet.Transporting.Yak.Server
{
	public class ServerSocket : CommonSocket
	{
		...
		internal void IterateIncoming()
		{
			if (GetLocalConnectionState() == LocalConnectionState.Started)
			{
				while (_incoming.Count > 0)
				{
					...
					ServerReceivedDataArgs receivedDataArgs = new ServerReceivedDataArgs(data, (Channel)localPacket.Channel, int.MaxValue, Transport.Index);
		...
		{
			if (GetLocalConnectionState() == LocalConnectionState.Started && connectionId == int.MaxValue)
			{
		...
		internal void OnLocalClientConnectionState(LocalConnectionState state)
		{
			if (state != LocalConnectionState.Started)
			{
				...
				{
					Transport.HandleRemoteConnectionState(new RemoteConnectionStateArgs(RemoteConnectionState.Stopped, int.MaxValue, Transport.Index));
				}
			}
			else
			{
				Transport.HandleRemoteConnectionState(new RemoteConnectionStateArgs(RemoteConnectionState.Started, int.MaxValue, Transport.Index));
			}
		}
