...
namespace NWH.VehiclePhysics2.Powertrain
{
	...
	public class EngineComponent : PowertrainComponent
	{
		...
		public float CalculateTorqueICE(float angularVelocity, float dt)
		{
			...
			if (revLimiterCutoffDuration <= 0f)
			{
				if (angularVelocity >= _revLimiterAngularVelocity)
				{
					_throttlePosition = 0f;
			...
			else if (angularVelocity >= _revLimiterAngularVelocity && !revLimiterActive)
			{
		...
		private float CalculateICEGeneratedTorqueFromPowerCurve()
		{
			...
			if (revLimiterActive && revLimiterCutoffDuration > 0f)
			{
		...
		private IEnumerator StarterCoroutine()
		{
			...
			float num = inertia + ((_output != null) ? _output.QueryInertia() : 0f);
			float value = (_idleAngularVelocity - outputAngularVelocity) * num / startDuration;
			float max = EstimatedPeakTorque * 0.4f;
			_starterTorque = Mathf.Clamp(value, 0f, max);
