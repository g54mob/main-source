...
namespace Shapes
{
	public class ChargeBar : MonoBehaviour
	{
		...
		public void DrawBar(FpsController fpsController, float barRadius)
		{
			...
			float num3 = num + MathF.PI;
			float num4 = num2 + MathF.PI;
			...
			for (int i = 0; i < 7; i++)
			{
				...
				Draw.Text(content: Mathf.RoundToInt(num8 * 100f) + "%", rot: Quaternion.Euler(0f, 0f, (num9 + MathF.PI) * 57.29578f), pos: vector4, align: TextAlign.Right);
			}
			...
			Draw.Disc(vector, ammoBarThickness * 2f, DiscColors.Radial(color, Color.clear));
