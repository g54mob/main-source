...
namespace JBooth.MicroSplat
{
	public class CurvatureMapGenerator
	{
		...
		private static float Horizontal(float dx, float dy, float dxx, float dyy, float dxy)
		{
		...
		private static float Vertical(float dx, float dy, float dxx, float dyy, float dxy)
		{
		...
		private static float Average(float dx, float dy, float dxx, float dyy, float dxy)
		{
		...
		private static void UpdateWaterMap(float[,] waterMap, float[,,] outFlow, int width, int height)
		{
		...
		private static void CalculateVelocityField(float[,] velocityMap, float[,,] outFlow, int width, int height)
		{
			for (int i = 0; i < height; i++)
			{
				for (int j = 0; j < width; j++)
		...
		private static void NormalizeMap(float[,] map, int width, int height)
		{
		...
		private static void ComputeOutflow(float[,] waterMap, float[,,] outFlow, float[,] heightMap, int width, int height)
		{
			for (int i = 0; i < height; i++)
			{
				for (int j = 0; j < width; j++)
