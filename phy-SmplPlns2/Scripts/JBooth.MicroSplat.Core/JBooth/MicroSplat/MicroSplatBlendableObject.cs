	...
	public class MicroSplatBlendableObject : MonoBehaviour
	{
		...
		private void OnDisable()
		{
			if (msObject != null)
			{
				...
				{
					RemoveMaterial(blendMatInstance);
				}
			}
		}
		...
		{
			Material[] array = GetComponent<Renderer>().sharedMaterials;
			for (int i = 0; i < array.Length; i++)
			{
				if (array[i] == m && m != null)
				{
					List<Material> list = new List<Material>(array);
					list.RemoveAt(i);
					array = list.ToArray();
					i--;
				}
