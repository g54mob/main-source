...
namespace JBooth.MicroSplat
{
	public class MicroSplatProceduralTextureUtil
	{
		...
		private static float PCFilter(int index, float height, float slope, float cavity, float flow, Vector3 worldPos, Vector2 uv, Color bMask, Color bMask2, out int texIndex, Vector3 pN, MicroSplatProceduralTextureConfig config, Texture2D procTexNoise, NoiseUVMode noiseMode, Vector4 noiseWeights)
		{
			...
			float w = noiseWeights.w;
			float t = color.r * noiseWeights.x + color.g * noiseWeights.y + color.b * noiseWeights.z;
			num *= 1f + Mathf.Max(0f, Mathf.Lerp(layer.noiseRange.x, layer.noiseRange.y, t) * w);
			num2 *= 1f + Mathf.Max(0f, Mathf.Lerp(layer.noiseRange.x, layer.noiseRange.y, color.g) * w);
			num3 *= 1f + Mathf.Max(0f, Mathf.Lerp(layer.noiseRange.x, layer.noiseRange.y, color.b) * w);
			num4 *= 1f + Mathf.Max(0f, Mathf.Lerp(layer.noiseRange.x, layer.noiseRange.y, color.a) * w);
		...
		private static void PCProcessLayer(ref Vector4 weights, ref Int4 indexes, ref float totalWeight, int curIdx, float height, float slope, float cavity, float flow, Vector3 worldPos, Vector2 uv, Color biomeMask, Color biomeMask2, Vector3 pN, MicroSplatProceduralTextureConfig config, Texture2D noiseMap, NoiseUVMode noiseMode, Vector4 noiseWeights)
		{
			int texIndex = 0;
			float b = PCFilter(curIdx, height, slope, cavity, flow, worldPos, uv, biomeMask, biomeMask2, out texIndex, pN, config, noiseMap, noiseMode, noiseWeights);
		...
		public static void Sample(Vector2 uv, Vector3 worldPos, Vector3 worldNormal, Vector3 up, NoiseUVMode noiseUVMode, Material mat, MicroSplatProceduralTextureConfig config, out Vector4 weights, out Int4 indexes, Vector4 noiseWeights = default(Vector4))
		{
			...
			Texture2D texture2D3 = (mat.HasProperty("_ProcTexBiomeMask2") ? ((Texture2D)mat.GetTexture("_ProcTexBiomeMask2")) : null);
			...
			if (texture2D3 != null)
			{
				biomeMask2 = texture2D3.GetPixelBilinear(uv.x, uv.y);
			}
