...
namespace JBooth.MicroSplat
{
	public class MicroSplatProceduralTextureConfig : ScriptableObject
	{
		...
		{
			int maxTextures = propData.maxTextures;
			int num = 128;
			...
			if (heightGradientTex == null)
			{
				heightGradientTex = new Texture2D(num, maxTextures, TextureFormat.RGBA32, mipChain: false);
			}
			...
			for (int k = heightGradients.Count; k < propData.maxTextures; k++)
			{
				for (int l = 0; l < num; l++)
				{
		...
		public Texture2D GetHeightHSVTexture(MicroSplatPropData propData)
		{
			...
			if (heightHSVTex == null)
			{
				heightHSVTex = new Texture2D(num, maxTextures, TextureFormat.RGBA32, mipChain: false);
			}
			Color grey = Color.grey;
			...
			for (int k = heightHSV.Count; k < propData.maxTextures; k++)
			{
				for (int l = 0; l < num; l++)
				{
		...
		public Texture2D GetSlopeGradientTexture(MicroSplatPropData propData)
		{
			...
			if (slopeGradientTex == null)
			{
				slopeGradientTex = new Texture2D(num, maxTextures, TextureFormat.RGBA32, mipChain: false);
			}
			Color grey = Color.grey;
			...
			for (int k = slopeGradients.Count; k < propData.maxTextures; k++)
			{
				for (int l = 0; l < num; l++)
				{
		...
		public Texture2D GetSlopeHSVTexture(MicroSplatPropData propData)
		{
			...
			if (slopeHSVTex == null)
			{
				slopeHSVTex = new Texture2D(num, maxTextures, TextureFormat.RGBA32, mipChain: false);
			}
			Color grey = Color.grey;
			...
			for (int k = slopeHSV.Count; k < propData.maxTextures; k++)
			{
				for (int l = 0; l < num; l++)
				{
		...
		private float CompFilter(Layer.Filter f, Layer.CurveMode mode, float v)
		{
		...
		public Texture2D GetCurveTexture(MicroSplatPropData propData)
		{
			...
			{
				UnityEngine.Object.DestroyImmediate(curveTex, allowDestroyingAssets: true);
				curveTex = null;
			}
			if (curveTex == null)
			{
				curveTex = new Texture2D(num, maxTextures, TextureFormat.RGBA32, mipChain: false, linear: true);
			}
