...
namespace GPUInstancerPro
{
	public static class GPUIHiZDepthTextureUtility
	{
		...
		public static void CopyHiZTextureWithComputeShader(ComputeShader cs, CommandBuffer commandBuffer, RenderTargetIdentifier sourceIdentifier, int sourceW, int sourceH, RenderTargetIdentifier destinationIdentifier, int offsetX)
		{
			...
			commandBuffer.DispatchCompute(cs, kernelIndex, Mathf.CeilToInt((float)sourceW / GPUIConstants.CS_THREAD_COUNT_2D), Mathf.CeilToInt((float)sourceH / GPUIConstants.CS_THREAD_COUNT_2D), 1);
		}
		...
		{
			GetMipSize(sourceW, sourceH, sourceMip, destinationMip, out var destinationW, out var destinationH, out sourceW, out sourceH);
			int kernelIndex = 0;
		...
		public static void ReduceTextureWithComputeShader(ComputeShader cs, CommandBuffer commandBuffer, RenderTargetIdentifier sourceIdentifier, int sourceW, int sourceH, int sourceMip, int destinationMip)
		{
			...
			commandBuffer.DispatchCompute(cs, kernelIndex, Mathf.CeilToInt((float)destinationW / GPUIConstants.CS_THREAD_COUNT_2D), Mathf.CeilToInt((float)destinationH / GPUIConstants.CS_THREAD_COUNT_2D), 1);
		}
