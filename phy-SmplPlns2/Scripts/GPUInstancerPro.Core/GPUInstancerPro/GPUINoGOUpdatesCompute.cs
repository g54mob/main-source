...
namespace GPUInstancerPro
{
	...
	public class GPUINoGOUpdatesCompute : MonoBehaviour
	{
		...
		private void Update()
		{
			if (_rendererKey != 0 && _instanceCount != 0 && !(computeShader == null) && GPUICoreAPI.TryGetTransformBufferData(_rendererKey, out var transformBufferData, out var bufferStartIndex, out var _))
			{
				computeShader.SetBuffer(0, GPUIConstants.PROP_gpuiTransformBuffer, transformBufferData.GetTransformBuffer().Buffer);
