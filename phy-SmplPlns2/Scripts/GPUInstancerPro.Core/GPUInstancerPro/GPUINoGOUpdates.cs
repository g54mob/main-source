...
namespace GPUInstancerPro
{
	...
	public class GPUINoGOUpdates : MonoBehaviour
	{
			...
			public void Execute(int r)
			{
				...
				for (int i = 0; i < vertical; i++)
				{
					...
					for (int j = 0; j < circular; j++)
					{
						...
						one.x = num3 * math.cos(num4 - time * spinSpeed.x) + posOffset.x;
						one.y = (float)i + posOffset.y;
						one.z = num3 * math.sin(num4 - time * spinSpeed.x) + posOffset.z;
		...
		private void Update()
		{
			...
			{
				_jobHandle.Complete();
				GPUICoreAPI.SetTransformBufferData(_rendererKey, _matrix4X4s, 0, 0, 0, isOverwritePreviousFrameBuffer: false);
				if (enableColorVariations)
				{
					_colorBuffer.SetData(_colors);
				}
		...
		{
			_jobHandle = IJobParallelForExtensions.Schedule(new MatrixDataGeneratorJob
			{
			...
			if (enableColorVariations)
			{
				_jobHandle = IJobParallelForExtensions.Schedule(new ColorDataGeneratorJob
				{
					colors = _colors,
					time = Time.time,
					radial = radial,
