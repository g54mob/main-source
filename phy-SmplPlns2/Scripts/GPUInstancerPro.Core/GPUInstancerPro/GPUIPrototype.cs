...
namespace GPUInstancerPro
{
	...
	public class GPUIPrototype : IEquatable<GPUIPrototype>
	{
		...
		[SerializeField]
		public bool isRequireTreeProxy;
		...
		public bool IsValid(bool logError)
		{
			...
			case GPUIPrototypeType.Prefab:
				if (prefabObject == null)
				{
					if (logError)
					{
						Debug.LogError(GPUIConstants.LOG_PREFIX + this?.ToString() + " prefabObject is null.");
					}
					errorCode = 1001;
					return false;
				}
			...
			case GPUIPrototypeType.LODGroupData:
				if (gpuiLODGroupData == null)
				{
					if (logError)
					{
						Debug.LogError(GPUIConstants.LOG_PREFIX + this?.ToString() + " gpuiLODGroupData is null.");
					}
					errorCode = 1002;
					return false;
				}
			...
			case GPUIPrototypeType.MeshAndMaterial:
				if (prototypeMesh == null)
				{
					if (logError)
					{
						Debug.LogError(GPUIConstants.LOG_PREFIX + this?.ToString() + " mesh is null.");
					}
					errorCode = 1003;
					return false;
				}
				if (prototypeMaterials == null)
				{
					if (logError)
					{
						Debug.LogError(GPUIConstants.LOG_PREFIX + this?.ToString() + " materials is null.");
					}
					errorCode = 1004;
					return false;
				}
		...
		public void GenerateBillboard(bool forceNew = true)
		{
			...
			{
				GPUIBillboardUtility.GenerateBillboardDelayed(billboardAsset, saveAsAsset: true);
			}
		}
