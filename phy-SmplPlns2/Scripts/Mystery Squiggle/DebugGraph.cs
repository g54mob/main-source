...
public static class DebugGraph
{
	...
	public static Color DefaultPurple = GetUniqueColor(3);
	...
	internal static readonly string[] Vector4ValueNames = new string[4] { "X", "Y", "Z", "W" };

	internal static readonly string[] QuaternionValueNames = new string[3] { "Yaw", "Pitch", "Roll" };

	internal static readonly string[] RectValueNames = new string[4] { "X", "Y", "Width", "Height" };
	...
	public static float MinDisplayTime => TimeRange.SharedMin;

	public static float MaxDisplayTime => TimeRange.SharedMax;
	...
	{
		if (value < TimeRange.SharedMin)
		{
			TimeRange.SharedMin = value;
		}
		if (value > TimeRange.SharedMax)
		{
			TimeRange.SharedMax = value;
		}
	}

	public static void CleanUpHistory(float length, bool onlyCleanUpSharedTime = true)
	{
		CleanUpHistory(length, MaxDisplayTime, onlyCleanUpSharedTime);
	}

	public static void CleanUpHistory(float length, float fromTime, bool onlyCleanUpSharedTime = true)
	{
		lock (graphs)
		{
			TimeRange.SharedMax = fromTime;
			float num = TimeRange.SharedMax - length;
			if (num > TimeRange.SharedMin)
			{
				TimeRange.SharedMin = num;
			}
			foreach (IGraphConsole value in graphs.Values)
			{
				value.CleanUpBefore(TimeRange.SharedMin, onlyCleanUpSharedTime);
			}
		}
	}
	...
	public static void ClearDeveloperConsole()
	{
		lock (graphs)
		{
			foreach (IGraphConsole value in graphs.Values)
			{
				value.Clear(resetRanges: false);
			}
		}
		graphs.Clear();
		TimeRange.SharedMin = float.MaxValue;
		TimeRange.SharedMax = float.MinValue;
		GC.Collect();
	}

	public static List<ILineGraphPoint[]> ExportData(string graphName)
	{
	...
	public static void SaveToStream(IGraphConsole console, StreamWriter sw, FileTypes fileType = FileTypes.CommaDelimited, bool format = true)
	{
		...
		case FileTypes.TabDelimited:
			...
			break;
		case FileTypes.Mesh:
			GraphExportUtility.WriteMesh(sw, console.Name, console.ValueNames, console.ExportData(), format);
			break;
		}
	}
	...
	public static void Write(string name, object value, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new StringGraphConsole(name, new StringLineGraphOverTime()));
			}
		}
	...
	public static void Log(string name, Enum value, Color color, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new SingleGraphConsole<float, long>(name, new EnumLineGraphOverTime(value.GetType())));
			}
		}
	...
	public static void Log(string name, bool value, Color color, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new SingleGraphConsole<float, bool>(name, new BooleanLineGraphOverTime()));
			}
		}
	...
	public static void Log(string name, double value, Color color, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new SingleGraphConsole<float, double>(name, new FloatingPointLineGraphOverTime()));
			}
		}
	...
	public static void Log(string name, long value, Color color, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new SingleGraphConsole<float, long>(name, new IntegerLineGraphOverTime()));
			}
		}
	...
	public static void Log(string name, Vector2 value, IEnumerable<string> valueNames, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new MultiGraphConsole<float, double>(name, (Type newGraphType) => new FloatingPointLineGraphOverTime()));
			}
		}
	...
	{
		Log(GetAnonymousName<Vector4>(), value);
	}

	public static void Log(string name, Vector4 value)
	{
		Log(name, value, GetDefaultTime());
	}

	public static void Log(string name, Vector4 value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, Vector4 value, float time)
	{
		Log(name, value, Vector4ValueNames, GetDefaultTime());
	}

	public static void Log(string name, Vector4 value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
		...
		obj.Push(time, value.y, DefaultGreen);
		obj.Push(time, value.z, DefaultBlue);
		obj.Push(time, value.w, DefaultPurple);
		UpdateTimeRange(time);
	}

	public static void Log(Quaternion value)
	{
		Log(GetAnonymousName<Quaternion>(), value);
	}

	public static void Log(string name, Quaternion value)
	{
		Log(name, value, GetDefaultTime());
	}

	public static void Log(string name, Quaternion value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, Quaternion value, float time)
	{
		Log(name, value, QuaternionValueNames, GetDefaultTime());
	}

	public static void Log(string name, Quaternion value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
		...
		obj.Push(time, eulerAngles.z, DefaultRed);
		obj.Push(time, eulerAngles.y, DefaultGreen);
		obj.Push(time, eulerAngles.x, DefaultBlue);
		UpdateTimeRange(time);
	}

	public static void Log(Rect value)
	{
		Log(GetAnonymousName<Rect>(), value);
	}

	public static void Log(string name, Rect value)
	{
		Log(name, value, GetDefaultTime());
	}

	public static void Log(string name, Rect value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, Rect value, float time)
	{
		Log(name, value, RectValueNames, GetDefaultTime());
	}

	public static void Log(string name, Rect value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
		...
		obj.Push(time, value.x, DefaultRed);
		obj.Push(time, value.y, DefaultGreen);
		obj.Push(time, value.width, DefaultBlue);
		obj.Push(time, value.height, DefaultPurple);
		UpdateTimeRange(time);
	}
	...
	public static void Log(double[] value)
	{
		Log(GetAnonymousName<double[]>(), value);
	}
	...
	public static void Log(string name, double[] value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, double[] value, float time)
	{
		Log(name, value, null, GetDefaultTime());
	}

	public static void Log(string name, double[] value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
	...
	public static void Log(string name, uint[] value, IEnumerable<string> valueNames, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new MultiGraphConsole<float, long>(name, (Type newGraphType) => new IntegerLineGraphOverTime()));
			}
		}
	...
	{
		Log(GetAnonymousName<byte[]>(), value);
	}
	...
	public static void Log(string name, byte[] value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, byte[] value, float time)
	{
		Log(name, value, null, GetDefaultTime());
	}

	public static void Log(string name, byte[] value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
	...
	{
		Log(GetAnonymousName<short[]>(), value);
	}
	...
	public static void Log(string name, short[] value, IEnumerable<string> valueNames)
	{
		Log(name, value, valueNames, GetDefaultTime());
	}

	public static void Log(string name, short[] value, float time)
	{
		Log(name, value, null, GetDefaultTime());
	}

	public static void Log(string name, short[] value, IEnumerable<string> valueNames, float time)
	{
		if (!loggingEnabled)
		{
			return;
	...
	public static void Log(string name, Color value, IEnumerable<string> valueNames, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new ColorGraphConsole<double>(name, (Type newGraphType) => new FloatingPointLineGraphOverTime()));
			}
		}
	...
	public static void Log(string name, Color32 value, IEnumerable<string> valueNames, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new ColorGraphConsole<long>(name, (Type newGraphType) => new IntegerLineGraphOverTime()));
			}
		}
	...
	public static void MultiLog(string name, Color color, Enum value, string valueName, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new MultiGraphConsole<float, long>(name, (Type newGraphType) => new EnumLineGraphOverTime(newGraphType)));
			}
		}
	...
	public static void MultiLog(string name, Color color, bool value, string valueName, float time)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new MultiGraphConsole<float, bool>(name, (Type newGraphType) => new BooleanLineGraphOverTime()));
			}
		}
	...
	public static void Draw(string name, Vector2 value, Color color)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new SingleGraphConsole<float, float>(name, new NonLinearFloatingPointLineGraph()));
			}
		}
	...
	public static void MultiDraw(string name, Color color, Vector2 value, IEnumerable<string> valueNames)
	{
		...
		if (!graphs.ContainsKey(name))
		{
			lock (graphs)
			{
				graphs.Add(name, new MultiGraphConsole<float, float>(name, (Type newGraphType) => new NonLinearFloatingPointLineGraph()));
			}
		}
