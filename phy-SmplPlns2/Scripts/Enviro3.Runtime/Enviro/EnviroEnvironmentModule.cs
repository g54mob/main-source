...
namespace Enviro
{
	...
	public class EnviroEnvironmentModule : EnviroModule
	{
		...
		public void UpdateSeason()
		{
			...
			{
				int dayOfYear = EnviroManager.instance.Time.Settings.date.DayOfYear;
				if (dayOfYear >= Settings.springStart && dayOfYear <= Settings.springEnd)
				{
					ChangeSeason(EnviroEnvironment.Seasons.Spring);
				}
				else if (dayOfYear >= Settings.summerStart && dayOfYear <= Settings.summerEnd)
				{
					ChangeSeason(EnviroEnvironment.Seasons.Summer);
				}
				else if (dayOfYear >= Settings.autumnStart && dayOfYear <= Settings.autumnEnd)
				{
					ChangeSeason(EnviroEnvironment.Seasons.Autumn);
				}
				else if (dayOfYear >= Settings.winterStart || dayOfYear <= Settings.winterEnd)
				{
