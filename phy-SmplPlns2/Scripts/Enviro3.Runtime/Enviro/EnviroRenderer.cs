		...
		private void OnRenderImage(RenderTexture src, RenderTexture dest)
		{
			...
			if (EnviroManager.instance.Fog != null && EnviroManager.instance.VolumetricClouds != null && flag && flag2)
			{
				...
				else
				{
					...
					if (EnviroManager.instance.VolumetricClouds.settingsGlobal.cloudShadows && myCam.cameraType != CameraType.Reflection)
					{
						EnviroManager.instance.VolumetricClouds.RenderVolumetricClouds(myCam, temporary, temporary2, volumetricCloudsRender, myQuality);
						EnviroManager.instance.VolumetricClouds.RenderCloudsShadows(temporary2, dest, volumetricCloudsRender);
					}
					else
					{
			...
			else if (EnviroManager.instance.VolumetricClouds != null && flag && !flag2)
			{
				if (EnviroManager.instance.VolumetricClouds.settingsGlobal.cloudShadows && myCam.cameraType != CameraType.Reflection)
				{
					...
					EnviroManager.instance.VolumetricClouds.RenderVolumetricClouds(myCam, src, temporary3, volumetricCloudsRender, myQuality);
					EnviroManager.instance.VolumetricClouds.RenderCloudsShadows(temporary3, dest, volumetricCloudsRender);
