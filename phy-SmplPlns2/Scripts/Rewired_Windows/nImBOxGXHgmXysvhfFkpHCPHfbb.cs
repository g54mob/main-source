...
internal abstract class nImBOxGXHgmXysvhfFkpHCPHfbb<_0001, _0002, _0003> : hlOkjDeqVWQCgnqAjZMKnjtRaEFeA where _0001 : class, global::HWTyxBmMFQveNIpALfzbflNTYGAN<_0002, _0003>, new() where _0002 : struct where _0003 : struct, CQLGJWnqRRTjXzVQgIYtWAvLPRYO
{
	...
	protected nImBOxGXHgmXysvhfFkpHCPHfbb(IntPtr P_0)
		: base(P_0)
	{
	}
	...
	public unsafe _0003[] yAWzeoCGGjKvxePumwcKThUmiImi()
	{
		_0003[] xcMkzSEyyndodxXvRKsMzSGjlxKe = XcMkzSEyyndodxXvRKsMzSGjlxKe;
		int num = luYaFPaftNInTWGPWfCvgYuDUqDyA.glwGxVPzunhdOUxGIRKtVKPYTQvO<LngOeBwvwgXecSawVCGWvvGjBiUq>();
		int num2 = -1;
		pOHWdCgXBgEUfvpDQjJXFPxMrcMm(num, IntPtr.Zero, ref num2, 1);
		if (num2 == 0)
		{
			return xcMkzSEyyndodxXvRKsMzSGjlxKe;
		}
		LngOeBwvwgXecSawVCGWvvGjBiUq* ptr = stackalloc LngOeBwvwgXecSawVCGWvvGjBiUq[num2];
		pOHWdCgXBgEUfvpDQjJXFPxMrcMm(num, (IntPtr)ptr, ref num2, 0);
		if (num2 == 0)
		{
			return xcMkzSEyyndodxXvRKsMzSGjlxKe;
		}
		xcMkzSEyyndodxXvRKsMzSGjlxKe = new _0003[num2];
		for (int i = 0; i < num2; i++)
		{
			xcMkzSEyyndodxXvRKsMzSGjlxKe[i] = new _0003
			{
				xuWnIZVAQMTnDELRGPGCYiUtSHvF = ptr[i].RnWzxJSdkCfzfEgLnLNWAtDKltJUB,
				LVziXCWSBxuqpjhZHuUQQcTXBrBv = ptr[i].fWEMREigsxdTPdNaXChLQvYOPBGd,
				HXwAsrhuaaqKUDPFvNuFdAsHADJGB = ptr[i].CGoVdUKAyptvzKpgnKVafHeUBGOk,
				DGxJfkrsDjIWuNHwIhpubKxrDPtkA = ptr[i].TunNljueMinkfPXhVyxLJTGpujQK
			};
		}
		return xcMkzSEyyndodxXvRKsMzSGjlxKe;
	}
	...
	public bqkAwqSafotyQAOrTnFIVdkzxoBR GHWpeUFbtQLaRRSdYAbxoLKsGYud(string P_0)
	{
		return LCyqHtUfqYLSpcKtDeEJgNQywleC(MAdwOyJHIJgKndqGRlWKNgoeZLDk(P_0).yPiSpoPQsQajctZjZsuxjpsSOyVQ, EoLpuCLXHDXPvpnkuJEEHGDRPkHB.Byoffset);
	}

	public eknQmirlopnXfygbjxHwrBvfBNnhA CPPiwhdJjwewkoAsIeCrmZgkEMKn(string P_0)
	{
		return new eknQmirlopnXfygbjxHwrBvfBNnhA(this, MAdwOyJHIJgKndqGRlWKNgoeZLDk(P_0).yPiSpoPQsQajctZjZsuxjpsSOyVQ, EoLpuCLXHDXPvpnkuJEEHGDRPkHB.Byoffset);
	}

	private hWVWhsQejgFPmATIkqbBBZrEFzrcb MAdwOyJHIJgKndqGRlWKNgoeZLDk(string P_0)
	{
		if (!ITINXvpSExsUUCBweKaiNjbAGziL.TryGetValue(P_0, out var value))
		{
			throw new ArgumentException(string.Format(CultureInfo.InvariantCulture, "Invalid name [{0}]. Must be in [{1}]", P_0, luYaFPaftNInTWGPWfCvgYuDUqDyA.sGWYSYOBevYodeZGFSuDJldOiDLs(";", ITINXvpSExsUUCBweKaiNjbAGziL.Keys)));
		}
		return value;
	}
	...
	private unsafe void BsvDEedshGwKYTOhksUPghPuJnjZ(lqsEFGvGeAeizRYAFkQDyzxyOJRL P_0)
	{
		lqsEFGvGeAeizRYAFkQDyzxyOJRL.NwaVXKQcUfDTKytqcHGUNZmqVStC nwaVXKQcUfDTKytqcHGUNZmqVStC = default(lqsEFGvGeAeizRYAFkQDyzxyOJRL.NwaVXKQcUfDTKytqcHGUNZmqVStC);
		P_0.bSzfngWFRiSItnEKTDbhcKqjLiuHA(ref nwaVXKQcUfDTKytqcHGUNZmqVStC);
		string name = typeof(_0002).Name;
		Console.WriteLine("{0}.dwSize     {1}", name, nwaVXKQcUfDTKytqcHGUNZmqVStC.tsuCaHBVymrkeCMunqJgnpwWeLtwA);
		Console.WriteLine("{0}.dwObjSize  {1}", name, nwaVXKQcUfDTKytqcHGUNZmqVStC.ZxbOodKxCFPUzyVYQAHWUVWIPWQC);
		Console.WriteLine("{0}.dwFlags    {1} ({2})", name, (int)nwaVXKQcUfDTKytqcHGUNZmqVStC.vzOFFZUMzSjziHCXHfTAGfcQWPCd, nwaVXKQcUfDTKytqcHGUNZmqVStC.vzOFFZUMzSjziHCXHfTAGfcQWPCd);
		Console.WriteLine("{0}.dwDataSize {1}", name, nwaVXKQcUfDTKytqcHGUNZmqVStC.PuuDnwmJNfkkmYXTbCyupfGREXnH);
		Console.WriteLine("{0}.dwNumObjs  {1}", name, nwaVXKQcUfDTKytqcHGUNZmqVStC.OcDSsRtGpwuucxTmSJJxJhEICxap);
		Console.WriteLine("{4,32};{0,38};{1,8},{2,8};{3,8}", "Guid", "Offset", "Type", "Flags", "Name");
		hWVWhsQejgFPmATIkqbBBZrEFzrcb.OrCfOpHoCWSiMFhUDkuSDIBBrQesd* ptr = (hWVWhsQejgFPmATIkqbBBZrEFzrcb.OrCfOpHoCWSiMFhUDkuSDIBBrQesd*)(void*)nwaVXKQcUfDTKytqcHGUNZmqVStC.iuqCPbtaBnskMuuqYRKjJQYErTEX;
		for (int i = 0; i < nwaVXKQcUfDTKytqcHGUNZmqVStC.OcDSsRtGpwuucxTmSJJxJhEICxap; i++)
		{
			hWVWhsQejgFPmATIkqbBBZrEFzrcb.OrCfOpHoCWSiMFhUDkuSDIBBrQesd orCfOpHoCWSiMFhUDkuSDIBBrQesd = ptr[i];
			string text = ((orCfOpHoCWSiMFhUDkuSDIBBrQesd.WxIRhbqRJbjCnFuyyVhhwNghsOvn == IntPtr.Zero) ? "" : ((Guid*)(void*)orCfOpHoCWSiMFhUDkuSDIBBrQesd.WxIRhbqRJbjCnFuyyVhhwNghsOvn)->ToString());
			Console.WriteLine("{5,32};{0,38};{1,8},{2:X8};{3:X8} ({4})", text, orCfOpHoCWSiMFhUDkuSDIBBrQesd.RXyqsxHXtHIuRMSVFkFpTjKnfzwK, orCfOpHoCWSiMFhUDkuSDIBBrQesd.XoALhRBPKxeQhndBtQROzOItXfee, (int)orCfOpHoCWSiMFhUDkuSDIBBrQesd.cKsiIrEXerLSRcMnnONPKBBJcflU, orCfOpHoCWSiMFhUDkuSDIBBrQesd.cKsiIrEXerLSRcMnnONPKBBJcflU, P_0.msHlyRwUUJbfBhpzcEZccYNuVKIUA[i].BpxWAnijsxTTRWWQZjjdqgtJhgYA);
		}
		Console.WriteLine();
	}
}
