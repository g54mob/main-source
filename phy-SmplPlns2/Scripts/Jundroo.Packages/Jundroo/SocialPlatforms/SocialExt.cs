...
using Jundroo.SocialPlatforms.Gog;
...
namespace Jundroo.SocialPlatforms
{
	public static class SocialExt
	{
		...
		public static void Initialize(AchievementDatabase achievementDatabase, bool forceInitializationInEditor = false)
		{
			ISocialPlatformExt socialPlatformExt = new DummyPlatform();
			if (TryInitializeSteam(forceInitializationInEditor))
			{
			...
			if (Device.IsUnityEditor && !(socialPlatformExt is DummyPlatform))
			{
				Debug.Log("Initialized " + socialPlatformExt.PlatformName);
			}
		}
		...
		private static bool TryInitializeSteam(bool forceInitializationInEditor = false)
		{
			if (Device.IsUnityEditor && !forceInitializationInEditor)
			{
				return false;
			}
			try
