	...
	public class DeveloperConsole : MonoBehaviour
	{
		private struct AutoCompleteItem
		{
			...
			public AutoCompleteItem(RegisteredCommandInfo command)
			{
				...
				DisplayText = string.Format("{0}{1} {2}", (command.ReturnType == typeof(void)) ? string.Empty : (command.ReturnType.Name + " "), command.CommandText, string.Join(" ", command.Parameters.Select((ParameterInfo x) => "[" + x.ParameterType.Name + "]").ToArray()));
			}
		}
		...
		public int MaxLogMessages = 1000;

		public int MaxLogMessagesCleanupCount = 100;

		public int MaxRecentCommands = 20;
		...
		private bool _neverOpened = true;
		...
		public void OpenConsole()
		{
			...
			if (_neverOpened)
			{
				...
				{
					Log("Some features of the developer console are not available in this demo version of the game.", string.Empty, LogType.Log);
				}
			}
		}
