...
{
	[AddComponentMenu("")]
	public class OODDCDOOOC : MonoBehaviour
	{
		public static void ODQDOCOCQD(ERCrossings scr, ref List<Vector3> meshVecs, ref List<Vector2> meshUVs, ref List<List<int>> triList, List<Material> materialList, List<Material> connectionMaterialList)
		{
			...
			meshUVs.Add(new Vector2(0.5f, OOCOCCQDOD.GetCenterUVY(scr, scr.frontRoadUVTiling, scr.startConnectionV3, 0)));
			float num = 0f;
			...
			if (scr.frontRoadUVTiling == scr.backRoadUVTiling && scr.frontRoadUVTiling != num)
			{
				...
				{
					meshUVs.Add(new Vector2(0.5f, OOCOCCQDOD.GetCenterUVY(scr, scr.leftRoadUVTiling, scr.leftConnectionV3, 2)));
				}
				else
				{
					meshUVs.Add(new Vector2(0.5f, OOCOCCQDOD.GetCenterUVY(scr, scr.rightRoadUVTiling, scr.rightConnectionV3, 3)));
				}
			}
			else if (scr.frontRoadUVTiling != scr.backRoadUVTiling)
			{
				...
				meshVecs.Add(Vector3.zero);
				meshUVs.Add(new Vector2(0.5f, OOCOCCQDOD.GetCenterUVY(scr, scr.backRoadUVTiling, scr.endConnectionV3, 1)));
			...
			OCDDCCCCOC(scr, scr.startConnectionV3, scr.startConnectionUV, 0, 0, ref meshVecs, ref meshUVs, ref triList, ref scr.startConnectionTris, 1, OQQOCDQCQD.OCQQDQCDOD(materialList, connectionMaterialList[0]), 0, ref scr.frontLeftRoadInts, ref scr.frontRightRoadInts, centerPointIndex);
			if (scr.tCrossing)
			{
				OCQDCOOCOO.ODDOQQDQOC(scr, 0, 0, 0, scr.startConnectionV3);
			}
			OCDDCCCCOC(scr, scr.endConnectionV3, scr.endConnectionUV, 0, 0, ref meshVecs, ref meshUVs, ref triList, ref scr.endConnectionTris, 1, OQQOCDQCQD.OCQQDQCDOD(materialList, connectionMaterialList[1]), 1, ref scr.backLeftRoadInts, ref scr.backRightRoadInts, centerPointIndex2);
			if (scr.tCrossing)
			{
				OCQDCOOCOO.ODDOQQDQOC(scr, 1, 0, 0, scr.endConnectionV3);
			}
			if (scr.tCrossingLeftRight == 0)
			{
				OCDDCCCCOC(scr, scr.leftConnectionV3, scr.leftConnectionUV, 0, 0, ref meshVecs, ref meshUVs, ref triList, ref scr.leftConnectionTris, 1, OQQOCDQCQD.OCQQDQCDOD(materialList, connectionMaterialList[2]), 2, ref scr.leftLeftRoadInts, ref scr.leftRightRoadInts, centerPointIndex3);
				if (scr.tCrossing)
				{
					OCQDCOOCOO.ODDOQQDQOC(scr, 2, 0, 0, scr.leftConnectionV3);
				}
			}
			if (scr.tCrossingLeftRight == 1)
			{
				OCDDCCCCOC(scr, scr.rightConnectionV3, scr.rightConnectionUV, 0, 0, ref meshVecs, ref meshUVs, ref triList, ref scr.rightConnectionTris, 1, OQQOCDQCQD.OCQQDQCDOD(materialList, connectionMaterialList[3]), 2, ref scr.rightLeftRoadInts, ref scr.rightRightRoadInts, centerPointIndex3);
				if (scr.tCrossing)
				{
					OCQDCOOCOO.ODDOQQDQOC(scr, 3, 0, 0, scr.rightConnectionV3);
				}
			}
		}

		public static void OCDDCCCCOC(ERCrossings scr, List<List<Vector3>> vecArray, List<List<Vector2>> uvArray, int frontLeft, int topBottom, ref List<Vector3> meshVecs, ref List<Vector2> meshUVs, ref List<List<int>> triList, ref List<List<int>> intArray, int startRow, int triArrayIndex, int connection, ref List<int> borderIntsLeft, ref List<int> borderIntsRight, int centerPointIndex)
		{
			...
			triList[triArrayIndex].AddRange(OODQCOCCQQ(intArray[0], intArray[1], startRow, 0));
			triList[triArrayIndex].AddRange(OODQCOCCQQ(intArray[3], intArray[4], startRow, 1));
			if (intArray[1].Count == intArray[2].Count)
			{
				triList[triArrayIndex].AddRange(OODQCOCCQQ(intArray[1], intArray[2], startRow, 0));
			}
			else
			{
			...
			{
				triList[triArrayIndex].AddRange(OODQCOCCQQ(intArray[2], intArray[3], startRow, 1));
			}
			else
			{
		...
		public static List<int> OODQCOCCQQ(List<int> col1, List<int> col2, int startRow, int type)
		{
			List<int> list = new List<int>();
			if (type == 0)
			{
