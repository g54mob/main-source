	...
	internal sealed class UserSvd : Svd
	{
		public static UserSvd Create(Matrix<double> matrix, bool computeVectors)
		{
			...
			for (int i = 0; i < num4; i++)
			{
				...
				if (i < num2)
				{
					double value = Dnrm2Column(matrix2, matrix2.RowCount, i, i);
					vector[i] = value;
					if (vector[i] != 0.0)
					{
				...
				for (int j = num5; j < matrix2.ColumnCount; j++)
				{
					...
					{
						double num6 = (0.0 - Ddot(matrix2, matrix2.RowCount, i, j, i)) / matrix2.At(i, i);
				...
				double num7 = Dnrm2Vector(array, num5);
				array[i] = num7;
				if (array[i] != 0.0)
				{
					if (array[num5] != 0.0)
				...
				if (num5 < matrix2.RowCount && array[i] != 0.0)
				{
					...
					for (int j = num5; j < matrix2.ColumnCount; j++)
					{
						double num8 = (0.0 - array[j]) / array[num5];
			...
			if (computeVectors)
			{
				...
				for (int i = num2 - 1; i >= 0; i--)
				{
					if (vector[i] != 0.0)
					{
						...
						{
							double num6 = (0.0 - Ddot(matrix3, matrix2.RowCount, i, j, i)) / matrix3.At(i, i);
			...
			if (computeVectors)
			{
				for (int i = matrix2.ColumnCount - 1; i >= 0; i--)
				{
					...
					if (i < num3 && array[i] != 0.0)
					{
						for (int j = num5; j < matrix2.ColumnCount; j++)
						{
							double num6 = (0.0 - Ddot(matrix4, matrix2.ColumnCount, i, j, num5)) / matrix4.At(num5, i);
			...
			for (int l = 0; l < num9; l++)
			{
				if (vector[l] != 0.0)
				{
					double num6 = vector[l];
					double num14 = vector[l] / num6;
					vector[l] = num6;
					if (l < num9 - 1)
					{
						array[l] /= num14;
			...
			while (num9 > 0)
			{
				...
				for (i = num9 - 2; i >= 0; i--)
				{
					double num17 = Math.Abs(vector[i]) + Math.Abs(vector[i + 1]);
					if ((num17 + Math.Abs(array[i])).AlmostEqualRelative(num17, 15))
					{
						array[i] = 0.0;
						break;
					}
				...
				else
				{
					...
					for (num19 = num9 - 1; num19 > i; num19--)
					{
						...
						if ((num17 + Math.Abs(vector[num19])).AlmostEqualRelative(num17, 15))
						{
							vector[num19] = 0.0;
							break;
						}
				...
				case 3:
				{
					...
					double num20 = vector[num9 - 1] / val;
					double num21 = vector[num9 - 2] / val;
					double num22 = array[num9 - 2] / val;
					double num23 = vector[i] / val;
					double num24 = array[i] / val;
					double num25 = ((num21 + num20) * (num21 - num20) + num22 * num22) / 2.0;
					double num26 = num20 * num22 * (num20 * num22);
					double num27 = 0.0;
					if (num25 != 0.0 || num26 != 0.0)
					{
						num27 = Math.Sqrt(num25 * num25 + num26);
						if (num25 < 0.0)
						{
							num27 = 0.0 - num27;
						}
					...
					double da = (num23 + num20) * (num23 - num20) + num27;
					double db = num23 * num24;
					for (int num28 = i; num28 < num9 - 1; num28++)
					{
						...
						vector[num28 + 1] = (0.0 - s) * array[num28] + c * vector[num28 + 1];
						db = s * array[num28 + 1];
						array[num28 + 1] = c * array[num28 + 1];
