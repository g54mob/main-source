...
namespace HorizonBasedAmbientOcclusion.Universal
{
	public class HBAORendererFeature : ScriptableRendererFeature
	{
		private class HBAORenderPass : ScriptableRenderPass
		{
			...
			{
				if (material == null || !hbao.IsActive())
				{
					return;
				}
				...
				CameraHistoryBuffers currentCameraHistoryBuffersRG = GetCurrentCameraHistoryBuffersRG(universalCameraData);
				currentCameraHistoryBuffersRG?.historyRTSystem.SwapAndSetReferenceSize(aoDesc.width, aoDesc.height);
				PassData passData;
				using IUnsafeRenderGraphBuilder unsafeRenderGraphBuilder = renderGraph.AddUnsafePass<PassData>("HBAO Pass", out passData, m_ProfilingSampler, "C:\\dev\\SimplePlanes2\\SimplePlanesNext\\Assets\\Packages\\ThirdParty\\Horizon Based Ambient Occlusion\\SRP\\URP\\Runtime\\HBAORendererFeature.cs", 641);
				unsafeRenderGraphBuilder.AllowGlobalStateModification(value: true);
				unsafeRenderGraphBuilder.AllowPassCulling(value: false);
				TextureHandle cameraDepthTexture = ((hbao.renderingPath.value == HBAO.RenderingPath.Deferred) ? universalResourceData.activeDepthTexture : universalResourceData.cameraDepthTexture);
				TextureHandle cameraNormalsTexture = universalResourceData.cameraNormalsTexture;
				TextureHandle motionVectorColor = universalResourceData.motionVectorColor;
				...
				bool num = hbao.perPixelNormals.value == HBAO.PerPixelNormals.Camera;
				bool value = hbao.temporalFilterEnabled.value;
				bool flag = hbao.mode.value == HBAO.Mode.LitAO;
				bool flag2 = hbao.blurType.value != HBAO.BlurType.None;
				bool value2 = hbao.temporalFilterEnabled.value;
				bool value3 = hbao.colorBleedingEnabled.value;
				bool flag3 = hbao.debugMode.value != HBAO.DebugMode.Disabled;
				...
				passData.AOTexture = UniversalRenderer.CreateRenderGraphTexture(renderGraph, aoDesc, "_HBAO_AOTexture0", clear: false, FilterMode.Bilinear);
				passData.TempTexture = (flag2 ? UniversalRenderer.CreateRenderGraphTexture(renderGraph, aoDesc, "_HBAO_AOTexture1", clear: false, FilterMode.Bilinear) : TextureHandle.nullHandle);
				passData.DestinationTexture = ((flag && !flag3) ? UniversalRenderer.CreateRenderGraphTexture(renderGraph, ssaoDesc, "_ScreenSpaceOcclusionTexture", clear: false, FilterMode.Bilinear) : UniversalRenderer.CreateRenderGraphTexture(renderGraph, cameraTargetDescriptor, "_ScreenSpaceOcclusionTexture", clear: false, FilterMode.Bilinear));
				...
				if (value)
				{
					unsafeRenderGraphBuilder.UseTexture(in motionVectorColor);
				}
				unsafeRenderGraphBuilder.UseTexture(passData.SourceTexture, (flag && !flag3) ? AccessFlags.Read : AccessFlags.ReadWrite);
				unsafeRenderGraphBuilder.UseTexture(passData.AOTexture, AccessFlags.ReadWrite);
				if (passData.TempTexture.IsValid())
				{
					unsafeRenderGraphBuilder.UseTexture(passData.TempTexture, AccessFlags.ReadWrite);
				}
				unsafeRenderGraphBuilder.UseTexture(passData.DestinationTexture, (flag && !flag3) ? AccessFlags.Write : AccessFlags.ReadWrite);
				if (flag && !flag3)
				{
					unsafeRenderGraphBuilder.SetGlobalTextureAfterPass(passData.DestinationTexture, ShaderProperties.screenSpaceOcclusionTexture);
				}
			...
			public override void FrameCleanup(CommandBuffer cmd)
			{
				if (hbao.mode.value == HBAO.Mode.LitAO)
				{
					...
					cmd.SetKeyword(in ShaderProperties.screenSpaceOcclusionKeyword, value: false);
				}
			...
			{
				BlitFullscreenTriangle(cmd, source, ShaderProperties.inputTex, material, fullscreenTriangle, 8);
			}

			private void AO(CommandBuffer cmd)
			{
				BlitFullscreenTriangleWithClear(cmd, (hbao.mode.value == HBAO.Mode.LitAO) ? source : ((RenderTargetIdentifier)ShaderProperties.inputTex), ShaderProperties.hbaoTex, material, new Color(0f, 0f, 0f, 1f), fullscreenTriangle);
			}
			...
			private void DeinterleavedAO(CommandBuffer cmd)
			{
				for (int i = 0; i < 4; i++)
				{
					...
					BlitFullscreenTriangle(cmd, BuiltinRenderTextureType.CameraTarget, m_RtsDepth, material, fullscreenTriangle, 2);
					BlitFullscreenTriangle(cmd, BuiltinRenderTextureType.CameraTarget, m_RtsNormals, material, fullscreenTriangle, 3);
				}
				for (int k = 0; k < 16; k++)
				{
					...
					BlitFullscreenTriangleWithClear(cmd, (hbao.mode.value == HBAO.Mode.LitAO) ? source : ((RenderTargetIdentifier)ShaderProperties.inputTex), ShaderProperties.aoSliceTex[k], material, new Color(0f, 0f, 0f, 1f), fullscreenTriangle, 1);
				...
				for (int l = 0; l < 16; l++)
				{
					...
					BlitFullscreenTriangle(cmd, ShaderProperties.aoSliceTex[l], ShaderProperties.tempTex, material, fullscreenTriangle, 4);
					cmd.ReleaseTemporaryRT(ShaderProperties.aoSliceTex[l]);
				}
				BlitFullscreenTriangle(cmd, ShaderProperties.tempTex, ShaderProperties.hbaoTex, material, fullscreenTriangle, 5);
			...
			private void Blur(CommandBuffer cmd)
			{
				if (hbao.blurType.value != HBAO.BlurType.None)
				{
					...
					BlitFullscreenTriangle(cmd, ShaderProperties.hbaoTex, ShaderProperties.tempTex, material, fullscreenTriangle, 6);
					cmd.SetGlobalVector(ShaderProperties.blurDeltaUV, new Vector2(0f, 1f / num2));
					BlitFullscreenTriangle(cmd, ShaderProperties.tempTex, ShaderProperties.hbaoTex, material, fullscreenTriangle, 6);
					cmd.ReleaseTemporaryRT(ShaderProperties.tempTex);
				}
			}
			...
			private void TemporalFilter(CommandBuffer cmd, CameraHistoryBuffers buffers)
			{
				if (hbao.temporalFilterEnabled.value && !renderingInSceneView && buffers != null)
				{
					...
					if (hbao.colorBleedingEnabled.value)
					{
						...
						BlitFullscreenTriangle(cmd, frameRT3, destinations, viewportRect, material, fullscreenTriangle, 7);
					...
					else
					{
						...
						BlitFullscreenTriangle(cmd, frameRT6, frameRT5, viewportRect, material, fullscreenTriangle, 7);
			...
			private void UpdateMaterialPropertiesRG(UniversalCameraData cameraData)
			{
				int width = cameraData.cameraTargetDescriptor.width;
				int height = cameraData.cameraTargetDescriptor.height;
				int num = ((!XRSettings.enabled || XRSettings.stereoRenderingMode != XRSettings.StereoRenderingMode.SinglePassInstanced || cameraData.cameraType == CameraType.SceneView) ? 1 : 2);
				for (int i = 0; i < num; i++)
				{
					Matrix4x4 projectionMatrix = cameraData.GetProjectionMatrix(i);
					float m = projectionMatrix.m00;
		...
		public override void AddRenderPasses(ScriptableRenderer renderer, ref RenderingData renderingData)
		{
			...
			else if (renderingData.cameraData.postProcessEnabled && m_HBAORenderPass != null)
			{
