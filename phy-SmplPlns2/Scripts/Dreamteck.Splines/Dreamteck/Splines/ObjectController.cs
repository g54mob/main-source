...
namespace Dreamteck.Splines
{
	...
	public class ObjectController : SplineUser
	{
		...
		protected override void Build()
		{
			...
			bool flag = _customOffsetRule != null;
			bool flag2 = _customRotationRule != null;
			bool flag3 = _customScaleRule != null;
			bool flag4 = _minScaleMultiplier != _maxScaleMultiplier;
			double num = 0.0;
			for (int i = 0; i < spawned.Length; i++)
			{
				if (spawned[i] == null)
				...
				if (spawned.Length > 1)
				{
					num2 = (_useCustomObjectDistance ? ((float)num) : ((!base.spline.isClosed) ? ((float)i / (float)(spawned.Length - 1)) : ((float)i / (float)spawned.Length)));
				}
				num2 += _evaluateOffset;
				if (num2 > 1f)
				{
					num2 -= 1f;
				}
				...
				{
					base.spline.Evaluate(num2, ref evalResult);
				}
				else
				{
					Evaluate(num2, ref evalResult);
				}
				...
				if (_applyScale)
				{
					...
					else
					{
						...
						if (flag4)
						{
							...
							{
								vector = Vector3.Lerp(new Vector3(_minScaleMultiplier.x, _minScaleMultiplier.y, _minScaleMultiplier.z), new Vector3(_maxScaleMultiplier.x, _maxScaleMultiplier.y, _maxScaleMultiplier.z), (float)scaleRandomizer.NextDouble());
							}
							else
							{
				...
				if (flag)
				{
					...
					vector2 = _customOffsetRule.GetOffset();
				}
				else if (_minOffset != _maxOffset)
				{
					if (_shellOffset)
					{
						float num3 = _maxOffset.x - _minOffset.x;
						float num4 = _maxOffset.y - _minOffset.y;
				...
				{
					num = ((objectPositioning != Positioning.Clip) ? Travel(num, Mathf.Lerp(_minObjectDistance, _maxObjectDistance, (float)distanceRandomizer.NextDouble())) : base.spline.Travel(num, Mathf.Lerp(_minObjectDistance, _maxObjectDistance, (float)distanceRandomizer.NextDouble())));
				}
			}
		}
