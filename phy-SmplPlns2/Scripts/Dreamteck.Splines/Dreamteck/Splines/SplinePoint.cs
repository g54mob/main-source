...
namespace Dreamteck.Splines
{
	...
	public struct SplinePoint
	{
		...
		private static void GetInterpolatedTangents(SplinePoint a, SplinePoint b, float t, ref SplinePoint target)
		{
			...
			target.tangent = vector4;
			target.tangent2 = vector5;
		}
		...
		{
			SplinePoint splinePoint = obj;
			if (position != splinePoint.position)
			{
				return false;
			}
			if (tangent != splinePoint.tangent)
			{
				return false;
			}
			if (tangent2 != splinePoint.tangent2)
			{
				return false;
			}
			if (normal != splinePoint.normal)
			{
				return false;
			}
			if (_type != splinePoint._type)
			{
				return false;
			}
			if (size != splinePoint.size)
			{
				return false;
			}
			if (color != splinePoint.color)
			{
				return false;
			}
			return true;
		...
		public void Flatten(LinearAlgebraUtility.Axis axis, float flatValue = 0f)
		{
			...
			switch (axis)
			{
			case LinearAlgebraUtility.Axis.X:
				normal = Vector3.right;
				break;
			case LinearAlgebraUtility.Axis.Y:
				normal = Vector3.up;
				break;
			case LinearAlgebraUtility.Axis.Z:
		...
		{
			return (((((17 * (17 * 23 + _type.GetHashCode()) * 23 + position.GetHashCode()) * 23 + normal.GetHashCode()) * 23 + tangent.GetHashCode()) * 23 + tangent2.GetHashCode()) * 23 + color.GetHashCode()) * 23 + size.GetHashCode();
		}
	}
}
