	...
	public class MB3_MeshCombinerSingle : MB3_MeshCombiner
	{
		public class MB3_MeshCombinerSimpleBones
		{
			...
			public bool CollectBonesToAddForDGO(MB_DynamicGameObject dgo, Renderer r, bool noExtraBonesForMeshRenderers, MeshChannelsCache meshChannelCache)
			{
				...
				BoneWeight[] array2 = (dgo._tmpSMR_CachedBoneWeights = meshChannelCache.GetBoneWeights(r, dgo.numVerts, dgo.isSkinnedMeshWithBones));
				Transform[] array3 = (dgo._tmpSMR_CachedBones = combiner._getBones(r, dgo.isSkinnedMeshWithBones));
				...
				for (int k = 0; k < array3.Length; k++)
				{
					bool flag3 = false;
					int num = array4[k];
					BoneAndBindpose boneAndBindpose2 = new BoneAndBindpose(array3[num], array[num]);
					int value;
					if (boneAndBindPose2idx.TryGetValue(boneAndBindpose2, out value) && array3[num] == combiner.bones[value] && !boneIdxsToDelete.Contains(value) && array[num] == combiner.bindPoses[value])
					{
						flag3 = true;
					}
				...
				dgo._tmpSMRIndexesOfSourceBonesUsed = array4;
				return flag;
			}
		...
		public class UVAdjuster_Atlas
		{
			...
			public bool MapSharedMaterialsToAtlasRects(Material[] sharedMaterials, bool checkTargetSubmeshIdxsFromPreviousBake, Mesh m, MeshChannelsCache meshChannelsCache, Dictionary<int, MB_Utility.MeshAnalysisResult[]> meshAnalysisResultsCache, OrderedDictionary sourceMats2submeshIdx_map, GameObject go, MB_DynamicGameObject dgoOut)
			{
				...
				{
					object obj = sourceMats2submeshIdx_map[sharedMaterials[i]];
					if (obj == null)
					{
					...
					{
						UnityEngine.Debug.LogError(string.Format("Update failed for object {0}. Material {1} is mapped to a different submesh in the combined mesh than the previous material. This is not supported. Try using AddDelete.", go.name, sharedMaterials[i]));
						return false;
					}
			...
			public void _copyAndAdjustUVsFromMesh(MB2_TextureBakeResults tbr, MB_DynamicGameObject dgo, Mesh mesh, int uvChannel, int vertsIdx, Vector2[] uvsOut, float[] uvsSliceIdx, MeshChannelsCache meshChannelsCache)
			{
				...
				for (int j = 0; j < dgo.targetSubmeshIdxs.Length; j++)
				{
					...
					{
						UnityEngine.Debug.Log(string.Format("Build UV transform for mesh {0} submesh {1} encapsulatingRect {2}", dgo.name, j, dgo.encapsulatingRect[j]));
					}
					bool considerMeshUVs = textureBakeResults.GetConsiderMeshUVs(idxInSrcMats, dgo.sourceSharedMaterials[j]);
					Rect rect = MB3_TextureCombinerMerging.BuildTransformMeshUV2AtlasRect(considerMeshUVs, dgo.uvRects[j], (dgo.obUVRects != null && dgo.obUVRects.Length != 0) ? dgo.obUVRects[j] : new Rect(0f, 0f, 1f, 1f), dgo.sourceMaterialTiling[j], dgo.encapsulatingRect[j]);
				...
				if (LOG_LEVEL >= MB2_LogLevel.trace)
				{
					UnityEngine.Debug.Log(string.Format("_copyAndAdjustUVsFromMesh copied {0} verts", uVChannel.Length));
				}
			}
		...
		private bool _Initialize(int numResultMats)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.trace)
			{
				UnityEngine.Debug.Log(string.Format("_initialize numObjsInCombined={0}", mbDynamicObjectsInCombinedMesh.Count));
			}
			return true;
		}
		...
		private bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
		{
			...
			{
				MB_DynamicGameObject dgoGameObject = null;
				InstanceID2DGO(array[i], out dgoGameObject);
				if (dgoGameObject != null)
				{
					...
					for (int j = 0; j < dgoGameObject.submeshNumTris.Length; j++)
					{
						array2[j] += dgoGameObject.submeshNumTris[j];
					}
				}
			...
			int num5 = 0;
			for (int k = 0; k < _goToAdd.Length; k++)
			{
				if (!instance2Combined_MapContainsKey(_goToAdd[k]) || Array.FindIndex(array, (int o) => o == _goToAdd[k].GetInstanceID()) != -1)
				{
					MB_DynamicGameObject mB_DynamicGameObject = new MB_DynamicGameObject();
					GameObject gameObject = _goToAdd[k];
					Material[] array4 = MB_Utility.GetGOMaterials(gameObject);
					if (LOG_LEVEL >= MB2_LogLevel.trace)
					{
						UnityEngine.Debug.Log(string.Format("Getting {0} shared materials for {1}", array4.Length, gameObject));
					}
					if (array4 == null)
					{
					...
					if (!uVAdjuster_Atlas.MapSharedMaterialsToAtlasRects(array4, false, mesh, meshChannelsCache, dictionary, orderedDictionary, gameObject, mB_DynamicGameObject))
					{
						_goToAdd[k] = null;
						return false;
					}
					...
					mB_DynamicGameObject.name = string.Format("{0} {1}", _goToAdd[k].ToString(), _goToAdd[k].GetInstanceID());
					mB_DynamicGameObject.instanceID = _goToAdd[k].GetInstanceID();
					mB_DynamicGameObject.gameObject = _goToAdd[k];
					...
					for (int num6 = 0; num6 < mB_DynamicGameObject._tmpSubmeshTris.Length; num6++)
					{
						array3[mB_DynamicGameObject.targetSubmeshIdxs[num6]] += mB_DynamicGameObject._tmpSubmeshTris[num6].data.Length;
					}
				...
				else
				{
					if (LOG_LEVEL >= MB2_LogLevel.warn)
					{
						UnityEngine.Debug.LogWarning("Object " + _goToAdd[k].name + " has already been added");
					}
					_goToAdd[k] = null;
				}
			}
			for (int num7 = 0; num7 < _goToAdd.Length; num7++)
			{
				if (_goToAdd[num7] != null && disableRendererInSource)
				{
					...
					if (LOG_LEVEL == MB2_LogLevel.trace)
					{
						UnityEngine.Debug.Log("Disabling renderer on " + _goToAdd[num7].name + " id=" + _goToAdd[num7].GetInstanceID());
					}
				}
			}
			int num8 = verts.Length + num4 - num2;
			int newBonesLength = mB3_MeshCombinerSimpleBones.GetNewBonesLength();
			int[] array5 = new int[num];
			int num9 = 0;
			if (base.settings.doBlendShapes)
			{
				num9 = blendShapes.Length + num5 - num3;
			}
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				UnityEngine.Debug.Log("Verts adding:" + num4 + " deleting:" + num2 + " submeshes:" + array5.Length + " bones:" + newBonesLength + " blendShapes:" + num9);
			}
			for (int num10 = 0; num10 < array5.Length; num10++)
			{
				array5[num10] = submeshTris[num10].data.Length + array3[num10] - array2[num10];
				if (LOG_LEVEL >= MB2_LogLevel.debug)
				{
					MB2_Log.LogDebug("    submesh :" + num10 + " already contains:" + submeshTris[num10].data.Length + " tris to be Added:" + array3[num10] + " tris to be Deleted:" + array2[num10]);
				}
			}
			...
			Vector3[] destinationArray13 = new Vector3[num8];
			...
			{
				destinationArray11 = new float[num8];
			}
			if (base.settings.doUV3)
			{
				destinationArray5 = new Vector2[num8];
			}
			if (base.settings.doUV4)
			{
				destinationArray6 = new Vector2[num8];
			}
			if (base.settings.doUV5)
			{
				destinationArray7 = new Vector2[num8];
			}
			if (base.settings.doUV6)
			{
				destinationArray8 = new Vector2[num8];
			}
			if (base.settings.doUV7)
			{
				destinationArray9 = new Vector2[num8];
			}
			if (base.settings.doUV8)
			{
				destinationArray10 = new Vector2[num8];
			}
			if (doUV2())
			{
			...
			{
				destinationArray12 = new Color[num8];
			}
			if (base.settings.doBlendShapes)
			{
				array6 = new MBBlendShape[num9];
			}
			BoneWeight[] array7 = new BoneWeight[num8];
			...
			{
				array10[num11] = new SerializableIntArray(array5[num11]);
			}
			for (int num12 = 0; num12 < array.Length; num12++)
			{
				MB_DynamicGameObject dgoGameObject2 = null;
				InstanceID2DGO(array[num12], out dgoGameObject2);
			...
			int num15 = 0;
			for (int num16 = 0; num16 < mbDynamicObjectsInCombinedMesh.Count; num16++)
			{
				MB_DynamicGameObject mB_DynamicGameObject2 = mbDynamicObjectsInCombinedMesh[num16];
				if (!mB_DynamicGameObject2._beingDeleted)
				{
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					{
						MB2_Log.LogDebug("Copying obj in combined arrays idx:" + num16, LOG_LEVEL);
					}
					Array.Copy(verts, mB_DynamicGameObject2.vertIdx, destinationArray13, num13, mB_DynamicGameObject2.numVerts);
					if (base.settings.doNorm)
					{
					...
					if (base.settings.doUV && textureBakeResults.resultType == MB2_TextureBakeResults.ResultType.textureArray)
					{
						Array.Copy(uvsSliceIdx, mB_DynamicGameObject2.vertIdx, destinationArray11, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV3)
					{
						Array.Copy(uv3s, mB_DynamicGameObject2.vertIdx, destinationArray5, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV4)
					{
						Array.Copy(uv4s, mB_DynamicGameObject2.vertIdx, destinationArray6, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV5)
					{
						Array.Copy(uv5s, mB_DynamicGameObject2.vertIdx, destinationArray7, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV6)
					{
						Array.Copy(uv6s, mB_DynamicGameObject2.vertIdx, destinationArray8, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV7)
					{
						Array.Copy(uv7s, mB_DynamicGameObject2.vertIdx, destinationArray9, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doUV8)
					{
						Array.Copy(uv8s, mB_DynamicGameObject2.vertIdx, destinationArray10, num13, mB_DynamicGameObject2.numVerts);
					}
					if (doUV2())
					{
						Array.Copy(uv2s, mB_DynamicGameObject2.vertIdx, destinationArray4, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doCol)
					{
						Array.Copy(colors, mB_DynamicGameObject2.vertIdx, destinationArray12, num13, mB_DynamicGameObject2.numVerts);
					}
					if (base.settings.doBlendShapes)
					{
						Array.Copy(blendShapes, mB_DynamicGameObject2.blendShapeIdx, array6, num14, mB_DynamicGameObject2.numBlendShapes);
					}
					if (base.settings.renderType == MB_RenderType.skinnedMeshRenderer)
					{
						Array.Copy(boneWeights, mB_DynamicGameObject2.vertIdx, array7, num13, mB_DynamicGameObject2.numVerts);
					}
					for (int num17 = 0; num17 < num; num17++)
					{
						int[] data = submeshTris[num17].data;
						int num18 = mB_DynamicGameObject2.submeshTriIdxs[num17];
						int num19 = mB_DynamicGameObject2.submeshNumTris[num17];
						if (LOG_LEVEL >= MB2_LogLevel.debug)
						{
							MB2_Log.LogDebug("    Adjusting submesh triangles submesh:" + num17 + " startIdx:" + num18 + " num:" + num19 + " nsubmeshTris:" + array10.Length + " targSubmeshTidx:" + array11.Length, LOG_LEVEL);
						}
						for (int num20 = num18; num20 < num18 + num19; num20++)
						{
							data[num20] -= num15;
						}
						Array.Copy(data, num18, array10[num17].data, array11[num17], num19);
					}
					mB_DynamicGameObject2.vertIdx = num13;
					mB_DynamicGameObject2.blendShapeIdx = num14;
					for (int num21 = 0; num21 < array11.Length; num21++)
					{
						mB_DynamicGameObject2.submeshTriIdxs[num21] = array11[num21];
						array11[num21] += mB_DynamicGameObject2.submeshNumTris[num21];
					}
					num14 += mB_DynamicGameObject2.numBlendShapes;
				...
				else
				{
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					{
						MB2_Log.LogDebug("Not copying obj: " + num16, LOG_LEVEL);
					}
			...
			for (int num22 = mbDynamicObjectsInCombinedMesh.Count - 1; num22 >= 0; num22--)
			{
				if (mbDynamicObjectsInCombinedMesh[num22]._beingDeleted)
				{
					instance2Combined_MapRemove(mbDynamicObjectsInCombinedMesh[num22].gameObject);
					objectsInCombinedMesh.RemoveAt(num22);
					mbDynamicObjectsInCombinedMesh.RemoveAt(num22);
				}
			}
			...
			int num23 = bones.Length - mB3_MeshCombinerSimpleBones.GetNumBonesToDelete();
			bindPoses = array8;
			bones = array9;
			submeshTris = array10;
			int num24 = 0;
			if (base.settings.renderType == MB_RenderType.skinnedMeshRenderer)
			{
				...
				{
					array9[num23 + num24] = item.bone;
					array8[num23 + num24] = item.bindPose;
					num24++;
				}
			}
			for (int num25 = 0; num25 < list.Count; num25++)
			{
				...
				GameObject gameObject2 = _goToAdd[num25];
				int num26 = num13;
				int index = num14;
				...
				float num27 = (matrix4x[2, 3] = 0f);
				num27 = (matrix4x[1, 3] = num27);
				matrix4x[0, 3] = num27;
				...
				if (base.settings.renderType != MB_RenderType.skinnedMeshRenderer)
				{
					for (int num30 = 0; num30 < destinationArray13.Length; num30++)
					{
						int num31 = num26 + num30;
						verts[num26 + num30] = localToWorldMatrix.MultiplyPoint3x4(destinationArray13[num30]);
						if (base.settings.doNorm)
						{
							normals[num31] = matrix4x.MultiplyPoint3x4(array12[num30]).normalized;
						}
						if (base.settings.doTan)
						{
							float w = array13[num30].w;
							tangents[num31] = matrix4x.MultiplyPoint3x4(array13[num30]).normalized;
							tangents[num31].w = w;
						}
					}
				}
				...
				{
					mB3_MeshCombinerSimpleBones.CopyVertsNormsTansToBuffers(mB_DynamicGameObject3, base.settings, num26, array12, array13, destinationArray13, normals, tangents, verts);
				}
				...
				if (base.settings.doUV)
				{
					uVAdjuster_Atlas._copyAndAdjustUVsFromMesh(textureBakeResults, mB_DynamicGameObject3, mesh2, 0, num26, uvs, uvsSliceIdx, meshChannelsCache);
				}
				if (doUV2())
				{
					_copyAndAdjustUV2FromMesh(mB_DynamicGameObject3, mesh2, num26, meshChannelsCache);
				}
				if (base.settings.doUV3)
				{
					destinationArray5 = meshChannelsCache.GetUVChannel(3, mesh2);
					destinationArray5.CopyTo(uv3s, num26);
				}
				if (base.settings.doUV4)
				{
					destinationArray6 = meshChannelsCache.GetUVChannel(4, mesh2);
					destinationArray6.CopyTo(uv4s, num26);
				}
				if (base.settings.doUV5)
				{
					destinationArray7 = meshChannelsCache.GetUVChannel(5, mesh2);
					destinationArray7.CopyTo(uv5s, num26);
				}
				if (base.settings.doUV6)
				{
					destinationArray8 = meshChannelsCache.GetUVChannel(6, mesh2);
					destinationArray8.CopyTo(uv6s, num26);
				}
				if (base.settings.doUV7)
				{
					destinationArray9 = meshChannelsCache.GetUVChannel(7, mesh2);
					destinationArray9.CopyTo(uv7s, num26);
				}
				if (base.settings.doUV8)
				{
					destinationArray10 = meshChannelsCache.GetUVChannel(8, mesh2);
					destinationArray10.CopyTo(uv8s, num26);
				}
				if (base.settings.doCol)
				{
					destinationArray12 = meshChannelsCache.GetColors(mesh2);
					destinationArray12.CopyTo(colors, num26);
				}
				if (base.settings.doBlendShapes)
				{
				...
				if (base.settings.renderType == MB_RenderType.skinnedMeshRenderer)
				{
					...
					MB3_MeshCombinerSimpleBones.AddBonesToNewBonesArrayAndAdjustBWIndexes(this, mB_DynamicGameObject3, renderer2, num26, array9, array7, meshChannelsCache);
				}
				for (int num32 = 0; num32 < array11.Length; num32++)
				{
					mB_DynamicGameObject3.submeshTriIdxs[num32] = array11[num32];
				}
				for (int num33 = 0; num33 < mB_DynamicGameObject3._tmpSubmeshTris.Length; num33++)
				{
					int[] data2 = mB_DynamicGameObject3._tmpSubmeshTris[num33].data;
					for (int num34 = 0; num34 < data2.Length; num34++)
					{
						data2[num34] += num26;
					}
					if (mB_DynamicGameObject3.invertTriangles)
					{
						for (int num35 = 0; num35 < data2.Length; num35 += 3)
						{
							int num36 = data2[num35];
							data2[num35] = data2[num35 + 1];
							data2[num35 + 1] = num36;
						}
					}
					int num37 = mB_DynamicGameObject3.targetSubmeshIdxs[num33];
					data2.CopyTo(submeshTris[num37].data, array11[num37]);
					mB_DynamicGameObject3.submeshNumTris[num37] += data2.Length;
					array11[num37] += data2.Length;
				}
				mB_DynamicGameObject3.vertIdx = num13;
				mB_DynamicGameObject3.blendShapeIdx = num14;
				...
				num13 += destinationArray13.Length;
				if (base.settings.doBlendShapes)
				{
					num14 += array6.Length;
				}
				for (int num38 = 0; num38 < mB_DynamicGameObject3._tmpSubmeshTris.Length; num38++)
				{
					mB_DynamicGameObject3._tmpSubmeshTris[num38] = null;
				}
		...
		{
			Vector2[] uVChannel = meshChannelsCache.GetUVChannel(2, mesh);
			if (base.settings.lightmapOption == MB2_LightmapOptions.preserve_current_lightmapping)
			{
				...
				{
					vector3.x = vector.x * uVChannel[i].x;
					vector3.y = vector.y * uVChannel[i].y;
					uv2s[vertsIdx + i] = vector2 + vector3;
				}
				if (LOG_LEVEL >= MB2_LogLevel.trace)
				{
					UnityEngine.Debug.Log("_copyAndAdjustUV2FromMesh copied and modify for preserve current lightmapping " + uVChannel.Length);
				}
			}
			else
			{
				uVChannel.CopyTo(uv2s, vertsIdx);
				if (LOG_LEVEL >= MB2_LogLevel.trace)
				{
					UnityEngine.Debug.Log("_copyAndAdjustUV2FromMesh copied without modifying " + uVChannel.Length);
				}
			}
		}
		...
		public override void Apply(GenerateUV2Delegate uv2GenerationMethod)
		{
			...
			Apply(true, true, base.settings.doNorm, base.settings.doTan, base.settings.doUV, doUV2(), base.settings.doUV3, base.settings.doUV4, base.settings.doUV5, base.settings.doUV6, base.settings.doUV7, base.settings.doUV8, base.settings.doCol, flag, base.settings.doBlendShapes, uv2GenerationMethod);
		}
		...
		{
			Apply(triangles, vertices, normals, tangents, uvs, uv2, uv3, uv4, false, false, false, false, colors, bones, blendShapesFlag, uv2GenerationMethod);
		}
		...
		public override void Apply(bool triangles, bool vertices, bool normals, bool tangents, bool uvs, bool uv2, bool uv3, bool uv4, bool uv5, bool uv6, bool uv7, bool uv8, bool colors, bool bones = false, bool blendShapesFlag = false, GenerateUV2Delegate uv2GenerationMethod = null)
		{
			...
			if (_mesh != null)
			{
				if (LOG_LEVEL >= MB2_LogLevel.trace)
				{
					UnityEngine.Debug.Log(string.Format("Apply called tri={0} vert={1} norm={2} tan={3} uv={4} col={5} uv3={6} uv4={7} uv2={8} bone={9} blendShape{10} meshID={11}", triangles, vertices, normals, tangents, uvs, colors, uv3, uv4, uv2, bones, blendShapesFlag, _mesh.GetInstanceID()));
				}
				...
				if (triangles && (bool)_textureBakeResults)
				{
					...
					else
					{
						...
						for (int l = 0; l < submeshTrisWithShowHideApplied.Length; l++)
						{
							...
							{
								_mesh.SetTriangles(submeshTrisWithShowHideApplied[l].data, num2);
								num2++;
							}
						}
						_updateMaterialsOnTargetRenderer(submeshTrisWithShowHideApplied, numNonZeroLengthSubmeshTris);
		...
		{
			return _updateGameObjects(gos, recalcBounds, updateVertices, updateNormals, updateTangents, updateUV, updateUV2, updateUV3, updateUV4, false, false, false, false, updateColors, updateSkinningInfo);
		}
		...
		private bool _updateGameObject(GameObject go, bool updateVertices, bool updateNormals, bool updateTangents, bool updateUV, bool updateUV2, bool updateUV3, bool updateUV4, bool updateUV5, bool updateUV6, bool updateUV7, bool updateUV8, bool updateColors, bool updateSkinningInfo, MeshChannelsCache meshChannelCache, Dictionary<int, MB_Utility.MeshAnalysisResult[]> meshAnalysisResultsCache, OrderedDictionary sourceMats2submeshIdx_map, UVAdjuster_Atlas uVAdjuster)
		{
			...
			if (base.settings.doUV && updateUV)
			{
				...
				if (!uVAdjuster.MapSharedMaterialsToAtlasRects(gOMaterials, true, mesh, meshChannelCache, meshAnalysisResultsCache, sourceMats2submeshIdx_map, go, dgo))
				{
					return false;
				}
			...
			Matrix4x4 matrix4x = localToWorldMatrix;
			float num2 = (matrix4x[2, 3] = 0f);
			num2 = (matrix4x[1, 3] = num2);
			matrix4x[0, 3] = num2;
			matrix4x = matrix4x.inverse.transpose;
			if (updateVertices)
			{
			...
			num2 = (localToWorldMatrix[2, 3] = 0f);
		...
		private void _copyUV2unchangedToSeparateRects()
		{
			...
			MB2_TexturePacker mB2_TexturePacker = new MB2_TexturePackerRegular();
			mB2_TexturePacker.atlasMustBePowerOfTwo = false;
			AtlasPackingResult[] rects = mB2_TexturePacker.GetRects(list, 8192, 8192, padding);
			for (int k = 0; k < mbDynamicObjectsInCombinedMesh.Count; k++)
			{
				MB_DynamicGameObject mB_DynamicGameObject = mbDynamicObjectsInCombinedMesh[k];
				float x;
			...
			foreach (string key2 in dictionary.Keys)
			{
				...
				for (int j = 0; j < num2; j++)
				{
					...
					for (int k = 0; k < list.Count; k++)
					{
						...
						MB_DynamicGameObject mB_DynamicGameObject = instance2Combined_MapGet(mBBlendShape3.gameObject);
						int vertIdx = mB_DynamicGameObject.vertIdx;
						...
						if (j == 0)
						{
							...
							string text2 = text;
							text = text2 + mBBlendShape3.gameObject.name + " " + vertIdx + ":" + (vertIdx + mBBlendShapeFrame.vertices.Length) + ", ";
						}
					}
					num4 += list.Count;
