...
namespace DigitalOpus.MB.Core
{
	internal class MB3_TextureCombinerPackerMeshBakerHorizontalVertical : MB3_TextureCombinerPackerMeshBaker
	{
		...
		public override AtlasPackingResult[] CalculateAtlasRectangles(MB3_TextureCombinerPipeline.TexturePipelineData data, bool doMultiAtlas, MB2_LogLevel LOG_LEVEL)
		{
			...
			if (LOG_LEVEL >= MB2_LogLevel.debug)
			{
				Debug.Log(string.Format("Splitting list of distinctMaterialTextures numHorizontalVertical={0} numRegular={1} maxAtlasWidth={2} maxAtlasHeight={3}", list.Count, list2.Count, data._maxAtlasWidth, data._maxAtlasHeight));
			}
			...
			if (list.Count > 0)
			{
				...
				{
					Debug.Log(string.Format("Packed {0} textures with edgeToEdge tiling into an atlas of size {1} by {2} usedW {3} usedH {4}", list.Count, array[0].atlasX, array[0].atlasY, array[0].usedW, array[0].usedH));
				}
			}
			else
			{
			...
			if (list2.Count > 0)
			{
				...
				int atlasRegularMaxWidth;
				int atlasRegularMaxHeight;
				...
				array2 = mB2_TexturePacker.GetRects(list5, list6, atlasRegularMaxWidth, atlasRegularMaxHeight, false);
				if (LOG_LEVEL >= MB2_LogLevel.debug)
				{
					Debug.Log(string.Format("Packed {0} textures without edgeToEdge tiling into an atlas of size {1} by {2} usedW {3} usedH {4}", list2.Count, array2[0].atlasX, array2[0].atlasY, array2[0].usedW, array2[0].usedH));
				}
			}
			else
			{
			...
			if (array.Length > 0 && array2.Length > 0)
			{
				atlasPackingResult = MergeAtlasPackingResultStackBonA(array[0], array2[0], data._maxAtlasWidth, data._maxAtlasHeight, true, pipeline);
			}
