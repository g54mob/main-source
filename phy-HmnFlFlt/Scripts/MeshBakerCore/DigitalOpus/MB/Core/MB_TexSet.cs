...
namespace DigitalOpus.MB.Core
{
	public class MB_TexSet
	{
		...
		private class PipelineVariationAllTexturesUseSameMatTiling : PipelineVariation
		{
			...
			public void SetTilingTreatmentAndAdjustEncapsulatingSamplingRect(MB_TextureTilingTreatment newTilingTreatment)
			{
				if (texSet.tilingTreatment == MB_TextureTilingTreatment.edgeToEdgeX)
				{
					...
					foreach (MeshBakerMaterialTexture meshBakerMaterialTexture in ts)
					{
						DRect encapsulatingSamplingRect = meshBakerMaterialTexture.GetEncapsulatingSamplingRect();
						encapsulatingSamplingRect.width = 1.0;
						meshBakerMaterialTexture.SetEncapsulatingSamplingRect(texSet, encapsulatingSamplingRect);
					}
				}
				else if (texSet.tilingTreatment == MB_TextureTilingTreatment.edgeToEdgeY)
				{
					MeshBakerMaterialTexture[] ts2 = texSet.ts;
					foreach (MeshBakerMaterialTexture meshBakerMaterialTexture2 in ts2)
					{
						...
						meshBakerMaterialTexture2.SetEncapsulatingSamplingRect(texSet, encapsulatingSamplingRect2);
					}
				}
				else if (texSet.tilingTreatment == MB_TextureTilingTreatment.edgeToEdgeXY)
				{
					MeshBakerMaterialTexture[] ts3 = texSet.ts;
					foreach (MeshBakerMaterialTexture meshBakerMaterialTexture3 in ts3)
					{
						...
						meshBakerMaterialTexture3.SetEncapsulatingSamplingRect(texSet, encapsulatingSamplingRect3);
					}
				}
			}
		...
		{
			MeshBakerMaterialTexture meshBakerMaterialTexture = ts[propIdx];
