...
namespace DigitalOpus.MB.Core
{
	...
	public class MB3_MultiMeshCombiner : MB3_MeshCombiner
	{
		...
		{
			Apply(triangles, vertices, normals, tangents, uvs, uv2, uv3, uv4, false, false, false, false, colors, bones, blendShapeFlag, uv2GenerationMethod);
		}
		...
		{
			return UpdateGameObjects(gos, recalcBounds, updateVertices, updateNormals, updateTangents, updateUV, updateUV2, updateUV3, updateUV4, false, false, false, false, updateColors, updateSkinningInfo);
		}
		...
		public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource = true)
		{
			...
			{
				MB2_Log.LogDebug("MB2_MultiMeshCombiner.AddDeleteGameObjects numCombinedMeshes: " + meshCombiners.Count + " added:" + gos.Length + " deleted:" + deleteGOinstanceIDs.Length + " disableRendererInSource:" + disableRendererInSource + " maxVertsPerCombined:" + _maxVertsInMesh);
			}
		...
		public override Dictionary<MBBlendShapeKey, MBBlendShapeValue> BuildSourceBlendShapeToCombinedIndexMap()
		{
			...
			for (int i = 0; i < meshCombiners.Count; i++)
			{
				...
				MB_BlendShape2CombinedMap component = meshCombiners[i].combinedMesh.targetRenderer.GetComponent<MB_BlendShape2CombinedMap>();
				if (component == null)
				{
					continue;
				}
				foreach (KeyValuePair<MBBlendShapeKey, MBBlendShapeValue> item in component.srcToCombinedMap.GenerateMapFromSerializedData())
				{
