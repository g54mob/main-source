		...
		public void MergeOverlappingDistinctMaterialTexturesAndCalcMaterialSubrects(List<MB_TexSet> distinctMaterialTextures)
		{
			...
			for (int m = 0; m < distinctMaterialTextures.Count; m++)
			{
				...
				for (int n = m + 1; n < distinctMaterialTextures.Count; n++)
				{
					...
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					{
						Debug.Log(string.Format("Considered merging {0} and {1} but there was not enough overlap. It is more efficient to bake these to separate rectangles.", mB_TexSet4.GetDescription() + uvRect, mB_TexSet3.GetDescription() + uvRect2));
					}
				}
			}
		...
		public void MergeDistinctMaterialTexturesThatWouldExceedMaxAtlasSizeAndCalcMaterialSubrects(List<MB_TexSet> distinctMaterialTextures, int maxAtlasSize)
		{
			...
			for (int i = 0; i < distinctMaterialTextures.Count; i++)
			{
				...
				for (int j = i + 1; j < distinctMaterialTextures.Count; j++)
				{
					...
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					{
						Debug.Log(string.Format("Considered merging {0} and {1} but there was not enough overlap. It is more efficient to bake these to separate rectangles.", mB_TexSet2.GetDescription() + uvRect, mB_TexSet.GetDescription() + uvRect2));
					}
				}
			}
