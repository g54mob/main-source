	...
	public class MB_TextureArrays
	{
		...
		internal static bool ConvertTexturesToReadableFormat(TexturePropertyData texturePropertyData, MB_AtlasesAndRects[] resultAtlasesAndRectSlices, bool[] hasTexForProperty, List<ShaderTextureProperty> textureShaderProperties, MB3_TextureCombiner combiner, MB2_LogLevel logLevel, List<Texture2D> createdTemporaryTextureAssets, MB2_EditorMethodsInterface textureEditorMethods)
		{
			for (int i = 0; i < hasTexForProperty.Length; i++)
			{
				...
				if (!textureEditorMethods.TextureImporterFormatExistsForTextureFormat(textureFormat))
				{
				...
				for (int j = 0; j < num; j++)
				{
					...
					if (texture2D != null)
					{
						...
						bool flag = textureEditorMethods.IsAnAsset(texture2D);
						if (logLevel >= MB2_LogLevel.trace)
						{
							Debug.Log(string.Concat("Considering format of texture: ", texture2D, " format:", texture2D.format));
						}
						if (texture2D.width != num2 || texture2D.height != num3 || (!flag && texture2D.format != textureFormat))
						{
		...
		public static IEnumerator _CreateAtlasesCoroutineSingleResultMaterial(int resMatIdx, MB_TextureArrayResultMaterial bakedMatsAndSlicesResMat, MB_MultiMaterialTexArray resMatConfig, List<GameObject> objsToMesh, MB3_TextureCombiner combiner, MB_TextureArrayFormatSet[] textureArrayOutputFormats, MB_MultiMaterialTexArray[] resultMaterialsTexArray, List<ShaderTextureProperty> customShaderProperties, ProgressUpdateDelegate progressInfo, MB3_TextureCombiner.CreateAtlasesCoroutineResult coroutineResult, bool saveAtlasesAsAssets = false, MB2_EditorMethodsInterface editorMethods = null, float maxTimePerFrame = 0.01f)
		{
			...
			bool[] hasTexForProperty = DetermineWhichPropertiesHaveTextures(bakedMatsAndSlicesResMat.slices);
			...
			try
			{
				...
				{
					if (hasTexForProperty[l])
					{
						dictionary[texPropertyNames[l].name] = new MB_TexArrayForProperty(texPropertyNames[l].name, new MB_TextureArrayReference[textureArrayOutputFormats.Length]);
					}
				}
				...
				mB3_TextureCombinerNonTextureProperties.AdjustNonTextureProperties(resMatConfig.combinedMaterial, texPropertyNames, editorMethods);
				for (int m = 0; m < textureArrayOutputFormats.Length; m++)
				{
					...
					FindBestSizeAndMipCountAndFormatForTextureArrays(texPropertyNames, combiner.maxAtlasSize, mB_TextureArrayFormatSet, bakedMatsAndSlicesResMat.slices, texturePropertyData);
					for (int n = 0; n < hasTexForProperty.Length; n++)
					{
						if (!hasTexForProperty[n])
						{
							continue;
						}
						...
						for (int num4 = 0; num4 < num; num4++)
						{
							if (bakedMatsAndSlicesResMat.slices[num4].atlases[n] == null)
							{
								...
								Color colorForTemporaryTexture = mB3_TextureCombinerNonTextureProperties.GetColorForTemporaryTexture(resMatConfig.slices[num4].sourceMaterials[0].sourceMaterial, texPropertyNames[n]);
								MB_Utility.setSolidColor(texture2D2, colorForTemporaryTexture);
								bakedMatsAndSlicesResMat.slices[num4].atlases[n] = editorMethods.CreateTemporaryAssetCopy(texPropertyNames[n], texture2D2, num2, num3, format, LOG_LEVEL);
								temporaryTextureAssets.Add(bakedMatsAndSlicesResMat.slices[num4].atlases[n]);
					...
					if (!ConvertTexturesToReadableFormat(texturePropertyData, bakedMatsAndSlicesResMat.slices, hasTexForProperty, texPropertyNames, combiner, LOG_LEVEL, temporaryTextureAssets, editorMethods))
					{
						continue;
					}
					if (LOG_LEVEL >= MB2_LogLevel.debug)
					...
					Texture2DArray[] array = CreateTextureArraysForResultMaterial(texturePropertyData, texPropertyNames, bakedMatsAndSlicesResMat.slices, hasTexForProperty, combiner, LOG_LEVEL);
					for (int num5 = 0; num5 < array.Length; num5++)
					{
						if (hasTexForProperty[num5])
						{
							MB_TextureArrayReference mB_TextureArrayReference = new MB_TextureArrayReference(mB_TextureArrayFormatSet.name, array[num5]);
							dictionary[texPropertyNames[num5].name].formats[m] = mB_TextureArrayReference;
							if (saveAtlasesAsAssets)
							{
								editorMethods.SaveTextureArrayToAssetDatabase(array[num5], mB_TextureArrayFormatSet.GetFormatForProperty(texPropertyNames[num5].name), bakedMatsAndSlicesResMat.slices[0].texPropertyNames[num5], num5, resMatConfig.combinedMaterial);
							}
						}
					}
				}
			...
			finally
			{
				...
				for (int num6 = 0; num6 < temporaryTextureAssets.Count; num6++)
				{
					editorMethods.DestroyAsset(temporaryTextureAssets[num6]);
				}
