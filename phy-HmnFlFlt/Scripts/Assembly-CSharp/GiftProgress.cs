...
public class GiftProgress : MonoBehaviour, INetBehavior
{
	...
	private IEnumerator Start()
	{
		...
		{
			if (GiftService.status != null)
			{
				GiftService_statusUpdated(GiftService.status);
			}
			GiftService.instance.RefreshStatus(delegate
			{
				GiftService_statusUpdated(GiftService.status);
				GiftService.statusUpdated += GiftService_statusUpdated;
				StartCoroutine(GiftServiceRefreshLoop());
			});
		}
		if (gunsUnlocked)
		{
			NetChat.RegisterCommand(true, false, "bang", LobbyGuns, ScriptLocalization.Get("XTRA/gunsHelp"));
			lobbyGuns = PlayerPrefs.GetInt("lobbyGuns", 1) > 0;
			NetChat.serverCommands.OnHelp("bang");
		}
	}

	private IEnumerator GiftServiceRefreshLoop()
	{
		while (true)
		{
	...
	{
		NetChat.UnRegisterCommand(true, false, "bang", LobbyGuns, ScriptLocalization.Get("XTRA/gunsHelp"));
	}
	...
	private void SetValue(uint newTotal, uint newGoal, uint newLastGoal, string newPrize)
	{
		...
		if (newTotal != total || newGoal != goal || newLastGoal != lastGoal)
		{
			...
			float num2 = ((goal == 0) ? 0f : Mathf.InverseLerp(lastGoal, goal, total));
	...
	private void Sync()
	{
		...
		materials[1].mainTextureOffset = new Vector2(materials[1].mainTextureOffset.y, Mathf.Lerp(0.985f, 0.525f, currentProgress));
		for (int i = 0; i < 6; i++)
		{
	...
	{
		uint num = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
		uint num2 = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
		uint num3 = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
	...
	{
		uint x = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
		uint x2 = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
		uint x3 = ((state0 != null) ? state0.ReadUInt32(24) : 0u);
