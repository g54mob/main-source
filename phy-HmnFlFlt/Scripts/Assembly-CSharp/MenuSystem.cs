...
public class MenuSystem : MonoBehaviour
{
	...
	public bool parentalControlButton = true;
	...
	public static bool CanInvoke
	{
		get
		{
			return CustomCanInvoke();
		}
	}

	public static bool CanInvokeFromGame
	{
		get
		{
			return CustomCanInvoke(false);
		}
	}
	...
	private void Awake()
	{
		...
		{
			if ((pagePrefabs[i] != null) ? true : false)
			{
				UnityEngine.Object.Instantiate(pagePrefabs[i], base.transform, false);
			}
		}
		gameMainObject = UnityEngine.Object.FindObjectOfType<Game>();
		string text = gameMainObject.state.ToString();
	}
	...
	public void ShowPauseMenu()
	{
		...
		MenuTransition to = ((Game.instance.currentLevelType != WorkshopItemSource.BuiltIn) ? GetMenu<PauseMenu>() : ((Game.instance.currentLevelNumber != Game.instance.levelCount) ? ((MenuTransition)GetMenu<PauseMenu>()) : ((MenuTransition)GetMenu<CreditsMenu>())));
		if (!NetGame.isLocal)
		{
	...
	private void Update()
	{
		if (state == MenuSystemState.MainMenu && NetGame.instance.players.Count == 1 && !NetGame.isNetStarted)
		{
			...
			if (human.transform.position.y < 50f)
			{
	...
	private void UpdateInputDeviceType()
	{
		...
		else if (mDevice != eInputDeviceType.Mouse && MouseInput())
		{
