...
public class ServoSound : MonoBehaviour
{
	...
	public void CrossfadeLoop()
	{
		...
		source.CrossFade(clip, volume, true, 0f, 0.5f);
	}
	...
	public void Play(float stopIn = 0f)
	{
		...
		AudioClip clip = ((!secondMedium) ? loopClips[Random.Range(0, loopClips.Length)] : loopClips2[Random.Range(0, loopClips2.Length)]);
		source.Play(audioClip, volume, false);
		source.CrossFade(clip, volume, true, audioClip.length - crossfadeStart, crossfadeStart);
	...
	public void Stop()
	{
		if (isPlaying)
		{
			...
			source.CrossFade(clip, volume, false, 0f, crossfadeEnd);
		}
	}
