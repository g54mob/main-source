...
public class HandMuscles
{
	...
	public void OnFixedUpdate()
	{
		...
		ProcessHand(leftMem, ragdoll.partLeftArm, ragdoll.partLeftForearm, ragdoll.partLeftHand, worldPos, leftExtend, grab, motion.legs.legPhase + 0.5f, false);
		ProcessHand(rightMem, ragdoll.partRightArm, ragdoll.partRightForearm, ragdoll.partRightHand, worldPos2, rightExtend, grab2, motion.legs.legPhase, true);
	}
	...
	private void ProcessHand(ScanMem mem, HumanSegment arm, HumanSegment forearm, HumanSegment hand, Vector3 worldPos, float extend, bool grab, float animationPhase, bool right)
	{
		...
		if (extend > 0.2f)
		{
			...
			PlaceHand(arm, hand, worldPos, true, hand.sensor.grabJoint != null, hand.sensor.grabBody);
	...
	private Vector3 FindTarget(ScanMem mem, Vector3 worldPos, out Collider targetCollider)
	{
		...
		{
			RaycastHit hitInfo;
			if (!Physics.SphereCast(ray2, num3, out hitInfo, vector4.magnitude, motion.grabLayers, QueryTriggerInteraction.Ignore))
			{
				continue;
			}
