...
public class WaterBodyCollider : MonoBehaviour
{
	...
	private static void RespawnRemove(NetBody netBody)
	{
		for (int num = objectsToRespawn.Count - 1; num >= 0; num--)
		{
			if (objectsToRespawn[num].respawnObject == netBody)
			{
	...
	private static NetBody CheckForNetBodyProxy(Collider other, NetBody netBody)
	{
		if (netBody == null)
		{
			WaterRespawnable component = other.gameObject.GetComponent<WaterRespawnable>();
			if (component != null)
			{
				netBody = component.BodyToRespawn;
			}
		}
		return netBody;
	}
	...
	private void RespawnProcess()
	{
		for (int num = objectsToRespawn.Count - 1; num >= 0; num--)
		{
			...
			if (objectsToRespawn[num].delay < 0f)
			{
				...
				RestartableRigid component = respawnObject.GetComponent<RestartableRigid>();
				if ((bool)component)
				{
					component.Reset(Vector3.zero);
				}
			}
		}
	}
	...
	public void OnTriggerEnter(Collider other)
	{
		...
		else
		{
			...
			component2 = CheckForNetBodyProxy(other, component2);
	...
	public void OnTriggerExit(Collider other)
	{
		...
		if (respawnNetBody && !NetGame.isClient)
		{
			NetBody component2 = other.gameObject.GetComponent<NetBody>();
			component2 = CheckForNetBodyProxy(other, component2);
			if (objectsToRespawn.Count > 0 && component2 != null && component2.respawn)
			{
				RespawnRemove(component2);
			}
		}
	}
	...
	private void Update()
	{
		...
		foreach (Collider collider in array)
		{
			...
			RaycastHit hitInfo;
			if ((!flag2 && flag) || ((flag2 || flag) && (collider.Raycast(ray, out hitInfo, magnitude) || collider.Raycast(ray2, out hitInfo, magnitude))))
			{
				if (flag)
				{
