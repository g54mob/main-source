...
namespace SimpleLightProbePlacer
{
	...
	public class TransformVolume : MonoBehaviour
	{
		...
		{
			Vector3 center = points.Aggregate(Vector3.zero, (Vector3 vector, Vector3 point) => vector + point) / points.Length;
			Bounds result = new Bounds(center, Vector3.zero);
			for (int num = 0; num < points.Length; num++)
			{
				result.Encapsulate(points[num]);
			}
			return result;
		}
