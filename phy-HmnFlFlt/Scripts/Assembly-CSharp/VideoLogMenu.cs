...
public class VideoLogMenu : MenuTransition
{
	...
	public VideoLogButton SelectedItem
	{
		get
		{
			return selectedItem;
	...
	{
		base.OnGotFocus();
		VideoLogButton[] componentsInChildren = GetComponentsInChildren<VideoLogButton>(true);
		...
		for (int i = 0; i < componentsInChildren.Length; i++)
		{
			...
			if (i < list.Count)
			{
				videoLogButton.gameObject.SetActive(true);
			...
			{
				videoLogButton.gameObject.SetActive(false);
			}
		}
		selectedItem = null;
		...
		if (transform2 != null && list.Count > 0)
		{
			...
			Link(componentsInChildren[list.Count - 1].GetComponent<Selectable>(), component, true);
			Link(component, componentsInChildren[0].GetComponent<Selectable>(), true);
		}
	}
