...
public class LevelSelectMenu2 : MenuTransition
{
	...
	public Color itemListNormal = new Color(0f, 0f, 0f, 0.19215687f);

	public Color itemListError = new Color(1f, 1f, 1f, 0.2509804f);
	...
	private string CustomDataPath
	{
		get
		{
			return PlayerPrefs.GetString("WorkshopRoot", Path.Combine(Application.dataPath, "Workshop"));
		}
	}
	...
	private bool IsLobbyMode()
	{
		...
		if (levelSelectMenuMode == LevelSelectMenuMode.BuiltInLobbies || levelSelectMenuMode == LevelSelectMenuMode.WorkshopLobbies)
		{
			return true;
		}
		return false;
	...
	public override void OnGotFocus()
	{
		...
		ButtonLegendBar.instance.ToogleCarouselLegend(true);
	...
	public override void OnLostFocus()
	{
		...
		ButtonLegendBar.instance.ToogleCarouselLegend(false);
	...
	public void Rebind()
	{
		...
		InLevelSelectMenu = displayMode != LevelSelectMenuMode.Campaign && displayMode != LevelSelectMenuMode.LocalWorkshop;
		...
		case LevelSelectMenuMode.WorkshopLobbies:
			if (flag2)
			{
				WorkshopRepository.instance.ReloadSubscriptions(true);
		...
		else
		{
			...
			GameSave.GetLastSave(out levelNumber, out checkpoint, out subObjectives, out timeSinceStart);
	...
	public void CustomFolder()
	{
		...
		{
			customSubtitle.SetActive(false);
			customFolderEditor.gameObject.SetActive(true);
	...
	public void SubmitCustomFolder()
	{
		...
		customFolderEditor.gameObject.SetActive(false);
	...
	public void WorkshopUploadClick()
	{
		...
		{
			SteamProgressOverlay.instance.ShowSteamProgress(true, null, null);
	...
	private void OnPublishOver(WorkshopItemMetadata meta, bool needAgreement, EResult error)
	{
		...
		{
			SteamProgressOverlay.instance.ShowSteamProgress(false, null, null);
		...
		else
		{
			...
			SteamProgressOverlay.instance.ShowSteamProgress(false, string.Empty, DismissSteamError);
		}
	...
	private void SelectItem(ListViewItem item)
	{
		...
		{
			selectedMenuItem.SetActive(false);
		}
		selectedMenuItem = workshopMenuItem;
		selectedMenuItem.SetActive(true);
		BindLevelIfNeeded(selectedMenuItem);
		bool flag = true;
		if (CouldShowFindMore())
		{
			if (DLC.instance.SupportsDLC())
			{
				...
				if (levelBundleID != DLC.DLCBundles.kNone)
				{
		...
		for (int i = num; i < length; i++)
		{
			...
			case DescriptionProcess.kBracketSearch:
				if (c == ']' || c == '=')
				{
					...
					IdTag(stringBuilder2, out mode, out replaceString, out tagIndex);
					if (flag)
					{
	...
	{
		inContinueMode = true;
		ButtonLegendBar.instance.SetContinueMode(true);
	}

	private void DisableLevelContinue()
	{
		inContinueMode = false;
		ButtonLegendBar.instance.SetContinueMode(false);
	}
	...
	private void UpdateTitle(bool forceDLCTitle = false)
	{
		...
		else
		{
			...
			multiplayerTitle.SetActive(false);
			customTitle.SetActive(false);
			editorPicksTitle.GetComponent<Localize>().OnLocalize(true);
