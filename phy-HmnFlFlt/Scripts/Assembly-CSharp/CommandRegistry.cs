...
public class CommandRegistry
{
	...
	internal void Execute(string txt)
	{
		...
		currentCommand = txt;
		Action value;
		if (commands.TryGetValue(txt, out value))
		{
			value();
			return;
		}
		Action<string> value2;
		if (commandsStr.TryGetValue(txt, out value2))
		{
			value2(null);
			return;
		}
	...
	public void OnHelp(string param)
	{
		...
		string[] array = param.ToLowerInvariant().Split(new char[1] { ' ' }, StringSplitOptions.RemoveEmptyEntries);
