...
namespace HumanAPI
{
	...
	public class CollisionAudioSensor : MonoBehaviour
	{
		...
		private void ReportCollision(Collision collision)
		{
			...
			float volume;
			float pitch;
			collision.Analyze(out pos, out impulse, out normalVelocity, out tangentVelocity, out mat, out mat2, out id, out volume, out pitch);
			if (this.id < id)
			{
		...
		public void OnReceiveCollisionAudio(NetStream stream)
		{
			...
			Vector3 vector = NetVector3.Read(stream, 3, 10).Dequantize(100f);
