	...
	public class Rope : RopeRender, INetBehavior, IRespawnable
	{
		...
		private void Initialize()
		{
			...
			if (fixStart && bones != null && bones.Length > 0)
			{
				...
				ConfigurableJointMotion configurableJointMotion = (configurableJoint2.zMotion = ConfigurableJointMotion.Locked);
				configurableJointMotion = (configurableJoint2.yMotion = configurableJointMotion);
				configurableJoint2.xMotion = configurableJointMotion;
				configurableJoint2.projectionMode = JointProjectionMode.PositionAndRotation;
				configurableJoint2.projectionDistance = 0.02f;
				if (fixStartDir)
				{
					configurableJointMotion = (configurableJoint2.angularZMotion = ConfigurableJointMotion.Locked);
					configurableJointMotion = (configurableJoint2.angularYMotion = configurableJointMotion);
			...
			if (fixEnd)
			{
				...
				ConfigurableJointMotion configurableJointMotion = (configurableJoint3.zMotion = ConfigurableJointMotion.Locked);
				configurableJointMotion = (configurableJoint3.yMotion = configurableJointMotion);
				configurableJoint3.xMotion = configurableJointMotion;
				configurableJoint3.projectionMode = JointProjectionMode.PositionAndRotation;
				configurableJoint3.projectionDistance = 0.02f;
				if (fixEndDir)
				{
					configurableJointMotion = (configurableJoint3.angularZMotion = ConfigurableJointMotion.Locked);
					configurableJointMotion = (configurableJoint3.angularYMotion = configurableJointMotion);
		...
		public override void ReadData()
		{
			for (int i = 0; i < rigidSegments; i++)
			{
				...
				boneRot[i] = quaternion2.eulerAngles;
				boneForward[i] = quaternion2 * Vector3.forward;
				boneUp[i] = quaternion2 * Vector3.up;
				boneRight[i] = quaternion2 * Vector3.right;
			}
		}
