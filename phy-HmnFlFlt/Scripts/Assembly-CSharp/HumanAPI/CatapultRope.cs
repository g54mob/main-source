...
namespace HumanAPI
{
	public class CatapultRope : RopeRender
	{
		...
		public override void GetPoint(float dist, out Vector3 pos, out Vector3 normal, out Vector3 binormal)
		{
			...
			{
				float num3 = (dist - straightT) / (1f - straightT);
				pos = num3 * (localTargetPos - exitWindPos) + exitWindPos;
		...
		public override void LateUpdate()
		{
			...
			if (applyForceToWheel && attachedBody != null && wheel != null)
			{
				...
				{
					Vector3 vector = base.transform.TransformPoint(exitWindPos);
					Vector3 vector2 = base.transform.TransformPoint(localTargetPos);
					Vector3 vector3 = vector2 - vector;
					float num = Vector3.Dot((attachedBody.transform.position - prevAttachedBodyPos).normalized, vector3.normalized);
					...
					if (num2 > 0f)
					{
						...
						rotation *= Quaternion.Euler(Vector3.up * num4);
			...
			{
				Vector3 vector4 = base.transform.TransformPoint(exitWindPos);
				Vector3 vector5 = base.transform.TransformPoint(localTargetPos);
				attachedBody.MovePosition(attachedBody.transform.position + (vector5 - vector4).normalized * lengthChange);
			}
		}
	}
}
