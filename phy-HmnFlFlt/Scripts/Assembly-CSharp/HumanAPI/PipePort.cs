...
namespace HumanAPI
{
	public class PipePort : SteamPort, IPostEndReset, IPostReset
	{
		...
		public float ignoreClearDistance = -1f;
		...
		private Vector3 lastBrokenConnectionPosition;
		...
		private void FixedUpdate()
		{
			...
			if (!flag || (ignorePipe != null && ignoreClearDistance > 0f && Vector3.Distance(base.transform.position, lastBrokenConnectionPosition) > ignoreClearDistance))
			{
				ignorePipe = null;
			}
			...
			if (base.transform.position.y < -40f && springIn != null)
			{
				DisconnectPipe();
				return;
			}
		...
		public void DisconnectPipe()
		{
			...
			lastBrokenConnectionPosition = connectedPipe.transform.position;
