...
namespace HumanAPI
{
	public class RagdollPresetMetadata : WorkshopItemMetadata
	{
		...
		public void ClearPaint()
		{
			...
			{
				main.bytes = null;
				main.suppressCustomTexture = true;
			}
			if (head != null)
			{
				head.bytes = null;
				head.suppressCustomTexture = true;
			}
			if (upperBody != null)
			{
				upperBody.bytes = null;
				upperBody.suppressCustomTexture = true;
			}
			if (lowerBody != null)
			{
				lowerBody.bytes = null;
				lowerBody.suppressCustomTexture = true;
			}
		}

		public void ClearCustomisation()
		{
			ClearColors();
			ClearPaint();
		}
		...
		{
			string path = Path.Combine(Application.persistentDataPath, "net/" + id.ToLower() + localCoopIndex);
