...
namespace HumanAPI
{
	...
	public class SignalSctiptPushButton1 : Node
	{
		...
		public void Update()
		{
			...
			bool flag3 = activationMode != ButtonActivationMode1.Physics && CalculateGrabbed();
			...
			if (flag3 && activationMode != ButtonActivationMode1.Physics)
			{
		...
		private float CalculateNormalizedOffset()
		{
			...
			Vector3 vector = buttonTransform.TransformPoint(slideJoint.anchor);
			Vector3 vector2 = slideJoint.connectedBody.transform.TransformPoint(slideJoint.connectedAnchor);
			float num = Vector3.Dot(vector - vector2, rhs);
