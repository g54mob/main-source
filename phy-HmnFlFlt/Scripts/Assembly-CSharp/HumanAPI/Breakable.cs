...
namespace HumanAPI
{
	public class Breakable : Node, INetBehavior, IGrabbable
	{
		...
		[SerializeField]
		[Tooltip("List of colliders to ignore impact from in \"breakOnImpact\" mode")]
		private Collider[] ignoreImpactFrom;

		private HashSet<Collider> ignoredImpactColliders;
		...
		private void Awake()
		{
			if (ignoreImpactFrom != null && ignoreImpactFrom.Length > 0)
			{
				ignoredImpactColliders = new HashSet<Collider>(ignoreImpactFrom);
			}
		}
		...
		private void HandleCollision(Collision collision, bool enter)
		{
			...
			if ((ignoredImpactColliders != null && ignoredImpactColliders.Contains(collision.collider)) || parent.name.StartsWith("Lock"))
			{
				return;
			}
			while (parent != null)
