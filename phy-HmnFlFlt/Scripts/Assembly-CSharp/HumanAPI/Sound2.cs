...
namespace HumanAPI
{
	public class Sound2 : MonoBehaviour
	{
		...
		public string sampleLabel
		{
			get
			{
				return (soundSample == null) ? ("MISS:" + sample) : (soundSample.category + ":" + sample);
			}
		}
		...
		private bool processUpdate
		{
			get
			{
				return fadeInDelay != 0f || fadeInDuration != 0f || fadeOutDuration != 0f || pauseSpeed != 0f;
			}
		}
		...
		protected virtual void OnSampleLoaded()
		{
			if (isPlaying)
			{
				...
				if (clip != null)
				{
					CrossfadeSound(clip, true, 0f, 0.1f);
				}
			}
			else if (startOnLoad)
			{
		...
		public void SetSample(SoundLibrary.SerializedSample sample)
		{
			...
			this.sample = ((sample == null) ? null : sample.name);
			soundSample = sample;
			if (soundSample != null)
			{
		...
		public void Switch(char choice, bool crossfade = true)
		{
			...
			if (soundSample != null && isPlaying && crossfade)
			{
				...
				{
					CrossfadeSound(serializedClip, true, 0f, soundSample.crossFade);
				}
			}
		}
		...
		public void Play(bool forceLoop = false)
		{
			...
			SoundLibrary.SerializedClip clip = soundSample.GetClip(null, currentChoice, SoundLibrary.SampleContainerChildType.Start);
			currentClip = clip;
			if (currentClip != null)
			{
			...
			{
				clip = soundSample.GetClip(currentClip, currentChoice, SoundLibrary.SampleContainerChildType.Loop);
				if (clip != null)
				{
					CrossfadeSound(clip, true, 0f, soundSample.crossFade);
				}
			}
		}
		...
		public void Stop()
		{
			...
			if (soundSample != null)
			{
				...
				if (clip != null)
				{
					CrossfadeSound(clip, false, 0f, soundSample.crossFade);
				}
				else
				{
		...
		public void PlayOneShot(SoundLibrary.SerializedClip clip, Vector3 pos, float volume = 1f, float pitch = 1f, float delay = 0f)
		{
			...
			ReleaseAudioSource(ref source, true);
		}
		...
		public void FadeOut(float duration)
		{
			if (activeClip != null)
			{
				...
				else
				{
					if (inactiveClip != null)
					{
						ReleaseAudioSource(ref inactiveSource, false);
					}
					if (inactiveClip != null)
					{
