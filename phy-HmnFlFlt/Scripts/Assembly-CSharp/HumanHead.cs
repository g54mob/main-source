...
public class HumanHead : WaterSensor, INetBehavior
{
	...
	private void Update()
	{
		...
		for (int i = 0; i < waterBodies.Count; i++)
		{
			...
			if (num > 0f)
			{
				diveTime += Time.deltaTime * waterBodies[i].drownSpeedFactor;
				if (num > drownDepth)
				{
	...
	private void StopUnderwaterSound(bool drown)
	{
		if (humanAudio.underwater.isPlaying)
		{
			if (drown)
			{
				humanAudio.underwater.Switch('B', false);
			}
			humanAudio.underwater.Stop();
			if (drown)
			{
				humanAudio.underwater.Switch('A', false);
			}
		}
	}
	...
	{
		uint num = ((state0 != null) ? state0.ReadUInt32(2) : 0u);
