...
public static class UL_Renderer
{
	...
	public static void Add(Vector3 position, float range, Color color)
	{
		...
		if (!(num2 >= 0f) || !(Mathf.Abs(num2) >= range))
		{
			...
			{
				Debug.LogError(string.Format("Add color: {0}", color));
			}
		}
	}
	...
	public static void SetupForCamera(Camera camera, MaterialPropertyBlock properties)
	{
		...
		for (int num4 = _lightsCount - 1; num4 >= 0; num4--)
		{
			...
			_lightsColors[num4] = light.color;
			colors[num4] = light.color;
			if (light.color.x < 0f || light.color.y < 0f || light.color.z < 0f || light.color.w < 0f)
			{
				Debug.LogError(string.Format("SetupForCamera: {0}", light.color));
			}
		...
		{
			Matrix4x4 inverse = GL.GetGPUProjectionMatrix(camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Left), true).inverse;
			inverse[1, 1] *= -1f;
			...
			Matrix4x4 inverse2 = GL.GetGPUProjectionMatrix(camera.GetStereoProjectionMatrix(Camera.StereoscopicEye.Right), true).inverse;
			inverse2[1, 1] *= -1f;
		...
		{
			Matrix4x4 inverse3 = GL.GetGPUProjectionMatrix(camera.projectionMatrix, true).inverse;
			inverse3[1, 1] *= -1f;
