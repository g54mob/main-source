...
public class HFFResources : MonoBehaviour
{
	...
	public Texture2D FindTextureResource(string path)
	{
		...
		{
			string b = GetName(path);
			for (int i = 0; i < mNumberLevelImages; i++)
			{
				if (string.Equals(mLevelNames[i], b, StringComparison.OrdinalIgnoreCase))
				{
		...
		{
			string b2 = GetName(path);
			for (int j = 0; j < mNumberLobbyImages; j++)
			{
				if (string.Equals(mLobbyNames[j], b2, StringComparison.OrdinalIgnoreCase))
				{
		...
		{
			string text = GetName(path);
			Texture2D texture2D = FindResource(mSkinNames, SkinTextures, text, mNumberSkinTextures);
	...
	{
		string text = GetName(path);
		return FindResource(mVideoSrtNames, VideoSrts, text, mNumberVideoSrts);
	}

	public Texture GetVideoThumb(string path)
	{
		string text = GetName(path);
		return FindResource(mVideoThumbsNames, VideoThumbs, text, mNumberVideoThumbs);
	}

	public AudioClip GetMusicTrack(string path)
	{
		string text = GetName(path);
		return FindResource(mMusicNames, Music, text, mNumberMusic);
	}
}
