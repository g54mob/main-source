	...
	private void FixedUpdate()
	{
		...
		if (process)
		{
			...
			if (frame == frameSkip)
			{
				frame = 0;
				RaycastHit hitInfo;
				b = ((!Physics.Raycast(base.transform.position + new Vector3(0f, 2f, 0f), Vector3.up, out hitInfo, float.PositiveInfinity)) ? maxEmit : minEmit);
			}
			else
			{
				frame++;
			...
			float strengthMultiplier = Mathf.Lerp(pFX.noise.strengthMultiplier, noise, speed * Time.deltaTime);
