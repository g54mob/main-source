...
public abstract class ShatterBase : MonoBehaviour, INetBehavior
{
	private struct NetState
	{
		...
		public static NetState Read(NetStream stream)
		{
			return new NetState
			{
				netId = stream.ReadNetId(),
				shatterPos = NetVector3.Read(stream, 10),
				shatterMagnitude = stream.ReadUInt32(10),
				shatterSeed = stream.ReadUInt32(10)
			};
		}
	}
	...
	protected virtual void OnEnable()
	{
		...
		if (renderer == null)
		{
			Debug.LogError("ShatterBase renderer not found: " + base.gameObject.name);
			base.enabled = false;
		}
		if (collider == null)
		{
			Debug.LogError("ShatterBase collider not found: " + base.gameObject.name);
			base.enabled = false;
		}
	}
	...
	{
		ShatterLocal(Vector3.zero, Vector3.zero);
	}
