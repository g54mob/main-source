...
public class CreditsBlock : MonoBehaviour, INetBehavior
{
	...
	public void Initialize(float lineY, string text, CreditsText ctext)
	{
		...
		for (int i = 0; i < text.Length; i++)
		{
			...
			if (c == '[' && text[i + 1] == 'H' && text[i + 3] == ']')
			{
				...
				zero += vector2 * lineHeight * (num - 1f);
				i += 3;
				continue;
			}
			switch (c)
			{
			case ' ':
				zero += vector * space * num;
				break;
			case '\n':
				zero.x = 0f;
				zero += vector2 * lineHeight;
				num = 1f;
				break;
			default:
			{
				...
				zero += vector * (ctext.GetLetterWidth(c) + 0.1f) * num;
				characters.Add(item);
				letters.Add(null);
				break;
			}
	...
	internal void Despawn(float v)
	{
		for (int num = letters.Count - 1; num >= 0; num--)
		{
			if (letters[num] != null && letters[num].transform.position.y > v)
			{
				DespawnLetter(num);
			}
		}
	...
	private void SpawnLetter(int idx)
	{
		if (!(letters[idx] != null))
		{
			...
			letter.transform.SetParent(base.transform, false);
	...
	public void CollectState(NetStream stream)
	{
		...
		if (isSpawned)
		{
			...
			for (int i = 0; i < letters.Count; i++)
			{
				...
				stream.Write(true);
				posEncoder.CollectState(stream, creditsLetter.transform.localPosition - characters[i].spawnPos);
	...
	{
		bool flag = state0 != null && state0.ReadBool();
		...
		for (int i = 0; i < letters.Count; i++)
		{
			...
			if (flag3 && flag4)
			{
				creditsLetter.transform.localPosition = posEncoder.ApplyLerpedState(state0, state1, mix) + characters[i].spawnPos;
				creditsLetter.transform.localRotation = rotEncoder.ApplyLerpedState(state0, state1, mix);
			}
			else if (flag4)
			{
				creditsLetter.transform.localPosition = posEncoder.ApplyState(state1) + characters[i].spawnPos;
	...
	public void ApplyState(NetStream state)
	{
		...
		for (int i = 0; i < letters.Count; i++)
		{
			...
			if (flag2)
			{
				creditsLetter.transform.localPosition = posEncoder.ApplyState(state) + characters[i].spawnPos;
	...
	public void AddDelta(NetStream state0, NetStream delta, NetStream result)
	{
		...
		if (flag2)
		{
			...
			for (int i = 0; i < letters.Count; i++)
			{
				...
				if (flag4)
				{
					NetStream state1 = ((!flag3) ? null : state0);
					posEncoder.AddDelta(state1, delta, result);
					rotEncoder.AddDelta(state1, delta, result);
				}
				else if (flag3)
				{
					posEncoder.ApplyState(state0);
