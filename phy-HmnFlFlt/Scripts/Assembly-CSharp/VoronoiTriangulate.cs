...
public class VoronoiTriangulate : MonoBehaviour, IReset
{
	...
	public void Deform(Vector3 worldPos, Vector3 impact)
	{
		...
		Vector3 vector2 = base.transform.InverseTransformVector(-impact);
		for (int i = 0; i < vertices.Length; i++)
		{
			float num = depth * Mathf.InverseLerp(1f, 0f, (vertices[i] - vector).sqrMagnitude);
			if (num > 0f)
			{
				vertices[i] += vector2 * num;
			}
		}
	...
	private void Generate()
	{
		...
		{
			Vector2 vector9 = new Vector2(array[l], array2[l]);
			float sqrMagnitude = (vector5 - vector9).sqrMagnitude;
			if (sqrMagnitude < num6)
			{
				num6 = sqrMagnitude;
				num7 = l;
			}
			float sqrMagnitude2 = (vector6 - vector9).sqrMagnitude;
			if (sqrMagnitude2 < num8)
			{
				num8 = sqrMagnitude2;
				num9 = l;
			}
			float sqrMagnitude3 = (vector7 - vector9).sqrMagnitude;
			if (sqrMagnitude3 < num10)
			{
				num10 = sqrMagnitude3;
				num11 = l;
			}
			float sqrMagnitude4 = (vector8 - vector9).sqrMagnitude;
		...
		{
			Vector2 vector10 = new Vector2(array[m], array2[m]);
			...
			{
				Vector2 vector11 = list8[num14] - zero;
				float num15 = Mathf.Atan2(vector11.x, vector11.y);
				int num16;
			...
			{
				Vector2 vector12 = list6[num17];
				list2.Add(To3D(new Vector3(vector12.x, vector12.y, z3)) + collider.center);
				list2.Add(To3D(new Vector3(vector12.x, vector12.y, z2)) + collider.center);
				if (num17 >= 2)
				{
					list3.Add(count3);
				...
				bool flag2 = Mathf.Abs(vector12.x - num) < 0.01f || Mathf.Abs(vector12.x - num2) < 0.01f || Mathf.Abs(vector12.y - num3) < 0.01f || Mathf.Abs(vector12.y - num4) < 0.01f;
				if (flag2 && flag)
				{
