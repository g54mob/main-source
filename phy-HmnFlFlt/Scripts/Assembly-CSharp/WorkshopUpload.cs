...
public static class WorkshopUpload
{
	...
	public static void Upload(WorkshopItemMetadata meta, string folder, string thumbnail, string updateNotes, Action<WorkshopItemMetadata, bool, EResult> uploadCallback)
	{
		if (!Initialize())
		{
			uploadCallback(meta, false, EResult.k_EResultConnectFailed);
			return;
		}
	...
	private static void OnSteamUGCQueryCompleted(SteamUGCQueryCompleted_t param, bool bIOFailure)
	{
		...
		if (param.m_eResult == EResult.k_EResultOK && SteamUGC.GetQueryUGCResult(param.m_handle, 0u, out pDetails) && pDetails.m_eResult == EResult.k_EResultFileNotFound)
		{
			meta.workshopId = 0uL;
			meta.Save(folder);
			Upload();
	...
	private static void PerformUpload(bool preExisting = false)
	{
		...
		if (!flag)
		{
			...
			else
			{
				...
				{
					list.Add(TagStrings("WORKSHOP/LevelTypeDesc", meta.typeTags, true));
				}
			}
			if (meta.playerTags > 0 && meta.playerTags < 8)
			{
				list.Add(TagStrings("WORKSHOP/RecommendedPlayersDesc", meta.playerTags, true));
			}
	...
	private static void OnSubmitItemUpdateResult(SubmitItemUpdateResult_t res, bool bIOFailure)
	{
		...
		else
		{
			...
			uploadCallback(meta, false, EResult.k_EResultOK);
			uploadCallback = null;
		}
	}
	...
	public static void UploadMonitor()
	{
		if (!NetTransportSteam.sSteamServersConnected)
		{
			...
			{
				uploadCallback(meta, false, EResult.k_EResultCancelled);
				uploadCallback = null;
			}
		}
	}
