...
public class MB2_LODManager : MonoBehaviour
{
	...
	private void LateUpdate()
	{
		...
		if (LOG_LEVEL == MB2_LogLevel.debug)
		{
			...
			frameInfo[num] = new BakeDiagnostic(this);
			if (num == 0)
			{
	...
	private void UpdateMeshesThatNeedToChange()
	{
		...
		if (statNumDirty > 0)
		{
			...
			{
				MB2_Log.Log(MB2_LogLevel.trace, string.Format("LODManager.UpdateMeshesThatNeedToChange called. dirty clusters= {0}", list.Count), LOG_LEVEL);
			}
			statLastNumBakes = 0;
	...
	private List<LODCombinedMesh> PrioritizeCombinedMeshs()
	{
		...
		{
			array2[j] = lodCameras[j].transform.position;
		}
