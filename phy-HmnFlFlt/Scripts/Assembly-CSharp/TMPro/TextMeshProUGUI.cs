	...
	public class TextMeshProUGUI : TMP_Text, ILayoutElement
	{
		...
		protected override void GenerateTextMesh()
		{
			...
			for (int i = 0; i < m_char_buffer.Length && m_char_buffer[i] != 0; i++)
			{
				...
				float num24 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType != TMP_TextElementType.Character) ? m_textInfo.characterInfo[m_characterCount].scale : (num2 / num17)) + m_baselineOffset;
				...
				float num25 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType != TMP_TextElementType.Character) ? m_textInfo.characterInfo[m_characterCount].scale : (num2 / num17)) + m_baselineOffset;
				...
				if (num4 == 10 || m_characterCount == totalCharacterCount - 1)
				{
					if (m_lineNumber > 0 && !TMP_Math.Approximately(m_maxLineAscender, m_startOfLineAscender) && m_lineHeight == -32767f && !m_isNewPage)
					{
						...
						AdjustLineOffset(m_firstCharacterOfLine, m_characterCount, num36);
						num26 -= num36;
						m_lineOffset += num36;
					}
					m_isNewPage = false;
					...
					m_maxDescender = ((!(m_maxDescender < num38)) ? num38 : m_maxDescender);
					if (!flag5)
					{
						num15 = m_maxDescender;
					}
					...
					m_textInfo.lineInfo[m_lineNumber].characterCount = m_textInfo.lineInfo[m_lineNumber].lastCharacterIndex - m_textInfo.lineInfo[m_lineNumber].firstCharacterIndex + 1;
					m_textInfo.lineInfo[m_lineNumber].visibleCharacterCount = m_lineVisibleCharacterCount;
					m_textInfo.lineInfo[m_lineNumber].lineExtents.min = new Vector2(m_textInfo.characterInfo[m_firstVisibleCharacterOfLine].bottomLeft.x, num38);
					m_textInfo.lineInfo[m_lineNumber].lineExtents.max = new Vector2(m_textInfo.characterInfo[m_lastVisibleCharacterOfLine].topRight.x, num37);
					...
					m_textInfo.lineInfo[m_lineNumber].baseline = 0f - m_lineOffset;
					m_textInfo.lineInfo[m_lineNumber].ascender = num37;
					m_textInfo.lineInfo[m_lineNumber].descender = num38;
					m_textInfo.lineInfo[m_lineNumber].lineHeight = num37 - num38 + num8 * num;
			...
			float num43 = (m_previousLossyScaleY = base.transform.lossyScale.y);
			...
			for (int j = 0; j < m_characterCount; j++)
			{
				...
				case TextAlignmentOptions.CaplineFlush:
				{
					...
					{
						if (lineNumber != num40 || j == 0 || j == m_firstVisibleCharacter)
						{
						...
						float num52 = (m_isRightToLeft ? (tMP_LineInfo.width + tMP_LineInfo.maxAdvance) : (tMP_LineInfo.width - tMP_LineInfo.maxAdvance));
						int num53 = tMP_LineInfo.visibleCharacterCount - 1;
						int num54 = ((!characterInfo[tMP_LineInfo.lastCharacterIndex].isVisible) ? (tMP_LineInfo.spaceCount - 1) : tMP_LineInfo.spaceCount);
						if (flag10)
						{
							num54--;
							num53++;
						}
						float num55 = ((num54 <= 0) ? 1f : m_wordWrappingRatios);
						if (num54 < 1)
						{
							num54 = 1;
						}
						if (character2 == '\t' || char.IsSeparator(character2))
						{
							if (!m_isRightToLeft)
							{
								vector11 += new Vector3(num52 * (1f - num55) / (float)num54, 0f, 0f);
							}
							else
							{
								vector11 -= new Vector3(num52 * (1f - num55) / (float)num54, 0f, 0f);
						...
						{
							vector11 += new Vector3(num52 * num55 / (float)num53, 0f, 0f);
						}
						else
						{
							vector11 -= new Vector3(num52 * num55 / (float)num53, 0f, 0f);
				...
				if (isVisible)
				{
					...
					case TMP_TextElementType.Character:
					{
						...
						float num56 = m_uvLineOffset * (float)lineNumber % 1f;
						...
						case TextureMappingOptions.Line:
							...
							{
								characterInfo[j].vertex_BL.uv2.x = (characterInfo[j].vertex_BL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num56;
								characterInfo[j].vertex_TL.uv2.x = (characterInfo[j].vertex_TL.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num56;
								characterInfo[j].vertex_TR.uv2.x = (characterInfo[j].vertex_TR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num56;
								characterInfo[j].vertex_BR.uv2.x = (characterInfo[j].vertex_BR.position.x + vector11.x - m_meshExtents.min.x) / (m_meshExtents.max.x - m_meshExtents.min.x) + num56;
							}
							break;
						...
						case TextureMappingOptions.MatchAspect:
						{
							...
							case TextureMappingOptions.Line:
								...
								break;
							case TextureMappingOptions.Paragraph:
								characterInfo[j].vertex_BL.uv2.y = (characterInfo[j].vertex_BL.position.y - m_meshExtents.min.y) / (m_meshExtents.max.y - m_meshExtents.min.y) + num56;
								characterInfo[j].vertex_TL.uv2.y = (characterInfo[j].vertex_TL.position.y - m_meshExtents.min.y) / (m_meshExtents.max.y - m_meshExtents.min.y) + num56;
								characterInfo[j].vertex_TR.uv2.y = characterInfo[j].vertex_BL.uv2.y;
								characterInfo[j].vertex_BR.uv2.y = characterInfo[j].vertex_TL.uv2.y;
								break;
							case TextureMappingOptions.MatchAspect:
					...
					if (j < m_maxVisibleCharacters && num39 < m_maxVisibleWords && lineNumber < m_maxVisibleLines && m_overflowMode != TextOverflowModes.Page)
					{
					...
					else if (j < m_maxVisibleCharacters && num39 < m_maxVisibleWords && lineNumber < m_maxVisibleLines && m_overflowMode == TextOverflowModes.Page && characterInfo[j].pageNumber == num10)
					{
						characterInfo[j].vertex_BL.position += zero3;
						characterInfo[j].vertex_TL.position += zero3;
						characterInfo[j].vertex_TR.position += zero3;
				...
				if (lineNumber != num40 || j == m_characterCount - 1)
				{
					if (lineNumber != num40)
					{
				...
				{
					num42 = ((j != m_characterCount - 1 || !char.IsLetterOrDigit(character2)) ? (j - 1) : j);
					flag11 = false;
				...
				if ((m_textInfo.characterInfo[j].style & FontStyles.Underline) == FontStyles.Underline)
				{
					...
					{
						num47 = Mathf.Max(num47, m_textInfo.characterInfo[j].scale);
						num48 = Mathf.Min((pageNumber != num49) ? TMP_Text.k_LargePositiveFloat : num48, m_textInfo.characterInfo[j].baseLine + base.font.fontInfo.Underline * num47);
						num49 = pageNumber;
					}
					if (!flag && flag14 && j <= tMP_LineInfo.lastVisibleCharacterIndex && character2 != '\n' && character2 != '\r' && (j != tMP_LineInfo.lastVisibleCharacterIndex || !char.IsSeparator(character2)))
					{
						...
						start = new Vector3(m_textInfo.characterInfo[j].bottomLeft.x, num48, 0f);
						color = m_textInfo.characterInfo[j].underlineColor;
					}
					if (flag && m_characterCount == 1)
					{
						flag = false;
						zero = new Vector3(m_textInfo.characterInfo[j].topRight.x, num48, 0f);
					...
					{
						flag = false;
						zero = new Vector3(m_textInfo.characterInfo[j - 1].topRight.x, num48, 0f);
						num46 = m_textInfo.characterInfo[j - 1].scale;
						DrawUnderlineMesh(start, zero, ref index, num45, num46, num47, num44, color);
						num47 = 0f;
						num48 = TMP_Text.k_LargePositiveFloat;
					}
				...
				if (flag15)
				{
					...
					{
						flag2 = true;
						num50 = m_textInfo.characterInfo[j].pointSize;
						num51 = m_textInfo.characterInfo[j].scale;
						start2 = new Vector3(m_textInfo.characterInfo[j].bottomLeft.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num51, 0f);
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j].topRight.x, m_textInfo.characterInfo[j].baseLine + strikethrough * num51, 0f);
					...
					{
						flag2 = false;
						zero2 = new Vector3(m_textInfo.characterInfo[j - 1].topRight.x, m_textInfo.characterInfo[j - 1].baseLine + strikethrough * num51, 0f);
						DrawUnderlineMesh(start2, zero2, ref index, num51, num51, num51, num44, underlineColor);
					}
				}
				else if (flag2)
