	...
	public class TMP_Text : MaskableGraphic
	{
		...
		protected virtual Vector2 CalculatePreferredValues(float defaultFontSize, Vector2 marginSize, bool ignoreTextAutoSizing)
		{
			...
			for (int i = 0; m_char_buffer[i] != 0; i++)
			{
				...
				num5 = ((m_textElementType != TMP_TextElementType.Character || isUsingAlternateTypeface || ((m_style & FontStyles.Bold) != FontStyles.Bold && (m_fontStyle & FontStyles.Bold) != FontStyles.Bold)) ? 1f : (1f + m_currentFontAsset.boldSpacing * 0.01f));
				m_internalCharacterInfo[m_characterCount].baseLine = 0f - m_lineOffset + m_baselineOffset;
				float num16 = m_currentFontAsset.fontInfo.Ascender * ((m_textElementType != TMP_TextElementType.Character) ? m_internalCharacterInfo[m_characterCount].scale : (num2 / num12)) + m_baselineOffset;
				...
				float num17 = m_currentFontAsset.fontInfo.Descender * ((m_textElementType != TMP_TextElementType.Character) ? m_internalCharacterInfo[m_characterCount].scale : (num2 / num12)) + m_baselineOffset;
		...
		protected virtual void SaveGlyphVertexInfo(float padding, float style_padding, Color32 vertexColor)
		{
			...
			{
				ref TMP_CharacterInfo reference = ref m_textInfo.characterInfo[m_characterCount];
				reference.vertex_BL.color = reference.vertex_BL.color * m_colorGradientPreset.bottomLeft;
				ref TMP_CharacterInfo reference2 = ref m_textInfo.characterInfo[m_characterCount];
				reference2.vertex_TL.color = reference2.vertex_TL.color * m_colorGradientPreset.topLeft;
				ref TMP_CharacterInfo reference3 = ref m_textInfo.characterInfo[m_characterCount];
				reference3.vertex_TR.color = reference3.vertex_TR.color * m_colorGradientPreset.topRight;
				ref TMP_CharacterInfo reference4 = ref m_textInfo.characterInfo[m_characterCount];
				reference4.vertex_BR.color = reference4.vertex_BR.color * m_colorGradientPreset.bottomRight;
			}
			if (!m_isSDFShader)
			{
				style_padding = 0f;
