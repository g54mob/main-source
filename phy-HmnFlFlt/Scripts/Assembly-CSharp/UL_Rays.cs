...
public static class UL_Rays
{
	public class Ray
	{
		...
		public void Trace(Vector3 pt, Vector3 dir, float range, Color lightColor, int layersToHit)
		{
			...
			hit = false;
			RaycastHit hitInfo;
			if (!Physics.Raycast(pt, dir, out hitInfo, range * 0.9f, layersToHit))
			{
				return;
			}
			...
			RaycastHit hitInfo2;
			position = ((!Physics.Raycast(origin, normal, out hitInfo2, 2f, layersToHit)) ? (point + 1f * normal) : (point + 0.5f * hitInfo2.distance * normal));
		...
		private static Color GetRayHitColor(Renderer r)
		{
			...
			Color value;
			if (_cachedRendererColor.TryGetValue(r, out value))
			{
				return value;
			}
