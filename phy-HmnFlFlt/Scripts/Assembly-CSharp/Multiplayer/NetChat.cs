...
namespace Multiplayer
{
	public class NetChat : MonoBehaviour
	{
		...
		public static CommandRegistry serverCommands = new CommandRegistry(Print, true);

		public static CommandRegistry clientCommands = new CommandRegistry(Print, true);
		...
		private void OnEnable()
		{
			...
			RegisterCommand(true, true, "list", OnList, "#MULTIPLAYER/CHAT.HelpList");
			RegisterCommand(true, true, "mute", OnMute, "#MULTIPLAYER/CHAT.HelpMute");
			RegisterCommand(true, false, "kick", OnKick, "#MULTIPLAYER/CHAT.HelpKick");
		}
		...
		private NetHost GetClient(string txt)
		{
			...
			Print(string.Format("{0} {1}", arg, result));
			OnList();
			return null;
		}
		...
		private void OnMute(string txt)
		{
			...
			if (client.mute)
			{
				...
				Print(text);
				Print(string.Format("{0} {1} {2}", text, txt, client.name));
			}
			else
			{
				string text2 = ScriptLocalization.Get("XTRA/NetChat_UnMuted");
				Print(text2);
				Print(string.Format("{0} {1} {2}", text2, txt, client.name));
			}
		}
		...
		private void Show(bool showMessages, bool showType)
		{
			...
			else
			{
				if (MenuSystem.instance != null)
				{
					MenuSystem.instance.FocusOnMouseOver(true);
				}
		...
		private void Update()
		{
			...
			if (MenuSystem.keyboardState != KeyboardState.None && MenuSystem.keyboardState != KeyboardState.NetChat)
			{
				return;
			}
		...
		public static void OnReceive(uint clientId, string nick, string msg)
		{
			...
			if (netHost != null && !netHost.mute)
			{
				msg = msg.Replace('<', '〈');
				msg = msg.Replace('>', '〉');
				Print(string.Format("<#{0}>{1}</color> {2}", HexConverter.ColorToHex(instance.colors[clientId % instance.colors.Length]), nick, msg));
			}
		}
