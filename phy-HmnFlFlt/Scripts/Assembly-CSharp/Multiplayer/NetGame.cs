...
namespace Multiplayer
{
	public class NetGame : MonoBehaviour, IDependency
	{
		...
		public static uint loadedLevel
		{
			get
			{
				return (uint)((!instance.currentLevelStarted) ? 15u : instance.currentLevel);
			}
		}
		...
		public void OnConnect(object connection)
		{
			...
			NetScope.RemoveAllRemoteState(netHost2, false);
		...
		public void OnHelo(object serverConnection, NetStream stream)
		{
			...
			else if (num == VersionDisplay.netCode)
			{
				...
				HumanAnalytics.instance.BeginMultiplayer(false);
				StopLocalGame();
				AddLocalPlayer();
		...
		public void OnData(object connection, byte[] buffer, int tier, int dataSize)
		{
			...
			NetStream netStream = NetStream.AllocStream(buffer, tier, 0, true);
			try
			{
				if (isServer)
				{
		...
		public void OnServerReceive(NetHost client, NetStream stream)
		{
			...
			case NetMsgId.Container:
			{
				...
				ignoreDeltasMask = (ignoreDeltasMask & 0xFFFFFFFEu) | (num & 1);
				break;
			}
			case NetMsgId.Helo:
		...
		public void AddLocalPlayer(uint localCoopIndex)
		{
			...
			AddServerPlayer(local, true, transport.getUserId((int)localCoopIndex), localCoopIndex, cRC);
		}
		...
		private void OnRequestAddPlayerServer(NetHost client, NetStream stream)
		{
			...
			AddServerPlayer(client, false, userId, localCoopIndex, crc);
		}
		...
		private NetPlayer AddServerPlayer(NetHost owner, bool isLocal, string userId, uint localCoopIndex, byte[] crc)
		{
			...
			if (owner == null || !owner.isReady)
			{
				...
				{
					UnityEngine.Debug.LogError("Request to spawn a player for a NetHost that isn't ready : hostid=" + owner.hostId);
				}
				return null;
			}
		...
		private void DestroyHostObjects(NetHost client)
		{
			...
			NetScope.RemoveAllRemoteState(client, true);
		}
		...
		public void HostGame(object sessionArgs)
		{
			...
			serverFrameId = 0;
			UpdateLevelInstanceID(currentLevel, false, 0u);
			transport.StartThread();
		...
		private void HostGameCallback(string error)
		{
			...
			if (string.IsNullOrEmpty(error))
			{
				...
				HumanAnalytics.instance.BeginMultiplayer(true);
			}
			else
			{
				lock (stateLock)
		...
		private void OnReceiveSkin(NetStream stream)
		{
			...
			for (int i = 0; i < players.Count; i++)
			{
				...
				{
					netPlayer.skin.ClearCustomisation();
					netPlayer.ApplyPreset(netPlayer.skin);
				}
				else
				{
					netPlayer.ApplyPreset(netPlayer.skin);
