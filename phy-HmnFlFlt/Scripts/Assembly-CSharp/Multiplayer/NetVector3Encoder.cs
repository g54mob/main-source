...
namespace Multiplayer
{
	public class NetVector3Encoder
	{
		...
		{
			Vector3 vector = NetVector3.Read(state0, fullBits).Dequantize(range);
			Vector3 vector2 = NetVector3.Read(state1, fullBits).Dequantize(range);
			if ((vector - vector2).sqrMagnitude > 15f)
			{
				vector = vector2;
			}
			return Vector3.Lerp(vector, vector2, mix);
		}
		...
		public bool CalculateWorstCaseDelta(NetStream state0, NetStream state1, NetStream delta, bool writeChanged = true)
		{
			...
			NetVector3.WorstCaseDelta(netVector, to).Write(delta, deltaSmall, deltaLarge, fullBits);
			return true;
		}
