	...
	public class NetBodySleep
	{
		...
		public void HandleSleep()
		{
			if (!NetGame.isClient && !body.IsSleeping())
			{
				...
				float angle;
				Vector3 axis;
				(rotation * Quaternion.Inverse(freezeRot)).ToAngleAxis(out angle, out axis);
				axis *= angle / Time.fixedDeltaTime * ((float)Math.PI / 180f);
				Vector3 vector = body.transform.InverseTransformVector(axis);
				Vector3 b = Quaternion.Inverse(inertiaRotation) * vector;
