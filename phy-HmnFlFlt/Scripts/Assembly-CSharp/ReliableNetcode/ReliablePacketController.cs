...
namespace ReliableNetcode
{
	internal class ReliablePacketController
	{
		...
		public void Reset()
		{
			...
			{
				FragmentReassemblyData fragmentReassemblyData = fragmentReassembly.AtIndex(i);
		...
		public void ReceivePacket(byte[] packetData, int bufferLength)
		{
			...
			uint ackBits2;
			byte channelID2;
			int num3 = PacketIO.ReadFragmentHeader(packetData, 0, bufferLength, config.MaxFragments, config.FragmentSize, out fragmentID, out numFragments, out fragmentBytes, out num4, out ack2, out ackBits2, out channelID2);
			FragmentReassemblyData fragmentReassemblyData = fragmentReassembly.Find(num4);
			if (fragmentReassemblyData == null)
			{
				fragmentReassemblyData = fragmentReassembly.Insert(num4);
				if (fragmentReassemblyData == null)
				{
					return;
				}
				fragmentReassemblyData.Sequence = num4;
			...
			if (numFragments == fragmentReassemblyData.NumFragmentsTotal && !fragmentReassemblyData.FragmentReceived[fragmentID])
			{
				...
				byte[] buffer = BufferPool.GetBuffer(2048);
				Buffer.BlockCopy(packetData, num3, buffer, 0, bufferLength - num3);
				fragmentReassemblyData.StoreFragmentData(channelID2, num4, ack2, ackBits2, fragmentID, config.FragmentSize, buffer, bufferLength - num3);
				BufferPool.ReturnBuffer(buffer);
				if (fragmentReassemblyData.NumFragmentsReceived == fragmentReassemblyData.NumFragmentsTotal)
				{
					...
					fragmentReassemblyData.PacketDataBuffer.SetSize(0);
					fragmentReassembly.Remove(num4);
				}
			}
		}
	}
