...
public class CreditsExperience : MonoBehaviour, INetBehavior
{
	...
	private const int AbsolutePosRangeOverride = 5000;
	...
	private void ParseContent()
	{
		...
		while (j < array.Length)
		{
			...
			if (j < array.Length)
			{
				...
				component.gameObject.name = "CrBlock" + netIdentity.sceneId;
			}
		}
		...
		for (int count = list.Count; k < count; k++)
		{
			...
			else
			{
				...
				blockBucket.go.transform.SetParent(base.gameObject.transform, false);
			...
			creditsBlock.transform.SetParent(blockBucket.go.transform, true);
	...
	private void Awake()
	{
		...
		componentInParent.prerespawn = delegate(int cp, bool startingLevel)
		{
			...
			{
				item.ControlVelocity(0.1f, false);
			}
		};
		foreach (NetPlayer player in NetGame.instance.players)
		{
			player.UpdateAbsolutePosRangeOverride(5000);
		}
	}
	...
	private void OnDestroy()
	{
		...
		if (NetGame.instance != null)
		{
			foreach (NetPlayer player in NetGame.instance.players)
			{
				player.UpdateAbsolutePosRangeOverride();
			}
		}
		canSetClounds = false;
	...
	private void FixedUpdate()
	{
		...
		Vector3 center;
		Vector3 avgSpeed;
		AnalyzeHumans(out lowHuman, out highHuman, out center, out avgSpeed);
		if (!(lowHuman != null))
		{
			return;
		}
		if (state < State.Ground)
		{
			for (int i = 0; i < Human.all.Count; i++)
			{
				...
				Vector3 vector = center - human.transform.position;
				Vector3 vector2 = avgSpeed - human.ragdoll.partBall.rigidbody.velocity;
				if (vector.magnitude > 1f)
				{
					human.ragdoll.partChest.rigidbody.SafeAddForce(vector * 40f + vector2 * 40f);
				}
			}
		}
		if (state < State.Ground)
		{
			float num = lowHuman.transform.position.y - startupDoorY;
	...
	private void Update()
	{
		...
		for (int i = 0; i < Human.all.Count; i++)
		{
			...
			float num = ((!(highHuman == null)) ? (human.transform.position.y - highHuman.transform.position.y - showBehind) : (-1f));
			if (num < 0f)
			{
		...
		if (!NetGame.isClient)
		{
			float num2 = ((!(lowHuman == null)) ? (lowHuman.transform.position.y - startupDoorY) : 1000f);
			if (state == State.Falling && num2 < 20f)
			{
				state = State.NearGround;
			}
			if (state == State.NearGround && num2 < 2f && lowHuman != null && lowHuman.onGround)
			{
				state = State.Ground;
			}
			if (state == State.Ground && num2 < -20f)
			{
				state = State.FadeOut;
			}
		}
	...
	private void AnalyzeHumans(out Human lowHuman, out Human highHuman, out Vector3 center, out Vector3 avgSpeed)
	{
		...
		for (int i = 0; i < Human.all.Count; i++)
		{
			...
			if (human.transform.position.y < lowHuman.transform.position.y)
			{
				lowHuman = human;
			}
			if (human.transform.position.y > highHuman.transform.position.y)
			{
				highHuman = human;
			}
			center += human.transform.position;
	...
	{
		float t = Mathf.InverseLerp(highHuman.transform.position.y - lowHuman.transform.position.y, 0f, 20f) * 0.5f + 0.5f;
		Vector3 vector = Vector3.Lerp(highHuman.transform.position, lowHuman.transform.position, t);
		Vector3 position = transform.position;
		if (!(vector.y - position.y < 1f))
		{
			float num = ((!snap) ? (Mathf.InverseLerp(5f, showAhead, lowHuman.transform.position.y - position.y) * 0.3f) : 1f);
			if (num != 0f)
			{
				position.x = vector.x;
				position.z = vector.z;
				transform.position = Vector3.Lerp(transform.position, position, num);
				Quaternion b = Quaternion.Euler(0f, Mathf.LerpAngle(highHuman.controls.targetYawAngle, lowHuman.controls.targetYawAngle, t), 0f);
				transform.rotation = Quaternion.Lerp(transform.rotation, b, Mathf.Pow(num, 2f));
			}
		}
	}
