...
public class CameraController3 : MonoBehaviour
{
	...
	{
		float result;
		if (!string.IsNullOrEmpty(param) && float.TryParse(param, out result))
		{
	...
	private Vector3 SmoothCamera(Vector3 target, float deltaTime)
	{
		...
		Vector3 vector = (target - oldTarget) / deltaTime;
		for (int i = 0; i < num; i++)
		{
			oldTarget += vector * num2;
			Vector3 vector2 = oldTarget + Vector3.ClampMagnitude(smoothTarget - oldTarget, num3);
			float num4 = Mathf.SmoothStep(0.05f, 2f, (vector2 - oldTarget).magnitude / num3);
			smoothTarget = Vector3.MoveTowards(vector2, oldTarget, num2 * num4);
		}
		deltaTime = 0f;
		return smoothTarget;
	}
	...
	public void LateUpdate()
	{
		...
		RaycastHit hitInfo;
		if (num2 < offset && !Physics.SphereCast(vector2 - vector * offset, num * 2f, vector, out hitInfo, offset - num2, wallLayers, QueryTriggerInteraction.Ignore))
		{
			offset = num2;
			offsetSpeed = 0f;
		}
	...
	private void CalculateFarCam(out Vector3 targetOffset, out float yaw, out float pitch, out float camDist, out float minDist, out float fov, out float nearClip)
	{
		...
		Quaternion quaternion = Quaternion.Euler(pitch, yaw, 0f);
		targetOffset = quaternion * farTargetOffset;
	...
	private void CalculateCloseCam(out Vector3 targetOffset, out float yaw, out float pitch, out float camDist, out float minDist, out float fov, out float nearClip)
	{
		...
		Quaternion quaternion = Quaternion.Euler(pitch, yaw, 0f);
		targetOffset = quaternion * closeTargetOffset;
	...
	private void CalculateFirstPersonCam(out Vector3 targetOffset, out float yaw, out float pitch, out float camDist, out float minDist, out float fov, out float nearClip)
	{
		...
		Quaternion quaternion = Quaternion.Euler(pitch, yaw, 0f);
		targetOffset = quaternion * fpsTargetOffset;
		camDist = 0f;
		minDist = 0f;
	}
	...
	{
		Vector3 vector = lookRot * Vector3.forward;
		float nearClipPlane = gameCam.nearClipPlane;
		Vector3 position = base.transform.position;
		Quaternion rotation = base.transform.rotation;
		base.transform.rotation = lookRot;
		base.transform.position = targetPos - vector * (gameCam.nearClipPlane + minDist);
		...
		{
			Ray ray = new Ray(rayStarts[i], -vector);
			RaycastHit hitInfo;
			if (Physics.Raycast(ray, out hitInfo, num, wallLayers) && hitInfo.distance < num)
			{
	...
	public void ApplyCamera(Vector3 target, Vector3 position, Quaternion rotation, float fov)
	{
		...
		else
		{
			...
			Vector3 vector = gameCam.ViewportToWorldPoint(position2);
			base.transform.position += human.transform.position - vector;
		}
