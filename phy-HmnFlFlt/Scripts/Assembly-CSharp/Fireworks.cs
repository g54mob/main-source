...
public class Fireworks : NetScope
{
	...
	private void SyncWeapons(bool fire)
	{
		...
		while (weapons.Count < num)
		{
			...
			fireworksWeapon.gameObject.SetActive(true);
		...
		for (int i = 0; i < num; i++)
		{
			...
			if (human.targetDirection == Vector3.zero || human.targetDirection == new Vector3(0f, 0f, 1f))
			{
				fireworksWeapon2.Place(human.ragdoll.partRightHand.transform.position, Quaternion.LookRotation(-human.ragdoll.partRightHand.transform.up));
			}
			else
			{
			...
			if (fire)
			{
				...
				human.controls.rightExtend = (rightGrab ? 1f : ((human.state != HumanState.Climb && human.state != HumanState.Idle && human.state != HumanState.Jump && human.state != HumanState.Slide && human.state != HumanState.Walk) ? 0f : 0.95f));
