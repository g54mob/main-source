...
public class PaintPicker : MonoBehaviour
{
	...
	private IEnumerator PickColor()
	{
		...
		if (readPixTex == null)
		{
			readPixTex = new Texture2D(width, height, TextureFormat.RGB24, false);
		}
		readPixTex.ReadPixels(new Rect(Input.mousePosition.x, Input.mousePosition.y, width, height), 0, 0);
		readPixTex.Apply();
	...
	public void Process(bool show, bool pick)
	{
		...
		if (allowPicking != pick)
		{
			...
			if (allowPicking)
			{
				MenuCameraEffects.SuspendEffects(true);
			...
			{
				MenuCameraEffects.SuspendEffects(false);
