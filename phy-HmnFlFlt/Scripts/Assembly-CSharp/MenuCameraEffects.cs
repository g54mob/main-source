...
public class MenuCameraEffects : MonoBehaviour, IDependency
{
	...
	{
		None = 0,
		ScalableAmbientObscurance = 1,
		MultiScaleVolumetricObscurance = 2
	}
	...
	private Color creditsSkyboxColor = new Color(0.4745098f, 25f / 51f, 0.49803922f);

	public bool CreditsAdjustInTransition
	{
		get
		{
			return credits.current != credits.to;
		}
	}
	...
	private static GameCamera WrapCamera(Camera camera)
	{
		...
		if (componentInParent != null)
		{
			...
			gameObject.transform.SetParent(postProcessLayer.volumeTrigger, false);
			gameObject.layer = 30;
	...
	{
		Camera component = UnityEngine.Object.Instantiate(Game.instance.cameraPrefab.gameObject, parent, false).GetComponent<Camera>();
	...
	public Camera OverrideCamera(Transform parent, bool applyEffects)
	{
		...
		if (component == null)
		{
			component = UnityEngine.Object.Instantiate(Game.instance.cameraPrefab.gameObject, parent, false).GetComponent<Camera>();
		...
		for (int i = 0; i < gameCameras.Count; i++)
		{
			gameCameras[i].camera.gameObject.SetActive(false);
		}
		activeCameras.Clear();
		if (applyEffects)
		{
	...
	public void RemoveOverride()
	{
		...
		for (int i = 0; i < gameCameras.Count; i++)
		{
			gameCameras[i].camera.gameObject.SetActive(true);
		}
		activeCameras.Clear();
	...
	private void ApplyEffect<T>(bool enable) where T : PostProcessEffectSettings
	{
		...
		{
			T outSetting;
			if (profiles[i].TryGetSettings<T>(out outSetting))
			{
	...
	private void ApplyOcclusion(AmbientOcclusionMode occlusion)
	{
		for (int i = 0; i < profiles.Length; i++)
		{
			AmbientOcclusion outSetting;
			if (profiles[i].TryGetSettings<AmbientOcclusion>(out outSetting))
			{
	...
	public static void SuspendEffects(bool suspend)
	{
		...
		if (instance.postProfile.TryGetSettings<ColorGrading>(out outSetting))
		{
	...
	private void ApplyVignette()
	{
		...
		{
			Vignette outSetting;
			if (profiles[i].TryGetSettings<Vignette>(out outSetting))
			{
	...
	{
		ColorGrading outSetting;
		if (!(instance == null) && instance.postProfile.TryGetSettings<ColorGrading>(out outSetting))
		{
	...
	{
		GameCamera gameCamera = instance.FindCamera(cam);
		if (gameCamera != null)
		{
			gameCamera.depthOfField.focusDistance.Override(value);
		}
	}
}
