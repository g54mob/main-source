...
namespace ProGrids
{
	public static class pg_Util
	{
		...
		{
			SnapEnabledOverride value;
			if (m_SnapOverrideCache.TryGetValue(t, out value))
			{
			...
			foreach (Component c in components)
			{
				...
				Type type = c.GetType();
				bool value2;
				if (m_NoSnapAttributeTypeCache.TryGetValue(type, out value2))
				{
					if (value2)
					{
				...
				else
				{
					...
					value2 = source.Any((object x) => x != null && x.ToString().Contains("ProGridsNoSnap"));
