...
public class UL_GUI_Examples : MonoBehaviour
{
	...
	private void Start()
	{
		UPGEN_Lighting outSetting;
		if ((bool)volumeProfile && volumeProfile.TryGetSettings<UPGEN_Lighting>(out outSetting))
		{
	...
	private void OnGUI()
	{
		...
		if (_fps > 0)
		{
			GUILayout.Label(string.Format("FPS: <b>{0}</b>", _fps), GUI.skin.box);
		}
		GUILayout.EndArea();
		OnGUI_Tools();
		OnGUI_Scene();
	...
	private void OnGUI_Tools()
	{
		...
		{
			if (animationKind != AnimationKind.AnimateX)
			{
	...
	private void OnGUI_Scene()
	{
		if (!string.IsNullOrEmpty(description))
		{
			...
			GUILayout.Label(string.Format("<size=24><b>{0}</b></size>", activeScene.name), GUI.skin.box);
