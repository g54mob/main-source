...
namespace MoonSharp.Interpreter.Tree
{
	internal class Lexer
	{
		...
		private Token ReadToken()
		{
			...
			case '\0':
			{
				SyntaxErrorException ex = new SyntaxErrorException(CreateToken(TokenType.Invalid, line, col), "unexpected symbol near '{0}'", CursorChar());
				ex.IsPrematureStreamTermination = true;
				throw ex;
			}
			default:
				...
				if (char.IsDigit(c))
				{
					return ReadNumberToken(line, col);
				}
		...
		private string ReadLongString(int fromLine, int fromCol, string startpattern, string subtypeforerrors)
		{
			...
			if (startpattern == null)
			{
				...
				while (true)
				{
					if (c == '\0' || !CursorNotEof())
					{
						SyntaxErrorException ex = new SyntaxErrorException(CreateToken(TokenType.Invalid, fromLine, fromCol), "unfinished long {0} near '<eof>'", subtypeforerrors);
						ex.IsPrematureStreamTermination = true;
						throw ex;
					}
					switch (c)
					...
					default:
					{
						SyntaxErrorException ex2 = new SyntaxErrorException(CreateToken(TokenType.Invalid, fromLine, fromCol), "invalid long {0} delimiter near '{1}'", subtypeforerrors, c);
						ex2.IsPrematureStreamTermination = true;
						throw ex2;
					}
					}
					break;
			...
			while (true)
			{
				if (c2 != '\r')
				{
					...
					{
						SyntaxErrorException ex3 = new SyntaxErrorException(CreateToken(TokenType.Invalid, fromLine, fromCol), "unfinished long {0} near '{1}'", subtypeforerrors, stringBuilder.ToString());
						ex3.IsPrematureStreamTermination = true;
						throw ex3;
					}
		...
		private Token ReadNumberToken(int fromLine, int fromCol)
		{
			...
			return CreateToken(tokenType, fromLine, fromCol, stringBuilder.ToString());
		}
		...
		private Token ReadSimpleStringToken(int fromLine, int fromCol)
		{
			...
			SyntaxErrorException ex = new SyntaxErrorException(CreateToken(TokenType.Invalid, fromLine, fromCol), "unfinished string near '{0}'", stringBuilder.ToString());
			ex.IsPrematureStreamTermination = true;
			throw ex;
		}
		...
		{
			Token token = new Token(tokenType, m_SourceId, fromLine, fromCol, m_Line, m_Col, m_PrevLineTo, m_PrevColTo);
			token.Text = text;
			Token result = token;
			m_PrevLineTo = m_Line;
			m_PrevColTo = m_Col;
