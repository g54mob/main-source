	...
	internal class FunctionDefinitionExpression : Expression, IClosureBuilder
	{
			...
			: base(lcontext)
		{
			...
			if (globalContext != null)
			{
				...
				m_Env = lcontext.Scope.TryDefineLocal("_ENV");
			}
			else
			{
		...
		private Statement CreateBody(ScriptLoadingContext lcontext)
		{
			...
			{
				SyntaxErrorException ex = new SyntaxErrorException(lcontext.Lexer.Current, "'end' expected near '{0}'", lcontext.Lexer.Current.Text);
				ex.IsPrematureStreamTermination = lcontext.Lexer.Current.Type == TokenType.Eof;
				throw ex;
			}
		...
		public int CompileBody(ByteCode bc, string friendlyName)
		{
			...
			Instruction instruction2 = bc.Emit_FuncMeta(funcName);
