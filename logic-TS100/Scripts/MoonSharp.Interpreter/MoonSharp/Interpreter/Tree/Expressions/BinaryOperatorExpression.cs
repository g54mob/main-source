...
namespace MoonSharp.Interpreter.Tree.Expressions
{
	internal class BinaryOperatorExpression : Expression
	{
		...
		public static void AddOperatorToChain(object chain, Token op)
		{
			LinkedList list = (LinkedList)chain;
			Node node = new Node();
			node.Op = ParseBinaryOperator(op);
			Node node2 = node;
			AddNode(list, node2);
		}
		...
		private static Expression CreateSubTree(LinkedList list, ScriptLoadingContext lcontext)
		{
			...
			if ((operatorMask & Operator.Power) != Operator.NotAnOperator)
			{
				node = PrioritizeRightAssociative(node, lcontext, Operator.Power);
			}
			if ((operatorMask & (Operator.Mul | Operator.Div | Operator.Mod)) != Operator.NotAnOperator)
			{
				node = PrioritizeLeftAssociative(node, lcontext, Operator.Mul | Operator.Div | Operator.Mod);
			}
			if ((operatorMask & (Operator.Add | Operator.Sub)) != Operator.NotAnOperator)
			{
				node = PrioritizeLeftAssociative(node, lcontext, Operator.Add | Operator.Sub);
			}
			if ((operatorMask & Operator.StrConcat) != Operator.NotAnOperator)
			{
				node = PrioritizeRightAssociative(node, lcontext, Operator.StrConcat);
			}
			if ((operatorMask & (Operator.Less | Operator.Greater | Operator.LessOrEqual | Operator.GreaterOrEqual | Operator.NotEqual | Operator.Equal)) != Operator.NotAnOperator)
			{
			...
			if ((operatorMask & Operator.And) != Operator.NotAnOperator)
			{
				node = PrioritizeLeftAssociative(node, lcontext, Operator.And);
			}
			if ((operatorMask & Operator.Or) != Operator.NotAnOperator)
			{
		...
		private static Node PrioritizeLeftAssociative(Node nodes, ScriptLoadingContext lcontext, Operator operatorsToFind)
		{
			...
			{
				Operator op = node.Op;
				if ((op & operatorsToFind) != Operator.NotAnOperator)
				{
		...
		public override DynValue Eval(ScriptExecutionContext context)
		{
			...
			if ((m_Operator & (Operator.Less | Operator.Greater | Operator.LessOrEqual | Operator.GreaterOrEqual | Operator.NotEqual | Operator.Equal)) != Operator.NotAnOperator)
			{
