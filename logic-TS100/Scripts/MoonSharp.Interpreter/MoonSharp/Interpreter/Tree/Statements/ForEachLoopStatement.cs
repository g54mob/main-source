...
namespace MoonSharp.Interpreter.Tree.Statements
{
	internal class ForEachLoopStatement : Statement
	{
			...
			: base(lcontext)
		{
			...
			m_Names = list.Select((string n) => lcontext.Scope.DefineLocal(n)).ToArray();
		...
		public override void Compile(ByteCode bc)
		{
			...
			Loop loop = new Loop();
			loop.Scope = m_StackFrame;
			Loop loop2 = loop;
			bc.LoopTracker.Loops.Push(loop2);
			...
			bc.Emit_Pop();
			foreach (Instruction breakJump in loop2.BreakJumps)
			{
