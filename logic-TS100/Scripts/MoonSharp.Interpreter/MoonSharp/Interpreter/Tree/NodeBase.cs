...
namespace MoonSharp.Interpreter.Tree
{
	internal abstract class NodeBase
	{
		...
		{
			SyntaxErrorException ex = new SyntaxErrorException(t, "unexpected symbol near '{0}'", t.Text);
			ex.IsPrematureStreamTermination = t.Type == TokenType.Eof;
			throw ex;
		}
		...
		protected static Token CheckMatch(ScriptLoadingContext lcontext, Token originalToken, TokenType expectedTokenType, string expectedTokenText)
		{
			...
			{
				SyntaxErrorException ex = new SyntaxErrorException(lcontext.Lexer.Current, "'{0}' expected (to close '{1}' at line {2}) near '{3}'", expectedTokenText, originalToken.Text, originalToken.FromLine, current.Text);
				ex.IsPrematureStreamTermination = current.Type == TokenType.Eof;
				throw ex;
			}
			lcontext.Lexer.Next();
			return current;
