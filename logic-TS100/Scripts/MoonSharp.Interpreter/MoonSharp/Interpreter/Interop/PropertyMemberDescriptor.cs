...
namespace MoonSharp.Interpreter.Interop
{
	public class PropertyMemberDescriptor : IMemberDescriptor, IOptimizableDescriptor
	{
		...
		public MemberDescriptorAccess MemberAccess
		{
			get
			{
				MemberDescriptorAccess memberDescriptorAccess = MemberDescriptorAccess.CanRead;
				if (m_Setter != null)
				{
				...
				if (m_Getter != null)
				{
					memberDescriptorAccess = memberDescriptorAccess;
				}
			...
			using (PerformanceStatistics.StartGlobalStopwatch(PerformanceCounter.AdaptersCompilation))
			{
				if (m_Getter != null)
				{
					if (IsStatic)
					{
						...
						MemberExpression expression = Expression.Property(null, PropertyInfo);
					...
					else
					{
						...
						UnaryExpression expression3 = Expression.Convert(parameterExpression2, PropertyInfo.DeclaringType);
						MemberExpression expression4 = Expression.Property(expression3, PropertyInfo);
			...
			using (PerformanceStatistics.StartGlobalStopwatch(PerformanceCounter.AdaptersCompilation))
			{
				if (m_Setter != null && !PropertyInfo.DeclaringType.IsValueType)
				{
					...
					if (IsStatic)
					{
						...
						UnaryExpression unaryExpression = Expression.Convert(parameterExpression2, PropertyInfo.PropertyType);
						MethodCallExpression body = Expression.Call(setMethod, unaryExpression);
					...
					else
					{
						...
						UnaryExpression unaryExpression2 = Expression.Convert(parameterExpression4, PropertyInfo.PropertyType);
						MethodCallExpression body2 = Expression.Call(instance, setMethod, unaryExpression2);
						Expression<Action<object, object>> expression2 = Expression.Lambda<Action<object, object>>(body2, new ParameterExpression[2] { parameterExpression3, parameterExpression4 });
		...
		public void SetValue(Script script, object obj, DynValue v)
		{
			...
			object obj2 = ScriptToClrConversions.DynValueToObjectOfType(v, PropertyInfo.PropertyType, null, false);
			try
			{
				if (obj2 is double)
				{
