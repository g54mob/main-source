...
namespace MoonSharp.Interpreter.Interop
{
	public abstract class FunctionMemberDescriptorBase : IOverloadableMemberDescriptor, IMemberDescriptor
	{
		...
		{
			MethodMemberDescriptor methodMemberDescriptor = new MethodMemberDescriptor(mi);
			return methodMemberDescriptor.GetCallbackAsDynValue(script, obj);
		}
			...
			for (int i = 0; i < array.Length; i++)
			{
				...
				else if (i == parameters.Length - 1 && VarArgsArrayType != null)
				{
					...
					if (list.Count == 1)
					{
						...
						if (dynValue2.Type == DataType.UserData && dynValue2.UserData.Object != null && VarArgsArrayType.IsAssignableFrom(dynValue2.UserData.Object.GetType()))
						{
