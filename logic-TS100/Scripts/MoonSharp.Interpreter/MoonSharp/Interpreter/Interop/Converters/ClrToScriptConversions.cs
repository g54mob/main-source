...
namespace MoonSharp.Interpreter.Interop.Converters
{
	internal static class ClrToScriptConversions
	{
		internal static DynValue TryObjectToSimpleDynValue(Script script, object obj)
		{
			...
			Func<object, DynValue> clrToScriptCustomConversion = Script.GlobalOptions.CustomConverters.GetClrToScriptCustomConversion(obj.GetType());
			if (clrToScriptCustomConversion != null)
			{
				DynValue dynValue = clrToScriptCustomConversion(obj);
				if (dynValue != null)
				{
					return dynValue;
				}
			...
			if (obj is Delegate)
			{
				...
				MethodInfo method = obj2.Method;
				if (CallbackFunction.CheckCallbackSignature(method, false))
				{
					return DynValue.NewCallback((Func<ScriptExecutionContext, CallbackArguments, DynValue>)obj2);
				}
			}
			return null;
		}
		...
		internal static DynValue ObjectToDynValue(Script script, object obj)
		{
			...
			if (obj is IList)
			{
				Table table = TableConversions.ConvertIListToTable(script, (IList)obj);
				return DynValue.NewTable(table);
			}
			if (obj is IDictionary)
			{
				Table table2 = TableConversions.ConvertIDictionaryToTable(script, (IDictionary)obj);
