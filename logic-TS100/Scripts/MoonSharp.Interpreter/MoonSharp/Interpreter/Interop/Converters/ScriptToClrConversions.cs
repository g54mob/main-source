	...
	internal static class ScriptToClrConversions
	{
		...
		internal static object DynValueToObjectOfType(DynValue value, Type desiredType, object defaultValue, bool isOptional)
		{
			...
			case DataType.Void:
				...
				if (!desiredType.IsValueType || type != null)
				{
					return null;
				}
				break;
			...
			case DataType.Number:
				...
				{
					Type underlyingType2 = Enum.GetUnderlyingType(desiredType);
					return NumericConversions.DoubleToType(underlyingType2, value.Number);
				}
			...
			case DataType.UserData:
				if (value.UserData.Object != null)
				{
					object obj3 = value.UserData.Object;
					IUserDataDescriptor descriptor = value.UserData.Descriptor;
					if (descriptor.IsTypeCompatible(desiredType, obj3))
					{
						return obj3;
					}
					if (stringSubtype != StringConversions.StringSubtype.None)
					{
						text = descriptor.AsString(obj3);
					}
				}
				break;
			case DataType.Table:
		...
		internal static int DynValueToObjectOfTypeWeight(DynValue value, Type desiredType, bool isOptional)
		{
			...
			case DataType.Number:
				if (desiredType.IsEnum)
				{
					Enum.GetUnderlyingType(desiredType);
					return 90;
				}
				if (NumericConversions.NumericTypes.Contains(desiredType))
				{
			...
			case DataType.UserData:
				if (value.UserData.Object != null)
				{
					object obj = value.UserData.Object;
					IUserDataDescriptor descriptor = value.UserData.Descriptor;
					if (descriptor.IsTypeCompatible(desiredType, obj))
					{
						return 100;
					}
					if (stringSubtype != StringConversions.StringSubtype.None)
