...
namespace MoonSharp.Interpreter.Interop
{
	public class StandardEnumUserDataDescriptor : DispatchingUserDataDescriptor
	{
		...
		{
			string[] names = Enum.GetNames(base.Type);
			Array values = Enum.GetValues(base.Type);
			for (int i = 0; i < names.Length; i++)
			{
				string name = names[i];
				object value = values.GetValue(i);
				DynValue value2 = UserData.Create(value, this);
				AddDynValue(name, value2);
			}
			object[] customAttributes = base.Type.GetCustomAttributes(typeof(FlagsAttribute), true);
