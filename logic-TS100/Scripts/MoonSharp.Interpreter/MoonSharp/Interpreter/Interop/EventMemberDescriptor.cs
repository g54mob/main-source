...
namespace MoonSharp.Interpreter.Interop
{
	public class EventMemberDescriptor : IMemberDescriptor
	{
		...
		{
			private EventMemberDescriptor m_Parent;
			...
			public DynValue Index(Script script, DynValue index, bool isDirectIndexing)
			{
				if (index.Type == DataType.String)
				{
					if (index.String == "add")
					{
						return DynValue.NewCallback((ScriptExecutionContext c, CallbackArguments a) => m_Parent.AddCallback(m_Object, c, a));
					}
					if (index.String == "remove")
					{
						return DynValue.NewCallback((ScriptExecutionContext c, CallbackArguments a) => m_Parent.RemoveCallback(m_Object, c, a));
					}
				}
		...
		public static EventMemberDescriptor TryCreateIfVisible(EventInfo ei, InteropAccessMode accessMode)
		{
			...
			{
				if (!ei.GetAddMethod().IsPublic)
				{
					goto IL_0049;
				}
				num = ei.GetRemoveMethod().IsPublic;
			}
			else
			{
				num = visibilityFromAttributes == true;
			}
			if (num)
			{
		...
		public static bool CheckEventIsCompatible(EventInfo ei, bool throwException)
		{
			...
			MethodInfo method = ei.EventHandlerType.GetMethod("Invoke");
			if (method == null)
			{
				if (throwException)
				{
		...
		private void RegisterCallback(object o)
		{
			...
			{
				Delegate obj = CreateDelegate(o);
				Delegate obj2 = Delegate.CreateDelegate(EventInfo.EventHandlerType, obj.Target, obj.Method);
				m_Add.Invoke(o, new object[1] { obj2 });
				return obj2;
			});
		}
