...
namespace MoonSharp.Interpreter.Interop
{
	public class OverloadedMethodMemberDescriptor : IOptimizableDescriptor, IMemberDescriptor
	{
		...
		public bool IsStatic
		{
			get
			{
				return m_Overloads.Any((IOverloadableMemberDescriptor o) => o.IsStatic);
			}
		}
		...
		private DynValue PerformOverloadedCall(Script script, object obj, ScriptExecutionContext context, CallbackArguments args)
		{
			...
			for (int j = 0; j < m_Overloads.Count; j++)
			{
				...
				{
					int num2 = CalcScoreForOverload(context, args, m_Overloads[j], false);
					if (num2 > num)
					{
						num = num2;
			...
			if (!IgnoreExtensionMethods && obj != null)
			{
				if (!flag)
				{
					...
					m_ExtOverloads = (from d in UserData.GetExtensionMethodsByName(Name)
						where d.ExtensionMethodType != null && d.ExtensionMethodType.IsAssignableFrom(DeclaringType)
						select d).Cast<IOverloadableMemberDescriptor>().ToList();
				}
				for (int num3 = 0; num3 < m_ExtOverloads.Count; num3++)
				{
					int num4 = CalcScoreForOverload(context, args, m_ExtOverloads[num3], true);
					if (num4 > num)
					{
						num = num4;
						overloadableMemberDescriptor = m_ExtOverloads[num3];
					}
				}
			}
		...
		private void Cache(bool hasObject, CallbackArguments args, IOverloadableMemberDescriptor bestOverload)
		{
			...
			for (int i = 0; i < m_Cache.Length; i++)
			{
				if (m_Cache[i] == null)
				{
					OverloadCacheItem overloadCacheItem2 = new OverloadCacheItem();
					overloadCacheItem2.ArgsDataType = new List<DataType>();
					overloadCacheItem2.ArgsUserDataType = new List<Type>();
					overloadCacheItem = overloadCacheItem2;
			...
			if (overloadCacheItem == null)
			{
				...
				OverloadCacheItem overloadCacheItem3 = new OverloadCacheItem();
				overloadCacheItem3.ArgsDataType = new List<DataType>();
				overloadCacheItem3.ArgsUserDataType = new List<Type>();
				overloadCacheItem = overloadCacheItem3;
			...
			for (int i = 0; i < method.Parameters.Length; i++)
			{
				...
				if (i == method.Parameters.Length - 1 && method.VarArgsArrayType != null)
				{
					...
					while (true)
					{
						...
						num3++;
						num4++;
						int val = CalcScoreForSingleArgument(method.VarArgsElementType, dynValue2, false);
						num = Math.Min(num, val);
					}
					if (num4 == 1 && dynValue.Type == DataType.UserData && dynValue.UserData.Object != null && method.VarArgsArrayType.IsAssignableFrom(dynValue.UserData.Object.GetType()))
					{
						num = num5;
						continue;
					}
				...
				else
				{
					...
					int val2 = CalcScoreForSingleArgument(type, arg, method.Parameters[i].HasDefaultValue);
