...
namespace MoonSharp.Interpreter
{
	public class CallbackArguments
	{
		...
		{
			DynValue dynValue = AsType(argNum, funcName, DataType.Number);
			double number = dynValue.Number;
			return (int)number;
		}
		...
		public CallbackArguments SkipMethodCall()
		{
			if (IsMethodCall)
			{
				Slice<DynValue> args = new Slice<DynValue>(m_Args, 1, m_Args.Count - 1, false);
				return new CallbackArguments(args, false);
			}
			return this;
		}
	}
