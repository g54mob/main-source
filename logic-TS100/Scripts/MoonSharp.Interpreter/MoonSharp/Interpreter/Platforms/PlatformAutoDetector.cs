...
namespace MoonSharp.Interpreter.Platforms
{
	public static class PlatformAutoDetector
	{
		...
		public static bool IsRunningOnAOT
		{
			get
			{
				if (!m_IsRunningOnAOT.HasValue)
				{
					try
					{
						Expression body = Expression.Constant(5, typeof(int));
						Expression<Func<int>> expression = Expression.Lambda<Func<int>>(body, new ParameterExpression[0]);
						expression.Compile();
		...
		private static void AutoDetectPlatformFlags()
		{
			...
			{
				IsRunningOnUnity = AppDomain.CurrentDomain.GetAssemblies().SelectMany((Assembly a) => a.GetTypes()).Any((Type t) => t.FullName.StartsWith("UnityEngine."));
