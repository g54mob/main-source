...
namespace MoonSharp.Interpreter.Execution.VM
{
	internal sealed class Processor
	{
		...
		private const int DUMP_CHUNK_VERSION = 2306;
		...
		internal int Dump(Stream stream, int baseAddress, bool hasUpvalues)
		{
			using (BinaryWriter binaryWriter = new BinDumpBinaryWriter(stream, Encoding.UTF8))
			{
				...
				SymbolRef[] array2 = dictionary.Keys.ToArray();
				foreach (SymbolRef symbolRef in array2)
				{
				...
				{
					array3[item.Value] = item.Key;
				}
				binaryWriter.Write(dictionary.Count);
				SymbolRef[] array4 = array3;
				foreach (SymbolRef symbolRef2 in array4)
				{
					symbolRef2.WriteBinary(binaryWriter);
				}
				SymbolRef[] array5 = array3;
				foreach (SymbolRef symbolRef3 in array5)
				{
					symbolRef3.WriteBinaryEnv(binaryWriter, dictionary);
				}
				for (int n = 0; n <= instruction.NumVal; n++)
				{
					m_RootChunk.Code[baseAddress + n].WriteBinary(binaryWriter, baseAddress, dictionary);
				}
		...
		internal int Undump(Stream stream, int sourceID, Table envTable, out bool hasUpvalues)
		{
			...
			using (BinaryReader binaryReader = new BinDumpBinaryReader(stream, Encoding.UTF8))
			{
				...
				SymbolRef[] array = new SymbolRef[num4];
		...
		public DynValue Coroutine_Resume(DynValue[] args)
		{
			...
			try
			{
				...
				{
					num = PushClrToScriptStackFrame(CallStackItemFlags.ResumeEntryPoint, null, args);
				}
				else
				{
					m_ValueStack.Push(DynValue.NewTuple(args));
					num = m_SavedInstructionPtr;
				}
				m_State = CoroutineState.Running;
				DynValue dynValue = Processing_Loop(num);
		...
		private void EnterProcessor()
		{
			...
			if (m_OwningThreadID >= 0 && m_OwningThreadID != managedThreadId && m_Script.Options.CheckThreadAccess)
			{
				string message = string.Format("Cannot enter the same MoonSharp processor from two different threads : {0} and {1}", m_OwningThreadID, managedThreadId);
				throw new InvalidOperationException(message);
			}
			m_OwningThreadID = managedThreadId;
			m_ExecutionNesting++;
			if (m_Parent != null)
			{
		...
		{
			if (m_Debug.DebuggerAttached.IsPauseRequested() || (instr.SourceCodeRef != null && instr.SourceCodeRef.Breakpoint && instr.SourceCodeRef != m_Debug.LastHlRef))
			{
		...
		private WatchItem Debugger_RefreshWatch(ScriptExecutionContext context, DynamicExpression dynExpr)
		{
			try
			{
				...
				watchItem.IsError = dynExpr.IsConstant();
				watchItem.LValue = lValue;
				watchItem.Value = value;
				watchItem.Name = dynExpr.ExpressionCode;
				return watchItem;
			}
			catch (Exception ex)
			{
				WatchItem watchItem2 = new WatchItem();
				watchItem2.IsError = true;
				watchItem2.Value = DynValue.NewString(ex.Message);
				watchItem2.Name = dynExpr.ExpressionCode;
				return watchItem2;
			}
		}
		...
		internal List<WatchItem> Debugger_GetCallStack(SourceRef startingRef)
		{
			...
			for (int i = 0; i < m_ExecutionStack.Count; i++)
			{
				...
				string name = ((instruction.OpCode == OpCode.FuncMeta) ? instruction.Name : null);
			...
			while (true)
			{
				try
				{
					while (true)
					{
						...
						case OpCode.Jf:
							instructionPtr = JumpBool(instruction, false, instructionPtr);
							if (instructionPtr != -99)
							{
								continue;
							}
						...
						case OpCode.ExpTuple:
							...
							continue;
						case OpCode.Local:
							m_ValueStack.Push(m_ExecutionStack.Peek().LocalScope[instruction.Symbol.i_Index].AsReadOnly());
							continue;
						case OpCode.Upvalue:
		...
		{
			Slice<DynValue> values = new Slice<DynValue>(m_ValueStack, m_ValueStack.Count - i.NumVal, i.NumVal, false);
		...
		private void ExecIterUpd(Instruction i)
		{
			...
			DynValue dynValue2 = m_ValueStack.Peek(1);
			dynValue2.Tuple[2] = dynValue;
		}
		...
		{
			int num = (int)m_ValueStack.Peek().Number;
			for (int i = 0; i < I.SymbolList.Length; i++)
			{
				...
				{
					int num2 = num - i;
					DynValue[] array = new DynValue[num2];
					for (int j = 0; j < num2; j++)
					{
						array[j] = m_ValueStack.Peek(num - i - j).CloneAsWritable();
					}
					AssignLocal(I.SymbolList[i], DynValue.NewTuple(Internal_AdjustTuple(array)));
				}
				else
				{
					AssignLocal(I.SymbolList[i], m_ValueStack.Peek(num - i).CloneAsWritable());
				}
			}
		}
		...
		private int PerformTCO(int instructionPtr, int argsCount)
		{
			...
			CallStackItem callStackItem = PopToBasePointer();
			int returnAddress = callStackItem.ReturnAddress;
			int num = (int)m_ValueStack.Pop().Number;
			m_ValueStack.RemoveLast(num + 1);
		...
		private int Internal_CheckForTailRequests(Instruction i, int instructionPtr)
		{
			...
			if (dynValue.Type == DataType.TailCallRequest)
			{
				...
				return Internal_ExecCall(tailCallData.Args.Length, instructionPtr, tailCallData.ErrorHandler, tailCallData.Continuation, false, null, tailCallData.ErrorHandlerBeforeUnwind);
			}
			if (dynValue.Type == DataType.YieldRequest)
			{
		...
		{
			DynValue dynValue = m_ValueStack.Pop().ToScalar();
			if (dynValue.CastToBool() == expectedValueForJump)
			{
				return i.NumVal;
			}
			return instructionPtr;
		...
		private int ExecShortCircuitingOperator(Instruction i, int instructionPtr)
		{
			...
			DynValue dynValue = m_ValueStack.Peek().ToScalar();
			if (dynValue.CastToBool() == flag)
			{
				return i.NumVal;
			}
			m_ValueStack.Pop();
		...
		private int ExecEq(Instruction i, int instructionPtr)
		{
			DynValue dynValue = m_ValueStack.Pop().ToScalar();
			DynValue dynValue2 = m_ValueStack.Pop().ToScalar();
			if (object.ReferenceEquals(dynValue, dynValue2))
			{
		...
		private DynValue[] Internal_AdjustTuple(IList<DynValue> values)
		{
			...
			{
				int num = values.Count - 1 + values[values.Count - 1].Tuple.Length;
				...
				if (array[array.Length - 1].Type == DataType.Tuple)
				{
