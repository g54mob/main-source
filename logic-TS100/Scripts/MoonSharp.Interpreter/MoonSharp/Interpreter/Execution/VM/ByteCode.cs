...
namespace MoonSharp.Interpreter.Execution.VM
{
	internal class ByteCode : RefIdObject
	{
		...
		{
			Instruction result = AppendInstruction(new Instruction(m_CurrentSourceRef)
			{
				OpCode = opcode
			});
		...
		{
			OpCode opCode = (isNameIndex ? OpCode.IndexN : ((!isExpList) ? OpCode.Index : OpCode.IndexL));
			return AppendInstruction(new Instruction(m_CurrentSourceRef)
			{
				OpCode = opCode,
				Value = index
		...
		{
			OpCode opCode = (isNameIndex ? OpCode.IndexSetN : ((!isExpList) ? OpCode.IndexSet : OpCode.IndexSetL));
			return AppendInstruction(new Instruction(m_CurrentSourceRef)
			{
				OpCode = opCode,
				NumVal = stackofs,
