...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class StringModule
	{
		...
		{
			StringRange stringRange = StringRange.FromLuaRange(vi, vj);
			string text = stringRange.ApplyToString(vs.String);
		...
		{
			DynValue dynValue = args.AsType(0, "len", DataType.String);
			return DynValue.NewNumber(dynValue.String.Length);
		}
		...
		{
			DynValue dynValue = args.AsType(0, "lower", DataType.String);
			return DynValue.NewString(dynValue.String.ToLower());
		}

		[MoonSharpModuleMethod]
		public static DynValue upper(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			DynValue dynValue = args.AsType(0, "upper", DataType.String);
			return DynValue.NewString(dynValue.String.ToUpper());
		}
		...
		public static DynValue sub(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			StringRange stringRange = StringRange.FromLuaRange(start, end, -1);
			string str = stringRange.ApplyToString(dynValue.String);
