...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class DebugModule
	{
		...
		public static DynValue debug(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			replInterpreter.HandleDynamicExprs = false;
			replInterpreter.HandleClassicExprsSyntax = true;
			ReplInterpreter replInterpreter2 = replInterpreter;
			while (true)
			{
				string input = script.Options.DebugInput(replInterpreter2.ClassicPrompt + " ");
				try
				{
					DynValue dynValue = replInterpreter2.Evaluate(input);
