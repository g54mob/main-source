	...
	public class BasicModule
	{
		...
		public static DynValue assert(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			if (!dynValue.CastToBool())
			{
				if (dynValue2.IsNil())
				{
					ScriptRuntimeException ex = new ScriptRuntimeException("assertion failed!");
					ex.DoNotDecorateMessage = true;
					throw ex;
				}
				ScriptRuntimeException ex2 = new ScriptRuntimeException(dynValue2.ToPrintString());
				ex2.DoNotDecorateMessage = true;
				throw ex2;
			}
		...
		public static DynValue error(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			...
			ScriptRuntimeException ex = new ScriptRuntimeException(dynValue.String);
			ex.DoNotDecorateMessage = true;
			throw ex;
		}
