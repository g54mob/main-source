...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	{
		private static readonly uint[] BITS = new uint[32]
		{
		...
		private static uint ToUInt32(DynValue v)
		{
			...
			number = Math.IEEERemainder(number, Math.Pow(2.0, 32.0));
			return (uint)number;
		}
		...
		{
			DynValue v = args.AsType(0, "extract", DataType.Number);
			uint num = ToUInt32(v);
			...
			uint num4 = (num >> num2) & NBitMask(num3);
		...
		{
			DynValue v = args.AsType(0, "replace", DataType.Number);
			uint num = ToUInt32(v);
			DynValue v2 = args.AsType(1, "replace", DataType.Number);
			uint num2 = ToUInt32(v2);
		...
		{
			DynValue v = args.AsType(0, "arshift", DataType.Number);
			int num = ToInt32(v);
			DynValue dynValue = args.AsType(1, "arshift", DataType.Number);
			int num2 = (int)dynValue.Number;
		...
		{
			DynValue v = args.AsType(0, "rshift", DataType.Number);
			uint num = ToUInt32(v);
			DynValue dynValue = args.AsType(1, "rshift", DataType.Number);
			int num2 = (int)dynValue.Number;
			num = ((num2 >= 0) ? (num >> num2) : (num << -num2));
			return DynValue.NewNumber(num);
		}
		...
		{
			DynValue v = args.AsType(0, "lshift", DataType.Number);
			uint num = ToUInt32(v);
			DynValue dynValue = args.AsType(1, "lshift", DataType.Number);
			int num2 = (int)dynValue.Number;
			num = ((num2 >= 0) ? (num << num2) : (num >> -num2));
			return DynValue.NewNumber(num);
		}
		...
		{
			return DynValue.NewBoolean(0 != Bitwise("btest", args, (uint x, uint y) => x & y));
		}
		...
		{
			DynValue v = args.AsType(0, "bnot", DataType.Number);
			uint num = ToUInt32(v);
			return DynValue.NewNumber(~num);
		}
		...
		{
			DynValue v = args.AsType(0, "lrotate", DataType.Number);
			uint num = ToUInt32(v);
			DynValue dynValue = args.AsType(1, "lrotate", DataType.Number);
			int num2 = (int)dynValue.Number % 32;
		...
		{
			DynValue v = args.AsType(0, "rrotate", DataType.Number);
			uint num = ToUInt32(v);
			DynValue dynValue = args.AsType(1, "rrotate", DataType.Number);
			int num2 = (int)dynValue.Number % 32;
			num = ((num2 >= 0) ? ((num >> num2) | (num << 32 - num2)) : ((num << -num2) | (num >> 32 + num2)));
			return DynValue.NewNumber(num);
		}
