...
namespace MoonSharp.Interpreter.CoreLib
{
	...
	public class ErrorHandlingModule
	{
		...
		private static DynValue SetErrorHandlerStrategy(string funcName, ScriptExecutionContext executionContext, CallbackArguments args, DynValue handlerBeforeUnwind)
		{
			...
			if (args[0].Type == DataType.ClrFunction)
			{
				try
				{
					...
					if (dynValue.Type == DataType.TailCallRequest)
					{
						...
						TailCallData tailCallData = new TailCallData();
			...
			TailCallData tailCallData2 = new TailCallData();
			tailCallData2.Args = array;
			tailCallData2.Function = function;
			tailCallData2.Continuation = new CallbackFunction(pcall_continuation, funcName);
			tailCallData2.ErrorHandler = new CallbackFunction(pcall_onerror, funcName);
			tailCallData2.ErrorHandlerBeforeUnwind = handlerBeforeUnwind;
			return DynValue.NewTailCallReq(tailCallData2);
		}
