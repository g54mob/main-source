	...
	public class DynamicModule
	{
		...
		public static DynValue eval(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			try
			{
				...
				DynamicExpression dynamicExpression = executionContext.GetScript().CreateDynamicExpression(dynValue.String);
				return dynamicExpression.Evaluate(executionContext);
			}
		...
		public static DynValue prepare(ScriptExecutionContext executionContext, CallbackArguments args)
		{
			try
			{
				...
				DynamicExprWrapper dynamicExprWrapper = new DynamicExprWrapper();
				dynamicExprWrapper.Expr = expr;
				return UserData.Create(dynamicExprWrapper);
			}
			catch (SyntaxErrorException ex)
			{
				throw new ScriptRuntimeException(ex);
