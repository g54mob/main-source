...
public class EnemyCrabbino : EnemyController
{
	...
	protected virtual void SetupPincers()
	{
		...
		List<Sprite> animation = SpriteManager.GetAnimation("XLPincer_i", 1, 4, "enemies2", flag);
		bool startRandomFrame = default(bool);
		Action onComplete = default(Action);
		bool autoSetAnimation = default(bool);
		_pincerLAnim.AddAnimation("idle", animation, 8, flag, startRandomFrame, onComplete, autoSetAnimation);
		_pincerRAnim.AddAnimation("idle", animation, 8, flag, startRandomFrame, onComplete, autoSetAnimation);
		PhaserSprite phaserSprite3 = _pincerSpriteL.setFlipX(flipX: false);
		PhaserSprite phaserSprite4 = _pincerSpriteR.setFlipX(flipX: true);
		...
		PhaserSprite phaserSprite5 = _pincerSpriteL.setTint(_saveTint);
		PhaserSprite phaserSprite6 = _pincerSpriteR.setTint(_saveTint);
		UpdatePincerTransforms();
	}
	...
	protected unsafe virtual void UpdatePincerTransforms()
	{
		...
		if ((object)_EnemyRenderer != null)
		{
			...
			if ((object)transform != null)
			{
				...
				if ((object)_pincerSpriteL != null && ((UnityEngine.Object)pincerSpriteL).m_CachedPtr != (IntPtr)0)
				{
					...
					Vector2 leftPincerPos = 0 * LeftOffset;
					_leftPincerPos = leftPincerPos;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v5 @ rsp-3C]");
					nint num = 0;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [this @ rcx (VampireSurvivors.Objects.Characters.Enemies.EnemyCrabbino)+2AC]");
					object obj4 = num * 0;
				...
				if ((object)_pincerSpriteR != null)
				{
					...
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v5 @ rsp-40]");
					Vector2 rightPincerPos = 0 * RightOffset;
					_rightPincerPos = rightPincerPos;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v5 @ rsp-3C]");
					nint num2 = 0;
					Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [this @ rcx (VampireSurvivors.Objects.Characters.Enemies.EnemyCrabbino)+2B4]");
					object obj8 = num2 * 0;
					...
					EnemyCrabbino cachedTransform2 = (EnemyCrabbino)(object)_cachedTransform;
