...
public class Enemy_TP_SpecialUnlock_Sniper : Enemy_TP_SpecialUnlock
{
	...
	protected override void OnKilledBySelectedWeapon()
	{
		...
		int tP_SniperCount = config.TP_SniperCount + 1;
		config.TP_SniperCount = tP_SniperCount;
		...
		if ((nint)obj != -1)
		{
			...
			if (config3.TP_SniperCount > 100)
			{
				GameManager core4 = GM.Core;
				PlayerOptionsData config4 = core4._playerOptions.Config;
				bool flag = core4._playerOptions.UnlockSecret(SecretType.tp_sniper, config4);
				GameManager core5 = GM.Core;
				core5._playerOptions.UnlockCharacter(CharacterType.TP_SNIPER);
			}
		}
	}
}
