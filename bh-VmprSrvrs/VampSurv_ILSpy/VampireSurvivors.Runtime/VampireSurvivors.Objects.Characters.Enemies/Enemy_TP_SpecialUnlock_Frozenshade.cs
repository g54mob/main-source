...
public class Enemy_TP_SpecialUnlock_Frozenshade : Enemy_TP_SpecialUnlock
{
	protected override void Awake()
	{
		...
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v40 @ rax_v3 (System.Collections.Generic.List`1<VampireSurvivors.Data.WeaponType>)+18]");
		nint num2 = 0;
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v71 @ rdx_v7+18]");
		if (num2 >= 0)
		{
	...
	protected override void OnKilledBySelectedWeapon()
	{
		...
		int tP_FrozenShadesCount = config.TP_FrozenShadesCount + 1;
		config.TP_FrozenShadesCount = tP_FrozenShadesCount;
		...
		if ((nint)obj != -1)
		{
			...
			if (config3.TP_FrozenShadesCount > 100)
			{
				GameManager core4 = GM.Core;
				PlayerOptionsData config4 = core4._playerOptions.Config;
				bool flag = core4._playerOptions.UnlockSecret(SecretType.tp_frozenshade, config4);
				GameManager core5 = GM.Core;
				core5._playerOptions.UnlockCharacter(CharacterType.TP_FROZENSHADE);
			}
		}
	}
}
