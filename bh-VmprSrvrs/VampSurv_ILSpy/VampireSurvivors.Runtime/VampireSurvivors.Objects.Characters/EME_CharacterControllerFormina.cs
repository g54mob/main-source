...
public class EME_CharacterControllerFormina : EME_CharacterControllerShowstopper
{
	...
	public override float PDuration()
	{
		...
		value = eggFloat._val + _bonusDuration;
		...
		if ((nint)obj != 2139095040)
		{
			...
			{
				Cpp2ILHelpers.NoteDecompilerIssue("Not implemented instruction: \"jp short 0000000187652836h\"");
				if (num == -1f / 0f)
				{
					num = -3.4028235E+38f;
				}
	...
	{
		float num = ++_techniquesCount * 0.001f;
		bool flag = !(5f > num);
	...
	public override void AfterFullInitialization()
	{
		...
		if (currentCharacterData._003CcurrentSkin_003Ek__BackingField == SkinType.SKIN_EME_SOLO_CANNON || currentCharacterData._003CcurrentSkin_003Ek__BackingField == SkinType.SKIN_EME_SOLO_PISTOL)
		{
			spawnFollowerNextFrame = false;
		}
		if (currentCharacterData._003CcurrentSkin_003Ek__BackingField == SkinType.SKIN_EME_F_PISTOL || currentCharacterData._003CcurrentSkin_003Ek__BackingField == SkinType.SKIN_EME_SOLO_PISTOL)
		{
			bool respectAnimationXPivots = base.RespectAnimationXPivots;
			Vector2 pivot = default(Vector2);
			string text = default(string);
			int num = default(int);
			bool flag = default(bool);
			List<Sprite> animationFrames = SpriteManager.GetAnimationFrames("EME_formina_pistol_r", 1, 4, pivot, text, num, flag);
			Action action = base.OnRangedAComplete;
			bool autoSetAnimation = default(bool);
			_spriteAnimation.AddAnimation("rangedB", animationFrames, 12, (byte)(int)text != 0, (byte)num != 0, (Action)flag, autoSetAnimation);
		}
	}
	...
	protected override void OnUpdate()
	{
		...
		CharacterController characterController = GM.Core.AddFollower(CharacterType.EME_PUNCHKICK, this, AIType.Defensive, manualLevelups, everyXLevels, spawnWithoutAuthority);
		if ((object)characterController == null || ((UnityEngine.Object)characterController).m_CachedPtr == (IntPtr)0)
		{
			return;
		}
		nint num = (nint)typeof(EME_CharacterControllerBonnie);
		nint num2 = (nint)characterController;
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v210 @ rdx_v8 (Il2CppClass<VampireSurvivors.Objects.Characters.EME_CharacterControllerBonnie>)+130]");
		object obj = 0;
		...
		if (num3 >= 0)
		{
			...
			if (0 == (nint)typeof(EME_CharacterControllerBonnie))
			{
				_ = 0;
	...
	{
		if (_rangedAnim != null && !((CharacterController)this)._isAnimForced)
		{
			...
			string animation = Extensions.PickRnd(firingAnims);
			_spriteAnimation.SetAnimation(animation);
			_currentAnimation = CharAnimationType.ranged;
		}
	}

	public EME_CharacterControllerFormina()
	{
		...
		{
			((List<object>)(object)list).AddWithResize((object)"rangedA");
		}
		else
		{
			list._size++;
		...
		{
			((List<object>)(object)list).AddWithResize((object)"rangedB");
		}
		else
		{
			list._size++;
