...
public class CharacterSkillCard_A007Ectoplasm : CharacterSkillCard_Base
{
	public CharacterSkillCard_A007Ectoplasm(ArcanaType type)
		: base(type)
	{
		float num = ((Edition == SkillCardEdition.Poly) ? 2f : ((Edition != SkillCardEdition.Inve) ? 1f : (-0.5f)));
		InitialBonus = new ModifierStats
		{
			_003CRegen_003Ek__BackingField = num,
			_003CArmor_003Ek__BackingField = num * -2f
		};
		AvailableSlots = 3;
	}
