using System;
using System.Collections.Generic;
using Cpp2ILInjected;
using Doozy.Engine.Nody.Models;
using Doozy.Engine.UI.Input;
using Doozy.Engine.Utils;
using UnityEngine;

namespace Doozy.Engine.UI.Nodes;

public class BackButtonNode : Node
{
	public enum BackButtonState
	{
		Disable,
		Enable,
		EnableByForce
	}

	public BackButtonState BackButtonAction = BackButtonState.Enable;

	public override void OnCreate()
	{
		base.m_canBeDeleted = true;
		base.m_nodeType = NodeType.General;
		UILanguagePack instance = UILanguagePack.Instance;
		base.m_name = instance.BackButtonNodeName;
	}

	public override void AddDefaultSockets()
	{
		//IL_001d: Unknown result type (might be due to invalid IL or missing references)
		//IL_0022: Expected O, but got Unknown
		//IL_0058: Unknown result type (might be due to invalid IL or missing references)
		//IL_005d: Expected O, but got Unknown
		Cpp2ILHelpers.NoteDecompilerIssue("Method not found @180B0AC10");
		object obj2 = default(object);
		object obj = obj2 + 32;
		Cpp2ILHelpers.NoteDecompilerIssue("Unknown call target operand: \"il2cpp_vm_reflection_get_type_object\"");
		Type valueType = default(Type);
		bool canBeReordered = default(bool);
		Socket socket = AddInputSocket(ConnectionMode.Multiple, valueType, canBeDeleted: false, canBeReordered);
		Cpp2ILHelpers.NoteDecompilerIssue("Method not found @180B0AC10");
		object obj4 = default(object);
		object obj3 = obj4 + 32;
		Cpp2ILHelpers.NoteDecompilerIssue("Unknown call target operand: \"il2cpp_vm_reflection_get_type_object\"");
		Type valueType2 = default(Type);
		Socket socket2 = AddOutputSocket(ConnectionMode.Override, valueType2, canBeDeleted: false, canBeReordered);
	}

	public override void CopyNode(Node original)
	{
		//IL_00b1: Expected I, but got O
		//IL_000d: Expected I, but got O
		//IL_001d: Expected O, but got I
		//IL_0059: Expected O, but got I
		base.CopyNode(original);
		nint num = (nint)typeof(BackButtonNode);
		nint num2 = (nint)original;
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v36 @ rdx_v2 (Il2CppClass<Doozy.Engine.UI.Nodes.BackButtonNode>)+130]");
		object obj = 0;
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v47 @ r8_v3 (Il2CppClass<Doozy.Engine.Nody.Models.Node>)+130]");
		nint num3 = 0;
		Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v36 @ rdx_v2 (Il2CppClass<Doozy.Engine.UI.Nodes.BackButtonNode>)+130]");
		if (num3 >= 0)
		{
			Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v47 @ r8_v3 (Il2CppClass<Doozy.Engine.Nody.Models.Node>)+C8]");
			object obj2 = 0;
			Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [v63 @ rax_v7+FFFFFFF8+v48 @ rax_v6*8]");
			if (0 == (nint)typeof(BackButtonNode))
			{
				Cpp2ILHelpers.NoteDecompilerIssue("Unmanaged memory load: [original @ rdx (Doozy.Engine.Nody.Models.Node)+80]");
				BackButtonAction = BackButtonState.Disable;
				return;
			}
		}
		throw new InvalidCastException();
	}

	public override void OnEnter(Node previousActiveNode, Connection connection)
	{
		//IL_0065: Expected O, but got I4
		base.OnEnter(previousActiveNode, connection);
		Graph activeGraph = base.m_activeGraph;
		if ((object)base.m_activeGraph == null || ((UnityEngine.Object)activeGraph).m_CachedPtr == (IntPtr)0)
		{
			return;
		}
		bool flag = BackButtonAction == BackButtonState.Disable;
		if (!flag)
		{
			object obj = BackButtonAction - 1;
			if (!flag)
			{
				if ((nint)obj == 1)
				{
					goto IL_00f9;
				}
			}
			else
			{
				BackButton instance = BackButton.Instance;
				int backButtonDisableLevel = instance.m_backButtonDisableLevel - 1;
				instance.m_backButtonDisableLevel = backButtonDisableLevel;
				BackButton instance2 = BackButton.Instance;
				if (instance2.m_backButtonDisableLevel < 0)
				{
					goto IL_00f9;
				}
			}
		}
		else
		{
			BackButton instance3 = BackButton.Instance;
			int backButtonDisableLevel2 = instance3.m_backButtonDisableLevel + 1;
			instance3.m_backButtonDisableLevel = backButtonDisableLevel2;
		}
		goto IL_020a;
		IL_00f9:
		BackButton instance4 = BackButton.Instance;
		instance4.m_backButtonDisableLevel = 0;
		goto IL_020a;
		IL_020a:
		Socket firstOutputSocket = base.FirstOutputSocket;
		List<Connection> connections = firstOutputSocket.m_connections;
		if (connections._size > 0)
		{
			Socket firstOutputSocket2 = base.FirstOutputSocket;
			Connection firstConnection = firstOutputSocket2.FirstConnection;
			Node nodeById = base.m_activeGraph.GetNodeById(firstConnection.m_inputNodeId);
			base.m_activeGraph.SetActiveNode(nodeById, firstConnection);
		}
	}

	private void ExecuteActions()
	{
		//IL_002f: Expected O, but got I4
		bool flag = BackButtonAction == BackButtonState.Disable;
		if (!flag)
		{
			object obj = BackButtonAction - 1;
			if (!flag)
			{
				if ((nint)obj != 1)
				{
					return;
				}
			}
			else
			{
				BackButton instance = BackButton.Instance;
				int backButtonDisableLevel = instance.m_backButtonDisableLevel - 1;
				instance.m_backButtonDisableLevel = backButtonDisableLevel;
				BackButton instance2 = BackButton.Instance;
				if (instance2.m_backButtonDisableLevel >= 0)
				{
					return;
				}
			}
			BackButton instance3 = BackButton.Instance;
			instance3.m_backButtonDisableLevel = 0;
		}
		else
		{
			BackButton instance4 = BackButton.Instance;
			int backButtonDisableLevel2 = instance4.m_backButtonDisableLevel + 1;
			instance4.m_backButtonDisableLevel = backButtonDisableLevel2;
		}
	}
}
