...
namespace Rewired.ComponentControls
{
	...
	public sealed class TouchPad : TouchInteractable, IPointerDownHandler, IEventSystemHandler, IPointerUpHandler
	{
		...
		private class fmmeBRFigGUxzgSSkUZLsWZevSKJA
		{
			private class KUprEwwOJGOaijcYJBcqtCEwOyOn
			{
				public float sxeueUdATfbwTZEryElzbkvQIhxeb;

				public float cXBjHKPVpVXOWDMgnfyiVQRMWyxb;

				public uint jRbiWhUEEeXzErUezzuIcATiEKJ;
			}

			private int HOLHINBPRzMROHWntWfWYtzJblqH;

			private KUprEwwOJGOaijcYJBcqtCEwOyOn[] idQkASYfHNsjEajfeMkUVHdRFxgDA;

			private int qMmtqIpPlJVjEdITAhktXWAxRHAQ;

			public fmmeBRFigGUxzgSSkUZLsWZevSKJA(int P_0)
			{
			}

			public void WbGLwpnXvmDaOkBEVwcspiQbOiFA(float P_0, float P_1)
			{
			}

			public Vector2 DZzNkJYodFrAgZWdgArUlRIAULAV()
			{
				return default(Vector2);
			}

			private void tnXtwKMMaHxCVGiVnUNTulUospXs()
			{
			}

			private static int PbyzmgjIsprHgByCyoNwItKwrucX(int P_0, int P_1)
			{
				return 0;
			}

			private int AOsCWzTpqxCwDCKqcpGAXmLGenIb(int P_0, int P_1)
			{
				return 0;
			}

			private static bool ttEYpdeBsIfkHMSMNKGCQNZGFPEc(uint P_0, uint P_1)
			{
				return false;
			}
		}
		...
		[NonSerialized]
		private bool ssfoEcCNAFIjAvIwbuIEGSkADnRCA;

		[NonSerialized]
		private bool MxgFEcvJSyrxbhFJPUwhwBdytZIA;
		...
		private fmmeBRFigGUxzgSSkUZLsWZevSKJA _smoothDelta;
		...
		internal StandaloneAxis2D RzWgoTMnzkknqRCXcldNvjThPxZu => null;

		private int romjTqhFjzwNLGfBnqJJCANrqQFe => 0;

		private bool uGtqIvZgSqCKzChZVHoZdbgXcGqs => false;
		...
		private void pLsnlARNvZXuyeAdHPquUwVWbNet()
		{
		}

		private void zmfiocjurkfCCrWlQlbxGyxxdeQV()
		{
		}

		private void rYYxhqOohhTHjKifwHWLJOuHontg(AxisDirection P_0)
		{
		}

		private void WxVosPWgxBBvoFPiZHFUYHQwKnsoA()
		{
		}

		private void dDNCQkKuknpcdQjfBrxTldGLNRkHA()
		{
		}

		private void iLYFJiTdRGnUvxolGlrlmwxslWrc()
		{
		}

		private void TvveXOHCBBctBOWJCKQLlCZwgraIA()
		{
		}

		private void gRQHcqFVLpAVbqHzqfboUBXvVUsS(ref Vector2 P_0)
		{
		}

		private void FzTQfheuaqavKOcJmolJhCqYTQtA(ref Vector2 P_0)
		{
		}

		private void TQSvDADpoHyKlxniyjnOUFTahHHfA()
		{
		}

		private Vector2 ZHQyqPZHjWsbWxILhUMkSnhgFQqf(Vector2 P_0)
		{
			return default(Vector2);
		}

		private void uaUrmfXmOmFEbWBDTLdLCRHDPyh(bool P_0)
		{
		}

		private void swxrsqydpWdkfdGrLaHKhxgBNOFJB(PointerEventData P_0)
		{
		}

		private void XKodaBRUMcFYtgJYnJJBorSYEcuc(PointerEventData P_0)
		{
		}

		private void tNwLfgRlNzFyYmvSghDjLDTGHSiM(PointerEventData P_0)
		{
		}

		private void GknFbfqaDTMhcIvjwnRnLZBLAbRt(PointerEventData P_0)
		{
		}

		private void YXkLECHKkSVXxdTxRQsONDlSBjZU(int P_0, Vector2 P_1)
		{
		}

		private void ywpRecCjpTDUxCiCjlfylQpdFjck()
		{
		}
		...
		private void DBhdGJIlnJkxXkIOFLBiIdZfhPJdA()
		{
		}

		private bool tSPJQjgcHLmomEwPJVTajEpFTSNe(int P_0)
		{
			return false;
		}

		private PointerEventData smyoFRkpxGCCfglcweWKsLHHpfjbb(int P_0, GameObject P_1)
		{
			return null;
		}

		private PointerEventData iucsEYaGBPrdlHhoAGomeEpjfeXB(int P_0, GameObject P_1)
		{
			return null;
		}

		private PointerEventData qgXHqiDMtYUbBeJILSRmVAoTloOQ(int P_0)
		{
			return null;
		}

		private void HkTeBZqiFwDefDGEpZOobpVrvbXtA(PointerEventData P_0)
		{
		}

		private PointerEventData cakFgoXosPpciDEBifoDgrIlLlcF(int P_0)
		{
			return null;
		}
	}
