	...
	public class PlayerController : IPlayerController
	{
		...
		public class Axis : ElementWithSource
		{
			public new class Definition : ElementWithSource.Definition
			{
				...
				internal override Element smEDjqxbvNwxprQujOCtwlPlXhYH(PlayerController P_0)
				{
					return null;
				}
			}

			internal const float ikOTKliFoybNxAedzTeKkEsTPZAn = 1f;

			internal const AbsoluteToRelativeScalingMode hwcmnbPXjVfZsmUHBEEsolKtQpyH = AbsoluteToRelativeScalingMode.None;

			[CustomObfuscation(rename = false)]
			internal const AxisCoordinateMode defaultAxisCoordinateMode = AxisCoordinateMode.Absolute;

			private float AcGvdzHWaEnnsdQXfvHMwtVSTEYx;

			private AxisCoordinateMode dEaPfKjbVSCoAkROXKTECRyKnJUWA;

			private AbsoluteToRelativeScalingMode UnnekoSImvkycjEvEzbLSUfcyrll;
		...
		public class Axis2D : CompoundElement
		{
			...
			{
				private Axis.Definition WomxnaqiTxIdFSAyNTlNWUhKLPiv;

				private Axis.Definition nfnVzUSFMQWMYcszAplcEOtQrHFX;
			...
			internal const int nLXdPQrMaefRCeEXqlByumtMpMWR = 0;

			internal const int yxXErhwzuOhViSkKHlHhVCfqjsIIA = 1;

			internal const int jbmHobpzkmwCYysfZqEiTZLujEpw = 2;
		...
		public abstract class CompoundElement : Element
		{
			...
			private readonly List<Element> AHkQtVnJpblwgpMcRThDGIdEQCSv;

			internal int QwBsKFybdgddwRnYsVZUhPRBwVrS => 0;
			...
			internal _0001 AHgltanzsVZjfcbRJYdATJHiiSsH<_0001>(int P_0) where _0001 : Element
			{
				return null;
			}

			internal void BStDSQJkWxJiLCInGdVGoJRainFEd(List<Element> P_0)
			{
			}

			internal void wqBADzXPnzdjntZyutbxvRGVqGau(Element P_0)
			{
			}
		}
		...
		public abstract class Element
		{
			...
			public abstract class Definition
			{
				...
				internal abstract Element smEDjqxbvNwxprQujOCtwlPlXhYH(PlayerController P_0);
			}

			internal struct PtLKUxxIyabJJvxoMbvgOMZNnUiJ
			{
				public ControllerElementType LnoXZLqZnnPSIVtnSjoyXnBeMZqv;

				public int kJfyChSXhAJxrLOMKasucbiYgHML;

				public float qRPrhmVINQAfxYOjTfLrGNasysoqA;

				public PtLKUxxIyabJJvxoMbvgOMZNnUiJ(ControllerElementType P_0, int P_1, float P_2)
				{
					LnoXZLqZnnPSIVtnSjoyXnBeMZqv = default(ControllerElementType);
					kJfyChSXhAJxrLOMKasucbiYgHML = 0;
					qRPrhmVINQAfxYOjTfLrGNasysoqA = 0f;
				}
			}
			...
			private readonly PlayerController iCMHoPjtMLjUvdYPVIZxNQtXYQiCA;

			private bool THgLDBtOCOevJQErHnXSMxXqFkTP;

			private bool QwAXwfWvpKoxJFoxDbnstDNBUiNh;

			private string FrZTXqWYLpGjdjfFOHhpZnvZuPoX;

			private static int[] MKngEraVFlxYsBNMkiwbEVURXNjA;

			private static int[] WkJYDDXZrqlGpspkNXwkrpDgGNBH;

			protected Player player => null;

			protected bool selfAndParentEnabled => false;

			internal bool YcseuJzHHyMdNQLhCAaSlXilkLyo
			{
				get
				{
					return false;
			...
			internal virtual void QtGOHWwoPuGoaCvRpdzuEXgtCTwu()
			{
			}
		...
		public abstract class ElementWithSource : Element
		{
			public new abstract class Definition : Element.Definition
			{
				private int beSjAkdTxndtZfVjegvDHmbfUDjlB;
			...
			private int GSTjTLKRypmKodaQBlHeTLIRloBd;
		...
		public sealed class MouseWheelAxis : Axis
		{
			...
			private const float tHXbQOXGrfdbnaUzfBFzolEMQfFoA = 0.01f;

			private float uBowSyOkZbGvLKiNrinnOlOPCazk;

			private double BroWlgXEEiGcIAbHoyqYpJxHlEBO;

			private float ERIGeudZvfLDgKshmWrJfMIgOkGV;
			...
			internal override void QtGOHWwoPuGoaCvRpdzuEXgtCTwu()
			{
			}
			...
			private float SufzacQjFoyEIKddBjTwLtJmPBUf()
			{
				return 0f;
			}

			private void zXHvZlcjRoHIsJhrKAmZFoFanoqg()
			{
			}
		}

		internal readonly int EtDTrCkgXWCVmfcsJUFnAASNpBvo;

		private bool bpRXeeKvcVladxNEdOzuZIhhPQJd;

		private int ZLxxoULFMZiSQIUJEfJDjagEXjlGB;

		private readonly AList<Element> mLSbTRnKSADMZHIQRzfZfFiYQYNuA;

		private readonly AList<Button> oHWKPuKKFChLvqMjouPrGNuwmsQc;

		private readonly AList<Axis> NmmVGdpNSZyfoJaGQdGXbMikjSkk;

		private readonly ReadOnlyCollection<Element> sRHgFbaUMhcQyMCOtexpAUmsFJaZ;

		private readonly ReadOnlyCollection<Button> chHxSZGlLaWaLCQqmUCcBDsYLwwV;

		private readonly ReadOnlyCollection<Axis> YBtkDAnCSGwMPchboalywBvcCgnhA;

		private readonly List<Element.PtLKUxxIyabJJvxoMbvgOMZNnUiJ> plyYEvxsAsMBXjylAjCxKRRjFujB;

		private Action<int, bool> QvliIZkuPjTCJjLDwARLhsDJBtud;

		private Action<int, float> bdEhIqjsnvfecLWGEfjQALNiNYHpB;

		private Action<bool> wJVdsoVtleIFhavQyhWfeqCATSdgA;

		private static Vector2 cdqfrFdTidTZqPoWJzuACYfVKvYr;
		...
		internal Player QMTyOBCODLzgqaVXvCyPMMbiIRYu => null;
		...
		private void qLPtsrIEPKuPqkvfgakokPGuSrAw(UpdateLoopType P_0)
		{
		}
		...
		internal void gYLncwnwYMMNpoFTNbmHCFQOgvrLA(Element P_0)
		{
		}

		private void YIxzfyNKAhqyFReUVFXZYgIzaoOKA(Element P_0, List<Element> P_1, List<Element> P_2, List<Button> P_3, List<Axis> P_4)
		{
		}

		internal static int qcFDkNBCLnKGIflXJVdrTQGBhzzWB<_0001>(IList<_0001> P_0, Predicate<_0001> P_1, int P_2) where _0001 : Element
		{
			return 0;
		}
	}
