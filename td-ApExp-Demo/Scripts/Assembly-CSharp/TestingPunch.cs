...
public class TestingPunch : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			LeanTween.delayedCall(0.05f, enterMiniGameStart).setOnCompleteParam(new object[1] { 5.ToString() ?? "" });
		}
		if (Input.GetKeyDown(KeyCode.U))
		{
			...
			{
				Vector2 vector = val;
				Debug.Log("tweening vec2 val:" + vector.ToString());
	...
	{
		Debug.Log("level:" + int.Parse((string)((object[])val)[0]));
	}
