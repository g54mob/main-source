...
public class StealthMissile : EnemyBase
{
	...
	[NonSerialized]
	public bool skipLaunchAnimation;

	private Vector3 randomStartTargetPos;
	...
	private new void Awake()
	{
		...
		randomStartTargetPos = new Vector3(UnityEngine.Random.Range(-2f, 2f), UnityEngine.Random.Range(0f - maxY, maxY), 0f);
	}

	private new void Update()
	{
		if (!base.IsEnemy && base.TargetUnit == null)
		{
		...
		if (base.IsEnemy)
		{
			...
			if (currentAnimatorStateInfo.IsName("MissileLaunching") && currentAnimatorStateInfo.normalizedTime < 1f && !skipLaunchAnimation)
			{
				return;
			}
	...
	private new void FixedUpdate()
	{
		...
		if (base.IsEnemy)
		{
			...
			if (flyStraightTimer > 0f)
			{
				...
				rotator.RotateTowardsPosition(randomStartTargetPos, base.TurnSpeed);
	...
	public void Raycast()
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (!base.IsEnemy && raycastHit2D.collider.TryGetComponent<Unit>(out var component) && component.IsEnemy && !component.ignoreProjectiles)
			{
				HitDeath();
			}
	...
	protected virtual void HitDeath()
	{
		...
		if (!base.IsEnemy)
		{
			component.Initialize(this, explosionScale, 10f);
		}
		else
		{
