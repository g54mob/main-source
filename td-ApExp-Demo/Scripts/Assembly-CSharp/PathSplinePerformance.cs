...
public class PathSplinePerformance : MonoBehaviour
{
	...
	private void Start()
	{
		...
		{
			float x = Mathf.Cos(num * (MathF.PI / 180f)) * circleLength + UnityEngine.Random.Range(0f, randomRange);
			float z = Mathf.Sin(num * (MathF.PI / 180f)) * circleLength + UnityEngine.Random.Range(0f, randomRange);
