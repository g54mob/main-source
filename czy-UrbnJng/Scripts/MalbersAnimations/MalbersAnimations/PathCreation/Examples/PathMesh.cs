...
namespace MalbersAnimations.PathCreation.Examples
{
	public class PathMesh : PathSceneTool
	{
		[Min(0f)]
		...
		private void CreateRoadMesh()
		{
			...
			for (int i = 0; i < base.path.NumPoints; i++)
			{
				...
				Vector3 vector3 = base.path.GetPoint(i) - vector2 * Mathf.Abs(Width);
				Vector3 vector4 = base.path.GetPoint(i) + vector2 * Mathf.Abs(Width);
				array[num2] = vector3;
		...
		private void AssignMeshComponents()
		{
			if (meshHolder == null)
			{
				meshHolder = new GameObject("Path Mesh");
				meshHolder.transform.parent = base.transform;
			}
		...
		private void AssignMaterials()
		{
			if (material != null)
			{
				meshRenderer.sharedMaterials = new Material[3] { material, material, material };
