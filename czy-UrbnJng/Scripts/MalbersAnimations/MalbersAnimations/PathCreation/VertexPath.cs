...
namespace MalbersAnimations.PathCreation
{
	public class VertexPath
	{
		...
		{
			TimeOnPathData timeOnPathData = CalculatePercentOnPathData(t, endOfPathInstruction);
			Vector3 p = Vector3.Lerp(localTangents[timeOnPathData.previousIndex], localTangents[timeOnPathData.nextIndex], timeOnPathData.percentBetweenIndices);
			return Quaternion.LookRotation(upwards: MathUtility.TransformDirection(Vector3.Lerp(localNormals[timeOnPathData.previousIndex], localNormals[timeOnPathData.nextIndex], timeOnPathData.percentBetweenIndices), transform, space), forward: MathUtility.TransformDirection(p, transform, space));
		}
