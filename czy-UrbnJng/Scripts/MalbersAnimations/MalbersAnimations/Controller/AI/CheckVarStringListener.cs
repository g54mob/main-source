...
namespace MalbersAnimations.Controller.AI
{
	...
	public class CheckVarStringListener : MAIDecision
	{
		...
		[Tooltip("Values to compare with the Listener. If any of the values match then it will return true")]
		[NonReorderable]
		...
		public override void PrepareDecision(MAnimalBrain brain, int Index)
		{
			...
			if (objective != null && objective.Length != 0)
			{
				...
				foreach (Transform transform in array2)
				{
					...
					MonoBehaviour[] components = GetComponents<StringVarListener>(transform.gameObject);
					array = components;
				}
			}
		...
		public override bool Decide(MAnimalBrain brain, int Index)
		{
			...
			foreach (Component component in array)
			{
				...
				StringVarListener stringVarListener = component as StringVarListener;
				for (int j = 0; j < value.Length; j++)
				{
					flag = stringVarListener.Value == value[j];
					if (flag)
					{
						break;
					}
