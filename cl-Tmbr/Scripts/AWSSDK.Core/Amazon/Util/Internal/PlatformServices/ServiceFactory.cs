	...
	public class ServiceFactory
	{
		...
		private ServiceFactory()
		{
			foreach (KeyValuePair<Type, InstantiationModel> instantationMapping in _instantationMappings)
			{
				...
				{
					object value = CreateInstance(_mappings[key]);
		...
		private static object CreateInstance(Type serviceType)
		{
			...
			if (serviceType == typeof(NetworkReachability))
			{
				return new NetworkReachability();
			}
			if (serviceType == typeof(ApplicationInfo))
			{
				return new ApplicationInfo();
			}
			if (serviceType == typeof(EnvironmentInfo))
			{
