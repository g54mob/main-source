...
namespace Amazon.Runtime.Internal.Util
{
	public class RequestMetrics : IRequestMetrics
	{
		...
		public string ToJSON()
		{
			...
			foreach (KeyValuePair<Metric, List<object>> property in Properties)
			{
				utf8JsonWriter.WritePropertyName(property.Key.ToString());
				List<object> value = property.Value;
				if (value.Count > 1)
				{
				...
				foreach (object item in value)
				{
					if (item == null)
					{
						utf8JsonWriter.WriteStringValue((string?)null);
					}
					else
					{
						utf8JsonWriter.WriteStringValue(item.ToString());
					}
				}
				if (value.Count > 1)
				{
			...
			utf8JsonWriter.WritePropertyName("timings");
			utf8JsonWriter.WriteStartObject();
			foreach (KeyValuePair<Metric, List<IMetricsTiming>> timing in Timings)
			{
				utf8JsonWriter.WritePropertyName(timing.Key.ToString());
				...
				foreach (IMetricsTiming item2 in timing.Value)
				{
					if (item2.IsFinished)
					{
						utf8JsonWriter.WriteNumberValue(item2.ElapsedTime.TotalMilliseconds);
					}
				}
				if (value2.Count > 1)
				{
			...
			utf8JsonWriter.WriteEndObject();
			utf8JsonWriter.WritePropertyName("counters");
			utf8JsonWriter.WriteStartObject();
			foreach (KeyValuePair<Metric, long> counter in Counters)
			{
				utf8JsonWriter.WritePropertyName(counter.Key.ToString());
				utf8JsonWriter.WriteNumberValue(counter.Value);
			}
			utf8JsonWriter.WriteEndObject();
			utf8JsonWriter.WriteEndObject();
