...
namespace Amazon.Runtime.Internal
{
	public class HttpErrorResponseExceptionHandler : ExceptionHandler<HttpErrorResponseException>
	{
		...
		public override bool HandleException(IExecutionContext executionContext, HttpErrorResponseException exception)
		{
			...
			{
				Stream responseStream = response.ResponseBody.OpenResponse();
				return HandleExceptionStream(requestContext, response, exception, responseStream);
			}
		}

		public override async Task<bool> HandleExceptionAsync(IExecutionContext executionContext, HttpErrorResponseException exception)
		{
			IRequestContext requestContext = executionContext.RequestContext;
			IWebResponseData httpErrorResponse = exception.Response;
			...
			{
				return HandleExceptionStream(requestContext, httpErrorResponse, exception, await httpErrorResponse.ResponseBody.OpenResponseAsync().ConfigureAwait(continueOnCapturedContext: false));
			}
		}
		...
		private bool HandleExceptionStream(IRequestContext requestContext, IWebResponseData httpErrorResponse, HttpErrorResponseException exception, Stream responseStream)
		{
			...
			try
			{
				...
				UnmarshallerContext unmarshallerContext = unmarshaller.CreateContext(httpErrorResponse, readEntireResponse, responseStream, requestContext.Metrics, isException: true, requestContext);
				try
				{
					ex = unmarshaller.UnmarshallException(unmarshallerContext, exception, httpErrorResponse.StatusCode);
				}
				catch (Exception ex2)
				{
					...
					string headerValue = httpErrorResponse.GetHeaderValue("x-amzn-RequestId");
					string responseBody = unmarshallerContext.ResponseBody;
					throw new AmazonUnmarshallingException(headerValue, null, responseBody, ex2, httpErrorResponse.StatusCode);
				}
		...
		private bool HandleSuppressed404(IExecutionContext executionContext, IWebResponseData httpErrorResponse)
		{
			...
			if (httpErrorResponse != null && httpErrorResponse.StatusCode == HttpStatusCode.NotFound && requestContext.Request.Suppress404Exceptions)
			{
				using (httpErrorResponse.ResponseBody)
				{
					...
					UnmarshallerContext input = unmarshaller.CreateContext(httpErrorResponse, readEntireResponse, httpErrorResponse.ResponseBody.OpenResponse(), requestContext.Metrics, isException: true, requestContext);
					try
					{
