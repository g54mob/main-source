	...
	public class DiagnosticsLogger : LevelFilteredLogger, IDisposable
	{
		...
		protected override void Log(LoggerLevel loggerLevel, string loggerName, string message, Exception exception)
		{
			if (eventLog != null)
			{
				...
				string message2 = ((exception != null) ? string.Format(CultureInfo.CurrentCulture, "[{0}] '{1}' message: {2} exception: {3} {4} {5}", loggerLevel, loggerName, message, exception.GetType(), exception.Message, exception.StackTrace) : string.Format(CultureInfo.CurrentCulture, "[{0}] '{1}' message: {2}", loggerLevel, loggerName, message));
