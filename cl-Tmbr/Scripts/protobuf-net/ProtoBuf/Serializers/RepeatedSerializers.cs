...
namespace ProtoBuf.Serializers
{
	internal static class RepeatedSerializers
	{
		...
		static RepeatedSerializers()
		{
			...
			Add(typeof(HashSet<>), (Type root, Type current, Type[] targs) => Resolve(typeof(RepeatedSerializer), "CreateSet", new Type[2]
			{
				root,
				targs[0]
			}));
			Add(typeof(ISet<>), (Type root, Type current, Type[] targs) => Resolve(typeof(RepeatedSerializer), "CreateSet", new Type[2]
			{
				root,
				targs[0]
			}));
		...
		private static MemberInfo GetProviderForType(Type type, Hashtable externalProviders)
		{
			...
			if (externalProviders != null)
			{
				...
				{
					if (TryGetExternalProvider(type, type3, bestMatchPriority, out var member, out var priority, externalProviders))
					{
				...
				{
					if (TryGetExternalProvider(type, current, bestMatchPriority, out var member2, out var priority2, externalProviders))
					{
			...
			while ((object)type3 != null && type3 != typeof(object))
			{
				if (TryGetProvider(type, type3, bestMatchPriority, out var member3, out var priority3))
				{
			...
			{
				if (TryGetProvider(type, current2, bestMatchPriority, out var member4, out var priority4))
				{
