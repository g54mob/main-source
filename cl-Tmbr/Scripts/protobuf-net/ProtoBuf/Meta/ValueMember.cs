...
namespace ProtoBuf.Meta
{
	public class ValueMember
	{
		...
		private const ushort OPTIONS_IsStrict = 1;

		private const ushort OPTIONS_IsPacked = 2;

		private const ushort OPTIONS_IsRequired = 4;

		private const ushort OPTIONS_OverwriteList = 8;

		private const ushort OPTIONS_NullWrappedValue = 16;

		private const ushort OPTIONS_NullWrappedValueGroup = 32;

		private const ushort OPTIONS_IsMap = 64;

		private const ushort OPTIONS_NullWrappedCollection = 128;

		private const ushort OPTIONS_NullWrappedCollectionGroup = 256;

		private const ushort OPTIONS_SupportNull = 512;
		...
		internal bool RequiresGroupModifier
		{
			get
			{
				if (!SupportNull)
				{
					return NullWrappedValueGroup;
				}
				return true;
			}
		}
		...
		private static object ParseDefaultValue(Type type, object value)
		{
			...
			if (value is string text)
			{
				...
				case ProtoTypeCode.IntPtr:
					return new IntPtr(long.Parse(text, NumberStyles.Any, CultureInfo.InvariantCulture));
				case ProtoTypeCode.UIntPtr:
					return new UIntPtr(ulong.Parse(text, NumberStyles.Any, CultureInfo.InvariantCulture));
				}
			}
			if (type.IsEnum)
			{
		...
		internal static IRuntimeProtoSerializerNode CreateMap(RepeatedSerializerStub repeated, RuntimeTypeModel model, DataFormat dataFormat, CompatibilityLevel compatibilityLevel, DataFormat keyFormat, DataFormat valueFormat, bool asReference, bool dynamicType, bool isMap, bool overwriteList, int fieldNumber, ValueMember member)
		{
			...
			if (!isMap)
			{
				listFeatures |= SerializerFeatures.OptionFailOnDuplicateKey;
			}
			if (overwriteList)
			{
				listFeatures |= SerializerFeatures.OptionClearCollection;
			}
			member?.ComposeListFeatures(ref listFeatures);
			SerializerFeatures serializerFeatures = defaultWireType2.AsFeatures();
			serializerFeatures |= listFeatures & (SerializerFeatures.OptionWrappedValue | SerializerFeatures.OptionWrappedValueGroup);
			listFeatures &= ~(SerializerFeatures.OptionWrappedValue | SerializerFeatures.OptionWrappedValueGroup);
			return MapDecorator.Create(repeated, keyType, valueType, fieldNumber, listFeatures, defaultWireType.AsFeatures(), effectiveCompatibilityLevel, keyFormat, serializerFeatures, effectiveCompatibilityLevel2, valueFormat);
		...
		private void ComposeListFeatures(ref SerializerFeatures listFeatures)
		{
			if (!IsPacked)
			{
				listFeatures |= SerializerFeatures.OptionPackedDisabled;
			}
			if (OverwriteList)
			{
				listFeatures |= SerializerFeatures.OptionClearCollection;
			...
			if (SupportNull)
			{
				...
				{
					ThrowHelper.ThrowNotSupportedException("SupportNull cannot be combined with IsPacked, NullWrappedValue or NullWrappedCollection");
				}
				listFeatures |= SerializerFeatures.OptionWrappedValue | SerializerFeatures.OptionWrappedValueGroup;
				return;
			}
			if (NullWrappedCollection)
			{
				...
				if (NullWrappedCollectionGroup)
				{
					listFeatures |= SerializerFeatures.OptionWrappedCollectionGroup;
				}
			}
			if (NullWrappedValue)
			{
				listFeatures |= SerializerFeatures.OptionWrappedValue | SerializerFeatures.OptionWrappedValueFieldPresence;
				if (NullWrappedValueGroup)
				{
					listFeatures |= SerializerFeatures.OptionWrappedValueGroup;
				}
			}
		}
		...
		private IRuntimeProtoSerializerNode BuildSerializer()
		{
			...
			try
			{
				...
				if (repeatedSerializerStub != null)
				{
					...
					{
						runtimeProtoSerializerNode = CreateMap(repeatedSerializerStub, model, DataFormat, CompatibilityLevel, MapKeyFormat, MapValueFormat, AsReference, DynamicType, IsMap, OverwriteList, FieldNumber, this);
					}
					else
					{
						...
						SerializerFeatures listFeatures = defaultWireType.AsFeatures();
						ComposeListFeatures(ref listFeatures);
						runtimeProtoSerializerNode = RepeatedDecorator.Create(repeatedSerializerStub, FieldNumber, listFeatures, CompatibilityLevel, DataFormat);
					}
				}
				else
				{
					if (NullWrappedCollection)
					{
						ThrowHelper.ThrowNotSupportedException("NullWrappedCollection can only be used with collection types");
					}
					...
					if (NullWrappedValue)
					{
						SerializerFeatures serializerFeatures = SerializerFeatures.OptionWrappedValue;
						if (NullWrappedValueGroup)
						{
							serializerFeatures |= SerializerFeatures.OptionWrappedValueGroup;
						}
						if (MemberType.IsValueType && (object)Nullable.GetUnderlyingType(MemberType) == null)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue cannot be used with non-nullable values");
						}
						if (_defaultValue != null)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue cannot be used with default values");
						}
						if (IsRequired)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue cannot be used with required values");
						}
						if (IsPacked)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue cannot be used with packed values");
						}
						if (DataFormat != DataFormat.Default)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue can only be used with DataFormat.Default");
						}
						if (!runtimeProtoSerializerNode.IsScalar)
						{
							ThrowHelper.ThrowNotSupportedException("NullWrappedValue can only be used with scalar types, or in a collection");
						}
						runtimeProtoSerializerNode = AnyTypeSerializer.Create(MemberType, serializerFeatures, CompatibilityLevel, DataFormat);
					}
		...
		{
			if (!SupportNull && !NullWrappedValueGroup)
			{
				return NullWrappedValue;
			}
			return true;
		}
