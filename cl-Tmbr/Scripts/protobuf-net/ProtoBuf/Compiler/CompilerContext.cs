...
namespace ProtoBuf.Compiler
{
	internal sealed class CompilerContext : IDisposable
	{
		...
		{
			if (head is IProtoTypeSerializer { ShouldEmitCreateInstance: not false } protoTypeSerializer)
			{
		...
		internal void EmitStateBasedWrite(string methodName, Local fromValue, Type type = null, Type argType = null)
		{
			...
			try
			{
				var anon = (from method in type.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public)
					...
					let paramType = args[method.IsStatic ? 1u : 0u].ParameterType
		...
		{
			Type underlyingType;
			if (type.IsValueType && type != tail.ExpectedType && (object)(underlyingType = Nullable.GetUnderlyingType(type)) != null && underlyingType == tail.ExpectedType)
			{
		...
		public void EmitCtor(Type type, params Type[] parameterTypes)
		{
			...
			ConstructorInfo ctor = Helpers.GetConstructor(type, parameterTypes, nonPublic: true) ?? throw new InvalidOperationException("No suitable constructor found for " + type.FullName);
			EmitCtor(ctor);
		}
		...
		internal void CheckAccessibility(ref MemberInfo member)
		{
			...
			if (!flag)
			{
				...
				{
					InvalidOperationException ex = ((memberType != MemberTypes.TypeInfo && memberType != MemberTypes.NestedType) ? new InvalidOperationException("Non-public member cannot be used with full dll compilation: " + member.DeclaringType.NormalizeName() + "." + member.Name) : new InvalidOperationException("Non-public type cannot be used with full dll compilation: " + ((Type)member).NormalizeName()));
					throw ex;
				}
			}
		}
