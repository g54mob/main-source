...
namespace HandlebarsDotNet.ObjectDescriptors
{
	...
	{
		private static readonly MethodInfo CreateDescriptorMethodInfo = typeof(GenericDictionaryObjectDescriptorProvider).GetMethod("CreateDescriptor", BindingFlags.Static | BindingFlags.NonPublic);
		...
		private static readonly Func<Type, DeferredValue<Type, Type>> InterfaceTypeValueFactory = (Type key) => new DeferredValue<Type, Type>(key, (Type type) => (from i in type.GetInterfaces()
			...
			where i.GetGenericTypeDefinition() == typeof(IDictionary<, >)
		...
		private static ObjectDescriptor CreateDescriptor<T, TK, TV>() where T : class, IDictionary<TK, TV>
		{
			return new ObjectDescriptor(typeof(T), new GenericDictionaryAccessor<T, TK, TV>(), (ObjectDescriptor descriptor, object o) => ((T)o).Keys, (ObjectDescriptor self) => new DictionaryIterator<T, TK, TV>());
		}
	}
}
