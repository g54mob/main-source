...
namespace HandlebarsDotNet.IO
{
	...
	{
		private static readonly DictionarySlim<Type, IFormatter, ReferenceEqualityComparer<Type>> Formatters = new DictionarySlim<Type, IFormatter, ReferenceEqualityComparer<Type>>(12, default(ReferenceEqualityComparer<Type>))
		{
		...
		{
			if (!Formatters.TryGetValue(in type, out formatter))
			{
