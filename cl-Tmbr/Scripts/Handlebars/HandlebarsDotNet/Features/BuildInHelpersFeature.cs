...
namespace HandlebarsDotNet.Features
{
	...
	internal class BuildInHelpersFeature : IFeature
	{
		...
		{
			configuration.BlockHelpers[(PathInfoLight)"with"] = new Ref<IHelperDescriptor<BlockHelperOptions>>(WithBlockHelperDescriptor);
			configuration.Helpers[(PathInfoLight)"lookup"] = new Ref<IHelperDescriptor<HelperOptions>>(LookupReturnHelperDescriptor);
			configuration.BlockDecorators[(PathInfoLight)"*inline"] = new Ref<IDecoratorDescriptor<BlockDecoratorOptions>>(InlineBlockHelperDescriptor);
		}
