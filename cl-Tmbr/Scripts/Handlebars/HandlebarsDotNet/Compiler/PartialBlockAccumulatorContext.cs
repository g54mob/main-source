...
namespace HandlebarsDotNet.Compiler
{
	internal class PartialBlockAccumulatorContext : BlockAccumulatorContext
	{
		...
		public PartialBlockAccumulatorContext(Expression startingNode)
			: base(startingNode)
		{
			_startingNode = ConvertToPartialExpression(BlockAccumulatorContext.UnwrapStatement(startingNode));
		}
		...
		{
			return HandlebarsExpression.Partial(_startingNode.PartialName, _startingNode.Argument, (_body.Count > 1) ? Expression.Block(_body) : _body.First());
		}
		...
		private bool IsClosingNode(Expression item)
		{
			...
			{
				return ((PathExpression)item).Path == "/" + BlockName;
			}
			return false;
		}
		...
		private PartialExpression ConvertToPartialExpression(Expression expression)
		{
			...
			int num = helperExpression.Arguments.Count();
			...
			return num switch
			{
				...
				1 => HandlebarsExpression.Partial(Expression.Constant(BlockName), helperExpression.Arguments.First()), 
