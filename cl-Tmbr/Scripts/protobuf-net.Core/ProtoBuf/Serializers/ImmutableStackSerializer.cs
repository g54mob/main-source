...
namespace ProtoBuf.Serializers
{
	internal sealed class ImmutableStackSerializer<T> : RepeatedSerializer<ImmutableStack<T>, T>
	{
		...
		internal override long Measure(ImmutableStack<T> values, IMeasuringSerializer<T> serializer, ISerializationContext context, WireType wireType)
		{
			...
			return RepeatedSerializer<ImmutableStack<T>, T>.Measure(ref values2, serializer, context, wireType);
		}
		...
		{
			Enumerator values2 = new Enumerator(values);
			RepeatedSerializer<ImmutableStack<T>, T>.Write(ref state, fieldNumber, category, wireType, ref values2, serializer, features);
		}
		...
		{
			Enumerator values2 = new Enumerator(values);
			RepeatedSerializer<ImmutableStack<T>, T>.WritePacked(ref state, ref values2, serializer, wireType);
		}
	}
}
