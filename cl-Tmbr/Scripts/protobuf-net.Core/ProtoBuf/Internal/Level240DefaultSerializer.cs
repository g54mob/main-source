...
{
	internal class Level240DefaultSerializer : ISerializer<DateTime>, ISerializer<DateTime?>, ISerializer<TimeSpan>, ISerializer<TimeSpan?>
	{
		...
		DateTime ISerializer<DateTime>.Read(ref ProtoReader.State state, DateTime value)
		{
			return PrimaryTypeProvider.ReadTimestamp(ref state, value);
		}
		...
		TimeSpan ISerializer<TimeSpan>.Read(ref ProtoReader.State state, TimeSpan value)
		{
			return PrimaryTypeProvider.ReadDuration(ref state, value);
		}
