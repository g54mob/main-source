...
namespace ProtoBuf.Internal
{
	...
	internal readonly struct KeyValuePairSerializer<TKey, TValue> : ISerializer<KeyValuePair<TKey, TValue>>
	{
		...
		public KeyValuePair<TKey, TValue> Read(ref ProtoReader.State state, KeyValuePair<TKey, TValue> pair)
		{
			...
			{
				val = CreateDefault(state.Context, _keySerializer, _keyFeatures);
			}
			if (TypeHelper<TValue>.IsReferenceType && TypeHelper<TValue>.ValueChecker.IsNull(value))
			{
				value = CreateDefault(state.Context, _valueSerializer, _valueFeatures);
			}
			return new KeyValuePair<TKey, TValue>(val, value);
		}

		private static T CreateDefault<T>(ISerializationContext context, ISerializer<T> serializer, SerializerFeatures features)
		{
			...
			{
				ProtoReader.State state = ProtoReader.State.Create(default(ReadOnlyMemory<byte>), context?.Model, context?.UserState);
				try
				{
					return serializer.Read(ref state, default(T));
				}
				finally
				{
					state.Dispose();
