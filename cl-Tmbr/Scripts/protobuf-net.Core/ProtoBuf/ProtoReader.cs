...
namespace ProtoBuf
{
	public abstract class ProtoReader : IDisposable, ISerializationContext
	{
		...
		public ref struct State
		{
			...
			internal readonly SolidState Solidify()
			{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal readonly void Advance(long count)
			{
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			internal readonly ProtoReader GetReader()
			{
				return _reader;
			}
			...
			private void ReadPackedScalar<TSerializer, TList, T>(ref TList list, WireType wireType, in TSerializer serializer) where TSerializer : ISerializer<T> where TList : ICollection<T>
			{
				...
				for (int i = 0; i < num2; i++)
				{
					...
					val2 = serializer;
					T item = val2.Read(ref this, default(T));
					reference.Add(item);
				}
				return;
				IL_00ff:
				...
				do
				{
					...
					val2 = serializer;
					T item2 = val2.Read(ref this, default(T));
					reference2.Add(item2);
				}
			...
			internal ReadBuffer<T> FillBuffer<TSerializer, T>(SerializerFeatures features, in TSerializer serializer, T initialValue) where TSerializer : ISerializer<T>
			{
				...
				try
				{
					...
					{
						ReadRepeatedCore<TSerializer, ReadBuffer<T>, T>(ref values, category, wireType, in serializer, features);
					}
					return values;
				}
				catch
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public readonly void Hint(WireType wireType)
			{
			...
			{
				ThrowInvalidOperationException("Maximum model depth exceeded (see TypeModel.MaxDepth): " + _reader._depth);
			}
			...
			{
				ThrowInvalidOperationException("Invalid length: " + length);
			}
			...
			public void AppendExtensionData(IExtensible instance)
			{
				...
				AppendExtensionDataImpl(instance.GetExtensionObject(createIfMissing: true));
			}
			...
			public void AppendExtensionData(ITypedExtensible instance, Type type)
			{
				...
				AppendExtensionDataImpl(instance.GetExtensionObject(type, createIfMissing: true));
			}
			...
			public T ReadAny<[DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.NonPublicMethods | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.NonPublicFields | DynamicallyAccessedMemberTypes.PublicNestedTypes | DynamicallyAccessedMemberTypes.NonPublicNestedTypes | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.NonPublicProperties)] T>(SerializerFeatures features, T value = default(T), ISerializer<T> serializer = null)
			{
				...
				{
					return ReadWrapped(features, value, serializer);
				}
			...
			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public readonly bool HasSubValue(WireType wireType)
			{
		...
		{
			return ++_depth >= ((_model == null) ? 512 : _model.MaxDepth);
		}
		...
		{
			if ((object)s_origin != null && (object)s_buffer != null && ms.GetType() == typeof(MemoryStream))
			{
				try
				{
		...
		{
			if (source is MemoryStream { CanSeek: not false } memoryStream && (memoryStream.TryGetBuffer(out var buffer) || ReflectionTryGetBuffer(memoryStream, out buffer)))
			{
