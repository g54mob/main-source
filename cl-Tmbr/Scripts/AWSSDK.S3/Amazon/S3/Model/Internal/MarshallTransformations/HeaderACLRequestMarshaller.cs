...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
	public static class HeaderACLRequestMarshaller
	{
		public static void Marshall(IRequest request, PutWithACLRequest aclRequest)
		{
			...
			if (aclRequest != null && aclRequest.Grants != null)
			{
			...
			foreach (S3Permission key in dictionary.Keys)
			{
				...
				{
					request.Headers["x-amz-grant-read"] = dictionary[key];
				}
				else if (S3Permission.WRITE == key)
				{
					request.Headers["x-amz-grant-write"] = dictionary[key];
				}
				else if (S3Permission.READ_ACP == key)
				{
					request.Headers["x-amz-grant-read-acp"] = dictionary[key];
				}
				else if (S3Permission.WRITE_ACP == key)
				{
					request.Headers["x-amz-grant-write-acp"] = dictionary[key];
				}
				else if (S3Permission.RESTORE_OBJECT == key)
				{
					request.Headers["x-amz-grant-restore-object"] = dictionary[key];
				}
				else if (S3Permission.FULL_CONTROL == key)
				{
					request.Headers["x-amz-grant-full-control"] = dictionary[key];
				}
			}
		}
	}
