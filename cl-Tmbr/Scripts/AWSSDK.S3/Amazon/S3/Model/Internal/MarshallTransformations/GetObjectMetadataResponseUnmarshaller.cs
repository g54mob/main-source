...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
	public class GetObjectMetadataResponseUnmarshaller : S3ReponseUnmarshaller
	{
		...
		private static void UnmarshallResult(XmlUnmarshallerContext context, GetObjectMetadataResponse response)
		{
			...
			{
				AmazonS3Util.ParseAmzRestoreHeader(responseData.GetHeaderValue("x-amz-restore"), out var restoreInProgress, out var restoreExpiration);
			...
			foreach (string text in headerNames)
			{
				...
				{
					response.Metadata[text] = (AWSConfigsS3.EnableUnicodeEncodingForObjectMetadata ? Uri.UnescapeDataString(responseData.GetHeaderValue(text)) : responseData.GetHeaderValue(text));
				}
			}
		}
	}
