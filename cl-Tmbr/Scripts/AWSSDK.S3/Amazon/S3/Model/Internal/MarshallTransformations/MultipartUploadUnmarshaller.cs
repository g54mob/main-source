		...
		public MultipartUpload Unmarshall(XmlUnmarshallerContext context)
		{
			...
			while (context.Read())
			{
				if (context.IsStartElement || context.IsAttribute)
				{
					...
					{
						multipartUpload.ChecksumAlgorithm = StringUnmarshaller.GetInstance().Unmarshall(context);
					}
					else if (context.TestExpression("Initiated", num))
					{
					...
					{
						multipartUpload.ChecksumType = StringUnmarshaller.GetInstance().Unmarshall(context);
					}
				}
