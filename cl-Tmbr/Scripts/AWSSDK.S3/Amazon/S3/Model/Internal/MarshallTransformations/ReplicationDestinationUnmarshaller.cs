		...
		public ReplicationDestination Unmarshall(XmlUnmarshallerContext context)
		{
			...
			while (context.Read())
			{
				if (context.IsStartElement || context.IsAttribute)
				{
					...
					if (context.TestExpression("StorageClass", num))
					{
					...
					if (context.TestExpression("AccessControlTranslation", num))
					{
						replicationDestination.AccessControlTranslation = AccessControlTranslationUnmarshaller.Instance.Unmarshall(context);
						continue;
					}
					if (context.TestExpression("EncryptionConfiguration", num))
					{
						replicationDestination.EncryptionConfiguration = EncryptionConfigurationUnmarshaller.Instance.Unmarshall(context);
						continue;
					}
					if (context.TestExpression("ReplicationTime", num))
					{
						replicationDestination.ReplicationTime = ReplicationTimeUnmarshaller.Instance.Unmarshall(context);
					}
					if (context.TestExpression("Metrics", num))
					{
						replicationDestination.Metrics = MetricsUnmarshaller.Instance.Unmarshall(context);
					}
				}
