	...
	public class VersionsItemUnmarshaller : IXmlUnmarshaller<S3ObjectVersion, XmlUnmarshallerContext>
	{
		...
		public S3ObjectVersion Unmarshall(XmlUnmarshallerContext context)
		{
			...
			while (context.Read())
			{
				...
				{
					if (context.TestExpression("ChecksumAlgorithm", num))
					{
						if (s3ObjectVersion.ChecksumAlgorithm == null)
						{
							s3ObjectVersion.ChecksumAlgorithm = new List<string>();
						}
						s3ObjectVersion.ChecksumAlgorithm.Add(StringUnmarshaller.GetInstance().Unmarshall(context));
					}
					...
					else if (context.TestExpression("RestoreStatus", num))
					{
						s3ObjectVersion.RestoreStatus = RestoreStatusUnmarshaller.Instance.Unmarshall(context);
					}
					else if (context.TestExpression("ChecksumType", num))
					{
						s3ObjectVersion.ChecksumType = StringUnmarshaller.GetInstance().Unmarshall(context);
					}
				}
