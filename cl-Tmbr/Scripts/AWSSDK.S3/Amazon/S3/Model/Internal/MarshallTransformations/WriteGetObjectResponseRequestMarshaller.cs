...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
	public class WriteGetObjectResponseRequestMarshaller : IMarshaller<IRequest, WriteGetObjectResponseRequest>, IMarshaller<IRequest, AmazonWebServiceRequest>
	{
		...
		{
			return Marshall((WriteGetObjectResponseRequest)input);
		}
		...
		public IRequest Marshall(WriteGetObjectResponseRequest writeGetObjectResponseRequest)
		{
			...
			{
				throw new ArgumentException("RequestRoute is a required property and must be set before making this call.", "WriteGetObjectResponseRequest.RequestRoute");
			}
			if (!writeGetObjectResponseRequest.IsSetRequestToken())
			{
				throw new ArgumentException("RequestToken is a required property and must be set before making this call.", "WriteGetObjectResponseRequest.RequestToken");
			}
			if (writeGetObjectResponseRequest.IsSetRequestRoute())
			{
			...
			Stream contentStream = writeGetObjectResponseRequest.Body ?? new MemoryStream();
			request.ContentStream = contentStream;
			if (writeGetObjectResponseRequest.IsSetContentLength())
			{
				long value = writeGetObjectResponseRequest.ContentLength.Value;
				request.Headers["Content-Length"] = value.ToString(CultureInfo.InvariantCulture);
			}
			...
			if (!HostPrefixUtils.IsValidLabelValue(anon.RequestRoute))
			{
			...
			request.HostPrefix = anon.RequestRoute + ".";
			request.ResourcePath = string.Format(CultureInfo.InvariantCulture, "/{0}", S3Transforms.ToStringValue("WriteGetObjectResponse"));
			return request;
		}
	}
}
