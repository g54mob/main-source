		...
		public S3Object Unmarshall(XmlUnmarshallerContext context)
		{
			...
			while (context.Read())
			{
				if (context.IsStartElement || context.IsAttribute)
				{
					...
					{
						if (s3Object.ChecksumAlgorithm == null)
						{
							s3Object.ChecksumAlgorithm = new List<string>();
						}
						s3Object.ChecksumAlgorithm.Add(StringUnmarshaller.GetInstance().Unmarshall(context));
					}
					...
					{
						s3Object.RestoreStatus = RestoreStatusUnmarshaller.Instance.Unmarshall(context);
					}
					else if (context.TestExpression("ChecksumType", num))
					{
						s3Object.ChecksumType = StringUnmarshaller.GetInstance().Unmarshall(context);
					}
				}
