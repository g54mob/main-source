...
namespace Amazon.S3.Model.Internal.MarshallTransformations
{
	public class ListMultipartUploadsResponseUnmarshaller : S3ReponseUnmarshaller
	{
		...
		public override AmazonWebServiceResponse Unmarshall(XmlUnmarshallerContext context)
		{
			...
			{
				listMultipartUploadsResponse.RequestCharged = RequestCharged.FindValue(responseData.GetHeaderValue(S3Constants.AmzHeaderRequestCharged));
			}
		...
		private static void UnmarshallResult(XmlUnmarshallerContext context, ListMultipartUploadsResponse response)
		{
			...
			while (context.Read())
			{
				if (context.IsStartElement || context.IsAttribute)
				{
					...
					{
						if (!context.TestExpression("CommonPrefixes", num))
						{
							continue;
						}
