...
namespace CTS
{
	public class RoomWallStyleGoal : QuestGoal
	{
		private EBarStyle _style;

		private List<RoomBuilding> _rooms = new List<RoomBuilding>();

		public RoomWallStyleGoal(Quest quest, int entryID, EBarStyle style)
			: base(quest, entryID)
		{
			_style = style;
		}
		...
		{
			if (!infos.WallsStyles.ContainsKey(_style) || infos.WallsStyles.Count > 1)
			{
				_rooms.Remove(room);
			}
