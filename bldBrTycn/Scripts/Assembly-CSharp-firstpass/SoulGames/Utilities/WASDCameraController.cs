...
namespace SoulGames.Utilities
{
	public class WASDCameraController : MonoBehaviour
	{
		...
		[SerializeField]
		private float movementLerpTime = 10f;
		...
		[SerializeField]
		private float rotationLerpTime = 10f;

		[Space]
		[Tooltip("Camera zoom amount")]
		[SerializeField]
		private Vector3 zoomAmount = new Vector3(0f, -0.5f, 0.5f);
		...
		[SerializeField]
		private KeyCode zoomInKey = KeyCode.Z;

		[SerializeField]
		private KeyCode zoomOutKey = KeyCode.X;
		...
		private void FixedUpdate()
		{
			...
			if (setHeigthByRaycast)
			{
				...
				raycastHeight = new Vector3(0f, raycastHeight.y, raycastHeight.y);
			}
		}
		...
		private void HandleInput()
		{
			...
			{
				newRotation *= Quaternion.Euler(Vector3.up * (0f - rotationAmount));
			}
			if (Input.GetKey(upKey) || Input.GetKey(KeyCode.UpArrow))
			{
				newPosition += base.transform.forward * movementSpeed;
			}
			if (Input.GetKey(downKey) || Input.GetKey(KeyCode.DownArrow))
			{
				newPosition += base.transform.forward * (0f - movementSpeed);
			}
			if (Input.GetKey(rightKey) || Input.GetKey(KeyCode.RightArrow))
			{
				newPosition += base.transform.right * movementSpeed;
			}
			if (Input.GetKey(leftKey) || Input.GetKey(KeyCode.LeftArrow))
			{
				newPosition += base.transform.right * (0f - movementSpeed);
			}
			...
			newZoom.z = Mathf.Clamp(newZoom.z, 0f - maxZoom, 0f - minZoom);
			...
			base.transform.rotation = Quaternion.Lerp(base.transform.rotation, newRotation, Time.deltaTime * rotationLerpTime);
			base.transform.position = Vector3.Lerp(base.transform.position, newPosition, Time.deltaTime * movementLerpTime);
		}

		private Vector3 GetRaycastHeight()
		{
			if (Physics.Raycast(Camera.main.transform.position, Camera.main.transform.forward, out var hitInfo, 999f, raycastLayerMask))
			{
