...
namespace SoulGames.EasyGridBuilderPro
{
	public class BuildableEdgeObject : MonoBehaviour
	{
		...
		private float previousEdgeRotation;
		...
		private void Update()
		{
			if (!isObjectBuilt && showGridBelowObject)
			{
				...
				if (edgeRotation != previousEdgeRotation)
				{
					HandleVisualCanvasGridRotation();
				}
		...
		{
			Transform transform = UnityEngine.Object.Instantiate(buildableEdgeObjectTypeSO.objectPrefab[UnityEngine.Random.Range(0, buildableEdgeObjectTypeSO.objectPrefab.Length)], Vector3.zero, Quaternion.identity);
			transform.name = transform.name.Replace("(Clone)", "").Trim();
			transform.rotation = Quaternion.Euler(0f, buildableEdgeObjectTypeSO.GetRotationAngle(dir), 0f);
			transform.localPosition = worldPosition;
			if (buildableEdgeObjectTypeSO.setBuiltObjectLayer)
			{
				SetLayerRecursive(transform.gameObject, LayerNumber(buildableEdgeObjectTypeSO.builtObjectLayer));
			}
			float gridCellSize = system.GetGridCellSize();
			Vector2Int vector2Int = buildableEdgeObjectTypeSO.CalculatePlacedObjectSize(gridCellSize);
			BuildableEdgeObject component = transform.GetComponent<BuildableEdgeObject>();
			if (system.gridAxis == GridAxis.XZ)
			{
				...
				else
				{
					...
					{
					case 0:
						transform.localPosition = new Vector3(transform.localPosition.x + (float)vector2Int.x * gridCellSize / 2f, worldPosition.y, transform.localPosition.z + (float)vector2Int.y * gridCellSize);
						transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y - edgeRotation, transform.localEulerAngles.z);
						if (edgeRotation == 0f)
						{
						...
						break;
					case 90:
						transform.localPosition = new Vector3(transform.localPosition.x + gridCellSize, worldPosition.y, transform.localPosition.z - (float)vector2Int.x * gridCellSize / 2f + gridCellSize);
						transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y - edgeRotation, transform.localEulerAngles.z);
						if (edgeRotation == 0f)
						{
						...
						break;
					case 180:
						transform.localPosition = new Vector3(transform.localPosition.x - (float)vector2Int.x * gridCellSize / 2f + gridCellSize, worldPosition.y, transform.localPosition.z - (float)vector2Int.y * gridCellSize + gridCellSize);
						transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y - edgeRotation, transform.localEulerAngles.z);
						if (edgeRotation == 0f)
						{
							transform.localPosition = new Vector3(transform.localPosition.x + component.GetObjectPivotOffset().x, transform.localPosition.y - component.GetObjectPivotOffset().y, transform.localPosition.z + component.GetObjectPivotOffset().z);
						...
						break;
					case 270:
						transform.localPosition = new Vector3(transform.localPosition.x, worldPosition.y, transform.localPosition.z + (float)vector2Int.x * gridCellSize / 2f);
						transform.localEulerAngles = new Vector3(transform.localEulerAngles.x, transform.localEulerAngles.y - edgeRotation, transform.localEulerAngles.z);
						if (edgeRotation == 0f)
						{
							transform.localPosition = new Vector3(transform.localPosition.x + component.GetObjectPivotOffset().z, transform.localPosition.y - component.GetObjectPivotOffset().y, transform.localPosition.z - component.GetObjectPivotOffset().x);
			...
			component.edgeRotation = edgeRotation;
			...
			{
				component.spawnedCanvas = UnityEngine.Object.Instantiate(component.buildableEdgeObjectTypeSO.objectGridCanvas.gameObject, Vector3.zero, Quaternion.identity);
		...
		{
			BuildableEdgeObject.OnBuildableEdgeObjectBuilt?.Invoke(this);
		}
		...
		private void HandleVisualCanvasGrid(EasyGridBuilderPro activeGridSystem)
		{
			if (activeGridSystem.gridAxis == GridAxis.XZ)
			{
				...
				if (!canvasHandleCalled)
				{
					Vector2 sizeDelta = new Vector2((float)buildableEdgeObjectTypeSO.CalculatePlacedObjectSize(activeGridSystem.GetGridCellSize()).x * activeGridSystem.GetGridCellSize(), (float)buildableEdgeObjectTypeSO.CalculatePlacedObjectSize(activeGridSystem.GetGridCellSize()).y * activeGridSystem.GetGridCellSize());
					...
					else
					{
						...
						spawnedCanvas.transform.localPosition = new Vector3(sizeDelta.x / -2f, 0f, 0f - sizeDelta.y);
			...
			else
			{
				...
				if (!canvasHandleCalled)
				{
					Vector2 sizeDelta2 = new Vector2((float)buildableEdgeObjectTypeSO.CalculatePlacedObjectSize(activeGridSystem.GetGridCellSize()).x * activeGridSystem.GetGridCellSize(), (float)buildableEdgeObjectTypeSO.CalculatePlacedObjectSize(activeGridSystem.GetGridCellSize()).y * activeGridSystem.GetGridCellSize());
		...
		private void HandleVisualCanvasGridRotation()
		{
			...
			if (activeGridSystem.gridAxis == GridAxis.XZ)
			{
				if (!IsRotateObjectForXZ())
				{
					...
					spawnedCanvas.transform.localPosition = new Vector3(spawnedCanvas.transform.localPosition.x, spawnedCanvas.transform.localPosition.y, spawnedCanvas.transform.localPosition.z * -1f);
				}
			}
			else
			{
		...
		public SaveObject GetSaveObject()
		{
			...
			{
				buildableEdgeObjectTypeSOName = buildableEdgeObjectTypeSO.name,
