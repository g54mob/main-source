...
{
	[CreateAssetMenu(menuName = "SoulGames/Easy Grid Builder Pro/BuildableEdgeObjectTypeSO", order = 0)]
	public class BuildableEdgeObjectTypeSO : ScriptableObject
	{
		...
		public Vector2Int GetRotationOffset(Dir dir, float cellSize)
		{
			...
			return dir switch
			{
				Dir.Left => new Vector2Int(0, 0), 
				Dir.Up => new Vector2Int(0, 0), 
				Dir.Right => new Vector2Int(0, 0), 
		...
		{
			int num = 0;
			float x = objectPrefab[0].GetComponent<BuildableEdgeObject>().GetObjectScale().x;
			float y = objectPrefab[0].GetComponent<BuildableEdgeObject>().GetObjectScale().y;
		...
		public List<Vector2Int> GetGridPositionList(Vector2Int offset, Dir dir, float cellSize)
		{
			...
			default:
			{
				for (int num = 0; num < vector2Int.x; num++)
				{
					for (int num2 = 0; num2 < vector2Int.y; num2++)
					{
						list.Add(offset + new Vector2Int(num, num2));
					}
				}
				break;
			}
			...
			case Dir.Left:
			{
				for (int m = 0; m < vector2Int.y; m++)
				{
					for (int n = 0; n < vector2Int.x; n++)
					{
						list.Add(offset + new Vector2Int(m, n));
					}
				}
				break;
			}
