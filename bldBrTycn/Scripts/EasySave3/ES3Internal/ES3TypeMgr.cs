...
namespace ES3Internal
{
	...
	public static class ES3TypeMgr
	{
		...
		private static Dictionary<Type, ES3GenericType> genericTypes = null;
		...
		internal static ES3Type CreateES3Type(Type type, bool throwException = true)
		{
			...
			else
			{
				...
				if (!ES3Reflection.IsGenericType(type) || !genericTypes.TryGetValue(ES3Reflection.GetGenericTypeDefinition(type), out var value))
				{
				...
				{
					Type[] genericArguments = ES3Reflection.GetGenericArguments(type);
					eS3Type = (ES3Type)Activator.CreateInstance(value.GetGenericES3Type().MakeGenericType(genericArguments), type);
				}
			}
		...
		internal static void Init()
		{
			lock (_lock)
			{
				...
				genericTypes = new Dictionary<Type, ES3GenericType>();
				ES3Reflection.GetInstances<ES3Type>();
				foreach (ES3GenericType instance in ES3Reflection.GetInstances<ES3GenericType>())
				{
					if (ES3Reflection.IsGenericType(instance.GetGenericType()))
					{
						genericTypes.TryAdd(instance.GetGenericType(), instance);
					}
				}
				if (types == null || types.Count == 0)
				{
