...
public abstract class ES3Reader : IDisposable
{
	...
	public readonly struct ES3ReaderPropertyEnumerator
	{
		public struct Enumerator : IEnumerator<string>, IEnumerator, IDisposable
		{
			private readonly ES3Reader _reader;

			public string Current { get; private set; }

			object IEnumerator.Current => Current;

			public Enumerator(ES3Reader reader)
			{
				_reader = reader;
				Current = null;
			}
			...
			public bool MoveNext()
			{
				...
				{
					string overridePropertiesName = _reader.overridePropertiesName;
					_reader.overridePropertiesName = null;
					Current = overridePropertiesName;
					return true;
				}
				string current;
				if ((current = _reader.ReadPropertyName()) == null)
				{
					return false;
				}
				Current = current;
		...
		public readonly ES3Reader reader;
	...
	internal Type ReadTypeFromHeader<T>()
	{
		...
		if (settings.typeChecking)
		{
			...
			if (type == null)
			{
				throw new TypeLoadException("Trying to load data of type " + typeof(T)?.ToString() + ", but the type of data contained in file no longer exists. This may be because the type has been removed from your project or renamed.");
			}
			if (type != typeof(T))
			{
