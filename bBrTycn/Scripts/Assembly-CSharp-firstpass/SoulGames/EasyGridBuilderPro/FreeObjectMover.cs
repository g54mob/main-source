...
namespace SoulGames.EasyGridBuilderPro
{
	public class FreeObjectMover : MonoBehaviour
	{
		...
		private void LateUpdate()
		{
			if (!(MultiGridManager.Instance.activeGridSystem == null) && movingObject != null)
			{
				...
				else
				{
					...
					base.transform.rotation = Quaternion.Lerp(base.transform.rotation, Quaternion.Euler(0f, buildableFreeObjectRotation2, 0f), Time.deltaTime * 25f);
				}
			}
		}
		...
		public void SetGridModeReset()
		{
			...
			{
				FreeObjectMover.OnBuildableFreeObjectStoppedMoving?.Invoke(movingObject.GetComponent<BuildableFreeObject>().GetOwnGridSystem(), movingObject);
		...
		private void HandleBuildingMovingXZ()
		{
			if (currentActiveSystem.GetGridObjectXZ(GetPlacedObjectCalculatedMouseWorldPositionXZ()) != null || currentActiveSystem.GetGridObjectXZ(GetMouseWorldPosition()) != null)
			{
				...
				if (buildableGridObject != null)
				{
					...
					FreeObjectMover.OnBuildableFreeObjectStartMoving?.Invoke(movingObject.GetComponent<BuildableGridObject>().GetOwnGridSystem(), movingObject);
					HookMovingObject();
				}
				else
				{
		...
		private void DeselectSelectedObject()
		{
			...
			{
				Debug.Log("Here");
				FreeObjectMover.OnBuildableFreeObjectStoppedMoving?.Invoke(movingObject.GetComponent<BuildableGridObject>().GetOwnGridSystem(), movingObject);
				movingObject.GetComponent<BuildableGridObject>().GetOwnGridSystem().SetGridMode(GridMode.None);
				movingObject.transform.position = movingObjectPreviousPosition;
				movingObject.transform.eulerAngles = movingObjectPreviousRotation;
				movingObject.transform.SetParent(null);
