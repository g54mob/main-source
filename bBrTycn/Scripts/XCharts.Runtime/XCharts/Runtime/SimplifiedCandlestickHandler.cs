...
namespace XCharts.Runtime
{
	...
	internal sealed class SimplifiedCandlestickHandler : SerieHandler<SimplifiedCandlestick>
	{
		...
		private void DrawCandlestickSerie(VertexHelper vh, SimplifiedCandlestick serie)
		{
			...
			ItemStyle itemStyle = serie.itemStyle;
			...
			for (int i = serie.minShow; i < num2; i++)
			{
				...
				Vector3 item = new Vector3(num5 + num + barWidth / 2f, num7 + num11 - num4);
				serie.context.dataPoints.Add(item);
				serie.context.dataIndexs.Add(serieData.index);
				...
				Vector3 vector = new Vector3(center.x, num6 + (float)((currData3 - num10) / num9 * (double)component3.context.height));
				...
				Vector3 vector2 = new Vector3(center.x, p3.y);
				if (flag2)
				{
					UGL.DrawLine(vh, vector, endPoint, num4, color2);
				}
				else if (barWidth > 2f * num4)
				{
					...
					if (flag3)
					{
						UGL.DrawLine(vh, startPoint, vector, num4, color2);
						UGL.DrawLine(vh, vector2, endPoint, num4, color2);
					}
					else
					{
						UGL.DrawLine(vh, vector2, vector, num4, color2);
						UGL.DrawLine(vh, startPoint, endPoint, num4, color2);
					}
				}
				else
				{
					UGL.DrawLine(vh, startPoint, vector2, Mathf.Max(num4, barWidth / 2f), color2);
				}
			}
