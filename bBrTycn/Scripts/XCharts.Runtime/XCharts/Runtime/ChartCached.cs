	...
	public static class ChartCached
	{
		...
		public static string NumberToStr(double value, string formatter)
		{
			...
			if (!s_NumberToStr[value].ContainsKey(formatter))
			{
				...
				else if (formatter.StartsWith("D") || formatter.StartsWith("d") || formatter.StartsWith("X") || formatter.StartsWith("x"))
				{
		...
		public static string GetSerieLabelName(string prefix, int i, int j)
		{
			...
			string text = prefix + "_" + i + "_" + j;
			s_SerieLabelName[key] = text;
			return text;
		}
		...
		internal static string GetComponentObjectName(MainComponent component)
		{
			...
			if (s_ComponentObjectName.TryGetValue(type, out var value))
			{
				if (!value.TryGetValue(component.index, out var value2))
				{
					value2 = GetTypeName(type) + component.index;
					value[component.index] = value2;
				}
				return value2;
			}
			string text = GetTypeName(type) + component.index;
			value = new Dictionary<int, string>();
			value.Add(component.index, text);
			s_ComponentObjectName[type] = value;
			return text;
		}

		internal static string GetAxisLabelName(int index)
		{
			if (!s_AxisLabelName.TryGetValue(index, out var value))
			{
				value = s_DefaultAxis + index;
				s_AxisLabelName[index] = value;
				return value;
			}
			return value;
		}
