...
namespace CTS
{
	public class BloodyRad : MachineBase, IProcessMachine, IDestructibleFurniture, IInteractiveFurniture, IVisibleBBTObject, IBBTObject, IObject, IVisible, IMachine, IManageableFurniture
	{
		[SerializeField]
		[Space(10f)]
		...
		private string _idleMachineInAnimatorTrigger = "Idle";

		[SerializeField]
		[BoxGroup("Animation Settings")]
		[AnimatorParam("_animator")]
		private string _processAnimatorTrigger = "Process";

		[SerializeField]
		[BoxGroup("Animation Settings")]
		[AnimatorParam("_animator")]
		private string _openDoorAnimatorTrigger = "OpenDoor";

		[SerializeField]
		[BoxGroup("Animation Settings")]
		[AnimatorParam("_animator")]
		private string _closeDoorAnimatorTrigger = "CloseDoor";

		[SerializeField]
		[Space(10f)]
		[BoxGroup("GameObject Links")]
		private StockItemSO _radioactiveBlood;
		...
		[SerializeField]
		private float _emissionAnimationLoopDuration = 0.25f;
		...
		public static event Action<BloodyRad, StockStack> BloodBagGenerated;
		...
		{
			if (!(machine is BloodyRad bloodyRad))
			{
				return false;
			}
			if (!bloodyRad.HasAVictim)
			{
				return false;
			}
			return bloodyRad._processDuration == 0f;
		}
		...
		protected override void OnMachineSwitchPower(EMachinePowerState value)
		{
			...
			case EMachinePowerState.On:
				...
				{
					StartCoroutine(GenerateRadBlood());
				}
				break;
		...
		protected override Sequence LoadIn()
		{
			...
			{
				_animator.SetTrigger(_openDoorAnimatorTrigger);
				_victim.Animator.PlayPunctualInstantly(AgentAnim.BloodyRadCustomerEnter);
			...
			if (!string.IsNullOrEmpty(_closeDoorAnimatorTrigger))
			{
				sequence.AppendCallback(delegate
				{
					_animator.SetTrigger(_closeDoorAnimatorTrigger);
					if ((bool)SFXMachineList)
					{
		...
		protected override Sequence Unload()
		{
			...
			if (!string.IsNullOrEmpty(_openDoorAnimatorTrigger))
			{
				sequence.AppendCallback(delegate
				{
					_animator.SetTrigger(_openDoorAnimatorTrigger);
			...
			sequence.AppendCallback(delegate
			{
				_victim.Animator.PlayPunctualInstantly(AgentAnim.BloodyRadCustomerExit);
			});
		...
		public IEnumerator GenerateRadBlood(float startTime = 0f)
		{
			Timer = startTime;
			...
			if (!string.IsNullOrEmpty(_processAnimatorTrigger))
			{
				_animator.SetTrigger(_processAnimatorTrigger);
			}
			_renderer.material.DOColor(_processingColor, "_EmissionColor", _emissionAnimationLoopDuration).SetLoops(-1, LoopType.Yoyo);
			while (BloodBagsGenerated < BloodBagsAmountTarget)
			{
				float deltaTime = Time.deltaTime;
				if (!Stocks.IsAtMaxCapacityWithRestriction(_radioactiveBlood))
				{
					if (_forceStopSuck)
					{
						if (!_isInUnloadProcess)
						{
							_animator.SetTrigger(_idleMachineInAnimatorTrigger);
						}
						if ((bool)SFXMachineList)
						{
							base.MachineSoundManager.StopAllSFXMachine();
					...
					if (_efficiencyTimer >= _efficiencyInterval)
					{
						...
						StockStack itemStack = new StockStack(_radioactiveBlood, 1, (_victim is Customer customer) ? customer.BloodQuality : 5);
						BloodyRad.BloodBagGenerated?.Invoke(this, itemStack);
			...
			_renderer.material.DOColor(_idleColor, "_EmissionColor", _emissionAnimationLoopDuration);
			if ((bool)SFXMachineList)
			{
				base.MachineSoundManager.StopAllSFXMachine();
			}
