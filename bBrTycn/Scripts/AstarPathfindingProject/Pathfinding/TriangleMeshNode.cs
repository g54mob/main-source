...
namespace Pathfinding
{
	public class TriangleMeshNode : MeshNode
	{
		...
		{
			GetVerticesInGraphSpace(out var @int, out var int2, out var int3);
		...
		public bool GetPortal(GraphNode toNode, List<Vector3> left, List<Vector3> right, bool backwards, out int aIndex, out int bIndex)
		{
			...
			case -1:
				if (connections != null)
				{
					...
					{
						if (connections[j].node.GraphIndex != base.GraphIndex && connections[j].node is NodeLink3Node nodeLink3Node && nodeLink3Node.GetOther(this) == triangleMeshNode)
						{
			...
			default:
			{
				...
				{
					INavmeshHolder navmeshHolder = GetNavmeshHolder(base.GraphIndex);
					navmeshHolder.GetTileCoordinates(num2, out var x, out var z);
					navmeshHolder.GetTileCoordinates(num3, out var x2, out var z2);
					int i;
					if (Math.Abs(x - x2) == 1)
					{
						i = 2;
