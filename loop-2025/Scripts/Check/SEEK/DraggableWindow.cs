...
{
	/// <summary>
	/// Handles window dragging functionality. Attach this to the main window panel.
	/// The entire panel becomes draggable, but you can specify a dragRegion for title-bar-only dragging.
	/// </summary>
	public class DraggableWindow : MonoBehaviour, IPointerDownHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler
	{
		...
		private void OnBeginDrag(int buttonIndex)
		{
			...
			if (moveToTopWhenDrag == true)
			{
				#region old approach
				/* before
				// Store original parent and move to root for proper layering
				originalParent = windowRect.parent;
				windowRect.SetParent(windowRect.root);
				windowRect.SetAsLastSibling(); // Bring to front\
				*/

				// Store original parent and move to root for proper layering
				/*
				originalParent = this.layerEntire.parent;
				layerEntire.SetParent(layerEntire.root);
				*/
				#endregion
				this.layerEntireAsTopLayerAtDragStart(); // Bring to front, its also handled by Attaching SetForegroundLayer
			}
