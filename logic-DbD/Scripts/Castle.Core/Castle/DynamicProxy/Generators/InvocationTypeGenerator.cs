...
namespace Castle.DynamicProxy.Generators
{
	internal abstract class InvocationTypeGenerator : IGenerator<AbstractTypeEmitter>
	{
		...
		protected virtual void ImplementInvokeMethodOnTarget(AbstractTypeEmitter invocation, ParameterInfo[] parameters, MethodEmitter invokeMethodOnTarget, Reference targetField)
		{
			...
			IExpression[] array = new IExpression[parameters.Length];
			...
			{
				invokeMethodOnTarget.CodeBuilder.AddStatement(callbackMethodInvocation);
			}
			AssignBackByRefArguments(invokeMethodOnTarget, dictionary);
			if (callbackMethod.ReturnType != typeof(void))
			{
				MethodInvocationExpression statement = new MethodInvocationExpression(SelfReference.Self, InvocationMethods.SetReturnValue, new ConvertExpression(typeof(object), localReference2.Type, localReference2));
				invokeMethodOnTarget.CodeBuilder.AddStatement(statement);
			}
		...
		private void AssignBackByRefArguments(MethodEmitter invokeMethodOnTarget, Dictionary<int, LocalReference> byRefArguments)
		{
			...
			foreach (KeyValuePair<int, LocalReference> byRefArgument in byRefArguments)
			{
				...
				invokeMethodOnTarget.CodeBuilder.AddStatement(new MethodInvocationExpression(SelfReference.Self, InvocationMethods.SetArgumentValue, new LiteralIntExpression(key), new ConvertExpression(typeof(object), value.Type, value)));
			}
		...
		private void CreateConstructor(AbstractTypeEmitter invocation)
		{
			...
			ArgumentReference[] baseCtorArguments = GetBaseCtorArguments(targetType, out baseConstructor);
			...
			ConstructorInfo constructorInfo = baseConstructor;
			IExpression[] args = baseCtorArguments;
			codeBuilder.AddStatement(new ConstructorInvocationStatement(constructorInfo, args));
		...
		{
			MethodInvocationExpression statement = new MethodInvocationExpression(InvocationMethods.ThrowOnNoTarget);
			invokeMethodOnTarget.CodeBuilder.AddStatement(statement);
			invokeMethodOnTarget.CodeBuilder.AddStatement(new ReturnStatement());
		}
		...
		private void ImplementChangeInvocationTarget(AbstractTypeEmitter invocation, FieldReference targetField)
		{
			...
			methodEmitter.CodeBuilder.AddStatement(new AssignStatement(targetField, new ConvertExpression(targetType, methodEmitter.Arguments[0])));
		...
		private void ImplementChangeProxyTarget(AbstractTypeEmitter invocation, ClassEmitter @class)
		{
			...
			FieldReference right = new FieldReference(InvocationMethods.ProxyObject);
			LocalReference localReference = methodEmitter.CodeBuilder.DeclareLocal(typeof(IProxyTargetAccessor));
			methodEmitter.CodeBuilder.AddStatement(new AssignStatement(localReference, new ConvertExpression(localReference.Type, right)));
			MethodInfo methodInfo = typeof(IProxyTargetAccessor).GetMethod("DynProxySetTarget");
			methodEmitter.CodeBuilder.AddStatement(new MethodInvocationExpression(localReference, methodInfo, methodEmitter.Arguments[0])
			{
				VirtualCall = true
			});
			methodEmitter.CodeBuilder.AddStatement(new ReturnStatement());
