...
public sealed class TMP_TableHyperlinkHandler : MonoBehaviour, ISerializationCallbackReceiver, IPointerClickHandler, IEventSystemHandler, IPointerEnterHandler, IPointerExitHandler
{
	[SerializeField]
	...
	private static bool isCursorClosed;
	...
	private IEnumerator SetCursorClose()
	{
		isCursorClosed = true;
		CursorManager.SetCursorClose();
		yield return new WaitForSeconds(0.2f);
		isCursorClosed = false;
	}
	...
	private IEnumerator CheckCursor()
	{
		while (isEntered)
		{
			...
			if (!isCursorClosed && IsHoverLink(Mouse.current.position.ReadValue()))
			{
				CursorManager.SetCursorOpen();
			}
			else if (!isCursorClosed)
			{
