...
{
	protected static int netsize = 256;
	...
	protected bool reserveSizeAutoTrans;
	...
	{
		byte[] array = new byte[3 * (reserveSizeAutoTrans ? (netsize + 1) : netsize)];
		int[] array2 = new int[reserveSizeAutoTrans ? (netsize + 1) : netsize];
		for (int i = 0; i < netsize; i++)
		{
