...
namespace RLD
{
	public class TriangPrismShape3D : Shape3D
	{
		...
		public Vector3 FrontCenter
		{
			get
			{
				return Center - Look * _depth * 0.5f;
			}
			set
			{
				_baseCenter = value + 0.5f * (Look * _depth - CentralAxis * _height);
			}
		}
		...
		public Vector3 Center
		{
			...
			set
			{
				_baseCenter = value - CentralAxis * _height * 0.5f;
			}
		}
		...
		public Vector3 MidTip
		{
			get
			{
				return BaseCenter + 0.5f * (CentralAxis * _height + Look * _depth);
			}
			set
			{
				_baseCenter = value - 0.5f * (Look * _depth - CentralAxis * _height);
		...
		public float PtContainEps
		{
			get
			{
				return _epsilon.PtContainEps;
			}
			set
			{
				_epsilon.PtContainEps = value;
			}
		}
		...
		{
			Graphics.DrawMeshNow(Singleton<MeshPool>.Get.UnitTriangularPrism, Matrix4x4.TRS(_baseCenter, _rotation, new Vector3(_width, _height, _depth)));
		}

		public override void RenderWire()
		{
			Graphics.DrawMeshNow(Singleton<MeshPool>.Get.UnitWireTriangularPrism, Matrix4x4.TRS(_baseCenter, _rotation, new Vector3(_width, _height, _depth)));
		}

		public void MakeEquilateral(float sideLength)
		{
			_width = sideLength;
			float num = 0.5f * _width;
			_depth = Mathf.Sqrt(sideLength * sideLength - num * num);
		}
