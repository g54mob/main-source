...
namespace RLD
{
	public class GizmoLineSlider3DCollection
	{
		...
		public GizmoLineSlider3D this[int id] => _handleIdToSlider[id];
		...
		{
			return _sliders.FindAll((GizmoLineSlider3D item) => item.Cap3DHandleId == capHandleId).Count != 0;
		}
		...
		{
			List<GizmoLineSlider3D> list = new List<GizmoLineSlider3D>(_sliders);
			Vector3 cameraPos = renderCamera.transform.position;
			list.Sort(delegate(GizmoLineSlider3D s0, GizmoLineSlider3D s1)
			{
