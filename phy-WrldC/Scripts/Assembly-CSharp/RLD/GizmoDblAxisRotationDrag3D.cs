...
namespace RLD
{
	public class GizmoDblAxisRotationDrag3D : GizmoScreenDrag
	{
		...
		public float RelativeRotation0 => _relativeRotation0;

		public float RelativeRotation1 => _relativeRotation1;

		public float TotalRotation0 => _totalRotation0;

		public float TotalRotation1 => _totalRotation1;
		...
		{
			Vector2 lhs = MonoSingleton<RTInputDevice>.Get.Device.GetFrameDelta();
			_relativeRotation0 = Vector2.Dot(lhs, _workData.ScreenAxis0) * base.Sensitivity;
			_relativeRotation1 = Vector2.Dot(lhs, _workData.ScreenAxis1) * base.Sensitivity;
			if (_relativeRotation0 == 0f && _relativeRotation1 == 0f)
			{
			...
			if (CanSnap())
			{
				...
				_accumSnapDrag0 %= 360f;
				_accumSnapDrag1 += _relativeRotation1;
				_accumSnapDrag1 %= 360f;
				if (_workData.SnapMode == GizmoSnapMode.Absolute && _adjustRotationForAbsSnap)
				{
					...
					float totalRotation = _totalRotation0;
					if (numSnapSteps.AbsFracSteps < 0.5f)
					{
						_totalRotation0 = (float)numSnapSteps.AbsIntNumSteps * _workData.SnapStep0 * Mathf.Sign(_totalRotation0);
					}
					else
					{
						_totalRotation0 = (float)(numSnapSteps.AbsIntNumSteps + 1) * _workData.SnapStep0 * Mathf.Sign(_totalRotation0);
					}
					_accumSnapDrag0 = 0f;
					_relativeRotation0 = _totalRotation0 - totalRotation;
					numSnapSteps = SnapMath.CalculateNumSnapSteps(_workData.SnapStep1, _totalRotation1);
					totalRotation = _totalRotation1;
					if (numSnapSteps.AbsFracSteps < 0.5f)
					{
						_totalRotation1 = (float)numSnapSteps.AbsIntNumSteps * _workData.SnapStep1 * Mathf.Sign(_totalRotation1);
					}
					else
					{
						_totalRotation1 = (float)(numSnapSteps.AbsIntNumSteps + 1) * _workData.SnapStep1 * Mathf.Sign(_totalRotation1);
					}
					_accumSnapDrag1 = 0f;
					_relativeRotation1 = _totalRotation1 - totalRotation;
			...
			else
			{
				...
				_accumSnapDrag0 = (_accumSnapDrag1 = 0f);
				_totalRotation0 += _relativeRotation0;
				_totalRotation1 += _relativeRotation1;
				_relativeDragRotation = Quaternion.AngleAxis(_relativeRotation1, _workData.Axis1) * Quaternion.AngleAxis(_relativeRotation0, _workData.Axis0);
			}
		...
		protected override void OnSessionEnd()
		{
			_accumSnapDrag0 = 0f;
			_accumSnapDrag1 = 0f;
			_relativeRotation0 = 0f;
			_relativeRotation1 = 0f;
			_totalRotation0 = 0f;
			_totalRotation1 = 0f;
		}
	}
