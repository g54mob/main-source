...
namespace RLD
{
	public class ArcShape2D : Shape2D
	{
		...
		public Vector2 Origin
		{
			...
			set
			{
				Vector2 normalized = (_startPoint - _origin).normalized;
				_origin = value;
		...
		public override void RenderBorder(Camera camera)
		{
			...
			if ((_borderRenderDesc.BorderFlags & BorderRenderFlags.ArcBorder) != BorderRenderFlags.None)
			{
				GLRenderer.DrawLines2D(_borderPoints, camera);
			}
			if ((_borderRenderDesc.BorderFlags & BorderRenderFlags.ExtremitiesBorder) != BorderRenderFlags.None)
			{
		...
		{
			Vector3 vector = _startPoint - _origin;
			_endPoint = _origin.ToVector3() + Quaternion.AngleAxis(DegreeAngleFromStart, Vector3.forward) * vector;
		}
	}
}
