...
namespace RLD
{
	public class PyramidShape3D : Shape3D
	{
		...
		{
			Graphics.DrawMeshNow(Singleton<MeshPool>.Get.UnitPyramid, Matrix4x4.TRS(_baseCenter, _rotation, new Vector3(_baseWidth, _height, _baseDepth)));
		}

		public override void RenderWire()
		{
			Graphics.DrawMeshNow(Singleton<MeshPool>.Get.UnitWirePyramid, Matrix4x4.TRS(_baseCenter, _rotation, new Vector3(_baseWidth, _height, _baseDepth)));
		}
		...
		{
			List<Vector3> baseCornerPoints = GetBaseCornerPoints();
			baseCornerPoints.Add(Tip);
			return new AABB(baseCornerPoints);
		}
