	...
	public class BoxShape3D : Shape3D
	{
		...
		public class WireRenderDescriptor
		{
			...
			public float CornerLinePercentage
			{
				get
				{
					return _cornerLinePercentage;
				}
				set
				{
					_cornerLinePercentage = Mathf.Clamp(value, 0f, 1f);
				}
			}
		}
		...
		public float DepthEps
		{
			get
			{
				return _epsilon.DepthEps;
			}
			set
			{
				_epsilon.DepthEps = value;
			}
		}
		...
		public Quaternion Rotation
		{
			...
			set
			{
				_rotation = QuaternionEx.Normalize(value);
			}
		}
		...
		{
			return direction.AbsDot(_rotation * _size);
		}
		...
		{
			Vector3 vector = BoxMath.CalcBoxFaceCenter(_center, _size, _rotation, boxFace);
			Center = newCenter + (_center - vector);
		}

		public override void RenderSolid()
		{
			Graphics.DrawMeshNow(Singleton<MeshPool>.Get.UnitBox, Matrix4x4.TRS(_center, _rotation, _size));
		}

		public override void RenderWire()
		{
			OBB box = new OBB(GetAABB(), _rotation);
			if (_wireRenderDesc.WireMode == WireRenderMode.Wire)
			{
				GraphicsEx.DrawWireBox(box);
			}
			else
			{
				GraphicsEx.DrawWireCornerBox(box, _wireRenderDesc.CornerLinePercentage);
			}
		}
