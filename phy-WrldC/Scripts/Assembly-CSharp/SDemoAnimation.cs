...
public class SDemoAnimation : MonoBehaviour
{
	...
	private static SDemoAnimation _instance;
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Move(targetGO, fromPosition, toPosition, time, 0f, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Move(targetGO, fromPosition, toPosition, time, delay, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Scale(targetGO, fromScale, toScale, time, 0f, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Scale(targetGO, fromScale, toScale, time, delay, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _Scale(GameObject targetGO, Vector3 fromScale, Vector3 toScale, float time, float delay, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		if (control.m_State != SDemoControl.State.Kill)
		{
			...
			case LoopType.Loop:
				...
				break;
			case LoopType.PingPong:
				StartCoroutine(_Scale(targetGO, toScale, fromScale, time, delay, loop, onComplete, control));
				break;
			}
		}
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Rotate(targetGO, fromEulerAngle, toEulerScale, time, 0f, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Rotate(targetGO, fromEulerAngle, toEulerScale, time, delay, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _Rotate(GameObject targetGO, Vector3 fromEulerAngle, Vector3 toEulerAngle, float time, float delay, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		if (control.m_State != SDemoControl.State.Kill)
		{
			...
			{
			case LoopType.Loop:
				StartCoroutine(_Rotate(targetGO, fromEulerAngle, toEulerAngle, time, delay, loop, onComplete, control));
				break;
			case LoopType.PingPong:
				StartCoroutine(_Rotate(targetGO, toEulerAngle, fromEulerAngle, time, delay, loop, onComplete, control));
				break;
			}
		}
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_FloatTo(fromValue, toValue, time, 0f, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_FloatTo(fromValue, toValue, time, delay, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _FloatTo(float fromValue, float toValue, float time, float delay, Action<float> onUpdate, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		while (elapsedTime < time)
		{
			...
			{
				elapsedTime += Time.deltaTime;
				float obj = Mathf.Lerp(fromValue, toValue, elapsedTime / time);
				onUpdate(obj);
			}
			else if (control.m_State == SDemoControl.State.Kill)
			{
		...
		if (control.m_State != SDemoControl.State.Kill)
		{
			...
			case LoopType.Loop:
				...
				break;
			case LoopType.PingPong:
				StartCoroutine(_FloatTo(toValue, fromValue, time, delay, onUpdate, loop, onComplete, control));
				break;
			}
		}
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector2To(fromValue, toValue, time, 0f, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector2To(fromValue, toValue, time, delay, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _Vector2To(Vector2 fromValue, Vector2 toValue, float time, float delay, Action<Vector2> onUpdate, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		while (elapsedTime < time)
		{
			...
			{
				elapsedTime += Time.deltaTime;
				Vector2 obj = Vector2.Lerp(fromValue, toValue, elapsedTime / time);
				onUpdate(obj);
			}
			else if (control.m_State == SDemoControl.State.Kill)
			{
		...
		if (control.m_State != SDemoControl.State.Kill)
		{
			...
			case LoopType.Loop:
				...
				break;
			case LoopType.PingPong:
				StartCoroutine(_Vector2To(toValue, fromValue, time, delay, onUpdate, loop, onComplete, control));
				break;
			}
		}
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector3To(fromValue, toValue, time, 0f, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector3To(fromValue, toValue, time, delay, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _Vector3To(Vector3 fromValue, Vector3 toValue, float time, float delay, Action<Vector3> onUpdate, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		while (elapsedTime < time)
		{
			...
			{
				elapsedTime += Time.deltaTime;
				Vector3 obj = Vector3.Lerp(fromValue, toValue, elapsedTime / time);
				onUpdate(obj);
			}
			else if (control.m_State == SDemoControl.State.Kill)
			{
		...
		if (control.m_State != SDemoControl.State.Kill)
		{
			...
			case LoopType.Loop:
				...
				break;
			case LoopType.PingPong:
				StartCoroutine(_Vector3To(toValue, fromValue, time, delay, onUpdate, loop, onComplete, control));
				break;
			}
		}
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector4To(fromValue, toValue, time, 0f, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	{
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Vector4To(fromValue, toValue, time, delay, onUpdate, loop, onComplete, sDemoControl));
		return sDemoControl;
	}
	...
	private IEnumerator _Vector4To(Vector4 fromValue, Vector4 toValue, float time, float delay, Action<Vector4> onUpdate, LoopType loop = LoopType.None, Action onComplete = null, SDemoControl control = null)
	{
		...
		while (elapsedTime < time)
		{
			...
			{
				elapsedTime += Time.deltaTime;
				Vector4 obj = Vector4.Lerp(fromValue, toValue, elapsedTime / time);
				onUpdate(obj);
			}
			else if (control.m_State == SDemoControl.State.Kill)
			{
	...
	public SDemoControl Wait(float time, Action onComplete)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Wait(time, 0f, onComplete, LoopType.None, sDemoControl));
		return sDemoControl;
	}
	...
	public SDemoControl Wait(float time, Action onComplete, LoopType loop = LoopType.None)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Wait(time, 0f, onComplete, loop, sDemoControl));
		return sDemoControl;
	}
	...
	public SDemoControl Wait(float time, float delay, Action onComplete, LoopType loop = LoopType.None)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_Wait(time, delay, onComplete, loop, sDemoControl));
		return sDemoControl;
	}
	...
	public SDemoControl WaitFrames(int frameNum, Action onComplete)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_WaitFrames(frameNum, 0f, onComplete, LoopType.None, sDemoControl));
		return sDemoControl;
	}
	...
	public SDemoControl WaitFrames(int frameNum, Action onComplete, LoopType loop = LoopType.None)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_WaitFrames(frameNum, 0f, onComplete, loop, sDemoControl));
		return sDemoControl;
	}
	...
	public SDemoControl WaitFrames(int frameNum, float delay, Action onComplete, LoopType loop = LoopType.None)
	{
		...
		SDemoControl sDemoControl = new SDemoControl();
		StartCoroutine(_WaitFrames(frameNum, delay, onComplete, loop, sDemoControl));
		return sDemoControl;
	}
