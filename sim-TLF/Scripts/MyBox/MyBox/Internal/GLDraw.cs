...
namespace MyBox.Internal
{
	public class GLDraw
	{
		...
		protected static bool SegmentRectIntersection(Rect bounds, ref Vector2 p1, ref Vector2 p2)
		{
			...
			float u2 = 1f;
			float num = p2.x - p1.x;
			if (ClipTest(0f - num, p1.x - bounds.xMin, ref u, ref u2) && ClipTest(num, bounds.xMax - p1.x, ref u, ref u2))
			{
				float num2 = p2.y - p1.y;
				if (ClipTest(0f - num2, p1.y - bounds.yMin, ref u, ref u2) && ClipTest(num2, bounds.yMax - p1.y, ref u, ref u2))
				{
					if ((double)u2 < 1.0)
					{
						p2.x = p1.x + u2 * num;
		...
		{
			if (Event.current != null && Event.current.type == EventType.Repaint && (!ClippingEnabled || SegmentRectIntersection(ClippingBounds, ref start, ref end)))
			{
				CreateMaterial();
				...
				else
				{
					...
					Vector3 vector2 = (vector - v2).normalized * width;
					Vector3 vector3 = new Vector3(start.x, start.y, 0f);
					Vector3 vector4 = new Vector3(end.x, end.y, 0f);
					GL.Vertex(vector3 - vector2);
					GL.Vertex(vector3 + vector2);
					GL.Vertex(vector4 + vector2);
					GL.Vertex(vector4 - vector2);
				}
				GL.End();
			}
		...
		{
			DrawBox(new Rect(topLeftCorner.x, topLeftCorner.y, bottomRightCorner.x - topLeftCorner.x, bottomRightCorner.y - topLeftCorner.y), color, width);
		}
