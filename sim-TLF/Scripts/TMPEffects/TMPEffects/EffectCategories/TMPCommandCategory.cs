...
namespace TMPEffects.EffectCategories
{
	internal class TMPCommandCategory : TMPEffectCategory<ITMPCommand>
	{
		...
		public override bool ValidateOpenTag(ParsingUtility.TagInfo tagInfo, out TMPEffectTag data, out int endIndex)
		{
			...
			ITMPCommand effect = database.GetEffect(tagInfo.name);
			if (!effect.ValidateParameters(tagParametersDict, keywordDatabase))
			{
				return false;
			}
			TMPEffectTag tMPEffectTag = new TMPEffectTag(tagInfo.name, tagInfo.prefix, tagParametersDict);
			data = tMPEffectTag;
			endIndex = ((effect.TagType == TagType.Index) ? tagInfo.startIndex : (-1));
			return true;
		}
