...
public class PPFXSpawnOnClick : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Physics.Raycast(ray, out var hitInfo) && GUIUtility.hotControl == 0 && hitInfo.collider.tag == tagName && inst != null)
		{
			GameObject gameObject = Object.Instantiate(position: new Vector3(hitInfo.point.x, 0f, hitInfo.point.z), original: inst, rotation: inst.transform.rotation);
			if (container != null)
			{
