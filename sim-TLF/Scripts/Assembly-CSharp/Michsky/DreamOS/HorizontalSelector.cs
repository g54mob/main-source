...
namespace Michsky.DreamOS
{
	...
	public class HorizontalSelector : MonoBehaviour
	{
		...
		public string saveKey = "DataKey";

		public DreamOSDataManager.DataCategory dataCategory = DreamOSDataManager.DataCategory.System;
		...
		private void Awake()
		{
			...
			cachedStateLength = DreamOSInternalTools.GetAnimatorClipLength(selectorAnimator, "HorizontalSelector_Next");
		}
		...
		private void OnEnable()
		{
			...
			if (items.Count > 0 && useLocalization && !string.IsNullOrEmpty(items[index].localizationKey) && localizedObject.CheckLocalizationStatus())
			{
		...
		public void InitializeSelector()
		{
			...
			if (saveSelected)
			{
				...
				{
					defaultIndex = DreamOSDataManager.ReadIntData(dataCategory, saveKey);
				}
				else
				{
					DreamOSDataManager.WriteIntData(dataCategory, saveKey, defaultIndex);
				}
			}
			...
			if (enableIcon)
			{
				if (labelIcon != null)
				{
					labelIcon.gameObject.SetActive(value: true);
					labelIcon.sprite = items[defaultIndex].itemIcon;
				}
				if (labelIconHelper != null)
				{
					labelIconHelper.gameObject.SetActive(value: true);
		...
		public void PreviousItem()
		{
			...
			if (saveSelected)
			{
				DreamOSDataManager.WriteIntData(dataCategory, saveKey, index);
			}
			if (enableIndicator)
			{
		...
		public void UpdateUI()
		{
			...
			else if (!enableIcon && labelIcon != null)
			{
				labelIcon.gameObject.SetActive(value: false);
				labelIconHelper.gameObject.SetActive(value: false);
			}
