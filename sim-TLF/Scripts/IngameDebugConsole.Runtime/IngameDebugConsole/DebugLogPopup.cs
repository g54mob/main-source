...
namespace IngameDebugConsole
{
	public class DebugLogPopup : MonoBehaviour, IPointerClickHandler, IEventSystemHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		private Vector2 SavedNormalizedPosition
		{
			get
			{
				if (!PlayerPrefs.HasKey("IDGPPos"))
				{
					return new Vector2(0.5f, 0f);
				}
				return JsonUtility.FromJson<Vector2>(PlayerPrefs.GetString("IDGPPos", "{}"));
			}
			set
			{
				PlayerPrefs.SetString("IDGPPos", JsonUtility.ToJson(value));
			}
		}
		...
		{
			if (RectTransformUtility.ScreenPointToLocalPointInRectangle(debugManager.canvasTR, data.position, data.pressEventCamera, out var localPoint))
			{
		...
		public void UpdatePosition(bool immediately)
		{
			...
			Vector2 vector = size * 0.5f + (immediately ? new Vector2(normalizedPosition.x * x, normalizedPosition.y * y) : (popupTransform.anchoredPosition - new Vector2(x2, y2)));
			float x3 = vector.x;
			float num = x - x3;
			float y3 = vector.y;
			float num2 = y - y3;
