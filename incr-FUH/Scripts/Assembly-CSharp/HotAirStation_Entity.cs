...
public class HotAirStation_Entity : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		else
		{
			...
			if (HotAirStation.GlobalInfo.CanBothSideAttribute.IsEnabled && base.transform.position.x < GameController.Instance.HoleLocation.transform.position.x - 0.5f)
			{
				MoveGarbage();
			}
		}
	...
	public void EmptyStorage()
	{
		...
		{
			GameController.Instance.GarbageController.PreLoadGarbage(base.transform.position, item.Weight, item.GarbageType, item.CameFrom, item.IsEvil, item.IsZap, randomForce: true);
		}
