...
public class TemplePanel : BaseBuildingPanel
{
	...
	private void Start()
	{
		...
		Title.Initialize(base.gameObject, LanguageText.GetText("Temple"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Temple"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	{
		return 0 + (StabilityRow.gameObject.activeSelf ? 1 : 0) + (LevelRow.gameObject.activeSelf ? 1 : 0) + (WorkerRow.gameObject.activeSelf ? 1 : 0);
	}
}
