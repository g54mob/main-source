...
public class ScreenCanvasController : MonoBehaviour
{
	...
	private void Update()
	{
		...
		{
			GameController.Instance.ToastPanel.AddItem(LanguageText.GetText("GainedBlueShard"));
			_cachedSkillIconVisible = true;
		...
		{
			ResearchText.text = "<color=#FFC0CB>" + GameController.Instance.ResearchPoint.Amount.ToNumber() + " " + LanguageText.GetText("RP") + " </color>";
		}
		...
		{
			HoleText.text = "<color=green>" + GameController.Instance.HoleFilled.Amount.ToNumber() + "</color>";
		}
		else if (GameController.Instance.MaxFilled >= 10000)
		{
		...
		{
			HoleText.text = "<color=green>" + GameController.Instance.HoleFilled.Amount.ToNumber() + "</color> / " + GameController.Instance.MaxFilled.ToNumber() + " " + LanguageText.GetText("Filled");
		}
	...
	private void FixedUpdate()
	{
		...
		if (CharDisplay.HasEndless)
		{
			EndingButton.GetComponent<Pulsing>().AlwaysStop();
		}
	...
	public void ProcessEndOfGame()
	{
		...
		EndOfGameController.Stats_QuestDone = AchievementDefinition.CountQuestDone(GameController.Instance.Achievements);
		EndOfGameController.Stats_QuestTotal = AchievementDefinition.CountQuestTotal(GameController.Instance.Achievements);
	...
	private void UpdateStats()
	{
		...
		if (_cachedSeeStats)
		{
			MoneyStats.text = "<color=yellow>" + GameController.Instance.Money.Get60SecAverage().ToNumber() + " $</color> / " + LanguageText.GetText("min");
			RpStats.text = "<color=#FFC0CB>" + GameController.Instance.ResearchPoint.Get60SecAverage().ToNumber() + " " + LanguageText.GetText("RP") + "</color> / " + LanguageText.GetText("min");
			HoleStats.text = "<color=green>" + GameController.Instance.HoleFilled.Get60SecAverage().ToNumber() + " " + LanguageText.GetText("Fill") + "</color> / " + LanguageText.GetText("min");
		}
	}
}
