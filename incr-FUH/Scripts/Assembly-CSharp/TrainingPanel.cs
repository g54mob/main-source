...
public class TrainingPanel : BaseBuildingPanel
{
	...
	private void Start()
	{
		Title.Initialize(base.gameObject, LanguageText.GetText("Training"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Training"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	{
		return 0 + (OverflowRow.gameObject.activeSelf ? 1 : 0) + (StabilityRow.gameObject.activeSelf ? 1 : 0) + (LevelRow.gameObject.activeSelf ? 1 : 0) + (CarryRow.gameObject.activeSelf ? 1 : 0) + (SpeedRow.gameObject.activeSelf ? 1 : 0) + (MiningRow.gameObject.activeSelf ? 1 : 0) + (ReduceCostRow.gameObject.activeSelf ? 1 : 0) + (ThrowOutputRow.gameObject.activeSelf ? 1 : 0) + (AutoDeviceRow.gameObject.activeSelf ? 1 : 0) + (WorkerRow.gameObject.activeSelf ? 1 : 0);
	}
