...
public class IndustryPanel : BaseBuildingPanel
{
	...
	private Industry Building => (Industry)PanelColumn.Buildings;

	private void Start()
	{
		Title.Initialize(base.gameObject, LanguageText.GetText("Factory"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Factory"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	public static string GetTooltip(Industry building, bool forNextLevel = false)
	{
		string text = "";
		if (!forNextLevel)
		{
			text = "Produce trash by making peons work. On the 10th cycle, produce more.\n\n";
		}
		...
		if (num == 0)
		{
			text = text + "Trash Produced: " + Industry.GlobalInfo.GetDefaultOutputPerCycle() + " per worker\n";
			text = text + "10th Cycle: " + Industry.GlobalInfo.GetDefaultOutputPer10Cycle() + " per worker\n";
			text = text + "Trash Value: " + Industry.GlobalInfo.GetDefaultGarbageSize() + "$ per level\n";
		...
		{
			text = text + "+" + Industry.GlobalInfo.GetDefaultGarbageSize() + "$ trash value\n";
