...
public class PowerPanel : BaseBuildingPanel
{
	...
	private void Start()
	{
		Title.Initialize(base.gameObject, LanguageText.GetText("Power"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Power"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	{
		return 0 + (OverflowRow.gameObject.activeSelf ? 1 : 0) + (StabilityRow.gameObject.activeSelf ? 1 : 0) + (LevelRow.gameObject.activeSelf ? 1 : 0) + (ThrowOutputRow.gameObject.activeSelf ? 1 : 0) + (AutoDeviceRow.gameObject.activeSelf ? 1 : 0) + (IncreaseRangeRow.gameObject.activeSelf ? 1 : 0) + (AddStabilityStopRow.gameObject.activeSelf ? 1 : 0) + (AddMoreCloudRow.gameObject.activeSelf ? 1 : 0) + (AddOutputWeightRow.gameObject.activeSelf ? 1 : 0) + (AddOutputAmountRow.gameObject.activeSelf ? 1 : 0) + (AddStabilityDownRow.gameObject.activeSelf ? 1 : 0) + (AddMoreRPTPRow.gameObject.activeSelf ? 1 : 0) + (WorkerRow.gameObject.activeSelf ? 1 : 0);
	}
