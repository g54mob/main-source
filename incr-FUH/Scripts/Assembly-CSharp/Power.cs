...
public class Power : BaseBuildingWorker
{
	...
	private List<PowerIncreaseType> _tempPowerSelected = new List<PowerIncreaseType>();
	...
	public BaseMoneyLevelAttribute HiddenOutputWeightAttribute = new BaseMoneyLevelAttribute("HiddenOutputWeight", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenOutputAmountAttribute = new BaseMoneyLevelAttribute("HiddenOutputAmount", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenStabilityDownAttribute = new BaseMoneyLevelAttribute("HiddenStabilityDown", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenStabilityStopAttribute = new BaseMoneyLevelAttribute("HiddenStabilityStop", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenFasterPeonAttribute = new BaseMoneyLevelAttribute("HiddenFasterPeon", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenMoreCloudAttribute = new BaseMoneyLevelAttribute("HiddenMoreCloud", 100, (int l) => 0, () => true);

	public BaseMoneyLevelAttribute HiddenMoreRP_TPAttribute = new BaseMoneyLevelAttribute("HiddenMoreRP_TP", 100, (int l) => 0, () => true);
	...
	private void SetPeonWork()
	{
		if (_tempPowerSelected.Count > 0)
		{
			PowerIncreaseType type = _tempPowerSelected[0];
			_tempPowerSelected.RemoveAt(0);
			AddPowerWorker(type);
		}
	}
	...
	{
		base.SetData(data);
		for (int i = 0; i < HiddenOutputWeightAttribute.Level; i++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.OutputWeight);
		}
		for (int j = 0; j < HiddenOutputAmountAttribute.Level; j++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.OutputAmount);
		}
		for (int k = 0; k < HiddenStabilityDownAttribute.Level; k++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.StabilityDown);
		}
		for (int l = 0; l < HiddenStabilityStopAttribute.Level; l++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.StabilityStop);
		}
		for (int m = 0; m < HiddenFasterPeonAttribute.Level; m++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.FasterPeon);
		}
		for (int n = 0; n < HiddenMoreCloudAttribute.Level; n++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.MoreCloud);
		}
		for (int num = 0; num < HiddenMoreRP_TPAttribute.Level; num++)
		{
			_tempPowerSelected.Add(PowerIncreaseType.MoreRP_TP);
		}
	}

	private void SaveHiddenAttribute()
	{
		HiddenOutputWeightAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.OutputWeight));
		HiddenOutputAmountAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.OutputAmount));
		HiddenStabilityDownAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.StabilityDown));
		HiddenStabilityStopAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.StabilityStop));
		HiddenFasterPeonAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.FasterPeon));
		HiddenMoreCloudAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.MoreCloud));
		HiddenMoreRP_TPAttribute.ForceLevel(GetPowerLevel(PowerIncreaseType.MoreRP_TP));
	}
	...
	{
		return new List<BaseSavableAttribute> { HasThrowOutputAttribute, HasIncreaseRangeAttribute, HasAutoDeviceAttribute, HiddenOutputWeightAttribute, HiddenOutputAmountAttribute, HiddenStabilityDownAttribute, HiddenStabilityStopAttribute, HiddenFasterPeonAttribute, HiddenMoreCloudAttribute, HiddenMoreRP_TPAttribute };
	}
