...
public class CompressorPanel : BaseBuildingPanel
{
	...
	private void Start()
	{
		Title.Initialize(base.gameObject, LanguageText.GetText("Compressor"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Compressor"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	{
		return 0 + (OverflowRow.gameObject.activeSelf ? 1 : 0) + (StabilityRow.gameObject.activeSelf ? 1 : 0) + (LevelRow.gameObject.activeSelf ? 1 : 0) + (CatchRow.gameObject.activeSelf ? 1 : 0) + (ThrowOutputRow.gameObject.activeSelf ? 1 : 0) + (AutoDeviceRow.gameObject.activeSelf ? 1 : 0) + (LVacuumRow.gameObject.activeSelf ? 1 : 0) + (RVacuumRow.gameObject.activeSelf ? 1 : 0) + (MoreStorageRow.gameObject.activeSelf ? 1 : 0) + (BanPeonRow.gameObject.activeSelf ? 1 : 0) + (WorkerRow.gameObject.activeSelf ? 1 : 0);
	}
