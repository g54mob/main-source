...
public class GameController : MonoBehaviour
{
	...
	public ResourceInfo BoughtBook = new ResourceInfo();
	...
	public const int DEBUG_MULTIPLIER = 1;

	public const int GOLUMN_COLUMN = 14;

	public const int SPECIAL_NUMBER = 8492;
	...
	private void Start()
	{
		...
		if (SaveManager.GameData != null)
		{
			...
			{
				if (achievement.AchievementType == AchievementDefinition.AchievementTypeEnum.GetNewBuilding && achievement.IsActivated)
				{
					Instance.SeeAllNodes = true;
		...
		if (PrestigeCount == 8 && HoleFilled.Amount >= MaxFilled && !CharDisplay.HasEndless)
		{
			EndlessModePanel.gameObject.SetActive(value: true);
		}
		else
		{
			EndlessModePanel.gameObject.SetActive(value: false);
		}
		AchievementDefinition.ResubmitAll(Achievements);
	}
	...
	private void Update()
	{
		...
		if (_achivementCheckTimer > 1f)
		{
			...
			if (AchievementDefinition.ProcessAchievements(Achievements))
			{
				...
				Instance.ToastPanel.AddItem(LanguageText.GetText("QuestCanComplete"));
			}
		}
		...
		ApiManager.Instance.API_RunCallbacks();
	}
	...
	public void EndlessModeClose()
	{
		CharDisplay.HasEndless = false;
		EndlessModePanel.gameObject.SetActive(value: false);
	}

	public void EndlessModeContinue()
	{
		CharDisplay.HasEndless = true;
		EndlessModePanel.gameObject.SetActive(value: false);
	}
	...
	public bool IsHoleFilled(bool force = false)
	{
		if (CharDisplay.HasEndless && !force)
		{
			return false;
		}
	...
	public void DropGarbage(GarbageInfo g)
	{
		...
		if (HoleFilled.Amount < MaxFilled || CharDisplay.HasEndless)
		{
	...
	private void LoadData()
	{
		...
		BoughtBook.Amount = SaveManager.GameData.BoughtBook;
		BoughtBook.TotalAmount = SaveManager.GameData.BoughtBook;
		...
		if (SaveManager.GameData.IsEndless == 1)
		{
			CharDisplay.HasEndless = true;
		}
		else
	...
	public void SaveData()
	{
		...
		SaveManager.ClearGameSaveData(CurrentSaveId);
		...
		SaveManager.GameData.BoughtBook = BoughtBook.Amount;
		...
		{
			SaveManager.GameData.IsEndless = 1;
		}
		else
		{
			SaveManager.GameData.IsEndless = 0;
		}
		...
		SaveManager.SaveGameData(CurrentSaveId);
		DiskImage.gameObject.SetActive(value: false);
		LastSave = 0f;
	}
