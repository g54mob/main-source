...
public class GarbageController : MonoBehaviour
{
	...
	private void FixedUpdate()
	{
		...
		if (_preLoadedIndex < _preLoadedGarbage.Count - 1)
		{
			for (int i = 0; i < 10; i++)
			{
				...
				if (_preLoadedGarbage[_preLoadedIndex].RandomForce)
				{
	...
	public void PreLoadGarbage(Vector3 pos, int weight, GarbageInfo.GarbageTypeEnum type, GarbageInfo.CameFromEnum cameFrom, bool isEvil, bool isZap, bool randomForce = false)
	{
		...
		preLoadedGarbage.RandomForce = randomForce;
		if (_preLoadedGarbage == null)
		{
	...
	public Garbage FindFreeGarbage(Vector3 origin, bool canBeShard)
	{
		...
		foreach (Garbage activeGarbage in ActiveGarbages)
		{
			...
			{
				if (Mathf.Abs(activeGarbage.transform.position.x - origin.x) < num && (canBeShard || (!activeGarbage.Info.IsShard && !activeGarbage.Info.IsBook)))
				{
