...
public class CatapultPanel : BaseBuildingPanel
{
	...
	private void Start()
	{
		Title.Initialize(base.gameObject, LanguageText.GetText("Catapult"));
		...
		Title.SetDynamicTooltip((TooltipPanel.TooltipInfo a) => a.Update(LanguageText.GetText("Catapult"), "Lvl " + Building.GetLevel() + "/10", GetTooltip(Building), ""));
	...
	{
		return 0 + (StabilityRow.gameObject.activeSelf ? 1 : 0) + (LevelRow.gameObject.activeSelf ? 1 : 0) + (LVacuumRow.gameObject.activeSelf ? 1 : 0) + (RVacuumRow.gameObject.activeSelf ? 1 : 0) + (MoreStorageRow.gameObject.activeSelf ? 1 : 0) + (ThrowMoreRow.gameObject.activeSelf ? 1 : 0) + (BanPeonRow.gameObject.activeSelf ? 1 : 0) + (WorkerRow.gameObject.activeSelf ? 1 : 0);
	}
}
