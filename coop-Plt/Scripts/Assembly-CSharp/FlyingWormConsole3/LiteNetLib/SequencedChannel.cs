	...
	internal sealed class SequencedChannel : BaseChannel
	{
			...
			else
			{
				lock (OutgoingQueue)
				{
					while (OutgoingQueue.Count > 0)
					{
						...
						{
							Peer.NetManager.NetPacketPool.Recycle(netPacket);
						}
					}
				}
			}
		...
		public override bool ProcessPacket(NetPacket packet)
		{
			...
			if (packet.Sequence < 32768 && num > 0)
			{
				...
				Peer.NetManager.CreateReceiveEvent(packet, (!_reliable) ? DeliveryMethod.Sequenced : DeliveryMethod.ReliableSequenced, 4, Peer);
				result = true;
			}
			if (_reliable)
			{
