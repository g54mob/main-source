...
namespace Cinemachine
{
	...
	public class CinemachineFramingTransposer : CinemachineComponentBase
	{
		...
		public override void MutateCameraState(ref CameraState curState, float deltaTime)
		{
			...
			bool flag2 = abstractFollowTargetGroup != null && m_GroupFramingMode != FramingMode.None;
			if (flag2)
			{
			...
			if (!orthographic && flag2)
			{
				...
				{
					num = a / (2f * Mathf.Tan(lens.FieldOfView * ((float)Math.PI / 180f) / 2f));
			...
			if (flag && m_TargetMovementOnly && m_prevRotation != rawOrientation)
			{
			...
			float orthoSize = (lens.Orthographic ? lens.OrthographicSize : (Mathf.Tan(0.5f * lens.FieldOfView * ((float)Math.PI / 180f)) * (num4 - vector6.z)));
		...
		private static Bounds GetScreenSpaceGroupBoundingBox(ICinemachineTargetGroup group, ref Vector3 pos, Quaternion orientation)
		{
			...
			if (zRange.x > 0f)
			{
				vector = Vector2.Max(maxAngles, UnityVectorExtensions.Abs(minAngles));
