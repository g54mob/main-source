...
namespace Cinemachine
{
	...
	public class CinemachineFollowZoom : CinemachineExtension
	{
		...
		protected override void PostPipelineStageCallback(CinemachineVirtualCameraBase vcam, CinemachineCore.Stage stage, ref CameraState state, float deltaTime)
		{
			...
			if (num > 0.0001f)
			{
				float min = num * 2f * Mathf.Tan(m_MinFOV * ((float)Math.PI / 180f) / 2f);
				float max = num * 2f * Mathf.Tan(m_MaxFOV * ((float)Math.PI / 180f) / 2f);
				value = Mathf.Clamp(value, min, max);
				if (deltaTime >= 0f && m_Damping > 0f && base.VirtualCamera.PreviousStateIsValid)
				{
					float num2 = num * 2f * Mathf.Tan(extraState.m_previousFrameZoom * ((float)Math.PI / 180f) / 2f);
