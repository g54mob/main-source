	...
	public class CinemachineFreeLook : CinemachineVirtualCameraBase
	{
		...
		private float UpdateXAxisHeading(CinemachineOrbitalTransposer orbital, float deltaTime, Vector3 up)
		{
			...
			if (mOrbitals != null && mOrbitals[1] == orbital)
			{
				...
				CachedXAxisHeading = orbital.UpdateHeading(PreviousStateIsValid ? deltaTime : (-1f), up, ref m_XAxis, ref m_RecenterToTargetHeading, CinemachineCore.Instance.IsLive(this));
				if (m_BindingMode == CinemachineTransposer.BindingMode.SimpleFollowWithWorldUp)
				{
					m_XAxis.Value = value;
				}
		...
		private void PushSettingsToRigs()
		{
			...
			for (int i = 0; i < m_Rigs.Length; i++)
			{
				...
				m_Rigs[i].m_StandbyUpdate = m_StandbyUpdate;
				m_Rigs[i].FollowTargetAttachment = base.FollowTargetAttachment;
				m_Rigs[i].LookAtTargetAttachment = base.LookAtTargetAttachment;
