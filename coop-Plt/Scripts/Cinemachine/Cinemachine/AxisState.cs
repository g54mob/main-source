	...
	public struct AxisState
	{
		...
		public struct Recentering
		{
			...
			public void DoRecentering(ref AxisState axis, float deltaTime, float recenterTarget)
			{
				...
				if (num2 != 0f && !(CinemachineCore.CurrentTime < mLastAxisInputTime + m_WaitTime))
				{
		...
		public bool Update(float deltaTime)
		{
			...
			else
			{
				...
				float dampTime = ((Mathf.Abs(num2) < Mathf.Abs(mCurrentSpeed)) ? m_DecelTime : m_AccelTime);
		...
		private bool MaxSpeedUpdate(float input, float deltaTime)
		{
			if (m_MaxSpeed > 0.0001f)
			{
				...
				if (Mathf.Abs(num) < 0.0001f || (Mathf.Sign(mCurrentSpeed) == Mathf.Sign(num) && Mathf.Abs(num) < Mathf.Abs(mCurrentSpeed)))
				{
					float num2 = Mathf.Min(Mathf.Abs(num - mCurrentSpeed) / Mathf.Max(0.0001f, m_DecelTime) * deltaTime, Mathf.Abs(mCurrentSpeed));
					mCurrentSpeed -= Mathf.Sign(mCurrentSpeed) * num2;
				}
				else
				{
					float num3 = Mathf.Abs(num - mCurrentSpeed) / Mathf.Max(0.0001f, m_AccelTime);
					mCurrentSpeed += Mathf.Sign(num) * num3 * deltaTime;
					if (Mathf.Sign(mCurrentSpeed) == Mathf.Sign(num) && Mathf.Abs(mCurrentSpeed) > Mathf.Abs(num))
					{
						mCurrentSpeed = num;
					}
				}
