...
namespace Kitchen
{
	...
	public class PruneDroppingFilledTrays : GenericSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(PruneDroppingFilledTrays componentSystem)
				{
			...
			public PruneDroppingFilledTrays hostInstance;
		...
		{
			if (proposal.Status == ItemTransferStatus.Pruned || (proposal.Flags & TransferFlags.Storage) != TransferFlags.Null || !Has<CLockHolderIfStorage>(proposal.Source) || !RequireBuffer(proposal.Source, out DynamicBuffer<CItemStored> comp))
			{
				return;
			}
			foreach (CItemStored item in comp)
			{
				if (Has<CStoredBy>(item.StoredItem))
				{
