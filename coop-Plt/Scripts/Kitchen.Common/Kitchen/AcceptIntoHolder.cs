...
namespace Kitchen
{
	public class AcceptIntoHolder : TransferAcceptSystem
	{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(AcceptIntoHolder componentSystem)
				{
			...
			public AcceptIntoHolder hostInstance;
		...
		{
			if (proposal.Status == ItemTransferStatus.Pruned || (proposal.Flags & TransferFlags.RequireMerge) != TransferFlags.Null || !Require<CItemHolder>(proposal.Destination, out CItemHolder comp) || comp.HeldItem != default(Entity) || !GameData.Main.TryGet<Item>(proposal.ItemData, out var output) || (Require<CItemHolderPreventTransfer>(proposal.Destination, out CItemHolderPreventTransfer comp2) && comp2.PreventInsertingInto))
			{
				return;
			}
			if (Require<CItemHolderFilter>(proposal.Destination, out CItemHolderFilter comp3))
			{
				if (comp3.NoDirectInsertion || !comp3.AllowCategory(output.ItemCategory))
				{
					return;
				}
			}
			else if (output.ItemCategory != ItemCategory.Generic)
			{
				return;
			}
			if (!Require<CItemHolderOnlySpecificItem>(proposal.Destination, out CItemHolderOnlySpecificItem comp4) || comp4.ItemID == proposal.ItemType)
			{
				Accept(e);
			}
		}
