...
namespace Kitchen
{
	public class TakeFromHolder : TransferInteractionProposalSystem
	{
		...
		public static void CreateProposal(SystemReference system, EntityContext ctx, Entity from, Entity to, TransferFlags flags = TransferFlags.Null, Entity interactor = default(Entity), Action<Entity> per_proposal = null)
		{
			...
			if ((!ctx.Require<CItemHolderPreventTransfer>(from, out var comp3) || !comp3.PreventTakingFrom) && flag)
			{
				Entity entity = TransferProposalSystem.CreateProposal(ctx, system, comp, from, to, flags | TransferFlags.Holder);
