...
namespace Kitchen
{
	...
	public class PruneByLocks : GenericSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(PruneByLocks componentSystem)
				{
			...
			public PruneByLocks hostInstance;
		...
		private void _003COnUpdate_003Eb__0_0(Entity e, ref CItemTransferProposal proposal)
		{
			...
			{
				if (Has<CDemoLock>(proposal.Item))
				{
					proposal.Status = ItemTransferStatus.Pruned;
				}
				if (Has<CPreventItemTransfer>(proposal.Destination) || Has<CPreventItemTransfer>(proposal.Source))
				{
					proposal.Status = ItemTransferStatus.Pruned;
				}
				if (proposal.Status == ItemTransferStatus.Pruned)
