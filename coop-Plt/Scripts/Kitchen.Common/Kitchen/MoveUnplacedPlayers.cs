...
namespace Kitchen
{
	public class MoveUnplacedPlayers : GenericSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(MoveUnplacedPlayers componentSystem)
				{
			...
			internal void OriginalLambdaBody(Entity e, ref CPosition pos, in CPlayer player)
			{
				...
				foreach (CPlayerSpawnLocation location in locations)
				{
					if (location.Index == player.Index)
					{
				...
				Vector3 position = (flag ? vector : vector2);
			...
			public void ScheduleTimeInitialize(MoveUnplacedPlayers componentSystem, ref _003C_003Ec__DisplayClass3_0 displayClass)
			{
		...
		{
			base.Initialise();
			Locations = GetEntityQuery(typeof(CPlayerSpawnLocation));
			Players = GetEntityQuery(typeof(CUnplacedPlayer));
		}
		...
		protected override void OnUpdate()
		{
			...
			{
				locations = Locations.ToComponentDataArray<CPlayerSpawnLocation>(Allocator.Temp)
			};
			_ = base.Entities;
			...
			base.EntityManager.RemoveComponent<CUnplacedPlayer>(Players);
			displayClass.locations.Dispose();
		}
		...
		public static EntityQuery _003C_003EGetEntityQuery_ForOnUpdate_LambdaJob0_From(ComponentSystemBase componentSystem)
		{
			...
			{
				ComponentType.ReadOnly<CUnplacedPlayer>(),
