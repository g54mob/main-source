...
namespace Kitchen
{
	...
	public class DetectPlayerPlacedItems : GameSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0
		{
			private struct LambdaParameterValueProviders
			{
				public struct Runtimes
				{
					...
					public LambdaParameterValueProvider_IComponentData<CItemTransferAccept>.StructuralChangeRuntime runtime_acceptance;
				}
				...
				public void ScheduleTimeInitialize(DetectPlayerPlacedItems componentSystem)
				{
				...
				public Runtimes PrepareToExecuteWithStructuralChanges(ComponentSystemBase p0, EntityQuery p1)
				{
					...
					result.runtime_acceptance = forParameter_acceptance.PrepareToExecuteWithStructuralChanges(p0, p1);
					return result;
				}
			}

			public DetectPlayerPlacedItems hostInstance;
			...
			public unsafe static void PerformLambda(void* jobStructPtr, void* runtimesPtr, Entity entity)
			{
				...
				CItemTransferAccept originalComponent;
				CItemTransferAccept acceptance = reference.runtime_acceptance.For(entity, out originalComponent);
				UnsafeUtility.AsRef<_003C_003Ec__DisplayClass_OnUpdate_LambdaJob0>(jobStructPtr).OriginalLambdaBody(e, ref acceptance);
			}
		...
		{
			if (acceptance.Status == ItemAcceptStatus.Accepted && Require<CItemTransferProposal>(acceptance.Proposal, out CItemTransferProposal comp) && comp.Status == ItemTransferStatus.Resolved && Has<CPlayer>(comp.Source) && (Has<CApplianceTable>(comp.Destination) || Has<CCustomerGroup>(comp.Destination)))
			{
				Entity e2 = base.EntityManager.CreateEntity();
				Set(e2, new AchievementAntisocial.CAntisocialTracker
				{
					HasLeft = false
				});
			}
