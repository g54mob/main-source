...
namespace Kitchen
{
	...
	public class DetermineDirectional : GameEffectSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass2_0
		{
			...
			public DetermineDirectional _003C_003E4__this;
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(DetermineDirectional componentSystem)
				{
			...
			internal void OriginalLambdaBody(Entity e, ref DynamicBuffer<CAffectedBy> affected_by, in CPosition position)
			{
				for (int i = 0; i < effectors.Length; i++)
				{
					...
					CPosition cPosition = _ComponentDataFromEntity_CPosition_0[entity];
					Vector3 vector = cPosition.Forward(1f) + cPosition.Position;
					if (!((position - vector).Chebyshev() > 0.25f))
					{
			...
			public void ScheduleTimeInitialize(DetermineDirectional componentSystem, ref _003C_003Ec__DisplayClass2_0 displayClass)
			{
		...
		{
			base.Initialise();
			EffectAppliers = GetEntityQuery(typeof(CAppliesEffect), typeof(CEffectRangeDirectional), typeof(CPosition));
		}
