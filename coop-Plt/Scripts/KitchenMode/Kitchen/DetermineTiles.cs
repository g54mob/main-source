...
namespace Kitchen
{
	...
	public class DetermineTiles : GameEffectSystemBase
	{
		...
		private struct _003C_003Ec__DisplayClass2_0
		{
			...
			public DetermineTiles _003C_003E4__this;
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(DetermineTiles componentSystem)
				{
			...
			[ReadOnly]
			[NoAlias]
			private ComponentDataFromEntity<CEffectRangeTiles> _ComponentDataFromEntity_CEffectRangeTiles_0;
			...
			internal void OriginalLambdaBody(Entity e, ref DynamicBuffer<CAffectedBy> affected_by, in CPosition position)
			{
				for (int i = 0; i < effectors.Length; i++)
				{
					...
					CEffectRangeTiles cEffectRangeTiles = _ComponentDataFromEntity_CEffectRangeTiles_0[entity];
					if ((position - _ComponentDataFromEntity_CPosition_1[entity].Position).Chebyshev() > (float)cEffectRangeTiles.Tiles + 0.25f)
					{
						continue;
					}
					if (!cEffectRangeTiles.PassThroughWalls)
					{
						int room = _003C_003E4__this.TileManager.GetRoom(position);
						int room2 = _003C_003E4__this.TileManager.GetRoom(_ComponentDataFromEntity_CPosition_1[entity]);
						if (room != room2)
						{
							continue;
						}
			...
			public void ScheduleTimeInitialize(DetermineTiles componentSystem, ref _003C_003Ec__DisplayClass2_0 displayClass)
			{
				_lambdaParameterValueProviders.ScheduleTimeInitialize(componentSystem);
				ReadFromDisplayClass(ref displayClass);
				_ComponentDataFromEntity_CEffectRangeTiles_0 = ((ComponentSystemBase)componentSystem).GetComponentDataFromEntity<CEffectRangeTiles>(true);
		...
		{
			base.Initialise();
			EffectAppliers = GetEntityQuery(typeof(CAppliesEffect), typeof(CEffectRangeTiles), typeof(CPosition));
		}
