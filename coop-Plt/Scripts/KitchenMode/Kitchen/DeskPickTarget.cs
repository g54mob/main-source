...
namespace Kitchen
{
	public class DeskPickTarget : GameSystemBase
	{
		[StructLayout(LayoutKind.Auto)]
		[CompilerGenerated]
		private struct _003C_003Ec__DisplayClass0_0
		{
			public DeskPickTarget _003C_003E4__this;

			public float time;

			internal void _003COnUpdate_003Eb__0(Entity e, ref CDeskTarget target, in CPosition pos)
			{
		...
		private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
		{
			private struct LambdaParameterValueProviders
			{
				...
				public void ScheduleTimeInitialize(DeskPickTarget componentSystem)
				{
			...
			internal void OriginalLambdaBody(Entity e, ref CDeskTarget target, in CPosition pos)
			{
				bool needs_enchant = _003C_003E4__this.Has<CDeskRequiresEnchantment>(e);
				if (!(target.NextTarget < time))
				{
					return;
				}
				target.Target = default(Entity);
				target.NextTarget = time + target.RetargetTime;
				int num = Random.Range(0, LayoutHelpers.AllNearby.Count);
				int room = _003C_003E4__this.TileManager.GetRoom(pos);
				for (int i = 0; i < LayoutHelpers.AllNearby.Count; i++)
				{
					LayoutPosition layoutPosition = LayoutHelpers.AllNearby[(i + num) % LayoutHelpers.AllNearby.Count];
					Entity occupant = _003C_003E4__this.TileManager.GetOccupant(pos.Position + (Vector3)layoutPosition);
					int room2 = _003C_003E4__this.TileManager.GetRoom(pos.Position + (Vector3)layoutPosition);
					if (room == room2 && _003C_003E4__this.MeetsConditions(occupant, target, needs_enchant))
					{
			...
			public void IterateEntities(ref ArchetypeChunk chunk, ref LambdaParameterValueProviders.Runtimes runtimes)
			{
				...
				{
					OriginalLambdaBody(runtimes.runtime_e.For(i), ref runtimes.runtime_target.For(i), in runtimes.runtime_pos.For(i));
				}
			}

			public void ScheduleTimeInitialize(DeskPickTarget componentSystem, ref _003C_003Ec__DisplayClass0_0 displayClass)
			{
		...
		private bool MeetsConditions(Entity ent, CDeskTarget conditions, bool needs_enchant = false)
		{
			...
			if (conditions.RequireUpgrade)
			{
				...
				if (!base.Data.TryGet<Appliance>(comp.ApplianceID, out var output2))
				{
					return false;
				}
				if (!output2.HasUpgrades)
				{
					return false;
				}
			}
