...
namespace TwitchLib.Api.Services.Core.LiveStreamMonitor
{
	internal class NameBasedMonitor : CoreMonitor
	{
		...
		public override async Task<Func<Stream, bool>> CompareStream(string channel)
		{
			...
			{
				channelId = (await _api.Helix.Users.GetUsersAsync(null, new List<string> { channel })).Users.FirstOrDefault()?.Id;
		...
		{
			return _api.Helix.Streams.GetStreamsAsync(null, null, channels.Count, null, null, "all", null, channels);
		}
