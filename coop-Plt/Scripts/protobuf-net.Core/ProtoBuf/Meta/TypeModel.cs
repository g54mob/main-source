...
namespace ProtoBuf.Meta
{
	public abstract class TypeModel : IProtoInput<Stream>, IProtoInput<ArraySegment<byte>>, IProtoInput<byte[]>, IProtoInput<ReadOnlyMemory<byte>>, IProtoInput<ReadOnlySequence<byte>>, IProtoOutput<Stream>, IProtoOutput<IBufferWriter<byte>>, IMeasuredProtoOutput<Stream>, IMeasuredProtoOutput<IBufferWriter<byte>>
	{
		...
		internal bool TrySerializeAuxiliaryType(ref ProtoWriter.State state, Type type, DataFormat format, int tag, object value, bool isInsideList, object parentList)
		{
			...
			{
				ObjectScope objectScope = NormalizeAuxScope(features, isInsideList, type);
				try
				{
			...
			if (value is IEnumerable enumerable)
			{
				...
				foreach (object item in enumerable)
				{
					...
					if (!TrySerializeAuxiliaryType(ref state, null, format, tag, item, isInsideList: true, enumerable))
					{
		...
		private static ObjectScope NormalizeAuxScope(SerializerFeatures features, bool isInsideList, Type type)
		{
		...
		internal void SerializeRootFallback(ref ProtoWriter.State state, object value)
		{
			...
			try
			{
				...
				{
					if (!TrySerializeAuxiliaryType(ref state, type, DataFormat.Default, 1, value, isInsideList: false, null))
					{
						ThrowUnexpectedType(type, this);
					}
					state.Close();
		...
		private object DeserializeWithLengthPrefix(Stream source, object value, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.NonPublicMethods | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.NonPublicFields | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.NonPublicProperties)] Type type, PrefixStyle style, int expectedField, TypeResolver resolver, out long bytesRead, out bool haveObject, SerializationContext context)
		{
			...
			try
			{
				...
				else if (!TryDeserializeAuxiliaryType(ref state, DataFormat.Default, 1, type, ref value, skipOtherFields: true, asListItem: false, autoCreate: true, insideList: false, null) && num != 0L)
				{
					ThrowUnexpectedType(type, this);
				}
		...
		internal object DeserializeRootAny(ref ProtoReader.State state, [DynamicallyAccessedMembers(DynamicallyAccessedMemberTypes.PublicConstructors | DynamicallyAccessedMemberTypes.NonPublicConstructors | DynamicallyAccessedMemberTypes.PublicMethods | DynamicallyAccessedMemberTypes.NonPublicMethods | DynamicallyAccessedMemberTypes.PublicFields | DynamicallyAccessedMemberTypes.NonPublicFields | DynamicallyAccessedMemberTypes.PublicProperties | DynamicallyAccessedMemberTypes.NonPublicProperties)] Type type, object value, bool autoCreate)
		{
			if (!DynamicStub.TryDeserializeRoot(type, this, ref state, ref value, autoCreate))
			{
				TryDeserializeAuxiliaryType(ref state, DataFormat.Default, 1, type, ref value, skipOtherFields: true, asListItem: false, autoCreate, insideList: false, null);
			}
			return value;
		}
		...
		private bool TryDeserializeList(ref ProtoReader.State state, DataFormat format, int tag, Type listType, Type itemType, ref object value)
		{
			...
			while (TryDeserializeAuxiliaryType(ref state, format, tag, itemType, ref value2, skipOtherFields: true, asListItem: true, autoCreate: true, insideList: true, value ?? listType))
			{
				result = true;
		...
		internal bool TryDeserializeAuxiliaryType(ref ProtoReader.SolidState state, DataFormat format, int tag, Type type, ref object value, bool skipOtherFields, bool asListItem, bool autoCreate, bool insideList, object parentListOrType)
		{
			...
			bool result = TryDeserializeAuxiliaryType(ref state2, format, tag, type, ref value, skipOtherFields, asListItem, autoCreate, insideList, parentListOrType);
		...
		internal bool TryDeserializeAuxiliaryType(ref ProtoReader.State state, DataFormat format, int tag, Type type, ref object value, bool skipOtherFields, bool asListItem, bool autoCreate, bool insideList, object parentListOrType)
		{
			...
			if (wireType == WireType.None)
			{
				...
				if ((object)t != null)
				{
					...
					flag = TryDeserializeList(ref state, format, tag, type, t, ref value);
					if (!flag && autoCreate)
					{
			...
			while (!(flag && asListItem))
			{
				...
				flag = true;
				state.Hint(wireType);
				ObjectScope scope = NormalizeAuxScope(features, insideList, type);
		...
		private object DeepCloneFallback(Type type, object value)
		{
			...
			{
				value = null;
				TryDeserializeAuxiliaryType(ref state2, DataFormat.Default, 1, type, ref value, skipOtherFields: true, asListItem: false, autoCreate: true, insideList: false, null);
			}
			finally
			{
				state2.Dispose();
