...
namespace ProtoBuf
{
	public abstract class ProtoWriter : IDisposable, ISerializationContext
	{
		...
		public ref struct State
		{
			...
			internal unsafe void LocalWriteString(string value)
			{
				int bytes;
				fixed (char* chars = value)
				{
					fixed (byte* reference = &MemoryMarshal.GetReference(_span))
					{
						bytes = UTF8.GetBytes(chars, value.Length, reference + OffsetInCurrent, RemainingInCurrent);
					}
				}
				OffsetInCurrent += bytes;
				RemainingInCurrent -= bytes;
			...
			{
				int num;
				if (MemoryMarshal.TryGetArray((ReadOnlyMemory<byte>)_memory, out ArraySegment<byte> segment))
				{
					num = source.Read(segment.Array, segment.Offset + OffsetInCurrent, RemainingInCurrent);
				}
				else
				{
					byte[] array = ArrayPool<byte>.Shared.Rent(RemainingInCurrent);
					try
					{
						num = source.Read(array, 0, RemainingInCurrent);
						if (num > 0)
						{
							new Span<byte>(array, 0, num).CopyTo(Remaining);
						}
					}
					finally
					{
			...
			internal ProtoWriter GetWriter()
			{
				return _writer;
			}
		...
		private class StreamProtoWriter : ProtoWriter
		{
			...
			private static void WriteFallback(ReadOnlySpan<byte> bytes, Stream stream)
			{
				byte[] array = ArrayPool<byte>.Shared.Rent(2048);
				try
				{
					Span<byte> destination = new Span<byte>(array);
					int length = destination.Length;
					while (bytes.Length >= length)
					{
						bytes.Slice(0, length).CopyTo(destination);
						stream.Write(array, 0, length);
						bytes = bytes.Slice(length);
					}
					bytes.CopyTo(destination);
					stream.Write(array, 0, bytes.Length);
				}
				finally
				{
					ArrayPool<byte>.Shared.Return(array);
			...
			private protected override void ImplWriteBytes(ref State state, ReadOnlySequence<byte> data)
			{
				...
				while (enumerator.MoveNext())
				{
					...
					byte[] array = ArrayPool<byte>.Shared.Rent(current.Length);
					try
					{
						current.CopyTo(array);
						dest.Write(array, 0, current.Length);
					}
					finally
					{
						ArrayPool<byte>.Shared.Return(array);
		...
		{
			if (depth != 0 || !TryFlush(ref state))
			{
				ThrowHelper.ThrowInvalidOperationException($"The writer is in an incomplete state (depth: {depth}, type: {GetType().Name}, field: {fieldNumber}, wire-type: {WireType}, position: {state.GetPosition()})");
			}
			_needFlush = false;
		}
		...
		private protected abstract void ImplWriteBytes(ref State state, ReadOnlyMemory<byte> data);
