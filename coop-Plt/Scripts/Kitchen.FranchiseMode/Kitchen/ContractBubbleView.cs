...
{
	[Serializable]
	public class ContractBubbleView : UpdatableObjectView<ContractBubbleView.ViewData>
	{
		public class UpdateView : IncrementalViewSystemBase<ViewData>
		{
			...
			private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
			{
				...
				public void OriginalLambdaBody(Entity entity, int entityInQueryIndex, [In] ref CLinkedView view, [In] ref CContractBubble info)
				{
			...
			[CompilerGenerated]
			private void _003COnUpdate_003Eb__0_0(Entity entity, int entityInQueryIndex, in CLinkedView view, in CContractBubble info)
			{
		...
		{
			if (GameData.Main.TryGet<Contract>(data.Contract, out var output))
			{
				Title.text = output.Name;
				Description.text = output.Description;
				Multiplier.text = $"x{Mathf.Round(output.ExperienceMultiplier * 10f) / 10f} XP";
				ExpBadge.SetActive(Math.Abs(output.ExperienceMultiplier - 1f) > 0.01f);
			}
			Animator?.Update(0f);
		}
	}
