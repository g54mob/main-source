...
namespace Kitchen
{
	public class TriggerWorkshopCrafting : InteractionSystem
	{
		...
		protected override bool IsPossible(ref InteractionData data)
		{
			...
			if (!Require<SWorkshopOutput>(out Output) || !Output.IsReady)
			{
				return false;
			}
			OutputPedestal = GetEntity<SWorkshopOutput>();
			if (!Require<CItemHolder>(OutputPedestal, out CItemHolder comp) || comp.HeldItem != default(Entity))
			{
				return false;
			}
			return true;
		...
		protected override void Perform(ref InteractionData data)
		{
			...
			if (!base.Data.TryGet<Appliance>(Output.OutputAppliance, out var output))
			{
				return;
			}
			int iD = ((output.CrateItem == null) ? AssetReference.ApplianceCrate : output.CrateItem.ID);
			...
			for (int i = 0; i < nativeArray.Length; i++)
			{
				...
				if (Require<CCrateAppliance>(cItemHolder.HeldItem, out CCrateAppliance comp))
				{
