...
namespace ProtoBuf.Compiler
{
	internal sealed class CompilerContextScope
	{
		...
		{
			internal static readonly ModuleBuilder Shared = AssemblyBuilder.DefineDynamicAssembly(new AssemblyName("SharedModule"), AssemblyBuilderAccess.Run).DefineDynamicModule("SharedModule");
		}
		...
		{
			MethodInfo method = interfaceType.GetMethod(name, BindingFlags.Instance | BindingFlags.Public);
			if ((object)method == null)
			{
				throw new ArgumentException("Declaration not found for '" + name + "'", "name");
			}
			ParameterInfo[] parameters = method.GetParameters();
			string text = name;
			MethodAttributes methodAttributes = (method.Attributes & ~MethodAttributes.Abstract) | MethodAttributes.Final;
			if (@explicit)
			{
			...
			MethodBuilder methodBuilder = type.DefineMethod(text, methodAttributes, method.ReturnType, Array.ConvertAll(parameters, (ParameterInfo x) => x.ParameterType));
