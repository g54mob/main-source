...
namespace ProtoBuf.Meta
{
	public sealed class MetaType : ISerializerProxy
	{
		...
		internal string GuessPackage()
		{
			...
			return text.Substring(0, length).Trim('.').Trim();
		}
		...
		internal void WriteSchema(HashSet<Type> callstack, StringBuilder builder, int indent, HashSet<string> imports, ProtoSyntax syntax, string package, SchemaGenerationFlags flags)
		{
			...
			if (Type.IsEnum)
			{
				...
				bool flag = IsValidEnum(enumValues);
				if (!flag)
				{
				...
				for (int k = 0; k < array2.Length; k++)
				{
					...
					{
						NewLine(builder, indent + 1).Append(enumMember2.Name).Append(" = 0;");
						flag3 = true;
					}
				}
				if (syntax == ProtoSyntax.Proto3 && !flag3)
				{
				...
				for (int l = 0; l < array3.Length; l++)
				{
					...
					if (num3.HasValue)
					{
						if (num3.Value != 0)
						{
							NewLine(builder, indent + 1).Append(enumMember3.Name).Append(" = ").Append(num3.Value)
								.Append(';');
						}
					}
					else
					{
						NewLine(builder, indent + 1).Append("// ").Append(enumMember3.Name).Append(" = ")
			...
			foreach (ValueMember valueMember in array4)
			{
				...
				{
					repeatedSerializerStub = model.TryGetRepeatedProvider(valueMember.MemberType);
					repeatedSerializerStub.ResolveMapTypes(out var keyType2, out var valueType2);
					string schemaTypeName = model.GetSchemaTypeName(callstack, keyType2, valueMember.MapKeyFormat, CompatibilityLevel, asReference: false, dynamicType: false, imports);
					schemaTypeName2 = model.GetSchemaTypeName(callstack, valueType2, valueMember.MapValueFormat, CompatibilityLevel, valueMember.AsReference, valueMember.DynamicType, imports);
				...
				{
					string value = (((object)valueMember.ItemType != null) ? "repeated " : ((syntax != ProtoSyntax.Proto2) ? "" : (valueMember.IsRequired ? "required " : "optional ")));
					NewLine(builder, indent + 1).Append(value);
					if (valueMember.DataFormat == DataFormat.Group)
					{
						builder.Append("group ");
					}
					schemaTypeName2 = valueMember.GetSchemaTypeName(callstack, applyNetObjectProxy: true, imports, out var altName);
					builder.Append(schemaTypeName2).Append(" ").Append(valueMember.Name)
						.Append(" = ")
						.Append(valueMember.FieldNumber);
					if (syntax == ProtoSyntax.Proto2 && valueMember.DefaultValue != null && !valueMember.IsRequired)
					{
						...
						{
							AddOption(builder, ref hasOption).Append("default = \"").Append(valueMember.DefaultValue).Append("\"");
						}
						else if (!(valueMember.DefaultValue is TimeSpan))
						{
							if (valueMember.DefaultValue is bool flag4)
							{
								AddOption(builder, ref hasOption).Append(flag4 ? "default = true" : "default = false");
							}
							else
							{
								object defaultValue = valueMember.DefaultValue;
								if (defaultValue is Enum && defaultValue.GetType() == valueMember.MemberType && model.IsDefined(valueMember.MemberType, valueMember.CompatibilityLevel))
								{
									MetaType metaType = model[valueMember.MemberType];
								...
								AddOption(builder, ref hasOption).Append("default = ").Append(valueMember.DefaultValue);
							}
						}
					}
					if (CanPack(valueMember.ItemType))
					{
					...
					if (syntax != ProtoSyntax.Proto2 && valueMember.DefaultValue != null && !valueMember.IsRequired && !IsImplicitDefault(valueMember.DefaultValue))
					{
				...
				if (schemaTypeName2 == ".bcl.NetObjectProxy" && valueMember.AsReference && !valueMember.DynamicType)
				{
					builder.Append(" // reference-tracked ").Append(valueMember.GetSchemaTypeName(callstack, applyNetObjectProxy: false, imports, out var _));
				}
			}
			if (_subTypes != null && _subTypes.Count != 0)
			{
