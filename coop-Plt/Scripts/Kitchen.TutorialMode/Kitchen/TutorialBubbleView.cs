...
{
	[Serializable]
	public class TutorialBubbleView : UpdatableObjectView<TutorialBubbleView.ViewData>
	{
		public class UpdateView : IncrementalViewSystemBase<ViewData>
		{
			...
			private struct _003C_003Ec__DisplayClass_OnUpdate_LambdaJob0 : IJobChunk
			{
				private struct LambdaParameterValueProviders
				{
					public struct Runtimes
					{
						...
						public LambdaParameterValueProvider_IComponentData<CTutorialBubble>.Runtime runtime_info;
					}
					...
					[ReadOnly]
					private LambdaParameterValueProvider_IComponentData<CTutorialBubble> forParameter_info;
				...
				public void OriginalLambdaBody(Entity entity, int entityInQueryIndex, [In] ref CLinkedView view, [In] ref CTutorialBubble info)
				{
			...
			[CompilerGenerated]
			private void _003COnUpdate_003Eb__0_0(Entity entity, int entityInQueryIndex, in CLinkedView view, in CTutorialBubble info)
			{
				SendUpdate(view.Identifier, new ViewData
				{
					Message = info.Text,
					ButtonPrompt = info.ButtonPrompt,
			...
			public static EntityQuery _003C_003EGetEntityQuery_ForOnUpdate_LambdaJob0_From(ComponentSystemBase componentSystem)
			{
				...
				(array[0] = new EntityQueryDesc()).All = new ComponentType[2]
				{
					...
					ComponentType.ReadOnly<CTutorialBubble>()
				};
		...
		public struct ViewData : IViewData, IViewResponseData, IViewData.ICheckForChanges<ViewData>
		{
			...
			{
				if (Message == check.Message && ButtonPrompt == check.ButtonPrompt)
				{
					return Animation != check.Animation;
				}
				return true;
			}
		}
		...
		private TutorialLocalisation TutorialLocalisation;

		protected override void UpdateData(ViewData data)
		{
			if (data.ButtonPrompt != Button.None)
			{
				PromptElement.SetButtonForAll(Controls.Get(data.ButtonPrompt));
				PromptElement.gameObject.SetActive(value: true);
				PromptElement.Animate(data.Animation);
			}
			else
			{
				PromptElement.gameObject.SetActive(value: false);
			}
			Description.text = TutorialLocalisation[data.Message].Description;
			if (Animator != null)
			{
				Animator.Update(0f);
			}
