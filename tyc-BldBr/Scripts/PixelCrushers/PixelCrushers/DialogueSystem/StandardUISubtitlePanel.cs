	...
	public class StandardUISubtitlePanel : UIPanel
	{
		...
		protected virtual void ShowContinueButtonNow()
		{
			...
			if (continueButton != null && continueButton.onClick.GetPersistentEventCount() == 0)
			{
				...
				StandardUIContinueButtonFastForward component = continueButton.GetComponent<StandardUIContinueButtonFastForward>();
				if (component != null)
				{
					continueButton.onClick.AddListener(component.OnFastForward);
				}
				else
				{
		...
		{
			UITools.Select(continueButton, allowStealFocus, base.eventSystem);
		}
		...
		public virtual void SetContentByPass(string actorname, string text)
		{
			if (portraitName.text != actorname)
			{
		...
		protected virtual void SetSubtitleTextContent(Subtitle subtitle)
		{
			...
			{
				subtitle.formattedText.text = FormattedText.Parse(string.Format(addSpeakerNameFormat, new object[2]
				{
			...
			SetFormattedText(subtitleText, text, subtitle);
			if (accumulateText)
			{
		...
		{
			string uIFormattedText = UITools.GetUIFormattedText(subtitle.formattedText);
			textField.text = previousText + uIFormattedText;
		...
		public virtual void SetPortraitImage(Sprite sprite)
		{
			if (!(portraitImage == null))
			{
				...
				{
					portraitImage.rectTransform.sizeDelta = (sprite.packed ? new Vector2(sprite.rect.width, sprite.rect.height) : new Vector2(sprite.texture.width, sprite.texture.height));
				}
			}
		}
