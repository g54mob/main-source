...
namespace Shapes
{
	...
	public abstract class ShapeRenderer : MonoBehaviour
	{
		...
		private bool IsInstanced
		{
			get
			{
				if (UsingDefaultZTests && UsingDefaultStencil)
				{
		...
		private bool UsingDefaultStencil
		{
			get
			{
				if (stencilComp == CompareFunction.Always && stencilOpPass == StencilOp.Keep && stencilRefID == 0 && stencilReadMask == byte.MaxValue)
				{
					return stencilWriteMask == byte.MaxValue;
				}
				return false;
			}
		}
		...
		{
			Mesh.bounds = GetBounds_Internal();
		}
		...
		{
			return GetBounds_Internal();
		}

		public Bounds GetWorldBounds()
		{
			Bounds bounds_Internal = GetBounds_Internal();
			...
			for (int i = -1; i <= 1; i += 2)
			{
				for (int j = -1; j <= 1; j += 2)
				{
					for (int k = -1; k <= 1; k += 2)
					{
						Vector3 rhs = transform.TransformPoint(bounds_Internal.center + Vector3.Scale(bounds_Internal.extents, new Vector3(i, j, k)));
