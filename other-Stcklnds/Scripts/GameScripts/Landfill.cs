...
public class Landfill : SewerCard
{
	...
	public override void UpdateCardText()
	{
		if (!IsOverflowing)
		{
			...
			{
				descriptionOverride = SokLoc.Translate("card_landfill_description", LocParam.Create("amount", PollutionOverflowMin.ToString()));
			}
			else
			{
				descriptionOverride = SokLoc.Translate("card_landfill_description_long", LocParam.Create("amount", PollutionOverflowMin.ToString()), LocParam.Create("current", StoredPollution.ToString()));
			}
		}
		else
		{
	...
	public override void UpdateCard()
	{
		if (!IsOverflowing)
		{
			...
			{
				MyGameCard.StartTimer(60f, DumpPollution, SokLoc.Translate("card_landfill_status_1", LocParam.Create("amount", PollutionRemovalRate.ToString())), GetActionId("DumpPollution"));
			}
