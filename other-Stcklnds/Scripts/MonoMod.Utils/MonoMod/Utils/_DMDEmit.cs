...
namespace MonoMod.Utils
{
	internal static class _DMDEmit
	{
		...
		public static void Generate(DynamicMethodDefinition dmd, MethodBase _mb, ILGenerator il)
		{
			...
			LocalBuilder[] array = definition.Body.Variables.Select((VariableDefinition var) => il.DeclareLocal(var.VariableType.ResolveReflection(), var.IsPinned)).ToArray();
			...
			foreach (Instruction instruction2 in definition.Body.Instructions)
			{
				...
				else
				{
					...
					{
						obj = ((memberReference == definition) ? _mb : memberReference.ResolveReflection());
					}
				...
				if (!flag)
				{
					foreach (Mono.Cecil.Cil.ExceptionHandler exceptionHandler2 in definition.Body.ExceptionHandlers)
					{
