...
namespace HarmonyLib
{
	public static class AccessTools
	{
		...
		public static MethodInfo EnumeratorMoveNext(MethodBase enumerator)
		{
			...
			ILContext iLContext = new ILContext(new DynamicMethodDefinition(enumerator).Definition);
			ILCursor iLCursor = new ILCursor(iLContext);
			Type enumeratorType;
			if (iLContext.Method.ReturnType.Name.StartsWith("UniTask"))
			{
				TypeReference firstVar = iLContext.Body.Variables.FirstOrDefault()?.VariableType;
				if (firstVar != null && !firstVar.Name.Contains(enumerator.Name))
				{
					Logger.Log(Logger.LogChannel.Warn, () => "EnumeratorMoveNext.Method: Unexpected type name " + firstVar.Name + ", should contain " + enumerator.Name);
					return null;
				}
			...
			else
			{
				...
				iLCursor.GotoNext((Instruction i) => i.MatchNewobj(out enumeratorCtor));
				...
				{
					Logger.Log(Logger.LogChannel.Warn, () => "EnumeratorMoveNext.Method: " + enumerator.FullDescription() + " does not create an enumerator (got " + enumeratorCtor.GetID(null, null, withType: true, simple: true) + ")");
					return null;
				}
