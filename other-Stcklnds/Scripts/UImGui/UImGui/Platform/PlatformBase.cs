...
namespace UImGui.Platform
{
	internal class PlatformBase : IPlatform
	{
		...
		public unsafe virtual bool Initialize(ImGuiIOPtr io, UIOConfig config, string platformName)
		{
			io.SetBackendPlatformName("Unity Input System");
			io.BackendFlags |= ImGuiBackendFlags.HasMouseCursors;
			if ((config.ImGuiConfig & ImGuiConfigFlags.NavEnableSetMousePos) != ImGuiConfigFlags.None)
			{
		...
		protected void UpdateCursor(ImGuiIOPtr io, ImGuiMouseCursor cursor)
		{
			...
			if (_lastCursor != cursor && (io.ConfigFlags & ImGuiConfigFlags.NoMouseCursorChange) == 0)
			{
				_lastCursor = cursor;
