...
namespace Febucci.UI.Core
{
	...
	public abstract class TypewriterCore : MonoBehaviour
	{
		...
		public void SkipTypewriter()
		{
			...
			{
				int renderedCharactersCountInsidePage = TextAnimator.GetRenderedCharactersCountInsidePage();
				if (renderedCharactersCountInsidePage > 0 && renderedCharactersCountInsidePage != TextAnimator.CharactersCount)
				{
					int num = TextAnimator.GetFirstCharacterIndexInsidePage();
					if (num < 0)
					{
						num = 0;
					}
					int num2 = num + renderedCharactersCountInsidePage;
		...
		private IEnumerator ShowTextRoutine()
		{
			...
			int num = TextAnimator.GetRenderedCharactersCountInsidePage();
			if (num <= 0)
			{
				num = TextAnimator.CharactersCount;
			}
			int num2 = TextAnimator.GetFirstCharacterIndexInsidePage();
			if (num2 < 0)
			{
				num2 = 0;
			}
			int lastCharacter = num2 + num;
			for (int i = num2; i < TextAnimator.CharactersCount && i < lastCharacter; i++)
			{
				if (actionsEnabled)
				{
					int maxIndex = i + 1;
		...
		private IEnumerator HideTextRoutine()
		{
			...
			default:
			{
				for (int k = num2; k < TextAnimator.CharactersCount && k < num3; k++)
				{
					indexes[k] = k;
				}
				break;
			...
			{
				for (int j = num2; j < TextAnimator.CharactersCount && j < num3; j++)
				{
					indexes[j] = num - j - 1;
				}
				break;
			...
			{
				for (int i = num2; i < TextAnimator.CharactersCount && i < num3; i++)
				{
					indexes[i] = i;
				}
			...
			foreach (int num4 in indexes)
			{
				if (!TextAnimator.Characters[num4].isVisible)
				{
					continue;
				}
				TextAnimator.SetVisibilityChar(num4, isVisible: false);
				float timeToWait = GetWaitDisappearanceTimeOf(num4);
				float deltaTime = GetDeltaTime(typingInfo);
				if (timeToWait < 0f)
				{
					timeToWait = 0f;
