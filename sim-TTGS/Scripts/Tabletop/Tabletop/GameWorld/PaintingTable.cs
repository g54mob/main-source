...
{
	public class PaintingTable : GroundFurniture
	{
		[Header("Painting")]
		[SerializeField]
		[Range(1f, 5f)]
		...
		[SerializeField]
		private List<PaintingTableWorkshop> m_workshops;
		...
		{
			base.OnEnable();
			foreach (PaintingTableWorkshop workshop in m_workshops)
			{
		...
		public override void Init(int gameID, Vector3 position, EFurnitureOrientation orientation, bool addScore = false)
		{
			...
			{
			case 2:
				ESteamAchievement.PAINT_STATION_LVL2.Trigger();
				break;
			case 3:
				ESteamAchievement.PAINT_STATION_LVL3.Trigger();
				break;
			}
		}
