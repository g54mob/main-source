...
public class ProjectileAttackNoGravity : MonoBehaviour
{
	...
	public void SpawnPredictedWarningAttack()
	{
		if (LocalClient.serverOwner && !(mob.target == null))
		{
			...
			Vector3 vector3;
			Vector3 vector2 = (vector3 = position + vector * timeToImpact) - vector3;
			float force = 0f;
			int id = warningAttack.id;
			int id2 = mob.id;
			ServerSend.MobSpawnProjectile(vector3, vector2, force, id, id2);
			ProjectileController.Instance.SpawnMobProjectile(vector3, vector2, force, id, id2);
		}
	}
