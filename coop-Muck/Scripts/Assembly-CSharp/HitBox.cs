...
public class HitBox : MonoBehaviour
{
	...
	public void UseHitbox()
	{
		...
		{
			RaycastHit raycastHit = array2[num4];
			...
			if (collider.gameObject.layer == LayerMask.NameToLayer("Object"))
			{
				...
				if ((currentItem.type == hitableResource.compatibleItem && currentItem.tier >= hitableResource.minTier) || hitableResource.compatibleItem == InventoryItem.ItemType.Item)
				{
					...
					num5 = (int)((float)currentItem.resourceDamage * damageMultiplier2 * resourceMultiplier * num2);
			...
			else
			{
				...
				int num6 = currentItem.attackDamage;
				if (currentItem.tag == InventoryItem.ItemTag.Arrow)
				{
					num6 = 1;
				}
				num5 = (int)((float)num6 * damageMultiplier2 * num2);
				...
				if ((bool)component2 && currentItem.attackTypes != null && component2.mobType.weaknesses != null)
				{
					...
					foreach (MobType.Weakness weakness in weaknesses)
					{
						...
						for (int num8 = 0; num8 < attackTypes.Length; num8++)
						{
							if (attackTypes[num8] == weakness)
							{
								flag = true;
								num5 *= 2;
							}
			...
			component.Hit(num5, sharpness, (int)hitEffect, raycastHit.collider.ClosestPoint(PlayerMovement.Instance.playerCam.position), hitWeaponType);
			num2 *= 0.5f;
			PlayerStatus.Instance.Heal(Mathf.CeilToInt((float)num5 * lifesteal));
			_ = 0;
			if (flag)
			{
		...
		if (flag2)
		{
			...
			{
				int num9 = 0;
				PowerupCalculations.Instance.SpawnOnHitEffect(num9, owner: true, pos, (int)((float)num * damageMultiplier.hammerMultiplier));
				ClientSend.SpawnEffect(num9, pos);
			}
		}
	}
