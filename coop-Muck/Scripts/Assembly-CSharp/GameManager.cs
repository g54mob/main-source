...
public class GameManager : MonoBehaviour
{
	...
	public List<Vector3> FindSurvivalSpawnPositions(int nPlayers)
	{
		...
		for (int i = 0; i < 100; i++)
		{
			...
			if (Physics.Raycast(new Vector3(vector2.x, 200f, vector2.y), Vector3.down, out var hitInfo, 500f, whatIsGround) && WorldUtility.WorldHeightToBiome(hitInfo.point.y) != TextureData.TerrainType.Water)
			{
		...
		for (int j = 0; j < 100; j++)
		{
			...
			if (Physics.Raycast(vector + new Vector3(vector3.x, 200f, vector3.y), Vector3.down, out var hitInfo2, 500f, whatIsGround) && WorldUtility.WorldHeightToBiome(hitInfo2.point.y) != TextureData.TerrainType.Water)
			{
	...
	public List<Vector3> FindVersusSpawnPositions(int nPlayers)
	{
		...
		for (int i = 0; i < 100; i++)
		{
			...
			if (Physics.Raycast(new Vector3(vector.x, 200f, vector.y), Vector3.down, out var hitInfo, 500f, whatIsGround) && WorldUtility.WorldHeightToBiome(hitInfo.point.y) != TextureData.TerrainType.Water)
			{
