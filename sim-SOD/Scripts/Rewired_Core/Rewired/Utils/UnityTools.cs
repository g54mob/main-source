...
namespace Rewired.Utils
{
	public static class UnityTools
	{
		...
		{
			public enum NKvAsrZDECtmgfrwUhCQptGgTKW
			{
				ifSBHxzGBKkztwjXLsyBHqLsRxE = 0,
				jeKXyOZaBAlNyHikQAvHeRuhuIt = 1,
				BwKAVTPhaWWuQHbBLSxQtANktOq = 2
			}
			...
			public readonly NKvAsrZDECtmgfrwUhCQptGgTKW type;
			...
			private string JJHvhIyhfwbBggJXtLNERJzmNnmg(NKvAsrZDECtmgfrwUhCQptGgTKW P_0)
			{
				return null;
			}
			...
			private static int khADneIiPImwEXnWCilpjaYeqdLR(NKvAsrZDECtmgfrwUhCQptGgTKW P_0)
			{
				return 0;
			}
		}

		private const UnityVersion gJkZhTInohOrxbZXNZauQBcHUSq = UnityVersion.UNITY_5_0;

		private static UnityVersionClass BYeEQMMVmwiGQjVrxbUXfAghOdYa;

		private static UnityVersion bdLECgXvhQleaKKzEWCbglpmaJt;

		private static string TgtwxeDsNZSlfBtrMGlLfnWsqKqP;

		private static Platform WHFYOigaUMOngzAyqWzCYQITwCT;

		private static EditorPlatform pbjciuBSIfkRsSdmAthIugFJXlTn;

		private static bool PsksXKgJcTGmxaVNwPiMlKusolF;

		private static bool rndbsEjWZlMdoPEVGeyzMptifxO;

		private static bool aZmmarqDeRifPKEYdabtAfGyGdDD;

		private static WebplayerPlatform FeazshaKdnXxuFfOgfWHiSgftET;

		private static bool axZYToIEfWSCLwrabKLfgRxgdJx;

		private static bool vedctjYgGzHVPxUzZNLyNRlFWPK;

		private static bool hYXTKaoGzOVAqaFDASuVEkimeBeI;

		private static bool bsbPkjhaQfvFPBwWWprvWJvzrEp;

		private static bool wmDBWbDLKKTaYDoQBmQXICTLgteA;

		private static bool BtMCKGUZuypmbEKEbDyjGZJuRY;

		private static string bYKbQgaEqWeKfjEQEndlrHhiBzas;

		private static ScriptingBackend fiqJKVEsojvXvojghbsbbEXGNfo;

		private static ScriptingAPILevel YgbqImuQSBfZsWPmsXafWRfmmeD;

		private static bool wFLdXjLttkOxFIJNaUMvnVdYGaD;

		private static IExternalTools XPMHDffqeyGQnuQUxaWSexaJRgXP;

		private static bool qGZpEUbxTnqwuRWyDbKGJlYWpgB;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper PRygasPEAMHPXAUkkMXUZxGaemad;
		...
		private static bool mvQFCmgEgTgRQPHHzcUlnMExtDqh()
		{
			return false;
		}

		internal static void yevEaEOpxaTseresMwWwEaZGFmnj(Platform P_0, EditorPlatform P_1, bool P_2, WebplayerPlatform P_3, ScriptingBackend P_4, ScriptingAPILevel P_5, IExternalTools P_6)
		{
		}
		...
		private static bool wgDEpPIiiHBegDCPDbcjoDeiMKdn(string P_0, out int P_1)
		{
			P_1 = default(int);
			return false;
		}

		private static void FVyTdramGParPSOGcOyykZCBVFT()
		{
		}

		private static UnityVersion bFFlOfBosjcsikBAxqfNaemAVZcj(string P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion cjEzwdWqFJlrwXfEBVLeJKhgfju(int P_0)
		{
			return default(UnityVersion);
		}

		private static UnityVersion bfZYoYBKYQVXKVwXUyZtzcVScMo(int P_0)
		{
			return default(UnityVersion);
		}

		private static void OeXaFXDLAPaaBLXEiwCvWlLqyMoV()
		{
		}

		internal static Type JmdeVPALsTdehZMnwnMEpCbetye(ooIOonQqBqnSxpKZHsgtmeqsFot P_0)
		{
			return null;
		}

		private static Type GKxNbwYcmfSmnDUlAJCwrgYHFeW(ooIOonQqBqnSxpKZHsgtmeqsFot P_0)
		{
			return null;
		}
		...
		private static T KJYMSudHECuAqnsdeWQERrPjdWqA<T>(T P_0) where T : class
		{
			return null;
		}

		internal static ButtonStateFlags WcOucUtnKGWGRHjhVcRwHvuvsTp(KeyCode P_0)
		{
			return default(ButtonStateFlags);
		}

		internal static ButtonStateFlags VxdmOUEPTHrbDmDfsUPqNkyKovq(string P_0)
		{
			return default(ButtonStateFlags);
		}
	}
