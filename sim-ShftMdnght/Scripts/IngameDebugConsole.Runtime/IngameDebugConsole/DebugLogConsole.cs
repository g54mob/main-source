...
namespace IngameDebugConsole
{
	public static class DebugLogConsole
	{
		...
		public static void SearchAssemblyForConsoleMethods(Assembly assembly)
		{
			try
			{
				...
				for (int i = 0; i < exportedTypes.Length; i++)
				{
					...
					{
						object[] customAttributes = methodInfo.GetCustomAttributes(typeof(ConsoleMethodAttribute), inherit: false);
						for (int k = 0; k < customAttributes.Length; k++)
						{
							if (customAttributes[k] is ConsoleMethodAttribute consoleMethodAttribute)
							{
		...
		{
			float result2;
			bool result = float.TryParse((!input.EndsWith("f", StringComparison.OrdinalIgnoreCase)) ? input : input.Substring(0, input.Length - 1), out result2);
			output = result2;
			return result;
		}
		...
		{
			double result2;
			bool result = double.TryParse((!input.EndsWith("f", StringComparison.OrdinalIgnoreCase)) ? input : input.Substring(0, input.Length - 1), out result2);
			output = result2;
			return result;
		}
		...
		{
			decimal result2;
			bool result = decimal.TryParse((!input.EndsWith("f", StringComparison.OrdinalIgnoreCase)) ? input : input.Substring(0, input.Length - 1), out result2);
			output = result2;
			return result;
		}
