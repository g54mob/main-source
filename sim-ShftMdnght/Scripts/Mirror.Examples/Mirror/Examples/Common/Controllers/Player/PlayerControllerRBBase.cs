...
namespace Mirror.Examples.Common.Controllers.Player
{
	...
	public class PlayerControllerRBBase : NetworkBehaviour
	{
		...
		public override void OnStartLocalPlayer()
		{
			...
			{
				if (runtimeData.controllerUI.TryGetComponent<PlayerControllerRBUI>(out var component))
				{
		...
		private void Start()
		{
			...
			Time.fixedDeltaTime = 1f / (float)NetworkManager.singleton.sendRate;
		}
		...
		private void FixedUpdate()
		{
			...
			if (Physics.Raycast(base.transform.position, Vector3.down, capsuleCollider.height / 2f + 0.1f))
			{
		...
		private void HandleTurning(float deltaTime)
		{
			...
			base.transform.Rotate(base.transform.up, runtimeData.turnSpeed * deltaTime, Space.World);
		}
		...
		private void ApplyMove(float fixedDeltaTime)
		{
			...
			velocity.y = runtimeData.jumpSpeed;
