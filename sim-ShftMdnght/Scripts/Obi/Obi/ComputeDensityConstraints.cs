...
namespace Obi
{
	public class ComputeDensityConstraints : ComputeConstraintsImpl<ComputeDensityConstraintsBatch>
	{
		...
		public ComputeDensityConstraints(ComputeSolverImpl solver)
			: base(solver, Oni.ConstraintType.Density)
		{
			sortParticlesShader = Object.Instantiate(Resources.Load<ComputeShader>("Compute/SortParticleData"));
			sortDataKernel = sortParticlesShader.FindKernel("SortData");
			constraintsShader = Object.Instantiate(Resources.Load<ComputeShader>("Compute/DensityConstraints"));
		...
		{
			ComputeDensityConstraintsBatch computeDensityConstraintsBatch = new ComputeDensityConstraintsBatch(this);
			batches.Add(computeDensityConstraintsBatch);
			return computeDensityConstraintsBatch;
		}
		...
		{
			if (((ComputeSolverImpl)base.solver).particleCount > 0 && m_Solver.particleGrid.sortedFluidIndices != null && m_Solver.cellCoordsBuffer != null && !(m_Solver.abstraction.parameters.maxAnisotropy <= 1f))
			{
				...
				CopyDataInSortedOrder(renderable: true);
