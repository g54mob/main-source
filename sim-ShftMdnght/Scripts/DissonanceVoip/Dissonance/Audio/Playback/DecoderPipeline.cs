...
namespace Dissonance.Audio.Playback
{
	...
	{
		private static readonly Log Log = Logs.Create(LogCategory.Playback, "DecoderPipeline");
		...
		float IVolumeProvider.TargetVolume => (VolumeProvider?.TargetVolume ?? 1f) * PlaybackOptions.AmplitudeMultiplier;
		...
		public DecoderPipeline([NotNull] IVoiceDecoder decoder, uint inputFrameSize, [NotNull] Action<DecoderPipeline> completionHandler, string id, bool softClip = true)
		{
			...
			if (!softClip)
			{
				ISampleSource resampler = _resampler;
				output = resampler;
			}
			else
			{
				ISampleSource resampler = new SoftClipSampleSource(_resampler);
				output = resampler;
			}
			_output = output;
		}
