...
public class RopeSweepCut : MonoBehaviour
{
	...
	private void ScreenSpaceCut(Vector2 lineStart, Vector2 lineEnd)
	{
		...
		{
			Vector3 vector = cam.WorldToScreenPoint(rope.solver.positions[rope.elements[i].particle1]);
			Vector3 vector2 = cam.WorldToScreenPoint(rope.solver.positions[rope.elements[i].particle2]);
