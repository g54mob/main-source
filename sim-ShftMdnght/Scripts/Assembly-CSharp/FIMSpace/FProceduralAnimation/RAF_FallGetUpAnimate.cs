		...
		public void ApplyOnGetUpSwitches()
		{
			...
			if (repositionBaseTransformV.GetBool())
			{
				...
				if (supportGetupRestoreV.GetBool() && parentRagdollHandler.GetUpCall_StandingRestore)
				{
					...
					{
						groundHit = default(RaycastHit);
						Vector3 reposePosition = RAF_ReposeOnFall.GetReposePosition(parentRagdollHandler, RAF_ReposeOnFall.EBaseTransformRepose.AnchorToFootPosition);
				...
				if (!flag)
				{
					RAF_ReposeOnFall.EBaseTransformRepose reposeMode = (RAF_ReposeOnFall.EBaseTransformRepose)modeV.GetInt();
					if (groundHit.transform == null)
					{
						groundHit = default(RaycastHit);
						Vector3 reposePosition2 = RAF_ReposeOnFall.GetReposePosition(parentRagdollHandler, reposeMode);
