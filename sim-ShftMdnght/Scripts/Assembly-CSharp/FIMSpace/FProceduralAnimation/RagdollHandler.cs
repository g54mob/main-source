...
namespace FIMSpace.FProceduralAnimation
{
	...
	public class RagdollHandler : IRagdollAnimator2HandlerOwner
	{
		...
		{
			RagdollChainBone anchor = _playmodeAnchorBone;
			bool isKinematic = anchor.GameRigidbody.isKinematic;
			...
			if (!animatingModeChanged || !IsFallingOrSleep || anchor.GameRigidbody.isKinematic == isKinematic || anchor.GameRigidbody.isKinematic)
			{
				return;
			}
		...
		private void ChangeAnchorKinematicState(bool isKinematic)
		{
			...
			if (isKinematic)
			{
				_playmodeAnchorBone.GameRigidbody.collisionDetectionMode = CollisionDetectionMode.Discrete;
			}
		...
		public void RefreshAllChainsDynamicParameters()
		{
			...
			foreach (RagdollBonesChain chain in chains)
			{
				foreach (RagdollChainBone boneSetup in chain.BoneSetups)
				{
					boneSetup.RefreshDynamicPhysicalParameters(chain, isFallingOrSleep);
