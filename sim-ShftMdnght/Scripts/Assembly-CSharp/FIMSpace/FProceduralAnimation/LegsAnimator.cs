	...
	public class LegsAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon
	{
		...
		public class HipsReference
		{
			...
			public Vector3 CalculateGlueMovePush()
			{
				...
				{
					Leg leg = ChildLegs[i];
					if ((leg.G_Attached || leg.G_DuringLegAdjustMovement) && leg.G_LastLegMoveDistanceFactor > leg.LegAnimatingSettings.DoStepAnimationOnDistanceFactor && leg.G_GlueInternalTransition > 0f && leg.G_GlueInternalTransition < 1f && leg.G_HandlerExecutingLegAnimationMode == EGlueMode.Idle)
					{
		...
		public class Leg
		{
			private struct GlueAttachement
			{
				...
				public GlueAttachement(Leg leg, RaycastHit legGroundHit)
				{
					...
					{
						RotInAttachementLocal = AttachedTo.rotation.QToLocal(leg.GetAlignedOnGroundHitRot(leg._SourceIKRot, legGroundHit.normal));
					}
				}
			...
			public void AssignOppositeLegIndex(int oppositeIndex)
			{
				...
				{
					if ((bool)Owner && Owner.Legs.ContainsIndex(oppositeIndex, true))
					{
			...
			public Leg GetOppositeLegReference(LegsAnimator legs)
			{
				...
				if (!legs.Legs.ContainsIndex(OppositeLegIndex, true))
				{
					return null;
				}
		...
		public class LegStepAnimatingParameters
		{
			...
			[Range(0f, 0.2f)]
			public float DoStepAnimationOnDistanceFactor = 0.055f;
		...
		public float GlueBlend
		{
			get
			{
				...
				return _MainBlend * MainGlueBlend;
			}
		}
		...
		{
			if (Legs.ContainsIndex(legIndex, true))
			{
		...
		{
			if (Legs.ContainsIndex(legID, true))
			{
		...
		private void FixedUpdate()
		{
			...
			{
				float delta = (UnscaledDeltaTime ? Time.fixedUnscaledDeltaTime : Time.fixedDeltaTime);
				PrepareValues(delta);
				ExtraFixedUpdate();
				if (AnimatePhysics)
				{
