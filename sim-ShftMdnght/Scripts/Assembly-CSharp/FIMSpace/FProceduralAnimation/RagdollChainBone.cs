...
namespace FIMSpace.FProceduralAnimation
{
	...
	public class RagdollChainBone
	{
		...
		public void RefreshDynamicPhysicalParameters(RagdollBonesChain chain, bool fallMode)
		{
			...
			{
				Joint.connectedMassScale = ConnectionMassOverride * fadeInBlend;
				return;
			}
			if (fallMode)
			{
				if ((bool)Joint)
				{
					...
					{
						Joint.connectedMassScale = chain.ConnectedMassScale;
					}
					else
					{
						Joint.connectedMassScale = chain.ConnectedMassScale * chain.ParentHandler.MassMultiplyOnFalling * fadeInBlend;
					}
				}
			...
			else
			{
				if ((bool)Joint)
				{
					...
					{
						Joint.connectedMassScale = chain.ConnectedMassScale * chain.ParentHandler.ConnectedMassMultiply * fadeInBlend;
					}
				}
				if (chain.ParentHandler.NoGravityOnStanding)
				{
