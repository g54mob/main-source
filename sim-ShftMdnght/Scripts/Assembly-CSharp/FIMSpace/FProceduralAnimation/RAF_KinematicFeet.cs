namespace FIMSpace.FProceduralAnimation
{
	public class RAF_KinematicFeet : RagdollAnimatorFeatureBase
	{
		public override bool OnInit()
		{
			...
			{
				if (chain.ChainType.IsLeg() && chain.ChainType.IsLeg())
				{
					...
					ragdollChainBone.RefreshDynamicPhysicalParameters(chain, chain.ParentHandler.IsFallingOrSleep);
				}
			}
			return base.OnInit();
		}
		...
		public override void OnDestroyFeature()
		{
			foreach (RagdollBonesChain chain in base.ParentRagdollHandler.Chains)
			{
				if (chain.ChainType.IsLeg())
				{
					...
					chain.BoneSetups[chain.BoneSetups.Count - 1].RefreshDynamicPhysicalParameters(chain, base.ParentRagdollHandler.IsInFallingMode);
				}
			}
		}
	}
