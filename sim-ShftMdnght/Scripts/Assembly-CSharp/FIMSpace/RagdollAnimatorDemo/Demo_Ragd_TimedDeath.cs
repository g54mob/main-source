	...
	public class Demo_Ragd_TimedDeath : FimpossibleComponent
	{
		...
		private void Update()
		{
			...
			if (elapsed > DeathAfter)
			{
				...
				if (stage == 0)
				{
					Ragdoll.Handler.GetExtraFeature<RAF_BlendOnCollisions>().Helper.Enabled = false;
				...
				if (elapsed > DeathAfter + FallAfter)
				{
					...
					Ragdoll.User_AddChainImpact(Ragdoll.Handler.GetChain(HitBody), -base.transform.forward * FallImpactPower, 0.15f, Force);
				}
			}
		}
	}
