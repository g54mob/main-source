...
namespace FIMSpace.RagdollAnimatorDemo
{
	public class Demo_Ragd_RopeSwingHero : FimpossibleComponent
	{
		...
		public float LeftHandToRightHandPushPower;
		...
		private void Update()
		{
			...
			{
				shooting = true;
				Mecanim.SetBool("Fall", value: false);
			...
			else
			{
				...
				if (!Mover.isGrounded && RagdollAnimator.IsInFallingOrSleepMode)
				{
					RagdollChainBone getAnchorBoneController2 = RagdollAnimator.Handler.GetAnchorBoneController;
					Mover.Rigb.velocity = getAnchorBoneController2.GameRigidbody.velocity;
					Mover.Rigb.position = RagdollAnimator.User_GetPosition_FeetMiddle();
				}
			}
		}
		...
		private void Shot()
		{
			if (Physics.Raycast(Camera.main.ScreenPointToRay(Input.mousePosition), out var hitInfo, float.PositiveInfinity, CatchLayer))
			{
				RopeAttacher.transform.position = hitInfo.point;
		...
		private void FixedUpdate()
		{
			...
			if (attached)
			{
				...
				{
					lHand.GameRigidbody.DragRigidbodyTowards(rHand.GameRigidbody.position, LeftHandToRightHandPushPower * 1.25f);
				}
				RagdollAnimator.User_TransitionMusclesPowerMultiplier(0.7f, Time.fixedDeltaTime * 2f);
			}
			else if (!Mecanim.GetBool("Grounded"))
			{
				...
				float magnitude = RagdollAnimator.User_GetChainBonesVelocity(ERagdollChainType.Core).magnitude;
				if (!shooting)
				{
					if (magnitude > 3f)
					{
						Mecanim.SetBool("Fall", value: true);
					}
					else
					{
						Mecanim.SetBool("Fall", value: false);
			...
			else if (RagdollAnimator.IsInFallingOrSleepMode)
			{
