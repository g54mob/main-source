...
namespace FIMSpace.Basics
{
	public class FBasic_TPPCameraBehaviour : MonoBehaviour
	{
		...
		private void UpdateMethods()
		{
			...
			RaycastCalculations();
			SwitchCalculations();
			DebugVelocity = base.transform.position - prePos;
		...
		private void FollowCalculations()
		{
			...
			Vector3 target = ToFollow.transform.position + FollowingOffset;
			if (HardFollowValue < 1f)
			{
				target = Vector3.SmoothDamp(targetPosition, target, ref movVelo, Mathf.Lerp(0.5f, 0f, HardFollowValue), float.PositiveInfinity, Time.deltaTime);
			}
