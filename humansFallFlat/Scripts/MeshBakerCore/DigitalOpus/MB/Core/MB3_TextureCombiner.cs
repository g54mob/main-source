...
namespace DigitalOpus.MB.Core
{
	...
	public class MB3_TextureCombiner
	{
		...
		public Texture2D _createTemporaryTexture(string propertyName, int w, int h, TextureFormat texFormat, bool mipMaps, bool linear)
		{
			...
			texture2D.name = string.Format("tmp{0}_{1}x{2}", _temporaryTextures.Count, w, h);
		...
		internal Texture2D _createTextureCopy(string propertyName, Texture2D t)
		{
			...
			texture2D.name = string.Format("tmpCopy{0}_{1}x{2}", _temporaryTextures.Count, texture2D.width, texture2D.height);
			TemporaryTexture item = new TemporaryTexture(propertyName, texture2D);
			_temporaryTextures.Add(item);
			return texture2D;
		}
		...
		internal Texture2D _resizeTexture(string propertyName, Texture2D t, int w, int h)
		{
			...
			texture2D.name = string.Format("tmpResampled{0}_{1}x{2}", _temporaryTextures.Count, w, h);
			TemporaryTexture item = new TemporaryTexture(propertyName, texture2D);
			_temporaryTextures.Add(item);
			return texture2D;
		}
		...
		public void SuggestTreatment(List<GameObject> objsToMesh, Material[] resultMaterials, List<ShaderTextureProperty> _customShaderPropNames)
		{
			...
			{
				string shaderName = ((!(resultMaterials[n] != null)) ? resultMaterials[n].shader.name : "None");
