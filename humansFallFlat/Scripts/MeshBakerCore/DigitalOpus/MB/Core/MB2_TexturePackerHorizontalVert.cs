...
namespace DigitalOpus.MB.Core
{
	public class MB2_TexturePackerHorizontalVert : MB2_TexturePacker
	{
		...
		public override AtlasPackingResult[] GetRects(List<Vector2> imgWidthHeights, int maxDimensionX, int maxDimensionY, int padding)
		{
			...
			return GetRects(imgWidthHeights, list, maxDimensionX, maxDimensionY, false);
		}
		...
		private AtlasPackingResult _GetRectsSingleAtlas(List<Vector2> imgWidthHeights, List<AtlasPadding> paddings, int maxDimensionX, int maxDimensionY, int minImageSizeX, int minImageSizeY, int masterImageSizeX, int masterImageSizeY, int recursionDepth)
		{
			...
			int newMinSizeX;
			int newMinSizeY;
			if (!ScaleAtlasToFitMaxDim(rootWH, list2, maxDimensionX, maxDimensionY, paddings[0], minImageSizeX, minImageSizeY, masterImageSizeX, masterImageSizeY, ref outW, ref outH, out padX, out padY, out newMinSizeX, out newMinSizeY))
			{
				...
				for (int j = 0; j < list2.Count; j++)
				{
					...
					Rect rect = ((packingOrientation != TexturePackingOrientation.vertical) ? (atlasPackingResult.rects[j] = new Rect((float)image2.x / (float)outW, (float)image2.y / (float)outH + padY, (!stretchImagesToEdges) ? ((float)image2.w / (float)outW) : 1f, (float)image2.h / (float)outH - padY * 2f)) : (atlasPackingResult.rects[j] = new Rect((float)image2.x / (float)outW + padX, (float)image2.y / (float)outH, (float)image2.w / (float)outW - padX * 2f, (!stretchImagesToEdges) ? ((float)image2.h / (float)outH) : 1f)));
		...
		private AtlasPackingResult[] _GetRectsMultiAtlasVertical(List<Vector2> imgWidthHeights, List<AtlasPadding> paddings, int maxDimensionPassedX, int maxDimensionPassedY, int minImageSizeX, int minImageSizeY, int masterImageSizeX, int masterImageSizeY)
		{
			...
			while (list2.Count > 0 || list3.Count > 0)
			{
				...
				if (image2 == null)
				{
					...
					{
						Rect rect = new Rect(list3[j].x, list3[j].y, list3[j].w, (!stretchImagesToEdges) ? list3[j].h : num2);
						array[j] = rect;
			...
			for (int k = 0; k < list.Count; k++)
			{
				...
				int newMinSizeX;
				int newMinSizeY;
				ScaleAtlasToFitMaxDim(new Vector2(list[k].atlasX, list[k].atlasY), list3, maxDimensionPassedX, maxDimensionPassedY, paddings[0], minImageSizeX, minImageSizeY, masterImageSizeX, masterImageSizeY, ref atlasX2, ref outH, out padX, out padY, out newMinSizeX, out newMinSizeY);
			}
		...
		private AtlasPackingResult[] _GetRectsMultiAtlasHorizontal(List<Vector2> imgWidthHeights, List<AtlasPadding> paddings, int maxDimensionPassedX, int maxDimensionPassedY, int minImageSizeX, int minImageSizeY, int masterImageSizeX, int masterImageSizeY)
		{
			...
			while (list2.Count > 0 || list3.Count > 0)
			{
				...
				if (image2 == null)
				{
					...
					for (int j = 0; j < list3.Count; j++)
					{
						Rect rect = new Rect(list3[j].x, list3[j].y, (!stretchImagesToEdges) ? list3[j].w : num2, list3[j].h);
						array[j] = rect;
						array2[j] = list3[j].imgId;
					}
					atlasPackingResult.rects = array;
			...
			for (int k = 0; k < list.Count; k++)
			{
				...
				int newMinSizeX;
				int newMinSizeY;
				ScaleAtlasToFitMaxDim(new Vector2(list[k].atlasX, list[k].atlasY), list3, maxDimensionPassedX, maxDimensionPassedY, paddings[0], minImageSizeX, minImageSizeY, masterImageSizeX, masterImageSizeY, ref outW, ref atlasY2, out padX, out padY, out newMinSizeX, out newMinSizeY);
			}
			for (int l = 0; l < list.Count; l++)
			{
				ConvertToRectsWithoutPaddingAndNormalize01(list[l], paddings[l]);
