...
public class StatsAndAchievements : MonoBehaviour
{
	...
	private static void SyncStat(Stat stat, ref float field, bool write)
	{
		...
		int pData;
		SteamUserStats.GetStat(SteamAchievementMap.StatToSteam(stat), out pData);
		field = pData;
	}
	...
	private void OnUserStatsReceived(UserStatsReceived_t pCallback)
	{
		...
		if (pCallback.m_eResult == EResult.k_EResultOK)
		{
			...
			for (int i = 0; i < SteamAchievementMap.achievementList.Count; i++)
			{
				...
				bool pbAchieved;
				if (SteamUserStats.GetAchievement(SteamAchievementMap.AchievementToSteam(achievement), out pbAchieved) && pbAchieved)
				{
	...
	public static void OnNotifyUnlockAchievement(Achievement achievement, float amount)
	{
		...
		case Achievement.ACH_SINGLE_RUN:
			...
			{
				UnlockAchievementInternal(Achievement.ACH_SINGLE_RUN, false);
			}
			break;
		case Achievement.ACH_TRAVEL_10KM:
		case Achievement.ACH_TRAVEL_100KM:
		case Achievement.ACH_FALL_1000:
			UnlockAchievementInternal(achievement, true);
			break;
		default:
			UnlockAchievementInternal(achievement, false);
			break;
		}
	}
	...
	public static void IncreaseFallCount(Human human)
	{
		...
		if (IsStatsPlayer(localPlayerIndex))
		{
			...
			if (fall >= 1)
			{
				UnlockAchievement(Achievement.ACH_FALL_1, false, localPlayerIndex);
			}
			Save(false);
		}
	}
