...
public class WorkshopRepository : MonoBehaviour, IDependency
{
	...
	public WorkshopRagdollModel[] developmentOnlyModels;
	...
	private static readonly string[] defaultMultiplayerLobbyNames = new string[4] { "WorkshopLobby", "Lobby", "Xmas", "Zodiac" };

	private static string[] multiplayerLobbyNames = null;
	...
	public void ReloadLocalModels()
	{
		...
		string[] array = FileTools.ListDirectories("mdl:", true);
	...
	private void CheckStoredLevelAndLobbySubscriptions(ref PublishedFileId_t[] subscriptions)
	{
		...
		if (!CheckValidLevel(ref subscriptions, level, false))
		{
			Options.lobbyLevel = 0;
		}
		if (!CheckValidLevel(ref subscriptions, multiplayerLobbyLevelStore, true))
		{
	...
	private IEnumerator RefreshTitleDescription()
	{
		...
		if (levelsCount != 0)
		{
			...
			while (batchCount > 0)
			{
				...
				SteamUGC.SetReturnLongDescription(m_UGCQueryHandle, true);
				...
				for (int j = 0; j < batchCount; j++)
				{
					...
					if (SteamUGC.GetQueryUGCResult(refreshResults, (uint)j, out UGCDetails))
					{
						bool flag = false;
	...
	private void InitializeSeasonalLobbies()
	{
		List<MultiplayerLobbyParams> lobbyParams = HFFResources.instance.LobbyParams;
		DateTime now = DateTime.Now;
		foreach (MultiplayerLobbyParams item in lobbyParams)
		{
			DateTime dateTime = new DateTime(now.Year, item.StartDateMonth, item.StartDateDay);
			DateTime dateTime2 = new DateTime(now.Year, item.EndDateMonth, item.EndDateDay);
			...
			{
				multiplayerLobbyNames = item.LobbyNamesOrder.ToArray();
				return;
			}
		}
		multiplayerLobbyNames = defaultMultiplayerLobbyNames;
	}
	...
	public static string GetLobbyNameEnglish(ulong id)
	{
		return multiplayerLobbyNames[id];
	}
	...
	public void LoadEditorPickLevels(bool requestLobbies = false)
	{
		...
		AddEditorPickLevel(3uL, "City");
		AddEditorPickLevel(4uL, "Forest");
		AddEditorPickLevel(5uL, "Lab");
		AddEditorPickLevel(6uL, "Lumber");
		AddEditorPickLevel(7uL, "RedRock");
		try
		{
			AddEditorPickLevel(8uL, "Tower");
		}
		catch (Exception)
		{
		}
		AddEditorPickLevel(9uL, "Miniature");
		AddEditorPickLevel(10uL, "CopperWorld");
		AddEditorPickLevel(12uL, "Naval_Ben");
		AddEditorPickLevel(13uL, "OceanAdventure");
		AddEditorPickLevel(14uL, "Dockyard");
		AddEditorPickLevel(15uL, "Museum");
		AddEditorPickLevel(16uL, "Hike");
		AddEditorPickLevel(17uL, "Candyland");
	}
