...
public class CustomizationPaintMenu : MenuTransition
{
	...
	public float cursorKernel
	{
		get
		{
			return Mathf.Lerp(0f, cursorSize, cursorHardness);
		}
	}

	public float cursorFalloff
	{
		get
		{
			return Mathf.Lerp(cursorSize, 0.001f, cursorHardness);
		}
	}
	...
	{
		ShowMask(false);
		cursor.Process(false);
		picker.Process(false, false);
		paint.Teardown();
	...
	protected override void Update()
	{
		...
		case PaintMode.Palette:
			paint.EndStroke();
			cursor.Process(false);
			picker.Process(true, false);
			break;
		case PaintMode.Picker:
			paint.EndStroke();
			cursor.Process(false);
			picker.Process(true, true);
			break;
