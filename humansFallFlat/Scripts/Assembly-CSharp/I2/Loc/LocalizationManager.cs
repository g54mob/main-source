...
namespace I2.Loc
{
	public static class LocalizationManager
	{
		...
		private static List<string> sLanguageList
		{
			get
			{
				return (sLocalisation == null) ? null : sLocalisation.mColumns;
			}
		}
		...
		public static string GetTermTranslation(string Term, int overrideLanguage = -1)
		{
			...
			if (value != null)
			{
				...
				{
					Debug.Log("GetTermTranslation() language column missing: " + num);
				}
			}
			else
			{
		...
		private static void SetLanguageColumn(string languageName)
		{
			...
			for (int i = 0; i < sLanguageList.Count; i++)
			{
				if (sLanguageList[i].Equals(languageName))
				{
					sLanguageColumn = i;
					break;
				}
