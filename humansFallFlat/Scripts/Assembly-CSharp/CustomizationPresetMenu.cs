...
public class CustomizationPresetMenu : MenuTransition
{
	...
	public static bool InCustomizationPresetMenu { get; private set; }
	...
	public override void OnGotFocus()
	{
		...
		noSubscriptionsMessage.SetActive(false);
		offlineMessage.SetActive(false);
		noPresetsMessage.SetActive(false);
		...
		if (items.Count == 0)
		{
			...
			{
				noSubscriptionsMessage.SetActive(true);
			}
			else
			{
				noPresetsMessage.SetActive(true);
			}
			BindButtons();
