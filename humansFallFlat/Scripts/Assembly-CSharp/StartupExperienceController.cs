...
public class StartupExperienceController : MonoBehaviour
{
	...
	private IEnumerator StartupExperienceRoutine()
	{
		...
		startupUI.noBrakesLogo.gameObject.SetActive(true);
		...
		startupUI.noBrakesLogo.gameObject.SetActive(false);
		startupUI.humanLogo.gameObject.SetActive(true);
	...
	private void EnterGameStartupXP()
	{
		...
		if (human != null)
		{
			...
			ragdoll.AllowHandBallRotation(false);
	...
	public void LeaveGameStartupXP()
	{
		if (human != null)
		{
			...
			ragdoll.AllowHandBallRotation(true);
	...
	private void Update()
	{
		if (!(human == null))
		{
			if (humanState == HumanState.HoldBeforeFall)
			{
				...
				human.ControlVelocity(15f, true);
				ragdoll.StretchHandsLegs(Vector3.forward, Vector3.right, 50);
			}
			else if (humanState == HumanState.Fall && human.transform.position.y > 2f)
			{
				human.transform.position = base.transform.position.SetY(human.transform.position.y);
				human.ControlVelocity(15f, true);
	...
	private void LateUpdate()
	{
		if (syncCamera)
		{
			if (cameraController.transform.position.y > -2f)
			{
