...
namespace HumanAPI
{
	public class SoundManager : MonoBehaviour
	{
		...
		public class SoundManagerState
		{
			...
			{
				SoundState value;
				soundStateMap.TryGetValue(name, out value);
				return value;
			}
			...
			{
				GrainState value;
				grainStateMap.TryGetValue(name, out value);
				return value;
			}
		}
		...
		public void OnEnable()
		{
			if (storedState != null)
			{
				...
				storedState.transform.SetParent(base.transform, false);
			}
			switch (type)
			{
		...
		private void LoadStoredSounds()
		{
			...
			Sound2[] componentsInChildren = GetComponentsInChildren<Sound2>(true);
			foreach (Sound2 sound in componentsInChildren)
			{
				...
				if (soundState != null)
				{
					Deserialize(sound, soundState, true);
				}
				if (grainState != null)
				{
		...
		public static void SoloSound(SoundMaster master, bool solo)
		{
			...
			if (solo)
			{
				...
				{
					main.sounds[i].Mute(true);
				}
				if (level != null)
				{
					for (int j = 0; j < level.sounds.Count; j++)
					{
						level.sounds[j].Mute(true);
					}
				}
				if (menu != null)
				{
			...
			{
				main.sounds[m].Mute(false);
			}
			if (level != null)
			{
				for (int n = 0; n < level.sounds.Count; n++)
				{
					level.sounds[n].Mute(false);
				}
			}
			if (menu != null)
			{
			...
			if (character != null)
			{
				...
				{
					character.sounds[num2].Mute(false);
				}
			}
		}
