...
namespace HumanAPI
{
	public class AngularJoint : JointImplementation
	{
		...
		public override void CreateMainJoint()
		{
			...
			Vector3 vector = body.InverseTransformPoint(axis.position);
			Vector3 vector2 = body.InverseTransformDirection(axis.up);
			localForward = body.InverseTransformDirection(axis.forward);
			Vector3 vector3 = body.InverseTransformDirection(axis.right);
			centerValue = 0f;
			if (!base.isKinematic)
			{
		...
		protected Joint CreateLimitJoint(float min, float max)
		{
			...
			ConfigurableJointMotion configurableJointMotion = (configurableJoint.zMotion = ConfigurableJointMotion.Free);
