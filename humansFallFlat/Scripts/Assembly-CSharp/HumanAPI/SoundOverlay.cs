	...
	public class SoundOverlay
	{
		...
		private void ShowSounds(SoundManager activeSounds)
		{
			...
			for (int i = 0; i < activeSounds.groups.Count; i++)
			{
				...
				if (flag != GUILayout.Toggle(flag, string.Format("{0} ({1})", text, soundGroup.sounds.Count)))
				{
					if (!flag)
					{
				...
				for (int j = 0; j < soundGroup.sounds.Count; j++)
				{
					...
					GUILayout.Label("lvl", AudioUI.style, GUILayout.ExpandWidth(false));
					Rect rect2 = GUILayoutUtility.GetRect(100f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("tune", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(75f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("dist", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(100f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("lp", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(75f, rect.height, GUILayout.ExpandWidth(false));
					...
					rect2 = GUILayoutUtility.GetRect(50f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("att", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(75f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("spread", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(50f, rect.height, GUILayout.ExpandWidth(false));
					...
					GUILayout.Label("3d", AudioUI.style, GUILayout.ExpandWidth(false));
					rect2 = GUILayoutUtility.GetRect(50f, rect.height, GUILayout.ExpandWidth(false));
					...
					{
						SoundManager.Deserialize(soundMaster, clipboard, false);
					}
					if (GUILayout.Button("S", GUILayout.Width(32f)) && clipboard != null)
					{
						SoundManager.Deserialize(soundMaster, clipboard, true);
					}
					bool flag3 = SoundManager.main.state.GetSoundState(soundMaster.master.fullName) != null;
					if (!flag3 && !soundMaster.master.useMaster && GUILayout.Button("Create Master", GUILayout.ExpandWidth(false)))
					{
						soundMaster.SetUseMaster(true);
					}
					else if ((flag3 || soundMaster.master.useMaster) && soundMaster.master.useMaster != GUILayout.Toggle(soundMaster.master.useMaster, "Master", GUILayout.ExpandWidth(false)))
					{
						...
						if (soundMaster.master.useMaster)
						{
							...
							if (soundState != null)
							{
								SoundManager.Deserialize(soundMaster, soundState, true);
							}
