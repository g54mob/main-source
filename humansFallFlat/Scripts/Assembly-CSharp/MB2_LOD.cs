...
public class MB2_LOD : MonoBehaviour
{
	...
	public float distanceSquaredToClosestCamera
	{
		get
		{
			return _distSqrToClosestCamera;
		}
	}
	...
	private bool Init()
	{
		...
		if (levels != null)
		{
			...
			if (CalculateSwitchDistances(fieldOfView, true))
			{
				for (int l = 0; l < levels.Length; l++)
				{
					...
					MySetActiveRecursively(l, false);
				}
		...
		for (int j = 0; j < levels.Length; j++)
		{
			...
			if (lodObject != null)
			{
				...
				if (lodObject is SkinnedMeshRenderer)
				{
					...
					SetActiveSelf(lodObject.gameObject, true);
	...
	public void ForceRemove()
	{
		...
		{
			MySetActiveRecursively(currentLODidx, false);
		}
		if (LOG_LEVEL >= MB2_LogLevel.trace)
		{
	...
	public void OnBakeAdded()
	{
		...
		{
			if (!_isInQueue || action != MB2_LODOperation.toAdd || isInCombined)
			{
	...
	public MB2_LODManager.BakerPrototype GetBaker(MB2_LODManager.BakerPrototype[] allPrototypes, bool ignoreLightmapping)
	{
		...
		for (int i = 0; i < levels.Length; i++)
		{
			...
			for (int l = 0; l < allPrototypes.Length; l++)
			{
				...
				if (allPrototypes[l].meshBaker.meshCombiner.renderType == MB_RenderType.meshRenderer && renderType != MB_RenderType.meshRenderer && bakerPrototype4 != null)
				{
					empty += "\n  rendertype did not match";
				}
				if (empty.Length == 0)
	...
	public override string ToString()
	{
		...
		return string.Format("[MB2_LOD {0} id={1}: inComb={2} inQ={3} act={4} nxt={5} curr={6} nxtRendInstId={7}]", base.name, GetInstanceID(), isInCombined, isInQueue, action, nextLODidx, currentLODidx, text);
	}
