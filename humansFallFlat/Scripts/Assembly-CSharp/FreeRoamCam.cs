...
public class FreeRoamCam : MonoBehaviour
{
	...
	private void Update()
	{
		...
		if (Game.GetKeyDown(KeyCode.F8))
		{
			...
			if (!allowFreeRoam)
			{
				...
				allowFreeRoam = true;
				cam = MenuCameraEffects.instance.OverrideCamera(base.transform, true);
			}
			else
			{
				CleanUp();
		...
		if (Game.GetKeyDown(KeyCode.F9))
		{
			...
			if (gameCam2 != null)
			{
				RenderTexture renderTexture = (gameCam2.targetTexture = new RenderTexture(1024, 576, 16));
				gameCam2.Render();
	...
	private void SyncCamera(CameraKeyFrame keyframe, CameraKeyFrame prevKeyframe, float t)
	{
		...
		Vector3 vector2 = Vector3.Lerp(prevKeyframe.targetPos, keyframe.targetPos, t);
		Quaternion quaternion = Quaternion.LookRotation(vector2 - vector, Vector3.up);
