...
public static class FileTools
{
	public struct NativeTextureHeader
	{
		...
		{
			return new NativeTextureHeader
			{
				magicWord = 0,
				width = 0,
				height = 0,
	...
	private static string ToWorkshopPath(string path)
	{
		...
		string pchFolder;
		uint punTimeStamp;
		SteamUGC.GetItemInstallInfo((PublishedFileId_t)num2, out punSizeOnDisk, out pchFolder, 1024u, out punTimeStamp);
	...
	{
		NativeTextureHeader header;
		if (!NativeTextureDecodeHeader(out header, data))
		{
			return null;
		}
		Texture2D texture2D = new Texture2D(header.width, header.height, header.format, false);
	...
	public static Texture2D TextureFromBytes(string name, byte[] bytes)
	{
		...
		try
		{
			Texture2D texture2D = NativeTextureDecode(bytes, true);
			if (texture2D != null)
			{
				texture2D.name = name;
				return texture2D;
