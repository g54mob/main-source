...
namespace DigitalOpus.MB.Lod
{
	public class LODCombinedMesh
	{
		...
		public class LODCombinerSplitterMerger
		{
			...
			public static void SplitCombiner(LODCombinedMesh src)
			{
				...
				foreach (MB2_LOD item in src.gosAssignedToMe)
				{
					...
					LODHierarchy value;
					if (!dictionary.TryGetValue(hierarchyRoot, out value))
					{
		...
		private void _CancelTransaction(MB2_LOD lod)
		{
			...
			{
				action = lod.action,
				toIdx = lod.nextLevelIdx,
				lod = lod
			};
			Transaction value;
			if (lodTransactions.TryGetValue(lod.gameObject.GetInstanceID(), out value))
			{
		...
		private void BakeClusterCombiner()
		{
			...
			combinedMesh.AddDeleteGameObjectsByID(list2.ToArray(), list.ToArray(), true);
			combinedMesh.Apply();
			numApproxVertsInQ = 0;
			numVertsInMesh = 0;
			for (int i = 0; i < combinedMesh.meshCombiners.Count; i++)
