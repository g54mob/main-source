...
namespace ProBuilder2.Examples
{
	...
	public class IcoBumpin : MonoBehaviour
	{
		...
		private void Start()
		{
			...
			pb_Face[] array = faces;
			foreach (pb_Face pb_Face2 in array)
			{
				pb_Face2.material = material;
			}
		...
		private void Update()
		{
			...
			for (int i = 0; i < outsides.Length; i++)
			{
				...
				Vector3 vector = outsides[i].nrm * ((fft[num2] + fft_history[num2]) * 0.5f * (frequencyCurve.Evaluate(num) * 0.5f + 0.5f)) * extrusion;
				int[] indices = outsides[i].indices;
				foreach (int num3 in indices)
				{
					displaced_vertices[num3] = original_vertices[num3] + vector;
				}
			}
