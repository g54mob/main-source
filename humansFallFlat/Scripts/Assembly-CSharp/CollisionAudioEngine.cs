...
public class CollisionAudioEngine : MonoBehaviour
{
	...
	public void RebuildMap()
	{
		...
		for (int j = 0; j < values.Length; j++)
		{
			...
			for (int k = 0; k < values.Length; k++)
			{
				...
				if (surfaceType2 != SurfaceType.Unknown)
				{
		...
		foreach (CollisionAudioSurfSurfConfig collisionAudioSurfSurfConfig in componentsInChildren2)
		{
			...
			foreach (SurfaceType item in new List<SurfaceType>(dictionary2.Keys))
			{
				...
				if (dictionary2[item] == null && item != SurfaceType.RagdollBall)
				{
					...
					collisionAudioSurfSurfConfig3.name = string.Format("{0}{1}Cloned{2}", collisionAudioSurfSurfConfig3.surf1, collisionAudioSurfSurfConfig3.surf2, collisionAudioSurfSurfConfig.name);
					collisionAudioSurfSurfConfig3.transform.SetParent(runtimeConfigs.transform, false);
	...
	private void AddCofigToMap(CollisionAudioSurfSurfConfig cfg)
	{
		...
		{
			Debug.LogError(string.Concat("CollisionAudioSurfSurfConfig already defined for ", cfg.surf1, " ", cfg.surf2), cfg);
		}
		...
		if (cfg.surf1 != cfg.surf2)
		{
			...
			collisionAudioSurfSurfConfig.name = string.Format("{0}{1}Mirror", collisionAudioSurfSurfConfig.surf1, collisionAudioSurfSurfConfig.surf2);
			collisionAudioSurfSurfConfig.transform.SetParent(runtimeConfigs.transform, false);
	...
	{
		Dictionary<SurfaceType, CollisionAudioSurfSurfConfig> value;
		CollisionAudioSurfSurfConfig value2;
		if (map.TryGetValue(surf1, out value) && value.TryGetValue(surf2, out value2))
		{
			return value2;
		}
		return null;
	...
	{
		CollisionAudioHitConfig value;
		configIdMap.TryGetValue(libId, out value);
		return value;
	}
	...
	public bool ReportCollision(CollisionAudioSensor sensor, SurfaceType surf1, SurfaceType surf2, Vector3 pos, float impulse, float normalVelocity, float tangentVelocity, float volume, float pitch)
	{
		...
		if (soloSurface != SurfaceType.Unknown && surf1 != soloSurface && surf2 != soloSurface)
		{
			return false;
		}
		CollisionAudioSurfSurfConfig collisionAudioSurfSurfConfig = Resolve(surf1, surf2);
