...
public class RagdollTexture : MonoBehaviour
{
	...
	public void LoadFromPreset(RagdollPresetMetadata preset, bool useBaseTexture = false)
	{
		ReleaseBaseTexture();
		if (!useBaseTexture)
		{
			...
			if (!textureLoadSuppressed)
			{
				...
				{
					ChangeBaseTexture(FileTools.TextureFromBytes(part.ToString(), ragdollPresetPartMetadata.bytes), false);
				}
		...
		if (model.meta.metaPath.StartsWith("builtin"))
		{
			if (baseTexture == null)
			{
				ChangeBaseTexture(HFFResources.instance.FindTextureResource("SkinTextures/" + model.meta.modelPrefab.name + "Color"), true);
			}
			if (model.maskTexture == null)
			{
				ChangeMaskTexture(HFFResources.instance.FindTextureResource("SkinTextures/" + model.meta.modelPrefab.name + "Mask"), true);
			}
		}
		if (baseTexture != null)
		{
	...
	public void SaveToPreset()
	{
		...
		ReleaseBaseTexture();
		ChangeBaseTexture(bakedTexture, false);
		bakedTexture = new Texture2D(width, height, TextureFormat.RGB24, false);
	...
	public void BakeTexture(RenderTexture rt, bool compress)
	{
		...
		{
			bakedTexture = new Texture2D(width, height, TextureFormat.RGB24, true);
			bakedTexture.name = "Baked" + base.name;
		}
		RenderTexture.active = rt;
		bakedTexture.ReadPixels(new Rect(0f, 0f, width, height), 0, 0, true);
		if (compress)
		{
	...
	private void EndStrokeFinalize()
	{
		...
		{
			ApplyColors(appliedColor1, appliedColor2, appliedColor3, true);
			return;
		}
