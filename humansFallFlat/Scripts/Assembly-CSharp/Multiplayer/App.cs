	...
	public class App : MonoBehaviour, IDependency
	{
		...
		public void StartupFinished()
		{
			lock (stateLock)
			{
				...
				{
					EnterMenu();
					MenuSystem.instance.ShowMainMenu(true);
				}
			}
		}
		...
		private void UpdateJoinable(bool startingGame = false)
		{
			...
			if (NetGame.instance.players.Count < Options.lobbyMaxPlayers)
			{
				...
				if (state == AppSate.ServerLobby && !startingGame)
				{
					...
					transport.SetJoinable(true, false);
				}
				else
				{
					UnityEngine.Debug.Log("UpdateJoinable : join in progress = " + Options.lobbyJoinInProgress);
					transport.SetJoinable(Options.lobbyJoinInProgress != 0, true);
				}
			}
			else
			{
				UnityEngine.Debug.Log("UpdateJoinable : noSlots");
				transport.SetJoinable(false, state != AppSate.ServerLobby || startingGame);
			}
		}
		...
		public void OnHostGameFail(string error)
		{
			lock (stateLock)
			{
				...
				{
					MenuSystem.instance.ShowMenu<MainMenu>(true);
					return;
				}
		...
		public void CancelHost()
		{
			...
			{
				EnterMenu();
				MenuSystem.instance.ShowMainMenu(false, true);
		...
		public void ChangeLobbyLevel(ulong level, WorkshopItemSource levelType)
		{
			lock (stateLock)
			{
				if (state == AppSate.ServerLobby)
				{
					NetGame.instance.ServerLoadLevel(level, levelType, false, 0u);
		...
		public void OnConnectFail(string error)
		{
			lock (stateLock)
			{
				...
				if (error != null && error.Equals("&"))
				{
					MenuSystem.instance.ShowMenu<MultiplayerSelectLobbyMenu>(true);
					return;
				}
		...
		public void OnLostConnection(bool suppressMessage = false)
		{
			lock (stateLock)
			{
				...
				if (RatingMenu.instance.ShowRatingMenu())
				{
					...
					GotoRatingsMenu(destination, false);
					return;
				}
		...
		public void ServerKicked()
		{
			...
			if (RatingMenu.instance.ShowRatingMenu())
			{
				GotoRatingsMenu(RatingMenu.RatingDestination.kLostConnection, false);
				return;
			}
			...
			lock (stateLock)
			{
				...
				if (!start)
				{
					...
					else if (isClient)
					{
						...
						if (RatingMenu.instance.ShowRatingMenu())
						{
							GotoRatingsMenu(RatingMenu.RatingDestination.kMultiplayerLobby, true);
						}
						else
						{
		...
		public void StartGameServer(ulong level, WorkshopItemSource levelType)
		{
			...
			lock (stateLock)
			{
				...
				{
					ExitLobby(true);
					NetGame.instance.ServerLoadLevel(level, levelType, true, 0u);
					LaunchGame(level, levelType, 0, 0, delegate
					{
		...
		public void NextLevelServer(ulong level, int checkpoint)
		{
			lock (stateLock)
			{
				...
				{
					NetGame.instance.ServerLoadLevel(level, WorkshopItemSource.BuiltIn, true, 0u);
		...
		public void LobbyLoadLevel(ulong level)
		{
			...
			if (NetGame.isServer)
			{
				NetGame.instance.ServerLoadLobby(level, WorkshopItemSource.BuiltInLobbies, false, 0u);
			}
		}
		...
		public void LevelLoadedServer(ulong level, WorkshopItemSource levelType, uint hash)
		{
			lock (stateLock)
			{
				...
				else
				{
					...
					NetGame.instance.ServerLoadLevel(level, levelType, true, (hash != 0) ? hash : 57005u);
				}
			}
		}
			...
			lock (stateLock)
			{
				...
				if (state == AppSate.PlayLevel)
				{
					...
					if (flag2)
					{
						...
						if (flag)
						{
							GotoRatingsMenu(RatingMenu.RatingDestination.kLevelSelectMenu, false);
						}
						else
						{
				...
				else if (state == AppSate.ServerPlayLevel)
				{
					if (!instantLeave)
					{
						...
						EnterLobby(true);
						NetGame.instance.ServerLoadLevel(NetGame.instance.currentLevel, NetGame.instance.currentLevelType, false, 0u);
					}
					else
					{
						EnterMenu();
				...
				else if (state == AppSate.ClientPlayLevel)
				{
					...
					if (flag)
					{
						GotoRatingsMenu(RatingMenu.RatingDestination.kMainMenu, false);
					}
					else
					{
						MenuSystem.instance.ShowMainMenu();
