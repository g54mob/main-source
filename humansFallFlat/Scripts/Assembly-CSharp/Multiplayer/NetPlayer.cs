...
namespace Multiplayer
{
	public class NetPlayer : NetScope
	{
		...
		private const int DefaultPosRangeOverride = 4000;
		...
		public static NetPlayer SpawnPlayer(uint id, NetHost host, bool isLocal, string skinUserId, uint localCoopIndex, byte[] skinCRC)
		{
			...
			component.human.ragdoll = Object.Instantiate(Game.instance.ragdollPrefab.gameObject, component.human.transform, false).GetComponent<Ragdoll>();
			...
			else
			{
				...
				if (Options.parental == 1 && ragdollPresetMetadata != null)
				{
			...
			{
				component.ApplyPreset(ragdollPresetMetadata, true, !isLocal && Options.parental == 1);
			}
			else
			{
				component.ApplyPreset(PresetRepository.CreateDefaultSkin(), false);
			}
			...
			{
				component.cameraController.gameObject.SetActive(false);
			}
			if (NetGame.netlog)
			{
		...
		private void SetupBodies()
		{
			...
			RegisterBody(ragdoll.partBall, null, true, true, num);
			RegisterBody(ragdoll.partHips, null, true, true, num);
			RegisterBody(ragdoll.partWaist, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partChest, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partHead, ragdoll.partHips, true, true, num + 2);
			RegisterBody(ragdoll.partLeftArm, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partLeftForearm, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partLeftThigh, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partLeftLeg, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partRightArm, ragdoll.partHips, false, true);
			RegisterBody(ragdoll.partRightForearm, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partRightThigh, ragdoll.partHips, false, true, num);
			RegisterBody(ragdoll.partRightLeg, ragdoll.partHips, false, true, num);
		}
		...
		private void RegisterBody(HumanSegment segment, HumanSegment relativeTo, bool pos, bool rot, int precision = 0)
		{
			...
			netBody.relativeTo = ((relativeTo == null) ? null : relativeTo.transform);
			...
			{
				segment.rigidbody.gameObject.SetActive(false);
			}
		}

		public void UpdateAbsolutePosRangeOverride(int value = 4000)
		{
			NetBody[] componentsInChildren = GetComponentsInChildren<NetBody>();
			foreach (NetBody netBody in componentsInChildren)
			{
				if (netBody.syncPosition == NetBodySyncPosition.Absolute)
				{
					netBody.RefreshAbsolutePosRangeOverride(value);
				}
			}
		}
		...
		{
			int num = PlayerPrefs.GetInt("MainSkinIndex", 0);
			num = (num + (int)localCoopIndex) % 2;
			return WorkshopRepository.instance.presetRepo.GetPlayerSkin(num);
		}

		public void ApplyPreset(RagdollPresetMetadata preset, bool bake = true, bool useBaseTexture = false)
		{
			if (customization == null)
			{
				customization = human.ragdoll.gameObject.AddComponent<RagdollCustomization>();
			}
			customization.ApplyPreset(preset, true, useBaseTexture);
			customization.RebindColors(bake, true);
