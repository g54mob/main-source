...
namespace PlayFab.Multiplayer.InteropWrapper
{
	public class PFMultiplayerServer
	{
		...
		public unsafe static int PFLobbyServerPostUpdateAsServer(PFLobbyHandle lobby, PFLobbyServerDataUpdate lobbyServerUpdate, object asyncIdentifier)
		{
			if (lobby == null)
			{
				return -2147024809;
			...
			using DisposableCollection disposableCollection = new DisposableCollection();
			PlayFab.Multiplayer.Interop.PFLobbyServerDataUpdate* lobbyServerUpdate2 = null;
			if (lobbyServerUpdate != null)
			{
				lobbyServerUpdate2 = lobbyServerUpdate.ToPointer(disposableCollection);
			}
			int num = Methods.PFLobbyServerPostUpdateAsServer(lobby.InteropHandle, lobbyServerUpdate2, intPtr.ToPointer());
			if (LobbyError.FAILED(num) && intPtr != IntPtr.Zero)
			{
				GCHandle.FromIntPtr(intPtr).Free();
			}
