...
namespace PlayFab.Multiplayer.InteropWrapper
{
	public static class Converters
	{
		...
		public static IntPtr StructToPtr<InteropStructType>(InteropStructType interopStruct, DisposableCollection disposableCollection)
		{
			bool isEnum = typeof(InteropStructType).IsEnum;
			int size = Marshal.SizeOf(isEnum ? Enum.GetUnderlyingType(typeof(InteropStructType)) : typeof(InteropStructType));
			DisposableBuffer disposableBuffer = disposableCollection.Add(new DisposableBuffer(size));
			IntPtr intPtr = disposableBuffer.IntPtr;
			Marshal.StructureToPtr(isEnum ? Convert.ChangeType(interopStruct, Enum.GetUnderlyingType(typeof(InteropStructType))) : ((object)interopStruct), disposableBuffer.IntPtr, fDeleteOld: false);
			return intPtr;
		}
		...
		internal unsafe static sbyte*[] StringArrayToPtr(string[] arr, DisposableCollection dc)
		{
			sbyte*[] array = new sbyte*[arr.Length];
			int num = 0;
			foreach (string str in arr)
			{
				array[num] = new UTF8StringPtr(str, dc).Pointer;
				num++;
			}
			return array;
		}
		...
		internal unsafe static string[] StringPtrToArray(sbyte** rawPtr, uint count)
		{
			string[] array = new string[count];
			for (int i = 0; i < count; i++)
			{
				array[i] = PtrToStringUTF8((IntPtr)rawPtr[i]);
			}
			return array;
		}
