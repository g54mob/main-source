...
namespace RoslynCSharp.Compiler
{
	public class RoslynCSharpCompiler
	{
		...
		public static bool loadCompiledAssemblies;
		...
		public CompilationResult CompileFromSyntaxTree(SyntaxTree[] syntaxTrees, IMetadataReferenceProvider[] additionalAsemblyReferences)
		{
			string text = null;
			...
			string assemblyName = Path.ChangeExtension(text2, null);
			...
			Compilation emitObject = CreateCompilationObject(assemblyName, references, syntaxTrees, compileOptions);
			...
			using (stream)
			{
				using (stream2)
				{
					CompilationResult compilationResult = EmitCompilationObject(emitObject, stream, stream2, loadCompiledAssemblies, loadAssemblyDomain, debugSymbolType);
					foreach (IAssemblyProcessor assemblyProcessor in assemblyProcessors)
					{
						try
						{
							assemblyProcessor.OnProcessAssembly(compilationResult.AssemblyOutput);
						}
					...
					lastCompileResult = compilationResult;
		...
		{
			if (processor != null && !assemblyProcessors.Contains(processor))
			{
		...
		{
			if (assemblyProcessors.Contains(processor))
			{
		...
		public static CompilationResult EmitCompilationObject(Compilation emitObject, Stream targetStream, Stream targetPDBStream = null, bool loadCompiledAssembly = true, AppDomain loadAssemblyDomain = null, DebugInformationFormat debugFormat = DebugInformationFormat.PortablePdb)
		{
			...
			{
				options = new EmitOptions(metadataOnly: false, debugFormat, null, null, 0, 0uL);
			}
