			...
			public static Type GenerateProxy()
			{
				...
				if (ProxyType == null)
				{
					...
					catch (ReflectionTypeLoadException ex)
					{
						...
						array[0] = ex;
						for (int num2 = 0; num2 < ex.LoaderExceptions.Length; num2++)
						{
							array[num2 + 1] = ex.LoaderExceptions[num2];
						}
					}
					...
					if (((array != null && array.Length != 0) ? 1 : 0) > (false ? 1 : 0))
					{
						stringBuilder.AppendLine("Listing all exceptions:");
						for (int num3 = 0; num3 < array.Length; num3++)
						{
							stringBuilder.Append("#").Append(num3).Append(": ")
								.AppendLine(array[num3]?.ToString() ?? "NULL");
						}
					}
