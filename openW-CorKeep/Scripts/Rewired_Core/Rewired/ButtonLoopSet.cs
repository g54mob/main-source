using System;
using Rewired.Config;
using Rewired.Utils;

namespace Rewired
{
	[CustomObfuscation(rename = false)]
	[CustomClassObfuscation(renamePubIntMembers = false, renamePrivateMembers = true)]
	internal class ButtonLoopSet : UpdateLoopDataSet<ButtonLoopSet.ButtonData>
	{
		[CustomObfuscation(rename = false)]
		[CustomClassObfuscation(renamePubIntMembers = false, renamePrivateMembers = true)]
		public class ButtonData
		{
			public readonly UpdateLoopType updateLoop;

			public readonly bool[] values;

			public readonly bool[] wasTrueThisFrame;

			private bool[] NcKJcIhisxvBSsXCitCPlFlhEVuO;

			private int PIvbNEJXxPxNJWNawHJkwIJhJvFuA;

			private readonly bool[] sFxbwHGQbBOnVfBXmDcDCXnDYXoMA;

			private readonly bool[] sXpqEzxTZpTaKQvcIyJOgEPugCHo;

			public bool[] effectiveValue
			{
				get
				{
					if (updateLoop == UpdateLoopType.FixedUpdate)
					{
						greJLMsyAafPBpoBlIxxcnRZYkFoA();
					}
					return NcKJcIhisxvBSsXCitCPlFlhEVuO;
				}
			}

			public ButtonData(int P_0, UpdateLoopType P_1)
			{
				updateLoop = P_1;
				values = new bool[P_0];
				sXpqEzxTZpTaKQvcIyJOgEPugCHo = new bool[P_0];
				wasTrueThisFrame = new bool[P_0];
				sFxbwHGQbBOnVfBXmDcDCXnDYXoMA = new bool[P_0];
				NcKJcIhisxvBSsXCitCPlFlhEVuO = new bool[P_0];
				PIvbNEJXxPxNJWNawHJkwIJhJvFuA = ReInput.timeScalePauseChangedCount;
			}

			public void SetValue(int index, bool value)
			{
				if (updateLoop == UpdateLoopType.FixedUpdate)
				{
					greJLMsyAafPBpoBlIxxcnRZYkFoA();
				}
				values[index] = value;
				if (value)
				{
					wasTrueThisFrame[index] = true;
					if (!sXpqEzxTZpTaKQvcIyJOgEPugCHo[index])
					{
						sFxbwHGQbBOnVfBXmDcDCXnDYXoMA[index] = true;
					}
				}
				NcKJcIhisxvBSsXCitCPlFlhEVuO[index] = value | sFxbwHGQbBOnVfBXmDcDCXnDYXoMA[index];
				sXpqEzxTZpTaKQvcIyJOgEPugCHo[index] = value;
			}

			public void ClearWasTrueThisFrame()
			{
				for (int i = 0; i < values.Length; i++)
				{
					wasTrueThisFrame[i] = false;
					sFxbwHGQbBOnVfBXmDcDCXnDYXoMA[i] = false;
					NcKJcIhisxvBSsXCitCPlFlhEVuO[i] = values[i];
				}
			}

			public void Clear()
			{
				Array.Clear(values, 0, values.Length);
				Array.Clear(sXpqEzxTZpTaKQvcIyJOgEPugCHo, 0, values.Length);
				Array.Clear(wasTrueThisFrame, 0, wasTrueThisFrame.Length);
				Array.Clear(sFxbwHGQbBOnVfBXmDcDCXnDYXoMA, 0, sFxbwHGQbBOnVfBXmDcDCXnDYXoMA.Length);
				Array.Clear(NcKJcIhisxvBSsXCitCPlFlhEVuO, 0, NcKJcIhisxvBSsXCitCPlFlhEVuO.Length);
				PIvbNEJXxPxNJWNawHJkwIJhJvFuA = ReInput.timeScalePauseChangedCount;
			}

			public void Import(ButtonData source)
			{
				if (source != null)
				{
					int num = MathTools.Min(values.Length, source.values.Length);
					for (int i = 0; i < num; i++)
					{
						values[i] = source.values[i];
						sXpqEzxTZpTaKQvcIyJOgEPugCHo[i] = source.sXpqEzxTZpTaKQvcIyJOgEPugCHo[i];
						wasTrueThisFrame[i] = source.wasTrueThisFrame[i];
						sFxbwHGQbBOnVfBXmDcDCXnDYXoMA[i] = source.sFxbwHGQbBOnVfBXmDcDCXnDYXoMA[i];
						NcKJcIhisxvBSsXCitCPlFlhEVuO[i] = source.NcKJcIhisxvBSsXCitCPlFlhEVuO[i];
						PIvbNEJXxPxNJWNawHJkwIJhJvFuA = source.PIvbNEJXxPxNJWNawHJkwIJhJvFuA;
					}
				}
			}

			private void greJLMsyAafPBpoBlIxxcnRZYkFoA()
			{
				if (ReInput.timeScalePauseChangedCount != PIvbNEJXxPxNJWNawHJkwIJhJvFuA)
				{
					ClearWasTrueThisFrame();
					PIvbNEJXxPxNJWNawHJkwIJhJvFuA = ReInput.timeScalePauseChangedCount;
				}
			}
		}

		public readonly int buttonCount;

		public ButtonLoopSet(UpdateLoopSetting P_0, int P_1)
			: base(P_0)
		{
			buttonCount = P_1;
			for (int i = 0; i < base.Count; i++)
			{
				base[i] = new ButtonData(P_1, GetUpdateLoopType(i));
			}
		}

		public void SetValue(int index, bool value, double timestamp)
		{
			int count = base.Count;
			for (int i = 0; i < count; i++)
			{
				base[i].SetValue(index, value);
			}
		}

		public void Clear()
		{
			int count = base.Count;
			for (int i = 0; i < count; i++)
			{
				base[i].Clear();
			}
		}

		public void Import(ButtonLoopSet set)
		{
			if (set == null)
			{
				throw new ArgumentNullException("set");
			}
			if (set.buttonCount != buttonCount)
			{
				throw new Exception("Cannot import from a set with a different button count.");
			}
			for (int i = 0; i < base.Count; i++)
			{
				base[i].Import(set[i]);
			}
		}
	}
}
