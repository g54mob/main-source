...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			hRoGYIBtIxJJzCeEizEYQKBhNiWpb(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return BwcfRaJHiUnYCxvMsUhRcEYAwcVCA().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				ILRzduXnCrOkMEEHVhgBHktOOgWJ(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return BwcfRaJHiUnYCxvMsUhRcEYAwcVCA().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				ILRzduXnCrOkMEEHVhgBHktOOgWJ(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			hRoGYIBtIxJJzCeEizEYQKBhNiWpb(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.JERDmxmjXTZcYmQpopFiMELrnhYp(_id);
			if (inputBehavior != null)
			{
				hRoGYIBtIxJJzCeEizEYQKBhNiWpb(inputBehavior, this, true);
			}
		}
		...
		internal SerializedObject BwcfRaJHiUnYCxvMsUhRcEYAwcVCA()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EndBkowpwOTxIGJnMBcsgiGqTpvf
			{
				...
				qqfRFgGAtDPLKSLpFGzHGleMdWxAb = 5.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EndBkowpwOTxIGJnMBcsgiGqTpvf
			{
				pIIbLDKqkVfRyNCGQyHEEVIpxwRdA = "id",
				qqfRFgGAtDPLKSLpFGzHGleMdWxAb = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EndBkowpwOTxIGJnMBcsgiGqTpvf
			{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.EndBkowpwOTxIGJnMBcsgiGqTpvf
			{
				...
				FnmpvPDmwsSGLmiBdhdjjjOdjSKDb = null,
				qqfRFgGAtDPLKSLpFGzHGleMdWxAb = string.Format("{0} {1}{2}{3}{4}{5}", "http://guavaman.com/rewired", "http://guavaman.com/schemas/rewired/", "1.4", "/", GetType().Name, ".xsd")
			});
