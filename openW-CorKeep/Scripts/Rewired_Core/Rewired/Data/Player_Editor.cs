...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal PFFTPKJTOCcLmdfRwTmvXmybQpDh iljbTpdIxSiAVvXcTZSItkySHgvOA()
			{
				return new PFFTPKJTOCcLmdfRwTmvXmybQpDh(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.ZwEHsomBYpCwhUwueCrLPncgybEq DDaKfJBBJGSMDIoJsPzOYNzGgtcn()
			{
				return new ControllerMapLayoutManager.ZwEHsomBYpCwhUwueCrLPncgybEq(_enabled, _loadFromUserDataStore, rVKpGVPRfmdnGKijnxjErtXXGpcJ());
			}

			private McokqOzRnXFuiyNVKSDKsVciAFJT[] rVKpGVPRfmdnGKijnxjErtXXGpcJ()
			{
				List<McokqOzRnXFuiyNVKSDKsVciAFJT> list = new List<McokqOzRnXFuiyNVKSDKsVciAFJT>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].qTFRFpPfCedmwqvaCGqXLttuFseBA());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.JmqjWaNbmLkTeEMjBbAurWsDFFCl BbNqtwmmHMnBNXmuTsQPhaWnwtOK()
			{
				return new ControllerMapEnabler.JmqjWaNbmLkTeEMjBbAurWsDFFCl(_enabled, yYncsEYaUFuypnPJxtcVALIMtUHi());
			}

			private McokqOzRnXFuiyNVKSDKsVciAFJT[] yYncsEYaUFuypnPJxtcVALIMtUHi()
			{
				List<McokqOzRnXFuiyNVKSDKsVciAFJT> list = new List<McokqOzRnXFuiyNVKSDKsVciAFJT>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal McokqOzRnXFuiyNVKSDKsVciAFJT qTFRFpPfCedmwqvaCGqXLttuFseBA()
			{
				return new McokqOzRnXFuiyNVKSDKsVciAFJT(_id, _enabled);
			}
		...
		internal FBvloJYAnsSEnqZpZgYHeIbOgKik fCPnkbiAoDdlxaOdUyEubSDoSpFs()
		{
			PFFTPKJTOCcLmdfRwTmvXmybQpDh[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new PFFTPKJTOCcLmdfRwTmvXmybQpDh[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].iljbTpdIxSiAVvXcTZSItkySHgvOA();
				}
			}
			PFFTPKJTOCcLmdfRwTmvXmybQpDh[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new PFFTPKJTOCcLmdfRwTmvXmybQpDh[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].iljbTpdIxSiAVvXcTZSItkySHgvOA();
				}
			}
			PFFTPKJTOCcLmdfRwTmvXmybQpDh[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new PFFTPKJTOCcLmdfRwTmvXmybQpDh[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].iljbTpdIxSiAVvXcTZSItkySHgvOA();
				}
			}
			PFFTPKJTOCcLmdfRwTmvXmybQpDh[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new PFFTPKJTOCcLmdfRwTmvXmybQpDh[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].iljbTpdIxSiAVvXcTZSItkySHgvOA();
				}
			}
			return new FBvloJYAnsSEnqZpZgYHeIbOgKik(array, array2, array3, array4);
		}
	}
}
