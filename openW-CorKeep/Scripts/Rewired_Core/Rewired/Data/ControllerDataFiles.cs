...
namespace Rewired.Data
{
	public sealed class ControllerDataFiles : ScriptableObject
	{
		...
		[NonSerialized]
		private readonly ADictionary<Guid, TOvbXCLGpcDMwICKloBsHgxZNTif> aeyEjpXWFcmPOkFHExWHZKZpsVRN = new ADictionary<Guid, TOvbXCLGpcDMwICKloBsHgxZNTif>(EqualityComparerNoAlloc<Guid>.Default);
		...
		internal ControllerTemplateElementIdentifier ehdHlrNGsSWjmvTIqxPMQtZBQGki(Guid P_0, int P_1, out HardwareJoystickMap P_2)
		{
			...
			{
				TOvbXCLGpcDMwICKloBsHgxZNTif tOvbXCLGpcDMwICKloBsHgxZNTif = mprDMFjMFiPKraeGwPqLbGSduYuub(templateGuid);
				if (tOvbXCLGpcDMwICKloBsHgxZNTif != null)
				{
					ControllerTemplateElementIdentifier controllerTemplateElementIdentifier = tOvbXCLGpcDMwICKloBsHgxZNTif.rMOiCrbVhOFYCRGEoqhGaNUixxVF(P_0, P_1);
		...
		internal int wQbKxjGGKJMTpxrjfiRpEjAcFpdcA(Guid P_0, Guid P_1, int P_2, List<HardwareControllerTemplateMap.ovLXoyvOyAIvHyJVxtrGqFYCplsV> P_3)
		{
			...
			return mprDMFjMFiPKraeGwPqLbGSduYuub(P_0)?.LpKSbSHMPLZgHoCXDluyNSRcIVJV(P_1, P_2, P_3) ?? 0;
		}

		internal HardwareJoystickMap_InputManager kuDTkCRUswjvXiZZxIFpUHpjvldV(Guid P_0, InputSource P_1)
		{
			roQOLxQduvaiTkEtzdsNGrfJPrdGb();
			...
			if (hardwareJoystickMap != null)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = rJEeMhZHzBEghvFiEhnJfihdLJIT(hardwareJoystickMap, bridgedController, true, out inputPlatform, out num, out platform);
		...
		internal HardwareJoystickMap_InputManager uajQFyhfQEGuDYnTVVKVqgGexwrV(BridgedControllerHWInfo P_0)
		{
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = HlGugChdLaabfrApkDsqioJDCbqL(P_0);
				if (hardwareJoystickMap_InputManager != null)
				{
					return hardwareJoystickMap_InputManager;
				}
			...
			for (int i = 0; i < hardwareJoystickMaps.Length; i++)
			{
				...
				int num;
				HardwareJoystickMap.Platform platform;
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager2 = rJEeMhZHzBEghvFiEhnJfihdLJIT(hardwareJoystickMaps[i], P_0, true, out inputPlatform, out num, out platform);
			...
			for (int j = 0; j < hardwareJoystickMaps.Length; j++)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager3 = rJEeMhZHzBEghvFiEhnJfihdLJIT(hardwareJoystickMaps[j], P_0, false, out inputPlatform2, out num2, out platform2);
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager4 = auWVlRcvPXkshLjZnLwHzcQuagLs(P_0, "[UNITY PRECONFIGURED JOYSTICK]");
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager5 = HlGugChdLaabfrApkDsqioJDCbqL(P_0);
		...
		{
			HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = auWVlRcvPXkshLjZnLwHzcQuagLs(P_0, "[STANDARDIZED GAMEPAD]");
			if (hardwareJoystickMap_InputManager == null)
			{
				return null;
			}
		...
		{
			if (aeyEjpXWFcmPOkFHExWHZKZpsVRN.TryGetValue(P_0, out var value))
			{
				return value;
			}
			...
			value = joystickTemplate.JjubZZCdhnGnrYlSKggStmjaadWE();
			aeyEjpXWFcmPOkFHExWHZKZpsVRN.Add(P_0, value);
			return value;
		}

		internal IHardwareControllerTemplateMap lzBTyjxmNRRmqklTrIqOieuylFJM(Guid P_0)
		{
			return mprDMFjMFiPKraeGwPqLbGSduYuub(P_0);
		}

		private void roQOLxQduvaiTkEtzdsNGrfJPrdGb()
		{
			if (!WDfPwokXkOeujyEMqAyhnSKkFFtJA)
			{
				...
				{
					WDfPwokXkOeujyEMqAyhnSKkFFtJA = true;
				}
			}
		}
	}
