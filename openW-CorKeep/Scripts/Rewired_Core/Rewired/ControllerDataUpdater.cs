...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] HAAbXaEETNeBzytfrnrokbfSjAQtA;
		...
		public ControllerDataUpdater(InputSource P_0, int P_1, int P_2, UnknownControllerHat[] P_3)
		{
			...
			axisCount = P_1;
			buttonCount = P_2;
			HAAbXaEETNeBzytfrnrokbfSjAQtA = P_3;
		...
		{
			if (HAAbXaEETNeBzytfrnrokbfSjAQtA == null)
			{
				return false;
			}
			for (int i = 0; i < HAAbXaEETNeBzytfrnrokbfSjAQtA.Length; i++)
			{
				if (HAAbXaEETNeBzytfrnrokbfSjAQtA[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < HAAbXaEETNeBzytfrnrokbfSjAQtA.Length; i++)
			{
				if (HAAbXaEETNeBzytfrnrokbfSjAQtA[i].ContainsButtonIndex(buttonIndex))
				{
					return HAAbXaEETNeBzytfrnrokbfSjAQtA[i].GetButtons();
				}
			}
			return null;
		}
