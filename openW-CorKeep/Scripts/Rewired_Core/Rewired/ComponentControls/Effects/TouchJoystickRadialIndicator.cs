...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchJoystickRadialIndicator : MonoBehaviour, IRegistrar<TouchJoystickAngleIndicator>
	{
		...
		private static readonly Vector2 AcZitxQsuQQnyCOhhJTWDagPXvDp = new Vector2(0.5f, 0.5f);

		private RectTransform qbiaWcgKGFBZWjgWTehzuCYCdfxBA;

		private List<TouchJoystickAngleIndicator> lZZKGBaAcPmefmaKKgqdExmNOXqU = new List<TouchJoystickAngleIndicator>(8);
		...
		private RectTransform acLAptKofVrBfnGZBirgMaVTUULjA => qbiaWcgKGFBZWjgWTehzuCYCdfxBA ?? (qbiaWcgKGFBZWjgWTehzuCYCdfxBA = GetComponent<RectTransform>());

		void IRegistrar<TouchJoystickAngleIndicator>.Register(TouchJoystickAngleIndicator registrant)
		{
			if (!(registrant == null) && ListTools.AddIfUnique(lZZKGBaAcPmefmaKKgqdExmNOXqU, registrant) && base.enabled)
			{
				zLJpJjVEGSjxMGRDMqByfAoAJfkfB(registrant);
			}
		}
		...
		void IRegistrar<TouchJoystickAngleIndicator>.Deregister(TouchJoystickAngleIndicator registrant)
		{
			if (!(registrant == null))
			{
				lZZKGBaAcPmefmaKKgqdExmNOXqU.Remove(registrant);
			}
		}
		...
		{
			lZZKGBaAcPmefmaKKgqdExmNOXqU.Clear();
		}

		private void JFVBgUIvelWIphwMHoieWbOEMDNkA()
		{
			for (int num = lZZKGBaAcPmefmaKKgqdExmNOXqU.Count - 1; num >= 0; num--)
			{
				TouchJoystickAngleIndicator touchJoystickAngleIndicator = lZZKGBaAcPmefmaKKgqdExmNOXqU[num];
				if (touchJoystickAngleIndicator.FrJxqCSRGedcwfyNpYTArFTuWYWf.IsNullOrDestroyed())
				{
					lZZKGBaAcPmefmaKKgqdExmNOXqU.RemoveAt(num);
				}
				else
				{
					zLJpJjVEGSjxMGRDMqByfAoAJfkfB(touchJoystickAngleIndicator);
				}
			}
		}
		...
		{
			if (!UnityTools.IsActiveAndEnabled(P_0.FrJxqCSRGedcwfyNpYTArFTuWYWf))
			{
				return;
			}
			RectTransform rectTransform = P_0.nFwuNngLrHbPvXYrFUfjNBkJJwgA;
			if (rectTransform == acLAptKofVrBfnGZBirgMaVTUULjA || rectTransform == null)
			{
				return;
			}
			Rect rect = acLAptKofVrBfnGZBirgMaVTUULjA.rect;
			if (_scale)
			{
				float num = (num = _aspectRatioX / _aspectRatioY);
				if (_preserveSpriteAspectRatio && P_0.CvfOkbgwjFIcCrAbCmODkHaUYj(out var vector))
				{
			...
			if (rectTransform.anchorMin != AcZitxQsuQQnyCOhhJTWDagPXvDp)
			{
				rectTransform.anchorMin = AcZitxQsuQQnyCOhhJTWDagPXvDp;
			}
			if (rectTransform.anchorMax != AcZitxQsuQQnyCOhhJTWDagPXvDp)
			{
				rectTransform.anchorMax = AcZitxQsuQQnyCOhhJTWDagPXvDp;
			}
		...
		private void HiwKJuIgskZMejQKwqaqtnkHuupt()
		{
			...
			for (int i = 0; i < childCount; i++)
			{
				...
				if (component != null)
				{
					lZZKGBaAcPmefmaKKgqdExmNOXqU.Add(component);
				}
			}
		}
	}
