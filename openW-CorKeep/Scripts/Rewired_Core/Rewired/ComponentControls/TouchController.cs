...
namespace Rewired.ComponentControls
{
	...
	public sealed class TouchController : CustomController
	{
		...
		[NonSerialized]
		private bool jrNlKjfaZcfXoFWvkYIieLateHBEb;
		...
		{
			base.OnDisable();
			if (base.nOUrFzHwiRQsdKlDZbDjWmtAGxuU && ReInput.isReady)
			{
				gNzcaXcLWRIbYQNsjMGMZHXbLnNgb(true);
			}
		}

		internal bool cxTaAWZKkcxJVjHciFvvWkNgczrHA()
		{
			if (!OnInitialize())
			{
				return false;
			...
			if (ReInput.isReady)
			{
				jrNlKjfaZcfXoFWvkYIieLateHBEb = ReInput.controllers.Mouse.enabled;
				gNzcaXcLWRIbYQNsjMGMZHXbLnNgb(false);
			}
			return true;
		}
		...
		private void gNzcaXcLWRIbYQNsjMGMZHXbLnNgb(bool P_0)
		{
			if (_disableMouseInputWhenEnabled)
			{
				if (P_0)
				{
					ReInput.controllers.Mouse.enabled = jrNlKjfaZcfXoFWvkYIieLateHBEb;
				}
				else
				{
		...
		private void eyebMmAWypNGkODjbIyxSYNmsLSjA()
		{
		}

		private bool KgKKMnaEYtNPCNsasOeWKezhtuPP()
		{
			if (ReInput.isReady)
			{
				return true;
