...
namespace Rewired.ComponentControls.Data
{
	...
	public sealed class CustomControllerElementSelector
	{
		...
		[HideInInspector]
		private int fjBsdijkpZMPkWJexmoIYqoQEbyl = -1;

		[HideInInspector]
		private int JuvCdrIaNyiwAroDtSxmZkBKBCezA = -1;
		...
		public int GetElementIndex(Rewired.CustomController customController)
		{
			...
			if (fjBsdijkpZMPkWJexmoIYqoQEbyl >= 0 && fjBsdijkpZMPkWJexmoIYqoQEbyl != customController.id)
			{
				ClearCache();
			}
			if (JuvCdrIaNyiwAroDtSxmZkBKBCezA >= 0)
			{
				return JuvCdrIaNyiwAroDtSxmZkBKBCezA;
			}
			fjBsdijkpZMPkWJexmoIYqoQEbyl = customController.id;
			switch (_selectorType)
			{
			case SelectorType.Id:
			{
				IList<ControllerElementIdentifier> list = ZwZuqTRQIAPDSWbMymaGPljNpSAy(customController, _elementType);
				for (int j = 0; j < list.Count; j++)
				{
					if (list[j].Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Eid == _elementId)
					{
						JuvCdrIaNyiwAroDtSxmZkBKBCezA = j;
						break;
					}
				}
				break;
			...
			case SelectorType.Index:
			{
				...
				JuvCdrIaNyiwAroDtSxmZkBKBCezA = _elementIndex;
				break;
			}
			case SelectorType.Name:
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					{
						JuvCdrIaNyiwAroDtSxmZkBKBCezA = i;
						break;
					}
				}
				break;
		...
		private IList<ControllerElementIdentifier> ZwZuqTRQIAPDSWbMymaGPljNpSAy(Rewired.CustomController P_0, ElementType P_1)
		{
			return P_1 switch
			{
		...
		{
			fjBsdijkpZMPkWJexmoIYqoQEbyl = -1;
			JuvCdrIaNyiwAroDtSxmZkBKBCezA = -1;
		}
	}
}
