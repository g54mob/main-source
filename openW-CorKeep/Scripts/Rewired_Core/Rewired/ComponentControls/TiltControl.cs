	...
	public sealed class TiltControl : CustomControllerControl
	{
		...
		public TiltDirection axesToUse
		{
			...
			set
			{
				...
				{
					znwpuDLYtTfyqJHaQsnPQPuTtCNR(value);
		...
		internal StandaloneAxis2D lCNTxVzBAKpbQKYbzcywArdTlkIo => _axis2D;

		private Vector3 rZtBYBqwYjaJxFXgvRZSeNLaIQBZA
		{
			get
			{
		...
		{
			Vector3 vector = rZtBYBqwYjaJxFXgvRZSeNLaIQBZA;
		...
		internal override void OnValidate()
		{
			...
			{
				rslBhUOkYkDlJsMqECVChYnwyfhO();
			}
		}

		internal bool TIKjaQWipmcQHGAcLBqPnPXoJNII()
		{
		...
		internal void dLffqVgDrsmrhvAxJVFgQeGyUolw()
		{
			base.ZadSFFqddMfzbdzzllVuUFOpUuig();
			if (base.kCCUxJdqnJEDkElVVvKpRTWHmWjo)
			{
				OUmClBGUPwdTRdHButPpNAnBpzJab();
			}
		}

		internal void JzBgecOFTcFPClVgtzHpWAkDqCAH()
		{
			if (base.kCCUxJdqnJEDkElVVvKpRTWHmWjo && lHrmyZFsaDtMgFwuiNoBbdveNATp)
			{
				if (_useFAxis)
				{
					RBVJiriJzxhTlbkVFcwgUMluaNDw(_forwardTiltCustomControllerElement, _axis2D.yAxis.value, _axis2D.yAxis.buttonActivationThreshold);
				}
				if (_useHAxis)
				{
					RBVJiriJzxhTlbkVFcwgUMluaNDw(_horizontalTiltCustomControllerElement, _axis2D.xAxis.value, _axis2D.xAxis.buttonActivationThreshold);
				}
			}
		}
		...
		public override void ClearValue()
		{
			...
			{
				base.NOHjKMqVvRxqcULJVMYRwUmUvaPl.ClearElementValue(_horizontalTiltCustomControllerElement);
				base.NOHjKMqVvRxqcULJVMYRwUmUvaPl.ClearElementValue(_forwardTiltCustomControllerElement);
			}
		}
		...
		private void OUmClBGUPwdTRdHButPpNAnBpzJab()
		{
			...
			{
				float rawValue;
				if (rZtBYBqwYjaJxFXgvRZSeNLaIQBZA == Vector3.zero)
				{
					rawValue = 0f;
				}
				else
				{
					float value = Mathf.Atan2(rZtBYBqwYjaJxFXgvRZSeNLaIQBZA.x, 0f - rZtBYBqwYjaJxFXgvRZSeNLaIQBZA.y) * 57.29578f + _horizontalRestAngle;
			...
			if (_useFAxis)
			{
				...
				{
					float value2 = Mathf.Atan2(rZtBYBqwYjaJxFXgvRZSeNLaIQBZA.z, 0f - rZtBYBqwYjaJxFXgvRZSeNLaIQBZA.y) * 57.29578f + _forwardRestAngle;
		...
		{
			znwpuDLYtTfyqJHaQsnPQPuTtCNR(_allowedTiltDirections);
			if (lHrmyZFsaDtMgFwuiNoBbdveNATp)
			{
				if (_useHAxis)
				{
					base.NOHjKMqVvRxqcULJVMYRwUmUvaPl.ValidateElements(_horizontalTiltCustomControllerElement);
				}
				if (_useFAxis)
				{
					base.NOHjKMqVvRxqcULJVMYRwUmUvaPl.ValidateElements(_forwardTiltCustomControllerElement);
				}
			}
		}
