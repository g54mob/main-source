...
namespace Rewired
{
	...
	public sealed class ControllerSetSelector : ISerializationCallbackReceiver, IDeepCloneable
	{
		...
		[NonSerialized]
		private Guid ceRpOoYCXMmArLthaDhbprlOBWfU;

		internal bool FirvBZgEHtrYUahPWTYTtjcqUpny => _type != Type.All;
		...
		public Guid hardwareTypeGuid
		{
			get
			{
				...
				return ceRpOoYCXMmArLthaDhbprlOBWfU;
			}
			set
			{
				...
				ceRpOoYCXMmArLthaDhbprlOBWfU = value;
		...
		public ControllerSetSelector(ControllerSetSelector P_0)
		{
			...
			ceRpOoYCXMmArLthaDhbprlOBWfU = P_0.ceRpOoYCXMmArLthaDhbprlOBWfU;
		}
		...
		internal ControllerSetSelector(Type P_0, ControllerType P_1, string P_2, string P_3, int P_4)
		{
			...
			_controllerType = P_1;
			_guid = P_2;
			ceRpOoYCXMmArLthaDhbprlOBWfU = StringTools.ToGuid(P_2);
		...
		public bool Matches(Controller controller)
		{
			...
			case Type.HardwareType:
				if (ceRpOoYCXMmArLthaDhbprlOBWfU != Guid.Empty)
				{
					return ceRpOoYCXMmArLthaDhbprlOBWfU == controller.hardwareTypeGuid;
				}
			...
			case Type.ControllerTemplateType:
				return controller.ImplementsTemplate(ceRpOoYCXMmArLthaDhbprlOBWfU);
			case Type.PersistentControllerInstance:
				return controller.deviceInstanceGuid == ceRpOoYCXMmArLthaDhbprlOBWfU;
		...
		private void XUnYlOOJDaeRpBrqWRyBsEONGwJO()
		{
			_guid = string.Empty;
			ceRpOoYCXMmArLthaDhbprlOBWfU = Guid.Empty;
		}

		void ISerializationCallbackReceiver.OnAfterDeserialize()
		{
			ceRpOoYCXMmArLthaDhbprlOBWfU = StringTools.ToGuid(_guid);
		}
