...
namespace Rewired
{
	...
	{
		internal class ZwEHsomBYpCwhUwueCrLPncgybEq
		{
			public bool WpubdwkhLgXzisitndoEFmNCjaLxA;

			public bool qcsyJVzcHiQwIHZdEZBStMBryCNK;

			public McokqOzRnXFuiyNVKSDKsVciAFJT[] OUHoMSGcvGfYtvDXMvOnJHEtBLeU;

			public ZwEHsomBYpCwhUwueCrLPncgybEq(bool P_0, bool P_1, McokqOzRnXFuiyNVKSDKsVciAFJT[] P_2)
			{
				WpubdwkhLgXzisitndoEFmNCjaLxA = P_0;
				qcsyJVzcHiQwIHZdEZBStMBryCNK = P_1;
				OUHoMSGcvGfYtvDXMvOnJHEtBLeU = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.FirvBZgEHtrYUahPWTYTtjcqUpny)
					{
		...
		private bool daUhusiptpdGMecAZrFeEyFoKnBfA;

		private bool lKECWoywhIrQUzQBqWaqiGndqRLr = true;

		private Player kKnXKAHwDnuLRBsuUjdUPiELippcA;

		private ZwEHsomBYpCwhUwueCrLPncgybEq qKhzOmBVIyrunVybKvdqNmWRuFFv;

		private readonly int ndsGPvdGkneRoeEFcRVRyWHmeeXab;

		private List<RuleSet> tRAVtBdDqQxboJbtBSGUmOtIVwDL;

		private Action cFAKeXoPnmIOvhAmsbCVVRnFteTm;

		public bool enabled
		{
			get
			{
				return daUhusiptpdGMecAZrFeEyFoKnBfA;
			}
			set
			{
				daUhusiptpdGMecAZrFeEyFoKnBfA = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return lKECWoywhIrQUzQBqWaqiGndqRLr;
			}
			set
			{
				lKECWoywhIrQUzQBqWaqiGndqRLr = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return tRAVtBdDqQxboJbtBSGUmOtIVwDL;
			}
			set
			{
				if (value == null)
				...
				tRAVtBdDqQxboJbtBSGUmOtIVwDL = value;
			}
		}

		internal event Action buhuabueJhyVhtyejBQPnQenGmUq
		{
			add
			{
				cFAKeXoPnmIOvhAmsbCVVRnFteTm = (Action)Delegate.Combine(cFAKeXoPnmIOvhAmsbCVVRnFteTm, b);
			}
			remove
			{
				cFAKeXoPnmIOvhAmsbCVVRnFteTm = (Action)Delegate.Remove(cFAKeXoPnmIOvhAmsbCVVRnFteTm, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, ZwEHsomBYpCwhUwueCrLPncgybEq P_1)
		{
			if (P_0 == null)
			{
			...
			ndsGPvdGkneRoeEFcRVRyWHmeeXab = ReInput.id;
			kKnXKAHwDnuLRBsuUjdUPiELippcA = P_0;
			qKhzOmBVIyrunVybKvdqNmWRuFFv = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != ndsGPvdGkneRoeEFcRVRyWHmeeXab)
			{
				ReInput.CheckInitialized(ndsGPvdGkneRoeEFcRVRyWHmeeXab);
				return;
			}
			cFAKeXoPnmIOvhAmsbCVVRnFteTm?.Invoke();
			if (!daUhusiptpdGMecAZrFeEyFoKnBfA || tRAVtBdDqQxboJbtBSGUmOtIVwDL == null)
			{
				return;
			}
			int count = tRAVtBdDqQxboJbtBSGUmOtIVwDL.Count;
			if (count == 0)
			{
				return;
			}
			...
			kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.maps.GetAllMaps(list);
			list2.AddRange(kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = tRAVtBdDqQxboJbtBSGUmOtIVwDL[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (lKECWoywhIrQUzQBqWaqiGndqRLr && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(kKnXKAHwDnuLRBsuUjdUPiELippcA.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							kKnXKAHwDnuLRBsuUjdUPiELippcA.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((qKhzOmBVIyrunVybKvdqNmWRuFFv != null && qKhzOmBVIyrunVybKvdqNmWRuFFv.OUHoMSGcvGfYtvDXMvOnJHEtBLeU != null) ? qKhzOmBVIyrunVybKvdqNmWRuFFv.OUHoMSGcvGfYtvDXMvOnJHEtBLeU.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(qKhzOmBVIyrunVybKvdqNmWRuFFv.OUHoMSGcvGfYtvDXMvOnJHEtBLeU[i].UWiqtNzAqWGmXHoHgqumKurdgkWdA);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = qKhzOmBVIyrunVybKvdqNmWRuFFv.OUHoMSGcvGfYtvDXMvOnJHEtBLeU[i].qgjgxJWDmFpcXvgQKstNkEcLcxzi;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (qKhzOmBVIyrunVybKvdqNmWRuFFv != null)
			{
				daUhusiptpdGMecAZrFeEyFoKnBfA = qKhzOmBVIyrunVybKvdqNmWRuFFv.WpubdwkhLgXzisitndoEFmNCjaLxA;
				lKECWoywhIrQUzQBqWaqiGndqRLr = qKhzOmBVIyrunVybKvdqNmWRuFFv.qcsyJVzcHiQwIHZdEZBStMBryCNK;
			}
			tRAVtBdDqQxboJbtBSGUmOtIVwDL = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return UPffjviSrpSpeQFDEYnGIQiHramvA().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return UPffjviSrpSpeQFDEYnGIQiHramvA().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				YKYcKIiEbpPGzgAWDcAtWIJSWmMgb(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				YKYcKIiEbpPGzgAWDcAtWIJSWmMgb(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject UPffjviSrpSpeQFDEYnGIQiHramvA()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			ewrTUcyvlcRWjhZIHhkkYzYJfLRv(serializedObject);
			return serializedObject;
		}

		private void ewrTUcyvlcRWjhZIHhkkYzYJfLRv(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", daUhusiptpdGMecAZrFeEyFoKnBfA);
			P_0.Add("loadFromUserDataStore", lKECWoywhIrQUzQBqWaqiGndqRLr);
			P_0.Add("ruleSets", tRAVtBdDqQxboJbtBSGUmOtIVwDL);
		}

		private bool YKYcKIiEbpPGzgAWDcAtWIJSWmMgb(SerializedObject P_0)
		{
			daUhusiptpdGMecAZrFeEyFoKnBfA = false;
			tRAVtBdDqQxboJbtBSGUmOtIVwDL = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref daUhusiptpdGMecAZrFeEyFoKnBfA);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref lKECWoywhIrQUzQBqWaqiGndqRLr);
