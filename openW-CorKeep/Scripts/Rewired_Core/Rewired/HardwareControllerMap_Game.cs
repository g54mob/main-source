...
namespace Rewired
{
	...
	internal class HardwareControllerMap_Game
	{
		...
		public DeviceLocalizationInfo deviceLocalizationInfo => HcEdmueDFZNJKLqFBMktbRBJwBmBB;
		...
		{
			controllerName = P_0;
			HcEdmueDFZNJKLqFBMktbRBJwBmBB = P_1;
			if (HcEdmueDFZNJKLqFBMktbRBJwBmBB == null)
			{
				HcEdmueDFZNJKLqFBMktbRBJwBmBB = new DeviceLocalizationInfo();
			}
			HcEdmueDFZNJKLqFBMktbRBJwBmBB.FinishRuntimeSetup();
			bool flag = HcEdmueDFZNJKLqFBMktbRBJwBmBB.controllerType != ControllerType.Keyboard && HcEdmueDFZNJKLqFBMktbRBJwBmBB.controllerType != ControllerType.Mouse;
			for (int i = 0; i < P_2.Length; i++)
			{
				...
				if (flag)
				{
					if (ControllerElementIdentifier.zStnLKifidbOXaBFoPmPwuIUUBLbA.LcuqTFgbPMssslxAydOjKFtVGxBM(HcEdmueDFZNJKLqFBMktbRBJwBmBB, P_2[i], out var controllerElementIdentifier))
					{
						P_2[i] = controllerElementIdentifier;
						continue;
					}
					ControllerElementIdentifier.zStnLKifidbOXaBFoPmPwuIUUBLbA.XgMTTRMFBNFBZCsCnSkVgKfKAFfEB(HcEdmueDFZNJKLqFBMktbRBJwBmBB, P_2[i]);
				}
				P_2[i].FinishRuntimeSetup(HcEdmueDFZNJKLqFBMktbRBJwBmBB, HcEdmueDFZNJKLqFBMktbRBJwBmBB.controllerType);
			}
			...
			for (int l = 0; l < num; l++)
			{
				int num6 = PVYRoCIQLyQSBVwGrWchesmauBrf(P_2, P_3[l]);
				if (num6 < 0)
				{
			...
			{
				int num7 = PVYRoCIQLyQSBVwGrWchesmauBrf(P_2, P_4[m]);
				if (num7 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num8 = PVYRoCIQLyQSBVwGrWchesmauBrf(P_2, array[n]);
				if (num8 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num10 = PVYRoCIQLyQSBVwGrWchesmauBrf(P_2, array2[num9]);
				if (num10 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
			...
			{
				int num12 = PVYRoCIQLyQSBVwGrWchesmauBrf(P_2, array3[num11]);
				if (num12 < 0)
				{
					Logger.LogError("Invalid hardware element identifier id!");
				}
		...
		public bool TryGetCompoundElementMemberCombinedLocalizedName(IList<ActionElementMap> aems, out string result)
		{
			...
			for (int i = 0; i < compoundElements.Length; i++)
			{
				...
				int num2;
				switch (tRnOxdMncwdRrQbdzARtYEVhXVwR(compoundElement, elementIdentifierById, aems, out num2))
				{
				case MbZXUOQejSjjvMYluRCWgEZyaHGJ.IsWholeElement:
					result = elementIdentifierById.Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Ename;
					break;
				case MbZXUOQejSjjvMYluRCWgEZyaHGJ.FoundIndex:
					if (num2 >= 0)
					{
		...
		private static MbZXUOQejSjjvMYluRCWgEZyaHGJ tRnOxdMncwdRrQbdzARtYEVhXVwR(HardwareJoystickMap.CompoundElement P_0, ControllerElementIdentifier P_1, IList<ActionElementMap> P_2, out int P_3)
		{
			...
			{
				return MbZXUOQejSjjvMYluRCWgEZyaHGJ.Error;
			}
			...
			AomZkhATSIadYOOLVfcgOnNtMQBs.oNzOlLehqVXtXsHwkBQOBMVHFvXX oNzOlLehqVXtXsHwkBQOBMVHFvXX;
			...
			case CompoundControllerElementType.Axis2D:
			{
				...
				if (num3 == 1)
				{
					if (num4 == 1)
					{
						return MbZXUOQejSjjvMYluRCWgEZyaHGJ.IsWholeElement;
					}
					if (num7 == 1 && num8 == 1)
					{
						return MbZXUOQejSjjvMYluRCWgEZyaHGJ.IsWholeElement;
				...
				else if (num5 == 1 && num6 == 1)
				{
					...
					if (AomZkhATSIadYOOLVfcgOnNtMQBs.kvsrEyzaoorJFgijytjCTPHeGAViA(resultCompoundElementType, out oNzOlLehqVXtXsHwkBQOBMVHFvXX))
					{
						P_3 = oNzOlLehqVXtXsHwkBQOBMVHFvXX.dgHhUFDLdjRgLayKedeJuiwMegLub(AxisDirection.Horizontal);
						if (P_3 >= 0)
						{
							return MbZXUOQejSjjvMYluRCWgEZyaHGJ.FoundIndex;
						}
						return MbZXUOQejSjjvMYluRCWgEZyaHGJ.Error;
					}
				}
				else
				{
					...
					if (AomZkhATSIadYOOLVfcgOnNtMQBs.kvsrEyzaoorJFgijytjCTPHeGAViA(resultCompoundElementType, out oNzOlLehqVXtXsHwkBQOBMVHFvXX))
					{
						P_3 = oNzOlLehqVXtXsHwkBQOBMVHFvXX.dgHhUFDLdjRgLayKedeJuiwMegLub(AxisDirection.Vertical);
						if (P_3 >= 0)
						{
							return MbZXUOQejSjjvMYluRCWgEZyaHGJ.FoundIndex;
						}
