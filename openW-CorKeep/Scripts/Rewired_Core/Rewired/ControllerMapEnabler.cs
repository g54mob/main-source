...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.FirvBZgEHtrYUahPWTYTtjcqUpny)
				{
		...
		internal class JmqjWaNbmLkTeEMjBbAurWsDFFCl
		{
			public bool UtieMOOnlTiecnvlfiZugJkAgdnM;

			public McokqOzRnXFuiyNVKSDKsVciAFJT[] ulSqdmmrqTQIxnYUNRKDrfhCIrFT;

			public JmqjWaNbmLkTeEMjBbAurWsDFFCl(bool P_0, McokqOzRnXFuiyNVKSDKsVciAFJT[] P_1)
			{
				UtieMOOnlTiecnvlfiZugJkAgdnM = P_0;
				ulSqdmmrqTQIxnYUNRKDrfhCIrFT = P_1;
			}
		}

		private bool XDauwCSlMtXCrahEPcONiAagkuBF;

		private Player bXVdasTwJfEKPBXnMlWIiMxvoNNdb;

		private JmqjWaNbmLkTeEMjBbAurWsDFFCl sVLwrQUrCLUJjoHfHbAvovKAXRJc;

		private readonly int xIofojDGqIoSyckzQOWbDScqqATdA;

		private List<RuleSet> kfqCknIbISsIyIYDcfrfLsAArrhC;

		public bool enabled
		{
			get
			{
				return XDauwCSlMtXCrahEPcONiAagkuBF;
			}
			set
			{
				XDauwCSlMtXCrahEPcONiAagkuBF = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return kfqCknIbISsIyIYDcfrfLsAArrhC;
			}
			set
			{
				if (value == null)
				...
				kfqCknIbISsIyIYDcfrfLsAArrhC = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, JmqjWaNbmLkTeEMjBbAurWsDFFCl P_1)
		{
			if (P_0 == null)
			{
			...
			xIofojDGqIoSyckzQOWbDScqqATdA = ReInput.id;
			bXVdasTwJfEKPBXnMlWIiMxvoNNdb = P_0;
			sVLwrQUrCLUJjoHfHbAvovKAXRJc = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != xIofojDGqIoSyckzQOWbDScqqATdA)
			{
				ReInput.CheckInitialized(xIofojDGqIoSyckzQOWbDScqqATdA);
			}
			else
			{
				if (!XDauwCSlMtXCrahEPcONiAagkuBF || kfqCknIbISsIyIYDcfrfLsAArrhC == null)
				{
					return;
				}
				int count = kfqCknIbISsIyIYDcfrfLsAArrhC.Count;
				if (count == 0)
				{
					return;
				}
				...
				bXVdasTwJfEKPBXnMlWIiMxvoNNdb.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = kfqCknIbISsIyIYDcfrfLsAArrhC[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((sVLwrQUrCLUJjoHfHbAvovKAXRJc != null && sVLwrQUrCLUJjoHfHbAvovKAXRJc.ulSqdmmrqTQIxnYUNRKDrfhCIrFT != null) ? sVLwrQUrCLUJjoHfHbAvovKAXRJc.ulSqdmmrqTQIxnYUNRKDrfhCIrFT.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(sVLwrQUrCLUJjoHfHbAvovKAXRJc.ulSqdmmrqTQIxnYUNRKDrfhCIrFT[i].UWiqtNzAqWGmXHoHgqumKurdgkWdA);
				...
				controllerMapEnablerRuleSetInstance.enabled = sVLwrQUrCLUJjoHfHbAvovKAXRJc.ulSqdmmrqTQIxnYUNRKDrfhCIrFT[i].qgjgxJWDmFpcXvgQKstNkEcLcxzi;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (sVLwrQUrCLUJjoHfHbAvovKAXRJc != null)
			{
				XDauwCSlMtXCrahEPcONiAagkuBF = sVLwrQUrCLUJjoHfHbAvovKAXRJc.UtieMOOnlTiecnvlfiZugJkAgdnM;
			}
			kfqCknIbISsIyIYDcfrfLsAArrhC = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return eQQdCEHlxnoLOVxgTIlaFSqUoxIkA().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return eQQdCEHlxnoLOVxgTIlaFSqUoxIkA().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				fMXyZfnMSitNMFnjPhqHePAilqGCb(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				fMXyZfnMSitNMFnjPhqHePAilqGCb(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject eQQdCEHlxnoLOVxgTIlaFSqUoxIkA()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			hFKrTEOwzNayCdgaZDJuzItPIcTl(serializedObject);
			return serializedObject;
		}

		private void hFKrTEOwzNayCdgaZDJuzItPIcTl(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", XDauwCSlMtXCrahEPcONiAagkuBF);
			P_0.Add("ruleSets", kfqCknIbISsIyIYDcfrfLsAArrhC);
		}

		private bool fMXyZfnMSitNMFnjPhqHePAilqGCb(SerializedObject P_0)
		{
			XDauwCSlMtXCrahEPcONiAagkuBF = false;
			kfqCknIbISsIyIYDcfrfLsAArrhC = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref XDauwCSlMtXCrahEPcONiAagkuBF);
