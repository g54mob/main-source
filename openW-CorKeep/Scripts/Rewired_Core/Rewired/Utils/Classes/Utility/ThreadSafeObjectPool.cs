...
namespace Rewired.Utils.Classes.Utility
{
	...
	internal class ThreadSafeObjectPool<T> : IObjectPool, IObjectPool<T> where T : class
	{
		...
		protected ulong InstanceCount => glwEfgvhuYYgeFixPbLfGJtLOxrI;
		...
		{
			while (Interlocked.Exchange(ref MdqVTgygweniCfMMOuKboHyMZRmd, 1) != 0)
			{
				Thread.SpinWait(1);
			}
			_pool.Clear();
			...
			Interlocked.Exchange(ref MdqVTgygweniCfMMOuKboHyMZRmd, 0);
		}
		...
		{
			glwEfgvhuYYgeFixPbLfGJtLOxrI = ((glwEfgvhuYYgeFixPbLfGJtLOxrI < ulong.MaxValue) ? (glwEfgvhuYYgeFixPbLfGJtLOxrI + 1) : 0);
			return glwEfgvhuYYgeFixPbLfGJtLOxrI;
		}
	}
}
