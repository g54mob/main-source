...
namespace Rewired.Utils
{
	...
	{
		private static byte[] VVQLVGDaUxVBNZjUDMsNOsbfNrec;
		...
		public static bool FillMemory(IntPtr buffer, int startIndex, int length, byte value, bool throwOnError = true)
		{
			...
			if (value != 0)
			{
				if (VVQLVGDaUxVBNZjUDMsNOsbfNrec == null)
				{
					VVQLVGDaUxVBNZjUDMsNOsbfNrec = new byte[8];
				}
				bool flag = false;
				if (num >= 8)
				{
					long val;
					lock (VVQLVGDaUxVBNZjUDMsNOsbfNrec)
					{
						for (int i = 0; i < 8; i++)
						{
							VVQLVGDaUxVBNZjUDMsNOsbfNrec[i] = value;
						}
						flag = true;
						val = BitConverter.ToInt64(VVQLVGDaUxVBNZjUDMsNOsbfNrec, 0);
					}
					int num2 = length / 8 * 8;
				...
				if (num >= 4)
				{
					...
					lock (VVQLVGDaUxVBNZjUDMsNOsbfNrec)
					{
						if (!flag)
						{
							for (int k = 0; k < 4; k++)
							{
								VVQLVGDaUxVBNZjUDMsNOsbfNrec[k] = value;
							}
							flag = true;
						}
						val2 = BitConverter.ToInt32(VVQLVGDaUxVBNZjUDMsNOsbfNrec, 0);
					}
					int num3 = length / 4 * 4;
				...
				if (num >= 2)
				{
					...
					lock (VVQLVGDaUxVBNZjUDMsNOsbfNrec)
					{
						if (!flag)
						{
							for (int m = 0; m < 2; m++)
							{
								VVQLVGDaUxVBNZjUDMsNOsbfNrec[m] = value;
							}
							flag = true;
						}
						val3 = BitConverter.ToInt16(VVQLVGDaUxVBNZjUDMsNOsbfNrec, 0);
					}
					int num4 = length / 2 * 2;
