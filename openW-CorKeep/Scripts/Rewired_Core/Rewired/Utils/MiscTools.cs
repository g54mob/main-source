...
namespace Rewired.Utils
{
	...
	{
		private static uint deDfVNiLUEDLtyirqqOdEgXXipkfA = 0u;

		private static uint IOPKsQdWMKUVAKHVoeiglIJruDzr = 1u;

		private static uint SeqdGZeXKASVxIwySUnpaRLxSUGQA = uint.MaxValue;

		private static int MLSFBnmfGrAlWMPrFVModeQPalqp = 0;

		private static int yTcHSiZPqLTcRgChROIqbTYaoCHE = int.MaxValue;

		private static int yyqaWUazrGRVjHbuTHhYyJOrdNiV = -1;
		...
		{
			counter = ((counter != SeqdGZeXKASVxIwySUnpaRLxSUGQA) ? (counter + 1) : IOPKsQdWMKUVAKHVoeiglIJruDzr);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != yTcHSiZPqLTcRgChROIqbTYaoCHE) ? (counter + 1) : MLSFBnmfGrAlWMPrFVModeQPalqp);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != IOPKsQdWMKUVAKHVoeiglIJruDzr) ? (counter - 1) : SeqdGZeXKASVxIwySUnpaRLxSUGQA);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= yyqaWUazrGRVjHbuTHhYyJOrdNiV)
			{
				return yTcHSiZPqLTcRgChROIqbTYaoCHE;
			}
			counter = ((counter != MLSFBnmfGrAlWMPrFVModeQPalqp) ? (counter - 1) : yTcHSiZPqLTcRgChROIqbTYaoCHE);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != deDfVNiLUEDLtyirqqOdEgXXipkfA;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > yyqaWUazrGRVjHbuTHhYyJOrdNiV;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == deDfVNiLUEDLtyirqqOdEgXXipkfA)
			{
				return false;
			}
			if (tick2 == deDfVNiLUEDLtyirqqOdEgXXipkfA)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= yyqaWUazrGRVjHbuTHhYyJOrdNiV)
			{
				return false;
			}
			if (tick2 <= yyqaWUazrGRVjHbuTHhYyJOrdNiV)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = SeqdGZeXKASVxIwySUnpaRLxSUGQA - num + num2;
			uint iOPKsQdWMKUVAKHVoeiglIJruDzr = IOPKsQdWMKUVAKHVoeiglIJruDzr;
			uint num4 = num3 - iOPKsQdWMKUVAKHVoeiglIJruDzr;
			if (tick1 >= tick2)
			{
				return 0L - (long)num4;
			}
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = yTcHSiZPqLTcRgChROIqbTYaoCHE - num + num2;
			int mLSFBnmfGrAlWMPrFVModeQPalqp = MLSFBnmfGrAlWMPrFVModeQPalqp;
			int num4 = num3 - mLSFBnmfGrAlWMPrFVModeQPalqp;
			if (tick1 >= tick2)
			{
				return -num4;
			}
