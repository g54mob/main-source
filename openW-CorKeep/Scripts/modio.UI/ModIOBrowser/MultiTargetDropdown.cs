...
namespace ModIOBrowser
{
	public class MultiTargetDropdown : TMP_Dropdown
	{
		...
		{
			base.Awake();
			foreach (OptionData option in base.options)
			{
				references.Add(option.text);
			}
		...
		{
			for (int i = 0; i < Math.Min(references.Count, base.options.Count); i++)
			{
				base.options[i].text = SelfInstancingMonoSingleton<TranslationManager>.Instance.Get(references[i]);
			}
			if (base.value >= 0 && base.value < base.options.Count)
			{
				base.captionText.text = base.options[base.value].text;
			}
		}
		...
		protected override void DoStateTransition(SelectionState state, bool instant)
		{
			...
			foreach (MultiTargetDropdown childDropdown in childDropdowns)
			{
				childDropdown.DoStateTransition(state, instant);
			}
		...
		{
			DropdownItem result = base.CreateItem(itemTemplate);
			if (border == null)
			{
				border = itemTemplate.transform.parent.Find("Border")?.gameObject;
			}
			MultiTargetButton component = itemTemplate.GetComponent<MultiTargetButton>();
			if (component != null)
			{
