		...
		protected override void DoStateTransition(SelectionState state, bool instant)
		{
			...
			foreach (Target extraTarget in extraTargets)
			{
				...
				case MultiTargetTransition.DisableEnable:
					if (extraTarget.isControllerButtonIcon && SelfInstancingMonoSingleton<InputNavigation>.Instance.mouseNavigation)
					{
						extraTarget?.target?.gameObject.SetActive(value: false);
					}
					else
					{
			...
			foreach (MultiTargetButton childButton in childButtons)
			{
				childButton.DoStateTransition(state, instant);
			}
