...
namespace Affixes.Components.Generated
{
	...
	public struct GhostComponentSerializerRegistrationSystem : ISystem, IGhostComponentSerializerRegistration
	{
		...
		internal delegate void __codegen__OnUpdate_0000003C_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

		internal static class __codegen__OnUpdate_0000003C_0024BurstDirectCall
		{
			...
			private static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnUpdate_0000003C_0024PostfixBurstDelegate>(__codegen__OnUpdate).Value;
				}
				P_0 = Pointer;
			}
		...
		public void OnCreate(ref SystemState state)
		{
			...
			{
				DisplayName = "ActiveAffixConditionsBuffer",
				Component = ComponentType.ReadWrite<ActiveAffixConditionsBuffer>(),
				Hash = 9095536301186797652uL,
				SelfIndex = -1,
				SerializerIndex = -1,
			...
			componentTypeSerializationStrategy = new ComponentTypeSerializationStrategy
			{
				DisplayName = "Affixes.Components.AffixCD",
				Component = ComponentType.ReadWrite<AffixCD>(),
				Hash = 15545961158267030270uL,
				SelfIndex = -1,
				SerializerIndex = -1,
				PrefabType = GhostPrefabType.All,
				SendTypeOptimization = GhostSendType.AllClients,
			...
			valueRW.AddSerializationStrategy(ref componentTypeSerializationStrategy);
			valueRW.AddSerializer(ActiveAffixConditionsBufferGhostComponentSerializer.GetState(ref state));
			valueRW.AddSerializer(AffixCDGhostComponentSerializer.GetState(ref state));
		}
		...
		{
			__codegen__OnUpdate_0000003C_0024BurstDirectCall.Invoke(self, state);
		}
