...
namespace ContainedMiniSim
{
	...
	public struct UpdateAquariumFishSystem : ISystem, ISystemCompilerGenerated
	{
		[StructLayout(LayoutKind.Sequential, Size = 1)]
		private readonly struct IFE_848063814_0
		{
		...
		{
			public IFE_848063814_0.TypeHandle __IFE_848063814_0_TypeHandle;

			[MethodImpl(MethodImplOptions.AggressiveInlining)]
			public void __AssignHandles(ref SystemState state)
			{
				__IFE_848063814_0_TypeHandle = new IFE_848063814_0.TypeHandle(ref state);
			}
		}

		private TypeHandle __TypeHandle;

		private EntityQuery __query_848063814_0;

		public void OnUpdate(ref SystemState state)
		{
			foreach (var (dynamicBuffer3, dynamicBuffer4, entity2) in IFE_848063814_0.Query(__query_848063814_0, __TypeHandle.__IFE_848063814_0_TypeHandle, ref state))
			{
				if (!(Manager.memory.GetEntityMono(entity2) is Aquarium aquarium))
				{
					continue;
				}
				for (int i = 0; i < dynamicBuffer3.Length; i++)
				{
					ContainedMiniSimElementVisualBuffer containedMiniSimElementVisualBuffer = dynamicBuffer3[i];
					aquarium.UpdateSimulationPosition(i, containedMiniSimElementVisualBuffer.elementVisual.position);
					ref ContainedMiniSimElementLastVisualBuffer reference = ref dynamicBuffer4.ElementAt(i);
					if (containedMiniSimElementVisualBuffer.elementVisual.animationCounter > reference.lastAnimationCounter)
					{
						...
						aquarium.PlayAnimationForVisual(i, containedMiniSimElementVisualBuffer.elementVisual.animation, containedMiniSimElementVisualBuffer.elementVisual.orientationHash, containedMiniSimElementVisualBuffer.elementVisual.flipX);
					}
				}
			}
		}
		...
		private void __AssignQueries(ref SystemState state)
		{
			...
			EntityQueryBuilder entityQueryBuilder2 = entityQueryBuilder.WithAll<AquariumTagCD>();
			...
			__query_848063814_0 = entityQueryBuilder2.Build(ref state);
		...
		{
			((UpdateAquariumFishSystem*)self.ToPointer())->OnUpdate(ref *(SystemState*)state.ToPointer());
		}

		[Unity.Entities.MonoPInvokeCallback(typeof(SystemBaseDelegates.Function))]
		internal unsafe static void __codegen__OnCreateForCompiler(IntPtr self, IntPtr state)
		{
			((UpdateAquariumFishSystem*)self.ToPointer())->OnCreateForCompiler(ref *(SystemState*)state.ToPointer());
		}
	}
}
