...
namespace I2.Loc
{
	...
	public class LanguageSourceData
	{
		...
		public string Export_CSV(string Category, char Separator = ',', bool specializationsAsRows = true, bool skipDisabled = false)
		{
			StringBuilder stringBuilder = new StringBuilder();
			int count = mLanguages.Count;
			...
			{
				LanguageData languageData = mLanguages[i];
				if (!skipDisabled || languageData.IsEnabled())
				{
					array[i] = true;
					...
					AppendString(stringBuilder, GoogleLanguages.GetCodedLanguage(languageData.Name, languageData.Code), Separator);
				}
			}
			stringBuilder.Append("\n");
			mTerms.Sort((TermData a, TermData b) => string.CompareOrdinal(a.Term, b.Term));
			foreach (TermData mTerm in mTerms)
			{
				...
				if (specializationsAsRows)
				{
					...
					{
						AppendTerm(stringBuilder, count, term, mTerm, allSpecialization, Separator, array);
					}
				}
				else
				{
					AppendTerm(stringBuilder, count, term, mTerm, null, Separator, array);
				}
			}
			return stringBuilder.ToString();
		}

		private static void AppendTerm(StringBuilder Builder, int nLanguages, string Term, TermData termData, string specialization, char Separator, bool[] shouldPrintLanguage)
		{
