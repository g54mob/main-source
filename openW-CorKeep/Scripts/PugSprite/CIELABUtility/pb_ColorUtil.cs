...
namespace CIELABUtility
{
	public static class pb_ColorUtil
	{
		...
		public static float DeltaE(pb_XYZ_Color lhs, pb_XYZ_Color rhs)
		{
			return Mathf.Sqrt(Mathf.Pow(lhs.x - rhs.x, 2f) + Mathf.Pow(lhs.y - rhs.y, 2f) + Mathf.Pow(lhs.z - rhs.z, 2f));
		}
		...
		public static pb_HsvColor RGBtoHSV(Color color)
		{
			...
			{
				s = num3 / num2;
				if (!(Mathf.Abs(r - num2) < Mathf.Epsilon))
				{
					num4 = ((!(Mathf.Abs(g - num2) < Mathf.Epsilon)) ? (4f + (r - g) / num3) : (2f + (b - r) / num3));
				}
				else
				{
					num4 = (g - b) / num3;
		...
		{
			float num = 1f / Scale;
			return pb_CIE_Lab_Color.FromXYZ(pb_XYZ_Color.FromRGB(R * num, G * num, B * num));
		}
	}
}
