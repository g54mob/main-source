...
namespace PlayFab.Party
{
	public class PlayFabMultiplayerManager : MonoBehaviour
	{
		...
		public delegate void OnDataMessageReceivedNoCopyHandler(object sender, PlayFabPlayer from, IntPtr buffer, uint bufferSize, PARTY_MESSAGE_RECEIVED_OPTIONS options);
		...
		public enum PartyNetworkConnectionType
		{
			None = -1,
			RelayServer = 0,
			DirectConnection = 1
		...
		private PARTY_ENDPOINT_HANDLE _sideChannelLocalEndPointHandle;
		...
		private PARTY_SEND_MESSAGE_QUEUING_CONFIGURATION _sideChannelQueuingConfiguration;
		...
		private const int _ENDPOINTS_PER_DEVICE_COUNT = 2;
		...
		private static List<Action> Errors = new List<Action>();

		private PARTY_ENDPOINT_STATISTIC[] _statisticTypes = new PARTY_ENDPOINT_STATISTIC[1] { PARTY_ENDPOINT_STATISTIC.PARTY_ENDPOINT_STATISTIC_AVERAGE_DEVICE_ROUND_TRIP_LATENCY_IN_MILLISECONDS };

		private ulong[] _statisticValues;

		private PARTY_ENDPOINT_HANDLE[] _endpoints = new PARTY_ENDPOINT_HANDLE[1];
		...
		public event Action<string> OnEndpointDestroyed;

		public event Action<string> OnNetworkDestroyed;
		...
		public void SendDataMessage(IntPtr buffer, uint bufferSize, IEnumerable<PlayFabPlayer> recipients, DeliveryOption deliveryOption, bool isSideChannel = false)
		{
			_SendDataMessage(buffer, bufferSize, recipients, deliveryOption, isSideChannel);
		}
		...
		public void KickPlayer(string entityId)
		{
			uint num = SDK.PartyNetworkKickUser(_networkHandle, entityId, null);
		...
		internal static void _LogError(uint code, PlayFabMultiplayerManagerErrorType type)
		{
			lock (Errors)
			{
				Errors.Add(delegate
				{
					...
					{
						_LogError($"{code} - {errorMessage}");
					}
					SingletonMonoBehaviour<PlayFabEventTracer>.instance.OnPlayFabPartyError(code, type);
				});
			}
		}
		...
		private PARTY_ENDPOINT_HANDLE[] EndPointHandlesFromPlayFabPlayerListNoGC(IEnumerable<PlayFabPlayer> playerList)
		{
			...
			if (num == 0 || _cachedSendMessageEndpointHandles == null)
			{
		...
		private void InitializeImpl()
		{
			...
			_sideChannelQueuingConfiguration = new PARTY_SEND_MESSAGE_QUEUING_CONFIGURATION
			{
				Priority = Convert.ToSByte((short)(-1)),
				IdentityForCancelFilters = 0u,
				TimeoutInMilliseconds = 0u
			};
		...
		private void CleanUpImpl()
		{
			if (_playFabMultiplayerManagerState > _InternalPlayFabMultiplayerManagerState.NotInitialized)
			{
				...
				_sideChannelQueuingConfiguration = null;
				_remotePlayers = null;
				...
				_sideChannelLocalEndPointHandle = null;
		...
		private void JoinNetworkImplComplete(string networkId)
		{
			...
			{
				_LogError(4163u, PlayFabMultiplayerManagerErrorType.NetworkJoinError);
			}
		}
		...
		private void AuthenticateLocalUserStart()
		{
			...
			PartySucceeded(SDK.PartyNetworkCreateEndpoint(_networkHandle, _localUserHandle, null, null, out _sideChannelLocalEndPointHandle));
		}
		...
		internal void _SendDataMessageToAllPlayers(byte[] buffer, bool isSideChannel = false)
		{
			...
			else if (_playFabMultiplayerManagerState >= _InternalPlayFabMultiplayerManagerState.LocalUserAuthenticated)
			{
				...
				{
					PartySucceeded(SDK.PartyEndpointSendMessage(isSideChannel ? _sideChannelLocalEndPointHandle : _localEndPointHandle, null, _defaultSendOptions, isSideChannel ? _sideChannelQueuingConfiguration : _defaultQueuingConfiguration, buffer));
				}
			}
		}

		internal bool _SendDataMessage(byte[] buffer, IEnumerable<PlayFabPlayer> recipients, DeliveryOption deliveryOption, bool isSideChannel = false)
		{
			...
			return PartySucceeded(SDK.PartyEndpointSendMessage(isSideChannel ? _sideChannelLocalEndPointHandle : _localEndPointHandle, targetEndpoints, options, isSideChannel ? _sideChannelQueuingConfiguration : _defaultQueuingConfiguration, buffer));
		}

		internal void _SendDataMessage(IntPtr buffer, uint bufferSize, IEnumerable<PlayFabPlayer> recipients, DeliveryOption deliveryOption, bool isSideChannel = false)
		{
			...
			PARTY_ENDPOINT_HANDLE[] targetEndpoints = EndPointHandlesFromPlayFabPlayerListNoGC(recipients);
			PARTY_SEND_MESSAGE_OPTIONS options = SendOptionsFromDeliveryOption(deliveryOption);
			PartySucceeded(SDK.PartyEndpointSendMessage(isSideChannel ? _sideChannelLocalEndPointHandle : _localEndPointHandle, targetEndpoints, options, isSideChannel ? _sideChannelQueuingConfiguration : _defaultQueuingConfiguration, buffer, bufferSize));
		}
		...
		{
			PARTY_SEND_MESSAGE_OPTIONS pARTY_SEND_MESSAGE_OPTIONS = (PARTY_SEND_MESSAGE_OPTIONS)(0 | ((deliveryOption != DeliveryOption.BestEffortNonSequential) ? 2 : 0));
		...
		internal void _RaiseDataMessageReceivedEvent(PlayFabPlayer fromPlayer, IntPtr buffer, uint bufferSize, PARTY_MESSAGE_RECEIVED_OPTIONS options)
		{
			...
			{
				this.OnDataMessageNoCopyReceived(this, fromPlayer, buffer, bufferSize, options);
			}
		}
		...
		private void ProcessStateChanges()
		{
			...
			foreach (PARTY_STATE_CHANGE partyStateChange in _partyStateChanges)
			{
				...
				{
					uint errorDetail = ((PARTY_LOCAL_CHAT_AUDIO_INPUT_CHANGED_STATE_CHANGE)partyStateChange).errorDetail;
				...
				{
					uint errorDetail2 = ((PARTY_LOCAL_CHAT_AUDIO_OUTPUT_CHANGED_STATE_CHANGE)partyStateChange).errorDetail;
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_NETWORK_DESTROYED:
				{
					...
					{
						SDK.PartyNetworkGetNetworkDescriptor(pARTY_NETWORK_DESTROYED_STATE_CHANGE.network, out var networkDescriptor);
						this.OnNetworkDestroyed(networkDescriptor.NetworkIdentifier);
					}
					break;
				}
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_ENDPOINT_CREATED:
				{
					...
					PartySucceeded(SDK.PartyEndpointGetEntityId(endpoint2, out entityId4));
					bool isLocal = false;
					...
					PlayFabPlayer playFabPlayer2 = GetPlayerByEntityId(entityId4);
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_ENDPOINT_DESTROYED:
				{
					...
					PartySucceeded(SDK.PartyEndpointGetEntityId(endpoint, out entityId2));
					if (!(entityId2 == _localPlayer.EntityKey.Id) && 0 == 0)
					{
						PlayFabPlayer playerByEntityId = GetPlayerByEntityId(entityId2);
						if (playerByEntityId != null)
						{
							_remotePlayers.Remove(playerByEntityId);
						}
						if (this.OnRemotePlayerLeft != null)
						{
							this.OnRemotePlayerLeft(this, playerByEntityId);
						}
					}
					this.OnEndpointDestroyed?.Invoke(entityId2);
					break;
				}
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_CHAT_CONTROL_CREATED:
					Debug.LogError("Received Party State Change Chat Control Created, even though chat is disabled. Other user might be using outdated game version.");
					break;
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_CHAT_TEXT_RECEIVED:
				{
					...
					PartySucceeded(SDK.PartyChatControlGetEntityId(senderChatControl2, out entityId5));
					PlayFabPlayer playerByEntityId3 = GetPlayerByEntityId(entityId5);
					if (playerByEntityId3 != null)
					{
						...
						_RaiseChatMessageReceivedEvent(playerByEntityId3, message2, ChatMessageType.Text);
					}
					else
					{
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_NETWORK_DESCRIPTOR_CHANGED:
				{
					...
					if (this.OnNetworkChanged != null)
					{
						...
						SDK.PartyNetworkGetNetworkDescriptor(network, out var networkDescriptor2);
						...
						UpdateNetworkId(invitationId, networkDescriptor2);
				...
				case PARTY_STATE_CHANGE_TYPE.PARTY_STATE_CHANGE_TYPE_ENDPOINT_MESSAGE_RECEIVED:
				{
					...
					PartySucceeded(SDK.PartyEndpointGetEntityId(senderEndpoint, out entityId3));
					...
					if (playerByEntityId2 != null)
					{
						...
						if (_platformPolicyProvider != null)
						{
							_platformPolicyProvider.ProcessEndpointMessage(playerByEntityId2, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageBuffer, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageSize, out isInternalMessage);
						}
						...
						{
							_RaiseDataMessageReceivedEvent(playerByEntityId2, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageBuffer, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.messageSize, pARTY_ENDPOINT_MESSAGE_RECEIVED_STATE_CHANGE.options);
						}
					}
					else
					{
		...
		public PartyNetworkConnectionType GetDevicePartyNetworkConnectionTypeByPlayer(PlayFabPlayer player = null)
		{
			if (player == null)
			{
				return GetDevicePartyNetworkConnectionTypeByDeviceHandle(_localDeviceHandle);
			}
			PlayFabPlayer[] playerList = new PlayFabPlayer[1] { player };
			PARTY_ENDPOINT_HANDLE[] array = EndPointHandlesFromPlayFabPlayerListNoGC(playerList);
			if (array.Length != 0 && PartySucceeded(SDK.PartyEndpointGetDevice(array[0], out var device)))
			{
				return GetDevicePartyNetworkConnectionTypeByDeviceHandle(device);
			}
			return PartyNetworkConnectionType.None;
		}

		private PartyNetworkConnectionType GetDevicePartyNetworkConnectionTypeByDeviceHandle(PARTY_DEVICE_HANDLE deviceHandle)
		{
			if (PartySucceeded(SDK.PartyNetworkGetDeviceConnectionType(_networkHandle, deviceHandle, out var deviceConnectionType)))
			{
				return (PartyNetworkConnectionType)deviceConnectionType;
			}
			return PartyNetworkConnectionType.None;
		}

		public int GetEndPointStatistics(PlayFabPlayer targetPlayer, PARTY_ENDPOINT_STATISTIC[] statisticTypes = null)
		{
			if (targetPlayer == null || targetPlayer._endPointHandle == null)
			{
				return -1;
			}
			_endpoints[0] = targetPlayer._endPointHandle;
			SDK.PartyEndpointGetEndpointStatistics(_localEndPointHandle, _endpoints, statisticTypes ?? _statisticTypes, out _statisticValues);
			int num = 0;
			for (int i = 0; i < _statisticValues.Length; i++)
			{
				num += (int)_statisticValues[i];
			}
			return num / _statisticValues.Length;
		}
	}
}
