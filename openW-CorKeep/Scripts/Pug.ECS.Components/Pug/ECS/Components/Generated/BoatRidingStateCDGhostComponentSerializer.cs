...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct BoatRidingStateCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<BoatRidingStateCD>(component), in GhostComponentSerializer.TypeCastReadonly<BoatRidingStateCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in BoatRidingStateCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref BoatRidingStateCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref BoatRidingStateCD component, in BoatRidingStateCD backup)
		{
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<BoatRidingStateCD>(),
					ComponentSize = UnsafeUtility.SizeOf<BoatRidingStateCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 1,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 7239215961602049582uL,
				...
				s_StateInitialized = ComponentSerializationHelper<BoatRidingStateCD, Snapshot, BoatRidingStateCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
