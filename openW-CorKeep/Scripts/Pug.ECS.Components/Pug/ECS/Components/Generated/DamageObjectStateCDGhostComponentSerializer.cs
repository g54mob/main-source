...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct DamageObjectStateCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<DamageObjectStateCD>(component), in GhostComponentSerializer.TypeCastReadonly<DamageObjectStateCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in DamageObjectStateCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref DamageObjectStateCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
			component.position.x = snapshotBefore.position_x;
			component.position.y = snapshotBefore.position_y;
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref DamageObjectStateCD component, in DamageObjectStateCD backup)
		{
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<DamageObjectStateCD>(),
					ComponentSize = UnsafeUtility.SizeOf<DamageObjectStateCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 2,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 18353379974854508156uL,
				...
				s_StateInitialized = ComponentSerializationHelper<DamageObjectStateCD, Snapshot, DamageObjectStateCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
