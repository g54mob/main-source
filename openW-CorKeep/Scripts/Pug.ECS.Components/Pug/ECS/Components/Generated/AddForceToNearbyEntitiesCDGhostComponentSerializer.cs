...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct AddForceToNearbyEntitiesCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<AddForceToNearbyEntitiesCD>(component), in GhostComponentSerializer.TypeCastReadonly<AddForceToNearbyEntitiesCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in AddForceToNearbyEntitiesCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref AddForceToNearbyEntitiesCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
			component.state = (AddForceToNearbyEntitiesCD.State)snapshotBefore.state;
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref AddForceToNearbyEntitiesCD component, in AddForceToNearbyEntitiesCD backup)
		{
		...
		private void CalculateChangeMaskGenerated(in Snapshot snapshot, in Snapshot baseline, [NoAlias] IntPtr changeMaskData, int startOffset)
		{
			...
			num |= (uint)((snapshot.stateTimer_startTick != baseline.stateTimer_startTick) ? 2 : 0);
			num |= (uint)((snapshot.stateTimer_targetTicks != baseline.stateTimer_targetTicks) ? 4 : 0);
			num |= (uint)((snapshot.stateTimer_stopTick != baseline.stateTimer_stopTick) ? 8 : 0);
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<AddForceToNearbyEntitiesCD>(),
					ComponentSize = UnsafeUtility.SizeOf<AddForceToNearbyEntitiesCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 4,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 1562778582758447388uL,
				...
				s_StateInitialized = ComponentSerializationHelper<AddForceToNearbyEntitiesCD, Snapshot, AddForceToNearbyEntitiesCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
