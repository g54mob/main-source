...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct HealOtherEntityStateCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<HealOtherEntityStateCD>(component), in GhostComponentSerializer.TypeCastReadonly<HealOtherEntityStateCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in HealOtherEntityStateCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref HealOtherEntityStateCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref HealOtherEntityStateCD component, in HealOtherEntityStateCD backup)
		{
		...
		{
			uint src = ((snapshot.targetEntity != baseline.targetEntity || snapshot.targetEntitySpawnTick != baseline.targetEntitySpawnTick) ? 1u : 0u);
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<HealOtherEntityStateCD>(),
					ComponentSize = UnsafeUtility.SizeOf<HealOtherEntityStateCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 1,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 11129861896131606578uL,
				...
				s_StateInitialized = ComponentSerializationHelper<HealOtherEntityStateCD, Snapshot, HealOtherEntityStateCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
