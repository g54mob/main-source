...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct WallBossHeadCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<WallBossHeadCD>(component), in GhostComponentSerializer.TypeCastReadonly<WallBossHeadCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in WallBossHeadCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref WallBossHeadCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref WallBossHeadCD component, in WallBossHeadCD backup)
		{
		...
		{
			uint src = ((snapshot.mainEntity != baseline.mainEntity || snapshot.mainEntitySpawnTick != baseline.mainEntitySpawnTick) ? 1u : 0u);
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<WallBossHeadCD>(),
					ComponentSize = UnsafeUtility.SizeOf<WallBossHeadCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 1,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 15993247530930978388uL,
				...
				s_StateInitialized = ComponentSerializationHelper<WallBossHeadCD, Snapshot, WallBossHeadCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
