...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct AffectedBySlipperyMovementConditionCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<AffectedBySlipperyMovementConditionCD>(component), in GhostComponentSerializer.TypeCastReadonly<AffectedBySlipperyMovementConditionCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in AffectedBySlipperyMovementConditionCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref AffectedBySlipperyMovementConditionCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref AffectedBySlipperyMovementConditionCD component, in AffectedBySlipperyMovementConditionCD backup)
		{
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<AffectedBySlipperyMovementConditionCD>(),
					ComponentSize = UnsafeUtility.SizeOf<AffectedBySlipperyMovementConditionCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 1,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.OnlyPredictedClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 12232487728859413436uL,
				...
				s_StateInitialized = ComponentSerializationHelper<AffectedBySlipperyMovementConditionCD, Snapshot, AffectedBySlipperyMovementConditionCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
