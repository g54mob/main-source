...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	internal struct ClientInputDataInputBufferDataReceiveCommandSystem : ISystem
	{
		[BurstCompile]
		private struct ReceiveJob : IJobChunk
		{
			public CommandReceiveSystem<ClientInputDataInputBufferDataSerializer, InputBufferData<ClientInputData>>.ReceiveJobData data;
		...
		private CommandReceiveSystem<ClientInputDataInputBufferDataSerializer, InputBufferData<ClientInputData>> m_CommandRecv;
		...
		public void OnUpdate(ref SystemState state)
		{
			...
			{
				data = m_CommandRecv.InitJobData(ref state)
			};
			state.Dependency = JobChunkExtensions.Schedule(jobData, m_CommandRecv.Query, state.Dependency);
		}
		...
		{
			((ClientInputDataInputBufferDataReceiveCommandSystem*)self.ToPointer())->OnCreate(ref *(SystemState*)state.ToPointer());
		}
		...
		{
			((ClientInputDataInputBufferDataReceiveCommandSystem*)self.ToPointer())->OnUpdate(ref *(SystemState*)state.ToPointer());
		}
	}
}
