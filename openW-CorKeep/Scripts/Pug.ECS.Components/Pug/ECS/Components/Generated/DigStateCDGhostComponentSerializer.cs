...
namespace Pug.ECS.Components.Generated
{
	...
	[BurstCompile]
	public struct DigStateCDGhostComponentSerializer : IGhostSerializer
	{
		...
		{
			CopyToSnapshotGenerated(in serializerState, ref GhostComponentSerializer.TypeCast<Snapshot>(snapshot), in GhostComponentSerializer.TypeCast<DigStateCD>(component));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		public void CopyFromSnapshot(in GhostDeserializerState serializerState, [NoAlias] IntPtr component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, [NoAlias][ReadOnly] IntPtr snapshotBefore, [NoAlias][ReadOnly] IntPtr snapshotAfter)
		{
			CopyFromSnapshotGenerated(in serializerState, ref GhostComponentSerializer.TypeCast<DigStateCD>(component), snapshotInterpolationFactor, snapshotInterpolationFactorRaw, in GhostComponentSerializer.TypeCastReadonly<Snapshot>(snapshotBefore), in GhostComponentSerializer.TypeCastReadonly<Snapshot>(snapshotAfter));
		}
		...
		{
			RestoreFromBackupGenerated(ref GhostComponentSerializer.TypeCast<DigStateCD>(component), in GhostComponentSerializer.TypeCastReadonly<DigStateCD>(backup));
		}

		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyToSnapshotGenerated(in GhostSerializerState serializerState, ref Snapshot snapshot, in DigStateCD component)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void CopyFromSnapshotGenerated(in GhostDeserializerState deserializerState, ref DigStateCD component, float snapshotInterpolationFactor, float snapshotInterpolationFactorRaw, in Snapshot snapshotBefore, in Snapshot snapshotAfter)
		{
		...
		[MethodImpl(MethodImplOptions.AggressiveInlining)]
		private void RestoreFromBackupGenerated(ref DigStateCD component, in DigStateCD backup)
		{
		...
		{
			uint num = ((snapshot.positionToPlaceAt_x != baseline.positionToPlaceAt_x) ? 1u : 0u);
			num |= (uint)((snapshot.positionToPlaceAt_y != baseline.positionToPlaceAt_y) ? 1 : 0);
			num |= (uint)((snapshot.positionToPlaceAt_z != baseline.positionToPlaceAt_z) ? 1 : 0);
			num |= (uint)((snapshot.placeDuration_startTick != baseline.placeDuration_startTick) ? 2 : 0);
			num |= (uint)((snapshot.placeDuration_targetTicks != baseline.placeDuration_targetTicks) ? 4 : 0);
			num |= (uint)((snapshot.placeDuration_stopTick != baseline.placeDuration_stopTick) ? 8 : 0);
		...
		internal static GhostComponentSerializer.State GetState(ref SystemState state)
		{
			if (!s_StateInitialized)
			{
				s_State = new GhostComponentSerializer.State
				{
					...
					ComponentType = ComponentType.ReadWrite<DigStateCD>(),
					ComponentSize = UnsafeUtility.SizeOf<DigStateCD>(),
					SnapshotSize = UnsafeUtility.SizeOf<Snapshot>(),
					ChangeMaskBits = 4,
					PrefabType = GhostPrefabType.All,
					SendMask = GhostSendType.AllClients,
					SendToOwner = SendToOwnerType.All,
					VariantHash = 12655483319173172252uL,
				...
				s_StateInitialized = ComponentSerializationHelper<DigStateCD, Snapshot, DigStateCDGhostComponentSerializer>.SetupFunctionPointers(ref s_State, ref state);
			}
			return s_State;
		}
