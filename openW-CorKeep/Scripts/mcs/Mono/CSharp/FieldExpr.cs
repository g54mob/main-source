...
namespace Mono.CSharp
{
	public class FieldExpr : MemberExpr, IDynamicAssign, IAssignMethod, IMemoryLocation, IVariableReference, IFixedExpression
	{
		...
		public void SetFieldAssigned(FlowAnalysisContext fc)
		{
			...
			if (spec.DeclaringType.IsStruct && InstanceExpression is IVariableReference { VariableInfo: not null } variableReference)
			{
			...
			if (fieldExpr != null)
			{
				...
				while (true)
				{
					...
					if (TypeSpec.IsReferenceType(fieldExpr.Type) && instanceExpression.Type.IsStruct && InstanceExpression is IVariableReference { VariableInfo: null } && (!(instanceExpression is IVariableReference variableReference3) || (variableReference3.VariableInfo != null && !fc.IsDefinitelyAssigned(variableReference3.VariableInfo))))
					{
		...
		{
			if (InstanceExpression is IVariableReference { VariableInfo: var variableInfo })
			{
