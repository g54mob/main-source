...
namespace Mono.CSharp
{
	public class Foreach : LoopStatement
	{
		...
		private sealed class CollectionForeach : IteratorStatement, OverloadResolver.IErrorHandler
		{
			...
			{
				if (!(MemberCache.FindMember(enumerator.ReturnType, MemberFilter.Method("MoveNext", 0, ParametersCompiled.EmptyReadOnlyParameters, rc.BuiltinTypes.Bool), BindingRestriction.InstanceOnly) is MethodSpec { IsPublic: not false } methodSpec))
				{
			...
			{
				if (!(MemberCache.FindMember(enumerator.ReturnType, MemberFilter.Property("Current", null), BindingRestriction.InstanceOnly) is PropertySpec { IsPublic: not false } propertySpec))
				{
					Error_WrongEnumerator(rc, enumerator);
					return null;
				}
