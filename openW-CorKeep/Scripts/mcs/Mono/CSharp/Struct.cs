...
namespace Mono.CSharp
{
	public sealed class Struct : ClassOrStruct
	{
		...
		{
			if (!(ts.MemberDefinition is Struct obj))
			{
				return true;
			}
			return obj.CheckStructCycles();
		}
		...
		private bool HasUserDefaultConstructor()
		{
			...
			{
				if (member is Constructor { IsStatic: false } constructor && constructor.ParameterInfo.IsEmpty)
				{
					return true;
				}
			}
		...
		public override bool IsUnmanagedType()
		{
			...
			if (first_nonstatic_field != null)
			{
				...
				foreach (MemberCore member in base.Members)
				{
					if (member is Field { IsStatic: false, MemberType: var memberType })
					{
						if (memberType == null)
						{
							return true;
