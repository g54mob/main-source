	...
	public class Tokenizer : yyInput
	{
		...
		public int xtoken()
		{
			...
			while ((num = get_char()) != -1)
			{
				...
				case 57:
					tokens_seen = true;
					return is_number(num, dotLead: false);
				case 10:
				case 8232:
				case 8233:
					any_token_seen |= tokens_seen;
				...
				case 46:
				{
					...
					if (num2 >= 48 && num2 <= 57)
					{
						return is_number(num, dotLead: true);
					}
					return 377;
				}
				case 35:
				{
					...
					while ((num = get_char()) != -1)
					{
						...
						{
							if (num == 35 && ParsePreprocessingDirective(caller_is_taking: false))
							{
								break;
							}
							flag3 = false;
				...
				case 64:
					...
					{
						return consume_identifier(num, quoted: true);
					}
		...
		private int TokenizeBackslash()
		{
			...
			default:
			{
				num = escape(num, out var surrogate);
				if (num == -1)
				{
					return 259;
				}
		...
		private void update_formatted_doc_comment(int current_comment_start)
		{
			...
			string[] array = xml_comment_buffer.ToString(current_comment_start, num).Replace("\r", "").Split(new char[1] { '\n' });
