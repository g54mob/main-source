	...
	public abstract class AssemblyDefinition : IAssemblyDefinition
	{
		...
		public void ApplyAttributeBuilder(Attribute a, MethodSpec ctor, byte[] cdata, PredefinedAttributes pa)
		{
			...
			if (a.Type == pa.AssemblyVersion)
			{
				...
				{
					Version version = IsValidAssemblyVersion(text2, allowGenerated: true);
					if (version == null)
					{
		...
		private void CheckReferencesPublicToken()
		{
			...
			{
				if (!(assembly is ImportedAssemblyDefinition { IsMissing: false } importedAssemblyDefinition))
				{
					continue;
				}
		...
		public void Resolve()
		{
			if (Compiler.Settings.Unsafe && module.PredefinedTypes.SecurityAction.Define())
			{
				...
				arguments2.Add(new NamedArgument("SkipVerification", location, new BoolLiteral(Compiler.BuiltinTypes, val: true, location)));
				Attribute attribute = new Attribute("assembly", new MemberAccess(expr, "SecurityPermissionAttribute"), new Arguments[2] { arguments, arguments2 }, location, nameEscaped: false);
