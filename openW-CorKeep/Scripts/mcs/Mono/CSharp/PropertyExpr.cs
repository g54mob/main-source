...
namespace Mono.CSharp
{
	internal sealed class PropertyExpr : PropertyOrIndexerExpr<PropertySpec>
	{
		...
		public override void EmitAssign(EmitContext ec, Expression source, bool leave_copy, bool isCompound)
		{
			...
			CallEmitter callEmitter = new CallEmitter
			{
		...
		{
			if (best_candidate.MemberDefinition is Property { BackingField: not null } property && InstanceExpression is IVariableReference { VariableInfo: var variableInfo })
			{
		...
		public void SetBackingFieldAssigned(FlowAnalysisContext fc)
		{
			...
			else if (IsAutoPropertyAccess && best_candidate.MemberDefinition is Property { BackingField: not null } property && best_candidate.DeclaringType.IsStruct && InstanceExpression is IVariableReference { VariableInfo: not null } variableReference)
			{
