...
namespace Mono.CSharp
{
	public abstract class TypeDefinition : TypeContainer, ITypeDefinition, IMemberDefinition
	{
		...
		public virtual void AddNameToContainer(MemberCore symbol, string name)
		{
			...
			else if (!symbol.EnableOverloadChecks(value) && !(value is InterfaceMemberBase { IsExplicitImpl: not false }))
			{
		...
		public override void ApplyAttributeBuilder(Attribute a, MethodSpec ctor, byte[] cdata, PredefinedAttributes pa)
		{
			if (a.Target == AttributeTargets.Method)
			{
				...
				{
					if (member is Constructor { IsPrimaryConstructor: not false } constructor)
					{
		...
		private void CheckPairedOperators()
		{
			...
			{
				if (!(members[i] is Operator { OperatorType: var operatorType } obj))
				{
					continue;
				}
				...
				{
					if (members[j] is Operator obj2 && obj2.OperatorType == matchingOperator && TypeSpecComparer.IsEqual(obj.ReturnType, obj2.ReturnType) && TypeSpecComparer.Equals(obj.ParameterTypes, obj2.ParameterTypes))
					{
		...
		private TypeSpec CheckRecursiveDefinition(TypeDefinition tc)
		{
			...
			if (iface_exprs != null)
			{
				...
				foreach (TypeSpec typeSpec in array)
				{
					if (typeSpec != null && typeSpec.MemberDefinition is Interface obj && obj.CheckRecursiveDefinition(this) != null)
					{
						return typeSpec;
					}
				}
		...
		protected virtual bool DoDefineMembers()
		{
			if (iface_exprs != null)
			{
				...
				foreach (TypeSpec typeSpec in array)
				{
					...
					if (typeSpec.MemberDefinition is Interface obj)
					{
						obj.Define();
					}
			...
			{
				if (members[k] is InterfaceMemberBase { IsExplicitImpl: not false } interfaceMemberBase)
				{
					try
					{
			...
			{
				if (!(members[l] is InterfaceMemberBase { IsExplicitImpl: not false }) && !(members[l] is TypeContainer))
				{
					try
					{
						members[l].Define();
			...
			if (requires_delayed_unmanagedtype_check)
			{
				...
				foreach (MemberCore member in members)
				{
					if (member is Field { MemberType: not null } field && field.MemberType.IsPointer)
					{
		...
		private void CheckAttributeClsCompliance()
		{
			...
			foreach (MemberCore member in members)
			{
				if (member is Constructor { HasCompliantArgs: not false })
				{
					return;
				}
			}
