...
namespace Rewired.InputManagers
{
	...
	internal class Initializer : PlatformInitializer
	{
		...
		private static PlatformInitializer IlKdkUZpqSHxFmCLZlDOvojFTXvS;

		public static PlatformInitializer GetPlatformInitializer()
		{
			if (IlKdkUZpqSHxFmCLZlDOvojFTXvS == null)
			{
				IlKdkUZpqSHxFmCLZlDOvojFTXvS = new Initializer();
			}
			return IlKdkUZpqSHxFmCLZlDOvojFTXvS;
		}
		...
		public override object Initialize(IConfigVars_Internal configVars)
		{
			if (UnityTools.platform == Platform.Windows || UnityTools.platform == Platform.WindowsAppStore)
			{
				...
				if (UnityTools.platform == Platform.Windows && configVars2.windowsStandalonePrimaryInputSource == WindowsStandalonePrimaryInputSource.SDL2)
				{
					try
					{
						if (new GpcAJitaieCxmUDitndJTHAqjiuH(configVars2, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId, true, false, false) == null)
						{
							throw new Exception();
						}
					}
				...
				try
				{
					return new CeXACVKYpvtGjhauNgXgTdoRydxn(configVars2, ReInput.GetHardwareJoystickMap_InputManager, ReInput.GetNewJoystickId);
				}
				catch (Exception)
				{
		...
		public override IElementIdentifierTool CreateTool(string inputSourceString)
		{
			...
			{
				return new zzRRrbuQaUGboReIDmRemUJRstYN();
			}
			if (inputSourceString == "RawInput")
			{
				return new JvtgYJebOAQHThEbNqDPmCoYMrlR();
			}
			return null;
		}
	}
