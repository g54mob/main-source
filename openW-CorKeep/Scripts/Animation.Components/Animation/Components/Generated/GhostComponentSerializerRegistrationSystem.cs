...
namespace Animation.Components.Generated
{
	...
	public struct GhostComponentSerializerRegistrationSystem : ISystem, IGhostComponentSerializerRegistration
	{
		...
		internal delegate void __codegen__OnUpdate_00000045_0024PostfixBurstDelegate(IntPtr self, IntPtr state);

		internal static class __codegen__OnUpdate_00000045_0024BurstDirectCall
		{
			...
			private static void GetFunctionPointerDiscard(ref IntPtr P_0)
			{
				if (Pointer == (IntPtr)0)
				{
					Pointer = BurstCompiler.CompileFunctionPointer<__codegen__OnUpdate_00000045_0024PostfixBurstDelegate>(__codegen__OnUpdate).Value;
				}
				P_0 = Pointer;
			}
		...
		public void OnCreate(ref SystemState state)
		{
			...
			{
				DisplayName = "AnimationBuffer",
				Component = ComponentType.ReadWrite<AnimationBuffer>(),
				Hash = 10498749032209275228uL,
				SelfIndex = -1,
				SerializerIndex = -1,
			...
			componentTypeSerializationStrategy = new ComponentTypeSerializationStrategy
			{
				DisplayName = "AnimationBufferPointer",
				Component = ComponentType.ReadWrite<AnimationBufferPointer>(),
				Hash = 8546127944632662382uL,
				SelfIndex = -1,
				SerializerIndex = -1,
				PrefabType = GhostPrefabType.All,
				SendTypeOptimization = GhostSendType.AllClients,
			...
			componentTypeSerializationStrategy = new ComponentTypeSerializationStrategy
			{
				DisplayName = "LocalAnimationBuffer",
				Component = ComponentType.ReadWrite<LocalAnimationBuffer>(),
				Hash = 153983586607298010uL,
				SelfIndex = -1,
				SerializerIndex = -1,
			...
			componentTypeSerializationStrategy = new ComponentTypeSerializationStrategy
			{
				DisplayName = "LocalAnimationBufferPointer",
				Component = ComponentType.ReadWrite<LocalAnimationBufferPointer>(),
				Hash = 12874524162375643580uL,
				SelfIndex = -1,
				SerializerIndex = -1,
				PrefabType = GhostPrefabType.Client,
				SendTypeOptimization = GhostSendType.AllClients,
			...
			valueRW.AddSerializationStrategy(ref componentTypeSerializationStrategy);
			valueRW.AddSerializer(AnimationBufferGhostComponentSerializer.GetState(ref state));
			valueRW.AddSerializer(AnimationBufferPointerGhostComponentSerializer.GetState(ref state));
		}
		...
		{
			__codegen__OnUpdate_00000045_0024BurstDirectCall.Invoke(self, state);
		}
