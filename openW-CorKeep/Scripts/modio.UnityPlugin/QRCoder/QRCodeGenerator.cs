...
namespace QRCoder
{
	public class QRCodeGenerator : IDisposable
	{
		...
		public static QRCodeData GenerateQrCode(string plainText, ECCLevel eccLevel, bool forceUtf8 = false, bool utf8BOM = false, EciMode eciMode = EciMode.Default, int requestedVersion = -1)
		{
			...
			if (num == -1)
			{
				num = GetVersion(dataLength + ((eciMode != EciMode.Default) ? 2 : 0), encodingFromPlaintext, eccLevel);
			}
			else if (GetVersion(dataLength + ((eciMode != EciMode.Default) ? 2 : 0), encodingFromPlaintext, eccLevel) > num)
			{
		...
		private static QRCodeData GenerateQrCode(string bitString, ECCLevel eccLevel, int version)
		{
			...
			for (int num3 = 0; num3 < eccInfo.BlocksInGroup1; num3++)
			{
				string bitString2 = bitString.Substring(num3 * eccInfo.CodewordsInGroup1 * 8, eccInfo.CodewordsInGroup1 * 8);
				...
				list.Add(new CodewordBlock(1, num3 + 1, bitString2, list2, list3, codeWordsInt, eccWordsInt));
			}
			bitString = bitString.Substring(eccInfo.BlocksInGroup1 * eccInfo.CodewordsInGroup1 * 8);
			for (int num4 = 0; num4 < eccInfo.BlocksInGroup2; num4++)
			{
				string bitString3 = bitString.Substring(num4 * eccInfo.CodewordsInGroup2 * 8, eccInfo.CodewordsInGroup2 * 8);
				...
				list.Add(new CodewordBlock(2, num4 + 1, bitString3, list4, list5, codeWordsInt2, eccWordsInt2));
			}
			StringBuilder stringBuilder = new StringBuilder();
			for (int num5 = 0; num5 < Math.Max(eccInfo.CodewordsInGroup1, eccInfo.CodewordsInGroup2); num5++)
			{
				foreach (CodewordBlock item in list)
				{
					if (item.CodeWords.Count > num5)
					{
						stringBuilder.Append(item.CodeWords[num5]);
					}
				}
			}
			for (int num6 = 0; num6 < eccInfo.ECCPerBlock; num6++)
			{
				foreach (CodewordBlock item2 in list)
				{
					if (item2.ECCWords.Count > num6)
					{
						stringBuilder.Append(item2.ECCWords[num6]);
					}
				}
			}
		...
		private static string ConvertToIso8859(string value, string Iso = "ISO-8859-2")
		{
			...
			byte[] array = Encoding.Convert(uTF, encoding, bytes);
		...
		private static Polynom MultiplyAlphaPolynoms(Polynom polynomBase, Polynom polynomMultiplier)
		{
			...
			{
				int intVal = polynom.PolyItems.Where((PolynomItem x) => x.Exponent == exponent).Aggregate(0, (int num, PolynomItem polynomOld) => num ^ GetIntValFromAlphaExp(polynomOld.Coefficient));
