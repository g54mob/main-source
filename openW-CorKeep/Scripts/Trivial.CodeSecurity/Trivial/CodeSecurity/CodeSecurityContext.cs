...
namespace Trivial.CodeSecurity
{
	internal class CodeSecurityContext
	{
		...
		public bool IsNamespaceReferenceAllowedCached(TypeReference potentialIllegalNamespace)
		{
			if (string.IsNullOrEmpty(potentialIllegalNamespace.Namespace))
			{
				return true;
			}
			bool value = false;
			if (namespaceReferenceCache.TryGetValue(potentialIllegalNamespace.Namespace, out value))
			{
				return value;
			}
			value = validator.IsNamespaceReferenceAllowed(potentialIllegalNamespace);
			namespaceReferenceCache.Add(potentialIllegalNamespace.Namespace, value);
			return value;
		}
