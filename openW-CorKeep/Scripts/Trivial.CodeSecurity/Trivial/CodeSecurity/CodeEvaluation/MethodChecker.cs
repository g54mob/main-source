...
namespace Trivial.CodeSecurity.CodeEvaluation
{
	internal class MethodChecker : CodeChecker<MethodDefinition>, ICodeUsageProvider
	{
		...
		public override void SecurityCheckCode(CodeSecurityContext context, MethodDefinition methodDefinition)
		{
			...
			methodReferenceChecker.SecurityCheckCodeMethodReturn(context, methodDefinition.ReturnType, methodDefinition);
			if (methodDefinition.HasParameters)
			{
				methodReferenceChecker.SecurityCheckCodeMethodParameters(context, methodDefinition.Parameters, methodDefinition);
			}
			if (methodDefinition.HasBody)
			{
				SecurityCheckCodeMethodBody(context, methodDefinition.Body);
			}
			if (methodDefinition.HasPInvokeInfo && !context.Validator.AllowPInvoke)
			{
				context.Report.AddIllegalPInvokeReferenceOccurence(methodDefinition.PInvokeInfo.EntryPoint, methodDefinition.PInvokeInfo.Module.Name, new IllegalReferenceUsage(methodDefinition));
			}
		}
