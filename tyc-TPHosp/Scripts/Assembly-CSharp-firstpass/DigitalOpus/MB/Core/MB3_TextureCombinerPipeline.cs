		...
		internal static bool _CollectPropertyNames(TexturePipelineData data, MB2_LogLevel LOG_LEVEL)
		{
			...
			for (int num = 0; num < shaderTexPropertyNames.Length; num++)
			{
				if (resultMaterial.HasProperty(shaderTexPropertyNames[num].name))
				{
					...
					if (!data.texPropertyNames.Contains(shaderTexPropertyNames[num]))
					{
						data.texPropertyNames.Add(shaderTexPropertyNames[num]);
					}
			...
			for (int num2 = 0; num2 < data._customShaderPropNames.Count; num2++)
			{
				if (resultMaterial.HasProperty(data._customShaderPropNames[num2].name))
				{
					text = text + ", " + data._customShaderPropNames[num2].name;
					data.texPropertyNames.Add(data._customShaderPropNames[num2]);
					if (resultMaterial.GetTextureOffset(data._customShaderPropNames[num2].name) != new Vector2(0f, 0f) && LOG_LEVEL >= MB2_LogLevel.warn)
					{
						UnityEngine.Debug.LogWarning("Result material has non-zero offset. This is probably incorrect.");
					}
					if (resultMaterial.GetTextureScale(data._customShaderPropNames[num2].name) != new Vector2(1f, 1f) && LOG_LEVEL >= MB2_LogLevel.warn)
					{
				...
				{
					UnityEngine.Debug.LogWarning("Result material shader does not use property " + data._customShaderPropNames[num2].name + " in the list of custom shader property names");
				}
			}
			return true;
		}
						...
						if (material.HasProperty(data.texPropertyNames[l].name))
						{
							...
							if (textureConsideringStandardShaderKeywords != null)
							{
								...
								if ((format != TextureFormat.ARGB32 && format != TextureFormat.RGBA32 && format != TextureFormat.BGRA32 && format != TextureFormat.RGB24 && format != TextureFormat.Alpha8) || flag2)
								{
									...
									{
										UnityEngine.Debug.LogWarning("Object " + gameObject.name + " in the list of objects to mesh uses Texture " + texture.name + " uses format " + format.ToString() + " that is not in: ARGB32, RGBA32, BGRA32, RGB24, Alpha8 or DXT. These textures cannot be resized at runtime. Try changing texture format. If format says 'compressed' try changing it to 'truecolor'");
										result.success = false;
										yield break;
									}
