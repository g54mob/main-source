		...
		public override bool UpdateGameObjects(GameObject[] gos, bool recalcBounds = true, bool updateVertices = true, bool updateNormals = true, bool updateTangents = true, bool updateUV = false, bool updateUV2 = false, bool updateUV3 = false, bool updateUV4 = false, bool updateColors = false, bool updateSkinningInfo = false)
		{
			...
			for (int k = 0; k < meshCombiners.Count; k++)
			{
				if (meshCombiners[k].gosToUpdate.Count > 0)
				{
					...
					flag = flag && meshCombiners[k].combinedMesh.UpdateGameObjects(gos2, recalcBounds, updateVertices, updateNormals, updateTangents, updateUV, updateUV2, updateUV3, updateUV4, updateColors, updateSkinningInfo);
				}
			}
			return flag;
		}
			...
			if (gos != null)
			{
				for (int i = 0; i < gos.Length; i++)
				{
					...
					for (int j = i + 1; j < gos.Length; j++)
					{
						if (gos[i] == gos[j])
						{
							Debug.LogError("GameObject " + gos[i]?.ToString() + "appears twice in list of game objects to add");
							return false;
						}
					}
					...
					if (!flag)
					{
						Debug.LogError("GameObject " + gos[i]?.ToString() + " is already in the combined mesh " + gos[i].GetInstanceID());
						return false;
					}
				}
			}
