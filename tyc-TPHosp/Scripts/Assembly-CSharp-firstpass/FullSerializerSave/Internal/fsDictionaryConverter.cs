...
namespace FullSerializerSave.Internal
{
	public class fsDictionaryConverter : fsConverter
	{
		...
		public override fsResult TryDeserialize(fsData data, ref object instance_, Type storageType)
		{
			...
			if (data.IsList)
			{
				...
				for (int i = 0; i < asList.Count; i++)
				{
					...
					object result = null;
					object result2 = null;
					if ((success += Serializer.TryDeserialize(subitem, keyStorageType, null, ref result)).Failed)
					{
						return success;
					}
					if ((success += Serializer.TryDeserialize(subitem2, valueStorageType, null, ref result2)).Failed)
					{
						return success;
					}
					if (result == null)
					{
						success.AddMessage("Key is null; not adding item to dictionary");
					}
					else
					{
			...
			else
			{
				...
				foreach (KeyValuePair<string, fsData> item in data.AsDictionary)
				{
					...
					object result3 = null;
					object result4 = null;
					result5 = (success += Serializer.TryDeserialize(data3, keyStorageType, null, ref result3));
					if (result5.Failed)
					{
						result5 = success;
					}
					...
					{
						fsResult fsResult2 = (success += Serializer.TryDeserialize(value, valueStorageType, null, ref result4));
						if (!fsResult2.Failed)
						{
							if (result3 == null)
							{
		...
		public override fsResult TrySerialize(object instance_, out fsData serialized, Type storageType)
		{
			...
			{
				if ((success += Serializer.TrySerialize(keyStorageType, null, enumerator.Key, out var data)).Failed)
				{
					return success;
				}
				if ((success += Serializer.TrySerialize(valueStorageType, null, enumerator.Value, out var data2)).Failed)
				{
					return success;
				}
				list.Add(data);
		...
		private fsResult AddItemToDictionary(IDictionary dictionary, object key, object value)
		{
			if (key == null || value == null)
			{
				...
				if (type == null)
				{
					return fsResult.Fail(dictionary.GetType()?.ToString() + " does not extend ICollection");
				}
