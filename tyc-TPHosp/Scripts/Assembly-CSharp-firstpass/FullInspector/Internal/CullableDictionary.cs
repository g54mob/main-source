	...
	public class CullableDictionary<TKey, TValue, TDictionary> : ICullableDictionary<TKey, TValue> where TDictionary : IDictionary<TKey, TValue>, new()
	{
		...
		public TValue this[TKey key]
		{
			get
			{
				...
				{
					TKey val = key;
					throw new KeyNotFoundException(val?.ToString() ?? "");
				}
				return value;
			}
			set
		...
		public bool TryGetValue(TKey key, out TValue value)
		{
			...
			{
				_culled.Remove(key);
				ref TDictionary primary = ref _primary;
				TValue value2 = value;
				primary.Add(key, value2);
				return true;
			}
		...
		public void EndCullZone()
		{
			...
			if (fiSettings.EmitGraphMetadataCulls && _culled.Count > 0)
			{
				...
				{
					Debug.Log("fiGraphMetadata culling \"" + item.Key?.ToString() + "\"");
				}
			}
			_culled.Clear();
		}
