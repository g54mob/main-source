...
namespace FullSerializer
{
	...
	public class fsMetaType
	{
		...
		private string DebuggerDisplay => $"{ReflectedType.Name}: {Properties.Length} properties";
		...
		private static bool IsAutoProperty(PropertyInfo property, MemberInfo[] members)
		{
			...
			if (getMethod != null)
			{
				return fsPortableReflection.HasAttribute(getMethod, typeof(CompilerGeneratedAttribute), shouldCache: false);
			}
			return false;
		}
		...
		private static bool CanSerializeProperty(fsConfig config, PropertyInfo property, MemberInfo[] members, bool annotationFreeValue)
		{
			...
			if (!config.SerializeNonPublicSetProperties && setMethod == null)
			{
				return false;
			}
