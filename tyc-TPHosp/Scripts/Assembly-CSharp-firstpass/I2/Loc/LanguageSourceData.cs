...
namespace I2.Loc
{
	...
	public class LanguageSourceData
	{
		...
		public string Import_CSV(string Category, List<string[]> CSV, eSpreadsheetUpdateMode UpdateMode = eSpreadsheetUpdateMode.Replace)
		{
			...
			if (array.Length > 1 && ArrayContains(array[0], texts))
			{
				...
				for (int count2 = CSV.Count; k < count2; k++)
				{
					...
					if (termData2 == null)
					{
						if (UpdateMode == eSpreadsheetUpdateMode.UpdateExistingTerms)
						{
							continue;
						}
					...
					for (int m = 0; m < array2.Length && m < array.Length - num; m++)
					{
						if (!string.IsNullOrEmpty(array[m + num]))
						{
							...
							if (num7 >= 0)
							{
								string translation = array[m + num];
								termData2.SetTranslation(num7, translation, text2);
							}
						}
					}
				}
		...
		private void SortTermsList()
		{
			mTerms.Sort((TermData x, TermData y) => string.CompareOrdinal(x.Term, y.Term));
		}
