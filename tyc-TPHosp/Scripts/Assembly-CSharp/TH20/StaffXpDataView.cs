...
namespace TH20
{
	public class StaffXpDataView : IDataViewMode
	{
		...
		public StaffXpDataView(DataViewManager.Config config, VisualManager visualManager, Level level)
		{
		...
		public void Update()
		{
			if (!_config.CharAttributeVisualisations.TryGetValue(CharacterAttributes.Type.XP, out var value))
			{
				return;
			}
			foreach (Staff staffMember in _level.CharacterManager.StaffMembers)
			{
				AttributeFloat attribute = staffMember.GetCharacterAttributes().GetAttribute(CharacterAttributes.Type.XP);
				if (attribute != null && staffMember.RankDefinition != null)
				{
					float time = (Mathf.Approximately(staffMember.RankDefinition.MaximumXP, 0f) ? 1f : (attribute.Value() / staffMember.RankDefinition.MaximumXP));
