...
namespace TH20
{
	public class StaffEnergyDataView : IDataViewMode
	{
		...
		public StaffEnergyDataView(DataViewManager.Config config, VisualManager visualManager, Level level)
		{
		...
		public void Update()
		{
			if (!_config.CharAttributeVisualisations.TryGetValue(CharacterAttributes.Type.Energy, out var value))
			{
				return;
			}
			foreach (Staff staffMember in _level.CharacterManager.StaffMembers)
			{
				CharacterDataView.SetCharacterValueColor(staffMember, CharacterAttributes.Type.Energy, value);
			}
		}
	}
}
