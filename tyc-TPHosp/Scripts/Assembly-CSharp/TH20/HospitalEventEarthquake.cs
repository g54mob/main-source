...
namespace TH20
{
	public class HospitalEventEarthquake : HospitalEvent
	{
		public new class Config : HospitalEvent.Config
		{
			...
			{
				if (objective is ChallengeEarthquake)
				{
					_level.HospitalEventLog.AddEvent(new HospitalEventEarthquake
					{
						_config = this,
						Date = _level.TimelineManager.CurrentGameDate,
						_eventName = objective.Definition.NameLocalised
					});
				}
			}
		}

		private LocalisedString _eventName;
		...
		{
			if (!_eventName.IsNull())
			{
				return _eventName.Translation;
			}
			return string.Empty;
		}
	}
