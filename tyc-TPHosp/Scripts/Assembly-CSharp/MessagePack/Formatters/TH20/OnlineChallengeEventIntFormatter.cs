...
{
	public sealed class OnlineChallengeEventIntFormatter : IMessagePackFormatter<OnlineChallengeEventInt>, IMessagePackFormatter
	{
		private readonly AutomataDictionary ____keyMapping;

		private readonly byte[][] ____stringByteKeys;

		public OnlineChallengeEventIntFormatter()
		{
		...
		public int Serialize(ref byte[] bytes, int offset, OnlineChallengeEventInt value, IFormatterResolver formatterResolver)
		{
			if (value == null)
			{
			...
			offset += MessagePackBinary.WriteInt32(ref bytes, offset, value.Data);
		...
		public OnlineChallengeEventInt Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				else
				{
					...
					{
					case 0:
						data = MessagePackBinary.ReadInt32(bytes, offset, out readSize);
						break;
					case 1:
