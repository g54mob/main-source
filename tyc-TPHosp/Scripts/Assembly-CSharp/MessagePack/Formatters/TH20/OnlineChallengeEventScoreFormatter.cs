...
{
	public sealed class OnlineChallengeEventScoreFormatter : IMessagePackFormatter<OnlineChallengeEventScore>, IMessagePackFormatter
	{
		private readonly AutomataDictionary ____keyMapping;

		private readonly byte[][] ____stringByteKeys;

		public OnlineChallengeEventScoreFormatter()
		{
		...
		public int Serialize(ref byte[] bytes, int offset, OnlineChallengeEventScore value, IFormatterResolver formatterResolver)
		{
			if (value == null)
			{
		...
		public OnlineChallengeEventScore Deserialize(byte[] bytes, int offset, IFormatterResolver formatterResolver, out int readSize)
		{
			...
			for (int i = 0; i < num2; i++)
			{
				...
				else
				{
					...
					{
					case 0:
						score = MessagePackBinary.ReadInt32(bytes, offset, out readSize);
						break;
					case 1:
