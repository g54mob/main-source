...
namespace Rewired
{
	...
	{
		internal class PseBURjmDgdQyBrNSFfUTuoWirpM
		{
			public bool LlgfVabWLxPZFgsjVvjcWTGeerxs;

			public bool eSIoOMjexsbBSirUzgHXOqCZGeH;

			public OvIEhpkUuYDGzFeekgFXDXgYKTqC[] HfsifiJbuMlcqPAumVCdmQNaoBY;

			public PseBURjmDgdQyBrNSFfUTuoWirpM(bool enabled, bool loadFromUserDataStore, OvIEhpkUuYDGzFeekgFXDXgYKTqC[] startingRuleSets)
			{
				LlgfVabWLxPZFgsjVvjcWTGeerxs = enabled;
				eSIoOMjexsbBSirUzgHXOqCZGeH = loadFromUserDataStore;
				HfsifiJbuMlcqPAumVCdmQNaoBY = startingRuleSets;
			}
		}
		...
		private bool TLoEgrOtRzdoXBzxJjTaLccSzJIn = true;

		private Player kBpLQLyZZWEsvrnaXVjYtMyWIFB;

		private PseBURjmDgdQyBrNSFfUTuoWirpM iEcBSOYAsKtgABwNtsQrKywyfaW;

		private readonly int fhCkCLBQpxfjvFtQcQZeUtCOKFGZ;

		private List<RuleSet> tZduIyCLhOiEgdmguQYusgirBmj;

		private Action fKOaiMLYzQEYXorwrHqByoabexw;
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return TLoEgrOtRzdoXBzxJjTaLccSzJIn;
			}
			set
			{
				TLoEgrOtRzdoXBzxJjTaLccSzJIn = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return tZduIyCLhOiEgdmguQYusgirBmj;
			}
			set
			{
				if (value == null)
				...
				tZduIyCLhOiEgdmguQYusgirBmj = value;
			}
		}
		...
		internal event Action ApplyCalledEvent
		{
			add
			{
				fKOaiMLYzQEYXorwrHqByoabexw = (Action)Delegate.Combine(fKOaiMLYzQEYXorwrHqByoabexw, value);
			}
			remove
			{
				fKOaiMLYzQEYXorwrHqByoabexw = (Action)Delegate.Remove(fKOaiMLYzQEYXorwrHqByoabexw, value);
			}
		}

		internal ControllerMapLayoutManager(Player player, PseBURjmDgdQyBrNSFfUTuoWirpM startingSettings)
		{
			if (player == null)
			{
			...
			kBpLQLyZZWEsvrnaXVjYtMyWIFB = player;
			iEcBSOYAsKtgABwNtsQrKywyfaW = startingSettings;
		}
		...
		public void Apply()
		{
			...
			fKOaiMLYzQEYXorwrHqByoabexw?.Invoke();
			if (!TAiAzEAcNOkrpYWJEmhYYqnFvpF || tZduIyCLhOiEgdmguQYusgirBmj == null)
			{
				return;
			}
			int count = tZduIyCLhOiEgdmguQYusgirBmj.Count;
			if (count == 0)
			{
				return;
			}
			...
			kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.maps.GetAllMaps(list);
			list2.AddRange(kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = tZduIyCLhOiEgdmguQYusgirBmj[i];
				...
				for (int j = 0; j < count2; j++)
				{
					...
					for (int num = count3 - 1; num >= 0; num--)
					{
						...
						{
							list.RemoveAt(num);
							kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (TLoEgrOtRzdoXBzxJjTaLccSzJIn && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(kBpLQLyZZWEsvrnaXVjYtMyWIFB.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							kBpLQLyZZWEsvrnaXVjYtMyWIFB.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((iEcBSOYAsKtgABwNtsQrKywyfaW != null && iEcBSOYAsKtgABwNtsQrKywyfaW.HfsifiJbuMlcqPAumVCdmQNaoBY != null) ? iEcBSOYAsKtgABwNtsQrKywyfaW.HfsifiJbuMlcqPAumVCdmQNaoBY.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(iEcBSOYAsKtgABwNtsQrKywyfaW.HfsifiJbuMlcqPAumVCdmQNaoBY[i].id);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = iEcBSOYAsKtgABwNtsQrKywyfaW.HfsifiJbuMlcqPAumVCdmQNaoBY[i].startEnabled;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (iEcBSOYAsKtgABwNtsQrKywyfaW != null)
			{
				TAiAzEAcNOkrpYWJEmhYYqnFvpF = iEcBSOYAsKtgABwNtsQrKywyfaW.LlgfVabWLxPZFgsjVvjcWTGeerxs;
				TLoEgrOtRzdoXBzxJjTaLccSzJIn = iEcBSOYAsKtgABwNtsQrKywyfaW.eSIoOMjexsbBSirUzgHXOqCZGeH;
			}
			tZduIyCLhOiEgdmguQYusgirBmj = list;
			Apply();
		}
		...
		private void ZpEgvAefsRlDDfhUwpzFAUZSfaaq(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("loadFromUserDataStore", TLoEgrOtRzdoXBzxJjTaLccSzJIn);
			P_0.Add("ruleSets", tZduIyCLhOiEgdmguQYusgirBmj);
		}

		private bool JYyEPkmZztzXfbEgKghAFieAytO(SerializedObject P_0)
		{
			TAiAzEAcNOkrpYWJEmhYYqnFvpF = false;
			tZduIyCLhOiEgdmguQYusgirBmj = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref TAiAzEAcNOkrpYWJEmhYYqnFvpF);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref TLoEgrOtRzdoXBzxJjTaLccSzJIn);
