...
namespace Rewired
{
	...
	{
		private enum RxhevXBpwUqHGHSaahmtlYqMbuNq
		{
			jGBnkrzuQKcHPLgjxfwiiXxTbfBm = 0,
			QTPiZFmnRsxmyQYmMuIoBQkOtfg = 1,
			zazlBHPRVyrynKCNPmRZyOVuNtp = 2
		}
		...
		[NonSerialized]
		private bool AFqcoBnRBIefJgtkXmqowTPaAaaP;
		...
		[NonSerialized]
		private bool HnEAYhquMsjAVhvljPSSevOzlPFv;
		...
		{
			if ((Application.isPlaying || _userData.ConfigVars.runInEditMode) && (!Application.isPlaying || yNYshUXYQBJZOZGhQOqqytqJOcP) && !SqipAxIcjKKBSnKUcHhsIAAfbiWH && !AFqcoBnRBIefJgtkXmqowTPaAaaP)
			{
				snBkLHuZRXhMApQCjeNEEoQGnFVI();
			}
		}
		...
		{
			SqipAxIcjKKBSnKUcHhsIAAfbiWH = false;
			HnEAYhquMsjAVhvljPSSevOzlPFv = false;
			AFqcoBnRBIefJgtkXmqowTPaAaaP = false;
			try
			{
				if (ReInput.rewiredInputManager == this)
				{
					ReInput.abrusMNmoPOkJcTYBXpUgMeQbqH();
				}
			}
			catch (Exception ex)
			{
				TtSGLDyxdGKIYZIZUKFtmJJKacKf(RxhevXBpwUqHGHSaahmtlYqMbuNq.zazlBHPRVyrynKCNPmRZyOVuNtp, "destruction", ex);
			}
			OnDeinitialized();
		}
		...
		{
			if (!HnEAYhquMsjAVhvljPSSevOzlPFv)
			{
				ReInput.IeShcUVzptWqhZMoWlGsVtKVtAL(isFocused);
				_ = SqipAxIcjKKBSnKUcHhsIAAfbiWH;
			}
		}
		...
		private void Start()
		{
			if ((!UnityTools.isEditor || Application.isPlaying) && SqipAxIcjKKBSnKUcHhsIAAfbiWH && !AFqcoBnRBIefJgtkXmqowTPaAaaP)
			{
				ReInput.PUfBGkQEoKKPRrTrZNGGdNNSToS();
			}
		}
		...
		private void Update()
		{
			if (SqipAxIcjKKBSnKUcHhsIAAfbiWH && !AFqcoBnRBIefJgtkXmqowTPaAaaP && (!UnityTools.isEditor || Application.isPlaying))
			{
		...
		private void LateUpdate()
		{
			...
			try
			{
				ReInput.GOTiAehRqEQcxuoCakrqQgWdDNS();
			}
			catch (Exception ex)
			{
				TtSGLDyxdGKIYZIZUKFtmJJKacKf(RxhevXBpwUqHGHSaahmtlYqMbuNq.QTPiZFmnRsxmyQYmMuIoBQkOtfg, "update (Late Update)", ex);
			}
		}
		...
		{
			if (!SqipAxIcjKKBSnKUcHhsIAAfbiWH || AFqcoBnRBIefJgtkXmqowTPaAaaP)
			{
				return;
			}
			try
			{
				CheckRecompile();
				ReInput.sXOEyENaQpysOtzkdOTtrqodKbs(updateLoopType);
				if ((_userData.ConfigVars.updateLoop & updateLoopSettingBit) != UpdateLoopSetting.None)
				{
					ReInput.QTPiZFmnRsxmyQYmMuIoBQkOtfg(updateLoopType);
				}
			}
			catch (Exception ex)
			{
				TtSGLDyxdGKIYZIZUKFtmJJKacKf(RxhevXBpwUqHGHSaahmtlYqMbuNq.QTPiZFmnRsxmyQYmMuIoBQkOtfg, "update (" + updateLoopType.ToString() + ")", ex);
			}
		}

		internal void TryStartRunInEditMode()
		{
			if (SqipAxIcjKKBSnKUcHhsIAAfbiWH || !Application.isEditor || Application.isPlaying)
			{
				return;
			}
			if (ReInput.isReady)
		...
		private bool snBkLHuZRXhMApQCjeNEEoQGnFVI()
		{
			...
			return SqipAxIcjKKBSnKUcHhsIAAfbiWH;
		}
		...
		private void EJpmrTgGvrhKjJnkpXbomYBpQTQ()
		{
			...
			try
			{
				if (!ZzRDruQMehlbtXZDrsMoBtKOWVy())
				{
					return;
				}
				...
				string text = bJASOUYwYhJydWjAVeYRHbQuKHa();
				UnityTools.EJpmrTgGvrhKjJnkpXbomYBpQTQ(platform, editorPlatform, isEditor, webplayerPlatform, scriptingBackend, scriptingAPILevel, GetExternalTools());
				ReInput.EJpmrTgGvrhKjJnkpXbomYBpQTQ(this, yvZAtTkpGHfYacGujfIUnfoxsxvA, _userData.ConfigVars, _controllerDataFiles, _userData);
			...
			{
				TtSGLDyxdGKIYZIZUKFtmJJKacKf(RxhevXBpwUqHGHSaahmtlYqMbuNq.jGBnkrzuQKcHPLgjxfwiiXxTbfBm, "initialization", ex);
			}
		}

		private object yvZAtTkpGHfYacGujfIUnfoxsxvA(ConfigVars P_0)
		{
			List<Assembly> list = ((UnityTools.unityVersion >= UnityTools.UnityVersion.UNITY_5_0) ? null : oNOpRRDIyYRoGmqtPaeRkzvgpCpk());
			return fGSYDPYatdKEeBAFoRjBvWMFmqp.EJpmrTgGvrhKjJnkpXbomYBpQTQ(sfxmLTvEdsBlscBLZWqbOgzHNxn(), list, P_0);
		}

		private List<Assembly> oNOpRRDIyYRoGmqtPaeRkzvgpCpk()
		{
			List<TextAsset> list = new List<TextAsset>();
			ZGzAIqISQhSZmSVvakkUHmtJAxZz(list, UnityTools.GetCurrentPlatformResourecesDLLPaths());
		...
		private List<Assembly> TQkaFaLIucbdpRqsnXvfaRBGaoT()
		{
			bool flag = false;
			try
			{
				string value = sfxmLTvEdsBlscBLZWqbOgzHNxn();
				...
				Assembly assembly = Array.Find(assemblies, (Assembly P_0) => string.Equals(P_0.GetName().Name, sfxmLTvEdsBlscBLZWqbOgzHNxn(), StringComparison.OrdinalIgnoreCase));
		...
		private byte[] ohJratXqEUGKkEnZiRZsMckjOCg()
		{
			try
			{
				string text = sfxmLTvEdsBlscBLZWqbOgzHNxn();
				if (string.IsNullOrEmpty(text))
				{
					return null;
				}
		...
		private void ZGzAIqISQhSZmSVvakkUHmtJAxZz(List<TextAsset> P_0, List<string> P_1)
		{
		...
		private string bJASOUYwYhJydWjAVeYRHbQuKHa()
		{
			if (editorPlatform == EditorPlatform.None)
			{
				return null;
			}
			if (KWuWIdDJxoLyxWLyBvKKoaoGbNs())
			{
				return null;
			}
		...
		private bool KWuWIdDJxoLyxWLyBvKKoaoGbNs()
		{
			switch (editorPlatform)
			{
			case EditorPlatform.Windows:
		...
		private string sfxmLTvEdsBlscBLZWqbOgzHNxn()
		{
		...
		private bool ZzRDruQMehlbtXZDrsMoBtKOWVy()
		{
			HnEAYhquMsjAVhvljPSSevOzlPFv = false;
			if (ReInput.isReady)
			{
				...
				HnEAYhquMsjAVhvljPSSevOzlPFv = true;
		...
		{
			ReInput.MahycecEhxFKucGZMGavXVWyPta();
			ReInput.abrusMNmoPOkJcTYBXpUgMeQbqH();
		}
		...
		protected void OnSceneLoaded()
		{
			if (ReInput.isReady)
			{
				ReInput.UnVOZrzcgqIQhnDYGcMYPsUSMjU();
			}
		}

		private void TtSGLDyxdGKIYZIZUKFtmJJKacKf(RxhevXBpwUqHGHSaahmtlYqMbuNq P_0, string P_1, Exception P_2)
		{
			P_1 = "Rewired: An exception occurred during " + P_1 + ".";
			bool flag = false;
			if (P_0 == RxhevXBpwUqHGHSaahmtlYqMbuNq.jGBnkrzuQKcHPLgjxfwiiXxTbfBm || P_0 == RxhevXBpwUqHGHSaahmtlYqMbuNq.zazlBHPRVyrynKCNPmRZyOVuNtp)
			{
			...
			if (flag)
			{
				AFqcoBnRBIefJgtkXmqowTPaAaaP = true;
			}
		}
		...
		{
			if (!SqipAxIcjKKBSnKUcHhsIAAfbiWH && !_detectedPlatformInEditor)
			{
				DetectPlatform();
			}
		...
		[CompilerGenerated]
		private bool fzZgQDkinPpIedDFcGssADVyDBtS(Assembly P_0)
		{
			return string.Equals(P_0.GetName().Name, sfxmLTvEdsBlscBLZWqbOgzHNxn(), StringComparison.OrdinalIgnoreCase);
		}
	}
}
