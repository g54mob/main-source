...
namespace Rewired.HID.Drivers
{
	...
	internal abstract class HIDDeviceDriver : IDisposable, IControllerDriver
	{
		...
		public enum DriverType
		{
			...
			KVZsqetvwMHZSngrEETKflyLBGk = 1,
			OuFLtYtcERQnwhaErclIlJcWAlx = 2,
			TqTNLHPoOWpEybrdwmUHLWNYQoN = 3
		}
		...
		protected bool disposed => jgbpvYJovPcfzmcAEJzdxdrBmcm;
		...
		public static HIDDeviceDriver GetDriver(DriverType driverId, InitArgs hidDriverInitArgs)
		{
			...
			switch (driverId)
			{
			case DriverType.KVZsqetvwMHZSngrEETKflyLBGk:
				if (UnityTools.effectivePlatform == Platform.OSX && hidDriverInitArgs.connectionType == DeviceConnectionType.gGXVuYpwdzdIqhuTWerNGskghzz)
				{
					return null;
				}
				return new DualShock4Driver(hidDriverInitArgs);
			case DriverType.OuFLtYtcERQnwhaErclIlJcWAlx:
				return new DualSenseDriver(hidDriverInitArgs);
			case DriverType.TqTNLHPoOWpEybrdwmUHLWNYQoN:
		...
		public static DriverType FindDriverId(int vendorId, int productId)
		{
			...
			{
				return DriverType.KVZsqetvwMHZSngrEETKflyLBGk;
			}
			if (DualSenseDriver.Matches(vendorId, productId))
			{
				return DriverType.OuFLtYtcERQnwhaErclIlJcWAlx;
			}
			if (RailDriverDriver.Matches(vendorId, productId))
			{
				return DriverType.TqTNLHPoOWpEybrdwmUHLWNYQoN;
			}
			return DriverType.DVDMTdEnkAaktJFJqNakDhECjSAS;
		}
