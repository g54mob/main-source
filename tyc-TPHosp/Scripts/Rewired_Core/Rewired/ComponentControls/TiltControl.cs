	...
	public sealed class TiltControl : CustomControllerControl
	{
		...
		internal override void OnoIpIJycGHhdFNrMapnMHSEpPu()
		{
			if (base.initialized && hasController)
			{
				if (_useFAxis)
				{
					npeGbTfGrbHYJDvUuqKBwJdsXleT(_forwardTiltCustomControllerElement, _axis2D.yAxis.value, _axis2D.yAxis.buttonActivationThreshold);
				}
				if (_useHAxis)
				{
					npeGbTfGrbHYJDvUuqKBwJdsXleT(_horizontalTiltCustomControllerElement, _axis2D.xAxis.value, _axis2D.xAxis.buttonActivationThreshold);
				}
			}
		}
		...
		private void okELkTJQSwozdkLZOWVqQNuWldu()
		{
			if (_useHAxis)
			{
				float rawValue;
		...
		private void GKKfUyCSOIfIHsEBISJhdgZDuHx()
		{
			pKeWVrSOISNzSBBNGegMrKsWCRO(_allowedTiltDirections);
			if (hasController)
			{
				if (_useHAxis)
				{
		...
		private void pKeWVrSOISNzSBBNGegMrKsWCRO(TiltDirection P_0)
		{
