...
namespace Rewired.ComponentControls
{
	...
	public abstract class TouchInteractable : TouchControl, IPointerDownHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler, IBeginDragHandler, IDragHandler, IEndDragHandler, IEventSystemHandler
	{
		...
		{
			private TouchInteractable FjaDfEfqGkjNQiWzjiRpRGGMria;

			private InteractionState vzyzyWJzMILGaVcOKuNZMHomdFs;

			private float AciCRpRRcLDeaCsvVGIjBFKEjAPQ;

			public TouchInteractable sender => FjaDfEfqGkjNQiWzjiRpRGGMria;

			public InteractionState state => vzyzyWJzMILGaVcOKuNZMHomdFs;

			public float duration => AciCRpRRcLDeaCsvVGIjBFKEjAPQ;

			internal InteractionStateTransitionArgs()
			{
			}

			internal void vJjhoRLlAcrjzWycVrrFomtsobA(TouchInteractable P_0, InteractionState P_1, float P_2)
			{
				FjaDfEfqGkjNQiWzjiRpRGGMria = P_0;
				vzyzyWJzMILGaVcOKuNZMHomdFs = P_1;
				AciCRpRRcLDeaCsvVGIjBFKEjAPQ = P_2;
			}
		}
		...
		private AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IVisibilityChangedHandler, bool> __hierarchyVisibilityChangedHandlers;

		private AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __hierarchyInteractionStateTransitionHandlers;

		private static AdtOIqFjosNYtDDJzoleKiXwEio.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __interactionStateTransitionHandlerDelegate;

		private AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IVisibilityChangedHandler, bool> hierarchyVisibilityChangedHandlers
		{
			get
			{
				if (__hierarchyVisibilityChangedHandlers == null)
				{
					__hierarchyVisibilityChangedHandlers = new AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IVisibilityChangedHandler, bool>(kRIDpurjMjwwxxsfIWuQHoPXIuoF.visibilityChangedHandlerDelegate);
		...
		private AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> hierarchyInteractionStateTransitionHandlers
		{
			get
			{
				if (__hierarchyInteractionStateTransitionHandlers == null)
				{
					__hierarchyInteractionStateTransitionHandlers = new AdtOIqFjosNYtDDJzoleKiXwEio.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs>(interactionStateTransitionHandlerDelegate);
		...
		internal static AdtOIqFjosNYtDDJzoleKiXwEio.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> interactionStateTransitionHandlerDelegate
		{
			get
			{
		...
		internal override void OnEnable()
		{
			...
			YSvBTCKnmpvKyvghjoSPCAyJuoW(InteractionState.Normal);
		...
		private void OaWKhnzlRQiKpJsyjNUHIvDDTtf(InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			{
				_transitionArgs.vJjhoRLlAcrjzWycVrrFomtsobA(this, P_0, P_1 ? 0f : _transitionColorTint.fadeDuration);
		...
		private void hSGffqRAFgtlAiVgUsapCJJuMnP(bool P_0)
		{
			...
			OaWKhnzlRQiKpJsyjNUHIvDDTtf(interactionState, P_0);
		}
		...
		internal virtual bool GVdTEbCGUhjDluCgHPTnyOZtsgt()
		{
			if (!zCDiilIuMmyrwiYynasIRcHvrxTh())
			{
				return false;
			}
			if (IqanMrGVhLvExACtCdEAMqyHqQT)
			{
		...
		internal void bxcFuVigGCNQnErQwkHMGEIXJZoF(BaseEventData P_0)
		{
			if (zCDiilIuMmyrwiYynasIRcHvrxTh() && IsInteractable())
			{
				InteractionState interactionState = aghTvRpmWvtfqIAHtFAwDXZUrnj(P_0);
				if (interactionState != _interactionState)
				{
					YSvBTCKnmpvKyvghjoSPCAyJuoW(interactionState);
					hSGffqRAFgtlAiVgUsapCJJuMnP(false);
				}
			}
		}

		internal virtual bool DBZNemCCbEjBmnIHgXKlkdDPrhE(GameObject P_0)
		{
			return base.gameObject == P_0;
		}

		private bool JCPzNHQQInDeAlzdFqpuMBunZDZ(BaseEventData P_0)
		{
			bool flag = P_0 is PointerEventData;
			return JCPzNHQQInDeAlzdFqpuMBunZDZ(flag, flag ? (P_0 as PointerEventData).pointerPress : null);
		}

		private bool JCPzNHQQInDeAlzdFqpuMBunZDZ(bool P_0, GameObject P_1)
		{
			if (!zCDiilIuMmyrwiYynasIRcHvrxTh())
			{
				return false;
			}
			if (GVdTEbCGUhjDluCgHPTnyOZtsgt())
			{
				return false;
			}
			bool flag = false;
			if (P_0)
			{
				return flag | ((traAtpHvLXNexVrCRTWuJipMjwd && !IqanMrGVhLvExACtCdEAMqyHqQT && DBZNemCCbEjBmnIHgXKlkdDPrhE(P_1)) || (!traAtpHvLXNexVrCRTWuJipMjwd && IqanMrGVhLvExACtCdEAMqyHqQT && DBZNemCCbEjBmnIHgXKlkdDPrhE(P_1)) || (!traAtpHvLXNexVrCRTWuJipMjwd && IqanMrGVhLvExACtCdEAMqyHqQT && P_1 == null));
			}
			return flag | IqanMrGVhLvExACtCdEAMqyHqQT;
		}

		private InteractionState aghTvRpmWvtfqIAHtFAwDXZUrnj(BaseEventData P_0)
		{
			if (GVdTEbCGUhjDluCgHPTnyOZtsgt())
			{
				return InteractionState.Pressed;
			}
			if (JCPzNHQQInDeAlzdFqpuMBunZDZ(P_0))
			{
		...
		private bool YSvBTCKnmpvKyvghjoSPCAyJuoW(InteractionState P_0)
		{
			...
			JICZtGZHUgECEabXmPwZxVwnhPJW();
			return true;
		}

		private void JICZtGZHUgECEabXmPwZxVwnhPJW()
		{
			FUipiDzqsSVvakiikzlTcQwwtoe();
		}

		private void FUipiDzqsSVvakiikzlTcQwwtoe()
		{
			if (Application.isPlaying && _hideWhenIdle)
			{
				WQilnnANdSjIrSssGhJTkCoyjtc(_interactionState == InteractionState.Pressed, false);
			}
		}
		...
		private void uqzbwBijFXEAXEtQmTGdDqgBOipS()
		{
		...
		{
			if (tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerDown))
			{
				traAtpHvLXNexVrCRTWuJipMjwd = true;
				bxcFuVigGCNQnErQwkHMGEIXJZoF(eventData);
			}
		}

		internal virtual void OnPointerUp(PointerEventData eventData)
		{
			if (tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerUp))
			{
				traAtpHvLXNexVrCRTWuJipMjwd = false;
				bxcFuVigGCNQnErQwkHMGEIXJZoF(eventData);
			}
		...
		{
			if (tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerEnter))
			{
		...
		{
			if (tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerExit))
			{
				IqanMrGVhLvExACtCdEAMqyHqQT = false;
				bxcFuVigGCNQnErQwkHMGEIXJZoF(eventData);
			}
		...
		{
			tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.BeginDrag);
		}

		internal virtual void OnDrag(PointerEventData eventData)
		{
			tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.Drag);
		}

		internal virtual void OnEndDrag(PointerEventData eventData)
		{
			tPJtiKnlJapLvOXtvbEAppvVFMJ(eventData.pointerId, _allowedMouseButtons, EventTriggerType.EndDrag);
		}
		...
		internal static bool FoLGlOgsGoUwoLufhJsVpiSxQYm(int P_0)
		{
			if (P_0 == int.MinValue)
			{
				return false;
			}
			if (EUNPoxeUSzLzbtpjIDKKKWBaQYL(P_0))
			{
				int num = tbfkchKxjqgxvLXCoApqymLwAJi(P_0);
				if (num < 0)
				{
					return false;
				}
			...
			if (FcibJdJVXbMzmpAdzCtyBEgZADhe(P_0) && Input.mousePresent)
			{
				int num2 = oefwXXroGxwauiAVPGHGZJTDHzL(P_0);
				if (num2 >= 0)
				{
		...
		internal static Vector3 iCtSdEdbUllBhkoEsdmIZxcKEoLh(int P_0)
		{
			if (EUNPoxeUSzLzbtpjIDKKKWBaQYL(P_0))
			{
				int num = tbfkchKxjqgxvLXCoApqymLwAJi(P_0);
			...
			else if (FcibJdJVXbMzmpAdzCtyBEgZADhe(P_0) && Input.mousePresent)
			{
		...
		internal static bool EUNPoxeUSzLzbtpjIDKKKWBaQYL(int P_0)
		{
			return P_0 >= 0;
		}

		internal static bool FcibJdJVXbMzmpAdzCtyBEgZADhe(int P_0)
		{
			if (P_0 != -1 && P_0 != -3)
			{
				return P_0 == -2;
		...
		private static int tbfkchKxjqgxvLXCoApqymLwAJi(int P_0)
		{
			if (!EUNPoxeUSzLzbtpjIDKKKWBaQYL(P_0))
			{
				return -1;
		...
		internal static bool tPJtiKnlJapLvOXtvbEAppvVFMJ(MouseButtonFlags P_0, int P_1)
		{
			if (FcibJdJVXbMzmpAdzCtyBEgZADhe(P_1))
			{
				if (!Cursor.visible)
				{
					return false;
			...
			if (EUNPoxeUSzLzbtpjIDKKKWBaQYL(P_1))
			{
				return true;
			}
			if (RrCXfUsxysqYSXoywsFeGqtWpgU(P_0, P_1))
			{
				return true;
			}
			return false;
		...
		private static bool RrCXfUsxysqYSXoywsFeGqtWpgU(MouseButtonFlags P_0, int P_1)
		{
			return P_1 switch
			{
		...
		private static int oefwXXroGxwauiAVPGHGZJTDHzL(int P_0)
		{
			if (!FcibJdJVXbMzmpAdzCtyBEgZADhe(P_0))
			{
				return -1;
			}
			return P_0 switch
		...
		internal static bool qlxzzsOdoUANRhJbnEPLSQzjWeJi(MouseButtonFlags P_0, out int P_1)
		{
			for (int i = 0; i < 3; i++)
			{
		...
		internal static bool tPJtiKnlJapLvOXtvbEAppvVFMJ(int P_0, MouseButtonFlags P_1, EventTriggerType P_2)
		{
			if (FcibJdJVXbMzmpAdzCtyBEgZADhe(P_0) && (P_2 == EventTriggerType.PointerEnter || P_2 == EventTriggerType.PointerExit) && P_1 != MouseButtonFlags.None)
			{
				P_1 |= MouseButtonFlags.LeftButton;
			}
			return tPJtiKnlJapLvOXtvbEAppvVFMJ(P_1, P_0);
		}

		internal static bool mjfqtOtDkxtBHvBGQKSoLmCVGJt(MouseButtonFlags P_0)
		{
			int num;
			return qlxzzsOdoUANRhJbnEPLSQzjWeJi(P_0, out num);
		}

		[CompilerGenerated]
		private void XORPjLdOTkpUSrbNcKqQRCCCjtDd(bool P_0)
		{
			_allowSendingEvents = P_0;
		}

		[CompilerGenerated]
		private static void PgeQGWfHfLZvhcWCJCoThSHsnuu(IInteractionStateTransitionHandler P_0, InteractionStateTransitionArgs P_1)
		{
			P_0.OnInteractionStateTransition(P_1);
		}
	}
