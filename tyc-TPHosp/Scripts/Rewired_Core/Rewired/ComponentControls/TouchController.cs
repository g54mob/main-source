...
namespace Rewired.ComponentControls
{
	...
	public sealed class TouchController : CustomController
	{
		...
		[NonSerialized]
		private bool qMyyqtoNfwMkLGAexpnkjXHPhmU;
		...
		internal override void OnDisable()
		{
			...
			{
				HpHSDXxYkJSuoIxfexlezCKvBWF(true);
			}
		}
		...
		internal override bool USdTaHHNWIGWTOHgBLrxEkaEfPs()
		{
			...
			if (ReInput.isReady)
			{
				qMyyqtoNfwMkLGAexpnkjXHPhmU = ReInput.controllers.Mouse.enabled;
				HpHSDXxYkJSuoIxfexlezCKvBWF(false);
			}
			return true;
		}
		...
		private void HpHSDXxYkJSuoIxfexlezCKvBWF(bool P_0)
		{
			if (_disableMouseInputWhenEnabled)
			{
				if (P_0)
				{
					ReInput.controllers.Mouse.enabled = qMyyqtoNfwMkLGAexpnkjXHPhmU;
				}
				else
				{
