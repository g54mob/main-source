...
namespace Rewired.Data
{
	...
	public sealed class Player_Editor
	{
		...
		public sealed class Mapping
		{
			...
			internal DcnWExKAVFAKzCOeUjNwIgwFSGqO UVxmSxWTWTPFIzLtbZSPYjWbGYt()
			{
				return new DcnWExKAVFAKzCOeUjNwIgwFSGqO(_categoryId, _layoutId, _enabled);
			}
		}
		...
		public sealed class ControllerMapLayoutManagerSettings : IDeepCloneable
		{
			...
			internal ControllerMapLayoutManager.PseBURjmDgdQyBrNSFfUTuoWirpM PrfAiLNTEsMCPZVXnxzgaYuqXXt()
			{
				return new ControllerMapLayoutManager.PseBURjmDgdQyBrNSFfUTuoWirpM(_enabled, _loadFromUserDataStore, EgstVCBwDFtqmpuvaePfkJWuJoR());
			}

			private OvIEhpkUuYDGzFeekgFXDXgYKTqC[] EgstVCBwDFtqmpuvaePfkJWuJoR()
			{
				List<OvIEhpkUuYDGzFeekgFXDXgYKTqC> list = new List<OvIEhpkUuYDGzFeekgFXDXgYKTqC>();
				int num = ((_ruleSets != null) ? _ruleSets.Count : 0);
				for (int i = 0; i < num; i++)
				{
					if (_ruleSets[i] != null)
					{
						list.Add(_ruleSets[i].bUdXkrsUYZmPRGPnUGXWvMXssRA());
					}
				}
				return list.ToArray();
			}
		...
		public sealed class ControllerMapEnablerSettings : IDeepCloneable
		{
			...
			internal ControllerMapEnabler.FIAqEJWjdCiOptKWtsxrOjilkTn PrfAiLNTEsMCPZVXnxzgaYuqXXt()
			{
				return new ControllerMapEnabler.FIAqEJWjdCiOptKWtsxrOjilkTn(_enabled, EgstVCBwDFtqmpuvaePfkJWuJoR());
			}
		...
		public sealed class RuleSetMapping : IDeepCloneable
		{
			...
			internal OvIEhpkUuYDGzFeekgFXDXgYKTqC bUdXkrsUYZmPRGPnUGXWvMXssRA()
			{
				return new OvIEhpkUuYDGzFeekgFXDXgYKTqC(_id, _enabled);
			}
		...
		internal HRJeTaRmGlgEoVaWhsuEDjticiT WypoOZTcWWXaNvnSqMgvRUqplkk()
		{
			DcnWExKAVFAKzCOeUjNwIgwFSGqO[] array = null;
			if (_defaultJoystickMaps != null)
			{
				array = new DcnWExKAVFAKzCOeUjNwIgwFSGqO[_defaultJoystickMaps.Count];
				for (int i = 0; i < _defaultJoystickMaps.Count; i++)
				{
					array[i] = _defaultJoystickMaps[i].UVxmSxWTWTPFIzLtbZSPYjWbGYt();
				}
			}
			DcnWExKAVFAKzCOeUjNwIgwFSGqO[] array2 = null;
			if (_defaultKeyboardMaps != null)
			{
				array2 = new DcnWExKAVFAKzCOeUjNwIgwFSGqO[_defaultKeyboardMaps.Count];
				for (int j = 0; j < _defaultKeyboardMaps.Count; j++)
				{
					array2[j] = _defaultKeyboardMaps[j].UVxmSxWTWTPFIzLtbZSPYjWbGYt();
				}
			}
			DcnWExKAVFAKzCOeUjNwIgwFSGqO[] array3 = null;
			if (_defaultMouseMaps != null)
			{
				array3 = new DcnWExKAVFAKzCOeUjNwIgwFSGqO[_defaultMouseMaps.Count];
				for (int k = 0; k < _defaultMouseMaps.Count; k++)
				{
					array3[k] = _defaultMouseMaps[k].UVxmSxWTWTPFIzLtbZSPYjWbGYt();
				}
			}
			DcnWExKAVFAKzCOeUjNwIgwFSGqO[] array4 = null;
			if (_defaultCustomControllerMaps != null)
			{
				array4 = new DcnWExKAVFAKzCOeUjNwIgwFSGqO[_defaultCustomControllerMaps.Count];
				for (int l = 0; l < _defaultCustomControllerMaps.Count; l++)
				{
					array4[l] = _defaultCustomControllerMaps[l].UVxmSxWTWTPFIzLtbZSPYjWbGYt();
				}
			}
			return new HRJeTaRmGlgEoVaWhsuEDjticiT(array, array2, array3, array4);
		}
	}
}
