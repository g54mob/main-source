...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool ytsxOaXRpfruEEiLupcatGwIsYS()
		{
			...
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = NCPcBMpvtBKVmmBXDdOihvFUeJW();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = vAVXgLXWcExkUHDoPEwSbQMOoEG();
					flag = true;
					break;
				}
			}
		...
		private float NCPcBMpvtBKVmmBXDdOihvFUeJW()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => DdqLKbVKxIQXlprxMcpAfHppbtge(num2), 
				_ => 0f, 
			};
		}

		private float vAVXgLXWcExkUHDoPEwSbQMOoEG()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float DdqLKbVKxIQXlprxMcpAfHppbtge(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
