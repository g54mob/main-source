...
namespace Rewired.Data
{
	public sealed class ControllerDataFiles : ScriptableObject
	{
		...
		[NonSerialized]
		private bool BntgFKYPiFupxXbFoEtpSmPZZaT;
		...
		internal ControllerTemplateElementIdentifier pqcPIenjzQxHCLBREMzpwCtZwAW(Guid P_0, int P_1)
		{
			if (P_1 < 0)
			{
				return null;
			...
			foreach (Guid templateGuid in hardwareJoystickMap.TemplateGuids)
			{
				...
				{
					ControllerTemplateElementIdentifier controllerTemplateElementIdentifier = joystickTemplate.inrqViYxvRaGThobySBFuWrrIeM(P_0, P_1);
		...
		internal HardwareJoystickMap_InputManager LVXijLLBGfgjuiYSfGhIuZvkjhB(Guid P_0, InputSource P_1)
		{
			OXrIQeroLVbMoFcmiWjNWQNcmkoI();
			...
			if (hardwareJoystickMap != null)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = PfQAPRLQLqrrjpTkVoUDNaSImvZ(hardwareJoystickMap, bridgedController2, true, out inputPlatform, out num, out platform);
		...
		internal HardwareJoystickMap_InputManager xMQTcyGiClhHYUNTtvqDTvtysfy(BridgedControllerHWInfo P_0)
		{
			if (P_0 == null)
			{
				return null;
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = CSVmJidMNvEYqMJIlHBIAYMBoEX(P_0);
				if (hardwareJoystickMap_InputManager != null)
				{
					return hardwareJoystickMap_InputManager;
				}
			...
			for (int i = 0; i < hardwareJoystickMaps.Length; i++)
			{
				...
				int num;
				HardwareJoystickMap.Platform platform;
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager2 = PfQAPRLQLqrrjpTkVoUDNaSImvZ(hardwareJoystickMaps[i], P_0, true, out inputPlatform, out num, out platform);
			...
			for (int j = 0; j < hardwareJoystickMaps.Length; j++)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager3 = PfQAPRLQLqrrjpTkVoUDNaSImvZ(hardwareJoystickMaps[j], P_0, false, out inputPlatform2, out num2, out platform2);
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager4 = yxcmLdcQPkdXOIuzscicYVzHDyi(P_0, "[UNITY PRECONFIGURED JOYSTICK]");
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager5 = CSVmJidMNvEYqMJIlHBIAYMBoEX(P_0);
		...
		private HardwareJoystickMap_InputManager PfQAPRLQLqrrjpTkVoUDNaSImvZ(HardwareJoystickMap P_0, BridgedControllerHWInfo P_1, bool P_2, out InputPlatform P_3, out int P_4, out HardwareJoystickMap.Platform P_5)
		{
			P_3 = InputPlatform.eDgdySKclHgXmmILffzdHPvUtEi;
			P_4 = -1;
			P_5 = null;
			if (P_0 == null)
			{
		...
		private HardwareJoystickMap_InputManager yxcmLdcQPkdXOIuzscicYVzHDyi(BridgedControllerHWInfo P_0, string P_1)
		{
		...
		private HardwareJoystickMap_InputManager CSVmJidMNvEYqMJIlHBIAYMBoEX(BridgedControllerHWInfo P_0)
		{
			HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = yxcmLdcQPkdXOIuzscicYVzHDyi(P_0, "[STANDARDIZED GAMEPAD]");
			if (hardwareJoystickMap_InputManager == null)
			{
				return null;
			}
		...
		private void OXrIQeroLVbMoFcmiWjNWQNcmkoI()
		{
			if (!BntgFKYPiFupxXbFoEtpSmPZZaT)
			{
				...
				{
					BntgFKYPiFupxXbFoEtpSmPZZaT = true;
				}
			}
		}
	}
