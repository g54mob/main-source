...
namespace Rewired.Utils
{
	public static class UnityTools
	{
		...
		{
			public enum jpvbWhEvRVLLrLJbzNWWqDvHwozb
			{
				KhCiVcHCNXSJcMUJiQPTjlJRzOn = 0,
				BMYgpXjAJNtTdvFaxMyXYyfEbgO = 1,
				duMVOEvniXzeRtxLytXGThTBaUX = 2
			}
			...
			public readonly jpvbWhEvRVLLrLJbzNWWqDvHwozb type;

			public readonly int build;

			public UnityVersionClass(string versionString)
			{
				type = jpvbWhEvRVLLrLJbzNWWqDvHwozb.KhCiVcHCNXSJcMUJiQPTjlJRzOn;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = jpvbWhEvRVLLrLJbzNWWqDvHwozb.BMYgpXjAJNtTdvFaxMyXYyfEbgO;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = jpvbWhEvRVLLrLJbzNWWqDvHwozb.duMVOEvniXzeRtxLytXGThTBaUX;
					}
			...
			{
				return major + "." + minor + "." + maintenance + dEJpuZYhzdvczzFeKenCvvFBCvT(type) + build;
			}

			private string dEJpuZYhzdvczzFeKenCvvFBCvT(jpvbWhEvRVLLrLJbzNWWqDvHwozb P_0)
			{
				return P_0 switch
				{
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.KhCiVcHCNXSJcMUJiQPTjlJRzOn => "f", 
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.BMYgpXjAJNtTdvFaxMyXYyfEbgO => "b", 
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.duMVOEvniXzeRtxLytXGThTBaUX => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (GYydnjYVPbATXFSzKSjqMIRQSep(a.type) > GYydnjYVPbATXFSzKSjqMIRQSep(b.type))
				{
					return 1;
				}
				if (GYydnjYVPbATXFSzKSjqMIRQSep(a.type) < GYydnjYVPbATXFSzKSjqMIRQSep(b.type))
			...
			private static int GYydnjYVPbATXFSzKSjqMIRQSep(jpvbWhEvRVLLrLJbzNWWqDvHwozb P_0)
			{
				return P_0 switch
				{
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.BMYgpXjAJNtTdvFaxMyXYyfEbgO => 0, 
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.KhCiVcHCNXSJcMUJiQPTjlJRzOn => 10, 
					jpvbWhEvRVLLrLJbzNWWqDvHwozb.duMVOEvniXzeRtxLytXGThTBaUX => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion KfyrGIgyNqiekZXBwwwFmqYcrDl = UnityVersion.UNITY_5_0;

		private static UnityVersionClass xYsGbTgkLfyfDdjZCrfTamMhGtp;

		private static UnityVersion FUFGGnGbrRWXzBsQxlbzgGnFTVQn = UnityVersion.Unknown;

		private static string rqxjMfjDWELuwjtMfEsZTCzRxcH;

		private static Platform eEZxKzIlWDaepZXmTUzEyeGinFm;

		private static EditorPlatform HvUsjbcKcvfjVDepXAAIQHsgOww;

		private static bool vikLxBWqiSQAmEZFRDZCNUqNOXa;

		private static bool FMnadxjVOaUgagvSxyTwkypLtEG;

		private static bool YpezioQrqKiEUyoIWlMliDAJZGy;

		private static WebplayerPlatform nMieJgQejclQhhHENtuZGMmIdmi;

		private static bool QRNmGrcRpXBqWMOcAclvaKnJwGIB;

		private static bool XuhXoukoEkgnECPbsPgsLxbosdrB;

		private static bool FNVQMhMtiVcqjQNyhLTHwebVqsT;

		private static bool FVruvuPJIkxpKnaGlcGvyJtOYdC;

		private static bool SMTWuxtOABCXJGSfyzEReFIqQgD;

		private static bool fMfEIPFaGzVMxVRWtkOmRBWuhBeF;

		private static string HAYUhWkmLGwsgeAkKKjdhrMyETx;

		private static ScriptingBackend PgeFTCHuosgTacWnDOIvQLMIfmJK;

		private static ScriptingAPILevel mFbcqBDIEMiXWfyJfRsiZsPALHun;

		private static IExternalTools tjCHjoFEebgaqOnGSdpCNMaozRg;

		private static bool QsXGxNLRuyQfpdbmkpoUbFCpFqe;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper lxmCstrGMPIbCaksJzmAodKPXPBf;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				...
				return xYsGbTgkLfyfDdjZCrfTamMhGtp;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return rqxjMfjDWELuwjtMfEsZTCzRxcH;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return eEZxKzIlWDaepZXmTUzEyeGinFm;
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return HvUsjbcKcvfjVDepXAAIQHsgOww;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return vikLxBWqiSQAmEZFRDZCNUqNOXa;
			}
		}

		public static bool isPlaying => FMnadxjVOaUgagvSxyTwkypLtEG;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return YpezioQrqKiEUyoIWlMliDAJZGy;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return nMieJgQejclQhhHENtuZGMmIdmi;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (vikLxBWqiSQAmEZFRDZCNUqNOXa || Application.isEditor)
				{
					return true;
				}
				...
				switch (eEZxKzIlWDaepZXmTUzEyeGinFm)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!YpezioQrqKiEUyoIWlMliDAJZGy)
					{
						return PgeFTCHuosgTacWnDOIvQLMIfmJK == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return HvUsjbcKcvfjVDepXAAIQHsgOww switch
				{
					EditorPlatform.Windows => eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.Windows, 
					EditorPlatform.OSX => eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.OSX, 
					EditorPlatform.Linux => eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return QRNmGrcRpXBqWMOcAclvaKnJwGIB;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return XuhXoukoEkgnECPbsPgsLxbosdrB;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.Android && eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.Ouya && eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.AmazonFireTV)
				{
					return eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.iOS)
				{
					return eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.Windows && eEZxKzIlWDaepZXmTUzEyeGinFm != Platform.Linux)
				{
					return eEZxKzIlWDaepZXmTUzEyeGinFm == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return FNVQMhMtiVcqjQNyhLTHwebVqsT;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_0)
				{
					return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool supportsStadia
		{
			get
			{
				...
				return FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_2019_3;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => PgeFTCHuosgTacWnDOIvQLMIfmJK;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => mFbcqBDIEMiXWfyJfRsiZsPALHun;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return tjCHjoFEebgaqOnGSdpCNMaozRg;
			}
		}
		...
		internal static IAndroidFallbackPlatformHelper androidFallbackPlatformHelper
		{
			...
			get
			{
				return lxmCstrGMPIbCaksJzmAodKPXPBf;
			}
			[CompilerGenerated]
			set
			{
				lxmCstrGMPIbCaksJzmAodKPXPBf = value;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => QsXGxNLRuyQfpdbmkpoUbFCpFqe;

		private static bool initialized => WdEMJhueAEdtXtJrCpLtcCuCkHX();

		private static bool WdEMJhueAEdtXtJrCpLtcCuCkHX()
		{
			if (QsXGxNLRuyQfpdbmkpoUbFCpFqe)
			{
				return true;
			}
			try
			{
				rqxjMfjDWELuwjtMfEsZTCzRxcH = Application.unityVersion;
				xYsGbTgkLfyfDdjZCrfTamMhGtp = new UnityVersionClass(rqxjMfjDWELuwjtMfEsZTCzRxcH);
				jeqfXyQIJOHCSaYvZHuiOMqccpq();
				QsXGxNLRuyQfpdbmkpoUbFCpFqe = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return QsXGxNLRuyQfpdbmkpoUbFCpFqe;
		}

		internal static void EJpmrTgGvrhKjJnkpXbomYBpQTQ(Platform P_0, EditorPlatform P_1, bool P_2, WebplayerPlatform P_3, ScriptingBackend P_4, ScriptingAPILevel P_5, IExternalTools P_6)
		{
			if (initialized)
			{
				...
				eEZxKzIlWDaepZXmTUzEyeGinFm = P_0;
				HvUsjbcKcvfjVDepXAAIQHsgOww = P_1;
				vikLxBWqiSQAmEZFRDZCNUqNOXa = P_2;
				nMieJgQejclQhhHENtuZGMmIdmi = P_3;
				PgeFTCHuosgTacWnDOIvQLMIfmJK = P_4;
				mFbcqBDIEMiXWfyJfRsiZsPALHun = P_5;
				if (tjCHjoFEebgaqOnGSdpCNMaozRg != null)
				{
					tjCHjoFEebgaqOnGSdpCNMaozRg.Destroy();
				}
				tjCHjoFEebgaqOnGSdpCNMaozRg = P_6;
				YpezioQrqKiEUyoIWlMliDAJZGy = Debug.isDebugBuild;
				FMnadxjVOaUgagvSxyTwkypLtEG = true;
				cUTXfClEOQIwOvBEZxijNXURfCZ();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			YBQzATMyUKCnUmNUHZdefyatrWv(minVersionStr, out var num);
			YBQzATMyUKCnUmNUHZdefyatrWv(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (flag && xYsGbTgkLfyfDdjZCrfTamMhGtp < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag2 && xYsGbTgkLfyfDdjZCrfTamMhGtp >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag2 && xYsGbTgkLfyfDdjZCrfTamMhGtp > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool YBQzATMyUKCnUmNUHZdefyatrWv(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void jeqfXyQIJOHCSaYvZHuiOMqccpq()
		{
			FUFGGnGbrRWXzBsQxlbzgGnFTVQn = RjFKpqlumeAuxdtIUEhPOjcuFqD(Application.unityVersion);
			if (FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_3_5 && FUFGGnGbrRWXzBsQxlbzgGnFTVQn < UnityVersion.UNITY_4_0)
			{
				QRNmGrcRpXBqWMOcAclvaKnJwGIB = true;
			}
			else if (FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_0)
			{
				XuhXoukoEkgnECPbsPgsLxbosdrB = true;
			}
		}

		private static UnityVersion RjFKpqlumeAuxdtIUEhPOjcuFqD(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion EeERvasGDMXXlpuQsawerivHSMZ(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion VURlOBfzQDTjLnbBxpRpTaXxyHX(int P_0)
		{
			return P_0 switch
			{
		...
		private static void cUTXfClEOQIwOvBEZxijNXURfCZ()
		{
			switch (eEZxKzIlWDaepZXmTUzEyeGinFm)
			{
			...
			case Platform.RazerForgeTV:
				FVruvuPJIkxpKnaGlcGvyJtOYdC = true;
				SMTWuxtOABCXJGSfyzEReFIqQgD = true;
				break;
			case Platform.PS4:
				FVruvuPJIkxpKnaGlcGvyJtOYdC = true;
				HAYUhWkmLGwsgeAkKKjdhrMyETx = "Empty";
				fMfEIPFaGzVMxVRWtkOmRBWuhBeF = true;
				break;
			case Platform.Windows:
				if ((FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_6_3p1 && FUFGGnGbrRWXzBsQxlbzgGnFTVQn < UnityVersion.UNITY_5_0) || FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_0_0p1)
				{
					FNVQMhMtiVcqjQNyhLTHwebVqsT = true;
					FVruvuPJIkxpKnaGlcGvyJtOYdC = true;
				}
				break;
			case Platform.Linux:
			...
			if (vikLxBWqiSQAmEZFRDZCNUqNOXa)
			{
				EditorPlatform hvUsjbcKcvfjVDepXAAIQHsgOww = HvUsjbcKcvfjVDepXAAIQHsgOww;
				if (hvUsjbcKcvfjVDepXAAIQHsgOww == EditorPlatform.Windows && ((FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_6_3p1 && FUFGGnGbrRWXzBsQxlbzgGnFTVQn < UnityVersion.UNITY_5_0) || FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_5_0_0p1))
				{
					FNVQMhMtiVcqjQNyhLTHwebVqsT = true;
					FVruvuPJIkxpKnaGlcGvyJtOYdC = true;
				}
		...
		internal static Type tmnaZUeceODQqvWhHfdAJYxBRjB(UkAlMoodJlUVkRzPsjNdSxwRnXI P_0)
		{
			if (!initialized)
			{
				return null;
			}
			if (FUFGGnGbrRWXzBsQxlbzgGnFTVQn >= UnityVersion.UNITY_4_3)
			{
				return onfURnmJuaRwqjophqacBWAgAHx(P_0);
			}
			return null;
		}

		private static Type onfURnmJuaRwqjophqacBWAgAHx(UkAlMoodJlUVkRzPsjNdSxwRnXI P_0)
		{
			return P_0 switch
			{
				UkAlMoodJlUVkRzPsjNdSxwRnXI.kuUUMNkeLLzTdBdmpyYxpVjoADX => typeof(RigidbodyInterpolation2D), 
				UkAlMoodJlUVkRzPsjNdSxwRnXI.YUuPEjgpJckBBrsmdPPWUZwwQNV => typeof(RigidbodySleepMode2D), 
				UkAlMoodJlUVkRzPsjNdSxwRnXI.enDRdfxKPtyYMjGZOkQIVVkuIev => typeof(CollisionDetectionMode2D), 
				UkAlMoodJlUVkRzPsjNdSxwRnXI.eGpGjYHIZtFkSfQcbWkABliABUCh => typeof(PhysicsMaterial2D), 
				UkAlMoodJlUVkRzPsjNdSxwRnXI.OYdlgwcYyFhvYKhMAAdxDSdlgpTl => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return eNWYJhNPERqVvHOtZztWnMDKfAH(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return eNWYJhNPERqVvHOtZztWnMDKfAH((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return eNWYJhNPERqVvHOtZztWnMDKfAH((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return eNWYJhNPERqVvHOtZztWnMDKfAH(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && FNVQMhMtiVcqjQNyhLTHwebVqsT)
				{
					return false;
				}
				if (FVruvuPJIkxpKnaGlcGvyJtOYdC)
				{
					return false;
				}
			}
			...
			{
				if (fMfEIPFaGzVMxVRWtkOmRBWuhBeF && name.Equals(HAYUhWkmLGwsgeAkKKjdhrMyETx, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (SMTWuxtOABCXJGSfyzEReFIqQgD && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static T eNWYJhNPERqVvHOtZztWnMDKfAH<T>(T P_0) where T : class
		{
		...
		internal static ButtonStateFlags wTAaLLJvKJshQxazybysdRuUePU(KeyCode P_0)
		{
			ButtonStateFlags buttonStateFlags = (Input.GetKey(P_0) ? ButtonStateFlags.azLWpLIAMvIpDxdUTFgLLfAIMtW : ButtonStateFlags.vzRTPkAxQouGEgDZGevsgfcpaRF);
		...
		internal static ButtonStateFlags tczGrLoSLQRKAWwrReBmbHatjKF(string P_0)
		{
			ButtonStateFlags buttonStateFlags = (Input.GetButton(P_0) ? ButtonStateFlags.azLWpLIAMvIpDxdUTFgLLfAIMtW : ButtonStateFlags.vzRTPkAxQouGEgDZGevsgfcpaRF);
