...
namespace Rewired.Utils.Classes.Data
{
	...
	{
		[Serializable]
		public struct eoOQbqxmgUtaGemRHdqFBdOnAKXj : IDisposable, IEnumerator, IEnumerator<T>
		{
			...
			internal eoOQbqxmgUtaGemRHdqFBdOnAKXj(AList<T> list)
			{
				this.list = list;
				index = 0;
				version = list.wxDwUInmywxWjbzUIARycmGyOtR;
				current = default(T);
			}
			...
			{
				AList<T> aList = list;
				if (version == aList.wxDwUInmywxWjbzUIARycmGyOtR && (uint)index < (uint)aList._count)
				{
			...
			{
				if (version != list.wxDwUInmywxWjbzUIARycmGyOtR)
				{
		...
		private const int YHooZNOqYgwPdBUZBuaJsXBvMYN = 4;

		private static readonly T[] IsggzgvXedEVjEWFZWkujKiejBwc = new T[0];

		private IEqualityComparer<T> PlaWXSBeFxfnzAqVJcbEOqQrVlM = EqualityComparerNoAlloc<T>.Default;

		public T[] _items;

		private int VEmNteWmpdRTYXxUtdCoLGPlsxd;

		public int _count;

		private int XkGTvONlnTFarqWIUkiYwbvNsZo;

		private bool ITtZcOxVLObpqYiEetXoaNmLkUI;

		private readonly int MnKGzVrxfyYgEVOHGQvzyMhuIsV;

		private readonly bool wPQpnlPJiFKRrKHpsZWeSfTuTqn;

		private int wxDwUInmywxWjbzUIARycmGyOtR;

		[NonSerialized]
		private object lRwArgfgKziwkpmDNUWAWEraLIK;

		public int Count => _count;

		public int Capacity => VEmNteWmpdRTYXxUtdCoLGPlsxd;

		public int FreeSpace => MnKGzVrxfyYgEVOHGQvzyMhuIsV - _count;

		public bool IsFixedSize => !wPQpnlPJiFKRrKHpsZWeSfTuTqn;
		...
		public int Version => wxDwUInmywxWjbzUIARycmGyOtR;
		...
		object IList.this[int index]
		{
			...
			set
			{
				if (!moQpLpoqgvROrFxNfKsegnWPszD(value))
				{
		...
		object ICollection.SyncRoot
		{
			get
			{
				if (lRwArgfgKziwkpmDNUWAWEraLIK == null)
				{
					Interlocked.CompareExchange<object>(ref lRwArgfgKziwkpmDNUWAWEraLIK, new object(), (object)null);
				}
				return lRwArgfgKziwkpmDNUWAWEraLIK;
			}
		}
		...
		public AList(int startingCapacity, int maxCapacity, int expansionIncrement)
		{
			...
			{
				wPQpnlPJiFKRrKHpsZWeSfTuTqn = true;
			}
			if (!wPQpnlPJiFKRrKHpsZWeSfTuTqn && startingCapacity == 0)
			{
				throw new ArgumentOutOfRangeException("startingCapacity must be > 0 if non-expandable.");
			}
			if (wPQpnlPJiFKRrKHpsZWeSfTuTqn && expansionIncrement == 0)
			{
				ITtZcOxVLObpqYiEetXoaNmLkUI = true;
				expansionIncrement = 1;
			}
			XkGTvONlnTFarqWIUkiYwbvNsZo = expansionIncrement;
			VEmNteWmpdRTYXxUtdCoLGPlsxd = startingCapacity;
			MnKGzVrxfyYgEVOHGQvzyMhuIsV = ((maxCapacity == 0) ? int.MaxValue : maxCapacity);
			_count = 0;
			if (VEmNteWmpdRTYXxUtdCoLGPlsxd == 0)
			{
				_items = IsggzgvXedEVjEWFZWkujKiejBwc;
			}
			else
			{
		...
		public AList(IEnumerable<T> collection, int maxCapacity, int expansionIncrement)
		{
			...
			if (collection is ICollection<T> { Count: var count } collection2)
			{
				if (count == 0)
				{
					array = IsggzgvXedEVjEWFZWkujKiejBwc;
				}
				else
				{
					array = new T[count];
			...
			if (!wPQpnlPJiFKRrKHpsZWeSfTuTqn && num == 0)
			{
				throw new ArgumentOutOfRangeException("startingCapacity must be > 0 if non-expandable.");
			}
			if (wPQpnlPJiFKRrKHpsZWeSfTuTqn && expansionIncrement == 0)
			...
			XkGTvONlnTFarqWIUkiYwbvNsZo = expansionIncrement;
			VEmNteWmpdRTYXxUtdCoLGPlsxd = num;
			MnKGzVrxfyYgEVOHGQvzyMhuIsV = ((maxCapacity == 0) ? int.MaxValue : maxCapacity);
			_items = ((array != null) ? array : IsggzgvXedEVjEWFZWkujKiejBwc);
			_count = num;
		}
		...
		{
			if (_count == VEmNteWmpdRTYXxUtdCoLGPlsxd && CmjDmlfTvcKuZkNLfeXkkqEWnZp(XkGTvONlnTFarqWIUkiYwbvNsZo) == 0)
			{
				return -1;
			}
			int count = _count;
		...
		public bool Add(T[] items, int count = 0, int startIndex = 0, bool allowPartialAdd = false)
		{
			...
			int num = VEmNteWmpdRTYXxUtdCoLGPlsxd - _count;
			if (count > num)
			{
				int num2 = CmjDmlfTvcKuZkNLfeXkkqEWnZp(Math.Max(num, XkGTvONlnTFarqWIUkiYwbvNsZo), true);
				if (num2 == 0)
				{
					return false;
				}
				...
				count = CmjDmlfTvcKuZkNLfeXkkqEWnZp(Math.Max(num, XkGTvONlnTFarqWIUkiYwbvNsZo));
			}
		...
		public int AddToFirstOpenSpace(T item)
		{
			...
			{
				if (PlaWXSBeFxfnzAqVJcbEOqQrVlM.Equals(_items[i], y))
				{
					_items[i] = item;
					return i;
				}
			}
			if (_count < MnKGzVrxfyYgEVOHGQvzyMhuIsV)
			{
				return Add(item);
			}
			return -1;
		...
		public int AddToFirstOpenSpace(T item, T openSpaceEquals)
		{
			for (int i = 0; i < _count; i++)
			{
				if (PlaWXSBeFxfnzAqVJcbEOqQrVlM.Equals(_items[i], openSpaceEquals))
				{
					_items[i] = item;
					return i;
				}
		...
		{
			return Contains(item, PlaWXSBeFxfnzAqVJcbEOqQrVlM);
		}
		...
		{
			return IndexOf(item, index, PlaWXSBeFxfnzAqVJcbEOqQrVlM);
		}

		public int IndexOf(T item, int index, int count)
		{
			return IndexOf(item, index, count, PlaWXSBeFxfnzAqVJcbEOqQrVlM);
		}
		...
		public void ForEach(Action<T> action)
		{
			...
			int num = wxDwUInmywxWjbzUIARycmGyOtR;
			for (int i = 0; i < _count; i++)
			{
				if (num != wxDwUInmywxWjbzUIARycmGyOtR)
				{
					break;
				}
				action(_items[i]);
		...
		{
			if (wPQpnlPJiFKRrKHpsZWeSfTuTqn && _count != VEmNteWmpdRTYXxUtdCoLGPlsxd)
			{
				AuvmjaZdaBCIGYAAWdUBnVbZKRw(_count);
				wxDwUInmywxWjbzUIARycmGyOtR++;
			}
		}

		private int CmjDmlfTvcKuZkNLfeXkkqEWnZp(int P_0, bool P_1 = false)
		{
			if (!wPQpnlPJiFKRrKHpsZWeSfTuTqn)
			{
				return 0;
			}
			if (VEmNteWmpdRTYXxUtdCoLGPlsxd >= MnKGzVrxfyYgEVOHGQvzyMhuIsV)
			{
				return 0;
			}
			if (ITtZcOxVLObpqYiEetXoaNmLkUI)
			{
				P_0 = YwwjVrvifCPCXkvAIUDFgXPUhob(VEmNteWmpdRTYXxUtdCoLGPlsxd, P_0);
			}
			P_0 = Math.Min(P_0, MnKGzVrxfyYgEVOHGQvzyMhuIsV - VEmNteWmpdRTYXxUtdCoLGPlsxd);
			if (P_0 <= 0)
			{
				return 0;
			}
			if (!AuvmjaZdaBCIGYAAWdUBnVbZKRw(VEmNteWmpdRTYXxUtdCoLGPlsxd + P_0))
			{
				return 0;
			}
			return P_0;
		...
		private int YwwjVrvifCPCXkvAIUDFgXPUhob(int P_0, int P_1)
		{
			int num = P_0 + P_1;
			if (num < 4)
			{
		...
		private bool AuvmjaZdaBCIGYAAWdUBnVbZKRw(int P_0, bool P_1 = false)
		{
			if (P_0 < 0)
			{
				P_0 = 0;
			}
			if (P_0 > MnKGzVrxfyYgEVOHGQvzyMhuIsV)
			{
				return false;
			}
			...
			if (P_0 != 0)
			{
				Array.Copy(_items, array, Math.Min(P_0, VEmNteWmpdRTYXxUtdCoLGPlsxd));
			}
			VEmNteWmpdRTYXxUtdCoLGPlsxd = P_0;
			if (_count > P_0)
			{
				_count = P_0;
			}
		...
		{
			return new eoOQbqxmgUtaGemRHdqFBdOnAKXj(this);
		}
		...
		private static bool moQpLpoqgvROrFxNfKsegnWPszD(object P_0)
		{
			if (!(P_0 is T))
			{
				if (P_0 == null)
