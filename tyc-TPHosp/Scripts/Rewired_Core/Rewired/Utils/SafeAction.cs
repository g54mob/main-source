...
namespace Rewired.Utils
{
	...
	{
		private static Action<object, Action> XJrypfBzlTPJARAReZgjorcrvIU;

		private static Action<object, Action> invokeDelegate => pIncveKGAgbWQiiPNjWYIyXClBY;
		...
		private static void pIncveKGAgbWQiiPNjWYIyXClBY(object P_0, Action P_1)
		{
			P_1?.Invoke();
		}
	...
	internal sealed class SafeAction<T> : SafeDelegate<Action<T>>
	{
		...
		private static Action<object, Action<T>> XJrypfBzlTPJARAReZgjorcrvIU;

		private static Action<object, Action<T>> invokeDelegate => pIncveKGAgbWQiiPNjWYIyXClBY;
		...
		private static void pIncveKGAgbWQiiPNjWYIyXClBY(object P_0, Action<T> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T> safeAction)
			{
				P_1(safeAction.BeRykoKbnRgpslCIPabtwVrJXcM);
			}
		}
	...
	internal sealed class SafeAction<T, T2> : SafeDelegate<Action<T, T2>>
	{
		...
		private T2 uyfLnjppJptVepdrHLncEyZtGhc;

		private static Action<object, Action<T, T2>> XJrypfBzlTPJARAReZgjorcrvIU;

		private static Action<object, Action<T, T2>> invokeDelegate => pIncveKGAgbWQiiPNjWYIyXClBY;
		...
		{
			BeRykoKbnRgpslCIPabtwVrJXcM = arg0;
			uyfLnjppJptVepdrHLncEyZtGhc = arg1;
			try
			{
				Invoke(invokeDelegate);
			}
			...
			BeRykoKbnRgpslCIPabtwVrJXcM = default(T);
			uyfLnjppJptVepdrHLncEyZtGhc = default(T2);
		}
		...
		private static void pIncveKGAgbWQiiPNjWYIyXClBY(object P_0, Action<T, T2> P_1)
		{
			if (P_1 != null && P_0 is SafeAction<T, T2> safeAction)
			{
				P_1(safeAction.BeRykoKbnRgpslCIPabtwVrJXcM, safeAction.uyfLnjppJptVepdrHLncEyZtGhc);
			}
		}
