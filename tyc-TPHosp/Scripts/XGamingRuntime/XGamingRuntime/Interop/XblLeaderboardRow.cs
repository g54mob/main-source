...
namespace XGamingRuntime.Interop
{
	internal struct XblLeaderboardRow
	{
		...
		internal XblLeaderboardRow(XGamingRuntime.XblLeaderboardRow row, DisposableCollection disposableCollection)
		{
			...
			rank = row.Rank;
			columnValues = Converters.ClassArrayToPtr(row.ColumnValues, (string s, DisposableCollection dc) => new UTF8StringPtr(s, dc), disposableCollection, out columnValuesCount);
		}
	}
}
