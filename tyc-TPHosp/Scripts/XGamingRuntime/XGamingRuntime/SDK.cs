...
namespace XGamingRuntime
{
	public class SDK
	{
		public class XBL
		{
			...
			{
				if (xblContextHandle == null || permissionsToCheck == null || targetXuids == null || targetAnonymousUserTypes == null || permissionsToCheck.Length == 0 || targetXuids.Length == 0 || targetAnonymousUserTypes.Length == 0 || targetXuids.Length != targetAnonymousUserTypes.Length)
				{
					completionRoutine(-2147024809, null);
					return;
				}
				...
				int num = XblInterop.XblPrivacyBatchCheckPermissionAsync(xblContextHandle.InteropHandle, permissionsToCheck, new SizeT(permissionsToCheck.Length), targetXuids, new SizeT(targetXuids.Length), targetAnonymousUserTypes, new SizeT(targetAnonymousUserTypes.Length), async);
				if (HR.FAILED(num))
				{
					completionRoutine(num, null);
				}
			...
			public static void XblPresenceGetPresenceForMultipleUsersAsync(XblContextHandle xblContextHandle, ulong[] xuids, XblPresenceQueryFilters filters, XblPresenceGetPresenceForMultipleUsersCompleted completionRoutine)
			{
				...
				using DisposableCollection disposableCollection = new DisposableCollection();
				int num = XblInterop.XblPresenceGetPresenceForMultipleUsersAsync(xblContextHandle.InteropHandle, xuids, new SizeT((xuids != null) ? xuids.Length : 0), (filters == null) ? null : new XblPresenceQueryFiltersRef(filters, disposableCollection), xAsyncBlockPtr);
				if (HR.FAILED(num))
				{
					AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
					completionRoutine(num, null);
			...
			public static int XblSocialManagerCreateSocialUserGroupFromList(XUserHandle user, ulong[] xboxUserIdList, out XblSocialManagerUserGroupHandle group)
			{
				...
				XGamingRuntime.Interop.XblSocialManagerUserGroupHandle group2;
				return XblSocialManagerUserGroupHandle.WrapAndReturnHResult(XblInterop.XblSocialManagerCreateSocialUserGroupFromList(user.InteropHandle, xboxUserIdList, new SizeT((xboxUserIdList != null) ? xboxUserIdList.Length : 0), out group2), group2, out group);
			}
			...
			public static int XblSocialManagerUpdateSocialUserGroup(XblSocialManagerUserGroupHandle group, ulong[] users)
			{
				...
				return XblInterop.XblSocialManagerUpdateSocialUserGroup(group.InteropHandle, users, new SizeT((users != null) ? users.Length : 0));
			}
			...
			public static void XblTitleManagedStatsWriteAsync(XblContextHandle xblContextHandle, ulong xboxUserId, XblTitleManagedStatistic[] statistics, XblTitleManagedStatsWriteCompleted completionRoutine)
			{
				if (xblContextHandle == null)
				{
					completionRoutine(-2147024809);
		...
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XGameInviteEventCallback, XGameInviteEventCallback>).userCallback?.Invoke(inviteUri.GetString());
		}
		...
		{
			updateHandle = null;
			if (container == null || string.IsNullOrEmpty(containerDisplayName))
			{
				return -2147024809;
			}
		...
		[MonoPInvokeCallback]
		private static NativeBool EnumerationCallback(IntPtr context, ref XGamingRuntime.Interop.XPackageDetails packageDetails)
		{
		...
		[MonoPInvokeCallback]
		private static void PackageInstalledCallback(IntPtr context, ref XGamingRuntime.Interop.XPackageDetails packageDetails)
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XPackageInstalledCallback, XPackageInstalledCallback>).userCallback?.Invoke(new XPackageDetails(packageDetails));
		}
		...
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XPackageInstallationProgressCallback, XPackageInstallationProgressCallback>).userCallback?.Invoke(new XPackageInstallationMonitorHandle(monitor));
		}
		...
		public static void XPackageUnregisterPackageInstalled(XRegistrationToken token)
		{
			if (token != null)
			{
				XGRInterop.XPackageUnregisterPackageInstalled(token.Token, wait: true);
				token.CallbackHandle.Free();
			}
		}
		...
		{
			mountHandle = null;
			XGamingRuntime.Interop.XPackageMountHandle mountHandle2;
			int num = XGRInterop.XPackageMount(Converters.StringToNullTerminatedUTF8ByteArray(packageIdentifier), out mountHandle2);
			if (HR.SUCCEEDED(num))
			{
				mountHandle = new XPackageMountHandle(mountHandle2);
			}
			return num;
		}
		...
		public static void XPackageUnregisterInstallationProgressChanged(XPackageInstallationMonitorHandle installationMonitor, XRegistrationToken token)
		{
			...
			{
				XGRInterop.XPackageUnregisterInstallationProgressChanged(installationMonitor.InteropHandle, token.Token, wait: true);
				token.CallbackHandle.Free();
			}
		}
		...
		{
			return XGRInterop.XPackageEstimateDownloadSize(Converters.StringToNullTerminatedUTF8ByteArray(packageIdentifier), 0u, null, out downloadSize, out shouldPresentUserConfirmation);
		}
		...
		public static void XStoreQueryGameAndDlcPackageUpdatesAsync(XStoreContext context, XStoreQueryGameAndDlcPackageUpdatesCompleted completionRoutine)
		{
			XAsyncBlockPtr xAsyncBlockPtr = AsyncHelpers.WrapAsyncBlock(defaultQueue.handle, delegate(XAsyncBlockPtr block)
			{
				...
				if (num2 == 0 && count != 0)
				{
					IntPtr intPtr = Marshal.AllocHGlobal((int)(Marshal.SizeOf((object)default(XGamingRuntime.Interop.XStorePackageUpdate)) * count));
					num2 = XGRInterop.XStoreQueryGameAndDlcPackageUpdatesResult(block, count, intPtr);
					if (num2 == 0)
					{
						packageUpdates = Converters.PtrToClassArray(intPtr, count, (XGamingRuntime.Interop.XStorePackageUpdate x) => new XStorePackageUpdate(x));
					}
		...
		public static void XStoreDownloadAndInstallPackagesAsync(XStoreContext context, string[] storeIds, XStoreDownloadAndInstallPackagesCompleted completionRoutine)
		{
			XAsyncBlockPtr xAsyncBlockPtr = AsyncHelpers.WrapAsyncBlock(defaultQueue.handle, delegate(XAsyncBlockPtr block)
			{
				...
				if (num2 == 0)
				{
					...
					if (count != 0)
					{
						byte[] array2 = new byte[count * 33];
						num2 = XGRInterop.XStoreDownloadAndInstallPackagesResult(block, count, array2);
						for (int i = 0; i < count; i++)
						{
							array[i] = Converters.ByteArrayToString(array2, i * 33, 33);
						}
					}
				}
				completionRoutine(num2, array);
			});
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(storeIds);
			int num = XGRInterop.XStoreDownloadAndInstallPackagesAsync(context.handle, disposableBuffer.IntPtr, new SizeT((storeIds != null) ? storeIds.Length : 0), xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num, null);
		...
		public static void XStoreDownloadAndInstallPackageUpdatesAsync(XStoreContext context, string[] packageIdentifiers, XStoreDownloadAndInstallPackageUpdatesCompleted completionRoutine)
		{
			...
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(packageIdentifiers);
			int num = XGRInterop.XStoreDownloadAndInstallPackageUpdatesAsync(context.handle, disposableBuffer.IntPtr, new SizeT((packageIdentifiers != null) ? packageIdentifiers.Length : 0), xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num);
		...
		public static void XStoreDownloadPackageUpdatesAsync(XStoreContext context, string[] packageIdentifiers, XStoreDownloadPackageUpdatesCompleted completionRoutine)
		{
			...
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(packageIdentifiers);
			int num = XGRInterop.XStoreDownloadPackageUpdatesAsync(context.handle, disposableBuffer.IntPtr, new SizeT((packageIdentifiers != null) ? packageIdentifiers.Length : 0), xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num);
		...
		public static void XStoreShowRedeemTokenUIAsync(XStoreContext context, string token, string[] allowedStoreIds, bool disallowCsvRedeption, XStoreShowRedeemTokenUICompleted completionRoutine)
		{
			...
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(allowedStoreIds);
			int num = XGRInterop.XStoreShowRedeemTokenUIAsync(context.handle, Converters.StringToNullTerminatedUTF8ByteArray(token), disposableBuffer.IntPtr, new SizeT((allowedStoreIds != null) ? allowedStoreIds.Length : 0), disallowCsvRedeption, xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num);
		...
		public static void XStoreQueryConsumableBalanceRemainingAsync(XStoreContext context, string storeProductId, XStoreQueryConsumableBalanceRemainingCompleted completionRoutine)
		{
			XAsyncBlockPtr xAsyncBlockPtr = AsyncHelpers.WrapAsyncBlock(defaultQueue.handle, delegate(XAsyncBlockPtr block)
			{
				XStoreConsumable result;
				int hresult = XGRInterop.XStoreQueryConsumableBalanceRemainingResult(block, out result);
		...
		public static void XStoreReportConsumableFulfillmentAsync(XStoreContext context, string storeProductId, uint quantity, Guid trackingId, XStoreReportConsumableFulfillmentCompleted completionRoutine)
		{
			...
			{
				XStoreConsumable result;
				int hresult = XGRInterop.XStoreReportConsumableFulfillmentResult(block, out result);
				completionRoutine(hresult, result.quantity);
			});
		...
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XStoreGameLicenseChangedCallback, XStoreGameLicenseChangedCallback>).userCallback?.Invoke();
		}

		[MonoPInvokeCallback]
		private static void LicenseLostCallback(IntPtr context)
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XStorePackageLicenseLostCallback, XStorePackageLicenseLostCallback>).userCallback?.Invoke();
		}
		...
		public static void XStoreQueryAddOnLicensesAsync(XStoreContext context, XStoreQueryAddOnLicensesCompleted completionRoutine)
		{
			...
			XAsyncBlockPtr xAsyncBlockPtr = AsyncHelpers.WrapAsyncBlock(defaultQueue.handle, delegate(XAsyncBlockPtr block)
			{
				...
				{
					IntPtr intPtr = Marshal.AllocHGlobal((int)count * Marshal.SizeOf((object)default(XGamingRuntime.Interop.XStoreAddonLicense)));
					num2 = XGRInterop.XStoreQueryAddOnLicensesResult(block, count, intPtr);
					XStoreAddonLicense[] licenses = null;
					if (HR.SUCCEEDED(num2))
					{
						licenses = Converters.PtrToClassArray(intPtr, count, (XGamingRuntime.Interop.XStoreAddonLicense x) => new XStoreAddonLicense(x));
					}
					Marshal.FreeHGlobal(intPtr);
		...
		public static void XStoreQueryLicenseTokenAsync(XStoreContext context, string[] productIds, string customDeveloperString, XStoreQueryLicenseTokenCompleted completionRoutine)
		{
			...
			XAsyncBlockPtr xAsyncBlockPtr = AsyncHelpers.WrapAsyncBlock(defaultQueue.handle, delegate(XAsyncBlockPtr block)
			{
				...
				byte[] array = new byte[size.ToUInt32()];
				num2 = XGRInterop.XStoreQueryLicenseTokenResult(block, size, array);
				string token = Converters.ByteArrayToString(array);
				completionRoutine(num2, token);
			});
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(productIds);
			int num = XGRInterop.XStoreQueryLicenseTokenAsync(context.handle, disposableBuffer.IntPtr, new SizeT((productIds != null) ? productIds.Length : 0), Converters.StringToNullTerminatedUTF8ByteArray(customDeveloperString), xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num, null);
		...
		public static void XStoreUnregisterGameLicenseChanged(XStoreContext context, XRegistrationToken token)
		{
			...
			{
				XGRInterop.XStoreUnregisterGameLicenseChanged(context.handle, token.Token, wait: true);
				token.CallbackHandle.Free();
			}
		}
		...
		public static void XStoreUnregisterPackageLicenseLost(XStoreLicense license, XRegistrationToken token)
		{
			...
			{
				XGRInterop.XStoreUnregisterPackageLicenseLost(license.Handle, token.Token, wait: true);
				token.CallbackHandle.Free();
			}
		}
		...
		{
			XGamingRuntime.Interop.XStoreProduct rawProduct = (XGamingRuntime.Interop.XStoreProduct)Marshal.PtrToStructure(product, typeof(XGamingRuntime.Interop.XStoreProduct));
			(GCHandle.FromIntPtr(context).Target as List<XStoreProduct>).Add(new XStoreProduct(rawProduct));
			return new NativeBool(value: true);
		}
		...
		private static void ExtractQueryResultAndComplete(XStoreQueryComplete completionRoutine, XAsyncBlockPtr block, QueryExtractionFunction extractionFunction)
		{
			...
			if (HR.SUCCEEDED(num))
			{
				...
				result = new XStoreQueryResult(queryHandle, result2, hasMorePages);
			}
		...
		public static void XStoreQueryProductsAsync(XStoreContext context, XStoreProductKind productKinds, string[] storeIds, string[] actionFilters, XStoreQueryComplete completionRoutine)
		{
			...
			using DisposableBuffer disposableBuffer = Converters.StringArrayToUTF8StringArray(storeIds);
			using DisposableBuffer disposableBuffer2 = Converters.StringArrayToUTF8StringArray(actionFilters);
			int num = XGRInterop.XStoreQueryProductsAsync(context.handle, productKinds, disposableBuffer.IntPtr, new SizeT((storeIds != null) ? storeIds.Length : 0), disposableBuffer2.IntPtr, new SizeT((actionFilters != null) ? actionFilters.Length : 0), xAsyncBlockPtr);
			if (HR.FAILED(num))
			{
				AsyncHelpers.CleanupAsyncBlock(xAsyncBlockPtr);
				completionRoutine(num, null);
		...
		{
			(GCHandle.FromIntPtr(context).Target as UnmanagedCallback<XGamingRuntime.Interop.XUserChangeEventCallback, XUserChangeEventCallback>).userCallback?.Invoke(userLocalId, eventType);
		}
