...
public class HeadController : MonoBehaviour, IDropHandler, IEventSystemHandler, IPointerClickHandler, IPointerEnterHandler, IPointerExitHandler, IEndDragHandler, IBeginDragHandler, IDragHandler
{
	...
	public void OnDrag(PointerEventData eventData)
	{
		...
		{
			ghost.sprite = Resources.Load<Sprite>(character.inventory.head.path);
		}
	...
	public void updateItem()
	{
		...
		{
			image.sprite = Resources.Load<Sprite>("Equipment/EmptyHelmet");
			border.color = Color.white;
			return;
		}
		image.sprite = Resources.Load<Sprite>(character.inventory.head.path);
		border.color = Color.gray;
		if (!character.inventory.head.removable)
		{
	...
	{
		message = character.inventoryController.itemTooltipText(-1);
	}

	public void updateHeadStats()
	{
		int id = character.inventory.head.id;
		if (id != 0)
		{
