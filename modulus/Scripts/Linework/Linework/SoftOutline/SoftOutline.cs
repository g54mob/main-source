	...
	public class SoftOutline : ScriptableRendererFeature
	{
		private class SoftOutlinePass : ScriptableRenderPass
		{
			...
			public bool Setup(ref SoftOutlineSettings softOutlineSettings, ref Material maskMaterial, ref Material silhouetteMaterial, ref Material silhouetteInstancedMaterial, ref Material blurMaterial, ref Material compositeMaterial)
			{
				...
				Material material2 = composite;
				int outlineGap = ShaderPropertyId.OutlineGap;
				dilationMethod = settings.dilationMethod;
				material2.SetFloat(outlineGap, (dilationMethod == DilationMethod.Box || dilationMethod == DilationMethod.Dilate || dilationMethod == DilationMethod.Gaussian) ? settings.gap : 0f);
			...
			public override void RecordRenderGraph(RenderGraph renderGraph, ContextContainer frameData)
			{
				...
				CreateRenderGraphTextures(renderGraph, universalResourceData, out var silhouetteHandle, out var blurHandle);
				if (!silhouetteHandle.IsValid() || !blurHandle.IsValid())
				{
					return;
				}
				PassData passData;
				using (IRasterRenderGraphBuilder rasterRenderGraphBuilder = renderGraph.AddRasterRenderPass<PassData>("Mask (Soft Outline)", out passData, "./Packages/dev.ameye.linework/Runtime/SoftOutline/SoftOutline.cs", 181))
				{
				...
				PassData passData2;
				using (IRasterRenderGraphBuilder rasterRenderGraphBuilder2 = renderGraph.AddRasterRenderPass<PassData>("Silhouette (Soft Outline)", out passData2, "./Packages/dev.ameye.linework/Runtime/SoftOutline/SoftOutline.cs", 205))
				{
				...
				PassData passData3;
				using (IUnsafeRenderGraphBuilder unsafeRenderGraphBuilder = renderGraph.AddUnsafePass<PassData>("Blur (Soft Outline)", out passData3, "./Packages/dev.ameye.linework/Runtime/SoftOutline/SoftOutline.cs", 242))
				{
				...
				using IRasterRenderGraphBuilder rasterRenderGraphBuilder3 = renderGraph.AddRasterRenderPass<PassData>("Outline (Soft Outline)", out passData3, "./Packages/dev.ameye.linework/Runtime/SoftOutline/SoftOutline.cs", 276);
			...
			private void CreateRenderGraphTextures(RenderGraph renderGraph, UniversalResourceData resourceData, out TextureHandle silhouetteHandle, out TextureHandle blurHandle)
			{
				...
				textureDesc.colorFormat = ((settings.dilationMethod == DilationMethod.Dilate) ? GraphicsFormat.R8G8B8A8_UNorm : ((settings.type == OutlineType.Hard) ? GraphicsFormat.R8_UNorm : GraphicsFormat.R8G8B8A8_UNorm));
				...
				desc.name = "_BlurBuffer";
				blurHandle = renderGraph.CreateTexture(in desc);
			}
			...
			public override void Execute(ScriptableRenderContext context, ref RenderingData renderingData)
			{
				...
				using (new ProfilingScope(commandBuffer, maskSampler))
				{
					...
					foreach (Outline outline in settings.Outlines)
					{
						...
						drawingSettings.overrideMaterial = mask;
						drawingSettings.overrideShaderPassIndex = 0;
						FilteringSettings filteringSettings = new FilteringSettings(opaque, outline.layerMask, outline.RenderingLayer);
				...
				using (new ProfilingScope(commandBuffer2, silhouetteSampler))
				{
					...
					foreach (Outline outline2 in settings.Outlines)
					{
						...
						FilteringSettings filteringSettings2 = new FilteringSettings(opaque2, outline2.layerMask, outline2.RenderingLayer);
		...
		{
			if (!(settings == null) && softOutlinePass != null && (renderingData.cameraData.cameraType != CameraType.SceneView || settings.ShowInSceneView))
			{
