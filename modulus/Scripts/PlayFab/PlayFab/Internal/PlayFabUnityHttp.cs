	...
	public class PlayFabUnityHttp : ITransportPlugin, IPlayFabPlugin
	{
		...
		private static IEnumerator SimpleCallCoroutine(string method, string fullUrl, byte[] payload, Action<byte[]> successCallback, Action<string> errorCallback)
		{
			...
			else
			{
				...
				if (PlayFabSettings.staticSettings.CompressResponses)
				{
					www2.SetRequestHeader("Accept-Encoding", "gzip");
					www2.downloadHandler = (PlayFabSettings.staticSettings.DecompressWithDownloadHandler ? ((DownloadHandler)new GzipDownloadHandler()) : ((DownloadHandler)new DownloadHandlerBuffer()));
				}
				else
				{
		...
		private IEnumerator Post(CallRequestContainer reqContainer)
		{
			...
			{
				www.SetRequestHeader("Accept-Encoding", "gzip");
				www.downloadHandler = (reqContainer.settings.DecompressWithDownloadHandler ? ((DownloadHandler)new GzipDownloadHandler()) : ((DownloadHandler)new DownloadHandlerBuffer()));
			}
			else
			{
