		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			...
			case Modes.AlongCurve:
				if (AllowAdditivePlays || _coroutine == null)
				{
					...
					_remapCurveZero = (RandomizeRemap ? UnityEngine.Random.Range(RemapCurveZero, RemapCurveZeroAlt) : RemapCurveZero);
					_remapCurveOne = (RandomizeRemap ? UnityEngine.Random.Range(RemapCurveOne, RemapCurveOneAlt) : RemapCurveOne);
					if (_coroutine != null)
					{
						Owner.StopCoroutine(_coroutine);
					}
