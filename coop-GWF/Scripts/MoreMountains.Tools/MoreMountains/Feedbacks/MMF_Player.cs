...
namespace MoreMountains.Feedbacks
{
	...
	public class MMF_Player : MMFeedbacks
	{
		...
		public override bool HasFeedbackStillPlaying()
		{
			...
			for (int i = 0; i < count; i++)
			{
				if ((FeedbacksList[i].IsPlaying && !FeedbacksList[i].Timing.ExcludeFromHoldingPauses) || FeedbacksList[i].Timing.RepeatForever || (FeedbacksList[i].Timing.NumberOfRepeats > 0 && FeedbacksList[i].PlaysLeft > 0))
				{
					return true;
				}
			}
