		...
		protected virtual void Update()
		{
			...
			if (_currentProperty.TimeScaleLerp)
			{
				...
				else
				{
					...
					ApplyTimeScale(Mathf.Lerp(t: _currentProperty.TimeScaleLerpCurve.Evaluate(t), a: Time.timeScale, b: TargetTimeScale));
		...
		protected virtual void Unfreeze()
		{
			...
			if (_timeScaleProperties.Count == 0)
			{
				if (_resetProperty.TimeScaleLerp && _resetProperty.TimeScaleLerpMode == MMTimeScaleLerpModes.Duration && _resetProperty.TimeScaleLerpOnReset)
				{
					...
					MMTimeScaleEvent.Trigger(MMTimeScaleMethods.For, NormalTimeScale, _resetProperty.TimeScaleLerpDuration, _resetProperty.TimeScaleLerp, _resetProperty.LerpSpeed, infinite: true, MMTimeScaleLerpModes.Duration, _resetProperty.TimeScaleLerpCurveOnReset, _resetProperty.TimeScaleLerpDurationOnReset);
				}
				else
				{
					ResetTimeScale();
