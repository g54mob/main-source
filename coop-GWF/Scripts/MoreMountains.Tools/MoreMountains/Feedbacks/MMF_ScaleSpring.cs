	...
	public class MMF_ScaleSpring : MMF_Feedback
	{
		...
		protected override void CustomPlayFeedback(Vector3 position, float feedbacksIntensity = 1f)
		{
			if (Active && FeedbackTypeAuthorized && !(AnimateScaleTarget == null))
			{
				...
				{
				case Modes.MoveTo:
					_targetValue.x = UnityEngine.Random.Range(MoveToScaleMin.x, MoveToScaleMax.x);
					_targetValue.y = UnityEngine.Random.Range(MoveToScaleMin.y, MoveToScaleMax.y);
					_targetValue.z = UnityEngine.Random.Range(MoveToScaleMin.z, MoveToScaleMax.z);
					break;
				...
				case Modes.Bump:
				{
					_velocity.x = UnityEngine.Random.Range(BumpScaleMin.x, BumpScaleMax.x);
					_velocity.y = UnityEngine.Random.Range(BumpScaleMin.y, BumpScaleMax.y);
					_velocity.z = UnityEngine.Random.Range(BumpScaleMin.z, BumpScaleMax.z);
