...
namespace MoreMountains.Tools
{
	...
	public class MMMultipleObjectPooler : MMObjectPooler
	{
		...
		protected virtual GameObject GetPooledGameObjectRandomBetweenObjects()
		{
			...
			if (_lastPooledObject != null)
			{
				while (!CanPoolSameObjectTwice && (Pool[num].GameObjectToPool.name == _lastPooledObject.name || !PoolObjectEnabled(Pool[num].GameObjectToPool)) && (float)num2 < (float)_objectPool.PooledGameObjects.Count * OverflowAmount)
				{
		...
		public virtual GameObject GetPooledGameObjectOfType(string searchedName)
		{
			...
			{
				return AddOneObjectToThePool(gameObject2);
			}
			return null;
		}
