...
namespace MoreMountains.Tools
{
	public class MMGridGenerator
	{
		...
		public static int[,] ApplySafeSpots(int[,] grid, List<MMTilemapGeneratorLayer.MMTilemapGeneratorLayerSafeSpot> safeSpots)
		{
			...
			{
				int num = Mathf.Min(safeSpot.Start.x, safeSpot.End.x);
				int num2 = Mathf.Max(safeSpot.Start.x, safeSpot.End.x);
				int num3 = Mathf.Min(safeSpot.Start.y, safeSpot.End.y);
				int num4 = Mathf.Max(safeSpot.Start.y, safeSpot.End.y);
