...
namespace MoreMountains.Tools
{
	public class MMPlotter : MonoBehaviour
	{
		...
		protected virtual void FillMethodList()
		{
			...
			_methodList = methods.OrderBy((MethodInfo item) => item.Name).ToList();
		}
		...
		protected virtual void Initialization()
		{
			...
			DistanceBetweenPoints = GraphSize / (float)Resolution;
		...
		{
			_axis = Object.Instantiate(Axis);
			_axis.SetLabel(TweenName(TweenMethodIndex).Replace("_", " "));
			_axis.transform.SetParent(base.transform);
		...
		protected virtual void DrawPoints()
		{
			...
			{
				_point = Object.Instantiate(PlotPointPrefab);
				_point.name = base.name + "Point" + i;
				_pointValues.x = (float)i * (1f / (float)Resolution);
				_parameter[0] = _pointValues.x;
				_pointValues.y = InvokeTween(TweenMethodIndex, _parameter);
				_position.x = (float)i * DistanceBetweenPoints;
				...
				_point.SetParent(base.transform, worldPositionStays: false);
				_points[i] = _point;
			}
		}
		...
		protected virtual void Update()
		{
			...
			if (Time.unscaledTime - _lastMovementEndedAt < MovementPauseDuration)
			{
				if (Time.unscaledTime - _lastMovementEndedAt < MovementPauseDuration / 2f)
				{
					...
					_axis.PlotterCurvePoint.localScale = _newScale * 0.1f;
			...
			{
				_axis.PlotterCurvePoint.localScale = Vector3.one * 0.1f;
