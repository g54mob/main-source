	...
	public class MMPropertyLinkColor : MMPropertyLink
	{
		...
		public override void CreateGettersAndSetters(MMProperty property)
		{
			...
			if (property.MemberType == MMProperty.MemberTypes.Property)
			{
				...
				{
					GetColorDelegate = (Func<Color>)Delegate.CreateDelegate(typeof(Func<Color>), firstArgument, property.MemberPropertyInfo.GetGetMethod());
				}
				if (property.MemberPropertyInfo.GetSetMethod() != null)
				{
					SetColorDelegate = (Action<Color>)Delegate.CreateDelegate(typeof(Action<Color>), firstArgument, property.MemberPropertyInfo.GetSetMethod());
				}
		...
		protected virtual Color GetValueOptimized(MMProperty property)
		{
			...
			{
				return (Color)GetPropertyValue(property);
			}
