...
namespace MoreMountains.Tools
{
	...
	public class MMGhostCamera : MonoBehaviour
	{
		...
		protected virtual void Start()
		{
			...
			HorizontalAction.performed += delegate(InputAction.CallbackContext context)
			{
				_currentInput.x = context.ReadValue<float>();
			};
			VerticalAction.performed += delegate(InputAction.CallbackContext context)
			{
				_currentInput.z = context.ReadValue<float>();
			};
			MousePositionAction.performed += delegate(InputAction.CallbackContext context)
			{
				_mouseInput = context.ReadValue<Vector2>();
			};
			HorizontalAction.canceled += delegate
			{
				_currentInput.x = 0f;
			};
			VerticalAction.canceled += delegate
			{
				_currentInput.z = 0f;
			};
			MousePositionAction.canceled += delegate
			{
		...
		protected virtual void Rotate()
		{
			if (RotationEnabled)
			{
				...
				{
					_newEulerAngles.x += (0f - RightStick.RawValue.y) * MobileStickSensitivity;
