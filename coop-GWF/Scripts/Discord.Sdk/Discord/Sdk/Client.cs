...
namespace Discord.Sdk
{
	public class Client : IDisposable
	{
		...
		public unsafe void IsDiscordAppInstalled(IsDiscordAppInstalledCallback callback)
		{
			if (disposed_ != 0)
			{
				throw new ObjectDisposedException("Client");
			}
			NativeMethods.Client.IsDiscordAppInstalledCallback callback2 = NativeMethods.Client.IsDiscordAppInstalledCallback_Handler;
			fixed (NativeMethods.Client* ptr = &self)
			{
				NativeMethods.Client.IsDiscordAppInstalled(ptr, callback2, NativeMethods.ManagedUserData.Free, NativeMethods.ManagedUserData.CreateHandle(callback));
			}
		}
