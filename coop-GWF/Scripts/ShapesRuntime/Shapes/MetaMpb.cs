...
namespace Shapes
{
	internal abstract class MetaMpb : IDisposable
	{
		...
		{
			bool flag = mpbOverride != null && this is MpbCustomMesh;
			if (HasMultipleInstances)
			{
				sdc = new ShapeDrawCall(drawState, instanceCount, matrices, flag ? mpbOverride : null);
				matrices = ArrayPool<Matrix4x4>.Alloc(1023);
			}
			else
			{
				sdc = new ShapeDrawCall(drawState, matrices[0], flag ? mpbOverride : null);
			}
			if (!flag)
			{
