...
namespace Dissonance.Audio.Capture
{
	...
	{
		private static readonly Log Log = Logs.Create(LogCategory.Recording, "BasicMicrophoneCapture");
		...
		public virtual WaveFormat StartCapture(string inputMicName)
		{
			...
			try
			{
				...
				try
				{
					...
					frequency = ((minFreq == 0 && maxFreq == 0) ? 48000 : Mathf.Clamp(48000, minFreq, maxFreq));
				}
				finally
				{
				}
				try
				{
					...
					_clip = Microphone.Start(_micName, loop: true, 10, frequency);
					if (_clip == null)
					{
		...
		private void DrainMicSamples()
		{
			...
			while (count > _readBuffer.MaxCount)
			{
				...
				{
					float num = Mathf.Min(_clip.samples, count - _readBuffer.MaxCount);
