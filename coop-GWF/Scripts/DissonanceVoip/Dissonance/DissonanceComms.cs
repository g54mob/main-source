...
namespace Dissonance
{
	...
	{
		private static readonly Log Log = Logs.Create(LogCategory.Core, "DissonanceComms");
		...
		public static readonly SemanticVersion Version = new SemanticVersion(9, 0, 8);
		...
		public GameObject PlaybackPrefab
		{
			...
			set
			{
				if (_started)
				{
					throw Log.CreateUserErrorException("Cannot set playback prefab when the DissonanceComms component has been started", "directly setting the 'PlaybackPrefab' property too late", "https://placeholder-software.co.uk/dissonance/docs/Reference/Components/Dissonance-Comms", "A0796DA8-A0BC-49E4-A1B3-F0AA0F51BAA0");
				}
				if (value != null && value.GetComponent<IVoicePlayback>() == null)
				{
					throw Log.CreateUserErrorException("Cannot set PlaybackPrefab to a prefab without an implementation of IVoicePlayback", "Setting the 'PlaybackPrefab' property to an incorrect prefab", "https://placeholder-software.co.uk/dissonance/docs/Reference/Components/Dissonance-Comms", "543EB2C1-8911-405B-8BEA-5DBC185DF0C3");
				}
		...
		private void Start()
		{
			...
			if (!base.gameObject.TryGetComponent<ICommsNetwork>(out var component))
			{
		...
		{
			if (!TryGetComponent<IMicrophoneCapture>(out var component))
			{
				return base.gameObject.AddComponent<BasicMicrophoneCapture>();
			}
			return component;
		}
		...
		public static DissonanceComms GetSingleton()
		{
			...
			{
				_singleton = null;
				_singleton = UnityEngine.Object.FindFirstObjectByType<DissonanceComms>();
			}
			return _singleton;
		}
		...
		{
			MonoBehaviour net = _net as MonoBehaviour;
			if (net != null)
			{
				net.enabled = false;
				for (int i = 0; i < 30; i++)
		...
		private void Net_PlayerStoppedSpeaking([NotNull] string player)
		{
			...
			else if (_players.TryGet(player, out state))
			{
				...
				this.OnPlayerStoppedSpeaking?.Invoke(state);
			}
		}
		...
		private void Net_PlayerStartedSpeaking([NotNull] string player)
		{
			if (!Log.AssertAndLogError(player != null, "CA95E783-CA35-441B-9B8B-FAA0FA0B41E3", "Received a player-started-speaking event for a null player ID") && _players.TryGet(player, out var state))
			{
				...
				this.OnPlayerStartedSpeaking?.Invoke(state);
			}
		}
		...
		private void Net_PlayerRoomEvent(RoomEvent evt)
		{
			if (!Log.AssertAndLogError(evt.PlayerName != null, "221D74AD-4F5B-4215-9ADE-CC4D5B455327", "Received a remote room event with a null player name") && _players.TryGet(evt.PlayerName, out var state))
			{
				if (evt.Joined)
				{
					...
					this.OnPlayerEnteredRoom?.Invoke(state, evt.Room);
				}
				else
				{
					state.InvokeOnExitedRoom(evt);
					this.OnPlayerExitedRoom?.Invoke(state, evt.Room);
				}
			}
		}
		...
		private void Net_PlayerLeft([NotNull] string playerId)
		{
			...
			if (voicePlayerState != null)
			{
				...
				this.OnPlayerLeftSession?.Invoke(voicePlayerState);
			}
		}
		...
		private void Net_PlayerJoined([NotNull] string playerId, CodecSettings codecSettings)
		{
			...
			{
				IVoicePlaybackInternal voicePlaybackInternal = _playbackPool.Get(playerId);
				voicePlaybackInternal.CodecSettings = codecSettings;
				RemoteVoicePlayerState remoteVoicePlayerState = new RemoteVoicePlayerState(voicePlaybackInternal);
				_players.Add(remoteVoicePlayerState);
				_playerTrackers.AddPlayer(remoteVoicePlayerState);
				((MonoBehaviour)voicePlaybackInternal).gameObject.SetActive(value: true);
				this.OnPlayerJoinedSession?.Invoke(remoteVoicePlayerState);
			}
		}
		...
		public void GetMicrophoneDevices(List<string> output)
		{
			...
			{
				microphoneCapture = GetComponent<IMicrophoneCapture>();
			}
			IMicrophoneDeviceList component = microphoneCapture as IMicrophoneDeviceList;
			if (component == null)
			{
				TryGetComponent<IMicrophoneDeviceList>(out component);
			}
			if (component != null)
			{
				component.GetDevices(output);
			}
			else
			{
