...
namespace Dissonance
{
	...
	public class VoiceBroadcastTrigger : BaseCommsTrigger, IVoiceActivationListener, IVoiceBroadcastTrigger
	{
		[SerializeField]
		...
		protected override void Update()
		{
			...
			bool flag = IsUserActivated();
			bool flag2 = ShouldActivate(flag);
			if (flag)
			{
			...
			else if (Math.Abs(_activationFader.EndVolume) > float.Epsilon)
			{
				_activationFader.FadeTo(0f, (float)_activationFaderSettings.FadeOut.TotalSeconds);
			}
			if (IsTransmitting == flag2)
			{
				return;
			}
			if (!flag2)
			{
				if (flag && (base.IsColliderTriggered || !UseColliderTrigger))
				{
					CloseChannel();
				}
				else if (CurrentFaderVolume <= float.Epsilon)
				{
					CloseChannel();
				}
			}
		...
		private void OpenChannel()
		{
			...
			{
				_roomChannel = base.Comms.RoomChannels.Open(new RoomName(RoomName), _broadcastPosition, _priority, CurrentFaderVolume);
			}
