...
namespace WebSocketSharp.Net
{
	internal static class HttpUtility
	{
		...
		internal static IPrincipal CreateUser(string response, AuthenticationSchemes scheme, string realm, string method, Func<IIdentity, NetworkCredential> credentialsFinder)
		{
			...
			if (!response.StartsWith(scheme.ToString(), comparisonType))
			{
				return null;
			}
		...
		public static void HtmlAttributeEncode(string s, TextWriter output)
		{
			...
			if (s.Length != 0)
			{
				string value = htmlEncode(s, minimal: true);
				output.Write(value);
			}
		}
		...
		public static void HtmlEncode(string s, TextWriter output)
		{
			...
			if (s.Length != 0)
			{
				string value = htmlEncode(s, minimal: false);
				output.Write(value);
			}
		}
		...
		public static string UrlDecode(byte[] bytes, Encoding encoding)
		{
			...
			byte[] bytes2 = urlDecodeToBytes(bytes, 0, num);
			return (encoding ?? Encoding.UTF8).GetString(bytes2);
		}
		...
		public static string UrlDecode(string s, Encoding encoding)
		{
			...
			byte[] bytes2 = urlDecodeToBytes(bytes, 0, bytes.Length);
			return (encoding ?? Encoding.UTF8).GetString(bytes2);
		}
		...
		public static string UrlDecode(byte[] bytes, int offset, int count, Encoding encoding)
		{
			...
			byte[] bytes2 = urlDecodeToBytes(bytes, offset, count);
			return (encoding ?? Encoding.UTF8).GetString(bytes2);
		}
		...
		public static string UrlEncode(byte[] bytes)
		{
			...
			byte[] bytes2 = urlEncodeToBytes(bytes, 0, num);
		...
		public static string UrlEncode(string s, Encoding encoding)
		{
			...
			byte[] bytes3 = urlEncodeToBytes(bytes, 0, bytes2);
		...
		public static string UrlEncode(byte[] bytes, int offset, int count)
		{
			...
			byte[] bytes2 = urlEncodeToBytes(bytes, offset, count);
			return Encoding.ASCII.GetString(bytes2);
		}
