...
namespace WebSocketSharp.Net
{
	...
	public class CookieCollection : ICollection<Cookie>, IEnumerable<Cookie>, IEnumerable
	{
			...
			for (int i = 0; i < list.Count; i++)
			{
				...
				else if (text2.Equals("expires", comparisonType))
				{
					...
					if (result != null && !(result.Expires != DateTime.MinValue))
					{
						...
						string[] formats = new string[2] { "ddd, dd'-'MMM'-'yyyy HH':'mm':'ss 'GMT'", "r" };
						CultureInfo provider = CultureInfo.CreateSpecificCulture("en-US");
						DateTimeStyles style = DateTimeStyles.AdjustToUniversal | DateTimeStyles.AssumeUniversal;
						if (DateTime.TryParseExact(s2, formats, provider, style, out var result3))
						{
				...
				else if (text2.Equals("max-age", comparisonType))
				{
					if (result != null && text3.Length != 0)
					{
						string s3 = text3.Unquote();
						if (int.TryParse(s3, out var result4))
						{
		...
		public void CopyTo(Cookie[] array, int index)
		{
			...
			if (index < 0)
			{
				string message = "Less than zero.";
				throw new ArgumentOutOfRangeException("index", message);
			}
			if (array.Length - index < _list.Count)
			{
				string message2 = "The available space of the array is not enough to copy to.";
