	...
	internal class ResponseStream : Stream
	{
		...
		private void flushBody(bool closing)
		{
			using (_bodyBuffer)
			{
				...
				{
					byte[] buffer = _bodyBuffer.GetBuffer();
					_writeBody(buffer, 0, (int)length);
				}
			}
			if (!closing)
			{
		...
		private bool flushHeaders()
		{
			...
			{
				streamWriter.Write(_response.StatusLine);
				string value = fullHeaders.ToStringMultiValue(response: true);
		...
		{
			byte[] chunkSizeStringAsBytes = getChunkSizeStringAsBytes(count);
			_innerStream.Write(chunkSizeStringAsBytes, 0, chunkSizeStringAsBytes.Length);
