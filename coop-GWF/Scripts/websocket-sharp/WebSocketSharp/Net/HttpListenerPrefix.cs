...
namespace WebSocketSharp.Net
{
	internal sealed class HttpListenerPrefix
	{
		...
		private void parse(string uriPrefix)
		{
			...
			_isSecure = uriPrefix.StartsWith("https", comparisonType);
			...
			_prefix = string.Format(format, _scheme, _host, _port, _path);
		}
		...
		public static void CheckPrefix(string uriPrefix)
		{
			...
			StringComparison comparisonType = StringComparison.Ordinal;
			if (!uriPrefix.StartsWith("http://", comparisonType) && !uriPrefix.StartsWith("https://", comparisonType))
			{
			...
			if (uriPrefix[num] != '/')
			{
				string message3 = "It ends without a forward slash.";
