...
namespace WebSocketSharp.Server
{
	public class WebSocketServer
	{
		...
		public WebSocketServer(IPAddress address, int port, bool secure)
		{
			...
			if (!address.IsLocal())
			{
				string message = "Not a local IP address.";
				throw new ArgumentException(message, "address");
			}
			if (!port.IsPortNumber())
			{
				string message2 = "Less than 1 or greater than 65535.";
		...
		{
			return !_isDnsStyle || Uri.CheckHostName(name) != UriHostNameType.Dns || name == _hostname;
		}
			...
			while (true)
			{
				...
				try
				{
					...
					ThreadPool.QueueUserWorkItem(delegate
					{
						try
						{
							TcpListenerWebSocketContext context = new TcpListenerWebSocketContext(cl, null, _isSecure, _sslConfigInUse, _log);
