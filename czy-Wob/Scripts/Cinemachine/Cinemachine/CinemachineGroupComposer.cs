	...
	public class CinemachineGroupComposer : CinemachineComposer
	{
		...
		public override void MutateCameraState(ref CameraState curState, float deltaTime)
		{
			...
			else
			{
				...
				if (flag)
				{
					float value = Mathf.Clamp(z + num / (2f * Mathf.Tan(curState.Lens.FieldOfView * ((float)Math.PI / 180f) / 2f)), z + m_MinimumDistance, z + m_MaximumDistance) - Vector3.Distance(curState.RawPosition, position);
		...
		private float GetTargetHeight(Vector2 boundsSize)
		{
			...
			case FramingMode.Horizontal:
				return Mathf.Max(0.0001f, boundsSize.x) / base.VcamState.Lens.Aspect;
			case FramingMode.Vertical:
				return Mathf.Max(0.0001f, boundsSize.y);
			default:
				return Mathf.Max(Mathf.Max(0.0001f, boundsSize.x) / base.VcamState.Lens.Aspect, Mathf.Max(0.0001f, boundsSize.y));
			}
		}
		...
		private static Bounds GetScreenSpaceGroupBoundingBox(ICinemachineTargetGroup group, Matrix4x4 observer, out Vector3 newFwd)
		{
			...
			Vector2 lhs = (maxAngles - vector) * ((float)Math.PI / 180f);
