...
namespace Cinemachine
{
	...
	public class CinemachineCollider : CinemachineExtension
	{
		...
		public bool m_IgnoreTarget = true;

		public List<Transform> m_ObjectsToIgnore = new List<Transform>();
		...
		private bool RaycastIgnoreTag(Ray ray, out RaycastHit hitInfo, float rayLength, int layerMask)
		{
			...
			while (Physics.Raycast(ray, out hitInfo, rayLength, layerMask, QueryTriggerInteraction.Ignore))
			{
				...
				if ((!(component != null) || component.enabled) && (!m_IgnoreTarget || (!(hitInfo.transform.root == base.VirtualCamera.Follow.transform.root) && !(hitInfo.transform.root == base.VirtualCamera.LookAt.transform.root))) && (m_ObjectsToIgnore.Count <= 0 || !m_ObjectsToIgnore.Contains(hitInfo.transform.root)) && (m_IgnoreTag.Length == 0 || !hitInfo.collider.CompareTag(m_IgnoreTag)))
				{
