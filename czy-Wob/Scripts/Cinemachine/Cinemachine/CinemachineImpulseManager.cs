		...
		public class ImpulseEvent
		{
			...
			public bool Expired
			{
				get
				{
					...
					if (duration > 0f)
					{
						return m_StartTime + duration <= Instance.CurrentTime;
					}
					return false;
				}
			}
			...
			public float DistanceDecay(float distance)
			{
				...
				case DissipationMode.SoftDecay:
					return 0.5f * (1f + Mathf.Cos((float)Math.PI * (distance / m_DissipationDistance)));
			...
			public bool GetDecayedSignal(Vector3 listenerPosition, bool use2D, out Vector3 pos, out Quaternion rot)
			{
				...
				{
					float num = Instance.CurrentTime - m_StartTime;
					...
					if (m_DirectionMode == DirectionMode.RotateTowardSource && num2 > 0.0001f)
					{
						...
						if (m_Radius > 0.0001f)
						{
							...
							quaternion = Quaternion.Slerp(quaternion, Quaternion.identity, Mathf.Cos((float)Math.PI * num4 / 2f));
						}
						pos = quaternion * pos;
