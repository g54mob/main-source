...
public class DogPenPane : MonoBehaviour
{
	...
	public void Load(ScalableUIContainer.LoadCallback loadCallback)
	{
		...
		dogPenBoxes.GetComponent<DogPenBoxes>().Preload();
	...
	public void ToggleScrollUp(bool toggleVal)
	{
		if (toggleVal)
		{
			...
			if (component == null)
			{
				...
				component.SetClickCallbacks(dogPenBoxes.GetComponent<DogPenBoxes>().ScrollUp);
			}
		}
		else
		{
	...
	public void ToggleScrollDown(bool toggleVal)
	{
		if (toggleVal)
		{
			...
			if (component == null)
			{
				...
				component.SetClickCallbackTime(Clickable.CallbackTime.CLICK_END);
				component.SetClickCallbacks(dogPenBoxes.GetComponent<DogPenBoxes>().ScrollDown);
			}
		}
		else
		{
	...
	{
		dogPenBoxes.GetComponent<DogPenBoxes>().Load(OnElementLoaded);
	}

	private void UnloadBoxes()
	{
		dogPenBoxes.GetComponent<DogPenBoxes>().Unload(OnElementUnloaded);
	}
	...
	public void SetScrollInteractability(bool val)
	{
		for (int i = 0; i < scrollList.Count; i++)
		{
			scrollList[i].GetComponent<Clickable>().enabled = val;
		}
	}
	...
	private void OnLoadComplete()
	{
		callback();
		callback = null;
		dogPenBoxes.GetComponent<DogPenBoxes>().UpdateScrolling();
	}
