...
namespace Poly2Tri
{
	public class Point2DList : IEnumerable<Point2D>, IEnumerable, IList<Point2D>, ICollection<Point2D>
	{
		...
		public static readonly double kAngularSlop = 1.0 / (720.0 * Math.PI);
		...
		public bool IsConvex()
		{
			...
			for (int i = 0; i < Count; i++)
			{
				...
				bool flag2 = ((num * num4 - num3 * num2 >= 0.0) ? true : false);
				if (i == 0)
				{
					flag = flag2;
				}
		...
		public void MergeParallelEdges(double tolerance)
		{
			...
			for (int j = 0; j < point2DList.Count; j++)
			{
				if (!array[j] && num != 0 && num10 != num)
				{
					if (num10 >= num)
					{
						throw new Exception("Point2DList::MergeParallelEdges - currIndex[ " + num10.ToString() + "] >= newNVertices[" + num + "]");
					}
