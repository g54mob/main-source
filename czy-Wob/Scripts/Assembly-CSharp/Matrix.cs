...
public class Matrix
{
	...
	{
		string[] array = Regex.Split(NormalizeMatrixString(ps), "\r\n");
	...
	private static Matrix StrassenMultiply(Matrix A, Matrix B)
	{
		...
		int num = Math.Max(Math.Max(A.rows, A.cols), Math.Max(B.rows, B.cols));
		Matrix matrix;
		if (num < 32)
		{
		...
		{
			int num5 = (int)Math.Pow(2.0, num3 - l - 1);
			for (int m = 0; m < 9; m++)
			{
		...
		for (int n = 0; n < Math.Min(num4, matrix.rows); n++)
		{
			for (int num6 = 0; num6 < Math.Min(num4, matrix.cols); num6++)
			{
				matrix[n, num6] = array[0, 2][n, num6] + array[0, 5][n, num6] - array[0, 6][n, num6] + array[0, 8][n, num6];
			}
		}
		for (int num7 = 0; num7 < Math.Min(num4, matrix.rows); num7++)
		{
			for (int num8 = num4; num8 < Math.Min(2 * num4, matrix.cols); num8++)
			{
				matrix[num7, num8] = array[0, 4][num7, num8 - num4] + array[0, 6][num7, num8 - num4];
			}
		}
		for (int num9 = num4; num9 < Math.Min(2 * num4, matrix.rows); num9++)
		{
			for (int num10 = 0; num10 < Math.Min(num4, matrix.cols); num10++)
			{
				matrix[num9, num10] = array[0, 3][num9 - num4, num10] + array[0, 5][num9 - num4, num10];
			}
		}
		for (int num11 = num4; num11 < Math.Min(2 * num4, matrix.rows); num11++)
		{
			for (int num12 = num4; num12 < Math.Min(2 * num4, matrix.cols); num12++)
			{
