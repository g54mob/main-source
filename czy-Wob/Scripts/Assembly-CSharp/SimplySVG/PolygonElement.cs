...
namespace SimplySVG
{
	public class PolygonElement : GraphicalElement
	{
		...
		public override bool AddShapeAttribute(string attributeName, string attributeValue)
		{
			...
			{
				throw new Exception("Failed to parse Polygon attribute " + attributeName + " with value " + attributeValue);
			}
			return true;
		}
		...
		protected override List<ContourPath> BuildShape(ImportSettings options)
		{
			...
			{
				new ContourPath(closed: true, points.GetRange(0, points.Count))
			};
		}
