...
namespace InControl
{
	public class PlayerAction : OneAxisInputControl
	{
		...
		public void ClearBindings(BindingSourceType sourceType)
		{
			for (int num = regularBindings.Count - 1; num >= 0; num--)
			{
				if (regularBindings[num].BindingSourceType == sourceType)
				{
					regularBindings.RemoveAt(num);
				}
			}
			for (int num2 = visibleBindings.Count - 1; num2 >= 0; num2--)
			{
				if (visibleBindings[num2].BindingSourceType == sourceType)
				{
					visibleBindings.RemoveAt(num2);
				}
			}
			triggerBindingChanged = true;
		}

		public bool HasAnyDefaultBinding()
		{
			return defaultBindings.Count > 0;
		}

		public List<BindingSource> GetAllCustomBindings()
		{
			List<BindingSource> list = new List<BindingSource>();
			for (int i = 0; i < visibleBindings.Count; i++)
			{
				bool flag = false;
				for (int j = 0; j < defaultBindings.Count; j++)
				{
					if (visibleBindings[i] == defaultBindings[j])
					{
						flag = true;
						break;
					}
				...
				if (!flag)
				{
					list.Add(visibleBindings[i]);
				}
			}
			return list;
		}

		public bool IsBindingTypeCleared(BindingSourceType typeRef)
		{
			bool flag = false;
			for (int i = 0; i < defaultBindings.Count; i++)
			{
				if (defaultBindings[i].BindingSourceType == typeRef)
				{
					flag = true;
					break;
				}
			...
			for (int j = 0; j < visibleBindings.Count; j++)
			{
				if (visibleBindings[j].BindingSourceType == typeRef)
				{
					return false;
				}
			}
		...
		public bool IsSetToDefaultBindings(BindingSourceType sourceType)
		{
			int num = 0;
			int num2 = 0;
			for (int i = 0; i < defaultBindings.Count; i++)
			{
				if (sourceType != BindingSourceType.None && defaultBindings[i].BindingSourceType != sourceType)
				{
					continue;
				}
				num++;
				bool flag = false;
				for (int j = 0; j < regularBindings.Count; j++)
				{
					if (regularBindings[j] == defaultBindings[i])
					{
						flag = true;
						break;
					}
			...
			for (int k = 0; k < regularBindings.Count; k++)
			{
				if (sourceType == BindingSourceType.None || regularBindings[k].BindingSourceType == sourceType)
				{
					num2++;
				}
			}
		...
		public void ResetBindings(BindingSourceType bindingSource)
		{
			ClearBindings(bindingSource);
			for (int i = 0; i < defaultBindings.Count; i++)
			{
				if (defaultBindings[i].BindingSourceType == bindingSource)
				{
					regularBindings.Add(defaultBindings[i]);
				}
			}
			int count = regularBindings.Count;
			for (int j = 0; j < count; j++)
			{
				BindingSource bindingSource2 = regularBindings[j];
				bindingSource2.BoundTo = this;
				if (bindingSource2.IsValid)
				{
					visibleBindings.Add(bindingSource2);
				}
			}
			triggerBindingChanged = true;
		}
		...
		{
			(ListenOptions ?? Owner.ListenOptions).ListenOnlyForBindingSourceType = false;
			ListenForBindingReplacing(null);
		}

		public void ListenForBinding(BindingSourceType sourceType)
		{
			BindingListenOptions obj = ListenOptions ?? Owner.ListenOptions;
			obj.ListenOnlyForBindingSourceType = true;
			obj.ListenForBindingSourceType = sourceType;
			ListenForBindingReplacing(null);
		}
		...
		private void DetectBindings()
		{
			...
			if (bindingListenOptions.ListenOnlyForBindingSourceType && bindingSource.BindingSourceType != bindingListenOptions.ListenForBindingSourceType)
			{
				bindingListenOptions.CallOnBindingRejected(this, bindingSource, BindingSourceRejectionType.IncorrectSourceType);
			}
			else
			{
