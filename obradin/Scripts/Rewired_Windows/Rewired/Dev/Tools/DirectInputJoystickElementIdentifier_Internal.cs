...
namespace Rewired.Dev.Tools
{
	internal sealed class DirectInputJoystickElementIdentifier_Internal : IElementIdentifierTool
	{
		...
		private hCkwWPjbZHHQuLPwssAiovZoKVX PSZKcVVfVmWuwyrmRaPnqSTTRBB;

		private Guid deviceInstanceGuid;

		private IList<rrkiWNHnEkzBYEXAvbDAWsEtjKd> TrlMVMJnBEgzUCjZPEEWgkjzDUkL;

		private IList<rrkiWNHnEkzBYEXAvbDAWsEtjKd> VQkWWQYcJVRbcBsWaSxsFRAHwlN;
		...
		public void Start()
		{
			...
			fTUmdlBklWTlvmAQYJjrEkdriSN = inputSourceWrapper.source;
		...
		public void Update()
		{
			...
			if (showAllDevices)
			{
				...
				foreach (rrkiWNHnEkzBYEXAvbDAWsEtjKd item in VQkWWQYcJVRbcBsWaSxsFRAHwlN)
				{
					Rewired.Internal.GUIText gUIText = this.text;
					object obj = gUIText.text;
					gUIText.text = string.Concat(obj, item.QuJoTfbshHjBObeWWlxFfdNiDKOz, ", ", item.IsHumanInterfaceDevice, ", ", new PidVid(item.vkIvCPKRwSgZgrfacRXklZVOEQA), ", ", item.Subtype, ", ", item.MtVcQGDaERwZYukISEeCDevOehH, ", ", item.ChVuJmqNQNkcCxPIAUfevOTNKRb, "\n");
				}
				this.text.text += "\n";
			}
			...
			{
				int deviceCount = fTUmdlBklWTlvmAQYJjrEkdriSN.GetDeviceCount(FTxufsFYYZLjZuOhPwjajrbMvoj.xQWouGqblDuyKscTMUlBgNchQZH, jQHcAazllAfioxyWFvAxWLMcwIf.ixnfIgjplOyvFHABieLYYzTzQUKU);
			...
			deviceInstanceGuid = TrlMVMJnBEgzUCjZPEEWgkjzDUkL[currentDeviceId].ubuqShzBecTPwZVAjKSSKhWTpPt;
			bool flag = false;
			...
			if (PSZKcVVfVmWuwyrmRaPnqSTTRBB == null || flag)
			{
				...
				{
					PSZKcVVfVmWuwyrmRaPnqSTTRBB.ZrHcJGgwwvDxGfSwHIvyriZRodVX();
				}
				PSZKcVVfVmWuwyrmRaPnqSTTRBB = new hCkwWPjbZHHQuLPwssAiovZoKVX(fTUmdlBklWTlvmAQYJjrEkdriSN, TrlMVMJnBEgzUCjZPEEWgkjzDUkL[currentDeviceId].ubuqShzBecTPwZVAjKSSKhWTpPt);
				if (PSZKcVVfVmWuwyrmRaPnqSTTRBB == null)
				{
					return;
				}
				IList<CwPulMNvQcCYLBIDFFYYMQMiYz> list = PSZKcVVfVmWuwyrmRaPnqSTTRBB.PJEcNGtbdKAvVzrSRMQRmtkgwNI();
				if (list != null)
				{
					for (int i = 0; i < list.Count; i++)
					{
						if ((list[i].PWmmnMBfsgZpBwBOPUTmjzcqGva.Flags & kwDQWJxJqLfGHqZNmyNdWGtdcVI.RnvlqNwbJnzAfbpYrjOARqAZFyM) != kwDQWJxJqLfGHqZNmyNdWGtdcVI.xQWouGqblDuyKscTMUlBgNchQZH)
						{
							PSZKcVVfVmWuwyrmRaPnqSTTRBB.Properties.Range = new TJtsekndLtkarLCycggFrDgcNKP(-65535, 65535);
						}
					}
				}
				PSZKcVVfVmWuwyrmRaPnqSTTRBB.MuhNEKLWnOVbFFtlAfKRHODWgpV();
			}
			POJetljaOyAGVgWkwdAAfDSJZTf pOJetljaOyAGVgWkwdAAfDSJZTf;
			try
			{
				pOJetljaOyAGVgWkwdAAfDSJZTf = PSZKcVVfVmWuwyrmRaPnqSTTRBB.xMyhUWueyTteqTQQKpHARcqPAoY();
			}
			catch
			{
				pOJetljaOyAGVgWkwdAAfDSJZTf = null;
			}
			if (pOJetljaOyAGVgWkwdAAfDSJZTf == null)
			{
				return;
			}
			if (num2 > 0)
			...
			{
				textBuffer = textBuffer + TrlMVMJnBEgzUCjZPEEWgkjzDUkL[j].QuJoTfbshHjBObeWWlxFfdNiDKOz + "\n";
			}
			textBuffer += "\n";
			object obj3 = textBuffer;
			textBuffer = string.Concat(obj3, "Current DI device ", currentDeviceId, ": ", TrlMVMJnBEgzUCjZPEEWgkjzDUkL[currentDeviceId].QuJoTfbshHjBObeWWlxFfdNiDKOz, "\n");
			textBuffer += "(Press + or - to change monitored device id.)\n\n";
			Log("Identifier", new PidVid(PSZKcVVfVmWuwyrmRaPnqSTTRBB.Information.vkIvCPKRwSgZgrfacRXklZVOEQA));
			Log("Instance GUID", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Information.ubuqShzBecTPwZVAjKSSKhWTpPt);
			Log("Product Id", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Properties.ProductId);
			Log("Device Type", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Capabilities.Type.ToString());
			textBuffer += "\n";
			Log("Axis Count", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Capabilities.bqtKAqSxAowWoUniiCjhuhBfRVu);
			Log("Button Count", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Capabilities.rQrTdoWPDpDHrLHNnouynDuUHKW);
			Log("Hat Count", PSZKcVVfVmWuwyrmRaPnqSTTRBB.Capabilities.CjrQIVLjQiXDtUXifPPiqmKRmmG);
			textBuffer += "\n";
			if (flag)
			{
				Logger.Log("Device Name: \"" + TrlMVMJnBEgzUCjZPEEWgkjzDUkL[currentDeviceId].QuJoTfbshHjBObeWWlxFfdNiDKOz + "\"");
				Logger.Log("Identifier: " + new PidVid(PSZKcVVfVmWuwyrmRaPnqSTTRBB.Information.vkIvCPKRwSgZgrfacRXklZVOEQA));
			}
			for (int k = 0; k < 32; k++)
			{
				int value = QkOJeQjNoGuvJJcCjzkxhFnepjH((DirectInputAxis)k, pOJetljaOyAGVgWkwdAAfDSJZTf);
				string key = ((DirectInputAxis)k).ToString();
				Log(key, value + " (" + NormalizeAxis(value) + ")");
			}
			int[] pointOfViewControllers = pOJetljaOyAGVgWkwdAAfDSJZTf.PointOfViewControllers;
			for (int l = 0; l < 4; l++)
			{
			...
			bool[] buttons = pOJetljaOyAGVgWkwdAAfDSJZTf.Buttons;
			string text = "";
			for (int m = 0; m < 128; m++)
			{
				if (buttons[m])
		...
		{
			TrlMVMJnBEgzUCjZPEEWgkjzDUkL = fTUmdlBklWTlvmAQYJjrEkdriSN.GetDevices(FTxufsFYYZLjZuOhPwjajrbMvoj.MRqcqejOtASJsQJRNVwXMlVPvNWT, jQHcAazllAfioxyWFvAxWLMcwIf.ixnfIgjplOyvFHABieLYYzTzQUKU);
			VQkWWQYcJVRbcBsWaSxsFRAHwlN = fTUmdlBklWTlvmAQYJjrEkdriSN.GetDevices(FTxufsFYYZLjZuOhPwjajrbMvoj.xQWouGqblDuyKscTMUlBgNchQZH, jQHcAazllAfioxyWFvAxWLMcwIf.ixnfIgjplOyvFHABieLYYzTzQUKU);
			rawDeviceCount = ((VQkWWQYcJVRbcBsWaSxsFRAHwlN != null) ? VQkWWQYcJVRbcBsWaSxsFRAHwlN.Count : 0);
		}
		...
		private void Clear()
		{
			...
			VQkWWQYcJVRbcBsWaSxsFRAHwlN = null;
		...
		private int QkOJeQjNoGuvJJcCjzkxhFnepjH(DirectInputAxis P_0, POJetljaOyAGVgWkwdAAfDSJZTf P_1)
		{
			switch (P_0)
			{
