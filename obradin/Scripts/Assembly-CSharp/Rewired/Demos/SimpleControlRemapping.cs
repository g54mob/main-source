...
namespace Rewired.Demos
{
	...
	public class SimpleControlRemapping : MonoBehaviour
	{
		...
		private Controller controller
		{
			get
			{
				return player.controllers.GetController(selectedControllerType, selectedControllerId);
			}
		}
		...
		private void InitializeUI()
		{
			...
			foreach (InputAction item3 in ReInput.mapping.ActionsInCategory("Default"))
			{
				if (item3.type == InputActionType.Axis)
				{
					...
					CreateUIRow(item3, AxisRange.Positive, string.IsNullOrEmpty(item3.positiveDescriptiveName) ? (item3.descriptiveName + " +") : item3.positiveDescriptiveName);
					CreateUIRow(item3, AxisRange.Negative, string.IsNullOrEmpty(item3.negativeDescriptiveName) ? (item3.descriptiveName + " -") : item3.negativeDescriptiveName);
				}
