...
public class Tower : MonoBehaviour, IBuildable
{
	...
	public virtual void SetStats()
	{
		...
		rps = 60f / rpm;
		range = baseRange + (float)heightBonus / 2f * (1f + TowerManager.instance.GetHeightRangeBonusMultiplier(towerType)) + TowerManager.instance.GetBonusRange(towerType);
	...
	{
		GameManager.instance.prioritiesClipboard = new Vector3Int((int)priorities[0], (int)priorities[1], (int)priorities[2]);
	}

	public void PastePriorities()
	{
		priorities[0] = (Priority)GameManager.instance.prioritiesClipboard.x;
		priorities[1] = (Priority)GameManager.instance.prioritiesClipboard.y;
		priorities[2] = (Priority)GameManager.instance.prioritiesClipboard.z;
	}
