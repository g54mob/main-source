...
public class ProjectileLine : MonoBehaviour
{
	...
	private void SetPoint(Vector2 point)
	{
		...
		for (int i = 0; i < points.Count; i++)
		{
			...
			float num2 = Mathf.Atan(Mathf.Abs(vector2.y - vector.y) / Mathf.Abs(vector2.x - vector.x));
			num2 = MathF.PI / 2f - num2;
			Vector2 vector3 = num * new Vector2(Mathf.Cos(num2), Mathf.Sin(num2));
			Vector2 item;
			Vector2 item2;
	...
	{
		StartCoroutine(WhiteFlash(this, line.startColor));
		StartCoroutine(Fader(this));
	}

	private IEnumerator WhiteFlash(ProjectileLine line, Color color)
	{
		_ = line.line.startColor;
		LineRenderer lineRenderer = line.line;
		Color endColor = (line.line.startColor = Color.white);
		lineRenderer.endColor = endColor;
		yield return Dungeon.Wait(5);
		LineRenderer lineRenderer2 = line.line;
		endColor = (line.line.startColor = color);
		lineRenderer2.endColor = endColor;
	}

	public IEnumerator Fader(ProjectileLine line, int f = 15)
	{
		float FRAMES = f;
