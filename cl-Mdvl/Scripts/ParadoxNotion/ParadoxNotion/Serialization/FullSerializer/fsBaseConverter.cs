...
namespace ParadoxNotion.Serialization.FullSerializer
{
	public abstract class fsBaseConverter
	{
		...
		public virtual bool RequestCycleSupport(Type storageType)
		{
			...
			if (!storageType.IsClass)
			{
				return storageType.IsInterface;
			}
			return true;
		}
		...
		{
			fsData data2;
			fsResult result = Serializer.TrySerialize(typeof(T), value, out data2, overrideConverterType);
			if (result.Succeeded)
			{
				data[name] = data2;
			}
		...
		protected fsResult DeserializeMember<T>(Dictionary<string, fsData> data, Type overrideConverterType, string name, out T value)
		{
			...
			object result = null;
			fsResult result2 = Serializer.TryDeserialize(value2, typeof(T), ref result, overrideConverterType);
			value = (T)result;
			return result2;
		}
	}
