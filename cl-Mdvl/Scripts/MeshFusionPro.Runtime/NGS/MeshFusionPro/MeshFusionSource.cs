...
namespace NGS.MeshFusionPro
{
	...
	public abstract class MeshFusionSource : MonoBehaviour
	{
		...
		private bool _firstCombineDone;
		...
		[field: SerializeField]
		public bool SkipLayerHide { get; set; }
		...
		[field: SerializeField]
		[field: Tooltip("If true, ShadowCastingModeValue will be set on all the renderers after combine is fininshed.")]
		public bool SetShadowCastingMode { get; set; } = true;

		[field: SerializeField]
		[field: Tooltip("ShadowCastingMode that will be set on all the renderers after combine is fininshed.")]
		public ShadowCastingMode ShadowCastingModeValue { get; set; }

		public event Action<MeshFusionSource, IEnumerable<ICombinedObjectPart>> onCombineFinished;

		public event Action<MeshFusionSource, IEnumerable<ICombinedObjectPart>> onFirstCombineFinished;
		...
		private void OnDestroy()
		{
			...
			onCombineFinishedUnityEvent = null;
			this.onCombineFinished = null;
			this.onFirstCombineFinished = null;
		}
		...
		private void OnCombineFinishedHandler()
		{
			...
			IEnumerable<ICombinedObjectPart> combinedParts = GetCombinedParts();
			this.onCombineFinished?.Invoke(this, combinedParts);
			if (!_firstCombineDone)
			{
				_firstCombineDone = true;
				this.onFirstCombineFinished?.Invoke(this, combinedParts);
			}
			onCombineFinishedUnityEvent?.Invoke(this, combinedParts);
