...
namespace NGS.MeshFusionPro
{
	public class MeshDataNativeArraysSTD : CombinedMeshDataInternal
	{
		...
		public NativeArray<Vector3> Vertices => _vertices.NativeListToArray();

		public NativeArray<Vector3> Normals => _normals.NativeListToArray();

		public NativeArray<Vector4> Tangents => _tangents.NativeListToArray();

		public NativeArray<Color32> Colors => _colors.NativeListToArray();

		public NativeArray<Bounds> PartsBounds => _partsBounds.NativeListToArray();

		public NativeArray<Bounds> PartsBoundsLocal => _partsBoundsLocal.NativeListToArray();
		...
		protected override void OnInitialized()
		{
			...
			_colors = new NativeList<Color32>(Allocator.Persistent);
		...
		protected override void OnMeshUpdated()
		{
			...
			_colors.ResizeUninitialized(vertexCount);
			using (Mesh.MeshDataArray meshDataArray = Mesh.AcquireReadOnlyMeshData(mesh))
			{
				Mesh.MeshData meshData = meshDataArray[0];
				if (meshData.vertexCount > 0)
				{
					meshData.GetVertices(_vertices.NativeListToArray());
					meshData.GetNormals(_normals.NativeListToArray());
					meshData.GetTangents(_tangents.NativeListToArray());
					meshData.GetColors(_colors.NativeListToArray());
				}
			}
