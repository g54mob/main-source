...
namespace Epic.OnlineServices
{
	public sealed class Helper
	{
		...
		private static IntPtr AddPinnedBuffer(Utf8String str)
		{
			...
			{
				GCHandle value = GCHandle.Alloc(str.Bytes, GCHandleType.Pinned);
				IntPtr intPtr = Marshal.UnsafeAddrOfPinnedArrayElement(str.Bytes, 0);
				s_PinnedBuffers.Add(intPtr, value);
				return intPtr;
			}
		}
		...
		internal static IntPtr AddPinnedBuffer(ArraySegment<byte> array)
		{
			...
			lock (s_PinnedBuffers)
			{
				GCHandle value = GCHandle.Alloc(array.Array, GCHandleType.Pinned);
				IntPtr intPtr = Marshal.UnsafeAddrOfPinnedArrayElement(array.Array, array.Offset);
				s_PinnedBuffers.Add(intPtr, value);
				return intPtr;
			}
		}
