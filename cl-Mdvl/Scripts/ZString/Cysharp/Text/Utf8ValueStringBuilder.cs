...
namespace Cysharp.Text
{
	public struct Utf8ValueStringBuilder : IDisposable, IBufferWriter<byte>, IResettableBufferWriter<byte>
	{
		...
		private static object CreateFormatter(Type type)
		{
			...
			{
				return (TryFormat<IntPtr>)delegate(IntPtr x, Span<byte> dest, out int written, StandardFormat _)
				{
					return (IntPtr.Size != 4) ? Utf8Formatter.TryFormat(x.ToInt64(), dest, out written) : Utf8Formatter.TryFormat(x.ToInt32(), dest, out written);
				};
			}
			if (type == typeof(UIntPtr))
			{
				return (TryFormat<UIntPtr>)delegate(UIntPtr x, Span<byte> dest, out int written, StandardFormat _)
				{
					return (UIntPtr.Size != 4) ? Utf8Formatter.TryFormat(x.ToUInt64(), dest, out written) : Utf8Formatter.TryFormat(x.ToUInt32(), dest, out written);
				};
			}
			return null;
		}
