...
namespace Cysharp.Text
{
	public struct Utf16ValueStringBuilder : IDisposable, IBufferWriter<char>, IResettableBufferWriter<char>
	{
		...
		private static object CreateFormatter(Type type)
		{
			...
			{
				return (TryFormat<IntPtr>)delegate(IntPtr x, Span<char> dest, out int written, ReadOnlySpan<char> _)
				{
					return (IntPtr.Size != 4) ? x.ToInt64().TryFormat(dest, out written) : x.ToInt32().TryFormat(dest, out written);
				};
			}
			if (type == typeof(UIntPtr))
			{
				return (TryFormat<UIntPtr>)delegate(UIntPtr x, Span<char> dest, out int written, ReadOnlySpan<char> _)
				{
					return (UIntPtr.Size != 4) ? x.ToUInt64().TryFormat(dest, out written) : x.ToUInt32().TryFormat(dest, out written);
				};
			}
			return null;
		}
