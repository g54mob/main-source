...
{
	public class WorkerMoodExtraPanel : SelectionExtraPanelBase
	{
		[SerializeField]
		...
		private void GetMoodModifiers()
		{
			...
			foreach (ModifierInstanceStack item in base.Humanoid.Stats.GetModifierInstanceStack())
			{
				foreach (ModifierInstance instance in item.Instances)
				{
					...
					if (!(instance is MoodBasicModifierInstance moodBasicModifierInstance) || effector == null || !effector.UIGroup.HasFlag(EffectorUiGroup.Mood))
					{
						continue;
					}
					...
					{
						FVLogWarningInterpolationHandler messageBuilder = new FVLogWarningInterpolationHandler(58, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\UI\\View\\Selection\\WorkerMoodExtraPanel.cs");
						if (isEnabled)
						{
					...
					{
						FVLogWarningInterpolationHandler messageBuilder = new FVLogWarningInterpolationHandler(37, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\UI\\View\\Selection\\WorkerMoodExtraPanel.cs");
						if (isEnabled)
						{
					...
					{
						FVLogWarningInterpolationHandler messageBuilder = new FVLogWarningInterpolationHandler(60, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\UI\\View\\Selection\\WorkerMoodExtraPanel.cs");
						if (isEnabled)
						{
							messageBuilder.AppendLiteral("ActiveEffectorInfo: ");
							messageBuilder.AppendFormatted(key);
							messageBuilder.AppendLiteral(", is default and/or 'Blueprint' is null!");
						}
						Log.Warning(messageBuilder);
					}
					else
