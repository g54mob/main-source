...
{
	public class AnimalHealthExtraPanel : SelectionExtraPanelBase
	{
		[SerializeField]
		...
		private readonly List<FillBarLayoutItemView> animalStats = new List<FillBarLayoutItemView>();
		...
		{
			if (base.Animal == null || base.Animal.HasDisposed || base.Animal.Stats == null)
			{
				return;
			}
			int num = 0;
			foreach (StatType stat2 in stats)
			{
				...
				string localizedName = StatUtils.GetLocalizedName(stat.Blueprint, base.Animal.GetInfo().BodyType);
				if (stat2 == StatType.Health)
				{
					hitpointsBar.SetBasicData(localizedName, stat2.ToString(), string.Empty, string.Empty, StatUtils.GetTooltipLines(stat, base.Animal.Gender), StatUtils.GetStatTrend(stat), StatUtils.GetSliderValues(stat), StatUtils.GetThresholds(stat), null, StatUtils.Invert(stat), string.Empty);
					continue;
				}
				animalStats.GetAt(statsParent, num).SetBasicData(localizedName, stat2.ToString(), string.Empty, string.Empty, StatUtils.GetTooltipLines(stat, base.Animal.Gender), StatUtils.GetStatTrend(stat), StatUtils.GetSliderValues(stat), StatUtils.GetThresholds(stat), null, StatUtils.Invert(stat), string.Empty);
				num++;
			}
			animalStats.SetActiveFromIndex(num, active: false);
			List<WoundData> list = new List<WoundData>();
			int num2 = 0;
			foreach (ActiveEffectorInfo activeEffector in base.Animal.Stats.GetActiveEffectors())
			{
				...
				float timeLeft = WoundUtils.GetTimeLeft(base.Animal.Stats, activeEffector.WoundInfo);
				...
				{
					if (effectDetailsHolder.Type == EffectorType.AttributeAdderModify && effectDetailsHolder.Parameters.Any((KeyValuePair<string, string> parameter) => parameter.Key == "Attribute" && (parameter.Value == 59.ToString() || parameter.Value == AttributeType.BloodLoss.ToString())))
					{
						int.TryParse(effectDetailsHolder.Parameters["Attribute"], out var result);
						num3 += result;
					}
					if (effectDetailsHolder.Type != EffectorType.AttributeModify || !effectDetailsHolder.Parameters.TryGetValue("Attribute", out var value))
					{
						continue;
					}
					if (!value.TryParseEnumNameOrInt<AttributeType>(out var parsedEnumValue))
					{
						bool isEnabled;
						FVLogErrorInterpolationHandler messageBuilder = new FVLogErrorInterpolationHandler(33, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\UI\\View\\Selection\\AnimalHealthExtraPanel.cs");
						if (isEnabled)
						{
					...
					float result2 = 1f;
					if (effectDetailsHolder.Parameters.ContainsKey("Multiplier"))
					{
						float.TryParse(effectDetailsHolder.Parameters["Multiplier"], NumberStyles.Float, CultureInfo.InvariantCulture, out result2);
					}
			...
			int fromIndex = 0;
			if (num2 > 0 && base.Animal.AnimalType != AnimalType.Wild && base.Animal.AnimalType != AnimalType.WildAggressive)
			{
			...
			foreach (ActiveEffectorInfo activeEffector2 in base.Animal.Stats.Stats.First().Value.Owner.GetActiveEffectors())
			{
				...
				if (activeEffector2.Blueprint != null)
				{
					...
					{
						if ((effectDetailsHolder2.Type == EffectorType.AttributeModify || effectDetailsHolder2.Type == EffectorType.AttributeAdderModify) && effectDetailsHolder2.Parameters.TryGetValue("Attribute", out var value2))
						{
							dictionary[value2] = string.Empty;
							if (effectDetailsHolder2.Parameters.TryGetValue("Multiplier", out var value3))
							{
								dictionary[effectDetailsHolder2.Parameters["Attribute"]] = value3;
							}
							if (effectDetailsHolder2.Parameters.TryGetValue("Value", out var value4))
							{
								dictionary[effectDetailsHolder2.Parameters["Attribute"]] = value4;
							}
						}
					}
				}
			...
			for (int num5 = 0; num5 < list4.Count; num5++)
			{
				...
				healthEffectors.GetAt(effectorsParent, num5).SetStatData(data, base.Animal, num5);
			}
