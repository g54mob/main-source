...
{
	public class WorkerHealthExtraPanel : SelectionExtraPanelBase
	{
		[SerializeField]
		...
		protected override void SetupTabPanel()
		{
			...
			foreach (StatType stat2 in stats)
			{
				...
				if (stat.IsDisabled)
				{
					workerStats.GetAt(statsParent, num).SetDisabled(localizedName, stat2.ToString(), StatUtils.GetTooltipLines(stat, base.Humanoid.Info.BodyType));
				}
				else
				{
			...
			foreach (ActiveEffectorInfo activeEffector in base.Humanoid.Stats.GetActiveEffectors())
			{
				...
				{
					if (effectDetailsHolder.Type == EffectorType.AttributeAdderModify && effectDetailsHolder.Parameters.ContainsKey("Value") && effectDetailsHolder.Parameters.TryGetValue("Attribute", out var value) && (value == 59.ToString() || value == AttributeType.BloodLoss.ToString()) && effectDetailsHolder.Parameters.TryGetValue("Value", out var value2))
					{
						int.TryParse(value2, out var result);
						num3 += result;
					}
					if (effectDetailsHolder.Type != EffectorType.AttributeModify || !effectDetailsHolder.Parameters.TryGetValue("Attribute", out var value3))
					{
						continue;
					}
					if (!value3.TryParseEnumNameOrInt<AttributeType>(out var parsedEnumValue))
					{
						bool isEnabled;
						FVLogErrorInterpolationHandler messageBuilder = new FVLogErrorInterpolationHandler(33, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\UI\\View\\Selection\\WorkerHealthExtraPanel.cs");
						if (isEnabled)
						{
			...
			int fromIndex2 = 0;
			foreach (WoundData item3 in list)
			{
				woundEntries.GetAt(woundsParent, fromIndex2++).SetBasicData(item3.Name, item3.WoundSeverity, item3.Tended, item3.Bleeding, item3.TooltipData);
			}
			...
			{
				EffectorViewData data = list4[j];
				healthEffectors.GetAt(effectorsParent, j).SetStatData(data, base.Humanoid, j);
			}
