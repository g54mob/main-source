...
{
	public class EnemyStatsExtraPanel : SelectionExtraPanelBase
	{
		[SerializeField]
		...
		protected override void SetupTabPanel()
		{
			...
			foreach (StatType stat2 in stats)
			{
				...
				if (stat != null)
				{
					...
					num++;
					at.SetBasicData(StatUtils.GetLocalizedName(stat.Blueprint, base.Humanoid.Info.BodyType), stat2.ToString(), string.Empty, string.Empty, StatUtils.GetTooltipLines(stat, base.Humanoid.Info.BodyType), stat.StatTrend, StatUtils.GetSliderValues(stat), StatUtils.GetThresholds(stat), null, invertArrows: false, string.Empty);
				}
			}
