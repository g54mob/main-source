...
{
	public class CommandSetPet : ConsoleCommand
	{
		...
		public CommandSetPet()
		{
			Command = "setPetAnimal";
			Description = "Sets animal as a pet";
		...
		{
			if (!(obj == null) && obj.GetAsCreature() is AnimalInstance animalInstance && animalInstance.Blueprint.CanBeTrained)
			{
				animalInstance.Stats.GetStat(StatType.AnimalWild).SetCurrent(0f);
				animalInstance.Stats.GetStat(StatType.AnimalUntrained).SetCurrent(0f);
