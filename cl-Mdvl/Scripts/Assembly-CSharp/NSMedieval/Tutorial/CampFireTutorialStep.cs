...
{
	public class CampFireTutorialStep : TutorialStep
	{
		private readonly Vec3Int campFirePosition = new Vec3Int(98, 5, 94);

		public CampFireTutorialStep(string name, string info)
			: base(name, info)
		{
			Tasks = new List<TutorialStepTask>
			{
				new TutorialStepTask("tut_open_build_production"),
				new TutorialStepTask("tut_select_campfire"),
				new TutorialStepTask("tut_place_campfire"),
		...
		private void OnShowConstructionCategory(BuildingCategoryUI category)
		{
			...
			{
				CompleteTask(0);
				MonoSingleton<UIController>.Instance.ConstructionPanel.ConstructionManager.SetCategoriesInteractable(new HashSet<string> { "camp_fire" }, interactable: true);
				MonoSingleton<TaskController>.Instance.WaitForNextFrameUnscaled().Then(delegate
				{
					MonoSingleton<TutorialViewManager>.Instance.ShowHighlightRect(MonoSingleton<UIController>.Instance.ConstructionPanel.ConstructionManager.GetSubcategoryTransform("camp_fire"));
				});
			}
		}
		...
		private void OnChangeBuildingToPlace()
		{
			...
			MonoSingleton<UIController>.Instance.ConstructionPanel.ConstructionManager.SetSubCategoriesInteractable(new HashSet<string> { "camp_fire" }, interactable: true);
			MonoSingleton<TutorialViewManager>.Instance.ShowMarker(new Vec3Int(campFirePosition.x, 15, campFirePosition.z), new Vec3Int(campFirePosition.x, 15, campFirePosition.z));
			MonoSingleton<ScreenPointerManager>.Instance.AddTarget(GridUtils.GetWorldPosition(campFirePosition), Vector3.up, hideIfTargetOnScreen: true);
			MonoSingleton<ConstructionController>.Instance.ChangeBuildingTypeToPlaceEvent -= OnChangeBuildingToPlace;
		...
		{
			if (buildingInstance.BlueprintId != "camp_fire")
			{
				return;
			}
			FVLogTraceInterpolationHandler messageBuilder = new FVLogTraceInterpolationHandler(19, 1, out var isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
			if (isEnabled)
			{
				messageBuilder.AppendLiteral("Campfire placed at ");
			...
			{
				Vec3Int lhs = position;
				if (lhs != campFirePosition)
				{
					messageBuilder = new FVLogTraceInterpolationHandler(19, 2, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
					if (isEnabled)
					{
						messageBuilder.AppendFormatted(lhs.ToString());
						messageBuilder.AppendLiteral(" Outside Allowed (");
						messageBuilder.AppendFormatted(campFirePosition);
				...
				messageBuilder = new FVLogTraceInterpolationHandler(18, 2, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
				if (isEnabled)
				{
					messageBuilder.AppendFormatted(lhs.ToString());
			...
			MonoSingleton<TaskController>.Instance.WaitForNextFrameUnscaled().Then(delegate
			{
				...
				MonoSingleton<TutorialViewManager>.Instance.HideAllMarkers();
				MonoSingleton<ScreenPointerManager>.Instance.TryRemoveTarget(GridUtils.GetWorldPosition(campFirePosition));
				CompleteTask(2);
			});
		}
		...
		private void CheckAllBuildingsFinished()
		{
			...
			Log.Trace("Checking Campfire finished", "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
			float num = 0f;
			bool isEnabled;
			...
			foreach (Dictionary<BaseBuildingInstance, BaseBuildingViewComponent> value in base.BuildingsManagerMain.TypeInstanceView.Values)
			{
				...
				{
					if (key == null || key.BlueprintId != "camp_fire")
					{
						continue;
					}
					if (key.ConstructionPhase != ConstructionPhase.Finished)
					{
						...
						messageBuilder = new FVLogTraceInterpolationHandler(37, 3, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
						if (isEnabled)
						{
			...
			messageBuilder = new FVLogTraceInterpolationHandler(9, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\Tutorial\\CampFireTutorialStep.cs");
			if (isEnabled)
			{
