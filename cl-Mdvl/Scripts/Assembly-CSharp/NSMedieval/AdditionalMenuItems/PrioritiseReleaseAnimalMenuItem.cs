...
{
	public class PrioritiseReleaseAnimalMenuItem : AdditionalMenuPrioritiseItem
	{
		public PrioritiseReleaseAnimalMenuItem(IAdditionalMenuOwner owner)
			: base(owner, JobType.Animal)
		{
			...
			base.Text = MonoSingleton<LocalizationController>.Instance.GetText("prioritise_release");
		...
		protected override void OnClickCallback()
		{
			...
			if (base.Owner.GetAsTarget() is AnimalInstance animalInstance)
			{
				...
				if (selectedWorker?.GetGoapAgent() != null && !selectedWorker.HasFainted && !selectedWorker.HasDisposed)
				{
					...
					ForceGoal("ReleaseAnimalGoal", animalInstance);
				}
			}
		}
	}
