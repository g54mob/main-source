...
{
	public class BrokerRoleGoal : RoleGoal
	{
		public BrokerRoleGoal(Agent selfAgent)
			: base("BrokerRoleGoal", selfAgent, GoalInterruptMode.HigherPriority)
		{
			base.AllowedRoleId = "broker";
		}
		...
		private GoapAction GotoAction()
		{
			...
			goapAction.TriggerAnimation("BrokerWalk", ActionAnimationMode.Interrupt);
			goapAction.OnComplete = delegate(ActionCompletionStatus state)
			{
				...
				{
					bool isEnabled;
					FVLogDebugInterpolationHandler messageBuilder = new FVLogDebugInterpolationHandler(30, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\GOAP\\Goal\\BrokerRoleGoal.cs");
					if (isEnabled)
					{
		...
		private GoapAction RoleIdleAction()
		{
			...
			bool isEnabled;
			FVLogDebugInterpolationHandler messageBuilder = new FVLogDebugInterpolationHandler(32, 1, out isEnabled, "C:\\GIT\\dev\\Assets\\Scripts\\GOAP\\Goal\\BrokerRoleGoal.cs");
			if (isEnabled)
			{
			...
			Log.Debug(messageBuilder);
			goapAction.TriggerAnimation("BrokerIdle", ActionAnimationMode.Interrupt).CompleteAfterTimeExpires(num).FailAtCondition(base.RoleNotAssigned);
