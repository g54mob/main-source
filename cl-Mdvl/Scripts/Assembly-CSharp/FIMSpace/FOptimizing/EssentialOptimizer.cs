...
namespace FIMSpace.FOptimizing
{
	...
	{
		[HideInInspector]
		public EssentialOptimizerSelector OptimizationTypes;
		...
		{
			if (ToOptimize.Count > 0 && ToOptimize[0].ControlerType != EssentialLODsController.EEssType.Unknown && ToOptimize[0].GetLODSettingsCount() - 2 != LODLevels)
			{
		...
		public override void AssignCustomComponentToOptimize(MonoBehaviour target)
		{
			if (ToOptimize == null)
			{
				ToOptimize = new List<EssentialLODsController>();
			}
			...
			{
				if (EssentialLODsController.GetEssentialTypeAll(components[i]) != EssentialLODsController.EEssType.Unknown)
				{
