...
namespace FIMSpace.FLook
{
	...
	public class FLookAnimator : MonoBehaviour, IDropHandler, IEventSystemHandler, IFHierarchyIcon
	{
		...
		public class LookBone
		{
			...
			{
				if (targetLook.x == 0f && targetLook.y == 0f && targetLook.z == 0f && targetLook.w == 0f)
				{
					targetLook = Quaternion.identity;
				}
		...
		public enum EFFollowMode
		{
			...
			LocalOffset = 1,
			WorldOffset = 2,
			ToFollowSpaceOffset = 3,
			FollowJustPosition = 4
		}
		...
		public Vector2 AnimateAnglesTowards(Vector2 angles)
		{
			if (!usingAxisCorrection)
			{
				Vector3 v = BaseTransform.rotation.eulerAngles - lastBaseRotation.eulerAngles;
				v = WrapVector(v) * BaseRotationCompensation;
				animatedLookAngles -= v;
			}
			if (!instantRotation)
			{
		...
		internal void RefreshLookBones()
		{
			...
			else if (LookBones.Count > 1)
			{
				LookBones.RemoveRange(1, LookBones.Count);
			}
		}
