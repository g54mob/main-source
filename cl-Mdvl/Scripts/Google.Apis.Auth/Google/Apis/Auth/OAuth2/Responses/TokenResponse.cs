		...
		public static async Task<TokenResponse> FromHttpResponseAsync(HttpResponseMessage response, IClock clock, ILogger logger)
		{
			...
			try
			{
				if (!response.IsSuccessStatusCode)
				{
					...
					throw new TokenResponseException((requestMessage != null && requestMessage.RequestUri?.AbsoluteUri.StartsWith("https://iamcredentials.", StringComparison.Ordinal) == true) ? new TokenErrorResponse
					{
						Error = text
