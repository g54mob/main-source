	...
	public class ServiceAccountCredential : ServiceCredential, IOidcTokenProvider, IGoogleCredential, ICredential, IConfigurableHttpClientInitializer, ITokenAccess, ITokenAccessWithHeaders, IHttpExecuteInterceptor, IBlobSigner
	{
		public new class Initializer : ServiceCredential.Initializer
		{
			...
			public Initializer(string id, string tokenServerUrl)
				: base(tokenServerUrl ?? "https://oauth2.googleapis.com/token")
			{
				Id = id;
			}
			...
			: base(initializer)
		{
			...
			GoogleAuthConsts.CheckIsDefaultUniverseDomain(UniverseDomain, initializer.User != null, "Domain-wide delegation is not supported in universes other than googleapis.com.");
			GoogleAuthConsts.CheckIsDefaultUniverseDomain(UniverseDomain, !initializer.UseJwtAccessWithScopes, "Only self signed JWTs are supported in universes other than googleapis.com.");
		...
		{
			return Task.FromResult(UniverseDomain);
		}
		...
		{
			GoogleAuthConsts.CheckIsDefaultUniverseDomain(UniverseDomain, "Only self signed JWTs are supported in universes other than googleapis.com.");
		...
		{
			GoogleAuthConsts.CheckIsDefaultUniverseDomain(UniverseDomain, "ID tokens are not currently supported in universes other than googleapis.com.");
