...
namespace Aura2API
{
	public class VolumesCommonDataManager
	{
		...
		public bool HasRegisteredVolumes => RegisteredVolumesList.Count > 0;

		public bool HasRegisteredLightProbesProxyVolumes => RegisteredLightProbesProxyVolumesList.Count > 0;
		...
		private Texture2DArrayComposer Texture2DMasksAtlasComposer
		{
			get
			{
				...
				{
					_volumesTexture2DMasksArray = new Texture2DArrayComposer(texture2DMaskSize, texture2DMaskSize, TextureFormat.RGBA32, bypassSrgb: true);
				}
				return _volumesTexture2DMasksArray;
			}
		}

		public bool HasTexture2DMasks => Texture2DMasksAtlasComposer.HasTexture;

		public Texture2DArray Texture2DMasksAtlas => Texture2DMasksAtlasComposer.ArrayTexture;
		...
		public bool HasTexture3DMasks => Texture3DMasksAtlasComposer.HasVolumeTexture;

		public Texture3D Texture3DMasksAtlas => Texture3DMasksAtlasComposer.VolumeTexture;
		...
		public void RegisterVolume(AuraVolume auraVolume)
		{
			if (!RegisteredVolumesList.Contains(auraVolume))
			{
				...
				if (this.OnRegisterVolume != null)
				{
					this.OnRegisterVolume(auraVolume);
				}
			}
		...
		private void AuraVolume_OnUninitialize(AuraVolume auraVolume)
		{
			if (RegisteredVolumesList.Contains(auraVolume))
			{
				...
				if (this.OnUnregisterVolume != null)
				{
					this.OnUnregisterVolume(auraVolume);
				}
