...
namespace Rewired.HID.Drivers
{
	...
	internal abstract class HIDDeviceDriver : IControllerDriver, IDisposable
	{
		...
		internal interface IHIDDevice
		{
			...
			bool WriteSync(fSMyuzvVmAACQsIYyLcgNLStbZVN outputReport, int timeoutMs);

			void WriteAsync(fSMyuzvVmAACQsIYyLcgNLStbZVN outputReport, int timeoutMs);
		...
		internal class InitArgs
		{
			...
			public readonly RWcHFhaLOdObDBlAnlGahPsMjmIp connectionType;
			...
			public InitArgs(UpdateLoopSetting P_0, RWcHFhaLOdObDBlAnlGahPsMjmIp P_1, int P_2, int P_3, int P_4, int P_5, IHIDDevice P_6)
			{
		...
		public dnWPfQfDfnEmaJKgzGFSEYqFnsqm[] axes;

		public WLKCiIfkjEHrYQVDYJcKGKPTVxLS[] buttons;

		public NrHOvbJwrZapXdtjKfrfNYbTfeqF[] hats;

		public ghshtHzRELMvoutgmAIqgcgGRfGD[] accelerometers;

		public cAuwuHpmXWfmQkNNNMuQLAbjJQeRA[] gyroscopes;

		public YbNvcxfeAOXeGxhYaCCOmgMgdTsT[] touchpads;

		public zjaGFxWobEvzfkfnDIafHMDeSyQp[] vibrationMotors;

		public ynsNWLqHUfktHdifyKAAkOjoGzXj[] lights;

		private bool CFgTWgLVMmSqbgiTgdFUKsxTxHGuA;
		...
		protected bool disposed => CFgTWgLVMmSqbgiTgdFUKsxTxHGuA;
		...
		public static HIDDeviceDriver GetDriver(DriverType driverId, InitArgs hidDriverInitArgs)
		{
			...
			case DriverType.SwitchProController:
				if (hidDriverInitArgs.connectionType != RWcHFhaLOdObDBlAnlGahPsMjmIp.Bluetooth)
				{
					return null;
				}
		...
		{
			if (!CFgTWgLVMmSqbgiTgdFUKsxTxHGuA)
			{
				CFgTWgLVMmSqbgiTgdFUKsxTxHGuA = true;
			}
		}
	}
}
