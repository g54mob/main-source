...
namespace NSubstitute.Core.DependencyInjection
{
	public class NSubContainer : IConfigurableNSubContainer, INSubContainer, INSubResolver
	{
		private class Registration
		{
			...
			public object Resolve(Scope scope)
			{
				...
				case NSubLifetime.Transient:
					return _003Cfactory_003EP(scope);
				case NSubLifetime.Singleton:
					return _singletonValue ?? (_singletonValue = _003Cfactory_003EP(scope));
				case NSubLifetime.PerScope:
				{
					...
					result = _003Cfactory_003EP(scope);
		...
		private class Scope : INSubResolver
		{
			...
			{
				_003CmostNestedContainer_003EP = mostNestedContainer;
				_cache = new Dictionary<Registration, object>();
				base._002Ector();
			}
			...
			{
				lock (_003CmostNestedContainer_003EP._syncRoot)
				{
					return (_003CmostNestedContainer_003EP.TryFindRegistration(type) ?? throw new InvalidOperationException("Type is not registered: " + type.FullName)).Resolve(this);
				}
			}
