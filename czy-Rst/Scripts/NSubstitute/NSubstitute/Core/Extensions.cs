...
namespace NSubstitute.Core
{
	internal static class Extensions
	{
		public static bool IsCompatibleWith(this object? instance, Type type)
		{
			...
			Type type3 = (type.IsGenericType ? type.GetGenericTypeDefinition() : null);
			if ((object)type3 != null)
			{
				foreach (Type compatibleType in GetCompatibleTypes(instance.GetType()))
				{
					if (compatibleType.IsGenericType && compatibleType.GetGenericTypeDefinition() == type3)
					{
						return CallSpecification.TypesAreAllEquivalent(compatibleType.GenericTypeArguments, type.GenericTypeArguments);
					}
				}
			}
		...
		private static IReadOnlyList<Type> GetCompatibleTypes(Type type)
		{
			...
			return new _003C_003Ez__ReadOnlyArray<Type>(array);
		}
