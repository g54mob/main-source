...
namespace NSubstitute
{
	public static class Arg
	{
		...
		public static class Compat
		{
			...
			public static AnyType Is<T>(Expression<Predicate<object>> predicate) where T : AnyType
			{
				return Arg.Is<T>(predicate);
			}
			...
			public static AnyType Do<T>(Action<object> useArgument) where T : AnyType
			{
				return Arg.Do<T>(useArgument);
			}
		}
		...
		{
			return ref ArgumentMatcher.Enqueue<T>(new AnyArgumentMatcher(typeof(AnyType)), delegate(object? x)
			{
				useArgument(x);
			});
		}
