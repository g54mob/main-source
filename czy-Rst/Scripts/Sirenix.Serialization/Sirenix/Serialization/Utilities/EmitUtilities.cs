...
namespace Sirenix.Serialization.Utilities
{
	internal static class EmitUtilities
	{
		...
		public static ValueGetter<InstanceType, FieldType> CreateInstanceFieldGetter<InstanceType, FieldType>(FieldInfo fieldInfo)
		{
			...
			string name = fieldInfo.ReflectedType.FullName + ".get_" + fieldInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(FieldType), new Type[1] { typeof(InstanceType).MakeByRefType() }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
		...
		public static WeakValueGetter<FieldType> CreateWeakInstanceFieldGetter<FieldType>(Type instanceType, FieldInfo fieldInfo)
		{
			...
			string name = fieldInfo.ReflectedType.FullName + ".get_" + fieldInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(FieldType), new Type[1] { typeof(object).MakeByRefType() }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
		...
		public static WeakValueGetter CreateWeakInstanceFieldGetter(Type instanceType, FieldInfo fieldInfo)
		{
			...
			string name = fieldInfo.ReflectedType.FullName + ".get_" + fieldInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(object), new Type[1] { typeof(object).MakeByRefType() }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			if (instanceType.IsValueType)
			{
				iLGenerator.Emit(OpCodes.Ldarg_0);
		...
		public static Action<PropType> CreateStaticPropertySetter<PropType>(PropertyInfo propertyInfo)
		{
			...
			string name = propertyInfo.ReflectedType.FullName + ".set_" + propertyInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, null, new Type[1] { typeof(PropType) }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			iLGenerator.Emit(OpCodes.Ldarg_0);
			iLGenerator.Emit(OpCodes.Call, setMethod);
			iLGenerator.Emit(OpCodes.Ret);
		...
		public static Func<PropType> CreateStaticPropertyGetter<PropType>(PropertyInfo propertyInfo)
		{
			...
			string name = propertyInfo.ReflectedType.FullName + ".get_" + propertyInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(PropType), new Type[0], restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			iLGenerator.Emit(OpCodes.Call, getMethod);
		...
		public static ValueGetter<InstanceType, PropType> CreateInstancePropertyGetter<InstanceType, PropType>(PropertyInfo propertyInfo)
		{
			...
			string name = propertyInfo.ReflectedType.FullName + ".get_" + propertyInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(PropType), new Type[1] { typeof(InstanceType).MakeByRefType() }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			if (typeof(InstanceType).IsValueType)
			{
				iLGenerator.Emit(OpCodes.Ldarg_0);
		...
		public static InstanceRefMethodCaller<InstanceType> CreateInstanceRefMethodCaller<InstanceType>(MethodInfo methodInfo)
		{
			...
			Type declaringType = methodInfo.DeclaringType;
			string name = methodInfo.ReflectedType.FullName + ".call_" + methodInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(void), new Type[1] { typeof(InstanceType).MakeByRefType() }, restrictedSkipVisibility: true);
			ILGenerator iLGenerator = dynamicMethod.GetILGenerator();
			if (declaringType.IsValueType)
			{
				iLGenerator.Emit(OpCodes.Ldarg_0);
		...
		public static InstanceRefMethodCaller<InstanceType, Arg1> CreateInstanceRefMethodCaller<InstanceType, Arg1>(MethodInfo methodInfo)
		{
			...
			Type declaringType = methodInfo.DeclaringType;
			string name = methodInfo.ReflectedType.FullName + ".call_" + methodInfo.Name;
			DynamicMethod dynamicMethod = new DynamicMethod(name, typeof(void), new Type[2]
			{
				typeof(InstanceType).MakeByRefType(),
				typeof(Arg1)
			}, restrictedSkipVisibility: true);
