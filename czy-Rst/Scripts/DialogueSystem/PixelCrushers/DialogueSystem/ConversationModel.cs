...
namespace PixelCrushers.DialogueSystem
{
	public class ConversationModel
	{
		...
		private bool useLinearGroupMode;
		...
		public ConversationModel(DialogueDatabase database, string title, Transform actor, Transform conversant, bool allowLuaExceptions, IsDialogueEntryValidDelegate isDialogueEntryValid, int initialDialogueEntryID = -1, bool stopAtFirstValid = false, bool skipExecution = false, bool useLinearGroupMode = false)
		{
		...
		public ConversationState GetState(DialogueEntry entry, bool includeLinks, bool stopAtFirstValid = false, bool skipExecution = false)
		{
			if (entry != null)
			{
				...
				if (!skipExecution)
				{
					ExecuteEntry(entry, characterInfo);
				}
				...
				if (includeLinks)
				{
					...
					{
						EvaluateLinks(entry, list, list2, 0, new List<DialogueEntry>(), null, stopAtFirstValid, skipExecution);
					}
				}
		...
		private void ExecuteEntry(DialogueEntry entry, CharacterInfo actorInfo)
		{
			...
			GameObject arg = ((actorInfo.transform != null) ? actorInfo.transform.gameObject : DialogueManager.instance.gameObject);
		...
		{
			return GetState(entry, includeLinks: true, stopAtFirstValid);
		}
		...
		public void UpdateResponses(ConversationState state)
		{
			List<Response> list = new List<Response>();
			List<Response> list2 = new List<Response>();
			EvaluateLinks(state.subtitle.dialogueEntry, list, list2, 0, new List<DialogueEntry>());
		...
		private void EvaluateLinks(DialogueEntry entry, List<Response> npcResponses, List<Response> pcResponses, int depth, List<DialogueEntry> visited, List<DialogueEntry> groupsToDisallow = null, bool stopAtFirstValid = false, bool skipExecution = false)
		{
			if (depth > 128 || entry == null || visited.Contains(entry))
			{
				return;
			}
			visited.Add(entry);
			if (groupsToDisallow == null)
			{
				groupsToDisallow = new List<DialogueEntry>();
			}
			for (int num = 4; num >= 0; num--)
			{
				EvaluateLinksAtPriority((ConditionPriority)num, entry, npcResponses, pcResponses, depth, visited, groupsToDisallow, stopAtFirstValid, skipExecution);
		...
		private void EvaluateLinksAtPriority(ConditionPriority priority, DialogueEntry entry, List<Response> npcResponses, List<Response> pcResponses, int depth, List<DialogueEntry> visited, List<DialogueEntry> groupsToDisallow, bool stopAtFirstValid = false, bool skipExecution = false)
		{
			if (depth > 128 || entry == null)
			{
				return;
			}
			for (int i = 0; i < entry.outgoingLinks.Count; i++)
			{
				...
				if (dialogueEntry == null || groupsToDisallow.Contains(dialogueEntry) || link.priority != priority)
				{
					continue;
				}
				...
				{
					if (dialogueEntry.isGroup || dialogueEntry.id == 0)
					{
						...
						if (!skipExecution)
						{
							ExecuteEntry(dialogueEntry, actorInfo);
						}
						flag = false;
						for (int num = 4; num >= 0; num--)
						{
							int num2 = npcResponses.Count + pcResponses.Count;
							EvaluateLinksAtPriority((ConditionPriority)num, dialogueEntry, npcResponses, pcResponses, depth + 1, visited, groupsToDisallow, stopAtFirstValid, skipExecution);
							if (npcResponses.Count + pcResponses.Count > num2)
							{
								flag = true;
								if (!stopAtFirstValid || npcResponses.Count <= 0)
								{
									break;
								}
								return;
						...
						for (int j = 0; j < entry.outgoingLinks.Count; j++)
						{
							...
							Link link2 = entry.outgoingLinks[j];
							if (j > i || link2.priority < priority)
							{
								DialogueEntry dialogueEntry2 = m_database.GetDialogueEntry(link2);
								if (dialogueEntry2 != null && dialogueEntry2.isGroup)
								{
									groupsToDisallow.Add(dialogueEntry2);
								}
							}
						}
						continue;
					...
					if (characterType == CharacterType.NPC)
					{
						...
						if (stopAtFirstValid && !DoesEntryRandomizeNextEntry(entry))
						{
							break;
						}
						continue;
				...
				else if (LinkUtility.IsPassthroughOnFalse(dialogueEntry))
				{
					...
					List<Response> list = new List<Response>();
					List<Response> list2 = new List<Response>();
					EvaluateLinks(dialogueEntry, list, list2, depth + 1, visited, groupsToDisallow);
		...
		private bool DoesEntryRandomizeNextEntry(DialogueEntry entry)
		{
			if (entry != null)
			{
				if (string.IsNullOrEmpty(entry.userScript) || !entry.userScript.Contains("RandomizeNextEntry()"))
				{
					if (!string.IsNullOrEmpty(entry.Sequence))
					{
						return entry.Sequence.Contains("RandomizeNextEntry()");
					}
					return false;
				}
				return true;
		...
		public string GetPCName()
		{
			...
			if (m_actorInfo != null && m_database.IsPlayerID(m_actorInfo.id))
			{
				return m_actorInfo.Name;
			}
			if (m_conversantInfo != null && m_database.IsPlayerID(m_conversantInfo.id))
			{
