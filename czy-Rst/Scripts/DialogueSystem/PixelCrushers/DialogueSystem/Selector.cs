...
namespace PixelCrushers.DialogueSystem
{
	...
	public class Selector : MonoBehaviour, IEventSystemUser
	{
		...
		public bool selectBehindUIObjects;
		...
		public virtual string defaultUseMessage
		{
			get
			{
				return m_defaultUseMessage;
			}
			set
			{
				m_defaultUseMessage = value;
			}
		}
		...
		{
			if (base.enabled && !(Time.timeScale <= 0f) && !(Camera.main == null) && (selectAt != SelectAt.MousePosition || selectBehindUIObjects || !(eventSystem != null) || !eventSystem.IsPointerOverGameObject()))
			{
				switch (runRaycasts)
				{
				case Dimension.In2D:
