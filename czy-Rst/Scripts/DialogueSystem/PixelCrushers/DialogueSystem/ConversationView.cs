...
namespace PixelCrushers.DialogueSystem
{
	public class ConversationView : MonoBehaviour
	{
		...
		private bool ShouldShowSubtitle(Subtitle subtitle)
		{
			...
			{
				if (subtitle.formattedText.noSubtitle || string.Equals(subtitle.sequence, "None()") || string.Equals(subtitle.sequence, "None();") || (!settings.cameraSettings.showSubtitleOnEmptyContinue && (string.Equals(subtitle.sequence, "Continue()") || string.Equals(subtitle.sequence, "Continue();"))))
				{
					return false;
				}
		...
		public void HandleContinueButtonClick()
		{
			if (Time.frameCount == initialFrameCount && initialFrameCount == ConversationController.frameLastConversationEnded)
			{
				if (DialogueDebug.logInfo)
				{
					Debug.Log($"Dialogue System: At frame {Time.frameCount}, just started a conversation but another just ended, so ignoring continue button.");
				}
			}
			else
			{
