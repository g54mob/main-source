...
namespace PixelCrushers.DialogueSystem
{
	...
	public class StandardUISubtitleControls : AbstractUISubtitleControls
	{
		...
		public void OverrideActorPanel(Actor actor, SubtitlePanelNumber subtitlePanelNumber, StandardUISubtitlePanel customPanel = null, bool immediate = false)
		{
			...
			bool flag = actor != null && subtitle.speakerInfo.id == actor.id;
			...
			if (standardUISubtitlePanel != panelFromNumber && standardUISubtitlePanel != null && standardUISubtitlePanel.portraitImage.sprite == actor.GetPortraitSprite())
			{
				bool flag2 = standardUISubtitlePanel.continueButton != null && standardUISubtitlePanel.continueButton.gameObject.activeInHierarchy;
				string portraitActorName = standardUISubtitlePanel.portraitActorName;
				Sprite portraitImage = ((standardUISubtitlePanel.portraitImage != null) ? standardUISubtitlePanel.portraitImage.sprite : null);
				...
				if (panelFromNumber.panelState != UIPanel.PanelState.Open)
				{
					...
					if (hasFocus || (actor != null && actor.id == DialogueManager.currentConversationState.subtitle.speakerInfo.id))
					{
				...
				if (standardUISubtitlePanel.portraitImage != null)
				{
			...
			else
			{
				if (panelFromNumber.panelState != UIPanel.PanelState.Open)
				{
					...
					if (DialogueManager.conversationView.isWaitingForContinue)
					{
						panelFromNumber.ShowContinueButton();
					}
				}
				panelFromNumber.SetPortraitImage(actor.GetPortraitSprite());
				string localizedDisplayNameInDatabase = CharacterInfo.GetLocalizedDisplayNameInDatabase(DialogueLua.GetActorField(actor.Name, "Name").asString);
				if (flag || panelFromNumber.portraitName.text != subtitle.speakerInfo.Name)
				{
					panelFromNumber.SetPortraitName(localizedDisplayNameInDatabase);
				}
			}
			if (actor != null && flag)
			{
				panelFromNumber.CheckSubtitleAnimator(subtitle);
			}
		}
		...
		public void OverrideActorPanel(DialogueActor dialogueActor, SubtitlePanelNumber subtitlePanelNumber, StandardUISubtitlePanel customPanel = null)
		{
			if (!(dialogueActor == null))
			{
				...
				case SubtitlePanelNumber.Default:
					...
					break;
				default:
					standardUISubtitlePanel = (allowDialogueActorCustomPanels ? GetPanelFromNumber(subtitlePanelNumber, customPanel) : ((actor != null && actor.IsPlayer) ? m_defaultPCPanel : m_defaultNPCPanel));
					break;
		...
		public void ShowActorInPanel(Actor actor, SubtitlePanelNumber subtitlePanelNumber, StandardUISubtitlePanel customPanel = null)
		{
			...
			{
				if (m_lastPanelUsedByActor[actor.id] == panelFromNumber)
				{
					return;
				}
				m_lastPanelUsedByActor[actor.id].Close();
			}
			string text = DialogueLua.GetLocalizedActorField(actor.Name, "Display Name").asString;
			if (string.IsNullOrEmpty(text))
			{
				text = actor.localizedName;
			}
			panelFromNumber.OpenOnStartConversation(actor.GetPortraitSprite(), text, null);
		}
		...
		public virtual void RecordActorPanelCache(out List<string> actorGOs, out List<SubtitlePanelNumber> actorGOPanels, out List<int> actorIDs, out List<SubtitlePanelNumber> actorIDPanels, out List<string> actorNames)
		{
			...
			foreach (KeyValuePair<Transform, StandardUISubtitlePanel> item in m_actorPanelCache)
			{
				...
				SubtitlePanelNumber subtitlePanelNumberFromPanel = GetSubtitlePanelNumberFromPanel(item.Value, allowReturnDefault: false);
			...
			foreach (KeyValuePair<int, StandardUISubtitlePanel> item2 in m_actorIdOverridePanel)
			{
				...
				SubtitlePanelNumber subtitlePanelNumberFromPanel2 = GetSubtitlePanelNumberFromPanel(item2.Value, allowReturnDefault: false);
		...
		protected virtual SubtitlePanelNumber GetSubtitlePanelNumberFromPanel(StandardUISubtitlePanel panel, bool allowReturnDefault = true)
		{
			if (allowReturnDefault && (panel == m_defaultNPCPanel || panel == m_defaultPCPanel))
			{
		...
		public override void ShowSubtitle(Subtitle subtitle)
		{
			...
			if (subtitle.formattedText.picConversant != 0)
			{
				Sprite listenerOverridePortrait = subtitle.GetListenerOverridePortrait();
				StandardUISubtitlePanel defaultPanel = (subtitle.listenerInfo.IsNPC ? defaultNPCPanel : defaultPCPanel);
				DialogueActor dialogueActor2;
				StandardUISubtitlePanel actorTransformPanel = GetActorTransformPanel(subtitle.listenerInfo.transform, defaultPanel, out dialogueActor2);
				if (actorTransformPanel != null && actorTransformPanel.portraitImage != null)
				{
					actorTransformPanel.portraitImage.sprite = listenerOverridePortrait;
				}
			}
		}
		...
		protected virtual void SupercedeOtherPanelsInList(List<StandardUISubtitlePanel> list, StandardUISubtitlePanel newPanel)
		{
			for (int i = 0; i < list.Count; i++)
			{
				...
				if (standardUISubtitlePanel == null || standardUISubtitlePanel == newPanel)
				{
					continue;
				}
				if (standardUISubtitlePanel.isOpen)
				{
					if (UITools.CanBeSuperceded(standardUISubtitlePanel.visibility))
					{
						standardUISubtitlePanel.Close();
					}
					else
				...
				{
					standardUISubtitlePanel.StopShowSubtitleCoroutines();
				}
			}
		}
		...
		public void OpenSubtitlePanelLikeStart(SubtitlePanelNumber subtitlePanelNumber)
		{
			...
			if (panelFromNumber.isOpen)
			{
				if (panelFromNumber.portraitImage != null && panelFromNumber.portraitImage.sprite != null)
				{
					panelFromNumber.portraitImage.gameObject.SetActive(value: true);
				}
				if (panelFromNumber.portraitName != null && !string.IsNullOrEmpty(panelFromNumber.portraitName.text))
				{
					panelFromNumber.portraitName.gameObject.SetActive(value: true);
				}
				return;
			}
			Conversation conversation = DialogueManager.MasterDatabase.GetConversation(DialogueManager.lastConversationStarted);
		...
		public virtual void SetTypewriterSpeed(float charactersPerSecond)
		{
			for (int i = 0; i < m_builtinPanels.Count; i++)
			{
				if (m_builtinPanels[i] != null)
				{
					TypewriterUtility.SetTypewriterSpeed(m_builtinPanels[i].subtitleText, charactersPerSecond);
				}
			}
			if (m_defaultNPCPanel != null && !m_builtinPanels.Contains(m_defaultNPCPanel))
			{
				TypewriterUtility.SetTypewriterSpeed(m_defaultNPCPanel.subtitleText, charactersPerSecond);
			}
			if (m_defaultPCPanel != null && !m_builtinPanels.Contains(m_defaultPCPanel))
			{
				TypewriterUtility.SetTypewriterSpeed(m_defaultPCPanel.subtitleText, charactersPerSecond);
			}
		}
