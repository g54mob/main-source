...
namespace PixelCrushers.DialogueSystem
{
	...
	public class ConversationStateSaver : Saver
	{
		...
		public override void ApplyData(string s)
		{
			...
			if (standardDialogueUI != null)
			{
				standardDialogueUI.CloseImmediately();
			}
			...
			if (standardDialogueUI2 != null)
			{
				standardDialogueUI2.conversationUIElements.standardSubtitleControls.QueueSavedActorPanelCache(data.actorGOs, data.actorGOPanels, data.actorIDs, data.actorIDPanels);
			}
			...
			for (int i = 0; i < standardDialogueUI2.conversationUIElements.subtitlePanels.Length; i++)
			{
				StandardUISubtitlePanel standardUISubtitlePanel = standardDialogueUI2.conversationUIElements.subtitlePanels[i];
			...
			{
				standardDialogueUI2.conversationUIElements.standardSubtitleControls.GetPanel(DialogueManager.currentConversationState.subtitle, out var _).ShowContinueButton();
			}
		}
		...
		private bool ShouldShouldContinueButton(ConversationState state)
		{
			...
			case DisplaySettings.SubtitleSettings.ContinueButtonMode.NotBeforeResponseMenu:
				if (state.hasPCResponses)
				{
					return !state.hasPCAutoResponse;
				}
				return false;
