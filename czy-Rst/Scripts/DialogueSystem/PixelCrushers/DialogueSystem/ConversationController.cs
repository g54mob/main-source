...
namespace PixelCrushers.DialogueSystem
{
	public class ConversationController
	{
		...
		public static int frameLastConversationEnded
		{
			get
			{
				return _frameLastConversationEnded;
			}
			set
			{
				_frameLastConversationEnded = value;
			}
		}
		...
		public ConversationController(ConversationModel model, ConversationView view, bool reevaluateLinksAfterSubtitle, bool alwaysForceResponseMenu, EndConversationDelegate endConversationHandler)
		{
			isActive = true;
			m_model = model;
			m_view = view;
		...
		public void Initialize(ConversationModel model, ConversationView view, bool reevaluateLinksAfterSubtitle, bool alwaysForceResponseMenu, EndConversationDelegate endConversationHandler)
		{
			isActive = true;
			m_model = model;
			m_view = view;
		...
		public void Initialize(ConversationModel model, ConversationView view, bool alwaysForceResponseMenu, EndConversationDelegate endConversationHandler)
		{
			...
			m_endConversationHandler = endConversationHandler;
			randomizeNextEntry = false;
			reevaluateLinksAfterSubtitle = false;
			DialogueManager.instance.currentConversationState = model.firstState;
			model.InformParticipants("OnConversationStart");
			view.FinishedSubtitleHandler += OnFinishedSubtitle;
			view.SelectedResponseHandler += OnSelectedResponse;
		...
		{
			if (reevaluateLinksAfterSubtitle)
			{
			...
			{
				GotoState(m_model.GetState(flag ? m_state.GetRandomNPCEntry(randomizeNextEntryNoDuplicate) : m_state.firstNPCResponse.destinationEntry));
			}
