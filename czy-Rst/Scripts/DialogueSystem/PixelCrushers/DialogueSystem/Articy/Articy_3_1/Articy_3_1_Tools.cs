...
namespace PixelCrushers.DialogueSystem.Articy.Articy_3_1
{
	public static class Articy_3_1_Tools
	{
		...
		private static bool isInTextTableDocument;
		...
		private static string GetStripStringValue(ReferenceStripPropertyType stripPropertyType)
		{
			...
			if (stripPropertyType != null && stripPropertyType.Reference != null)
			{
				...
				{
					if (!string.IsNullOrEmpty(referenceType.IdRef))
					{
		...
		private static ArticyData.Node ConvertNode(ArticyData articyData, NodeType node)
		{
			...
			if (node != null)
			{
				...
				if (isInTextTableDocument && string.Equals(node.Type, "TextObject") && LookupByIdRef(node.IdRef) is TextObjectType textObjectType)
				{
					articyData.textTableFields.Add(GetDefaultLocalizedString(textObjectType.DisplayName));
				}
				...
				if (node.Node != null)
				{
					...
					{
						documentDepth++;
						if (!string.IsNullOrEmpty(_prefs.TextTableDocument) && LookupByIdRef(node.IdRef) is DocumentType documentType && string.Equals(GetDefaultLocalizedString(documentType.DisplayName), _prefs.TextTableDocument))
						{
		...
		private static object LookupByIdRef(string idRef)
		{
			...
			{
				object obj = _currentExport.Content.Items[i];
				if (obj is DocumentType)
				{
					if (string.Equals((obj as DocumentType).Id, idRef))
					{
						return obj;
					}
				}
				else if (obj is TextObjectType && string.Equals((obj as TextObjectType).Id, idRef))
				{
					return obj;
				}
			}
