	...
	public class ArticyData
	{
		public class LocalizableText
		{
			...
			public string DefaultText
			{
				get
				{
					if (localizedString.TryGetValue(string.Empty, out var value) && !string.IsNullOrEmpty(value))
					{
						return value;
					}
					if (localizedString.TryGetValue("en", out value) && !string.IsNullOrEmpty(value))
					{
						return value;
					}
					foreach (KeyValuePair<string, string> item in localizedString)
					{
		...
		public string ProjectTitle => project.displayName;

		public string ProjectVersion => project.createdOn;

		public string ProjectAuthor => $"{project.creatorTool} {project.creatorVersion}";

		public static string FullVariableName(VariableSet variableSet, Variable variable)
		{
			if (variableSet == null || variable == null)
			{
				return string.Empty;
			}
			return $"{variableSet.technicalName}.{variable.technicalName}";
		}
