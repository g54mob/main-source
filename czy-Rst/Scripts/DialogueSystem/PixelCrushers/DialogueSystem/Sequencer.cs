...
namespace PixelCrushers.DialogueSystem
{
	public class Sequencer : MonoBehaviour
	{
		...
		private bool m_hasCachedDialoguePanelInfo;
		...
		private bool HandleAudioInternally(string commandName, string[] args)
		{
			...
			if (SequencerTools.IsAudioMuted())
			{
				if (DialogueDebug.logInfo)
				{
					Debug.Log(string.Format("{0}: Sequencer: Audio({1}, {2}): skipping; audio is muted", new object[3] { "Dialogue System", clipName, subject }), subject);
				}
				return true;
			}
			if (DialogueDebug.logInfo)
		...
		{
			if (standardDialogueUI == null || m_hasCachedDialoguePanelInfo)
			{
				return;
			}
			...
			{
				if (standardDialogueUI.conversationUIElements.subtitlePanels[i] == null)
				{
					continue;
				}
				...
				if (standardDialogueUI.conversationUIElements.subtitlePanels[i].isOpen && standardDialogueUI.conversationUIElements.subtitlePanels[i].panelState != UIPanel.PanelState.Closing)
				{
					...
					if (immediate)
					{
						standardDialogueUI.conversationUIElements.subtitlePanels[i].HideImmediate();
					}
					else
					{
						standardDialogueUI.conversationUIElements.subtitlePanels[i].Close();
					}
					m_setDialoguePanelPreviouslyOpenSubtitlePanels.Add(i);
				}
			}
			for (int j = 0; j < standardDialogueUI.conversationUIElements.menuPanels.Length; j++)
			{
				if (!(standardDialogueUI.conversationUIElements.menuPanels[j] == null) && standardDialogueUI.conversationUIElements.menuPanels[j].isOpen)
				{
					m_setDialoguePanelPreviouslyOpenMenuPanels.Add(j);
					if (immediate)
					{
						standardDialogueUI.conversationUIElements.menuPanels[j].HideImmediate();
					}
					else
					{
						standardDialogueUI.conversationUIElements.menuPanels[j].Close();
					}
				}
			}
			if (immediate)
		...
		private bool HandleHidePanelInternally(string commandName, string[] args)
		{
			...
			else
			{
				...
				if (flag)
				{
					Tools.SetGameObjectActive(standardUISubtitlePanel.portraitImage, value: false);
		...
		private bool HandleSetPanelInternally(string commandName, string[] args)
		{
			...
			SubtitlePanelNumber subtitlePanelNumber = ((!string.Equals(parameter2, "default", StringComparison.OrdinalIgnoreCase)) ? (string.Equals(parameter2, "bark", StringComparison.OrdinalIgnoreCase) ? SubtitlePanelNumber.UseBarkUI : (string.Equals(parameter2, "custom", StringComparison.OrdinalIgnoreCase) ? SubtitlePanelNumber.Custom : PanelNumberUtility.IntToSubtitlePanelNumber(Tools.StringToInt(parameter2)))) : SubtitlePanelNumber.Default);
