...
internal static class FLTutQdraUqptwGNJUGqpFFtDAEy
{
	internal static bool IAGSFRHlNDLlmowOxcOCGkpQIJhR(KeyedGlyph P_0, string P_1, string P_2, uint P_3, DeviceLocalizationInfo P_4, eXRjOdORfaNOqMSguWnRpnOIZGBy P_5, int P_6, AxisRange P_7, int P_8, out object P_9)
	{
		...
		if (P_5 != eXRjOdORfaNOqMSguWnRpnOIZGBy.Keyboard)
		{
			sharedStringBuilder.Length = 0;
		...
		if (P_5 == eXRjOdORfaNOqMSguWnRpnOIZGBy.Joystick && P_6 >= 0 && P_4 != null && P_4.controllerTemplateGuids != null)
		{
			for (int j = 0; j < P_4.controllerTemplateGuids.Count; j++)
			{
				if (!dXDhgciBpvPiLRoZXBpiBCxofOAPA.oNfjKzoZltzComUJpBGatWqkciaB(P_4.guid, P_4.controllerTemplateGuids[j], P_6, P_7, P_8, out var _, out var value) || string.IsNullOrEmpty(value))
				{
					continue;
				}
				sharedStringBuilder.Length = 0;
	...
	internal static GlyphManager.GetAndUpdateGlyphResultFlags qyAZFylUpdGRHbgtLQIAfvzcAJtE(KeyedGlyph P_0, string P_1, string P_2, DeviceLocalizationInfo P_3, eXRjOdORfaNOqMSguWnRpnOIZGBy P_4, int P_5, AxisRange P_6, int P_7, out object P_8)
	{
		...
		if (!P_0.hasCachedValue || glyphProviderVersionChanged)
		{
			...
			if (IAGSFRHlNDLlmowOxcOCGkpQIJhR(P_0, P_1, P_2, GlyphManager.version, P_3, P_4, P_5, P_6, P_7, out P_8))
			{
