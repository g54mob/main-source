...
namespace Rewired.ComponentControls.Effects
{
	...
	public sealed class TouchInteractableTransitioner : MonoBehaviour, IVisibilityChangedHandler, TouchInteractable.IInteractionStateTransitionHandler
	{
		...
		private TouchInteractable.InteractionState JEkwkgeniFZmzkErcmCHadHtRnhd;
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					gCTbmsKxLKtefFrSYfHzgkphtgxZB(value, false);
					akCXXxNzKuezhVNjnlmEfbfTUNAS();
				}
			}
		}
		...
		private void Awake()
		{
			if (Application.isPlaying)
			{
				...
				gCTbmsKxLKtefFrSYfHzgkphtgxZB(_visible, true);
			}
		}
		...
		private void OnEnable()
		{
			...
			NQvAiYMAunLklaQAFbpfDautkaeD(true);
		}

		[CustomObfuscation(rename = false)]
		private void OnDisable()
		{
			zazivCYGGGxqaHamIVDgzvsBaFCp();
		}
		...
		private void OnValidate()
		{
			...
			{
				LuffnamfYpaCEuFapMUqLamkevwN(null);
				smvFJmXOhVDalqUMmzStJUekgZHc(Color.white, true);
				kgnpGfBdGgWjpQDlTEnhKXutCkbQ(_transitionAnimationTriggers.normalTrigger);
				NQvAiYMAunLklaQAFbpfDautkaeD(true);
			}
			OVwcLsFISJYfNrjUXeYIQIbbOMpOA();
		}
		...
		private void akCXXxNzKuezhVNjnlmEfbfTUNAS()
		{
			OVwcLsFISJYfNrjUXeYIQIbbOMpOA();
		}

		private void OVwcLsFISJYfNrjUXeYIQIbbOMpOA()
		{
			if (!Application.isPlaying)
			{
				NQvAiYMAunLklaQAFbpfDautkaeD(true);
			...
			{
				NQvAiYMAunLklaQAFbpfDautkaeD(false);
			}
		}

		private void NQvAiYMAunLklaQAFbpfDautkaeD(bool P_0)
		{
			EmBCksiICYovOPsQeLNRiGVFjLgRB(JEkwkgeniFZmzkErcmCHadHtRnhd, P_0);
		}

		private void gCTbmsKxLKtefFrSYfHzgkphtgxZB(bool P_0, bool P_1)
		{
			if (_visible != P_0 || P_1)
			{
				_visible = P_0;
		...
		private bool yuBEUyfmmjLMCUgZHktZffPdULQhB()
		{
			return UnityTools.IsActiveAndEnabled(this);
		}

		private void zazivCYGGGxqaHamIVDgzvsBaFCp()
		{
			...
			{
				kgnpGfBdGgWjpQDlTEnhKXutCkbQ(normalTrigger);
			}
		}

		private void EmBCksiICYovOPsQeLNRiGVFjLgRB(TouchInteractable.InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					smvFJmXOhVDalqUMmzStJUekgZHc(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					smvFJmXOhVDalqUMmzStJUekgZHc(color, P_1);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.SpriteSwap) != TouchInteractable.TransitionTypeFlags.None)
				{
					LuffnamfYpaCEuFapMUqLamkevwN(sprite);
				}
				if ((_transitionType & TouchInteractable.TransitionTypeFlags.Animation) != TouchInteractable.TransitionTypeFlags.None)
				{
					kgnpGfBdGgWjpQDlTEnhKXutCkbQ(text);
				}
			}
		}

		private void smvFJmXOhVDalqUMmzStJUekgZHc(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void LuffnamfYpaCEuFapMUqLamkevwN(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void kgnpGfBdGgWjpQDlTEnhKXutCkbQ(string P_0)
		{
		...
		{
			JEkwkgeniFZmzkErcmCHadHtRnhd = args.state;
