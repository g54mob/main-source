...
namespace Rewired.ComponentControls.Data
{
	...
	public sealed class CustomControllerElementSelector
	{
		...
		[HideInInspector]
		private int oUyDewYyGNFefRHSLaSnCidWEfZlA = -1;

		[HideInInspector]
		private int QoWjuhPuyoKAJtsjLNkPCzQYnrJi = -1;
		...
		public int GetElementIndex(Rewired.CustomController customController)
		{
			...
			if (oUyDewYyGNFefRHSLaSnCidWEfZlA >= 0 && oUyDewYyGNFefRHSLaSnCidWEfZlA != customController.id)
			{
				ClearCache();
			}
			if (QoWjuhPuyoKAJtsjLNkPCzQYnrJi >= 0)
			{
				return QoWjuhPuyoKAJtsjLNkPCzQYnrJi;
			}
			oUyDewYyGNFefRHSLaSnCidWEfZlA = customController.id;
			switch (_selectorType)
			{
			case SelectorType.Id:
			{
				IList<ControllerElementIdentifier> list = CJsaiXqUbSmxNBawOlEttlkZptdP(customController, _elementType);
				for (int j = 0; j < list.Count; j++)
				{
					if (list[j].Rewired_002EInterfaces_002EIControllerElementIdentifierCommon_Internal_002Eid == _elementId)
					{
						QoWjuhPuyoKAJtsjLNkPCzQYnrJi = j;
						break;
					}
				}
				break;
			...
			case SelectorType.Index:
			{
				...
				QoWjuhPuyoKAJtsjLNkPCzQYnrJi = _elementIndex;
				break;
			}
			case SelectorType.Name:
			{
				...
				for (int i = 0; i < list.Count; i++)
				{
					...
					{
						QoWjuhPuyoKAJtsjLNkPCzQYnrJi = i;
						break;
					}
				}
				break;
		...
		private IList<ControllerElementIdentifier> CJsaiXqUbSmxNBawOlEttlkZptdP(Rewired.CustomController P_0, ElementType P_1)
		{
			return P_1 switch
			{
		...
		{
			oUyDewYyGNFefRHSLaSnCidWEfZlA = -1;
			QoWjuhPuyoKAJtsjLNkPCzQYnrJi = -1;
		}
	}
}
