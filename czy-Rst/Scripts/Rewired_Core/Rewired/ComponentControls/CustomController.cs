...
namespace Rewired.ComponentControls
{
	...
	public class CustomController : ComponentController
	{
		...
		private struct OpNzyzdgxHuMLjDQcptcFcOiqPQL
		{
			public CustomControllerElementSelector.ElementType OHVnQEVfOakhIUewaNUYJCzJmgVR;

			public int CmCTUBWxcUGBUGDpsgAGgNGtMlFxA;

			public float oQIgfZUfarOeEqlrMvSKyxwYuROV;

			public OpNzyzdgxHuMLjDQcptcFcOiqPQL(CustomControllerElementSelector.ElementType P_0, int P_1, float P_2)
			{
				OHVnQEVfOakhIUewaNUYJCzJmgVR = P_0;
				CmCTUBWxcUGBUGDpsgAGgNGtMlFxA = P_1;
				oQIgfZUfarOeEqlrMvSKyxwYuROV = P_2;
			}

			public OpNzyzdgxHuMLjDQcptcFcOiqPQL(CustomControllerElementSelector.ElementType P_0, int P_1, bool P_2)
			{
				OHVnQEVfOakhIUewaNUYJCzJmgVR = P_0;
				CmCTUBWxcUGBUGDpsgAGgNGtMlFxA = P_1;
				oQIgfZUfarOeEqlrMvSKyxwYuROV = (P_2 ? 1f : 0f);
			}

			public bool mSZTAUWlrdwozJBjDdAXyuawjgIGA(CustomControllerElementSelector.ElementType P_0, int P_1)
			{
				if (OHVnQEVfOakhIUewaNUYJCzJmgVR == P_0)
				{
					return CmCTUBWxcUGBUGDpsgAGgNGtMlFxA == P_1;
				}
				return false;
			}

			public void JZoEUokXXKeNgahvUCazrUAcRUQqA(float P_0)
			{
				oQIgfZUfarOeEqlrMvSKyxwYuROV = MathTools.MaxMagnitude(oQIgfZUfarOeEqlrMvSKyxwYuROV, P_0);
			}

			public void miNfpDTNLcerMEoYCaGQVuHEnrxkA(bool P_0)
			{
				if (P_0)
				{
					oQIgfZUfarOeEqlrMvSKyxwYuROV = 1f;
				}
			}
		}
		...
		private List<OpNzyzdgxHuMLjDQcptcFcOiqPQL> lYRnSqetlyCrQILLQHhzEAPlOtb = new List<OpNzyzdgxHuMLjDQcptcFcOiqPQL>(10);

		[NonSerialized]
		private int fPxMrZIbhANmQtACwPzddgiafsvA = -1;

		private Action wASfcZBFMvPzXabLgzqncnCZEoTlb;
		...
		internal event Action InputSourceUpdateEvent
		{
			add
			{
				wASfcZBFMvPzXabLgzqncnCZEoTlb = (Action)Delegate.Combine(wASfcZBFMvPzXabLgzqncnCZEoTlb, value);
			}
			remove
			{
				wASfcZBFMvPzXabLgzqncnCZEoTlb = (Action)Delegate.Remove(wASfcZBFMvPzXabLgzqncnCZEoTlb, value);
			}
		}
		...
		{
			return BROJREFJcdXOiwOGnExpuJhiIMhS(false);
		}
		...
		{
			base.OnEnable();
			_ = base.aVrFdNyTWVUeoPzBhtEtmuMUQPob;
		}
		...
		{
			base.OnDisable();
			if (base.aVrFdNyTWVUeoPzBhtEtmuMUQPob)
			{
				lYRnSqetlyCrQILLQHhzEAPlOtb.Clear();
			}
		}
		...
		{
			base.OnDestroy();
			gpgnomRwsbaYbUtEgweyhwtGSBwh();
		}

		internal virtual bool OnInitialize()
		{
			if (!base.zMetDDwGNeNhdJkVLipdWuoGPxCb())
			{
				return false;
			}
			if (GetUseCustomController())
			{
				if (!hPUupzykKCzcQClhDXVRvRIdFgHL())
				{
					return false;
				}
				if (BROJREFJcdXOiwOGnExpuJhiIMhS(true) == null)
				{
		...
		{
			base.SQRbGbEVsSfaUTCQMaxJAnffpgTGb();
			UvofaosyXbMDCgHukBdPkCwNVybJ();
			if (ReInput.isReady)
			{
				ReInput.InputSourceUpdateEvent += ilGhdkqoAIQebzYEIyBPklQwSjsb;
			}
		}

		internal virtual void OnUnsubscribeEvents()
		{
			base.UvofaosyXbMDCgHukBdPkCwNVybJ();
			ReInput.InputSourceUpdateEvent -= ilGhdkqoAIQebzYEIyBPklQwSjsb;
		}
		...
		{
			if (!base.aVrFdNyTWVUeoPzBhtEtmuMUQPob || element == null || !GetUseCustomController())
			{
				return;
			}
			Rewired.CustomController customController = BROJREFJcdXOiwOGnExpuJhiIMhS(false);
			...
			_ = lYRnSqetlyCrQILLQHhzEAPlOtb.Count;
			for (int i = 0; i < lYRnSqetlyCrQILLQHhzEAPlOtb.Count; i++)
			{
				OpNzyzdgxHuMLjDQcptcFcOiqPQL value2 = lYRnSqetlyCrQILLQHhzEAPlOtb[i];
				if (value2.mSZTAUWlrdwozJBjDdAXyuawjgIGA(element.elementType, elementIndex))
				{
					value2.JZoEUokXXKeNgahvUCazrUAcRUQqA(value);
					lYRnSqetlyCrQILLQHhzEAPlOtb[i] = value2;
					return;
				}
			}
			lYRnSqetlyCrQILLQHhzEAPlOtb.Add(new OpNzyzdgxHuMLjDQcptcFcOiqPQL(element.elementType, elementIndex, value));
		}
		...
		internal void SetButtonValue(CustomControllerElementSelector element, bool value)
		{
			...
			for (int i = 0; i < lYRnSqetlyCrQILLQHhzEAPlOtb.Count; i++)
			{
				...
				if (value2.mSZTAUWlrdwozJBjDdAXyuawjgIGA(element.elementType, elementIndex))
				{
					value2.miNfpDTNLcerMEoYCaGQVuHEnrxkA(value);
					lYRnSqetlyCrQILLQHhzEAPlOtb[i] = value2;
					return;
				}
			}
		...
		internal void ClearElementValue(CustomControllerElementSelector element)
		{
			...
			_ = lYRnSqetlyCrQILLQHhzEAPlOtb.Count;
			for (int num = lYRnSqetlyCrQILLQHhzEAPlOtb.Count - 1; num >= 0; num--)
			{
				if (lYRnSqetlyCrQILLQHhzEAPlOtb[num].mSZTAUWlrdwozJBjDdAXyuawjgIGA(element.elementType, elementIndex))
				{
					lYRnSqetlyCrQILLQHhzEAPlOtb.RemoveAt(num);
				}
			}
		}
		...
		private void VposSkvXMUvlyijRFwEsbNxmmHWc()
		{
			if (base.aVrFdNyTWVUeoPzBhtEtmuMUQPob)
			{
				lYRnSqetlyCrQILLQHhzEAPlOtb.Clear();
		...
		private bool hPUupzykKCzcQClhDXVRvRIdFgHL()
		{
			if (ReInput.isReady)
			{
				return true;
		...
		private void uXoDBLLTLZtEdNywDdrocmAXDUwGA()
		{
			if (lYRnSqetlyCrQILLQHhzEAPlOtb.Count == 0)
			{
				return;
			}
			Rewired.CustomController customController = BROJREFJcdXOiwOGnExpuJhiIMhS(false);
			...
			for (int i = 0; i < lYRnSqetlyCrQILLQHhzEAPlOtb.Count; i++)
			{
				OpNzyzdgxHuMLjDQcptcFcOiqPQL opNzyzdgxHuMLjDQcptcFcOiqPQL = lYRnSqetlyCrQILLQHhzEAPlOtb[i];
				switch (opNzyzdgxHuMLjDQcptcFcOiqPQL.OHVnQEVfOakhIUewaNUYJCzJmgVR)
				{
				case CustomControllerElementSelector.ElementType.Axis:
					customController.SetAxisValue(opNzyzdgxHuMLjDQcptcFcOiqPQL.CmCTUBWxcUGBUGDpsgAGgNGtMlFxA, opNzyzdgxHuMLjDQcptcFcOiqPQL.oQIgfZUfarOeEqlrMvSKyxwYuROV);
					break;
				case CustomControllerElementSelector.ElementType.Button:
					customController.SetButtonValue(opNzyzdgxHuMLjDQcptcFcOiqPQL.CmCTUBWxcUGBUGDpsgAGgNGtMlFxA, opNzyzdgxHuMLjDQcptcFcOiqPQL.oQIgfZUfarOeEqlrMvSKyxwYuROV != 0f);
					break;
				default:
					throw new NotImplementedException();
				}
		...
		private Rewired.CustomController BROJREFJcdXOiwOGnExpuJhiIMhS(bool P_0)
		{
			if (!GetUseCustomController())
			{
				return null;
			...
			if (fPxMrZIbhANmQtACwPzddgiafsvA >= 0)
			{
				customController = ReInput.controllers.GetCustomController(fPxMrZIbhANmQtACwPzddgiafsvA);
				if (customController == null)
				{
					fPxMrZIbhANmQtACwPzddgiafsvA = -1;
				}
			}
			else
			{
			...
			if (customController == null)
			{
				if (_createCustomControllerSettings.createCustomController)
				{
					...
					if (customController != null)
					{
						fPxMrZIbhANmQtACwPzddgiafsvA = customController.id;
						wQUjiPlIRfswWQFQioRuqGsFcJoD(customController);
					}
				}
				else
				{
		...
		private void wQUjiPlIRfswWQFQioRuqGsFcJoD(Rewired.CustomController P_0)
		{
			if (P_0 == null)
			{
				return;
		...
		private void gpgnomRwsbaYbUtEgweyhwtGSBwh()
		{
			if (fPxMrZIbhANmQtACwPzddgiafsvA >= 0 && _createCustomControllerSettings.destroyCustomController)
			{
				Rewired.CustomController customController = BROJREFJcdXOiwOGnExpuJhiIMhS(false);
		...
		private void ilGhdkqoAIQebzYEIyBPklQwSjsb()
		{
			if (wASfcZBFMvPzXabLgzqncnCZEoTlb != null)
			{
				wASfcZBFMvPzXabLgzqncnCZEoTlb();
			}
			uXoDBLLTLZtEdNywDdrocmAXDUwGA();
		}
	}
}
