...
namespace Rewired.ComponentControls
{
	...
	public abstract class TouchInteractable : TouchControl, IPointerDownHandler, IEventSystemHandler, IPointerUpHandler, IPointerEnterHandler, IPointerExitHandler, IBeginDragHandler, IDragHandler, IEndDragHandler
	{
		...
		{
			private TouchInteractable YaxVoafqdctFTxKVkGIuVYOcNjIL;

			private InteractionState HwybfnPSbabDVMJwIKWPyJJnkpxi;

			private float MmFvhDelzsEwesJVzNTSudWNeDtp;

			public TouchInteractable sender => YaxVoafqdctFTxKVkGIuVYOcNjIL;

			public InteractionState state => HwybfnPSbabDVMJwIKWPyJJnkpxi;

			public float duration => MmFvhDelzsEwesJVzNTSudWNeDtp;

			internal InteractionStateTransitionArgs()
			{
			}

			internal void puYFGDdVaxXEqtbvBNaXhnJEXoVQ(TouchInteractable P_0, InteractionState P_1, float P_2)
			{
				YaxVoafqdctFTxKVkGIuVYOcNjIL = P_0;
				HwybfnPSbabDVMJwIKWPyJJnkpxi = P_1;
				MmFvhDelzsEwesJVzNTSudWNeDtp = P_2;
			}
		}
		...
		[Serializable]
		private sealed class QQkweBiToOUnyrzwkqRvRSoEqpso
		{
			public static readonly QQkweBiToOUnyrzwkqRvRSoEqpso _003C_003E9 = new QQkweBiToOUnyrzwkqRvRSoEqpso();

			public static ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> _003C_003E9__152_0;

			internal void gRXxVvXoeNSfbEoROlSlgcottiZb(IInteractionStateTransitionHandler P_0, InteractionStateTransitionArgs P_1)
			{
		...
		[NonSerialized]
		private bool akdbiKEpVwTENWRfwWhcpLSbhGDQ;

		[NonSerialized]
		private bool wSxfeFYVbGcyfGkvSVMJDRcUjnIOA;
		...
		private ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IVisibilityChangedHandler, bool> __hierarchyVisibilityChangedHandlers;

		private ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __hierarchyInteractionStateTransitionHandlers;

		private static ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> __interactionStateTransitionHandlerDelegate;

		private ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IVisibilityChangedHandler, bool> FTHhdpAhecWToVeSSraRmCPnRVvs
		{
			get
			{
				if (__hierarchyVisibilityChangedHandlers == null)
				{
					__hierarchyVisibilityChangedHandlers = new ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IVisibilityChangedHandler, bool>(ltLaQJLsUywwbNwySSTkGERtNqwC.YvWEsQcRJOZYUElIJIXpbFONrzsO);
		...
		private ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> NqHFmMKKWHecBlgLmuVtzqLXGJHk
		{
			get
			{
				if (__hierarchyInteractionStateTransitionHandlers == null)
				{
					__hierarchyInteractionStateTransitionHandlers = new ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.HierarchyEventHelper<IInteractionStateTransitionHandler, InteractionStateTransitionArgs>(vvSWwZZANPIMvbhMnPaQTwjRDGccA);
		...
		public bool visible
		{
			...
			set
			{
				if (visible != value)
				{
					jpGxiZOMyhOKelTmctWxjVJbUSfQ(value, false);
					HAozJjtCaBQEIiVJLswvClYOjXTs();
				}
			}
		}
		...
		internal static ZOyhNNnLEbZLlzwWpEOGPqGMuZsL.EventFunction<IInteractionStateTransitionHandler, InteractionStateTransitionArgs> vvSWwZZANPIMvbhMnPaQTwjRDGccA
		{
			get
			{
				if (__interactionStateTransitionHandlerDelegate == null)
				{
					__interactionStateTransitionHandlerDelegate = QQkweBiToOUnyrzwkqRvRSoEqpso._003C_003E9.gRXxVvXoeNSfbEoROlSlgcottiZb;
				}
		...
		internal override void OnEnable()
		{
			...
			eTZYNnLPlWGRATuKeCueoMMWKHDC(InteractionState.Normal);
			LuIDCmWpaYCxWQpmQAGjawsQMUJY(true);
		}

		[CustomObfuscation(rename = false)]
		internal override void OnDisable()
		{
			ZNDflEiyERQemuQfxcmNcWeftcFAb();
			base.OnDisable();
		}
		...
		internal override void OnValidate()
		{
			...
			if (GlaXMdVzEWtLRKxLWJPCCCZtpeXE())
			{
				...
				OdxPRJxXfrRRyouArejRBCeBLLCB(null);
				PLucYaRzMaeVziaxuaMMgrPPbkuY(Color.white, true);
				sJluuSuCuNIaCUVyneApZpDADbwV(_transitionAnimationTriggers.normalTrigger);
				LuIDCmWpaYCxWQpmQAGjawsQMUJY(true);
			}
			kYKRgqqgWOqEDeNGoCbnFhjkKfnD();
			usBgJBzTnsyWmaavlQXfBWZFjvRG();
		}
		...
		internal virtual void RxXwmSTcDqekzOuyZktlaLRThxqq()
		{
			zvtwPLVyKfGRcAeJsUBCKwELbDvH();
			usBgJBzTnsyWmaavlQXfBWZFjvRG();
		}

		internal virtual void KlDGvCYjJfAyEEZZAtJVeJsIJzDGb()
		{
			base.yvFsmjTmMlGFbdTAOPvckifnkrae();
			kYKRgqqgWOqEDeNGoCbnFhjkKfnD();
		}

		private void ZNDflEiyERQemuQfxcmNcWeftcFAb()
		{
			string normalTrigger = _transitionAnimationTriggers.normalTrigger;
			akdbiKEpVwTENWRfwWhcpLSbhGDQ = false;
			wSxfeFYVbGcyfGkvSVMJDRcUjnIOA = false;
			...
			{
				sJluuSuCuNIaCUVyneApZpDADbwV(normalTrigger);
			}
		}

		private void KIuTiFuNrCszGqThJASYgrZOjZrS(InteractionState P_0, bool P_1)
		{
			Color color;
			Sprite sprite;
			string text;
			...
			if (base.gameObject.activeInHierarchy)
			{
				if (flag)
				{
					PLucYaRzMaeVziaxuaMMgrPPbkuY(color * _transitionColorTint.colorMultiplier, P_1);
				}
				else
				{
					PLucYaRzMaeVziaxuaMMgrPPbkuY(color, P_1);
				}
				if ((_transitionType & TransitionTypeFlags.SpriteSwap) != TransitionTypeFlags.None)
				{
					OdxPRJxXfrRRyouArejRBCeBLLCB(sprite);
				}
				if ((_transitionType & TransitionTypeFlags.Animation) != TransitionTypeFlags.None)
				{
					sJluuSuCuNIaCUVyneApZpDADbwV(text);
				}
			}
			if (_allowSendingEvents)
			{
				_transitionArgs.puYFGDdVaxXEqtbvBNaXhnJEXoVQ(this, P_0, P_1 ? 0f : _transitionColorTint.fadeDuration);
				NqHFmMKKWHecBlgLmuVtzqLXGJHk.ExecuteOnAll(_transitionArgs);
		...
		private void PLucYaRzMaeVziaxuaMMgrPPbkuY(Color P_0, bool P_1)
		{
			if (!(_targetGraphic == null))
			{
		...
		private void OdxPRJxXfrRRyouArejRBCeBLLCB(Sprite P_0)
		{
			if (!(image == null))
			{
		...
		private void sJluuSuCuNIaCUVyneApZpDADbwV(string P_0)
		{
		...
		private void LuIDCmWpaYCxWQpmQAGjawsQMUJY(bool P_0)
		{
			...
			KIuTiFuNrCszGqThJASYgrZOjZrS(interactionState, P_0);
		}
		...
		internal virtual bool KrGIZjncSOxUndERuUeAOuvSPFve()
		{
			if (!GlaXMdVzEWtLRKxLWJPCCCZtpeXE())
			{
				return false;
			}
			if (akdbiKEpVwTENWRfwWhcpLSbhGDQ)
			{
				return wSxfeFYVbGcyfGkvSVMJDRcUjnIOA;
			}
			return false;
		}

		internal void vPLwwZScbMSKSuXMXuQiQJCkGUmW(BaseEventData P_0)
		{
			if (GlaXMdVzEWtLRKxLWJPCCCZtpeXE() && IsInteractable())
			{
				InteractionState interactionState = BJZEghGyrMVleEPEysNZpROcMeBcb(P_0);
				if (interactionState != _interactionState)
				{
					eTZYNnLPlWGRATuKeCueoMMWKHDC(interactionState);
					LuIDCmWpaYCxWQpmQAGjawsQMUJY(false);
				}
			}
		}

		internal virtual bool yhSTFcgNFyBkbahIEXDHacLdqfjKA(GameObject P_0)
		{
			return base.gameObject == P_0;
		}

		private bool lWlAPLgIMekcUdgXACQXBCYWDmciA(BaseEventData P_0)
		{
			bool flag = P_0 is PointerEventData;
			return LFdHPmHrxqnVvXJZwpUrEWUrILcW(flag, flag ? (P_0 as PointerEventData).pointerPress : null);
		}

		private bool LFdHPmHrxqnVvXJZwpUrEWUrILcW(bool P_0, GameObject P_1)
		{
			if (!GlaXMdVzEWtLRKxLWJPCCCZtpeXE())
			{
				return false;
			}
			if (KrGIZjncSOxUndERuUeAOuvSPFve())
			{
				return false;
			}
			bool flag = false;
			if (P_0)
			{
				return flag | ((wSxfeFYVbGcyfGkvSVMJDRcUjnIOA && !akdbiKEpVwTENWRfwWhcpLSbhGDQ && yhSTFcgNFyBkbahIEXDHacLdqfjKA(P_1)) || (!wSxfeFYVbGcyfGkvSVMJDRcUjnIOA && akdbiKEpVwTENWRfwWhcpLSbhGDQ && yhSTFcgNFyBkbahIEXDHacLdqfjKA(P_1)) || (!wSxfeFYVbGcyfGkvSVMJDRcUjnIOA && akdbiKEpVwTENWRfwWhcpLSbhGDQ && P_1 == null));
			}
			return flag | akdbiKEpVwTENWRfwWhcpLSbhGDQ;
		}

		private InteractionState BJZEghGyrMVleEPEysNZpROcMeBcb(BaseEventData P_0)
		{
			if (KrGIZjncSOxUndERuUeAOuvSPFve())
			{
				return InteractionState.Pressed;
			}
			if (lWlAPLgIMekcUdgXACQXBCYWDmciA(P_0))
			{
		...
		private bool eTZYNnLPlWGRATuKeCueoMMWKHDC(InteractionState P_0)
		{
			...
			URHqZgnBZnGMYQglOantOQcBTSBG();
			return true;
		}

		private void URHqZgnBZnGMYQglOantOQcBTSBG()
		{
			qPEpWpsimakTvqYIDFAsjGGnSaOf();
		}

		private void qPEpWpsimakTvqYIDFAsjGGnSaOf()
		{
			if (Application.isPlaying && _hideWhenIdle)
			{
				jpGxiZOMyhOKelTmctWxjVJbUSfQ(_interactionState == InteractionState.Pressed, false);
			}
		}

		private void jpGxiZOMyhOKelTmctWxjVJbUSfQ(bool P_0, bool P_1)
		{
			if (_visible == P_0 && !P_1)
			{
				return;
			...
			if (_allowSendingEvents)
			{
				FTHhdpAhecWToVeSSraRmCPnRVvs.ExecuteOnAll(P_0);
		...
		private void rErzkawXnMgtxTDXpJMVQQoSBMsW()
		{
			...
			{
				jpGxiZOMyhOKelTmctWxjVJbUSfQ(_visible, true);
				qPEpWpsimakTvqYIDFAsjGGnSaOf();
			}
			kYKRgqqgWOqEDeNGoCbnFhjkKfnD();
			if (_allowSendingEvents)
			{
				FTHhdpAhecWToVeSSraRmCPnRVvs.ExecuteOnAll(_visible);
				if (_onVisibilityChanged != null)
				{
		...
		private void NbbKpsORSgmjlgtQwhETKDaahzwb()
		{
		...
		private void usBgJBzTnsyWmaavlQXfBWZFjvRG()
		{
			NbbKpsORSgmjlgtQwhETKDaahzwb();
			qPEpWpsimakTvqYIDFAsjGGnSaOf();
			if (!Application.isPlaying)
			{
				LuIDCmWpaYCxWQpmQAGjawsQMUJY(true);
		...
		private void kYKRgqqgWOqEDeNGoCbnFhjkKfnD()
		{
			FTHhdpAhecWToVeSSraRmCPnRVvs.GetHandlers(base.transform);
			NqHFmMKKWHecBlgLmuVtzqLXGJHk.GetHandlers(base.transform);
		}

		internal virtual void OnPointerDown(PointerEventData eventData)
		{
			if (QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerDown))
			{
				wSxfeFYVbGcyfGkvSVMJDRcUjnIOA = true;
				vPLwwZScbMSKSuXMXuQiQJCkGUmW(eventData);
			}
		}

		internal virtual void OnPointerUp(PointerEventData eventData)
		{
			if (QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerUp))
			{
				wSxfeFYVbGcyfGkvSVMJDRcUjnIOA = false;
				vPLwwZScbMSKSuXMXuQiQJCkGUmW(eventData);
			}
		...
		{
			if (QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerEnter))
			{
				akdbiKEpVwTENWRfwWhcpLSbhGDQ = true;
				vPLwwZScbMSKSuXMXuQiQJCkGUmW(eventData);
			}
		}
		...
		{
			if (QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.PointerExit))
			{
				akdbiKEpVwTENWRfwWhcpLSbhGDQ = false;
				vPLwwZScbMSKSuXMXuQiQJCkGUmW(eventData);
			}
		...
		{
			QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.BeginDrag);
		}

		internal virtual void OnDrag(PointerEventData eventData)
		{
			QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.Drag);
		}

		internal virtual void OnEndDrag(PointerEventData eventData)
		{
			QFueiYlivNoOUGWavitZpFHwnfzU(eventData.pointerId, _allowedMouseButtons, EventTriggerType.EndDrag);
		}
		...
		internal static bool IZIBozfCzyDkZOWWaGlLiEtSPYyB(int P_0)
		{
			if (P_0 == int.MinValue)
			{
				return false;
			}
			if (DrDqLttSkyvwiIzPAjMBncpcYrPv(P_0))
			{
				int num = JAKDktiUnDWUqaINkvKeQMuiIknw(P_0);
				if (num < 0)
				{
					return false;
				}
			...
			if (kFwYjYQjSYfVGTyWeFqBXijYCzsB(P_0) && Input.mousePresent)
			{
				int num2 = WdPjxIUMcDctqwebvIjZIZOlAQyHA(P_0);
				if (num2 >= 0)
				{
		...
		internal static Vector3 RKVcirKVVrPBttTpsXyGcfygGdOQ(int P_0)
		{
			if (DrDqLttSkyvwiIzPAjMBncpcYrPv(P_0))
			{
				int num = JAKDktiUnDWUqaINkvKeQMuiIknw(P_0);
			...
			else if (kFwYjYQjSYfVGTyWeFqBXijYCzsB(P_0) && Input.mousePresent)
			{
		...
		internal static bool DrDqLttSkyvwiIzPAjMBncpcYrPv(int P_0)
		{
			return P_0 >= 0;
		}

		internal static bool kFwYjYQjSYfVGTyWeFqBXijYCzsB(int P_0)
		{
			if (P_0 != -1 && P_0 != -3)
			{
				return P_0 == -2;
		...
		private static int JAKDktiUnDWUqaINkvKeQMuiIknw(int P_0)
		{
			if (!DrDqLttSkyvwiIzPAjMBncpcYrPv(P_0))
			{
				return -1;
		...
		internal static bool QjozrJDpwVsQlzuWnoScLVVzFPrs(MouseButtonFlags P_0, int P_1)
		{
			if (kFwYjYQjSYfVGTyWeFqBXijYCzsB(P_1))
			{
				if (!Cursor.visible)
				{
					return false;
			...
			if (DrDqLttSkyvwiIzPAjMBncpcYrPv(P_1))
			{
				return true;
			}
			if (ssCUGBtMyQKhczqQbHBkNFRrnTHL(P_0, P_1))
			{
				return true;
			}
			return false;
		...
		private static bool ssCUGBtMyQKhczqQbHBkNFRrnTHL(MouseButtonFlags P_0, int P_1)
		{
			return P_1 switch
			{
		...
		private static int WdPjxIUMcDctqwebvIjZIZOlAQyHA(int P_0)
		{
			if (!kFwYjYQjSYfVGTyWeFqBXijYCzsB(P_0))
			{
				return -1;
			}
			return P_0 switch
		...
		internal static bool lkQTlwjROwXyeNpzHpZbVfPTZrXg(MouseButtonFlags P_0, out int P_1)
		{
			for (int i = 0; i < 3; i++)
			{
		...
		internal static bool QFueiYlivNoOUGWavitZpFHwnfzU(int P_0, MouseButtonFlags P_1, EventTriggerType P_2)
		{
			if (kFwYjYQjSYfVGTyWeFqBXijYCzsB(P_0) && (P_2 == EventTriggerType.PointerEnter || P_2 == EventTriggerType.PointerExit) && P_1 != MouseButtonFlags.None)
			{
				P_1 |= MouseButtonFlags.LeftButton;
			}
			return QjozrJDpwVsQlzuWnoScLVVzFPrs(P_1, P_0);
		}

		internal static bool sZOlWOgCsSeAIquqRVBCziBDPwZl(MouseButtonFlags P_0)
		{
			int num;
			return lkQTlwjROwXyeNpzHpZbVfPTZrXg(P_0, out num);
		}

		[CompilerGenerated]
		private void HxLnFYOqEPaONlfsBKndxfJXBCDhA(bool P_0)
		{
			_allowSendingEvents = P_0;
		}
	}
