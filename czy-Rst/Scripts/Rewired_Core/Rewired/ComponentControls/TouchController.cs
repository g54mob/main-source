...
namespace Rewired.ComponentControls
{
	...
	public sealed class TouchController : CustomController
	{
		...
		[NonSerialized]
		private bool cMeCjnGawwjhDPTQGiVajzGlweRc;
		...
		{
			base.OnDisable();
			if (base.aVrFdNyTWVUeoPzBhtEtmuMUQPob && ReInput.isReady)
			{
				jtUqyLwrQZLvLWITDyvvdGUpUjad(true);
			}
		}

		internal bool pmuzMCyGDuTlKifUMoJOqMKuBAYDA()
		{
			if (!OnInitialize())
			{
				return false;
			...
			if (ReInput.isReady)
			{
				cMeCjnGawwjhDPTQGiVajzGlweRc = ReInput.controllers.Mouse.enabled;
				jtUqyLwrQZLvLWITDyvvdGUpUjad(false);
			}
			return true;
		}
		...
		private void jtUqyLwrQZLvLWITDyvvdGUpUjad(bool P_0)
		{
			if (_disableMouseInputWhenEnabled)
			{
				if (P_0)
				{
					ReInput.controllers.Mouse.enabled = cMeCjnGawwjhDPTQGiVajzGlweRc;
				}
				else
				{
		...
		private void viHQPgxNLjaEdYAVJaQKaXSkTinQ()
		{
		}

		private bool PthYRrXbvzboVUASCKWtchevJImm()
		{
			if (ReInput.isReady)
			{
				return true;
