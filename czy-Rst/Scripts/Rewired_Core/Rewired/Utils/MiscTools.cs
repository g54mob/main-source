...
namespace Rewired.Utils
{
	...
	{
		private static uint kysNzDcqnGbFepGJKeuSUiYVJSVM = 0u;

		private static uint NVwedCMijOsjLJMnIAQVXIOhuDIl = 1u;

		private static uint NDLYwNaJtYjzmtJCeFkKzfAzdMpG = uint.MaxValue;

		private static int PrheTvZelpXcVFnHxUxFLfRJMKPX = 0;

		private static int ljViNeeOePjfrbBczVaTZYAwbJun = int.MaxValue;

		private static int xOXzQYZfAIqhkKBUzSDtSmRvdmLn = -1;
		...
		{
			counter = ((counter != NDLYwNaJtYjzmtJCeFkKzfAzdMpG) ? (counter + 1) : NVwedCMijOsjLJMnIAQVXIOhuDIl);
			return counter;
		}

		public static int Tick(int counter)
		{
			counter = ((counter != ljViNeeOePjfrbBczVaTZYAwbJun) ? (counter + 1) : PrheTvZelpXcVFnHxUxFLfRJMKPX);
			return counter;
		}

		public static uint TickPrev(uint counter)
		{
			counter = ((counter != NVwedCMijOsjLJMnIAQVXIOhuDIl) ? (counter - 1) : NDLYwNaJtYjzmtJCeFkKzfAzdMpG);
			return counter;
		}

		public static int TickPrev(int counter)
		{
			if (counter <= xOXzQYZfAIqhkKBUzSDtSmRvdmLn)
			{
				return ljViNeeOePjfrbBczVaTZYAwbJun;
			}
			counter = ((counter != PrheTvZelpXcVFnHxUxFLfRJMKPX) ? (counter - 1) : ljViNeeOePjfrbBczVaTZYAwbJun);
			return counter;
		}

		public static bool IsTickValid(uint tick)
		{
			return tick != kysNzDcqnGbFepGJKeuSUiYVJSVM;
		}

		public static bool IsTickValid(int tick)
		{
			return tick > xOXzQYZfAIqhkKBUzSDtSmRvdmLn;
		}
		...
		public static bool IsTickNewer(uint tick1, uint tick2)
		{
			...
			if (tick1 == kysNzDcqnGbFepGJKeuSUiYVJSVM)
			{
				return false;
			}
			if (tick2 == kysNzDcqnGbFepGJKeuSUiYVJSVM)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static bool IsTickNewer(int tick1, int tick2)
		{
			...
			if (tick1 <= xOXzQYZfAIqhkKBUzSDtSmRvdmLn)
			{
				return false;
			}
			if (tick2 <= xOXzQYZfAIqhkKBUzSDtSmRvdmLn)
			{
				return true;
			}
			if (tick1 < tick2)
		...
		public static long TickDifference(uint tick1, uint tick2)
		{
			...
			uint num3 = NDLYwNaJtYjzmtJCeFkKzfAzdMpG - num + num2;
			uint nVwedCMijOsjLJMnIAQVXIOhuDIl = NVwedCMijOsjLJMnIAQVXIOhuDIl;
			uint num4 = num3 - nVwedCMijOsjLJMnIAQVXIOhuDIl;
			if (tick1 >= tick2)
			{
				return 0L - (long)num4;
			}
		...
		public static int TickDifference(int tick1, int tick2)
		{
			...
			int num3 = ljViNeeOePjfrbBczVaTZYAwbJun - num + num2;
			int prheTvZelpXcVFnHxUxFLfRJMKPX = PrheTvZelpXcVFnHxUxFLfRJMKPX;
			int num4 = num3 - prheTvZelpXcVFnHxUxFLfRJMKPX;
			if (tick1 >= tick2)
			{
				return -num4;
			}
