...
namespace Rewired.Utils
{
	...
	{
		internal struct AvCdBwJqgMgreVrAsELpddWPgcmn
		{
			public Platform vRyLELIfDlwbBLAZevKKdGzrNfpC;

			public Platform DLnSvKkPXCHLgesiOZnChilGEszr;

			public EditorPlatform PCZYUhJPuEcvRknCwwgTQtQHzjQj;

			public bool WVmxxkoiRlGvxGAThGAxpeHootsb;

			public WebplayerPlatform mGchoPaLgZhtDKjeojOrWIDgcCxFb;

			public ScriptingBackend CDhfivBsBBljqmLVqpLgbSeawUsU;

			public ScriptingAPILevel HjRhouACEQNoGImSZotmRXPYfqJO;

			public IExternalTools AgaZpabsDxiHwDTJNQlpusxDOUah;

			public AvCdBwJqgMgreVrAsELpddWPgcmn(Platform P_0, Platform P_1, EditorPlatform P_2, bool P_3, WebplayerPlatform P_4, ScriptingBackend P_5, ScriptingAPILevel P_6, IExternalTools P_7)
			{
				vRyLELIfDlwbBLAZevKKdGzrNfpC = P_0;
				DLnSvKkPXCHLgesiOZnChilGEszr = P_1;
				PCZYUhJPuEcvRknCwwgTQtQHzjQj = P_2;
				WVmxxkoiRlGvxGAThGAxpeHootsb = P_3;
				mGchoPaLgZhtDKjeojOrWIDgcCxFb = P_4;
				CDhfivBsBBljqmLVqpLgbSeawUsU = P_5;
				HjRhouACEQNoGImSZotmRXPYfqJO = P_6;
				AgaZpabsDxiHwDTJNQlpusxDOUah = P_7;
			}
		}
		...
		{
			public enum yOgRwQXMdAZwhdjuzkjaOxyxFynh
			{
				Normal = 0,
				Beta = 1,
				Patch = 2
			...
			public readonly yOgRwQXMdAZwhdjuzkjaOxyxFynh type;

			public readonly int build;

			public UnityVersionClass(string P_0)
			{
				type = yOgRwQXMdAZwhdjuzkjaOxyxFynh.Normal;
				...
				if (Regex.IsMatch(text, ".*[a-zA-Z]+.*"))
				{
					...
					{
						type = yOgRwQXMdAZwhdjuzkjaOxyxFynh.Beta;
					}
					else if (Regex.IsMatch(text, ".*[pP]+.*", RegexOptions.IgnoreCase))
					{
						type = yOgRwQXMdAZwhdjuzkjaOxyxFynh.Patch;
					}
			...
			public override string ToString()
			{
				...
				array[5] = wqllkesnclSnclGpktpOGCdXMOkV(type);
				num = build;
				array[6] = num.ToString();
				return string.Concat(array);
			}

			private string wqllkesnclSnclGpktpOGCdXMOkV(yOgRwQXMdAZwhdjuzkjaOxyxFynh P_0)
			{
				return P_0 switch
				{
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Normal => "f", 
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Beta => "b", 
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Patch => "p", 
			...
			public static int Comparison(UnityVersionClass a, UnityVersionClass b)
			{
				...
				if (hNvPrNKJXvYvAYfOlDAIHfFBeitO(a.type) > hNvPrNKJXvYvAYfOlDAIHfFBeitO(b.type))
				{
					return 1;
				}
				if (hNvPrNKJXvYvAYfOlDAIHfFBeitO(a.type) < hNvPrNKJXvYvAYfOlDAIHfFBeitO(b.type))
			...
			private static int hNvPrNKJXvYvAYfOlDAIHfFBeitO(yOgRwQXMdAZwhdjuzkjaOxyxFynh P_0)
			{
				return P_0 switch
				{
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Beta => 0, 
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Normal => 10, 
					yOgRwQXMdAZwhdjuzkjaOxyxFynh.Patch => 100, 
					_ => throw new NotImplementedException(), 
				};
			}
		}

		private const UnityVersion lJqfNquTERHRnhyGQEPxNHJMOTzYA = UnityVersion.UNITY_5_0;

		private static UnityVersionClass EvZwqPCkwFFPPkoqVKtwMBEGqwSFA;

		private static UnityVersion lXSQPRGdMqttgHRelmFeWpvlbiTo = UnityVersion.Unknown;

		private static string dHOQCiJONWvrkWhStMJZhHkPEsnjA;

		private static Platform DzcSevbPmAjmOYROutmlJuokdyVV;

		private static EditorPlatform BtpJmRjnPHgjycWZoeArtnCkPjHgb;

		private static bool hGITsSFNpKYVDuwoIhJRdmFymGLX;

		private static bool MLfDKfjfgXtfBLIvBebaNXWnHnit;

		private static bool owQugpvAEhclCJpvpwwEKYhhlFDH;

		private static WebplayerPlatform gzAvZjCVWAlgUMNqwytYyEOcDCUO;

		private static bool TZlIKGlCCRGXVDLsFGOuDCZbqUAH;

		private static bool SfZYYopfmbhoiciHRkatHFyhFJlT;

		private static bool gObvHQZqTNEQpaAXbrNIpYRznYUL;

		private static bool NtVBpyLtHMMJhRQmaJJUXGFzDsLeA;

		private static bool yOQZCMISUSAuUdigARVJdlrWVdVS;

		private static bool huVhllLSWHCUuuubnwOZVPmBVmRD;

		private static string efLuUPgFXLcYlZJEqCqsauDcprWHA;

		private static ScriptingBackend QszvzClUEdUbCcZxJunVpecckeBF;

		private static ScriptingAPILevel DpErZQIaxsRJcTqZvtxOSTvFjsAY;

		private static bool lTUQYwLXvOAWpDoFzxaBMAWjBytD;

		private static IExternalTools cmMHyNjZChQXQCEvsSKApSoSacrZ;

		[CompilerGenerated]
		private static IAndroidFallbackPlatformHelper TeXCYyahWYbcxiixGaiGOlDCyRVYb;

		private static bool EpeHwZkVoDNlRUbACRQjSHIiGhRq;
		...
		internal static UnityVersionClass unityVersionObj
		{
			get
			{
				if (!DclSYUxNNXTqzMMLYaebhVtkqTgk)
				{
					return null;
				}
				return EvZwqPCkwFFPPkoqVKtwMBEGqwSFA;
			}
		}
		...
		public static UnityVersion unityVersion
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo;
			}
		}
		...
		public static string unityVersionString
		{
			get
			{
				...
				return dHOQCiJONWvrkWhStMJZhHkPEsnjA;
			}
		}
		...
		public static Platform platform
		{
			get
			{
				...
				return DzcSevbPmAjmOYROutmlJuokdyVV;
			}
		}
		...
		public static EditorPlatform editorPlatform
		{
			get
			{
				...
				return BtpJmRjnPHgjycWZoeArtnCkPjHgb;
			}
		}
		...
		public static bool isEditor
		{
			get
			{
				...
				return hGITsSFNpKYVDuwoIhJRdmFymGLX;
			}
		}

		public static bool isPlaying => MLfDKfjfgXtfBLIvBebaNXWnHnit;
		...
		public static bool isDebugBuild
		{
			get
			{
				...
				return owQugpvAEhclCJpvpwwEKYhhlFDH;
			}
		}
		...
		public static WebplayerPlatform webplayerPlatform
		{
			get
			{
				...
				return gzAvZjCVWAlgUMNqwytYyEOcDCUO;
			}
		}
		...
		public static bool logToDebugLog
		{
			get
			{
				...
				if (hGITsSFNpKYVDuwoIhJRdmFymGLX || Application.isEditor)
				{
					return true;
				}
				...
				switch (DzcSevbPmAjmOYROutmlJuokdyVV)
				{
				case Platform.Windows:
				case Platform.OSX:
				case Platform.Linux:
					if (!owQugpvAEhclCJpvpwwEKYhhlFDH)
					{
						return QszvzClUEdUbCcZxJunVpecckeBF == ScriptingBackend.IL2CPP;
					}
					return true;
		...
		internal static bool editorPlatformMatchesBuildPlatform
		{
			get
			{
				...
				return BtpJmRjnPHgjycWZoeArtnCkPjHgb switch
				{
					EditorPlatform.Windows => DzcSevbPmAjmOYROutmlJuokdyVV == Platform.Windows, 
					EditorPlatform.OSX => DzcSevbPmAjmOYROutmlJuokdyVV == Platform.OSX, 
					EditorPlatform.Linux => DzcSevbPmAjmOYROutmlJuokdyVV == Platform.Linux, 
					_ => true, 
				};
			}
		}
		...
		public static bool isSupportedVersion3
		{
			get
			{
				...
				return TZlIKGlCCRGXVDLsFGOuDCZbqUAH;
			}
		}
		...
		public static bool isSupportedVersion4
		{
			get
			{
				...
				return SfZYYopfmbhoiciHRkatHFyhFJlT;
			}
		}
		...
		public static bool supports2DColliders
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_3;
			}
		}
		...
		public static bool supportsUnityUI
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_6;
			}
		}
		...
		public static bool supportsTouchControls
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_0;
			}
		}
		...
		public static bool supportsPhysicalKeys
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_2021_2;
			}
		}
		...
		public static bool isAndroidPlatform
		{
			get
			{
				...
				if (DzcSevbPmAjmOYROutmlJuokdyVV != Platform.Android && DzcSevbPmAjmOYROutmlJuokdyVV != Platform.Ouya && DzcSevbPmAjmOYROutmlJuokdyVV != Platform.AmazonFireTV)
				{
					return DzcSevbPmAjmOYROutmlJuokdyVV == Platform.RazerForgeTV;
				}
				return true;
			}
		}
		...
		public static bool isIOSPlatform
		{
			get
			{
				...
				if (DzcSevbPmAjmOYROutmlJuokdyVV != Platform.iOS)
				{
					return DzcSevbPmAjmOYROutmlJuokdyVV == Platform.tvOS;
				}
				return true;
			}
		}
		...
		public static bool isStandalonePlatform
		{
			get
			{
				...
				if (DzcSevbPmAjmOYROutmlJuokdyVV != Platform.Windows && DzcSevbPmAjmOYROutmlJuokdyVV != Platform.Linux)
				{
					return DzcSevbPmAjmOYROutmlJuokdyVV == Platform.OSX;
				}
				return true;
			}
		}
		...
		public static bool windowsJoystickNamesReturnsEmptyStringsIfJoystickNull
		{
			get
			{
				...
				return gObvHQZqTNEQpaAXbrNIpYRznYUL;
			}
		}
		...
		public static bool supportsUnityUIGraphicRaycastTarget
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_2;
			}
		}
		...
		public static bool supportsNestedPrefabs
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_2018_3;
			}
		}
		...
		public static bool supportsWindowsAppStore
		{
			get
			{
				...
				if (lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_0)
				{
					return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_0_1;
				}
				return true;
			}
		}
		...
		public static bool supportsWindowsUWP_IL2CPP
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_3;
			}
		}
		...
		public static bool supportsXboxOne
		{
			get
			{
				...
				return lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_5;
			}
		}
		...
		public static bool windowsStandalone_supportsRawInputForwarding
		{
			get
			{
				...
				return lTUQYwLXvOAWpDoFzxaBMAWjBytD;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static ScriptingBackend scriptingBackend => QszvzClUEdUbCcZxJunVpecckeBF;

		[CustomObfuscation(rename = false)]
		internal static ScriptingAPILevel scriptingAPILevel => DpErZQIaxsRJcTqZvtxOSTvFjsAY;
		...
		public static IExternalTools externalTools
		{
			get
			{
				...
				return cmMHyNjZChQXQCEvsSKApSoSacrZ;
			}
		}

		internal static IAndroidFallbackPlatformHelper bogHKChnwWGVEctpAkleZxzXilUtA
		{
			[CompilerGenerated]
			get
			{
				return TeXCYyahWYbcxiixGaiGOlDCyRVYb;
			}
			[CompilerGenerated]
			set
			{
				TeXCYyahWYbcxiixGaiGOlDCyRVYb = teXCYyahWYbcxiixGaiGOlDCyRVYb;
			}
		}

		[CustomObfuscation(rename = false)]
		internal static bool isInitialized => EpeHwZkVoDNlRUbACRQjSHIiGhRq;

		private static bool DclSYUxNNXTqzMMLYaebhVtkqTgk => JkPeeATPiGtnakPvZVTffKnxWExE();

		private static bool JkPeeATPiGtnakPvZVTffKnxWExE()
		{
			if (EpeHwZkVoDNlRUbACRQjSHIiGhRq)
			{
				return true;
			}
			try
			{
				dHOQCiJONWvrkWhStMJZhHkPEsnjA = Application.unityVersion;
				EvZwqPCkwFFPPkoqVKtwMBEGqwSFA = new UnityVersionClass(dHOQCiJONWvrkWhStMJZhHkPEsnjA);
				TrILJoigzhkrDFRqRoRKMzrXJxQl();
				EpeHwZkVoDNlRUbACRQjSHIiGhRq = true;
			}
			catch
			{
				Logger.LogError("Could not determine Unity version.");
			}
			return EpeHwZkVoDNlRUbACRQjSHIiGhRq;
		}

		internal static void tpEOqexFnXfkJhDrrEnkWwcFlCwlA(AvCdBwJqgMgreVrAsELpddWPgcmn P_0)
		{
			if (DclSYUxNNXTqzMMLYaebhVtkqTgk)
			{
				if (P_0.DLnSvKkPXCHLgesiOZnChilGEszr == Platform.Windows81Store)
				{
					P_0.DLnSvKkPXCHLgesiOZnChilGEszr = Platform.WindowsUWP;
				}
				DzcSevbPmAjmOYROutmlJuokdyVV = P_0.DLnSvKkPXCHLgesiOZnChilGEszr;
				BtpJmRjnPHgjycWZoeArtnCkPjHgb = P_0.PCZYUhJPuEcvRknCwwgTQtQHzjQj;
				hGITsSFNpKYVDuwoIhJRdmFymGLX = P_0.WVmxxkoiRlGvxGAThGAxpeHootsb;
				gzAvZjCVWAlgUMNqwytYyEOcDCUO = P_0.mGchoPaLgZhtDKjeojOrWIDgcCxFb;
				QszvzClUEdUbCcZxJunVpecckeBF = P_0.CDhfivBsBBljqmLVqpLgbSeawUsU;
				DpErZQIaxsRJcTqZvtxOSTvFjsAY = P_0.HjRhouACEQNoGImSZotmRXPYfqJO;
				if (cmMHyNjZChQXQCEvsSKApSoSacrZ != null)
				{
					cmMHyNjZChQXQCEvsSKApSoSacrZ.Destroy();
				}
				cmMHyNjZChQXQCEvsSKApSoSacrZ = P_0.AgaZpabsDxiHwDTJNQlpusxDOUah;
				owQugpvAEhclCJpvpwwEKYhhlFDH = Debug.isDebugBuild;
				MLfDKfjfgXtfBLIvBebaNXWnHnit = true;
				KAfNBfVIoMyJeKudafHWIfTrBepF();
			}
		}
		...
		public static bool IsUnityVersionInRange(string minVersionStr, string maxVersionStr)
		{
			...
			QKOIljoxNjNxIyDiQbUdcMySxJowA(minVersionStr, out var num);
			QKOIljoxNjNxIyDiQbUdcMySxJowA(maxVersionStr, out var num2);
			if (num > 0)
			{
			...
			if (num3 && EvZwqPCkwFFPPkoqVKtwMBEGqwSFA < new UnityVersionClass(minVersionStr))
			{
				return false;
			}
			if (num2 > 0)
			{
				if (flag && EvZwqPCkwFFPPkoqVKtwMBEGqwSFA >= new UnityVersionClass(maxVersionStr))
				{
					return false;
				}
			}
			else if (flag && EvZwqPCkwFFPPkoqVKtwMBEGqwSFA > new UnityVersionClass(maxVersionStr))
			{
				return false;
			}
			return true;
		...
		private static bool QKOIljoxNjNxIyDiQbUdcMySxJowA(string P_0, out int P_1)
		{
			P_1 = 0;
		...
		private static void TrILJoigzhkrDFRqRoRKMzrXJxQl()
		{
			lXSQPRGdMqttgHRelmFeWpvlbiTo = xSXUIZGMOnkOrewwfTnMxytpboCc(Application.unityVersion);
			if (lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_3_5 && lXSQPRGdMqttgHRelmFeWpvlbiTo < UnityVersion.UNITY_4_0)
			{
				TZlIKGlCCRGXVDLsFGOuDCZbqUAH = true;
			}
			else if (lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_0)
			{
				SfZYYopfmbhoiciHRkatHFyhFJlT = true;
			}
		}

		private static UnityVersion xSXUIZGMOnkOrewwfTnMxytpboCc(string P_0)
		{
			if (string.IsNullOrEmpty(P_0))
			{
				return UnityVersion.Unknown;
		...
		private static UnityVersion YAPCHilkAKmmvKTjibpAVqDegNOH(int P_0)
		{
			return P_0 switch
			{
		...
		private static UnityVersion hIAakUIpXSfNKOTNyzxUmPlzKOFH(int P_0)
		{
			return P_0 switch
			{
		...
		private static void KAfNBfVIoMyJeKudafHWIfTrBepF()
		{
			switch (DzcSevbPmAjmOYROutmlJuokdyVV)
			{
			...
			case Platform.RazerForgeTV:
				NtVBpyLtHMMJhRQmaJJUXGFzDsLeA = true;
				yOQZCMISUSAuUdigARVJdlrWVdVS = true;
				break;
			case Platform.PS4:
				NtVBpyLtHMMJhRQmaJJUXGFzDsLeA = true;
				efLuUPgFXLcYlZJEqCqsauDcprWHA = "Empty";
				huVhllLSWHCUuuubnwOZVPmBVmRD = true;
				break;
			case Platform.Windows:
				if ((lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_6_3p1 && lXSQPRGdMqttgHRelmFeWpvlbiTo < UnityVersion.UNITY_5_0) || lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_0_0p1)
				{
					gObvHQZqTNEQpaAXbrNIpYRznYUL = true;
					NtVBpyLtHMMJhRQmaJJUXGFzDsLeA = true;
				}
				break;
			case Platform.Linux:
			...
			if (hGITsSFNpKYVDuwoIhJRdmFymGLX && BtpJmRjnPHgjycWZoeArtnCkPjHgb == EditorPlatform.Windows && ((lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_6_3p1 && lXSQPRGdMqttgHRelmFeWpvlbiTo < UnityVersion.UNITY_5_0) || lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_5_0_0p1))
			{
				gObvHQZqTNEQpaAXbrNIpYRznYUL = true;
				NtVBpyLtHMMJhRQmaJJUXGFzDsLeA = true;
			}
			if ((!hGITsSFNpKYVDuwoIhJRdmFymGLX && DzcSevbPmAjmOYROutmlJuokdyVV == Platform.Windows) || (hGITsSFNpKYVDuwoIhJRdmFymGLX && BtpJmRjnPHgjycWZoeArtnCkPjHgb == EditorPlatform.Windows))
			{
				lTUQYwLXvOAWpDoFzxaBMAWjBytD = lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_2021_2;
			}
		}

		internal static Type uumIgROTRDxNUBKrQgiJWSKSxgEA(TqTJDRSbKsVncdGiivBDNDhtaQMh P_0)
		{
			if (!DclSYUxNNXTqzMMLYaebhVtkqTgk)
			{
				return null;
			}
			if (lXSQPRGdMqttgHRelmFeWpvlbiTo >= UnityVersion.UNITY_4_3)
			{
				return pyodROIpkCbHrhqShebOMRYJMhDpb(P_0);
			}
			return null;
		}

		private static Type pyodROIpkCbHrhqShebOMRYJMhDpb(TqTJDRSbKsVncdGiivBDNDhtaQMh P_0)
		{
			return P_0 switch
			{
				TqTJDRSbKsVncdGiivBDNDhtaQMh.RigidbodyInterpolation2D => typeof(RigidbodyInterpolation2D), 
				TqTJDRSbKsVncdGiivBDNDhtaQMh.RigidbodySleepMode2D => typeof(RigidbodySleepMode2D), 
				TqTJDRSbKsVncdGiivBDNDhtaQMh.CollisionDetectionMode2D => typeof(CollisionDetectionMode2D), 
				TqTJDRSbKsVncdGiivBDNDhtaQMh.PhysicsMaterial2D => typeof(PhysicsMaterial2D), 
				TqTJDRSbKsVncdGiivBDNDhtaQMh.Collider2D => typeof(Collider2D), 
				_ => null, 
			};
		}
		...
		public static T GetComponent<T>(GameObject gameObject) where T : class
		{
			...
			return aahkfpAcwqClaiKuDYFsNvyFVArY(gameObject.GetComponent(typeof(T)) as T);
		}
		...
		public static T Instantiate<T>(UnityEngine.Object original, Vector3 position, Quaternion rotation, Transform parent, bool instantiateInWorldSpace) where T : UnityEngine.Object
		{
			...
			if (IsNullOrDestroyed(obj as T))
			{
				...
				{
					return aahkfpAcwqClaiKuDYFsNvyFVArY((obj as GameObject).GetComponent(typeof(T)) as T);
				}
				if (obj as Transform != null)
				{
					return aahkfpAcwqClaiKuDYFsNvyFVArY((obj as Transform).GetComponent(typeof(T)) as T);
				}
			}
			return aahkfpAcwqClaiKuDYFsNvyFVArY(obj as T);
		}
		...
		public static bool IsValidUnityJoystickName(string name)
		{
			...
			{
				if (ReInput.isWindowsStandaloneWebplayerOrEditorPlatform && gObvHQZqTNEQpaAXbrNIpYRznYUL)
				{
					return false;
				}
				if (NtVBpyLtHMMJhRQmaJJUXGFzDsLeA)
				{
					return false;
				}
			}
			...
			{
				if (huVhllLSWHCUuuubnwOZVPmBVmRD && name.Equals(efLuUPgFXLcYlZJEqCqsauDcprWHA, StringComparison.OrdinalIgnoreCase))
				{
					return false;
				}
				if (yOQZCMISUSAuUdigARVJdlrWVdVS && name.IndexOf("keyboard", 0, StringComparison.OrdinalIgnoreCase) >= 0)
				{
					return false;
				}
			}
		...
		private static _0001 aahkfpAcwqClaiKuDYFsNvyFVArY<_0001>(_0001 P_0) where _0001 : class
		{
			if (P_0 == null)
			{
				return null;
		...
		internal static ButtonStateFlags dKawKyqlqiRmdrcKERSdnfDnikoBA(KeyCode P_0)
		{
		...
		internal static ButtonStateFlags XsmRWvkcgJeGIMJpHoCVDggltoEg(string P_0)
		{
