...
namespace Rewired.Utils.Classes.Utility
{
	...
	{
		private const int pmJGuVGGRlGLsFaSYqVcMGVYFjPg = 1;

		private const int uiiAXrRvxkdBoCtKilEBmDXMEjHAb = 0;
		...
		private ulong zMVEEeOQUCawpClRjMBEmpkLFjEDA;

		private int BgBHXoPRBePJHwdgoAGOJGUCmcLRA;

		protected ulong InstanceCount => zMVEEeOQUCawpClRjMBEmpkLFjEDA;
		...
		{
			while (Interlocked.Exchange(ref BgBHXoPRBePJHwdgoAGOJGUCmcLRA, 1) != 0)
			{
				Thread.SpinWait(1);
			}
			_pool.Clear();
			...
			Interlocked.Exchange(ref BgBHXoPRBePJHwdgoAGOJGUCmcLRA, 0);
		}
		...
		private object emQkyeLIseBHFcMSzAQXJZVBYnhuA()
		{
			return Get();
		}
		...
		{
			//ILSpy generated this explicit interface implementation from .override directive in emQkyeLIseBHFcMSzAQXJZVBYnhuA
			return this.emQkyeLIseBHFcMSzAQXJZVBYnhuA();
		}

		private bool DgioWvnLVDIUDknkHVulXqfyIDtr(object P_0)
		{
		...
		{
			//ILSpy generated this explicit interface implementation from .override directive in DgioWvnLVDIUDknkHVulXqfyIDtr
			return this.DgioWvnLVDIUDknkHVulXqfyIDtr(P_0);
		}
		...
		{
			zMVEEeOQUCawpClRjMBEmpkLFjEDA = ((zMVEEeOQUCawpClRjMBEmpkLFjEDA < ulong.MaxValue) ? (zMVEEeOQUCawpClRjMBEmpkLFjEDA + 1) : 0);
			return zMVEEeOQUCawpClRjMBEmpkLFjEDA;
		}
	}
}
