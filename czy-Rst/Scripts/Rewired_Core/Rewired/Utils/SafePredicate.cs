...
namespace Rewired.Utils
{
	...
	{
		private T GXcManqkFxVkZnGSNfVdFCpyBQXjA;

		private bool UxcBSSyFTxvuiEaGIJUIyByiAGuiA;

		private static Action<object, Predicate<T>> gUpIOEDNesfzFbyGcQAFZrEQFJwEA;

		private static Action<object, Predicate<T>> invokeDelegate => FyUCpxMfsWateFdaJZaTuOTJyvnIA;
		...
		{
			GXcManqkFxVkZnGSNfVdFCpyBQXjA = arg0;
			try
			{
				Invoke(invokeDelegate);
				return UxcBSSyFTxvuiEaGIJUIyByiAGuiA;
			}
			catch
			{
			...
			{
				GXcManqkFxVkZnGSNfVdFCpyBQXjA = default(T);
				UxcBSSyFTxvuiEaGIJUIyByiAGuiA = false;
			}
		}
		...
		private static void FyUCpxMfsWateFdaJZaTuOTJyvnIA(object P_0, Predicate<T> P_1)
		{
			if (P_1 != null && P_0 is SafePredicate<T> safePredicate)
			{
				safePredicate.UxcBSSyFTxvuiEaGIJUIyByiAGuiA = P_1(safePredicate.GXcManqkFxVkZnGSNfVdFCpyBQXjA);
			}
		}
