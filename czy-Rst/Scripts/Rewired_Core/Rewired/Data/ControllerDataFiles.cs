...
namespace Rewired.Data
{
	public sealed class ControllerDataFiles : ScriptableObject
	{
		...
		[NonSerialized]
		private bool TIcaYRZpAUqqhgGOYKEVBGwmBYc;

		[NonSerialized]
		private readonly ADictionary<Guid, WRScrEekSojpdBXyEFARqvkVFcPPb> xALzFfakistXRtiroykyhaYnFosr = new ADictionary<Guid, WRScrEekSojpdBXyEFARqvkVFcPPb>(EqualityComparerNoAlloc<Guid>.Default);
		...
		internal ControllerTemplateElementIdentifier dXKjXduMTUsejefoEAVtAuQHAvLuA(Guid P_0, int P_1, out HardwareJoystickMap P_2)
		{
			P_2 = null;
			if (P_1 < 0)
			{
			...
			{
				WRScrEekSojpdBXyEFARqvkVFcPPb wRScrEekSojpdBXyEFARqvkVFcPPb = jmIIHVhuciwhwfawEGIykNHwVjFw(templateGuid);
				if (wRScrEekSojpdBXyEFARqvkVFcPPb != null)
				{
					ControllerTemplateElementIdentifier controllerTemplateElementIdentifier = wRScrEekSojpdBXyEFARqvkVFcPPb.iOfeqbJMUGEbLAYcEKWdoWXjwQwjb(P_0, P_1);
		...
		internal int joEbdxxCjJftaoMFFlzUoPFsSzMJ(Guid P_0, Guid P_1, int P_2, List<HardwareControllerTemplateMap.bSeoxkAXPAPaGzvxBXEzIEDOfGVn> P_3)
		{
			if (P_2 < 0)
			{
				return 0;
			...
			return jmIIHVhuciwhwfawEGIykNHwVjFw(P_0)?.GmrpXIezcZGwIxbrhYOTnHUqsfmK(P_1, P_2, P_3) ?? 0;
		}

		internal HardwareJoystickMap_InputManager phiAqYaMZyDTStMfJMbIwkqnvGEn(Guid P_0, InputSource P_1)
		{
			unjxYrhLZvdOMzDFPcWyGVgXpSCAb();
			...
			if (hardwareJoystickMap != null)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = aVdgyjoyQBkRcwQEcBfsXGkzivvK(hardwareJoystickMap, bridgedController, true, out inputPlatform, out num, out platform);
		...
		internal HardwareJoystickMap_InputManager zhWfYoeUlKoYGQLdjyueeIDwHVGoA(BridgedControllerHWInfo P_0)
		{
			if (P_0 == null)
			{
				return null;
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = EZfPiAUNEggYkacVWdlRWTEDgbBZ(P_0);
				if (hardwareJoystickMap_InputManager != null)
				{
					return hardwareJoystickMap_InputManager;
				}
			...
			for (int i = 0; i < hardwareJoystickMaps.Length; i++)
			{
				...
				int num;
				HardwareJoystickMap.Platform platform;
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager2 = aVdgyjoyQBkRcwQEcBfsXGkzivvK(hardwareJoystickMaps[i], P_0, true, out inputPlatform, out num, out platform);
			...
			for (int j = 0; j < hardwareJoystickMaps.Length; j++)
			{
				...
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager3 = aVdgyjoyQBkRcwQEcBfsXGkzivvK(hardwareJoystickMaps[j], P_0, false, out inputPlatform2, out num2, out platform2);
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager4 = zobBdJLncTQUeIhlDZtwJbRymFaY(P_0, "[UNITY PRECONFIGURED JOYSTICK]");
			...
			{
				HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager5 = EZfPiAUNEggYkacVWdlRWTEDgbBZ(P_0);
		...
		private HardwareJoystickMap_InputManager aVdgyjoyQBkRcwQEcBfsXGkzivvK(HardwareJoystickMap P_0, BridgedControllerHWInfo P_1, bool P_2, out InputPlatform P_3, out int P_4, out HardwareJoystickMap.Platform P_5)
		{
		...
		private HardwareJoystickMap_InputManager zobBdJLncTQUeIhlDZtwJbRymFaY(BridgedControllerHWInfo P_0, string P_1)
		{
		...
		private HardwareJoystickMap_InputManager EZfPiAUNEggYkacVWdlRWTEDgbBZ(BridgedControllerHWInfo P_0)
		{
			HardwareJoystickMap_InputManager hardwareJoystickMap_InputManager = zobBdJLncTQUeIhlDZtwJbRymFaY(P_0, "[STANDARDIZED GAMEPAD]");
			if (hardwareJoystickMap_InputManager == null)
			{
				return null;
			}
		...
		internal WRScrEekSojpdBXyEFARqvkVFcPPb jmIIHVhuciwhwfawEGIykNHwVjFw(Guid P_0)
		{
			if (xALzFfakistXRtiroykyhaYnFosr.TryGetValue(P_0, out var value))
			{
				return value;
			}
			...
			value = joystickTemplate.SUFEzFdfSjFmkiJVcpGhAFpgGRjwA();
			xALzFfakistXRtiroykyhaYnFosr.Add(P_0, value);
			return value;
		}

		internal IHardwareControllerTemplateMap mIcqijShwPhGhdGdRwEjKGtmlewv(Guid P_0)
		{
			return jmIIHVhuciwhwfawEGIykNHwVjFw(P_0);
		}

		private void unjxYrhLZvdOMzDFPcWyGVgXpSCAb()
		{
			if (!TIcaYRZpAUqqhgGOYKEVBGwmBYc)
			{
				...
				{
					TIcaYRZpAUqqhgGOYKEVBGwmBYc = true;
				}
			}
		}
	}
