...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_LogitechGRacingWheelPedals : CustomCalculation
	{
		...
		[NonSerialized]
		private Mode SBtTXLjATvtvjylCfuFnOHBqwqCq;

		TypeWrapper.DataType SerializedMethod.ResultType => TypeWrapper.DataType.Single;

		internal bool FYHKKIbqdJrIEGMKpFzuSqviDTXE()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				_result = cJQXTtIJpCAaIGfiTLvePXbzKpGfA();
				flag = true;
			}
			ClearData();
			_resultIsValid = flag;
		...
		private float cJQXTtIJpCAaIGfiTLvePXbzKpGfA()
		{
			if (base.DataCount < 2)
			{
				return 0f;
			...
			float num = _data[0];
			float num2 = _data[1];
			ceSFoWHUIEYHSNaJQtElPbtmsrKx(num, num2);
			if (SBtTXLjATvtvjylCfuFnOHBqwqCq == Mode.SharedAxis)
			{
				float oldValue = num2;
			...
			else if (SBtTXLjATvtvjylCfuFnOHBqwqCq == Mode.SeparateAxes)
			{
				result = num;
			}
			return result;
		...
		private void ceSFoWHUIEYHSNaJQtElPbtmsrKx(float P_0, float P_1)
		{
			switch (SBtTXLjATvtvjylCfuFnOHBqwqCq)
			{
			case Mode.SharedAxis:
				if (MathTools.Abs(P_0) >= 0.01f && MathTools.Abs(P_1) <= 0.01f)
				{
					SBtTXLjATvtvjylCfuFnOHBqwqCq = Mode.SeparateAxes;
				}
				break;
			case Mode.SeparateAxes:
				if (MathTools.Abs(P_1) >= 0.01f && MathTools.Abs(P_0) <= 0.01f)
				{
					SBtTXLjATvtvjylCfuFnOHBqwqCq = Mode.SharedAxis;
				}
				break;
			}
		}
