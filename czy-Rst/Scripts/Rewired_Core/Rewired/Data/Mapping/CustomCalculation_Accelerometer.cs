...
namespace Rewired.Data.Mapping
{
	...
	public sealed class CustomCalculation_Accelerometer : CustomCalculation
	{
		...
		internal bool sEYHVibAUVylsGSKFYGoeoWHaIph()
		{
			bool flag = false;
			ClearResult();
			if (base.DataCount >= 1)
			{
				...
				case CalculationType.Pitch:
					_result = ZYLSILYMUyHPyGNhiEDLSpQQBPQdA();
					flag = true;
					break;
				case CalculationType.Roll:
					_result = rGSmPSdoRPVUFPBOLtqHqAEnyQEB();
					flag = true;
					break;
				}
			}
		...
		private float ZYLSILYMUyHPyGNhiEDLSpQQBPQdA()
		{
			...
			return _outputType switch
			{
				...
				OutputType.Axis => WNnCClbndxlMOLOOTaPLHeIvVQFPA(num2), 
				_ => 0f, 
			};
		}

		private float rGSmPSdoRPVUFPBOLtqHqAEnyQEB()
		{
			Vector3 vector = default(Vector3);
			int num = MathTools.Min(base.DataCount, 3);
			for (int i = 0; i < num; i++)
		...
		private float WNnCClbndxlMOLOOTaPLHeIvVQFPA(float P_0)
		{
			if (P_0 == 0f)
			{
				return 0f;
