...
namespace Rewired
{
	...
	public sealed class InputBehavior
	{
		...
		public InputBehavior(InputBehavior P_0)
			: this()
		{
			arZSNCItzrfjsvesVwApwYAPFgnJ(P_0, this, true);
		}
		...
		public string ToXmlString()
		{
			try
			{
				return KRVBMquvREgfPqzgWFfysCJIbJeW().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public bool ImportXmlString(string xmlString)
		{
			try
			{
				VYeOxhmFQfpiFTIczaLsFdgAEtjbb(SerializedObject.FromXml(GetType(), xmlString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			try
			{
				return KRVBMquvREgfPqzgWFfysCJIbJeW().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportJsonString(string jsonString)
		{
			try
			{
				VYeOxhmFQfpiFTIczaLsFdgAEtjbb(SerializedObject.FromJson(GetType(), jsonString));
				return true;
			}
			catch (Exception ex)
			{
		...
		public bool ImportData(InputBehavior inputBehavior)
		{
			...
			arZSNCItzrfjsvesVwApwYAPFgnJ(inputBehavior, this, false);
			return true;
		}
		...
		public void Reset()
		{
			InputBehavior inputBehavior = ReInput.mapping.SackBhJauJUmRxJwYfyNeCmbMjbh(_id);
			if (inputBehavior != null)
			{
				arZSNCItzrfjsvesVwApwYAPFgnJ(inputBehavior, this, true);
			}
		}

		internal SerializedObject KRVBMquvREgfPqzgWFfysCJIbJeW()
		{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.HSMGFcRrEEtwLPynqpRDQWJesQYg
			{
				...
				lPGTilhMaDlHVZPffTpyFffKvRGC = 5.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.HSMGFcRrEEtwLPynqpRDQWJesQYg
			{
				ielDRFPPVThNrLWgcnBdvoVjXqeg = "id",
				lPGTilhMaDlHVZPffTpyFffKvRGC = _id.ToString()
			});
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.HSMGFcRrEEtwLPynqpRDQWJesQYg
			{
			...
			serializedObject.xmlInfo.attributes.Add(new SerializedObject.XmlInfo.HSMGFcRrEEtwLPynqpRDQWJesQYg
			{
				...
				MFDdXiyHcPkUibxNoPMtNRhjvlXA = null,
				lPGTilhMaDlHVZPffTpyFffKvRGC = string.Format("{0} {1}{2}{3}{4}{5}", "http://guavaman.com/rewired", "http://guavaman.com/schemas/rewired/", "1.4", "/", GetType().Name, ".xsd")
			});
		...
		internal void VYeOxhmFQfpiFTIczaLsFdgAEtjbb(SerializedObject P_0)
		{
			Reset();
		...
		private static void arZSNCItzrfjsvesVwApwYAPFgnJ(InputBehavior P_0, InputBehavior P_1, bool P_2)
		{
			if (P_2)
			{
				P_1._id = P_0._id;
