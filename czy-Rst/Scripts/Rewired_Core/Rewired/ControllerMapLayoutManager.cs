...
namespace Rewired
{
	...
	{
		internal class SLxTBaXfrhZCyLfCEqNyvKbuXYzr
		{
			public bool DmXlRoEPccVSndVLFWtrGwWYLKyF;

			public bool ffJvcLUjemNcZCrNmfRfFzCfTDqL;

			public LWDgTQCdATINjrbviHrtIhbeAymU[] NlmWkIfGOEUqiwYlaTwShfDpBgPo;

			public SLxTBaXfrhZCyLfCEqNyvKbuXYzr(bool P_0, bool P_1, LWDgTQCdATINjrbviHrtIhbeAymU[] P_2)
			{
				DmXlRoEPccVSndVLFWtrGwWYLKyF = P_0;
				ffJvcLUjemNcZCrNmfRfFzCfTDqL = P_1;
				NlmWkIfGOEUqiwYlaTwShfDpBgPo = P_2;
			}
		}
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			public ControllerSetSelector controllerSetSelector
			{
				...
				set
				{
					...
					if (!value.QfKPjZJacbKsLddxcfkcEFxqPKEoA)
					{
		...
		private bool eJpiueJUrzolPjiTlntHSEwuYDcC;

		private bool gXlWOqPMYYgyRguhAeAJqAclnqqVA = true;

		private Player dWAcOWknkbwnGAQGwzRjtyJRVUWy;

		private SLxTBaXfrhZCyLfCEqNyvKbuXYzr jWYcKscpzyuSuGoNwTZHtZXXHeqm;

		private readonly int kBBVMpWJkppRrHrkEfvqiKfcBxeE;

		private List<RuleSet> epvlGPUxBSFjpAPZfyznCqsWwomf;

		private Action vhreOBDRYicwcpuWGJuqpvcTOZeDb;

		public bool enabled
		{
			get
			{
				return eJpiueJUrzolPjiTlntHSEwuYDcC;
			}
			set
			{
				eJpiueJUrzolPjiTlntHSEwuYDcC = value;
				if (value)
				{
					Apply();
				}
		...
		public bool loadFromUserDataStore
		{
			get
			{
				return gXlWOqPMYYgyRguhAeAJqAclnqqVA;
			}
			set
			{
				gXlWOqPMYYgyRguhAeAJqAclnqqVA = value;
			}
		}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return epvlGPUxBSFjpAPZfyznCqsWwomf;
			}
			set
			{
				if (value == null)
				...
				epvlGPUxBSFjpAPZfyznCqsWwomf = value;
			}
		}

		internal event Action ehSAyjNmedqbmedMJpcyZCbhtRlW
		{
			add
			{
				vhreOBDRYicwcpuWGJuqpvcTOZeDb = (Action)Delegate.Combine(vhreOBDRYicwcpuWGJuqpvcTOZeDb, b);
			}
			remove
			{
				vhreOBDRYicwcpuWGJuqpvcTOZeDb = (Action)Delegate.Remove(vhreOBDRYicwcpuWGJuqpvcTOZeDb, value2);
			}
		}

		internal ControllerMapLayoutManager(Player P_0, SLxTBaXfrhZCyLfCEqNyvKbuXYzr P_1)
		{
			if (P_0 == null)
			{
			...
			kBBVMpWJkppRrHrkEfvqiKfcBxeE = ReInput.id;
			dWAcOWknkbwnGAQGwzRjtyJRVUWy = P_0;
			jWYcKscpzyuSuGoNwTZHtZXXHeqm = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != kBBVMpWJkppRrHrkEfvqiKfcBxeE)
			{
				ReInput.CheckInitialized(kBBVMpWJkppRrHrkEfvqiKfcBxeE);
				return;
			}
			vhreOBDRYicwcpuWGJuqpvcTOZeDb?.Invoke();
			if (!eJpiueJUrzolPjiTlntHSEwuYDcC || epvlGPUxBSFjpAPZfyznCqsWwomf == null)
			{
				return;
			}
			int count = epvlGPUxBSFjpAPZfyznCqsWwomf.Count;
			if (count == 0)
			{
				return;
			}
			...
			dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.maps.GetAllMaps(list);
			list2.AddRange(dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.Controllers);
			IControllerMapStore controllerMapStore = ReInput.userDataStore as IControllerMapStore;
			for (int i = 0; i < count; i++)
			{
				RuleSet ruleSet = epvlGPUxBSFjpAPZfyznCqsWwomf[i];
				...
				for (int j = 0; j < num; j++)
				{
					...
					for (int num2 = list.Count - 1; num2 >= 0; num2--)
					{
						...
						{
							list.RemoveAt(num2);
							dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.maps.RemoveMap(controllerMap.controllerType, controllerMap.controllerId, controllerMap.id);
						}
					}
					foreach (Controller controller in dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.Controllers)
					{
						...
						{
							ControllerMap controllerMap2 = dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.maps.GetMap(controller, categoryIds[k], rule.layoutId);
							if (controllerMap2 != null)
							{
								continue;
							}
							if (gXlWOqPMYYgyRguhAeAJqAclnqqVA && controllerMapStore != null)
							{
								try
								{
									controllerMap2 = controllerMapStore.LoadControllerMap(dWAcOWknkbwnGAQGwzRjtyJRVUWy.id, controller.identifier, categoryIds[k], rule.layoutId);
								}
								catch (Exception exception)
								{
								...
								if (controllerMap2 != null)
								{
									dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.maps.AddMap(controller, controllerMap2);
									continue;
								}
							}
							dWAcOWknkbwnGAQGwzRjtyJRVUWy.controllers.maps.LoadMap(controller.type, controller.id, categoryIds[k], rule.layoutId, startEnabled: true);
						}
					}
				}
			}
		...
		public void LoadDefaults()
		{
			...
			int num = ((jWYcKscpzyuSuGoNwTZHtZXXHeqm != null && jWYcKscpzyuSuGoNwTZHtZXXHeqm.NlmWkIfGOEUqiwYlaTwShfDpBgPo != null) ? jWYcKscpzyuSuGoNwTZHtZXXHeqm.NlmWkIfGOEUqiwYlaTwShfDpBgPo.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapLayoutManagerRuleSetInstance = ReInput.mapping.GetControllerMapLayoutManagerRuleSetInstance(jWYcKscpzyuSuGoNwTZHtZXXHeqm.NlmWkIfGOEUqiwYlaTwShfDpBgPo[i].XcDxFXSZiKKiCttsQAFuceepVFxd);
				...
				controllerMapLayoutManagerRuleSetInstance.enabled = jWYcKscpzyuSuGoNwTZHtZXXHeqm.NlmWkIfGOEUqiwYlaTwShfDpBgPo[i].byQfuRjxcPRPWaVymOvuMdzHEFSV;
				list.Add(controllerMapLayoutManagerRuleSetInstance);
			}
			if (jWYcKscpzyuSuGoNwTZHtZXXHeqm != null)
			{
				eJpiueJUrzolPjiTlntHSEwuYDcC = jWYcKscpzyuSuGoNwTZHtZXXHeqm.DmXlRoEPccVSndVLFWtrGwWYLKyF;
				gXlWOqPMYYgyRguhAeAJqAclnqqVA = jWYcKscpzyuSuGoNwTZHtZXXHeqm.ffJvcLUjemNcZCrNmfRfFzCfTDqL;
			}
			epvlGPUxBSFjpAPZfyznCqsWwomf = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return VXKxLvtoMbPXpOflmBdxkmxBBdVl().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return VXKxLvtoMbPXpOflmBdxkmxBBdVl().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				BQvEZAnEYvkRuNJyEwQCmbYILBxH(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				BQvEZAnEYvkRuNJyEwQCmbYILBxH(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject VXKxLvtoMbPXpOflmBdxkmxBBdVl()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			vkCAQuHNUahaeCisvpKFocRPOcylA(serializedObject);
			return serializedObject;
		}

		private void vkCAQuHNUahaeCisvpKFocRPOcylA(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", eJpiueJUrzolPjiTlntHSEwuYDcC);
			P_0.Add("loadFromUserDataStore", gXlWOqPMYYgyRguhAeAJqAclnqqVA);
			P_0.Add("ruleSets", epvlGPUxBSFjpAPZfyznCqsWwomf);
		}

		private bool BQvEZAnEYvkRuNJyEwQCmbYILBxH(SerializedObject P_0)
		{
			eJpiueJUrzolPjiTlntHSEwuYDcC = false;
			epvlGPUxBSFjpAPZfyznCqsWwomf = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref eJpiueJUrzolPjiTlntHSEwuYDcC);
			P_0.TryGetDeserializedValueByRef("loadFromUserDataStore", ref gXlWOqPMYYgyRguhAeAJqAclnqqVA);
