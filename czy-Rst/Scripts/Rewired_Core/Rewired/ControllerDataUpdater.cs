...
namespace Rewired
{
	...
	internal class ControllerDataUpdater
	{
		...
		private readonly UnknownControllerHat[] CBldBcveiRghkBeVVGBBIVubWvpVb;
		...
		public ControllerDataUpdater(InputSource P_0, int P_1, int P_2, UnknownControllerHat[] P_3)
		{
			...
			axisCount = P_1;
			buttonCount = P_2;
			CBldBcveiRghkBeVVGBBIVubWvpVb = P_3;
		...
		{
			if (CBldBcveiRghkBeVVGBBIVubWvpVb == null)
			{
				return false;
			}
			for (int i = 0; i < CBldBcveiRghkBeVVGBBIVubWvpVb.Length; i++)
			{
				if (CBldBcveiRghkBeVVGBBIVubWvpVb[i].IsButtonIndexCardinal(buttonIndex))
				{
					return true;
				}
			}
		...
		public UnknownControllerHat.HatButtons GetUnknownHatButtons(int buttonIndex)
		{
			...
			for (int i = 0; i < CBldBcveiRghkBeVVGBBIVubWvpVb.Length; i++)
			{
				if (CBldBcveiRghkBeVVGBBIVubWvpVb[i].ContainsButtonIndex(buttonIndex))
				{
					return CBldBcveiRghkBeVVGBBIVubWvpVb[i].GetButtons();
				}
			}
			return null;
		}
