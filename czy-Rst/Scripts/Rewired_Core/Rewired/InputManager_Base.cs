...
namespace Rewired
{
	...
	{
		private enum YWcsYyVMnZDbIeElnMJPNZnJDHNl
		{
			Initialization = 0,
			Update = 1,
			Destroy = 2
		...
		internal struct gAovxugbbzdKCKlVLTYrDvJFjNlo
		{
			public Platform eJueoggTXeOJlUYUIeuniGZSJExt;

			public EditorPlatform dVlHgdgewjfUuPryduWmfpYwBuUE;

			public WebplayerPlatform paJVEGkpcLbfvVVreDBYuTRzaCtN;
		}

		private sealed class yeuOXKfabqqdzwGmxXQiPfHfoBHu
		{
			public InputManager_Base gFlZWzdMITpJgWkUqcumhhLdqith;

			public UnityTools.AvCdBwJqgMgreVrAsELpddWPgcmn HUWNRnGBFdkQqEIpATOyGIuNpham;

			public string zGwaHHOedOwjZvFIbBgfRlNifJGl;

			internal void mpEgVNdiMhVeBNYPZBVpcqCKeSywB(gAovxugbbzdKCKlVLTYrDvJFjNlo P_0)
			{
				gFlZWzdMITpJgWkUqcumhhLdqith.platform = P_0.eJueoggTXeOJlUYUIeuniGZSJExt;
				gFlZWzdMITpJgWkUqcumhhLdqith.editorPlatform = P_0.dVlHgdgewjfUuPryduWmfpYwBuUE;
				gFlZWzdMITpJgWkUqcumhhLdqith.webplayerPlatform = P_0.paJVEGkpcLbfvVVreDBYuTRzaCtN;
				HUWNRnGBFdkQqEIpATOyGIuNpham.DLnSvKkPXCHLgesiOZnChilGEszr = P_0.eJueoggTXeOJlUYUIeuniGZSJExt;
				HUWNRnGBFdkQqEIpATOyGIuNpham.PCZYUhJPuEcvRknCwwgTQtQHzjQj = P_0.dVlHgdgewjfUuPryduWmfpYwBuUE;
				HUWNRnGBFdkQqEIpATOyGIuNpham.mGchoPaLgZhtDKjeojOrWIDgcCxFb = P_0.paJVEGkpcLbfvVVreDBYuTRzaCtN;
				UnityTools.tpEOqexFnXfkJhDrrEnkWwcFlCwlA(HUWNRnGBFdkQqEIpATOyGIuNpham);
				zGwaHHOedOwjZvFIbBgfRlNifJGl = null;
			}

			internal UnityTools.AvCdBwJqgMgreVrAsELpddWPgcmn VxOWmFzALeBaSAgPlcHXygHDpPKY()
			{
				return HUWNRnGBFdkQqEIpATOyGIuNpham;
			}
		}
		...
		[NonSerialized]
		private bool whhKsnUcvHuYLvyNRrWlWZDNIWBd;

		[NonSerialized]
		private bool CKxLoEVsxyVblsTfgLeExXLmxbfk;
		...
		[NonSerialized]
		private bool IWMQXXjeNHpbzGKUeqGUjUOjcYHq;

		private bool lsmqfEHGdpISjPRqIQThpEPxCDeBA;
		...
		{
			if ((Application.isPlaying || _userData.ConfigVars.runInEditMode) && (!Application.isPlaying || lsmqfEHGdpISjPRqIQThpEPxCDeBA) && !whhKsnUcvHuYLvyNRrWlWZDNIWBd && !CKxLoEVsxyVblsTfgLeExXLmxbfk)
			{
				qHSGgQeEuPyTVCcSuMfqYvaKYkdLA();
			}
		}
		...
		private void OnDisable()
		{
			...
			{
				euzHntncUQQxuskfClbOKTWTyNAb(true);
			}
		}
		...
		{
			euzHntncUQQxuskfClbOKTWTyNAb(false);
		}

		private void euzHntncUQQxuskfClbOKTWTyNAb(bool P_0)
		{
			whhKsnUcvHuYLvyNRrWlWZDNIWBd = false;
			IWMQXXjeNHpbzGKUeqGUjUOjcYHq = false;
			CKxLoEVsxyVblsTfgLeExXLmxbfk = false;
			try
			{
				if (ReInput.rewiredInputManager == this)
				{
					ReInput.btfHcAgCHOXvXAviVBHVqFUZFozY();
				}
			}
			catch (Exception ex)
			{
				iXiUufBumMUQTComjmiPrBumIAtu(YWcsYyVMnZDbIeElnMJPNZnJDHNl.Destroy, "destruction", ex);
			}
			OnDeinitialized();
		}
		...
		{
			if (!IWMQXXjeNHpbzGKUeqGUjUOjcYHq)
			{
				ReInput.sFbDNbNNWsZbRDgBLjfkLwKmsjrT(isFocused);
				_ = whhKsnUcvHuYLvyNRrWlWZDNIWBd;
			}
		}
		...
		private void OnApplicationPause(bool isPaused)
		{
			if (!IWMQXXjeNHpbzGKUeqGUjUOjcYHq)
			{
				ReInput.KcgCeWGmtpVeZwrqrfZDdNOuwZWn(isPaused);
				_ = whhKsnUcvHuYLvyNRrWlWZDNIWBd;
			}
		}
		...
		private void Start()
		{
			if ((!UnityTools.isEditor || Application.isPlaying) && whhKsnUcvHuYLvyNRrWlWZDNIWBd && !CKxLoEVsxyVblsTfgLeExXLmxbfk)
			{
				ReInput.FTKOPNPjVzGYtcIaIYyWOsBHawpaA();
			}
		}
		...
		private void Update()
		{
			if (whhKsnUcvHuYLvyNRrWlWZDNIWBd && !CKxLoEVsxyVblsTfgLeExXLmxbfk && (!UnityTools.isEditor || Application.isPlaying) && _userData.ConfigVars.updateMode != UpdateMode.Manual)
			{
		...
		{
			if (!whhKsnUcvHuYLvyNRrWlWZDNIWBd || CKxLoEVsxyVblsTfgLeExXLmxbfk || (UnityTools.isEditor && !Application.isPlaying))
			{
				return;
			}
			try
			{
				ReInput.XhUBkfACghuNiQmSGcrcgFSkIQly();
			}
			catch (Exception ex)
			{
				iXiUufBumMUQTComjmiPrBumIAtu(YWcsYyVMnZDbIeElnMJPNZnJDHNl.Update, "update (Late Update)", ex);
			}
		}

		internal void OnGUIUpdate()
		{
			if (whhKsnUcvHuYLvyNRrWlWZDNIWBd && !CKxLoEVsxyVblsTfgLeExXLmxbfk && (!UnityTools.isEditor || Application.isPlaying) && _userData.ConfigVars.updateMode != UpdateMode.Manual && (_userData.ConfigVars.updateLoop & UpdateLoopSetting.OnGUI) != UpdateLoopSetting.None)
			{
		...
		{
			if (!whhKsnUcvHuYLvyNRrWlWZDNIWBd || CKxLoEVsxyVblsTfgLeExXLmxbfk)
			{
				return;
			}
			try
			{
				CheckRecompile();
				ReInput.tOgHoLRobXhMmcDcEEODOIlxwilH(updateLoopType);
				if ((_userData.ConfigVars.updateLoop & updateLoopSettingBit) != UpdateLoopSetting.None)
				{
					ReInput.tOBaUBzDiNesOQThghoARSYwjdIP(updateLoopType);
				}
			}
			catch (Exception ex)
			{
				iXiUufBumMUQTComjmiPrBumIAtu(YWcsYyVMnZDbIeElnMJPNZnJDHNl.Update, "update (" + updateLoopType.ToString() + ")", ex);
			}
		}

		internal void TryStartRunInEditMode()
		{
			if (whhKsnUcvHuYLvyNRrWlWZDNIWBd || !Application.isEditor || Application.isPlaying)
			{
				return;
			}
			if (ReInput.isReady)
		...
		private bool qHSGgQeEuPyTVCcSuMfqYvaKYkdLA()
		{
			if (whhKsnUcvHuYLvyNRrWlWZDNIWBd)
			{
				return true;
			}
			NdmEBDMVxJYtHOostGTRbUkYhKPmA();
			...
			return whhKsnUcvHuYLvyNRrWlWZDNIWBd;
		}

		private void NdmEBDMVxJYtHOostGTRbUkYhKPmA()
		{
			yeuOXKfabqqdzwGmxXQiPfHfoBHu yeuOXKfabqqdzwGmxXQiPfHfoBHu2 = new yeuOXKfabqqdzwGmxXQiPfHfoBHu();
			yeuOXKfabqqdzwGmxXQiPfHfoBHu2.gFlZWzdMITpJgWkUqcumhhLdqith = this;
			if (IWMQXXjeNHpbzGKUeqGUjUOjcYHq)
			{
				return;
			}
			try
			{
				if (!viFEjNYIufiJRbMABjqFffqqieMIb())
				{
					return;
				}
				if (_dontDestroyOnLoad && Application.isPlaying)
				...
				yeuOXKfabqqdzwGmxXQiPfHfoBHu2.zGwaHHOedOwjZvFIbBgfRlNifJGl = ObZrBYhioGNKJAAFwIabBDzlOjkC();
				yeuOXKfabqqdzwGmxXQiPfHfoBHu2.HUWNRnGBFdkQqEIpATOyGIuNpham = new UnityTools.AvCdBwJqgMgreVrAsELpddWPgcmn(platform, this.platform, editorPlatform, isEditor, webplayerPlatform, scriptingBackend, scriptingAPILevel, GetExternalTools());
				Action<gAovxugbbzdKCKlVLTYrDvJFjNlo> action = yeuOXKfabqqdzwGmxXQiPfHfoBHu2.mpEgVNdiMhVeBNYPZBVpcqCKeSywB;
				UnityTools.tpEOqexFnXfkJhDrrEnkWwcFlCwlA(yeuOXKfabqqdzwGmxXQiPfHfoBHu2.HUWNRnGBFdkQqEIpATOyGIuNpham);
				ReInput.TpzbFjxRzvBxrjNiklbFUFukIxgM(this, mxyGiOOzheVAZqcDafmKHUnksdlZ, _userData.ConfigVars, _controllerDataFiles, _userData, yeuOXKfabqqdzwGmxXQiPfHfoBHu2.VxOWmFzALeBaSAgPlcHXygHDpPKY, RkuLLFyNmEDLvghmbVqJUTgazPKv, action);
				whhKsnUcvHuYLvyNRrWlWZDNIWBd = true;
				CKxLoEVsxyVblsTfgLeExXLmxbfk = false;
				if (!string.IsNullOrEmpty(yeuOXKfabqqdzwGmxXQiPfHfoBHu2.zGwaHHOedOwjZvFIbBgfRlNifJGl))
				{
					Logger.LogWarning(yeuOXKfabqqdzwGmxXQiPfHfoBHu2.zGwaHHOedOwjZvFIbBgfRlNifJGl);
				}
				OnInitialized();
			}
			catch (Exception ex)
			{
				iXiUufBumMUQTComjmiPrBumIAtu(YWcsYyVMnZDbIeElnMJPNZnJDHNl.Initialization, "initialization", ex);
			}
		}

		private void RkuLLFyNmEDLvghmbVqJUTgazPKv(Platform P_0)
		{
			platform = P_0;
		}

		private object mxyGiOOzheVAZqcDafmKHUnksdlZ(ConfigVars P_0)
		{
			List<Assembly> list = ((UnityTools.unityVersion >= UnityTools.UnityVersion.UNITY_5_0) ? null : waGfjcQxPbJICCmunHLHiCYjDbqAA());
			return sBZPLewyWajIYnWNsnbxikBrIXfS.bEIGIUeAbmZeiMteVulhetDrPkwMA(xTSOsFlXkYvagKUibKLRhaLhqAMP(), list, P_0);
		}

		private List<Assembly> waGfjcQxPbJICCmunHLHiCYjDbqAA()
		{
			List<TextAsset> list = new List<TextAsset>();
			YDNzrGuBuQDKFRvLawckWyhxyYvE(list, UnityTools.GetCurrentPlatformResourecesDLLPaths());
		...
		private List<Assembly> nUJJtSNfWNMIBpwHtrkeMKeuWzfp()
		{
			bool flag = false;
			try
			{
				if (string.IsNullOrEmpty(xTSOsFlXkYvagKUibKLRhaLhqAMP()))
				{
					return null;
				}
				...
				Assembly assembly = Array.Find(assemblies, (Assembly P_0) => string.Equals(P_0.GetName().Name, xTSOsFlXkYvagKUibKLRhaLhqAMP(), StringComparison.OrdinalIgnoreCase));
				if (assembly == null)
				{
					flag = true;
					throw new Exception();
		...
		private byte[] vooakXdagqnkGOVejOcelyATTMuHA()
		{
			try
			{
				string text = xTSOsFlXkYvagKUibKLRhaLhqAMP();
				if (string.IsNullOrEmpty(text))
				{
					return null;
				}
		...
		private void YDNzrGuBuQDKFRvLawckWyhxyYvE(List<TextAsset> P_0, List<string> P_1)
		{
		...
		private string ObZrBYhioGNKJAAFwIabBDzlOjkC()
		{
			if (editorPlatform == EditorPlatform.None)
			{
				return null;
			}
			if (wranHjkNGzDQLJiVwKJeJcKKbUwIA())
			{
				return null;
			}
		...
		private bool wranHjkNGzDQLJiVwKJeJcKKbUwIA()
		{
			switch (editorPlatform)
			{
			case EditorPlatform.Windows:
		...
		private string xTSOsFlXkYvagKUibKLRhaLhqAMP()
		{
		...
		private bool viFEjNYIufiJRbMABjqFffqqieMIb()
		{
			IWMQXXjeNHpbzGKUeqGUjUOjcYHq = false;
			if (ReInput.isReady)
			{
				...
				IWMQXXjeNHpbzGKUeqGUjUOjcYHq = true;
		...
		{
			ReInput.NVMVawGpCNIvQhSBwORBFwWWPYER();
			ReInput.btfHcAgCHOXvXAviVBHVqFUZFozY();
		}
		...
		protected void OnSceneLoaded()
		{
			if (ReInput.isReady)
			{
				ReInput.NXIwBldpplggVZjcBcapKmcHkjre();
			}
		}

		private void iXiUufBumMUQTComjmiPrBumIAtu(YWcsYyVMnZDbIeElnMJPNZnJDHNl P_0, string P_1, Exception P_2)
		{
			P_1 = "An exception occurred during " + P_1 + ".";
			bool flag = false;
			if (P_0 == YWcsYyVMnZDbIeElnMJPNZnJDHNl.Initialization || P_0 == YWcsYyVMnZDbIeElnMJPNZnJDHNl.Destroy)
			{
			...
			if (flag)
			{
				CKxLoEVsxyVblsTfgLeExXLmxbfk = true;
			}
		}
		...
		{
			if (!whhKsnUcvHuYLvyNRrWlWZDNIWBd && !_detectedPlatformInEditor)
			{
				DetectPlatform();
			}
		...
		[CompilerGenerated]
		private bool sOiVQqGIPYbmEDPyYMzaEXbqiFkv(Assembly P_0)
		{
			return string.Equals(P_0.GetName().Name, xTSOsFlXkYvagKUibKLRhaLhqAMP(), StringComparison.OrdinalIgnoreCase);
		}
	}
}
