...
namespace Rewired
{
	...
	{
		private const int CxlqZkqnPjMbbdmfbsGfBUUMFZMH = 132;

		private static HardwareControllerMap_Game BRNDptRODieZFFhITYSyTvoZTXwW;

		private bool clbxjaXCpwgFlHQaMtyiiMlEmyUc;

		private bool uLFqPTCwYqvWjZOtXDIHRLKSJvKD;

		bool IGetSetEnabled.enabled
		{
			get
			{
				return clbxjaXCpwgFlHQaMtyiiMlEmyUc;
			}
			set
			{
				if (clbxjaXCpwgFlHQaMtyiiMlEmyUc != value)
				{
					clbxjaXCpwgFlHQaMtyiiMlEmyUc = value;
					Clear();
		...
		HardwareControllerMap_Game IUnifiedKeyboardSource.hardwareMap
		{
			get
			{
				if (BRNDptRODieZFFhITYSyTvoZTXwW == null)
				{
					BRNDptRODieZFFhITYSyTvoZTXwW = CreateHardwareMap();
				}
				return BRNDptRODieZFFhITYSyTvoZTXwW;
			}
		}
		...
		{
			if (clbxjaXCpwgFlHQaMtyiiMlEmyUc)
			{
		...
		{
			if (!uLFqPTCwYqvWjZOtXDIHRLKSJvKD)
			{
				if (disposing && clbxjaXCpwgFlHQaMtyiiMlEmyUc)
				{
					ThreadSafeUnityInput.keyboard.Monitor(state: false);
				}
				uLFqPTCwYqvWjZOtXDIHRLKSJvKD = true;
			}
		}
		...
		public static ControllerElementType GetHardwareElementType(int elementIdentifierId)
		{
			...
			return BRNDptRODieZFFhITYSyTvoZTXwW.GetElementType(elementIdentifierId);
		}
	}
}
