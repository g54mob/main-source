...
namespace Rewired
{
	public sealed class ControllerMapEnabler
	{
		...
		public sealed class Rule : IDeepCloneable
		{
			...
			{
				if (_controllerSetSelector != null && !_controllerSetSelector.QfKPjZJacbKsLddxcfkcEFxqPKEoA)
				{
		...
		internal class MpLdxmCiVDCEhjPNCfTXmDrkNyfGc
		{
			public bool NfDCXQbYIJWfrgJUNhaHGxsEUYOe;

			public LWDgTQCdATINjrbviHrtIhbeAymU[] lPbDwuFrZJcoyFywfFloTFwUdOuLA;

			public MpLdxmCiVDCEhjPNCfTXmDrkNyfGc(bool P_0, LWDgTQCdATINjrbviHrtIhbeAymU[] P_1)
			{
				NfDCXQbYIJWfrgJUNhaHGxsEUYOe = P_0;
				lPbDwuFrZJcoyFywfFloTFwUdOuLA = P_1;
			}
		}

		private bool YfRhuWriUzMdsfIevOQwUWzeVjeG;

		private Player wqmscaeZavoLICeVgavpqGiziyyo;

		private MpLdxmCiVDCEhjPNCfTXmDrkNyfGc fpeQulbMGDVmAbxlxhVvILmCjmoaA;

		private readonly int mtTkTvdRZWapltdJmyaUwElgFxws;

		private List<RuleSet> jTZmGqpSQCuCtDNiQPXMhmbYLCAq;

		public bool enabled
		{
			get
			{
				return YfRhuWriUzMdsfIevOQwUWzeVjeG;
			}
			set
			{
				YfRhuWriUzMdsfIevOQwUWzeVjeG = value;
				if (value)
				{
					Apply();
				}
		...
		public List<RuleSet> ruleSets
		{
			get
			{
				return jTZmGqpSQCuCtDNiQPXMhmbYLCAq;
			}
			set
			{
				if (value == null)
				...
				jTZmGqpSQCuCtDNiQPXMhmbYLCAq = value;
			}
		}

		internal ControllerMapEnabler(Player P_0, MpLdxmCiVDCEhjPNCfTXmDrkNyfGc P_1)
		{
			if (P_0 == null)
			{
			...
			mtTkTvdRZWapltdJmyaUwElgFxws = ReInput.id;
			wqmscaeZavoLICeVgavpqGiziyyo = P_0;
			fpeQulbMGDVmAbxlxhVvILmCjmoaA = P_1;
		}

		public void Apply()
		{
			if (ReInput._id != mtTkTvdRZWapltdJmyaUwElgFxws)
			{
				ReInput.CheckInitialized(mtTkTvdRZWapltdJmyaUwElgFxws);
			}
			else
			{
				if (!YfRhuWriUzMdsfIevOQwUWzeVjeG || jTZmGqpSQCuCtDNiQPXMhmbYLCAq == null)
				{
					return;
				}
				int count = jTZmGqpSQCuCtDNiQPXMhmbYLCAq.Count;
				if (count == 0)
				{
					return;
				}
				...
				wqmscaeZavoLICeVgavpqGiziyyo.controllers.maps.GetAllMaps(list);
				int count2 = list.Count;
				for (int i = 0; i < count; i++)
				{
					RuleSet ruleSet = jTZmGqpSQCuCtDNiQPXMhmbYLCAq[i];
		...
		public void LoadDefaults()
		{
			...
			int num = ((fpeQulbMGDVmAbxlxhVvILmCjmoaA != null && fpeQulbMGDVmAbxlxhVvILmCjmoaA.lPbDwuFrZJcoyFywfFloTFwUdOuLA != null) ? fpeQulbMGDVmAbxlxhVvILmCjmoaA.lPbDwuFrZJcoyFywfFloTFwUdOuLA.Length : 0);
			for (int i = 0; i < num; i++)
			{
				RuleSet controllerMapEnablerRuleSetInstance = ReInput.mapping.GetControllerMapEnablerRuleSetInstance(fpeQulbMGDVmAbxlxhVvILmCjmoaA.lPbDwuFrZJcoyFywfFloTFwUdOuLA[i].XcDxFXSZiKKiCttsQAFuceepVFxd);
				...
				controllerMapEnablerRuleSetInstance.enabled = fpeQulbMGDVmAbxlxhVvILmCjmoaA.lPbDwuFrZJcoyFywfFloTFwUdOuLA[i].byQfuRjxcPRPWaVymOvuMdzHEFSV;
				list.Add(controllerMapEnablerRuleSetInstance);
			}
			if (fpeQulbMGDVmAbxlxhVvILmCjmoaA != null)
			{
				YfRhuWriUzMdsfIevOQwUWzeVjeG = fpeQulbMGDVmAbxlxhVvILmCjmoaA.NfDCXQbYIJWfrgJUNhaHGxsEUYOe;
			}
			jTZmGqpSQCuCtDNiQPXMhmbYLCAq = list;
			Apply();
		}
		...
		public string ToXmlString()
		{
			...
			try
			{
				return zPlOxWUaSthpLAGOnPiDcIzWEAbu().ToXmlString(writeDocumentTag: true);
			}
			catch (Exception ex)
			{
		...
		public string ToJsonString()
		{
			...
			try
			{
				return zPlOxWUaSthpLAGOnPiDcIzWEAbu().ToJsonString();
			}
			catch (Exception ex)
			{
		...
		public bool ImportXml(string xmlString)
		{
			...
			try
			{
				gwaHUpQFrkDlJUXJjwOwfiPupDrq(SerializedObject.FromXml(GetType(), xmlString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		public bool ImportJson(string jsonString)
		{
			...
			try
			{
				gwaHUpQFrkDlJUXJjwOwfiPupDrq(SerializedObject.FromJson(GetType(), jsonString));
				Apply();
				return true;
			}
			catch (Exception ex)
		...
		private SerializedObject zPlOxWUaSthpLAGOnPiDcIzWEAbu()
		{
			SerializedObject serializedObject = new SerializedObject(GetType(), SerializedObject.ObjectType.Object);
			uwxfjIEdOBxKBhoMjljDCBwCRzcxB(serializedObject);
			return serializedObject;
		}

		private void uwxfjIEdOBxKBhoMjljDCBwCRzcxB(SerializedObject P_0)
		{
			if (P_0.xmlInfo == null)
			{
			...
			P_0.Add("enabled", YfRhuWriUzMdsfIevOQwUWzeVjeG);
			P_0.Add("ruleSets", jTZmGqpSQCuCtDNiQPXMhmbYLCAq);
		}

		private bool gwaHUpQFrkDlJUXJjwOwfiPupDrq(SerializedObject P_0)
		{
			YfRhuWriUzMdsfIevOQwUWzeVjeG = false;
			jTZmGqpSQCuCtDNiQPXMhmbYLCAq = null;
			P_0.TryGetDeserializedValueByRef("enabled", ref YfRhuWriUzMdsfIevOQwUWzeVjeG);
