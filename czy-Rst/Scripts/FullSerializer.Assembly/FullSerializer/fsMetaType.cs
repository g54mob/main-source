...
namespace FullSerializer
{
	public class fsMetaType
	{
		...
		private static ConcurrentDictionary<fsConfig, ConcurrentDictionary<Type, fsMetaType>> _configMetaTypes = new ConcurrentDictionary<fsConfig, ConcurrentDictionary<Type, fsMetaType>>();
		...
		{
			ConcurrentDictionary<Type, fsMetaType> metaTypes;
			lock (typeof(fsMetaType))
			{
				if (!_configMetaTypes.TryGetValue(config, out metaTypes))
				{
					metaTypes = new ConcurrentDictionary<Type, fsMetaType>();
					_configMetaTypes.AddOrUpdate(config, metaTypes, (fsConfig _, ConcurrentDictionary<Type, fsMetaType> _) => metaTypes);
				}
			}
			if (!metaTypes.TryGetValue(type, out var metaType))
			{
				metaType = new fsMetaType(config, type);
				metaTypes.AddOrUpdate(type, metaType, (Type _, fsMetaType _) => metaType);
			}
			return metaType;
		}
		...
		public static void ClearCache()
		{
			lock (typeof(fsMetaType))
			{
				_configMetaTypes = new ConcurrentDictionary<fsConfig, ConcurrentDictionary<Type, fsMetaType>>();
			}
		}
		...
		private static bool CanSerializeField(fsConfig config, FieldInfo field, bool annotationFreeValue)
		{
			...
			if (config.IgnoreSerializeTypes.Any((Type t) => t == field.FieldType))
			{
				return false;
			}
		...
		{
			fsAotCompilationManager.AotCandidateTypes.AddOrUpdate(ReflectedType, 0, (Type _, byte _) => 0);
			if (!throwException)
			{
				return;
			}
