...
namespace FullSerializer.Internal
{
	...
	{
		private static readonly ConcurrentDictionary<Type, fsOption<fsVersionedType>> _cache = new ConcurrentDictionary<Type, fsOption<fsVersionedType>>();
		...
		{
			if (!_cache.TryGetValue(type, out var optionalVersionedType))
			{
				...
				if (attribute != null && (!string.IsNullOrEmpty(attribute.VersionString) || attribute.PreviousModels != null))
				{
					...
					optionalVersionedType = fsOption.Just(fsVersionedType2);
				}
				_cache.AddOrUpdate(type, optionalVersionedType, (Type _, fsOption<fsVersionedType> _) => optionalVersionedType);
			}
