...
namespace FullSerializer.Internal
{
	public static class fsPortableReflection
	{
		...
		private static ConcurrentDictionary<AttributeQuery, Attribute> _cachedAttributeQueries = new ConcurrentDictionary<AttributeQuery, Attribute>(new AttributeQueryComparator());
		...
		public static Attribute GetAttribute(MemberInfo element, Type attributeType, bool shouldCache)
		{
			...
			if (!_cachedAttributeQueries.TryGetValue(key, out var attribute))
			{
				object[] customAttributes = element.GetCustomAttributes(attributeType, inherit: true);
				if (customAttributes.Any())
				{
					attribute = (Attribute)customAttributes.First();
				}
				if (shouldCache)
				{
					_cachedAttributeQueries.AddOrUpdate(key, attribute, (AttributeQuery _, Attribute _) => attribute);
				}
			}
			return attribute;
		}
