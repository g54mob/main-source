	...
	public class PerlinNoiseDeformer : NoiseDeformer, IFactor
	{
		...
		public struct _3DNoiseJob : IJobParallelFor
		{
			...
			public void Execute(int index)
			{
				...
				xyz += math.float3(noise.cnoise(math.float4(float5.x - float6.x + offset.x, float5.y - float6.y + offset.y, float5.z - float6.z + offset.z, offset.w)), noise.cnoise(math.float4(float5.x + offset.x, float5.y + offset.y, float5.z + offset.z, offset.w)), noise.cnoise(math.float4(float5.x + float6.x + offset.x, float5.y + float6.y + offset.y, float5.z + float6.z + offset.z, offset.w))) * magnitude;
		...
		public struct DirectionalNoiseJob : IJobParallelFor
		{
			...
			{
				float3 xyz = math.mul(meshToAxis, math.float4(vertices[index], 1f)).xyz;
				xyz += math.float3(0f, 0f, 1f) * noise.cnoise(math.float4(xyz.x * frequency.x + offset.x, xyz.y * frequency.y + offset.y, xyz.z * frequency.z + offset.z, offset.w)) * magnitude;
				vertices[index] = math.mul(axisToMesh, math.float4(xyz, 1f)).xyz;
			}
		}
		...
		public struct NormalNoiseJob : IJobParallelFor
		{
			...
			{
				float3 xyz = math.mul(meshToAxis, math.float4(vertices[index], 1f)).xyz;
				xyz += normals[index] * noise.cnoise(math.float4(xyz.x * frequency.x + offset.x, xyz.y * frequency.y + offset.y, xyz.z * frequency.z + offset.z, offset.w)) * magnitude;
				vertices[index] = math.mul(axisToMesh, math.float4(xyz, 1f)).xyz;
			}
		}
		...
		public struct SphericalNoiseJob : IJobParallelFor
		{
			...
			{
				float3 xyz = math.mul(meshToAxis, math.float4(vertices[index], 1f)).xyz;
				xyz += math.normalize(xyz) * noise.cnoise(math.float4(xyz.x * frequency.x + offset.x, xyz.y * frequency.y + offset.y, xyz.z * frequency.z + offset.z, offset.w)) * magnitude;
				vertices[index] = math.mul(axisToMesh, math.float4(xyz, 1f)).xyz;
			}
		}
		...
		public struct ColorNoiseJob : IJobParallelFor
		{
			...
			{
				float3 xyz = math.mul(meshToAxis, math.float4(vertices[index], 1f)).xyz;
				xyz += colors[index].xyz * noise.cnoise(math.float4(xyz.x * frequency.x + offset.x, xyz.y * frequency.y + offset.y, xyz.z * frequency.z + offset.z, offset.w)) * magnitude;
				vertices[index] = math.mul(axisToMesh, math.float4(xyz, 1f)).xyz;
			}
		}
