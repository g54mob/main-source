...
namespace GLTFast
{
	internal static class PrimitiveComparer
	{
		...
		public static bool HaveEqualVertexBuffers(MeshPrimitiveBase x, MeshPrimitiveBase y)
		{
			...
			if (Equals(x.attributes, y.attributes))
			{
		...
		{
			return HashCode.Combine(GetHashCode(primitive.attributes), GetHashCode(primitive.targets));
		}
