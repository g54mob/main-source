...
namespace Enviro
{
	...
	public class EnviroVolumetricCloudsModule : EnviroModule
	{
		...
		public void RenderVolumetricCloudsURP(EnviroURPRenderGraph pass, RenderGraph renderGraph, UniversalResourceData resourceData, UniversalCameraData cameraData, TextureHandle src, TextureHandle target, EnviroVolumetricCloudRenderer renderer, EnviroQuality quality)
		{
			...
			{
				RenderTextureDescriptor desc = new RenderTextureDescriptor(descriptor.width, descriptor.height, RenderTextureFormat.ARGBHalf, 0);
				desc.vrUsage = descriptor.vrUsage;
				desc.dimension = descriptor.dimension;
				desc.volumeDepth = descriptor.slices;
				TextureHandle textureHandle = UniversalRenderer.CreateRenderGraphTexture(renderGraph, desc, "Temp2", clear: false);
				if (renderer.shadowMat == null)
				{
				...
				{
					pass.Blit("Blend", renderGraph, renderer.blendAndLightingMat, src, textureHandle, 0, renderer.downsampledDepthHandle, "_DownsampledDepth", renderer.fullBufferHandles[renderer.fullBufferIndex ^ 1], "_CloudTex");
					pass.Blit("Apply Cloud Shadows", renderGraph, renderer.shadowMat, textureHandle, target, 0, renderer.undersampleBufferHandle, "_CloudsTex");
				}
				else
				{
					pass.Blit("Apply Cloud Shadows", renderGraph, renderer.shadowMat, src, textureHandle, 0, renderer.undersampleBufferHandle, "_CloudsTex");
					pass.Blit("Blend", renderGraph, renderer.blendAndLightingMat, textureHandle, target, 0, renderer.downsampledDepthHandle, "_DownsampledDepth", renderer.fullBufferHandles[renderer.fullBufferIndex ^ 1], "_CloudTex");
				}
			}
			else
			{
