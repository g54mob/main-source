...
namespace ES3Types
{
	...
	public class ES3Type_Texture2D : ES3UnityObjectType
	{
		...
		protected override void ReadUnityObject<T>(ES3Reader reader, object obj)
		{
			...
			if (!IsReadable(texture2D))
			{
				ES3Debug.LogWarning("Easy Save cannot load the properties or pixels for this Texture " + texture2D.name + " because it is not read/write enabled, so it will be loaded by reference. To load the properties and pixels for this Texture, check the 'Read/Write Enabled' checkbox in its Import Settings.", texture2D);
			}
