...
namespace NAudio.MediaFoundation
{
	...
	{
		void GetStreamSelection([In] int dwStreamIndex, out bool pSelected);

		void SetStreamSelection([In] int dwStreamIndex, [In] bool pSelected);
		...
		void SetCurrentPosition([In] Guid guidTimeFormat, [In] IntPtr varPosition);
		...
		void GetServiceForStream([In] int dwStreamIndex, [In] Guid guidService, [In] Guid riid, out IntPtr ppvObject);

		[PreserveSig]
		int GetPresentationAttribute([In] int dwStreamIndex, [In] Guid guidAttribute, [Out] IntPtr pvarAttribute);
	}
}
